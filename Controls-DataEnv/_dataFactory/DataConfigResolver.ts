...
import { IFlatContextConfig, IHierarchyContextConfig } from './interface/IDataFactory';
...
export function callGetConfig(
    ...
    getterArguments.push(dataContext);
...
export default class DataConfigResolver {
    ...
    static isFlatContextConfig(value: any): value is IFlatContextConfig {
        return (
            value instanceof Object &&
            !!value.loadResults &&
            DataConfigResolver.isDataConfigs(value.configs)
        );
    }

    static isFlatObjectTree(treeData: any): treeData is Record<string, IDataNodeConfigGetter> {
        return (
            typeof treeData === 'object' &&
            Object.values(treeData).some((value: any) => {
                return (
                    value.hasOwnProperty('parentId') &&
                    !!(value as Record<string, unknown>).parentId
                );
            })
        );
    }

    static isIHierarchyContextConfig(value: any): value is IHierarchyContextConfig {
        return (
            value instanceof Object &&
            !!value.loadResults &&
            DataConfigResolver.isFlatObjectTree(value.configs)
        );
    }
