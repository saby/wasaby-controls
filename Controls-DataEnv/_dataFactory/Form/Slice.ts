    ...
    implements IObject<TData>
{
    ...
    protected _initState(
        ...
        return {
            ...
            store: this._normalizeLoadResults(loadResult),
    ...
    private _set<K extends keyof TData>(name: K, value: TData[K]): void {
        ...
        if (this.dynamicFields) {
            this.store.addField(getFieldDeclaration(name, value));
    ...
    private _normalizeLoadResults(loadResults: SbisRecord): SbisRecord {
        if (loadResults && loadResults['[Types/_entity/Record]']) {
            return loadResults;
        }
