import { ConfirmationManager } from 'Controls-DataEnv/_dataFactory/Form/ConfirmationManager';
import { EntityMarker } from 'Types/declarations';
import { adapter, format, Model as SbisRecord, Record } from 'Types/entity';
...
export interface IFormSlice extends ISlice {
 ...
 * @public
 */
export default class FormSlice<TData = any> extends Slice<IFormSliceState> implements IFormSlice {
    ...
    protected _initState(
        ...
        this._config = config;
        const store = new DataWrapper<TData>(this._normalizeLoadResults(loadResult), {
            newRecord: !config.id,
        });
    ...
    protected async _beforeApplyState(nextState: IFormSliceState): Promise<IFormSliceState> {
    ...
    validate(name: string, showInfobox: boolean = true): Promise<TValidationResult> {
        if (showInfobox) {
            this._closeInfobox();
        }
    ...
    validateAll(): Promise<TValidationResult[]> {
        ...
        Object.keys(this._validators).forEach((name) => {
            promises.push(this.validate(name, false));
        });
    ...
    create(sourceMeta?: object): Promise<void> {
        ...
        return this.loader.create(sourceMeta || this._config.createMetaData).then((data) => {
            this.setState({ store: new DataWrapper(data as SbisRecord) });
        });
    }
    ...
    async update(
        ...
        const data = this.store.getStore().get('record');

        return this.loader.update(data, sourceMeta).then(() => {
            this._onDataSave?.(this._getStoreRecord(), type);
        });
    }
    ...
    delete(sourceMeta?: object): Promise<void> {
        ...
        return this.loader
            ...
            .then(() => {
                this.setState({ store: new DataWrapper() });
            });
    }
    ...
    private _normalizeLoadResults(loadResult: SbisRecord): IFormDataFactoryResult {
        if (!loadResult) {
        ...
        result.acceptChanges(['record']);
        return result;
    }

    private _getStoreRecord(): SbisRecord {
        return this.store.getStore().get('record');
    }
...
export class DataWrapper {
    ...
    private readonly _newRecord: boolean = false;
