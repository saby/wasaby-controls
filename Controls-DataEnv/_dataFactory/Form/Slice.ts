 ...
 *
 */
export type NameBindingType = string[];
...
function isIValidatorResult(validator: any): validator is IValidatorResult {
 ...
 *
 */
export type TValidationResult = boolean | IValidatorResult;

/**
 * @public
 */
export type TName<T> = string[] | INestedArray<NestedKeys<T>>;

/**
 * Тип значения поля из состоянии формы
 * @public
 */
export type TResultWrap<T, K> = T extends INestedArray<NestedKeys<K>>
    ? TResult<Flattened<K>, TupleToString<T>>
    : unknown;

/**
 * @public
 */
export interface IFormSlice<TData = Record<string, unknown>> {
    /**
     *
     */
    ...
    get<TKeys extends TName<TData>>(name: TKeys): TResultWrap<TKeys, TData> | unknown;
     ...
     */
    update(type?: FormSliceActionType, meta?: object): Promise<unknown>;
...
/**
 * Вернет все ключи объекта в нужном для нас виде
 * @public
 */
export type NestedKeys<T, Path extends string = ''> = T extends object
    ? {
          [K in keyof T]: K extends string
              ? T[K] extends (infer U)[]
                  ?
                        ...
                        | NestedKeys<
                              U,
                              Path extends '' ? `${K}[${number}]` : `${Path}.${K}[${number}]`
                          >
                  :
...
/**
 * Вернет все значения для ключей
 * @public
 */
export type DeepValueType<T, Path extends string> = Path extends keyof T
    ? T[Path]
...
/**
 * Вернет интерфейс с ключем и значеним
 * @public
 */
export type Flattened<T> = {
    [K in NestedKeys<T>]: DeepValueType<T, K>;
};
 ...
 * @public
 */
export type TResult<Res, Key> = Res extends object
...
/**
 * Возвращает корректный тип для ключей (['1','2'] -> '1.2')
 * @public
 */
 ...
 * @public
 */
export type INestedArray<T extends string = string> = Split<T, '.'>;
...
/**
 * Базовый слайс для работы с {@link Types/entity:Record} в формах
 * @public
 */
export default class FormSlice<
        TFormSliceState extends IFormSliceState = IFormSliceState,
        TFormSliceData = Record<string, unknown>,
    >
    extends Slice<TFormSliceState>
    implements IFormSlice<TFormSliceData>
{
    ...
    private _pendingResolveFunction: Function | undefined;
    ...
    protected _beforeApplyState(
        nextState: TFormSliceState
    ): TFormSliceState | Promise<TFormSliceState> {
        ...
        if (nextState.record && nextState.record !== this.store.getStore()) {
            nextState.store = new DataWrapper(
                this._normalizeLoadResults(nextState.record) as SbisRecord
            );
        }
    ...
    setEditing(editing: boolean): void {
        this.setState({
            editing,
        } as Partial<TFormSliceState>);
    }

    registerValidators(name: string, fns: Function[], ref: MutableRefObject<any>): void {
        if (!ref?.current) {
            logger.warn(
                `Ошибка регистрации валидатора: не указан ref на виджет для ${name}. Фокус при валидации невозможен.`
            );
        }
        ...
        const cb = () => {
            if (msg && validator.ref?.current) {
                this._infoboxOpener.openPopup({
                    target: getNode(validator.ref.current),
                    message: msg,
    ...
    protected _activateValidatorInput(validator: IValidator) {
        const node = getNode(validator.ref?.current);
    ...
    private _validate(name: string, showInfobox: boolean = true): Promise<TValidationResult> {
        ...
        if (this._validators[name]) {
            let status = true;
            const promises: Promise<IValidatorResult>[] = [];
            let msg: string;
            this._validators[name].callbacks.map((validator) => {
                // @ts-ignore
                const normalizedName = name instanceof Array ? name : name.split(',');
                const value = this.get(normalizedName as unknown as TName<TFormSliceData>);
    ...
    validateAll(): Promise<TValidationResult[]> {
        ...
        return Promise.all(promises).then((results) => {
            ...
            const invalidValidators: IValidatorResult[] = [];

            for (let i = 0; i < results.length; i++) {
                const validatorResult = results[i];
                const state = computeValidationState(validatorResult);
                if (
                    (state === VALIDATION_STATE.invalid ||
                        state === VALIDATION_STATE.invalidAccent) &&
                    isIValidatorResult(validatorResult)
                ) {
                    if (validatorResult.validator.ref?.current) {
                        invalidValidators.push(results[i] as IValidatorResult);
                    }
                }
            ...
            if (invalidValidators.length) {
                const [firstValidator] = invalidValidators;

                this._activateValidatorInput(firstValidator?.validator);
                this._showInfobox(firstValidator.validator, firstValidator.msg);
            }
    ...
    async send(): Promise<unknown> {
        return this.dataContext?.execute('Send', {});
    }
    ...
    create(sourceMeta?: object): Promise<void> {
        ...
        return this.loader
            ...
            .then((data) => {
                this.setState({
                    store: new DataWrapper(data as SbisRecord<IFormDataModel>),
                    error: this._getErrorState(data as unknown as Error),
                } as Partial<TFormSliceState>);
                this._isChanges = false;
            })
            .catch((e) => {
                this._errorManager.processSourceError(e);
            });
    }
    ...
    async update(
        ...
        return this.loader
            .update(data as SbisRecord, sourceMeta)
            .then(() => {
                this._onDataSave?.(this._getStoreRecord(), type);
                this._isChanges = false;
                return true;
    ...
    async delete(sourceMeta?: object): Promise<void> {
        if (this._isLocalData) {
            throw new Error(`${this._moduleName} Не задан источник в опции source.`);
        }
    ...
    registerAction(actionName: string, actionCallback: Function): void {
        this._actions[actionName] = actionCallback;
    }
    ...
    initChangeRecordPending(): void {
        if (!this._actions.registerPending) {
            throw new Error('Функция регистрации пендинга не определена на слайсе.');
        }
        const pendingPromise = new Promise((resolve) => {
            this._pendingResolveFunction = resolve;
        });
        this._actions.registerPending(pendingPromise, {
            showLoadingIndicator: false,
            validate: (): boolean => {
                return this.hasChanges();
            },
            onPendingFail: (forceFinishValue: boolean): void => {
                this._showConfirmDialog(forceFinishValue);
            },
        });
    }

    protected _showConfirmDialog(forceFinishValue: boolean): void {
        if (forceFinishValue !== undefined) {
            this._confirmDialogResult(forceFinishValue);
        } else {
            this._showConfirmPopup(
                'yesnocancel',
                rk('Чтобы продолжить редактирование, нажмите «Отмена»')
            ).then((answer) => {
                this._confirmDialogResult(answer as boolean);
                return answer;
            });
        }
    }

    protected async _showConfirmPopup(
        type: string,
        details?: string
    ...
    protected async _confirmDialogResult(answer: boolean) {
        if (answer === true) {
            await this.validateAll();
            if (!this.isValid()) {
                return this._actions.cancelFinishingPending?.();
            }
            this.send().then(
                (res) => {
                    if (this._pendingResolveFunction) {
                        this._pendingResolveFunction();
                        this._pendingResolveFunction = undefined;
                    }
                    return res;
                },
                () => {
                    this._actions.cancelFinishingPending?.();
                }
            );
    ...
    get<TKeys extends TName<TFormSliceData>>(name: TKeys): TResultWrap<TKeys, TFormSliceData> {
        return this.store.get(name) as TResultWrap<TKeys, TFormSliceData>;
    }

    set<TKeys extends TName<TFormSliceData>>(
        name: TKeys,
        value: TResultWrap<TKeys, TFormSliceData>
    ): void {
    ...
    private _beforeRecordClose(): Promise<unknown> {
        if (!this.store.getStore()) {
            return Promise.resolve();
        }
        return this._confirmationManager
            .confirmSave(this.store.getStore() as unknown as SbisRecord)
            .then(async () => {
                await this.validateAll();
                if (!this.isValid()) {
            ...
            .catch(() => {
                if (this.store.needDestroy()) {
                    return this.loader.destroy(this.store.getId() as any as CrudEntityKey);
                }
            });
    }
...
export class DataWrapper {
    readonly '[Types/_entity/IObject]': EntityMarker;
    private readonly _store: SbisRecord<IFormDataModel>;
     ...
     * @param options
     */
    constructor(store?: SbisRecord<IFormDataModel>, options?: IDataWrapperOptions) {
        this._store = this._normalizeStore(store);
        this._newRecord = !!options?.newRecord;
    }

    set(name: NameBindingType | string, value: any): boolean {
        ...
        const injectedValue = injectValue(this._store as unknown as SbisRecord, path, value);
    ...
    getStore(): SbisRecord<IFormDataModel> {
        return this._store;
    }
    ...
    getType(name: NameBindingType | string): format.Field | undefined {
        ...
        if (this._store.has(binding)) {
            return this._getType(this._store as unknown as SbisRecord, binding);
        }
    ...
    private _normalizeStore(store?: SbisRecord<IFormDataModel>): SbisRecord<IFormDataModel> {
        // @ts-ignore
        if (store && store['[Types/_entity/Record]']) {
            return store;
        }
        return new SbisRecord<IFormDataModel>({
            adapter: new adapter.Sbis(),
        });
    }

    private _normalizeBinding(name: NameBindingType | string): {
        binding: keyof IFormDataModel;
        path: NameBindingType;
        root: keyof IFormDataModel;
    } {
        if (!Array.isArray(name)) {
            name = (name as string).split(DEFAULT_DELIMITER);
        }
        const root = name[0] as keyof IFormDataModel;

        if (this._store.has(root)) {
            return {
                binding: name.join(DEFAULT_DELIMITER) as keyof IFormDataModel,
                path: [root, ...name],
                root: root as keyof IFormDataModel,
            };
        }
...
function extractValue(obj: SbisRecord<any>, path: NameBindingType): unknown | undefined {
