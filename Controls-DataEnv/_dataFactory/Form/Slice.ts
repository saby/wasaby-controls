...
type TValidationState = Record<string, TValidationResult>;
...
export default class FormSlice<TData = any> extends Slice<IFormSliceState> implements IFormSlice {
    ...
    protected _beforeApplyState(nextState: IFormSliceState): IFormSliceState {
    ...
    async update(
        ...
        const data = this.store.get(DEFAULT_USERDATA_PROPERTY)
            ? this.store.getStore()
            : this.store.get(DEFAULT_DATA_PROPERTY);
    ...
    delete(sourceMeta?: object): Promise<void> {
        ...
        return this.loader.destroy(this.key, sourceMeta || this._config.destroyMetaData);
    }
