...
async function loadConfigGetter(config: IDataConfigLoader): Promise<{
...
interface ILoadDataConfigResultSerializable {
    configs: Record<string, IDataConfigLoader & { isAsyncConfigGetter }>;
}
    ...
    static async loadByConfigs(
        ...
        for (const [key, dataConfig] of Object.entries(configGettersResults.dataConfigs)) {
            ...
            loadDataPromises.push(
                loadData(dataConfig, loadTimeout, Router, loadedRootResults)
                    .then((loadResult) => {
