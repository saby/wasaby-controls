...
//@ts-ignore;
import { wrapTimeout } from 'SbisUI/PromiseLib';
import { RPC } from 'Browser/Transport';

/**
 * @public
 */
export interface IDataConfigLoadResult {
    type: string;
...
async function loadConfigGetter(
    config: IDataConfigLoader,
    dataContext: DataContext
): Promise<IGetConfigResult> {
    const module = await loadModule<IConfigGetterModule>(config.configGetter);
    let dataConfigs = {};
...
async function loadDataConfigs(
    ...
    for (const [key, configGetter] of Object.entries(configGetters)) {
        dataConfigsPromises.push(
            loadConfigGetter(configGetter, dataContext).then(
                ({ dataConfigs, isAsyncConfigGetter, error }) => {
...
function callLoaderWithDependencies(
    ...
    if (dependencies.length) {
        return loadDependencies(dependencies, startedLoaders, configs, loadTimeout, Router).then(
            (results) => {
                return callLoader(
                    key,
                    configs[key],
...
async function loadData(
    ...
    const loadResults = await load(configs, loadTimeout, Router, loadedDependencies, errorHandler);
    const result: Record<keyof TDataConfigs, unknown> = {};
    const promises = [];

    const minLoadOrder = calculateMinOrder(configs);
    for (const [key, value] of Object.entries(loadResults)) {
        const dataFactoryCreationOrder = configs[key]?.dataFactoryCreationOrder || 0;
        if (dataFactoryCreationOrder === minLoadOrder) {
            promises.push(
                value.then((loaderResult) => {
...
export default class DataLoader {
    ...
    static async loadByConfigs(
        ...
        let rootElements: IGetConfigResult | undefined;
        let rootResults: Record<string, unknown> = {};

        if (configGetters.hasOwnProperty(STORE_ROOT_NODE_KEY)) {
            rootElements = await loadConfigGetter(configGetters[STORE_ROOT_NODE_KEY], dataContext);
        }
        ...
        if (rootElements && Object.keys(rootElements.dataConfigs).length) {
            rootResults = await loadData(rootElements.dataConfigs, loadTimeout, Router);
            dataContext.addNode({
        ...
        const configGettersResults = await loadDataConfigs(configGetters, dataContext);
        ...
        for (const [key, dataConfigs] of Object.entries(configGettersResults.dataConfigs)) {
            ...
            if (rootDeps[key]?.length) {
                rootDeps[key].forEach((rootDep) => {
                    loadedRootResults[rootDep] = Promise.resolve(rootResults[rootDep]);
                });
            }
            const errorHandler = (error: unknown, config: IDataConfig) => {
                if (error instanceof Error) {
                    const logger = resolveLogger(error);
                    logger(
                        `Controls-DataEnv/dataLoader::Произошла ошибка при загрузке фабрики данных ${config.dataFactoryName} из геттера ${configGetters[key].configGetter} с ключом в конфигурации ${key} ${error.message}`,
                        this,
                        error
                    );
                }
            ...
            loadDataPromises.push(
                loadData(dataConfigs, loadTimeout, Router, loadedRootResults, errorHandler).then(
                    (loadResult) => {
