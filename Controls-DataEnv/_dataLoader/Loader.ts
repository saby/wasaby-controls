...
export default class DataLoader {
        ...
        if (validateErrors.length) {
            ...
            return new Promise((resolve, reject) => {
                ...
                Object.keys(loadResults).forEach((key) => {
                    const dataFactoryCreationOrder =
                        configs[key].dataFactoryCreationOrder || 0;
    ...
    private static _callLoader(
        ...
        if (!config.dataFactoryName) {
            ...
            if (loadTimeout && !config.dataFactoryArguments?.loadDataTimeout) {
                config.dataFactoryArguments.loadDataTimeout = loadTimeout;
            }
            if (!startedLoaders[id]) {
                startedLoaders[id] = DataLoader._loadDataFactory(
                    ...
                    .then((dataFactory) => {
                        ...
                        if (dataFactory.getDataFactoryArguments) {
                            dataFactoryArguments =
                                dataFactory.getDataFactoryArguments(
                                    dataFactoryArguments,
                                    dependenciesResults
                                );
                        }
                        return dataFactory
                            ...
                            .catch((error) => {
                                Logger.error(
                                    `Ошибка при вполнении метода
                                      loadData для фабрики ${config.dataFactoryName} с id: ${id}`,
                                    this,
                                    error
                                );
                                return error;
                    ...
                    .catch((error) => {
                        Logger.error(
                            `Ошибка загрузки фабрики ${config.dataFactoryName}`,
                            this,
                            error
                        );
                        return error;
