...
export default class DataLoader {
    ...
    private static _calcAllDependencies(config: IDataConfig): string[] {
        let dependencies = config.dependencies || [];
        const extraValues = config.dataFactoryArguments.loaderExtraValues;
        if (extraValues) {
            const extraDependencies = Object.values(extraValues).map(([sliceName]) => {
                return sliceName;
            });
            dependencies = dependencies.concat(
                extraDependencies.filter((item) => dependencies.indexOf(item) < 0)
            );
        }
        return dependencies;
    }
    ...
    private static _callLoaderWithDependencies(
        ...
        const dependencies = this._calcAllDependencies(config);
        if (dependencies) {
            return this._loadDependencies(dependencies, startedLoaders, configs, loadTimeout).then(
                (results) => {
            ...
            if (!startedLoaders[id]) {
                startedLoaders[id] = DataLoader._loadDataFactory(config.dataFactoryName)
                    .then((dataFactory) => {
                        ...
                        if (extraValues) {
                            const properties = {};
                            Object.entries(extraValues).forEach(
                                ([propName, [loaderName, depProperty]]) => {
                                    properties[propName] =
                                        dependenciesResults[loaderName][depProperty];
                                }
                            );
