...
function getDataFactoryArguments(
    ...
    if (extraValues) {
        ...
        extraValues.forEach(({ propName, dependencyName, dependencyPropName, prepare }) => {
            ...
            if (prepare) {
                properties[propName] = prepare(
                    valueFromSlice,
                    properties.hasOwnProperty(propName)
                        ? properties[propName]
                        : (dataFactoryArguments as Record<string, unknown>)[propName]
                );
            } else {
...
export default class DataFactoryLoader {
    ...
    private async _load(): Promise<unknown> {
        ...
        try {
            if (this._$data !== undefined) {
                ...
                const loadPromise = dataFactory.loadData(
                    dataFactoryArguments,
                    this._$dependencies,
                    this._$router,
                    false,
                ...
                if (this._$loadTimeout && !dataFactory.isHandlingTimeoutError) {
                    loadResult = await wrapTimeout(loadPromise, this._$loadTimeout);
                } else {
            ...
            this._$errorHandler(
                error,
                `Controls-DataEnv/dataLoader:DataFactoryLoader::Произошла ошибка во время выполнения метода loadData у фабрики ${
                    this._$config.dataFactoryName
                } с ключом ${this._$name}
             и параметрами ${this._$config.dataFactoryArguments}. Узел контекста ${
                 this._$parent
             } Страница: ${this._$router?.maskResolver?.calculateUrlParams('page/:pageId')?.pageId}`
            );
            loadResult = error;
        }
    ...
    getDataFactoryName(): string {
        return this._$config.dataFactoryName;
    }
}
