    ...
    implements IAbstractListAPI
{
    ...
    private async _endUpdate(nextState: TState): Promise<TState> {
        ...
        await this._dispatcher.dispatch(endUpdate(currentState, nextState) as TAction);
        ...
        if (
            nextState.itemActions !== currentState.itemActions &&
            nextState.items &&
            isValidActions(nextState.itemActions)
        ) {
            nextState.itemActionsMap = Initializer.actions.createItemActionsMap(
                nextState.items,
                nextState
            );
        }
        ...
        const newCollectionType = getCollectionType(collectionType, nextState.viewMode);
        ...
        if (collectionType !== newCollectionType) {
            ...
            if (collectionType) {
                nextState.collection = createCollection(collectionType, nextState);
                if (!nextState.isThinInteractor) {
                    ...
                    nextState.needShowStub = Initializer.emptyView.needShowEmptyView(
                        nextState.items
                    );
                }
            }
