    ...
    implements IAbstractListAPI
{
    ...
    isIdle(): boolean {
        ...
        return !this._isSliceUpdating && (this.__dispatcher ? this.__dispatcher.isIdle() : true);
    }
    ...
    private async _endUpdate(nextState: TState): Promise<TState> {
        ...
        if (
            nextState.itemActions !== this.state.itemActions &&
            nextState.items &&
    ...
    destroy() {
        this._debugger?.onSliceDestroyed();
    ...
    next(key?: TKey): void {
        this._addAction(
            AbstractListActionCreators.source.loadNext(
                undefined,
                undefined,
                undefined,
                undefined,
    ...
    select(key: CrudEntityKey, options?: TSelectionOptions): void {
        this._addAction(
            AbstractListActionCreators.selection.select(
                key,
                options?.direction,
