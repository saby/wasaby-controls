...
import { isValidViewMode as isValidViewModeBase } from '../validators/viewMode';
...
export const initViewMode = (viewMode?: TViewMode, searchValue?: string): TViewMode => {
    const isValidViewMode = (value: unknown): value is TViewMode => {
        return isValidViewModeBase(value, searchValue);
    };

    return validateOption(viewMode, isValidViewMode) || (searchValue ? 'search' : 'table');
};
