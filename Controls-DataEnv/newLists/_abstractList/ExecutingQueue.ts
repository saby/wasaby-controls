...
import * as predicates from './validators/predicates';
import AbstractListActionCreators, { type TAbstractListActions } from './actions';

const {
    interactorCore: { publicSetState },
} = AbstractListActionCreators;

const { isFunction, isObject, isString } = predicates;
...
export default class ExecutingQueue {
    ...
    prepareForExecution<TState extends IAbstractListState>(
        ...
        this._actionsQueue = removeIrrelevantLoadingSetState(this._actionsQueue);
...
export function isObjectSetState<TState = IAbstractListState>(
    action: unknown
...
/**
 * Для удаления неактуальных неактуального обновления состояния флага loading
 */
function removeIrrelevantLoadingSetState<
    TState extends IAbstractListState,
    TAction extends TAbstractListActions.TAnyAbstractListAction<TState>,
>(actions: TAction[]): TAction[] {
    ...
    // Экшены, которые запускают загрузку на startUpdate
    const firstPhaseLoadActions = ['reload'];
    actions.forEach((action) => {
        if (isString(action.type) && firstPhaseLoadActions.includes(action.type)) {
            needRemove = true;
        }
        if (
            needRemove &&
            isObjectSetState<TState>(action) &&
            action.payload.nextState.loading === false
        ) {
            const {
                nextState: { loading, ...restState },
            } = action.payload;
            if (Object.keys(restState).length) {
                result.push(publicSetState(restState as Partial<TState>) as TAction);
            }
        } else {
            result.push(action);
        }
