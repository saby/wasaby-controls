...
import { isObject } from './validators/predicates';
import { EMPTY_EXECUTING_ACTIONS_ON_REJECT } from './ErrorDescriptors';
...
function isObjectSetState<TState = IAbstractListState>(
    ...
    return (
        isObject(action) &&
        'type' in action &&
        action.type === PublicSetStateSymbol &&
        'payload' in action &&
        isObject(action.payload) &&
        'nextState' in action.payload &&
