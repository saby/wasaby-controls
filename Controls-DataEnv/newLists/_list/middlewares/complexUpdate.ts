...
async function beforeApplyState_fn(
    ...
    dispatch: Function,
    registerPendingPromise: TRegisterPendingPromise,
    ...
    if (rootChanged && nextState.rootHistoryId && nextState.root) {
        USER.set(nextState.rootHistoryId, JSON.stringify(nextState.root));
    }
    if (currentState.items && nextState.items && currentState.items !== nextState.items) {
