...
export const expandCollapseMiddleware: TListMiddleware = asyncMiddlewareFactory(
    ...
    ({ getState }, action) => {
        if (action.type === 'onEndUpdate') {
            const {
                prevState: { expandedItems: prevExpandedItems, collapsedItems: prevCollapsedItems },
            } = action.payload;
            const { expandedItems, collapsedItems } = getState();
            return prevExpandedItems !== expandedItems || prevCollapsedItems !== collapsedItems;
        }
    }
);
...
export const filterMiddleware: TListMiddleware = asyncMiddlewareFactory(
    ...
    ['setFilterDescription', 'setFilter', 'applyFilterDescriptionToFiller']
);
...
export const stubMiddleware: TAbstractListMiddleware = asyncMiddlewareFactory(
    ...
    ['setStubVisibility', 'onItemsAdded', 'onItemsRemoved', 'onItemsReset', 'onAllItemsReplaced']
);
