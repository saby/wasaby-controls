...
import type { TFilter, TKey, TViewMode, IFilterDescriptionItem } from 'Controls-DataEnv/interface';
import {
    Direction,
    IBaseSourceConfig,
...
> extends AbstractListSlice<TState, TAction, TMiddlewareContext> {
    ...
    setSelectionViewMode(selectionViewMode: string): void {
        this.setState({ selectionViewMode } as Partial<TState>);
    }
    ...
    isExpanded(key: CrudEntityKey): boolean {
        Logger.error('Метод Slice.isExpanded устарел и будет удален в 25.1100');
        if (isStateHierarchy(this.state) && isLoaded('Controls/listsCommonLogic')) {
            const { isExpanded } = loadSync<typeof import('Controls/listsCommonLogic')>(
    ...
    isExpandAll(): boolean {
        Logger.error('Метод Slice.isExpandAll устарел и будет будет удален в 25.1100');
        if (isStateHierarchy(this.state) && isLoaded('Controls/listsCommonLogic')) {
            const { isExpandAll } = loadSync<typeof import('Controls/listsCommonLogic')>(
                'Controls/listsCommonLogic'
            ).UILogic.Hierarchy;
