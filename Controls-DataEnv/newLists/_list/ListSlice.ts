...
import { calculateFilterByFilterDescription } from 'Controls-DataEnv/newLists/_list/helpers/calculateFilterByFilterDescription';
...
import { isStateHierarchy } from 'Controls-DataEnv/newLists/_list/helpers/isStateHierarchy';
import {
    filterMiddleware,
...
> extends AbstractListSlice<TState, TAction, TMiddlewareContext> {
    ...
    protected _initState(
        ...
        if (initConfig.listConfigStoreId) {
            storeFields =
                loadSync<typeof import('Controls/dataSource')>(
                    ...
                    initConfig.listConfigStorePropsNames
                ) || {};
        }

        const config = { ...initConfig, ...storeFields };
        ...
        if (propsForMigration?.sliceProperties) {
            propsForMigration.sliceProperties.previousViewMode = state.viewMode;
        }
    ...
    protected _load(
        ...
        return this._addAsyncAction(
            oldSliceLoad({
                ...
                disableSetState,
                isDefaultError: !direction || key !== undefined,
            }) as TAction
        );
    }
