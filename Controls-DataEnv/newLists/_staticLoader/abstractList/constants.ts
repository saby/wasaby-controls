...
export enum LibPaths {
    ...
    Sticky = 'Controls/stickyBlock',
    TreeGridDisplay = 'Controls/treeGridDisplay',
    Tree = 'Controls/tree',
    Columns = 'Controls/columns',
    ListDisplay = 'Controls/display',
    ...
    ListCommands = 'Controls/listCommands',
    DataSource = 'Controls/dataSource',
    ListWebReducers = 'Controls/listWebReducers',
    Filter = 'Controls/filter',
    CurrentList = 'Controls-DataEnv/currentList',
    AbstractList = 'Controls-DataEnv/abstractList',
}

/**
 * Объект, в котором перечислены зависимости библиотек от состояния интерактора.
 */
export const UI_DEPENDENCIES: TUI_Dependencies<TState> = {
    ...
    [LibPaths.Actions]: [
        (state) => {
            return !!state.itemActions?.length;
        },
    ],
};
...
const getCatalogConfig = (items: TState['items']) => {
    const meta = items && items.getMetaData();
