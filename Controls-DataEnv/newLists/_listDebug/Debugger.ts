...
import { IUpdateSessionMeta, Session } from './debugger/Session';
import { setActionTrace, setPublicAsSender, setSender } from './debugger/utils/patchAction';
...
import * as MSG from './debugger/printer/common/MSG';
...
export class Debugger {
    ...
    private _sessionsHistory: IUpdateSessionMeta[] = [];
    ...
    getHistory(): IUpdateSessionMeta[] {
        return this._sessionsHistory;
    }

    onSliceInitialized(loadResult: unknown = {}, config: unknown = {}, state: unknown = {}) {
        this.ifEnabled(() => {
            this._output
                .add('groupCollapsed', [`Slice(${this._name}) initialized`])
    ...
    onSliceDestroyed() {
        this.ifEnabled(() => {
            this._output.add('info', [`Slice(${this._name}) destroyed`]);
            this._renderNew();
        });
    }
    ...
    onEndUpdateSession(resultState: unknown): void {
        ...
        const meta = currentSession.getMeta();

        this._sessionsHistory.push(meta);

        Printer.print(this._name, String(this._sessionNumber), this._output, meta, this._debugMode);
