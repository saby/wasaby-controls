...
import * as MessageDescriptors from './debugger/MessageDescriptors';
import { DurationTimer } from './debugger/DurationTimer';
...
export class Debugger {
    ...
    handleUserUpdateChanges(prevState: object, nextState: object): void {
        if (!this._isEnabled) {
            return;
        }
        this._getSession().handleUserUpdateChanges(prevState, nextState);
    }
    ...
    logSliceChangeEnd(state: object): void {
        ...
        method.call(this._output, {
            type: 'groupCollapsed',
            args: [
                MessageDescriptors.SLICE_UPDATED(
                    this._name,
                    this._sliceSetStateDurationTimer?.stop(),
    ...
    handleDispatchError(e: Error): void {
        ...
        this._getSession().handleDispatchError(e);
    }
