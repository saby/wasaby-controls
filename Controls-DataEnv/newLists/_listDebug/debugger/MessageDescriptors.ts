...
export const DURATION = (duration: number, style: TOutputStyle) => {
    if (style === 'All' || style === 'Significant') {
        return 'Длительность: ' + duration + 'ms.';
    } else {
        return `(${duration}ms)`;
    }
};
...
export const USER_UPDATE_CHANGES = (_style: TOutputStyle) =>
    'Изменения в прикладном _beforeApplyState';
