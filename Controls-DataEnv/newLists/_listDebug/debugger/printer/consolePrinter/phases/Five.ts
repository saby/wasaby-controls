...
// Пока мы тут ничего не делаем, но в этой точке можно обрабатывать состояние,
...
export const printPhaseFive = (
    ...
    _phase<IFivePhaseMeta>(output, meta, debugMode, 4, 'Handle result state', (meta, printer) => {
        ...
        if (!meta.changes.length && !meta.dispatches.length && !loadedDeps.length) {
            ...
            // В этой фазе не может быть мутаций, по крайней мере пока
            output.add('groupCollapsed', [meta.title], meta.changes.length ? 'error2' : 'default');
