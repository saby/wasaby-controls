...
> {
    ...
    async dispatch(action: TAction): Promise<TState> {
        ...
        const { _debugger: stateDebugger } = this;
        if (stateDebugger) {
            stateDebugger.markPublicAction(action);
        }

        const dispatchingInfo = (this._dispatchingInfo = {
            action,
            promise: this._dispatch(action),
            isRejected: false,
        });
        await dispatchingInfo.promise;

        const result =
            dispatchingInfo.isRejected || this._isDestroyed ? initialState : this._getState();
    ...
    rejectDispatch(): Promise<void> {
        const { _asyncOperationsOrchestrator: orchestrator, _dispatchingInfo: dispatchingInfo } =
            this;
        if (!orchestrator.isIdle) {
            orchestrator.rejectPendingOperations();
        }
        if (dispatchingInfo) {
            dispatchingInfo.isRejected = true;
            return dispatchingInfo.promise;
        }
            ...
            middleware({
                ...
                registerPendingPromise: async (key, promise, strategy) => {
                    const {
                        ...
                        _dispatchingInfo: dispatchingInfo,
                        _asyncOperationsOrchestrator: orchestrator,
                    } = this;
                    if (isDestroyed) {
                        return getError('DESTROYED_INSTANCE');
                    }
                    if (dispatchingInfo?.isRejected) {
                        return getError('DISPATCH_CANCELED');
                    }
                    return (
                        orchestrator.registerPendingPromise(
                            `${middleware.name}:${key}`,
                            promise,
                            strategy
                        ) ?? promise
    ...
    private async _dispatch(action: TAction) {
        const {
            _isDestroyed: isDestroyed,
            _dispatchingInfo: dispatchingInfo,
            _debugger: stateDebugger,
        } = this;
        if (isDestroyed || dispatchingInfo?.isRejected) {
            return;
        }
        stateDebugger?.startDispatch(action);
    ...
    private _validateSession(
        ...
        const { _isDestroyed: dispatcherIsDestroyed, _dispatchingInfo: dispatchingInfo } = this;
        if (dispatcherIsDestroyed) {
            if (isDestroyed) {
        ...
        if (dispatchingInfo) {
            if (isDispatching) {
                getError('DISPATCH_COLLISION', dispatchingInfo.action, action);
            }
            return true;
        }
