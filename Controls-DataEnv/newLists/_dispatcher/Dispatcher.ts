...
import type { AsyncOperationsOrchestrator } from 'Controls-DataEnv/abstractList';
...
> {
    ...
    private _asyncOperationsOrchestrator?: AsyncOperationsOrchestrator;
    ...
    constructor({
        ...
        this._asyncOperationsOrchestrator = idleDetectorInstance;
    ...
    rejectDispatch(): Promise<void> {
        if (this._asyncOperationsOrchestrator && !this._asyncOperationsOrchestrator.isIdle) {
    ...
    private _initMiddlewares(
        ...
        this._middlewares = creators.map((middleware) =>
            middleware({
                ...
                registerPendingPromise: (key, promise, strategy) => {
                    if (this._dispatchingInfo?.isRejected) {
                        return Promise.reject(
                            'Ошибка регистрации висячего промиса. Распространение экшена отменено, поэтому регистрация недоступна'
                        );
                    }
                    return (
