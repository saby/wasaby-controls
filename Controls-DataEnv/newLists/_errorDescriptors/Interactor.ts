...
export const DEPRECATED_USED = (
    ...
    msg += `Свойство/метод/опция ${deprecatedName} устарело`;
...
/**
 * Дескриптор ошибки использования несписочного слайса
 */
export const WRONG_SLICE = (moduleName: string) => {
    logger.error(
        'ПРИКЛАДНАЯ ОШИБКА использования списка.\n' +
            'Использован слайс, неподходящий для данного списка.\n' +
            `Необходимо убедиться, что прикладной слайс является наследником списочного из модуля ${moduleName}`
    );
};
