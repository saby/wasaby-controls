import { IListDataFactoryArguments, IListDataFactoryLoadResult } from 'Controls-DataEnv/list';
...
import { getConfigAfterLoadError } from 'Controls-DataEnv/newLists/_listDataLoader/utils/getConfigAfterLoadError';
...
export default async function load(
    ...
    if (config.listConfigStoreId) {
        Object.assign(stateFromPage, getStateFromMemory(config.listConfigStoreId) || {});
    }
...
async function getLoadResult(
    ...
    if (config.rootHistoryId && config.parentProperty) {
        loadResult.root = dataResultMetaData?.correctRoot ?? paramsResult.root;
    }
