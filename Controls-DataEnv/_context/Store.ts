...
export default class StoreWithSlices {
    ...
    protected _resolveExtraValues(
        ...
        const properties = {};
        sliceExtraValues.forEach(({ propName, dependencyPropName, dependencyName }) => {
            properties[propName] = (slices[dependencyName] as Slice).state[dependencyPropName];
        });
        return properties;
    }
                ...
                Object.entries(this._sliceExtraValues).forEach(([sliceName, sliceExtraValues]) => {
                    sliceExtraValues.forEach((sliceExtraValue) => {
                        if (
                            ...
                            partialChangedSliceState.hasOwnProperty(
                                sliceExtraValue.dependencyPropName
                            )
                        ) {
                            stateForUpdate[sliceName] = stateForUpdate[sliceName] || {};
                            stateForUpdate[sliceName][sliceExtraValue.propName] =
                                partialChangedSliceState[sliceExtraValue.dependencyPropName];
                        }
                    });
                });
            }
