...
function resolveConfigsFromGetters(
    ...
    for (const [name, configGetter] of Object.entries(configGetters)) {
        ...
        configs[name] = {
            dataConfigs: !configGetter.isAsyncConfigGetter
...
export default class StoreWithSlices {
    ...
    createElementSync(
        ...
        if (node.getElement(elementName, false)) {
            contextElementAlreadyExist(elementName, 'createElementSync');
            return;
        }

        node.createElement(elementNode, loadResult, config);
