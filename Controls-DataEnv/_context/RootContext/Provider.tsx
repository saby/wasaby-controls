    ...
    const rootContextValue = React.useMemo(() => {
        return {
            ...
            getNode(root: string): Record<string, Slice> {
                const localNode = store.getState(root);
                if (localNode) {
                    return localNode;
                }
                /*
                 * Пока не реализован иерархический контекст: если ноду не нашли в текущем провайдере, то обращаемся к родительскому провайдеру.
                 * Будет удалено по задаче: https://online.sbis.ru/opendoc.html?guid=b8ed9e02-9986-4480-9e40-9a894cd49eed&client=3
                 */
                return parentRootContext?.getNode?.(root);
            },
        };
