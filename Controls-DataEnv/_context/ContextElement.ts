...
import { IoC, ILoggerInterface } from 'Env/Env';
...
function getChangedFields(currentState: unknown, nextState: unknown): Record<string, unknown> {
    const changes: Record<string, unknown> = {};
    if (isObjectSliceState(currentState) && isObjectSliceState(nextState)) {
        Object.keys(nextState).forEach((stateName) => {
            if (currentState[stateName] !== nextState[stateName]) {
                changes[stateName] = nextState[stateName];
            }
        });
    }
    return changes;
...
function getLogger(): ILoggerInterface {
...
function isFlatObjectTree(treeData: any): treeData is Record<string, IDataNodeConfigGetter> {
    return (
        typeof treeData === 'object' &&
        Object.values(treeData).some((value: any) => {
            return (
                value.hasOwnProperty('parentId') && !!(value as Record<string, unknown>).parentId
            );
        })
    );
}
...
export default class ContextElement {
    ...
    private _$destroyed: boolean = false;
    ...
    setState(newElementPartialState: IContextElementChange): void {
        if (isSlice(this._$value)) {
            this._$value.setState(newElementPartialState);
        } else {
    ...
    hasContextConfigs(): boolean {
        const dataFactory = loadSync<IDataFactory>(this._$props.config.dataFactoryName);

        return !!dataFactory.getContextConfig;
    }

    getContextConfigs(): IDataContextConfigs | undefined {
        const config = this._$props.config;
        const dataFactory = loadSync<IDataFactory>(config.dataFactoryName);

        if (dataFactory.getContextConfig) {
            const elementContextConfigs = dataFactory.getContextConfig(
                this._$props.loadResult,
            ...
            if (
                elementContextConfigs.configs &&
                DataConfigResolver.isDataConfigs(elementContextConfigs.configs) &&
                elementContextConfigs.loadResults
            ) {
                return {
                    configs: elementContextConfigs.configs,
                    data: elementContextConfigs.loadResults,
                };
            } else if (isFlatObjectTree(elementContextConfigs.configs)) {
                const tree = new relation.Tree<any>({
                    parentProperty: 'parentId',
                    childrenProperty: 'children',
                    keyProperty: 'name',
                });
                Object.entries(elementContextConfigs.configs).forEach(([name, value]) => {
                    //@ts-ignore
                    value.data = elementContextConfigs.loadResults?.[name] || {};
                });
                tree.parseTree(elementContextConfigs.configs);

                return tree.toObject();
            } else {
                return elementContextConfigs as IDataContextConfigs;
            }
        }
    }
    ...
    protected _onElementInitError(e: unknown): void {
        if (e instanceof Error) {
            getLogger().error(
                'Controls-DataEnv/context:ContextElement',
                `Произошла ошибка при инициализации слайса ${this._$name} из фабрики ${
                    this._$props.config.dataFactoryName
                }.
             Узел контекста ${this.getParent()?.getName()}.
    ...
    protected _notifySubscribers(nextState: unknown): void {
        if (isSlice(this._$value)) {
            this._$props.onSnapshot(
                this._$props.name,
                getChangedFields(this._$value.state, nextState)
            );
        }
    }
    ...
    protected _initSlice(
        ...
        return new sliceCtr({
            ...
            onInitError: this._onElementInitError.bind(this),
            onNotifySubscribers: this._notifySubscribers.bind(this),
            name,
        });
    }
    ...
    static getExtraValueByName(config: IDataConfig, name: string): IExtraValue | undefined {
        return config.dataFactoryArguments?.sliceExtraValues?.find(({ dependencyName }) => {
