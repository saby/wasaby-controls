...
export default class StoreWithSlices {
    ...
    private _$root: string;
    version: number = 0;
    listeners: Function[] = [];

    get prettyValue(): unknown {
    ...
    toObject(): unknown {
        return this._$value.toObject();
    }

    getDataContext(path: string[]): DataContextAPI {
        const node = this.getNodeByPath(path);

        if (!node) {
            throw new Error(
                'Получение контекста данных невозможно, возможно вы делаете вызов вне контекста'
            );
        }

        return node.getDataContext();
    }
    ...
    async reloadElement(
        ...
        const element = node.getElement(elementName);
        const nodeWithElement = element?.getParent();

        if (nodeWithElement) {
            return nodeWithElement.reloadElement(elementName, dataFactoryArguments, isReloadDeps);
        }
    }
    ...
    createNodeSync(
        ...
        const contextNode = this._$value.getNodeByPath(nodePath);
        ...
        contextNode.addChild(nodeName, nodeConfig);
    ...
    createRoot(
        parentPath: string[],
        rootName: string,
        rootConfig: IDataContextConfigs,
        onChange?: Function
    ): void {
        const contextNode = this._$value.getNodeByPath(parentPath);

        if (contextNode) {
            contextNode.addChild(rootName, rootConfig, onChange, true);
        }
    }
    ...
    getElementsFromPath(path: string[], withStates: boolean = false): Record<string, unknown> {
        ...
        if (path) {
            ...
            const levels = [];
            while (currentNode) {
                const valueFromNode = currentNode.getValue();
                if (withStates) {
                    const statesFromNode: Record<string, unknown> = {};

                    Object.entries(valueFromNode).forEach(([element, value]) => {
                        statesFromNode[element] = value instanceof Slice ? value.state : value;
                    });

                    levels.push(statesFromNode);
                } else {
                    levels.push(valueFromNode);
                }
    ...
    getState(node: string = this._$root): Record<string, Slice | unknown> {
    ...
    getNode(nodeName: string): ContextNode {
        const node = this.getNodeByPath([nodeName]) || this.getNodeByPath([this._$root]);
        return node as ContextNode;
    }

    update(contextConfigs: IDataContextConfigs): void {
        this._$value.update(contextConfigs, true);
        this.notifyOnChange();
    }
    ...
    updateNodeElements(
        ...
        const contextNode = this._$value.getNodeByPath(path);

        if (contextNode) {
            contextNode.updateElements(configs, loadResults);
        }

        this.notifyOnChange();
    }

    updateNode(path: string[], configs: IDataContextConfigs): void {
        const contextNode = this._$value.getNodeByPath(path);

        if (contextNode) {
            contextNode.update(configs);
        }
    }
    ...
    updateRoot(
        ...
        if (contextNode) {
            if (isPartialUpdate) {
                contextNode.update(configs, isPartialUpdate);
            } else {
                contextNode.reInitState(configs);
            }
        }
    }

    replaceRoot(newParentPath: string[], currentPath: string[]): void {
        const currentNode = this.getNodeByPath(currentPath);
        const newParentNode = this.getNodeByPath(newParentPath);
