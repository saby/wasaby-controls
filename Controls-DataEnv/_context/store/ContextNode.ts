...
export default class ContextNode {
    ...
    protected _$dataContext: DataContextAPI;
        ...
        if ('configs' in contextConfigs) {
            ...
            if (
                ...
                const contextConfigsFromGetter = this._$props.getSlicesConfig(
                    this.getDataContext().getNodeData(this.getPath(true))
                );
                return {
                    ...dataConfigs,
    ...
    fillDataContext(ctx: DataContext): void {
        ctx.addNode(this.getPath(false), this._$name, this._$data);
    ...
    getDataContext(): DataContext {
        ...
        if (rootNode) {
            rootNode.fillDataContext(ctx);
        }

        if (!ctx.hasNode(this.getPath(true))) {
            ctx.addNode(this.getPath(false), this._$name, this._$data);
        }
    ...
    addElement(name: string, loadResult: unknown, config: IDataConfig): void {
        ...
        const element = new ContextElement({
            ...
            parentNode: this,
            dataContext: () => {
                return this.getDataContext().getAPI(this.getPath(true));
            },
        });
