...
import DataContext from '../dataContextAPI/DataContextAPI';
import { IDataContextField } from 'Controls-DataEnv/_context/dataContextAPI/interface';
...
function hasContextField(name: string, ctx: IDataContextField[]): boolean {
    return !!ctx.find((el) => {
        return el.name === name;
    });
}
...
export default class ContextNode {
    ...
    private _getDataConfigs(contextConfigs = this._$contextConfigs): TDataConfigs {
        if ('configs' in contextConfigs) {
            ...
            if (
                ...
                const contextConfigsFromGetter = this._$props.getSlicesConfig(this._$data);
                return {
                    ...dataConfigs,
    ...
    fillDataContext(ctx: IDataContextField[]): void {
        Object.entries(this._$data).forEach(([name, value]) => {
            if (value instanceof Model && value.isTyped() && !hasContextField(name, ctx)) {
                ctx.unshift({
                    name,
                    value,
                });
        ...
        if (this._$children.size) {
            this._$children.forEach((child) => {
                child.fillDataContext(ctx);
            });
        }
    }

    getDataContext(): DataContext {
        let rootNode: ContextNode | null = null;
        const context: IDataContextField[] = [];
        ...
        if (rootNode) {
            rootNode.fillDataContext(context);
        }
    ...
    async reloadElement(
        ...
        const loadResult = await loader.load(reloadedConfigs, undefined, this._$props.router);
    ...
    static getDependencies(nodeConfigs: Record<string, IDataConfig>): string[] {
        const allDependencies = Object.values(nodeConfigs).flatMap((elementConfig) =>
            ContextElement.getDependencies(elementConfig)
        );
