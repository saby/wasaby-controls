...
export default class ContextNode {
    ...
    private _getDataConfigs(): TDataConfigs {
        const contextConfigs = this._$contextConfigs;
    ...
    private _initState(): void {
        Object.entries(this._$configs).forEach(([configName, config]) => {
            ...
            if (dataFactory.getContextConfig) {
                if (dataFactory.getDataFactoryArguments) {
                    ...
                    if (config.dependencies) {
                        config.dependencies.forEach((dependency) => {
                            dependenciesResults[dependency] = this._$data[dependency];
                        });
                    }
                ...
                const contextConfig = dataFactory.getContextConfig(
                    this._$data[configName],
                    dataFactoryArguments
                );

                if (
                    DataConfigResolver.isDataConfigs(contextConfig.configs) &&
                    contextConfig.loadResults
                ) {
                    this._$data = {
                        ...this._$data,
                        ...contextConfig.loadResults,
                ...
                } else if (isFlatObjectTree(contextConfig.configs)) {
                    const tree = new relation.Tree<any>({
                        parentProperty: 'parentId',
                        childrenProperty: 'children',
                        keyProperty: 'name',
                    });
                    Object.entries(contextConfig.configs).forEach(([name, value]) => {
                        //@ts-ignore
                        value.data = contextConfig.loadResults?.[name] || {};
                    });
                    tree.parseTree(contextConfig.configs);
                    this._$contextConfigs.children = tree.toObject();
                } else {
                    const childrenConfigs = contextConfig as IDataContextConfigs;
                    this._$contextConfigs.children = {
                        ...this._$contextConfigs.children,
                        [childrenConfigs.name || 'root']: childrenConfigs,
                    };
                }
            }
        });
    ...
    addElement(name: string, loadResult: unknown, config: IDataConfig): void {
        ...
        this._$elements[name] = new ContextElement({
            name,
    ...
    addChild(name: string, nodeConfig: IDataConfigLoader, data: Record<string, unknown>): void {
        const node = new ContextNode({
            router: this._$props.router,
            parentNode: this,
            contextConfigs: { ...nodeConfig, data },
            name,
            onChange: this._$props.onChange,
            onSnapshot: this._$props.onSnapshot,
            dataContext: this._$dataContext,
