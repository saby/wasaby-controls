...
export default function useSliceInternal<T>(
    ...
    const getSnapshot = useCallback(
        (currentStore) => {
            ...
            if (path) {
                currentSlice = currentStore?.getElement(storeId, path) as T;
            } else {
                currentSlice = currentStore?.getElement(storeId, [STORE_ROOT_NODE_KEY]) as T;
            }
    ...
    const getVersion = useCallback((source) => {
        return source?.getVersion() || 0;
    }, []);

    const snapshot = useSyncExternalStore(store, getVersion, getSnapshot, subscribe);
