...
/**
 * Интерфейс для управления текущим узлом контекста данных
 */
...
/**
 * Хук для получения текущего узла контекста данных
 * @private
 */
    ...
    return useMemo<IUsedContextNode>(() => {
        return {
            ...
            unsafe__getElementsByTree(): Record<string, unknown> {
                return storeRef.current?.getElementsFromPath(nodeContext.path) || {};
            },
        };
    }, [nodeContext]);
}
