...
import { IDataContextAPI } from 'Controls-DataEnv/context';
 ...
 * @remark
 * Полезные ссылки:
 * Про реализацию своего собственного читайте в {@link /doc/platform/developmentapl/interface-development/context-data/new-data-store/create-context/ статье}.
 * @public
 */
export default abstract class AbstractSlice<State = unknown> {
    ...
    protected _onCompleteBeforeApplyState: Function | undefined;
    private _isExecuting: boolean = false;
    private _queue: Partial<State>[] = [];
    private _destroyed: boolean;
    private _stateForApply: Partial<State> | null;
    private _beforeApplyReject: Function | null;
    private readonly _$onNotifySubscribers?: Function;
    private _$dataContext?: () => IDataContextAPI;

    get dataContext(): IDataContextAPI | undefined {
    ...
    constructor(props: ISliceConstructorProps<unknown, unknown>) {
        ...
        this._onCompleteBeforeApplyState = props.onCompleteBeforeApplyState;
        this._$onNotifySubscribers = props.onNotifySubscribers;
    ...
    protected _notifySubscribers(nextState: State): void {
        this._$onNotifySubscribers?.(nextState);
    }
    ...
    private _setState(state: State): void {
        if (this.isDestroyed()) {
            Logger.error(
                'Controls-DataEnv/slice:AbstractSlice::Произошла попытка установить состояние в уничтоженный экземпляр слайса'
            );
        }
        const nextState = this._onSnapshot(state) || state;
        this._notifySubscribers(nextState);
    ...
    setState(
        ...
        if (beforeApplyResult instanceof Promise) {
            ...
            return beforeApplyPromise
                .then((result: State) => {
                    // зануляем состояние, так как оно протухло. Самое актуальное состояние - результат beforeApplyState
                    // основной проблемный кейс:
                    // В beforeApplyState мутируется состояние, но в колбэк, который вызывается по синхронной отрисовке,
                    // приходит неактуальное состояние
                    this._stateForApply = null;
                ...
                .finally(() => {
                    ...
                    this._isExecuting = false;
                    this._stateForApply = null;
                    this._onCompleteBeforeApplyState?.();
                    this._executeQueueIfNeed();
                });
