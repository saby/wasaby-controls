...
export default abstract class AbstractSlice<State = unknown> {
    ...
    setState(partialState: Partial<State>): void {
        ...
        if (isObjectState(this.state) && isObjectState(partialState)) {
            stateAreEqual = Object.keys(partialState).every((key) => {
                return (
                    (this.state as Record<string, unknown>)[key] ===
                    (partialState as Record<string, unknown>)[key]
                );
            });
        } else {
            stateAreEqual = this.state === partialState;
        }
        ...
        if (beforeApplyResult instanceof Promise) {
            ...
            beforeApplyPromise
                .then((result: State) => {
                    this._setState(this._onSnapshot(result) || result);
                })
                .catch((error) => {
                    if (!error.isCanceled) {
            ...
            this._setState(this._onSnapshot(beforeApplyResult));
            this._isExecuting = false;
            this._executeQueueIfNeed();
        }
    }
