...
function reorderChildren(
    prevItems: IItemsOptions['variants']['items'],
    newItems: IItemsOptions['variants']['items'],
    children: unknown[]
): unknown[] {
    const prevIdToIndex = new Map<number, number>();
    prevItems.forEach((item, index) => {
        prevIdToIndex.set(item.id, index);
    });

    let hasChanged = false;
    const newChildren = newItems.map((item, i) => {
        const index = prevIdToIndex.get(item.id) as number;
        if (index !== i) {
            hasChanged = true;
        }
        return children[index];
    });
    return hasChanged ? newChildren : children;
}
...
export function changeHandler(
    ...
    if (prevItemsCount !== nextItemsCount) {
        ...
        const orderChildren = reorderChildren(
            prevVariants.items,
            nextVariants.items,
            changedChildren
        );
        if (orderChildren !== changedChildren) {
            changedChildren = orderChildren;
            isChildrenChanged = true;
        }
    }
