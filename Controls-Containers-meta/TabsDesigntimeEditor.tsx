...
const TABS_STYLE = {
    ...
    paddingLeft:
        'var(--outer_padding, var(--item_container_margin-horizontal_tabs_local, var(--offset_m)))',
...
export default function TabsDesigntimeEditor(props: IControlDesignerProps) {
    ...
    const selectedKeyChangeHandler = (selectedKey: number) => {
        ...
        if (selectedKey === staticProperties.variants?.selectedKeys?.[0]) {
            variants.fUpdate = (staticProperties.variants?.fUpdate || 0) + 1;
        }
    ...
    const alignClass = staticProperties?.align === 'right' ? 'tw-justify-end' : 'tw-justify-start';
    ...
    return (
        <div className={className}>
            <div className={`tw-flex tw-w-full ${alignClass}`} style={TABS_STYLE}>
                {items.map((item) => {
                    return (
                        <div
                            title={item.title}
