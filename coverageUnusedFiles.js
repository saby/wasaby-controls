...
let path = require('path'),
   ...
   coverageControlsPath = path.join(
      __dirname,
      'artifacts',
      'coverageControls.json'
   ),
   coverageComponentsPath = path.join(
      __dirname,
      'artifacts',
      'coverageComponents.json'
   ),
   allFiles = [];
...
let rawCover = fs.readFileSync(coveragePath, 'utf8'),
   ...
   componentsFiles = allFiles.filter((file) =>
      file.includes('/SBIS3.CONTROLS/')
   );
...
fs.writeFileSync(
   coverageControlsPath,
   JSON.stringify(controlsCoverage),
   'utf8'
);
fs.writeFileSync(
   coverageComponentsPath,
   JSON.stringify(componentsCoverage),
   'utf8'
);
