...
export default class Layout extends Control<ILayoutOptions> implements IStep {
    ...
    protected _needHideUncompletedSteps(index: number): boolean {
        return (
            this._isAdaptive &&
            this._options?.hideUncompletedSteps &&
            ((!!this._options.items?.[index]?.nextButtonCaption &&
                this._options.items?.[index]?.nextButtonCaption !== 'Далее') ||
                this._options?.nextButtonCaption !== 'Далее')
        );
    }
