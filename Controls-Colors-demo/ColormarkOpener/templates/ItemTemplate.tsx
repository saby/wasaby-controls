        ...
        <ItemTemplate
            ...
            contentTemplate={(templateProps: IItemTemplateProps) => {
                ...
                if (!keys || (keys && keys.length < 1)) {
                    return <div className="Controls-Colors-demo_list_lineHeight">{title}</div>;
                }
                const titleResult = [];
                const iconResult = [];
                items.forEach((item) => {
                    if (keys?.includes?.(item.id)) {
                        ...
                        if (itemType === 'color') {
                            titleResult.push(<div style={{ color: getColor(item) }}>{title}</div>);
                        }
                        if (itemType === 'style') {
                            titleResult.push(
                                <div
                                    style={{
                                        ...
                                        textDecoration: item.value.style.s
                                            ? 'line-through'
                                            : 'none',
                                        borderBottom: item.value.style.u
                                            ? '1px solid ' + getColor(item)
                                            : '',
                                    }}
                                >
                                    {title}
                        ...
                        if (item.icon) {
                            iconResult.push(
                                <div
                                    className={
                                        'Controls-Colors-demo_iconItem controls-margin_left-s icon-' +
                                        item.icon +
                                        ' controls-icon_style-' +
                                        item.iconStyle +
                                        ' controls-icon_size-' +
                ...
                return (
                    <div className="tw-flex tw-justify-between Controls-Colors-demo_list_lineHeight">
                        ...
                        <div className="tw-flex">{iconResult.length >= 1 && iconResult}</div>
                    </div>
                );
            }}
        />
