...
import { AdaptiveSideBar } from './stackBody/AdaptiveSideBar';
...
export const ContentTemplate = forwardRef(
    (props: IStackContentProps, ref: LegacyRef<Async>): JSX.Element | null => {
        ...
        const { configs, multiSelect, selectedItems } = selectSlice.state;
        ...
        const [expandableCollectionVisible, setExpandableCollectionVisible] = useState(true);

        const { selectedKeys, items, nodeProperty, searchValue, viewMode, displayProperty } =
            currentListSlice.state;
        ...
        }, [selectedItems, props.listName]);

        useEffect(() => {
            if (multiSelect && selectedItemsKeys && !isEqual(selectedItemsKeys, selectedKeys)) {
                if (stackWithTabs && expandableCollectionVisible) {
                    if (selectedKeys.includes(null)) {
                        setExpandableCollectionVisible(false);
        ...
        return (
            <>
                {expandableCollectionVisible && multiSelect && stackWithTabs && (
                ...
                >
                    <Container
                        backgroundStyle={viewMode === 'tile' ? 'unaccented' : 'default'}
                        className={`controls-air-m controls-Layout-SelectorStack__content 
                                           ${
                                               viewMode === 'tile'
                                                   ? ''
                ...
                {props.isAdaptive ? <AdaptiveSideBar listName={props.listName} /> : null}
            </>
        );
    }
);
