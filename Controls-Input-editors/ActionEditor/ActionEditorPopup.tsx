import { memo, useCallback, useContext, useMemo, useState, ComponentProps } from 'react';
...
interface IListItemTemplateContent {
    item: {
        contents: {
...
function ListItemTemplateContent(props: IListItemTemplateContent) {
    return (
...
function ListItemTemplate(props: IItemTemplateProps) {
    return (
        <ItemTemplate {...props}>
...
function ListGroupTemplate(props: ComponentProps<typeof GroupTemplate>) {
    return (
        <GroupTemplate
...
export const ActionEditorPopup = memo((props: IActionEditorPopupProps) => {
    ...
    const items = useMemo<RecordSet<IActionItems>>(() => {
        const isInsideRights = constants.product.includes('sabyinside') && hasRights(['application_administration']);
        const data = props.actions
            ...
            .filter(({ rights }: IActionConfig) => {
                return isInsideRights || hasRights(rights as TActionRights);
            });
