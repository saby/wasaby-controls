...
import { Context, StackOpener } from 'Controls/popup';
...
} from 'Controls-Input-editors/ActionEditor';
...
export const ActionsListEditorPopup = function (props): JSX.Element {
    ...
    }, [props, popupRef.current, onActionTypeChanged, stackOpener]);

    const editingObject = useMemo(() => {
        return actionConfig?.actionProps || actionConfig?.commandOptions || {};
    }, [actionConfig]);
    const typeDescription = useMemo(() => {
        ...
        if (actionConfig?.info?.title) {
            additionalDescription.unshift({
                type: 'string',
                name: 'actionButton',
        ...
        return [...additionalDescription, ...propTypes];
    }, [actionConfig?.propTypes, actionConfig?.info?.title]);
        ...
        return (
            <ValidationController ref={validationContainerRef}>
                <Dialog
                    ...
                    bodyContentTemplate={
                        <ActionEditorPopupTemplate
                            className="controls-margin_left-m controls-margin_right-m controls-margin_top-xs controls-margin_bottom-s"
