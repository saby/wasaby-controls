...
function TitleTemplate(props): JSX.Element | null {
    ...
    return (
        <ColumnTemplate
            ...
            cursor={'default'}
            contentTemplate={
                <div className="controls-max-w-full">
                    <div className="ws-flexbox ws-align-items-baseline">
                        ...
                        {!props.item.isEditing() ? (
                            <>
                                <span className="controls-margin_left-xs controls-margin_right-xs ws-ellipsis">
                                    {props.item.contents.get('title')}
                                </span>
                                {props.expanderTemplate && (
                                    <props.expanderTemplate
                                        className={'controls-ActionsListEditor__expander'}
                                    />
                                )}
                            </>
                        ) : (
...
export const ActionsListEditor = memo((props: IActionsListEditorProps) => {
    ...
    const applyActionConfigByActionType = useCallback(
        (item: Model) => {
            ...
            const propTypesWithDataFactory = getPropTypesWithDataFactory(selectedAction);
            selectedAction.actionProps = {
                ...actionProps,
                title: item.get('title'),
                icon: item.get('icon'),
                dataFactory: propTypesWithDataFactory?.dataFactory,
            };
            const itemHasValidation = selectedAction.propTypes?.some((item) => item.validators);
            if (selectedAction.id || itemHasValidation) {
                // Если редактирование
            ...
            if (action.id) {
                ...
                const items = props.value.items.map((item) => {
                    if (item.id === action.id) {
                        ...
                        if (actionProps) {
                            const actionValueProperties = getActionValueProperties(action);
                            const actionValue =
                                actionValueProperties?.length &&
                                actionValueProperties?.every(
                                    (actionProp) => actionProps[actionProp] !== undefined
                                );
                            actionProps.dataFactory = actionValue
                                ? null
                                : getPropTypesWithDataFactory(action)?.dataFactory;
                        }
                        return {
                            ...item,
                            ...
                            action: {
                                id: action.type,
                                actionProps: actionProps || action.commandOptions,
                            },
                        };
                    }
                    return item;
    ...
    const getPropTypesWithDataFactory = useCallback(
        (selectedAction: IItemAction) => {
            return selectedAction.propTypes?.find((selectedIem) => !!selectedIem.dataFactory);
        },
        [props.value.items]
    );

    const getActionValueProperties = useCallback(
        (selectedAction: IItemAction) => {
            return selectedAction?.propTypes
                ?.filter((action) => action.dataFactory)
        ...
        (action: IItemAction, item: Model) => {
            ...
            switch (action.id) {
                case 'configuration':
                    ...
                    break;
                case 'up':
                    movingAction = treeRef.current.moveItemUp(item.getKey());
                    break;
                case 'down':
                    movingAction = treeRef.current.moveItemDown(item.getKey());
                    break;
            }
            if (movingAction) {
                movingAction.then(() => {
                    onChange({
                        items: [...treeRef.current.getItems().getRawData()],
                    });
                });
            }
        },
        ...
        <LayoutComponent>
            {!!props.groupCaption ? (
                <div className={'controls-ActionsListEditor_heading-caption'}>
                    ...
                    <Button
                        className="controls-margin_left-s"
                        data-qa="ActionsListEditor__Add"
                        viewMode="filled"
                        buttonStyle="pale"
                    ...
                    <Button
                        className="controls-margin_left-s"
                        data-qa="ActionsListEditor__AddFolder"
                        viewMode="filled"
                        buttonStyle="pale"
