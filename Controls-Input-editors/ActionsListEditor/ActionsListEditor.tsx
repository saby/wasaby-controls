...
export const ActionsListEditor = memo((props: IActionsListEditorProps) => {
    ...
    const applyActionConfigByActionType = useCallback(
        (item: Model) => {
            ...
            const propTypesWithDataFactory = selectedAction.propTypes?.find(
                (selectedIem) => !!selectedIem.dataFactory
            );
            ...
            if (selectedAction.id || itemHasValidation) {
                ...
                if (propTypesWithDataFactory?.dataFactory) {
                    menuItem.actionSubMenu = {
                        moduleName: 'Controls-Input/dropdownConnected:ActionsSource',
                        options: {
                            keyProperty: 'id',
                            dataFactory: propTypesWithDataFactory?.dataFactory,
                            actionKeyProperty: propTypesWithDataFactory.name,
                            parentId: selectedAction.type,
                        },
                    };
                }
                onChange({
