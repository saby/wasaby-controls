import { Fragment, ReactElement, useEffect, useState, useCallback, useMemo } from 'react';
...
import 'css!Controls-Input-editors/StyleEditor/StyleEditor';
...
function parseStyleInClassName(
    ...
    if (name) {
        let regStyleValue = name.match(/controls-([^_]+)_([^-]+-same)-([^-]+)-style/);
        if (!regStyleValue) {
            regStyleValue = name.match(/controls-([^_]+)_([^-]+)-([^-]+)-style/);
        }
...
export function StyleEditor(props: IStyleEditorProps) {
    ...
    const style = useMemo<IButtonStyle>(() => {
        return parseStyleInClassName(value);
    }, [value]);

    const typeDescription: IPropertyGridProperty[] = useMemo(() => {
        return [
            {
                name: 'viewMode',
                type: 'string',
    ...
    const editingObjectChangedHandler = useCallback((editingObject: IButtonStyle) => {
        if (editingObject && (editingObject.buttonStyle === 'label' || editingObject.buttonStyle === 'link') && editingObject.viewMode !== 'link') {
            editingObject.buttonStyle = 'secondary';
        }
