...
const items = new RecordSet({
    ...
    rawData: [
        ...
        // скрываем бледные и квадратные кнопки, так как они пока не нужны
...
function _getButtonStyle(
    buttonStyle: IButtonStyle['buttonStyle'],
    viewMode?: string
): string | unknown {
    if (!((buttonStyle === 'link' || buttonStyle === 'label') && viewMode !== 'link')) {
        if (buttonStyle === 'pale' && viewMode !== 'filled') {
            return undefined;
        }
        return buttonStyle;
    }
...
export default forwardRef(function InlineHeightEditor(
    ...
    const ContentTemplate = useCallback(
        (itemTemplateProps: Record<string, unknown>) => {
            return (
                <Button
                    {...itemTemplateProps}
        ...
        [propertyValue, props.editorStyle?.buttonStyle]
    );
