...
interface IMultilineEditorProps extends IPropertyGridPropertyEditorProps<IValue> {
...
export const MultilineEditor = memo((props: IMultilineEditorProps) => {
    ...
    const onMinValueChanged = (_, minValue) => {
        return onChange({ ...value, minLines: Number(minValue) || 0 });
    };
    const onMaxValueChanged = (_, maxValue) => {
            ...
            <div className="tw-flex tw-w-full tw-items-baseline">
                ...
                {showInput && (
                    <>
                        {' ' + rk('от')}
                        <NumberInputControl
                            className="controls-Input__width-4ch tw-self-center controls-margin_left-xs controls-margin_right-xs Controls-Input-editors_MultilineEditor__value-from"
                        ...
                        {rk('до')}
                        <NumberInputControl
                            className="controls-Input__width-4ch tw-self-center controls-margin_left-xs controls-margin_right-xs Controls-Input-editors_MultilineEditor__value-to"
                            value={maxLines}
                            onlyPositive={true}
                            precision={0}
                            placeholder={rk('авто')}
