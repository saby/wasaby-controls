...
export default function DateRangeEditor(props: IDateRangeEditorProps): JSX.Element {
    ...
    const loadAndSetMetaType = useCallback((newMetaType) => {
        Promise.all([
            newMetaType.getAttributes().dateRangeType.getEditor().load(),
            newMetaType.getAttributes().defaultDateRange.getEditor().load(),
            newMetaType.getAttributes().type.getEditor().loader(),
            newMetaType.getAttributes().arrows.getEditor().loader(),
            newMetaType.getAttributes().position.getEditor().loader(),
        ]).then(() => {
    ...
    const calculateDateRangeTypes = useCallback(
        (newDateRangeTypeValue, newMinRangeValue) => {
            const attributes = resultMeta?.getAttributes();
            ...
            const newDateRangeType = attributes.dateRangeType.editorProps({
                ...
                menuPopupOptions,
                itemTemplate: 'Controls-ListEnv-editors/dateRangeEditor:DateRangeEditorItemTemplate',
    ...
    const changeMetaTypes = useCallback(
        (newDateRangeTypeValue, newMinRangeValue) => {
            ...
            const newMetaType = resultMeta?.attributes({
                ...attributes,
