...
export default class Buttons extends Control<IVerticalTabs> {
    ...
    protected _onButtonClick(_event: SyntheticEvent, item: Model<IVerticalTabsItem>): void {
        const clickHandler = item.get('buttonClickHandler') as unknown as Function | undefined;
        if (clickHandler) {
            clickHandler();
        }
    }
    ...
    protected _getItemStyle(item: Model<IVerticalTabsItem>): CSSProperties {
        const style: CSSProperties = {};
        if (item.get('backgroundImage')) {
            style.backgroundImage = `url('${item.get('backgroundImage')}')`;
        }

        if (item.get('height') || this._options.itemHeight) {
            style.height = item.get('height') || this._options.itemHeight;
        }
        return style;
    }
