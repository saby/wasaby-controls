...
class View extends Control<IColoredTabsOptions> {
    ...
    private _updateItemsHeightAfterResize: boolean = false;

    private _shouldStopNativeTouchMove: boolean;
    private _shouldNotUpdatePosition: boolean;
    private _shouldPreventTouchMove: boolean;

    private _scrollTimer: number;

    private _currentPositionToScroll: number | null;
    ...
    protected _componentDidMount(options: IColoredTabsOptions): void {
        // Подписываемся через addEventListener, для того чтобы указать passive: false, что позволит останаливать
        // нативный скролл.
        this._container.addEventListener('touchmove', this._documentTouchMove.bind(this), {
            passive: false,
        });
        ...
        this._calcSizes();
        // Вкладки на СП строятся без расчетов в процентном соотношении с минимальной высотой. (Т.к. нет доступа к DOM)
        // На момент componentDidMount мы уже сеттим рассчитанные коордианты, но для правильного построения мешает
        // стиль с минимальной высотой. Уберем его синхронно, чтобы не было скачков вкладок в окне.
        this._container.className += ' controls-ColoredTabs_disableMinHeight';
            ...
            this._getSelectedKeyByStorageId(options.propStorageId).then((selectedKey) => {
                if (selectedKey) {
                    ...
                    if (selectedItem) {
                        ...
                        if (this._items[0].key !== selectedKey) {
                            this._setSelectedItem(selectedItem);
                            const itemIndex = this._getItemIndex(selectedItem);
                            for (let i = 0; i <= itemIndex; i++) {
                                this._setStackedTop(i);
                            }
                        }
                    } else {
    ...
    protected _beforeUpdate(options: IColoredTabsOptions): void {
        ...
        if (this._options.items.length !== options.items.length) {
            ...
            if (!this._container?.closest(POPUP_HIDDEN_CLASS)) {
                ...
                this._updateItemsHeightAfterResize = true;
            }
        } else {
            this.updateSizes();
        }
        if (
            typeof options.selectedKey !== 'undefined' &&
            this._selectedKey !== options.selectedKey
        ) {
            this._saveSelectedKey(options.selectedKey, options.propStorageId);
            this._calcSizes();
        }
        this._updateItemContentStyle();
    }
    ...
    protected _afterRender(options: IColoredTabsOptions) {
        if (options.items.length !== this._options.items.length) {
            this._calcSizes();
        }
        this._updateItemContentStyle();
    }
    ...
    private _updateItemsHeight(): void {
        this._itemsHeight = [];
        ...
        if (!this._mousePositionDnD && !this._touchPositionDnD) {
            window.requestAnimationFrame(() => {
                ...
                this._items.forEach((item: IItemTab, index: number) => {
                    ...
                    this._setItemContentStyle(item);
                });
                if (shouldCalcSizes) {
                    this._calcSizes();
                }
    ...
    private _resizeObserverCallback(entries: []): void {
        ...
        // Если на момент обновления items контейнер был скрыт, обновим позиционирование только после того,
        // как компонент снова покажут и появятся размеры
        if (this._updateItemsHeightAfterResize) {
            this._updateItemsHeight();
            this._updateItemsHeightAfterResize = false;
        ...
        if (Math.abs(this._containerWidth - newWidth) > 0) {
    ...
    private _updateItemContentStyle(): void {
        this._items.forEach((item: IItemTab) => {
            this._setItemContentStyle(item);
        });
    }

    protected _setItemContentStyle(item: IItemTab): void {
        ...
        if (this._children[`contentId${index}`]) {
            this._children[`contentId${index}`].style.cssText = style;
        }
    }

    private _getItemIndex(item: IItemTab): number {
        let index = this._items.indexOf(item);
        if (index === -1) {
            index = this._items.findIndex((itemFromItems: IItemTab) => {
                return item.key === itemFromItems.key;
            });
        }
        return index;
    }
    ...
    protected _mouseDownHandler(event: SyntheticEvent<MouseEvent>): void {
        // Остановим скролл, который инициировало нажатие на колесико мышки
    ...
    protected _intersectHandler(event: Event, entries: IntersectionObserverSyntheticEntry[]): void {
        ...
        if (!this._shouldNotAnimate) {
            entries.forEach((entry: IntersectionObserverSyntheticEntry) => {
                ...
                if (!entry.nativeEntry.isIntersecting) {
                    if (entry.data.position === 'top') {
                        ...
                        const entryBottom = Math.ceil(
                            entry.nativeEntry.target.getBoundingClientRect().bottom
                        );
                        const containerBottom = this._container.getBoundingClientRect().bottom;
                        if (
                            entryBottom + 10 < containerBottom &&
                            entryBottom - 10 < containerBottom
                        ) {
                            return;
                        }
            ...
            if (lastStackedItemIndex >= 0 && lastStackedItemIndex <= this._items.length - 1) {
                ...
                this._saveSelectedKey(lastStackedItem.key, this._options.propStorageId);
            }
        }
    }
    ...
    protected _touchMoveHandler(event: SyntheticEvent<TouchEvent>, item: IItemTab): void {
        ...
        if (!this._shouldStopNativeTouchMove && this._hasScrollAvailable(index, positionToScroll)) {
            if (this._stackedTop.includes(index)) {
                if (positionToScroll > 0) {
                    if (this._scrollStatesList[index].verticalPosition !== 'end') {
                        this._shouldPreventTouchMove = false;
                        this._shouldNotUpdatePosition = true;
                    }
                } else {
                    if (this._scrollStatesList[index].verticalPosition !== 'start') {
                        this._shouldPreventTouchMove = false;
        ...
        if (this._shouldNotUpdatePosition) {
            return;
        }
        this._shouldPreventTouchMove = true;
        this._shouldStopNativeTouchMove = true;
        this._calcTabsPosition(index, positionToScroll, index !== 0 || positionToScroll > 0);
        this._touchPositionDnD = currentTouchPosition;
    }
    ...
    protected _touchStartHandler(): void {
        ...
        this._shouldNotUpdatePosition = false;
        this._shouldStopNativeTouchMove = false;
    }

    protected _touchEndHandler(event: SyntheticEvent<TouchEvent>, item: IItemTab): void {
        this._shouldNotUpdatePosition = false;
        this._shouldStopNativeTouchMove = false;
        const index = this._getItemIndex(item);
        if (this._stackedTop.includes(index)) {
    ...
    private _documentTouchMove(event: TouchEvent): void {
        if (event.cancelable && this._shouldPreventTouchMove) {
            event.preventDefault();
        }
    }

    protected _contentResizeHandler(): void {
        if (this._getContentsHeight() <= this._containerHeight) {
            this._calcSizes();
        }
    }
    ...
    protected _wheelHandler(event: SyntheticEvent<WheelEvent>, initialItem: IItemTab): void {
        ...
        const initialIndex = this._getItemIndex(initialItem);
        let itemToScroll = initialIndex;
        let nativeScroll = true;
        if (
            this._hasScrollAvailable(initialIndex, delta) &&
            delta < 0 &&
            this._scrollStatesList[initialIndex].verticalPosition !== 'start' &&
            this._items.length - 1 > initialIndex &&
            this._itemsHeight[initialIndex + 1]
        ) {
            nativeScroll = false;
            itemToScroll++;
            if (this._scrollStatesList[initialIndex].verticalPosition !== 'end') {
                if (typeof this._currentPositionToScroll === 'number') {
                    this._currentPositionToScroll = Math.max(
                        this._currentPositionToScroll + delta,
                        0
                    );
                } else {
                    this._currentPositionToScroll =
                        this._scrollStatesList[initialIndex].scrollTop + delta;
                }
                this._children['scrollContainer' + initialIndex].scrollTo(
                    this._currentPositionToScroll,
                    'vertical',
                    true
                );
                this._scrollInterval = setTimeout(() => {
                    this._currentPositionToScroll = null;
                }, 100);
            }
        } else {
            this._currentPositionToScroll = null;
        }
        if (nativeScroll && this._hasScrollAvailable(initialIndex, delta)) {
        ...
        event.preventDefault();
        this._calcTabsPosition(itemToScroll, delta);
        this._validatePosition();
    }

    private _setScrollTimer(): void {
        if (this._scrollTimer) {
    ...
    private _getContentHeight(index: number): number {
        if (!this._contentContainers[index] || !this._contentContainers[index].offsetHeight) {
