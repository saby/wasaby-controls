 ...
 * {@link https://wi.sbis.ru/docs/js/TabsLayout/_colored/View/options/headTemplate headTemplate}.
 * Пример использования смотрите {@link https://wi.sbis.ru/docs/js/TabsLayout/_colored/View/options/headTemplate здесь}.
 * @variant headTemplateOptions Опции для заголовка вкладки.
 */

/**
 * @name Controls-TabsLayout/_colored/View#items
 ...
 */
/**
 * @name Controls-TabsLayout/_colored/View#headerContentTemplate
...
/**
 * @name Controls-TabsLayout/_colored/View#backgroundFill
 ...
 * </pre>
 * @demo Controls-TabsLayout-demo/colored/BaseView/BackgroundFill/Index
 */

/**
 * @name Controls-TabsLayout/_colored/View#propStorageId
 * @cfg {String} Уникальный идентификатор, по которому будут сохраняться текущая выбранная вкладка.
 */

/**
 * @name Controls-TabsLayout/_colored/View#headTemplate
 ...
 * @demo Controls-TabsLayout-demo/colored/BaseView/HeadCounter/Index
 * @example
 * WML:
 * <pre>
 *     <Controls-TabsLayout.colored:View
...
/**
 * @name Controls-TabsLayout/_colored/View/typedefs/ItemTab#headTemplate
 ...
 *                headTemplate="Controls-TabsLayout/colored:HeadCounter">
 *         <ws:itemTemplate>
...
/**
 * @name Controls-TabsLayout/_colored/View#maxHeaderWidth
 * @cfg {number} Максимальная ширина текста заголовка вкладки в px.
 * @demo Controls-TabsLayout-demo/colored/BaseView/MaxHeaderWidth/Index
 * @default undefined
 * @remark
 * Задайте для равномерной ширины вкладок в развёрнутом состоянии.
 */

export interface IColoredTabsOptions extends IControlOptions {
    items?: IItemTab[];
...
class View extends Control<IColoredTabsOptions> {
    ...
    protected _isCompactTab(itemIndex: number): boolean {
        if (this._stackedTop.indexOf(itemIndex) === -1) {
            return;
        }
        if (!this._container) {
            return false;
    ...
    private _removeStackedBottom(itemIndex: number): void {
        const index = this._stackedBottom.indexOf(itemIndex);
        if (index !== -1) {
            for (let i = itemIndex; i >= 0; i++) {
                if (this._stackedTop.includes(i)) {
                    const stackedIndex = this._stackedTop.indexOf(i);
                    this._stackedTop.splice(stackedIndex, 1);
                    this._tabsPositionBottom[i] = 0;
                }
            }
        }
    }
    ...
    private _getContentHeight(index: number): number {
        ...
        const content = this._container.querySelector(contentClassName);
        if (typeof content?.offsetHeight === 'number') {
    ...
    private _maximizeTabHeight(itemIndex: number, maxHeight: number): void {
        ...
        let maxItemHeight = this._container.querySelector(
            '.controls-ColoredTabs__custom-content__index-' + itemIndex
        ).offsetHeight;
        ...
        const validateResult = () => {
            ...
            if (resultValue !== maxHeight) {
                ...
                if (this._items[nextItemIndex]) {
                    const contentHeight = this._container.querySelector(
                        '.controls-ColoredTabs__custom-content__index-' + nextItemIndex
                    ).offsetHeight;
