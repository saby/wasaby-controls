 ...
 *
 * @class TabsLayout/_colored/HorizontalView
    ...
    const onWheelHandler = (event, index) => {
        ...
        if (scrollState.canVerticalScroll) {
            if (index !== selectedItem) {
                if ((scrollState.verticalPosition !== 'end' || direction !== 'bottom') &&
            ...
            if (index === selectedItem) {
                ...
                if (scrollState.verticalPosition === 'start' && direction === 'top') {
                    const newSelectedItem = index - 1;
                    if (isValidItemIndex(newSelectedItem, props.items)) {
                        setSelectedItem(newSelectedItem);
                    }
    ...
    return (
        <div className='ColoredTabsHorizontal'>
            ...
            { props.items.map((item, index) => {
                const position = calculatePosition(props.items, index, props.activeTabWidth, props.width, selectedItem);
                return <div className='ColoredTabsHorizontal__item-container'
                            style={{
                                left: `${position}px`
                            }}>
                    ...
                    >
                        <ScrollContainer
                            onScrollStateChanged={(scrollState) => {scrollStateChangedHandler(scrollState, index);}}
                            onWheel={(event) => {onWheelHandler(event, index);}}
...
/**
 * @name TabsLayout/_colored/HorizontalView#width
...
/**
 * @name TabsLayout/_colored/HorizontalView#activeTabWidth
