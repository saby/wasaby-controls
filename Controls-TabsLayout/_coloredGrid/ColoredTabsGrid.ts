 ...
 * @property {String} style Стиль {@link /docs/js/Controls/interface/IFontColorStyle/options/fontColorStyle/ цвета} вкладки.
...
/**
 * Табличная обертка над {@link /docs/js/ColoredTabs/ColoredTabs/ ColoredTabs/ColoredTabs}
 * @remark
 * Предназначена для упрощения взаимодействия с ColoredTabs/ColoredTabs в раскладках списка и его окружения:
 * <ol>
 *     <li>{@link /docs/js/ColoredTabs/browsers/Browser/ ColoredTabs/browsers/Browser}.</li>
 *     <li>{@link /docs/js/ColoredTabs/Selector/Browser/ ColoredTabs/Selector/Browser}.</li>
 * </ol>
 *
...
export default class ColoredTabsGrid extends Control<IColoredTabsGridOptions> {
    ...
    updateItem(item: Model): void {
        let itemIndex;
        this._colorItems.forEach((colorItem: IColorItems, index) => {
            if (colorItem.key === item.get('id')) {
                itemIndex = index;
            }
        });
        if (itemIndex !== undefined) {
            this._colorItems[itemIndex] = this._buildTab(item);
        }
    }
    ...
    private _buildTabs(options?: IColoredTabsGridOptions): void {
        this._listItems = [];
        this._colorItems = [];
        const listItems: RecordSet[] = [];

        this._dataOptions.items.each((rec: Model) => {
            const tabItem = this._buildTab(rec, options);
            this._colorItems.push(tabItem);
            listItems.push(tabItem.templateOptions.listItems);
        });
        if (this._mounted) {
            this._notify('colorTabsItemsReady', [this._colorItems]);
            this._notify('itemsLinksChanged', [(this._listItems = listItems)]);
        }
    }

    private _buildTab(record: Model, options?: IColoredTabsGridOptions): IColorItems {
        const opts = options || this._options;
        let itemsData = record.get('items');
        const tabName = record.get('name');
        const tabCount = record.get('total');
        const keyValue = record.get('id');
    ...
    protected _excludedKeysTabChangedHandler(
        ...
        // Обработка ситуации, когда отметили все записи через ПМО
        // А потом в одном из списков руками убрали все галки
        // Этот список стрельнёт событием: selectedKeys: [], excludedKeys[]
        // Тогда как в списке на другой вкладке по прежнему всё отмечено и у него selectedKeys: [null], excludedKeys: [null]
        // Надо в итоговый excludedKeys добавить все записи списка, в котором сняли галки, чтобы метод БЛ отработал правильно
        const isInSelectedKeys =
            deleted.find((key) => this._selectedKeysByTabs[tabName].includes(key)) !== undefined;
        const isExcludedAllInOtherTabs = Object.keys(this._excludedKeysByTabs).find(
            (tab) => tab !== tabName && this._excludedKeysByTabs[tab].includes(null)
        );
    ...
    private _itemsReadyCallback(items: RecordSet, nodeLoad?: boolean): void {
        if (items && !this._listItems.includes(items)) {
