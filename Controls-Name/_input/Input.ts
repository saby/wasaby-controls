...
import { Base, Field, ITextViewModelOptions, TextViewModel } from 'Controls/input';
...
export default class Input extends Control<IInputOptions, IReceivedState> {
    ...
    protected _beforeMount(
        ...
        if (options.autoComplete) {
            ...
            this._autocomplete = receivedState?.autocomplete || 'autocomplete-' + Date.now();
            this._fieldName = receivedState?.fieldName || Date.now();
        }
        return {
    ...
    protected _afterUpdate(oldOptions: IInputOptions): void {
        const isActive =
            this._container === document.activeElement ||
        ...
        if (JSON.stringify(oldOptions.inputOrder) !== JSON.stringify(this._options.inputOrder)) {
            ...
            if (this._lastActiveInput && oldOptions.inputOrder) {
                ...
                if (lastActiveInputIndex !== -1 && isActive) {
                    ...
                    (
                        this._children[this._getFieldName(this._lastActiveInput)] as Field<
                            string,
        ...
        if (this._focusInputName) {
            const inputTarget = this._children[this._getFieldName(this._focusInputName)] as Field<
                string,
                ITextViewModelOptions
            >;
        ...
        if (!this._options.readOnly && this._drawInputs) {
            ...
            this._options.inputOrder.forEach((inputId: string) => {
                ...
                Input.updateInputValue(
                    this._children[this._getFieldName(inputModel.get('id'))] as HTMLInputElement,
    ...
    protected _getFieldName(name: string): string {
        return `ws-${name}_${this._fieldName}`;
    }
    ...
    protected _deactivatedContainerHandler(): void {
        ...
        if (this._options.trim) {
            this._inputs.each((input) => {
                if (input.isChanged()) {
                    if (!isChanged) {
                        isChanged = !input.getChanged().includes('marker');
                    }
                }
        ...
        if (this._stringInputsValue !== currentValue || isChanged) {
    ...
    protected _inputChangeHandler(event: SyntheticEvent<InputEvent>, inputRecord: Record): void {
        ...
        if (
            ...
            if (action) {
                const actionInput = this._children[this._getFieldName(action.input)] as Field<
                    string,
                    ITextViewModelOptions
                >;
    ...
    protected _inputPasteHandler(event: SyntheticEvent<ClipboardEvent>, inputRecord: Record): void {
        ...
        if (this._options.source) {
            ...
            if (pasteString && pasteString.split(' ').length > 1 && !this._blockInputPromise) {
                isDefaultUpdateValue = false;
    ...
    protected _keyPressHandler(event: SyntheticEvent<KeyboardEvent>, inputRecord: Record): void {
        ...
        if (action) {
            const input = this._children[this._getFieldName(action.input)] as Field<
                string,
                ITextViewModelOptions
            >;
