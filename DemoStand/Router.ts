...
export default class RootRouterDemo extends Control<IControlOptions> {
    ...
    reload(): void {
        ...
        // reloadDemo обновляет содержимое страницы, окна находятся выше. Очищаем окна вручную.
        requirejs('Controls/popup').Controller.getController()?.getPopupItems().clear();
    ...
    _beforeMount(options: IDemoRouter, context: object, receivedState: void | IDemoRouter): void {
        const _state = {
