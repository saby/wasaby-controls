...
class Index extends Control<IControlOptions> {
    ...
    protected _beforeMount(options: IDemoIndex, ctx, receivedState: any): void | Promise<void> {
        ...
        this._dataConfigs = {
            configs: {
                root: {
                    configGetter: 'DemoStand/ConfigGetter',
                    configGetterArguments: this._hasLoadConfig ? demoModule.getLoadConfig() : {},
                },
            },
            loadResults: {
                root: this._loadResult,
            },
        };
        ...
        this._settingsController = {
            getSettings(ids: string[]): Promise<object> {
                ...
                if (!storage[controlId]) {
                    ...
                    // Окна в качестве propStorageId указывают имя шаблона
                    // Чтобы не разоломать демки, не будем присваивать значение по умолчанию
...
export function getDataToRender(url: string, params: any, Router: IRouter): Promise<unknown> {
    ...
    if (demoModuleName) {
        return loadAsync(demoModuleName).then((demoModule: any) => {
            ...
            if (loadConfig) {
                return Loader.load(loadConfig, undefined, Router);
            }
        });
    }
}
