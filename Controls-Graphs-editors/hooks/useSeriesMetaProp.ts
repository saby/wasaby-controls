...
function useSeriesMetaProp<TValue extends ISeriesEntry = ISeriesEntry>(
    ...
    React.useEffect(() => {
        ...
        const newValues: TValue[] = frameFields.map((frameField, frameFieldIndex) => {
            ...
            if (colorSelectionEnabled) {
                newValue.colorIndex = getColorIndexForSerieEntry(value, frameFieldIndex);
            }
...
function getColorIndexForSerieEntry(values: ISeriesEntry[], valueIndex: number): number {
    if (values[valueIndex]?.colorIndex) {
        return values[valueIndex].colorIndex as number;
    }
    let newColorIndex = valueIndex + 1;
    const hasSameColorIndex = values.some((value) => value.colorIndex === valueIndex);
    if (hasSameColorIndex) {
        const unrepeatedIndex = Array.from({ length: MAX_COLOR_INDEX }).findIndex(
            (_, index) => !values.some((value) => value.colorIndex === index)
        );
        newColorIndex = unrepeatedIndex !== -1 ? unrepeatedIndex : valueIndex;
    }
