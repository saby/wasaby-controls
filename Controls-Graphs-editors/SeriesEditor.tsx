        ...
        if (!props.value?.map?.((item) => item.name)?.includes?.(item?.get?.('Title'))) {
            ...
            const newValues: TValue[] = [
                ...value,
                {
                    ...
                    name: item.get('Title'),
                    colorIndex: value?.length ? value[value.length - 1].colorIndex + 1 : 1,
                    xValueProperty,
                },
            ];
            ...
            {!!value.length
                ? value?.map?.((item) => (
                      <div className="tw-grid tw-grid-cols-3 controls-margin_top-s" key={item.name}>
                          <div className="controls-fontsize-xl ws-ellipsis" title={item.name}>
                              {item.name}
                          </div>
                          <Button
                          ...
                          <Icon
                              ...
                              className="tw-cursor-pointer tw-place-self-end controls-margin_left-m"
                              icon="icon-CloseNew"
                          />
                      </div>
                  ))
