...
function Editor(props: { LayoutComponent: ReactNode; onChange: Function }) {
    ...
    const handleChangeRoundChartType = useCallback(
        (key: string) => {
            setRoundChartType(key);
            onChange({ ...editorContext, type: key }, { multiple: true });
        },
        [editorContext]
    );
    const [size, setSize] = useState(() => {
        return editorContext.size ?? 'm';
    });
        ...
        <LayoutComponent title={null}>
            ...
            <div className="controlsGraphsEditors-roundChartDiagramEditor__editorsGrid">
                ...
                <Tumbler
                    ...
                    selectedKey={roundChartType}
                    onSelectedKeyChanged={
                        handleChangeRoundChartType as ISingleSelectableOptions['onSelectedKeyChanged']
                    }
                    keyProperty="id"
                />
                ...
                >
                    <Chips items={SIZE_VARIANTS_RS} keyProperty="id" />
                </SelectedKey>
            </div>
        </LayoutComponent>
    );
