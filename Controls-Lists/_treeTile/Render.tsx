...
import { CollectionProvider, ListContextProvider } from 'Controls/listsCommonLogic';
...
function Render(props: IRenderProps): JSX.Element {
    const { collection, needShowStub } = props;

    const className = 'tw-relative' + (needShowStub ? ' tw-h-full' : '');

    return (
        // div тут существует, т.к. рендер плитки старый и заточен под рождение в BaseControl.
        // После реализации React плитки, весь данный файл должен быть удален
        <div className={className}>
            <ListContextProvider actionHandlers={actionHandlers}>
