...
>(props: TViewCommandHandlers): TRenderEventHandlers {
    ...
    const handlers = React.useMemo<ITileRenderEventHandlers>(
        () => ({
            itemHandlers: base.itemHandlers,
            onItemClick: (item, e) => {
                props.onItemClick(e as unknown as React.MouseEvent, item);
            },
            onCheckBoxClick: (item, e) => {
                props.onCheckboxClick(e as unknown as React.MouseEvent, item.contents);
            },
        }),
        [base]
    );
