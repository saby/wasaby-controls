...
import { _ListScrollContext, IListScrollContextState, scrollToElement } from 'Controls/scroll';
...
import { IViewTriggerProps } from 'Controls/interface';
...
export interface IScrollControllerWrapperProps {
    ...
    resetScroll?: boolean;
    viewCommands: IAbstractListState['viewCommands'];
    isPhone: boolean;
}
...
function getScrollToItemPosition(
    key: TKey,
    state: IScrollControllerWrapperState,
    props: IScrollControllerWrapperProps
): 'top' | 'bottom' | null {
    const item = state.itemsSizes.find((item) => item.key === key) as ScrollControllerLib.IItemSize;
    const topEdge = state.placeholders.backward + state.scrollState.position;
    const bottomEdge = topEdge + props.listScrollContextState.getViewportRect().height;
    if (item?.offset < topEdge) {
        return 'top';
    }
    if (item?.offset + item?.size > bottomEdge) {
        return 'bottom';
    }
    return null;
}

function needToShiftRangeBeforeScrollToItem(
    index: number,
    ...
    const hasEnoughContent = ScrollControllerLib.CalculatorUtil.hasEnoughContentToScroll({
        range: state.range,
        viewportSize,
        itemIndex: index,
        itemsSizes: fillItemSizesByRange(state.itemsSizes, state.range, state.allItemsCount),
        itemPosition: position,
        calcByOffset: true,
    });
    const itemOffset = state.itemsSizes[index].offset - state.placeholders.backward;
    const backwardViewportBorderPosition = state.scrollState.position;
    const forwardViewportBorderPosition = backwardViewportBorderPosition + viewportSize;
    const itemInsideViewport =
        itemOffset >= backwardViewportBorderPosition && itemOffset <= forwardViewportBorderPosition;
    const allItemsIsDisplayed =
        state.range.endIndex - state.range.startIndex === state.allItemsCount;
    const isItemInRange = index >= state.range.startIndex && index < state.range.endIndex;
    return (
        (!isItemInRange || (!hasEnoughContent && (!itemInsideViewport || force))) &&
        !allItemsIsDisplayed
    );
}
...
function fillItemSizesByRange(
    ...
    const lastItem = itemsSizes[itemsSizes.length - 1];
    for (let i = range.startIndex; i < count; i++) {
        itemsSizes[i] = itemsSizes[i] || {
            key: '',
...
> {
    private _markTimeout: null | ReturnType<typeof setTimeout> = null;
    ...
    constructor(props: IScrollControllerWrapperProps) {
        ...
        if (props.apiRef.current) {
            props.apiRef.current.scrollToItem = (
                key: TItemKey,
                position?: 'top' | 'bottom',
                force?: boolean
            ) => this._scrollToItem(key, position, force);
        } else {
            props.apiRef.current = {
                scrollToItem: (key: TItemKey, position?: 'top' | 'bottom', force?: boolean) =>
                    this._scrollToItem(key, position, force),
            };
        }
    }
    ...
    private _scrollToItem(
        ...
        let range = this.state.disableVirtualScroll
            ? { startIndex: 0, endIndex: this.state.allItemsCount }
            : this.state.range;
        let placeholders = this.state.placeholders;

        if (needToShiftRangeBeforeScrollToItem(index, position, force, this.state, this.props)) {
            range = ScrollControllerLib.CalculatorUtil.getRangeByIndex({
                pageSize,
                start: startIndex,
                end: null,
                totalCount: this.state.allItemsCount,
            });
            placeholders = ScrollControllerLib.CalculatorUtil.getPlaceholdersByRange({
                range,
                totalCount: this.state.allItemsCount,
        ...
        const scroll = () => {
            const scrollPosition =
                ...
                (position === 'bottom'
                    ...
                    : 0);
            const itemContianer = this.state.itemsContainer?.querySelector(
                `.controls-ListView__itemV[item-key='${key}']`
            ) as HTMLElement;
            if (itemContianer) {
                scrollToElement(itemContianer, position, force);
            }
    ...
    scrollToPage(direction: ScrollControllerLib.IDirectionNew) {
        ...
        return this._scrollToItem(edgeItem.key, scrollPosition, true).then(() => {
            if (this._markTimeout) {
                clearTimeout(this._markTimeout);
            }

            // setTimeout потому что нужна асинхронная пауза, чтобы васаби-скролл обновил свое состояние
            // и можно было рассчитать крайний эелмент по новой фактической позиции
            this._markTimeout = setTimeout(() => {
                const edgeItemAfterScroll = getEdgeItem(
                    this.props.listScrollContextState,
                    this.props.collection,
                    this.state
                )?.key;

                if (edgeItemAfterScroll) {
                    this.props.viewModelAPI.mark(edgeItemAfterScroll);
                }
                this._markTimeout = null;
            });
        });
    }
    ...
    componentDidMount() {
        this.setState((prevState) => ({
            pagingVisible: this._needShowPaging({
                isPhone: this.props.isPhone,
                currentPagingVisibility: prevState.pagingVisible,
            }),
        }));
    }
    ...
    getSnapshotBeforeUpdate(
        ...
        if (
            ...
            !ScrollControllerInnerWrapper.needToResetScroll(this.props)
        ) {
            const edgeItem = getEdgeItem(
            ...
            if (edgeItem) {
                snapshot.edgeItem = edgeItem;
            }
        }
        return snapshot;
    }

    private _needShowPaging({
        isPhone,
        currentPagingVisibility,
    }: {
        isPhone: boolean;
        currentPagingVisibility: boolean;
    }): boolean {
        return !isPhone && this._needShowPagingByScrollSize(currentPagingVisibility);
    }
    ...
    static getDerivedStateFromProps(
        ...
        const resetScroll = ScrollControllerInnerWrapper.needToResetScroll(nextProps);

        const contextEvent = nextProps.listScrollContextState.contextEvent;

        if (contextEvent && !contextEvent.handled && contextEvent !== prevState.contextEvent) {
    ...
    static needToResetScroll(props: IScrollControllerWrapperProps): boolean {
        return props.resetScroll || props.viewCommands.includes('resetScroll');
    }
    ...
    componentDidUpdate(
        ...
        if (ScrollControllerInnerWrapper.needToResetScroll(this.props)) {
            this.props.listScrollContextState.scrollTo(0);
            this.props.viewModelAPI.onExecutedViewCommand('resetScroll');
        }
        ...
        if (snapshot && snapshot.edgeItem && !this.state.scrollCallback) {
            ...
            if (newEdgeItem && isNeedRestoreScroll(snapshot.edgeItem, newEdgeItem)) {
                restoreScroll(prevProps.listScrollContextState, snapshot.edgeItem, this.state);
            }
        }

        const pagingVisible = this._needShowPaging({
            isPhone: this.props.isPhone,
            currentPagingVisibility: prevState.pagingVisible,
        });
        ...
        if (this.state.contextEvent) {
            ...
            this.state.contextEvent.handled = true;
        }

        let newState: Partial<IScrollControllerWrapperState> | null = null;
        if (
            this.state.scrollCallback ||
            this.state.range.startIndex !== prevState.range.startIndex ||
            this.state.range.endIndex !== prevState.range.endIndex
        ) {
            ...
            if (
                !this.props.loading &&
                ScrollControllerInnerWrapper.checkTrigger(this.props, this.state, 'forward')
            ) {
            ...
            newState = { scrollCallback: null };
        }

        if (pagingVisible !== prevState.pagingVisible) {
            newState = newState || {};
            newState.pagingVisible = pagingVisible;
        }

        if (this.props.markedKey !== prevProps.markedKey) {
            setTimeout(() => {
                const scrollToItemPosition =
                    this.props.markedKey &&
                    getScrollToItemPosition(this.props.markedKey, this.state, this.props);
                if (scrollToItemPosition) {
                    this._scrollToItem(this.props.markedKey, scrollToItemPosition);
                }
            });
        }

        if (newState) {
            this.setState(newState as IScrollControllerWrapperState);
        }
    }

    componentWillUnmount(): void {
        if (this._markTimeout) {
            clearInterval(this._markTimeout);
            this._markTimeout = null;
        }
    }
...
export default function ScrollContainer(
    props: Omit<IScrollControllerWrapperProps, 'listScrollContextState' | 'isPhone'>
) {
    const listScrollContextState = React.useContext(
        _ListScrollContext
    ) as unknown as IListScrollContextState;
    const { device } = useAdaptiveMode();
    return (
