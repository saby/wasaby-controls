...
import type { dndCore } from 'Controls/listDragNDrop';
...
const getDndCore = async (
    cb: (core: (typeof import('Controls/listDragNDrop'))['dndCore']) => void
) => {
    const libPath = 'Controls/listDragNDrop';
    const core = (
        isLoaded(libPath)
            ? loadSync<typeof import('Controls/listDragNDrop')>(libPath)
            : await loadAsync<typeof import('Controls/listDragNDrop')>(libPath)
    ).dndCore;
    cb(core);
};
...
const EMPTY_FN = () => {};
const REGISTRAR_OBJ: { unmountCallback?: () => void } = {};
...
>(Component: React.FunctionComponent<TOuter & TRegistrarProvidedProps>) {
    function Composed(props: TOuter) {
        ...
        React.useEffect(() => {
            return () => {
                REGISTRAR_OBJ.unmountCallback?.();
                // Необходимо очищать unmountCallback, так как REGISTRAR_OBJ - глобальная переменная и она остается в памяти всегда
                delete REGISTRAR_OBJ.unmountCallback;
            };
        }, []);
        ...
        const register = useStableCallback(
            (eventName: string, handler: Function): void => {
                notify('register', [eventName, REGISTRAR_OBJ, handler, { id }], true);
            },
            [id, notify]
        );
        ...
        const unregister = useStableCallback(
            (eventName: string): void => {
                notify('unregister', [eventName, REGISTRAR_OBJ, { id }], true);
            },
            [id, notify]
        );
...
function mouseOrTouchEventExtractor(
    ...
    const nativeEvent = helpers.events.getNativeMouseEvent(e);

    return nativeEvent ? [nativeEvent] : undefined;
}
