 ...
 */
import { type CrudEntityKey, DataSet } from 'Types/source';
...
import { TSelectionRecord, TSelectionRecordContent } from 'Controls/interface';
 ...
 * @private
 */
const MARKER_DIRECTION_TO_EXTERNAL_DIRECTION: Record<TSingleAxisDirection, number> = {
    forward: ExternalDirection.FORWARD,
    backward: ExternalDirection.BACKWARD,
};

export class SourceController extends mixin<ObservableMixin>(ObservableMixin) {
    protected _filter: TListMobileSourceControllerParams['filter'];
    protected _pagination: TListMobileSourceControllerParams['pagination'];
    protected _root: TListMobileSourceControllerParams['root'];
    protected _source: TListMobileSourceControllerParams['source'];
    protected _handleId: string;
    protected _storageHandleId: string;
    protected _observerId: string;
    protected _pageId: string;
    protected _eventChannel: EventChannel;

    constructor({ source, ...options }: TListMobileSourceControllerParams) {
        super();
        this._source = source;
    ...
    getPagination(): TListMobileSourceControllerParams['pagination'] {
    ...
    protected async _initHandleId(): Promise<void> {
        ...
        this._pageId = KeepAliveControllerSingleton.getInstance().connect(this._observerId);

        const providerResult = await this._call({
            method: RpcCommandMethod.INSTANCE,
            params: {
                _PageId: this._pageId,
            },
        });
        const providerHandle = providerResult.getProperty();

        const storageResult = await this._call({
            method: RpcCommandMethod.GET_STORAGE,
            params: {
                _PageId: this._pageId,
        ...
        this._storageHandleId = storageResult.getProperty();
        await this.reInitCollection();
    }

    protected async reInitCollection(): Promise<void> {
        const collectionResult = await this._call({
            method: RpcCommandMethod.GET,
            params: {
                _PageId: this._pageId,
                _Handle: this._storageHandleId,
            },
        });
        const collectionHandleId = collectionResult.getProperty();

        await this.setObserverOnCollection(collectionHandleId);
    }

    protected async setObserverOnCollection(collectionHandleId: string) {
        this._handleId = collectionHandleId;
        await this._call({
            method: RpcCommandMethod.SET_OBSERVER,
            params: {
                _Handle: this._handleId,
    ...
    updateOptions({
        ...
        pagination,
        root,
    }: DeepPartial<TListMobileSourceControllerParams>): void {
    ...
    async connect(): Promise<void> {
        const eventHandler: IStompEventListener = (_: unknown, event: TRawStompEvent) => {
            if (event.get('obj_id') === this._observerId) {
     ...
     * @returns {Promise<void>}
     */
    async select(position: number, direction?: TSingleAxisDirection): Promise<void> {
    ...
    async setFilter(filter: Record<string, unknown>): Promise<void> {
        const result = await this._call({
            method: RpcCommandMethod.CHANGE_FILTER,
            params: {
                _Handle: this._storageHandleId,
                _PageId: this._pageId,
                Filter: buildRecord(filter),
            },
        });
        const newCollectionHandleId = result.getProperty();
        await this.setObserverOnCollection(newCollectionHandleId);
    }

    /**
     * Сменить корень через хлебные крошки
     * @public
     * @returns {Promise<DataSet>}
     */
    async move(root: SbisRecord | null): Promise<DataSet> {
        const result = await this._call({
            method: RpcCommandMethod.MOVE,
            params: {
                _Handle: this._storageHandleId,
                _PageId: this._pageId,
                Path: root ?? null,
        ...
        const newCollectionHandleId = result.getProperty();
        this.setObserverOnCollection(newCollectionHandleId);
        return result;
    }

    async showSelected(): Promise<DataSet> {
        const result = await this._call({
            method: RpcCommandMethod.SHOW_SELECTED,
            params: {
                _PageId: this._pageId,
                _Handle: this._storageHandleId,
            },
    ...
    /**
     * Сменить корень через проваливание в списке
     * @public
     * @returns {Promise<DataSet>}
     */
    async nextDisplay(root: CrudEntityKey | null): Promise<DataSet> {
        const result = await this._call({
            method: RpcCommandMethod.NEXT_DISPLAY,
            params: {
                _Handle: this._storageHandleId,
                _PageId: this._pageId,
                View: root ?? null,
    ...
    /**
     * Смена корня через кнопку назад в хлебных крошках
     * @public
     * @returns {Promise<DataSet>}
     */
    async prevDisplay(root: CrudEntityKey | null): Promise<DataSet> {
        const result = await this._call({
            method: RpcCommandMethod.PREV_DISPLAY,
            params: {
                _Handle: this._storageHandleId,
                _PageId: this._pageId,
                Folder: root ?? null,
