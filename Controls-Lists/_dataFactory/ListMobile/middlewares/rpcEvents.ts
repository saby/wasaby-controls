...
import { ListMobileActionsNames, TListMobileActions } from '../actions';
import {
...
import { eventRaisingMuteWrapper } from 'Controls-DataEnv/abstractList';
...
export const rpcEventsMiddleware: TListMobileMiddleware = (ctx) => (next) => async (action) => {
    ...
    const { virtualCollection, sourceController, getState, getCollection, applyState, setState } =
        ctx;
    const uiEvents = getUIEventsFromStomp(
        (action as TListMobileActions.rpcEvents.TReceiveEventGroupAction).payload.event
    );
        ...
        switch (event.type) {
            case StompEventType.Begin: {
                ...
                if (collection) {
                    ...
                    Logger.warn(
                        'Controls-Lists/dataFactory/ListMobile/middlewares:rpcEventsMiddleware::Пустая коллекция!'
                    );
                }
            ...
            case StompEventType.End: {
                ...
                const actions = virtualCollection.getActions();
                const itemsMute = eventRaisingMuteWrapper(ctx.getState().items);
                itemsMute.mute();
                for (const interactorAction of actions) {
                    await ctx.dispatch(interactorAction);
                }
                itemsMute.unmute();
                virtualCollection.resetActions();
...
const convertAtomicEvent = (e: TRawAtomicStompEvent): TUIEvent => {
    ...
    switch (type) {
        ...
        case StompEventType.OnSelect:
            ...
            const counterStatus = counter.get('status');
            args = {
                selected,
                size: counterStatus === ExternalSelectStatus.UNSET ? 0 : counter.get('size'),
                isAllSelected: counterStatus === ExternalSelectStatus.SET,
            };
            break;
        case StompEventType.End:
