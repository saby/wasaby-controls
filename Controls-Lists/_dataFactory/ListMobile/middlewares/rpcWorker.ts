...
export const rpcWorkerMiddleware: TListMobileMiddleware = ({
    ...
    const onCommandStarted = (_: unknown, command: TRpcCommand) =>
        scheduleDispatch(rpcWorker.sendCommandStart(command));
    const onCommandSucceeded = (_: unknown, command: TRpcCommand, response: unknown) =>
        scheduleDispatch(rpcWorker.sendCommandSuccess(command, response));
    const onCommandFailed = (_: unknown, command: TRpcCommand, error: Error) =>
        scheduleDispatch(rpcWorker.sendCommandFail(command, error));
    ...
    return (next) => async (action) => {
        switch (action.type) {
            case 'connect': {
                ...
                // Не ждем подключения.
                RpcFacade.connect()
                    .then(() => scheduleDispatch(rpcWorker.openConnectionSuccess()))
                    .catch((error) => scheduleDispatch(rpcWorker.openConnectionFail(error)));
