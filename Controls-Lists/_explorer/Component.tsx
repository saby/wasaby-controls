...
export function Component(props: IComponentProps) {
    const slice = useStrictSlice<AbstractListSlice>(props.storeId);

    if (!slice) {
        return null;
    }

    const InnerComponent = Components[slice.state.viewMode || 'table']();

    if (!InnerComponent) {
        return null;
    }

    return <InnerComponent {...props} changeRootByItemClick={true} />;
}
