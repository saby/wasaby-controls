...
interface UseDragDependencyProps {
    onAddEventDependency: (eventFrom: string, eventTo: string) => void;
}
        ...
        (event: MouseEvent) => {
            ...
            if (currentState.startEventKey) {
                ...
                if (eventCell) {
                    ...
                    const dataKeyValue = eventCell.getAttribute('data-key');

                    const eventFrom = buttonPosition === 'right' ? startEventKey : dataKeyValue;
                    const eventTo = buttonPosition === 'left' ? startEventKey : dataKeyValue;

                    onAddEventDependency(eventFrom, eventTo);
                }
            }
            setEventInDraggingProcess({
                startEventKey: null,
