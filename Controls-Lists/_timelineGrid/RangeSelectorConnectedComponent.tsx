...
interface IQuantumInfo {
    quantums: IQuantum[];
    quantumScale?: number;
    quantum: Quantum;
}

function getQuantumInfoFromSlice(slice: TimelineGridSlice, range: IRange): IQuantumInfo {
...
function getRangeChangedHandler(slice: TimelineGridSlice): TRangeChangedHandler {
    return (start: Date, end: Date) => {
        const { quantum } = getQuantumInfoFromSlice(slice, { start, end });
        const range: IRange = {
        ...
        if ([Quantum.Hour, Quantum.Minute, Quantum.Second].includes(quantum)) {
            range.needScroll = true;
        }
...
function getCaptionFormatter(slice: TimelineGridSlice): (start: Date, end: Date) => string {
    return (start, end) => {
        const { quantumScale, quantum } = getQuantumInfoFromSlice(slice, { start, end });
...
function getArrowClickHandler(
    ...
    return () => {
        const { quantum } = getQuantumInfoFromSlice(slice, slice.visibleRange);
...
function getAvailableRanges(quantums: IQuantum[]): object {
    ...
    if ((quantums?.length === 1 || (days && days.weekRangeAvailable === false)) && months) {
        return {
            months: [MONTHS_COUNT],
            quarters: [1],
            halfyears: [1],
            ...
            >
                {isPhone || showGoUpRangeButton ? (
                    <GoUpRangeButton
                        ...
                        weekRangeAvailable={getWeekRangeAvailability(slice.state.quantums)}
                    />
                ) : null}
