...
export interface ITimelineContextData {
    ...
    hasDragStartEventKey: () => string | null;
    ...
    implements IRangeSlice
{
    ...
    get shouldIgnoreMaxWidth(): boolean {
        return this.state.shouldIgnoreMaxWidth;
    }
    ...
    protected _getDynamicColumnsDataGeneratorParams(
        ...
        return {
            ...superParams,
            quantum,
            rangeLimit: state.columnsNavigation.sourceConfig.range,
            quantums: state.quantums,
        };
    }
    ...
    protected _prepareDynamicColumnsFilter(
        ...
        let customRanges;
        if (state.quantum === 'hour') {
            customRanges = getCustomRanges(state.quantums);
        }
    ...
    updateQuantsReplacementMap(nextQuantum: Quantum): TQuantsReplacementMap {
        ...
        if (areQuantsInSameRangeGroup(nextQuantum, Quantum.Hour)) {
            quantsReplacementMap[Quantum.Hour] = nextQuantum;
        } else if (areQuantsInSameRangeGroup(nextQuantum, Quantum.Day)) {
            quantsReplacementMap[Quantum.Day] = nextQuantum;
        } else if (areQuantsInSameRangeGroup(nextQuantum, Quantum.Month)) {
            quantsReplacementMap[Quantum.Month] = nextQuantum;
        }
    ...
    setQuantum(quantum: Quantum, direction: TScaleDirection): void {
        const quantsReplacementMap = this.updateQuantsReplacementMap(quantum);
        ...
        const shiftFactor =
            areQuantsInSameRangeGroup(quantum, this.state.quantum) && rangeSize > 1
                ? rangeSize - 1
                : 1;
        shiftDate(endDate, 'forward', quantum, shiftFactor);
        const range = {
            start: startDate,
            end: endDate,
        };
     ...
     * @param shouldIgnoreMaxWidth Флаг, определяющий, игнорировать ли maxWidth
     * @param nextQuantum Заранее рассчитанный квант
     * @see setQuantum
     * @see zoomIn
     * @see zoomOut
     */
    setRange(
        ...
        const quantum = nextQuantum || getQuantum(newRange, this.state.quantsReplacementMap);
        const quantsReplacementMap = this.updateQuantsReplacementMap(quantum);
        let endDate = newRange.end;
        this.setShouldIgnoreMaxWidth(shouldIgnoreMaxWidth);

        // Подстраиваем период под ширину столбцов и вьюпорта,
        if (!isQuantumLessThanDay(quantum) && this.state.availableRanges) {
            const maxRange =
                this.state.availableRanges[quantum.toLowerCase() + 's'][
                    this.state.availableRanges[quantum.toLowerCase() + 's'].length - 1
                ];
            ...
            if (endDate > maxEndDate) {
                endDate = maxEndDate;
                this.setShouldIgnoreMaxWidth(true);
            }
        }
        const range: IRange = {
        ...
        if (
            ...
            (this._loadedRange.start.getTime() === range.start.getTime() &&
                ...
                this.visibleRange.start.getTime() === range.start.getTime() &&
                this.visibleRange.end.getTime() === range.end.getTime() &&
    ...
    setShouldIgnoreMaxWidth(enable?: boolean): void {
        this.state.shouldIgnoreMaxWidth = enable || false;
    }
     ...
     */
    zoomOut(targetDate?: Date): void {
        this.zoom(targetDate, 'decrease', 'zoom');
    }

    /**
     * Расчитывает дату, к которой нужно зумить.
     * @param targetDate
     * @private
     */
    private _calcZoomTargetDate(targetDate?: Date): Date {
        let calculatedDate = targetDate && new Date(targetDate);
        if (!calculatedDate) {
            // в режиме часов/минут/30минут/15минут зумим к началу диапазона,
            // во всех остальных к текущей дате.
            calculatedDate = isQuantumLessThanDay(this.state.quantum)
                ? this.state.visibleRange.start
                : new Date();
        }
        // Если дата не попадает в отбражаемый диапазон, то зумим к началу диапазона
        if (
            calculatedDate.getTime() < this.state.visibleRange.start.getTime() ||
            calculatedDate.getTime() > this.state.visibleRange.end.getTime()
        ) {
            calculatedDate = this.state.visibleRange.start;
        }
    ...
    zoom(
        ...
        const {
            ...
            targetDate: this._calcZoomTargetDate(targetDate),
            scaleDirection,
            accessibility,
 ...
 * @remark
 * Значения содержат:
 * - quantum {@link Controls-Lists/timelineGrid:Quantum Quantum} Квант периода
