...
> extends DynamicGridSlice<TColumnsNavigationPosition, TColumnsGridData> {
    ...
    protected _getDynamicColumnsDataGeneratorParams(
        ...
        return { ...superParams, rangeLimit: state.columnsNavigation.sourceConfig.range };
    }
    ...
    private _processRangeByLoadedData(items, nextState) {
        ...
        if (quantumConfig && typeof quantumConfig.loadedRangeAdjustmentCallback === 'function') {
            const currentRange = {
                start: new Date(nextState.range.start),
                end: new Date(nextState.range.end),
            };
            const newRange = quantumConfig.loadedRangeAdjustmentCallback(items, currentRange);
    ...
    setQuantumScale(quantumScale: number, quantum: Quantum, direction: TScaleDirection): void {
        ...
        shiftDate(endDate, 'forward', quantum, rangeSize * quantumScale - 1);
        const range = {
            start: startDate,
            end: endDate,
        };
