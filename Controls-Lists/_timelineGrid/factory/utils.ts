...
export function getQuantsReplacementMap(quants?: IQuantum[]): TQuantsReplacementMap {
    ...
    if (quants) {
        const quantsGroups = getQuantsGroups();
        quantsGroups.forEach((group) => {
            const filteredQuants = quants
                .filter((quantum) => group.includes(quantum.name))
                .reverse();
            if (filteredQuants.length > 0) {
                const nextDefaultQuantum =
                    filteredQuants.find((quantum) => quantum.default) || filteredQuants[0];
                quantsReplacementMap[group[0]] = nextDefaultQuantum?.name || group[0];
            }
        });
    }
