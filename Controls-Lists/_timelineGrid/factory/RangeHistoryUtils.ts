...
function parseSavedRangeData(json: string): ISavedRangeData {
    ...
    const { saveTime, range, quantsReplacementMap, timeZone } = JSON.parse(json) as ISavedRangeData;
...
export const RangeHistoryUtils = {
    ...
    store(
        ...
        const timeZone = DateTime.getClientTimezoneOffset();
    ...
    restoreAll(key: string, quantums?: IQuantum[]): Promise<IRangeData> {
        ...
        return USER.load([key]).then(async (config) => {
            const { range, quantsReplacementMap, saveTime, needScroll, timeZone } =
                parseSavedRangeData(config.get(key));
            if (!range) {
                return getNullObj();
            }
            ...
            const currentTimeZone = DateTime.getClientTimezoneOffset();
            const correctedRange = {
                start: new Date(range.start),
                end: new Date(range.end),
            };
            if (currentTimeZone !== timeZone) {
                const tzOffset = timeZone - currentTimeZone;
            ...
            await RangeHistoryUtils.store(key, correctedRange, quantsReplacementMap, needScroll);
