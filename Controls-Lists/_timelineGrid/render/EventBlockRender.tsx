...
export interface IEventRenderProps extends ITagProps {
    ...
    tagTooltip?: string;
    /**
     * Настройка, позволяющая выровнять содержимое шаблона по высоте
     * @cfg
     * @default 'start'
     */
...
function getPointerEventsClassName({ interactionMode, caption, description }: IEventRenderProps) {
    let className = ' ControlsLists-timelineGrid__EventBlockRender_pointerEvents';
    ...
    if (interactionMode === 'none') {
        if (!caption && !description) {
            ...
            className += ' ControlsLists-timelineGrid__Event_non-interactive';
        }
    }
    return className;
}
...
function getEventBlockClassName(props: IEventRenderProps) {
    const { fontSize, fontColorStyle, align, overflow, offset, readOnly, interactionMode, valign } =
        props;
    let className =
        `tw-items-${valign} ` +
        'ControlsLists-timelineGrid__Event ControlsLists-timelineGrid__EventBlockRender ' +
        ...
        `ControlsLists-timelineGrid__EventBlockRender_pointerEvents_${
            interactionMode === 'block' || true ? 'auto' : 'none'
        } `;
    className += getPointerEventsClassName(props);
    return className;
}
...
export default function EventBlockRender(props: IEventRenderProps): React.ReactElement {
    ...
    const halign = fittingMode === 'icon' ? 'center' : props.align;
    const { quantum, range } = React.useContext(TimelineDataContext);

    const className = getEventBlockClassName({ ...props, align: halign });
            ...
            {fittingMode && (icon || caption || description) && (
                <span
                    ...
                    className={
                        ...
                        (fittingMode === 'icon'
                            ? 'ControlsLists-timelineGrid__EventBlockRender_icon_align '
                            : ' ')
                    }
                >
                    {icon && fittingMode ? (
                        <Icon
                            ...
                            iconSize={iconSize || 's'}
                            className={`js-ControlsLists-timelineGrid__Event_icon ${
                                fittingMode !== 'icon'
                                    ? 'ControlsLists-timelineGrid__EventBlockRender_icon '
                                    : ' '
                            }`}
                        />
                    ) : null}
