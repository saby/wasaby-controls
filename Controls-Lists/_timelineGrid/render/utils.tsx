...
import { IDragLineStyle, ILineStyle } from 'Controls-Lists/_timelineGrid/render/DragLineLayer';
...
export function drawArrow(
    ...
    g.beginFill(hexToNumber(arrowStyle?.color), arrowStyle?.opacity);
    g.lineStyle(1, hexToNumber(arrowStyle?.color), arrowStyle?.opacity);
...
/**
 * Функция для отрисовки простой линии
 * @param g
 * @param fromX
 * @param fromY
 * @param toX
...
export function drawLine(
    ...
    toX: number,
    toY: number,
    lineStyle: IDragLineStyle
) {
    const START_CIRCLE_RADIUS = 3;
    // Установка стиля линии
    g.beginFill(hexToNumber(lineStyle.color));
    g.lineStyle(1, hexToNumber(lineStyle.color));
    // Рисуем заполненный круг в начальной точке
    g.drawCircle(fromX, fromY, START_CIRCLE_RADIUS);

    g.endFill();
    // Рисуем прямую линию от начальной до конечной точки
    g.moveTo(fromX, fromY);
    g.lineTo(toX, toY);
}
