...
function DragLineLayer({ data }: ITimelineLayerComponentProps<IDragLineLayerProps>) {
    ...
    const { container, viewportWidth } = React.useContext(TimelineDataContext);
                ...
                if (startElement && firstDynamicRowRect && (mousePosition.x || mousePosition.y)) {
                    ...
                    const fromX =
                        buttonPosition === 'right'
                            ? startElementRect.right - firstDynamicRowRect.left - viewportWidth
                            : startElementRect.left - firstDynamicRowRect.left - viewportWidth;
                    return {
                        type: 'dragLine',
                        fromX,
                        fromY: startElementRect.top + startElementRect.height / 2 - containerTop,
                        toX: mousePosition.x - firstDynamicRowRect.left - viewportWidth,
    ...
    return (
        <TimelineCanvasRender<IDragLineDrawData>
            viewportWidth={viewportWidth + sumFixedColumnsWidth}
            drawData={drawData}
