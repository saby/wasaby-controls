...
/**
 * Варианты визуальных эффектов, применяемых к блоку события
 * @typedef TEventEffect
 * @variant fade-to-right Плавный градиент к прозрачному фону справа
 * @variant fade-to-left Плавный градиент к прозрачному фону слева
 * @variant fade-both Плавный градиент к прозрачному фону слева и справа
 * @variant none Без эффектов.
 */
export type TEventEffect = 'fade-to-right' | 'fade-to-left' | 'fade-both' | 'none';

/**
 * Интерфнейс события, к которому можно применить визуальный эффект
 * @public
 */
export interface IEventWithEffectProps {
    /**
     * Визуальный эффект, применяемый к блоку события.
     * @default 'none'
     * @cfg
     * @demo Controls-Lists-demo/timelineGrid/EventRender/WI/Squircle/Effect
     */
    effect?: TEventEffect;
}
 ...
 * @public
 */
export interface IEventRenderProps extends IEventWithEffectProps {
    /**
     * Передаваемые классы
     * @cfg
     */
...
/**
 * Задаёт CSS класс визуального эффекта, который применяется к фону события
 * @param backgroundColorStyle
 * @param effect
 */
function getBackgroundClassName(
    backgroundColorStyle: IEventRenderProps['backgroundColorStyle'],
    effect: IEventRenderProps['effect'] = 'none'
) {
    if (effect !== 'none') {
        return `ControlsLists-timelineGrid__EventBlockRender_bg_${backgroundColorStyle}_effect_${effect} `;
    }
...
export default function EventSquircleRender(props: IEventRenderProps): React.ReactElement {
    const {
        ...
        backgroundColorStyle = 'unaccented',
        caption,
        description,
        icon,
        ...
        >
            <div
                ...
                className={
                    ...
                    getBackgroundClassName(backgroundColorStyle, effect) +
            ...
            <span
                ...
                className={
                    ...
                    (fittingMode === 'icon'
                        ? 'ControlsLists-timelineGrid__EventSquircleRender_icon_align '
                        : ' ') +
                    (overflow !== 'visible' && fittingMode !== 'icon'
                        ? `ws-line-clamp ws-line-clamp_${maxLines} `
                        : '')
                }
            >
                {icon && fittingMode ? (
