...
import { getWeekRangeAvailability, IQuantum, Quantum } from 'Controls-Lists/_timelineGrid/utils';
import {
...
function TimeRender({ date, className }: { date: Date; className: string }): React.ReactElement {
...
function HeaderDateComponent(props: {
    ...
    const { quantum, dataDensity, renderValues, isNeedOpacity, weekRangeAvailable } = props;
    ...
    const content =
        quantum === Quantum.Minute || quantum === Quantum.Hour ? (
            <TimeRender date={date} className={contentClassName} />
        ) : (
            <DateRender
                date={date}
                quantum={quantum}
...
function SuperHeaderDateComponent(props: { quantum: Quantum; renderValues?: { date?: Date } }) {
    ...
    return (
        <div className={'ControlsLists-timelineGrid__headerCellContent_baseline'}>
            <div className={'tw-truncate tw-inline-flex'}>{caption}</div>
        </div>
    );
}
...
export function getPatchedDynamicHeader(
    ...
    let superHeaders;
    const weekRangeAvailable = getWeekRangeAvailability(quantums);
    if (isAdaptive) {
        superHeaders = [
            {
                key: 'Super',
