...
function DateRender({
    ...
    let startDateParts: [string, string] = ['', ''];
    ...
    // Разбиваем дату только если требуется разделение
    if (shouldSplit) {
        const formatted = formatDate(startDate, mask);
        startDateParts = formatted.split(' ') as [string, string];
    }
    ...
    const showEndDate = !shouldSplit && endDate && quantum === Quantum.Week;
        ...
        <div className={className}>
            {shouldSplit ? (
                <div>
                    ...
                    <span className={'controls-text-unaccented'}>{startDateParts[1]}</span>
                </div>
            ) : (
                <span
                    className={`ControlsLists-timelineGrid__headerCellContent_date controls-text-${fontColorStyle}`}
