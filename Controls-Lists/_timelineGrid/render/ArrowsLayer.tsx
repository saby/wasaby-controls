...
function ArrowsLayer({ data }: ITimelineLayerComponentProps<IArrow>) {
        ...
        const calculatedData = data
            .map(({ from, to, style }) => {
                ...
                if (elementOne && elementTwo && firstDynamicRowRect) {
                    ...
                    return {
                        type: 'arrow',
                        fromX: rectOne.right - firstDynamicRowRect.left - viewportWidth,
                        fromY: rectOne.top + rectOne.height / 2 - containerTop,
                        toX: rectTwo.left - firstDynamicRowRect.left - viewportWidth,
    ...
    }, [range, data, dragStartEventKey, columnWidth, containerHeight, itemsVersion, viewportWidth]);
