...
function ArrowsLayer({ data }: ITimelineLayerComponentProps<IArrow>) {
    ...
    const { containerRef, range, hasDragStartEventKey } = React.useContext(TimelineDataContext);
            ...
            .map(({ from, to, style }): IArrowDrawData => {
                ...
                if (elementOne && elementTwo && firstDynamicRowRect) {
                    ...
                    return {
                        ...
                        style: { ...style, opacity: arrowFullOpacity ? 1 : OTHER_ARROWS_OPACITY },
                    };
                }
                return null;
            })
            .filter(Boolean);
    }, [data, containerRef.current, version, dragStartEventKey]);
