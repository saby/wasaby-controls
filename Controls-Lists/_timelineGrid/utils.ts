...
// Кол-во дней при достижении, которых по колонкам отображаются месяца
const DAYS_COUNT_LIMIT = 62;

export function getQuantum(range: IRange): Quantum {
    // Метод проверяет и совпадение года
    const isMonthsEqual = BaseDateUtils.isMonthsEqual(range.start, range.end);
    const isDaysEqual = range.start.getDate() === range.end.getDate();
    if (isMonthsEqual && isDaysEqual) {
        return Quantum.Hour;
    }
 ...
 * В зависимости от квантума меняется единица измерения размера диапазона.
 * @param range
 * @param quantum
 */
