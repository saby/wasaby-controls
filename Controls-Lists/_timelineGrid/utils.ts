...
function updateRange(params: IUpdateRange): IRange {
    const { range, quantums, quantumScale, nextViewportWidth, currentViewportWidth } = params;
    const quantum = getQuantum(range, quantums, quantumScale);
    const rangeSize = getRangeSize(range, quantum);
    if (quantum !== 'month' || rangeSize < MONTHS_COUNT) {
        const dynamicColumnWidth = currentViewportWidth / rangeSize;
        const newRangeSize = Math.floor(nextViewportWidth / dynamicColumnWidth);
