...
function useColumnsScrollPositionHandler({
    ...
    const onPositionChanged = React.useCallback((newPosition) => {
        ...
        if (mousePressedRef.current) {
            // Если потащили скролл, отменяем загрузку.
    ...
    // 3. При скролле на целый диапазон драг скроллом, колонки сдвинутся,
    //    скролл восстановится и position останется старым.
    //    Поэтому, смотрим на версию колонок, чтобы пересчитать visibleRange
    const visibleRange = React.useMemo(() => {
        return calcVisibleRange(position, columnWidth, columnsSpacing, visibleRangeSize);
    }, [position, columnsSpacing, slice.state.columnsDataVersion]);
...
function getActivityScrollPosition(
    ...
    const columnsCountBeforeActivity = slice.dynamicColumnsGridData.findIndex(
        (it) => it.getHours() === hours && it.getDate() === startDate.getDate()
    );
...
function useScrollToRangeStart(
    ...
    React.useLayoutEffect(() => {
        ...
        // TODO: Временно отображаем 24 часа, подскролл к активности не нужен
        // if (slice.range.needScroll) {
        //     scrollToActivityStart();
        // } else {
        scrollToRangeStartUtil();
        // }
    }, [loadedRange, columnWidth]);
