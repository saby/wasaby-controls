...
function useColumnsScrollPositionHandler({
    ...
    const scrollPositionOnCancel = React.useRef(-1);
    ...
    const onPositionChanged = React.useCallback((newPosition) => {
        ...
        if (mousePressedRef.current) {
            if (slice.state.sourceController.isLoading()) {
                scrollPositionOnCancel.current = newPosition;
    ...
    React.useEffect(() => {
        const _onMouseUp = () => {
            ...
            setPositionByMouseUpTimeoutIDRef.current = setTimeout(() => {
                ...
                if (scrollPositionOnCancel.current === scrollPositionRef.current) {
                    slice.reload();
                    scrollPositionOnCancel.current = -1;
                }
...
function useScrollToRangeStart(
    ...
    React.useLayoutEffect(() => {
        ...
        scrollToRangeStartUtil();
    }, [viewportWidth, slice.root, hasCheckboxColumn, slice]);
}
