...
import { ADVANCED_DATA_COLUMN_WIDTH, END_DAY_HOUR } from 'Controls-Lists/_timelineGrid/constants';
...
export {
    ...
    TFooterPosition,
    IEventWithEffectProps,
} from './_timelineGrid/render/EventSquircleRender';
import { default as TimelineTag } from 'Controls-Lists/_timelineGrid/render/TimelineTag';
...
export { IEventRenderProps } from 'Controls-Lists/dynamicGrid';
...
function getLeftOffset(
    ...
    const index = datesArray.findIndex((elem: Date) => {
        ...
        return datesEqualByQuantum(currentDate, elem, quantum);
    });
    if (index !== -1) {
        const positionInDay = getPositionInPeriod(currentDate, quantum);
        return index + positionInDay;
    } else {
        return undefined;
    }
}
 ...
 * @param quantum {Quantum} Режим отображения
 * @param fixedTimelineDate {Date} Фиксированная дата для линии дня
 * @param wrapperStyle {React.CSSProperties} Стили обертки TimeMarker
 * @param isAggregationVisible {Boolean} Видимость колонки итогов
 * @param timeMarkerLineStyle {React.CSSProperties} Стили линии TimeMarker
 */
function getTimeMarkerComponent(
    ...
    timeMarkerLineStyle?: React.CSSProperties
) {
    const leftOffset = getLeftOffset(datesArray, quantum, fixedTimelineDate);
    const spacing = getOffsetStyle(spaceSize);
    if (leftOffset) {
        const lineLeftOffset = {
            left: `calc((${columnWidth}px + ${spacing}) * ${leftOffset}`,
        };

        return (
            <div style={{ ...wrapperStyle, zIndex: 4 }}>
                <TimeMarker
                    style={{ ...wrapperStyle, ...lineLeftOffset }}
                    lineStyle={timeMarkerLineStyle}
                />
            </div>
        );
    }
...
function ConnectedComponentRef(
    ...
    const slice = React.useContext(DataContext)[props.storeId] as unknown as TimelineGridSlice &
        ITimelineGridSliceState;
    const dynamicMinWidthHourQuantumRef = React.useRef(props.dynamicColumnMinWidths?.hour);
    ...
    const gridContainer = containerRef.current ? getDimensions(containerRef.current) : null;

    React.useLayoutEffect(() => {
        if (gridContainer && gridContainer.height) {
            setContainerHeight(gridContainer.height);
        }
    ...
    // Для часового представления необходимо вмещать столько колонок, сколько помещается, при изменении минимальной ширины колонки
    React.useEffect(() => {
        if (
            dynamicMinWidthHourQuantumRef.current &&
            props.dynamicColumnMinWidths?.hour < dynamicMinWidthHourQuantumRef.current &&
            quantum === Quantum.Hour
        ) {
            const endDate = new Date(slice.range.start.getTime());
            endDate.setHours(END_DAY_HOUR);
            slice.setRange({
                start: slice.state.range.start,
                end: endDate,
            });
        }
        dynamicMinWidthHourQuantumRef.current = props.dynamicColumnMinWidths?.hour;
    }, [props.dynamicColumnMinWidths?.hour]);

    // Если находимся в часовом представлении и включили кастомные диапазоны - нужно включить возможность ограничения скролла, если убрали кастомные диапазоны - выключить
    React.useEffect(() => {
        if (quantum !== Quantum.Hour) return;

        const shouldIgnore = !customRanges;
        slice.setShouldIgnoreMaxWidth(shouldIgnore);
    }, [customRanges]);

    const { columnWidth, rangeSize, dynamicColumnsGridData } = React.useMemo(() => {
        const initialData = slice.dynamicColumnsGridData;
        const { start, end } = slice.range;

        // Фильтрация данных
        let filteredData = initialData;
        let currentRangeSize = getRangeSize(slice.range, quantum, customRanges);

        // Фильтрация по выходным
        if (slice.filterHolidays && quantum === Quantum.Day) {
            filteredData = initialData.filter(weekendFilter);
            currentRangeSize = filteredData.filter((date) => date >= start && date <= end).length;
        }

        // Проверка на минимальное количество дней
        const visibleDates = filteredData.filter((date) => date >= start && date <= end);
        if (visibleDates.length < 2) {
            filteredData = initialData;
            currentRangeSize = getRangeSize(slice.range, quantum, customRanges);
        }

        // Расчет границ ширины
        ...
        const isHourQuantum = quantum === Quantum.Hour;
        const hasCustomRanges = getCustomRanges(slice.state.quantums);
        const shouldLimitWidth = !isHourQuantum || (isHourQuantum && hasCustomRanges);

        const maxWidth =
            shouldLimitWidth &&
            !slice.shouldIgnoreMaxWidth &&
            props.limitRangeIfFits &&
            getMaxColumnWidth(slice.dynamicColumn, props.dynamicColumnMaxWidths, quantum);

        // Расчет ширины колонки
        const multiSelectVisible =
            slice.multiSelectVisibility !== 'hidden' && slice.multiSelectPosition !== 'custom';

        const { width: calculatedWidth, rangeSize: finalRangeSize } = getDynamicColumnWidth(
            viewportWidth,
            currentRangeSize,
            slice.dynamicColumn,
            columnGapSize,
        ...
        // Огранчение скролла, если есть максимальная ширина
        if (maxWidth && calculatedWidth === maxWidth) {
            const startDate = new Date(start);
            const endDate = new Date(end);
            if (customRanges && quantum === Quantum.Hour) {
                startDate.setHours(customRanges[0].start);
                endDate.setHours(customRanges[customRanges.length - 1].start);
            }
            filteredData = findStrictRange({
                dynamicColumnsGridData: filteredData,
                startDate,
                endDate,
            });
        }
        ...
        return {
            ...
            rangeSize: finalRangeSize || currentRangeSize,
            dynamicColumnsGridData: filteredData,
        };
    }, [
    ...
    const onEventResized = React.useCallback(
        (item, event) => {
            ...
            if (shouldUpdateSource) {
                const eventsProperty = slice.eventsProperty || props.eventsProperty;
                const events = item.get(eventsProperty);
                events.getRecordById(event.getKey()).setRawData(event.getRawData());
    ...
    const itemsVersion = slice.items?.getVersion();
            ...
            if (props.verticalSeparatorsMode === 'line') {
                if (dynamicColumn.subColumns) {
                    dynamicColumnProps.className +=
                        ' ControlsLists-timelineGrid__verticalSeparatorsMode_sub_columns_lines';
                } else {
                    dynamicColumnProps.className +=
    ...
    const beforeHeader = React.useMemo(() => {
    ...
    const onRangeChange = React.useCallback(
        (newRange: IRange, calledByScroll?: boolean) => {
            if (
                slice.range.end.getTime() !== newRange.end.getTime() ||
                slice.range.start.getTime() !== newRange.start.getTime() ||
                slice.visibleRange.end.getTime() !== newRange.end.getTime() ||
                slice.visibleRange.start.getTime() !== newRange.start.getTime()
            ) {
                slice.setRange(newRange, false, calledByScroll);
            }
        },
        [slice]
    );
                ...
                >
                    ...
                    >
                        <DynamicGridComponent
                            ...
                            needStrictRange={props.limitRangeIfFits}
                            ...
                            contextMenuVisibility={props.contextMenuVisibility}
                            ...
                            headerCellsMultiSelectVisibility={
                                slice.headerCellsMultiSelectVisibility
                            }
                            headerCellsMultiSelectAccessibilityCallback={
                                slice.headerCellsMultiSelectAccessibilityCallback
                            }
                    ...
                    {eventInDraggingProcess.startEventKey && (
                        <div
                            className={
                                ' ControlsLists-timelineGrid__dependency_dragging_area_cursor'
                            }
                        />
                    )}
...
interface IPrepareDynamicColumnParams {
    ...
    onTagHover?: (event: React.MouseEvent, item: Model<any>, date: Date) => void;
    onTagClick?: (event: React.MouseEvent, item: Model<any>, date: Date) => void;
}
...
export function prepareDynamicColumn(
    ...
    return {
        ...
        getCellProps: (item, date) => {
            ...
            const onTagClickCallback = (event: React.MouseEvent) => {
                params?.onTagClick?.(event, item, date);
            };
            const onTagHoverCallback = (event: React.MouseEvent) => {
                params?.onTagHover?.(event, item, date);
            };
            return {
                ...
                tagRender: superResult.tagStyle ? (
                    <TimelineTag
                        tagStyle={superResult.tagStyle}
                        className={superResult.tagClassName}
                        onClick={onTagClickCallback}
                        onMouseEnter={onTagHoverCallback}
                    />
                ) : null,
