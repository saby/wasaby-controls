...
import {
    ...
    getColumnGapSize,
    getPositionInPeriod,
    IDynamicHeaderCellsColspanCallback,
    TColumnDataDensity,
...
export { default as EventSquircleRender } from './_timelineGrid/render/EventSquircleRender';
...
/*
 * Автоматический расчёт видимости горизонтальной рамки вокруг записей.
 */
function getBorderVisibility(props): string {
    // Для работы "прицела-перекрестия"
    if (props.hoverMode === 'cross') {
        return 'onhover';
    }
    // Для работы режима горизонтальных разделителей "Сплошная линия"
    if (props.horizontalSeparatorsMode === 'line') {
        return 'visible';
    }
    return 'hidden';
}
...
export function TimelineGridConnectedComponent(
    ...
    const staticColumns = React.useMemo(
        () => prepareStaticColumns(slice.staticColumns, props.hoverMode),
    ...
    const getRowProps = React.useCallback(
        (item: Model): IRowProps => {
            ...
            return {
                ...
                padding: {
                    top: props.horizontalSeparatorsMode === 'gap' ? '3xs' : null,
                    bottom: props.horizontalSeparatorsMode === 'gap' ? '3xs' : null,
                },
                ...userRowProps,
                borderVisibility: getBorderVisibility(props),
            };
        },
        [props.getRowProps]
    );
    ...
    const dynamicHeaderCellsColspanCallback = React.useCallback<IDynamicHeaderCellsColspanCallback>(
        props.autoColspanHeaders
            ? getDynamicHeaderCellsColspanCallback(columnWidth, dynamicColumn)
            : () => 1,
        [columnWidth, dynamicColumn, props.autoColspanHeaders]
    );
...
function prepareStaticColumns(staticColumns: IColumnConfig[], hoverMode: THoverMode): IColumnConfig[] {
    return staticColumns.map((staticColumn) => {
        return {
            ...
            getCellProps: (item) => {
                ...
                return {
                    className:
                        `ControlsLists-dynamicGrid__staticCellsWrapper_${hoverMode} ` +
