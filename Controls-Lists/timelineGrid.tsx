...
function ConnectedComponentRef(
    ...
    const dynamicColumnsGridData = React.useMemo(() => {
        let filteredData = slice.dynamicColumnsGridData;

        // 1. Фильтрация по выходным
        if (slice.filterHolidays && quantum === 'day') {
            filteredData = filteredData.filter(weekendFilter);
        }

        // 2. Проверка на минимальное количество дней
        const dateRangeFilteredData = filteredData.filter(
            (date) => date >= slice.range.start && date <= slice.range.end
        );
        if (dateRangeFilteredData.length < 2) {
            filteredData = slice.dynamicColumnsGridData; // Возвращаем исходные данные, если дней меньше 2
        }

        // 3. Ограничение скролла
        if (maxWidth === columnWidth) {
            filteredData = findStrictRange({
                dynamicColumnsGridData: filteredData,
                startDate: slice.range.start,
                endDate: slice.range.end,
            });
        }
