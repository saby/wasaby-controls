 ...
 */
export function useDynamicEditCell(columnIndex: number): {
    ...
    const callback = React.useCallback(() => {
        if (
            item.getEditingColumnIndex() === columnIndex ||
            previousEditColumnIndex.current === columnIndex
        ) {
    ...
    return { editingColumnIndex: item.getEditingColumnIndex(), editing: item.isEditing() };
}
