    ...
    if (columnsCountBeforeRange === -1 && activeDynamicColumn) {
        ...
        if (columnsCountBeforeRange !== -1 && availableViewportWidth) {
            ...
            if (dynamicColumnsWidth - activeElementPosition < availableViewportWidth) {
                if (dynamicColumnsWidth - availableViewportWidth >= 0) {
...
function getRangeByVisibleRange(
    ...
    switch (quantum) {
        case Quantum.Week:
            end = BaseDateUtils.getEndOfWeek(end);
            break;
        case Quantum.Month:
            end = BaseDateUtils.getEndOfMonth(end);
            break;
        case Quantum.Quarter:
            end = BaseDateUtils.getEndOfQuarter(end);
            break;
        case Quantum.HalfYear:
            end = BaseDateUtils.getEndOfHalfyear(end);
            break;
        case Quantum.Year:
            end = BaseDateUtils.getEndOfYear(end);
            break;
    }
...
function useScrollToRangeStart(
    ...
    const activeElementPosition = React.useMemo(
        () => dynamicColumnsGridData.indexOf(activeDynamicColumn) ?? null,
        [activeDynamicColumn, dynamicColumnsGridData]
    );
...
function useColumnsScrollPositionHandler({
    ...
    }, [slice.viewMode, dynamicColumnsGridData.length]);
...
function useColumnScrollObserveAndRecovery(props: IDynamicGridComponentProps) {
    ...
    useScrollToRangeStart(
        ...
        props.viewportWidth,
        parseFloat(columnWidth) || calculatedWidth,
    ...
    const { visibleRange, initialColumnsPosition, onColumnsPositionChanged } =
        useColumnsScrollPositionHandler({
            ...
            columnWidth: parseFloat(columnWidth) || calculatedWidth,
    ...
    React.useEffect(() => {
        if (!props.isAdaptive || !slice.getLoadingRangeByViewportWidthChanged?.()) {
            ...
            const newActiveDynamicColumn = props.dynamicColumnsGridData[visibleRange.startIndex];
            props.onActiveDynamicColumnChanged?.(newActiveDynamicColumn);
            if (newRange.start && newRange.end) {
                props.onRangeChange?.(newRange, true);
            }
        }
    }, [visibleRange]);
