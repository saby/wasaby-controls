...
export function updateHasSiblingForRowSelectionMutable(
    selection: TRowSelection,
    ...
    Object.keys(hasSibling).forEach((key) => {
        if (
            selection[key].hasBorderTop !== !hasSibling[key].up ||
            selection[key].hasBorderBottom !== !hasSibling[key].down
        ) {
            selection[key].hasBorderTop = !hasSibling[key].up;
            selection[key].hasBorderBottom = !hasSibling[key].down;
            isChanged = true;
