...
import { IBorderRadius } from '../components/SelectionHighlight';
...
interface IDragSelectionControllerProps {
    ...
    headerColumnSelectorMask: TSelectorMask;
...
class DragSelectionController {
    ...
    manageStart(_mouseEvent: React.MouseEvent, selectionBounds: TSelectionBounds): void {
        this._startKeyPairs = {
            topLeft: {
        ...
        // Корректируем высоту рамки выделения на высоту вьюпорта.
        if (this._startKeyPairs.topLeft.itemKey === 'header') {
            const viewPortRect = getViewportBoundingClientRect(this._props.containerRef.current);
            this._startCell.height = viewPortRect.bottom;
        }
    ...
    private _getPlainSelection(): ISelection {
        ...
        if (topLeft.itemKey !== 'header') {
            startItemIndex = items.findIndex(
                (i) => i.getAttribute(this._props.itemKeyAttribute) === `${topLeft.itemKey}`
            );
            endItemIndex = items.findIndex(
        ...
        for (let i = startItemIndex; i <= endItemIndex; i++) {
            ...
            const cellKeys = Array.from(
                item.querySelectorAll(
                    `${this._props.columnSelector}, ${this._props.headerColumnSelector}`
                )
            ).map((cell, index) => {
    ...
    private _getCellContainerByKey(itemKey: TItemKey, columnKey: TColumnKey): HTMLDivElement {
        let selector = '';
        if (itemKey === 'header') {
            selector = this._props.headerColumnSelectorMask.replace(
                KEY_MASK_TOKEN,
        ...
        return this._props.containerRef.current.querySelector(selector);
    }

    private _getLastItemCell() {
        const selector = `${this._props.itemSelector}:last-child ${this._props.columnSelector}`;
        return this._props.containerRef.current.querySelector(selector);
    }
    ...
    private _prepareCSSSelector(selector: string): string {
        return selector[0] === '.' ? selector.slice(1) : selector;
    }

    private _getCellContainerByTarget(target: Element): HTMLDivElement {
        const dataCellSelector = this._prepareCSSSelector(this._props.columnSelector);
        const headerCellSelector = this._prepareCSSSelector(this._props.headerColumnSelector);
        ...
        const isDesiredTarget =
            className.indexOf(dataCellSelector) !== -1 ||
            className.indexOf(headerCellSelector) !== -1;

        return (isDesiredTarget
            ? target
            : target.closest &&
              target.closest(
                  `.${dataCellSelector}, .${headerCellSelector}`
