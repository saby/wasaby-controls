...
import {
    ...
    THeaderCellsMultiSelectAccessibilityCallback,
...
export interface IGridSelectionContextProviderProps {
    ...
    headerCellsMultiSelectVisibility?: TCellsMultiSelectVisibility;
    headerCellsMultiSelectAccessibilityCallback?: THeaderCellsMultiSelectAccessibilityCallback;
...
interface INotifyBeforeSelectionChangedNotifier {
    oldSelectionGetter: () => ISelection;
    newSelectionGetter: () => ISelection;
}

function headerMultiSelectDefaultAccessibilityCallback() {
...
export function GridSelectionContextProvider(
    ...
    const notifyBeforeSelectionChanged = useHandler(
        React.useCallback(
            (config: INotifyBeforeSelectionChangedNotifier) => {
                return props.onBeforeSelectionChange
                    ? props.onBeforeSelectionChange(
                          config.oldSelectionGetter(),
                          config.newSelectionGetter()
                      )
                    : config.newSelectionGetter();
            },
            [props.onBeforeSelectionChange]
    ...
    const headerCellsMultiSelectAccessibilityCallback =
        React.useMemo<THeaderCellsMultiSelectAccessibilityCallback>(() => {
            return (
                props.headerCellsMultiSelectAccessibilityCallback ||
                headerMultiSelectDefaultAccessibilityCallback
            );
        }, [props.headerCellsMultiSelectAccessibilityCallback]);
    ...
    const handleSelection = React.useCallback<IGridSelectionContext['handleSelection']>(
        (itemKey, columnKey) => {
            const newSelection = notifyBeforeSelectionChanged({
                oldSelectionGetter: () => SelectionModel.clone(selectionModel.getSelection()),
                newSelectionGetter: () =>
    ...
    const changeSelection = React.useCallback<IGridSelectionContext['changeSelection']>(
        (oldPartialPlainSelection, newPartialPlainSelection) => {
            const callHeaderCallback = Object.keys(oldPartialPlainSelection)?.[0] === 'header';
            const newSelection = notifyBeforeSelectionChanged({
                oldSelectionGetter: () => SelectionModel.clone(selectionModel.getSelection()),
                newSelectionGetter: () =>
                    SelectionModel.mergeSelections(
                        ...
                        SelectionModel.filterSelections(
                            ...
                            (itemKey, columnKey) => {
                                if (callHeaderCallback) {
                                    return (
                                        headerCellsMultiSelectAccessibilityCallback(columnKey) ===
    ...
    const contextValue = React.useMemo<IGridSelectionContext>(() => {
        const value: IGridSelectionContext = {
            ...
            isRowSelectionEnabled: props.multiSelectVisibility !== 'hidden',
            isHeaderSelectionEnabled: props.headerCellsMultiSelectVisibility === 'onhover',
