...
class SelectionModel {
    ...
    getBoundingSelectionKeys(itemKey: TItemKey, columnKey: TColumnKey): TSelectionBounds {
        const startColumnKey = columnKey;
        const endColumnKey = columnKey;
        const startItemKey = itemKey;
        const endItemKey = itemKey;
        const plainSelection: TSelectionMap = {};
        plainSelection[itemKey] = [columnKey];
    ...
    /*
     * Метод вызывается, когда необходимо обновить границы выдлеления.
     * Строки упорядочиваются по ключу.
     * Если В ключах находится значение="header", то оно будет перемещено наверх, чтобы правильно обновлять видимость границ
     */
    private _changeSelection(oldSelection: TSelectionMap, newSelection: TSelectionMap): void {
        ...
        // Перемещаем header в самое начало, чтобы он первым сравнивался в _changeSelection
        if (addedItemsKeys[addedItemsKeys.length - 1] === 'header') {
            addedItemsKeys.pop();
            addedItemsKeys.unshift('header');
        }
    ...
    /*
     * Метод определяет по выделенным записям параметры для отрисовки границ выделения
     */
    private _updateSibling(itemKey: TItemKey): void {
        const item = this._getCollection().getItemBySourceKey(itemKey);
        // Первой после заголовка идёт первая запись в реестре
        const firstDataItem = this._getCollection().getFirst();
        const prevItem = item && this._getCollection().getPrevious(item);
        const nextItem =
            itemKey === 'header' ? firstDataItem : item && this._getCollection().getNext(item);
        // Если запись первая в реестре, то предыдущая это заголовок
        const prevItemKey = prevItem ? prevItem.key : firstDataItem === item ? 'header' : undefined;
    ...
    static mergeSelections(
        ...
        const resultSelection = SelectionModel.clone(selection);
        let shouldUnselect = true;
        const itemsKeys = Object.keys(partialSelection);
        for (let i = 0; i < itemsKeys.length; i++) {
            const itemKey = itemsKeys[i];
            const columnsKeys = partialSelection[itemKey];
            for (let j = 0; j < columnsKeys.length; j++) {
                const columnKey = columnsKeys[j];
                if (
                    !resultSelection[itemKey] ||
                    resultSelection[itemKey].indexOf(columnKey) === -1
                ) {
