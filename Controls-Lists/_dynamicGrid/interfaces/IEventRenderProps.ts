...
import { IRowspanProps } from 'Controls/gridRender';
...
export interface IDependencyButtonsConfig {
    /**
     * Показывать кнопки слева/справа
     */
 ...
 * Если указаны {@link Controls-Lists/_dynamicGrid/interfaces/IEventRenderProps/ICustomEventRenderProps#startRow startRow} и {Controls-Lists/_dynamicGrid/interfaces/IEventRenderProps/ICustomEventRenderProps#endRow endRow}, может сложиться ситуация, когда одно или несколько событий рендерятся ниже первой строки сетки с динамически генерируемыми ячейками. Так как сетка настроена не статически (мы не знаем число строк, которое может быть заполнено событиями, а CSS не умеет заполнять их автоматически, неоюходимо вычислять число строк и передавать их в параметре {@link Controls-Lists/_dynamicGrid/interfaces/IDynamicGridComponent/IDynamicCellProps#maxRow maxRow} объекта, который возвращается из коллбека {@link Controls-Lists/_dynamicGrid/interfaces/IDynamicGridComponent/IDynamicColumnConfig#getCellProps getCellProps} для динамической ячейки.
...
export interface ICustomEventRenderProps extends IRowspanProps {
    ...
    /**
     * Видимость ресайза события
     */
    resizerVisible?: boolean;
    /**
     * Конфиг кнопок добавления зависимостей
     */
    dependencyButtonsConfig?: IDependencyButtonsConfig;
}
