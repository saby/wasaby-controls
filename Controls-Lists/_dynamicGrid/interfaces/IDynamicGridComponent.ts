...
/**
 * Интерфейс свойств, возвращаемых для динамически создаваемой ячейки.
 * @interface Controls-Lists/_dynamicGrid/interfaces/IDynamicGridComponent/IDynamicCellProps
 * @public
 */
export interface IDynamicCellProps extends ICellProps {
    /**
     * Максимальный номер строки, до которого растягивается ячейка в координатах сетки динамически создаваемых ячеек.
     * Если в {@link Controls-Lists/_timelineGrid/interface/ITimelineGridConnectedComponentProps#getEventRenderProps параметрах для рендера событий} были указаны {@link Controls-Lists/_dynamicGrid/interfaces/IEventRenderProps/ICustomEventRenderProps#startRow startRow} и {Controls-Lists/_dynamicGrid/interfaces/IEventRenderProps/ICustomEventRenderProps#endRow endRow}, может сложиться ситуация, когда одно или несколько событий рендерятся ниже первой строки сетки с динамически создаваемыми ячейками. Так как сетка настроена не статически (мы не знаем число строк, которое может быть заполнено событиями, а CSS не умеет заполнять их автоматически, неоюходимо вычислять число строк и передавать их в параметре maxRow объекта, который возвращается из коллбека {@link Controls-Lists/_dynamicGrid/interfaces/IDynamicGridComponent/IDynamicColumnConfig#getCellProps getCellProps} для динамической ячейки.
     */
    maxRow?: number;
}

/**
 * Коллбек, возвращающий {@link Controls/gridRender:ICellProps параметры для динамически создаваемой ячейки}.
 ...
 * @extends Controls/gridRender:IColumnConfig
 * @public
 */
 ...
 * @extends Controls/gridRender:IBaseColumnConfig
 * @public
 */
...
/**
 * Тип для обработчика изменения активной колонки
 * @typedef {Function} Controls-Lists/_dynamicGrid/interfaces/IDynamicGridComponent/TActiveDynamicColumnChanged
 */
export type TActiveDynamicColumnChanged = (newActiveDynamicColumn: number | Date) => void;
...
export interface IBaseDynamicGridComponentProps
    ...
    /**
     * Функция обратного вызова при изменении активной колонки
     * @cfg {Controls-Lists/_dynamicGrid/interfaces/IDynamicGridComponent/TActiveDynamicColumnChanged.typedef}
     */
    onActiveDynamicColumnChanged?: TActiveDynamicColumnChanged;
    /**
    ...
    /**
     * Количество отображаемых на экране колонок
     */
    dynamicColumnsCount: number;
    /**
     * Перезагрузка списка после остановки скролла
     * @default false
     */
    needReloadGridAfterScrollStops?: boolean;
    resizerVisibility?: boolean;
    /**
     * Метод реакции на изменение диапазона
     */
    onRangeChange?: (newRange: IRange, calledByScroll: boolean) => void;
    /**
     * Ключ активной колонки.
     */
    activeDynamicColumn?: string;
    /**
     * Флаг, позволяющий отключить плавность горизонтального скролла
     * @variant keyboard Отключает для Shift + Arrow.
     * @variant all Отключает для всех случаев
     */
    smoothHorizontalScroll?: 'keyboard' | 'all';
    /**
     * Флаг, позволяющий отрендерить динамические колонки в разделителе
     */
...
export interface IStaticColumnConfig extends IColumnConfig {
    /**
     * Минимальная ширина статичной колонки в мобильном представлении
     * При свайпе влево-вправо по статичной колонке, она может сворачиваться и разворачиваться. В развернутом состояии используется
     * параметр width, а в свернутом minWidth.
     * @demo Controls-Lists-demo/timelineGrid/WI/Base/Index В демо-примере в конфигурации статичной колонки свойство minWidth установлено в значение 60px.
     * @see width
     */
    minWidth?: Pixels;
}
...
export interface IDynamicGridComponentProps
     ...
     * @cfg {Array.<Controls/gridRender:IColumnConfig>}
     ...
     * @cfg {Array.<Controls/gridRender:IHeaderConfig>}
     ...
     * @cfg {Array.<Controls/gridRender:IFooterConfig>}
     ...
     * @cfg {Controls/gridRender:IHeaderConfig}
     ...
     * @cfg {Array<Controls/gridRender:TColumnKey>}
    ...
    /**
     * Контент отображаемый внутри списочного контрола, над заголовком.
     */
    ...
    activeDynamicColumn?: string;
    /**
     * Флаг, включающий ограничение скролла, если выбранный в календаре диапазон целиком поместился во вьюпорте
     * @default false
     */
    limitRangeIfFits?: boolean;
    dynamicGridRef?: React.ForwardedRef<typeof TreeGridComponent>;
    /**
     * Ключ активной колонки, обновляемый при горизонтальной прокрутке
     */
    innerActiveDynamicColumn?: number | Date;
}
...
export interface IDynamicCellsProps {
    render: React.ReactElement;
