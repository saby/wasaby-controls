...
import { InfoBoxContext, IInfoBoxPopupOptions } from 'Controls/popup';
import { SyntheticEvent } from 'UICommon/Events';
import { date as formatDate } from 'Types/formatter';
import { InfoBox } from 'Controls/popupTemplate';
import { MutableRefObject } from 'react';
import { RESIZE_LINE_WIDTH } from 'Controls-Lists/_dynamicGrid/constants';
...
function EventCellRender(props: IEventCellRenderProps) {
    ...
    const leftResizerRef = React.useRef<ResizingLine>();
    const leftResizerWrapperElement = React.useRef<HTMLDivElement>();
    const rightResizerRef = React.useRef<ResizingLine>();
    const rightResizerWrapperElement = React.useRef<HTMLDivElement>();
    const editingDate = React.useRef<Date | null>(null);
    ...
    const onResize = React.useCallback(
        (side: 'left' | 'right') => {
            const infoboxTarget =
                side === 'left' ? leftResizerRef.current : rightResizerRef.current;
            const resizerWrapperElement =
                side === 'left'
                    ? leftResizerWrapperElement.current
                    : rightResizerWrapperElement.current;
            const resizeArea = resizerWrapperElement.getElementsByClassName(
                'controls-ResizingLine__area'
            )[0];
            if (resizeArea) {
                const closeEvent = new SyntheticEvent(null, {
                    target: resizeArea || infoboxTarget,
                    type: 'closeInfoBox',
                });
                infoboxContext.onCloseInfoBox(closeEvent);
            }
            const newEvent = event.clone();
            if (side === 'left') {
                newEvent.set(eventStartProperty, editingDate.current);
            } else if (side === 'right') {
                newEvent.set(eventEndProperty, editingDate.current);
            }
    ...
    const onDragMove = React.useCallback(
        (side: 'left' | 'right') => {
            return (params) => {
                const { offset } = params;
                const direction = offset.x > 0 ? 'forward' : 'backward';
                const resizerWidth =
                    direction === 'forward' ? RESIZE_LINE_WIDTH * -1 : RESIZE_LINE_WIDTH;
                const validatedOffset = validateEventResizeOffset(
                    Math.abs(offset.x + resizerWidth),
                    columnWidth,
                    columnSpacing,
                    side === 'left' ? startOffset : 1 - endOffset
                );
                ...
                editingDate.current = nextDate;

                const infoboxTarget =
                    side === 'left' ? leftResizerRef.current : rightResizerRef.current;
                const startDay = side === 'left' ? nextDate.getDate() : startDate.getDate();
                const endDay = side === 'left' ? endDate.getDate() : nextDate.getDate();
                const resizerWrapperElement =
                    side === 'left'
                        ? leftResizerWrapperElement.current
                        : rightResizerWrapperElement.current;
                ...
                if (resizeArea) {
                    const config: IInfoBoxPopupOptions = {
                        target: resizeArea,
                        message: `${startDay} ${formatDate(
                            side === 'left' ? nextDate : startDate,
                            'MMMl'
                        )} -  ${endDay} ${formatDate(
                            side === 'left' ? endDate : nextDate,
                            'MMMl'
                        )}`,
        ...
        [startDate, endDate, columnWidth, columnSpacing, startOffset, endOffset, quantum]
    );
    ...
    const cellRender = (
        <>
            {customRenderProps?.resizerVisible && (
                <div ref={leftResizerWrapperElement}>
                    <ResizingLine
                        ref={leftResizerRef}
                        onOffset={() => {
                            onResize('left');
            ...
            {customRenderProps?.resizerVisible && (
                <div ref={rightResizerWrapperElement}>
                    <ResizingLine
                        ref={rightResizerRef}
                        onOffset={() => {
                            onResize('right');
