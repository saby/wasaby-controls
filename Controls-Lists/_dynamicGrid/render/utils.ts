...
export function getPositionInPeriod(date: Date, quantum: TQuantumType): number {
    switch (quantum) {
        case 'year':
            return calculateYearFraction(date);
        case 'halfYear':
            return calculateYearFraction(date, 0.5);
        case 'quarter':
            return calculateYearFraction(date, 0.25);
        case 'month':
            return calculateMonthFraction(date);
        case 'week':
            return calculateWeekFraction(date);
        case 'day':
...
/**
 * Возвращает долю прошедшего времени относительно года.
 * @param date
 * @param fraction
 */
function calculateYearFraction(date: Date, fraction: number = 1): number {
    const MONTHS_IN_YEAR = 12;
    const monthsInPeriod = MONTHS_IN_YEAR * fraction;
    return calculateMonthFraction(date) / monthsInPeriod;
}
 ...
 * @param date
 */
function calculateWeekFraction(date: Date): number {
    const daysInWeek = 7;
    return calculateDayFraction(date) / daysInWeek;
}
...
export function validateEventResizeOffset(
    ...
    let currentOffset = 0;
    if (startOffset > 0) {
        if ((columnWidth - columnSpacing) * startOffset > offset) {
            return offset;
        } else {
            currentOffset = (columnWidth - columnSpacing) * startOffset;
        }
    }
    const fraction = Math.abs(offset - currentOffset) / columnWidth;
