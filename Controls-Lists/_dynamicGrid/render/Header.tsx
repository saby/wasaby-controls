...
// TODO удалить, когда уберём Header в timeline
function extractQuantum(quantum: Quantum, quantums?: IQuantum[]): IQuantum | null {
...
function isRangeAvailable(
    ...
    quantums: IQuantum[],
    scale: number,
    accessibility: 'zoom' | 'header' | 'all' = 'all'
): boolean {
    const quantumConfig = extractQuantum(quantum, quantums);
    return !!quantumConfig?.scales?.some((found) => {
        return (
            found.value === scale && (!found.accessibility || found.accessibility === accessibility)
        );
    });
}
...
function shouldHoverSeparatedHeaderCell(
    ...
    const lessThanDay = quantums?.some((q) => isQuantumLessThanDay(q.name));
    ...
    // Если не отображается режим неделя, но отображается месяц
    const isWeekRangeAvailable = isRangeAvailable(Quantum.Day, quantums, DAYS_IN_WEEK, 'all');
    if (quantum === Quantum.Day && !isWeekRangeAvailable) {
        return true;
    }
...
export function getPreparedDynamicHeader(props: IGetPreparedExtraRowDynamicColumnBaseProps) {
    const { dynamicColumn, extraRowDynamicSubColumns } = props;
    ...
    if (extraRowDynamicSubColumns) {
        if (dynamicColumn.subColumns?.length !== extraRowDynamicSubColumns.length) {
            Logger.error(
                'Controls-Lists.dynamicGrid:Component. Ошибка конфигурации компонента: Число подколонок и подзаголовков не совпадает.'
            );
        }
    }
    ...
    const getCellProps = (headerKey) => {
        return {
            ...props.extraRowDynamicColumn.getCellProps?.(headerKey),
