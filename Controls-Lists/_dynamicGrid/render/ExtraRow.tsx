...
function calcNeedOpacity(value: number | Date, range: IRange, quantum: TQuantumType): boolean {
    if (value instanceof Date && quantum === 'day') {
        return range.start.getMonth() !== value.getMonth();
    }
    if (value instanceof Date && quantum === 'month') {
        return range.start.getFullYear() !== value.getFullYear();
    }
    if (value instanceof Date && quantum === 'hour') {
        return range.start.getDate() !== value.getDate();
    }
    return false;
}
