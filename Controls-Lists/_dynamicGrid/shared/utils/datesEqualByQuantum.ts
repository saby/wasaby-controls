...
export function datesEqualByQuantum(date1: Date, date2: Date, quantum: TQuantumType) {
    ...
    if (quantum === 'month' || quantum === 'quarter' || quantum === 'halfYear') {
        return equalYear && equalMonth;
    }
    if (quantum === 'week') {
        const equalWeek =
            DateUtils.getStartOfWeek(date1).getDate() === DateUtils.getStartOfWeek(date2).getDate();
        return equalYear && equalMonth && equalWeek;
    }
    ...
    if (quantum === 'minute' || quantum === 'quarterHour' || quantum === 'halfHour') {
