...
> {
    ...
    constructor(props: IDynamicGridComponentProps) {
        ...
        this.state = {
            ...
            preparedColumns: prepareDynamicGridColumns({
                ...
                task88221034408419: props.task88221034408419,
                task88221034654496: props.task88221034654496,
            }),
    ...
    static getDerivedStateFromProps(
        ...
        if (
            ...
            changes.preparedColumns = prepareDynamicGridColumns({
                ...
                editingConfig: nextProps.editingConfig,
                onDependencyButtonMouseDown: nextProps.onDependencyButtonMouseDown,
                onDependencyButtonClick: nextProps.onDependencyButtonClick,
                task88221034408419: nextProps.task88221034408419,
                task88221034654496: nextProps.task88221034654496,
            });
    ...
    getSnapshotBeforeUpdate(prevProps, prevState) {
        if (
            prevState.columnsDataVersion !== this.state.columnsDataVersion ||
            (prevState.dynamicColumnWidth !== this.state.dynamicColumnWidth &&
                prevProps.needRestoreScrollOnColumnsResize)
        ) {
            ...
            for (let idx = 0; idx < scrollableCells.length; idx++) {
                ...
                if (
                    Math.trunc(scrollableCellRect.left + scrollableCellRect.width) >
                    Math.trunc(fixedCellRect.left + fixedCellRect.width)
                ) {
    ...
    _onEdgesStateChanged(leftEdgeState: EdgeState, rightEdgeState: EdgeState) {
        if (this._rightEdgeState !== rightEdgeState && !this.props.limitRangeIfFits) {
            this._rightEdgeState = rightEdgeState;
            if (rightEdgeState === EdgeState.Visible) {
                this.props.columnsEndedCallback('forward');
            }
        }
        if (this._leftEdgeState !== leftEdgeState && !this.props.limitRangeIfFits) {
    ...
    render() {
        ...
        return (
            <div
                ...
                style={{
                    '--dynamic-columns_count': this.state.dynamicColumnsGridData.length,
                    '--dynamic-sub_columns_count': this.state.dynamicColumn.subColumns?.length,
            ...
            >
                <SelectionContainer
                    ...
                    headerCellsMultiSelectVisibility={this.props.headerCellsMultiSelectVisibility}
                    headerCellsMultiSelectAccessibilityCallback={
                        this.props.headerCellsMultiSelectAccessibilityCallback
                    }
                    ...
                    headerColumnSelector={`.${CLASS_DYNAMIC_HEADER_CELL}`}
                    headerColumnSelectorMask={`.${CLASS_DYNAMIC_HEADER_CELL}[column-key='$key$']`}
                >
                    <GridComponent
                        ref={(el) => {
                        ...
                        shouldStretchGridContainer={this.props.shouldStretchGridContainer}
                        collectionRef={this._gridCollectionRef}
                        ...
                        columnScroll={true}
                        smoothHorizontalScroll={this.props.smoothHorizontalScroll}
                        ...
                        contextMenuVisibility={this.props.contextMenuVisibility}
                        ...
                        rowSeparatorSize={
                            this.props?.horizontalSeparatorsMode === 'line' ? 's' : null
                        }
                        itemsDragNDrop={this.props.itemsDragNDrop}
                        onCustomdragStart={this.props.onCustomdragStart}
                        onCustomdragEnd={this.props.onCustomdragEnd}
                        draggingTemplate={this.props.draggingTemplate}
                        task88221034408419={this.props.task88221034408419}
                        _$rowSelector={'.js-ControlsLists-dynamicGrid__dynamicCellsWrapper'}
                        _$editableCellSelector={
                            'js-ControlsLists-dynamicGrid__dynamicDataCell-editable'
                        }
                    />
                </SelectionContainer>
            </div>
    ...
    componentDidUpdate(prevProps, prevState, snapshot) {
        const gridContainer = this._gridComponentRef.current._container;
        if (
            (!this.props.onActiveDynamicColumnChanged ||
                this.props.activeDynamicColumn !== this.props.innerActiveDynamicColumn) &&
            (prevProps.activeDynamicColumn !== this.props.activeDynamicColumn ||
                this.props.dynamicColumnsGridData.indexOf(this.props.activeDynamicColumn) !==
                    prevProps.dynamicColumnsGridData.indexOf(prevProps.activeDynamicColumn))
        ) {
        ...
        return (
            <DynamicColumnsInfoContext.Provider value={dynamicColumnsInfoContextValue}>
                <DynamicGridClassComponent
                    ...
                    shouldStretchGridContainer={props.shouldStretchGridContainer}
