...
> extends List.slice {
    ...
    protected _initState(
        ...
        const dynamicColumnsFilter = this._prepareDynamicColumnsFilter(config, 'bothways');
    ...
    private _loadColumns(direction: Exclude<TNavigationDirection, 'bothways'>): void {
        ...
        this.load('forward', undefined, preparedFilter, false, navigation).then((loadedItems) => {
            unstable_batchedUpdates(() => {
    ...
    protected _beforeApplyState(
        ...
        if (nextState.root !== this.state.root) {
            ...
            const dynamicColumnsFilter = this._prepareDynamicColumnsFilter(nextState, 'bothways');
