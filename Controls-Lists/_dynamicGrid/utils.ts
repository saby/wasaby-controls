...
import type { IColumnConfig, TColumnKey } from 'Controls/gridRender';
...
export function getViewportWidth(
    ...
    const staticColumnsWidth = getStaticColumnsWidth(
        staticColumns,
        staticWorkspaceSize,
        customWidth
    );
...
export function getDynamicColumnWidth(
    ...
    let width = availableWidth / rangeSize;
    if (width < minWidth) {
        ...
        if (maxRangeSize > 1) {
            ...
            const calculatedWidth = availableWidth / maxRangeSize;
            return {
                width: maxWidth && maxWidth < calculatedWidth ? maxWidth : calculatedWidth,
