...
export const IPhoneType = ObjectType.id('Controls-Input-meta/inputConnected:IPhoneType')
    ...
    .properties<IFlagVisibleOptions & IOnlyMobileOptions>({
        onlyMobile: BooleanType.optional().order(1).defaultValue(false),
        flagVisible: BooleanType.optional().order(2).defaultValue(false),
        flagPosition: StringType.optional().oneOf(['start', 'end']).order(3).defaultValue('start'),
    })
    .order(1)
