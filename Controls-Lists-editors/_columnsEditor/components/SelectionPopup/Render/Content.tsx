...
export function Content(props: IContentRenderProps) {
    ...
    const contentDataModel: TDataModel = React.useMemo(() => {
        return [
            {
                type: 'paragraph',
                data: {
    ...
    const emptyTemplate = useCallback(
        (emptyTemplateProps) => {
            return (
                <EmptyTemplate {...emptyTemplateProps} topSpacing={null}>
                    <EmptyView
                        {...emptyTemplateProps}
                        layout="column"
                        size="l"
                        ...
                        clickHandlers={{ reset: { handler: slice.resetSearch } }}
                    />
                </EmptyTemplate>
            );
        },
        [contentDataModel, slice]
    );
