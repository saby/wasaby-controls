...
function updateDragProperties(
    ...
    const dragElementNodeIdx = dragElementNode.parent.children.findIndex((child) => {
        return child === dragElementNode;
    });
    const targetElementNodeIdx = targetElementNode.parent.children.findIndex((child) => {
        return child === targetElementNode;
    });
    if (dragElementNodeIdx !== -1 && targetElementNodeIdx !== -1) {
        if (dragElementNode.parent === targetElementNode.parent) {
            const commonParent = dragElementNode.parent;
            commonParent.children[dragElementNodeIdx] = targetElementNode;
            commonParent.children[targetElementNodeIdx] = dragElementNode;
        } else {
            let dragElementNodeParent = dragElementNode.parent;
            ...
            while (dragElementNodeParent.children.length === 0) {
                const parent = dragElementNodeParent.parent;
                const childIdx = parent.children.findIndex((child: INode) => {
                    return child === dragElementNodeParent;
                });
                parent.children.splice(childIdx, 1);
                dragElementNodeParent = parent;
            }
            const targetElementNodeParent = targetElementNode.parent;
            if (direction === 'left') {
                targetElementNodeParent.children.splice(targetElementNodeIdx, 0, dragElementNode);
            } else {
                targetElementNodeParent.children.splice(
                    targetElementNodeIdx + 1,
                    0,
                    dragElementNode
                );
            }
            dragElementNode.parent = targetElementNodeParent;
        }
...
function Markup(params: IMarkup) {
    ...
    for (let i = 0; i < headers.length; i++) {
        ...
        const isMainColumn: boolean = i === 0;
        ...
        markupContent.push(
            <MarkupElement
                ...
                onRightAddButtonClick={!isFolder && !areAllColumns ? onAddButtonClick : undefined}
                ...
                dragElementId={dragObjectId}
                onCheckboxClick={!isMainColumn ? onCheckboxClick : undefined}
                selectedCheckboxVisibility={isFolder ? 'onhover' : 'visible'}
