...
import { INode, ITreeControlItem } from 'Controls-Lists-editors/_columnsEditor/utils/markup';
...
export function Content(props: ISubTreeEditorContentProps) {
    const { onChange, LayoutComponent, treeItems, unusedColumns, emptyFolders, rootKey } = props;
    ...
    React.useEffect(() => {
        emptyFoldersRef.current = emptyFolders.map((folder, idx) => {
            return { ...folder, key: `empty_${idx}`, parent: rootKey };
        });
    ...
    const selectionItems = React.useMemo(() => {
        ...
        // Выделить элементы соответствующие колонкам, которые есть в таблице, но не вложены в папку
        for (let i = 0; i < treeItems.length; i++) {
            const node: INode = treeItems[i].node;
            if (node.column !== undefined && node.parent.parent === null) {
                editingItems.push({ ...treeItems[i] });
                editingItems[idx].key = idx;
                idx += 1;
            }
        }
    ...
    }, [treeItems, unusedColumns]);
    const folders = React.useMemo(() => {
        return getFolderItems(treeItems, rootKey, emptyFoldersRef.current);
        ...
        (
            ...
            return (newValue: IEditingValue) => {
                ...
                emptyFolderItem.headerOptions = {
                    ...
                    textOverflow: updatedValue.textOverflow,
    ...
    const onAddFolderClick = React.useCallback(() => {
        ...
        const defaultValue = {
            ...
            textOverflow: FOLDER_DEFAULT_PARAMS.textOverflow,
        };
                            ...
                            templateOptions: {
                                ...
                                eventHandlers: {
                                    onResult: (data: RecordSet | null) => {
                                        const containerItem =
                                            data === null ? data : data.getRawData();
                                        ...
                                        if (containerItem !== null && shouldUpdateEditor) {
                                            emptyFoldersRef.current = deleteEmptyFolder(
                                                containerItem,
                                                emptyFoldersRef.current
                                            );
                                        }
                                        if (shouldUpdateEditor) {
                                            onChange({
                                                action: 'move',
                                                items: [movingItem.current, containerItem],
                                                savedOptions: {
                                                    emptyFolders: emptyFoldersRef.current,
                                                },
                                            });
