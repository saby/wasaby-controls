...
export function Content(props: ISubTreeEditorContentProps) {
    ...
    const selectionItems = React.useMemo(() => {
        ...
        const rootItem = treeItems.find((item: ITreeControlItem) => {
            return item.key === rootKey;
        });
        // Выделить элементы соответствующие колонкам, которые есть в таблице, но не вложены в редактируемую папку
    ...
    }, [treeItems, unusedColumns, rootKey]);
