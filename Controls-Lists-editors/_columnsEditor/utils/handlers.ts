...
async function loadTreeData(
    ...
    const loadConfig = getLoadConfig(items, subRootKey, markedKey);
...
export async function openPopupEditor(props: IOpenPopupEditor) {
    ...
    if (view === 'column') {
        ...
        const markedKey = markedNode
            ? treeControlItems.find((item) => {
                  return (
                      item.node.startColumn === markedNode.startColumn &&
                      item.node.endColumn === markedNode.endColumn &&
                      item.node.startRow === markedNode.startRow &&
                      item.node.endRow === markedNode.endRow
                  );
              })?.key
            : null;
        ...
        const data = await loadTreeData(tree, allColumns, allHeader, subRootKey, markedKey);
