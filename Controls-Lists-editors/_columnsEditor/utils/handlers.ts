...
export function onOpenColumnsList(
    ...
    if (!dialog.isOpened() && allColumns.length + 1 !== columns.length) {
        dialog.open({
...
function getLoadConfig(items: ITreeControlItem[], rootKey: number | string) {
    return {
        subTree: {
...
async function loadTreeData(
    ...
    const loadConfig = getLoadConfig(items, rootKey);
...
function getRootKey(root: INode, items: ITreeControlItem[]): number | string | undefined {
...
export async function openPopupEditor(props: IOpenPopupEditor) {
    ...
    if (view === 'column') {
        ...
        editor = new MainColumnEditor(value, elementRef, containerWidth);
    } else {
        ...
        const rootKey = getRootKey(subTree, treeControlItems);
        const data = await loadTreeData(tree, allColumns, allHeader, rootKey);
    ...
    opener.open({
        metaType: ObjectType.id(String(id)).properties(editor.getMetaType()).title('Колонка'),
        ...
        onChange: (newValue: IEditingValue) => {
            ...
            if (newValue.subTree !== undefined) {
                ...
                if (action === ACTION_DELETE) {
                    ...
                    const items: ITreeControlItem[] = newValue.subTree.items;
                    const containerItem: ITreeControlItem | null = items.pop();
                    const movingItem = items.pop();
                    const movingNode: INode = findNode(
                        tree,
                        movingItem.node.startColumn,
                        movingItem.node.endColumn,
                        movingItem.node.startRow,
                        movingItem.node.endRow
                    );
                    let containerNode = tree;
                    if (containerItem !== null) {
                        containerNode = containerItem.node;
                        if (!containerNode) {
                            containerNode = {
                                parent: subTree,
                                selected: false,
                                children: [],
                                header: {
                                    caption: containerItem.title,
                                    ...containerItem.headerOptions,
                                },
                            };
                            subTree.children.push(containerNode);
                        } else {
                            containerNode = findNode(
                                tree,
                                containerItem.node.startColumn,
                                containerItem.node.endColumn,
                                containerItem.node.startRow,
                                containerItem.node.endRow
                            );
                        }
                    }
                    let shouldChangeParent = false;
                    if (movingNode.children.length === 0) {
                        if (movingNode.parent !== containerNode) {
                            shouldChangeParent = true;
                        }
                    } else {
                        const isNested: INode | undefined = findNode(
                            movingNode,
                            containerNode.startColumn,
                            containerNode.endColumn,
                            containerNode.startRow,
                            containerNode.endRow
                        );
                        if (!isNested) {
                            shouldChangeParent = true;
                        }
                    ...
                    if (shouldChangeParent) {
                        changeParent(movingNode, containerNode);
                    }
                }
                updateTree(tree);
            ...
            if (editingNodeExists && newIdx !== -1) {
                ...
                const newHeaderProperties = {
                    ...
                    textOverflow: updatedMarkupValue.textOverflow,
                };
                newStaticProperties = {
