...
export function swipeNodes(dragNode: INode, targetNode: INode, direction: 'left' | 'right') {
    ...
    const targetElementNodeIdx = targetNode.parent.children.findIndex((child) => {
    ...
    if (dragElementNodeIdx !== -1 && targetElementNodeIdx !== -1) {
        if (dragNode.parent === targetNode.parent) {
            ...
            commonParent.children[dragElementNodeIdx] = targetNode;
            commonParent.children[targetElementNodeIdx] = dragNode;
        } else {
...
export function countSelectedColumns(node: INode) {
