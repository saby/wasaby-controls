 ...
 * @param columns
 */
export function validateColumnsConfig(columns: TColumnsForCtor): TColumnsForCtor {
    const result = [...columns];
...
export function validateHeaderConfig(header: THeaderForCtor): THeaderForCtor {
    const result = [...header];
    result.map((header) => {
...
function getWIDemoData(columns: TColumnsForCtor, header: THeaderForCtor): IColumnsProps {
    const newColumns = [...columns];
    let newHeader = [...header];
    // Удалим папки
    newHeader = newHeader.filter((elem) => {
        return elem.endColumn - elem.startColumn === 1;
    });
    // Удалим главную колонку, тк ее нет в списке всех колонок
    newColumns.shift();
    newHeader = newHeader.filter((elem) => {
        return elem.startColumn !== 1;
    });
    // Восстановить соответствие между колонками и заголовками
    newHeader = newHeader.map((elem) => {
        return {
            ...elem,
            startColumn: elem.startColumn - 1,
            endColumn: elem.endColumn - 1,
        };
    });
    return {
        columns: newColumns,
...
export const getContextDataConfig = (
    ...
    const installationColumns = validateColumnsConfig(installationParams.columns);
    const installationHeader = validateHeaderConfig(installationParams.header);
    ...
    let allOptions = {
        columns: params.allColumns,
        header: params.allHeader,
    };
    if (allOptions.columns.length === 0) {
        // На wi нет возможности эмулировать запросы на БЛ.
        // Добавлять такую возможность дорого. Дешевле просто брать список колонок из инсталляционных,
        // чтобы корректно показать возможности редактора.
        allOptions = getWIDemoData(installationParams.columns, installationParams.header);
    }
                    ...
                    frame: new JsonmlFrameFacade([
                        ...
                        [
                            widget,
                            {
                                id: 'widget',
                                ...widgetProps,
                                editingColumns: value.columns,
                                editingHeaders: value.header,
