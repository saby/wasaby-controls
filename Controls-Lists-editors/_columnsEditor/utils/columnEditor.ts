...
import { IHeaderCell, TColumnsForCtor, THeaderForCtor, IColumn } from 'Controls/gridDisplay';
...
export function updateWidthAfterReduction(
    widthInPixels: number,
    nextWidth: IColumnWidth,
    isMainColumn?: boolean,
    prevMinLimit?: number
): IColumnWidth {
    let result = { ...nextWidth };
    if (!prevMinLimit) {
        prevMinLimit = isMainColumn ? DEFAULT_MIN_MAIN_COLUMN_WIDTH : MIN_COLUMN_WIDTH;
    }
    if (widthInPixels > prevMinLimit) {
        result.maxLimit = widthInPixels;
        result.minLimit = prevMinLimit;
    } else {
        result = {
            mode: FIXED_MODE,
            amount: widthInPixels,
            units: PIXEL_UNIT,
        };
    }
    return result;
...
export function getUnusedColumns(
    ...
    allColumns.filter((initColumn: IColumn, columnIdx: number) => {
        ...
        if (!editingColumn) {
            const unusedColumnHeader = allHeader.find((initHeader: IHeaderCell) => {
                return initHeader.startColumn - 1 === columnIdx;
            });
