...
import { onElementDeleteCallback } from 'Controls-Lists-editors/_columnsEditor/utils/handlers';
...
const confirmationPopupOptions: IConfirmationOptions = {
    ...
    markerStyle: 'primary',
    style: 'default',
    message: rk('Колонки не выбраны'),
    details: rk('Отметьте колонки, которые необходимо удалить'),
};
        ...
        if (actualColumns) {
            ...
            if (selectedColumnsIdxs.length > 0) {
                while (selectedColumnsIdxs.length > 0) {
                    ...
                    const newConfig = onElementDeleteCallback(
                        newHeader,
                        newColumns,
                        selectedColumnsIdxs,
                        columnToDeleteId
