...
import { TColumnsForCtor, THeaderForCtor } from 'Controls/gridDisplay';
...
import { addFakeColumn, addFakeHeader } from 'Controls-Lists-editors/_columnsEditor/utils/data';
...
function openFolderEditorPopup(params: IOpenFolderEditorPopupParams) {
    ...
    openPopupEditor({
        ...
        value: {
            ...
            whiteSpace: editingParams.header[idx].whiteSpace,
...
export class AddNewFolder extends BaseAction {
    onExecuteHandler = async () => {
        ...
        if (actualColumns) {
            ...
            // Удалим фейковую колонку и заголовок
            headers.pop();
            columns.pop();
            ...
            if (actualSelectedColumnsIdxs.length > 0) {
                ...
                addFakeColumn(newParams.columns);
                addFakeHeader(newParams.header);
