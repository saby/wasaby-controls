...
export default class TreeGridNodeFooterRow extends mixin<TreeGridNodeExtraRow, NodeFooterMixin>(
    ...
    protected _nextVersion() {
        // Если узлы и футеры узлов были пересозданы (RESET в результате смены фильтра), то у них может оказаться
        // та же версия, что и до пересоздания, но с разными hasMoreStorage('forward').
        // Рендер таблицы смотрит только на версию и contents (который СТРОКА). Они получаются одинаковые,
        // И до следующего nextVersion кнопка "ещё" будет отображаться или наоборот, не отображаться.
        // При клике на неё или ничего не будет происходить, или будут ошибки в консоль.
        // Поэтому тут при hasMoreStorage добавляем к версии +1
        super._nextVersion(1 + +this.hasMoreStorage('forward'));
    }
