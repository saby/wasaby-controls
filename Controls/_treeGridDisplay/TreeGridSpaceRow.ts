...
import type { IRowComponentProps, IGridRowOptions } from 'Controls/grid';
import { mixin } from 'Types/util';
import { TreeItem } from 'Controls/baseTree';

export interface ITreeGridSpaceRowOptions<T extends Model = Model> extends IGridRowOptions<T> {
    itemsSpacing?: TOffsetSize;
}

export default class TreeGridSpaceRow<T extends Model = Model> extends mixin<
    TreeItem<T>,
    GridRowMixin<T>
>(TreeItem, GridRowMixin) {
    readonly '[Controls/_display/grid/Row]': boolean;
    readonly '[Controls/_display/SpaceCollectionItem]': boolean = true;
    ...
    getWrapperClasses(): string {
        let result =
            ...
            ' controls-List__SpaceItem tw-col-start-1 tw-col-end-1';
    ...
    getRowComponentProps(): IRowComponentProps {
        const superRowProps = super.getRowComponentProps();
        const className =
            (superRowProps.className ? superRowProps.className : '') + ' controls-List__SpaceItem';
        return {
