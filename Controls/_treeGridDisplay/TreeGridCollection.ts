...
import { NODE_TYPE_PROPERTY_GROUP, Tree, TreeItem } from 'Controls/baseTreeDisplay';
import { GridGroupRow, GridMixin, TColspanCallback } from 'Controls/gridDisplay';
...
import type { IOptions } from 'Controls/_gridDisplay/mixins/Grid';
import type { IGridControlOptions } from 'Controls/grid';
import GroupNodeBlocks from './itemsStrategy/GroupNodeBlocks';
import { UILogic } from 'Controls/_listsCommonLogic/UILogic';
...
> extends mixin<Tree<S, T>, GridMixin<S, T>>(Tree, GridMixin) {
    ...
    setNodeFooter(newOptions: IGridControlOptions): void {
        const isCompatibleView = this.hasCompatibilityProps(['nodeFooterTemplate']);
        const nodeFooter: INodeFooterConfig[] = newOptions.nodeFooter;
        ...
        if (!isCompatibleView && this._$nodeFooter !== nodeFooter) {
            ...
            this._nextVersion();
        } else if (isCompatibleView) {
            // Если на колонках задан nodeFooterTemplate, то возможно нужно обновить nodeFooter
            const newNodeFooter = this._$columns.map((column) => {
                return {
                    key: column.key || 'node-footer-' + column.displayProperty,
                    template: column.nodeFooterTemplate,
                };
            });
            if (this._$nodeFooter !== newNodeFooter) {
                this._$nodeFooter = newNodeFooter;
                this._updateItemsProperty(
                    'setColumnsConfig',
                    this._$nodeFooter,
                    '[Controls/treeGrid:TreeGridNodeFooterRow]'
    ...
    protected _initializeUserStrategies(options) {
        super._initializeUserStrategies(options);
    ...
    setResultsVerticalPadding(resultsVerticalPadding: boolean): void {
        if (this._$resultsVerticalPadding !== resultsVerticalPadding) {
            this._$resultsVerticalPadding = resultsVerticalPadding;
            this._nextVersion();
        }
    }
    ...
    setEmptyTemplate(emptyTemplate: TemplateFunction): boolean {
        ...
        return this._setEmptyTemplate(superResult);
    }
    ...
    setMultiSelectVisibility(visibility: string): void {
        ...
        this._setMultiSelectVisibility(visibility, oldVisibility, () => {
    ...
    setIndexes(start: number, stop: number, shiftDirection: IDirection): void {
        ...
        this._getItems().forEach((item: T) => {
            ...
            } else if (item['[Controls/treeGrid:TreeGridNodeHeaderRow]']) {
    ...
    protected _handleAfterCollectionItemChange(item: T, index: number, properties?: object): void {
        // _handleAfterCollectionItemChange есть одновременно в Tree и Grid. Поэтому логика обновления детей у TreeGridCollection своя
        if (properties?.hasOwnProperty(this.getParentProperty())) {
            this._updateChildren(item);
        }
        super._handleAfterCollectionItemChange(item, index, properties);
    }
    ...
    protected _hasStickyGroup(): boolean {
        return !!(
            this.at(0) &&
            (this.at(0)['[Controls/_display/GroupItem]'] || this.at(0).$TGGR) &&
    ...
    shouldHideGroupNode(hasChildren: boolean, groupIndex: number): boolean {
        ...
        if (
            ...
            // Если в подвале узла кнопка "еще" в виде разделителя с тремя точками, то надо скрывать заголовок, если папка пуста
            const moreButtonViewIsSeparator =
                this.getNavigation()?.viewConfig?.buttonView === 'separator';
            if (this._$nodeFooterTemplate && !moreButtonViewIsSeparator) {
                return false;
            } else {
                return isEmptyGroup;
            }
    ...
    calculateGroupNodesVisibility(nodes: TreeGridDataRow[]) {
        nodes.map((node, index) => {
            if (node.GroupNodeItem) {
                const hasChildren =
                    node.hasChildren() ||
                    node.hasChildrenByRecordSet() === undefined ||
                    node.hasChildrenByRecordSet();
    ...
    protected _itemsFactoryResolver(
        ...
        const NodeFooterFactory = (
            ...
            factoryOptions.columnsConfig = this._$nodeFooter;
        ...
        const NodeHeaderFactory = (
            ...
            factoryOptions.gridColumnsConfig = this._$columns;
            factoryOptions.columnsConfig = this._$nodeHeader;
        ...
        if (options.itemModule === 'Controls/baseTreeDisplay:PseudoParentCollectionItem') {
            options.itemModule = 'Controls/treeGrid:TreeGridDataRow';
            options.getRowProps = () => ({ className: 'tw-hidden' });
        }
    ...
    protected _updateInteractorStateProps(interactorStateProps: IInteractorStateProps): boolean {
        return (
            super._updateInteractorStateProps(interactorStateProps) &&
            this._updateInteractorGridStateProps(interactorStateProps)
        );
    }
}
