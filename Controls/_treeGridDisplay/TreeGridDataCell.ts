...
import type { IItemsContainerPadding, ICellComponentProps, IRowComponentProps } from 'Controls/grid';
...
export default class TreeGridDataCell<T extends Model> extends GridDataCell<T, TreeGridDataRow<T>> {
    ...
    getCellComponentProps(rowProps: IRowComponentProps): ICellComponentProps {
        ...
        const borderProps = {
            borderStyle: this.getOwner().isDragTargetNode() ? 'primary' : rowProps.borderStyle,
            borderMode: this.getOwner().isDragTargetNode() ? 'row' : rowProps.borderMode,
            borderVisibility: this.getOwner().isDragTargetNode()
                ? 'visible'
                : rowProps.borderVisibility,
        };
