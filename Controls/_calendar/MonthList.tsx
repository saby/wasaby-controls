 ...
 */
import { NewSourceController, error } from 'Controls/dataSource';
...
import { ICellProps, IColumnConfig, IRowProps } from 'Controls/gridRender';
import {
...
import { getAmountOfMonths, getStartOfQuarter } from 'Controls/_utils/dateUtils/Date';
 ...
 * @remark
 * Полезные ссылки:
 * * {@link https://git.sbis.ru/saby/wasaby-controls/-/blob/rc-24.6100/Controls-default-theme/variables/_calendar.less переменные тем оформления}
 *
        ...
        IDisplayedRanges
{
    ...
    private _shouldPositionBelowChanged: boolean = false;
    ...
    constructor(props: IMonthListOptions) {
        ...
        const normalizedPosition = this._normalizeDate(
            position,
            props.viewMode,
            props.amountOfCustomItems
        );
        ...
        if (this._extData && props.source && props.source.getInitialData) {
            this._extData.updateInitialData(position);
        }
    ...
    componentDidMount() {
        ...
        if (
            this._extData &&
            ((this.props.source && !this.props.source.getInitialData) || this.props.holidaysGetter)
        ) {
    ...
    shouldComponentUpdate(props: IMonthListOptions): boolean {
        ...
        if (sourceUpdated) {
            this._displayedDates = this._getDisplayedRanges(
                props.position || this._lastNotifiedPositionChangedDate,
                this.props.virtualPageSize,
                this.props.viewMode
            );
            this._enrichItems();
    ...
    private _getDisplayedRanges(
        ...
        if (viewMode === VIEW_MODE.custom) {
            const monthsInYear = 12;
            virtualPageSize *= monthsInYear / this.props.amountOfCustomItems;
    ...
    private _updateSource(props: IMonthListOptions, oldProps?: IMonthListOptions): boolean {
        if (
            ...
            props.amountOfCustomItems !== oldProps.amountOfCustomItems
        ) {
            ...
            let normalizedPosition = this._normalizeDate(
                position,
                props.viewMode,
                props.amountOfCustomItems
            );
    ...
    private _scrollToPosition(position: Date): void {
        ...
        const normalizedPosition: Date = this._normalizeDate(
            position,
            this.props.viewMode,
            this.props.amountOfCustomItems
        );
    ...
    private _normalizeDate(date: Date, viewMode: VIEW_MODE, amountOfCustomItems: number): Date {
        if (viewMode === VIEW_MODE.custom) {
            const monthsInYear = 12;
            const monthsInCustomItem = monthsInYear / amountOfCustomItems;
            const column = Math.floor(date.getMonth() / monthsInCustomItem);
            return new Date(date.getFullYear(), column * monthsInCustomItem);
        }
        return viewMode !== VIEW_MODE.month
    ...
    private _updateDisplayedPosition(entries: IntersectionObserverSyntheticEntry[]): void {
        ...
        for (const entry of entries) {
            if (entry.data) {
                ...
                const boundingClientRect: DOMRect = entry.nativeEntry.boundingClientRect;
                ...
                if (boundingClientRect.top - rootBounds.top <= 0) {
                    ...
                    if (boundingClientRect.bottom - rootBounds.top >= SCALE_ROUNDING_ERROR_FIX) {
                        ...
                        this.props.viewMode !== VIEW_MODE.custom
                    ) {
    ...
    private _scrollToDate(date: Date): boolean {
        ...
        if (containerToScroll) {
            scrollToElement(
                containerToScroll,
                this._shouldPositionBelow && !this._shouldPositionBelowChanged ? 'bottom' : 'top',
                true,
                false,
                0,
                false,
            ...
            this._shouldPositionBelowChanged = false;
    ...
    private _findElementByDate(date: Date): HTMLElement {
        ...
        const getShouldPositionBelowPosition = () => {
            ...
            if (this.props.viewMode !== VIEW_MODE.month) {
                ...
                if (startValue && endValue) {
                    const startValueQuarter = dateUtils.getStartOfQuarter(startValue);
                    const endValueQuarter = dateUtils.getEndOfQuarter(endValue);
                    const amountOfMonths = dateUtils.getAmountOfMonths(
                        startValueQuarter,
                        endValueQuarter
                    );
                    if (amountOfMonths <= 9) {
                        return new Date(endValue.getFullYear(), endValue.getMonth() + 3, 1);
                    }
                    if (amountOfMonths > 9) {
                        this._shouldPositionBelowChanged = true;
                        return new Date(startValue.getFullYear(), 0);
                    }
                }
        ...
        const templates = {
            custom: {
                condition: () => {
                    return this._shouldPositionBelow && !this._shouldPositionBelowChanged;
                },
            ...
            day: {
                condition: () => {
                    return (
                        (date.getDate() !== 1 || this._shouldPositionBelow) &&
                        !this._shouldPositionBelowChanged
                    );
                },
            ...
            month: {
                condition: () => {
                    return (
                        (date.getMonth() !== 0 || this._shouldPositionBelow) &&
                        !this._shouldPositionBelowChanged
                    );
                },
                dateId: dateUtils.getStartOfMonth(date),
