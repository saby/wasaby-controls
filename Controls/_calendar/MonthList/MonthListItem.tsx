...
interface IMonthListItemOptions extends IDateRangeOptions, IComponentProps, TInternalProps {}
...
export default class MonthListItem extends React.Component<IMonthListItemOptions> {
    ...
    private _getId(): string {
        if (this.props.viewMode !== 'custom') {
            return this.props.item.get('id');
        }
        const date = dateUtils.getStartOfYear(this.props.item.get('date'));
        return monthListUtils.dateToId(date);
    }
    ...
    render() {
        return (
            <MonthListItemTemplate
                ...
                extData={this.props.extData.getData(this._getId())}
                holidaysData={this.props.extData.getHolidaysData(this.props.item.get('id'))}
                calendarData={this.props.extData}
