...
export default class MonthsSource extends Memory {
    ...
    protected _amountOfCustomItems: number;
    ...
    constructor(
        ...
        this._amountOfCustomItems = options.amountOfCustomItems;
    ...
    private _getDefaultDisplayedRanges(): [Date, Date][] {
        ...
        const lastMonth = 11;
        return [
            [new Date(dateUtils.MIN_YEAR_VALUE, 0), new Date(dateUtils.MAX_YEAR_VALUE, lastMonth)],
        ];
    }
    ...
    getItemsData(
        ...
        monthLt?: string,
        monthGt?: string
    ): { items: []; month: Date; hasBeforeItems: boolean } {
        ...
        return {
            ...
            month,
            hasBeforeItems: items.length
                ? this._isDisplayed(this._shiftRange(items[0].date, delta * -1))
                : false,
        };
    }
    ...
    query(query: Query): Promise<DataSet> {
        ...
        const executor = () => {
            ...
            const {
                items,
                month: itemsDataMonth,
                hasBeforeItems,
            } = this.getItemsData(limit, month, monthLt, monthGt);
            this._hasBeforeItems = hasBeforeItems;
            ...
            return this._prepareQueryResult(
                {
                    ...
                    meta: {
                        total: monthEqual
                            ? {
                                  before: hasBeforeItems,
                                  after: Boolean(itemsDataMonth),
                              }
                            : Boolean(itemsDataMonth),
                    },
                },
                null
            );
    ...
    private _shiftRange(date: Date, delta: number): Date {
        if (this._amountOfCustomItems) {
            const range = (12 / this._amountOfCustomItems) * delta;
            return new this._dateConstructor(date.getFullYear(), date.getMonth() + range);
        }
