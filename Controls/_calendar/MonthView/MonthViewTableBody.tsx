...
class DayTemplate extends React.Component<IMonthViewProps> {
    ...
    protected _handler(e: React.BaseSyntheticEvent): void {
        switch (e.type) {
            ...
            case 'mouseleave':
                ...
                break;
            case 'click':
                // На ios chrome по какой-то причине срабатывает двойной клик
                // В ios chrome будем обрабатывать touchstart как клик
                if (!detection.isChromeIOS) {
    ...
    private _oldDayTemplate(): React.ReactElement {
        ...
        return (
            <div
                data-qa={this.props.value.selected ? 'controls-MonthViewVDOM__item-selected' : ''}
...
export default function MonthViewTableBody(props: IMonthViewProps): React.ReactElement[] {
        ...
        return (
            <div
                ...
                className={
                    ...
                    (props.newMode !== true ? ' controls-MonthViewVDOM__tableRow-old' : '')
                }
            >
                {week.map((weekValue) => {
                    return <DayTemplate {...props} key={weekValue.id} value={weekValue} />;
                })}
            </div>
        );
    });
