...
export default class MonthViewModel extends VersionableMixin {
    ...
    constructor(cfg: object) {
        ...
        const isQuantumSelection =
            cfg.selectionType === 'quantum' && cfg.ranges;
        let isSingleDayQuantum;
        if (isQuantumSelection) {
            isSingleDayQuantum =
                'days' in cfg.ranges && cfg.ranges.days.indexOf(1) !== -1;
        }
        if (
            cfg.selectionType === 'workdays' ||
    ...
    protected _prepareClass(
        ...
        if (
            scope.selected &&
            (scope.isCurrentMonth || scope.mode === 'extended')
        ) {
            ...
            const borderStylePostfix = borderStyle
                ? '_style-' + borderStyle
                : '';
            ...
            if (!scope.readOnly) {
                ...
                if (!scope.selected) {
                    ...
                    if (scope.selectionEnabled && this._singleDayHover) {
                        borderStyleLocal =
                            'controls-MonthView__border-currentMonthDay-unselected';
                    } else if (scope.hovered) {
                        borderStyleLocal = 'controls-MonthView__border-hover';
                    }
                    if (borderStyleLocal) {
                        borderStyleLocal += backgroundStyle
    ...
    protected _getDayObject(date: Date, state: {}, dayIndex: number): object {
        ...
        obj.selectionBase = DateUtil.isDatesEqual(
            date,
            state.selectionBaseValue
        );
    ...
    private _getDaysArray(outerState?: object) {
        ...
        const weeks = calendarUtils.getWeeksArray(
            state.month,
            state.mode,
            state.dateConstructor
        );
        let dayIndex = 0;
