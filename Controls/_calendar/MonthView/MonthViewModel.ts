...
export default class MonthViewModel extends VersionableMixin {
    ...
    getMonthAvailable() {
        for (const week of this._modelArray) {
            for (const day of week) {
                if (!day.isDayAvailable) {
                    return false;
                }
            }
        }
    ...
    protected _normalizeState(state: object): object {
        return {
            ...
            enableWeekends: state.enableWeekends,
    ...
    protected _isStateChanged(state: object): object {
        return (
            ...
            state.readOnly !== this._state.readOnly
        );
    }
    ...
    protected _getDayObject(date: Date, state: {}, dayIndex: number): object {
        ...
        obj.weekend = (obj.dayOfWeek === 5 || obj.dayOfWeek === 6) && state.enableWeekends;
