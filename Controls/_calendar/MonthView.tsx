...
export default class MonthView extends React.Component<IComponentPropsWithReadonly> {
    _month: Date | string;
    _days: object[];
    ...
    constructor(props: IComponentPropsWithReadonly) {
        super(props);
        ...
        if (this.props.isDayAvailable) {
            this._isAvailable = this._monthViewModel.getMonthAvailable();
        }
        if (!props.newMode) {
            Logger.warn(
    ...
    shouldComponentUpdate(props: IComponentPropsWithReadonly): boolean {
        this._updateView(props);
    ...
    private _isDayAvailable(date: Date, extData?: object): boolean {
        ...
        if (
            this.props.selectionProcessing &&
            (this.props.selectionType === IDateRangeSelectable.SELECTION_TYPES.range ||
                this.props.selectionType === IDateRangeSelectable.SELECTION_TYPES.quantum)
        ) {
            const iterationDate = new Date(this.props.startValue.getTime());
            const dayInMilliseconds = 86400000;
            let result = true;
            const endValue = new Date(this.props.endValue.getTime() + dayInMilliseconds);
            while (result && iterationDate.getTime() !== endValue.getTime()) {
                const newExtData = this.props.calendarData?.getData(
                    this._dateToDataString(
                        new Date(iterationDate.getFullYear(), iterationDate.getMonth())
                    )
                )?.[iterationDate.getDate() - 1];
                result = this.props.isDayAvailable(iterationDate, newExtData);
                iterationDate.setTime(iterationDate.getTime() + dayInMilliseconds);
            }
            return result;
        }
    ...
    private _updateView(props: IComponentPropsWithReadonly): void {
    ...
    static defaultProps: Partial<IComponentPropsWithReadonly> = {
