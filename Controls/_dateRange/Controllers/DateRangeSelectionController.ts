...
import RangeSelectionController, {
    IRangeSelectionController,
} from './RangeSelectionController';
...
export default class DateRangeSelectionController extends RangeSelectionController {
        ...
        const isSingleQuant = () => {
            ...
            return (
                options.selectionType ===
                    DateRangeSelectionController.SELECTION_TYPES.quantum &&
    ...
    protected _prepareState(
        state: IDateRangeSelectionController
    ): IDateRangeSelectionController {
        ...
        if (newState.hasOwnProperty('selectionBaseValue')) {
            newState.selectionBaseValue = DateUtil.normalizeDate(
                newState.selectionBaseValue
            );
        }
        if (newState.hasOwnProperty('selectionHoveredValue')) {
            newState.selectionHoveredValue = DateUtil.normalizeDate(
                newState.selectionHoveredValue
            );
        }
        return newState;
    }
    ...
    protected _itemClickHandler(event: Event, item: Date): void {
        ...
        if (
            ...
            DateRangeSelectionController.SELECTION_TYPES.workdays
        ) {
    ...
    protected _getDisplayedRangeEdges(item: Date): Date[] {
        ...
        if (this._rangeSelectedCallback) {
            range = this._rangeSelectedCallback(
                range[0],
                range[1],
                this.getSelectionBaseValue()
            );
        }
        return range;
    }
