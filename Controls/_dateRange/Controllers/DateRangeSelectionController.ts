...
export default class DateRangeSelectionController extends RangeSelectionController {
    ...
    protected _beforeUpdate(options: IDateRangeSelectionController): void {
        ...
        if (
            options.quantum &&
            !options.shouldNotChangeSelectionType &&
            this._isExternalChanged('quantum', options, this._state)
        ) {
            this._quantum = options.quantum;
        }
    ...
    private _isSingleQuant(selectionType, quantum): boolean {
        // Пустой years означает что можно выбирать любые года.
        if (quantum?.years?.length === 0) {
            return false;
        }
        ...
        // не единственный.
        return (
            selectionType === DateRangeSelectionController.SELECTION_TYPES.quantum &&
    ...
    protected _itemClickHandler(event: Event, item: Date, additionalConfig: object = {}): void {
        this._selectionType = additionalConfig.selectionType || this._state.selectionType;
        this._quantum = additionalConfig.quantum || this._quantum;

        // Такое же событие может прилететь от списка
        if (!(item instanceof Date)) {
            return;
        }
        if (this._selectionType === DateRangeSelectionController.SELECTION_TYPES.workdays) {
            this._processSingleSelection(item);
        }
        if (this._selectionType === DateRangeSelectionController.SELECTION_TYPES.quantum) {
            if (this._isSingleQuant(this._selectionType, this._quantum)) {
                this._processSingleSelection(item);
            } else {
    ...
    protected _itemMouseEnterHandler(
        event: Event,
        item: Date,
        ...
        this._selectionType = additionalConfig.selectionType || this._state.selectionType;
        this._quantum = additionalConfig.quantum || this._quantum;
        super._itemMouseEnterHandler(event, item);
    }
