...
export default class RelationController extends Control<IRelationController> {
    ...
    protected _validateAfterMountCallback: Function;

    protected _beforeMount(options: IRelationController): void {
        this._model = new Model(options);
        const oldRanges = [...this._model.ranges];
        // this._model.validateRanges();
        // // Дождемся afterMount, т.к. на beforeMount notify не работает
        // // При этом, визуально будет показан корректный период, т.к. значения прокидываются
        // // в контент через пропсы
        // this._validateAfterMountCallback = () => {
        //     this._notifyRangeChanged(this._model.ranges, oldRanges);
        // };
    }

    protected _afterMount(): void {
        // this._validateAfterMountCallback();
        // this._validateAfterMountCallback = null;
    }
 ...
 * Данная опция работает только в том случае тип связи {@link Controls/_dateRange/RelationController#bindType опция bindType} имеет значение 'byCapacity'.
 * Тип связи можно установить с помощью {@link Controls/dateRange:RelationButton RelationButton}.
 * @default true
 */
