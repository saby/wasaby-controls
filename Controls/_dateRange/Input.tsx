...
interface IDateRangeInputOptions
...
export default class Input extends React.Component<IDateRangeInputOptions> {
    ...
    private _endValueFieldRef: React.RefObject<HTMLDivElement> =
        React.createRef();
    private _startValueFieldRef: React.RefObject<HTMLDivElement> =
        React.createRef();
        ...
        if (
            ...
            this._rangeModel.update({
                ...
                endValueChangedCallback:
                    this._endValueChangedCallback.bind(this),
                rangeChangedCallback: this._rangeChangedCallback.bind(this),
            });
        }
    ...
    protected _startFieldInputControlHandler(event: SyntheticEvent): void {
        const [_event, value, displayValue, selection] = getArgs(event);
    ...
    protected _inputCompletedStartValueHandler(
        event: Event,
        value: Date
    ): void {
        this._rangeModel.startValue = value;
    ...
    private _updateStartValueValidators(validators?: Function[]): void {
        const startValueValidators: Function[] =
            validators || this.props.startValueValidators;
    ...
    private _updateEndValueValidators(validators?: Function[]): void {
        const endValueValidators: Function[] =
            validators || this.props.endValueValidators;
    ...
    private _loadDependencies(): Promise<unknown> {
        try {
            if (!this._loadCalendarPopupPromise) {
                this._loadCalendarPopupPromise = import(
                    'Controls/datePopup'
                ).then((datePopup) => {
    ...
    private _onStartTagClick(): void {
        if (this.props.onStarttagclick) {
            this.props.onStarttagclick(...arguments);
        }
    }

    private _onEndTagClick(): void {
        if (this.props.onEndtagclick) {
            this.props.onEndtagclick(...arguments);
        }
    }

    private _onStartTagHover(): void {
        if (this.props.onStarttaghover) {
            this.props.onStarttaghover(...arguments);
        }
    }

    private _onEndTagHover(): void {
        if (this.props.onEndtaghover) {
            this.props.onEndtaghover(...arguments);
        }
    }
            ...
            >
                <div className="controls-Input-DateRange__inputs">
                    <BaseInput
                        ...
                        calendarButtonVisible={this.props.calendarButtonVisible}
                        onValueChanged={this._startValueChangedHandler.bind(
                            this
                        )}
                        customEvents={[
                            'onMouseDown',
                        ...
                        onInputControl={this._startFieldInputControlHandler.bind(
                            this
                        )}
                        onInputCompleted={this._inputCompletedStartValueHandler.bind(
                            this
                        )}
                        onTagClick={this._onStartTagClick.bind(this)}
                        onTagHover={this._onStartTagHover.bind(this)}
                    ></BaseInput>
                ...
                <div className="controls-Input-DateRange__dash"></div>
                <div className="controls-Input-DateRange__inputs">
                    <BaseInput
                        forwardedRef={this.props.forwardedRef}
                        ...
                        onMouseDown={this._inputMouseDownHandler.bind(this)}
                        onInputControl={this._endFieldInputControlHandler.bind(
                            this
                        )}
                        onInputCompleted={this._inputCompletedEndValueHandler.bind(
                            this
                        )}
                        onTagClick={this._onEndTagClick.bind(this)}
                        onTagHover={this._onEndTagHover.bind(this)}
                    ></BaseInput>
                    {this.props.rightFieldTemplate ? (
                        <this.props.rightFieldTemplate
                            value={this._rangeModel.endValue}
