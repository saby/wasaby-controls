...
const ModuleClass = cExtend.extend([ObservableMixin.prototype, VersionableMixin], {
    ...
    constructor: function DateRangeModel(cfg?: { dateConstructor?: DateTime }) {
    ...
    _getDisplayedRange(range: [Date, Date], direction: number): [Date, Date] {
        ...
        if (this._options.displayedRanges[arrayIndex + direction]) {
            ...
            return [new Date(year, startValueMonth, 1), new Date(year, endValueMonth, 0)];
        }
        return range;
    },
