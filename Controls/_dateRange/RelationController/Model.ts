...
class ModuleClass {
    ...
    private _autoRelation(
        ...
        if (this._rangeIsNotEmpty(ranges[changedRangeIndex])) {
            oldPeriodType = getPeriodType(ranges[changedRangeIndex][0], ranges[changedRangeIndex][1]);
        }
        ...
        if (this._bindType === 'normal' && (periodType === 'year' || periodType === 'years')
        ...
        const updatedPeriodType = getPeriodType(
            updatedStartValue,
            updatedEndValue
        );
    ...
    private _updateRelation(
        ...
        if (
            updatedPeriodType === periodTypes.year ||
            updatedPeriodType === periodTypes.years ||
            (!capacityChanged &&
                updatedStartValue?.getFullYear() !==
    ...
    private _updateSteps(dateRanges: TRange[]): void {
        ...
        for (let i = 0; i < dateRanges.length - 1; i++) {
            ...
            if (
                this._rangeIsNotEmpty(currentRange) &&
                this._rangeIsNotEmpty(nextRange)
            ) {
                this._steps[i] = this._getMonthCount(
                    currentRange[0],
                    nextRange[0]
                );
            }
    ...
    private _periodTypeIsDay(periodType: periodTypes): boolean {
        return periodType === periodTypes.day || periodType === 'days';
    }

    private _periodTypeIsYears(periodType: periodTypes): boolean {
        return (
            periodType === periodTypes.year || periodType === periodTypes.years
        );
    }
    ...
    private _getUpdatedRanges(
        ...
        const getStep = (value: number): number => {
            ...
            if (
                (!(
                    this._periodTypeIsDay(periodType) &&
                    this._periodTypeIsYears(oldPeriodType)
                ) &&
                    bindType === 'byCapacity') ||
                (capacityChanged &&
        ...
        const oldPeriodType =
            oldStart && oldEnd
                ? getPeriodType(oldStart, oldEnd) || 'days'
                : null;
        ...
        if (this._periodTypeIsDay(periodType)) {
            ...
            periodLength = periodType
                ? Range.getPeriodLengthInMonths(start, end)
                : null;
        }

        if (
            this._periodTypeIsDay(periodType) &&
            this._periodTypeIsDay(oldPeriodType)
        ) {
            capacityChanged =
                periodLength !== Range.getPeriodLengthInDays(oldStart, oldEnd) && this._autoRangeChange;
        } else {
            capacityChanged = this._getPeriodChanged(
                periodType,
                oldPeriodType,
                ranges,
                selectionType
        ...
        if (!capacityChanged && this._isAdjacentPeriod(respRanges)) {
    ...
    private _isAdjacentPeriod(ranges: Date[][]): boolean {
        if (ranges[0] && ranges[0][1] && ranges[1] && ranges[1][0]) {
            ...
            return dateUtils.isDatesEqual(startDate, adjacentValue);
        }
        return false;
    }
    ...
    protected _getPeriodChanged(
        ...
        if (!oldPeriodType) {
            if (oldRanges[0][0] && oldRanges[0][1]) {
                adjacentPeriodType = getPeriodType(
                    oldRanges[0][0],
                    oldRanges[0][1]
                ) || 'days';
            }
