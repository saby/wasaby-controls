...
class ModuleClass {
    ...
    private _isRangesSameLength(ranges: Date[][], periodType: string, oldPeriodType: string): boolean {
        const [firstRange, secondRange] = ranges;
        if (this._periodTypeIsDay(periodType) && this._periodTypeIsDay(oldPeriodType)) {
            return Range.getPeriodLengthInDays(firstRange[0], firstRange[1]) ===
                                                          Range.getPeriodLengthInDays(secondRange[0], secondRange[1]);
        } else {
            return Range.getPeriodLengthInMonths(firstRange[0], firstRange[1]) ===
                                                          Range.getPeriodLengthInMonths(secondRange[0], secondRange[1]);
        }
    }

    private _isAdjacentPeriod(ranges: Date[][], periodType: string, oldPeriodType: string): boolean {
