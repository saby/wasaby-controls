...
class ModuleClass {
    ...
    // Добавляли логику вот тут https://online.sbis.ru/opendoc.html?guid=aa36c727-bbe4-40be-ac23-94ff18fc87a9&client=3
    // Не подошла, возможно когда-то пригодится

    // validateRanges(): void {
    //     if (this.ranges) {
    //         let highestRangeTypeInMonths = 0;
    //         let highestRangeIndex;
    //         let stopValidation = false;
    //         const getIsHighestPeriod = (range, index) => {
    //             const daysInMonth = dateUtils.getDaysInMonth(range[0]);
    //             const periodLengthInDays = Range.getPeriodLengthInDays(range[0], range[1]);
    //             let periodLengthInMonths = 0;
    //             if (daysInMonth <= periodLengthInDays) {
    //                 periodLengthInMonths = Range.getPeriodLengthInMonths(range[0], range[1]);
    //             }
    //             const isHighestPeriod = periodLengthInMonths >= highestRangeTypeInMonths;
    //             if (isHighestPeriod) {
    //                 highestRangeTypeInMonths = periodLengthInMonths;
    //                 highestRangeIndex = index;
    //             }
    //             return isHighestPeriod;
    //         };
    //         // Проходимся по всем элементам и находим самый большой период
    //         this.ranges.forEach((range, index) => {
    //             if (range[0] === null || range[1] === null) {
    //                 stopValidation = true;
    //                 return;
    //             }
    //             getIsHighestPeriod(range, index);
    //         });
    //         if (stopValidation) {
    //             return;
    //         }
    //         this.ranges.forEach((range, index) => {
    //             if (!getIsHighestPeriod(range, index)) {
    //                 const displayedRanges = [this.ranges[highestRangeIndex]];
    //                 if (
    //                     dateUtils.hitsDisplayedRanges(range[0], displayedRanges) ||
    //                     dateUtils.hitsDisplayedRanges(range[1], displayedRanges)
    //                 ) {
    //                     if (highestRangeIndex !== undefined && highestRangeIndex !== index) {
    //                         this.ranges[index] = this.ranges[highestRangeIndex];
    //                     }
    //                     const delta = highestRangeIndex > index ? -1 : 1;
    //                     this.ranges[index] = Range.shiftPeriod(
    //                         this.ranges[index][0],
    //                         this.ranges[index][1],
    //                         delta
    //                     );
