 ...
 */
import { controller as localesController } from 'I18n/i18n';
...
let localeCode = localesController.currentLocaleConfig.code;
...
const getDayRange = (startDate: Date, endDate: Date, quantum: number): [Date, Date] => {
...
const Utils = {
     ...
     */
    getWeekdaysCaptions() {
        if (!weekdaysCaptions || localeCode !== localesController.currentLocaleConfig.code) {
            localeCode = localesController.currentLocaleConfig.code;
            const calendarEntities = localesController.currentLocaleConfig.calendarEntities;
            const isEnCode = localeCode.split('-')[0] === 'en';
            const daysSmall = [...calendarEntities.minDays];
            if (!isEnCode) {
                daysSmall.forEach((value, index) => {
                    daysSmall[index] = value.toLowerCase();
                });
            }
     ...
     * @returns {*}
     */
    formatDateRangeCaption(startValue: Date, endValue: Date, emptyCaption?: string): string {
    ...
    getWeeksArray(date: Date, mode: WEEKS_MODE, dateConstructor: Function = WSDate): Date[][] {
        ...
        const weeksInMonth: number =
            mode === WEEKS_MODE.extended ? 6 : this.getWeeksInMonth(year, month);
    ...
    updateRangeByQuantum(baseDate: Date, date: Date, quantum: object): [Date, Date] {
        ...
        if ('years' in quantum && quantum.years.length === 0) {
            lastQuantumType = 'years';
            if (baseDate < date) {
                start = DateUtil.getStartOfYear(baseDate);
                end = DateUtil.getEndOfYear(date);
            } else {
                start = DateUtil.getStartOfYear(date);
                end = DateUtil.getEndOfYear(baseDate);
            }
            return [start, end];
        }
