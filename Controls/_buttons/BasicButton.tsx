...
import { IComponentPropsWithReadonly, TFontSize } from 'Controls/interface';
 ...
 * @public
 */
export interface IBasicButtonProps extends IComponentPropsWithReadonly {
    /**
     * Контент кнопки
     */
    children: ReactNode;
...
function Button(props: IBasicButtonProps, ref: Ref<HTMLElement>) {
    ...
    const buttonType =
        viewMode !== 'empty'
            ? viewMode === 'link' || viewMode === 'linkButton'
                ? 'linkButton'
                : 'button'
            : 'empty';
    ...
    let className =
        ...
        ` controls-${buttonType}-style controls-button_${viewMode}-style ${
            buttonSize ? `controls-button_size-${buttonSize}` : ''
        }`;
    ...
    return (
        <FocusRoot
            {...useDataAttrs(props as unknown as Record<string, unknown>)}
