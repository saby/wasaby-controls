...
// Сейчас многие используют этот метод, и найти все вхождения затратно.
// Используется в кнопке меню для задания _offsetClassName
...
// Можно будет удалить когда удалим опцию contrastBackground.
// Сейчас из-за совместимости не можем удалить, так как есть места где не задают viewMode,
// но передают contrastBackground ожидая что кнопка станет filled
...
// Необходимо для совместимости. Раньше опция принимала boolean, а сейчас строку.
// Сами поправить всех прикладников не можем, поэтому обрабатываем и boolean тип
...
// Утилита используется во множестве мест, и вычисленные значения используются не только для кнопок.
export function simpleCssStyleGeneration(
    ...
    // Выравнивание текста в кнопке пока что не на css переменных.
    if (
        !['filled', 'filled-same', 'outlined', 'outlined-same', 'ghost'].includes(
            this._viewMode as string
        )
    ) {
    ...
    if (options.icon || options.iconTemplate) {
        ...
        // Если указана контрасная иконка, то игнорируем это зачение.
        // Сейчас это самый безопасный вариант. Потом нужно придумать решение получше
        this._iconStyle = options.readOnly
            ? 'readonly'
            : options.iconStyle === 'contrast' &&
              this._viewMode === 'filled' &&
...
export function getDefaultOptions(): object {
    return {
        // Не удалить, так как могут задать buttonStyle, и тогда не соберется нужный класс с переменными
        viewMode: 'outlined',
        // Есть места где указали inlineHeight=xs и ожидают что размер текста будет m.
        // На подобную логику завязались например в журнале длительных операций
        fontSize: 'm',
        // Есть места где завязались на текущую логику, из-за чего задавая какой-то buttonStyle, ожидают что иконка будет secondary.
        // Но сейчас мы сами явно задаем нужный цвет иконки в зависимости от viewMode и buttonStyle
        // /DemoStand/app/Controls-Lists-editors-demo/ColumnEditor/Index
        iconStyle: 'secondary',
        // Есть места, где задают размер кнопки m, и не указывают размеры иконки.
        // Сейчас в большинстве случаев для кнопки размера m должна быть иконка размера s. Но есть места, где для размера m нужно оставить размер иконки m.
        // /DemoStand/app/Controls-Lists-editors-demo/ColumnEditor/Index
        // Подобных мест может быть довольно много
        // Также много мест где передают кастомный шаблон для иконки, и на него навешивают классы icon-size
        // и не пользуются опцией iconSize из-за чего в этих местах также поломается отображение иконки.(но проблема уже не из-за значения по умолчанию см &__icon в стилях)
        // /DemoStand/app/Engine-demo%2FNewViewConfiguration%2FIndex
        iconSize: 'm',
        textAlign: 'center',
        captionPosition: 'end',
        translucent: 'none',
 ...
 * Полезные ссылки:
 * <ul>
 *     <li>{@link /materials/DemoStand/app/Controls-demo%2FButtons%2FIndex демо-пример}</li>
 *     <li>{@link /doc/platform/developmentapl/interface-development/controls/input-elements/buttons-switches/new-buttons/ руководство разработчика}</li>
 *     <li>{@link https://git.sbis.ru/saby/wasaby-controls/-/blob/rc-24.6100/Controls-default-theme/variables/_buttons.less переменные тем оформления}</li>
 * </ul>
 * Если вы хотите использовать бледные кнопки, то необходимо самостоятельно подгрузить css-файл стилей. Сделать это можно следующим образом:
 ...
 * @demo Controls-demo/Buttons/Same/Index
 */
...
function Button(props: IButtonOptions, btnRef: LegacyRef<HTMLElement>) {
                ...
                loadFontWidthConstants().then(() => {
                    ...
                    if (captionRef.current) {
                        ...
                        const captionWidth = Math.floor(
                            // @ts-ignore
                            getFontWidth(props.caption, props.fontSize || 'm')
                        );
                        if (
    ...
    const fontColorStyle =
        state._viewMode === 'filled' &&
        props.fontColorStyle === 'contrast' &&
        state._buttonStyle === 'primary'
            ? 'empty'
