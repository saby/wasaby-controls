...
export function simpleCssStyleGeneration(props: IButtonOptions, context = {}): void {
    ...
    if (this._translucent !== 'none') {
        this._fontColorStyle =
            this._translucent === 'dark' ? 'forTranslucent' : 'forTranslucent_light';
    } else {
    ...
    if (!['filled', 'filled-same', 'outlined', 'ghost'].includes(this._viewMode)) {
...
function Button(options: IButtonOptions, btnRef) {
    ...
    const captionRef = React.useRef<HTMLElement>();

    const state = React.useMemo<IStateOptions>(() => {
    ...
    const [tooltip, setTooltip] = React.useState(state._tooltip);
