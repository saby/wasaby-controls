...
interface IBaseButtonOptions extends IControlOptions, TInternalProps, IControlProps {
    ...
    onTouchStart?: (event: React.SyntheticEvent<HTMLElement, TouchEvent>) => void;
    onMouseDown?: (event: React.SyntheticEvent<HTMLElement, MouseEvent>) => void;
    onMouseEnter?: (event: React.SyntheticEvent<HTMLElement, MouseEvent>) => void;
    onMouseOver?: (event: React.SyntheticEvent<HTMLElement, MouseEvent>) => void;
    onMouseMove?: (event: React.SyntheticEvent<HTMLElement, MouseEvent>) => void;
    onMouseLeave?: (event: React.SyntheticEvent<HTMLElement, MouseEvent>) => void;
    onKeyPress?: (event: React.SyntheticEvent<HTMLElement, KeyboardEvent>) => void;
    onKeyDown?: (event: React.SyntheticEvent<HTMLElement, KeyboardEvent>) => void;
}

function getShadowSize(height: string): string {
    return height && !['xs', 's', 'm', 'l', 'xl', '2xl', 'default'].includes(height)
        ? 'big'
        : 'small';
}
...
export default React.forwardRef(function ButtonBase(
    ...
    const wasabyRef = ref || props.$wasabyRef || props._options?.$wasabyRef;
    ...
    return props._options?.href ? (
        ...
        <FocusRoot
            ...
            style={style}
            data-qa={attrs['data-qa'] || props['data-qa']}
            autofocus={attrs['ws-autofocus'] || props['ws-autofocus']}
            as="span"
            ref={wasabyRef}
            className={buttonWrapperClass}
            title={title}
