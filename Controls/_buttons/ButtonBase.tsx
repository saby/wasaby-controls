...
interface IBaseButtonOptions extends IComponentPropsWithReadonly, TInternalProps {
...
function getIconTemplate(props: IBaseButtonOptions): ReactElement | null {
    ...
    if (props._iconTemplate) {
        ...
        return (
            <Icon
                className={`controls-BaseButton__icon ${className}`}
                icon={props._icon}
                iconSize={props._iconSize}
...
export default forwardRef(function ButtonBase(
    ...
    let buttonWrapperClass =
        (props._fontColorStyle
            ...
            : '') +
        ` ${getCircleClass(props)} controls-fontsize-${props._fontSize} controls-button_fontsize-${
            props._fontSize
        }`;
    ...
    if (
        detection.isIE ||
        (detection.chrome && detection.chromeVersion <= 110) ||
        (detection.isWin7 && detection.yandex)
    ) {
        buttonWrapperClass += ' controls-button-old-browser';
    }
            ...
            buttonStyle={
                buttonType === 'linkButton' && !readOnly
                    ? // todo для совместимости, сейчас много мест где задают
                      ...
                      // в надежде что кнопка будет primary
