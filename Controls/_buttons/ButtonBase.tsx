...
function getButtonTemplatePadding(props: IBaseButtonOptions): string {
    if (
        ...
        return ` controls-button_padding-${props._height}`;
    }
    return '';
}
...
export default React.forwardRef(function ButtonBase(
    ...
    const readOnly = useReadonly(props) || props._options?.readOnly;
    const buttonType =
    ...
    let buttonWrapperClass =
        ...
        ` controls-BaseButton controls-${buttonType}-style controls-button_${props._viewMode}-style controls-${buttonType}_size-${props._height}` +
        ` controls-${buttonType}_fontColorStyle-${props._fontColorStyle}-style` +
        `${props._toggled ? ' controls-Button_toggled' : ''}` +
        ` ${getCircleClass(props)} controls-button_fontsize-${
            props._fontSize
    ...
    if (isTranslucentStyle(props._translucent) && !readOnly) {
        ...
        buttonWrapperClass += ` controls-button_mode-${
            props._viewMode === 'ghost' ? 'ghost' : 'button'
        }_style-translucent-${translucentStyle}`;
    } else {
        buttonWrapperClass += ` controls-${buttonType}_${props._viewMode}-${
            buttonType === 'linkButton' ? props._fontColorStyle : props._buttonStyle
        }-style${props._toggled ? '_toggled' : ''}`;
    }
    if (props._hasIcon) {
        buttonWrapperClass += ` controls-button_icon-style-${
            readOnly ? 'readonly' : props._iconStyle
        }`;
        if (
            !readOnly &&
    ...
    buttonWrapperClass += ` ${props.className}`;
