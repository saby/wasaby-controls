 ...
 * @variant adaptive Записи распределяются по колонкам с учетом их заполненности.
 */
export type ColumnsMode = 'auto' | 'fixed' | 'adaptive';
...
export interface IColumnsView {
    ...
    /**
     * @name Controls/_columns/interface/IColumnsView#itemHeightCallback
     * @cfg {Function} Коллбек для определения высоты записи. Используется для равномерного распределения записей при серверной верстке.
     * @default auto
     * @see Controls/_columns/interface/IColumnsView#columnsMode
     */
    itemHeightCallback: Function;
    ...
    /**
     * Автоматический пересчёт распределения элементов по колонкам при изменении коллекции (удалении, перемещении, добавлении).
     * В значении true при любом изменении будет перераспределение. (Например: удалили запись - изменилось положение последующих записей). Рекомендуется использовать, когда записи одной высоты и значительного визуального перестроения не будет.
     * В значении false записи будут по возможности сохранять колонку (например: удалили запись - записи в столбце сдвинулись вверх, в остальных столбцах записи остались на месте). Рекомендуется, когда записи могут быть разной высоты и перерисовка всего списка приведет к визуальной путанице.
     * @name Controls/_columns/interface/IColumnsView#autoColumnsRecalculating
     * @cfg
     * @default false
     * @demo Controls-demo/list_new/ColumnsView/VirtualScroll2/Index
     * @example
     * <pre class="brush: html">
     * <!-- WML -->
     * <Controls.columns:View source="{{_viewSource}}
