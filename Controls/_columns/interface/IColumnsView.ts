...
export interface IColumnsView {
    ...
    /**
     * Включает глобальный пересчет принадлежности элементов колонкам при изменении коллекции (при удалении, перемещении, добавлении записей).
     * В значении true при изменении коллекции запииси смещаются во всех колонках. Рекомендуется использовать, когда записи одной высоты и значительного визуального перестроения не будет.
     * В значении false при изменении коллекции запииси смещаются в рамках своей колонки. Рекомендуется, когда записи могут быть разной высоты и перерисовка всего списка приведет к визуальной путанице. Важно заметить, что в этом режиме нет строгой зависимости принадлежности записи колонке от положения записи в коллекции.
