...
export default class ColumnsVirtualScrollController {
    ...
    afterRenderListControl(): void {
        // isItemsContainersAlive сделано ввиду особенностей react - в некоторых случаях множественные изменения дочерних
        // элементов приводят к вызову react'ом remount, а не update-стратегии.
        // Это приводит к тому, что itemsContainer становится unmounted от DOM и требуется его актуализация.
        // https://online.sbis.ru/opendoc.html?guid=e431e58d-dc00-4cbd-9c7b-74919e86bd54
        if (
            this._scheduleUpdateContainers ||
            (this._columnControllers && this._columnControllers?.[0].isItemsContainersAlive())
        ) {
    ...
    viewportResized(viewportSizes: ScrollControllerLib.IViewportSize): void {
        this._viewportSize = viewportSizes.height;
        Object.values(this._columnControllers).forEach((it) => {
            return it.viewportResized(viewportSizes);
        });
    }
