...
export default class ColumnsVirtualScrollController {
    ...
    constructor(options: IListVirtualScrollControllerOptions) {
        ...
        this._handleColumnCountChanges =
            this._handleColumnCountChanges.bind(this);
        if (this._collection) {
    ...
    resetItems(): void {
        ...
        const pageSizeInColumn = this._options.virtualScrollConfig.pageSize
            ? Math.ceil(
                  this._options.virtualScrollConfig.pageSize /
                      this._collection.getColumnsCount()
              )
            : undefined;
        for (
            let columnIndex = 0;
            columnIndex < this._collection.getColumnsCount();
            columnIndex++
        ) {
            this._columnControllers[columnIndex] =
                new ColumnVirtualScrollController({
                    ...this._options,
    ...
    addItems(
        ...
        const addedItemsByColumns: Record<
            number,
            { count: number; position: number }
        > = {};

        const addedItems = this._collection
            .getItems()
        ...
        addedItems.forEach((item) => {
            ...
            if (addedItemsByColumns.hasOwnProperty(column)) {
                ...
                addedItemsByColumns[column] = {
                    count: 1,
                    position:
                        this._collection.getIndexInColumnByIndex(
                            itemCollectionIndex
                        ),
                };
            }
    ...
    moveItems(
        ...
        if (prevColumn !== undefined && column !== prevColumn) {
            const movedItemIndex =
                this._collection.getIndexInColumnByIndex(addPosition);
    ...
    afterRenderListControl(): void {
        ...
        const [firstRenderedGroup, lastRenderedGroups] = this._collection
            ...
            .filter((it) => {
                return (
                    this._collection.isFirstGroup(it) ||
                    this._collection.isLastGroup(it)
                );
            });
        if (
    ...
    virtualScrollPositionChange(
        ...
        Object.values(this._columnControllers).forEach((it) => {
            return it.virtualScrollPositionChange(
                position,
                applyScrollPositionCallback
            );
        });
    }
    ...
    scrollToItem(
        ...
        const controllerWithElement = Object.values(
            this._columnControllers
        ).find((it) => {
        ...
        return controllerWithElement.scrollToItem(
            key,
            position,
            force,
            selfScroll
