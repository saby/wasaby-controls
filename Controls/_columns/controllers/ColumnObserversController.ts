...
export class ColumnObserversController extends AbstractObserversController {
    ...
    protected _getTriggerPosition(direction: IDirectionNew, contentSizeBeforeList: number): number {
        let position = super._getTriggerPosition(direction, contentSizeBeforeList);
        if (direction === 'backward') {
