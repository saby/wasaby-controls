...
export default class ColumnVirtualScrollController extends ListVirtualScrollController<IOptions> {
    ...
    protected _scheduleScroll(scrollParams: IScheduledScrollParams): void {
        if (
            ...
            let placeholderInDOM = this._collection.getPreviousColumnPlaceholder(this._column);
            if (typeof placeholderInDOM !== 'number') {
                placeholderInDOM = this._collection.getColumnPlaceholder(this._column);
            }
            scrollParams.params = {
                ...scrollParams.params,
