...
type TColumnsShadowVisibility = Extract<TShadowVisibility, 'hidden' | 'visible'>;
...
export default class CollectionItem<T extends Model = Model> extends TreeItem<T> {
     ...
     */
    getItemPaddingClasses(isAdaptive: boolean): string {
        let classes = '';
        if (!isAdaptive) {
            ...
            classes += ' controls-ColumnsView__item_spacingTop_is-adaptive';
            classes += ' controls-ColumnsView__item_spacingBottom_is-adaptive';
        }
    ...
    getContentPaddingClasses(contentPadding: IItemPadding = {}): string {
        ...
        classes += ` controls-padding_bottom-${contentPadding.bottom || 'null'}`;
    ...
    getWrapperClasses(
        ...
        const itemPaddingClasses = this.getItemPaddingClasses(isAdaptive);
        if (result.indexOf(itemPaddingClasses) === -1) {
            result += ` ${itemPaddingClasses}`;
        }
    ...
    getContentClasses(shadowVisibility: TColumnsShadowVisibility = 'visible'): string {
        let classes: string = '';
    ...
    getImageUrl(): string {
        const baseUrl =
            object.getPropertyValue<string>(this.contents, this._$imageProperty) ||
    ...
    getItemActionClasses(itemActionsPosition: string, viewMode?: string): string {
        let className = `controls-ColumnsView__itemActionsV_${itemActionsPosition}`;
        if (viewMode === 'filled') {
            className +=
    ...
    getItemActionPositionClasses(
        ...
        const itemActionClass =
            templateItemActionClass || 'controls-itemActionsV_position_bottomRight';
