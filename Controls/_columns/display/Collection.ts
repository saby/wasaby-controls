...
> extends Tree<S, T> {
    ...
    private _recalculateColumnsCountByWidth(
        ...
        const newColumnsCount = Math.floor(
            (width - this.getSideSpacing() + this.getSpacing()) / (columnMinWidth + this.getSpacing())
        );
    ...
    getSideSpacing(): number {
        return SPACING[this._$leftContainerPadding || this._$leftPadding]
            + SPACING[this._$rightContainerPadding || this._$rightPadding];
    }

    protected _setItemsContainerPadding(
        itemsContainerPadding: IItemPadding,
    ): void {
    ...
    private getColumnsSnapshotByItem(
        ...
        if (groupFunction) {
            ...
            const groupId = typeof contents === 'string' ?
                contents :
                groupFunction(contents, itemIndex, item);
