...
export interface IColumnsOptions<S extends Model = Model, T extends CollectionItem = CollectionItem>
        ...
        if (action === IObservable.ACTION_ADD) {
            ...
            if (!this._$autoColumnsRecalculating) {
                newItems.forEach((item, index) => {
                    ...
                    const indexRelativeToGroup = Math.max(
                        0,
            ...
            if (draggedItem) {
                ...
                if (position.position !== 'on') {
                    if (!this._$autoColumnsRecalculating) {
                        ...
                        const curIndex = draggedItem.index;
