...
> extends Tree<S, T> {
    ...
    getItemsCountInColumns(column: number): number {
        if (this._groupsColumnsSnapshot && Object.keys(this._groupsColumnsSnapshot).length) {
            let count = 0;
            Object.keys(this._groupsColumnsSnapshot).forEach((groupKey) => {
                count += this._groupsColumnsSnapshot[groupKey].columns[column]?.length || 0;
            });
            return count;
        } else {
    ...
    getMinBackwardColumnPlaceholder(): number {
        ...
        return placeholders.reduce((min, placeholders) => {
            ...
            return resPlaceholders.backward < min ? resPlaceholders.backward : min;
        }, Number.MAX_VALUE);
    }
    ...
    getIndexInColumnByIndex(itemCollectionIndex: number, collectionItem?: CollectionItem): number {
        ...
        let indexInColumn = snapshot.columns[column]?.indexOf(itemCollectionIndex) || 0;
            ...
            return (
                ...
                columnsSnapshot.columns.find((columnSnapshot, columnIndex) => {
                    ...
                    return columnSnapshot.find((columnItemCollectionIndex) => {
                        const columnItemIndex =
                            this.getIndexInColumnByIndex(columnItemCollectionIndex);
                        return (
    ...
    getItemToRight(item: T): T {
        ...
        if (curColumn < this._$columnsCount - 1) {
            const nextColumn = snapshot.columns.find((col: number[], index: number) => {
