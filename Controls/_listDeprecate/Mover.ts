...
import {
    ...
    IMoveItemsParams,
} from './interface/IMoverAndRemover';
...
const _private = {
        ...
        return _private
            ...
            .then((beforeItemsMoveResult) => {
                if (
                    ...
                    BEFORE_ITEMS_MOVE_RESULT.MOVE_IN_ITEMS
                ) {
                    return _private.moveInItems(self, items, target, position);
                } else if (
                    beforeItemsMoveResult !== BEFORE_ITEMS_MOVE_RESULT.CUSTOM
                ) {
                    return _private
    ...
    beforeItemsMove(self, items, target, position) {
        const beforeItemsMoveResult = self._notify('beforeItemsMove', [
            items,
            target,
            position,
        ]);
    ...
    moveItemToSiblingPosition(self, item, position) {
        const direction =
            position === LOCAL_MOVE_POSITION.Before
                ? MOVE_DIRECTION.UP
                : MOVE_DIRECTION.DOWN;
        const selectedItem = item.getKey
            ? item
            : self._items.getRecordById(item);
    ...
    updateDataOptions(self, newOptions, contextDataOptions) {
        ...
        if (contextDataOptions) {
            controllerOptions.source =
                newOptions.source || contextDataOptions.source;
            self._keyProperty =
                newOptions.keyProperty || contextDataOptions.keyProperty;
        ...
        return new Promise((resolve, reject) => {
            new MoveWithDialogAction({
                ...
                popupOptions: {
                    ...
                    beforeMoveCallback: (
                        resultSelection: ISelectionObject,
                        target: Model
                    ) => {
                        _private
                            .moveItems(
...
const Mover = BaseAction.extend({
    ...
    moveItemUp(item) {
        return _private.moveItemToSiblingPosition(
            this,
            item,
    ...
    moveItemsWithDialog(items: [] | IMoveItemsParams): Promise<any> {
        ...
        this._useDefaultMoveMethod = !items.selected;
        this._moveDialogTemplate =
            this._options.moveDialogTemplate.templateName;
        this._moveDialogOptions =
            this._options.moveDialogTemplate.templateOptions;
        if (this.validate(items)) {
            return _private
                .getItemsBySelection(this, items)
                .then((selectItems: []) => {
                    const selection = _private.prepareSelection(selectItems);
                    return _private.moveWithDialog(
                        this,
                        selection,
                        this._filter
                    );
