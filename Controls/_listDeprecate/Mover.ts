 ...
 */
import { BaseAction, IListActionAdditionalConfig } from 'Controls/list';
...
//   Кроме того, ломается обращение к контролу через children
...
const _private = {
    ...
    moveWithDialog(
        ...
        if (!config?.target?.closest) {
            const menuRow = config?.target?.closest?.('.controls-Menu__row');
            if (menuRow) {
                const rect = menuRow.getBoundingClientRect();
                config.target.left = rect.left;
                config.target.top = rect.top;
                config.target.width = rect.width;
                config.target.height = rect.height;
            }
        }
 ...
 *
 * @remark
 * Контрол должен располагаться в одном контейнере {@link Controls/listDataOld:DataContainer} со списком.
    ...
    moveItemsWithDialog(
        ...
        if (this.validate(items)) {
            return _private.getItemsBySelection(this, items).then((selectItems: []) => {
                ...
                return _private.moveWithDialog(this, selection, this._filter, config);
            });
        }
        return Promise.resolve();
    },
