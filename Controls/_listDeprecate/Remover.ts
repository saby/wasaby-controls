...
const _private = {
    ...
    beforeItemsRemove(self, keys) {
        const beforeItemsRemoveResult = self._notify('beforeItemsRemove', [
            keys,
        ]);
    ...
    afterItemsRemove(self, keys, result) {
        const afterItemsRemoveResult = self._notify('afterItemsRemove', [
            keys,
            result,
        ]);
    ...
    updateDataOptions(self, newOptions?: IOptions): void {
        const dataOptions = self.context.dataOptions;
        ...
        if (!self._source) {
            Logger.warn(
                "source не был задан ни в опциях Remover'а ни в контексте.",
                this
            );
        }
    },
...
const Remover = BaseAction.extend({
    ...
    removeItems(keys: string[]): Promise<void> {
        const both = (result) => {
            ...
            return _private
                .afterItemsRemove(this, keys, result)
                .then((eventResult) => {
        ...
        return _private.getItemsBySelection(this, keys).then((selection) => {
            return _private
                .beforeItemsRemove(this, selection)
                .then((result) => {
...
Remover.contextType = DataContext;
