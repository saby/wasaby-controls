...
> implements IItemsStrategy<S, T>
{
    ...
    static sortItems<S extends Model = Model, T extends CollectionItem<S> = CollectionItem<S>>(
        ...
        newNodeCompositeItemContents.forEach((compositeItemContent, index) => {
            if (compositeItemsContents.indexOf(compositeItemContent) === -1) {
                ...
                const viewMode = CompositeItems._getViewMode(options.display, type);
    ...
    private static _getViewMode(display: Collection<Model>, type: string): string {
        const nodesViewMode = display.getCompositeNodesViewMode();
        const leavesViewMode = display.getCompositeLeavesViewMode();
    ...
    private static _countNodesWithCompositeItem(
        ...
        for (let itemIndex = 0; itemIndex < items.length; itemIndex++) {
            ...
            if (item.isNode()) {
                ...
                if (CompositeItems._getViewMode(display, 'nodes') === 'tile') {
