...
> implements IItemsStrategy<S, T>
{
    ...
    private _actualizeCompositeItems(
        ...
        addedRecords.forEach((addedItem) => {
            ...
            const addedItemIsLeaf = !addedItem.get(nodeProperty);
    ...
    static sortItems<S extends Model = Model, T extends CollectionItem<S> = CollectionItem<S>>(
        ...
        newNodeCompositeItemContents.forEach((compositeItemContent, index) => {
            if (compositeItemsContents.indexOf(compositeItemContent) === -1) {
                ...
                const viewMode = CompositeItems._getViewMode(options.display, item, type);
                const compositeViewConfig =
                    options.display.getCompositeViewConfigFromCallback(item) ||
                ...
                const compositeItem = options.display.createItem({
                    ...
                    displayProperty: options.display.getDisplayProperty(),
                    parentProperty: options.display.getParentProperty(),
                    nodeProperty: options.display.getNodeProperty(),
                    keyProperty: options.display.getKeyProperty(),
                    expandedItems: options.display.getExpandedItems(),
                    collapsedItems: options.display.getCollapsedItems(),
                    multiSelectVisibility: options.display.getMultiSelectVisibility(),
                    parent: item,
                    compositeViewConfig,
                    compositeViewMode: viewMode,
                    type,
                    canShowFooter: hasCompositeItemsBothTypes
                        ? type === 'leaves' || type === 'all'
                        : true,
                    hasCompositeItemsBothTypes,
                    list,
                });
            ...
            if (options.rootItemsPositionBottom === false) {
                ...
                if (node.isRoot()) {
                    ...
                    const firstLevelFirstLeafIndex = items.findIndex((item) => {
                        return item.getLevel() === 2 && !item.isNode() && item.getParent() === node;
                    });
    ...
    private static _getCompositeChildren(
        ...
        const processItem = (listItem) => {
            ...
            // В плитке false значение выводится как листья, поэтому нужно тут тоже поддержать это поведение
            if (!nodePropertyValue) {
                childLeaves.push(listItem);
            } else {
                childNodes.push(listItem);
                options.hierarchy
                    .getChildren(listItem.getKey(), options.display.getCollection())
                    .forEach(processItem);
            }
        };
        options.hierarchy
        ...
        if (type === 'all' || type === 'leaves') {
            listItems = listItems.concat(childLeaves);
        }
        return {
    ...
    private static _getViewMode(
        display: Collection<Model>,
        item: CollectionItem<Model<any>>,
        type: string
    ): string {
        const nodesViewMode =
            display.getCompositeViewConfigFromCallback(item)?.nodesViewMode ||
            display.getCompositeNodesViewMode();
        const leavesViewMode =
            display.getCompositeViewConfigFromCallback(item)?.leavesViewMode ||
            display.getCompositeLeavesViewMode();
        if (nodesViewMode !== leavesViewMode) {
            Logger.error(
                'Controls/expandedCompositeTree:View: Указаны разные значения для nodesViewMode и leavesViewMode.' +
                    '\n Такой режим работы не поддерживается. Следует указать одно значение для указанных параметров'
            );
            return leavesViewMode;
        } else {
            return type === 'nodes' ? nodesViewMode : leavesViewMode;
        }
    }
    ...
    private static _countNodesWithCompositeItem(
        ...
        for (let itemIndex = 0; itemIndex < items.length; itemIndex++) {
            ...
            if (item.isNode()) {
                ...
                const level = this.getLevel(item.contents, display, items);
                const allowNodesItemByLevel = level >= display.getCompositeNodesLevel() - 1;
                const hasChildrenLeaves = children.some((child) => {
                    return !child.get(nodeProperty);
                });
                if (CompositeItems._getViewMode(display, item, 'nodes') === 'tile') {
                    ...
                    // TODO: Тут можно будет реализовать разделение узлов/листьев на разные виды отображения
                    if (level < display.getCompositeNodesLevel()) {
                        if (hasChildrenLeaves || (hasChildrenNodes && allowNodesItemByLevel)) {
                            nodesWithCompositeItem.push({ item, type: 'all' });
                        }
                    }
                }
            }
