...
> implements IItemsStrategy<S, T>
{
    ...
    private _separators: Map<string | number | null, RootSeparatorItem> =
        new Map();
    ...
    getCollectionIndex(index: number): number {
        ...
        let sourceIndex =
            ...
            (this._options.rootItemsPositionBottom === false
                ? 0
                : this._separators?.size);
    ...
    private _actualizeCompositeItems(
        ...
        const addedRecords: S[] = addedItems.map((item) => {
            return item['[Controls/_display/CollectionItem]']
                ? (item as T).contents
                : item;
        });
            ...
            if (compositeItem) {
                ...
                if (shouldAddByType) {
                    const sourceCollection =
                        this._options.display.getSourceCollection();
                    ...
                    if (start === 0) {
                        ...
                        const nearbyItemIndex =
                            compositeCollection.getIndex(nearbyItem);
    ...
    private _removeRootSeparators(removedItems: T[]): void {
        ...
        const getParentKey = (itemKey: CrudEntityKey) => {
            const parent = hierarchy.getParent(
                itemKey,
                sourceCollection
            ) as Model;
        ...
        removedItemsKeys.forEach((removedItemKey) => {
            const parentKey = getParentKey(removedItemKey);
            if (this._separators.has(parentKey)) {
                const childNodes = [];
                const childLeaves = [];
                hierarchy
                    .getChildren(parentKey, sourceCollection)
                    .forEach((listItem) => {
                        const nodePropertyValue = listItem.get(
                            collection.getNodeProperty()
                        );
                        const itemKey = listItem.get(
                            collection.getKeyProperty()
                        );
    ...
    protected _getItems(): T[] {
        const separators =
            ...
            this._separators?.size
                ...
                : [];
        return [
            ...(this._compositeItems as unknown as T[]),
            ...separators,
            ...this.source.items,
        ];
    }
    ...
    >(items: T[], options: ISortOptions<S, T>): number[] {
        ...
        newNodeCompositeItemContents.forEach((compositeItemContent, index) => {
            if (compositeItemsContents.indexOf(compositeItemContent) === -1) {
                ...
                if (
                    (item.getLevel() === 1 ||
                        item.key === options.display.getRoot().key) &&
        ...
        const countOfServiceItems =
            ...
            (options.rootItemsPositionBottom !== false
                ? options.separators.size
                : 0);
        ...
        options.compositeItems.forEach((compositeItem, index) => {
            ...
            if (options.rootItemsPositionBottom === false) {
                ...
                insertIndex = itemsOrder.indexOf(
                    originIndex + countOfServiceItems
                );
                itemsOrder.splice(insertIndex, 0, compositeItemIndex);
                ...
                if (
                    compositeItem.getType() === 'leaves' &&
                    options.separators.has(node.key)
                ) {
                    const separatorIndex =
                        Array.from(options.separators.keys()).indexOf(
                            node.key
                        ) + options.compositeItems.length;
