...
import { RecordSet, EnumeratorCallback } from 'Types/collection';
import { TreeTileView, TreeTileCollection, TreeTileCollectionItem } from 'Controls/treeTile';
...
>(TreeItem) {
    ...
    setDragPosition(position: IDragPosition<TreeTileCollectionItem>): void {
        ...
        if (this._renderCollection !== position.dispItem.getOwner() as unknown as TreeTileCollection) {
            return;
        }
    ...
    getPaddingClasses(): string {
        ...
        if (this._$type === 'nodes') {
            ...
            classes += ` controls-ExpandedCompositeTree-item_spacingTop_${tileTopPadding}`;
            classes += ` controls-ExpandedCompositeTree-item_spacingBottom_${tileBottomPadding}`;
        }
