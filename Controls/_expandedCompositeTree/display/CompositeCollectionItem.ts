...
import {
    CompositeFooter,
    GridCompatibleCompositeFooter,
} from 'Controls/_expandedCompositeTree/render/CompositeFooter';
...
>(TreeItem) {
    ...
    protected _renderCollection: TreeTileCollection | TreeGridCollection;
    protected _$compositeViewConfig: ICompositeViewConfig;

    protected _$compositeViewMode: TCompositeViewMode;
    ...
    private _updateTileCollection(
        oldConfig: ICompositeViewConfig,
        newConfig: ICompositeViewConfig
    ): void {
        const renderCollection = this._renderCollection as TreeTileCollection;
        if (oldConfig?.tileSize !== newConfig.tileSize) {
            renderCollection.setTileSize(newConfig.tileSize);
        }
        if (oldConfig?.tileWidth !== newConfig.tileWidth) {
            renderCollection.setTileWidth(newConfig.tileWidth);
        }
        if (oldConfig?.folderWidth !== newConfig.folderWidth) {
            renderCollection.setFolderWidth(newConfig.folderWidth);
        }
        if (oldConfig?.multiSelectVisibility !== newConfig.multiSelectVisibility) {
            renderCollection.setMultiSelectVisibility(newConfig.multiSelectVisibility);
        }
        if (!isEqual(oldConfig?.itemTemplateOptions, newConfig.itemTemplateOptions)) {
            renderCollection.setItemTemplateOptions(newConfig.itemTemplateOptions);
        }
    }

    private _updateGridCollection(
        oldConfig: ICompositeViewConfig,
        newConfig: ICompositeViewConfig
    ): void {
        const renderCollection = this._renderCollection as TreeGridCollection;
        if (!isEqual(oldConfig?.columns, newConfig.columns)) {
            renderCollection.setColumns(newConfig.columns);
        }
        if (!isEqual(oldConfig?.header, newConfig.header)) {
            renderCollection.setHeader(newConfig.header);
        }
    }
    ...
    private _updateRenderCollection(
        ...
        if (newConfig?.leavesViewMode === 'table') {
            this._updateGridCollection(oldConfig, newConfig);
        } else {
            this._updateTileCollection(oldConfig, newConfig);
        }
    }
    ...
    setCompositeViewConfig(compositeViewConfig: ICompositeViewConfig): void {
        ...
        if (
            ...
            this._initializeCollection(this.getList());
            this._nextVersion();
        } else {
    ...
    protected _getTableRenderParams(): IRenderScopeProps {
            ...
            ? [
                  {
                      ...
                      template: GridCompatibleCompositeFooter,
                  },
              ]
            : undefined;
        ...
        return {
            ...
            collection: this._renderCollection,
            collectionVersion: this._renderCollection.getVersion(),
            itemActions: this._$compositeViewConfig?.itemActions,
            contextMenuConfig: this._$compositeViewConfig?.contextMenuConfig,
            itemActionsClass: this._$compositeViewConfig?.itemActionsClass,
            itemActionsPosition: this._$compositeViewConfig?.itemActionsPosition,
