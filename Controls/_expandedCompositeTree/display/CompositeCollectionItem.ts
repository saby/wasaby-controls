...
> extends mixin<TreeItem<Model>>(TreeItem) {
    ...
    eachChild(
        callback: EnumeratorCallback<TreeTileCollectionItem>,
        context?: object
    ): void {
    ...
    private _updateRenderCollection(
        ...
        if (
            oldConfig?.multiSelectVisibility !== newConfig.multiSelectVisibility
        ) {
            this._renderCollection?.setMultiSelectVisibility(
                newConfig.multiSelectVisibility
            );
        }
        if (
            !isEqual(
                oldConfig?.itemTemplateOptions,
                newConfig.itemTemplateOptions
            )
        ) {
            this._renderCollection.setItemTemplateOptions(
                newConfig.itemTemplateOptions
            );
        }
    }
