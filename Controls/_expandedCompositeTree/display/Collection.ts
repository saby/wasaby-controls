...
import { Tree, ITreeCollectionOptions, RootSeparatorItem, TreeItem } from 'Controls/baseTree';
...
> extends Tree<S, T> {
    ...
    protected _$compositeViewConfigCallback: (item: Model) => ICompositeViewConfig;
    ...
    getDeepIndex(item: T): number {
        ...
        const relatedCompositeItem = this.find((it) => {
            return (
                it['[Controls/expandedCompositeTree:CompositeCollectionItem]'] &&
    ...
    protected _initializeUserStrategies() {
        super._initializeUserStrategies({ addPseudoParentStrategy: true });
    ...
    protected _initializeFilter(userFilter?: Function[]): void {
     ...
     * @private
     */
    private _getCompositeItemByChild(item: TreeItem): CompositeCollectionItem {
    ...
    setExpandedItems(expandedKeys: CrudEntityKey[]): void {
        ...
        this._updateItemsProperty('setExpandedItems', expandedKeys, 'setExpandedItems');
    }

    setCollapsedItems(collapsedKeys: CrudEntityKey[]): void {
        super.setCollapsedItems(collapsedKeys);
        this._updateItemsProperty('setCollapsedItems', collapsedKeys, 'setCollapsedItems');
    }
    ...
    setCompositeViewConfig(compositeViewConfig: ICompositeViewConfig): void {
        if (!isEqual(this._$compositeViewConfig, compositeViewConfig)) {
            if (
                this._$compositeViewConfig.leavesViewMode !== compositeViewConfig.leavesViewMode ||
                this._$compositeViewConfig.nodesViewMode !== compositeViewConfig.nodesViewMode
            ) {
                this._$compositeViewConfig = compositeViewConfig;
                this._composer.update(CompositeItem, {});
                this._reBuild(true);
            } else {
                this._$compositeViewConfig = compositeViewConfig;
    ...
    setCompositeViewConfigCallback(
        compositeViewConfigCallback: (item: Model) => ICompositeViewConfig
    ): void {
        // Проверка, является ли айтем CompositeCollectionItem`ом
        const isCompositeCollectionItem = (
            item: CollectionItem<S> | CompositeCollectionItem<Model<any>>
        ): item is CompositeCollectionItem<Model<any>> => 'setCompositeViewConfig' in item;

        this._getItems().forEach((item) => {
            if (!isCompositeCollectionItem(item)) {
                return;
            }

            this._$compositeViewConfigCallback = compositeViewConfigCallback;
            item.setCompositeViewConfig(compositeViewConfigCallback(item.getParent()));
        });

        this._nextVersion();
    }

    getCompositeViewConfigFromCallback(item: Model): ICompositeViewConfig | undefined {
        return this._$compositeViewConfigCallback?.(item);
    }

    getCompositeNodesViewMode(): TCompositeViewMode {
        return this._$compositeViewConfig.nodesViewMode || 'tile';
    }

    getCompositeLeavesViewMode(): TCompositeViewMode {
        return this._$compositeViewConfig.leavesViewMode || 'tile';
    }
...
Object.assign(Collection.prototype, {
    ...
    _$compositeViewConfigCallback: null,
