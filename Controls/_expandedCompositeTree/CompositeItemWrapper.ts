...
import { IItemEventHandlers, callWithCollectionItem } from 'Controls/baseList';
...
export default class CompositeItemWrapper extends Control<IProps> {
    ...
    protected _itemHandlers: IItemEventHandlers;
    ...
    constructor(props: IProps) {
        ...
        this._itemHandlers = {
            onSwipeCallback: (event, item) => {
                return callWithCollectionItem(item, this._getCollection(), (collectionItem) => {
            ...
            onMouseDown: (event, item) => {
                return callWithCollectionItem(item, this._getCollection(), (collectionItem) => {
                    return this._onItemMouseDown(event, collectionItem, event);
                });
            },
            onMouseUp: (event, item) => {
                return callWithCollectionItem(item, this._getCollection(), (collectionItem) => {
                    return this._onItemMouseUp(event, collectionItem, event);
                });
            },
            onMouseEnter: (event, item) => {
                return callWithCollectionItem(item, this._getCollection(), (collectionItem) => {
                    return this._onItemMouseEnter(event, collectionItem, event);
                });
            },
            onMouseMove: (event, item) => {
                return callWithCollectionItem(item, this._getCollection(), (collectionItem) => {
                    return this._onItemMouseMove(event, collectionItem, event);
                });
            },
        };
    }
    ...
    private _getCollection() {
        return this._options.renderScope.listModel || this._options.renderScope.collection;
    }

    protected _beforeMount(props: IProps): void {
        const collection = props.renderScope.listModel || props.renderScope.collection;
        collection.subscribe('onCollectionChange', this._onCollectionChanged);
    ...
    protected _onItemMouseMove(
        ...
        this._notify('itemMouseMove', [item, wasabyEvent], { bubbling: true });
    }
    ...
    protected _onItemMouseEnter(
        ...
        this._notify('itemMouseEnter', [item, wasabyEvent], { bubbling: true });
    }
    ...
    protected _onItemMouseDown(
        ...
        this._notify('itemMouseDown', [item, wasabyEvent], { bubbling: true });
    }
    ...
    protected _onItemMouseUp(
        ...
        this._notify('itemMouseUp', [item, wasabyEvent], { bubbling: true });
    }
    ...
    protected _onItemClick(
        ...
        this._notify('itemClick', [item, wasabyEvent || event], { bubbling: true });
    }
    ...
    protected _onCheckBoxClick(
        ...
        this._notify('checkBoxClick', [item, wasabyEvent], { bubbling: true });
    }
    ...
    protected _onItemContextMenu(
        ...
        if (this._getItemActionsController()) {
            this._openItemActionsMenu(null, wasabyEvent, item, true);
        }
    }
    ...
    protected _onItemSwipe(event: SyntheticEvent<Event>, item: object, wasabyEvent: Event): void {
        this._notify('itemSwipe', [item, wasabyEvent], { bubbling: true });
    }
    ...
    destroy(): void {
        ...
        const collection =
            this._options.renderScope.listModel || this._options.renderScope.collection;
        if (collection) {
            collection.unsubscribe('onCollectionChange', this._onCollectionChanged);
        }
    }
    ...
    private static _getItemActionsOptions(
        ...
        return {
            ...
            iconSize: 'm',
            menuIconSize: 'm',
            collection: props.listModel || props.collection,
            theme,
