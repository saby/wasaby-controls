...
import {
    RegisterUtil as registerUtil,
    UnregisterUtil as unregisterUtil,
} from 'Controls/event';
...
export default class CompositeItemWrapper extends Control<IProps> {
    ...
    protected _beforeMount(props: IProps): void {
        props.renderScope.listModel.subscribe(
            'onCollectionChange',
    ...
    protected _beforeUpdate(props: IProps): void {
        ...
        const shouldRecountItemActions =
            ...
            oldProps.itemActionVisibilityCallback !==
                newProps.itemActionVisibilityCallback ||
    ...
    protected _onItemMouseEnter(
        ...
        if (this._getItemActionsController() && !this._isActionsAssigned()) {
            this._recountItemActions();
        }
    ...
    protected _onCompositeItemClick(
        event: SyntheticEvent<MouseEvent>,
        item: object
    ): void {
    ...
    protected _onItemActionClick(
        ...
        if (!action.isMenu) {
            this._handleItemActionClick(action, event, item, false);
        }
    }
    ...
    private _handleItemActionClick(
        ...
        this._notify(
            'actionClick',
            [action, contents, this._targetItem, clickEvent.nativeEvent],
            { bubbling: true }
        );
        if (action.handler) {
    ...
    private _openItemActionsMenu(
        ...
        const menuConfig =
            this._getItemActionsController().prepareActionsMenuConfig(
                item,
                clickEvent,
                action,
         ...
         */
        this._targetItem = (clickEvent.target as HTMLElement).closest(
            ITEM_SELECTOR
        );
    ...
    private _itemActionsMenuResultHandler(
        ...
        if (eventName === 'itemClick') {
            ...
            if (action && !action['parent@']) {
                ...
                this._handleItemActionClick(
                    action,
                    clickEvent,
                    item as IItemActionsItem,
                    true
                );
            }
        }
    ...
    private _isActionsAssigned(): boolean {
        return (
    ...
    private _getItemActionsController(
        props: IProps = this._options
    ): ItemActionsController {
        if (
            !this._hasItemActionsController() &&
            props.renderScope &&
            (props.renderScope.itemActions?.length ||
                props.renderScope.itemActionsProperty)
        ) {
            const controllerOptions =
                CompositeItemWrapper._getItemActionsOptions(
                    props.renderScope,
                    props.theme
                );
