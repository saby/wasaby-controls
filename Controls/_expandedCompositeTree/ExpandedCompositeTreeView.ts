...
const SELECTOR_MORE_BUTTON =
    '.controls-ExpandedCompositeTree-footer_moreButton';
...
export default class ExpandedCompositeTreeView extends TreeView {
    _beforeUpdate(newOptions: any) {
        if (
            !isEqual(
                this._options.compositeViewConfig,
                newOptions.compositeViewConfig
            )
        ) {
            newOptions.listModel.setCompositeViewConfig(
                newOptions.compositeViewConfig
            );
        }
    }
