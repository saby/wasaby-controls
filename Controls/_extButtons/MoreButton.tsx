...
import { IFontSizeOptions, IComponentPropsWithReadonly } from 'Controls/interface';
...
export interface IMoreButtonOptions
    ...
    count?: number | string;
    /**
     * Callback, который вызывается при нажатии не кнопку
     */
...
export default React.forwardRef(function MoreButton(
    ...
    return (
        <div
            ...
            ref={ref}
            className={`controls-MoreButton ${props.className || attrs.className}`}
        >
            <div className={moreButtonClass}>
                <span
                    className={`controls-text-label controls-MoreButton_text-label controls-fontsize-${fontSize}`}
                >
                    {rk('Еще')} {props.count}
                </span>
            </div>
