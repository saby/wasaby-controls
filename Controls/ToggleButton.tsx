...
export interface IToggleButtonProps
    ...
    icons?: string[];
    captions?: string[];
    viewMode?: 'button' | 'link' | 'pushButton' | 'ghost';
...
const calculateState = (
    newOptions: IToggleButtonProps,
    self: IToggleButtonState
) => {
    ...
    self._caption = captions
        ...
        : '';
    self._stringCaption =
        typeof self._caption === 'string' || self._caption instanceof String;
    const clonedOptions = { ...newOptions };
    clonedOptions.icon = self._icon;
    self._iconSize = (
        self._icon ? ActualApi.iconSize(iconSize, self._icon) : ''
    ) as TIconSize;
...
const calcStyles = (
    ...
    self._iconStyle = self._icon
        ? ActualApi.iconStyle(
              iconStyle,
              self._icon,
              readOnly,
              newOptions.translucent
          )
        : '';
    ...
    const buttonStyle =
        self._viewMode === 'ghost'
            ...
            : buttonStyles[0];
    self._buttonStyle =
        (readOnly ? 'readonly' : buttonStyle) || self._buttonStyle;
    ...
    const fontColorStyle = newOptions.translucent
        ...
        ? fontColorStyles[1]
        : fontColorStyles[0];
    self._fontColorStyle =
        (readOnly ? 'readonly' : fontColorStyle) || self._fontColorStyle;
...
export default React.forwardRef(function ToggleButton(
    ...
    const [state, setState] = React.useState<IToggleButtonState>(options);
    ...
    React.useEffect(() => {
        ...
        calculateState({ ...getDefaultOptions(), ...props }, options);
        setState({ ...options });
    }, [props]);
