...
const calcStyles = (newOptions: IToggleButtonProps, self: IToggleButtonState) => {
    ...
    const fontColorStyles = newOptions.fontColorStyles || [
        newOptions.fontColorStyle,
        newOptions.fontColorStyle,
    ];
    ...
    self._fontSize = newOptions.fontSize;
...
export default React.forwardRef(function ToggleButton(
    ...
    const state = React.useMemo<IToggleButtonState>(() => {
        return getState({ ...props, readOnly });
    }, [props, readOnly]);
    ...
    const keyUpHandler = (e: SyntheticEvent<KeyboardEvent>): void => {
        if (e.nativeEvent.keyCode === constants.key.enter && !readOnly) {
            clickHandler(e);
        }
    };
    const attrs = wasabyAttrsToReactDom(props.attrs || {});
    const attrsClassName =
        `controls_toggle_theme-${useTheme(props) || 'default'} ` +
    ...
    return (
        <ButtonTemplate
            ...
            _fontColorStyle={
                state._fontColorStyle || (props.viewMode === 'link' ? 'link' : undefined)
            }
 ...
 * Полезные ссылки:
 * <ul>
 *     <li>{@link /materials/DemoStand/app/Controls-demo%2FButtons%2FStandart%2FIndex демо-пример}</li>
 *     <li>{@link https://git.sbis.ru/saby/wasaby-controls/-/blob/rc-24.6100/Controls-default-theme/variables/_toggle.less переменные тем оформления}</li>
 * </ul>
 *
 ...
 * Вторая иконка отображается, когда переключатель выключен.
...
/**
 * Первый стиль отображается, когда переключатель включен.
 * Второй стиль отображается, когда переключатель выключен.
 ...
 * @variant pushButton В виде гиперссылки, которая меняет свой внешний вид в зажатом состоянии
 * @default link
 * @example
