...
import { IItems, IItemTemplateListProps } from 'Controls/interface';
...
import { getAdaptiveModeForLoaders, withAdaptiveMode } from 'UI/Adaptive';
import { Control, TemplateFunction } from 'UI/Base';
import { isContentOption } from 'UICommon/Vdom';
import { SyntheticEvent } from 'UI/Events';
import Marker from './Buttons/Marker';
import { ITabButtonItem, ITabsButtons, ITabsButtonsOptions } from './interface/ITabsButtons';
...
export interface ITabsTemplateOptions extends IItemTemplateListProps {
 ...
 * Полезные ссылки:
 * <ul>
 *     <li>{@link https://git.sbis.ru/saby/wasaby-controls/-/blob/rc-24.6100/Controls-default-theme/variables/_tabs.less переменные тем оформления}</li>
 * </ul>
 *
    ...
    implements ITabsButtons, IItems, ITabsTemplate
{
    ...
    private _defaultFontSelectedStyle: string = 'tabs-selected';
    private _defaultFontUnselectedStyle: string = 'tabs-unselected';
    ...
    protected _beforeUpdate(newOptions: ITabsOptions): void {
        ...
        const selectedKeyChanged = newOptions.selectedKey !== this._options.selectedKey;
    ...
    protected _updateMarkerCssClass(options: ITabsButtonsOptions): void {
        ...
        const markerStyle = this._options.selectedStyle || style || this._defaultSelectedStyle;
        this._markerCssClass = `controls-Tabs__marker_style-${markerStyle} controls-Tabs__marker_thickness`;
    }

    protected _onItemMouseEnter(event: SyntheticEvent<MouseEvent>, key: string): void {
        this._notify('itemMouseEnter', [event, key]);
    }

    protected _onItemMouseLeave(event: SyntheticEvent<MouseEvent>, key: string): void {
        this._notify('itemMouseLeave', [event, key]);
    }

    protected _onItemClick(event: SyntheticEvent<MouseEvent>, key: string): void {
        if (isLeftMouseButton(event)) {
            this._notify('selectedKeyChanged', [key]);
            // selectedKey может вернуться в контрол значительно позже, если снаружи есть асинхронный код.
    ...
    protected _isAdaptiveMode(): boolean {
        return getAdaptiveModeForLoaders()?.device.isPhone() || this._options.isAdaptive;
    }

    protected _getMobilePrefix(): string {
        if (this._isAdaptiveMode()) {
            return '_mobile';
        }
        return '';
    }
    ...
    protected _getCanShrink(): boolean {
        if (typeof this._options.canShrink === 'undefined') {
            return !this._isAdaptiveMode();
        }
        return this._options.canShrink;
    }

    /*
     * Для совместимости со старым стандартом и API не даем сжиматься всем вкладкам, кроме главной,
    ...
    protected _getInlineHeight(): string {
        let inlineHeight = this._options.inlineHeight;
        if (!inlineHeight) {
            inlineHeight = this._isAdaptiveMode() ? 'l' : 's';
        }
    ...
    protected _prepareItemClass(item: ITabButtonItem, index: number): string {
        ...
        const isSelected = item[options.keyProperty] === options.selectedKey;
        const selectionStatus = isSelected ? 'selected' : 'unselected';

        const classes: string[] = [
            'controls-Tabs__item',
            `controls-Tabs__item-${selectionStatus}`,
            `controls-Tabs__item_inlineHeight-${this._getInlineHeight()}${this._getMobilePrefix()}`,
        ];
    ...
    /**
     * Если ширина задана числом, то считаем, что это пиксели, если строка - то проценты, строку отдаем как есть.
     * @param value
     * @private
     */
    ...
    protected _prepareItemStyles(
        ...
        if (width !== undefined) {
            style += `width: ${this._getWidthValue(width)}; justify-content: center;`;
            if (!canShrink) {
                style += ' flex-shrink: 0;';
            }
        }
    ...
    protected _prepareItemSelectedClass(item: ITabButtonItem, isHidden: boolean): string {
        const classes = [];
        const options = this._options;
        const style = TabsButtons._prepareStyle(options.style);
        let fontColorStyle = '';
        if (item.isMainTab) {
            classes.push('controls-Tabs__item_view_main' + this._getMobilePrefix());
        } else if (item[options.keyProperty] === options.selectedKey || isHidden) {
            fontColorStyle =
                this._options.selectedStyle ||
                this._selectedItemStyleMap[style] ||
                this._options.fontColorStyle ||
                this._defaultFontSelectedStyle;

            if (style === 'unaccented' || this._options.selectedStyle === 'unaccented') {
                classes.push('controls-Tabs__item_view_selected_style_unaccented');
                fontColorStyle = 'default';
            } else {
                if (options.templateType !== 'Icon') {
            ...
            if (this._options.selectedStyle === 'default' && !this._options.fontColorStyle) {
                fontColorStyle = 'secondary';
            } else {
                fontColorStyle = this._options.fontColorStyle || this._defaultFontUnselectedStyle;
            }
        }
    ...
    protected _prepareItemTypeClass(item: ITabButtonItem): string {
        const itemType: string = item.type || this._options.direction + '-default';
    ...
    protected _prepareItemMarkerClass(item: ITabButtonItem): string {
        ...
        if (item.isMainTab) {
            ...
            classes.push(`controls-marker controls-marker_${this._options.direction}`);
            classes.push(`controls-marker_tabsThickness_${this._options.direction}`);
            if (
                ...
                if (markerStyle === 'primary' || markerStyle === 'secondary') {
                    ...
                    classes.push(
                        `controls-marker_style-${
                            markerStyle === 'default' ? 'unaccented' : markerStyle
                        }`
                    );
                }
            } else {
        ...
        if (constants.isServerSide && receivedState?.items?.getCount) {
            ...
            for (let i = 0; i < length; i++) {
                ...
                for (const key in item) {
                    if (item.hasOwnProperty(key) && isContentOption(item[key])) {
                        return true;
                    }
                }
            }
 ...
 * <ul>
 *     <li>{@link /page/autodoc-ts/Controls/tabs/IIconCounterTabTemplate/ Controls/tabs:IconCounterTabTemplate} - иконка и счетчик</li>
 *     <li>{@link /page/autodoc-ts/Controls/tabs/IIconTextSelectedTabTemplate/ Controls/tabs:IconTextSelectedTabTemplate} - сменный шаблон, где текст будет отображаться у активной вкладки</li>
 *     <li>{@link /page/autodoc-ts/Controls/tabs/ITextCounterTabTemplate/ Controls/tabs:TextCounterTabTemplate} - текст и счетчик</li>
 * </ul>
 *
 * @example
 ...
 */
/**
 * @name Controls/_tabs/ITabsTemplate#canShrink
 ...
 */
/**
 * @event Controls/_tabs/ITabsTemplate#itemMouseEnter Происходит при наведении на элемент вкладки.
 * @param {UI/Events:SyntheticEvent} event Дескриптор события.
 * @param {UI/Events:SyntheticEvent} itemEvent Дескриптор события элемента вкладки.
 * @param {String} key Id вкладки, на которую навели.
 * @demo Controls-demo/Tabs/Buttons/ItemMouseEnter/Index
 */
/**
 * @event Controls/_tabs/ITabsTemplate#itemMouseLeave Происходит при уходе мышки с элемента вкладки.
 * @param {UI/Events:SyntheticEvent} event Дескриптор события.
 * @param {UI/Events:SyntheticEvent} itemEvent Дескриптор события элемента вкладки.
 * @param {String} key Id вкладки, на которую навели.
 * @demo Controls-demo/Tabs/Buttons/ItemMouseEnter/Index
 */
export { TabsButtons as TabsButtonsForTests };
export default withAdaptiveMode(TabsButtons);
