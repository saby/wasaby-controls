...
import { IItemTemplateListProps } from 'Controls/interface';
...
/**
 * Опции для адаптивных вкладок.
 * @public
 */
export interface ITabsAdaptiveButtonsOptions extends ITabsButtonsOptions, IItemTemplateListProps {
    /**
     ...
     * @cfg {RecordSet.<Controls/tabs:ITabButtonItem>} Рекордсет с конфигурацией вкладок.
     */
    items?: RecordSet<ITabAdaptiveButtonItem>;
    ...
    /**
     * @name Controls/_tabs/ITabsAdaptiveButtons#itemTemplate
     * @cfg {TemplateFunction} Шаблон вкладки.
     * @demo Controls-demo/Tabs/AdaptiveButtons/ItemTemplate/ItemTemplate
     */
    itemTemplate?: TemplateFunction | string;
    /**
     * @name Controls/_tabs/ITabsAdaptiveButtons#itemRightTemplate
     * @cfg {TemplateFunction} Шаблон элемента, находящегося справа от основного содержимого.
     * @remark
     * @demo Controls-demo/Tabs/AdaptiveButtons/ItemRightTemplate/ItemRightTemplate
     */
    itemRightTemplate?: TemplateFunction;
    /**
     * @name Controls/_tabs/ITabsAdaptiveButtons#rightTemplateProperty
     * @cfg {String} Опция указывает на свойство конфигурации вкладки, в которой указана функция рендера элемента справа от основного содержимого.
     * @remark Приоритетна относительно itemRightTemplate
     * @see Controls/_tabs/ITabsAdaptiveButtons#itemRightTemplate
     */
    rightTemplateProperty?: string;
    /**
     * @name Controls/_tabs/ITabsAdaptiveButtons#displayProperty
     * @remark Если не указано, то во вкладках текст будет отображаться из поля 'caption', а в меню из поля 'title'.
     * Если указано, то текст во вкладках и в меню будет отображаться из поля указанного поля.
     * Если требуется разный текст во вкладках и в меню, используйте itemTemplate для настройки отображения вкладок, при этом в меню будет использоваться значение из displayProperty.
 ...
 * Контрол обладает тем же функционалом, что Controls/tabs:Button, а также позволяет проскроллить вкладки, отображая кнопку "Ещё" по необходимости.
 *
...
class AdaptiveButtons extends Control<ITabsAdaptiveButtonsOptions, IReceivedState> {
    ...
    protected _setHasPadding() {
        if (this._children.ScrollContainer) {
            this._hasPadding = this._children.ScrollContainer.isHorizontalScroll();
        }
    }
    ...
    protected _afterMount() {
        ...
        // Активная вкладка при построении может находиться неизвестно где, поэтому сами проскроллим к активной.
        this._scrollToItem();
    }
    ...
    protected _getTemplate(
        ...
        if (itemTemplateProperty) {
            const templatePropertyByItem =
                item.get?.(itemTemplateProperty) || item[itemTemplateProperty];
    ...
    private _createMemoryForMenu(keyProperty: string): Memory {
        return new Memory({
            keyProperty,
            adapter: this._items.getAdapter(),
