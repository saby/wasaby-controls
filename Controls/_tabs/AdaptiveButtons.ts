...
export type TAdaptiveButtonsTemplate = 'TextAndIcon' | 'Icon' | 'IconAndCounter';
...
class AdaptiveButtons extends Control<ITabsAdaptiveButtonsOptions, IReceivedState> {
    ...
    private _getItemsWidth(
        ...
        for (let i = 0; i < items.getCount(); i++) {
            ...
            let itemTextWidth =
                options.templateType === 'IconAndCounter'
                    ? getFontWidth(`${item.get(displayProperty)}`, 'l')
                    : item.get(displayProperty) && options.templateType !== 'Icon'
                    ? Math.max(this._getTextWidth(item.get(displayProperty), 'l'), MIN_ITEM_WIDTH)
                    : 0;
            let iconWidth = 0;
            if (item.get('icon')) {
                iconWidth +=
                    options.templateType === 'IconAndCounter'
                        ? item.get('iconSize')
                            ? ICON_WIDTHS[item.get('iconSize')]
                            : ICON_WIDTHS.s
                        : options.templateType === 'Icon'
                        ? ICON_WIDTHS.m
                        : ICON_WIDTHS.s + PADDING_OF_MORE_BUTTON;
            }
            if (item.get('icons')) {
                item.get('icons').forEach(() => {
                    iconWidth += ICON_WIDTHS.s + PADDING_OF_MORE_BUTTON;
                });
            }
            if (options.templateType === 'TextAndIcon' && itemTextWidth < MIN_WIDTH) {
                itemTextWidth = MIN_WIDTH;
            } else if (options.templateType === 'IconAndCounter') {
                itemTextWidth += MARGIN_BETWEEN_ICON_COUNTER;
            }
