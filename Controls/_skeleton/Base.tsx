 ...
 * @public
 */
export interface IBaseSkeletonOptions extends IComponentProps {
    active?: boolean;
}
...
function Base(props: IBaseSkeletonOptions) {
    ...
    // на mac и ipad если скелетон широкий, то он начинает моргать, поэтому временно отключаем анимацию
    // isMac=true как на маке так и на ipad
    // верну тут:
    // https://online.sbis.ru/opendoc.html?guid=eb9b12f3-9c55-43a6-9286-8ff6e396893d&client=3
    if (props.active && !detection.isMac) {
    ...
    return (
        <span className={className} data-qa={props['data-qa']}>
