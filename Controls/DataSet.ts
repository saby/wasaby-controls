...
/**
 * Тип ключа узла дерева
 */
export type TRoot = CrudEntityKey | null;
...
type TSourceConstructor = new (options: unknown) => ICrud;
...
export default class DataSet {
    ...
    #getNavigationController(): NavigationController | void {
    ...
    #query(queryParams?: IQueryParams): Promise<RecordSet> {
        return this.#getSource().then((source) => {
            return source.query(this.#getQuery(queryParams)).then((dataSet) => {
                let result;
    ...
    #getSource(): Promise<ICrud> {
        if (this.#props.source) {
            return Promise.resolve(this.#props.source);
        } else if (this.#props.sourceType) {
            return loadAsync<TSourceConstructor>(this.#props.sourceType).then((Source) => {
                return new Source(this.#props.sourceOptions || {});
            });
        } else {
