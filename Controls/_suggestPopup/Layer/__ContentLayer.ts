...
const _private = {
    ...
    calcOffsets(self, { suggest }): void {
        const parent = _private.findOverflowParent(self._options.target);
        const { left, right } = parent.getBoundingClientRect();
        if (suggest.right > right || suggest.left < left) {
            self._offsets.left = parent.clientWidth - (suggest.right - left) + self._inputOffset;
    ...
    findOverflowParent(element: HTMLElement): HTMLElement {
        const overflow = getComputedStyle(element).overflow;

        if (overflow === 'hidden' || overflow === 'clip') {
            return element;
        } else if (element.parentElement) {
            return _private.findOverflowParent(element.parentElement);
        } else {
...
export default class ContentLayer extends Control {
    ...
    _getSuggestPopupMinWidth(options): string {
        if (!options.isAdaptive) {
            return String(options.suggestWidth).endsWith('px')
                ? options.suggestWidth
                : options.suggestWidth + 'px';
        }
    }

    _getSuggestPopupWidth(options): string | void {
        if (detection.isIE11 || options.isAdaptive) {
            return options.suggestWidth ? this._getSuggestPopupMinWidth(options) : this._width;
        }
    }
