...
const _private = {
    getSizes(self, dropDownContainer?: HTMLElement): object {
        ...
        const suggestBCR = boundingClientToJSON(
            container.getBoundingClientRect()
        );
        const containerBCR = boundingClientToJSON(
            targetContainer.getBoundingClientRect()
        );
        const dropDownContainerBCR =
            _private.getDropDownContainerSize(dropDownContainer);
    ...
    determineOpenDirection(self): void {
        ...
        const containerToInputSize =
            sizes.container.left - dropDownContainerSize.left;

        self._openDirection =
            containerToInputSize + suggestSize.width >
                dropDownContainerSize.width &&
    ...
    updateMaxHeight(self): void {
        ...
        if (dropDownContainerBCR !== self._dropDownContainerBCR) {
            ...
            self._maxHeight =
                dropDownContainerBCR.height - suggestBCR.top + 'px';
        }
    },
};
