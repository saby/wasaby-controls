...
const _private = {
    openPopup(self, opener, options): void {
        ...
        const dynamicConfig = {
            ...
            opener: self,
            closeOnOutsideClick: options.isAdaptive && options.allowAdaptive,
            actionOnScroll: detection.isMobileIOS || detection.isMobileAndroid ? 'track' : 'close',
    ...
    setPopupOptions(self, options) {
        const config = {
            ...
            autofocus: false,
            allowAdaptive: options.isAdaptive && options.allowAdaptive,
            direction: {
                vertical: 'bottom',
            },
            targetPoint: {
        ...
        self._popupOptions = {
            ...
            // Ширина и так вешается на контент попапа автодополнения и попап тянется по контенту
            width: null,
        };
    },
};
...
const PopupLayer = Control.extend({
    ...
    _afterUpdate(oldOptions) {
        if (
            ...
            !isEqual(this._options.filter, oldOptions.filter) ||
            ...
            this._options.loadResult !== oldOptions.loadResult
        ) {
