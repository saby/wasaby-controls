 ...
 * @example
 * В примере ниже демонстриурется типовой пример использования Controls.suggestPopup:ListContainer в шаблоне автодополнения
 ...
 *        keyProperty="id"
 *        source="{{_source}}">
 *        <ws:suggestTemplate templateName="MyModule/myList:MySuggestTemplate"/>
 *     </Controls.SuggestInput>
 * </pre>
 *
 * <pre class="brush: html">
 * <!-- MyModule/myList:MySuggestTemplate -->
 ...
 * @public
 * @demo Controls-demo/Suggest_new/Input/AutoDropDown/AutoDropDown
 */
...
const List = Control.extend({
    ...
    _beforeMount(options) {
        ...
        this._setSuggestConfigs(options._suggestListOptions.loadResult);
        if (options.tabsOptions) {
            this._tabsItems = options.tabsOptions.items;
        } else if (options._dataOptionsValue) {
            const tabs = [];
            this._listItems.forEach((listOption) => {
                tabs.push({
                    id: listOption.id,
    ...
    _beforeUpdate(newOptions) {
        ...
        if (
            !isEqual(
                this._options._suggestListOptions?.loadResult,
                newOptions._suggestListOptions?.loadResult
            )
        ) {
            this._setSuggestConfigs(newOptions._suggestListOptions.loadResult);
        }
    },

    _setSearchValue(options) {
        const suggestOptions = options._dataOptionsValue?.suggestLoader?.state;
        this._searchValue = suggestOptions?.searchValue;
    },
    ...
    _needRenderTabs(options) {
        ...
        return (
            !dialogMode &&
            ((options.tabsOptions && (!tabsOptionsItems || tabsOptionsCount > 1)) ||
                this._tabsItems?.getCount() > 1)
        );
    },

    _handleSearch(event, value) {
        // this._searchValue = value;
        this._options._dataOptionsValue?.suggestLoader?.search(value);
    },

    _handleResetSearch(event) {
        const suggesSlice = this._options._dataOptionsValue?.suggestLoader;
        suggesSlice?.resetSearch();
        suggesSlice?.reload();
    },
    ...
    _itemClick(event: SyntheticEvent, item: Model): void {
        ...
        // Из списка событие itemClick тоже нотифицируется с bubbling: true
        // TODO переделать на передачу callback'a через context
        this._notify('itemClick', [item], { bubbling: true });
    },

    _propertyValueChangedHandler(
        event: SyntheticEvent,
        editorOptions: object,
        editorValue: any
    ): void {
        const editorItems = editorOptions?.sourceController?.getItems();
        const itemKey = Array.isArray(editorValue.value) ? editorValue.value[0] : editorValue.value;
        const selectedItem = editorItems.getRecordById(itemKey);
        this._notify('itemClick', [selectedItem, editorOptions?.id], { bubbling: true });
    },

    _setSuggestConfigs(loadResult) {
        this._listEditors = [];
        this._listItems = [];
        const categoriesConfig = {};
        this._sortDataByOrder(loadResult).forEach((listOption) => {
            if (listOption.category) {
                if (!categoriesConfig[listOption.category]) {
                    categoriesConfig[listOption.category] = [];
                }
                categoriesConfig[listOption.category].push(listOption);
            } else {
                this._listItems.push(listOption);
            }
        });
        Object.values(categoriesConfig).forEach((items) => {
            if (items.length > 1) {
                this._listItems = items;
            } else {
                this._listEditors.push(items[0]);
            }
        });
    },
});
