...
import { default as ContextOptionsConsumer } from 'Controls/_contextDeprecated/ContextOptionsConsumer';

interface IInnerComponent extends Control {
    displayName?: string;
    name?: string;
}

export default function connectData<T extends IInnerComponent>(
    WrappedComponent: new () => T,
    sliceName?: string
): new () => T {
    // @ts-ignore
        ...
        render() {
            return (
                <ContextOptionsConsumer
                    sliceName={sliceName}
                    {...this.props}
                    contextWrapperRef={(element: HTMLDivElement) => this.ref(element)}
