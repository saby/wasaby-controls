...
import { isFunctionComponent, isReactComponentType } from 'UICore/Executor';
import { Component, ReactElement, RefObject, cloneElement, createRef } from 'react';
 ...
 * Полезные ссылки:
 * <ul>
 *     <li>{@link /materials/DemoStand/app/Controls-demo%2FInfoBox%2FOpener%2FInfoBox демо-пример}</li>
...
class InfoBox extends Component<IInfoBoxOptions, IInfoboxState> {
    ...
    open(options?: IInfoBoxOptions): void {
        ...
        if (isNewEnvironment() && this.context) {
    ...
    protected _contentMousedownHandler(event: SyntheticEvent<MouseEvent>): void {
        if (
            this.props.trigger === 'click' ||
            (this.props.trigger === 'hover|touch' && TouchDetect.getInstance().isTouch())
        ) {
            if (!this.state.opened && this.props.trigger === 'click') {
                this.open();
            }
    ...
    private _contentTouchEndHandler(event: Event): void {
        event.stopPropagation();
    }
    ...
    render(): ReactElement {
        ...
        const contentProps = {
            ...
            onTouchEnd: this._contentTouchEndHandler.bind(this),
    ...
    static contextType = InfoBoxContext;
}
