...
const ERROR_MSG = {
    ...
    CAN_NOT_SKIP:
        'Can not skip item, because there is no item passed into options.',
};
...
type TBeforeBeginEditCallback = (
    ...
    | TBeforeCallbackBaseResult
    | IBeginEditUserOptions
    | Promise<IBeginEditUserOptions>;
 ...
 */
export type IBeforeEndEditCallbackParams = ICallbackParams<
...
export class Controller extends mixin<DestroyableMixin>(DestroyableMixin) {
    ...
    private _beginEdit(
        ...
        this._operationsPromises.begin = new Promise((resolve) => {
            ...
            .then((callbackResult?: IBeginEditUserOptions | CONSTANTS) => {
                ...
                return this._options?.onAfterBeginEdit(
                    this._getEditingItem(),
                    isAdd
                );
            })
        ...
        const tryEditNext = (
            ...
            if (_userOptions?.item) {
                current = this._options.collection.getItemBySourceKey(
                    _userOptions.item.getKey()
                );
                if (!current) {
            ...
            if (
                ...
                if (typeof _options.columnIndex === 'number') {
                    const newColumnIndex =
                        ...
                        (position === CONSTANTS.PREV_COLUMN ? -1 : 1);
                    if (
                        newColumnIndex >
                        this._options.collection.getGridColumnsConfig().length -
                            1
                    ) {
                        ...
                        _options.columnIndex =
                            this._options.collection.getGridColumnsConfig()
                                .length - 1;
            ...
            if (!next && !options.isAdd) {
                ...
                return this._beginEdit(
                    { ..._userOptions, item: next && next.contents },
                    _options
                );
            }
        };
    ...
    private _endEdit(
        ...
        const willSave =
            commitStrategy === 'hasChanges' && !editingItem.isChanged()
                ? false
                : commit;
        ...
        const notifyBeforeEndEdit = () => {
            if (this._options.onBeforeEndEdit) {
                ...
                if (this._addParams.targetItem) {
                    ...
                    sourceIndex =
                        collectionIndex +
                        (this._addParams.addPosition === 'bottom' ? 1 : 0);
                }
        ...
        const notifyAfterEndEdit = () => {
            return this._options?.onAfterEndEdit?.(
    ...
    private _isTargetEditing(targetItem: Model, columnIndex?: number): boolean {
        ...
        const isSameItem =
            !!targetItem && editingItem.contents.isEqual(targetItem);

        if (this._options.mode === 'cell') {
            return (
                isSameItem &&
                editingItem.getEditingColumnIndex() === columnIndex
            );
        } else {
            return isSameItem;
        }
