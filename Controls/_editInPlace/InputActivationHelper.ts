...
function getElementFromPointUnderMirror<T extends Element = Element>({clientX, clientY}: IPoint): T {
...
function getElementFromPointWithDescription<T extends Element = Element>(point: IPoint): {
    element: T;
    isInput: boolean;
    isNewInput: boolean;
    isCombobox: boolean;
...
export class InputActivationHelper {
    ...
    protected _tryActivateByInputClick(beforeFocus?: TBeforeFocusCallback): boolean {
        const {
            ...
            isCombobox,
            isLookup
        } = getElementFromPointWithDescription<HTMLInputElement | HTMLTextAreaElement>(this._clickItemInfo);
