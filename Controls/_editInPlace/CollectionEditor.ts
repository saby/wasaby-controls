...
export class CollectionEditor extends mixin<DestroyableMixin>(DestroyableMixin) {
    ...
    private _validateAddingItem(editingItem: object, targetItem: object): void | never {
        ...
        if (
            ...
            editingItem['[Controls/_propertyGridBase/PropertyGridCollectionItem]']
        ) {
            return;
        }

        // Многоколоночный список может быть как с иерархией так и без.
        // Нужно распилить на два отдельных контрола.
        // https://online.sbis.ru/opendoc.html?guid=508568f2-0bfe-4ad9-9be2-cbe517d9fad6&client=3
        if (
            editingItem['[Controls/_columns/display/CollectionItem]'] &&
            !editingItem.getOwner().getParentProperty()
        ) {
            return;
        }
