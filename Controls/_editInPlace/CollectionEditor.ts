...
export const ERROR_MSG = {
    ...
    COLLECTION_IS_REQUIRED:
        'Options ICollectionEditorOptions:collection is required.',
 ...
 */
export class CollectionEditor extends mixin<DestroyableMixin>(
    DestroyableMixin
) {
    ...
    private _validateOptions(
        options: Partial<ICollectionEditorOptions>
    ): boolean | never {
        ...
        if (
            !options.collection.getSourceCollection()[
    ...
    edit(item: Model, columnIndex?: number): void {
        ...
        this._editingItem = this._options.collection.getItemBySourceKey(
            item.getKey(),
            false
        );
        if (!this._editingItem) {
    ...
    add(
        ...
        if (
            addingItemKey &&
            this._options.collection.getItemBySourceKey(addingItemKey, false)
        ) {
            throw Error(
                `${ERROR_MSG.ADD_ITEM_KEY_DUPLICATED} Duplicated key: ${addingItemKey}.`
            );
        }
        ...
        let targetIndex;
        if (targetItem) {
            targetIndex = this._options.collection
                .getSourceCollection()
                .getIndex(targetItem);
            if (targetIndex === -1) {
    ...
    private _getNextEditableItem(
        ...
        while (next && !next.EditableItem) {
            next =
                direction === 'after'
                    ? collection.getNext(next)
                    : collection.getPrevious(next);
        }
    ...
    private _validateAddingItem(
        ...
        if (
            editingItem['[Controls/_propertyGrid/PropertyGridCollectionItem]']
        ) {
            return;
        }
        ...
        if (editingItem && editingItem['[Controls/_display/TreeItem]']) {
            ...
            const editingItemParentKey =
                editingItem.contents.get(parentProperty);
            const collectionRootKey = this._options.collection
                .getRoot()
                .getContents();
            let rootItem;
            if (
