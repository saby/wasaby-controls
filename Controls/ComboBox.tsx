...
function getDisplayState(
    ...
    selectedItems: Model[]
): { value: string; placeholder: string; isEmptyItem: boolean } {
    let value = '';
    ...
    if (selectedItems) {
        ...
        if (isEmptyItem && props.emptyText) {
            ...
            value = String(selectedItems[0]?.get(props.displayProperty) ?? '');
        }
    }
    return {
        value,
...
const ContentTemplate = function (props: {
    ...
    if (
        ...
        return (
            <div
                className={`controls-ComboBox__placeholder ${
        ...
        return (
            <span className={props.readOnly ? 'tw-cursor-text' : ''}>{props.value || null}</span>
        );
    }
};
...
class ComboBox extends BaseDropdown<IComboboxOptions, IComboboxState> {
    ...
    constructor(props: IComboboxOptions): void {
        super(props);
    ...
    render() {
        ...
        const { value, placeholder, isEmptyItem } = getDisplayState(props, state.selectedItems);
        ...
        const openedPopupType = state.isOpened
            ? props.isAdaptive && this._controller.isAllowAdaptive()
                ? 'sliding'
                : 'menu'
            : undefined;
        return (
