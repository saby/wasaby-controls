...
export function getScrollableViewPortWidth(widths: IColumnScrollWidths): number {
    return _minMax(widths.viewPortWidth - widths.fixedWidth, [0, getScrollableWidth(widths)]);
}
...
export function getLimitedPosition(current: number, widths: IColumnScrollWidths): number {
...
export function getTransformCSSRule<T extends number | string>(
    position: T
): TTransform<T> {
