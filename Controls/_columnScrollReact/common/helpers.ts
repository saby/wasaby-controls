...
import { EdgeState, TScrollIntoViewAlign, TAutoScrollMode } from './types';
...
export function getPrevPagePosition(
    ...
    return _calcNearbyPagePosition(currentPosition, -1, widths, smooth);
}
...
function _calcNearbyPagePosition(
    ...
    let delta = scrollableViewPortWidth * PAGE_SMOOTH_SCROLLING_COEFFICIENT;

    if (!smooth) {
        delta = scrollableViewPortWidth;
    }

    return _minMax(currentPosition + coef * delta, [0, getMaxScrollPosition(widths)]);
}
...
/**
 * Возвращает CSS правило трансформации элемента по горизонтали на заданное значение.
 * Используется оптимальное, с точки зрения производительности, правило.
 * @param {Number | String} position Значение смещения
 * @return {String}
 */
...
/**
 * Возвращает объект с React совместимым CSS правилом трансформации элемента по горизонтали на заданное значение.
 * Используется оптимальное, с точки зрения производительности, правило.
 * @param {Number | String} position Значение смещения
...
/**
 * Helper расчета горизонтальной позиции по событию onWheel.
 * Величина смещения при вращении колесика мыши отличается в разных браузерах,
 * данный класс старается свести это различие к минимуму.
 */
export class WheelHelper {
    ...
    /**
     * Рассчитывает новую позицию скролла.
     * @param {React.WheelEvent} e Событие прокрутки
     * @param {Number} currentPosition Текущая позиция скролла
     * @param {IColumnScrollWidths} widths Ширины структурных частей области скроллирования
     */
...
function getLeftTargetPosition(
    ...
    return targetRect.left - rootRect.left - widths.startFixedWidth;
}
...
export function calcTargetPositionWithAlign(
    ...
    switch (align) {
        ...
        default: {
            logger.warn(
                'Передано неизвестное значение параметра align.\n' +
                    'Значение должно удовлетворять типу TScrollIntoViewAlign.'
            );
            return position;
        }
    }
...
export function getAutoScrollTargetParams(
    ...
    position: number,
    widths: IColumnScrollWidths,
    autoScrollMode: TAutoScrollMode = 'closest'
):
    | {
          target: HTMLElement;
          align: 'start' | 'end';
    ...
    if (targetIndex === -1) {
        // В случае падения не по вине контекста, переписать текст ошибки на ошибку
        //  использования и расписать шаги отладки.
        logger.warn(
            'Внутренняя ошибка Controls/columnScrollReact\n' +
                'Не найден target для автоподскролла.\n' +
                'Метод getAutoScrollTargetParams у редьюсера скролла ' +
                '(Controls/_columnScrollReact/common/helpers.ts)'
        );
        return;
    }

    if (autoScrollMode === 'closest' && direction === 'backward' && targetIndex > 0) {
        targetIndex--;
    }

    if (autoScrollMode === 'mostVisible' && targetIndex > 0) {
        const currentDiff = Math.abs(targetsParams[targetIndex].position - position);
        const neighborDiff = Math.abs(targetsParams[targetIndex - 1].position - position);
        if (currentDiff > neighborDiff) {
            targetIndex--;
        }
    }
