...
function _calcNearbyPagePosition(
    ...
    return _minMax(Math.round(currentPosition + coef * delta), [
        0,
...
export class WheelHelper {
    ...
    calcNewPositionByWheelEvent(e: React.WheelEvent,
                                currentPosition: number,
                                widths: IColumnScrollWidths): number {
...
export function getEdgesState(
    ...
    return {
        ...
        right: position < getMaxScrollPosition(widths)
            ? EdgeState.Invisible
            : EdgeState.Visible,
    };
}
