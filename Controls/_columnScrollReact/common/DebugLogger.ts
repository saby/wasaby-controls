...
export class DebugLogger {
    ...
    static contextSetPositionCalled(
        ...
        if (privateContextUserSymbol === PrivateContextUserSymbol) {
            ...
            const msg = baseMsg + '// Публичный метод контекста, извне библиотеки ColumnScrollReact.';
            log(msg, 'warning');
        }
    }

    static contextSetPositionByGridControl(methodName: string, newPosition: number, smooth?: boolean) {
        if (!DebugLogger._debug) {
            return;
        }
    ...
    static contextSetPositionPublic(userControlName: string, newPosition: number, smooth?: boolean) {
        if (!DebugLogger._debug) {
            return;
        }
