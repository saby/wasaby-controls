...
// Класс для включения GPU оптимизаций, лечит проблему с белыми мерцающими и пропадающими
// блоками при скроллировании.
// Хак основан на использовании transform и не может быть включен во время скроллирования мышью,
// т.к. transform создает новый контекст наложения, а скроллирование мышью работает через оверлей
// на весь экран с position: fixed.
// При одновременном включении двух механизмов, оверлей будет по размеру блока на котором
// включена оптимизация, а не по размеру экрана.
...
export const DEFAULT_SELECTORS = {
    ...
    AUTOSCROLL_TARGET: `${prefix}_autoScrollTarget` as `${typeof prefix}_autoScrollTarget`,
...
export type TSelectorsProps = {
    ...
    // Фиксации с прижатием к левой границе не реализовано.
    // Таких задач пока нет, чтобы например в таблице фиксировали первую, третью и пятую колонки.
    // FIXED_ELEMENT не прижимает контент, а исключительно компенсирует скроллирование.
...
/**
 * Внутренние, приватные CSS селекторы скролла.
 * Получаются на основании TSelectorsProps, обогащают пользовательские селекторы GUID'ом и добавляют
 * приватные утилитарные селекторы.
 */
export interface ISelectorsState extends Required<TSelectorsProps> {
    /**
    ...
    /**
     * Утилитарный класс целей автоподскролла.
     */
    AUTOSCROLL_TARGET: `${typeof DEFAULT_SELECTORS.AUTOSCROLL_TARGET}${_TSeparator}${string}`;
}

/**
 * Возвращает внутренние, приватные CSS селекторы скролла на основании селекторов из опций.
 * @param {TSelectorsProps} propsSelectors Селекторы из опций.
 * @param {string} guid Уникальный, в рамках приложения, идентификатор скролла.
 * @return ISelectorsState
 */
export function getSelectorsState(
    propsSelectors: TSelectorsProps = {},
    guid: string = ''
): ISelectorsState {
    ...
    const result: ISelectorsState = {
        ...
        AUTOSCROLL_TARGET: `${DEFAULT_SELECTORS.AUTOSCROLL_TARGET}${separator}${guid}`,
    };
...
/**
 * Возвращает селекторы из опций, в которых определены все неопределенные поля.
 * @param {TSelectorsProps} propsSelectors Селекторы из опций.
 * @return Required<TSelectorsProps>
 */
function _getPropsSelectors(propsSelectors: TSelectorsProps): Required<TSelectorsProps> {
    return {
