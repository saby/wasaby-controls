...
export function NavigationComponent(
    ...
    const onArrowClick = React.useCallback((direction: 'backward' | 'forward') => {
        const newPosition =
            direction === 'backward'
                ? getPrevPagePosition(contextRef.current.position, contextRef.current)
                : getNextPagePosition(contextRef.current.position, contextRef.current);

        contextRef.current.setPosition(newPosition, true, PrivateContextUserSymbol);
    }, []);
    ...
    >((isDragging) => {
        if (!context.contextRefForHandlersOnly.current.isMobile) {
            context.contextRefForHandlersOnly.current.setIsScrollbarDragging(isDragging);
        }
    }, []);
