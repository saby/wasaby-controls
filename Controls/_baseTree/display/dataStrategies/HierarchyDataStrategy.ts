...
export interface IHierarchyDataStrategyOptions
    extends IFlatDataStrategyOptions {
    /**
...
> extends FlatDataStrategy<TOptions> {
    ...
    getSourceIndexBySourceItem(item: TSourceItem): number {
        ...
        const key = object.getPropertyValue<TKey>(
            item,
...
function iterator(
    ...
    const iterator = (collection: TCollection) => {
        ...
        for (let index = 0; index < itemsCount; index++) {
            item = isArray
                ? collection[index]
                : (collection as RecordSet).at(index);
            callback(item, throughIndex);
            throughIndex++;

            const children = object.getPropertyValue<TCollection>(
                item,
