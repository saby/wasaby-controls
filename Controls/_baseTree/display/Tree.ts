...
export type TNodeFooterVisibilityCallback<S extends Model = Model> = (
    contents: S
) => boolean;
...
export interface ISerializableState<S, T>
    extends IDefaultSerializableState<S, T> {
    _root: T;
}
...
function onCollectionItemChange<T extends Model>(
    ...
    if (this.instance.getExpanderVisibility() === 'hasChildren') {
        if (
            ...
            properties.hasOwnProperty(this.instance.getHasChildrenProperty())
        ) {
            this.instance._recountHasNodeWithChildren();
        }
    }
...
> extends Collection<S, T> {
            ...
            this._userStrategies.unshift({
                ...
                options: {
                    ...
                    extraItemVisibilityCallback:
                        this._$nodeFooterVisibilityCallback,
    ...
    _getSerializableState(
        state: IDefaultSerializableState<S, T>
    ...
    setDragPosition(position: IDragPosition<T>): void {
        const dragStrategy = this.getStrategyInstance(
            this._dragStrategy
        ) as TreeDrag;
    ...
    setNodeFooterVisibilityCallback(
        callback: TNodeFooterVisibilityCallback
    ): void {
    ...
    setKeyProperty(keyProperty: string): void {
        ...
        const adjacencyList = this._composer.getInstance<
            AdjacencyListStrategy<S, T>
    ...
    protected _isChangedValueInParentProperty(
        ...
        if (changedProperties) {
            ...
            for (let i = 0; i < oldItems.length; i++) {
                ...
                if (
                    !oldCollectionItem ||
                    !oldCollectionItem['[Controls/_display/TreeItem]']
                ) {
                    continue;
                }
    ...
    protected _wrapRootStrategy(
        composer: ItemsStrategyComposer<S, CollectionItem<S>>
    ): void {
    ...
    protected _spaceItemFactory(item: T): SpaceCollectionItem {
        ...
        spaceItem.setParent(item.getParent());
        return spaceItem;
    }
    ...
    protected _replaceItems(start: number, newItems: S[]): ISplicedArray<T> {
        const replaced = super._replaceItems(
            start,
            newItems
        ) as ISplicedArray<T>;
    ...
    protected _getItemState(item: T): ITreeSessionItemState<T> {
        ...
        if (item instanceof TreeItem) {
            ...
            state.childrenCount = item
                .getOwner()
                ._getChildrenArray(item, false).length;
    ...
    getChildrenByRecordSet(parent: T | S | CrudEntityKey): T[] {
        ...
        if (parent && parent['[Types/_entity/Model]']) {
            treeItemParent = this.getItemBySourceKey(
                (parent as unknown as Model).getKey()
            );
        ...
        return treeItemParent
            ? this._getChildrenArray(treeItemParent, false)
            : [];
    }
    ...
    private _recountHasChildrenByRecordSet(): void {
        ...
        nodes.forEach((it) => {
            const hasChildrenByRecordSet =
                !!this.getChildrenByRecordSet(it).length;
            changed =
                it.setHasChildrenByRecordSet(hasChildrenByRecordSet) || changed;
        });
        ...
        if (this.getStrategyInstance(AddStrategy)) {
            ...
            if (parentOfAddingItem) {
                const hasChildrenByRecordSet =
                    parentOfAddingItem.hasChildrenByRecordSet();
    ...
    getRecordSetProjection(
        ...
        for (let i = 0; i < children.length; i++) {
            ...
            if (isExpandAll || expandedItems.indexOf(key) !== -1) {
                projection.push(
                    ...this.getRecordSetProjection(key, expandedItems)
                );
            }
        }
        return projection;
    ...
    protected _recountHasNodeWithChildren(): void {
        ...
        for (let i = 0; i < collection.getCount(); i++) {
            ...
            const isGroupNode =
                item.get(this.getNodeTypeProperty()) ===
        ...
        if (this.getStrategyInstance(AddStrategy) && !hasNodeWithChildren) {
            const parentOfAddingItem = this._getParentOfAddingItem();
            hasNodeWithChildren =
                parentOfAddingItem && !parentOfAddingItem.isRoot();
        }
    ...
    protected _shouldRecountExpanderByAddInPlace(): boolean {
        const addStrategy = this.getStrategyInstance(
            AddStrategy
        ) as AddStrategy<S, T>;
        if (!addStrategy) {
            return;
        ...
        return (
            addingItem.getContents().get(this.getParentProperty()) !== undefined
        );
    }
    ...
    protected _getParentOfAddingItem(): T {
        ...
        if (addStrategy) {
            const addingItem = addStrategy.getAddingItem();
            const parentKey = addingItem
                .getContents()
                .get(this.getParentProperty());
