...
function validateTypeOfKeyAndParentKey<S extends Model = Model>(
    item: S,
    keyProperty: string,
    parentProperty: string
): void {
    const key = object.getPropertyValue(item, keyProperty);
    const parentKey = object.getPropertyValue(item, parentProperty);
    const keyType = typeof key;
    const parentKeyType = typeof parentKey;
    // По хорошему undefined нужно тоже проверять, но это сотни демок и сотни прикладников
    if (parentKey !== null && parentKey !== undefined && keyType !== parentKeyType) {
        logger.error(
            `Тип ключа записи и тип ключа родителя записи отличается у записи с ключом "${key}" и с ключом родителя "${parentKey}". Тип ключа "${keyType}" и тип ключа родителя "${parentKeyType}".`
        );
    }
}
    ...
    implements IItemsStrategy<S, T>
{
    ...
    at(index: number): T {
        ...
        if (sourceItem['[Controls/_display/GroupItem]']) {
            ...
            if (!this.options.display?.isFeature1189922556?.()) {
                validateTypeOfKeyAndParentKey(
                    item.contents,
