...
interface ISourceOptions<S, T extends TreeItem<S>>
...
function buildChildrenMap<T>(
    ...
    for (let position = 0; position < count; position++) {
        ...
        parentId = normalizeId(
            object.getPropertyValue(itemContents, parentProperty)
        );
...
function buildTreeIndex<T>(
    ...
    if (
        ...
        path.indexOf(parentId) < path.length - 1
    ) {
        logger.error(
    ...
    for (let i = 0; i < childrenCount; i++) {
        ...
        if (childContents && keyProperty) {
            const childId = normalizeId(
                object.getPropertyValue(childContents, keyProperty)
            );
            path.push(childId);
            ...
            if (!child.isAdd) {
                const itemsToPush = buildTreeIndex(
                    options,
                    parentsMap.length - 1
                );
    ...
    implements IItemsStrategy<S, T>
{
    ...
    at(index: number): T {
        ...
        if (sourceItem === undefined) {
            throw new ReferenceError(
                `Collection index ${index} is out of bounds.`
            );
        }
    ...
    splice(start: number, deleteCount: number, added?: S[]): T[] {
        ...
        if ((added as ISplicedArray).hasBeenRemoved) {
            added.forEach((item, index) => {
                ...
                const startInSource = source.getDisplayIndex(
    ...
    protected _syncItemsOrder(): void {
        ...
        for (let newIndex = 0; newIndex < newOrder.length; newIndex++) {
            ...
            if (
                innerItem &&
                innerItem.getContents() === sourceItem.getContents()
            ) {
    ...
    protected _createItemsOrder(): number[] {
        ...
        let rootId: unknown =
            root && root.getContents ? root.getContents() : root;
        ...
        if (
            rootId === null &&
            !childrenMap.has(rootId) &&
            childrenMap.has(undefined)
        ) {
            rootId = undefined;
        }
