...
> extends itemsStrategy.AbstractStrategy<S, T> {
    ...
    at(index: number): T {
        ...
        if (!items[index]) {
            const collection = this._getCollection() as unknown as IEnumerable<unknown> &
                IList<unknown> &
                unknown[];
    ...
    protected _getParent(index: number, path: number[]): T {
        ...
        if (parentPath.length) {
            ...
            const collection = this._getCollection() as unknown as IEnumerable<unknown> &
                IList<unknown> &
                unknown[];
            const parentContents = MaterializedPathUtils.getItemByPath(parentPath, {
                collection,
                childrenProperty: this._options.childrenProperty,
                indexToPath: this._indexToPath,
            });
...
export const MaterializedPathUtils = {
     ...
     */
    getPathToItem: (index: number, options: IMaterializedPathUtilOptions): number[] => {
        ...
        const iterator = (search, parent, path) => {
            ...
            for (;;) {
                ...
                children = object.getPropertyValue(item, options.childrenProperty);
     ...
     */
    getItemByPath: (path: number[], options: IMaterializedPathUtilOptions): unknown => {
