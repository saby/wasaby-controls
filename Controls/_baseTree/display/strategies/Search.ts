...
function getBreadCrumbsReference<S extends Model, T extends TreeItem<S>>(
    ...
    if (
        last &&
        last !== root &&
        !last['[Controls/treeGrid:TreeGridGroupDataRow]']
    ) {
        ...
        if (!breadCrumbs) {
            ...
            const parent =
                item.getParent() &&
                item.getParent()['[Controls/treeGrid:TreeGridGroupDataRow]']
                    ? item.getParent()
                    : root;
    ...
    implements IItemsStrategy<S, T>
{
     ...
     */
    protected _treeItemToBreadcrumbs: Map<T, BreadcrumbsItem<S>> = new Map<
        T,
        BreadcrumbsItem<S>
    >();
    ...
    getCollectionIndex(index: number): number {
        ...
        return sourceIndex >= 0
            ? this.source.getCollectionIndex(sourceIndex)
            : -1;
    }
    ...
    static sortItems<S extends Model, T extends TreeItem<S> = TreeItem<S>>(
         ...
         */
        function addBreadCrumbsItself(
            reference: IBreadCrumbsReference<S, T>
        ): void {
            ...
            if (
                ...
                if (item.isNode()) {
                    ...
                    if (
                        ...
                        const dedicatedBreadcrumbsReference =
                            getBreadCrumbsReference(
                                item,
                                treeItemToBreadcrumbs,
                                breadcrumbsToData,
                                display
                        ...
                        prevBreadCrumbs =
                            dedicatedBreadcrumbsReference.breadCrumbs;
                    ...
                    const nextIsTreeItem =
                        next && next['[Controls/_display/TreeItem]'];
                    ...
                    if (isLastBreadcrumb) {
                        ...
                        prevBreadCrumbs =
                            lastNodeBreadcrumbsReference.breadCrumbs;
                ...
                const breadcrumbsReference: IBreadCrumbsReference<S, T> =
                    getBreadCrumbsReference(
                        parent,
                        treeItemToBreadcrumbs,
                        breadcrumbsToData,
                ...
                if (currentBreadcrumbs) {
                    ...
                    if (
                        currentBreadcrumbs !== prevBreadCrumbs &&
                        breadcrumbsReference.itsNew
                    ) {
                    ...
                    if (!decoratedItem) {
                        ...
                        const itsDescendant =
                            item.getParent() !== breadcrumbsReference.last;
                        ...
                        decoratedItem = create(
                            options.treeItemDecoratorModule,
                            {
                                source: item,
                                parent: parentLocal,
                                multiSelectVisibility:
                                    display.getMultiSelectVisibility(),
