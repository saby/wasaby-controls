...
> extends itemsStrategy.Drag<S, T> {
    ...
    protected _isDisplayItem(
        item: Model,
        index: number,
        collectionItem: TreeItem
    ): boolean {
        ...
        const itemIsStartDraggableItem =
            startDraggableItemKey === collectionItem.key;
        if (itemIsStartDraggableItem) {
            return true;
        }

        return this._isDisplayByParents(
            collectionItem,
            this._options.draggedItemsKeys
        );
    }

    protected _isDisplayByParents(
        item: TreeItem,
        draggedItemsKeys: CrudEntityKey[]
    ): boolean {
        const itemParent =
            item['[Controls/_display/TreeItem]'] && item.getParent();
    ...
    protected _createItems(): T[] {
        ...
        if (this.avatarItem) {
            const parent = this._getParentConsideringHiddenItems(
                this.avatarItem
            );
            if (parent) {
