...
type TResolver = {
    resolver?: () => void;
    isLoading?: boolean;
    timers?: [number | undefined, number | undefined, number | undefined];
};
...
const scheduleResolverResolve = (resolver: TResolver) => {
    const timers: [number | undefined, number | undefined, number | undefined] = [
        undefined,
        undefined,
        undefined,
    ];
    ...
    // Таймауты необходимы из-за наличия в LCCC ожидания рендера через setTimeout,
    // иначе логика экспандера будет отрабатывать раньше, чем установится новое состояние

    // 0 - размещает следующий таймаут сразу после регистрации первого таймаута в LCCC
    timers[0] = setTimeout(() => {
        // 1 - ожидает рендер в LCCC от уведомления о новом состоянии маркера и установку нового состояния в слайс
        timers[1] = setTimeout(() => {
            // 2 - ожидает рендер в LCCC от уведомления о раскрытости узлов и установку нового состояния в слайс
            timers[2] = setTimeout(() => {
