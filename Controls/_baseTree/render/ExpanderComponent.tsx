...
function getExpanderProps<TItem extends TreeItem = TreeItem>(
    ...
    return {
        ...
        withoutLevelPadding: item.getWithoutLevelPadding(
            props.withoutLevelPadding
        ),
...
function getExpanderClassName(props: IExpanderProps): string {
    const expanderIcon =
        props.expanderIcon || (props.node ? 'node' : 'hiddenNode');
    ...
    if (props.style === 'master') {
        expanderIconSize =
            expanderPosition === 'default' ? 'master' : 'default';
    ...
    let expanderClasses =
        'js-controls-Tree__row-expander controls-TreeGrid__row-expander';
    ...
    if (expanderPosition === 'default') {
        ...
        expanderClasses +=
            ' tw-inline-flex tw-justify-center tw-items-center tw-flex-shrink-0';
    } else {
    ...
    const style =
        props.style === 'master' && expanderPosition !== 'right'
            ? 'master'
            : 'default';
    ...
    expanderClasses +=
        ...
        (props.expanded ? '_expanded' : '_collapsed');
    // добавляем класс свертнутости развернутости стилевой
    expanderClasses +=
        expanderIconClass + (props.expanded ? '_expanded' : '_collapsed');
...
function shouldDisplayExpanderPadding(props: IExpanderProps): boolean {
    ...
    if (props.expanderVisibility === 'hasChildren') {
        // Завязаться тут на expanderSize нельзя. Т.к. есть уже списки, где используется hasChildren
        // и в таком кейсе нужны отступы под экспандер. Поэтому добавил опцию withoutExpanderPadding
        // по аналогии с withoutLevelPadding.
        return !props.hasChildren && displayExpanderPadding;
    } else {
        // expanderSize - очень странная завязка. Нужно заменить все использования на withoutExpanderPadding.
        // Как я понял, она нужна чтобы не добавлялся отступ, если в строке есть изображение.
        // На месте отступа отображается изображение.
        return !props.expanderSize && displayExpanderPadding;
    }
}
...
function getExpanderPaddingClassName(props: IExpanderProps): string {
    ...
    const expanderSize = props.expanderSize || 'default';
    let className =
        'controls-Tree__row-expanderPadding tw-flex-shrink-0 tw-invisible';
...
function LevelPaddingsComponent(
    props: ILevelPaddingsComponentProps
): React.ReactElement {
    ...
    if (props.expanderSize && props.levelIndentSize) {
        ...
        if (
            SIZES.indexOf(props.expanderSize) >=
                SIZES.indexOf(props.levelIndentSize) &&
            props.expanderPosition === 'default'
        ) {
...
function ExpanderBlockComponent(props: IExpanderProps): React.ReactElement {
    ...
    return (
        <div className={className}>
            ...
            />
            {isDefaultPosition && (
                <ExpanderComponent {...props} className={undefined} />
            )}
