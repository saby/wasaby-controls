...
const _private = {
    ...
    loadNodeChildren(
        ...
        const loader = self._options.slice || sourceController;
        return loader
...
> extends BaseControl<TOptions> {
    ...
    private _getLastExpandedNodeKey(direction: Direction, item: TreeItem): CrudEntityKey | null {
        ...
        } else if (item.isExpanded() && item.hasMoreStorage('forward')) {
            return itemKey;
        }
        return null;
    }
    ...
    protected _loadMore(direction: Direction): void | Promise<RecordSet | void> {
        ...
        // Если был получен key узла, то загружаем его дочерние элементы (предварительно проверив их наличие)
        // https://online.sbis.ru/opendoc.html?guid=130e65d9-6c77-466b-be17-3a5372567db4&client=3
        const sourceController = this.getSourceController();
        const loader = this._options.slice || sourceController;
        if (lastExpandedNodeKey && loader.hasMoreData(direction, lastExpandedNodeKey)) {
    ...
    protected _shouldHandleItemMouseUp(item: TreeItem): boolean {
        return !item.$TNF && !item['[Controls/tree:TreeNodeHeaderItem]'];
    }
    ...
    protected _afterReloadCallback(options: TOptions, loadedList: RecordSet): void {
        if (this._listViewModel) {
            ...
            // TODO https://online.sbis.ru/opendoc.html?guid=58c797b4-c320-4fa3-9273-3e03bfa2cb98&client=3
            //  Установка рута из sourceController напрямую потенциально приводит к нежелательным эффектам.
            //  Кейс: Смена viewMode searchTile -> tile, search -> table
            //  Здесь происходит попытка установки нового рута в поисковую коллекцию, хотя items ещё не набраны.
            //  Далее поисковая коллекция пытается построить хлебные крошки из нового рута и старого рута.
            //  У старого рута нет физической записи коллекции, его contents === ключ.
            //  Хлебная крошка вызывает от него getKey и весь реестр умирает. Слайс при этом отрабатывает корректно,
            //  и если на этом уровне не устанавливать root в коллекцию, предоставив это слайсу, то ошибки нет.
            //  Совместной отладкой с интеракторами пришли к тому, что пока делаем костыль-опцию fix88221034906708
            //  И смотрим. Если всё будет работать, то стоит задуматься о необходимости кода ниже.
            if (options.fix88221034906708 !== true && sourceControllerRoot !== undefined) {
