...
const _private = {
    ...
    reloadItem(
        ...
        reloadFilter[collection.getParentProperty()] =
            getRootsForHierarchyReload(collection, key);
...
> extends BaseControl<TOptions> {
    ...
    constructor(options: TOptions, context?: object) {
        ...
        this._nodeDataMoreLoadCallback =
            this._nodeDataMoreLoadCallback.bind(this);
    ...
    protected _beforeMount(
        ...
        const doBeforeMount = () => {
            ...
            if (options.sourceController) {
                ...
                const sourceControllerState =
                    options.sourceController.getState();

                if (
                    (options.parentProperty &&
                        sourceControllerState.parentProperty !==
                            options.parentProperty) ||
                    (options.root !== undefined &&
                        options.root !== sourceControllerState.root)
                ) {
    ...
    protected _afterMount(): void {
        ...
        if (this._expandedItemsToNotify) {
            if (this._options.onExpandedItemsChanged) {
                this._options.onExpandedItemsChanged(
                    this._expandedItemsToNotify
                );
            } else {
    ...
    private _canLoadNodeDataOnScroll(
        ...
        const hasMoreParentData =
            ...
            this._sourceController.hasMoreData('down', parentKey);
     ...
     * @private
     */
    private _loadNodeChildrenRecursive(
        item: TreeItem
    ): Promise<RecordSet | void> {
        const nodeKey = item.getContents().getKey();
        const hasMoreData = this._sourceController?.hasMoreData(
            'down',
            nodeKey
        );
    ...
    protected _loadMore(
        ...
        if (
            ...
            return this._loadNodeChildrenRecursive(lastRootItem).then(
                (result) => {
    ...
    private _updateTreeControlModel(newOptions: TOptions): void {
        ...
        if (
            newOptions.nodeFooterVisibilityCallback !==
            this._options.nodeFooterVisibilityCallback
        ) {
            viewModel.setNodeFooterVisibilityCallback(
                newOptions.nodeFooterVisibilityCallback
            );
        }

        if (
            newOptions.nodeFooterTemplate !== this._options.nodeFooterTemplate
        ) {
        ...
        if (
            newOptions.expanderVisibility !== this._options.expanderVisibility
        ) {
        ...
        if (
            newOptions.withoutLevelPadding !== this._options.withoutLevelPadding
        ) {
        ...
        if (
            newOptions.hasChildrenProperty !== this._options.hasChildrenProperty
        ) {
    ...
    protected _startBeforeUpdate(newOptions: TOptions): void {
        ...
        if (
            typeof newOptions.root !== 'undefined' &&
    ...
    protected _endBeforeUpdate(newOptions: TOptions): void {
        ...
        const isSourceControllerLoading =
            sourceController && sourceController.isLoading();
        ...
        const expandedItemsChanged =
            newOptions.expandedItems &&
            !isEqual(newOptions.expandedItems, currentExpandedItems);
        ...
        if (wasResetExpandedItems) {
            ...
            if (
                ...
                if (viewModel) {
                    ...
                    this._expandController.setExpandedItems(
                        newOptions.expandedItems
                    );
            ...
            if (
                ...
                if (newOptions.childrenLoadMode === 'always') {
                    sourceController.resetCollapsedNodes(
                        newOptions.expandedItems
                    );
                }
        ...
        if (sourceController) {
            sourceController.setNodeDataMoreLoadCallback(
                this._nodeDataMoreLoadCallback
            );
    ...
    private _applyNewRoot(newRoot: CrudEntityKey): void {
        if (this._itemActionsController) {
            this._itemActionsController = null;
        }

        this._listViewModel.setRoot(newRoot);
    }
    ...
    reloadItems(ids: TKey[]): Promise<RecordSet | Error> {
        ...
        filter[this._options.parentProperty] = getReloadItemsHierarchy(
            collection,
            ids
        );
    ...
    protected _onItemClick(
        ...
        const result = super._onItemClick(
            event,
            contents,
            originalEvent,
            columnIndex
        ...
        if (item && result !== false) {
            const shouldToggleExpanded = this._shouldExpandItemByClick(
                item,
                originalEvent
            );
    ...
    private _shouldExpandItemByClick(
        ...
        const clickOnExpander = clickEvent.target.closest(
    ...
    private _onExpanderMouseUp(event: MouseEvent, item: TreeItem): void {
        ...
        if (
            this._mouseDownExpanderKey === item.key &&
            MouseUp.isButton(event, MouseButtons.Left)
        ) {
            this._toggleExpanded(item);
    ...
    _onViewKeyDown(event): void {
        if (
            this._options.supportExpand !== false &&
            this._listViewModel.SupportExpand !== false
        ) {
    ...
    protected _afterReloadCallback(
        ...
        if (this._listViewModel) {
            ...
            const root = options.root !== undefined ? options.root : this._root;
    ...
    protected _onCollectionChangedScroll(
        ...
        if (
            ...
            this._listVirtualScrollController.setPredicatedRestoreDirection(
                'backward'
            );
        }
    ...
    protected _loadedItemsIsHidden(loadedItems: RecordSet): boolean {
        const itemIsVisible = (item: Model) => {
            ...
            const itemFromNewRoot =
                item.get(this._listViewModel.getParentProperty()) === newRoot;
            return hasItem || (rootChanged && itemFromNewRoot);
        };
        return (
            loadedItems &&
            factory(loadedItems).filter(itemIsVisible).count() === 0
        );
    }
    private _setMarkerOnFirstLeaf(
        options: ITreeControlOptions,
        startKey?: CrudEntityKey
    ): void {
    ...
    private _getMarkedLeaf(
        ...
        const hasNextLeaf =
            model.getLast('Markable') !== model.getItemBySourceKey(key) ||
            model.hasMoreData();
        let hasPrevLeaf = false;
        for (let i = index - 1; i >= 0; i--) {
            if (
                model.at(i).isNode() === null ||
                !this._isExpanded(model.at(i).getContents())
            ) {
                hasPrevLeaf = true;
                break;
            }
        ...
        return new Promise((resolve) => {
            ...
            const goToNextItem = () => {
                ...
                const markerController =
                    mController || this.getMarkerController();
                if (item) {
                    ...
                    if (item.get(this._options.nodeProperty) !== null) {
                        ...
                        if (this._isExpanded(item)) {
                            ...
                            const expandResult = this.toggleExpanded(
                                this._tempItem,
                                model
                            );
    ...
    getMarkedNodeKey(): TKey {
        ...
        return currentRoot.isRoot()
            ? currentRoot.contents
            : currentRoot.contents.getKey();
    }

    private _applyMarkedLeaf(
        key: CrudEntityKey,
        model,
        markerController
    ): void {
    ...
    protected _getFooterSpacingClasses(options): string {
        ...
        if (
            this._listViewModel &&
            this._listViewModel['[Controls/_display/Tree]']
        ) {
            const expanderVisibility =
                this._listViewModel.getExpanderVisibility();
            ...
            if (hasExpander) {
                ...
                result += `__expanderPadding-${
                    options.expanderSize || 'default'
                }`;
            }
        }
    ...
    protected _shouldCancelEditing(toggleItemKeys: CrudEntityKey[]): boolean {
        ...
        if (
            ...
            const editingKey = this._editInPlaceController
                .getEditableItem()
                .getContents()
                .getKey();
    ...
    private _prepareHasMoreStorage(
        ...
        realExpandedItems.forEach((nodeKey) => {
            hasMore[nodeKey] = {
                backward: sourceController
                    ? sourceController.hasMoreData('up', nodeKey)
                    : false,
                forward: sourceController
    ...
    private _doExpand(item: TreeItem): Promise<unknown> {
        ...
        const doExpand = () => {
            return new Promise((resolve) => {
                return Promise.resolve(
                    this._expandController.toggleItem(nodeKey)
                )
                    .then((result) => {
                        if (this._destroyed) {
        ...
        this._options.notifyCallback(expanded ? 'itemExpand' : 'itemCollapse', [
            contents,
        ]);
    ...
    private _expandToFirstLeaf(
        ...
        if (items.getCount()) {
            ...
            while (
                curItem &&
                curItem.contents.get(options.nodeProperty) !== null
            ) {
    ...
    private _resetExpandedItems(): void {
        ...
        if (
            this._shouldCancelEditing(this._expandController.getExpandedItems())
        ) {
    ...
    private _changeExpandCollapseState(
        ...
        if (collapsedItemsChanged) {
            this._options.notifyCallback('collapsedItemsChanged', [
                collapsedItems,
            ]);
        }
    }
    ...
    protected _nodeDataMoreLoadCallback(
        nodeKey: CrudEntityKey,
        items: RecordSet
    ): void {
