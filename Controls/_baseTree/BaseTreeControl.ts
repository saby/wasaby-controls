...
> extends BaseControl<TOptions> {
    ...
    protected _afterMount(): void {
        ...
        if (this._expandedItemsToNotify) {
            if (checkWasabyEvent(this._options.onExpandedItemsChanged)) {
    ...
    protected _onItemClick(
        ...
        if (item && result !== false && !this._options.useCollection) {
    ...
    protected _notifyItemClick(
        ...
        if (originalEvent.target.closest('.js-controls-Tree__row-expander')) {
            if (this._options.useCollection) {
                this._options.onExpanderClick(item.key);
            }
    ...
    protected _setMarkerOnItemClick(contents: Model | Model[], originalEvent: MouseEvent): void {
        ...
        super._setMarkerOnItemClick(contents, originalEvent);
    }
    ...
    private _expandLoader(nodeKey: TKey): void | Promise<RecordSet | void> {
        ...
        const shouldLoadChildren = this._options.sourceController || !this._options.items;
        const item = this._listViewModel.getItemBySourceKey(nodeKey);

        if (item?.isRoot() || !shouldLoadChildren || this._expandController.isAllExpanded()) {
            return;
        }

        let expandResult;

        if (checkWasabyEvent(this._options.onLoadExpandedItem)) {
            expandResult = this._options.onLoadExpandedItem(nodeKey);
        } else {
            expandResult = this._notify('loadExpandedItem', [nodeKey]) as Promise<RecordSet | void>;
        }
