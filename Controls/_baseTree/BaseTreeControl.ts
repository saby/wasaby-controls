...
import Tree, { IHasMoreStorage, IOptions as ITreeCollectionOptions } from './display/Tree';
...
import { BASE_TREE_CONTROL_DEFAULT_OPTIONS } from './compatibility/BaseTreeControlEmulationCompatibility';
...
> extends BaseControl<TOptions> {
    ...
    protected _draggingItemMouseMove(item: TreeItem, event: MouseEvent): boolean {
        ...
        if (!item.isExpanded() && position.position === 'on') {
            this._timeoutForExpandOnDrag = setTimeout(() => {
                this._options.expand(item.key, this, this._options);
    ...
    protected _beforeStartDrag(draggedKey: CrudEntityKey): void {
        ...
        if (draggedItem && draggedItem.isNode() !== null) {
            this._options.beforeStatDragCompatible(draggedKey, this, this._options);
        }
    }
    ...
    private _applyMarkedLeaf(key: CrudEntityKey, model, markerController): void {
        ...
        if (this._isMounted) {
            this._options.slicelessChangeMarkedKey(this, this._options, this._currentItem);
        } else {
