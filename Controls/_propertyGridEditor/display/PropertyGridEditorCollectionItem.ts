...
export default class PropertyGridCollectionItem<T> extends TreeGridDataRow<T> {
    ...
    getItemPaddingClasses(
        ...
        const captionPosition =
            templateCaptionPosition || this.getCaptionPosition();
        const totalColumns =
            !gridColumnIndex ||
            captionPosition === 'top' ||
            captionPosition === 'none'
    ...
    isEditable(): boolean {
        return (
            this.contents.get('isEditable') &&
            !!(this.getEditorTemplateName() || this.isNode())
        );
    }
    ...
    getUid(): string {
        return `property-grid-item-${this.getContents().get(
            this._$keyProperty
        )}`;
    }
    ...
    isFocused(): boolean {
        ...
        const currentName = this.getContents().get(
            this.getOwner().getKeyProperty()
        );
        return editorName === currentName;
    }

    getJumpingLabel(): boolean {
        const itemJumpingLabel =
            this.contents.get('editorOptions')?.jumpingLabel;
