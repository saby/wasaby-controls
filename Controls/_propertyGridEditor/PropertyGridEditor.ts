...
export default class PropertyGridEditor extends Control<IPropertyGridEditorOptions> {
    ...
    protected _itemClick(
        ...
        const isExpandClick = clickEvent?.target.closest(
            '.controls-PropertyGridEditor__editor-expander'
        );
        if (isExpandClick) {
            event.stopPropagation();
            const collectionItem = this._getViewModel()?.getItemBySourceItem(displayItem);
            const expandedItems = this._getExpandedItemsAsArray(collectionItem);
            this._getViewModel().setExpandedItems(expandedItems);
            nodeHistoryUtil.store(expandedItems, this._options.propStorageId);
        } else {
            this._notify('itemClick', [event, displayItem, clickEvent]);
        }
    }
    ...
    private _getExpandedItemsAsArray(item: PropertyGridCollectionItem<Model>): TKey[] {
        let currentExpandedItems = [...this._getViewModel()?.getExpandedItems()];
    ...
    /**
     * Прокрутка до элемента с именем равным значению key
     * @param key
     * @param position
     * @param force
     * @public
     */
    async scrollToItem(key: string | number, position: string, force: boolean) {
        return this._children.listRender.scrollToItem(key, position, force);
    }
