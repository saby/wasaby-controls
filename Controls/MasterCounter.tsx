...
function MasterCounter(
    ...
    return (
        <div
            ref={ref}
            className={`controls__MasterCounter__wrapper ${
                !counters?.length ? 'controls__MasterCounter__wrapper_with-arrow' : ''
            }`}
        >
            {counters?.length ? (
                counters.map((counter, index) => (
                    <div
                        ...
                        onClick={(event) => onClick(event, counter)}
                        data-qa={
                            index
                                ? 'controls-EditorList__additionalCounter'
                                : 'controls-EditorList__mainCounter'
                        }
                    >
                ...
                <Icon
                    iconSize="s"
                    dataQa="controls-MasterCounter__ArrowTimeForward"
                    icon="icon-ArrowTimeForward"
                    iconStyle="label"
                    onClick={onClick}
                    className={`${
                        props.isAdaptive
                            ? 'controls__MasterCounter__arrow_adaptive'
                            : 'controls__MasterCounter__arrow'
                    }`}
                />
            )}
        </div>
