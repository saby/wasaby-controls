...
const SCALABLE_DIMENSIONS_VALUES = [
    'height',
    'width',
    'top',
    'bottom',
...
class DimensionsMeasurer {
    private _zoomCache: Map<HTMLElement, number> = new Map<
        HTMLElement,
        number
    >(); // Иначе на любой чих меряется DOM.
    /**
    ...
    protected _getMouseCoordsByMouseEvent(
        ...
        if (event.touches || event.changedTouches) {
            ...
            Logger.error(
                'DimensionsMeasurer: Event type must be must be mouse or touch event.'
            );
        }
    }
     ...
     * @protected
     */
    protected _getBoundingClientRect(
        element: HTMLElement,
        scaleToBodyZoom: boolean
    ): DOMRect {
    ...
    protected _getElementDimensions(
        ...
        const zoomValue =
            calculatedZoomValue === undefined
                ? this.getZoomValue(element)
                : calculatedZoomValue;

        // Если передали расчитанный зум, то надо скейлить
        const forcedScale =
            scaleToBodyZoom || calculatedZoomValue !== undefined;
