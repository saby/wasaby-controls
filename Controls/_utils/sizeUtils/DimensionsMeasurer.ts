...
class DimensionsMeasurer {
    ...
    private _enableNewZoom: boolean = false;

    constructor() {
        this._setEnableNewZoom();
    }

    private _setEnableNewZoom(): void {
        ...
        // Значения в getBoundingClientRect не учитывают zoom, а clientHeight - учитывают.
        // При этом ЯБ может определяться как хром и корректно проставлять версию в useragent. А может определиться как
        // yandex и вообще не иметь версии хрома. Из-за этой особенности будем сами определять, нужно ли включить работу с новым зумом.
        if (detection.chromeVersion >= 128 || detection.EdgeVersion >= 128) {
            this._enableNewZoom = true;
            return;
        }
        const element = document?.querySelector?.('.controls-Zoom');
        if (element) {
            const elementRectWidth = element.getBoundingClientRect().width;
            const elementClientWidth = element.clientWidth;
            this._enableNewZoom =
                elementRectWidth - 1 >= elementClientWidth ||
                elementRectWidth + 1 <= elementClientWidth;
        }
    }
    /**
    ...
    protected _getScaledElementDimensions<T>(
        ...
        needToScale: boolean = true
    ): T {
        return fields.reduce((accumulator, field) => {
            if (
                this._enableNewZoom &&
                // false передадут в том случае, если мы вычисляем clientHeight, а он уже и так учитывает зум.
                needToScale
            ) {
                accumulator[field] = element[field] / zoom;
                if (scaleToBodyZoom) {
                    accumulator[field] = this._calcScaledValue(
                        accumulator[field],
                        zoom,
                        scaleToBodyZoom
                    );
                ...
                accumulator[field] = this._calcScaledValue(
                    accumulator[field] || element[field],
                    zoom,
                    scaleToBodyZoom
                );
            }
