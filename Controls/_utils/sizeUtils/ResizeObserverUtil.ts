...
export default class ResizeObserverUtil {
    ...
    constructor(
        ...
        this._resizeObserverSupported =
            constants.isBrowserPlatform && window.ResizeObserver;
        this._resizeObserverCallback = resizeObserverCallback.bind(
            this._control
        );
        ...
        if (!controlResizeCallback) {
            ...
            this._controlResizeCallback = controlResizeCallback.bind(
                this._control
            );
        }
    }
    ...
    observe(
        ...
        options?: { box: RESIZE_OBSERVER_BOX }
    ): void {
        if (this._resizeObserverSupported) {
            this._initResizeObserver();
