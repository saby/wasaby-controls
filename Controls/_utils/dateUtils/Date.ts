...
export function getEndOfQuarter(date: Date): Date {
    return new date.constructor(
        date.getFullYear(),
        (Math.floor(date.getMonth() / 3) + 1) * 3,
        0
    );
}
...
export function getEndOfHalfyear(date: Date): Date {
    return new date.constructor(
        date.getFullYear(),
        (Math.floor(date.getMonth() / 6) + 1) * 6,
        0
    );
}
...
export function isStartOfYear(date: Date): boolean {
    return (
        this.isValidDate(date) && date.getDate() === 1 && date.getMonth() === 0
    );
}
...
export function hitsDisplayedRanges(
    date: Date,
    displayedRanges: Date[][]
): boolean {
    if (
        !displayedRanges ||
        (displayedRanges[0][0] === null && displayedRanges[0][1] === null)
    ) {
        return true;
    }
    if (displayedRanges) {
