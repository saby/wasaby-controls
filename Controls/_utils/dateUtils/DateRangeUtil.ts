...
export function shiftPeriod(
    ...
    if (compare.isFullInterval(startValue, endValue, compare.DateUnits.Month)) {
        ...
        result = shiftPeriodByDays(
            startValue,
            endValue,
            direction * periodLengthInDays
        );
    }
    return result;
}
...
export function shiftPeriodByMonth(
    ...
    return [
        new WSDate(
            ...
            startValue.getMonth() + direction,
            1
        ),
        new WSDate(
            endValue.getFullYear(),
            endValue.getMonth() + direction + 1,
            0
        ),
    ];
}
 ...
 * @returns {Number}
 */
export function getPeriodLengthInMonths(
    startValue: Date,
    endValue: Date
): number {
    return (
 ...
 * @param {Date} endValue Конец периода
 */
export function getPeriodLengthInDays(
    startValue: Date,
    endValue: Date
): number {
    ...
    return (
        Math.ceil(
            Math.abs((startValue.getTime() - endValue.getTime()) / oneDay)
        ) + 1
    );
}
...
export function getResetButtonVisible(
    ...
    return (
        (hasResetStartValue && !isDatesEqual(startValue, resetStartValue)) ||
