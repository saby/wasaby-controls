...
> extends Control<T> {
    ...
    protected _registerFormOperationHandler(
        event: Event,
    ...
    protected _isEqualId(oldRecord: Record, newRecord: Record): boolean {
        ...
        if (
            !this._checkRecordType(oldRecord) ||
            !this._checkRecordType(newRecord)
        ) {
            return false;
        }
    ...
    update(): Promise<void> {
        ...
        if (result && result.then) {
            this._notify(
                'registerPending',
                [result, { showLoadingIndicator: false }],
                { bubbling: true }
            );
            result.then(
                (defResult) => {
            ...
            this._notify(
                'registerPending',
                [updatePromise, { showLoadingIndicator: false }],
                { bubbling: true }
            );
