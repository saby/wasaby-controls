...
import {
    CRUD_EVENTS,
    default as CrudController,
    ICrudConfig,
} from 'Controls/_form/CrudController';
...
class FormController extends ControllerBase<IFormController> {
        ...
        if (initializingWay === INITIALIZING_WAY.PRELOAD) {
            ...
            if (
                ...
                const hasKey: boolean =
                    options.entityKey !== undefined &&
                    options.entityKey !== null;
                if (!hasKey) {
                    this._throwInitializingWayException(
                        initializingWay,
                        'entityKey'
                    );
                }
    ...
    protected _beforeUpdate(newOptions: IFormController): void {
        const isPreloadWay =
            newOptions.initializingWay === INITIALIZING_WAY.PRELOAD;
            ...
            this._confirmRecordChangeHandler(
                ...
                () => {
                    this._tryDeleteNewRecord().then(() => {
                        this.read(
                            newOptions.entityKey,
                            newOptions.readMetaData
                        );
                    });
                    updateData();
                }
    ...
    private _createRecordBeforeMount(
        cfg: IFormController
    ): Promise<ICrudResult> {
    ...
    private _readRecordBeforeMount(cfg: IFormController): Promise<ICrudResult> {
        ...
        // до монитрования в DOM не можем сделать notify событий (которые генерируются в CrudController,
        // а стреляются с помощью FormController'а, в данном случае), поэтому будем создавать рекорд напрямую.
        return readWithAdditionalFields(
            cfg.source,
            cfg.entityKey,
            cfg.readMetaData
        )
    ...
    private _readRecordBeforeMountNotify(): void {
        if (!this._readInMounting.isError) {
            this._notifyHandler(CRUD_EVENTS.READ_SUCCESSED, [
                this._readInMounting.result,
            ]);

            // перерисуемся
            this._readHandler(this._record);
        } else {
            this._notifyHandler(CRUD_EVENTS.READ_FAILED, [
                this._readInMounting.result,
            ]);
        }
    ...
    private _createRecordBeforeMountNotify(): void {
        if (!this._createdInMounting.isError) {
            this._notifyHandler(CRUD_EVENTS.CREATE_SUCCESSED, [
                this._createdInMounting.result,
            ]);

            // зарегистрируем пендинг, перерисуемся
            this._createHandler(this._record);
        } else {
            this._notifyHandler(CRUD_EVENTS.CREATE_FAILED, [
                this._createdInMounting.result,
            ]);
        }
    ...
    private _tryDeleteNewRecord(): Promise<undefined> {
        if (this._needDestroyRecord()) {
            return this._options.source.destroy(
                this._getRecordId(),
                this._options.destroyMetaData
            );
        }
        return Promise.resolve();
    }
    ...
    update(config?: IUpdateConfig): Promise<unknown> {
        ...
        const result = this._notify('requestCustomUpdate', [
            this._record,
            config,
        ]);
        ...
        this._notify(
            'registerPending',
            [this._updatePromise, { showLoadingIndicator: false }],
            { bubbling: true }
        );
        ...
        validationDef.then(
            (results) => {
                if (!results.hasErrors) {
                    ...
                    let res = this._crudController.update(
                        record,
                        this._isNewRecord,
                        {
                    ...
                    const validationErrors =
                        this._validateController.getValidationResult();
                    this._notify('validationFailed', [validationErrors]);
                    const error = this._createError(
                        rk('Некорректно заполнены обязательные поля')
                    );
                    updateDef.errback(error);
                }
            },
    ...
    private _getErrorProcessingMode(): ErrorViewMode {
        ...
        return DELAYED_INITIALIZING_WAYS.includes(initializingWay) &&
            this._record
    ...
    private _notifyHandler(
        eventName: string,
        args: [Model, string | number, object?]
    ): void {
        this._notifyToOpener(eventName, args);
        this._notify(eventName, args);
    }

    private _notifyToOpener(
        eventName: string,
        args: [Model, string | number, object?]
    ): void {
        const handlers = {
    ...
    private _getUpdateSuccessedData(
        record: Model,
        key: string,
        config?: object
    ): IResultEventData {
