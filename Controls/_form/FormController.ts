...
export interface IResultEventData<TRecord extends Model = Model> {
 ...
 * @demo Controls-demo/FormController/RegisterFromOperations/Index
 */
...
class FormController extends ControllerBase<IFormController> {
    ...
    private _queryProcessing: boolean = false;
    ...
    protected _beforeMount(
        ...
        if (initializingWay === INITIALIZING_WAY.PRELOAD) {
            ...
            if (
                ...
                this._finishCreatePromiseInMount =
                    initializingWay !== INITIALIZING_WAY.DELAYED_CREATE;
    ...
    protected _beforeUpdate(newOptions: IFormController): void {
        ...
        const isLocalWay = newOptions.initializingWay === INITIALIZING_WAY.LOCAL;
        ...
        if (
            ...
            this._options.record !== newOptions.record
        ) {
        ...
        if (needRead) {
            ...
            if (this._finishCreatePromiseInMount) {
                ...
                this._confirmRecordChangeHandler(() => {
                    this._createMetaDataOnUpdate = createMetaData;
                    updateData();
                    this.create(newOptions.createMetaData)
                        .then(() => {
                        ...
                        .catch((e: Error) => {
                            this.processError(e).then(this._getState);
                        });
                });
            }
        } else {
    ...
    update(config?: IUpdateConfig): Promise<unknown> {
        if (!this._queryProcessing) {
            this._queryProcessing = true;
            ...
            const updateCallback = (updResult) => {
                ...
                if (updResult !== true) {
                    ...
                    this._startFormOperations('save')
                        .then(() => {
                            const res = this._update(config)
                                .then(this._getData)
                                .finally(() => {
                                    this._queryProcessing = false;
                                });
            ...
            this._queryResult = updateResult;
            return updateResult;
        } else {
