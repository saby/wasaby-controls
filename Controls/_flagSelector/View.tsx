...
export default React.forwardRef(function View(props: IFlagSelectorProps, ref) {
    ...
    const [selectedLocaleData, setSelectedLocaleData] =
        React.useState<IAreaCodeData>(() => {
    ...
    const defaultItems = new Memory({
        data: areaCodes,
        keyProperty: 'id',
        filter: (
            model: Model<IAreaCodeData>,
    ...
    const selectedItemsChangedHandler = (event) => {
        const localeId = getArgs(event)[1][0];
    ...
    const itemTemplate = (templateOptions) => {
        return (
            <ItemTemplate
                {...templateOptions}
                marker={false}
    ...
    return (
        <Selector
            ...
            onDropDownOpen={() => {
                if (!revertText) {
                    import('Controls/Utils/keyboardLayoutRevert').then(
                        (res) => {
