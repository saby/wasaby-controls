...
export default class Model extends mixin<VersionableMixin, ObservableMixin>(
    ...
    constructor(options: IModelOptions) {
        ...
        if (options.displayValue) {
            this._textValue = options.displayValue.replace(
                ALL_SPACES_REGEXP,
                this._replacer
            );
            this._value = this._stringValueConverter.getValueByString(
                this._textValue
            );
        } else {
            this._value = options.value;
            this._textValue = this._stringValueConverter.getStringByValue(
                options.value
            );
        }
    ...
    set textValue(value: string) {
        ...
        const newValue = this._stringValueConverter.getValueByString(
            value,
            this._lastValue
        );
    ...
    autocomplete(textValue: string, autocompleteType: string) {
        ...
        if (dateUtils.isValidDate(this.value)) {
            ...
            if (monthDayPart && monthDayPart[1].includes('0')) {
                this._textValue = this._textValue.replace(
                    RegExp(this._replacer, 'g'),
                    '0'
                );
                this._notify('valueChanged', [this._value, this.displayValue]);
                if (this._valueChangedCallback) {
    ...
    setCurrentDate(): void {
        this.value = this._stringValueConverter.getCurrentDate(
            this._lastValue,
            this._mask
        );
    }
    ...
    private _updateDisplayValue(displayValue: string): void {
        const normalizedDisplayValue = displayValue.replace(
            ALL_SPACES_REGEXP,
            this._replacer
        );
        const oldTextValue = this._textValue;
