...
export default class StringValueConverter {
    ...
    getStringByValue(value: Date, mask?: string): string {
        ...
        if (dateUtils.isValidDate(value)) {
            ...
            if (
                ...
                dateString = this._getFormatedStringByValue(
                    value,
                    actualMask,
                    tzOffset
                );
    ...
    private _getFormatedStringByValue(
        value: Date,
        mask: string,
        tzOffset?: number
    ): string {
    ...
    getValueByString(
        ...
        if (
            autoCompleteType &&
            !this._isValueModelFilled(valueModel) &&
            !this._isEmpty(str)
        ) {
            this._autocomplete(
                valueModel,
    ...
    getCurrentDate(baseValue: Date, mask: string): DateTime | Date {
        baseValue = dateUtils.isValidDate(baseValue)
            ? baseValue
            : new Date(1904, 0, 1);
        ...
        if (
            mask.indexOf('DD') > -1 ||
            date > dateUtils.getEndOfMonth(now).getDate()
        ) {
            date = now.getDate();
        }
    ...
    private _isPartlyFilled(value: string): boolean {
        return (
            value &&
            RE_NUMBERS.test(value) &&
            value.indexOf(this._replacer) > -1
        );
    }
    ...
    private _isValueModelFilled(valueModel: IValueModel): boolean {
        for (const value in valueModel) {
            if (
                valueModel.hasOwnProperty(value) &&
                valueModel[value].valid === false
            ) {
                return false;
            }
        }
    ...
    private _getFullYearBy2DigitsYear(valueYear: number): number {
        ...
        // 1935 год.
        const fullYear =
            valueYear <= shortCurYear + 10
                ? curCentury + valueYear
                : curCentury - 100 + valueYear;
        if (
            this._yearSeparatesCenturies &&
            fullYear < this._yearSeparatesCenturies.getFullYear()
        ) {
        ...
        for (i = 0; i < maskItems.length; i++) {
            ...
            if (this._isFilled(strItems[i])) {
                ...
                if (
                    ...
                    const shortStrYear = String(valueObject.value).slice(
                        shortYearLengthStr
                    );
                    ...
                    valueObject.value = this._getFullYearBy2DigitsYear(
                        valueObject.value
                    );
    ...
    private _fillFromBaseValue(valueModel: IValueModel, baseValue: Date): void {
        ...
        if (
            valueModel.date.str === null &&
            this._mask !== Range.dateMaskConstants.MM_YYYY
        ) {
    ...
    private _autocomplete(
        ...
        const getDate = (autocompleteDefaultDate?: number): number => {
            ...
            if (autocompleteType === 'start') {
                ...
                return dateUtils
                    .getEndOfMonth(
                        new Date(valueModel.year.value, valueModel.month.value)
                    )
                    .getDate();
            } else {
                return defaultDate;
        ...
        if (
            ...
            itemValue = parseInt(
                valueModel.year.str.replace(this._replacerRegExp, ' '),
                10
            );
            isZeroAtBeginning =
                valueModel.year.str.split(itemValue)[0].indexOf('0') === -1;
            if (!isNaN(itemValue) && itemValue < 100 && isZeroAtBeginning) {
                setValue(
                    valueModel.year,
                    this._getFullYearBy2DigitsYear(itemValue)
                );
            } else {
                return;
            }
        ...
        for (item in valueModel) {
            if (valueModel.hasOwnProperty(item)) {
                if (!valueModel[item].valid) {
                    ...
                    if (
                        getMaskType(item) === TIME_MASK_TYPE &&
                        this._isPartlyFilled(itemValue)
                    ) {
                        itemValue = itemValue.replace(
                            this._replacerRegExp,
                            '0'
                        );
                    }
    ...
    private _createDate(
        ...
        if (autoCorrect) {
            ...
            endDateOfMonth = dateUtils
                .getEndOfMonth(new dateConstructor(year, month, 1))
                .getDate();
    ...
    private _isValidDate(
        ...
        const lastMonthDay = dateUtils
            .getEndOfMonth(new Date(year, month))
            .getDate();
        return (
            seconds <= MAX_SECONDS &&
            minutes <= MAX_MINUTES &&
