...
export default class StringValueConverter {
    ...
    private _getFullYearBy2DigitsYear(valueYear: number): number {
        ...
        if (this._yearSeparatesCenturies) {
            const startValueCentury =
                this._yearSeparatesCenturies.getFullYear() -
                (this._yearSeparatesCenturies.getFullYear() % 100);
            if (
                startValueCentury === curCentury &&
    ...
    private _parseString(str: string): IValueModel {
        let valueModel = {
            ...
            milliseconds: { str: null, value: 0, valid: false },
        };
        if (this._mask) {
    ...
    private _fillFromBaseValue(valueModel: IValueModel, baseValue: Date): void {
        ...
        if (valueModel.date.str === null && this._mask !== Range.dateMaskConstants.MM_YYYY) {
            ...
            // вводит 12 в качестве месяца и мы получаем 30.12.2018...
        ...
        if (valueModel.milliseconds.str === null) {
            valueModel.milliseconds.value = baseValue.getMilliseconds();
        }
        for (const value in valueModel) {
    ...
    private _createDate(
        ...
        if (!this._isValidDate(year, month, date, hours, minutes, seconds, milliseconds)) {
            return new dateConstructor('Invalid');
        }

        return new dateConstructor(year, month, date, hours, minutes, seconds, milliseconds);
    }
