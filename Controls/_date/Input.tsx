...
import { IMaskOptions } from 'Controls/interface';
...
export interface IDateInput
    ...
    onValueChanged?: (value: Date, displayValue?: string) => void;
    inputTargetRef?: React.Ref<unknown>;
}
 ...
 * Полезные ссылки:
 * <ul>
 *     <li>{@link https://git.sbis.ru/saby/wasaby-controls/-/blob/rc-24.6100/Controls-default-theme/variables/_input.less переменные тем оформления input}</li>
 *     <li>{@link /doc/platform/developmentapl/interface-development/controls/input-elements/date-time/date/ руководство разработчика}</li>
 * </ul>
 *
 * <b>Правило подстановки века</b>
 * Если вводимый год попадает в промежуток [00 - "текущий год + 10 лет"], то указывается текущий век, если нет - подставляется прошлый.
 * Например, дата "01.01.15" понимается как "01.01.2015", а дата "01.01.37" - как "01.01.1937".
 * Рекомендуется использовать маски с 4-х значным годом, если пользовательский сценарий предполагает ввод даты или периода более чем на 10 лет вперед.
 *
...
export default class Input extends React.Component<IDateInput> {
    ...
    private _openerRef: React.RefObject<BaseInput>;
    ...
    constructor(props: IDateInput) {
        ...
        this._setInputTargetRef = this._setInputTargetRef.bind(this);
    ...
    openPopup(): void {
        ...
        if (this.props.datePopupType === 'compactDatePicker') {
            ...
            className = `controls-PeriodDialog__picker controls_datePicker_theme-${this.context.theme}`;
        }
        const cfg = {
            ...
            templateOptions: {
                ...
                selectionPosition: this._openerRef.current?.getCursorPosition?.(),
    ...
    protected _onResult(startValue: Date): void {
        ...
        if (this.props.onValuechanged) {
            this.props.onValuechanged(value, textValue);
        }
        if (this.props.onValueChanged) {
            this.props.onValueChanged(value, textValue);
        }
        ...
        if (this.props.onInputcompleted) {
            this.props.onInputcompleted(value, textValue);
        }
        if (this.props.onInputCompleted) {
            this.props.onInputCompleted(value, textValue);
        }
    ...
    protected _textValueChangedHandler(value: Date): void {
    ...
    private _onSelectionStartChanged(selectionPosition: number): void {
        if (!this.props.calendarButtonVisible) {
            this._selectionPosition = selectionPosition;
        }
        this._proxyEvent('SelectionStartChanged');
    }

    private _proxyEvent(callbackName: string) {
        if (this.props[callbackName]) {
            this.props[callbackName](...Array.prototype.slice.call(arguments, 1));
        }
    }
    ...
    private _setInputTargetRef(element: BaseInput): void {
        this._openerRef.current = element;
        if (this.props.inputTargetRef) {
            this.props.inputTargetRef(element?._container || null);
        }
    }
            ...
            >
                <BaseInput
                    ...
                    onValidateFinished={(result) => {
                        this._proxyEvent('onValidateFinished', result);
                    }}
                    onValueChanged={(value, textValue) => {
                        this._proxyEvent('onValueChanged', value, textValue);
                    }}
                    onTextValueChanged={this._textValueChangedHandler.bind(this)}
                    onInputCompleted={(value, textValue) => {
                        this._proxyEvent('onInputCompleted', value, textValue);
                    }}
                    onTagClick={(tag) => {
                        this._proxyEvent('onTagClick', tag);
                    }}
                    onClick={() => {
                        this._inputClickHandler();
                        this._proxyEvent('onClick');
                    }}
                    onTagHover={() => {
                        this._proxyEvent('onTagHover');
                    }}
                    onSelectionEndChanged={() => {
                        this._proxyEvent('onSelectionEndChanged');
                    }}
                    onFocus={(event) => {
                        this._proxyEvent('onFocus', event);
                    }}
                    onBlur={(event) => {
                        this._proxyEvent('onBlur', event);
                    }}
                    customEvents={[
                        'onTextValueChanged',
                        'onValueChanged',
...
/**
 * @event Controls/date:Input#validateFinished Происходит после завершения валидации.
...
/**
 * @name Controls/date:Input#inputTargetRef
 * @cfg {React.Ref<unknown>} React Ref, в который инициализируется HTMLElement поля ввода без иконки календаря.
 * @remark Используется для указания поля ввода даты в качестве таргета у Controls/popupTargets:InfoboxTarget
 * @example
 * <InfoboxTarget>
 *    {(infoboxTargetProps) => {
 *       <Input inputTargetRef={infoboxTargetProps.forwardedRef}/>
 *    }
 * </InfoboxTarget>
 */
