    ...
    private _proxyEvent(event: Event, eventName: string) {
        const callCallback = (callbackName, props) => {
            if (this.props[callbackName]) {
                this.props[callbackName](event, ...Array.prototype.slice.call(props, 2));
                return true;
            }
            return false;
        };
        ...
        // TODO: Проверить, возможно уже неактуально
        const callbackCalled = callCallback(`on${eventNameArr.join('')}`, arguments);
        if (!callbackCalled) {
            ...
            >
                <BaseInput
                    ...
                    onValueChanged={(event, value, textValue) => {
                        this._proxyEvent(event, 'ValueChanged', value, textValue);
                    }}
                    onTextValueChanged={this._textValueChangedHandler.bind(this)}
                    onInputCompleted={(event, value, textValue) => {
                        this._proxyEvent(event, 'InputCompleted', value, textValue);
                    }}
                    onTagClick={(event, tag) => {
                        this._proxyEvent(event, 'TagClick', tag);
                    }}
                    onClick={(event) => {
                    ...
                    onFocus={(event) => {
                        this._proxyEvent(event, 'Focus');
                    }}
                    onBlur={(event) => {
                        this._proxyEvent(event, 'Blur');
                    }}
                    customEvents={[
                        'onTextValueChanged',
                        'onValueChanged',
