...
import { getWasabyContext, IWasabyContextValue } from 'UI/Contexts';
...
export default class LinkView extends React.Component<ILinkView> {
    ...
    private _shouldUpdatedPassed: boolean;
    private _ref: {};
    ...
    shouldComponentUpdate(
        props: ILinkView,
        newSate: null,
        newContext: IWasabyContextValue
    ): boolean {
        const oldCaption = this._caption;
        const oldFontColorStyle = this._fontColorStyle;
        this._updateProps(props, newContext?.readOnly ?? props.readOnly);
        const shouldUpdate =
            props.value !== this.props.value ||
            oldFontColorStyle !== this._fontColorStyle ||
            this.props.validationStatus !== props.validationStatus ||
            props.fontWeight !== this.props.fontWeight ||
            props.fontSize !== this.props.fontSize ||
            props.readOnly !== this.props.readOnly ||
            this._caption !== oldCaption;
        if (shouldUpdate) {
    ...
    UNSAFE_componentWillUpdate(
        ...
        // На случай, если не сработал shouldComponentUpdate
    ...
    private _updateProps(props: ILinkView, readOnly: boolean): void {
        this._resetButtonVisible = this._getResetButtonVisible(props);
        this._caption = this._getCaption(props);
        this._fontColorStyle = this._getFontColorStyle(props.fontColorStyle, readOnly);
    }
    ...
    private _formatDateCaption(startValue: Date, endValue: Date, emptyCaption: string): string {
        return dateRangeFormatter(startValue, endValue, {
            undefinedPeriod: emptyCaption || '\xA0',
        });
    }
    ...
    private _mouseDownHandler(event: Event): void {
        ...
        if (!this.context.readOnly && !this.props.readOnly && this.props.onLinkClick) {
            this.props.onLinkClick(event);
        }
    }

    private _touchStartHandler(): void {
        if (!detection.isMobilePlatform) {
            return;
        }
        if (!this.context.readOnly && !this.props.readOnly && this.props.onLinkClick) {
    ...
    private _mouseEnterHandler(event: SyntheticEvent): void {
    ...
    getArrowButtonReadOnly(direction: 'left' | 'right'): boolean {
        if (direction === 'left') {
            return this.props.startValue <= new Date(1400, 0);
        }
        const now = new Date();
        return this.props.endValue >= new Date(now.getFullYear() + 1000, 0);
    }

    render() {
        if (!this._fontColorStyle) {
            this._fontColorStyle = this._getFontColorStyle(
                this.props.fontColorStyle,
                this.context.readOnly ?? props.readOnly
            );
        }
        ...
        return (
            ...
            >
                {!this.context.readOnly && !this.props.readOnly ? (
                    <Button
                        ...
                        className={`controls-DateLinkView__button-value controls-DateLinkView_fontWeight-${this.props.fontWeight}`}
                ...
                {this._resetButtonVisible && !this.context.readOnly && !this.props.readOnly ? (
                    <span
                        className="controls-DateLinkView__resetButton
                        controls-icon icon-Close"
                        data-qa="DateLinkView__closeButton"
                        onClick={this._resetButtonClickHandler}
                    ></span>
                ) : null}
                <div
                    className={
                        this.props.validationStatus && this.props.validationStatus !== 'valid'
