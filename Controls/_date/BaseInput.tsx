...
function BaseInputContent(props): React.ReactElement {
    ...
    return (
        <Mask
            selectionStart={props.selectionPosition}
            selectionEnd={props.selectionPosition}
            {...newProps}
            onFocusin={onFocusIn}
...
function BaseInputStretcherTemplate(props): React.ReactElement {
    ...
    return (
        <div className='controls-BaseInput__stretcher__container'>
            <div className='controls-InputBase__stretcher-block'>
                {
                    getMask()
                }
            </div>
...
> extends React.Component<T> {
    ...
    protected _focusHandler(event: Event, focus: boolean) {
    ...
    private _onInputControl(): void {
        if (checkWasabyEvent(this.props.onInputControl)) {
    ...
    private _onSelectionStartChanged(position: number): void {
        ...
        if (this._placeholder && !this._model.clearTextValue) {
            selectionPosition = 0;
        }
    ...
    render() {
        return (
            <this._validationContainer
                data-qa={this.props['data-qa'] || this.props.attrs?.['data-qa'] || 'controls-BaseInput-Date'}
                content={BaseInputContent}
                stretcherContentTemplate={
                    this.props.placeholder ?
                        <BaseInputStretcherTemplate placeholder={this.props.placeholder} mask={this.props.mask}/>
                        : null
                }
                ...
                onInputCompleted={withWasabyEventObject(
                    this._inputCompletedHandler.bind(this)
                )}
                onTagClick={withWasabyEventObject(this._onTagClick.bind(this))}
                onTagHover={withWasabyEventObject(this._onTagHover.bind(this))}
                onValueChanged={withWasabyEventObject(
                    this._valueChangedHandler.bind(this)
                )}
