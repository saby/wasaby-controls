...
import { IExtendedTimeFormat } from 'Controls/date';
...
const BaseInputContent = React.forwardRef(function InputContent(props, ref): React.ReactElement {
    ...
    React.useEffect(() => {
        props.setGetCursorPositionCallback(inputRef.current?.getCursorPosition);
        if (props.selectionPosition) {
            inputRef.current?.setCursorPosition(props.selectionPosition);
        }
    }, []);
    ...
    // {...props} - передавая таким образом пропсы дочерним контролам прокидываются ВСЕ опции
...
export default class BaseInput<T extends IDateBaseOptions> extends React.Component<T> {
    ...
    _setGetCursorPositionCallback(callback: Function): void {
        this.getCursorPosition = callback;
    }
    ...
    render() {
        return (
            <this._validationContainer
                ...
                setGetCursorPositionCallback={this._setGetCursorPositionCallback.bind(this)}
                onFocusin={(event) => {
