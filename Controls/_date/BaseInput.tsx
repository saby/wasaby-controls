...
import IBaseInputMask, {
    IBaseInputMaskOptions,
} from 'Controls/_date/interface/IBaseInputMask';
...
function BaseInputContent(props): React.ReactElement {
    ...
    return (
        <Mask
            ...
            validationStatus={props.getValidationStatus(props.validationStatus)}
        />
    );
}

export default class BaseInput<
    T extends IDateBaseOptions
> extends React.Component<T> {
    ...
    protected _focusHandler(focus: boolean) {
        if (this.props.placeholder) {
            ...
            // TODO: https://online.sbis.ru/opendoc.html?guid=ac6cc306-8f4e-4fef-98d2-fcbde092381e&client=3
            setTimeout(() => {
                this.setState(() => {
                    return {
    ...
    protected _valueChangedHandler(event: SyntheticEvent<KeyboardEvent>): void {
        const [_event, value, textValue] = getArgs(event);
        ...
        if (!this.props.readOnly) {
            ...
            if (key === constants.key.plus || key === constants.key.minus) {
                ...
                if (this._model.value) {
                    ...
                    const localDate = new this._dateConstructor(
                        this._model.value
                    );
    ...
    private _updateValidationController(props: {
        validateByFocusOut: boolean;
    }): void {
        this._validationContainer = props.validateByFocusOut
            ? InputContainer
            : Container;
    }
    ...
    private _updateDateConstructor(props, oldProps?): void {
        if (!oldProps || props.mask !== oldProps.mask) {
            this._dateConstructor =
                props.dateConstructor || this._getDateConstructor(props.mask);
        }
    }
    ...
    private _onTagClick(event: Event): void {
        const eventArguments = getArgs(event);
        if (this.props.onTagclick) {
            this.props.onTagclick(...eventArguments);
        }
        if (this.props.onTagClick) {
            this.props.onTagClick(...eventArguments);
        }
    ...
    private _onTagHover(event: Event): void {
        const eventArguments = getArgs(event);
        if (this.props.onTaghover) {
            this.props.onTaghover(...eventArguments);
        }
        if (this.props.onTagHover) {
            this.props.onTagHover(...eventArguments);
        }
    ...
    private _onInputControl(event: Event): void {
        if (this.props.onInputControl) {
            this.props.onInputControl(event);
        }
    }
    ...
    render() {
        return (
            <this._validationContainer
                ...
                onInputCompleted={this._inputCompletedHandler.bind(this)}
                onTagClick={this._onTagClick.bind(this)}
                onTagHover={this._onTagHover.bind(this)}
