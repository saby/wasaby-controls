        ...
        Object.entries(loadResults).forEach(([key, value]) => {
            ...
            if (!factory) {
                const isAvailableType =
                    valueIsObject && value.type && !!COMPATIBLE_MAP_TYPE[value.type];
            ...
            const configWithFactoryFormat =
                config && !DataFormatConverter.isOldDataFormat({ [key]: config });
            const valueWithFactoryFormat =
                valueIsObject && !DataFormatConverter.isOldDataFormat({ [key]: value });
