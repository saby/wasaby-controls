...
export const CompatibleContextProvider = React.memo(
    React.forwardRef(
        (props: IOptions, ref: React.RefObject<HTMLElement>): JSX.Element => {
            const isOldOptionsFormat =
                props.value && !(props.loadResults && props.configs);
            const isEmptyProps =
                !props.value && !(props.loadResults && props.configs);
            const isOldOptionsWithNew =
                props.value && props.loadResults && props.configs;
            ...
            const isNotExistConfigs =
                props.configs &&
                props.loadResults &&
                Object.keys(props.loadResults).length !==
                    Object.keys(props.configs).length;
            if (isEmptyProps) {
                Logger.error(
            ...
            const { configs, loadResults } = React.useMemo(() => {
                if (isOldOptionsFormat) {
                    return {
                        configs:
                            DataFormatConverter.convertLoadResultsToFactory(
                                props.value
                            ),
