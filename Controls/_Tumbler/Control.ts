...
import { ITumblerItem } from './Interfaces/ITumblerItem';
import { TouchDetect } from 'EnvTouch/EnvTouch';
import { DimensionsMeasurer, getTextWidth } from 'Controls/sizeUtils';
...
export interface ITumblerOptions
    extends IItemTemplateListProps,
        IContrastBackgroundOptions,
        IBackgroundStyleOptions,
        ISingleSelectableOptions,
        IControlOptions,
...
export default class Tumbler extends Control<ITumblerOptions> {
    ...
    protected _beforeMount(options: ITumblerOptions): void {
        ...
        this._markedKey = options.selectedKey;
    ...
    protected _beforeUpdate(newOptions: ITumblerOptions): void {
        if (
            ...
            this._options.itemTemplate !== newOptions.itemTemplate
        ) {
        ...
        if (this._options.selectedKey !== newOptions.selectedKey) {
            ...
            this._markedKey = newOptions.selectedKey;
        }
    ...
    protected _onMouseEnterHandler(e: SyntheticEvent, item: Model): void {
        if (!TouchDetect.getInstance().isTouch()) {
            if (!item.get('tooltip')) {
                ...
                const caption =
                    item.get(this._options.displayProperty || 'title') || item.get('caption');
                const captionWidth = getTextWidth(caption, undefined, true);
                const elWidth =
                    (e.currentTarget as HTMLElement)?.children[0]?.clientWidth ??
                    (e.currentTarget as HTMLElement)?.clientWidth;
                if (captionWidth > elWidth) {
                    item.set('tooltip', caption);
                    // Принудительно вызываем перерисовку, чтобы значение в title поменялось.
                    // Если этого не делать, то подсказка покажется после перерисовку.
                    this.setState({});
                }
            }
        }
    ...
    protected _onItemClick(event: SyntheticEvent<Event>, item: Model): void {
        ...
        const isNewItemMarked = currentItemClick !== this._markedKey;
        if (!this._options.readOnly && isNewItemSelected) {
            this._selectedKey = nextSelectedKey;
        ...
        if (!this._options.readOnly && isNewItemMarked) {
            this._markedKey = nextSelectedKey;
        }
    }
    ...
    protected _keyUpHandler(e: SyntheticEvent<KeyboardEvent>): void {
        if (e.nativeEvent.keyCode === constants.key.space && !this._options.readOnly) {
            e.preventDefault();
            const newActiveItem = this._options.items.getRecordById(this._markedKey);
            if (newActiveItem) {
    ...
    protected _arrowKeysHandler(e: SyntheticEvent<KeyboardEvent>): void {
        if (!this._highlightedOnFocus) return;
        const leftOrUp =
        ...
        if (newActiveItem) {
            this._markedKey = newActiveItem.get(keyProperty);
        }
    }

    protected _focusHandler(): void {
        this._setBackgroundPosition();
        this._markedKey = this._selectedKey;
    }
    ...
    private _getNextMarkedItem(isForwardMove: boolean = false): Model {
        ...
        const thisActiveItem = this._options.items.getRecordById(this._markedKey);
    ...
    protected _getButtonBackgroundStyle(): string {
        if (this._backgroundPosition[this._selectedKey]) {
    ...
    private _setBackgroundPosition(): void {
        if (this._backgroundPosition.isEmpty) {
            const tumblerClientRect = DimensionsMeasurer.getBoundingClientRect(this._container);
            ...
            this._options.items.forEach((item: Model, index: number) => {
                ...
                const clientRect = DimensionsMeasurer.getBoundingClientRect(
 ...
 * @cfg {Types/collection:RecordSet.<Controls/_Tumbler/Interfaces/ITumblerItem>} Определяет набор записей по которым строится контрол.
