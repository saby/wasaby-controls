...
/**
 * Интерфейс для контрола Чипсы
 * @extends Controls/interface:IMultiSelectable
 * @extends Controls/interface:IItems
...
export interface IChipsOptions
    ...
    keyProperty?: string;
    /**
     * Поведение кнопок, если они не умещаются.
    ...
    multiline?: boolean;
    /**
     * Использование единичного выбора с возможностью сбросить значение.
     * @demo Controls-demo/toggle/Chips/AllowEmptySelection/Index
     */
    allowEmptySelection?: boolean;
    /**
     * Имя поля элемента, значение которого будет отображаться в названии кнопок тумблера.
     *
     * @example
     * Пример описания.
     * <pre>
    ...
    /**
     * Стиль отображения кнопок в выбранном состоянии.
     * @variant primary
     * @variant secondary
     * @variant success
     * @variant warning
    ...
    /**
     * Класс, который навесится на элоемент
     */
    ...
    /**
     * Происходит при клике по элементу.
     */
 ...
 *  * Полезные ссылки:
 * * {@link /materials/DemoStand/app/Controls-demo%2Ftoggle%2FChips%2FIndex демо-пример}
 * * {@link /doc/platform/developmentapl/interface-development/controls/input-elements/buttons-switches/new-button-group/ руководство разработчика}
...
export default React.memo(
    React.forwardRef(function Chips(props: IChipsOptions, ref: React.ForwardedRef<HTMLElement>) {
        const {
            keyProperty = 'id',
            fontSize = 'm',
        ...
        // Есть кейсы, когда в контрол приходят новые props с событияеми, из-за чего кнопки перерисовываются.
        // И если пользователь в момент перерисовки успел нажать на чипс, то событие клика терялось
        // Поэтому сохраняем callback в ref, и каждую перерисовку перезаписываем.
        // Благодаря такому подходу сами чипсы при смене onSelectedKeysChanged не будут перисовываться.
        ...
        const notifyItemClick = React.useCallback(
            (event: SyntheticEvent<Event>, item: Model): void => {
                callbacksRef.current.onItemClick?.(event, item);
            },
            []
        );
        ...
        const onItemClick = React.useCallback(
            (event: SyntheticEvent<Event>, item: Model): void => {
                if (!readOnly && !item.get('readOnly')) {
                    ...
                    callbacksRef.current.onSelectedKeysChanged?.(
                        event,
                        selectedKeysClone,
                        added,
                        deleted
            ...
            [selectedKeys, setSelectedKeys, readOnly]
        );

        const onMouseEnterHandler = React.useCallback(
            (e: React.MouseEvent<HTMLSpanElement>, item: Model): void => {
                if ((!readOnly || !item.get('readOnly')) && !TouchDetect.getInstance().isTouch()) {
                    if (!item.get('tooltip')) {
                        ...
                        const caption =
                            item.get(props.displayProperty || 'title') || item.get('caption');
            ...
            [props.displayProperty, readOnly]
        );

        const getFontColorStyle = React.useCallback(
            (itemData: Model, contrastBg: boolean, selectedStyle?: string): string => {
            ...
            [props.selectedStyle, isSelectedItem, readOnly]
        );
        const buttons = React.useMemo(() => {
            const buttonsChips: React.ReactNode[] = [];
            items.forEach((item: Model, index) => {
                    ...
                    item.get('counter')
                        ? (captionProps = {}) => {
                              if (
                                  ...
                                  ItemTemplate instanceof String
                              ) {
                                  return (
                                      <Async
                ...
                buttonsChips.push(
                    <div
                        ...
                        name={`controls-ButtonGroup_button-${item.getKey()}`}
                    >
                        <ChipsButton
                            key={item.getKey() ?? index}
        ...
        let className = `controls_toggle_theme-${theme} controls-ButtonGroup`;
        if (props.className) {
        ...
        const attrs = (wasabyAttrsToReactDom(props.attrs as object) || {}) as Record<
            string,
            unknown
        >;
        return (
            ...
            >
                <div
                    className={
                        ...
                        ` controls-ButtonGroup__wrapper-${direction}_${
                            multiline ? 'milti' : ''
                        }line${
                            adaptiveMode?.device?.isPhone?.()
                                ? ' controls-ButtonGroup__wrapper_adaptive'
                                : ''
                        }`
                    }
                >
 ...
 *    keyProperty: 'key',
 *    rawData: [
 *       {key: 1, caption: 'Element 1', counter: 5, icon: 'icon-RoundPlus'},
 *       {key: 2, caption: 'Element 2', counter: 3},
 *       {key: 3, caption: 'Element 3', counter: 7, icon: 'icon-RoundPlus'}
 *    ]
 *});
 * </pre>
 *
 ...
 * @cfg {string} Имя класса, которое навесится на компонент
 */

/**
 * @name Controls/Chips:Control#itemClassName
...
/**
 * @name Controls/Chips:Control#iconTemplate
