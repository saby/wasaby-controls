...
export default React.forwardRef(function Chips(props: IChipsOptions, ref) {
    ...
    const onItemClick = (event: SyntheticEvent<Event>, item: Model): void => {
        if (!readOnly && !item.get('readOnly')) {
            ...
            const isClickOnSelectedItem =
                selectedKeysClone.indexOf(itemKeyProperty);
            if (
                !allowEmptySelection &&
    ...
    const onMouseEnterHandler = (
        ...
        if (!readOnly || !item.get('readOnly')) {
            if (!item.get('tooltip')) {
                if (
                    item.getOwner().getFormat().getFieldIndex('tooltip') === -1
                ) {
    ...
    const getFontColorStyle = (
        ...
        if (isSelected) {
            ...
            return (
                selectedPrefix +
                (contrastBg ? 'contrast' + colorSuffix : 'same')
            );
        }
    ...
    return (
        <FocusRoot {...attrs} as="div" className={className} ref={ref}>
            <div
                className={
