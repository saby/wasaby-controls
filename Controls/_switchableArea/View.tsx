...
import { ContextProvider as FreezeContextProvider } from 'Controls/freezeContext';
...
export interface ISwitchableOptions extends TInternalProps, IComponentProps {
        ...
        const content = (
            <>
                {this.props.items.map((item, index) => {
                    const isHidden = this._selectedKey !== item.key && !this.props.showAllItems;
                    if (this._viewModel && this._viewModel.getItems()[index].loaded) {
                        return (
                            <FreezeContextProvider key={'provider_' + item.key} isFreeze={isHidden}>
                                <EventSubscriber
                                    onValidateCreated={this._validateEventCb[item.key]}
                                >
                                    <ItemTemplate
                                        currentItemKey={item.key}
                                        className={isHidden ? 'ws-hidden' : ''}
                                        key={item.key}
...
/**
 * @typedef {Object} ISwitchableAreaItem
 ...
 * @cfg {Array.<ISwitchableAreaItem>} Данные элементов.
 */
