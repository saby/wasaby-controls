...
import { TKey, IControlProps } from 'Controls/interface';
...
export function correctSelectedKey(
    options: ISwitchableOptions,
    _this?: unknown
): TKey {
    let selectedKey;
...
> {
    ...
    protected _correctSelectedKey(options: ISwitchableOptions) {
        ...
        if (this._viewModel) {
            this.setState({
                indicatorVisible:
                    !this._viewModel.getVisibilityStatus(selectedKey),
            });
        }
        return selectedKey;
    }
    ...
    render(): React.ReactElement {
        ...
        const content = (
            <>
                {this.props.items.map((item, index) => {
                    if (
                        this._viewModel &&
                        this._viewModel.getItems()[index].loaded
                    ) {
                        return (
                            <ItemTemplate
                                ...
                                selectedKey={
                                    this.props.selectedKey || this._selectedKey
                                }
        ...
        return (
            <LoadingIndicator
                ...
                attrs={attrs}
                className={
                    'controls-SwitchableArea ws-flex-grow-1 ' + getAttrClass()
                }
                isGlobal={false}
                small={false}
