...
import { IAreaDataFactoryResult } from 'Controls/dataFactory';
...
function getDataOptions(
    context: IAreaDataFactoryResult,
    selectedKey: string | number
...
export default React.forwardRef(function WrappedView(
    ...
    const contextValue = contextOptions?.[props.storeId];
    const [selectedKey, setSelectedKey] = React.useState(
        correctSelectedKey(props)
    );
    const [dataOptions, setDataOptions] = React.useState(
        getDataOptions(contextValue, selectedKey)
    );
        ...
        if (contextValue) {
            if (!!contextValue.results[selectedKey]) {
                ...
                contextValue
                    ...
                    .then((config: IAreaDataFactoryResult) => {
                        setDataOptions(config.results[selectedKey]);
                        setSelectedKey(selectedKey);
                    });
            }
        }
