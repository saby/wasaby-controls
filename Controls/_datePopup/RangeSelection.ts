...
const Utils = {
    ...
    isSelected: (
        ...
        const start = range[0];
        const end = range[1];
        const itemStartValue = itemValue;
        const itemEndValue = selectionProcessing ? itemValue : DateUtil.getEndOfMonth(itemValue);
        return Boolean(
            (itemStartValue >= start || start === null) &&
                (itemEndValue <= end || end === null) &&
                (start || end)
        );
    },
