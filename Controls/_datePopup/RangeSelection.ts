...
const Utils = {
    ...
    prepareSelectionClass: (
        ...
        const isStart = isPeriodsEqual(itemValue, start, cfg);
        const isEnd = isPeriodsEqual(itemValue, end, cfg);
        ...
        if (
            isEnd &&
            ((selectionProcessing && !isStart) || !selectionProcessing)
        ) {
            css.push('end');
        }
    ...
    prepareHoveredClass: (
        ...
        if (
            Utils.isHovered(itemValue, hoveredStartValue, hoveredEndValue, cfg)
        ) {
    ...
    buildCssClass: (cfg: IPeriodConfig, parts: string[]): string => {
        ...
        return (
            ((cfg && cfg.cssPrefix) || DEFAULT_CSS_CLASS_BASE + '__') +
            parts.join('-')
        );
    },
