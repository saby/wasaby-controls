...
class Component extends Control<IMonthsRangeOptions> {
    ...
    protected _beforeMount(options: IMonthsRangeOptions): void {
        this._position = options.position || new options.dateConstructor();
    ...
    protected _getHoveredStartValue(date: Date): Date | null {
        if (!this._options.hoveredStartValue || this._options.hoveredStartValue?.getFullYear() !== date.getFullYear()) {
            return null;
        }
    ...
    protected _getHoveredEndValue(date: Date): Date | null {
        if (!this._options.hoveredEndValue || this._options.hoveredEndValue?.getFullYear() !== date.getFullYear()) {
            return null;
        }
    ...
    private _updateSelectionType(options: IMonthsRangeOptions): void {
        if (
            ...
            this._selectionViewType =
                MonthsRangeItem.SELECTION_VIEW_TYPES.months;
        } else {
