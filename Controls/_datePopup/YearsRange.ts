...
class YearsRange extends Control<IYearsRangeOptions> {
    ...
    protected _beforeMount(options: IYearsRangeOptions): void {
        BUTTONS_COUNT = options.adaptiveMode?.device.isPhone() ? 4 : 5;
    ...
    protected _prepareItemClass(itemValue: number): string {
        ...
        const getClassName = (): string => {
            ...
            if (yearSelected && this._options.canShowSelection) {
...
export { YearsRange as YearsRangeForTest };

export default withAdaptiveMode(YearsRange);
