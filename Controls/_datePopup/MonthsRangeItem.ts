...
import { IDayAvailableOptions, MonthViewDayTemplate } from 'Controls/calendar';
...
export default class MonthsRangeItem extends Control<IMonthsRangeItemOptions> {
    ...
    protected _isYearHovered(): boolean {
        const { selectionType, isDayAvailable, hoveredYear, date } = this._options;
        const canHover = selectionType === 'range' && !isDayAvailable;
        return canHover && hoveredYear && hoveredYear === date?.getFullYear();
    }
    ...
    protected _onMonthBodyClick(event: Event, date: Date): void {
        const canSelectAnything =
            this._monthsSelectionEnabled ||
            this._quarterSelectionEnabled ||
            this._halfyearSelectionEnabled ||
            this._yearSelectionEnabled;
        if (TouchDetect.getInstance().isTouch() && canSelectAnything) {
            return;
        }
    ...
    protected _prepareItemClass(itemValue: Date): string {
        ...
        const startValue = this._options.startValue;
        const endValue = this._options.endValue;
        if (this._monthsSelectionEnabled && this._isMonthEnabled(itemValue)) {
            css.push(
    ...
    protected _isYearEnabled(date: Date): boolean {
        if (!this._yearSelectionEnabled) {
            return false;
        }
        ...
        return isRangeEnabled(date, dateUtils.isYearsEqual, this._options.isDayAvailable);
    }
