...
export default class MonthsRangeItem extends Control<IMonthsRangeItemOptions> {
    ...
    protected _onMonthClick(event: Event, date: Date): void {
        ...
        } else if (this._options.isAdaptive && !this._options.selectionProcessing) {
    ...
    protected _isQuarterSelected(): boolean {
        ...
        return dateUtils.isDatesEqual(startOfQuarter, startValue) && dateUtils.isDatesEqual(endOfQuarter, endValue);
    }
    ...
    protected _isHalfYearSelected(): boolean {
        ...
        return dateUtils.isDatesEqual(startOfHalfYear, startValue) && dateUtils.isDatesEqual(endOfHalfYear, endValue);
    }
    ...
    protected _isSelectedStart(value: Date): boolean {
        ...
        return ( dateUtils.isDatesEqual(startValue, value) ||
            (!this._options.selectionProcessing && dateUtils.isDatesEqual(startValue, value))) &&
            !this._isQuarterSelected() && !this._isHalfYearSelected();
    }
    ...
    protected _isSelectedEnd(value: Date): boolean {
        ...
        return (dateUtils.isDatesEqual(endValue, value) ||
                (!this._options.selectionProcessing && dateUtils.isDatesEqual(endValue, value))) &&
            !this._isQuarterSelected() && !this._isHalfYearSelected();
    }
    ...
    private _getAdjacentDay(value: Date, delta: number): Date {
        return new Date(
            value.getFullYear(),
    ...
    private _getBorderClasses(value: Date, nextMonthSelected: boolean, previousMonthSelected: boolean): string {
        const className = [];

        const monthsInQuarter = 3;
        const monthOverCurrentMonth = this._getAdjacentDay(value, -monthsInQuarter);
        const monthUnderCurrentMonth = this._getAdjacentDay(value, monthsInQuarter);

        const monthOverCurrentMonthSelected = this._isPeriodSelected(monthOverCurrentMonth);
        const monthUnderCurrentMonthSelected = this._isPeriodSelected(
        ...
        const hasBorderRight = () => {
            return (
                EDGE_MONTHS_RIGHT.indexOf(value.getMonth()) !== -1 ||
                !nextMonthSelected
            );
        };
        ...
        const hasBorderLeft = () => {
            return (
                EDGE_MONTHS_LEFT.indexOf(value.getMonth()) !== -1 ||
    ...
    protected _prepareItemClass(itemValue: Date): string {
        ...
        if (this._options.yearsRangeSelectionProcessing || this._selectionViewType === 'days') {
            css.push('controls-PeriodDialog-MonthsRange__item-unselected');
            return css.join(' ');
        }
        const nextMonth = this._getAdjacentDay(itemValue,1);
        const previousMonth = this._getAdjacentDay(itemValue, -1);
        ...
        if (!startValue) {
            ...
            const startOfQuarter = dateUtils.getStartOfQuarter(startValue);
            const endOfQuarter = dateUtils.getEndOfQuarter(startValue);
            quarterSelected = dateUtils.isDatesEqual(startOfQuarter, startValue) &&
        ...
        if (!halfYearSelected) {
            ...
            halfYearSelected = dateUtils.isDatesEqual(startOfHalfYear, startValue) &&
