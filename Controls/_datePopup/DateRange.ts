...
export default class DateRange extends Control<IDatePopupDateRangeOptions> {
    ...
    private _monthListElement: HTMLElement | null;
    ...
    protected _beforeMount(options: IDatePopupDateRangeOptions): void {
        ...
        this._monthsMouseEnterHandler = this._monthsMouseEnterHandler.bind(this);
        if (options.position) {
            if (
                options.shouldPositionBelow &&
                options.position.getMonth() === 0 &&
                options.position.getFullYear() >= new Date().getFullYear()
            ) {
                this._monthsPosition = new Date(options.position.getFullYear() - 1, 0);
            } else {
    ...
    protected _afterMount(_options?: IDatePopupDateRangeOptions | undefined): void {
        // Нужно для совместимости с wasaby
        this._monthListElement = this._container.querySelector(
            '.controls-PeriodDialog-DateRangeItem__months'
        );
        this._monthListElement?.addEventListener('mouseenter', this._monthsMouseEnterHandler);
    }
    ...
    protected _beforeUnmount(): void {
        ...
        this._monthListElement?.removeEventListener('mouseenter', this._monthsMouseEnterHandler);
    }
    ...
    protected _getDateRangeStartValue(): Date {
        const isStartOfMonth = dateUtils.isStartOfMonth(this._options.startValue);
        if (
            this._dateRangeSelectionProcessing ||
            !isStartOfMonth ||
            !dateUtils.isEndOfMonth(this._options.endValue) ||
            (this._options.isAdaptive && isStartOfMonth)
        ) {
    ...
    protected _getDateRangeEndValue(): Date {
        const isEndOfMonth = dateUtils.isEndOfMonth(this._options.endValue);
        if (
            this._dateRangeSelectionProcessing ||
            !dateUtils.isStartOfMonth(this._options.startValue) ||
            !isEndOfMonth ||
            (this._options.isAdaptive && isEndOfMonth)
        ) {
    ...
    protected _onHeaderYearClick(): void {
    ...
    protected _onHeaderYearMouseEnter(): void {
        this._hoveredYear = dateUtils.getStartOfYear(this._monthsPosition);
    }

    protected _onHeaderYearMouseLeave(): void {
