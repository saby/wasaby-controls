...
export default class DateRange extends Control<IDatePopupDateRangeOptions> {
    ...
    protected _getDateRangeStartValue(): Date {
        if (this._dateRangeSelectionProcessing ||
            (!dateUtils.isStartOfMonth(this._options.startValue) || !dateUtils.isEndOfMonth(this._options.endValue))) {
    ...
    protected _onDateRangeHoveredChanged(event: Event, value: boolean): Date {
    ...
    protected _getMonthSelectionClass(value: Date): string {
        ...
        const nextMonthSelected = isSelected(startOfNextMonth) &&
            value.getFullYear() === startOfNextMonth.getFullYear();

        const startOfPrevMonth = new Date(startOfMonth.getFullYear(), startOfMonth.getMonth() -1);
        const prevMonthSelected = isSelected(startOfPrevMonth) &&
            value.getFullYear() === startOfPrevMonth.getFullYear();
