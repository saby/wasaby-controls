...
function getIconSizeClass({ iconSize, icon }: Pick<IButtonOptions, 'icon' | 'iconSize'>) {
    if (icon) {
...
export function cssStyleGeneration(options: IDropdownButtonProps) {
    ...
    const isLink = options.viewMode === 'link' || options.viewMode === 'linkButton';
    const currentHeight = options.inlineHeight || defaultHeight(options.viewMode, isCircle);
    offsetClassName += ` controls-MenuButton_inlineHeight-${currentHeight}_popup `;

    if (isCircle) {
        offsetClassName += 'controls-MenuButton_isCircle_popup ';
    } else {
        offsetClassName += 'controls-MenuButton_isDefault_popup ';
    }
    ...
    if (options.icon) {
        ...
        offsetClassName += getIconSizeClass(options);
    }

    if (isLink && !options.icon && !currentHeight) {
    ...
    offsetClassName += `controls-MenuButton_popup controls-MenuButton_${options.viewMode}_popup`;
