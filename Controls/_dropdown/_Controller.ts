...
export default class Controller implements IDropdownController {
    ...
    update(
        ...
        if (
            newOptions.readOnly &&
            newOptions.readOnly !== oldOptions.readOnly
        ) {
        ...
        if (
            selectedKeysChanged &&
            (newOptions.navigation || oldOptions.selectedItems)
        ) {
            newKeys = this._getUnloadedKeys(this._items, newOptions);
        }
        if (
            (newOptions.source && !this._items && newKeys.length) ||
            (!(
    ...
    loadDependencies(
        ...
        return (this._loadDependsPromise = Promise.allSettled(deps).then(
            (results) => {
                this._loadDependsPromise = null;
    ...
    handleSelectedItems(data: RecordSet): void {
    ...
    handleSelectorResult(selectedItems: RecordSet): void {
        const newItems = this._getNewItems(
            this._items,
            selectedItems,
            this._options.keyProperty
        );
    ...
    handleClose(searchValue: string): void {
        if (searchValue && this._options.searchParam) {
            ...
            if (
                ...
                this._options.sourceController.setFilter(
    ...
    private _getLoadItemsPromise(source?: ICrudPlus): Promise<any> {
        if (this._items) {
            ...
            (this._loadItemsPromise.resolved && !this._items)
        ) {
            if (
                (this._options.source || this._options.sourceController) &&
                !this._items
            ) {
    ...
    private _createSourceController(
        ...
        return this._getSourceController(options, source).then(
            (sourceController) => {
                return sourceController
                    .load(undefined, undefined, this._filter)
                    .catch((error) => {
                        if (!error.isCanceled) {
    ...
    private _updateSelectedItems(
        ...
        if (options.selectedItemsChangedCallback) {
            if (selectedAllText && (emptyText || selectedKeys[0] === selectedAllKey)) {
                ...
                if (options.parentProperty && options.multiSelect && isLoaded(filterBySelectionUtil)) {
                    selectedItems = loadSync(filterBySelectionUtil).
                    filterRecordSetBySelection(selectedKeys, excludedKeys, items, options);
                } else {
    ...
    private _getNewItems(
        ...
        factory(selectedItems).each((item) => {
            if (
                !this._getItemByKey(items, item.get(keyProperty), keyProperty)
            ) {
                newItems.push(item);
            }
        });
    ...
    private _updateHistory(items: RecordSet): void {
        if (isHistorySource(this._source)) {
            ...
            this._updateHistoryPromise = this._source
                .update(items, getMetaHistory())
                .then(() => {
    ...
    private _loadMenuTemplates(options: object): Promise<unknown> {
        if (!this._loadMenuTempPromise) {
            this._loadMenuTempPromise = loadMenuTemplates(
                options,
                options.theme
            );
        }
    ...
    private _isHistoryMenu(): boolean {
        return (
            ...
            this._items &&
            this._items.at(0)?.has('HistoryId')
        );
    }
    ...
    private _getPopupOptions(popupOptions?: object): object {
        ...
        const templateOptions = {
            ...
            keyProperty: this._isHistoryMenu()
                ? 'copyOriginalId'
                : baseConfig.keyProperty,
            headerTemplate:
                this._options.headTemplate || this._options.headerTemplate,
            ...
            filter:
                this._filter ||
                this._options.sourceController?.getFilter() ||
                this._options.filter,
        ...
        const popupConfig = Merge(
            popupOptions,
            this._options.menuPopupOptions || {}
        );
