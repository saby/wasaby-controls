...
export default class Controller {
    ...
    private _updateHistory(items: RecordSet): void | Promise<unknown> {
        if (
            ...
            return (this._updateHistoryPromise = updateRecent(
                items,
                getMetaHistory(),
                this._getHistoryId()
            ).then(() => {
    ...
    private async _getHistoryItemsBySource(items): Promise<RecordSet> {
        ...
        return this._getItemsWithHistory(items, historyId);
    }

    private _getItemsWithHistory(items: RecordSet, historyId: string): RecordSet {
        return getItemsWithHistory(items.clone(), historyId, this._source, {
            parentProperty: this._options.parentProperty,
            nodeProperty: this._options.nodeProperty,
        });
    }
    ...
    private _getPopupOptions(popupOptions?: object): object {
        ...
        if (isSelector && historyId) {
            selectorItems = this._getItemsWithHistory(this._items, historyId);
            selectorItems.setKeyProperty('copyOriginalId');
        } else {
            selectorItems = isSelector ? this._items?.clone(true) : this._items;
        }
