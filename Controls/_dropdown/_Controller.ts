...
import { THistoryLoadConfig } from 'Controls/HistoryStore';
...
export default class Controller {
    ...
    private _loadItemsPromise: CancelablePromise<void> = null;
    ...
    update(newOptions: IDropdownControllerOptions): Promise<RecordSet | void> | void {
        ...
        if (newOptions.excludedKeys && !isEqual(newOptions.excludedKeys, this._excludedKeys)) {
        ...
        const needReload = !this._isOpened && sourceChanged;

        let newKeys = [];
        if (selectedKeysChanged && (newOptions.navigation || oldOptions.selectedItems)) {
            newKeys = this._getUnloadedKeys(
                needReload ? null : this._items || this._selectedItems,
                newOptions
            );
        }
    ...
    loadDependencies(source?: ICrudPlus): Promise<unknown[]> {
        ...
        const historyId = this._getHistoryId();
        deps.push(this._getHistoryStore(historyId));
        if (historyId) {
            deps.push(loadAsync('Controls/Utils/History/PrepareMenuItems'));
            deps.push(this._loadHistory());
        }
    ...
    setMenuPopupOpener(opener: PopupOpener): void {
        this._popupOpener = opener;
    }
    ...
    setExcludedKeys(keys: TSelectedKeys): void {
    ...
    setSelectedItems(selectedItems: RecordSet): void {
        ...
        this._setItemsAndMenuSource(null);
        this._getSourceController(this._options);
    }
    ...
    handleSelectorResult(selectedItems: RecordSet): void {
        ...
        const historyUpdatePromise = this._updateHistory(factory(selectedItems).toArray());
        if (historyUpdatePromise instanceof Promise) {
            historyUpdatePromise.then(updateSelectedItems);
        } else {
            updateSelectedItems();
        }
    }
    ...
    pinClick(item: Model): void {
        ...
        this._getPrepareHistoryUtils().then(({ updatePinned }) => {
            updatePinned(
                this._items.getRecordById(item.get(this._getKeyProperty(this._options))),
                {
    ...
    private _open(popupOptions?: object): Promise<unknown[]> {
        ...
        return this.loadDependencies(source)
            .finally(() => {
    ...
    private _openPopup(): void {
        this._isOpened = true;
        this._opening = false;
        (this._popupOpener || this._sticky).open(this._getPopupOptions(this._popupOptions));
    }

    private _handleClose({ searchValue }: { searchValue: string }): void {
        if ((searchValue && this._options.searchParam) || this._options.menuMode === 'selector') {
            if (this._options.menuMode !== 'selector' || !!this._items?.getMetaData()?.path) {
                this._setItemsAndMenuSource(null);
            }
            if (
            ...
            if (this._options.menuMode === 'selector' && this._options.multiSelect) {
                this.reload();
            }
        }
    ...
    private _getLoadItemsPromise(source?: ICrudPlus): Promise<any> {
        ...
        if (this._items) {
            ...
            this._setItemsAndMenuSource(this._items);
            loadItemsPromise = Promise.resolve();
        } else if (
            !this._loadItemsPromise ||
            (this._loadItemsPromise.promise?.resolved && !this._items)
        ) {
            if ((this._options.source || this._options.sourceController) && !this._items) {
                loadItemsPromise = this._loadItems(this._options, source);
            } else {
                loadItemsPromise = Promise.resolve(this._options.items);
                if (this._options.items) {
                    this._resolveLoadedItems(this._options, this._options.items);
                }
            }
        }
        this._loadItemsPromise = new CancelablePromise(loadItemsPromise);
        return this._loadItemsPromise.promise;
    }
    ...
    private async _createMenuSource(items: RecordSet): Promise<void> {
        if (this._options.items && !this._options.source) {
            ...
            const isHistory =
                (items instanceof RecordSet && isHistorySource(this._source)) ||
                this._getHistoryId();
            this._menuSource = new PrefetchProxy({
                target: this._source,
                data: {
                    query: isHistory ? this._getHistoryItemsBySource(items) : items,
                },
            });
        }
    }
    ...
    private _getSourceController(
        ...
        this._source = source || options.source;
        if (isHistorySource(this._source)) {
    ...
    private _getItemsFromSource(
        ...
        const sourceController = this._getSourceController(options, source, filter);
        const promises = [sourceController.load(undefined, undefined, this._filter)];
        if (this._hasHistory(options)) {
            promises.push(this._loadHistory());
            promises.push(loadAsync('Controls/Utils/History/PrepareMenuItems'));
        }
        return Promise.all(promises).then(([items]) => items);
    }
    ...
    private _getSelectedItems(options: IDropdownControllerOptions, items: RecordSet): Model[] {
        ...
        const excludedKeys = this._excludedKeys || options.excludedKeys;
        const { emptyKey, emptyText, selectedAllText, selectedAllKey } = options;
        let selectedItems = [];
    ...
    private _updateHistory(items: RecordSet): void | Promise<unknown> {
        if (
            isHistorySource(this._source) ||
            (this._getHistoryId() && this._isLocalSource(this._source)) ||
            ...
            return (this._updateHistoryPromise = this._getPrepareHistoryUtils().then(
                ({ updateRecent }) => {
                    return updateRecent(items, getMetaHistory(), this._getHistoryId()).then(() => {
    ...
    private _closeDropdownList(): void {
        this._getOpener().close();
        this._isOpened = false;
    }

    private _getOpener(): PopupOpener | StickyOpener {
        return this._popupOpener || this._sticky;
    }
    ...
    private _getHistoryStore(historyId?: string): Promise {
        if (historyId) {
            return loadAsync('Controls/HistoryStore');
        }
    }

    private _loadHistory(): Promise<unknown> {
        const historyId = this._getHistoryId();
        if (historyId) {
            return this._getHistoryStore(historyId).then(({ Store }) => {
                return Store.load(historyId, {
                    parentProperty: this._options.parentProperty,
                    nodeProperty: this._options.nodeProperty,
                    ...this._getHistoryConfigFromSource(this._options.source),
                });
            });
        }
    }

    private _getHistoryConfigFromSource(source): THistoryLoadConfig {
        if (isHistorySource(source)) {
            return source.getHistoryLoadConfig();
        }
        return {};
    }

    private async _getHistoryItemsBySource(items): Promise<RecordSet> {
        const historyId = this._getHistoryId();
        await this._loadHistory();
        const { getItemsWithHistory } = await this._getPrepareHistoryUtils();

        return Promise.resolve(
            getItemsWithHistory(items.clone(), historyId, this._source, {
                parentProperty: this._options.parentProperty,
                nodeProperty: this._options.nodeProperty,
            })
        );
    ...
    private _getPopupOptions(popupOptions?: object): object {
        ...
        const historyId = this._getHistoryId();
        const isSelector = this._options.menuPopupComponent; // Избавиться от проверки
        let selectorItems;
        if (isSelector && historyId) {
            const { getItemsWithHistory } = loadSync('Controls/Utils/History/PrepareMenuItems');
            selectorItems = getItemsWithHistory(this._items.clone(), historyId, this._source, {
                parentProperty: this._options.parentProperty,
                nodeProperty: this._options.nodeProperty,
            });
            selectorItems.setKeyProperty('copyOriginalId');
        } else {
            selectorItems = isSelector ? this._items?.clone() : this._items;
        }
        ...
        const items =
            !this._isHistoryMenu() && !this._hasHistory(this._options) ? this._items : null;
        ...
        const templateOptions = {
            ...
            keyProperty:
                this._isHistoryMenu() || this._hasHistory(this._options)
                    ? 'copyOriginalId'
            ...
            items: isSelector ? selectorItems : items,
            historyId: isSelector ? historyId : this._options.historyId,
            footerVisibility: this._options.footerVisibility,
            error: this._sourceController?.getLoadError(),
            source,
            ...
            focusable: false,
            isMinHeightSet: true,
            menuClosedCallback: this._handleClose,
            onItemClick: this._options.onItemClick,
        };
        ...
        const itemsInRoot =
            ...
            factory(this._items)
                ...
                .value();
        const isAutoHeight =
            this._options.searchParam || (itemsInRoot && itemsInRoot.length <= MIN_ITEMS_ADAPTIVE);
        const config = {
            allowAdaptive,
            ...
            className:
                (this._options.getPopupClassName
                    ? this._options.getPopupClassName()
            ...
            template: this._options.menuPopupComponent || 'Controls/menu:Popup',
            actionOnScroll: 'close',
            target: this.target || this._options.target,
            targetPoint: this._options.targetPoint,
            opener: this._popupOptions.opener || this._options.openerControl,
            fittingMode: {
                vertical: this._options.searchParam ? 'initialAdaptive' : 'adaptive',
                horizontal: 'overflow',
            },
            popupType: allowAdaptive ? 'smallDictionary' : 'list',
            autofocus: false,
    ...
    private _getPrepareHistoryUtils(): any {
        return loadAsync('Controls/Utils/History/PrepareMenuItems');
    }
}
