...
import { ACTIVATE_RESULT, BaseDropdown } from 'Controls/_dropdown/BaseDropdown';
...
class DropdownButton extends BaseDropdown<IButtonOptions, IButtonState> {
    ...
    protected _itemClick(data: object, nativeEvent: MouseEvent): typeof ACTIVATE_RESULT | boolean {
        const item = this._controller.getPreparedItem(data);
        const res = this._onItemClickHandler([item], nativeEvent);

        if (res !== false && res !== ACTIVATE_RESULT.ENTER && res !== ACTIVATE_RESULT.EXPAND) {
