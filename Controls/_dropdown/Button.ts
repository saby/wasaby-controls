...
export interface IButtonOptions
    extends IButtonBaseOptions,
        IMenuPopupTriggerOptions {}
/**
...
export default class Button extends BaseDropdown {
    ...
    _onItemClickHandler(result: object, nativeEvent: Event) {
        ...
        const menuItemActivateResult = this._notify('menuItemActivate', [
            result[0],
            nativeEvent,
        ]);
        let handlerResult;
    ...
    openMenu(
        ...
        return this._openMenu(popupOptions, key, subMenuOptions).then(
            (result) => {
                if (result) {
...
/**
 * @event menuItemActivate Происходит при выборе элемента из списка.
 * @name Controls/dropdown:IButton#menuItemActivate
