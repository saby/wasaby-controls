...
class ComboBox
    ...
    implements IInputPlaceholder, IContrastBackground
{
    ...
    protected _afterMount(options: IComboboxOptions): void {
        if (this._countItems === 1) {
            if (
                this._selectedItem.get(options.keyProperty) !==
                options.selectedKey
            ) {
    ...
    _getControllerOptions(options: IComboboxOptions): object {
        ...
        const popupClassName = `controls-ComboBox-popup ${
    ...
    _setText(
        ...
        this._isEmptyItem =
            getPropValue(selectedItems[0], keyProperty) === emptyKey ||
        ...
        if (this._isEmptyItem && emptyText) {
            ...
            this._value = String(
                getPropValue(selectedItems[0], displayProperty) ?? ''
            );
    ...
    protected _deactivated(event: SyntheticEvent<Event>): void {
        ...
        if (
            ...
            !event.nativeEvent.relatedTarget?.closest(
