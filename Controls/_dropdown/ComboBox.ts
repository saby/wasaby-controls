...
class ComboBox extends BaseDropdown implements IInputPlaceholder, IContrastBackground {
    ...
    _beforeMount(options: IComboboxOptions): Promise<void> {
    ...
    protected _beforeUpdate(newOptions: IComboboxOptions): void {
        ...
        this._setHighlightedOnFocus(newOptions);
    }
    ...
    protected _deactivated(event: SyntheticEvent<Event>): void {
        if (this._options.closeMenuOnOutsideClick) {
            ...
            if (!relatedTarget || !goUpByControlTree(relatedTarget).includes(this)) {
                this.closeMenu();
            }
        }
    }
