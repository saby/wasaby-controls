...
function fillItems(
    ...
    let historyItem;
    const { keyProperty, parentProperty, nodeProperty, root = null } = historySourceConfig;
    history[historyType].forEach((id: string) => {
        ...
        if (!oldItem) {
            if (keyProperty) {
                const oldItemIndex = oldItems.getIndexByValue(keyProperty, id);
                oldItem = oldItems?.at(oldItemIndex);
            } else {
...
function getHistoryStore(): typeof import('Controls/HistoryStore') {
    let historyStoreLib;
    try {
