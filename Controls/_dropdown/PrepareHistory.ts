...
function getRecentIds(
    ...
    const maxCountRecent =
        MAX_HISTORY - (filteredPinned.length + filteredFrequent.length);
    let item;
    let id;
...
function checkPinnedAmount(pinned: RecordSet): boolean {
    return pinned.getCount() !== MAX_HISTORY;
}
...
function getFilterHistory(
    ...
    //historySourceConfig?.pinned.map((el) => String(el)) ||
    const pinnedIds = getPinnedIds(rawHistoryData.pinned);
    const frequentIds = getFrequentIds(rawHistoryData.frequent, pinnedIds, oldItems, nodeProperty);
...
function prepareOriginItems(
    ...
    oldItems.forEach((item: Model): void => {
        ...
        let newItem;
        if (historyItem === -1 || needDuplicateItem(item, historySourceConfig)) {
            newItem = new Model({
...
function fillItems(
    ...
    let historyItem;
    const {parentProperty, nodeProperty, root, pinned} = historySourceConfig;
