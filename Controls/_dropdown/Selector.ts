...
export default class Selector extends BaseDropdown {
    ...
    protected _defaultContentTemplate: TemplateFunction =
    ...
    _getControllerOptions(options: ISelectorOptions): object {
        ...
        return {
            ...
            ...{
                ...
                allowPin: false,
                selectedItemsChangedCallback: this._prepareDisplayState.bind(
                    this,
                    options
                ),
    ...
    _selectedItemsChangedHandler(
        ...
        if (
            !isEqual(this._options.selectedKeys, newSelectedKeys) ||
            this._options.task1178744737
        ) {
    ...
    _prepareDisplayState(options: ISelectorOptions, items: Model[]): void {
        if (items.length) {
            ...
            this._needInfobox =
                options.readOnly && this._selectedItems.length > 1;
    ...
    openMenu(popupOptions?: IStickyPopupOptions): void {
        ...
        this._controller
            .openMenu(Merge(config, popupOptions || {}))
            .then((result) => {
                if (result) {
    ...
    protected _applyClick(
        ...
        this._updateSelectedItems(data, selectedKeys, excludedKeys);
    ...
    protected _selectorResult(data: object): void {
        ...
        const selectedItems = this._controller.getFormattedSelectedItems(
            data,
            selectedKeys
        );
    ...
    private _setCountItems(items: RecordSet, options: ISelectorOptions): void {
        ...
        this._needOpenMenuOnClick =
            options.selectedItems || this._isNeedOpenMenuOnClick(options);
    }
    ...
    private _updateSelectedItems(
        ...
        this._selectedItemsChangedHandler(items, selectedKeys, excluded);
        this._controller.setSelectedKeys(selectedKeys);
            ...
            if (!maxVisibleItems || texts.length < maxVisibleItems) {
                if (parentProperty) {
                    ...
                    while (
                        ...
                        const parent =
                            parentKey && items.getRecordById(parentKey);
    ...
    private _getText(
        ...
        if (
            ...
            isSingleSelectionItem(item, emptyText, keyProperty, emptyKey)
        ) {
    ...
    private _getMoreText(items: Model[], maxVisibleItems: number): string {
        ...
        if (maxVisibleItems) {
            if (items.length > maxVisibleItems) {
                moreText =
                    ', ' + rk('еще') + ' ' + (items.length - maxVisibleItems);
            }
        }
        return moreText;
    }
