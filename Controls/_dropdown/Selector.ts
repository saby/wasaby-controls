...
import { IValidationStatusOptions, TSelectedKeys, IUnderlineOptions } from 'Controls/interface';
...
export default class Selector extends BaseDropdown {
    ...
    _beforeMount(options: ISelectorOptions): void | Promise<void> {
        this._controller = new Controller(this._getControllerOptions(options));
        this._setHighlightedOnFocus(options);

        if (options.navigation && options.selectedKeys && options.selectedKeys.length) {
            return loadSelectedItems(this._controller, options);
        } else {
            return loadItems(this._controller, options);
        }
    }
    ...
    protected _setHighlightedOnFocus(options): void {
    ...
    _getControllerOptions(options: ISelectorOptions): object {
        ...
        const selectedKeys =
            !options.selectedKeys || !options.selectedKeys.length
                ? [options.emptyKey]
    ...
    protected _applyClick(data: Model[], selectedKeys?: TKey[], excludedKeys?: TKey[]): void {
        const selected = this._getSelectedKeys(factory(data).toArray(), this._options.keyProperty);
