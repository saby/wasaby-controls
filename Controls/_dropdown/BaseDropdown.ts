...
export const ACTIVATE_RESULT = {
    ENTER: 'enter',
    EXPAND: 'expand',
};

/**
 * Валидирует значения переданных опций
 * @param {object} props
 */
...
/**
 * Базовый класс выпадающих списков
 * @public
 */
export class BaseDropdown<P, S> extends React.Component<P, S> {
    ...
    protected _popupOpenerRef = React.createRef();
    protected _itemResult: Model[] | null = null;
    protected _clickHandled: boolean = false;
    ...
    /**
     * Перезагружает контроллер выпадающих списков
     */
    reload(): void {
    ...
    /**
     * Вызывает метод закрытия меню контролера выпадающих списков
     */
    closeMenu(): void {
    ...
    /**
     * Устанавливает фокус на выпадающий список.
     */
    activate() {
    ...
    protected _handleKeyDown(event: SyntheticEvent<KeyboardEvent>): void {
        ...
        if (key === 'Escape' && this.state.isOpened) {
            this._controller.closeMenu();
            this._stopEvent(event);
        } else if (
            (key === 'Space' && !this.state.isOpened) ||
            ((key === 'ArrowDown' || key === 'ArrowUp') && this.state.isOpened)
        ) {
    ...
    protected _handleClick(event?: SyntheticEvent): void {
        if (this._itemResult) {
            this._onSingleItemClick(this._itemResult, event);
        ...
        if (items && items.getCount() && !this.props.readOnly) {
    ...
    protected _handleMouseMove(event: SyntheticEvent): void {
        if (this.props.onMouseMove) {
            this.props.onMouseMove(event);
        }
    }

    protected _handleTouchStart(event: SyntheticEvent): void {
    ...
    protected _handleMouseLeave(event: SyntheticEvent): void {
        if (this._itemResult) {
            // кейс, когда зажали кнопку мыши и увели мышь с кнопки, в этом случае onClick не стрельнет
            this._itemResult = null;
        }
    ...
    protected _openMenuResultHandler(result: Model[], event: SyntheticEvent): void {
        if (result) {
            if (this._clickHandled) {
                // Если клик уже сработал
                this._onSingleItemClick(result, event);
            } else {
                // этот код срабатывает на onMouseDown, но прикладнику нужно отдать результат на onClick, чтобы отработали события фокусов
                // здесь сохраняем результат, событием стрельнем в onClick
                this._itemResult = result;
            }
        }
    }
    ...
    protected _onSingleItemClick(data: Model[], event: SyntheticEvent): void {
        // for overrides
    }

    protected _onResult(action: string, data: object, nativeEvent: Event): void {
        switch (action) {
            case 'slidingHeightUpdate':
                this.openMenu({ slidingPanelOptions: data });
                break;
