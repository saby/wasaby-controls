...
import { InfoboxTarget, isLeftMouseButton, IStickyPopupOptions } from 'Controls/popup';
import { IBaseDropdownOptions } from 'Controls/_dropdown/interface/IBaseDropdown';
import getDropdownControllerOptions from 'Controls/_dropdown/Utils/GetDropdownControllerOptions';
import * as Merge from 'Core/core-merge';
import 'css!Controls/dropdown';
import 'css!Controls/CommonClasses';
import { IUnderlineOptions, IValidationStatusOptions, TSelectedKeys } from 'Controls/interface';
...
interface ISelectorOptions
    ...
    itemTemplate?: React.Component | React.FunctionComponent;
 ...
 */
class DropdownSelector extends BaseDropdown<ISelectorOptions, ISelectorState> {
    ...
    _selectedItemsChangedHandler(
        ...
        const text =
            this._getText(items, this.props) + this._getMoreText(items, this.props.maxVisibleItems);
    ...
    _handleMouseDown(event: SyntheticEvent<MouseEvent>): void {
        if (!isLeftMouseButton(event) || this.props.readOnly) {
            return;
        }
        this.openMenu();
    }
    ...
    private _isNeedOpenMenuOnClick(props: ISelectorOptions): boolean {
        return !(this._countItems < 2 && !props.footerTemplate);
    }
            ...
            if (!maxVisibleItems || texts.length < maxVisibleItems) {
                if (parentProperty) {
                    ...
                    while (
                        parentKey !== undefined &&
                        parentKey !== null &&
                        parentKey !== this.props.root
                    ) {
                        const parent = items.getRecordById(parentKey);
    ...
    render() {
        ...
        const needInfobox = props.readOnly && state.selectedItems?.length > 1;
        ...
        const text = this._getText(state.selectedItems, props);
        const hasMoreText = this._getMoreText(state.selectedItems, props.maxVisibleItems);
        const tooltip = this._getFullText(
            state.selectedItems,
...
/**
 * @name Controls/_dropdown/Selector#buildByItems
