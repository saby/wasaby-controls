...
export class ItemsStateManager extends AbstractAspectStateManager<IItemsState> {
    ...
    resolveChanges(prevState: IItemsState, nextState: IItemsState): IListChange[] {
        ...
        if (changes.length && isLoaded('Controls/markerListAspect')) {
    ...
    getNextState(state: IItemsState, _changes: IListChange[]): IItemsState {
        ...
        const hasMoreChanges = _changes.filter((change) => change.name === SetHasMoreChangeName);

        if (hasMoreChanges.length) {
            nextState.hasMoreStorage = (
                hasMoreChanges[hasMoreChanges.length - 1] as IHasMoreChange
            ).args.hasMoreStorage;
        }
        ...
        for (const change of changes) {
            switch (change.name) {
                ...
                case ListChangeNameEnum.SET_HAS_MORE: {
                    (collection as unknown as Tree).setHasMoreStorage?.(change.args.hasMoreStorage, true);
                    break;
                }
