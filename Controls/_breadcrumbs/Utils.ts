...
export default {
    ...
    getItemsWidth(
        ...
        items.forEach((item, index) => {
            ...
            const itemWidth =
                index !== 0 ? itemTitleWidth + ARROW_WIDTH : itemTitleWidth;
    ...
    calculateItemsWithShrinkingLast(
        ...
        if (items.length <= 2) {
            ...
            firstContainerItems = items.map((item, index, arr) => {
                const withOverflow =
                    arr[index].get(options.displayProperty).length > 3;
                return PrepareDataUtil.getItemData(
                    index,
                    arr,
                    false,
                    withOverflow
            ...
            const minWidthOfLastItem = this.getMinWidth(
                items,
                options,
                indexEdge,
                getTextWidth
            ...
            visibleItems =
                PrepareDataUtil.drawBreadCrumbsItems(firstContainerItems);
    ...
    getMinWidth(
        ...
        if (text) {
            let textWidth =
                getTextWidth(text.substring(0, 3) + '...', options.fontSize) +
                PADDING_RIGHT;
    ...
    calculateItemsWithDots(
        ...
        if (secondContainerWidth > width && crumbsItems.length > 2) {
            ...
            for (index = crumbsItems.length - 3; index >= indexEdge; index--) {
                currentMinWidth = this.getMinWidth(
                    crumbsItems,
                    options,
                    index,
                    getTextWidth
            ...
            for (let j = indexEdge; j <= index; j++) {
                const itemTitle =
                    crumbsItems[j].get(options.displayProperty) || '';
            ...
            const preLastItemTitle =
                crumbsItems?.[crumbsItems.length - 2]?.get(
                    options.displayProperty
                ) || '';
            for (let j = indexEdge; j < crumbsItems.length; j++) {
                const withOverflow =
                    j === crumbsItems.length - 2 && preLastItemTitle.length > 3;
                secondContainerItems.push(
                    PrepareDataUtil.getItemData(
                        j,
                        crumbsItems,
            ...
            if (secondContainerItems.length <= 2) {
                secondContainerItems.forEach((item) => {
                    const itemTitle =
                        item.item.get(options.displayProperty) || '';
    ...
    updateBreadcrumbsSize(
        backButtonSize: TFontSize,
        defaultSize: TFontSize
    ): TFontSize {
