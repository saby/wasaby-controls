...
export default class NavigationMenu extends Control<INavigationMenu> {
    ...
    protected _componentDidMount(options?: INavigationMenu) {
        const items = this._children.treeGrid.getItems();
        let hasNodeWithChildren = false;
        if (items.getCount()) {
            for (let i = 0; i < items.getCount(); i++) {
                const children = this._hierarchy.getChildren(items.at(i), items);
        ...
        if (!hasNodeWithChildren) {
            this._itemPadding = {
                left: 'm',
            };
        } else if (options?.headerVisible) {
            this._header = [
                {
                    render: React.createElement(NavigationMenuHeader, {
                        caption: options?.caption,
