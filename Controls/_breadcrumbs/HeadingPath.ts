...
import calculateBreadcrumbsUtil, {
    ARROW_WIDTH,
    PADDING_RIGHT,
} from 'Controls/_breadcrumbs/Utils';
...
class BreadCrumbsPath extends Control<IHeadingPath, IReceivedState> {
    ...
    protected _initStatesBeforeMount(
        ...
        if (options.containerWidth) {
            ...
            this._dotsWidth = this._getDotsWidth(
                options.fontSize,
                getTextWidth
            );
    ...
    protected _beforeUpdate(newOptions: IHeadingPath): void {
        ...
        const isFontSizeChanged =
            newOptions.fontSize !== this._options.fontSize;
        if (isItemsChanged) {
        ...
        const isDataChange =
            isItemsChanged || isContainerWidthChanged || isFontSizeChanged;
            ...
            if (isDataChange) {
                ...
                if (this._breadCrumbsItems) {
                    if (newOptions.containerWidth) {
                        this._calculateBreadCrumbsToDraw(
                            this._breadCrumbsItems,
                            newOptions
                        );
                    } else {
                        this._visibleItems =
                            PrepareDataUtil.drawBreadCrumbsItems(
                                this._breadCrumbsItems
                            );
                    }
                }
    ...
    protected _onHomeMouseEnter(): void {
        ...
        this._homeHighlightClass = applyHighlighter(
            this._options.highlighter,
            rootModel.getKey()
        );
    }
    ...
    protected _onPathChanged(event: SyntheticEvent, path: Path): void {
        const newRoot = path.length
            ? path[path.length - 1]
            : this._buildRootModel();
    ...
    private _updateBreadCrumbsClasses(
        ...
        if (options.withoutBreadcrumbs) {
            this._backButtonClass =
                'controls-BreadCrumbsPath__unrestrictedWidth';
            return;
        }
        ...
        if (this._crumbsWidth > this._backButtonWidth) {
            this._breadCrumbsWrapperClass =
                'controls-BreadCrumbsPath__unrestrictedWidth';
            this._backButtonClass =
                'controls-BreadCrumbsPath__widthRestriction';
        } else {
            this._breadCrumbsWrapperClass =
                'controls-BreadCrumbsPath__widthRestriction';
            this._backButtonClass =
    ...
    private _prepareItems(
        ...
        if (this._items?.length > 0) {
            ...
            if (this._items?.length > 1) {
                this._breadCrumbsItems = this._items.slice(
                    0,
                    this._items.length - 1
                );
                this._breadCrumbsClass =
                    'controls-BreadCrumbsPath__breadCrumbs_short';
                ...
                if (options.displayMode === 'default') {
                    this._updateBreadCrumbsClasses(
                        options,
                        receivedState,
                        getTextWidth
                    );
...
/**
 * @event arrowActivated Происходит при клике на кнопку "Просмотр записи".
 * @name Controls/_breadcrumbs/HeadingPath#arrowActivated
 * @param {UI/Events:SyntheticEvent} eventObject Дескриптор события.
 * @demo Controls-demo/BreadCrumbs/ArrowActivated/Index
 */
