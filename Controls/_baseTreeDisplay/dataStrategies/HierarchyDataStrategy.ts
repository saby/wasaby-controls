...
> extends FlatDataStrategy<TOptions> {
    ...
    protected _onCollectionChange(_event: EventObject, action: string): void {
        if (
    ...
    protected _onCollectionItemChange(
        ...
        if (
            this._options.childrenProperty &&
            properties.hasOwnProperty(this._options.childrenProperty)
        ) {
            this._initializeState();
        }
    }
...
function iterator<TItem>(
    ...
    const iterator = (collection: TCollection) => {
        ...
        for (let index = 0; index < itemsCount; index++) {
            ...
            callback(item as TItem, throughIndex);
            throughIndex++;
