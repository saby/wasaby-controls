...
import type { INodeFooterConfig } from 'Controls/gridRender';
import PseudoParent from './strategies/PseudoParent';
...
function onCollectionChange<T>(
    ...
    if (
        ...
        if (
            this.instance.getExpanderVisibility() === 'hasChildren' ||
            this.instance.getGroupNodeViewMode?.() === 'headerless'
        ) {
...
> extends Collection<S, T> {
     ...
     * Иконка экспандера
     */
    protected _$expanderIcon: TExpanderIconViewMode;
    ...
    private _$expansionModel?: Map<CrudEntityKey, boolean>;
    ...
    //# region Подготовка к отуплению
    // В этом регионе собраны стейты, которые должны прилетать сверху и использоваться при создании
    // элементов.

    private __hasChildrenByRecordSet: Map<CrudEntityKey, boolean>;

    // FIXME: в коллекциях неправильный порядок создания строк из за наследования.
    //  При создании коллекции, строки строит базовая коллекция в своем конструкторе, используя фабрику наследника.
    //  Всё что наследник устанавливает в конструкторе будет выполнено после вызова базового.
    //  На практике хочется использовать в фабрике наследника свои же стейты,
    //  но это невозможно, они еще не проинициализировались.
    //  Поэтому тут геттер с || new Map().
    private get _hasChildrenByRecordSet(): Map<CrudEntityKey, boolean> {
        if (!this.__hasChildrenByRecordSet) {
            this.__hasChildrenByRecordSet = new Map();
        }
        return this.__hasChildrenByRecordSet;
    }

    private set _hasChildrenByRecordSet(map: Map<CrudEntityKey, boolean>) {
        if (this.__hasChildrenByRecordSet) {
            this.__hasChildrenByRecordSet.clear();
        }
        this.__hasChildrenByRecordSet = map;
    }
    ...
    protected _initializeUserStrategies(options?: { addPseudoParentStrategy: boolean }) {
        ...
        if (!this._$childrenProperty) {
            this._userStrategies.unshift({
                ...
                after: itemsStrategy.User,
            });
        }

        const hasInfinityNavigation = this.getNavigation()?.view === 'infinity';
        const hasBothwaysOrBackwardDirection =
            this.getNavigation()?.sourceConfig?.direction === 'bothways' ||
            this.getNavigation()?.sourceConfig?.direction === 'backward';
        const hasNodeHeader = !this?.hasNodeHeaderConfig();

        const needPseudoParent =
            options?.addPseudoParentStrategy ??
            (hasInfinityNavigation && hasBothwaysOrBackwardDirection && hasNodeHeader);
    ...
    protected _handleAfterCollectionItemChange(item: T, _index: number, properties?: object): void {
    ...
    getExpanderIcon(): TExpanderIconViewMode {
    ...
    setDragPosition(position: IDragPosition<T>): void {
        ...
        if (dragStrategy) {
            ...
            const beforePosition = this.getIndex(dragStrategy.avatarItem);
            ...
            // setDragPosition вносит изменения в коллекцию и обновляет версию через this.nextVersion(), что работает для вызова ререндера WML
            // Однако эти изменения не вызывают уведомления onCollectionChange, в связи с чем не происходит ререндер React.
            // Уведомление об изменении порядка элементов в коллекции сделаны по аналогии с Controls/_columns/display/Collection.ts
            if (dragStrategy.avatarItem) {
                this._notifyBeforeCollectionChange();
                this._notifyCollectionChange(
                    IObservable.ACTION_MOVE,
                    [dragStrategy.avatarItem],
                    this.getIndex(dragStrategy.avatarItem),
                    [dragStrategy.avatarItem],
                    beforePosition
                );
    ...
    setNodeFooterTemplate(nodeFooterTemplate: TemplateFunction): void {
        ...
        if (this._$nodeFooterTemplate !== nodeFooterTemplate) {
            this._$nodeFooterTemplate = nodeFooterTemplate;
            this._updateItemsProperty('setNodeFooterTemplate', nodeFooterTemplate, '$TNF');
            this._nextVersion();
        }
    ...
    setMoreFontColorStyle(moreFontColorStyle: string): void {
        if (this._$moreFontColorStyle !== moreFontColorStyle) {
            ...
            this._updateItemsProperty('setMoreFontColorStyle', moreFontColorStyle, '$TNF');
            this._nextVersion();
        }
    }
    ...
    setFilter(...args: Parameters<Collection['setFilter']>): ReturnType<Collection['setFilter']> {
    ...
    getNodeFooter(): INodeFooterConfig[] {
        return this._$nodeFooter;
    }
    ...
    /**
     * Рекурсивно поднимает версию на всех дочерних элементах узла
     * */
    protected _updateChildren(parent: T): void {
        this.getChildrenByRecordSet(parent).forEach((child) => {
            child.updateVersion();
            this._updateChildren(child);
        });
        // Кейсы с ререндером шапок и подвалов не были протестированы. Если что-то сломается, необходимо заменить на _rebuildItemsStrategy
    ...
    setExpansionModel(expansionModel: Map<CrudEntityKey, boolean>): void {
        if (this.isThinInteractor()) {
            const updateItemsExpansion = (model: Map<CrudEntityKey, boolean>) => {
                model.forEach((status, key) => {
                    const item =
                        this.getItemBySourceKey(key, false) ||
                        this.getItems().find((i) => i.key === key);
                    if (item) {
            ...
            updateItemsExpansion(new Map([...(this._$expansionModel || []), ...expansionModel]));
            // Стратегия ExtraNodeItem зависит от состояния раскрытости узлов. Узлы обновляет слайс по завершению расчетов,
            // а стратегия инвалидируется по изменению рекордсета в onCollectionChange, что приводит к рассинхрону:
            // стратегии уже обновились, а состояние узлов еще нет.
            // Пока есть рассинхрон вызываем обновление вручную.
            this._rebuildItemsStrategy();
        }
        this._$expansionModel = expansionModel;
    }
    ...
    setExpandedItems(expandedKeys: TKey[]): void {
        ...
        if (typeof this._$markedKey !== 'undefined') {
            const itemToMark = this.getItems().find((i) => i.key === this._$markedKey);
            if (itemToMark && !itemToMark.isMarked()) {
                itemToMark.setMarked(true, true);
            }
        }
    ...
    protected _getItemsFactory(): ItemsFactory<T> {
        ...
        return function TreeItemsFactory(options: IItemsFactoryOptions<S>): T {
            ...
            options.hasChildrenByRecordSet = self._hasChildrenByRecordSet.get(key);

            options.expanded =
                this.getExpandedItems().includes(key) ||
                (this.getExpandedItems().includes(null) &&
                    !this.getCollapsedItems().includes(key)) ||
                (this._$expansionModel && this._$expansionModel.get(key));
    ...
    protected _spaceItemFactory(
        ...
        const spaceItem = super._spaceItemFactory(item, options);
        const parent =
            options?.parent ||
            (item['[Controls/_display/TreeItem]'] ? item.getParent() : this.getRoot());
    ...
    private _rebuildItemsStrategy(reset: boolean = false) {
        const itemsStrategy = this._getItemsStrategy();
    ...
    protected _reBuildNodeHeaders(reset: boolean = false): void {
        if (this.SupportNodeHeaders) {
            this._rebuildItemsStrategy(reset);
        }
    }
    ...
    protected _getItemState(item: T): ITreeSessionItemState<T> {
        ...
        if (item instanceof TreeItem) {
            ...
            state.expanded = this._$expansionModel
                ? this._$expansionModel.has(item.key)
                    ? (this._$expansionModel.get(item.key) as boolean)
                    : false
                : item.isExpanded();
        }
    ...
    // При иерархической группировке необходимо скрывать заголовки пустых групп и заголовок первой непустой группы (в режиме 'без заголовка')
    calculateGroupNodesVisibility(nodes: T[]): void {}
    ...
    private _recountHasChildrenByRecordSet(): void {
        ...
        let changed = false;
        this._hasChildrenByRecordSet = new Map();
        ...
        nodes.forEach((it) => {
            ...
            this._hasChildrenByRecordSet.set(it.key, it.hasChildrenByRecordSet());
        });

        this.calculateGroupNodesVisibility(nodes);
        ...
        if (this.getStrategyInstance(AddStrategy)) {
            ...
            if (parentOfAddingItem) {
                ...
                this._hasChildrenByRecordSet.set(parentOfAddingItem.key, true);
            }
        }
    ...
    protected _getFirstNearbyItem(
        ...
        return getTreeNearbyItem(
            this.getRoot(),
            enumerator,
            item,
            isNext,
            conditionProperty,
            conditionProperty !== 'Markable'
        );
    }
    ...
    protected _getNearbyItem(
        ...
        return getTreeNearbyItem(this.getRoot(), enumerator, item, isNext, conditionProperty, true);
    }
    ...
    protected _updateInteractorStateProps(interactorStateProps: IInteractorStateProps): boolean {
        ...
        if (
            'hasMoreStorage' in interactorStateProps &&
            this.interactorStateProps.hasMoreStorage !== interactorStateProps.hasMoreStorage
        ) {
            this.setHasMoreStorage(
                interactorStateProps.hasMoreStorage ?? {},
                this.isThinInteractor()
            );
            shouldUpdate = true;
        }
        if (
            'root' in interactorStateProps &&
            this.interactorStateProps.root !== interactorStateProps.root
        ) {
            this.interactorStateProps.root = interactorStateProps.root;
            this.setRoot(interactorStateProps.root);
        }
        if (
            'collapsedItems' in interactorStateProps &&
            this.interactorStateProps.collapsedItems !== interactorStateProps.collapsedItems
        ) {
            this.interactorStateProps.collapsedItems = interactorStateProps.collapsedItems;
            this.setCollapsedItems(interactorStateProps.collapsedItems || []);
        }
        if (
            'expandedItems' in interactorStateProps &&
            this.interactorStateProps.expandedItems !== interactorStateProps.expandedItems
        ) {
            this.interactorStateProps.expandedItems = interactorStateProps.expandedItems;
            this.setExpandedItems(interactorStateProps.expandedItems || []);
        }
        if (
            'expansionModel' in interactorStateProps &&
            this.interactorStateProps.expansionModel !== interactorStateProps.expansionModel
        ) {
            this.interactorStateProps.expansionModel = interactorStateProps.expansionModel;
            this.setExpansionModel(interactorStateProps.expansionModel || new Map());
            shouldUpdate = true;
        }

        return super._updateInteractorStateProps(interactorStateProps) || shouldUpdate;
    }
}
...
Object.assign(Tree.prototype, {
    ...
    _$expanderIconSize: null,
    _$expanderIconStyle: null,
