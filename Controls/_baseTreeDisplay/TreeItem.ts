...
import { getPlainItemContents, IItemTemplateProps } from 'Controls/baseList';
...
export default class TreeItem<T extends Model = Model>
    extends mixin<CollectionItem<any>, ExpandableMixin>(CollectionItem, ExpandableMixin)
    implements IGroupNode
{
    ...
    isNode(): boolean | null {
        ...
        const contents = getPlainItemContents(this);
        const isNode = contents.get
            ? contents.get(this.getNodeProperty())
            : contents[this.getNodeProperty()];
    ...
    setDragTargetNode(isTarget: boolean): void {
        if (this._isDragTargetNode !== isTarget) {
            ...
            this._notifyItemChangeToOwner('dragTargetNode');
            this._nextVersion();
        }
    }
    ...
    getItemAttrs(props: IItemTemplateProps): Record<string, unknown> {
        const superAttrs = super.getItemAttrs(props);
        const treeAttrs = {
            ...superAttrs,
        };
        const itemContents = this.contents;
        if (itemContents instanceof Model) {
            const parentKeyAttribute = itemContents?.get(this.getOwner().getParentProperty());
            if (parentKeyAttribute) {
                treeAttrs['item-parent-key'] = parentKeyAttribute;
            }
        }
        return treeAttrs;
    }
    ...
    /**
     * Возвращает первый дочерний элемент для текущего
     */
    getFirstChildItem(): TreeItem<T> {
        const children = this.getChildren();
        return children.at(0);
    }
    /**
     * Возвращает последний видимый дочерний элемент
     */
    getLastVisibleItem(): TreeItem<T> | null {
        const children = this.getChildren();
        const index = children.getCount() - 1;
        for (let i = index; i >= 0; i--) {
            const element = children.at(i);
            if (!(element.GroupNodeItem && !element.hasChildren())) {
    ...
    getNodeHeaderTemplate(): TemplateFunction {
        return this.getOwner().getNodeHeaderTemplate();
    }
    ...
    shouldDisplayExpanderBlock(expanderPaddingVisibility: TExpanderPaddingVisibility): boolean {
        // В этом случае нужно expanderBlock показывать всегда, чтобы при скроле вниз в мобильном интеракторе
        // не пропадали отступы. (такое происходит, если данные сверху обрезаются при скролле)
        if (this._$owner.getWasMobileExpanderDisplayed()) return true;
        ...
        if (expanderPaddingVisibility === 'visible') {
            this._$owner.setWasMobileExpanderDisplayed();
            return true;
        }
        // TODO мы знаем, что не работает кейс, когда, например, у нас есть список из 30 пустых папок,
        //  при этом мы видим только 10 из них, остальные не загружены. Чтобы перейти к 30-ой папке скролим вниз,
        //  проваливаемся в нее. Создаем в этой папке запись. Возвращаемся обратно в корень. Теперь в списке есть
        //  непустая папка, следовательно, должен быть отступ. Но его не будет, так как пока используется навигация
        //  по страницам, и мы вернемся в начало списка, куда данные о измененной папке не пришли. Для решения проблемы
        //  необходимо перейти к курсорной навигации
        if (
            this._$owner.areNotAtRoot() ||
    ...
    getWithoutExpanderPadding(
        withoutExpanderPadding: boolean | undefined,
        templateExpanderSize?: TSize
    ): boolean {
        // Перенёс следующую логику из ExpanderComponent:
        // 1. Если используется hasChildren и если есть дети, то не нужны отступы под экспандер.
        if (this.getExpanderVisibility() === 'hasChildren') {
            const hasChildren = this.getHasChildrenProperty()
                ? this.hasChildren()
                : this.hasChildrenByRecordSet();
            return hasChildren || !!withoutExpanderPadding;
        }
        // 2. Если на шаблоне задали withoutExpanderPadding, то берём заданное значение.
        if (withoutExpanderPadding !== undefined) {
            return withoutExpanderPadding;
        }
        // 3. Когда на списке указан expanderSize, считается, что прикладник устанавливает
        // изображение, которое и будет у элементов вместо отступа.
        // Это довольно странная логика, но на это завязаны прикладники.
        // Правильно будет использовать withoutExpanderPadding из опций шаблона.
        const expanderSize = this.getExpanderSize(templateExpanderSize);
        return this.isNode() === null && expanderSize && expanderSize !== 'null';
    }
