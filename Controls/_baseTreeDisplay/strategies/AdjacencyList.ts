...
function shouldAddGroupSpacings(groupViewMode: TGroupViewMode): boolean {
    return groupViewMode === 'blocks' || groupViewMode === 'titledBlocks';
}
 ...
 * @param groupViewMode Вид группировки (с подложкой, с подложкой под заголовком)
...
function buildChildrenMap<T>(
    ...
    for (let position = 0; position < count; position++) {
        ...
        if (
            shouldAddGroupSpacings(groupViewMode) &&
            item['[Controls/_display/SpaceCollectionItem]']
        ) {
            parentId = item.getParent()?.key;
        } else {
    ...
    implements IItemsStrategy<S, T>
{
    ...
    protected _createItemsOrder(): number[] {
        ...
        const groupViewMode = this.options.display.getGroupViewMode?.();
        const childrenMap = buildChildrenMap(sourceItems, options.parentProperty, groupViewMode);
