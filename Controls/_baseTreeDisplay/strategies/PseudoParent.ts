...
const PSEUDO_PARENT_MODULE = 'Controls/baseTreeDisplay:PseudoParentCollectionItem';
 ...
 *
 * @remark
 * Стратегия создает псевдо узел на основе пришедших данных.
 * Это нужно в ситуации, когда в дереве в последнем узле много элементов и подгрузка данных идёт с конца.
 * Без стратегии мы начнем грузить детей последнего узла до тех пор, пока не дойдём до родителя и только после этого сможем отрисоваться.
 * Со стратегией, когда подгрузилась часть детей, добавляется скрытый псевдо-родительским узел, что позволяет отрисовать список с одними детьми, пока не загрузился их родитель.
 * Псевдо-родительский узел будет отрисован до тех пор, пока не будет загружен настоящий родительский узел.
 *
 * Стратегия не решает проблему, при наличии иерархического отступа.
 *
 * @demo Controls-demo/treeGridNew/VirtualScroll/WithPseudoParent/Index
 *
 * @private
 */
    ...
    implements IItemsStrategy<S, T>
{
    ...
    splice(start: number, deleteCount: number, added?: S[]): T[] {
        ...
        if (this._pseudoParent) {
            const keyProperty = this._options.display.getKeyProperty();
            // Проверяем, не появился ли реальный родитель с тем же ключом.
            // В этом случае обязательно надо удалять _pseudoParent,
            // иначе все записи внутри родителя будут продублированы и страница перестанет работать.
            const hasRealParent = !!added.find((item) => {
                return item.get(keyProperty) === this._pseudoParent.key;
            });

            const parentProperty = this._options.display.getParentProperty();
            const removeChildren = removedItems.some((item) => {
                return item.contents.get(parentProperty) === this._pseudoParent.key;
            });

            if (hasRealParent || removeChildren) {
                this._pseudoParent = null;
            }
        }
    ...
    protected _sortItems(items: T[], options: ISortOptions<S, T>): number[] {
        ...
        const firstItemParentKey =
            items.find((item) => item.contents.get)?.contents.get(parentProperty) ?? null;
        const rootKey = display.getRoot().key;
        const isCreatePseudoParent =
            firstItemParentKey !== null &&
...
Object.assign(PseudoParent.prototype, {
    '[Controls/baseTreeDisplay:PseudoParent]': true,
    _moduleName: 'Controls/baseTreeDisplay:PseudoParent',
});
