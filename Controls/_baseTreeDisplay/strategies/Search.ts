...
import { IItemsStrategy, IItemsStrategyOptions, itemsStrategy } from 'Controls/display';
...
function getBreadCrumbsReference<S extends Model, T extends TreeItem<S>>(
    ...
    if (last && last !== root && !last.$TGGR) {
        ...
        if (!breadCrumbs) {
            ...
            const parent = item.getParent() && item.getParent().$TGGR ? item.getParent() : root;
