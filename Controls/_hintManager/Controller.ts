...
class Controller {
    ...
    /**
     * Метод обновляет рекордсет шагов маршрута.
     * @param {Types/collection:RecordSet} scheme Новый рекордсет шагов маршрута.
     * @return {Types/collection:RecordSet} Обновленный рекордсет шагов маршрута.
     */
    updateScheme(scheme: RecordSet<IStepModel>): RecordSet<IStepModel> {
        return this._routeModel.updateScheme(scheme);
    }

    private _showStep(): void {
        const step = this._routeModel.getStepByIndex(this._currentStepIndex);
        this._callBeforeOpenCallback(step).then(() => {
            this._player.showStep(step, this._routeModel.isOnlyHighlightAllowed());
        });
    }

    private _callBeforeOpenCallback(step: Model<IStepModel>): Promise<void> {
        const args = {
            ...this._routeModel.getOnBeforeOpenCallbackArguments(),
            step
        };

        if (this._beforeOpenCallback) {
            return this._beforeOpenCallback(args);
        }

        const callback = this._routeModel.getOnBeforeOpenCallback();
        switch (typeof callback) {
            case 'function':
                this._beforeOpenCallback = callback;
                return callback(args);
            case 'string':
                return this._loadBeforeOpenCallback(callback).then((callbackFn: Function | null) => {
                    if (callbackFn) {
                        this._beforeOpenCallback = callbackFn;
    ...
    private _loadBeforeOpenCallback(callbackPath: string): Promise<Function | null> {
        return loadAsync(callbackPath).then((callback: Function) => {
            return typeof callback === 'function' ? callback : null;
        }, (err: Error) => {
            IoC.resolve('ILogger').error(
                'Controls/_hintManager/Controller:_loadBeforeOpenCallback - ошибка загрузки onBeforeOpenCallback',
                err
            );
            return null;
        });
