...
import {
    ...
    IStepDisplay,
    IHighlighterOffset,
} from 'Controls/_hintManager/interface/IStepModel';
...
class Player {
    ...
    showStep(step: Model<IStepModel>, isOnlyHighlightAllowed?: boolean): void {
        ...
        if (step.get('message') || step.get('widgets')) {
            ...
            this._highlightTarget(target, highlighterOffset, step.get('display').style);
        }
    }
    ...
    private _getTarget(stepDisplay: IStepDisplay): HTMLElement | null {
        ...
        if (targetId) {
            const targetSearchArea = targetSearchAreaSelector
                ? document.querySelector(targetSearchAreaSelector)
    ...
    private _openStepPopup(step: Model<IStepModel>, target: HTMLElement, isBanner: boolean): void {
        ...
        } else if (this._stickyOpener.isOpened()) {
            this._stickyOpener.close();
        }

        const style = step.get('display').style;
        ...
        this._stickyOpener
            .open({
                ...
                eventHandlers: {
                    onClose: this._stepCloseHandler.bind(this, target, style),
                },
            })
            .then(() => {
                if (!isBanner) {
                    this._highlighter.addHighlightToTarget(style);
    ...
    private _stepCloseHandler(target: HTMLElement, style: TStyle): void {
        this._highlighter?.removeHighlightFromTarget(target, style);
        this._targetObserver?.setCurrentHintTarget(null);
    }
