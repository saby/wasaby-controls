 ...
 * @class Controls/_hintManager/Highlighter
 * @public
 */
class Highlighter {
    ...
    /**
     * Метод позиционирует div, который выделяет активный таргет подсказки, относительно таргета подсказки.
     * @param {HTMLElement} target Таргет подсказки.
     */
    updateTargetHighlighterPosition(target: HTMLElement): void {
        Highlighter._updateElementPosition(this._targetHighlighter, target);
    }

    /**
     * Метод добавляет класс на выделяющий div для визуального выделения активного таргета подсказки, а также
     * запоминает активный таргет подсказки.
     * @param {HTMLElement} target Таргет подсказки.
     */
    addHighlightToTarget(target: HTMLElement): void {
        if (this._targetHighlighter) {
    ...
    /**
     * Метод удаляет класс для визуального выделения активного таргета подсказки.
     * @param {HTMLElement} target Таргет подсказки.
     */
    ...
    /**
     * Метод удаляет класс для визуального выделения текущего активного таргета подсказки.
     */
    ...
    private _targetPositionChangeHandler = (): void => {
        if (
            ...
            Highlighter._updateElementPosition(this._targetHighlighter, this._activeTarget);
        }
    };
    ...
    static _updateElementPosition(element: HTMLElement | null, target: HTMLElement | null): void {
        if (!element || !target) {
            return;
        }
    ...
    static _getElementPosition(
        ...
        const top = maxTop - TARGET_HIGHLIGHTER_OFFSET;
        const bottom = minBottom + TARGET_HIGHLIGHTER_OFFSET;
        const left = maxLeft - TARGET_HIGHLIGHTER_OFFSET;
        const right = minRight + TARGET_HIGHLIGHTER_OFFSET;
