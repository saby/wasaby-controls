...
class RouteModel {
    ...
    /**
     * Метод получает функцию или путь до функции обратного вызова перед показом шага маршрута.
     * @return {String | Function}
     */
    getOnBeforeOpenCallback(): string | Function {
        return this._route.get('onBeforeOpenCallback');
    }

    /**
     * Метод получает объект с аргументами, который будет передан в onBeforeOpenCallback.
     * @return {Object}
     */
    getOnBeforeOpenCallbackArguments(): Record<string, unknown> {
        return this._route.get('onBeforeOpenCallbackArguments');
    }
    ...
    updateScheme(scheme: RecordSet<IStepModel>): RecordSet<IStepModel> {
        if (scheme instanceof RecordSet) {
            this._route.set('scheme', scheme);
        } else {
            IoC.resolve('ILogger').error(
                'Controls/_hintManager/RouteModel:updateScheme - список шагов должен быть экземпляром ' +
