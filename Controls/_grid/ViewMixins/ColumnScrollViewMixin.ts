...
import {
    ...
    IAbstractViewOptions,
} from './ColumnScroll/IColumnScroll';
import {
...
const ERROR_MESSAGES = {
    ...
    NEEDLESS_DRAG_SCROLL_CONTROLLER_ALREADY_EXISTS:
        'Внутренняя ошибка Controls.grid:View! Возмодность скроллирования мышью только включилась, но контроллер уже создан!',
    ...
    return Boolean(
        !model.destroyed &&
            (!!model.getHeader() ||
                !!(
                    model.getResults() &&
                    model.getResults().getResultsPosition() === 'top'
                )) &&
...
const createColumnScroll = (
    self: TColumnScrollViewMixin,
    options: IAbstractViewOptions
) => {
    const styleContainers =
        self._children.columnScrollStyleContainers.getContainers();
    ...
    self._$columnScrollController = new ColumnScrollController({
        ...
        initialScrollPosition:
            typeof options.columnScrollStartPosition === 'number'
                ...
                : undefined,
        getFixedPartWidth: () => {
            return getFixedPartWidth(
                self._children.gridWrapper,
                getViewHeader(self)
            );
        },
        containers: {
...
const updateSizesInDragScrollController = (
    self: TColumnScrollViewMixin
): void => {
...
function manageControllersOnDidUpdate(
    ...
    shouldCheckSizes =
        forcedCheckSizes ||
        isSizeAffectsOptionsChanged(self._options, oldOptions);
    ...
    if (
        ...
        if (self._$columnScrollController) {
            Logger.error(
                ERROR_MESSAGES.NEEDLESS_COLUMN_SCROLL_CONTROLLER_ALREADY_EXISTS
            );
    ...
    if (
        ...
        if (self._$dragScrollController) {
            Logger.error(
                ERROR_MESSAGES.NEEDLESS_DRAG_SCROLL_CONTROLLER_ALREADY_EXISTS
            );
            destroyColumnScroll(self);
        }
    }
...
const scrollToColumnEdge = (self): void => {
    ...
    const newScrollPosition =
        self._$columnScrollController.getScrollPositionWithinContainer(
            getViewHeader(self)
        );
...
const recalculateSizes = (
    ...
    if (wasUpdated) {
        ...
        !viewOptions.isFullGridSupport &&
        viewOptions.columnScrollViewMode === 'arrows'
    ) {
        updateScrollBar();
    }
};

const getFixedPartWidth = (
    gridWrapper: HTMLDivElement,
    header: HTMLDivElement
) => {
    // Находим последнюю фиксированную ячейку заголовка / результата
    const fixedElements = header.querySelectorAll(
        `.${COLUMN_SCROLL_JS_SELECTORS.FIXED_ELEMENT}`
    );
    const lastFixedCell = fixedElements[
        fixedElements.length - 1
    ] as HTMLElement;

    let fixedCellOffset: number;
    if (controller.currentLocaleConfig.directionality === 'rtl') {
        fixedCellOffset =
            gridWrapper.getBoundingClientRect().right -
            lastFixedCell.getBoundingClientRect().right;
    } else {
        fixedCellOffset =
            lastFixedCell.getBoundingClientRect().left -
            gridWrapper.getBoundingClientRect().left;
    }
...
export abstract class ColumnScrollViewMixin {
    ...
    _columnScrollOnViewBeforeMount(options: IAbstractViewOptions): void {
        ...
        if (options.columnScroll) {
            this._$columnScrollSelector =
                ColumnScrollController.createUniqSelector(options.uniqueId);
    ...
    _columnScrollOnViewDidMount(): void {
        ...
        this._gridVisibilityState = this._children.gridWrapper.closest(
            '.ws-hidden'
        )
            ? 'hidden'
            : 'visible';
        ...
        if (!canShowColumnScroll(this, this._options)) {
            if (this._options.needShowEmptyTemplate) {
                this._$columnScrollEmptyViewMaxWidth =
                    ColumnScrollController.getEmptyViewMaxWidth(
                        this._children,
                        this._options
                    );
            }
        ...
        if (!shouldDrawResult.status) {
            if (
                this._$columnScrollUseFakeRender &&
    ...
    _columnScrollOnViewBeforeUpdate(newOptions: IAbstractViewOptions): void {
        if (newOptions.columnScroll && !this._$columnScrollSelector) {
            this._$columnScrollSelector =
                ColumnScrollController.createUniqSelector(newOptions.uniqueId);
        }

        // Скроллирование мышью отключили -> разрушаем контроллер скроллирования мышью
        if (
            this._$dragScrollController &&
            !this._isDragScrollEnabledByOptions(newOptions)
        ) {
        ...
        if (
            ...
            this._options.needShowEmptyTemplate !==
                newOptions.needShowEmptyTemplate
        ) {
            this._$columnScrollController.setIsEmptyTemplateShown(
                newOptions.needShowEmptyTemplate
            );
        }
        ...
        if (
            ...
            this.getListModel().setStickyColumnsCount(
        ...
        if (!isScrollDisabled) {
            if (
                ...
                if (this._gridVisibilityState === 'showed') {
                    if (this._$columnScrollUseFakeRender) {
                        this._$columnScrollUseFakeRender = false;
                        this._$pendingMouseEnterForActivate =
    ...
    _resetColumnScroll(
        position?: IAbstractViewOptions['columnScrollStartPosition']
    ): void {
        if (!this._$columnScrollController) {
            return;
        }
    ...
    _getColumnScrollThumbStyles(options: IAbstractViewOptions): string {
        ...
        const hasMultiSelectColumn = hasCheckboxColumn(
            options,
            this.getListModel()
        );
        ...
        return gridColumns
            .map((width, idx, arr) => {
                if (idx === 0 && hasMultiSelect) {
                    ...
                    this._hasItemActionsCell(options) &&
                    idx === arr.length - 1
                ) {
    ...
    _onColumnScrollThumbPositionChanged(
    ...
    _onColumnScrollViewWheel(e: SyntheticEvent<WheelEvent>): void {
        if (this._$columnScrollController) {
            ...
            const newScrollPosition =
                this._$columnScrollController.scrollByWheel(e);
    ...
    _onColumnScrollViewResized(): void {
        if (this._options.columnScroll && this._$columnScrollIsMounted) {
            if (canShowColumnScroll(this, this._options)) {
                ...
                const controller =
                    this._$columnScrollController || ColumnScrollController;
                ...
                if (shouldResetColumnScroll) {
                    this._resetColumnScroll(
                        this._options.columnScrollStartPosition
                    );
                }
    ...
    _onColumnScrollDragScrolling(
        ...
        if (this._$dragScrollController) {
            const newPosition = this._$dragScrollController.moveDragScroll(
                e.nativeEvent
            );
