...
import { default as EditArrowComponent,  IProps as IEditArrowProps } from 'Controls/_grid/RenderReact/EditArrowComponent';
...
function EditArrowWrapper(props: IEditArrowProps & ICellProps) {
    ...
    return (
        <EditArrowComponent textOverflow={props.textOverflow} backgroundStyle={backgroundStyle} />
    );
}

// Возвращает функциональный компонент для передачи в contentTemplate прикладнику
function getEditArrowTemplate(props: Partial<ICellProps>): React.FunctionComponent {
    return React.forwardRef((editArrowTemplateProps, ref: React.ForwardedRef) => {
        return props.column.shouldDisplayEditArrow() ? (
            <EditArrowWrapper ref={ref} {...props} {...editArrowTemplateProps} />
        ) : null;
    });
}
...
function CellContentRender(
    ...
    return (
        <CellContentRender
            ...
            fontColorStyle={
                props.column.config.fontColorStyle || props.fontColorStyle
            }
...
function TooltipTemplate(props: ICellProps): JSX.Element {
    ...
    const popupTemplate = (templateProps: TInternalProps) => {
        return (
            <div
                className={templateProps.attrs?.className}
                ref={templateProps.$wasabyRef}
            >
...
function Content(
    props: ICellProps & { wasabyContext: Record<string, unknown> }
): JSX.Element {
        ...
        return (
            <div
                className={getEditorClassName(
                    props.align,
                    props.item.getEditingConfig()?.mode
                )}
            >
                {createElement(
    ...
    if (column.config?.reactContentTemplate) {
...
export default React.memo(function CellContent(
    props: ICellProps
): React.ReactElement {
    ...
    if (
        userAttrs.hasOwnProperty('className') ||
    ...
    const shouldDisplayEditArrow = column.shouldDisplayEditArrow(
        props.contentTemplate
    );
    const editArrowTemplate = shouldDisplayEditArrow && (
        <EditArrowWrapper
            {...props}
            textOverflow={column.config.textOverflow}
        />
    );

    const beforeContentTemplate =
        props.beforeContentTemplate &&
        createElement(props.beforeContentTemplate, clearProps);
    const afterContentTemplate =
        props.afterContentTemplate &&
        createElement(props.afterContentTemplate, clearProps);

    const tagTemplate = column.TagCell &&
        column.shouldDisplayTag(props.tagStyle) && (
