...
export default class GridView extends mixin<ListView, ColumnScrollViewMixin>(
    ...
    protected _horizontalScrollSelector: string = '';

    constructor(options: IGridOptions) {
        super(options);
        ColumnScrollViewMixin.call(this, options);
        this._onEditArrowClick = this._onEditArrowClick.bind(this);
    ...
    protected _beforeMount(options: IGridOptions): void {
        ...
        this._horizontalScrollSelector = `controls-GridView__horizontalScroll-${options.uniqueId}`;
    ...
    _applyChangedOptionsToModel(
        ...
        if (changes.includes('columnScrollViewMode')) {
            listModel.setColumnScrollViewMode(
                options.columnScrollViewMode || 'scrollbar'
            );
        }
        ...
        if (changes.includes('itemEditorTemplateOptions')) {
            listModel.setItemEditorTemplateOptions(
                options.itemEditorTemplateOptions
            );
        }
     ...
     */
    _applyNewOptionsAfterReload(
        oldOptions: IGridOptions,
        newOptions: IGridOptions
    ): void {
        const changes = [];
        ...
        if (changedOptions) {
            ...
            if (
                ...
                changedOptions.hasOwnProperty('columnScrollViewMode')
            ) {
                changes.push('columnScrollViewMode');
            }
        ...
        if (changes.length) {
            ...
            this._doAfterReload(() => {
                ...
                this._applyChangedOptionsToModel(
                    this._listModel,
                    newOptions,
                    changes
                );
    ...
    _applyNewOptionsOnComponentMount(newOptions: unknown): void {
        ...
        if (
            this._listModel.hasNewColumnScroll() !==
            !!newOptions.newColumnScroll
        ) {
            this._listModel.setNewColumnScroll(!!newOptions.newColumnScroll);
        }
    }
    ...
    _beforeUpdate(newOptions: IGridOptions): void {
        ...
        if (
            this._options.columnSeparatorSize !== newOptions.columnSeparatorSize
        ) {
            this._listModel.setColumnSeparatorSize(
                newOptions.columnSeparatorSize
            );
        }

        if (
            this._options.emptyTemplateColumns !==
            newOptions.emptyTemplateColumns
        ) {
            this._pendingRedraw = true;
        }
        ...
        this._listModel.setColspanGroup(
            newOptions.newColumnScroll
                ? false
                : !newOptions.columnScroll || !this.isColumnScrollVisible()
        );
    }
    ...
    _getGridEmptyTemplateRows(options: IGridOptions): string {
        ...
        if (options.columnScroll) {
            styles += ' auto auto';
        } else if (options.newColumnScroll) {
            styles += ' auto';
        }
        ...
        return Boolean(
            options.isFullGridSupport &&
                (options.stickyItemActions ||
                    ((options.columnScroll || options.newColumnScroll) &&
    ...
    _getLadderTopOffsetStyles(): string {
        ...
        const groups = this._container.getElementsByClassName(
            'controls-ListView__groupContent'
        );

        const hasTopResults =
            results && this._listModel.getResultsPosition() !== 'bottom';
        ...
        if (
            !(headerHeight || resultsHeight || groupHeight) &&
            !this._options.ladderOffset
        ) {
            return '';
        }
        return (
    ...
    _getGridViewWrapperClasses(options: IGridOptions): string {
        let classes = `controls_list_theme-${
            options.theme
        } ${this._getColumnScrollWrapperClasses(options)}`;
        ...
        if (options.newColumnScroll) {
            classes += ` ${this._horizontalScrollSelector} controls-GridView__gridWrapper_withHorizontalScroll`;
        }
        return classes;
    }
    ...
    _getGridViewClasses(
        ...
        if (options._needBottomPadding) {
            classes +=
                ' controls-GridView__paddingBottom__itemActionsV_outside';
            if (
                options.rowSeparatorSize &&
                options.rowSeparatorSize !== 'null'
            ) {
        ...
        classes += ` ${this._getColumnScrollContentClasses(
            options,
            columnScrollPartName
        )}`;
        return classes;
    }
    ...
    _getGridViewStyles(
        options: IGridOptions,
        isOnlyFixedColumns: boolean = false
    ): string {
    ...
    _observeScrollHandler(_: Event, eName: string, params: object): void {
        ...
        const convertParams = () => {
            return {
                scrollContainerWidth: params.clientWidth,
                contentContainerWidth: params.scrollWidth,
                scrollPosition: params.scrollLeft || 0,
                fixedPartWidth: this._options.fixedColumnsWidth,
            };
        };

        if (!('horizontalScrollTransformer' in this._children)) {
            return;
        }

        switch (eName) {
            case 'canHorizontalScroll':
                this._children.horizontalScrollTransformer.activateTransformer(
                    convertParams()
                );
                break;
            case 'horizontalScrollMoveSync':
            case 'horizontalScrollMove':
            case 'horizontalScrollResize':
            case 'horizontalViewportResize':
                this._children.horizontalScrollTransformer.updateTransformer(
                    convertParams()
                );
                break;
            case 'cantHorizontalScroll':
                this._children.horizontalScrollTransformer.deactivateTransformer();
                break;
        }
    }
    ...
    _onFocusIn(e: SyntheticEvent): void {
        ...
        if (
            (this.isColumnScrollVisible() || this._options.newColumnScroll) &&
            ...
            const isCellColspaned = !!target.closest(
                '.js-controls-Grid__cell_colspaned'
            );
            const correctTarget =
                target.closest(
                    isCellColspaned
            ...
            if (this._options.newColumnScroll) {
                e.stopPropagation();
                this._notify('doHorizontalScroll', [correctTarget, e]);
            } else {
    ...
    _onHeaderRowClick(event: SyntheticEvent): void {
        ...
        if (
            headerRow.isSticked() &&
            target.closest('.js-BreadCrumbsPath__backButtonArrow')
        ) {
            event.stopPropagation();
    ...
    protected _onItemMouseUp(event: SyntheticEvent, item: Row): void {
        const clickOnEditArrow = event.target.closest(
            '.js-controls-Grid__editArrow'
        );
        if (clickOnEditArrow) {
            event.stopPropagation();
            return;
    ...
    _onItemMouseMove(event: SyntheticEvent, item: Row) {
        super._onItemMouseMove.apply(this, arguments);
        this._setHoveredCell(item.getContents(), event.nativeEvent);
    }

    _onItemMouseLeave(): void {
        super._onItemMouseLeave.apply(this, arguments);
        this._setHoveredCell(null, null);
    }

    protected _onEditArrowClick(event: SyntheticEvent, row: Row): void {
        this._notify('editArrowClick', [row.getContents()]);
        event.stopPropagation();
    }

    _setHoveredCell(item: Row, nativeEvent: SyntheticEvent): void {
        const hoveredCellIndex = getCellIndexByEventTarget(
            nativeEvent,
            this._listModel
        );
        if (
            item !== this._hoveredCellItem ||
            hoveredCellIndex !== this._hoveredCellIndex
        ) {
            ...
            if (nativeEvent) {
                const target = correctEventTargetFF(nativeEvent.target);
                container = target.closest('.controls-ListView__itemV');
                hoveredCellContainer = getCellElementByEventTarget(target);
            }
            this._notify('hoveredCellChanged', [
                item,
                container,
                hoveredCellIndex,
    ...
    beforeRowActivated(target: HTMLElement): void {
        ...
        if (this._options.newColumnScroll) {
            this._notify('doHorizontalScroll', [correctTarget]);
        } else {
            this._columnScrollScrollIntoView(correctTarget);
        }
    }

    _onDragScrollOverlayReady(
        e: SyntheticEvent,
        inst: DragScrollOverlay
    ): void {
        if (this._options.newColumnScroll) {
            this._notify('dragScrollOverlayReady', [inst]);
        }
    }

    _onDragScrollOverlayMoveDrag(
        e: SyntheticEvent<TouchEvent | MouseEvent>,
        startBy: 'mouse' | 'touch'
    ): void {
        if (this._options.columnScroll) {
            this._onColumnScrollDragScrolling(e, startBy);
        } else if (startBy === 'mouse') {
            this._notify('dragScrollOverlayMoveDrag', [e]);
        }
    }

    _onDragScrollOverlayStopDrag(
        e: SyntheticEvent<TouchEvent | MouseEvent>,
        startBy: 'mouse' | 'touch'
    ): void {
        if (this._options.columnScroll) {
            ...
            this._notify('dragScrollOverlayStopDrag', [e]);
        }
    }
}
