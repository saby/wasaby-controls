...
import { IOptions as IGridCollectionOptions } from 'Controls/_baseGrid/display/mixins/Grid';
...
export default class GridView extends mixin<ListView, ColumnScrollViewMixin>(
    ...
    protected _beforeMount(options: IGridOptions): void {
        ...
        validateGridParts(options, 'error', this);
    }
    ...
    _applyChangedOptionsToModel(
        ...
        if (changes.includes('resultsVerticalPadding')) {
            listModel.setResultsVerticalPadding(options.resultsVerticalPadding);
        }
    ...
    _applyNewOptionsAfterReload(oldOptions: IGridOptions, newOptions: IGridOptions): void {
        ...
        const changedOptions = getChangedOptions(newOptions, this._options);
        ...
        if (changedOptions) {
            ...
            if (changedOptions.hasOwnProperty('resultsVerticalPadding')) {
                changes.push('resultsVerticalPadding');
            }
        }
    ...
    _getGridViewClasses(
        ...
        if (options.groupViewMode === 'blocks' || options.groupViewMode === 'titledBlocks') {
            classes +=
                ` controls-Grid__groupViewMode_${options.groupViewMode}` +
                ` controls-List__groupViewMode_${this._options.groupViewMode}`;
        }

        if (GridLadderUtil.isSupportLadder(options.ladderProperties)) {
            classes += ` ${this._ladderOffsetSelector}`;
        }
        ...
        const bottomPaddingMode = options.bottomPaddingMode;
        if (bottomPaddingMode) {
            classes += ` controls-GridView__paddingBottom__${bottomPaddingMode}_outside`;
    ...
    protected _onItemClick(event: SyntheticEvent<MouseEvent>, item: Row): void {
        // Кристально чистая логика, без BaseControl.
        //  Вероятно, сейчас не выполняется никогда, но старую вьюху могут вставить когда-то в новый список.
        if (helpers.isWithoutBaseControl(this._options)) {
            return super._onItemClick(event, item);
        }

        // TODO: Заменить на вызов супера, по логике они идентичны, но реализация методов разная.
        //  Возможно падение тестов.
        helpers.events.parseGridViewItemClick(
            {
                event,
                item,
            },
