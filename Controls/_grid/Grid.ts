...
import { isReactView } from './utils/PropsConverter';
...
> extends List<TControl> {
    ...
    protected _beforeMount(options: IGridOptions): Promise<void> | void {
        ...
        this._renderPreloadedItemsRightAway = isReactView(options);

        resolveViewControls(
            this,
            options,
            isFullGridSupport() ? GridView : (loadSync('Controls/gridIE:GridView'))(GridView),
            GridControl
        );
        return superMountResult;
    }

    protected _onColumnScrollEdgeStateChanged(
        edgesState: [EdgeState, EdgeState]
    ): void {
