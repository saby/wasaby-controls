...
export default class HeaderRow extends Row<null> {
    ...
    setRowsCount(start: number, stop: number): void {
        if (this._resizerCell) {
            const headerRowsCount =
                this.getBounds().row.end - this.getBounds().row.start;
            const dataRowsCount = stop - start;
            this._resizerCell.setRowspan(headerRowsCount + dataRowsCount);
            this._nextVersion();
        }
    }
    ...
    setColumnsWidths(columnsWidths) {
        ...
        this._nextVersion();
        this._resizerCell.setWidth(
            parseInt(columnsWidths[this._resizingGridColumnIndex], 10)
        );
    }

    calcResizerRowspan = (rowsCount: number) => {
    ...
    protected _initializeColumns(): void {
        if (this._$columnsConfig) {
            ...
            const getColumnParams = (column: IHeaderCell, index: number) => {
                return {
                    sorting: this._getSortingBySortingProperty(
                        column.sortingProperty
                    ),
                    backgroundStyle: this._$backgroundStyle,
                    cellPadding: this._getCellPaddingForHeaderColumn(
                        column,
                        index
                    ),
            ...
            this._$columnItems = this._$columnsConfig.map((column, index) => {
                const isFixed =
                    (this.isMultiline()
                        ...
                        : totalColspan) < this.getStickyColumnsCount();
            ...
            if (this.getOwner().hasResizer()) {
                const column = {};
                const resizerCellIndex =
                    this.getStickyColumnsCount() + +this._hasCheckBoxCell();
                this._resizingGridColumnIndex =
                    this.getStickyColumnsCount() - 1;
    ...
    protected _getLeftSeparatorSizeForColumn(
        ...
        if (columnIndex > 0) {
            const currentColumn = this._getHeaderColumnWithSeparator(
                column,
                columnIndex
            );
            ...
            const prevColumnIndex =
                columnIndex -
                (columnIndex > 1 &&
                this._$columnsConfig[columnIndex - 1].startColumn ===
                    column.startColumn &&
            ...
            return this._resolveColumnSeparatorSize(
                currentColumn,
                previousColumn
            );
        }
    ...
    protected _getRightSeparatorSizeForColumn(
        ...
        if (columnIndex < this._$columnsConfig.length - 1) {
            ...
            const nextColumnIndex =
                ...
                this._$columnsConfig[columnIndex + 1].endColumn ===
                    column.endColumn &&
    ...
    setSorting(sorting: ISortItem[]): void {
        ...
        if (this._$columnItems) {
            this._$columnItems.forEach((cell) => {
                ...
                if ((cell as ItemActionsCell).SupportItemActions) {
                    return;
                }
