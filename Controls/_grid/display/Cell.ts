    ...
    implements IInstantiable, IVersionable
{
    ...
    getMinHeightClasses(): string {
        ...
        return ` controls-Grid__row-cell_${size}_style-${this.getStyle()}_min_height`;
    }
    ...
    getCellComponentProps(
        ...
        let backgroundStyle;
        let hoverBackgroundStyle;

        if (this.isEditing()) {
            backgroundStyle = `editing_${this._$owner.getEditingBackgroundStyle()}`;
            hoverBackgroundStyle = 'none';
        } else {
            const getDefaultBackgroundStyleWithColumnScroll = () => {
                if (this.hasColumnScrollReact() && (this._$isFixed || this._$isFixedToEnd)) {
                    return 'default';
                }
            };

            backgroundStyle = getInheritableCellProp(
                'backgroundStyle',
                getDefaultBackgroundStyleWithColumnScroll()
            );
