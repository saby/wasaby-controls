    ...
    implements IInstantiable, IVersionable
{
    ...
    get shadowVisibility(): string {
        return this._$isActsAsRowTemplate && this._$owner.hasNewColumnScroll()
            ? 'hidden'
            : this._$shadowVisibility;
    }
    ...
    getCellContentRender(): React.FunctionComponent<
        IDataTypeRenderProps<unknown>
    > {
     ...
     * @function Controls/_grid/display/Cell#_getColspanParams
    ...
    getWrapperClasses(
        ...
        if (this.isEditing()) {
            ...
            if (this._$owner.getEditingConfig()?.mode === 'cell') {
                wrapperClasses +=
                    ' controls-Grid__row-cell-editing_singleCellEditable';
            }
        }
    ...
    protected _getBackgroundColorWrapperClasses(
        ...
        if (isRowEditingMode) {
            ...
            wrapperClasses +=
                this._getHoverBackgroundClasses(hoverBackgroundStyle);
    ...
    protected _getControlsBackgroundClass(
        backgroundColorStyle: TBackgroundStyle
    ): string {
        let wrapperClasses = '';
        if (backgroundColorStyle) {
            ...
            this._$backgroundStyle === 'default' &&
    ...
    protected _getRoundBorderClasses(): string {
        ...
        if (this._$roundBorder && this.isLastColumn()) {
            ...
            if (
                this.getItemActionsPosition() === 'inside' ||
    ...
    getStickyBackgroundStyle(): string {
        return this._$backgroundStyle === 'default' &&
            this.getStyle() !== 'default'
            ? this.getStyle()
    ...
    getRelativeCellWrapperClasses(): string {
        ...
        const shouldFixAlignment =
            this._$owner.getColumns().length ===
            (this._$owner.hasMultiSelectColumn() ? 2 : 1);
    ...
    getRelativeCellWrapperStyles(): React.CSSProperties {
        ...
        if (this._$owner.hasColumnScroll() && this._$isFixed && this.config) {
            const width =
                this.config.compatibleWidth || this.config.width || '';
    ...
    getContentClasses(
        backgroundColorStyle: TBackgroundStyle = this._$column
            .backgroundColorStyle,
        ...
        let contentClasses =
            'controls-Grid__row-cell__content js-controls-ListView__editingTarget';
        ...
        contentClasses += this._getHorizontalPaddingClasses(
            this._$column.cellPadding
        );
        contentClasses += this._getVerticalPaddingClasses();

        contentClasses +=
            ' controls-Grid__row-cell_withoutRowSeparator_size-null';

        contentClasses += this._getContentAlignClasses();

        if (this.getOwner().hasNewColumnScroll()) {
            contentClasses += ' controls-Grid__row-cell__content_noFit';
        }
        ...
        if (
            ...
            contentClasses += this._getHoverBackgroundClasses(
                templateHoverBackgroundStyle
            );
        }
    ...
    protected _getHoverBackgroundClasses(
        templateHoverBackgroundStyle?: TBackgroundStyle
    ): string {
        const hoverBackgroundStyle =
            this._$column?.hoverBackgroundStyle ||
    ...
    getContentTextStylingClasses(
        ...
        const fontColorStyle =
            this.config.fontColorStyle || templateFontColorStyle;
    ...
    getStickyHeaderMode(stickyCallback: Function): string {
        if (this.isHorizontalStickied()) {
            ...
            return this.getOwner().isSticked() || stickyCallback
                ? 'replaceable'
                : 'notsticky';
        }
    }
    ...
    setLeftSeparatorSize(columnSeparatorSize: TColumnSeparatorSize): void {
    ...
    protected _getColumnSeparatorClasses(): string {
        if (
            this.getColumnIndex() >
            (this._$owner.hasMultiSelectColumn() ? 1 : 0)
        ) {
    ...
    protected _getColumnScrollWrapperClasses(): string {
        ...
        if (this._$isFixed) {
            if (this._$owner.hasNewColumnScroll()) {
                return ` controls-NewColumnScroll__fixedElement ${DRAG_SCROLL_JS_SELECTORS.NOT_DRAG_SCROLLABLE}`;
            } else {
    ...
    protected _getHorizontalPaddingClasses(cellPadding: ICellPadding): string {
        ...
        const isFirstColumnAfterCheckbox =
            this.getColumnIndex(false, false) === 1 &&
    ...
    getColumnIndex(
        ...
        return this._$owner.getColumnIndex(
            this,
            takeIntoAccountColspans,
            takeIntoHiddenColumns
        );
    }
    ...
    getMarkerClasses(
        ...
        let classes = this.getOwner().getMarkerClasses(
            markerSize,
            addVerticalPaddings
        );
    ...
    getCellComponentProps(
        ...
        const paddingLeft = isFirstColumnAfterCheckbox
            ? 'null'
            : cellProps?.padding?.left ||
              (this.isFirstColumn() && 'm') ||
              undefined;
        const paddingRight =
            cellProps?.padding?.right ||
            (this.isLastColumn() && 'm') ||
            undefined;
        ...
        if (this.hasColumnScrollReact()) {
            ...
            className += this._$isFixed
                ? `${selectors.FIXED_ELEMENT_STATIC} ${selectors.FIXED_ELEMENT}`
                : '';
        }
        ...
        return {
            ...
            fontSize: cellProps?.fontSize || rowProps.fontSize,
            fontWeight: cellProps?.fontWeight || rowProps.fontWeight,
            fontColorStyle:
                cellProps?.fontColorStyle || rowProps.fontColorStyle,
            cursor: cellProps?.cursor || rowProps.cursor,
            tooltip: cellProps?.tooltip,
            textOverflow: cellProps?.textOverflow,

            markerVisible,
            markerSize: markerVisible ? rowProps.markerSize : undefined,
            markerClassName: markerVisible
                ? rowProps.markerClassName
                : undefined,
