...
export default abstract class Row<T extends Model = Model> {
    ...
    protected _rowProps: IRowProps | null = null;
    ...
    protected constructor(options?: IOptions<T>) {
        ...
        this._cellsIterator = (callback: TCellsIteratorCallback) => {
            this.getColumnsFromEnumerator().forEach(callback);
        };
    ...
    getRowComponentProps(): IRowComponentProps {
        return getRowComponentProps(this._rowProps || {}, this);
    }
    ...
    getMultiSelectClasses(
        ...
        let classes =
            'js-controls-ListView__notEditable controls-List_DragNDrop__notDraggable';
        ...
        if (
            this.getMultiSelectVisibility() === 'onhover' &&
        ...
        if (
            (!isRowEditingMode &&
                templateHighlightOnHover !== false &&
                !isCellEditingMode) ||
        ...
        classes +=
            ' controls-GridView__checkbox controls-Grid__border-radius-inherit';
    ...
    getStickyLadderCellsCount(): number {
        const stickyProperties = this.getStickyLadderProperties(
            this._$gridColumnsConfig[0]
        );
    ...
    shouldDrawLadderContent(
        ...
        const isFirstCell =
            (index === 0 && hasMainCell) || (index === 1 && !hasMainCell);
        if (
    ...
    getLadderWrapperClasses(
        ...
        const index = stickyProperties?.indexOf(stickyProperty);
        const hasMainCell =
            stickyLadder && !!stickyLadder[stickyProperties[0]].ladderLength;
        ...
        if (!isVisibleForLadder) {
            if (this._$ladderMode === 'visibility') {
                ladderWrapperClasses +=
                    ' controls-Grid__row-cell__ladder-content_hiddenForLadder';
    ...
    protected _getStickyLadderStyle(
        column: IColumn,
        stickyProperty: string
    ): string {
        const stickyLadder = this.getStickyLadder();
    ...
    updateLadder(
        ...
        if (this._$ladder !== newLadder) {
            ...
            const isStickyLadderChanged = !isEqual(
                this._$stickyLadder,
                newStickyLadder
            );
    ...
    isVisibleForLadder(property: string | number | symbol): boolean {
        const ladder = this.getLadder();
        return (
            !ladder || !ladder[property] || ladder[property].ladderLength >= 1
        );
    }
    ...
    protected _processStickyLadderCells(
        ...
        stickyLadderCellCtor: new (
            options
        ) => StickyLadderCell = StickyLadderCell
    ): void {
        ...
        const stickyLadderStyleForSecondProperty =
            ...
            this._getStickyLadderStyle(
                this.getGridColumnsConfig()[0],
                stickyLadderProperties[1]
            );

        if (
            stickyLadderStyleForSecondProperty ||
            stickyLadderStyleForFirstProperty
        ) {
        ...
        if (stickyLadderStyleForSecondProperty) {
            ...
            const leftOffset = this.getGridColumnsConfig()[0].width.match(
                '(([1-9][0-9]*px)|0px)'
            )?.[0];
            this._$columnItems.splice(
                1,
                ...
                new stickyLadderCellCtor({
                    ...
                    isPointerEventsDisabled:
                        this._$itemActionsPosition === 'outside',
        ...
        if (stickyLadderStyleForFirstProperty) {
            // Для выравнивания ячейки стики-лесенки подходит только заданная в px ширина колонки.
            const rightOffset = this.getGridColumnsConfig()[0].width.match(
                '(([1-9][0-9]*px)|0px)'
            )?.[0];
            this._$columnItems = (
                [
    ...
    getColumnsEnumerator(): IColumnsEnumerator {
        if (!this._$columnsEnumerator) {
            this._$columnsEnumerator = this.getOwner().getColumnsEnumerator();
        }
        return this._$columnsEnumerator;
    }

    getStickyGroupPosition(stickyCallback?: Function): string {
        if (this.isSticked()) {
            return this.getColumnsFromEnumerator()[0].getVerticalStickyHeaderPosition(
                stickyCallback
            );
        }
    }

    getColumnsFromEnumerator(): Cell[] {
        return this.getColumnsEnumerator().getColumns(
            this.getColumns(),
            !!this.getRowTemplate()
        );
    }
    ...
    getColumnIndex(
        ...
        let result = takeIntoAccountColspans
            ? columnItemIndexWithColspan
            : columnItemIndex;
    ...
    getColumnsFactory(
        ...
        if (!this._cellModule) {
            throw new Error(
                'Controls/_display/Row:getColumnsFactory can not resolve cell module!'
            );
        }
        return (options) => {
    ...
    protected _prepareColumnItems(
        ...
        const gridColumnsConfig = this.getColumnsEnumerator().getColumnsConfig(
        ...
        for (
            ...
            if (
                ...
                stickyLadderCellsCount =
                    (stickyLadderProperties && stickyLadderProperties.length) ||
                    0;
        ...
        if (
            creatingColumnsParams.length === 1 &&
            this.getGridColumnsConfig().length > 1
        ) {
            ...
            if (
                ...
                if (
                    !preventSeparateColspanedRow &&
                    (this.hasNewColumnScroll() ||
                        (this.hasColumnScroll() && hasResizer))
                ) {
        ...
        if (
            creatingColumnsParams.length === 1 &&
            (this._$rowTemplate || columns.length > 1)
        ) {
            ...
            if (prepareStickyLadderCellsStrategy === 'offset') {
                if (stickyLadderCellsCount === 1) {
                    creatingColumnsParams[0].colspan =
                        creatingColumnsParams[0].colspan - 1;
                ...
                if (stickyLadderCellsCount === 2) {
                    creatingColumnsParams[0].colspan =
                        creatingColumnsParams[0].colspan - 3;
                    creatingColumnsParams.unshift(
                        {
                            isFixed: true,
                            isHidden: true,
        ...
        if (hasSpacingColumn && shouldAddSpacingCell) {
            ...
            const lastCellParams =
                creatingColumnsParams[creatingColumnsParams.length - 1];
            creatingColumnsParams.push({
                column,
                ...
                leftSeparatorSize:
                    this._resolveColumnSeparatorSizeForSpacingColumn(
    ...
    protected _getColumnFactoryParams(
        ...
        return {
            ...
            leftSeparatorSize: this._getLeftSeparatorSizeForColumn(
                column,
                columnIndex
            ),
            rightSeparatorSize: this._getRightSeparatorSizeForColumn(
                column,
                columnIndex
            ),
    ...
    protected _initializeColumns(
        ...
        const prepareStickyLadderCellsStrategy =
            options.prepareStickyLadderCellsStrategy || 'add';
        if (this._$columnsConfig) {
            ...
            if (
                ...
                const ctor =
                    options.extensionCellsConstructors?.multiSelectCell ||
                    CheckboxCell;
            ...
            if (this.hasItemActionsSeparatedCell()) {
                const ctor =
                    options.extensionCellsConstructors?.separatedActionsCell ||
                    ItemActionsCell;
    ...
    hasNewColumnScroll(): boolean {
        ...
        return (
            this._$owner.hasNewColumnScroll && this._$owner.hasNewColumnScroll()
        );
    }
    ...
    protected _updateSeparatorSizeInColumns(
        separatorName: 'Left' | 'Right' | 'Row'
    ): void {
        ...
        this._$columnItems.forEach((cell, cellIndex) => {
            ...
            cell[`set${separatorName}SeparatorSize`](
                this[`_get${separatorName}SeparatorSizeForColumn`](
                    column,
                    columnIndex
                )
            );
    ...
    protected _getRowSeparatorSizeForColumn(
        column: IColumn,
        columnIndex: number
    ): string {
    ...
    setRowTemplate(rowTemplate: TemplateFunction): void {
        if (rowTemplate) {
            ...
            this._$rowTemplate = rowTemplate;
            this._$columnsConfig = this._savedColumns
                ? this._savedColumns
                : null;
        } else {
            return;
    ...
    protected abstract _getCursorClasses(
        cursor: string,
        clickable: boolean
    ): string;
