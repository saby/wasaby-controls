...
> {
    ...
    getColumnWidths(isOnlyFixedColumns: boolean = false): string[] {
        ...
        const columnWidths: string[] = columns.map((config, index) => {
            if (this._$columnsWidths && this._$columnsWidths[index]) {
                if (index === 0 && this.shouldCalcOuterPadding(this._$columnsWidths[index])) {
            ...
            if (this.isFullGridSupport() && config.width) {
                if (index === 0 && this.shouldCalcOuterPadding(config.width)) {
            ...
            if (!this.isFullGridSupport() && (config.compatibleWidth || config.width)) {
                if (
                    index === 0 &&
                    this.shouldCalcOuterPadding(config.compatibleWidth || config.width)
                ) {
    ...
    shouldCalcOuterPadding(width: string) {
        return width.indexOf('px') !== -1 && width.indexOf('minmax') === -1;
    }
