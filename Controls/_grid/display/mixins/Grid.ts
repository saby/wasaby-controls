...
import type { TGetRowPropsCallback, IResultConfig, IEmptyViewProps, IFooterConfig } from 'Controls/gridReact';
...
> {
    ...
    static initMixin(instance, options: IOptions) {
        ...
        if (options.emptyView) {
            this._$emptyTemplateColumns = options.emptyView;
        }
    ...
    protected _initializeResults(options: Partial<IOptions>): void {
        ...
        this._$results = new resultsRowClass({
            ...
            gridColumnsConfig: this._$resultsConfig || options.columns,
    ...
    setStickyColumnsCount(stickyColumnsCount: number): void {
        ...
        [this.getHeader(), this.getResults(), this.getFooter(), this.getEmptyGridRow()].forEach(
            (gridUnit) => {
                if (gridUnit) {
                    gridUnit.setStickyColumnsCount(stickyColumnsCount);
                }
            }
        );
...
/**
 * @interface Controls/_grid/display/mixins/Grid/IEmptyTemplateColumn
 * @public
 * @description
...
/**
 * @name Controls/_grid/display/mixins/Grid/IEmptyTemplateColumn#template
 * @cfg {TemplateFunction} Элемент, для которого рассчитывается объединение
 */

/**
 * @name Controls/_grid/display/mixins/Grid/IEmptyTemplateColumn#startColumn
 * @cfg {Number} Начальный индекс колонки.
 */

/**
 * @name Controls/_grid/display/mixins/Grid/IEmptyTemplateColumn#endColumn
