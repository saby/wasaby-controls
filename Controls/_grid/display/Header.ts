 ...
 * @private
 */
export default class Header extends mixin<
...
>(InitStateByOptionsMixin, VersionableMixin) {
    ...
    isSticked(): boolean {
        return (
            this._$owner.isStickyHeader() && this._$owner.isFullGridSupport()
        );
    }
    ...
    setColumnScrollViewMode(
        ...
        this._callRowsMethod('setColumnScrollViewMode', [
            newColumnScrollViewMode,
        ]);
    }

    setMultiSelectVisibility(multiSelectVisibility: string): void {
        this._callRowsMethod('setMultiSelectVisibility', [
    ...
    setColumnsEnumerator(enumerator: IColumnsEnumerator): void {
        this._callRowsMethod('setColumnsEnumerator', [enumerator]);
    }

    updateColumnsIndexes(): void {
        this._callRowsMethod('updateColumnsIndexes');
        this._nextVersion();
    }
    ...
    protected _getGridHeaderBounds(options: IHeaderRowOptions): IHeaderBounds {
        ...
        for (let i = 0; i < options.columnsConfig.length; i++) {
            if (typeof options.columnsConfig[i].startRow === 'number') {
                bounds.row.start = Math.min(
                    options.columnsConfig[i].startRow,
                    bounds.row.start
                );
            } else {
            ...
            if (typeof options.columnsConfig[i].endRow === 'number') {
                bounds.row.end = Math.max(
                    options.columnsConfig[i].endRow,
                    bounds.row.end
                );
            } else {
                // Одноуровневая шапка либо невалидная конфигурация шапки
