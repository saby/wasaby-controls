...
export const GRID_RESULTS_CELL_DEFAULT_TEMPLATE: string =
...
class ResultsCell extends Cell<null, ResultsRow> {
    ...
    protected _prepareDataAndFormat(): void {
        ...
        if (results && displayProperty) {
            ...
            const displayPropertyFormatIndex =
                metaResultsFormat.getIndexByValue('name', displayProperty);
            this._data = results.get(displayProperty);
            if (displayPropertyFormatIndex !== -1) {
                this._format = metaResultsFormat
                    .at(displayPropertyFormatIndex)
                    .getType() as string;
            }
        }
    }
    ...
    getWrapperStyles(): string {
        ...
        if (this._$owner.isFullGridSupport() && !this._getColspanParams()) {
            wrapperStyles += ` grid-column: ${
                this.getColumnIndex(true) + 1
    ...
    getContentClasses(
        ...
        let classes =
            ...
            ' controls-Grid__results-cell__content';
...
export { ResultsCell, IResultsCellOptions };
