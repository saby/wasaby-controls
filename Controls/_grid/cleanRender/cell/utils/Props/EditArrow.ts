...
import { isGroupCell, isTreeNodeFooterOrHeaderCell } from 'Controls/_grid/utils/Type';
...
export function shouldDisplayEditArrow({ cell, row }: IEditArrowUtilProps): boolean {
    ...
    const cellDoesNotSupportEditArrow =
        isGroupCell(cell) ||
        cell['[Controls/treeGrid:TreeGridGroupDataCell]'] ||
        isTreeNodeFooterOrHeaderCell(cell) ||
        cell['[Controls/_searchBreadcrumbsGrid/BreadcrumbsItemCell]']; // Это дешевле, чем хранить вск утилиты в grid
