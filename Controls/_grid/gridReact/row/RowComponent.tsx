...
import StickyCell from 'Controls/_grid/dirtyRender/cell/StickyCell';
...
import { isGroupCell, isTreeGroupNodeCell } from 'Controls/_grid/compatibleLayer/utils/Type';
...
function getClassName({
    ...
    // Этот класс вешаем только для строк с данными, а так же
    // у строк групп существуют кейс, когда по groupProperty у прикладников из данных достается boolean,
    // и соответственно это значение попадает в item. Если item === false,
    // то мы сюда не попадаем и получим ошибку в консоль
    ...
    if (showItemActionsOnHover) {
        ...
        className += ` controls-GridView__item_showActions controls-GridView__item_showActions_${hoverMode}`;
    }
...
function getCellComponentProps(cell: GridCell, rowProps: IRowComponentProps): ICellComponentProps {
    ...
    if (isGroupCell(cell) || isTreeGroupNodeCell(cell)) {
        cellComponentProps = {
