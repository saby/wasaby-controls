...
function getPaddingClasses({
    ...
    paddingBottom: ICellComponentProps['paddingBottom'];
    paddingLeft: ICellComponentProps['paddingLeft'];
    paddingRight: ICellComponentProps['paddingRight'];
    cellType: ICellComponentProps['cellType'];
}) {
    // В случае чекбокса отступы paddingTop, paddingBottom вешаются на сам CheckboxComponent.
...
function CellComponent(
    ...
    if (isOldBrowser) {
        // В старых браузерах перебиваем display ячейки с flex на table-cell, но верстка внутри контента ячейки ожидает,
        // что ячейка display:flex. Поэтому необходима обертка.
