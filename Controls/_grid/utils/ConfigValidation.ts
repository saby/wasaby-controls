...
const checkRequiredCellIndexes = (
    ...
    for (let i = 0; i < headerConfig.length; i++) {
        if (
            !checkCell(headerConfig[i], 'Row') ||
            !checkCell(headerConfig[i], 'Column')
        ) {
            return false;
        }
    }
    ...
    for (let i = 0; i < partColumns.length; i++) {
        ...
        if (startColumn !== prevColumnEnd) {
            if (i === 0) {
                ...
                notify(
                    prefix + ERROR_MSG.END_OF_COLUMN_IS_NOT_A_START_OF_NEXT,
                    logType
                );
            }
            return false;
...
const validateColumnsWidths = (
    ...
    return columns.reduce((acc, column) => {
        if (!column.width || GridLayoutUtil.isValidWidthValue(column.width)) {
            ...
            notify(
                ERROR_MSG.INVALID_COLUMN_WIDTH.replace(
                    '${width}',
                    column.width
                ),
                logType
...
const validateGridParts = (
    { header, columns, footer, emptyTemplateColumns }: IGridParts,
    logType: TLogType = 'error'
): boolean => {
