...
export function useDeprecatedStore(
    props: IUseStoreDeprecatedParams
): IUseDeprecatedStoreResult {
    ...
    useEffect(() => {
        subscribesRef.current = Object.entries(props.handlers || {}).map(
            ([key, handler]) => {
                return getStore().onPropertyChanged(
                    key,
                    handler as unknown as any
                );
            }
        );
        return () => {
