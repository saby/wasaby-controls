...
/**
 * Создаёт экземпляр Action для действий с записью
 * @param actionConfig
 * @param item
 * @param slice
 */
function createExecutableAction(actionConfig: IAction, item: Model, slice: ListSlice): IAction {
    const { createAction } = loadSync('Controls/actions');
    const listActionContext: { [p: string]: unknown } = {
        item,
    };
    if (actionConfig.storeId) {
        listActionContext.slice = slice;
    }
    return createAction(actionConfig.actionName, { context: listActionContext, ...actionConfig });
}
...
/**
 * Компонент Операций над записью, используемый в списках без BaseControl.
 * Для обработки событий использует контекст Controls/itemActions:ItemActionsContext
 * @param props
 * @public
 */
export default function ItemActionsComponent(props: IProps): React.ReactElement {
    ...
    const { menuPopupOpener, onActionClick, menuDependencyLoader, slice } =
    ...
    const executableActions = React.useMemo(() => {
        return props.actions.map((action) =>
            createExecutableAction(action, collectionItem?.contents, slice)
        );
    }, [props.actions]);
