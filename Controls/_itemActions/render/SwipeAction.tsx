...
import { IActionHandlers } from 'Controls/_itemActions/render/ActionsComponent';
...
function getClassName(props: IProps): string {
    let className =
        'controls-Swipe__action js-controls-ItemActions__ItemAction controls-itemActionsV__action_padding_swipe';
    ...
    className += ` controls-itemActionsV__action_style_${props.action.style || 'secondary'}`;
...
export default function SwipeAction(props: IProps): JSX.Element {
    ...
    if (props.needIcon(props.action, props.actionCaptionPosition, props.hasActionWithIcon)) {
        if (props.action.isSVGIcon) {
            icon = (
    ...
    if (props.needTitle(props.action, props.actionCaptionPosition)) {
        title = <span className={getTitleClassName(props)}>{props.action.title}</span>;
    }
