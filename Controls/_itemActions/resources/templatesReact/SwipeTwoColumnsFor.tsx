...
export default function SwipeTwoColumnsFor(props: ISwipeActionsComponentProps) {
    return props.actions.map((column, columnIndex) => {
        const actions = (column as unknown as IShownItemAction[]).map(
            (action, actionIndex) => {
        ...
        return (
            <>
                <div
                    className={'controls-Swipe__action_columns'}
                    key={column.id}
                >
                    {actions}
                </div>
                {columnIndex === 0 && (
                    <div
                        className={
                            'controls-Swipe__separator controls-Swipe__separator_horizontal'
                        }
                    />
                )}
            </>
