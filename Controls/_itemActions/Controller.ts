...
import {
    IItemActionsObject,
    IShownItemAction,
} from './interface/IItemActionsObject';
...
export type IItemActionsTemplateUnmountedCallback = (
    item: CollectionItem
) => void;
 ...
 * @param isEditing
 */
const defaultVisibilityCallback = (
    action: IItemAction,
    item: Model,
    isEditing: boolean
) => {
...
export class Controller {
    ...
    private _setItemActionsStateCallbacks: Record<
        CrudEntityKey,
        ISetItemActionsStateCallback
    >;
    ...
    constructor(options?: IControllerOptions) {
        ...
        this._getItemActionsTemplateMountedCallback = (
            item,
            setStateCallback
        ) => {
    ...
    updateOptions(options: IControllerOptions): void {
        ...
        this._actionsAlignment =
            options.actionAlignment || DEFAULT_ACTION_ALIGNMENT;
        this._itemActionsPosition =
            options.itemActionsPosition || DEFAULT_ACTION_POSITION;
        ...
        if (
            ...
            this._commonItemActions =
                !options.itemActions && options.editArrowAction
                    ? []
                    : options.itemActions;
            this._itemActionsProperty = options.itemActionsProperty;
    ...
    activateSwipe(
        ...
        if (this._itemActionsPosition !== 'outside') {
            ...
            this._updateSwipeConfig(
                item,
                actionsContainerWidth,
                actionsContainerHeight
            );
        } else if (
            this._editArrowAction &&
            this._editArrowVisibilityCallback(item.getContents())
        ) {
    ...
    prepareActionsMenuConfig(
        ...
        const target = isContextMenu
            ? null
            : this._calculateTargetPoint(clickEvent.target);
    ...
    getActiveItem(): IItemActionsItem {
        ...
        if (
            ...
            activeItem = this._collection.getItemBySourceKey(
                this._activeItemKey
            );
        }
        return activeItem;
    ...
    startSwipeCloseAnimation(): void {
        ...
        // Вызываем перерисовку SwipeTemplate
        this._updateActionsOnParticularItem(swipeItem);
    }
    ...
    updateSwipeConfigIfNeed(
        ...
        const itemNode = baseContainer.querySelector(
            itemSelector
        ) as HTMLElement;
        ...
        const swipeContainerSize = Controller.getSwipeContainerSize(
            itemNode,
            measurableSelector
        );
        const need =
        ...
        if (need) {
            this._updateSwipeConfig(
                item,
                swipeContainerSize.width,
                swipeContainerSize.height
            );
        }
    }
    ...
    private _getActionsMenuTemplateConfig(
        ...
        const iconSize =
            (this._contextMenuConfig && this._contextMenuConfig.iconSize) ||
            DEFAULT_ACTION_SIZE;
    ...
    private _loadDependencies(): Promise<unknown[]> {
        if (!this._loadMenuTempPromise) {
            ...
            this._loadMenuTempPromise = mStubs
                .require(templatesToLoad)
                .then((loadedDeps) => {
        ...
        if (editingItem) {
            ...
            unstable_batchedUpdates(() => {
                this._collection.getViewIterator().each((item) => {
                    ...
                    const itemChanged =
                        this._updateActionsOnParticularItem(item);
                ...
                if (
                    hasChanges &&
                    (!this._setItemActionsStateCallbacks ||
                        !Object.keys(this._setItemActionsStateCallbacks).length)
                ) {
    ...
    private _updateActionsOnParticularItem(item: IItemActionsItem): boolean {
        ...
        const actionsObject = this._fixActionsDisplayOptions(
            this._getActionsObject(item)
        );
        return this._setItemActions(
            item,
            actionsObject,
    ...
    private _getMenuActions(
        ...
        if (visibleActions) {
            ...
            if (!!parentAction && !parentAction.isMenu) {
                visibleActions = this._getActionsByParentRecursive(
                    visibleActions,
                    parentAction
                );
            } else {
                visibleActions = this._getItemsWithFilterRecursive(
                    visibleActions,
                    (action) => {
    ...
    private _cutOrphans(
        ...
        let availableActionsKeys: (string | number)[] =
            this._getAvailableActionsKeys(visibleActions, parentAction);
        while (
            ...
            availableActionsKeys = this._getAvailableActionsKeys(
                visibleActions,
                parentAction
            );
        }
    ...
    private _getAvailableActionsKeys(
        ...
        const availableActionsKeys: (string | number)[] = visibleActions.map(
            (action) => {
                return action.id;
            }
        );
        if (parentAction !== undefined) {
            availableActionsKeys.push(parentAction ? parentAction.id : null);
        }
    ...
    private _getActionsByParentRecursive(
        ...
        visibleActions.forEach((action) => {
            ...
            if (parent.id === action.parent) {
                result.push(action);
                if (action['parent@']) {
                    result = result.concat(
                        this._getActionsByParentRecursive(
                            visibleActions,
                            action
                        )
                    );
    ...
    private _updateSwipeConfig(
        ...
        const contents = Controller._getItemContents(item);
        const menuButtonVisibility = this._hasMenuHeaderOrFooter()
            ? 'visible'
            : 'adaptive';
        ...
        const actionsTemplateConfig =
            this._collection.getActionsTemplateConfig();
        actionsTemplateConfig.actionAlignment = this._actionsAlignment;

        if (
            this._editArrowAction &&
            this._editArrowVisibilityCallback(contents)
        ) {
        ...
        swipeConfig.itemActions.showed = visibleActions.map(
            this._prepareSwipeAction
        );
    ...
    private _hasMenuHeaderOrFooter(): boolean {
        return (
            this._contextMenuConfig &&
            !!(
                this._contextMenuConfig.footerTemplate ||
                this._contextMenuConfig.headerTemplate
            )
        );
    }
    ...
    private _getActionsObject(item: IItemActionsItem): IItemActionsObject {
        ...
        const showed =
            this._actionMode !== 'adaptive'
                ? this._filterActionsToShowOnHover(
                      all,
                      contents,
                      item.isEditing()
                  )
    ...
    private _filterActionsToShowOnHover(
        ...
        for (let i = 0; i < itemActions.length; i++) {
            ...
            const isMenuAction =
                !itemActions[i].showType ||
                itemActions[i].showType === TItemActionShowType.MENU;
            ...
            if (
                isMenuAction ||
                itemActions[i].showType === TItemActionShowType.MENU_TOOLBAR
            ) {
        ...
        if (actionsToShowOnHover.length > 0) {
            ...
            if (
                (actionsToShowInMenu.length === 1 &&
                    actionsToShowOnHover.indexOf(actionsToShowInMenu[0]) ===
                        -1) ||
                actionsToShowInMenu.length > 1 ||
    ...
    private _filterVisibleActions(
        ...
        return itemActions.filter((action) => {
            return this._itemActionVisibilityCallback(
                action,
                contents,
                isEditing
            );
    ...
    private _prepareMenuAction(action: IItemAction): IItemAction {
        ...
        shownAction.iconStyle =
            Controller._getIconStyleForStaticBackground(action);
        return shownAction;
    }
     ...
     * @private
     */
    private _fixActionsDisplayOptions(
     ...
     * @private
     */
    private static _getIconStyleForStaticBackground(
        action: IItemAction
    ): TIconStyle {
        return action.viewMode === 'filled'
            ? DEFAULT_ICON_STYLE
            : Controller._getIconStyle(action);
    }
    ...
    private _setItemActions(
        ...
        if (
            !oldActionsObject ||
            (actionsObject &&
                !Controller._isMatchingActions(
                    oldActionsObject,
                    actionsObject,
                    skipShowedChecking
                ))
        ) {
            this._updateActionsOnItem(item, actionsObject);
            return true;
        }
        return false;
    }
    ...
    private _updateActionsOnItem(
        ...
        const setItemActionsStateCallback =
            this._setItemActionsStateCallbacks?.[item.key];
        item.setActions(actionsObject, !!setItemActionsStateCallback);
        if (setItemActionsStateCallback) {
            ...
            this._itemActionsVisibility !== 'visible' &&
    ...
    private static _isMatchingActions(
        ...
        return skipShowedChecking
            ? isMatchedAll
            : isMatchedAll && isMatchedShowed;
    }
