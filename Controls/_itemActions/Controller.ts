...
import { ActionsMenuUtils, MENUTYPE } from './utils/ActionsMenuUtils';
import { SwipeUtils } from './utils/SwipeUtils';
import { ActionsMapUtils } from './utils/ActionsMapUtils';
import { ActionsStyleUtils } from './utils/ActionsStyleUtils';
...
export type IItemActionsTemplateUnmountedCallback = (itemKey: CrudEntityKey) => void;
 ...
 */
const defaultVisibilityCallback = (_action: IItemAction, _item: Model, _isEditing: boolean) => {
    return true;
};
...
export class Controller {
    ...
    private _getItemActionsTemplateMountedCallback: IItemActionsTemplateMountedCallback | null;
    private _getItemActionsTemplateUnmountedCallback: IItemActionsTemplateUnmountedCallback | null;
    ...
    constructor(options?: IControllerOptions) {
        ...
        this._getItemActionsTemplateMountedCallback = (itemKey, setStateCallback) => {
            if (!this._setItemActionsStateCallbacks) {
                this._setItemActionsStateCallbacks = {};
            }

            this._setItemActionsStateCallbacks[itemKey] = setStateCallback;
        };

        this._getItemActionsTemplateUnmountedCallback = (itemKey) => {
            if (
                this._setItemActionsStateCallbacks &&
                Object.keys(this._setItemActionsStateCallbacks).length
            ) {
                if (this._setItemActionsStateCallbacks[itemKey] === undefined) {
                    Logger.warn(
                        'Для записи в реестре был сохранён сеттер операций над записью, но по ключу ' +
                            itemKey +
                            ' в кеше сеттеров ничего не найдено. В некоторых случаях это может указывать на утечку памяти в ItemActionsController.'
                    );
                }
                delete this._setItemActionsStateCallbacks[itemKey];
            }
        };
    ...
    destroy(): void {
        this._setItemActionsStateCallbacks = {};
        this._getItemActionsTemplateUnmountedCallback = null;
        this._getItemActionsTemplateMountedCallback = null;
    }
    ...
    activateSwipe(
        ...
        if (this._itemActionsVisibility === 'visible') {
            this._saveItemActions(item.getActions());
        }
    ...
    prepareActionsMenuConfig(
        ...
        return ActionsMenuUtils.prepareActionsMenuConfig({
            actions: actionsObject.all,
            excludedActions: actionsObject.showed,
            clickEvent,
            parentAction,
            opener,
            actionsMenuConfig: this._contextMenuConfig,
            allowConfigWithoutActions: this._hasMenuHeaderOrFooter() && !this._task1183329228,
            menuLayoutContext: {
                item: Controller._getItemContents(item),
            },
            menuType: isContextMenu
                ? MENUTYPE.CONTEXT
                : item.isSwiped()
                ? MENUTYPE.SWIPE
                : MENUTYPE.ADDITIONAL,
    ...
    private _updateActionsOnItems(editingItem?: IItemActionsItem): (number | string)[] {
        ...
        // Свайпнутый элемент имеет свои опции записи, которые нельзя набирать в общем цикле,
        // т.к. это приводит к неправильной перерисовке.
        const swipedItem = this._itemActionsPosition !== 'outside' && this.getSwipeItem();
        ...
        if (editingItem) {
            ...
            unstable_batchedUpdates(() => {
                this._collection.getViewIterator().each((item) => {
                    ...
                    const itemChanged = this._updateActionsOnParticularItem(
                        item,
                        item !== swipedItem
                    );
        ...
        // Опции для swipedItem не считаются в общем цикле.
        if (hasChanges && swipedItem) {
    ...
    private _updateActionsOnParticularItem(
        item: IItemActionsItem,
        shouldChangeItemState?: boolean
    ): boolean {
    ...
    private _saveItemActions(actionsObject: IItemActionsObject): void {
        this._savedItemActions = actionsObject;
    }
    ...
    private _setItemActions(
        ...
        if (
            ...
            return this._updateActionsOnItem(item, actionsObject, shouldChangeItemState);
        }
        return false;
    }

    // при silent не будет вызываться кешированный setState, и не будет перерисовки.
    private _updateActionsOnItem(
        item: IItemActionsItem,
        actionsObject: IItemActionsObject,
        shouldChangeItemState: boolean = true
    ): boolean {
