...
class HorizontalMeasurer implements IMeasurer {
    ...
    getSwipeConfig(
        ...
        const actualActions: IItemAction[] =
    ...
    needIcon(
        ...
        return (
            !!action.icon ||
            (hasActionWithIcon && actionCaptionPosition !== 'none')
        );
    }
    ...
    needTitle(
        ...
        return (
            !action.icon || (actionCaptionPosition !== 'none' && !!action.title)
        );
    }
    ...
    private static _calculateActionSize(
        ...
        return this._calculateActionsSizes(
            [itemAction],
            templateConfig,
            theme
        )[0];
    ...
    private static _calculateVisibleActions(
        ...
        const itemActions = MeasurerUtils.sliceAndFixActions(
            actions,
            MAX_ACTIONS_COUNT
        );
        ...
        if (
            ...
            itemActions.length === actions.length
        ) {
        ...
        if (
            menuButtonVisibility === 'visible' ||
            visibleActions.length < itemActions.length
        ) {
            menuItemAction = HorizontalMeasurer._getMenuItemAction();
            menuActionSize = this._calculateActionSize(
                menuItemAction,
                templateConfig,
                theme
            );
