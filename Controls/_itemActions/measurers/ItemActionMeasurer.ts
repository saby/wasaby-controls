...
export function getActions(
    ...
    const availableActionsCount = getAvailableActionsCount(
        iconSize,
        containerSize
    );
    if (
        rootActions.length > availableActionsCount ||
        rootActions.length < allActions.length
    ) {
        showedActions = MeasurerUtils.sliceAndFixActions(
            rootActions,
