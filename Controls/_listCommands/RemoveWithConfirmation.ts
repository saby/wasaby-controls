...
const NO_RECORDS = rk(
    'Нет записей для обработки команды',
    'ОперацииНадЗаписями'
);
...
export default class RemoveWithConfirmation extends Remove {
    execute(meta?: Partial<IOptions>): Promise<string | void> {
        ...
        const confirmationConfig =
            RemoveWithConfirmation._getConfirmationConfig(config);
        return Confirmation.openPopup(confirmationConfig)
            .then((result) => {
                if (confirmationConfig.message === NO_RECORDS || !result) {
                    return Promise.reject();
                }
                return this._getProvider(config.providerName).then(
                    (provider) => {
    ...
    private static _getConfirmationConfig(
        meta: IOptions
        ...
        if (
            ...
            config = {
                ...
                style: 'default',
                message:
                    meta.selectedKeysCount === 1 ? removeSingle : removeMany,
            };
        }
        return config;
    }
