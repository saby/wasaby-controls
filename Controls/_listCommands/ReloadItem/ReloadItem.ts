...
export default class ReloadItem
    implements ICommand<IReloadItemCommandOptions, IReloadItemResult>
{
    /**
    ...
    execute(options: IReloadItemCommandOptions): IReloadItemResult {
        if (options.itemKey !== undefined) {
            return options.sourceController.reloadItem(
                options.itemKey,
                options
            );
        } else {
            throw new Error(
                'ReloadItemCommand::В опции не передан ключ элемента для перезагрузки'
            );
        }
    }
}
