...
export default class ShowOnlySelected implements ICommand<ICommandOptions> {
    ...
    private _filterItemsBySelection(): void {
        const itemsToRemove = [];
        const itemsToAdd = [];
        const { items, parentProperty, root, selectedItems } = this._options;
        const newItems = items.clone(true);

        newItems.setEventRaising(false);

        selectedItems.slice().forEach((item) => {
            ...
            if (
                ...
                if (parentProperty) {
                    ...
                    if (!selectedItems.find((item) => item.getKey() === itemParent)) {
    ...
    private _isSelected(itemKey: TKey): boolean {
        const isSelected = !!this._options.selectedItems.find((selectedItem) => {
            return selectedItem.getKey() === itemKey;
        });

        if (isSelected || !this._options.selectDescendants) {
            return isSelected;
        }

        if (this._options.selection.excluded.includes(itemKey)) {
            return false;
        }

        // Если selectDescendants true, необходимо проверить не выбран ли родитель
        return this._hasSelectedParent(itemKey);
    }
    ...
    private _hasSelectedParent(itemKey: TKey): boolean {
        const { parentProperty, items, selection, root } = this._options;

        if (itemKey === null) {
            return itemKey === root && selection.selected.includes(root);
        }

        if (typeof parentProperty === 'undefined' || typeof root === 'undefined') {
            return false;
        }

        let currentParentKey = itemKey;

        let hasSelectedParent = false;

        do {
            const item = items.getRecordById(currentParentKey);
            // значит мы дошли до самого корня
            if (!item) {
                // Проверяем что это точно корень, а не просто кривая иерархия и проверяем, что корень выбран
                if (root !== currentParentKey || !selection.selected.includes(root)) {
                    break;
                }
            ...
            currentParentKey = item.get(parentProperty);

            if (selection.selected.includes(currentParentKey)) {
                hasSelectedParent = true;
                break;
            }
