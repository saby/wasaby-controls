...
export default class Move implements IAction {
    ...
    protected _getTargetKey(config: Partial<IOptions>): Promise<CrudEntityKey> {
        if (config.targetKey !== undefined || config.target !== undefined) {
            const targetKey =
                config.targetKey !== undefined ? config.targetKey : config.target?.getKey();
        ...
        if (config.position && config.position !== LOCAL_MOVE_POSITION.On) {
            const target =
                config.position === LOCAL_MOVE_POSITION.Before
                    ? config.siblingStrategy.getPrevByKey(config.selection.selected[0])
                    : config.siblingStrategy.getNextByKey(config.selection.selected[0]);
    ...
    private static _getProvider(
        ...
        return loadAsync(providerName).then((provider: IProvider<IMoveProviderOptions>) => {
            return new provider();
        });
    }
}
