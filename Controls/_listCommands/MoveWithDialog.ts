 ...
 * @class Controls/_listCommands/MoveWithDialog
...
export default class MoveWithDialog extends Move {
    ...
    protected _getTargetKey(config: Partial<IOptions>): Promise<CrudEntityKey> {
        ...
        const root =
            (popupOptions?.templateOptions as IMoverDialogTemplateOptions)
                ?.root || null;
        return this._openMoveDialog({ ...config, popupOptions }).then(
            (target) => {
     ...
     * @private
     */
    private _openMoveDialog(
        config: Partial<IOptions>
    ): Promise<Model | CrudEntityKey> {
        ...
        const templateOptions: IMoverDialogTemplateOptions = {
            ...
            ...(config.popupOptions
                .templateOptions as IMoverDialogTemplateOptions),
        };
     ...
     * @private
     */
    private _applyBeforeMoveCallback(
        config: Partial<IOptions>
        ...
        if (beforeMoveCallback) {
            ...
            if (typeof beforeMoveCallback === 'string') {
                callbackResult = loadAsync(beforeMoveCallback).then(
                    (callback: Function) => {
    ...
    private static _validateBeforeOpenDialog(
        ...
        if (!popupOptions.template) {
            result.message =
                'MoveWithDialog: MoveDialogTemplate option is undefined';
            result.isError = true;
    ...
    private static _getPopupOptions(
        config: Partial<IOptions>
    ): IMoveDialogOptions {
        return (
            config.popupOptions || {
