...
export default class MoveProvider implements IProvider<IOptions> {
    ...
    execute(meta: Partial<IOptions>): Promise<void | DataSet> {
        ...
        // В случае PrefetchProxy у source отсутствует метод call, делаем вызовы напрямую в target source.
        const source: SbisService = meta.source['[Types/_source/IDecorator]']
            ? ((meta.source as unknown as IDecorator).getOriginal() as SbisService)
            : (meta.source as SbisService);
        /*
         ...
         */
        if (!meta.useDefaultMoveMethod && source.call && meta.position === LOCAL_MOVE_POSITION.On) {
        ...
        return source.move([...meta.selection.selected], meta.targetKey, {
