 ...
 * См стандарт https://n.sbis.ru/article/94e586e2-bcc2-41eb-a5f3-69b3666487d7 Позиционирование окна на экране
...
export function getTargetCoordinates(
    ...
    if (datasetKey === 'popupRect' && target?.dataset?.[datasetKey]) {
        ...
        if (datasetKey === 'rowRect' && target?.dataset?.[datasetKey]) {
            domRect = JSON.parse(target.dataset[datasetKey] as string) as unknown as DOMRect;
        } else if (target?.closest?.(LIST_ITEM_SELECTOR)) {
            // Если не указан datasetKey или отсутствует dataset И окно вызвано для записи списка,
            // то берём координаты кнопки, которая вызвала окно.
            domRect = target?.left
                ? target
                : target?.closest?.(BUTTON_SELECTOR)?.getBoundingClientRect();

            // !!! Во всех остальных случаях открываем окно ПО ЦЕНТРУ РАБОЧЕЙ ОБЛАСТИ, см стандарт !!!
        }
