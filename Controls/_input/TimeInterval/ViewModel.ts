...
export interface IOptions extends IBaseInputOptions {
    mask: string;
    shouldNotRemoveStartZeros?: boolean;
}

/**
 * Модель для поля интервала
 * @private
 */
export class ViewModel extends BaseViewModel<IOptions> {
    ...
    handleInput(splitValue: ISplitValue, inputType: InputType) {
        ...
        return super.handleInput(ViewModel._prepareData(result as IInputConfig), inputType);
    }
    ...
    protected override _convertToValue(displayValue: string): string | TimeInterval {
    ...
    protected _convertToDisplayValue(value: string) {
        ...
        if (value === null) {
            ...
            preResult = this._timeIntervalToValueConverter(value as unknown as TimeInterval);
    ...
    private _timeIntervalToValueConverter(value: TimeInterval): string {
        ...
        const maskMatch = this._options.mask.match(/H/g);

        while (maskMatch !== null && hours.toString().length < maskMatch.length) {
    ...
    private _valueToTimeIntervalConverter(value: string): TimeInterval {
        const parsedValue: (number | string)[] = this._displayValueParser(value);
        ...
        const formatTimeInterval: string =
            'P0DT' +
            ViewModel._formatTimesUnits(parsedValue, [hours, minutes, seconds], this._options.mask);
    ...
    private static _prepareData(result: IInputConfig) {
