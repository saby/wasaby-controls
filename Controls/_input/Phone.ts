...
class Phone extends Base<IPhoneOptions> {
    ...
    protected _inputHandler(): void {
        if (
            ...
            this._viewModel.displayValue.indexOf(')') ===
                this._viewModel.displayValue.length - 1 &&
            this._viewModel.displayValue.length - 1 ===
                this._viewModel.selection.start
        ) {
    ...
    protected _initProperties(options: IPhoneOptions): void {
        ...
        if (options.flagVisible) {
            const flagSize = ['default', 's', 'm'].includes(
