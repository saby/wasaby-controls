...
import { View as PhoneFlag } from 'Controls/flagSelector';
...
export interface IPhoneOptions extends IBaseInputOptions {
    ...
    countryCode?: string;
    showCountryCodeInPlaceholder?: boolean;
}
...
class Phone extends Base<IPhoneOptions, ViewModel> {
    ...
    protected _getPlaceholderValue(): IBaseInputOptions['placeholder'] {
        if (this.props.onlyMobile && this.props.showCountryCodeInPlaceholder) {
            // Есть места, где указали подсказку с кодом страны. На всякий случай чистим эти места.
            const placeholder =
                (this.props.placeholder as string)?.replace?.(/\+\d+ /, '') || rk('Телефон');
    ...
    shouldComponentUpdate(nextProps: IPhoneOptions, nextState: IBaseInputState): boolean {
        if (this.props.value !== nextProps.value) {
            // Если меняют значение, надо очистить значение по умолчанию из модели
    ...
    protected _mouseDownHandler(event: SyntheticEvent<HTMLElement, MouseEvent>): void {
        if (
            ...
            if (event.nativeEvent.x - 13 > clientRect.x) {
                ...
                // Также есть проблеима на десктопе, из-за чего контрол перерисовывается до того, как придет новый props.value
                setTimeout(() => {
                    this._viewModel.moveCarriageToEnd();
                    this._getField()?.setSelectionRange?.(selection, selection);
                    this._nextLocalVersion();
        ...
        if (options.flagVisible && options.onlyMobile) {
            ...
            const flagTemplate = {
                ...
                scope: {
                    ...
                    value: this._viewModel.value,
                    areaPhoneCode: this._getAreaPhoneCode(),
                },
            };
    ...
    static defaultProps = {
        ...
        flagPosition: 'start',
        countryCode: 'RU',
        showCountryCodeInPlaceholder: true,
    };
}
 ...
 */
/**
 * @name Controls/_input/Phone#showCountryCodeInPlaceholder
 * @cfg {Boolean} Определяет будет ли отображаться код страны в подсказке или нет
 * @default true
 */
