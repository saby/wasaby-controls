...
class Password extends Base<IPasswordOptions, ViewModel> {
    ...
    shouldComponentUpdate(nextProps: IPasswordOptions, nextState: IBaseInputState): boolean {
        const res = super.shouldComponentUpdate(nextProps, nextState);
        if (
            this.props.revealable !== nextProps.revealable ||
            this.props.passwordVisible !== nextProps.passwordVisible
        ) {
 ...
 * @cfg {Function} Сallback, который будет вызван при нажатии на глазик. Если callback вернет false или Promise<false>, то отменяется стандартное поведение, и пароль не показывается.
