...
const _private = {
     ...
     * }
     */
    getDataBySplitValue(format: IFormat, splitValue: ISplitValue): IInputConfig | false {
...
const InputProcessor = {
     ...
     * @return {Object}
     */
    getClearSplitValue(splitValue: ISplitValue, clearData: IClearData): ISplitValue {
        const clearSplitValue: ISplitValue = {
            before: '',
            after: '',
            insert: '',
            delete: '',
     ...
     * @param shouldShiftReplacer если true, то заменитель будет вставлен в начало строки.
     * @returns {{value: (String) новая строка, position: (Integer) позиция курсора}}
     */
    insert(
        format: IFormat,
        clearSplitValue: ISplitValue,
        replacer: string,
        ...
        for (let i = 0; i < clearSplitValue.insert.length; i++) {
            ...
            if (shouldShiftReplacer && replacer === oldClearSplitValue.after.slice(-1)) {
                ...
                data = _private.getDataBySplitValue(format, newClearSplitValue as ISplitValue);
            } else {
                ...
                // Если не получилось, то пробуем заменить.
                if (!data) {
                    newClearSplitValue = {
                        before: oldClearSplitValue.before + char,
     ...
     * @returns {{value: (String) новая строка, position: (Integer) позиция курсора}}
     */
    delete(format: IFormat, clearSplitValue: ISplitValue, replacer: string) {
     ...
     * @returns {{value: (String) новая строка, position: (Integer) позиция курсора}}
     */
    deleteForward(format: IFormat, clearSplitValue: ISplitValue, replacer: string) {
        let newClearSplitValue;
        ...
        return _private.getDataBySplitValue(format, newClearSplitValue as ISplitValue);
    },
     ...
     * @returns {{value: (String) новая строка, position: (Integer) позиция курсора}}
     */
    deleteBackward(format: IFormat, clearSplitValue: ISplitValue, replacer: string) {
        let newClearSplitValue;
    ...
    _getNewSplitValue(oldSplitValue: ISplitValue, value: string): ISplitValue {
    ...
    _getDataByCurDisplayValue(
        oldValue: string,
        oldSplitValue: ISplitValue,
        oldFormat: IFormat,
        curDisplayValue: string | undefined
    ): { splitValue: ISplitValue; value: string } | undefined {
        let newSplit;
        if (curDisplayValue) {
    ...
    /**
     * Ввод.
     * @param splitValueSrc значение разбитое на части before, insert, after, delete.
     ...
     * @param curDisplayValue текущее отображаемое значение.
     * @param shouldShiftReplacer если true, то заменитель будет вставлен в начало строки.
     * @return {{value: (String) новая строка, position: (Integer) позиция курсора}}
     */
    input(
        splitValueSrc: ISplitValue,
        inputType: string,
        replacer: string,
        oldFormat: IFormat,
        newFormat: IFormat,
        curDisplayValue: string | undefined = undefined,
        shouldShiftReplacer: boolean | undefined = undefined
    ): IInputConfig | undefined {
        ...
        if (!clearData) {
            clearData = Formatter.clearData(oldFormat, curDisplayValue as string);
            if (!clearData) {
                return;
            }
        }
