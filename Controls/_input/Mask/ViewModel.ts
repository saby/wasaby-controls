...
import { IMaskOptions } from 'Controls/_input/Mask';
 ...
 */
const _private = {
    updateFormatMaskChars(self: ViewModel, formatMaskChars: IFormatMaskChars) {
    ...
    prepareSplitValue(result: IInputConfig) {
...
/**
 * Модель для поля ввода маски
 * @private
 */
class ViewModel extends BaseViewModel<IMaskOptions> {
    protected _shouldShiftReplacer: boolean = true;
    _formatMaskChars: object = {};
    formatMaskCharsRegExp: RegExp;
    protected _format: IFormat;
    protected newValue: string;

    constructor(options: IMaskOptions, value?: any) {
        super(options, value);
        this.setCarriageDefaultPosition(options.selectionStart || 0);
    }

    checkMaskValue(moduleName: string): void {
        if ((this.options.mask?.length || 0) > CORRECT_MASK_LENGTH && this.options.replacer) {
    ...
    _convertToValue(displayValue: string): string {
        ...
        if (Array.isArray(this.options.mask)) {
            ...
            value = this._getValue(displayValue, this.options.mask as string);
        }
    ...
    protected _setFormat(mask: string): void {
        ...
        this._format = FormatBuilder.getFormat(
            mask,
            this.options.formatMaskChars as IFormatMaskChars,
            replacer as string
        );
    }
    ...
    _convertToDisplayValue(value: string): string {
        ...
        _private.updateFormatMaskChars(this, this.options.formatMaskChars as IFormatMaskChars);
    ...
    private _getFormattedValue(): string {
        ...
        const format = FormatBuilder.getFormat(
            mask,
            this.options.formatMaskChars as IFormatMaskChars,
            this.options.replacer as string
        );
    ...
    handleInput(splitValue: ISplitValue, inputType: InputType): boolean {
        const results: (IInputConfig | undefined)[] = [];
        let result;
        ...
        if (Array.isArray(this.options.mask)) {
            ...
            result = this._getInputProcessorResult(
                splitValue,
                inputType,
                this.options.mask as string
            );
            results.push(result);
        }
        ...
        return super.handleInput.call(
            this,
            _private.prepareSplitValue(matchResult as IInputConfig)
        );
    }

    private _getBestMatchResult(
        results: (IInputConfig | undefined)[],
        resValue: string
