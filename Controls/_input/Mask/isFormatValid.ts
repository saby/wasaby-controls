...
function isFormatValid(
    value: string,
    mask: string,
    replacer: string | undefined = DEFAULT_OPTIONS.replacer,
    formatMaskChars: IFormatMaskChars | undefined = DEFAULT_OPTIONS.formatMaskChars,
    skipDelimeters: boolean = true
): boolean {
    try {
        const format = FormatBuilder.getFormat(
            mask,
            formatMaskChars as IFormatMaskChars,
            replacer as string,
            skipDelimeters
        );
