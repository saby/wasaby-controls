...
export interface IBaseTextInputOptions extends ITextOptions, IBaseInputOptions<string> {
    /**
...
> extends Base<TBaseTextOptions> {
    ...
    constructor(props: TBaseTextOptions) {
        super(props);
        this._inputMode = props.inputMode as string;
        if (props.convertPunycode) {
            this._loadConverterPunycode().then(() => {
                BaseText._validateConstraint(props.constraint as string | RegExp);
    ...
    shouldComponentUpdate(nextProps: TBaseTextOptions, nextState: IBaseInputState): boolean {
        const res = super.shouldComponentUpdate(nextProps, nextState);
        if (this.props.constraint !== nextProps.constraint) {
            BaseText._validateConstraint(nextProps.constraint as string | RegExp);
        }
        return res;
    }
    ...
    protected _recalculateLocationVisibleArea(
        ...
        super._recalculateLocationVisibleArea(field, displayValue, selection);
    }
