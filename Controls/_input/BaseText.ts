...
export class BaseText extends Base<IBaseTextInputOptions> {
    ...
    protected _beforeMount(
        ...
        if (options.convertPunycode) {
            return this._loadConverterPunycode().then(() => {
                return this._syncBeforeMount(options, context, receivedState);
            });
        }
    ...
    private _syncBeforeMount(
        options: IBaseTextInputOptions,
        context: object,
        receivedState: object
    ): void | object {
        const res = super._beforeMount(options, context, receivedState);
