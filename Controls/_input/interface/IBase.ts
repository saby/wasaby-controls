 ...
 * @remark В случаях, когда нужно чтобы фон поля ввода отличался от фона по умолчанию, поле отображалось с заливкой и
 * горизонтальными отступами от границ, передайте в опцию значение true. В остальных случаях поле ввода прозрачное и без горизонтальных отступов, значение false.
 * Важно!
 * Если установлено значение true, и включено отображение границы(borderVisibility), то обводка отобразится при наведении, согласно {@link http://axure.tensor.ru/StandardsV8/%D0%BF%D0%BE%D0%BB%D1%8F_%D0%B2%D0%B2%D0%BE%D0%B4%D0%B0_%D0%B8_%D0%BC%D0%B5%D1%82%D0%BA%D0%B8.html стандарту}
 */

/**
 * @name Controls/_input/interface/IBase#autoWidth
 * @cfg {Boolean} Определяет будет ли у поля ввода динамическая ширина.
 * @remark Важно! В случае задания авто ширины, позаботьтесь о том, чтобы у поля была задана минимальная и максимальная ширина.
 * @demo Controls-demo/Input/AutoWidth/Index
 */

/**
 * @name Controls/_input/interface/IBase#shortPlaceholder
 * @cfg {String} Определяет текст подсказки, которая будет отображена в правой части поля ввода с веденным значением.
 * @demo Controls-demo/Input/ShortPlaceholder/Index
 */
/**
 * @name Controls/_input/interface/IBase#inputMode
 * @cfg {String} Определяет какая клавиатура отобразится на мобильном устройстве при фокусе. Список всех доступных значений смотрите {@link https://developer.mozilla.org/ru/docs/Web/HTML/Global_attributes/inputmode тут}
 */
