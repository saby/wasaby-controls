...
export class ViewModel extends BaseViewModel {
    ...
    private _isReplaceWithAsterisks(
        options: IPasswordOptions
    ): boolean | string {
        return (
            !(options.autoComplete || options.passwordVisible) ||
            options.readOnly
        );
    }
    ...
    private _adjustSplitValue(splitValue: ISplitValue, value: string): void {
        ...
        splitValue.after = value.substring(
            value.length - splitValue.after.length
        );
    }

    private _calcDisplayValue(
        replaceWithAsterisks: string | boolean,
        value: string
    ): string {
    ...
    protected _convertToDisplayValue(value: string | null): string {
        ...
        const replaceWithAsterisks = this._isReplaceWithAsterisks(
            this._options
        );
    ...
    protected handleInput(
        ...
        this._displayValue = this._calcDisplayValue(
    ...
    private static _limitLength(
        splitValue: ISplitValue,
        maxLength: number
    ): void {
        const maxInsertionLength: number =
            maxLength -
            Array.from(splitValue.before).length -
            Array.from(splitValue.after).length;
        splitValue.insert = Array.from(splitValue.insert)
            .slice(0, maxInsertionLength)
            .join('');
    }
