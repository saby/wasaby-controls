...
import { default as BaseViewModel } from 'Controls/_input/BaseViewModel';
...
export interface IBaseInputOptions<Value = any>
    ...
    onValueChanged?: (value: Value, displayValue: string) => void;
    valueChangedCallback?: (value: Value, displayValue: string) => void;
    onInputCompleted?: (value: Value, displayValue: string) => void;
    inputCompletedCallback?: (value: Value, displayValue: string) => void;
 ...
 * @remark
 * Полезные ссылки:
 * * {@link https://git.sbis.ru/saby/wasaby-controls/-/blob/rc-24.6100/Controls-default-theme/variables/_input.less переменные тем оформления}
 *
 ...
 * @implements Controls/interface:IComponentPropsWithReadonly
...
class Base<
    ...
    TViewModel extends
        | ViewModel<TBaseInputOptions>
        | BaseViewModel<any, any> = ViewModel<TBaseInputOptions>,
> extends Component<TBaseInputOptions, IBaseInputState> {
    ...
    protected forCalcRef = createRef<HTMLElement>() as IFixedReactRef<HTMLElement>;
    ...
    protected _isBeforeRender: boolean = false;
    ...
    protected _useEvent;
    private _ignoreContextMenu: boolean = false;

    protected _timeoutId: ReturnType<typeof window.setTimeout> | undefined;
    protected _wasabyPlaceholderTemplate: TemplateFunction;
    ...
    constructor(props: TBaseInputOptions) {
        ...
        this._useEvent = {
            ...
            onContextMenu: this._contextMenuHandler,
        ...
        // Есть проблема с васаби и связана она с тем, что при каждой перерисовке будет приходить новое значение для шаблона плейсхолдера.
        // Чтобы обойти эту проблему, сохраняем шаблон в состоянии и уже используем его.
        if (props.placeholder && (props.placeholder as TemplateFunction).isWasabyTemplate) {
            this._wasabyPlaceholderTemplate = props.placeholder;
        }
        ...
        if (props.autoWidth) {
            this._autoWidth = props.autoWidth;
        }
    ...
    componentDidMount(): void {
        ...
        // Есть проблема при быстром ввода. Проблема заключается в том, что переходят в поле ввода и продолжают что-то вводить.
        // Браузер не успевает выделить текст, а ввод идет, из-за чего пользователь получает не то поведение, что хотел.
        // Поэтому заранее выделяем весь текст, чтобы текст точно успел выделиться
        if (
            this.props.selectOnClick &&
            this._controlName !== 'Money' &&
            !detection.isMac &&
            this._viewModel.displayValue.length
        ) {
    ...
    shouldComponentUpdate(nextProps: IBaseInputOptions, nextState: IBaseInputState): boolean {
        ...
        if (this.props.autoWidth !== nextProps.autoWidth) {
            this._autoWidth = !!nextProps.autoWidth;
        }
        return this.props !== nextProps || this.state !== nextState;
    }

    componentDidUpdate(prevProps: IBaseInputOptions): void {
        // Костыль до тех пор, пока не перепишем поля ввода на функциональные компоненты.
        // https://online.sbis.ru/opendoc.html?guid=aac30b39-c1bc-4446-9456-bd092e69b6a1&client=3
        // Проблема в том, что в контексте меняется readOnly и в компонент приходит fieldTemplate.
        // А мы ставим корректное значение на shouldComponentUpdate, поэтому до тех пор,
        // пока с полем не повзаимодействуют, fieldTemplate будет отображаться старый
        if (
            this.props.leftFieldTemplate !== prevProps.leftFieldTemplate ||
            this.props.rightFieldTemplate !== prevProps.rightFieldTemplate
        ) {
            if (
                this._leftFieldWrapper.template !== this.props.leftFieldTemplate ||
                this._rightFieldWrapper.template !== this.props.rightFieldTemplate
            ) {
                this._nextLocalVersion();
            }
            this._initProperties(this.props);
        }
    }
    ...
    protected _mouseLeaveHandler(event: MouseEvent<HTMLDivElement>): void {
    ...
    protected _inputHandler(event: SyntheticEvent<HTMLElement, KeyboardEvent>): void {
        ...
        if (this.props.isReact) {
            ...
            // Значение для timeout подобрано экспериментально
            // При значении в 100, у некоторых пользователей наше обновление происходило раньше чем придет новый value
            // При значении 200 шанс воспроизвести ошибку стремится к 0, но для подстраховки добавляем еще 50мс
    ...
    protected _focusInHandler(event: SyntheticEvent<HTMLElement, FocusEvent>): void {
        // Выделяем весь текст только если стоит опция, и что-то введено в поле
        if (this.props.selectOnClick && this._viewModel.displayValue.length) {
            this._viewModel.select();
            const inputField = this._getField()?.getContainer?.();
            ...
            if (inputField && inputField.getAttribute?.('contenteditable') === 'true') {
                ...
                // На мобилке вместе с выделением текста стреляет событие contextMenu, которое не должно было выстрелить
                // Поэтому устанавливаем флаг, чтобы самостоятельно остановить событие
            ...
            if (detection.isMac) {
                ...
                // Есть проблема с выделением текста. Суть в том, что мы выделяем текст до того,
                // как произойдет нативное событие установки каретки.
                // Из-за чего наше выделение сбрасывается, и каретка ставится туда, куда нажал пользователь.
                // Поэтому устанавливаем каретку по timeout
                setTimeout(() => {
                    this._viewModel.select();
    ...
    protected _contextMenuHandler(e: SyntheticEvent): void {
        if (this._ignoreContextMenu) {
            e.preventDefault();
            e.stopPropagation();
        } else {
    ...
    private _getStretcherValue(): string {
        ...
        // в SabyGet немоноширинный шрифт, из-за этого поле ввода прыгает. Возьмем самый широкий символ, чтобы по
    ...
    _isFieldFocused(): boolean {
        ...
        if (this._isBrowserPlatform && this._mounted) {
            return this._getActiveElement() === this._getField()?.getContainer?.();
        }
    ...
    protected _getValue(options: IBaseInputOptions): string {
        ...
        if (this._viewModel) {
            return this._viewModel.value as string;
        }
    ...
    setCursorPosition(position: number | ISelection): void {
        this._viewModel.selection = position;
        this._updateSelection(this._viewModel.selection, true);
    }

    getCursorPosition(): number {
        return this._getField()?.getFieldData('selectionStart') || 0;
    }
    ...
    protected _getContent(contentProps: Record<string, unknown>): ReactNode {
        const attrs =
    ...
    protected _setRef(el: HTMLElement): void {
        ...
        // @ts-ignore $wasabyRef для совместимости, чтобы ничего не поломалось
    ...
    protected _getPlaceholderValue(): IBaseInputOptions['placeholder'] {
        return this._wasabyPlaceholderTemplate || this.props.placeholder;
    }
    ...
    render() {
        ...
        return (
            <Render
                ...
                placeholder={this._getPlaceholderValue()}
