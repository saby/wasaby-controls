...
> {
    ...
    protected _isEqualValueProps = true;
    ...
    protected _isEqualValue(props) {
        if (this._timeOutId) {
            clearTimeout(this._timeOutId);
        }
        if (this.props.hasOwnProperty('value')) {
            this._isEqualValueProps = this.props.value === props.value;
        }
    }
    ...
    shouldComponentUpdate(nextProps: IBaseInputOptions, nextState: IBaseInputState): boolean {
        ...
        this._updateViewModel(newViewModelOptions, this._getValue(nextProps));
    ...
    componentDidUpdate(prevProps: IBaseInputOptions, prevState: IBaseInputState): void {
        ...
        this._updateViewModel(newViewModelOptions, value);
        this._isEqualValue(prevProps);
    ...
    protected _nextVersion() {
        ...
        if (this._isEqualValueProps) {
            ...
            // Чтобы этого избежать, вызываем перерисовку по setTimeout.
            this._timeOutId = setTimeout(() => {
                if (
                    this.state.modelVersion !== this._viewModel.getVersion() &&
                    !(
                        this.props.hasOwnProperty('value') &&
                        this.props.value === this._viewModel.value
                    )
                ) {
                    this.setState({
