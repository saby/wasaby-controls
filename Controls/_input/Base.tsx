...
> extends Component<TBaseInputOptions, IBaseInputState> {
    ...
    protected _focusOutHandler(event: SyntheticEvent<HTMLElement, FocusEvent>): void {
        ...
        // Если стоит опция выделять текст при клике, и мы на десктопе, но не маке, то выделяем весь текст при потере фокуса.
        // Это нужно для того, чтобы при повторном фокусе браузеру не пришлось выделять весь текст
        // Смотрим именно на десктопе и не мак, так как мак и мобилка будут возвращать фокус на компонент
        if (this.props.selectOnClick && detection.isDesktop && !detection.isMac) {
            this.setCursorPosition({
                start: 0,
                end: this._viewModel.displayValue.length,
            });
