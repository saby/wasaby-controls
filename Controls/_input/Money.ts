...
import { default as FieldTemplate } from 'Controls/_input/Money/Field';
import ReadOnlyFieldTemplate from 'Controls/_input/Money/ReadOnly';
...
class Money extends Base<IMoneyOptions> implements INumberLength, IOnlyPositive {
    ...
    constructor(props: IMoneyOptions) {
        super(props);
    ...
    protected _mouseEnterHandler(event: MouseEvent<HTMLDivElement>): void {
        if (!this.focused) {
            super._mouseEnterHandler(event);
        }
        this.focused = true;
    }
    ...
    protected _mouseLeaveHandler(event: MouseEvent<HTMLDivElement>): void {
        this.focused = false;
        super._mouseLeaveHandler(event);
    }
    ...
    shouldComponentUpdate(nextProps: IMoneyOptions, nextState: IBaseInputState): boolean {
        ...
        if (
            ...
            nextProps.value === this._getValue(nextProps)
        ) {
            this._viewModel.displayValueBeforeUpdate = this._viewModel.displayValue;
        }
        if (!detection.isIE && this.props.fontColorStyle !== nextProps.fontColorStyle) {
            this._field.scope.fontColorStyle = nextProps.fontColorStyle;
        }
        if (!detection.isIE && this.props.fontSize !== nextProps.fontSize) {
            this._field.scope.fontSize = nextProps.fontSize;
        }
        if (!detection.isIE && this.props.fontWeight !== nextProps.fontWeight) {
            this._field.scope.fontWeight = nextProps.fontWeight;
        }
        return res;
    }

    componentDidUpdate(prevProps: IMoneyOptions) {
        super.componentDidUpdate(prevProps);
        if (!detection.isIE && this.props.fontColorStyle !== prevProps.fontColorStyle) {
            this._field.scope.fontColorStyle = this.props.fontColorStyle;
        }
        if (!detection.isIE && this.props.fontSize !== prevProps.fontSize) {
            this._field.scope.fontSize = this.props.fontSize;
        }
        if (!detection.isIE && this.props.fontWeight !== prevProps.fontWeight) {
            this._field.scope.fontWeight = this.props.fontWeight;
        }
    }
 ...
 */
/**
 * @name Controls/_input/Money#useGrouping
