 ...
 *
 * @mixes Controls/input:IAreaOptions
...
export default class Area extends BaseText<IAreaOptions> {
    ...
    protected _faceResizeObserver: ResizeObserverUtil;
    ...
    protected _oldFakeElementHeight: number = 0;

    protected _setForCalcRef(el: HTMLDivElement) {
        this._faceResizeObserver.terminate();
        if (el) {
            this._faceResizeObserver.observe(el);
            this.forCalcRef.current = el;
        }
    }
    ...
    constructor(props: IAreaOptions) {
        ...
        this._setForCalcRef = this._setForCalcRef.bind(this);
        this._validateLines(props.minLines as number, props.maxLines as number);
        this._faceResizeObserver = new ResizeObserverUtil(
            this,
            this._observerLocationVisibleArea.bind(this)
        );
    }
     ...
     */
    protected _observerLocationVisibleArea(entity: IEntity[]): void {
        if (!entity?.[0]) {
            return;
        }
        const scroll = this.scrollRef.current as Container;

        const positionCursor = entity[0].contentRect.height;
        ...
        if (!hasVisibilityCursor || positionCursor !== this._oldFakeElementHeight) {
            /*
             * At the time of the scroll position change, the DOM must be updated.
             * So wait until the control redraws.
             */
            this._oldFakeElementHeight = positionCursor;
    ...
    private _fixSyncFakeArea(): void {
        /*
         * 1) На MacOS иногда между выполнением обработчика и перестроением успевает перерисоваться страница. Из-за этого происходят скачки.
         ...
         * Так как размеры textarea зависят от fakeField, поэтому их значения на момент перерисовки страницы должны быть одинаковыми. Иначе
         * возникают проблемы 1-2. Чтобы избежать проблем меняем значение fakeField в обработчике.
         */
        if (!this._getReadOnly() && detection.isMacOSDesktop && this.fakeFieldRef.current) {
            this.fakeFieldRef.current.textContent =
                ...
                >
                    {this._getReadOnly()
                        ...
                        : getContent(this._field.template, {
                              ...
                              forCalcRef: this._setForCalcRef,
    ...
    render() {
        ...
        const style = {
            ...
            '--calculated-line-min-height_inputArea':
                'calc(' + this._minLines + ' * var(--line-height_' + this.props.fontSize + '))',
...
/**
 * @name Controls/_input/Area#borderVisibility
 * @cfg {String} Видимость границ контрола.
 * @variant partial
 * @variant hidden
 * @variant bottom
 * @default partial
