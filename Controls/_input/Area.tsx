...
export default class Area extends BaseText<IAreaOptions> {
    ...
    protected containerRef = createRef<Render>();
    ...
    constructor(props) {
        ...
        this._validateLines(props.minLines, props.maxLines);
    }
    ...
    shouldComponentUpdate(nextProps: IAreaOptions, nextState): boolean {
        ...
        if (
            ...
            this._validateLines(nextProps.minLines, nextProps.maxLines);
        }
    ...
    protected _scrollStateChanged(scrollState, oldScrollState): void {
    ...
    protected _getBorderVisibility(): TBorderVisibilityArea {
        ...
        return this.props.borderVisibility;
    }
    ...
    protected _keyUpHandler(event: SyntheticEvent<KeyboardEvent>): void {
        ...
        if (keyCode >= constants.key.end && keyCode <= constants.key.down) {
            const container: HTMLElement = this.fakeFieldRef.current;
    ...
    private _recalculateLocationVisibleArea(
        ...
        const positionCursor = this._calcPositionCursor(
            this.fieldWrapperRef.current, // this._children
            textBeforeCursor
        );
    ...
    private _fixSyncFakeArea(): void {
        ...
        if (!this._getReadOnly() && (detection.isMacOSDesktop || detection.chrome)) {
    ...
    protected _getContent(contentProps = {}) {
        ...
        const style: CSSProperties = {
            minHeight: this._isIE
                ? this._lineHeightForIE[this.props.fontSize] * this._minLines + 4 + 'px'
                : 'var(--calculated-line-min-height_inputArea-scroll)',
        };
        if (this._maxLines) {
            style.maxHeight = this._isIE
                ? this._lineHeightForIE[this.props.fontSize] * this._maxLines + 4 + 'px'
                ...
                >
                    {this._getReadOnly()
                        ...
                        : getContent(this._field.template, {
                              ...
                              lineHeightForIE: this._lineHeightForIE[this.props.fontSize],
                              value: this._viewModel.displayValue,
