...
export interface IMobileFocusController {
    blurHandler(event: SyntheticEvent<HTMLElement, FocusEvent>): void;

    focusHandler(event: SyntheticEvent<HTMLElement, FocusEvent>): void;

    touchStartHandler(event: SyntheticEvent<HTMLElement, TouchEvent>): void;
}
...
class MobileFocusController implements IMobileFocusController {
    ...
    private _addItem(target: EventTarget): IItem {
        let addedItem: IItem | undefined = this._at(target);
    ...
    private _at(target: EventTarget): IItem | undefined {
        return this._items[this._findIndex(target) as number];
    }
