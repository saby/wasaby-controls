...
export interface IConfig<Value, ModelOptions extends {}> {
    tag: Tag;
    model: BaseViewModel<Value, ModelOptions>;
}

type Handler<T extends Event, Value, ModelOptions extends {}> = (
    event: SyntheticEvent<HTMLElement, T> | undefined,
 ...
 * @private
 */
class ChangeEventController<Value, ModelOptions extends {}> {
    /**
    ...
    /**
     * Зафиксировать новое значение. Используется в случаях, когда родитель самостоятельно вызвал обработчик, или его вызов не требуется.
    ...
    keyDownHandler: Handler<KeyboardEvent, Value, ModelOptions> = (event, config) => {
        if (
            event &&
            ChangeEventController._isTriggeredOnKeyDown(event.nativeEvent.key, config.tag)
        ) {
            this._callChangeHandler(config.model);
        }
    };
