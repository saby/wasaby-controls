...
import ChangeEventController, {
    IConfig as ChangeEventConfig,
} from './Field/ChangeEventController';
    ...
    implements IField
{
    ...
    private _workWithSelection: WorkWithSelection<void> =
        new WorkWithSelection<void>();
    ...
    constructor(cfg: IFieldOptions<Value, ModelOptions>, context?: object) {
        ...
        this._selectionFromFieldToModel =
            this._selectionFromFieldToModel.bind(this);
    }
    ...
    private _getField(): HTMLInputElement | null {
        const field: HTMLInputElement = this._children[
            this._options.name
        ] as HTMLInputElement;
    ...
    protected _inputHandler(event: SyntheticEvent<KeyboardEvent>): void {
        ...
        const inputType: InputType = calculateInputType(
            value,
            selection,
            text,
            nativeInputType
        );
        const splitValue: ISplitValue = split(
            value,
            newValue,
            position,
            selection,
    ...
    protected _keyDownHandler(event: SyntheticEvent<KeyboardEvent>): void {
        ...
        if (
            ((event.nativeEvent.altKey &&
                event.nativeEvent.keyCode === T_KEY_CODE) ||
                event.nativeEvent.keyCode === PAUSE_KEY_CODE) &&
            this._options.transliterate
        ) {
            transliterate(this._model.value, this._getFieldSelection()).then(
                (value) => {
    ...
    static getOptionTypes(): Partial<
        Record<keyof IFieldOptions<string, {}>, Function>
    > {
        return {
