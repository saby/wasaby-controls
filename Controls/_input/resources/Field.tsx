...
export interface IFieldOptions<Value, ModelOptions>
    extends IControlOptions, IControlProps, TInternalProps, IFieldEvents {
    tag: Tag;
    name: string;
    ...
    recalculateLocationVisibleArea?: (field: HTMLInputElement, value: string, selection: ISelection) => void;
}
 ...
 *
 */
class Field<Value extends string, ModelOptions> extends Component<IFieldOptions<Value, ModelOptions>>
    implements IField {
        ...
        this._fixBugs = new FixBugs(
            {
                updatePositionCallback: () => {
                    ...
                    // На Mac есть проблема с позицией каретки.
                    // Это связано с тем, что изначально мы поставили позицию каретки в правильное положение,
                    // но после происходит нативное браузерное событие onSelect, и каретка ставится в самый конец текста.
                    // Поэтому сами ставим каретку в нужное место через setTimeout, если она отличается
                    if (detection.isMac) {
                        const selectionStart = this._model.selection.start;
                        const selectionEnd = this._model.selection.end;
                        setTimeout(() => {
                            if (this._model.selection.start !== selectionStart) {
                                this._model.selection = {start: selectionStart, end: selectionEnd};
                            }
                            setSelection();
                        }, 0);
                    } else {
    ...
    private _getField(): HTMLInputElement | null {
        const field: HTMLInputElement = this._fieldRef.current;
    ...
    activate() {
        activate(this._fieldRef.current);
    }
    ...
    render() {
        const contentProps = this._getContentProps(`controls-Field js-controls-Field${
            this.props.highlightedOnFocus ? ' controls-Field-focused-item' : ''
        }${this.props.className ? ` ${this.props.className}` : ''}`);
        if (this.props.tag === 'textarea') {
            contentProps.defaultValue = this._isBrowserPlatform ? this._model.displayValue : undefined;
        }
