...
import { Component, createRef, LegacyRef, RefObject, SyntheticEvent } from 'react';
...
import { SyntheticEvent as VEvent } from 'UI/Vdom';
...
interface IFieldEvents {
    onBlur: (event: SyntheticEvent<HTMLElement, FocusEvent>) => void;
    onInput: (event: SyntheticEvent<HTMLInputElement, KeyboardEvent>) => void;
    onKeyUp: (event: SyntheticEvent<HTMLInputElement, KeyboardEvent>) => void;
    onFocus: (event: SyntheticEvent<HTMLElement, FocusEvent>) => void;
    onClick: (event: SyntheticEvent<HTMLInputElement, MouseEvent>) => void;
    onSelect: (event: SyntheticEvent<HTMLInputElement, MouseEvent>) => void;
    ...
    onTouchStart: (event: SyntheticEvent<HTMLInputElement, TouchEvent>) => void;
    onKeyPress: (event: SyntheticEvent<HTMLInputElement, KeyboardEvent>) => void;
    onCopy: (event: SyntheticEvent<HTMLInputElement, ClipboardEvent>) => void;
    onCut: (event: SyntheticEvent<HTMLInputElement, ClipboardEvent>) => void;
    onPaste: (event: SyntheticEvent<HTMLInputElement, ClipboardEvent>) => void;
    onContextMenu?: (event: SyntheticEvent<HTMLInputElement>) => void;
}

export interface IFieldOptions<Value, ModelOptions extends {}>
    ...
    fieldRef: LegacyRef<unknown> | RefObject<unknown>;
    spellCheck?: boolean;
    fontColorStyle?: string;
}

export interface IContentProps extends IComponentPropsWithReadonly, IFieldEvents {
    ref: any;
    key?: string;
    defaultValue?: string;
    value?: string;
 ...
 *
 */
class Field<Value extends string, ModelOptions extends {}>
    extends Component<
        IFieldOptions<Value, ModelOptions>,
        {
            currentVersionModel: number;
            inputValue: string;
        }
    >
    implements IField
{
    ...
    protected _selectTimeOut: ReturnType<typeof window.setTimeout> | null;
    protected _fieldRef =
        createRef<HTMLInputElement | null>() as IFixedReactRef<HTMLInputElement | null>;
    ...
    constructor(props: IFieldOptions<Value, ModelOptions>) {
        ...
        this.state = {
            ...
            inputValue: this._model.displayValue,
        };
    ...
    componentDidMount(): void {
        ...
        if (field && field.getAttribute?.('contenteditable') !== 'true') {
            ...
            // Поэтому сами добавляем сеттер, и стреляем событие
    ...
    shouldComponentUpdate(nextProps: IFieldOptions<Value, ModelOptions>): boolean {
        ...
        if (this._model.displayValue !== this.state.inputValue) {
            this.setState({
                inputValue: this._model.displayValue,
            });
        }
     ...
     * Удалить после:
     * https://online.sbis.ru/opendoc.html?guid=677946fc-7f91-46aa-843e-280c6659d229&client=3
     */
    ...
    getContainer(): HTMLInputElement {
        return this._fieldRef.current as HTMLInputElement;
    }
    ...
    private _getContentEditableSelection(): ISelection {
        const field: HTMLInputElement = this.getContainer() as HTMLInputElement;
        let start = 0;
        let end = 0;
        ...
        if (selection.rangeCount) {
            ...
            if (range.startContainer !== range.endContainer) {
                offset = range.startContainer?.textContent?.length || 0;
                start = range.startOffset;
            } else {
                const value = detection.firefox
                    ? range.startContainer.nodeValue || range.startContainer.textContent || null
                    : (range.startContainer.nodeValue as string);
                // Если value равен null, значит выбрано все поле ввода, поэтому говорим модели, что все содержимое выделено
                if (value !== null) {
                    offset = Math.max(field.innerText.indexOf(value), 0);
                    start = range.startOffset + offset;
                } else {
                    start = 0;
                    offset = field.innerText.length;
                }
            }
    ...
    protected _notifyEvent(name: EventName): void {
        // @ts-ignore
        const nameEvent = new VEvent(null, {
            type: name,
    ...
    private _handleInput(splitValue: ISplitValue, inputType: InputType): void {
        ...
        const value: Value = this._model.value as Value;
        const selection: ISelection = { ...this._model.selection };
        ...
        if (this._model.handleInput(splitValue, inputType)) {
            if (this.props.inputCallback) {
                const formattedText: IFieldData = this.props.inputCallback({
                    value: this._model.value as Value,
    ...
    protected _inputHandler(event: SyntheticEvent<HTMLInputElement, KeyboardEvent>): void {
        const field = this.getContainer();
        const model = this._model;
        ...
        /*
         * В событиях браузера есть баг, связанный с inputType.
    ...
    protected _selectionChanged(): void {
        /*
           ...
           еще одно изменение selection -> событие о изменение selection
         */
    ...
    protected _selectHandler(event: SyntheticEvent<HTMLInputElement, MouseEvent>): void {
    ...
    protected _clickHandler(event: SyntheticEvent<HTMLInputElement, MouseEvent>): void {
        /*
    ...
    protected _keyDownHandler(event: SyntheticEvent<HTMLElement, KeyboardEvent>): void {
        ...
        if (
            ...
            transliterate(this._model.value as string, this._getFieldSelection()).then((value) => {
                this._updateField(value, this._getFieldSelection());
                this._updateModel({ value: value as Value });
                this._notifyEvent('valueChanged');
            });
        }
    ...
    protected _keyUpHandler(event: SyntheticEvent<HTMLInputElement, KeyboardEvent>): void {
        if (PROCESSED_KEYS.includes(event.nativeEvent.key)) {
            this._selectionFromFieldToModel();
        }
    ...
    protected _blurHandler(event: SyntheticEvent<HTMLElement, FocusEvent>): void {
        const field: HTMLInputElement = this.getContainer();
         ...
         * Делаем прокрутку на уровне контрола для унификации поведения.
         */
        field.scrollLeft = 0;
    ...
    protected _touchStartHandler(event: SyntheticEvent<HTMLInputElement, TouchEvent>): void {
        MobileFocusController.touchStartHandler(event);
        this.props.onTouchStart?.(event);
    }

    protected _focusHandler(event: SyntheticEvent<HTMLElement, FocusEvent>): void {
    ...
    setValue(value: string): boolean {
        ...
        if (field.value !== undefined) {
            field.value = value;
            // На android есть нативная проблема, связанная с тем, что при смене value в поле ввода, курсор ставится в конец.
    ...
    setSelectionRange(start: number, end: number, force: boolean = false): boolean {
        const field: HTMLInputElement = this.getContainer();
        if (!field) {
            return false;
        }
        ...
        return this._workWithSelection.setSelectionRange(field, selection, force);
    }

    getFieldData<T>(name: string): T {
        // @ts-ignore
        return this.getContainer()?.[name] || 0;
    }
    ...
    hasHorizontalScroll(): boolean {
        ...
        if (this._fieldRef.current) {
            return hasHorizontalScroll(this.getContainer());
        }
        return false;
    }
    ...
    scrollTo(scrollTop: number): void {
        if (this.getContainer()) {
            this.getContainer().scrollTop = scrollTop;
        }
    }
    ...
    activate(cfg?: IFocusConfig) {
        activate(this._fieldRef.current as HTMLInputElement, cfg);
    }

    protected _setRef(ref: HTMLInputElement): void {
    ...
    protected _getContentProps(className: string) {
        const attrs = wasabyAttrsToReactDom(this.props.attrs as Record<string, unknown>) || {};
        ...
        if (this._isBrowserPlatform) {
            ...
            if (typeof this._model.displayValue === 'string') {
                contentProps.defaultValue = this._model.displayValue;
            }
        } else if (this.state.inputValue) {
            contentProps.value = this.state.inputValue;
        }
        return contentProps;
    }
