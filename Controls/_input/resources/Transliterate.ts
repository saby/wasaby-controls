...
export default function transliterate(
    ...
    const firstLocale =
        i18Controller.currentLocale.indexOf('ru') !== -1 ? 'ru' : 'en';
    const secondLocale = firstLocale === 'ru' ? 'en' : 'ru';
    return import('I18n/keyboard').then(({ changeLayout }) => {
        return changeLayout(text, firstLocale, secondLocale).then(
            (firstRevertedText) => {
                if (firstRevertedText === text) {
                    return changeLayout(text, secondLocale, firstLocale).then(
                        (secondRevertedText) => {
                            return transliterateSelectedText(
                                secondRevertedText,
                                value,
                                selection
                            );
