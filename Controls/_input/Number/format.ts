...
type NumberOptions = INumberLength & INumberDisplay & INumberSign & INumberLimits;
...
function correctSign(
    parsedNumber: IParsedNumber,
    { onlyPositive, minValue, maxValue }: NumberOptions,
    carriagePosition: number
): number {
    let position: number = carriagePosition;

    const limitsNegative = minValue && minValue < 0 && maxValue && maxValue < 0;
    if (onlyPositive && !limitsNegative) {
...
function correctIntegerPart(
    ...
    const needlessChars: number | null = calcNeedlessChars(parsedNumber, 'integer', options);
...
function correctFractionalPart(
    ...
    const needlessChars: number | null = calcNeedlessChars(parsedNumber, 'fractional', options);
    const positionRelativeIntegerPart: number = getPositionRelativePart(
        parsedNumber,
        'fractional',
        carriagePosition
...
function join(original: string[], _: number, additionalZeros: number): string {
...
function concat(
    ...
    const additionalZeros = options.useAdditionToMaxPrecision
        ? (options.precision as number) - original.fractional.length
        : 0;
