...
function correctIntegerPart(
    ...
    const needlessChars: number = calcNeedlessChars(
        parsedNumber,
        'integer',
        options
    );
    ...
    const position: number = shouldBeFractional(
    ...
    if (
        needlessChars === 0 &&
        parsedNumber.negative + processedData.value.length ===
            processedData.carriagePosition
    ) {
...
function limitValue(
     ...
     */
    const replacingChars: number = Math.min(
        needlessChars,
        value.length - position.relative
    );
    ...
    return {
        ...
        value: remove(
            value,
            removeStartPosition,
            position.relative + replacingChars
        ),
    };
}
...
function moveIntegerToFractional(
    ...
    const { value: fractional, carriagePosition: shiftedPosition }: IText =
        pasteWithRepositioning(
            {
...
function handleInsignificantZero(
    ...
    const firstSignificantDigitPosition: number =
        calcFirstSignificantDigit(value);
...
function shouldBeFractional(
    fractional: string,
    options: INumberDisplay
): boolean {
