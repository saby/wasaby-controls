...
export class FixBugs {
    ...
    constructor(config: IConfig, inst: BaseInput) {
        ...
        this._carriagePositionBug = new CarriagePositionWhenFocus(
            config.updatePositionCallback
        );
    ...
    focusHandler(event: FocusEvent): void {
        ...
        const positionChanged: boolean =
            this._carriagePositionBug.focusHandler();
        if (!positionChanged) {
    ...
    dataForInputProcessing(data: IInputData): IInputData {
        ...
        processingResult =
            this._minusProcessingBug.inputProcessing(processingResult);

        this._valueInFieldBug.startInputProcessing();

        this._heightAfterInputInSafari.inputHandler(
            this._inst._getField(),
            data
        );
