...
class NumberInput extends Base<INumberInputOptions, ViewModel> implements IOnlyPositive {
    readonly '[Controls/_baseDecorator/interfaces/IOnlyPositive]': boolean = true;
    _inputMode: string = 'decimal';
    _controlName: string = 'Number';

    constructor(props: INumberInputOptions) {
        super(props);
        if (this._getReadOnly()) {
            (this._viewModel as unknown as ViewModel).trimTrailingZeros(false);
        }

        if (props.minValue && props.maxValue && props.minValue > props.maxValue) {
            Logger.error('Number: minValue должно быть меньше или равно maxValue.');
        }

        const limitsNegative =
            props.minValue && props.minValue < 0 && props.maxValue && props.maxValue < 0;
        if (props.onlyPositive && limitsNegative) {
            Logger.warn(
                'Number: при отрицательных minValue и maxValue опция onlyPositive будет игнорироваться.'
            );
        }
    }
    ...
    protected _getViewModelOptions(options: INumberInputOptions): object {
        ...
        return {
            precision: NumberInput._convertToNumber(options.precision as number),
            useGrouping: options.useGrouping,
            onlyPositive: options.onlyPositive,
            minValue: options.minValue,
            maxValue: options.maxValue,
            integersLength: NumberInput._convertToNumber(options.integersLength as number),
    ...
    protected _notifyInputCompleted(): void {
        if ((this._viewModel as unknown as ViewModel).trimTrailingZeros(true)) {
    ...
    protected _focusOutHandler(event: SyntheticEvent<HTMLElement, FocusEvent>): void {
        let isChanged = false;
        if ((this._viewModel as unknown as ViewModel).trimTrailingZeros(false)) {
            isChanged = true;
        }

        if (this.props.minValue && Number(this._viewModel.value) < this.props.minValue) {
            this._viewModel.value = this.props.minValue;
            isChanged = true;
        }

        if (this.props.maxValue && Number(this._viewModel.value) > this.props.maxValue) {
            this._viewModel.value = this.props.maxValue;
            isChanged = true;
        }
    ...
    protected _keyDownHandler(event: SyntheticEvent<HTMLElement, KeyboardEvent>): void {
        const { maxValue, minValue, onlyPositive, integersLength, changeValueByArrows } =
            this.props;
        const { key } = event.nativeEvent;
        const preparedValue = Number(this._viewModel.value) || 0;
        const preparedMinValue =
            typeof minValue === 'number' && isFinite(minValue) ? minValue : -Infinity;
        const preparedMaxValue =
            typeof maxValue === 'number' && isFinite(maxValue) ? maxValue : Infinity;

        if (!changeValueByArrows || !['ArrowUp', 'ArrowDown'].includes(key)) {
            return super._keyDownHandler(event);
        }

        let newValue = key === 'ArrowUp' ? preparedValue + 1 : preparedValue - 1;

        newValue = Math.min(preparedMaxValue, Math.max(preparedMinValue, newValue));

        if (Math.trunc(Math.abs(newValue)).toString().length > (integersLength || Infinity)) {
        ...
        this._viewModel.value = newValue.toString();
        // для precision, так как value не форматированное
        this._viewModel.value = this._viewModel.displayValue.replace(/\s/g, '');
        this._notifyValueChanged();
        super._keyDownHandler(event);
    }

    private static _validateOptions(options: INumberInputOptions): void {
        if ((options.integersLength as number) <= 0) {
            Logger.error(
...
/**
 * @name Controls/input:Number#value
...
/**
 * @name Controls/input:Number#valueChanged
 ...
 * @example
 * Контрол ввода числа с информационной подсказкой. Подсказка содержит информацию об уникальности цифр в числе.
 * <pre class="brush: html; highlight: [4]">
 * <!-- WML -->
 * <Controls.input:Number
 *    name="number"
 ...
 * @name Controls/input:Number#inputCompleted
 * @param {UI/Events:SyntheticEvent} eventObject Дескриптор события.
 * @param {String|Number} value Значение контрола ввода.
 * @param {String} displayValue Отображаемое значение контрола ввода.
 * @remark
...
/**
 * @name Controls/input:Number#minValue
 * @cfg {Number} Минимальное допустимое значение.
 * @demo Controls-demo/Input/Number/LimitedValue/Index
 * @remark Если вводимое значение оказалось меньше minValue, то значение после расфокуса установится равным minValue. Если minValue и maxValue отрицательные, то опция onlyPositive будет игнорироваться.
 * @see maxValue
 * @see onlyPositive
 */

/**
 * @name Controls/input:Number#maxValue
 * @cfg {Number} Максимальное допустимое значение.
 * @demo Controls-demo/Input/Number/LimitedValue/Index
 * @remark Если вводимое значение оказалось больше maxValue, то значение после расфокуса установится равным maxValue. Если minValue и maxValue отрицательные, то опция onlyPositive будет игнорироваться.
 * @see minValue
 * @see onlyPositive
 */
 ...
 * Если minValue и maxValue отрицательные, то опция onlyPositive будет игнорироваться.
...
/**
 * @name Controls/input:Number#changeValueByArrows
 * @cfg {boolean}
 * @remark
 * Использование опции включает изменение значения контрола по стрелочкам вверх/вниз на 1
 * @example
 * <pre class="brush: html; highlight: [4]">
 * <!-- WML -->
 * <Controls.input:Number changeValueByArrows />
 * </pre>
 * <pre class="brush: js; highlight: [3]">
 */
