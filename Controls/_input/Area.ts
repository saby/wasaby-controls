...
export default class Area extends BaseText<IAreaOptions> {
    ...
    protected _syncBeforeMount(
        options: IAreaOptions,
        context: object,
        receivedState: object
    ): void {
        const res = super._syncBeforeMount(options, context, receivedState);
    ...
    private _isTextSelected(): boolean {
        return (
            this._viewModel.selection.start !== this._viewModel.selection.end
        );
    }
    ...
    protected _keyUpHandler(event: SyntheticEvent<KeyboardEvent>): void {
        ...
        if (keyCode >= constants.key.end && keyCode <= constants.key.down) {
            ...
            const textBeforeCursor: string =
                this._viewModel.displayValue.substring(
                    0,
                    this._viewModel.selection.end
                );
            const cursorPosition: number = this._calcPositionCursor(
                container,
                textBeforeCursor
            );
            const firstLinePosition: number = this._calcPositionCursor(
                container,
                ''
            );
            ...
            if (cursorPosition === firstLinePosition) {
                ...
                this._children.scroll.scrollTo(
                    this._getField().getFieldData('offsetHeight')
                );
            }
        }
    }
    ...
    private _recalculateLocationVisibleArea(
        ...
        const endingVisibleArea =
            beginningVisibleArea + this._scrollClientHeight;
    ...
    private _newLineHandler(
        ...
        if (this._isPressEnter(event.nativeEvent)) {
            if (
                this._options.newLineKey === 'ctrlEnter' &&
                this._isPressCtrl(event.nativeEvent)
            ) {
                if (isNewLinePaste) {
                    this.paste('\n');
                }
                ...
                this._options.newLineKey !== 'enter' ||
                this._isPressAdditionalKeys(event.nativeEvent)
            ) {
