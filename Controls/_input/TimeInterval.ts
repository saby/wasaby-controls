 ...
 */
import Base, { IBaseInputOptions, IBaseInputState } from 'Controls/_input/Base';
import { descriptor, TimeInterval as TTimeInterval } from 'Types/entity';
import { IOptions as IModelOptions, ViewModel } from 'Controls/_input/TimeInterval/ViewModel';
import { SyntheticEvent } from 'react';
import { detection } from 'Env/Env';
import { ITimeIntervalOptions } from 'Controls/_input/interface/ITimeInterval';

type IOptions = IModelOptions;

interface ITimeIntervalProps
    extends IBaseInputOptions<TTimeInterval>,
        Omit<ITimeIntervalOptions, 'value'> {
    shouldNotRemoveStartZeros?: boolean;
}
...
class TimeInterval extends Base<ITimeIntervalProps> {
    ...
    shouldComponentUpdate(nextProps: ITimeIntervalProps, nextState: IBaseInputState): boolean {
    ...
    protected _focusInHandler(event: SyntheticEvent<HTMLElement, FocusEvent>) {
        const isTab: boolean = !this._focusByMouseDown;

        /*
         * По стандарту "При получении фокуса по Tab в заполненном поле для ввода даты или времени курсор устанавливается
...
/**
 * @name Controls/_input/TimeInterval#value
