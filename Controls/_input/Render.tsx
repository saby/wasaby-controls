...
export default forwardRef(function Render(props: IRenderOptions & TInternalProps, ref: LegacyRef<HTMLDivElement>) {
    const {
    ...
    const isFieldZIndex = typeof props.placeholder === 'string' || props.placeholder instanceof String;
    ...
    const tagClickHandler = useCallback((event: MouseEvent<HTMLElement>): void => {
        props.onTagClick?.(event, tag.current?._container || tag.current);
    }, [props.onTagClick, tag]);
    const tagHoverHandler = useCallback((event: MouseEvent<HTMLElement>): void => {
    ...
    const placeholderTemplate = useCallback((placeholderProps) => {
        return <div className={`controls-Render__placeholder${
    ...
    const contentProps = {
        ...
        className: `controls-Render__field controls-Render__field_textAlign_${props.textAlign}` +
    ...
    return (
        <FocusRoot
            ...
            as="div"
            className={`controls-Render ${RENDER_JS_SELECTOR} ${getBorderClass()}` +
                ...
                `${props.fontSize !== 'inherit' ? (' controls-Render-fontsize-' + props.fontSize) : ''}` +
                ` controls-text-${props.fontColorStyle}` +
                `${highlightedOnFocus ? ' controls-Render-focused-item' : ''}` +
                ` controls-Render_state-${renderState}_${props.wasActionByUser ? 'caretFilled' : 'caretEmpty'}` +
                `${props.multiline ? ' controls-Render_multiline' : (' controls-inlineheight-' + props.inlineHeight +
                    ' controls-Render-inlineheight-' + props.inlineHeight)}` +
        ...
        >
            <>
                <div
                    className={`controls-Render__wrapper${props.footerTemplate ? ' controls-Render__wrapper_footer' : ''}`}>
                    {!props.multiline && <span className="controls-Render__baseline">&#65279;</span>}
                    {props.leftFieldWrapper && getContent(props.leftFieldWrapper, {
                        className: 'controls-Render__beforeField'
                    })}
                    {getContent(ContentTemplate, contentProps)}
                    {props.rightFieldWrapper && getContent(props.rightFieldWrapper, {
                ...
                {props.tagStyle && (
                    <Async
                        ...
                        className={`controls-Render_tag_padding-right${horizontalPadding !== 'null' ? '-empty' : ''}`}
                        ref={tag}
