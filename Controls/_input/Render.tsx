...
export interface IRenderOptions
    extends IComponentPropsWithReadonly,
        ...
        IContrastBackgroundOptions {
    /**
     * @name Controls/_input/Render#readonlyViewMode
     ...
     * @cfg {HTMLElement} Шаблон, отображаемый в левой части поля.
     */
    leftFieldWrapper?: TemplateFunction;
    /**
     * @name Controls/_input/Render#rightFieldWrapper
     * @cfg {HTMLElement} Шаблон, отображаемый в правой части поля.
     */
    rightFieldWrapper?: TemplateFunction;
    /**
     * @name Controls/_input/Render#state
     * @cfg {String} Определяет состояние поля ввода.
     */
    state?: string;
    /**
     * @name Controls/_input/Render#border
     * @cfg {IBorder} Определяет то, как будет отображаться бордер.
     */
    border?: IBorder;
    /**
     * @name Controls/_input/Render#wasActionByUser
     * @cfg {Boolean} Определяет как произошла активация контрола(программная или пользовательская).
     */
    wasActionByUser?: boolean;
    /**
     * @name Controls/_input/Render#minLines
     * @cfg {Number} Определяет минимальное количество строк, которое будет отображено в поле.
     */
    minLines?: number;
    /**
     * @name Controls/_input/Render#horizontalPadding
     * @cfg {String} Размер отступов контрола по горизонтали.
     */
    horizontalPadding?: string;

    /**
     * @name Controls/_input/Render#textAlign
     * @cfg {String} Выравнивание текста по горизонтали в поле.
     */
    textAlign?: string;
    autoFocus?: boolean;
    /**
     * @name Controls/_input/Render#footerTemplate
     * @cfg {HTMLElement} Шаблон, содержащие контент подвала, который будет отображаться в поле.
     */
    footerTemplate?: TemplateFunction;
    /**
     * @name Controls/_input/Render#tagStyle
     * @cfg {String} Стиль отображения тега.
     */
    tagStyle?: string;
    /**
     * @name Controls/_input/Render#onTagClick
     * @cfg {Function} Вызываается при клике на тег.
     */
    onTagClick?: (event: MouseEvent<HTMLElement>, target: HTMLElement) => void;
    /**
     * @name Controls/_input/Render#onTagHover
     * @cfg {Function} Вызываается при наведении на тег.
     */
...
export default forwardRef(function Render(
    ...
    }, [state, contentActive, readOnly, validationStatus, props.multiline, props.borderStyle]);
    ...
    const placeholderTemplate = useCallback(
        (placeholderProps) => {
            return (
                ...
                >
                    {placeholderProps.shortPlaceholder || getContent(props.placeholder)}
                </div>
            );
        },
