 ...
 */
class Base<
    TBaseInputOptions extends IBaseInputOptions = {}
> extends Control<TBaseInputOptions> {
    /**
    ...
    protected _beforeMount(
        ...
        /*
         * Использовать getStateReceiver не получится, так при его использовании,
         * нужно задавать уникальное значение для каждого поля ввода
         * Проще сделать через Date.now и использовать receivedState, чтобы избежать лишних перерисовок.
         */
        this._fieldName = receivedState?.fieldName || 'input-' + Date.now();
        ...
        if (!receivedState) {
            return { fieldName: this._fieldName };
        }
    }
    ...
    private _updatePlaceholderDisplay(options: IBaseInputOptions): void {
        if (this._options.placeholder !== options.placeholder) {
            ...
            const isStringPlaceholder =
                ...
                options.placeholder instanceof String;
    ...
    protected _initProperties(options: IBaseInputOptions): void {
        ...
        this._field = {
            ...
            scope: {
                ...
                calculateValueForTemplate:
                    this._calculateValueForTemplate.bind(this),
    ...
    protected _notifyValueChanged(): void {
        ...
        if (this._options.valueChangedCallback) {
            this._options.valueChangedCallback(
                this._viewModel.value,
                this._viewModel.displayValue
            );
        }
    ...
    protected _getField(): Field<String, IViewModelOptions> {
        if (this._children.hasOwnProperty(this._fieldName)) {
            return this._children[this._fieldName] as Field<
                String,
                IViewModelOptions
            >;
        }
        ...
        if (field) {
            ...
            if (
                ...
                if (field._container) {
                    ...
                    hasFieldHorizontalScroll =
                        parseFloat(computedStyle.width) < tooltipWidth;
                }
            } else {
    ...
    private _recalculateLocationVisibleArea(
        ...
        const textWidthBeforeCursor = this._getTextWidth(
            displayValue.substring(0, selection.end)
        );
        ...
        const hasVisibilityCursor =
            beginningVisibleArea < positionCursor &&
            positionCursor < endingVisibleArea;
