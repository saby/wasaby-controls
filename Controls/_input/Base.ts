...
class Base<TBaseInputOptions extends IBaseInputOptions = {}> extends Control<TBaseInputOptions> {
    ...
    protected _beforeMount(options: IBaseInputOptions, context?: object): void {
        ...
        const storeValue = getStore<IInputCounterStore>(INPUT_COUNTER_STORAGE_ID)?.get('value') || 1;
        ...
        this._fieldName = receivedState?.fieldName || `ws-input_${new Date().toISOString().split('T')[0]}`;
    ...
    protected _focusInHandler(event: SyntheticEvent<FocusEvent>): void {
        ...
        if (detection.isMac) {
            setTimeout(() => {
                if (this._getField()?._getField?.()?.selectionStart !== this._viewModel.selection.start) {
