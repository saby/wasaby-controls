...
type TFontColorStyle = 'label' | 'unaccented' | 'link' | 'primary' | 'success' | 'info' | 'default';
...
export default React.forwardRef(function Label(
    ...
    const labelClass =
        ...
        ` ${
            props.multiline
                ? isFull
                    ? 'ws-line-clamp'
                    : `ws-line-clamp ws-line-clamp_${lines}`
                : 'ws-ellipsis'
        }`;

    let bodyClass = `controls-Label controls-Label_underline_${underline}`;
    const autoHeight =
        typeof props.autoHeight === 'undefined' ? underline !== 'none' : props.autoHeight; // underline !== none - костыль, так как опция underline
    // устанавливает высоту, и есть прикладные разработчики, которые завязались на это.
    if (autoHeight) {
        bodyClass += ' controls-Label_autoHeight';
    }
    if (props.className) {
    ...
    return (
        <a {...attrs} onClick={_clickHandler} ref={ref} className={bodyClass} href={props.href}>
            <div className="controls-Label__wrapper">
                <div className={labelClass}>{_isCaptionString() ? caption : <props.caption />}</div>
                {required ? <div className="controls-Label__asterisk">&nbsp;*</div> : ''}
            </div>
        </a>
    );
});
