...
import { IDraggableCollection, IDraggableItem, IDragStrategy } from './interface';
...
export interface IDragNDropProps<
    TPositionDescription extends IDragPosition<IDraggableItem> = IDragPosition<IDraggableItem>,
> extends Pick<IComponentPropsWithReadonly, 'readOnly'> {
    ...
    Strategy?: IDragStrategy<TPositionDescription>;
}
...
function createDndController(
    ...
    CustomStrategy?: IDragNDropProps['Strategy']
): DndController<IDragPosition<IDraggableItem>> {
    const strategy = CustomStrategy ?? (parentProperty ? TreeStrategy : FlatStrategy);
