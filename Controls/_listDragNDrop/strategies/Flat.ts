...
export interface IDraggableFlatCollection<
...
> extends BaseDragStrategy<IDragPosition<T>, T, C> {
    ...
    constructor(model: C, draggableItem: T) {
        ...
        this._startPosition = {
            index: this._model.getIndexBySourceItem(
                draggableItem?.getContents()
            ),
    ...
    calculatePosition({
        ...
        if (mouseOffsetInTargetItem) {
            position =
                mouseOffsetInTargetItem.top <= DRAG_MAX_OFFSET
                    ? 'before'
                    : 'after';
        } else {
            let prevIndex = -1;
            ...
            if (prevIndex === -1) {
                ...
                position =
                    currentPosition && currentPosition.position === 'after'
                        ? 'before'
                        : 'after';
            }
        }
        ...
        if (targetItem['[Controls/_display/GroupItem]'] && targetIndex > 0) {
            const shouldChangePosition = this._shouldChangePosition(
                targetIndex,
                position
            );
    ...
    protected _targetItemIsDraggable(targetItem: T): boolean {
        return (
            ...
            targetItem &&
            !targetItem['[Controls/_display/GroupItem]'] &&
            this._draggableItem.getContents().getKey() ===
                targetItem.getContents().getKey()
        );
    }
     ...
     * @protected
     */
    protected _shouldChangePosition(
        targetIndex: number,
        position: string
    ): boolean {
        ...
        const correctExpandable = () => {
            return (
                position === 'before' ||
                (position === 'after' && item.isExpanded())
            );
        };
        const inBounds = () => {
