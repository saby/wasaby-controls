...
export default class Controller<P> {
    ...
    constructor(
        ...
        this._strategy = new this._strategyConstructor(
            this._model,
            this._draggableItem
        );
    }
    ...
    startDrag(
        entity: ItemsEntity,
        dragStartedFromAnotherList: boolean = false
    ): void {
        this._entity = entity;
    ...
    calculateDragPosition(params: IDragStrategyParams<P>): P {
        if (!this._strategy) {
            throw new Error(
                'Strategy was not created. Should be called Controller::startDrag'
            );
        }
    ...
    getDraggableKeys(
        ...
        if (
            draggedKeys.length >= selection.selected.length &&
        ...
        return controller
            ...
            .then((list) => {
                ...
                const startDraggableItemKey = this._draggableItem
                    .getContents()
                    .getKey();
    ...
    static canStartDragNDrop(
        ...
        return (
            ...
            (!canStartDragNDropOption ||
                (typeof canStartDragNDropOption === 'function' &&
                    canStartDragNDropOption())) &&
            allowByTarget &&
