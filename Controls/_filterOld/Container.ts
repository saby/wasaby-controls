...
class Container extends Control<IFilterViewContainerOptions> {
    ...
    protected _beforeMount(options: IFilterViewContainerOptions): void {
        if (options.storeId) {
            ...
            if (options.preloadedSources) {
                ...
                this._setFilterController(options);
            }
        }
    }
    ...
    protected _setFilterController(options: IFilterViewContainerOptions) {
    ...
    private _initCompatible(
        ...
        if (!oldPreloadedSources && this._source) {
            ...
            delete this._source.historyId;
            delete this._source.source;
        }
    }
        ...
        if (source) {
            if (typeof source === 'function') {
                ...
                result = FilterDescription.mergeFilterDescriptions(
                    FilterController._cloneItems(source),
                    filterFromUrl
                );
                ...
                result = FilterController._cloneItems(source);
            }
        }
        return result;
    }
