...
>(ObservableMixin, SerializableMixin, OptionsToPropertyMixin) {
   ...
   можно получить идентификатор закэшированных данных для этого фильтра */
    ...
    constructor(options: Partial<IFilterControllerOptions>) {
        ...
        this._$filterDescriptionUsed = !!options.filterDescription && !options.filterButtonSource;
    ...
    update(newOptions: IFilterControllerOptions): void | boolean {
        ...
        const newPrefetchSessionId = newOptions.prefetchSessionId;
        const prefetchSessionIdChanged = this._options.prefetchSessionId !== newPrefetchSessionId;
        ...
        if (
            ...
            (!newPrefetchSessionId || !prefetchSessionIdChanged)
        ) {
    ...
    private _processHistoryOnItemsChanged(
        ...
        this._processPrefetchOnItemsChanged(
            options,
            items,
            doNotDeleteCurrentHistory,
            isFilterFromHistory
    ...
    private _processPrefetchOnItemsChanged(
        ...
        if (history) {
            ...
            const resultFilter = FilterCalculator.getFilterByFilterDescription(
                this._$filter,
                items
            );
            const needInvalidate =
                prefetchParams &&
                ((!isFilterFromHistory && !isEqual(resultFilter, this._$filter)) ||
                    prefetch.needInvalidatePrefetch(history.data));
    ...
    static _cloneItems(items: IFilterItem[] | RecordSet<IFilterItem>): IFilterItem[] {
        let resultItems;
