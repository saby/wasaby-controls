...
export default class TreeGridViewTable extends TreeGridView {
    protected _template: TemplateFunction = loadSync<typeof import('Controls/gridIE')>('Controls/gridIE').GridViewTemplate;

    protected _getGridViewWrapperClasses(options: ITreeGridOptions): string {
        return `${super._getGridViewWrapperClasses(
            options
        )} controls-Grid__Wrapper_table-layout`;
    }
