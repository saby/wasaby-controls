...
> extends BaseTreeControl<TOptions> {
    ...
    protected _onItemClick(
        ...
        const clickOnEditArrow = originalEvent.target.closest(
            `.${EDIT_ARROW_SELECTOR}`
        );
        if (!!clickOnEditArrow) {
    ...
    protected _shouldHandleItemMouseUp(item): boolean {
        return super._shouldHandleItemMouseUp(item) && !item['[Controls/treeGrid:TreeGridGroupDataRow]'];
    }

    protected _shouldHandleItemMouseDown(item): boolean {
        return super._shouldHandleItemMouseDown(item) && !item['[Controls/treeGrid:TreeGridGroupDataRow]'];
    }
