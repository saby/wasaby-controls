...
export default class TreeGridNodeFooterRow extends TreeGridNodeExtraRow {
    ...
    shouldDisplayExtraItem(content: TemplateFunction): boolean {
        ...
        return this.hasMoreStorage('forward')
            ? this.needMoreButton()
            : !!content;
    }
    ...
    needMoreButton(): boolean {
        ...
        const dataRowIsLastCollectionItem =
            collection.getRoot().getLastChildItem() === dataRow;
        const hideForLastNode =
            collection.getMoreButtonVisibility() ===
            MoreButtonVisibility.exceptLastNode;

        const needHide =
            !this.hasMoreStorage('forward') ||
            (hideForLastNode &&
                dataRowIsLastCollectionItem &&
