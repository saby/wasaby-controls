...
> extends GridFooterCell<TOwner> {
    ...
    getWrapperStyles(containerSize?: number): string {
        ...
        if (containerSize && this._$isActsAsRowTemplate && this._$owner.isFullGridSupport()) {
    ...
    // FIXME: Убрать после выполнения.
    //  https://online.sbis.ru/opendoc.html?guid=1c18ad72-594b-4d5f-8a50-9488ee140de4
    private _getContainerWidthStyles(containerSize: number, expanderSize: string): string {
        const getStyle = (iconSize, paddingSize) => {
            ...
            return `width: calc(${containerSize}px - var(${iconVar}) - var(${paddingVar}));`;
        };
        switch (expanderSize) {
            case 's':
