    ...
    implements IDisplaySearchValue, IGroupNode
{
        ...
        if (this._$columnItems) {
            this._$columnItems.forEach((cell, cellIndex) => {
                if (cell.DisplaySearchValue) {
                    (cell as unknown as GridDataRow).setSearchValue(
                        searchValue
                    );
                }
            });
    ...
    getRowComponentProps(handlers?: IItemEventHandlers, actionHandlers?: IItemActionsHandler): ITreeRowComponentProps {
        ...
        return {
            ...
            expanderIconSize: this.getExpanderIconSize(
                rowProps.expanderIconSize
            ),
            expanderIconStyle: this.getExpanderIconStyle(
                rowProps.expanderIconStyle
            ),
            ...
            withoutLevelPadding: this.getWithoutLevelPadding(
                rowProps.withoutLevelPadding
            ),
        };
    }
    ...
    getWithoutExpanderPadding(withoutExpanderPadding: boolean): boolean {
        const without = super.getWithoutExpanderPadding(withoutExpanderPadding);
        if (without) {
            return true;
        }
        ...
        if (group.GroupNodeItem && parentIsInRoot) {
            ...
            const hasChildNodeWithChildren = childNodes.some((it) => {
                return it.getHasChildrenProperty()
                    ? it.hasChildren()
                    : it.hasChildrenByRecordSet();
            });
            const allowByChilds =
