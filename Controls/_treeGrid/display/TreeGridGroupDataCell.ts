...
import TreeGridDataCell, {
    ITreeGridDataCellOptions,
} from 'Controls/_treeGrid/display/TreeGridDataCell';
...
export interface ITreeGridGroupDataCell
    extends ITreeGridDataCellOptions<Model> {
    isExpanded: boolean;
}
 ...
 * @private
 */
export default class TreeGridGroupDataCell<
    T extends Model = Model
> extends mixin<TreeGridDataCell<T>, GroupMixin>(TreeGridDataCell, GroupMixin) {
     ...
     * @param templateSeparatorVisibility Видимость разделителя группы
     */
    getContentTextWrapperClasses(
        ...
        templateSeparatorVisibility?: boolean
    ): string {
        const config = this.getColumnConfig() as IGroupNodeColumn;
        const fontColorStyle =
            config.groupNodeConfig?.fontSize ||
            config.fontColorStyle ||
        ...
        const fontSize =
            ...
            templateFontSize;
        const fontWeight =
            config.groupNodeConfig?.fontWeight ||
            config.fontWeight ||
            templateFontWeight;
        const textTransform =
            config.groupNodeConfig?.textTransform || templateTextTransform;
        const separatorVisibility =
            config.groupNodeConfig?.separatorVisibility ||
            templateSeparatorVisibility;

        return super.getContentTextWrapperClasses(
            fontColorStyle,
            fontSize,
            fontWeight,
            textTransform,
    ...
    isNeedSubPixelArtifactFix(
        tmplSubPixelArtifactFixOption: boolean = false
    ): boolean {
