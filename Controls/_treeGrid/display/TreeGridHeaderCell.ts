...
export default class TreeGridHeaderCell extends GridHeaderCell<null> {
    ...
    getContentClasses(): string {
        ...
        result += this._getExpanderPaddingClasses();
        return result;
    }
    ...
    private _getExpanderPaddingClasses(): string {
        let result = '';
        const isBreadcrumbsHeaderCell =
        ...
        const isFirstColumn =
            ...
            (this.getColumnIndex() === 1 && this.getOwner().hasMultiSelectColumn());
        const hasExpander =
            this.getOwner().getExpanderIcon() !== 'none' &&
            (this.getOwner().getExpanderVisibility() === 'visible' ||
                this.getOwner().hasNodeWithChildren());

        // Отступ под экспандер добавляется для первой колонки, если нет MultiSelect,
        // И для второй колонки, если есть multiSelect
        if (
            hasExpander &&
            isFirstColumn &&
            this._$displayExpanderPadding &&
            !isBreadcrumbsHeaderCell
        ) {
