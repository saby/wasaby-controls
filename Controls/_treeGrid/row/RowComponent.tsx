...
import { GridHeaderCell, GridFooterCell, GridResultsCell } from 'Controls/baseGrid';
import {
...
export function getDirtyCellComponentContentRender(
    ...
    if (DisplayTypeUtils.isTreeGroupNodeCell(cell)) {
        ...
        renderCellProps.className += ' tw-items-baseline controls-GridReact__cell-baseline';
        if (renderCellProps.minHeight !== 'null' && cellProps.baseline) {
            renderCellProps.className += ` controls-GridReact__cell-baseline_${cellProps.baseline}`;
        }
    }
