 ...
 * @returns boolean
 */
function isNumeric(value: number, controlName?: string, optionName?: string): boolean {
 ...
 * @returns boolean
 */
function isSumInRange(data: object[], maxValue: number = 100, controlName?: string): boolean {
    const logError = controlName && controlName.length;
    const sum = Math.round(
        data.map(Object).reduce((curSum, item) => {
