...
export interface IStateIndicatorOptions
     ...
     * </pre>
     */
    data?: IIndicatorCategory[] | null;
    sectorSize?: 's' | 'm' | 'l' | 'xl' | '2xl';
    horizontalPadding?: 'xs' | 'null';
    onItemEnter?: (e: React.MouseEvent<HTMLSpanElement>, item: HTMLSpanElement) => void;
    theme?: string;
}
 ...
 * @remark
 * Полезные ссылки:
 * * {@link /doc/platform/developmentapl/interface-development/controls/progress/state-indicator/ руководство разработчика}
...
export default class StateIndicator extends React.Component<IStateIndicatorOptions> {
    ...
    private _checkData(data: IIndicatorCategory[], props: Required<IStateIndicatorOptions>): void {
        Utils.isNumeric(props.scale, 'StateIndicator', 'Scale');
        Utils.isValueInRange(props.scale, 1, maxPercentValue, 'StateIndicator', 'Scale');
    ...
    private _setColors(data: IIndicatorCategory[]): string[] {
        ...
        for (let i = 0; i < data.length; i++) {
            colors[i] = (
                data[i].className ? data[i].className : defaultColors[i] ? defaultColors[i] : ''
            ) as string;
        }
        return colors;
    }
    ...
    private _calculateColorState(
        props: Required<IStateIndicatorOptions>,
        ...
        let longestValueStart: number = 0;
        let itemValue;
        let itemNumSectors;
        let excess;
    ...
    private _applyNewState(props: Required<IStateIndicatorOptions>): void {
        ...
        this._colorState = this._calculateColorState(props, data, this._colors, this._numSectors);
    }

    protected _mouseEnterIndicatorHandler(e: React.MouseEvent<HTMLSpanElement>): void {
        if (this.props.onItemEnter) {
            this.props.onItemEnter(e, e.target as HTMLSpanElement);
        }
    }

    protected constructor(props: Required<IStateIndicatorOptions>) {
        super(props);
        this._applyNewState(props);
        this._mouseEnterIndicatorHandler = this._mouseEnterIndicatorHandler.bind(this);
    }

    shouldComponentUpdate(props: Required<IStateIndicatorOptions>): boolean {
        if (props.data !== this.props.data || props.scale !== this.props.scale) {
            this._applyNewState(props);
        }
        ...
        for (let i = 0; i < this._numSectors; i++) {
            ...
            contentData.push(
                <span
                    ...
                    data-index={i + 1}
                    title={this.props.data?.[this._colorState[i] - 1]?.tooltip}
                    key={i}
 ...
 * @cfg {String} Размер одного сектора категории.
 * @variant s
 * @variant m
 * @variant l
 * @variant xl
 ...
 * @event Происходит при наведении курсора мыши на элемент категории.
