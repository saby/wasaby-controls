...
export class StickyIntersectionController {
    ...
    init(scrollContainer: HTMLElement): void {
        ...
        this._intersectionObserver = new IntersectionObserver(this._observeHandler.bind(this), {
    ...
    observe(block: IStickyBlockData): void {
        ...
        block.observers.observerTop.current.setAttribute('data-stickyblockid', block.id);
        block.observers.observerTop2.current.setAttribute('data-stickyblockid', block.id);
        block.observers.observerTop2Right.current.setAttribute('data-stickyblockid', block.id);
        block.observers.observerBottomLeft.current.setAttribute('data-stickyblockid', block.id);
        block.observers.observerBottomRight.current.setAttribute('data-stickyblockid', block.id);
        block.observers.observerLeft.current.setAttribute('data-stickyblockid', block.id);
        block.observers.observerRight.current.setAttribute('data-stickyblockid', block.id);

        this._intersectionObserver.observe(block.observers.observerTop.current);
        this._intersectionObserver.observe(block.observers.observerTop2.current);
        this._intersectionObserver.observe(block.observers.observerTop2Right.current);
        this._intersectionObserver.observe(block.observers.observerBottomLeft.current);
        this._intersectionObserver.observe(block.observers.observerBottomRight.current);
        this._intersectionObserver.observe(block.observers.observerLeft.current);
        this._intersectionObserver.observe(block.observers.observerRight.current);
    }

    unobserve(id: string): void {
        const block = this._blocks.get(id);

        if (!block.observers.observerTop.current || !this._intersectionObserver) {
            return;
        }

        this._intersectionObserver.unobserve(block.observers.observerTop.current);
        this._intersectionObserver.unobserve(block.observers.observerTop2.current);
        this._intersectionObserver.unobserve(block.observers.observerTop2Right.current);
        this._intersectionObserver.unobserve(block.observers.observerBottomLeft.current);
        this._intersectionObserver.unobserve(block.observers.observerBottomRight.current);
        this._intersectionObserver.unobserve(block.observers.observerLeft.current);
        this._intersectionObserver.unobserve(block.observers.observerRight.current);
    }
