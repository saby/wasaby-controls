...
export function getGeneralParentNode(
    block0: HTMLElement,
    block1: HTMLElement
): Node {
    const getGeneralParentNodeFunc = (container0, container1) => {
        ...
        if (parentElementOfContainer0 === document.body) {
            const group0 = container0.closest(
                '.controls-StickyHeader__isolatedGroup'
            );
            const group1 = container1.closest(
                '.controls-StickyHeader__isolatedGroup'
            );
...
export function getStickyBlockHeight(
    stickyBlockContainer: HTMLElement
): number {
    let height;
    if (!isHidden(stickyBlockContainer)) {
        const isGroup =
            getComputedStyle(stickyBlockContainer).display === 'contents';
        if (isGroup) {
            ...
            if (isMultilineGroup) {
                ...
                const lastStickyNode =
                    stickyBlockContainer.children[
                        stickyBlockContainer.childElementCount - 1
                    ];
                height =
...
export function resetSticky(blocs: object): void {
    for (const id in blocs) {
        if (blocs.hasOwnProperty(id)) {
            if (
                getComputedStyle(blocs[id].stickyRef.current).display ===
                'contents'
            ) {
...
function _resetPosition(id: string, blocs: object): void {
    ...
    for (const position of [
        ...
        let element;
        if (
            getComputedStyle(blocs[id].stickyRef.current).display === 'contents'
        ) {
