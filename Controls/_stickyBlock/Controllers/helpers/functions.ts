...
const _blocksDisplay: Record<string, string> = {};

function _getBlockDisplay(blocs: object, id: string): string {
    if (!_blocksDisplay[id]) {
        _blocksDisplay[id] = getComputedStyle(blocs[id].stickyRef.current).display;
    }
    return _blocksDisplay[id];
}

export function removeSticky(id: string): void {
    delete _blocksDisplay[id];
}

export function resetSticky(blocs: object): void {
    for (const id in blocs) {
        if (blocs.hasOwnProperty(id)) {
            if (_getBlockDisplay(blocs, id) === 'contents') {
...
export function getOffsetObjectByContainer(
    targetElement: HTMLElement,
    position: StickyPosition,
    parentContainer: HTMLElement,
    stickyFix?: boolean
    ...
    const offset = getDimensions(targetElement);
    const parentOffset = getDimensions(parentContainer, null, stickyFix);
    return {
        top: offset.top - parentOffset.top,
        left: offset.left - parentOffset.left,
        right: parentOffset.right - offset.right,
        bottom: parentOffset.bottom - offset.bottom,
    };
}
