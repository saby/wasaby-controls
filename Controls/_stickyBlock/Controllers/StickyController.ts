...
import { getDevicePixelRatio } from './helpers/functions';
...
export default class StickyController {
    ...
    scrollStateChanged(scrollState: ScrollModel): void {
        const verticalPositionChanged =
            this._scrollState?.verticalPosition !==
            scrollState.verticalPosition;
        const horizontalPositionChanged =
            this._scrollState?.horizontalPosition !==
            scrollState.horizontalPosition;
        const canVerticalScrollChanged =
            this._scrollState?.canVerticalScroll !==
            scrollState.canVerticalScroll;
        const canHorizontalScrollChanged =
            this._scrollState?.canHorizontalScroll !==
            scrollState.canHorizontalScroll;
        this._scrollState = scrollState.clone();

        if (
            this._initialized &&
            (canVerticalScrollChanged || canHorizontalScrollChanged)
        ) {
    ...
    setShadowVisibility(
        ...
        if (
            this._shadowVisibilityFromScrollContainer[StickyPosition.Top] !==
                topShadowVisibility ||
    ...
    getBlocksHeight(
        ...
        const headersSize = this.getBlocksSizes(
            position,
            type,
            false,
            considerOffsetTop
    ...
    getBlocksSizes(
        ...
        for (const id of this._blocks.getStack()[position]) {
            ...
            hasOffsetTop =
                hasOffsetTop || !!(block.props as IStickyBlock).offsetTop;
            ...
            ignoreHeight =
                ignoreHeight && !(hasOffsetTop && block.fixedInitially);
            ...
            if (block.props.mode === Mode.Stackable) {
                ...
                replaceableHeight = getStickyBlockHeight(
    ...
    getFixedBlocksHeightByScrollTop(scrollTop: number): number {
        let resultHeight = 0;
        const fixedBlocksByScrollTop =
            this._getFixedBlocksByScrollTop(scrollTop);
        fixedBlocksByScrollTop.forEach((stickyBlock) => {
            if (
                stickyBlock.props.shadowVisibility !==
                StickyShadowVisibility.Hidden
            ) {
    ...
    private _dispatch(): void {
        ...
        if (
            Object.keys(this._prevModels).length >
            Object.keys(this._dataContext.models).length
        ) {
            Object.entries(this._dataContext.models).forEach(
                ([id, data]): void => {
                    if (this._prevModels[id]) {
                        if (
                            JSON.stringify(this._prevModels[id]) !==
                            JSON.stringify(data)
                        ) {
                            needUpdate = true;
                        }
            ...
            needUpdate =
                JSON.stringify(this._prevModels) !==
                JSON.stringify(this._dataContext.models);
        }

        const isStackEmpty =
            !this._blocks.getStack().top.length &&
            !this._blocks.getStack().bottom.length;
        if (isStackEmpty) {
            needUpdate = false;
            ...
            Object.entries(this._dataContext.models).forEach(
                ([id, data]): void => {
                    const isShadowChanged =
                        this._prevModels[id] &&
                        JSON.stringify(this._prevModels[id].shadow) !==
                            JSON.stringify(data.shadow);
                    if (isShadowChanged) {
                        needUpdate = true;
                    }
                }
    ...
    private _register(block: IStickyBlockRegisterData): Promise<void> {
        ...
        const isStickedOnTop =
            block.props.position.toLowerCase().indexOf(StickyPosition.Top) !==
            -1;
        const isStickedOnBottom =
            block.props.position
                .toLowerCase()
                .indexOf(StickyPosition.Bottom) !== -1;
    ...
    private _unregister(id: string): void {
        if (!this._blocks.get(id).isGroup) {
            ...
            this._sizeController.unobserve(
    ...
    private _registerDelayed(async: boolean = true): Promise<void> {
        ...
        const registerDelayedFunction = () => {
            ...
            this._delayedBlocks = this._delayedBlocks.filter(
                (block: IStickyBlockData) => {
                    if (!block.isGroup) {
                        this._resizeObserve(block.stickyRef.current, block.id);
                    }

                    const addedToStack = this._blocks.addToStack(
                        block.id,
            ...
            this._blocks.getStack()[position].forEach((id) => {
                ...
                if (block) {
                    ...
                    fixedPositions.forEach((fixedPosition) => {
                        if (
                            block.props.shadowVisibility !==
                            StickyShadowVisibility.Hidden
                        ) {
                            const lastBlockId =
                                this._getLastFixedBlockWithShadowId(
                                    fixedPosition
                                );
                            const shadowPosition =
                                getOppositePosition(fixedPosition);
                            let shadowVisible;

                            if (
                                this._shadowVisibilityFromScrollContainer[
                                    fixedPosition
                                ]
                            ) {
                                if (
    ...
    private _updateOffsets(): void {
        ...
        for (const position of [StickyPosition.Top, StickyPosition.Bottom]) {
            this._blocks
                .getStack()
                [position].reduce((offset: number, id: string, i: number) => {
                    block = this._blocks.get(id);
                    ...
                    prevBlock = this._blocks.get(
                        this._blocks.getStack()[position][i - 1]
                    );
                    if (
                        ...
                        if (parentNode === document.body) {
                            const prevStackableBlock =
                                this._getPrevStackableBlock(i, position);
                            let prevIsolatedGroup;
                            if (prevStackableBlock) {
                                prevIsolatedGroup =
                                    this._getIsolatedGroup(prevStackableBlock);
                            }
                            const size = prevIsolatedGroup
                                ...
                                : this._getPrevStackableBlockHeight(
                                      i,
                                      position
                                  );
                            offset -= size;
                    ...
                    if (block.props.mode === Mode.Stackable) {
                        if (
                            !StickyController.isLastIndex(
                                this._blocks.getStack()[position],
                                i
                            )
                        ) {
                            const curHeaderId =
                            ...
                            for (let j = i; j >= 0; j--) {
                                prevBlock = this._blocks.get(
                                    this._blocks.getStack()[position][j]
                                );
                                ...
                                if (generalParentNode !== document.body) {
                                    ...
                                    j > 0 &&
                                    prevBlock.props.mode === Mode.Stackable
                                ) {
                                    const isolatedGroup0 =
                                        this._getIsolatedGroup(curBlock);
                                    const isolatedGroup1 =
                                        this._getIsolatedGroup(prevBlock);
                                    ...
                                    if (isolatedGroup0 && isolatedGroup1) {
                                        const headersHeightIsolatedGroup =
                                            this._getIsolatedGroupBlocksHeight(
                                                isolatedGroup1,
                                                position
                                            );
                                        offset -= headersHeightIsolatedGroup;
                                        offset += this._getBlockSize(
                                            prevBlock.id
                                        ).height;
                                        return offset;
                                    } else {
        ...
        for (const position of [
            ...
            this._blocks.getStack()[position].forEach((id) => {
                ...
                const isolatedGroup = this._getIsolatedGroup(
                    this._blocks.get(id)
                );
                let observerTop2Offset;
                if (isolatedGroup) {
                    const height1 =
                        this._getPrevStackableBlockHeightWithoutIsolatedGroup(
                            isolatedGroup,
                            position
                        );
                    const height2 =
                        this._getPrevStackableBlocksHeightInIsolatedGroup(
                            isolatedGroup,
                            id,
                            position
                        );
                    ...
                    observerTop2Offset =
                        height2 === 0
                            ? height1
                            : height1 + (height2 - stickyBlockHeight) + 2;
                } else {
                    observerTop2Offset = (
                        this._blocks.get(id).props as IStickyBlock
                    ).offsetTop
                ...
                if (this._blocks.get(id).isGroup) {
                    ...
                    groupObserversTop2Right.forEach(
                        (observerTop2Right: HTMLElement) => {
    ...
    private _fixedPositionChange(
        ...
        updatedBlocks.forEach((updatedBlock) => {
            ...
            if (
                this._blocks.get(updatedBlock.id).props.mode ===
                Mode.Replaceable
            ) {
                const propPosition = this._blocks.get(updatedBlock.id).props
                    .position;
                for (const position of getDecomposedPositionFromString(
                    propPosition
                )) {
            ...
            this._blocks.get(updatedBlock.id).fixedPosition =
                updatedBlock.fixedPosition;
            ...
            this._dataContext.models[updatedBlock.id].fixedPosition =
                updatedBlock.fixedPosition;
        });
        this._stickyFixedCallback();
        this._updateShadows();
    ...
    private _groupChangeFixedPositionCallback(
        ...
        const decomposedPositions =
            getDecomposedPositionFromString(fixedPosition);
        ...
        for (const id in updatedBlocks) {
            if (updatedBlocks.hasOwnProperty(id)) {
                if (updatedBlocks[id].operation === StackOperation.Add) {
                    const isDelayedBlock = this._delayedBlocks.some(
                        (delayedBlock) => {
    ...
    private _getStickyElementsById(id: string): HTMLElement[] {
        const elements = [];
        if (this._blocks.get(id).isGroup) {
            const stickyGroupedBlockElements =
                this._blocks.get(id).stickyRef.current.children;
            for (const stickyElementKey in stickyGroupedBlockElements) {
                if (
                    stickyGroupedBlockElements.hasOwnProperty(stickyElementKey)
                ) {
    ...
    private _updateFixedInitially(
        position: StickyPosition = StickyPosition.Top
    ): void {
        ...
        let blocksHeight: number = 0;

        const updatedBlocks = [];
        for (const id of stack) {
            if (
                this._blocks.get(id).props.shadowVisibility ===
                StickyShadowVisibility.Hidden
            ) {
                continue;
            }
            const isolatedGroup = this._getIsolatedGroup(this._blocks.get(id));
            let blockOffset = getOffsetByContainer(
                this._blocks.get(id).stickyRef.current,
                position,
                contentContainer
            );
            if (blockOffset !== 0) {
                ...
                blockOffset -= Math.abs(1 - getDevicePixelRatio());
            }

            blockOffset += (this._blocks.get(id).props as IStickyBlock)
                .offsetTop;

            if (
                (this._blocks.get(id).props as IStickyBlock).initiallyFixed !==
                    false &&
                blocksHeight >= blockOffset &&
                !isolatedGroup
            ) {
    ...
    private _initScrollContainerNodeByStickyContainer(
        ...
        if (!this._scrollContainer) {
            ...
            this._updateScrollContainer(
                stickyContainer.closest('.controls-Scroll')
            );
        }
    }
    ...
    private _getPrevStackableBlockHeightWithoutIsolatedGroup(
        ...
        let height = 0;
        this._blocks.getStack()[position].forEach((id) => {
            const prevIsolatedGroup = this._getIsolatedGroup(
                this._blocks.get(id)
            );
    ...
    private _getPrevStackableBlockHeight(
        blockIndex: number,
        position: StickyPosition
    ): number {
        let stackableHeight = 0;
        for (let i = blockIndex - 1; i >= 0; i--) {
            const prevBlock = this._blocks.get(
                this._blocks.getStack()[position][i]
            );
    ...
    private _getIsolatedGroupBlocksHeight(
        ...
        const headersOfIsolatedGroup = this._getBlocksOfIsolatedGroup(
            isolatedGroup,
            position
        );
    ...
    private _getBlocksOfIsolatedGroup(
        ...
        return this._blocks.getStack()[position].filter((id) => {
            return (
                this._getIsolatedGroup(this._blocks.get(id)) === isolatedGroup
            );
        });
    }

    private _getIsolatedGroup(block: IStickyBlockData): HTMLElement {
        return block?.stickyRef.current.closest(
            '.controls-StickyHeader__isolatedGroup'
        );
    }
    ...
    private _getFixedBlocksByScrollTop(scrollTop: number): IStickyBlockData[] {
        ...
        for (const id of this._blocks.getStack().top) {
            ...
            if (offsetByContainer > scrollTop) {
                ...
                resultBlocks.push(this._blocks.get(id));
            }
        }
        return resultBlocks;
    }
