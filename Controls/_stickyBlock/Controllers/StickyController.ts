...
export default class StickyController {
    ...
    private _dispatch(): void {
        ...
        if (
            ...
            this.hasFixed(StickyPosition.Top)
        ) {
    ...
    private _registerDelayed(async: boolean = true): Promise<void> {
        ...
        // Если запланирована регистрация, и произошло синхронная регистрация стики блоков,
        // то нужно синхронно зарегистрировать стики блоки, не дожидаясь запланированной регистрации
        if (this._registerDelayedPlanned && async) {
            return;
        }
                            ...
                            if (generalParentNode !== document.body) {
                                ...
                                if (isolatedGroup0 && isolatedGroup1) {
                                    ...
                                    if (parentIsolatedGroup) {
                                        ...
                                        const parentCurIsolatedGroup =
                                            isolatedGroup0.parentElement.closest(
                                                ISOLATED_GROUP_CLASS
                                            );
                                        if (parentCurIsolatedGroup === parentIsolatedGroup) {
                                            if (offset < 0) {
                                                offset = -this._getBlockSize(prevBlock.id).height;
                                                offset += this._getIsolatedGroupBlocksHeight(
                                                    parentIsolatedGroup as HTMLElement,
                                                    position
                                                );
                                            }
    ...
    private _groupChangeFixedPositionCallback(
        ...
        if (
            fixedPosition === '' &&
            (this._blocks.get(id).props.position === 'leftRight' ||
                this._blocks.get(id).props.position === 'right' ||
                this._blocks.get(id).props.position === 'left')
        ) {
            this._dispatchCallback(this._dataContext, this._groupDataContext);
        }
    }
        ...
        for (const id in updatedBlocks) {
            if (updatedBlocks.hasOwnProperty(id)) {
                if (updatedBlocks[id].operation === StackOperation.Add) {
                    ...
                    this._blocks.removeFromStack(id);
                    // Удаляем и сразу регистрируем блок.
                    this._register(this._blocks.get(id));
                }
            }
        }
        for (const id in this._blocks.getBlocks()) {
