...
function StickyGroup(
            ...
            if (blocks.getBlocks().hasOwnProperty(id)) {
                ...
                if (blocks.getBlocks()[id].props.shadowVisibility !== 'hidden') {
                    ...
                    if (blocks.get(id).props.position) {
                        ...
                        const fixedPosition = blocks.getBlocks()[id]?.model?.fixedPosition;
                        let isLeftSticky = true;
                        let isRightSticky = true;
                        if (typeof fixedPosition !== 'undefined') {
                            isLeftSticky = !(fixedPosition
                                ? fixedPosition.toLowerCase()?.includes(StickyPosition.Left)
                                : true);
                            isRightSticky = !(fixedPosition
                                ? fixedPosition.toLowerCase()?.includes(StickyPosition.Right)
                                : true);
                        }
    ...
    const sizeChanged = (updatedBlocks: object, sizeChangedBlocks: object) => {
        ...
        if (Object.keys(updatedBlocks).length === 0) {
            ...
            const newHeight = sizeChangedBlocks[stickyKeys[0]]?.height || height.current;
    ...
    const [sizeController] = React.useState(() => {
        const fn = debounce(sizeChanged, 100);
        return new StickySizeController(blocks, fn);
    });
