...
const getVerticalOffsets = (
    ...
    for (const position of [StickyPosition.Top, StickyPosition.Bottom]) {
        if (stickyPosition.toLowerCase().indexOf(position) !== -1) {
            const offset = getOffsetByContainer(
    ...
    for (const position of [StickyPosition.Left, StickyPosition.Right]) {
        if (stickyPosition.toLowerCase().indexOf(position) !== -1) {
            ...
            if (prevBlock.props.mode !== Mode.Replaceable && indexBlock > 0) {
                offset +=
                    prevBlock.stickyRef.current.getBoundingClientRect().width;
            }
...
function StickyGroup({
    ...
    const getProps = () => {
        const firstStickyProps = blocks.get(
            Object.keys(blocks.getBlocks())[0]
        ).props;
        return {
            mode: props.mode || firstStickyProps.mode,
            position: props.position || firstStickyProps.position,
            shadowVisibility:
                shadowVisibility || firstStickyProps.shadowVisibility,
            offsetTop,
        };
    };
    ...
    const getGroupId = () => {
        if (groupId) {
            ...
            return blocks.get(Object.keys(blocks.getBlocks()).shift()).groupData
                .id;
        }
    };
    ...
    const register = (block: IStickyBlockRegisterData) => {
        ...
        const isStickedOnTop =
            props.position && props.position.indexOf(StickyPosition.Top) !== -1;
        const isStickedOnBottom =
            props.position &&
            props.position.indexOf(StickyPosition.Bottom) !== -1;
        const isStickedOnLeft =
            block.props.position &&
            block.props.position.indexOf(StickyPosition.Left) !== -1;
        const isStickedOnRight =
            block.props.position &&
            block.props.position.indexOf(StickyPosition.Right) !== -1;

        dataContext[block.id] = {
            offset: {
                top: isStickedOnTop ? 0 : undefined,
    ...
    const unregister = (id: string) => {
        ...
        const isMultiHeader = blocks
            .get(id)
            .stickyRef.current.className.includes('multi-header');
    ...
    const getWidth = () => {
        ...
        if (
            !props.position &&
            blocks.getStack()[StickyPosition.Left].length > 0
        ) {
                ...
                if (props.position && props.stickyModel) {
                    ...
                    for (const position of decomposedPositions) {
                        if (position === StickyPosition.Top) {
                            dataContext[id].shadow.bottom =
                                props.stickyModel.shadow.bottom;
                        }
                        if (position === StickyPosition.Bottom) {
                            dataContext[id].shadow.top =
                                props.stickyModel.shadow.top;
                        }
                    }
                }
    ...
    const sizeChanged = (updatedBlocks: object, sizeChangedBlocks: object) => {
        ...
        if (Object.keys(updatedBlocks).length === 0) {
            const newHeight = sizeChangedBlocks[Object.keys(blocks.getBlocks())[0]]?.height || height.current;
        ...
        const hasHorizontalBlocks =
            blocks.getStack().left.length || blocks.getStack().right.length;
    ...
    const fixedPositionChange = (
        ...
        updatedBlocks.forEach((updatedBlock) => {
            dataContext[updatedBlock.id].fixedPosition =
        ...
        if (!registeredInController) {
            updateFixed(groupFixedPosition, {
                ...
                prevPosition: groupFixedPosition
                    ? FixedPosition.None
                    : FixedPosition.Left,
            });
            updateShadows();
    ...
    const [intersectionObserver] = React.useState(() => {
        return new StickyIntersectionController(
            blocks,
            props.scrollState,
            fixedPositionChange
        );
    ...
    const propsAreEqualUpdate = (prevProps, curProps) => {
        ...
        const prevPropsStickyModel =
            prevProps.stickyModel || EMPTY_STICKY_MODEL;
    ...
    const prevProps = usePreviousProps(
        { blocksId: Object.keys(dataContext), ...props },
        undefined
    );
    if (Object.keys(dataContext).length) {
        ...
        if (props.position) {
            const fixedPosition =
                props.stickyModel?.fixedPosition ||
                EMPTY_STICKY_MODEL.fixedPosition;
    ...
    return (
        <Ctx.StickyContext.Provider value={stickyContextProviderValue}>
            {props.children ||
                createElement(
                    props.content,
                    { $wasabyRef, ...clearProps },
                    { class: userAttrs?.className },
                    undefined,
                    context
                )}
...
const propsAreEqual = (prevProps, curProps) => {
    ...
    if (
        prevProps.scrollState?.horizontalPosition !==
        curProps.scrollState?.horizontalPosition
    ) {
        return false;
    }
    if (
    ...
    return (
        JSON.stringify(prevProps.stickyModel) ===
        JSON.stringify(curProps.stickyModel)
    );
};

export default React.memo(StickyGroup, propsAreEqual);
