...
const getPositionOffset = (position, stickyModel, props, stickyRef): number => {
    ...
    if (isFirstRender && isStickedOnPosition) {
        return position === StickyPosition.Top
            ? props.offsetTop
...
function StickyBlockReactView({ stickyModel, setStickyId, shadowMode, ...props }, ref) {
    ...
    const getClasses = () => {
        const classResult = [];
        if (stickyModel?.fixedPosition && props.fixedClassName && stickyRef.current) {
            classResult.push(props.fixedClassName);
        }
        ...
        if (props.pixelRatioBugFix && stickyRef.current) {
    ...
    const setRef = (node) => {
        ...
        // Отложено читаем стили, чтобы избежать forcedReflow
...
export default React.memo(React.forwardRef(StickyBlockReactView), propsAreEqual);
