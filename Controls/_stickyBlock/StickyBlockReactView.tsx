...
const getBackgroundClass = (
    ...
    const isBackgroundDefaultSticky = isBackgroundDefault(
        backgroundStyle,
        contextModel
    );
...
const getPositionOffset = (position, stickyModel, props, stickyRef): number => {
    ...
    if (isFirstRender && isStickedOnPosition) {
        return position === StickyPosition.Top ? props.offsetTop : 0;
    }
...
function StickyBlockReactView({ stickyModel, setStickyId, ...props }) {
    ...
    const shadowStyle =
        props.horizontalScrollMode === 'custom' ? 'custom' : 'default';
        ...
        return () => {
            if (isStickyEnabled(props.mode, stickyRef)) {
                ...
                props.onFixedCallback({
                    ...
                    prevPosition:
                        stickyModel?.syntheticFixedPosition.prevPosition ||
                        FixedPosition.None,
    ...
    React.useLayoutEffect(() => {
        ...
        if (
            prevProps.mode &&
            props.mode !== prevProps.mode &&
            prevProps.mode !== undefined
        ) {
        ...
        if (
            !nowInitialization &&
            ((props.offsetTop !== prevProps.offsetTop &&
                prevProps.offsetTop !== undefined) ||
                (props.offsetLeft !== prevProps.offsetLeft &&
                    prevProps.offsetLeft !== undefined))
        ) {
            const offsetValue =
                props.position === StickyPosition.Left
                    ? props.offsetLeft
                    : props.offsetTop;
        ...
        if (
            ...
            props.onFixedCallback({
                id: getId(),
                fixedPosition:
                    stickyModel?.syntheticFixedPosition.fixedPosition ||
                    FixedPosition.None,
                prevPosition:
                    stickyModel?.syntheticFixedPosition.prevPosition ||
                    FixedPosition.None,
        ...
        props.offsetTop,
        props.offsetLeft,
        stickyModel?.syntheticFixedPosition,
    ]);
    ...
    const styles = React.useMemo(() => {
        const isIosOptimizedMode =
            detection.isMobileIOS && props.task1181007458 !== true;
        const top = getPositionOffset(
            StickyPosition.Top,
            stickyModel,
            props,
            stickyRef
        );
        const bottom = getPositionOffset(
            StickyPosition.Bottom,
            stickyModel,
            props,
            stickyRef
        );
        const left = getPositionOffset(
            StickyPosition.Left,
            stickyModel,
            props,
            stickyRef
        );
        const right = getPositionOffset(
    ...
    const getClasses = () => {
        const classResult = [];
        const backgroundStyle = isBackgroundDefault(
        ...
        if (props.subPixelArtifactFix) {
            classResult.push(getSubPixelArtifactBugFixClass(backgroundStyle));
        }
        classResult.push(
            getBackgroundClass(
                props.backgroundStyle,
        ...
        classResult.push(clearProps.className);
    ...
    function updateBorderWidth(): void {
        if (prevProps.className === null || prevProps.className !== className) {
            ...
            if (
                JSON.stringify(borderWidth) !== JSON.stringify(newBorderWidth)
            ) {
...
const propsAreEqual = (prevProps, curProps) => {
    ...
    const emptyStickyModel =
        curProps.position === 'left'
            ? EMPTY_STICKY_GROUPED_MODEL
            : EMPTY_STICKY_MODEL;
    ...
    const prevPropsStickyModel = prevProps.stickyModel || {
        ...emptyStickyModel,
        offset: {
            top: getPositionOffset(
                StickyPosition.Top,
                prevProps.stickyModel,
                curProps,
                { current: undefined }
            ),
            bottom: getPositionOffset(
                StickyPosition.Bottom,
                prevProps.stickyModel,
                curProps,
                { current: undefined }
            ),
            left: getPositionOffset(
                StickyPosition.Left,
                prevProps.stickyModel,
                curProps,
                { current: undefined }
            ),
            right: getPositionOffset(
                StickyPosition.Right,
                prevProps.stickyModel,
                curProps,
                { current: undefined }
    ...
    return (
        JSON.stringify(prevPropsStickyModel) ===
        JSON.stringify(curPropsStickyModel)
    );
};
