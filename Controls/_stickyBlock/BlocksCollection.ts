...
export default class BlocksCollection {
    ...
    add(block: IStickyBlockRegisterData): void {
        ...
        const position = {
            vertical: block.groupData
                ? block.groupData?.position
    ...
    addToStack(id: string, scrollContainer: HTMLElement): boolean {
        ...
        for (const position of positions) {
            ...
            const headerContainerSizes =
                this._blocks[id].stickyRef.current.getBoundingClientRect();
