...
type TMode = 'replaceable' | 'stackable' | 'dynamic';
type TPosition = 'top' | 'bottom' | 'topBottom' | 'left' | 'right';

export interface IStickyBlockProps extends IControlOptions {
    /**
     * Опция позволяет включить/отключить фикс, решающий проблему с разрывами между заголовками.
     */
    subPixelArtifactFix?: boolean;
    /**
     * Опция, которая решает проблему возникновения разрыва над прилипающем заголовком на масштабах и safari.
     */
    ...
    mode?: TMode;
    /**
     * Устанавливает видимость тени.
     */
    shadowVisibility?: 'visible' | 'hidden';
    /**
     * Определяет позицию прилипания.
     * @example
    ...
    position?: TPosition;
    /**
     * Класс, добавляемый к стики блоку, когда он зафиксирован
     */
    fixedClassName?: string;
    /**
     * Определяет постфикс стиля для настройки фона контрола, когда он зафиксирован
     */
    fixedBackgroundStyle?: string;
    /**
     * Определяет постфикс стиля для настройки фона контрола
     */
    backgroundStyle?: string;
    /**
     * Определяет значение z-index на заголовке, когда он зафиксирован
     */
    fixedZIndex?: number;
    /**
     * Определяет значение z-index на заголовке, когда он не зафиксирован
     */
    zIndex?: number;
    /**
     * Определяет смещение позиции прилипания вниз относительно позиции прилипания по умолчанию
     */
    offsetTop?: number;
    /**
    ...
    shadowMode?: string;
    /**
     * Происходит при изменении состояния фиксации.
     ...
     */
    onFixed?: (information: IFixedEventData) => void;
}
...
export default class StickyBlockWasaby extends Control<IStickyBlockProps> {
 ...
 *     </Controls.stickyBlock:StickyBlock/>
 * </pre>
 * @remark Важно! Если вы используете горизонтальное прилипания, то необходимо обернуть StickyBlock в StickyGroup для корректной работы.
 * @default 'top'
 * @demo Controls-demo/Scroll/StickyBlock/Position/Index
 */

/**
 * @name Controls/_stickyBlock/StickyBlock#fixedClassName
 * @cfg {String} Класс, добавляемый к стики блоку, когда он зафиксирован
 */
