...
class BaseCut extends Control<ICutOptions> implements IBackgroundStyle, IExpandable {
    ...
    protected _children: {
        cutButton: HTMLElement;
        content: HTMLElement;
    };
    protected _shouldActivateButton: boolean = false;
    ...
    protected _afterMount(options: ICutOptions): void {
        ...
        if (this._hasResizeObserver()) {
            ...
            this._resizeObserver.observe(this._children.content as HTMLElement, {
    ...
    protected _beforeUpdate(options?: ICutOptions): void {
        if (
            ...
            (this._options.expanded !== options.expanded || this._expanded !== options.expanded)
        ) {
            this._expanded = options.expanded;
            this._shouldActivateButton = true;
        }
    }
    ...
    protected _afterUpdate(): void {
        ...
        // Кат обновляется только во время смены состояния, поэтому будем стрелять ресайзом без услвоия
    ...
    protected _afterRender(oldOptions?: ICutOptions) {
        if (this._shouldActivateButton) {
            this._children.cutButton?.activate();
            this._shouldActivateButton = false;
        }
    }
    ...
    protected _resizeObserverCallback(entries: [ResizeObserverEntry]): void {
        if (this._isChanged(entries) && this._expanded) {
