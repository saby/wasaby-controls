...
class TextCut extends BaseCut {
    ...
    protected _getStyle(): { [key: string]: string } {
        const lines = (this._lines || 0) + 1;
        const lineHeightCssVar = `var(--line-height_${this._options.lineHeight})`;
        const contentMaxHeight = `calc(${lines} * ${lineHeightCssVar})`;
        const buttonMarginTop = detection.isPhone
            ? 'calc(var(--offset_s) + var(--height_adaptive_BigSeparator))'
            : `calc(${lines} * ${lineHeightCssVar})`;

        return {
            '--calculated-content-max-height_inputArea': contentMaxHeight,
            '--calculated-btn-margin-top_inputArea': buttonMarginTop,
        };
    }
