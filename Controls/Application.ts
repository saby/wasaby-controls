...
import {
    ControllerClass as DnDController,
    IDragObject,
...
const BODY_CLASSES = {
     ...
     */
    /* eslint-enable */
    scrollingClass: detection.isMobileIOS
...
export default class Application extends Control<IApplication> {
    ...
    protected _beforeMount(options: IApplication): void {
        ...
        if (constants.isBrowserPlatform) {
            /* eslint-disable */
            if (
                document.getElementsByClassName('head-custom-block').length > 0
            ) {
                this.head = undefined;
            }
            /* eslint-enable */
    ...
    protected _resizePage(e: SyntheticEvent<Event>): void {
        this._prevInnerHeight =
            (e.target as VisualViewport).height ||
    ...
    protected _suggestStateChangedHandler(
    ...
    private _dragStartHandler(dragObject?: IDragObject): void {
        ...
        this._shouldNotUpdateBodyClass =
            dragObject?.entity?.shouldNotUpdateBodyClass;
        if (!this._shouldNotUpdateBodyClass) {
            this._updateTouchClass();
        }
    }
    ...
    protected _unregisterHandler(
        ...
        if (this._registers[registerType]) {
            this._registers[registerType].unregister(
                event,
                registerType,
                component,
                config
        ...
        this._dragnDropController.unregisterHandler(
            event,
            registerType,
            component,
            config
    ...
    private _createPopupManager(
        cfg: ConstructorParameters<typeof PopupManager>[0]
    ): void {
    ...
    protected _openDialogHandler(
        ...
        return this._globalPopup.openDialogHandler(
            event,
            templ,
            templateOptions,
            opener
    ...
    protected _closeInfoBoxHandler(
        event: SyntheticEvent<Event>,
        withDelay?: boolean
    ): void {
    ...
    protected _popupEventHandler(event: string, action: unknown[]): void {
        ...
        this._popupManager.eventHandler.apply(this._popupManager, [
            action,
            args,
        ]);
    }
