...
import { ICrudPlus, Memory, PrefetchProxy } from 'Types/source';
import { factory, RecordSet } from 'Types/collection';
import { factory as chainFactory } from 'Types/chain';
import { descriptor, Model, Record } from 'Types/entity';
...
import { Button, IViewMode, TButtonStyle } from 'Controls/buttons';
import { getMenuPopupClassName, IGroupedOptions } from 'Controls/dropdown';
...
export interface IToolbarOptions
    ...
    /**
     * @name Controls/_toolbars/IToolbar#menuItemsExpandMode
     * @cfg {String} Режим работы кнопки "три точки". Работает только при установленной опции {@link direction}: "horizontal".
     * @variant menu
     * @variant toolbar
     * @default menu
     * @demo Controls-demo/Toolbar/MenuItemsExpandMode/Index
     */
    menuItemsExpandMode?: 'menu' | 'toolbar';
    /**
    ...
    /**
     * @name Controls/_toolbars/IToolbar#hintTextProperty
     * @cfg {String}
     * Имя свойства, содержащего текст всплывающей подскаски элемента тулбара.
     * @default undefined
     * @demo Controls-demo/Toolbar/HintTextProperty/Index
     */
    hintTextProperty?: string;
    /**
     * @name Controls/_toolbars/IToolbar#hintBackgroundStyleProperty
     * @cfg {String}
     * Имя свойства, содержащего цвет фона всплывающей подскаски элемента тулбара.
     * @default undefined
     * @demo Controls-demo/Toolbar/HintTextProperty/Index
     */
    hintBackgroundStyleProperty?: string;
    /**
     * @name Controls/_toolbars/IToolbar#hintBorderStyleProperty
     * @cfg {String}
     * Имя свойства, содержащего цвет маркера всплывающей подскаски элемента тулбара.
     * @default undefined
     * @demo Controls-demo/Toolbar/HintTextProperty/Index
     */
    ...
    onItemclick?: (event: Event, item: TItem, nativeEvent: SyntheticEvent) => boolean | void;
    onItemClick?: (event: Event, item: TItem, nativeEvent: SyntheticEvent) => boolean | void;
    ...
    className?: string;
    viewMode?: string;
    menuIconStyle?: TIconStyle;
    menuButtonStyle?: TButtonStyle;
    buttonStyle?: string;
    isAdaptive?: boolean;
    /**
     * @name Controls/_toolbars/IToolbar#afterMenuTemplate
     * @cfg {String|TemplateFunction} Контент располагающийся справа от кнопки открытия меню
     */
    afterMenuTemplate?: TemplateFunction | string;
}
 ...
 * @remark
 * Полезные ссылки:
 * * {@link https://git.sbis.ru/saby/wasaby-controls/-/blob/rc-24.6100/Controls-default-theme/variables/_toolbars.less переменные тем оформления}
 *
...
export default class View extends React.Component<IToolbarOptions, IToolbarState> {
    ...
    protected _itemsVersion: number;
    private _loadMenuDependiesPromise: Promise<unknown> = null;
    private _loadMenuTemplatesPromise: Promise<unknown> = null;

    constructor(props: IToolbarOptions) {
        super(props);
        this._toolbarItemsRef = React.createRef();
        this._menuTargetRef = React.createRef();
        const items = props.items ? props.items : new RecordSet({ rawData: [] });
        this.state = {
            items,
            countShowItems: props.items
                ? this._getCountShowItems(props.items, props.isAdaptive, props.direction)
                : 0,
            firstItem: props.items
                ? this._getFirstToolbarItem(props.isAdaptive, props.direction, props.items)
                : null,
            needShowMenu: props.items
                ? needShowMenu(props.items, props.parentProperty) ||
                  (props.isAdaptive && props.direction === 'vertical' && props.items.getCount() > 1)
                : null,
            menuItemsExpanded: false,
        };
        this._itemsVersion = items?.getVersion?.();
    ...
    componentDidUpdate(prevProps: IToolbarOptions, prevState: IToolbarState): void {
        const currentVersion = this.props.items?.getVersion?.();
        const itemsChanged =
            prevProps.items !== this.props.items ||
            (this.props.items && this._itemsVersion !== currentVersion);
        const filterChanged = prevProps.filter !== this.props.filter;
        const menuSourceChanged = prevProps.menuSource !== this.props.menuSource;
        if (this._needChangeState(this.props)) {
            this._setState(this.props);
        }
        if (hasSourceChanged(this.props.source, prevProps.source) || filterChanged) {
            this._isLoadMenuItems = false;
            this._loadMenuTemplatesPromise = null;
            this._sticky?.close();
        ...
        if (itemsChanged) {
            this._isLoadMenuItems = false;
            this._sourceByItems = null;
            this._itemsVersion = currentVersion;
            this._loadMenuTemplatesPromise = null;
        ...
        if (itemsChanged || menuSourceChanged) {
            if (this._sticky?.isOpened() && !this.props.closeMenuOnOutsideClick) {
                this.openMenu();
            }
        }
    }
    ...
    protected _needChangeState(newOptions: IToolbarOptions): boolean {
        ...
        return ['keyProperty', 'parentProperty', 'nodeProperty', 'popupClassName'].some(
            (optionName: string) => {
    ...
    protected _createPrefetchProxy(source: ICrudPlus, items: TItems): ICrudPlus {
    ...
    protected _getFirstToolbarItem(isAdaptive: boolean, direction: string, items: TItems): TItem {
        const count = items.getCount();
        if (count) {
            if (count === 1) {
                // Если элемент один, покажем его только в тулбаре.
                const isFirstItemShowTypeToolbar = items.at(0).get('showType') === showType.TOOLBAR;
    ...
    protected _isShowToolbar(
        ...
        const showOnlyMenu = isAdaptive && direction === 'vertical' && items.getCount() > 1;
        const itemShowType = showOnlyMenu ? showType.MENU : item.get('showType');
    ...
    protected _setStateByItems(
        ...
        if (this.state.items !== items || forceUpdate) {
            // у первой записи тулбара не требуется показывать отступ слева
            this.setState({
                firstItem: this._getFirstToolbarItem(isAdaptive, direction, items) as TItem,
                items,
                countShowItems: this._getCountShowItems(items, isAdaptive, direction),
            });
            if (source) {
                this._source = this._createPrefetchProxy(source, this.state.items);
            }
            this.setState((prevState) => {
                return {
                    needShowMenu:
                        needShowMenu(prevState.items, this.props.parentProperty) ||
                        (isAdaptive && direction === 'vertical' && prevState.items.getCount() > 1),
                };
            });
        }
    }

    protected _getCountShowItems(items: TItems, isAdaptive: boolean, direction: string): number {
        let result = 0;
        items.forEach((item) => {
            if (
                this._isShowToolbar(
    ...
    protected _loadSourceAndTemplatesForMenu(item: TItem): Promise<unknown> {
        const promises = [];
        if (item.get('menuOptions')) {
            const options = item.get('menuOptions');
            if (options.menuPopupComponent && options.breadCrumbsVisibility) {
                promises.push(loadAsync('Controls-ListEnv/breadcrumbs:Back'));
            }
        }
        if (item.get('subMenuTemplate')) {
            promises.push(loadAsync(item.get('subMenuTemplate')));
        } else if (this.props.menuSource) {
            promises.push(
                this._getMenuSource(item).then((menuSource) => {
                    return menuSource;
                })
            );
        }

        promises.push(this._getSynchronousSourceForMenu());
        return Promise.all(promises);
    }

    protected _getMenuSource(item: TItem): Promise<ICrudPlus> {
        const itemKey = item.get(this.props.keyProperty);
        const reloadOnOpen = item.get('reloadOnOpen');
        if (!this._menuItems[itemKey] || reloadOnOpen) {
    ...
    protected _setMenuSource(menuSource: ICrudPlus = this.props.menuSource): void {
        if (this.props.menuSource) {
            ...
            const source = this.props.source || this._getSynchronousSourceForMenu(menuItems);
    ...
    protected _resultHandler(action: string, data: Model, nativeEvent: SyntheticEvent): void {
        if (action === 'itemClick' || action === 'rightTemplateClick' || action === 'applyClick') {
            const item = action === 'applyClick' ? data.items : data;
            const wasabyActionEnvironment =
                'on' + action[0].toUpperCase() + action.slice(1).toLowerCase();
            const reactActionEnvironment = 'on' + action[0].toUpperCase() + action.slice(1);
            let eventResult;
            ...
            if (this.props[reactActionEnvironment]) {
                eventResult = this.props[reactActionEnvironment](
                    nativeEvent,
                    item,
                    nativeEvent._nativeEvent ?? nativeEvent,
                    data?.menuItems,
            ...
            eventResult = eventResult ?? nativeEvent?.result;
            const isCloseOnItemClick =
                (action === 'applyClick' || !item.get(this._nodeProperty)) && eventResult !== false;

            /*
             * menuOpener may not exist because toolbar can be closed by toolbar parent in item click handler
             */
            if (this._sticky?.isOpened() && (isCloseOnItemClick || eventResult === true)) {
                this._sticky.close();
            }
        }
    }

    protected _getMenuPopupClassName(item: TItem, root: number, items: RecordSet): string {
        const { direction, parentProperty, theme } = this.props;
        const isVerticalDirection = direction === 'vertical';
        const itemMenuOptions = item.get('menuOptions');

        let className = '';
        if (isVerticalDirection) {
            className += `controls-Toolbar-${direction}__popup__list`;
        } else {
            const itemButtonProps = this._getSimpleButtonTemplateOptionsByItem(item);
            className += getMenuPopupClassName({
                icon: itemButtonProps._icon,
                iconTemplate: itemButtonProps._iconTemplate,
                iconSize: itemButtonProps._iconSize,
                viewMode: itemButtonProps._viewMode,
                showHeader: item.get('showHeader') && item.get('title'),
                fontSize: itemButtonProps._fontSize,
                inlineHeight: itemButtonProps._inlineHeight,
                caption: itemButtonProps._caption,
                headerContentTemplate: itemMenuOptions?.headerContentTemplate,
                markerVisibility: itemMenuOptions?.markerVisibility,
                items,
                root,
                parentProperty,
            });
    ...
    protected _getMenuConfigByItem(
        ...
        const menuTemplateOptions = this._getMenuTemplateOptions();

        const isVerticalDirection = direction === 'vertical';

        const itemMenuOptions = item.get('menuOptions');
        const popupOptions = this._getMenuOptions();
        const config = {
            ...popupOptions,
            ...itemMenuOptions,
            template: itemMenuOptions?.menuPopupComponent || popupOptions.template,
            opener: this,
            className: this._getMenuPopupClassName(item, root, items),
            templateOptions: {
                ...
                ...itemMenuOptions,
                subMenuTemplate: item.get('subMenuTemplate'),
    ...
    protected _openMenu(config: IStickyPopupOptions, event?: SyntheticEvent): void {
        if (!this._sticky) {
    ...
    protected _isOnlyOneItem(menuItems: RecordSet): boolean {
        return menuItems.getCount() === 1 && !menuItems.at(0).get(this.props.itemTemplateProperty);
    }

    protected _itemClickHandler(event: SyntheticEvent<Event>, item: TItem): void {
        const readOnly: boolean = item.get('readOnly') || this.props.readOnly;
        const currentTarget = event.currentTarget;
        let itemClickResult: boolean;
        if (readOnly) {
        ...
        if (this.props.onItemclick) {
            itemClickResult = this.props.onItemclick(event, item, event.nativeEvent ?? event);
        }
        if (this.props.onItemClick) {
            itemClickResult = this.props.onItemClick(event, item, event.nativeEvent ?? event);
        }

        if (item.get(this._nodeProperty) && itemClickResult !== 'preventOpen') {
            this._loadSourceAndTemplatesForMenu(item).then(([source]) => {
                ...
                if (!item.get('subMenuTemplate') && menuItems && this._isOnlyOneItem(menuItems)) {
    ...
    protected _mouseDownHandler(event: Event): void {
        ...
        // Есть проблема с источником, происходит повторное ображение к источнику, и данные приходят не корректно.
        // Поэтому смотрим открыто ли меню, и пересоздаем источник
        // https://online.sbis.ru/opendoc.html?guid=084b6d1c-9607-4452-b3cc-ac5931944b5d&client=3
        if (this._sticky?.isOpened?.()) {
            this._setMenuSource();
        }
        if (!this.props.readOnly) {
            if (this._isMenuExpandedMode(this.props)) {
    ...
    protected _loadDependencies(): Promise<unknown> {
        try {
            ...
            if (!this._loadMenuDependiesPromise) {
                this._loadMenuDependiesPromise = import('Controls/menu');
            }
            if (!this._loadMenuTemplatesPromise) {
                this._loadMenuTemplatesPromise = this._preloadMenuTemplates();
            }
            return Promise.all([this._loadMenuDependiesPromise, this._loadMenuTemplatesPromise]);
        } catch (e) {
    ...
    /**
     * Предзагружает необходимые шаблоны для элементов,
     * которые находятся в выпадающем списке
     * @private
     */
    private _preloadMenuTemplates(): Promise<unknown> {
        const { itemTemplateProperty, isAdaptive, direction } = this.props;
        if (!itemTemplateProperty) {
            return Promise.resolve();
        }
        const menuItems = View._calcMenuItems(this.state.items, isAdaptive, direction);
        const modulesToPreload = chainFactory<TItem>(menuItems).reduce(
            (acc: Set<Promise<unknown>>, item: TItem) => {
                const itemTemplate = item.get(itemTemplateProperty);
                if (itemTemplate && typeof itemTemplate === 'string') {
                    acc.add(loadAsync(itemTemplate));
                }
                return acc;
            },
            new Set()
        );
        return Promise.all([...modulesToPreload]);
    }

    protected _getSimpleButtonTemplateOptionsByItem(item: TItem): IBaseButtonOptions {
    ...
    protected _createItemTemplate(
        ...
        const attrsForItemTemplate = {
            ...
            className: `controls-Toolbar__item${
                item.get('className') ? ` ${item.get('className')}` : ''
            }`,
        };
        const itemTemplateOptions = {
            ...(this.props.itemTemplateOptions || {}),
            item,
            direction: this.props.direction,
            itemsSpacing: this.props.itemsSpacing,
            buttonTemplate: getButtonTemplate(),
            buttonTemplateOptions: this._getSimpleButtonTemplateOptionsByItem(item),
            isFirstItem,
            type: 'toolbar',
            attrs: attrsForItemTemplate,
            hintTextProperty: this.props.hintTextProperty,
            hintBackgroundStyleProperty: this.props.hintBackgroundStyleProperty,
            hintBorderStyleProperty: this.props.hintBorderStyleProperty,
            onClick: (e) => {
        ...
        return <ItemTemplate key={index} {...itemTemplateOptions} />;
    }

    private _isMenuExpandedMode(props: IToolbarOptions): boolean {
        return props.direction !== 'horizontal' || props.menuItemsExpandMode !== 'toolbar';
    }

    render(): React.ReactElement {
        const attrs = this.props.attrs || {};
        const attrsClassName =
            'controls-Toolbar ' + (this.props.className || attrs.className || '');
        const isMenuMode = this._isMenuExpandedMode(this.props);
        const menuIcon = isMenuMode
            ? this.props.menuIcon
            : this.state.menuItemsExpanded
            ? 'icon-ArrowTimeUp'
            : 'icon-SwipeMenu';
        const toolbarItems = [];
        const addedToolbarItems = [];
            ...
            if (
                ...
                toolbarItems.push(
                    this._createItemTemplate(
                        item,
                        item.getKey() || index,
                        this.state.firstItem === item
                    )
                );
            } else if (this.state.menuItemsExpanded) {
                addedToolbarItems.push(item);
            }
        });
        const addedToolbarTemplates = [];
        while (addedToolbarItems.length) {
            const addItems = addedToolbarItems
                .splice(0, toolbarItems.length + 1)
                .map((item, index) =>
                    this._createItemTemplate(item, item.getKey() || index, !index)
                );

            addedToolbarTemplates.push(
                <div
                    key={addedToolbarTemplates.length}
                    className={`controls-Toolbar__item_vertical-spacing_${this.props.itemsSpacing}`}
                >
                    {addItems}
                </div>
            );
        ...
        return (
            ...
            >
                ...
                >
                            ...
                            className={`controls-Toolbar__menu controls-Toolbar__menu_${
                                ...
                                this.state.countShowItems
                                    ? this.props.contrastBackground
                                        ...
                                        : 'medium'
                                    : 'none'
                            } ${this.props.afterMenuTemplate ? 'controls-margin_right-m' : ''}`}
                            translucent={this.props.translucent ? 'dark' : 'none'}
                            viewMode={
                                this.props.translucent ? 'filled' : this.props.menuButtonViewMode
                            }
                            buttonStyle={
                                this.props.translucent ? 'pale' : this.props.menuButtonStyle
                            }
                            iconStyle={
                                this.props.translucent
                                    ? 'contrast'
                                    : this.props.viewMode && this.props.menuIconStyle
                                    ? this.props.menuIconStyle
                                    : 'secondary'
                            }
                            tooltip={isMenuMode ? rk('Открыть меню') : rk('Еще')}
                    ...
                    {this.props.afterMenuTemplate ? (
                        <div className="controls-margin_top-s">{this.props.afterMenuTemplate}</div>
                    ) : null}
                </div>
                {this.state.menuItemsExpanded && addedToolbarTemplates.length ? (
                    <div className="tw-flex tw-flex-col">{addedToolbarTemplates}</div>
                ) : null}
            </div>
        );
    }
    ...
    private static _calcMenuItems(
        ...
        const showOnlyMenu = isAdaptive && direction === 'vertical' && items.getCount() > 1;
        return getMenuItems<TItem>(items, showOnlyMenu).value(factory.recordSet, {
            adapter: items.getAdapter(),
            keyProperty: items.getKeyProperty(),
...
/**
 * @event Controls/_toolbars/View#dropDownOpen Происходит при открытии выпадающего списка.
 * @param {UI/Events:SyntheticEvent} eventObject Дескриптор события.
 */

/**
 * @event Controls/_toolbars/View#dropDownClose Происходит при закрытии выпадающего списка.
 * @param {UI/Events:SyntheticEvent} eventObject Дескриптор события.
 */
 ...
 * @description Для того чтобы задать шаблон элемента и в тулбаре и в выпадающем списке, используйте опцию {@link Controls/interface:IItemTemplateListProps itemTemplateProperty}.
 *
 * @example
 * <pre class="brush: html">
 * <!-- WML -->
