...
interface IItemTemplateOptions extends IComponentPropsWithReadonly {
...
const ItemContentTemplate = React.forwardRef(function ItemContent(
    ...
    const hasCounter =
        props.item.get('counter') &&
        typeof props.item.get('counter') === 'number' &&
    ...
    return (
        ...
        >
            ...
            />
            {hasCounter ? <Counter item={props.item} /> : null}
        </div>
    );
});

const Counter = (props: ICounterOptions) => {
    return (
        <div
            className={`controls-Toolbar_item-counter controls-text-contrast controls-background-contrast-${
                props.item.get('counterStyle') || 'info'
            }`}
        >
            {props.item.get('counter')}
        </div>
