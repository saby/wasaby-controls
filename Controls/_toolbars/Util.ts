...
import { TIconStyle, TFontColorStyle, TFontSize, TIconSize } from 'Controls/interface';
...
// todo потом разобраться и удалить все лишнее
function defaultFontColorStyle(viewMode: string): string | undefined {
...
export function getSimpleButtonTemplateOptionsByItem(
    ...
    const toolbarViewMode =
        toolbarOptions.viewMode !== 'search' ? toolbarOptions.viewMode : undefined;
    const cfg: IBaseButtonOptions = {};
    ...
    if (cfg._hasIcon && iconTemplate) {
        cfg._iconTemplate = iconTemplate;
        cfg._iconOptions = {
            icon: item.get('icon'),
            iconSize,
            caption,
