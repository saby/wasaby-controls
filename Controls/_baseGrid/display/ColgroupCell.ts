...
export default class ColgroupCell<T extends Model = Model> extends mixin<InitStateByOptionsMixin>(
    ...
    private static _isCompatibleWidthValue(value: string): boolean {
        return (
            !!value.match(REG_EXP_PERCENT_WIDTH_VALUE) || !!value.match(REG_EXP_PIXEL_WIDTH_VALUE)
        );
    }
}
