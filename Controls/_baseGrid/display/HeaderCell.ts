...
export default class HeaderCell extends Cell<null, HeaderRow> {
    ...
    getWrapperClasses(backgroundColorStyle: TBackgroundStyle): string {
        ...
        if (!this.CheckBoxCell) {
            ...
            const horizontalPadding = this.getStyle() === 'master' ? '_master' : '';
            wrapperClasses += ` js-controls-Grid__header-cell-checkbox controls-Grid__header-cell-checkbox controls-Grid__header-cell-checkbox_min-width${horizontalPadding}`;
        }
    ...
    protected _getContentSeparatorClasses(): string {
        ...
        const isMultiLineHeader = this._$owner.isMultiline();
        let classes = '';
        if (isMultiLineHeader && !this.getOwner().isReactView()) {
            if (
    ...
    getWhiteSpace(): string {
        return this._$column.whiteSpace;
    }
    ...
    protected _getHorizontalPaddingClasses(cellPadding: ICellPadding): string {
        ...
        return super._getHorizontalPaddingClasses(cellPadding);
    }
    ...
    get headerCellPadding() {
        return this._$cellPadding;
    }
}
