...
import type { ICellComponentProps, ICellProps, IRowComponentProps } from 'Controls/gridReact';
    ...
    implements IInstantiable, IVersionable
{
    ...
    protected calculationCellPadding(rowProps: IRowComponentProps): IGridPaddingProps {
        ...
        const paddingLeft = isFirstColumnAfterCheckbox
            ? 'null'
            : (cellPadding?.left && `grid_${cellPadding?.left}`) ||
              cellProps?.padding?.left ||
              (this.isFirstColumn() && rowProps.paddingLeft) ||
              'grid_default';
        const paddingRight =
            (cellPadding?.right && `grid_${cellPadding?.right}`) ||
            cellProps?.padding?.right ||
            (this.isLastColumn() && rowProps.paddingRight) ||
            'grid_default';
    ...
    getCellComponentProps(
        ...
        const hoverBackgroundStyle = getInheritableCellProp('hoverBackgroundStyle');
        ...
        if (rowProps.backgroundColorStyle) {
            backgroundStyle = rowProps.backgroundColorStyle;
        }
        ...
        let className: string = cellProps?.className ? `${cellProps?.className} ` : '';

        if (this.hasColumnScrollReact()) {
            const selectors = this.getColumnScrollSelectors();

            if (this._$isFixed) {
                className += `${selectors.FIXED_ELEMENT} ${selectors.FIXED_CELL}`;
            } else if (this._$isFixedToEnd) {
                className += `${selectors.FIXED_TO_RIGHT_EDGE_ELEMENT} ${selectors.FIXED_CELL}`;
            } else {
        ...
        return {
            ...
            fadedClass: rowProps.fadedClass || '',
            ...
            hoverMode: this._$owner.getEditingConfig()?.mode || 'row',
            ...
            halign: cellProps?.halign || columnConfig?.align,
            ...
            tagStyle: cellProps?.tagStyle || this?.item?.get?.(this.config?.tagStyleProperty), // this?.item может быть ID нгруппы
            tagPosition: cellProps?.tagPosition || 'border',

            editable: this.isEditable(),
            editing: this.isEditing(), // editing - флаг редактируемой строки, а не редактируемой ячейки
