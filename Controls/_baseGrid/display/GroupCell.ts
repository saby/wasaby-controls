...
>(DataCell, GroupMixin) {
    ...
    getCellComponentProps(
        ...
        let GroupCellRender;
        if (this._$groupTemplate) {
            GroupCellRender = this._$groupTemplate;
        } else if (rowProps.groupTemplate) {
            const { groupTemplate: GroupTemplate } = rowProps;
            if (typeof GroupTemplate === 'string') {
                const GroupTemplateComponent = loadSync(GroupTemplate);
                GroupCellRender = GroupTemplateComponent;
            } else {
                GroupCellRender = GroupTemplate;
            }
        } else {
            GroupCellRender = GroupContentComponent;
        }
        ...
        cellProps.render = React.createElement(GroupCellRender, {
            ...
            className: 'controls-ListView__groupContent_height',
        });
        ...
        // Горизонтальные паддинги нельзя сбрасывать в null, т.к. контент группы должен выравниваться аналогично
        // остальным элементам списка https://online.sbis.ru/opendoc.html?guid=7a6dc17d-4ad9-4aea-9a4b-209ff8f7f4d9
        ...
        cellProps.className =
            (cellProps.className ?? '') +
            this._getGroupContentWrapperClasses() +
            this._getHideSeparatorClass(cellProps.backgroundStyle);
        return cellProps;
    }
