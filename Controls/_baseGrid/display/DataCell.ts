...
import { IColumn, IDisplayTypeOptions } from './interface/IColumn';
    ...
    implements IMarkable, ITagCell, IItemActionsCell, ILadderContentCell, IDisplaySearchValue
{
    ...
    getCellComponentProps(
        ...
        let backgroundStyle = superProps.backgroundStyle;
        ...
        let borderMode = superProps.borderMode;
        ...
        if (actionsVisible) {
            if (
                ...
                actionsClassName =
                    (superProps.actionsClassName || 'controls-itemActionsV_position_bottomRight') +
                    ` ${selectors.FIXED_TO_RIGHT_EDGE_ELEMENT}`;
            } else {
                actionsClassName = superProps.actionsClassName;
            }
        }
    ...
    isEditable(): boolean {
        return (
            (!!this.config?.editorRender || !!this.config?.editorTemplate) &&
