...
import { default as HeaderCellModel } from '../../display/HeaderCell';
import SortingButtonComponent from '../../SortingButtonComponent';
import { getUserClassName } from '../../cell/utils/getUserArgsCompatible';
import { getTruncateClasses } from '../../SortingButtonComponent';
...
function HeaderCellComponent(
    ...
    if (props.contentTemplate) {
        content = <props.contentTemplate colData={column} column={column} />;
        ...
        content = (
            <SortingButtonComponent
                ...
                sortingIcon={column.getSortingIcon()}
                whiteSpace={column.getWhiteSpace()}
            />
        );
    } else {
        const textOverflow = column.getTextOverflow();
        const whiteSpace = column.getWhiteSpace();
        const innerContentClassName =
            'controls-Grid__header-cell__content__innerWrapper' +
            ' tw-cursor-text' +
            ` ${getTruncateClasses(textOverflow, whiteSpace)}`;
        content = (
            <div
                title={column.config?.tooltip ?? column.getCaption()}
