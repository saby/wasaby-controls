...
export default class Container extends Control<ISearchInputContainerOptions> {
    ...
    protected _keyDown(event: SyntheticEvent<KeyboardEvent>): void {
        if (
            (event.nativeEvent?.which || event.which) === constants.key.enter &&
            (!event.nativeEvent?.ctrlKey || event.which !== constants.key.ctrl)
        ) {
...
/**
 * @event Controls/_searchDeprecated/Input/Container#search Происходит при начале поиска.
 * @param {UI/Events:SyntheticEvent} eventObject Дескриптор события.
 * @param {value} string Значение по которому производится поиск.
 */

/**
 * @event Controls/_searchDeprecated/Input/Container#searchReset Происходит при сбросе поиска.
 * @param {UI/Events:SyntheticEvent} eventObject Дескриптор события.
 */
