...
function getStyles(props: IGridViewProps): React.CSSProperties {
    ...
    if (props._$fakeGridTemplateColumns) {
        style.gridTemplateColumns = props._$fakeGridTemplateColumns;
    } else if (props.cCountStart) {
...
function RowWrapperPropsAreEqual(prevProps: RowWrapperProps, nextProps: RowWrapperProps) {
    return (
        ...
        // Нужно, чтобы возникала перерисовка, когда модель не поменялась, но коллекция пересоздала items
...
const ReactGridViewRef = React.forwardRef(function ReactGridView(
    ...
    const preparedProps = React.useMemo(
        () => ({
            ...props,
            _$checkTemplateValidator: () => {} /*checkTemplateValidator*/,
        }),
...
function ReactGridView(props: IGridViewProps) {
    ...
    return (
        <ReactGridViewMemo
            ...
            //column scroll
            _$fakeGridTemplateColumns={props._$fakeGridTemplateColumns}
        />
    );
}
