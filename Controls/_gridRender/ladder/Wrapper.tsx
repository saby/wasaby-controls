...
function Wrapper(props: ILadderWrapperProps) {
    ...
    const item = collectionItemContextValue.item as GridRow;
    const { ladderProperty, className, children, content } = props;

    let stickyProperty = item?.getGridColumnsConfig()?.[0]?.stickyProperty;
    stickyProperty = typeof stickyProperty === 'string' ? [stickyProperty] : stickyProperty;

    const ladderData = useItemData<Model>([ladderProperty]);
    const { renderValues } = useItemData<Model>(stickyProperty);

    const hiddenClassName = item?.getStickyLadder()?.[ladderProperty]?.headingStyle
