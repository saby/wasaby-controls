...
import type { TOverflow } from 'Controls/gridDisplay';
import { TResultsPosition } from 'Controls/_gridRender/interface/CommonInterface';
...
} from 'Controls/_gridRender/cell/utils/Classes/Align';
...
import { getFontSizeClasses as getFontSizeClassesUtil } from 'Controls/_gridRender/cell/utils/Classes/Font';
...
import SortingButtonComponent from 'Controls/_gridRender/components/SortButton';
...
function getBaseClasses(
    ...
    let baseClasses =
        ...
        ' tw-h-full tw-box-border tw-inline-flex' +
...
function getCheckboxClasses(
    cellType: IHeaderCellComponentProps['cellType'],
    decorationStyle: IHeaderCellComponentProps['decorationStyle']
) {
    if (cellType === 'checkbox') {
        return (
            ' js-controls-Grid__header-cell-checkbox' +
            ` controls-Grid__row-cell-checkbox-${decorationStyle}`
        );
    }
...
function getCursorClasses(
    cursor?: TCursor,
    isScrollable?: IHeaderCellComponentProps['isScrollable']
): string {
    return ` tw-cursor-${getCursor(cursor, isScrollable)}`;
}
...
function getRowSeparatorElement(
    ...
    if (isMultiline && headerEndRow !== endRow && startRow && endRow && endRow - startRow === 1) {
        const paddingClasses = getHorizontalPaddingsClasses(
            props?.padding?.left,
            props?.padding?.right
        );
        return (
            <div
                className={`controls-Grid__cell_header-content_border-bottom tw-absolute tw-bottom-0 tw-left-0 tw-right-0 ${
                    paddingClasses ?? ''
                }`}
            ></div>
        );
...
function getContentRender(
    ...
    if (textOverflow === 'ellipsis' && !whiteSpace) {
        contentRenderClassName += ' tw-truncate';
    } else {
...
function Header(
    ...
    const wrapperRenderClassName =
        ...
        getCheckboxClasses(cellType, decorationStyle) +
        getColumnScrollClasses(props) +
        getPaddingClasses(padding) +
        getBackgroundColorStyleClasses(backgroundColorStyle, isSticky) +
        getMinHeightClasses(isMultiline) +
        getFontSizeClasses(fontSize) +
        getCursorClasses('default', props.isScrollable) +
    ...
    const contentRender = (
        <>
            ...
            <div
                className={
                    'controls-Grid__header-cell__content tw-box-border tw-flex tw-w-full ' +
