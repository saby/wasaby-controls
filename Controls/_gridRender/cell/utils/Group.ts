...
import type { GridCell, GridGroupCell } from 'Controls/gridDisplay';
...
/**
 * Возвращает пропсы, необходимые для рендера ячейки группы
 * @private
 */
export function getGroupCellProps({
    ...
    if (groupProps.backgroundColorStyle) {
        stickyProps.stickiedBackgroundStyle = groupProps.backgroundColorStyle;
    }
    if (groupProps.fixedBackgroundStyle) {
        stickyProps.fixedBackgroundStyle = groupProps.fixedBackgroundStyle;
    }
