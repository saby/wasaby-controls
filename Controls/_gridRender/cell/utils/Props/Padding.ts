...
import type { GridCell, ICellPadding } from 'Controls/gridDisplay';
import { TGridHPaddingSize } from 'Controls/interface';
import { IVerticalRowPadding } from 'Controls/_gridRender/row/interface/IRowComponent';
...
function getCellPaddingGivenColspan(props: IGetPaddingPropsParams): ICellPadding {
    ...
    const columnIndex = cell.getColumnIndex(true, false) - multiselectCorrection;
...
interface IGetPaddingsObjectParams {
    cell: GridCell;
    row: GridDataRow;
    paddingTop?: IVerticalRowPadding['top'];
    paddingBottom?: IVerticalRowPadding['bottom'];
    paddingTopDefault?: IVerticalRowPadding['top'];
    paddingRightDefault?: IHorizontalCellPadding['right'];
    paddingBottomDefault?: IVerticalRowPadding['bottom'];
    paddingLeftDefault?: IHorizontalCellPadding['left'];
    getCellsPropsParams?: Function | null;
}

// Тип для для объекта, содержащего все отступы
export type TPadding = IVerticalRowPadding | IHorizontalCellPadding;

// Утилита возвращает все отступы - вертикальные и горизонтальные в одном объекте,
export function getPaddingsObject(props: IGetPaddingsObjectParams): TPadding {
    // const rowPadding = calcRowPadding(rowProps, item);
    const DEFAULT_VERTICAL_PADDING = 'grid_s';
    const {
        cell,
        row = cell.getOwner(),
        paddingTopDefault = DEFAULT_VERTICAL_PADDING,
        paddingBottomDefault = DEFAULT_VERTICAL_PADDING,
    } = props;

    if (cell.CheckBoxCell) {
        return { left: 'grid_null', right: 'grid_null', top: 'null', bottom: 'null' };
    }
    const currentColumnConfig = cell.getColumnConfig();
    const rowLeftPadding = row.getLeftPadding() || props.paddingLeftDefault;
    const rowRightPadding = row.getRightPadding() || props.paddingRightDefault;
    const getCellsPropsParams =
        props?.getCellsPropsParams === undefined ? () => ({}) : props?.getCellsPropsParams;
    ...
    row.getGridColumnsConfig().forEach((gridColumn) => {
        columnsConfig.push({
            ...
            // eslint-disable-next-line @typescript-eslint/ban-ts-comment
            // @ts-ignore
            getCellProps: currentColumnConfig.getCellProps as Function,
        });
    });

    const hPadding = getHorizontalPaddingProp({
        cell,
        columnsConfig,
        ...
        // TODO: Нужно будет от такого избавиться Controls/_gridRender/cleanRender/cell/utils/Props/Padding.ts:71
        getCellsPropsParams,
    });

    const paddingTop = props.paddingTop !== 'default' ? props.paddingTop : paddingTopDefault;
    const paddingBottom =
        props.paddingBottom !== 'default' ? props.paddingBottom : paddingBottomDefault;
    return { ...hPadding, top: paddingTop, bottom: paddingBottom };
}
