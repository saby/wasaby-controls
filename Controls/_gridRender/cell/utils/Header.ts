...
import { IHeaderCellComponentProps, IHeaderCellConfig } from 'Controls/_gridRender/cell/Header';
import type { GridHeaderCell, GridHeaderRow, TCellVerticalAlign } from 'Controls/gridDisplay';
...
function calcContentOrientation(
    ...
    // Для IE и Webkit попадаем в метод get для чекбокс ячеек, а не должны. Используем этот флаг для проверки.
    const isOldBrowserCheckBox = cell.CheckBoxCell && isOldBrowser;
    ...
    if (alignFromGetCellProps.align || alignFromGetCellProps.halign) {
        ...
        const getAlignResult = !isOldBrowserCheckBox ? get('align') : null;
        if (getAlignResult) {
    ...
    if (alignFromGetCellProps.valign) {
        ...
        const getVAlignResult = !isOldBrowserCheckBox ? get('valign') : null;
        if (getVAlignResult) {
...
/**
 * Возвращает пропсы, необходимые для рендера ячейки заголовка
 * @private
 */
export function getHeaderCellProps(props: IGetHeaderCellComponentProps): IHeaderCellComponentProps {
