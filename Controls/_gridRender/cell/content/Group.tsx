...
import * as FontUtils from 'Controls/_gridRender/cell/utils/Classes/Font';
import {
    TFontColorStyle,
    TFontSize,
    TFontWeight,
...
function getFontSizeClasses(fontSize?: TFontSize): string {
    return (
        FontUtils.getFontSizeClasses(fontSize) || ' controls-ListView__groupContent-text_default'
    );
}

function getFontColorStyleClasses(fontColorStyle?: TFontColorStyle): string {
    return (
        FontUtils.getFontColorStyleClasses(fontColorStyle) ||
        ' controls-ListView__groupContent-text_color_default'
    );
}

function getTextTransformClasses(textTransform?: TTextTransform, fontSize?: TFontSize) {
...
export function getGroupContentTextStylingClasses(
    ...
    return (
        ...
        FontUtils.getFontWeightClasses(fontWeight) +
        getTextTransformClasses(textTransform, fontSize)
    );
}
...
function getTextWrapperClassName(
    ...
    className += getGroupContentTextStylingClasses(
        fontColorStyle,
        fontSize,
        fontWeight,
        textTransform
...
function getTextClassName(
    ...
    let className = `controls-ListView__groupContent-text ${EDITOR_SELECTOR_CAPTION}`;
...
export interface IGroupContentRenderProps extends IBaseGroupTemplate, IDecorationStyleProps {
...
function getRightTemplate(props: IGroupContentRenderProps) {
    ...
    if (hasRightTemplate) {
        let rightTemplateClasses =
            ...
            // eslint-disable-next-line @typescript-eslint/ban-ts-comment
            // @ts-ignore
            (props.padding?.right ? ` controls-padding_right-${props.padding.right}` : '');
