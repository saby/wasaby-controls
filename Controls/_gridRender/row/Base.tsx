...
import type { GridCell, GridRow } from 'Controls/grid';
...
import Checkbox from 'Controls/_gridRender/components/Checkbox';
import StickyLadderCellComponent from 'Controls/_gridRender/cell/dirty/Ladder';
import type { GridStickyLadderCell as StickyLadderCell } from 'Controls/gridDisplay';
...
import { StickyActions } from 'Controls/_gridRender/components/StickyActions';
import { getDirtyCellComponentContentRender } from './dirty/Resolvers/Cell';
import { getCompatibleCellComponent } from 'Controls/_gridRender/cL/row/resolvers/Cell';
import { getCleanCellComponent } from 'Controls/_gridRender/row/utils/Resolvers/Cell';
import { isGroupCell } from 'Controls/_gridRender/utils/Type';
...
import { getRenderBoundaryError } from 'Controls/_gridRender/utils/compatibleValidator';
import { getCheckboxClasses } from 'Controls/_gridRender/cell/utils/Classes/CheckboxClasses';
...
function getClassName({
    ...
    // Этот класс необходим только для sequential editing, см. setInputForFastEdit,
    // его надо накидывать только на редактируемые строки.
    if (editable) {
        className += ' js-controls-Grid__row_editable';
    }
...
function getCellComponent(
    ...
    if (cell.getOwner().hasItemActionsSeparatedCell() && cell.$GIAC) {
        return (
            <StickyActions
                key={cell.key}
    ...
    if (cell.$GDC) {
        cellProps = getDataCellProps({
            cell: cell as unknown as GridCell,
            row: cell.getOwner() as unknown as GridRow,
            cellComponentProps: cellProps as IDataCellComponentProps,
            decorationStyle: rowProps?.decorationStyle,
    ...
    const FunctionalCellComponent = rowProps._$FCC || DefaultCellComponent;
...
function getCellComponentProps(cell: GridCell, rowProps: IRowComponentProps): ICellComponentProps {
    ...
    if (cell.$GDC || cell.$GCC) {
        cellComponentProps = getDataCellProps({
            cellComponentProps: cellComponentProps as IDataCellComponentProps,
            cell: cell as GridCell,
            row: cell.getOwner() as GridRow,
            decorationStyle: rowProps.decorationStyle,
        }) as ICellComponentProps;
    }

    cellComponentProps.className += getCheckboxClasses(
        cellComponentProps.cellType,
        rowProps.decorationStyle
    );
    ...
    if (rowProps.decorationStyle === 'master' && cell.$GCC) {
        cellComponentProps = {
            ...cellComponentProps,
            ...getCellProps(cell, cellComponentProps),
        };
    }
...
function GridReactRowComponent(props: IRowComponentProps, ref): React.ReactElement {
    ...
    React.useEffect(() => {
        // Если шаблон с RowComponent рендерят вне списка, то стрельнем затной ошибкой
        if (props._$checkTemplateValidator === undefined) {
            getRenderBoundaryError(focusRootRef as React.MutableRefObject<HTMLDivElement>);
        }
    }, []);
    ...
    const handlers = props.handlers
        ? helpers.events.getItemEventHandlers(props.item as Model, props.handlers)
        : null;
...
export default React.forwardRef(GridReactRowComponent);
