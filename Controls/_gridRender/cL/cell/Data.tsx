...
import { ICompatibleCellComponentProps, ICCCPCProps } from './interface';
import { default as DefaultCell } from 'Controls/_gridRender/cell/Data';
...
import { EditArrowComponent, IEditArrowProps } from 'Controls/listsCommonLogic';
...
export function validateProps(props: ICompatibleCellComponentProps) {
    if (props.column === undefined) {
        executeSyncOrAsync(['Controls/listErrors'], (errs) => errs.ColumnTemplateError());
    }
}
 ...
 * @param textOverflow
 * @param showEditArrowByDefault показываем шеврон редактирования по умолчанию - рендер в CellComponent
 */
 ...
 * @param props
 */
export function getCompatibleCellProps(
    ...
    const backgroundStyle =
        props.backgroundColorStyle !== undefined
    ...
    const className =
        (props.className || '') +
        // Чистый рендер не должен знать про attrs. Здесь этого быть не должно, только в слое совместимости.
        (props?.attrs?.className !== props.className ? props?.attrs?.className || '' : '') +
        column.getRecordTypeClasses();
    ...
    const cursor = getCursor(props.cursor, columnScrollProps.isScrollable, 'pointer');
    ...
    const compatibleProps: ICompatibleCellComponentProps = prepareCommonCompatibleProps({
        ...props,
        ...
        editArrowVisible: showEditArrowByDefault,
    ...
    const getItemActionsTemplateBase = getVisualComponent('getItemActionsTemplate');
    // TODO убрать useMemo тут, это не легально с точки зрения eslint
    // eslint-disable-next-line react-hooks/rules-of-hooks
    ...
    compatibleProps.content = null;
    compatibleProps.className = cellEditorProps.className;
...
function getCompatibleCellContentRenderProps(
    ...
    delete compatibleProps.borderStyle;
    delete compatibleProps.backgroundStyle;
    delete compatibleProps.backgroundColorStyle;
...
export function getCompatibleCellContentRender(
    ...
    if (column?.$GGC && (props.textRender || compatibleContentRenderProps.textRender)) {
    ...
    if (column?.$GDC) {
        ...
        return contentRenderResolver(column, props);
    }
 ...
 * @private
 */
export const CCCPC = React.forwardRef(function CCCPC(
    props: ICCCPCProps<ICellComponentProps>,
    ref: React.ForwardedRef<HTMLElement>
) {
    const { getCCCP } = props;
    const FunctionalCellComponent = props._$FCC || DefaultCell;

    const compatibleCellComponentProps = getCCCP(props);
...
const CompatibleGridCellComponentForwardRef = React.forwardRef(function CompatibleGridCellComponent(
    ...
    return <CCCPC {...props} ref={ref} getCCCP={getCompatibleCellProps} />;
});

export const CompatibleGridCellComponent = CompatibleGridCellComponentForwardRef;
