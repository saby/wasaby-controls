...
class InfoBoxController extends StickyController {
    ...
    elementCreated(item: IStickyItem, container: HTMLDivElement): boolean {
        ...
        if (this._openedPopupId) {
            Controller.getController()?.remove(this._openedPopupId);
        }
    ...
    resizeInner(item: IStickyItem, container: HTMLDivElement): boolean {
        // Сбрасываем высоту инфобокса, чтобы синхронно посчитать новые размеры
        const oldMaxHeight = container.style.maxHeight;
        container.style.maxHeight = '';
        const newSizes = this._getPopupSizes(item, container);
        container.style.maxHeight = oldMaxHeight;
        if (item.positionConfig) {
            item.positionConfig.sizes = newSizes;
        }
    ...
    getDefaultConfig(item: IStickyItem): Promise<void> {
        ...
        const initConstantsConfig = {
            ARROW_WIDTH: 'margin-left',
            ARROW_H_OFFSET: 'margin-right',
            ARROW_V_OFFSET: 'margin-bottom',
            TARGET_OFFSET: 'margin-top',
            MAX_WIDTH: 'max-width',
        };
        return themeInitConstants(
            ...
            if (item.popupOptions.target) {
                ...
                if (!position.hasOwnProperty('height')) {
                    position.height = popupConfig.sizes.height;
                }
