...
interface IViewModel {
    ...
    removeFromParentConfig: Function;
}
...
class ViewModel {
    ...
    private _removeFromParentConfig: Function;
    ...
    constructor(props: IViewModel) {
        ...
        this._removeFromParentConfig = props.removeFromParentConfig;
    }
    ...
    private _closeChildren(item: IPopupItem): Promise<boolean> {
        ...
        for (let i = 0; i < item.childs.length; i++) {
            if (
                !this.find(item.childs[i].id) ||
                item.childs[i].popupState === item.childs[i].controller.POPUP_STATE_DESTROYING
            ) {
                this._removeFromParentConfig(item.childs[i]);
                if (!item.childs.length) {
                    return Promise.resolve(true);
                }
            } else {
