...
class Manager {
    ...
    private _trackTarget(item: IPopupItem): void {
        ...
        if (item.popupOptions.trackTarget && target && !item.trackIntervalId) {
            ...
            if (registrator) {
                ...
                item.removePending.addCallbacks(
                    ...
                    (e) => {
                        ...
                        // Если была открыта шторка, то после отмены пендинга она будет отображаться за пределами экрана
                        // Поэтому восстанавливаем предыдущую позицию шторки.
                        if (item.controller.TYPE === 'SlidingPanel') {
                            delete item.sizes.height;
                            if (item.position) {
                                item.position.height = item.previousSizes?.height || 'auto';
                            }
                        }
                        if (e.canceled !== true) {
                            Logger.error(
