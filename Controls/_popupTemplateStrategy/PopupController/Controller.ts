...
import { debounce } from 'Types/function';
import ViewModel from 'Controls/_popupTemplateStrategy/PopupController/ViewModel';
...
const HIDE_ON_SCROLL_SHOW_DELAY = 100;

class Manager {
    private _contextIsTouch: boolean = false;
    private _pageScrolled: Function;
    private _popupResizeOuter: Function;
    private _dndPopup: HTMLElement;
    private _dragTimer: number;
    private _hideOnScrollTimeout: number;
    private _logTemplateNameCache: object = {};
    // Закрытие окна могут позвать в то время, когда оно еще не открылось, но уже имеет id.
    // Будем дожидаться инициализации итема и затем закроем окно.
    private _itemsToRemove: string[] = [];
    ...
    constructor(options: IManagerOptions = {}) {
        ...
        this._viewModel = new ViewModel({
            ...
            removeFromParentConfig: this._removeFromParentConfig.bind(this),
        });
    ...
    show(options: IPopupOptions, controller: IPopupController): string {
        ...
        // https://online.sbis.ru/opendoc.html?guid=b28c4e7c-3e24-48b0-81b3-22f2595e4511&client=3
        ...
        //     }
        //     if (
        //         options.template === 'SabyPage/stackLayout:Template' ||
        ...
        if (defaultConfigResult instanceof Promise) {
            defaultConfigResult.then(() => {
                const removeIndex = this._itemsToRemove.findIndex((id) => {
                    return id === options.id;
                });
                if (removeIndex !== -1) {
                    this._removeFromParentConfig(item);
                    this._itemsToRemove.splice(removeIndex, 1);
                    return options.id;
                }
    ...
    isDestroying(id: string): boolean {
        ...
        if (!item) {
            ...
            const removedItems = Controller.getContainer()?.getRemovingItems() || [];
    ...
    private _trackTarget(item: IPopupItem): void {
        ...
        const target = item.popupOptions.target?._container || item.popupOptions.target;
        ...
        if (item.popupOptions.trackTarget && target) {
            if (!item.trackIntervalId) {
                item.targetPosition = target.getBoundingClientRect();
    ...
    private _clearTrackInterval(item: IPopupItem): void {
        if (item.trackIntervalId) {
            ...
            item.trackIntervalId = null;
        }
    }

    private _orientationChangeHandler(): void {
    ...
    private _registerPopupLink(popupConfig: IPopupItem): void {
        if (popupConfig.popupOptions.opener) {
            const parentId = findPopupParentId(popupConfig.popupOptions.opener);
            if (parentId) {
                const item = this.find(parentId);
                const hasChild = item?.childs.find((childPopupItem) => {
                    return childPopupItem.id === popupConfig.id;
                });
                if (item && !this.isDestroying(parentId) && !hasChild) {
    ...
    private _closeChildHandler(item: IPopupItem): void {
        ...
        if (item && !item.childs.length) {
            if (item.closeChildrenPromiseResolver) {
                item.closeChildrenPromiseResolver(true);
            }
        }
    }
    ...
    private _removeElement(item: IPopupItem): Promise<void> {
        ...
        return removeDeferred.addCallback(() => {
            ...
            if (item.popupOptions?.removeCallback) {
                item.popupOptions.removeCallback();
            }
        });
    }
    ...
    protected _popupCreated(id: string, controller: IPopupController): boolean {
        const item = this.find(id);
        if (item) {
            // Register new popup
            ...
            // Если item не найден, значит элемент удалился еще до фазы _popupCreated, надо сказать об этом контроллеру
            if (controller.TYPE === 'Stack') {
    ...
    protected _popupMovingSize(id: string, offset: object | number, position: string): boolean {
        ...
        if (element) {
            ...
            const popupItems = this._viewModel.getPopupItems();
            element.popupOptions?.updateCallback?.(element);
    ...
    protected _popupFullscreen(id: string, state: boolean): boolean {
        ...
        if (element) {
            ...
            Manager._notifyEvent('managerPopupFullscreen', [
                element,
                this._viewModel.getPopupItems(),
            ]);
            return true;
        }
        return false;
        ...
        if (isClickToOverlay) {
            ...
            popupItems.each((item) => {
                if (item) {
                    ...
                    if (this._needClosePopupByOutsideClick(item) && !clickInsideChildPopup) {
    ...
    protected _popupResizeInner(id: string): boolean {
        ...
        if (item) {
            ...
            if (!parentItem || parentItem.position.hidden !== true) {
                const result = item.controller.resizeInner(item, this._getItemContainer(id));
                if (result) {
                    this._resizeUpdateCallback(item);
                }
                return result;
            }
        }
    ...
    protected _pageScrolledBase(): void {
        this._updateAfterScroll();
    }

    private _resizeUpdateCallback(item): void {
        item.popupOptions.updateCallback?.(item);
    }

    protected _hideOnScroll(popupId: string): void {
        const popupItem = this.find(popupId);
        if (!popupItem.hiddenByScroll) {
            popupItem.hiddenByScroll = true;
            this._updateAfterScroll();
        }
        clearTimeout(this._hideOnScrollTimeout);
        this._hideOnScrollTimeout = setTimeout(() => {
            popupItem.hiddenByScroll = false;
            this._updateAfterScroll();
        }, HIDE_ON_SCROLL_SHOW_DELAY);
    }

    private _updateAfterScroll(): void {
    ...
    private _updatePopupPosition(callbackName: string): boolean {
        ...
        popupItems.each((item: IPopupItem) => {
            // Состояние startDestroying может быть у окна, которое закроется только после
            // подтверждения действия (Сохранить имзенения, например). В таком случае нужно
            // отреагировать на ресайз
            if (
                !this.isDestroying(item.id) ||
                item.popupState === item.controller.POPUP_STATE_START_DESTROYING
            ) {
                const needUpdate = item.controller[callbackName](
                    item,
                    this._getItemContainer(item.id)
                );
                if (needUpdate) {
    ...
    protected _popupResult(id: string): boolean {
        ...
        const popupConfig =
            this.find(id)?.popupOptions || this._getPopupContainer()?.getPopupById(id)?._options;
        this._callEvents(popupConfig, 'onResult', args);
    }

    protected _popupClose(id: string): boolean {
        const item = this.find(id);
        if (item && item.popupState !== item.controller.POPUP_STATE_START_DESTROYING) {
    ...
    private _getItemContainer(id: string): HTMLElement {
        return document.querySelector(`[popupkey="${id}"]`);
    }

    private _removeContainerItem(removedItem: IPopupItem, removedCallback: Function): void {
        const popupItems = this._viewModel.getPopupItems();
        Controller.getContainer()?.removePopupItem(popupItems, removedItem, removedCallback);
        this._redrawItems();
    }
    ...
    private _redrawItemsWithoutUpdatingZIndex(): Promise<void> {
        ...
        popupItems.each((popupItem, index) => {
            if (
                popupItem.popupState === popupItem.controller.POPUP_STATE_CREATED &&
                popupItem.popupOptions?.updateClassNameCallback
            ) {
                const className =
                    popupItems.getCount() - 1 === index
                        ? 'controls-Scroll_webkitOverflowScrollingTouch controls-Popup__lastItem'
                        : '';
                popupItem.popupOptions?.updateClassNameCallback(className);
            }
        });
        return Controller.getContainer()?.setPopupItems(popupItems);
    }
    ...
    private _preparePopupList(): List<IPopupItemInfo> {
        ...
        popupItems.each((item: IPopupItem) => {
            ...
            if (item.parentId) {
                const index = popupItems && popupItems.getIndexByValue('id', item.parentId);
                if (index > -1) {
            ...
            if (registrator) {
                ...
                item.removePending.addCallbacks(
                    ...
                    (e) => {
                        ...
                        item.controller.cancelElementDestroyed?.(item);
                        if (item.parentId) {
                            const parentItem = this._findItemById(item.parentId);
                            if (parentItem && parentItem.closeChildrenPromiseResolver) {
                                parentItem.closeChildrenPromiseResolver(false);
                            }
                        }
                        if (e.canceled !== true) {
                            Logger.error(
                                'Controls/_popup/Popup/Container: Не получилось завершить пендинги: ' +
                                    '(name: ' +
                                    e.name +
                                    ', message: ' +
                                    e.message +
    ...
    private _getCompoundArea(popupContainer: HTMLElement): Control {
        const compoundContainer = popupContainer.querySelectorAll('.controls-CompoundArea')[0];
        ...
        if (result) {
            ...
            this._redrawItems().then(() => {
                ...
                if (item.popupOptions.autofocus !== false) {
                    Controller.getContainer()?.activatePopup(id);
                }
            });
        } else {
    ...
    private _navigationHandler(
        ...
        popupItems.each((item) => {
            ...
            // нотификационные окна ( ДО, звонки ) и окна подверждения не закрываем
            if (
                !item.parentId &&
                item.controller.TYPE !== 'Notification' &&
                item.popupOptions._type !== 'confirmation' &&
    ...
    // Приватный для использования извне метод, используется только внутри кода окон
