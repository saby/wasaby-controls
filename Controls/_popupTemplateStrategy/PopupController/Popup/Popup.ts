...
class Popup extends Control<IPopupControlOptions> {
    ...
    protected _applyScrollOnBodyClasses: boolean = false;
    ...
    protected _beforeMount(options: IPopupControlOptions): void | Promise<void> {
        ...
        this._applyScrollOnBodyClasses = !!this.context.isScrollOnBody;
    }
    ...
    protected _getStylePositionSize(position: string | number): string {
    ...
    private _resizeObserverCallback(entries: object[]): void {
        ...
        const heightChanged =
            typeof currentHeight === 'number' && currentHeight !== entries[0].contentRect.height;
        const widthChanged =
            typeof currentWidth === 'number' && currentWidth !== entries[0].contentRect.width;
        if (this._needListenResizeObserver() && (heightChanged || widthChanged)) {
            this._notifyResizeInner();
        }
        this._resizeObserverData = entries[0].contentRect;
    }
    ...
    private _needListenResizeObserver(): boolean {
        ...
        const hasSizes =
            ...
            (this._options.position.height !== undefined || isNaN(this._options.position.height));
        return !hasSizes && !detection.isIE && !this._options.isAdaptive;
    }
    ...
    protected _sendResultContext(...args: any[]): void {
        this._sendResult(null, args);
    }

    protected _maximizedContext(...args: any[]): void {
...
Object.assign(Popup.prototype, {
    _moduleName: 'Controls/_popupTemplateStrategy/PopupController/Popup/Popup',
});
