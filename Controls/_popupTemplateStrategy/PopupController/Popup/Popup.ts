...
class Popup extends Control<IPopupControlOptions> {
    ...
    protected _beforeMount(options: IPopupControlOptions): void | Promise<void> {
        ...
        this._templateName = this._getTemplateName(options);
        ...
        this._hideIndicatorHandlerContext = this._hideIndicatorHandlerContext.bind(this);
        ...
        this._popupDragStartContext = this._popupDragStartContext.bind(this);
        this._popupDragEndContext = this._popupDragEndContext.bind(this);
        // Проверка на то, что страница открыта в приложении браузера. Если страница открыта вне браузера, то безопасные зоны уже учтены.
        if (!window?.navigator?.standalone && detection.isPhone) {
    ...
    private _getTemplateName(options: IPopupOptions): string {
        if (options.isCompoundTemplate) {
            return options.templateOptions.template || options.template;
    ...
    protected _afterMount(options: IPopupOptions): void {
        const containerSizes = DimensionsMeasurer.getBoundingClientRect(this._container);
        this._resizeObserverData = {
        ...
        if (this.waitForPopupCreated) {
            this.callbackCreated = () => {
                ...
                GlobalController.getController()?.eventHandler('popupCreated', [
                    this._options.id,
                    this._options.item.controller,
                ]);
            };
        } else {
    ...
    protected _afterRender(oldOptions: IPopupOptions): void {
        ...
        if (this._options.isPortal && this._options.autofocus && !oldOptions.autofocus) {
            this.activatePopup();
        }
    }

    protected _updateProps(props: any): void {
        const newProps = { ...this._options, ...props };
        delete newProps.item;
        GlobalController.getController().update(this._options.id, newProps);
    }
    ...
    protected _getStylePositionCoord(coord: string | number, name: string): string {
        ...
        // Будет округлять left и right в разные стороны, в таком случаек при смене координат не
        // произойдет разницы в 1px.
        if (name === 'right' || name === 'bottom') {
            return `${Math.ceil(coord)}px`;
        }
    ...
    protected _getTWContainerClass(): string {
        // Будем выставлять класс только тогда, когда у стека есть фиксированная ширина
        if (this._options._type === 'stack' && this._options.item.popupOptions.width) {
            // Временно комментируем, подробности:
            // https://online.sbis.ru/page/dialog/0ab40fc0-31ca-4247-bc7a-58352563937b?inviteduser=df304491-fb5f-4747-988f-25d349b37d91
            // return 'tw-@container';
        }
        return '';
    }
    ...
    protected _getHiddenClass(item: IPopupItem): string {
        ...
        if (item.popupOptions._type === 'sliding') {
            return '';
        }
        if (item.position?.hidden) {
            return 'controls-Popup__hidden';
        }
        if (item.position?.invisible) {
    ...
    protected _popupDragStartContext(offset: number, sizes): void {
        this._popupDragStart(null, offset, sizes);
    }

    protected _popupDragEndContext(): void {
        this._popupDragEnd();
    }
    ...
    protected _hideIndicatorHandlerContext(indicatorId: string): string {
        return this._notify('hideIndicator', [indicatorId], {
            bubbling: true,
        }) as string;
    }

    protected _showIndicatorHandlerContext(config: object = {}, promise?: Promise<any>): string {
        return this._showIndicatorHandler(null, config, promise);
    }
    ...
    private _needListenResizeObserver(): boolean {
        ...
        return (
            (!hasSizes || this._options.isServerSidePopup) &&
            !detection.isIE &&
            this._options.item.popupOptions._type !== 'sliding'
        );
    }
