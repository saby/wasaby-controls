...
export class StickyController extends BaseController {
    ...
    elementCreated(item: IStickyItem, container: HTMLElement): boolean {
        if (this._isTargetVisible(item)) {
            ...
            Controller.getController()?.remove(item.id);
        }
        return true;
    }
    ...
    protected _getStickyTemplateVisibilityClass(item: IStickyItem): string {
        ...
        const getIsVisible = () => {
            return (
                !item.hiddenByScroll &&
                this._isVisibleTarget(item.popupOptions.target as HTMLElement)
            );
        };

        item.isVisible = getIsVisible();
        const parentItem = Controller.getController()?.find(item.parentId) as IStickyItem;
    ...
    protected _getShouldNotUpdatePosition(item: IStickyItem): boolean {
        // Если android, то пересчитываем позицию окна, так как при открытой клавиатуре, изменение размеров рабочей области
        // может произойти в 2 этапа, из-за чего окно спозиционируется не корректно.
        // Также на android нет тех проблем, что есть на ios при открытой клавиатуре
        if (
    ...
    elementUpdated(item: IStickyItem, container: HTMLElement): boolean {
        const fixDirection = () => {
            if (
                item.popupOptions.offset?.vertical &&
                item.popupOptions.direction?.vertical !== item.positionConfig.direction?.vertical
            ) {
                item.popupOptions.offset.vertical = -item.popupOptions.offset.vertical;
            }
            if (
                item.popupOptions.offset?.horizontal &&
                item.popupOptions.direction?.horizontal !==
                    item.positionConfig.direction?.horizontal
            ) {
                item.popupOptions.offset.horizontal = -item.popupOptions.offset.horizontal;
            }
            item.popupOptions.direction = item.positionConfig.direction;
            item.popupOptions.targetPoint = item.positionConfig.targetPoint;
        };
        if (typeof item.popupOptions.fittingMode === 'string') {
            if (item.popupOptions.fittingMode === 'initialAdaptive') {
                item.popupOptions.fittingMode = 'fixed';
                fixDirection();
            }
        } else {
            if (item.popupOptions.fittingMode?.vertical === 'initialAdaptive') {
                item.popupOptions.fittingMode.vertical = 'fixed';
                fixDirection();
            }
            if (item.popupOptions.fittingMode?.horizontal === 'initialAdaptive') {
                item.popupOptions.fittingMode.horizontal = 'fixed';
                fixDirection();
            }
        }
        ...
        if (this._isTargetVisible(item)) {
            ...
            if (this._isTargetVisible(item)) {
                ...
                if (item.popupOptions.fixPosition) {
                ...
                item.positionConfig = getStickyConfig(item, item.sizes);
    ...
    protected getContentSizes(container?: HTMLElement = null): IPopupSizes {
        ...
        // Для того чтобы обезопаситься от поведения браузера при полупиксельных значениях, округляем в большую сторону
        return {
    ...
    private _isVisibleTarget(target: HTMLElement, scrollTarget: HTMLElement = null): boolean {
        ...
        if (scrollContainer) {
            ...
            if (
                ...
                (targetDimensions.right < scrollDimensions.left ||
                    targetDimensions.left > scrollDimensions.right)
            ) {
                return false;
            }
            ...
            if (JSON.stringify(item.position) !== JSON.stringify(prevItemPosition)) {
                for (const child of item.childs) {
                    if (child.controller.TYPE === this.TYPE) {
                        ...
                        setTimeout(() => {
                            // TODO Переделать
                            // Controller.getController().updatePosition(child.id);
                        }, delay);
                    }
                }
            }
    ...
    private _resetItemStyle(
        ...
        container.style[verticalPosition] = Math.floor(item.position[verticalPosition]) + 'px';
    ...
    elementAfterUpdated(item: IStickyItem, container: HTMLElement): boolean {
        ...
        const zoom = DimensionsMeasurer.getZoomValue();
        const vValue = 100 / zoom;
        ...
        this._resetItemStyle(item, container, width, maxWidth, inset);
    ...
    prepareConfig(item: IStickyItem, container: HTMLElement): void {
        if (container) {
            const externalButtonWidth =
                container?.querySelector('.controls-StickyTemplate__close_button-external')
                    ?.offsetWidth || 0;
            item.externalButtonOffset = externalButtonWidth / 2;
        }
