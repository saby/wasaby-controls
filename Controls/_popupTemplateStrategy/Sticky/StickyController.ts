 ...
 * @class Controls/_popupTemplateStrategy/Sticky/StickyController
 *
 * @private
 */
export class StickyController extends BaseController {
    ...
    elementCreated(item: IStickyItem, container: HTMLElement): boolean {
        if (this._isTargetVisible(item)) {
            ...
            item.popupOptions.className +=
                this._getStickyTemplateVisibilityClass(item);
        } else {
    ...
    elementUpdated(item: IStickyItem, container: HTMLElement): boolean {
        ...
        if (this._isTargetVisible(item)) {
            this._setStickyContent(item);
            const targetCoords = this._getTargetCoords(
                item,
                item.positionConfig.sizes
            );
            ...
            if (this._isTargetVisible(item)) {
                ...
                if (!item.popupOptions.disableAnimation) {
                    item.popupOptions.className +=
                        ' controls-StickyTemplate-animation';
                }
                ...
                if (detection.isMobileAndroid) {
                    const height =
                        item.position.height || container.clientHeight;
    ...
    private _childUpdatePos(item: IStickyItem, prevItemPosition: object): void {
        if (item.childs.length) {
            ...
            if (
                ...
                JSON.stringify(prevItemPosition)
            ) {
    ...
    private _resetItemStyle(item: IStickyItem, container: HTMLElement, width: string, maxWidth: string): void {
        ...
        container.style.maxHeight = item.position.maxHeight
            ...
            : '';
        container.style.height = item.position.height
            ? item.position.height + 'px'
            : 'auto';
        ...
        const revertVerticalPosition =
            item.position.top !== undefined ? 'bottom' : 'top';
        container.style[verticalPosition] =
            item.position[verticalPosition] + 'px';
    ...
    private _getHorizontalCoords(
        ...
        const horizontalPosition =
            item.popupOptions.direction?.horizontal || 'right';
        return (
            window.innerWidth -
            (item.position[horizontalPosition] + item.sizes.width)
        );
    }

    private _getVerticalCoords(
        item: IStickyItem,
    ...
    popupDragStart(
        ...
        if (!item.startPosition) {
            item.startPosition = {
                [horizontalProperty]: this._getHorizontalCoords(
                    item,
                    horizontalProperty
                ),
                ...
                [verticalProperty]: this._getVerticalCoords(
                    item,
                    verticalProperty
                ),
            };
    ...
    private _prepareConfig(item: IStickyItem, sizes: IPopupSizes): void {
        ...
        if (this._isTargetVisible(item)) {
            ...
            popupCfg.direction.horizontal = getDirection(
                popupCfg.direction.horizontal
            );
            //
            if (!this._getShouldNotUpdatePosition(item)) {
                item.position = StickyStrategy.getPosition(
    ...
    private _getOrientationClasses(popupOptions: IStickyPopupOptions): string {
        let className =
            'controls-Popup-corner-vertical-' +
            popupOptions.targetPoint.vertical;
        className +=
            ' controls-Popup-corner-horizontal-' +
            popupOptions.targetPoint.horizontal;
        className +=
            ' controls-Popup-align-horizontal-' +
            popupOptions.direction.horizontal;
        className +=
            ' controls-Popup-align-vertical-' + popupOptions.direction.vertical;
        return className;
    }
    ...
    private _removeOrientationClasses(item: IStickyItem): void {
        if (item.popupOptions.className) {
            item.popupOptions.className = item.popupOptions.className
                .replace(
                    /controls-Popup-corner\S*|controls-Popup-align\S*/g,
                    ''
                )
                .trim();
        }
    ...
    private _updateStickyPosition(
        ...
        if (
            JSON.stringify(item.popupOptions.stickyPosition) !==
            JSON.stringify(newStickyPosition)
        ) {
