...
export class StickyStrategy {
       ...
       */
        ...
        if (
            (resultPosition.bottom ||
                (resultPosition.top && detection.isMobileAndroid && this._isKeyboardVisible())) &&
            detection.isMobilePlatform &&
            !detection.isMobileSafari
        ) {
            resultPosition.position = 'fixed';
        }
    ...
    private _getPosition(
        ...
        if (popupCfg.fixPosition) {
            if (popupCfg.direction[direction] === coord) {
                ...
                if (typeof popupCfg.position[coord] === 'undefined' && isHorizontal) {
                    popupCfg.position[coord] =
                        visualViewport.width -
                        popupCfg.position[oldCoord] -
                        popupCfg.position.width;
                }
            }
            if (typeof popupCfg.position[coord] === 'undefined' && !isHorizontal) {
                if (coord === 'top') {
            ...
            // На android планшете, окно позиционируется относительно viewPort, из-за чего оно может отобразиться не там где ожидаем.
            // Поэтому смещаем позицию самостоятельно, чтобы окно находилось в пределах видимой области
            if (detection.isMobileAndroid && !detection.isPhone) {
                position[coord] -= targetCoords[`${coord}Scroll`] || 0;
            }
        } else {
            if (popupCfg.direction[direction] === coord) {
                coord = isHorizontal ? 'right' : 'bottom';
                ...
                if (!getStore('AdaptiveInitializer').get('isScrollOnBody') || viewportOffset) {
                    if (
                        !(
                            detection.isMobileAndroid ||
                            (detection.isMobileIOS && detection.IOSVersion <= 16)
                        )
                    ) {
            ...
            if (direction === 'horizontal') {
                ...
                if (shouldOffsetLeft && shouldOffsetRight) {
                    ...
                    const offsetRight =
                        ...
                        windowDimensions.innerWidth - offsetElementPosition.left - position.right;
                    const useRightOffset =
                        Math.abs(offsetLeft) > Math.abs(offsetRight) ||
                        offsetLeft + (position.left || 0) + sizes.width >
                            windowDimensions.innerWidth;
                    const offset = useRightOffset ? offsetRight : offsetLeft;
                    if (typeof position.left === 'number') {
                        position.left += offset;
                    } else {
                        const newPosition = position.right + offset;
                        if (newPosition > 0) {
                            position.right = newPosition;
                        } else {
                            if (useRightOffset) {
                                position.right += offsetLeft;
                            } else {
                                position.right += offsetRight;
                            }
                        }
                        const windowWidth = this._getWindowSizes().width;
                        position.right = Math.min(position.right, windowWidth - sizes.width);
                    }
                }
            }
        });
    ...
    private _checkOverflowKeyboard(
        ...
        const popupSize: number = popupCfg.sizes[isHorizontal ? 'width' : 'height'];
        const windowSize =
            windowData?.[isHorizontal ? 'width' : 'height'] ||
            this._getWindowSizes(targetElement)[isHorizontal ? 'width' : 'height'];
    ...
    private _fixPosition(
        ...
        if (position.top) {
            if (detection.isMobileIOS && this._isKeyboardVisible() && position.top < 0) {
                const visualViewport = this._getVisualViewport(targetElement);
                position.top += visualViewport?.offsetTop;
            }
    ...
    private _setMaxSizes(
        ...
        if (popupCfg.config.maxHeight) {
            ...
            if (detection.isMobileSafari || detection.isMobileAndroid) {
                ...
                if (this._isKeyboardVisible()) {
                    if (typeof position.bottom !== 'undefined') {
                        // На android инеаче происходит позиционирования для position: fixed относительно viewPort c открытой клавиатурой.
                        // Поэтому сами корректируем позицию
                        if (
                            detection.isMobileAndroid &&
                            !getStore('AdaptiveInitializer').get('isScrollOnBody') &&
                            this._isKeyboardVisible()
                            ...
                            const maxHeight = targetPosition.top;
                            if (maxHeight > 0 && position.bottom < targetPosition.bottom) {
                                position.maxHeight = maxHeight;
                                position.bottom = window.outerHeight - targetPosition.top;
                            }
                        } else {
                            if (popupCfg.fittingMode?.vertical !== 'fixed') {
                                const targetPosition = targetElement.getBoundingClientRect();
                                // Посчитаем visualViewport.pageTop, то только для bottom
                                const pageBottom =
                                    this._getBody(targetElement).height -
                                    (visualViewport.height + visualViewport.pageTop);
                                const maxHeight =
                                    targetPosition.bottom - targetPosition.height - pageBottom;
                                if (maxHeight > 0 && position.bottom < targetPosition.bottom) {
                                    position.maxHeight = maxHeight;
                                }
                            }
