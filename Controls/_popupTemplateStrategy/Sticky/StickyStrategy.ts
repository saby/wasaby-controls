...
export class StickyStrategy {
       ...
       */
        if (positionOverflow >= 1) {
            if (popupCfg.fittingMode[direction] === 'fixed') {
                ...
                popupCfg.fittingMode[direction] === 'overflow' ||
                            ...
                            if (direction === 'vertical') {
                                if (
                                    ...
                                    const diff =
                                        ...
                                        position.height;
                                    if (
                                        popupCfg.direction[direction] === 'top'
                                    ) {
        ...
        this._calculateRestrictionContainerCoords(
            popupCfg,
            resultPosition,
            targetElement
        );
    ...
    private _getPosition(
        ...
        if (popupCfg.fixPosition) {
            ...
            const targetSize: number =
                targetCoords[isHorizontal ? 'width' : 'height'];
            const popupSize: number =
            ...
            position[coord] =
                targetCoord +
                targetSize / middleCoef -
                popupSize / middleCoef +
                margins;
        } else {
            let coord: string = isHorizontal ? 'left' : 'top';
            if (popupCfg.direction[direction] === coord) {
                ...
                const viewportPage: number =
                    visualViewport[isHorizontal ? 'pageLeft' : 'pageTop'];
                const viewportSize: number =
                    visualViewport[isHorizontal ? 'width' : 'height'];
                const topSpacing: number =
                    viewportSize + viewportPage + viewportOffset;
                const bottomSpacing: number =
                    this._getBody(targetElement)[
                ...
                const margins: number = this._getMargins(
                    popupCfg,
                    direction,
                    zoom
                );
                position[coord] =
                    bottomSpacing + (topSpacing - targetCoord) - margins;
            } else {
                const targetCoord: number = this._getTargetCoords(
                    popupCfg,
                    targetCoords,
                ...
                if (detection.isMobilePlatform) {
                    position[isHorizontal ? 'left' : 'top'] +=
                        targetCoords[isHorizontal ? 'leftScroll' : 'topScroll'];
                }
            }
        }
        return position;
    ...
    private _checkOverflowNoFixPosition(
        ...
        const restrictiveContainerPosition =
            popupCfg.restrictiveContainerCoords;
        const restrictiveContainerCoord =
            restrictiveContainerPosition?.[popupDirection] || 0;
    ...
    private _checkOverflowKeyboard(
        ...
        const popupSize: number =
            popupCfg.sizes[isHorizontal ? 'width' : 'height'];
        const windowSize =
            this._getWindowSizes(targetElement)[
                isHorizontal ? 'width' : 'height'
                ];
    ...
    private _checkOverflow(
        ...
        if (
            position.hasOwnProperty(isHorizontal ? 'right' : 'bottom') &&
            !popupCfg.fixPosition
        ) {
            return this._checkOverflowNoFixPosition(popupCfg, targetCoords, position, direction, targetElement);
        }
        if (position[isHorizontal ? 'left' : 'top'] < 0) {
            return -position[isHorizontal ? 'left' : 'top'];
        }
        return this._checkOverflowKeyboard(popupCfg, targetCoords, position, direction, targetElement);
    }
    ...
    private _invertPosition(
        ...
        popupCfg.targetPoint[direction] =
            INVERTING_CONST[popupCfg.targetPoint[direction]];
        popupCfg.direction[direction] =
            INVERTING_CONST[popupCfg.direction[direction]];
    ...
    private _calculateRestrictionContainerCoords(
        ...
        const height =
            position.height > 0 ? position.height : popupCfg.sizes?.height;
    ...
    private _fixPosition(
        ...
        if (
            position.bottom &&
            (this._isMobileDevices() || body.height === body.scrollHeight)
        ) {
    ...
    private _prepareRestrictiveCoords(
        ...
        if (popupCfg.restrictiveContainerCoords) {
            ...
            if (
                popupCfg.restrictiveContainerCoords.bottom < targetCoords.bottom
            ) {
                targetCoords.bottom =
                    popupCfg.restrictiveContainerCoords.bottom;
            }
        }
    }
    ...
    private _getWindowSizes(targetElement: HTMLElement): {
        ...
        return {
            width: DimensionsMeasurer.getElementDimensions(document.body)
                .clientWidth,
            height,
        };
    }
    ...
    getPopupZoom(targetZoom: number = 1): number {
        const bodyZoom = constants.isBrowserPlatform
            ? DimensionsMeasurer.getZoomValue()
            : 1;
    ...
    private _fixBottomPositionForIos(
        position: IPopupPosition,
        targetCoords: ITargetCoords
    ): void {
        if (position.bottom) {
            ...
            if (this._isIOS12()) {
                ...
                if (win.innerHeight + win.scrollY > win.innerWidth) {
                    ...
                    const dif =
                        win.innerHeight - targetCoords.boundingClientRect.top;
