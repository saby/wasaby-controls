...
class NotificationController extends BaseController {
    ...
    private _verticalDirection: string = 'up';
    private _horizontalDirection: string = 'right';
    ...
    elementCreated(item: INotificationItem, container: HTMLDivElement): boolean {
        ...
        item.width = Math.ceil(DimensionsMeasurer.getBoundingClientRect(container).width);
    ...
    getDefaultConfig(item: INotificationItem): void | Promise<void> {
        item.popupOptions.className =
            (item.popupOptions.className || '') + ' controls-Notification__animation_visible';
    ...
    popupDragStart(item: INotificationItem, container: HTMLElement, offset: IDragOffset): void {
        ...
        if (controller.currentLocaleConfig.directionality === 'rtl') {
            ...
            leftPosition = this._startPosition.left + horizontalOffset;
        } else {
            rightPosition = this._startPosition.right + horizontalOffset;
            leftPosition = this._startPosition.left - horizontalOffset;
        }
    ...
    private _validatePosition(): void {
        ...
        let newLeftPosition = this._historyCoords.left;
        ...
        maxWidth = Math.min(maxWidth, windowDimensions.innerWidth);
        ...
        if (restrictiveCoords) {
            ...
            newBottomPosition = Math.min(
                restrictiveCoords.top - this._stack.at(0).height,
                newBottomPosition
            );
            ...
            newLeftPosition = Math.min(newLeftPosition, windowDimensions.innerWidth - maxWidth);
            newRightPosition = Math.min(newRightPosition, windowDimensions.innerWidth - maxWidth);
            newLeftPosition = Math.min(newLeftPosition, windowDimensions.innerWidth);
            newBottomPosition = Math.min(
                newBottomPosition,
            ...
            newLeftPosition = Math.max(newLeftPosition, 0);
        }
    ...
    private _calculateDirection(container: HTMLElement): void {
        ...
        if (this._stack.getCount() === 1) {
            ...
            this._verticalDirection = windowHeight / 2 > this._historyCoords.bottom ? 'up' : 'down';
            // Опеределяет, в какую сторорону по горизонтали будут строится окна
            this._horizontalDirection =
                windowWidth / 2 > this._historyCoords.right ? 'right' : 'left';
        }
    }
    ...
    private _updatePositions(): void {
        if (this._stack.getCount()) {
            ...
            this._stack.each((item: INotificationItem) => {
                ...
                if (this._horizontalDirection === 'left') {
                    item.position.right = null;
                    item.position.left = left;
                }
                bottom += this._verticalDirection === 'up' ? item.height : -item.height;
            });
        }
    }
