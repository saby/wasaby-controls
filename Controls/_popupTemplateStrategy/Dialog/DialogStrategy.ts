...
export class DialogStrategy {
    ...
    private _getPositionForFixPositionDialog(
        ...
        if (containerSizes.width && windowData.right) {
            if (horizontalPositionProperty === 'left') {
                const containerWidth = Math.min(windowData.width, containerSizes.width);
                horizontalValue = Math.min(horizontalValue, windowData.right - containerWidth);
            } else {
                if (document?.body) {
    ...
    private _getVerticalPostion(
        ...
        const viewPortHeight = window
            ? DimensionsMeasurer.getVisualViewportDimensions(window)?.height
            : windowHeight;
        if (window && windowHeight > viewPortHeight) {
