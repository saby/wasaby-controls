...
import {
    getInvertedHorizontalPositionProperty,
...
import { isKeyboardVisible } from 'Controls/_popupTemplateStrategy/Util/isKeyboardVisible';
...
export class DialogStrategy {
    ...
    getPosition(
        ...
        if (detection.isMobileSafari && isKeyboardVisible()) {
            const visualViewport = DimensionsMeasurer.getVisualViewportDimensions(
                item.popupOptions.target
            );
            position.top -= visualViewport.pageTop;
        }
    ...
    private _validateCoordinate(
        ...
        const outsideBottomBorderValue =
            position.top + height - windowData.height - (windowData.top || 0);
    ...
    private _getPositionForFixPositionDialog(
        ...
        let horizontalValue = Math.max(windowData.left || 0, horizontalPosition);
        if (containerSizes.width && windowData.right) {
            if (horizontalPositionProperty === 'left') {
                horizontalValue = Math.min(
                    horizontalValue,
                    windowData.right - containerSizes.width
                );
            } else {
                if (document?.body) {
                    const realWindowWidth = DimensionsMeasurer.getElementDimensions(document?.body)
                        ?.clientWidth;
                    horizontalValue = Math.min(
                        horizontalValue,
                        realWindowWidth - containerSizes.width
                    );
                }
            }
        }
        let verticalValue = Math.max(windowData.top || 0, verticalPosition);
        if (containerSizes.height && windowData.bottom) {
            verticalValue = Math.min(verticalValue, windowData.bottom - containerSizes.height);
        }
        // check overflowX
        let containerWidth;
        if (!containerSizes || containerSizes.width === 0) {
            containerWidth = width;
        } else {
            containerWidth = Math.min(containerSizes.width, width || containerSizes.width);
        }
        containerWidth = containerWidth || 0;
        if (limitedSizes.maxWidth) {
            containerWidth = Math.min(containerWidth, limitedSizes.maxWidth);
        }

        // check overflowY
        let containerHeight;
        if (!containerSizes || containerSizes.height === 0) {
            containerHeight = height;
        } else {
            containerHeight = Math.min(containerSizes.height, height || containerSizes.height);
        }
        containerHeight = containerHeight || 0;
        const diff = verticalPosition + containerHeight - (windowData.height + windowData.top);
    ...
    private _calculateLimitOfSizes(item: IDialogItem, windowData: IPopupPosition): ILimitingSizes {
        ...
        if (item.fixPosition && !item.startPosition) {
            maxHeight = Math.min(maxHeight, windowData.height - item.position?.top);
        } else {
        ...
        let maxWidth = popupOptions.maxWidth || windowData.width;

        if (item.fixPosition && !item.popupOptions.width && !item.startPosition) {
            maxWidth = Math.min(maxWidth, windowData.width - item.position?.left + windowData.left);
            maxWidth = Math.max(maxWidth, popupOptions.minWidth || 0);
        }

        const sizes = {
            minWidth: popupOptions.minWidth,
            maxHeight,
            maxWidth: Math.min(maxWidth, windowData.width),
        };
    ...
    private _getMinHeight(item: IDialogItem, windowData: IPopupPosition): number {
        ...
        if (item.resizeMinHeight && !item.popupOptions.hasOwnProperty('top')) {
    ...
    private _getHorizontalPosition(
        ...
        if (optionsPosition) {
            ...
            const invertedPositionProperty = getInvertedHorizontalPositionProperty(
                horizontalPositionProperty
            );
            // Не будем считать позицию сами, если ее уже передали через опции
            if (popupOptions[invertedPositionProperty]) {
                return;
            }
        }
        if (
            typeof width !== 'number' ||
            isNaN(width) ||
            // При первом построении, строим окно за пределами экрана.
            // Не будет высчитывать позицию, если не передали ширину
            (!width && popupState === 'initializing')
        ) {
            return;
        }
    ...
    private _getVerticalPostion(
        ...
        const middleCoef = 2;
        const scrollTop: number = windowData.topScroll || 0;
        let windowHeight = windowData.height;
        if (
            window &&
            windowHeight > DimensionsMeasurer.getVisualViewportDimensions(window)?.height
        ) {
            windowHeight = window.visualViewport.height;
        }
        return Math.round((windowHeight - height) / middleCoef) + scrollTop + (windowData.top || 0);
    }
}
