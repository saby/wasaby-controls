...
export class DialogStrategy {
    ...
    private _validateCoordinate(
        ...
        const outsideBottomBorderValue =
            position.top + height - windowData.height;
    ...
    private _resetMargins(item: IDialogItem, position: IDialogPosition): void {
        const topOffset =
            (item.sizes?.margins?.top || 0) +
            (item.popupOptions?.offset?.vertical || 0);
        const horizontalOffset =
            (item.sizes?.margins?.left || 0) +
    ...
    private _getPositionCoordinates(
        ...
        if (popupItem.fixPosition) {
            ...
            const properties = getPositionProperties(
                popupOptions.resizeDirection
            );
    ...
    private _updateCoordsByOptions(
        ...
        const isRightCoordinate =
            typeof popupItem.popupOptions.right !== 'undefined';
        ...
        if (
            (topCoordinate === undefined &&
                horizontalCoordinate === undefined) ||
            isMaximizePopup
        ) {
            return;
        }
        ...
        if (horizontalCoordinate !== undefined) {
            const popupWidth =
                popupItem.popupOptions.width || popupItem.sizes?.width;
            ...
            if (differenceWindowWidth > 0) {
                position[coordinate] =
                    horizontalPosition - differenceWindowWidth;
            } else {
    ...
    private _getDefaultPosition(
        ...
        if (
            ...
            position[verticalPositionProperty] =
                verticalPositionProperty === VERTICAL_DIRECTION.TOP
                    ? 0
                    : containerSizes.height;
        } else {
    ...
    private _calculateLimitOfSizes(
        item: IDialogItem,
        windowData: IPopupPosition
    ): ILimitingSizes {
        const popupOptions = item.popupOptions;
        ...
        if (
            !maxHeight &&
            (popupOptions.top || popupOptions.maximize || maxHeight === null)
        ) {
        ...
        const sizes = {
            ...
            maxHeight,
            maxWidth: Math.min(
                popupOptions.maxWidth || windowData.width,
                windowData.width
            ),
        };
        ...
        if (unsafe_getRootAdaptiveMode().device.isPhone()) {
            sizes.minWidth = popupOptions.minWidth
                ? Math.min(
                      popupOptions.minWidth || windowData.width,
                      windowData.width
                  )
                : undefined;
        }
    ...
    private _calculateValue(
        ...
        const availableMaxSize = maxValue
            ? Math.min(windowValue, maxValue)
            : windowValue;
        ...
        if (
            type === 'width' &&
            (containerValue >= availableMaxSize ||
                popupValue >= availableMaxSize)
        ) {
            value = Math.max(availableMaxSize, availableMinSize);
        }

        if (
            containerValue < availableMinSize ||
            popupValue < availableMinSize
        ) {
    ...
    private _getVerticalPostion(
        ...
        return (
            Math.round((windowData.height - height) / middleCoef) + scrollTop
        );
    }
}
