...
class DialogController extends BaseController {
    ...
    popupDragStart(
        ...
        const verticalProperty =
            item.position.top !== undefined ? VERTICAL_DIRECTION.TOP : VERTICAL_DIRECTION.BOTTOM;
        ...
        if (!item.popupOptions?.minHeight && container && item.resizeMinHeight) {
    ...
    resizeInner(item: IDialogItem, container: HTMLElement): boolean {
        ...
        const getScrollContainer = () => {
            ...
            return Array.prototype.filter.call(scrolls, (node) => {
                return node.clientWidth > 0 && node.scrollTop;
            })[0];
        };
        // Если внутри лежит скроллконтейнер, то восстанавливаем позицию скролла после изменения размеров
    ...
    dragNDropOnPage(
        ...
        if (item.popupOptions.target) {
            if (!isInsideDrag && !item.hasSavedPosition && type === 'customdragStart') {
                item.fixPosition = false;
                this._prepareConfigWithSizes(item, container);
            }
        }
    ...
    private _updateSizes(item: IDialogItem, container: HTMLElement): Promise<void> | undefined {
        if (
            item.popupOptions.propStorageId &&
            item.popupOptions.templateOptions?.resizable &&
            item.hasSavedPosition
        ) {
            this._getPopupSizesByStorageId(item.popupOptions.propStorageId, item, container).then(
                (sizes) => {
    ...
    private _setSizes(sizes: IPopupSizes, item: IDialogItem, container: HTMLElement): void {
        item.sizes = sizes;
    ...
    private _calcStickyCoords(item: IDialogItem, sizes: IPopupSizes = {}): void {
        ...
        // Не будем делать перерасчет окна, если таргет скрыли
        if (!!targetCoords.width) {
                ...
                getSettings([propStorageId]).then((storage) => {
                    if (storage && storage[propStorageId]) {
                        ...
                        if (vertical !== undefined && horizontal !== undefined) {
                            // Не будем проставлять сохраненные координаты, если есть таргет.
                            if (!item.popupOptions.target) {
                                ...
                                item.position[horizontalPositionProperty] = horizontal;
                            ...
                            if (storage[propStorageId].width) {
                                item.position.width = storage[propStorageId].width;
                                item.popupOptions.width = storage[propStorageId].width;
                            }
                            if (storage[propStorageId].height) {
                                item.position.height = storage[propStorageId].height;
                                item.popupOptions.height = storage[propStorageId].height;
                            }
                        }
                    }
                    resolve();
    ...
    protected _saveDataByStorageId(propStorageId: string, data: TStorageData): void {
    ...
    protected _getDefaultConfig(
        ...
        const invertedHorizontalPositionProperty = getInvertedHorizontalPositionProperty(
            horizontalPositionProperty
        );
        if (
            typeof item.position[horizontalPositionProperty] === 'undefined' &&
            typeof item.position[invertedHorizontalPositionProperty] === 'undefined'
        ) {
    ...
    private _getRestrictiveContainerSize(item: IDialogItem): IPopupPosition {
        ...
        if (isBodyTargetContainer) {
            return BaseController.getCoordsByContainer('body', item) as IPopupPosition;
        }
        ...
        const edgeOffset = item.popupOptions.edgeOffset;
        if (edgeOffset) {
            if (typeof position.top === 'number') {
                position.top += edgeOffset.top || 0;
            }
            if (typeof position.bottom === 'number') {
                position.bottom -= edgeOffset.bottom || 0;
            }
            if (typeof position.height === 'number') {
                position.height -= (edgeOffset.top || 0) + (edgeOffset.bottom || 0);
            }

            if (typeof position.left === 'number') {
                position.left += edgeOffset.left || 0;
            }
            if (typeof position.right === 'number') {
                position.right -= edgeOffset.right || 0;
            }
            if (typeof position.width === 'number') {
                position.width -= (edgeOffset.left || 0) + (edgeOffset.right || 0);
            }
        }
        return position;
    }
