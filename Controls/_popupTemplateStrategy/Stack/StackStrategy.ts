...
export class StackStrategy {
    ...
    getPosition(tCoords, item: IStackItem, isAboveMaximizePopup: boolean = false): IPopupPosition {
        if (item.popupOptions.fullscreen && !getAdaptiveModeForLoaders().device.isPhone()) {
        ...
        const width =
            item.popupOptions.fullscreen || item.popupOptions.isWorkspacePopup
                ? maxPanelWidth
    ...
    getMaxPanelWidth(stackParentCoords: IPopupPosition, popupOptions: IPopupOptions): number {
        ...
        if (
            ...
            sidebarCanHide ||
            (popupOptions.maximize && !right)
        ) {
            return maxWidth + MINIMAL_PANEL_DISTANCE;
        }
        if (popupOptions.isWorkspacePopup) {
            return maxWidth + MINIMAL_PANEL_DISTANCE - stackParentCoords.left;
        }
        if (fullscreen) {
    ...
    private _getHorizontalPosition(
        ...
        position: string,
        fullscreen: boolean,
        hasDefaultStackTemplate: boolean = true
    ): number {
