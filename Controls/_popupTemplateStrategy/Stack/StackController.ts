...
export class StackController extends BaseController {
    ...
    onTemplateUpdated(item: IStackItem, container: HTMLElement): void {
    ...
    static calcStackParentCoords(item: IStackItem): IPopupPosition {
        ...
        if (detection.chromeVersion >= 128) {
            Object.keys(documentDimensions).forEach((itemIndex: string) => {
                documentDimensions[itemIndex] /= DimensionsMeasurer.getZoomValue(document.body);
            });
        }
