...
interface IPopupOptions extends IStackPopupOptions {
    ...
    fullscreen: boolean;
    leftContentTemplateWidth?: number;
...
export class StackController extends BaseController {
    ...
    elementCreated(item: IStackItem, container: HTMLElement): boolean {
        const leftContentTemplateWidth = this.getLeftContentWidth(container);
        if (leftContentTemplateWidth) {
            item.popupOptions.leftContentTemplateWidth = leftContentTemplateWidth;
        }
        ...
        // Если стековое окно 1, то перерисовку нужно звать только в двух случаях:
        // 1. После маунта позиция и размеры изменились
        // 2. У окна есть затемнение, нужно перерисовать, т.к. оно появляется только после полной инициализации окна
        // Если окон больше, то перерисовка должна быть, меняются классы, видимость.
        return !isSinglePopup || positionUpdate || item.modal;
    }

    elementUpdateOptions(item: IStackItem, container: HTMLElement): boolean | Promise<boolean> {
        this._preparePropStorageId(item);
        this._prepareMinWidth(item);
    ...
    onTemplateUpdated(item: IStackItem, container: HTMLElement): boolean {
        if (!item.popupOptions.leftContentTemplateWidth) {
            const leftContentTemplateWidth = this.getLeftContentWidth(container);
            if (leftContentTemplateWidth) {
                item.popupOptions.leftContentTemplateWidth = leftContentTemplateWidth;
    ...
    elementMaximized(
        ...
        const maxPanelWidth =
            StackStrategy.getMaxPanelWidth(stackParentCoords, item.popupOptions) ||
    ...
    updateItems(): void {
        let deletedItem: IStackItem;
        this._stack.each((item: IStackItem) => {
            if (!deletedItem && !Controller.getController().find(item.id)) {
                deletedItem = item;
            }
        });
        if (deletedItem) {
            this._stack.remove(deletedItem);
            this._update();
        }
    }

    updateElementsVisibility(): void {
        this._update();
    }
    ...
    private _updateMaximizedState(item: IStackItem, state?: boolean): void {
        const stackParentCoords = this._getStackParentCoords(item);
        const maxPanelWidth = StackStrategy.getMaxPanelWidth(stackParentCoords, item.popupOptions);
        const maximized =
    ...
    resizeOuter(item, container): boolean {
        const result = super.resizeOuter(item, container);
    ...
    popupMovingSize(item: IStackItem, offset: object, position?: string): boolean {
        ...
        const midWidth = this._getMiddleWidth(item, this._getMaxPanelWidth(item, true));
    ...
    getLeftContentWidth(container: HTMLElement) {
        return container?.querySelector('.controls-StackTemplate__leftArea-wrapper')?.offsetWidth;
    }
    ...
    protected _getMaxPanelWidth(item: IStackItem, isPopup: boolean = true): number {
        ...
        if (!isPopup) {
            ...
            const stackParentCoords = this._getStackParentCoords(item);
            return StackStrategy.getMaxPanelWidth(stackParentCoords, item.popupOptions);
        }
    }
    ...
    private _getMaximizedState(item: IStackItem, maxPanelWidth: number): boolean {
        if (item.popupOptions.minWidth && item.popupOptions.maxWidth) {
            ...
            let width = item.position?.width || item.popupOptions.width;
            if (
                maxPanelWidth &&
                width > maxPanelWidth &&
            ...
            if (item.popupState !== this.POPUP_STATE_DESTROYING) {
                ...
                if (currentWidth) {
                    const cacheItem = cache.find((el) => {
                        ...
                        return (
                            ...
                            (!item.currentZIndex || item.currentZIndex >= el.currentZIndex) &&
                ...
                cache = cache.filter((el) => {
                    ...
                    const isVisiblePopup =
                        itemWidth >= (currentWidth || 0) ||
                        item.currentZIndex < el.currentZIndex ||
                        item.popupOptions.asyncShow;
                    if (!isVisiblePopup) {
                        this._hidePopup(el);
                    }
    ...
    protected _prepareSizes(item: IStackItem, container?: HTMLElement): void {
        ...
        if (container) {
            ...
            // По какой-то причине может не быть элемента, не удалось найти кейс повторения
        ...
        if (!item.popupOptions.isWorkspacePopup) {
        ...
        item.popupOptions.width = this._prepareSize(
    ...
    private _getStackParentCoords(item: IStackItem): IPopupPosition {
        ...
        let parentItem = Controller.getController()?.find(item.parentId);
        ...
        do {
            ...
            if (parentItem) {
                parentItem = Controller.getController()?.find(parentItem.parentId);
            }
        } while (parentItem);
        return coords;
    }
    ...
    private _showPopup(item: IStackItem): void {
        ...
        // Если позвали showPopup сразу после hidePopup, надо сбросить временно выставленный display: none;
        const element = document?.querySelector(`[popupkey="${item.id}"]`);
    ...
    private _hidePopup(item: IStackItem): void {
        if (!item.popupOptions.shouldNotHideOverlayedPopup && !item.popupOptions.fullscreen) {
            ...
            if (
                ...
                !item.popupOptions.leftContentTemplateWidth &&
                !item.popupOptions.isCompoundTemplate
            ) {
                if (!item.popupOptions.asyncShow) {
                    const element = document.querySelector(`[popupkey="${item.id}"]`);
                    // Для того чтобы избежать моргания двойной тени (от нового окна и от того, которое перекрылось новым)
                    // синхронно выставим display: none
    ...
    private _prepareMinWidth(item: IStackItem): void {
        if (!item.popupOptions.minWidth) {
            const defaultOptions = this._getDefaultOptions(item);
            if (
                !item.popupOptions.isCompoundTemplate &&
                !defaultOptions.minWidth &&
                item.popupOptions.propStorageId !== null
            ) {
                Logger.warn(
                    'Controls/popup:Stack: Отсутствуют обязательные опции minWidth или width, изменение размера окна невозможно. ' +
                        'Если изменеине размеров не предполагается, нужно выставить значение null'
                );
            }
        }
    }
    ...
    private _preparePropStorageId(item: IStackItem): void {
        if (!item.popupOptions.propStorageId) {
            ...
            if (
                ...
                if (typeof item.popupOptions.template === 'string') {
                    item.popupOptions.propStorageId = item.popupOptions.template;
                } else {
                    Logger.warn(
                        'Controls/popup:Stack: Отсутствует обязательная опция propStorageId, изменение размера окна невозможно. ' +
                            'Если изменеине размеров не предполагается, нужно выставить значение null'
                    );
                }
            }
    ...
    private _writeCompatiblePopupWidth(
        ...
        if (typeof widthState === 'number') {
            ...
            if (!isNaN(widthState.width)) {
                item.popupOptions.width = widthState.width;
            }
            if (!isNaN(widthState.maxSavedWidth)) {
                item.maxSavedWidth = widthState.maxSavedWidth;
            }
            if (!isNaN(widthState.minSavedWidth)) {
    ...
    static calcStackParentCoords(item: IStackItem): IPopupPosition {
        ...
        if (!isNewEnvironment()) {
            ...
            if (item.popupOptions.isWorkspacePopup) {
                const selector = '.sabyPage-MainLayout__content';
                rootCoords = BaseController.getCoordsByContainer(selector, item);
            } else {
                const selector = '.controls-Popup__stack-target-container';
                rootCoords =
                    BaseController.getRootContainerCoords(item, selector, rightPanelWidth) || {};
            }
        }

        // calc with scroll only on desktop devices, because stack popup has fixed position and can scroll with page
        const leftPageScroll = detection.isMobilePlatform ? 0 : rootCoords?.leftScroll || 0;
        ...
        if (documentDimensions.scrollWidth === documentDimensions.clientWidth) {
            // Если у окна есть правая панель - отступ справа не может быть меньше размера правой панели
            stackParentCoords.right = Math.max(stackParentCoords.right, rightPanelWidth);
        }
