...
import { RIGHT_PANEL_WIDTH, getRightPanelWidth as utilGetRightPanelWidth } from 'Controls/popupTemplate';
 ...
 * @class Controls/_popupTemplateStrategy/BaseController
 * @private
 */
abstract class BaseController implements IPopupController {
    ...
    elementDestroyedWrapper(
        ...
        if (item.popupState !== this.POPUP_STATE_DESTROYED) {
            item.popupState = this.POPUP_STATE_DESTROYING;
            item._destroyDeferred = this.elementDestroyed.apply(
                this,
                arguments
            );
    ...
    protected _popupDefaultResizing(
        item: IPopupItem,
        offset: object,
        position: string
    ): boolean {
    ...
    protected _isAboveMaximizePopup(item: IPopupItem): boolean {
        ...
        const parents: Control[] = this._goUpByControlTree(openerContainer || item.activeNodeAfterDestroy);
    ...
    protected _getTargetCoords(item: IPopupItem, sizes: IPopupSizes = {}) {
        ...
        if (item.popupOptions.nativeEvent || hasPosition) {
            ...
            return {
                ...
                height: height || size,
                top: y + fullTopOffset,
                left: x + fullLeftOffset,
                bottom: y + (height || size) + fullTopOffset,
                right: x + (width || size) + fullLeftOffset,
    ...
    private _getFakeDivMargins(item: IPopupItem): {
        ...
        fakeDiv.className =
            ...
            ` controls_popupTemplate_theme-${theme}`;
    ...
    static getRootContainerCoords(
        ...
        const getRestrictiveContainer = (popupItem: IPopupItem) => {
            ...
            if (popupItem.parentId && popupItem.controller?.TYPE !== 'Stack') {
                ...
                const parentItem = require('Controls/popup').Controller.find(
                    popupItem.parentId
                );
                if (!parentItem) {
        ...
        const getCoords = (container) => {
            if (container) {
                let coordsByContainer =
                    BaseController.getCoordsByContainer(container);
        ...
        const itemRestrictiveContainerCoords = getCoords(
            itemRestrictiveContainer
        );
        ...
        if (contentData) {
            const bodyCoords =
                BaseController.getCoordsByContainer(bodySelector);
            return {
    ...
    static getCoordsByContainer(
        ...
        const restrictiveContainerNode =
            document?.querySelector(restrictiveContainer);

        if (restrictiveContainerNode) {
            if (
                !restrictiveContainerNode.offsetWidth &&
                !restrictiveContainerNode.offsetHeight
            ) {
                const errorMessage =
            ...
            const isPopupHidden =
                popup && popup.classList.contains(hiddenClass);
            if (isPopupHidden) {
