...
abstract class BaseController implements IPopupController {
    ...
    protected _popupDefaultResizing(item: IPopupItem, offset: object, position: string): boolean {
        ...
        } else if (position?.includes('right') && item.popupOptions?.targetPoint?.horizontal === 'center') {
            if (typeof item.position.left !== 'undefined') {
                item.position.left -= (offset.x / 2);
            }
        }
    ...
    protected _isAboveMaximizePopup(item: IPopupItem): boolean {
        ...
        if (!openerContainer) {
            const popupItems = ManagerController.getPopupItems() || [];
            ...
            popupItems.forEach((popupItem) => {
                ...
                if (!popupItemWasInStack && popupItem.popupOptions?.fullscreen) {
    ...
    static getCoordsByContainer(restrictiveContainer: any): IPopupPosition | void {
        ...
        const restrictiveContainerNodes = [...(document?.querySelectorAll(restrictiveContainer) || [])];
