...
export default function getTargetCoords(target: HTMLElement): ITargetCoords {
    ...
    */
    ...
    if (isScrollOnBody) {
        ...
        const isKeyboard = isKeyboardVisible();
        const offsetTop: number = isKeyboard ? 0 : viewportDimensions.offsetTop || 0;
    ...
    let fullTopOffset: number =
        ...
        0;
    // Если android и нет скрола на боди, то не добавляем offsetTop в targetCoords, так как он ломает корректное позиционирование
    if (detection.isMobileAndroid && !isScrollOnBody && isKeyboardVisible()) {
        fullTopOffset = 0;
    }
