...
import { BaseTreeControl as TreeControl, BaseTreeControlComponent } from 'Controls/baseTree';
 ...
 * @remark
 * <ol>
 *     <li>Допустимы значения направления сортировки ASC/DESC.
 *         <ul>
 *             <li>В таблицах можно изменять сортировку нажатием кнопки сортировки в конкретной ячейке заголовкв таблицы. Для этого нужно в {@link /doc/platform/developmentapl/interface-development/controls/list/grid/header/ конфигурации ячейки шапки} задать свойство {@link Controls/grid:IHeaderCell#sortingProperty sortingProperty}. Подробнее читайте {@link /doc/platform/developmentapl/interface-development/controls/list/grid/header/sorting/ здесь}</li>
 *             <li>При отсутствии заголовков в реестре можно воспользоваться кнопкой открытия меню сортировки. Для этого нужно добавить на страницу и настроить контрол {@link Controls/sorting:Selector}. Подробнее читайте {@link /doc/platform/developmentapl/interface-development/controls/list/sorting/table/ здесь}</li>
 *         </ul>
 *     </li>
 *     <li>Выбранную сортировку можно сохранять. Для этого используют опцию {@link Controls/grid:IPropStorage#propStorageId propStorageId}.</li>
 *     <li>Список отображает данные в том порядке, в котором их предоставляет БЛ. Для отображения узлов дерева в начале списка, перед листами необходимо так настроить БЛ, чтобы узлы возвращались первыми независимо от запрошенной сортировки.</li>
 * </ol>
 */
