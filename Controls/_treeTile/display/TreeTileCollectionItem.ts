...
import { default as TreeTileItemActions } from '../itemActions/Control';
 ...
 * @private
 */
export default class TreeTileCollectionItem<T extends Model = Model> extends mixin<
    TreeItem,
    TileItemMixin
>(TreeItem, TileItemMixin) {
    ...
    getItemStyles({
        ...
        if (this.isNode() && (itemType === 'default' || itemType === 'small')) {
            ...
            const widthCalculated = this.getTileWidth(width, imagePosition, imageViewMode);
            return {
        ...
        switch (itemType) {
            ...
            case 'preview':
                if (this.isNode()) {
                    ...
                    classes = classes.replace('controls-fontsize-m', 'controls-fontsize-l');
                }
                break;
            case 'small':
                break;
