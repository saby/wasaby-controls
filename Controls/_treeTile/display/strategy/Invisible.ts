...
import TreeTileCollectionItem, {
    ITreeTileCollectionItemOptions,
} from '../TreeTileCollectionItem';
...
> extends TileInvisibleStrategy<S, T> {
    ...
    >(items: T[], options: ISortOptions<S, T>): number[] {
        ...
        function isPlainItem(item: T): boolean {
            return (
                isGroupItem(item) ||
                isAdditionalItem(item) ||
                isSearchSeparatorItem(item)
            );
        }
        ...
        for (let i = 0; i < items.length; i++) {
            let itemIndex = i;
            let item = items[i];
            let itemIsNode = !isPlainItem(item) && item.isNode() !== null;
            ...
            let nextItemInRoot =
                hasNextItem &&
                !nextItemIsPlain &&
                nextItem.getParent() === display.getRoot();
            while (hasNextItem && !nextItemIsPlain && !nextItemInRoot) {
                i++;
                item = items[i];
                itemIndex = i;
                itemIsNode = !isPlainItem(item) && item.isNode() !== null;
                nextItem = items[i + 1];
            ...
            const nextItemIsLeaf =
                hasNextItem && !nextItemIsPlain && nextItem.isNode() === null;

            // Добавляем невидимые элементы перед группой и разделителем для того, чтобы предшествующие плитки ужались
            if (
                hasNextItem &&
                (isGroupItem(nextItem) || isSearchSeparatorItem(nextItem))
            ) {
                if (
                    InvisibleStrategy._needCreateInvisibleItems(
                        item,
                        options.invisibleItems
                    )
                ) {
                    ...
                    newInvisibleItems.push(
                        options.invisibleItems.filter(
                            (invisibleItem: InvisibleTreeTileItem) => {
            ...
            if (
                (itemIsNode ||
                    (!isPlainItem(item) &&
                        item.getParent() !== display.getRoot())) &&
        ...
        if (display.getTileMode() === 'static' || !!display.getGroup()) {
            ...
            const lastItemIsAdditional =
                hasLastItem && isAdditionalItem(lastItem);
            const lastItemIsLeaf =
                hasLastItem && !lastItemIsPlain && lastItem.isNode() === null;
                ...
                if (
                    ...
                    newInvisibleItems.push(
                        options.invisibleItems.filter(
                            (invisibleItem: InvisibleTreeTileItem) => {
                                return (
                                    invisibleItem.contents === lastItem.contents
                                );
                            }
                        )
                    );
        ...
        for (let i = 0; i < newInvisibleItems.length; i++) {
            ...
            for (let j = 0; j < newItems.length; j++) {
                ...
                itemsOrder.splice(
                    insertIndex + invisibleItemIndex,
                    0,
                    invisibleItemIndex
                );
            }
    ...
    protected static _getInvisibleItemParams(
        ...
        const params = super._getInvisibleItemParams(
            display,
            prevItem,
            options
        );
