...
> extends mixin<DestroyableMixin, SerializableMixin>(
    ...
    constructor(collection?: Collection<S, T>) {
        ...
        this._onEventRaisingChangeHandler =
            this._onEventRaisingChange.bind(this);
    ...
    setCollection(collection: Collection<S, T>): void {
        ...
        if (this._collection && !this._collection.destroyed) {
            this._collection.unsubscribe(
                'onCollectionChange',
                this._onCollectionChangeHandler
            );
            this._collection.unsubscribe(
    ...
    protected _onCollectionChange(
        ...
        const checkItems = (items, count, startIdx) => {
            return count > 0
                ? this._checkRange(startIdx - 1, items.length + 2)
                : [];
        };
        ...
        switch (action) {
            case IObservable.ACTION_ADD:
                ...
                push.apply(
                    result,
                    checkItems(newItems, newItems.length, newItemsIndex)
                );
                break;
            case IObservable.ACTION_REMOVE:
                ...
                push.apply(
                    result,
                    checkItems([], oldItems.length, oldItemsIndex)
                );
                break;
            ...
            case IObservable.ACTION_MOVE:
                ...
                const startIndex =
                    newItemsIndex > oldItemsIndex
                        ? oldItemsIndex
                        : oldItemsIndex + newItems.length;
    ...
    protected _onEventRaisingChange(
        event: EventObject,
        enabled: boolean,
        analyze: boolean
    ): void {
    ...
    protected _onAfterCollectionChange(): void {
        if (
            this._collectionItems &&
            this._collection.getCount() !== this._collectionItems.length
        ) {
            this._applyCollection();
        }
    }
    ...
    protected _isPrimaryIndex(
        ...
        const collectionItems = this._collectionItems;
        const prev = (
            byOriginal ? collection.at(idx - 1) : collectionItems[idx - 1]
        ).getContents();
        const curr = (
            byOriginal ? collection.at(idx) : collectionItems[idx]
        ).getContents();
