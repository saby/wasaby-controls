...
const getFilterItemProperty = FilterDescription.getFilterItemProperty;
const getExtendedCaption = FilterDescription.getExtendedCaption;

function validateFilterItem(item: IFilterItem, localItemProp?: TFilterItemLocalName): void {
    if (constants.isProduction) {
        return;
    }
    if (localItemProp && item[localItemProp]) {
        const wrongPropsForLocalItem = [
            'name',
            'value',
            'resetValue',
            'textValue',
        ...
        Object.keys(item[localItemProp]).forEach((propName) => {
            if (wrongPropsForLocalItem.includes(propName)) {
                Logger.error(
                    `Для элемента фильтра нельзя передавать опцию ${propName} в ${localItemProp}. Задайте опцию на самом элементе фильтра, а не в ${localItemProp}`
                );
            }
        });
    }

    const extendedCaption = getExtendedCaption(item);
    const viewMode = getFilterItemProperty(item, 'viewMode', localItemProp);

    if (extendedCaption && !viewMode) {
        Logger.error(`Для элемента фильтра c именем ${item.name} необъодимо определить viewMode`);
    }
}
...
export default class FilterViewModel extends mixin<VersionableMixin>(VersionableMixin) {
    ...
    private _getSource(
        ...
        let result = source
            .flatMap((item) => {
                const resultItem = this._prepareFilterItem({ ...item }, filterItemLocalProperty);
                return this._isFilterVisible(resultItem) ? resultItem : [];
            })
            .map((item, index) => {
                validateFilterItem(item, filterItemLocalProperty);
                ...
                if (!item.hasOwnProperty('editorCaption')) {
                    const caption = getFilterItemProperty<string>(
                        item,
                        'caption',
                        filterItemLocalProperty
                    );
                    item.editorCaption = typeof caption !== 'undefined' ? caption : item.group;
                }
                item.caption = '';
                if (item.jumpingLabel && isEqual(item.value, item.resetValue)) {
                    item.editorCaption = '';
                }
                if (needCalcViewMode) {
        ...
        const firstItemName = this._getFirstBasicItemName(result);
        const firstFilterItem = firstItemName && result.find(({ name }) => name === firstItemName);

        if (firstFilterItem) {
            firstFilterItem.isFirstEditor = true;
        }
    ...
    private _resetSourceViewMode(): void {
        this._source.forEach((item) => {
            item.viewMode = getExtendedCaption(item) ? 'extended' : item.viewMode;
        });
    }

    private _resetFilterItem(item: IFilterItem): void {
        const { filterViewMode, editorsViewMode, filterItemLocalProperty } = this._options;
        item.value = item.resetValue;
        item.textValue = '';
        item.viewMode = this._getItemViewMode(
            item,
    ...
    private _getItemViewMode(
        ...
        const extendedCaption = getExtendedCaption(item, filterItemLocalProperty);
    ...
    private _getFilterItemEditorOptions(
        ...
        const { viewMode, name, resetValue, emptyText, emptyKey, textValue, editorOptions } =
            filterItem;
        ...
        // при перестроении панели (например выбрали 'extended' фильтр) inferno может разрушить редактор,
        // и чтобы при mount'e не было запроса за данными, надо взять закэшированный sourceController
        const oldSourceItem = this._source?.[filterIndex];
        if (
            !sourceController &&
        ...
        const itemEditorOptions = {
            ...
            extendedCaption: getExtendedCaption(filterItem),
    ...
    private _prepareFilterItem(
        ...
        if (!filterItemLocalProperty || !item[filterItemLocalProperty]) {
            return item;
        }

        const localItem = item[filterItemLocalProperty];
    ...
    setViewModeForItem(filterName: string, viewMode: IFilterItem['viewMode']): void {
        ...
        item.viewMode = item.editorOptions.viewMode = viewMode;
        this._source = source;
        this._filterDescriptionChanged(this._source);
        this._nextVersion();
    }
    ...
    needShowSeparator(): boolean {
        ...
        return this._options.editorsViewMode === 'cloud' && this.hasBasicItems();
    }
    ...
    needCloseItemByCaptionClick(filterName: string): boolean {
        ...
        return !!(item.expanderVisible && getExtendedCaption(item));
    }
}
