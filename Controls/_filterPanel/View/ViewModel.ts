...
export default class FilterViewModel extends mixin<VersionableMixin>(VersionableMixin) {
    ...
    private _getSource(
        ...
        if (filterViewMode === 'popup') {
            result = result.sort((item1, item2) => {
                ...
                const canItem1VisibilityChanged = item1.hasOwnProperty('visibility') &&
                                                item1.filterVisibilityCallback &&
                                                item1.viewMode !== 'extended';
                const canItem2VisibilityChanged = item2.hasOwnProperty('visibility') &&
                ...
                if (canItem1VisibilityChanged || canItem2VisibilityChanged || isExtended1 === isExtended2) {
                    return 0;
                }
    ...
    private _isFilterVisible(filterItem: IFilterItem): boolean {
        ...
        const isBasicFilterItem =
            viewModeFilter('basic', filterItem) && !this._getExtendedCaption(filterItem);
        return (
    ...
    private _setValueToSourceItem(item: IFilterItem, editorValue: object | unknown): void {
        item.value = editorValue && editorValue.hasOwnProperty('value') ? editorValue.value : editorValue;
    }
    ...
    private _getBasicFilterItems(source: IFilterItem[]): IFilterItem[] {
        ...
        const isBasicByValue = (item) => {
            return (
                isFrequentFilterItem(item, this._options.filterViewMode, this._options.editorsViewMode) &&
    ...
    private _needResetValueUnchangedItems(editingObject: Record<string, IExtendedPropertyValue>): boolean {
        ...
        if (this._options.multiSelect === false) {
            ...
            needResetValueUnchangedItems = Object.keys(editingObject).some((key) => {
                ...
                const newValue = editorValue && editorValue.hasOwnProperty('value') ? editorValue.value : editorValue;
    ...
    getItemClasses(item: CollectionItem, viewMode: TPanelViewMode, isAdaptive: boolean): string {
