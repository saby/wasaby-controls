...
export default class FilterViewModel extends mixin<VersionableMixin>(VersionableMixin) {
    ...
    private _isFilterVisible(filterItem: IFilterItem): boolean {
        ...
        const isBasicFilterItem =
            ...
            !FilterDescription.getExtendedCaption(filterItem);
    ...
    private _resetSourceViewMode(): void {
        this._source.forEach((item) => {
            item.viewMode = FilterDescription.getExtendedCaption(item) ? 'extended' : item.viewMode;
        });
    }
    ...
    private _getItemViewMode(
        ...
        const extendedCaption = FilterDescription.getExtendedCaption(item, filterItemLocalProperty);
    ...
    private _getFilterItemEditorOptions(
        ...
        const itemEditorOptions = {
            ...
            extendedCaption: FilterDescription.getExtendedCaption(filterItem),
    ...
    needShowSeparator(): boolean {
        ...
        return (
            ...
            this._source.some((item) => {
                return item.viewMode === 'extended' || FilterDescription.getExtendedCaption(item);
            })
        );
    }
    ...
    needCloseItemByCaptionClick(filterName: string): boolean {
        ...
        return !!(item.expanderVisible && FilterDescription.getExtendedCaption(item));
    }
}
