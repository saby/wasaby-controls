...
import {isFrequentFilterItem, getExtendedItems, viewModeFilter} from './ExtendedItemsUtil';
...
export default class FilterViewModel extends mixin<VersionableMixin>(VersionableMixin) {
    ...
    private _getBasicFilterItems(source: IFilterItem[]): IFilterItem[] {
        ...
        const isBasicByValue = (item) => {
            return isFrequentFilterItem(item, this._options.filterViewMode) && !isEqual(item.value, item.resetValue);
        };
