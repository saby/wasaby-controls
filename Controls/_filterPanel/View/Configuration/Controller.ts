 ...
 */
import { IFilterItem, IFilterItemConfiguration, FilterLoader } from 'Controls/filter';
...
export default class FilterConfigurationController {
    ...
    update(options: IControllerOptions): void {
        ...
        if (options.viewMode !== 'popup' && options.editorsViewMode !== 'popupCloudPanelDefault') {
    ...
    private _setConfiguration(configuration: IFilterItemConfiguration[]): void {
        if (this.isStorageEnabled()) {
            USER.set(
                this.getStorageKey() + FilterLoader.FILTER_USER_PARAM_POSTFIX,
