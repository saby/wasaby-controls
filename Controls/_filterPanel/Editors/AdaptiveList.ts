...
import createSyntheticItem from 'Controls/_filterPanel/Editors/List/Utils/createSyntheticItem';
...
export default class ListEditor extends Control<IAdaptiveListEditorOptions> {
    ...
    protected _getRoots(
        ...
        if (items) {
            ...
            if (options.emptyText && !items.getRecordById(options.emptyKey)) {
                this._prependItem(options, options.emptyKey, options.emptyText, roots);
            }
            if (options.selectedAllText && !items.getRecordById(options.selectedAllKey)) {
    ...
    protected _selectedKeysChanged(event: SyntheticEvent, keys: TKey[], added: TKey[]): void {
    ...
    private _prependItem(options, key: TKey, text: string, items: RecordSet): void {
