...
function ImageTemplate(props: IImageColumnProps): JSX.Element {
    const tplProps = props.column.getTemplateOptions() as unknown as IImageColumnProps;
    const imageProperty = props.column.config.imageProperty;
    if (tplProps.imageTemplateName) {
        return <Async templateName={tplProps.imageTemplateName} templateOptions={props} />;
        ...
        return (
            <img
                ...
                data-qa="controls-ListEditor__column-img"
...
/**
 * Шаблон колонки для отображения картинки.
 * @private
 */
export default function ImageColumn(props: IImageColumnProps): JSX.Element {
    if (props.column.getTemplateOptions().emptyKey === props.item.key) {
        return <TitleColumn column={props.column} item={props.item} />;
    } else {
        return <ImageTemplate column={props.column} item={props.item} />;
    }
}
