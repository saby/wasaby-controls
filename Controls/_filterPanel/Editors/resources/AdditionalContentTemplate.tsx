...
function CounterTemplate(props: IAdditionalColumnProps): JSX.Element {
    const templateOptions = {
        ...props,
        item: props.collectionItem || props.item,
    };
            ...
            >
                {mainCounter.charAt ? (
                    ...
                    <DecoratorNumber
                        {...props}
                        tooltip={
                            props.mainCounterTooltip || item.get(props.mainCounterTooltipProperty)
                        }
                        value={item.get(props.mainCounterProperty)}
                    />
                )}
            </div>
        );
...
function AdditionalTextProperty(props: IAdditionalColumnProps): JSX.Element | null {
    const {
        item,
        mainCounterProperty,
 ...
 * @private
 */
export default forwardRef(function AdditionalContentTemplate(
    props: IAdditionalColumnProps,
    ref
): JSX.Element {
    const {
    ...
    const clickableCounterProps = useMemo(() => {
        if (onCounterClick || onMainCounterClick || onAdditionalCounterClick) {
            ...
            if (item.get(additionalTextProperty)) {
        ...
        return null;
    }, [item, item.getVersion(), mainCounterProperty, additionalTextProperty]);

    if (
        (additionalTextProperty && item.get(additionalTextProperty) !== undefined) ||
        onCounterClick ||
        onMainCounterClick ||
        onAdditionalCounterClick
    ) {
