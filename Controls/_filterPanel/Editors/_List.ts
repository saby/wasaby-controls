...
import addItemsFromSelector from './List/Utils/addItemsFromSelector';
...
import { TItemActionShowType, IItemAction, TItemActionVisibilityCallback } from 'Controls/itemActions';
...
export default class ListEditor extends Control<IListEditorOptions> {
    ...
    protected _beforeUpdate(options: IListEditorOptions): void {
        ...
        const additionalCounterTooltipChanged = additionalCounterTooltip !== this._options.additionalCounterTooltip;
    ...
    protected _needNavigateItem(item: Model): boolean {
        ...
        if (pageId && mask) {
            ...
            if (markedKey) {
                markedItemPageId = this._items.getRecordById(markedKey)?.get('pageId');
            }
    ...
    protected _setColumns(options: IListEditorOptions): void {
        this._columns = getColumns(options, this._getTextValueForItem.bind(this));
    }
    ...
    protected _colspanCallback(item: Model): number {
    ...
    private _getMarkedKey(
        ...
        if (!multiSelect) {
            ...
            if (!selectedKeys.length || selectedKeys[0] === resetKey) {
                ...
                return this._markedKey && resetKey === undefined ? null : resetKey;
            } else {
    ...
    private _getItemPadding({ markerStyle }: IListEditorOptions): IItemPadding {
        return markerStyle === 'primary' ? null : { top: 'null', bottom: 'null' };
    }
