...
export default class ListEditor extends Control<IListEditorOptions> {
        ...
        if (sourceController) {
            ...
            if (this._isHistoryInTree(options)) {
                const {
                    ...
                    COPY_ORIG_ID,
                } = loadSync('Controls/Utils/History/PrepareFilterPanelItems');

                let historyItems = sourceController.getItems();

                if (!this._hasHistoryElements(sourceController.getItems())) {
                    historyItems = prepareFilterPanelHistoryItems(
    ...
    protected _itemActionVisibilityCallback(action: IItemAction, item: Model): boolean {
        ...
        if (this._isHistoryInTree(this._options)) {
            if (item.get(this._getPrepareHistoryUtils().IS_DUPLICATED_ITEM) === true) {
                return false;
            }
        }
    ...
    private _handleDataLoad(_event: SyntheticEvent, items: RecordSet, direction: Direction): void {
        ...
        if (this._isHistoryInTree(this._options)) {
            this._setHistory(items, this._options);
    ...
    protected _getPrepareHistoryUtils(): any {
        return loadSync('Controls/Utils/History/PrepareFilterPanelItems');
    }

    protected _getHistoryStoreLib(): Promise<typeof import('Controls/HistoryStore')> {
    ...
    private _hasHistoryElements(items: RecordSet) {
        const { COPY_ORIG_ID } = loadSync('Controls/Utils/History/PrepareFilterPanelItems');
        return items.getCount() && items.at(0).get(COPY_ORIG_ID);
    }

    private _setHistory(items: RecordSet, options): RecordSet {
        const {
            prepareFilterPanelHistoryItems,
            setHistoryItemsGroupProperty,
            getGroupProperty,
        ...
        prepareFilterPanelHistoryItems(items, options.historyId, {
            parentProperty: options.parentProperty,
            keyProperty: options.keyProperty,
            nodeProperty: options.nodeProperty,
        });

        setHistoryItemsGroupProperty(items, this._options);
        this._groupProperty = getGroupProperty(items, this._options);
        items.setKeyProperty(COPY_ORIG_ID);
        return items;
    }
