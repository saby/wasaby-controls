...
const ExtendedTemplate = React.forwardRef((props: IDateMenuOptions, ref) => {
    return (
        <FrequentItem
            ...
            attrs={props.attrs}
            frequentItemText={props.fastItem?.get(props.displayProperty)}
...
const DateMenu = React.forwardRef((props: IDateMenuOptions, ref: React.ForwardedRef<unknown>) => {
    function updateTextValue(): void {
        const textValueRes = getTextValue(props, items, selectedKeys, props.propertyValue);
    ...
    React.useEffect(() => {
        ...
        setSelectedKeys(getSelectedKeys(items, props));
        updateTextValue();
