 ...
 * @demo Controls-demo/filterPanelPopup/Editors/Lookup/Index
 * @public
 */

class LookupEditor
    extends Control<ILookupEditorOptions>
    implements ILookupEditor
{
    ...
    protected _beforeMount(options: ILookupEditorOptions): void {
        ...
        this._sourceControllerItemsChanged =
            this._sourceControllerItemsChanged.bind(this);
    ...
    private _initItems(options: ILookupEditorOptions): void {
        ...
        if (
            ...
            this._items = this._getItemsByPropValueAndTextValue(
                options,
                options.textValue
            );
        } else if (
        ...
        if (this._items) {
            this._textValue = this._getTextValue(
                this._items as RecordSet,
                options
            );
        }
    }
    ...
    private _getFilteredItems(
        items: RecordSet,
        options: ILookupEditorOptions
    ): RecordSet {
        const { propertyValue, value, keyProperty, source, multiSelect } =
            options;
    ...
    private _getItemsFromSourceController(
        options: ILookupEditorOptions
    ): RecordSet {
        const {
            propertyValue,
        ...
        let result;
        if (items?.getCount() && !isEqual(propertyValue, resetValue)) {
            const selectedKeys = Array.isArray(propertyValue)
                ? propertyValue
                : [propertyValue];
    ...
    private _sourceControllerItemsChanged(
        ...
        newItems.getFormat(true).each((field) => {
            if (currentItemsFormat.getFieldIndex(field.getName()) === -1) {
                currentItems.addField({
                    ...
                    kind:
                        field.getType() === 'array'
                            ? (field as format.ArrayField).getKind()
                            : null,
                });
            }
        });
    ...
    protected _extendedCaptionClickHandler(event: SyntheticEvent): void {
        loadAsync('Controls/lookup').then(({ showSelector }) => {
            showSelector(
                this,
                this._getLookupPopupOptions(),
                this._options.multiSelect
            );
        });
    }

    protected _frequentItemClickHandler(
        event: SyntheticEvent,
        extendedValue
    ): void {
    ...
    protected _handleLookupClick(): void {
        this._children.lookupEditor.showSelector(this._getLookupPopupOptions());
    }

    protected _resetClick(): void {
        if (
            this.isResetValueEmpty() &&
    ...
    private isResetValueEmpty(): boolean {
        ...
        return multiSelect
            ? !resetValue.length
            : resetValue === null || resetValue === undefined;
    }
