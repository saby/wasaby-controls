...
export default function addItemsFromSelector(
    ...
    const maxItemsCount =
        options.historyId && options.parentProperty
            ? (getMaxItemsCount(options.navigation) || 0) + getHistoryItemsCount(items)
            : getMaxItemsCount(options.navigation); // Выбранные элементы надо добавлять после запиненных записей и записей для сброса параметра фильтрации
...
function getHistoryItemsCount(items: RecordSet): number {
    const maxHistoryItemsCount = 10;
    let counter = 1;
    items.each((item) => {
        if (
            (item.get('pinned') || item.get('frequent') || item.get('recent')) &&
            counter < maxHistoryItemsCount
        ) {
            counter++;
        }
    });
...
export function getMaxItemsCount(navigation): number | void {
    let pageSize;
