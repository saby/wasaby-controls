...
import type { Move } from 'Controls/listCommands';
...
export default class DragNDropProvider {
    ...
    dragEnd(
        ...
        return this._getMoveAction().then((moveAction) => {
            return moveAction.execute({
                selection,
                targetKey: target.getKey(),
                position,
                sorting: this._options.sourceController.getSorting(),
            });
        });
    }
    ...
    private _getMoveAction(): Promise<Move> {
        return loadAsync<typeof import('Controls/listCommands')>('Controls/listCommands').then(
            ({ Move }) => {
                return new Move({
