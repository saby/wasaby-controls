...
export interface IBaseEditor extends IComponentProps, IEditorOptions<unknown> {
    ...
    filterIndex?: number;
    closeButtonViewMode?: 'external' | 'toolButton';
}
...
export default forwardRef(function BaseEditor(
    ...
    const filterItem = useContext(FilterDescriptionContext);
    const viewMode = filterItem.viewMode || props.viewMode;
    const resetValue = filterItem.resetValue || props.resetValue;
    ...
    const editorTemplateAttrs = useMemo(() => {
        return {
    ...
    if (viewMode === 'basic') {
        const wrapperClassName = `${props.attrs
            ...
            !isEqual(props.propertyValue, resetValue)
                ? 'controls-FilterViewPanel__basicEditor-filterChanged'
                : ''
        } controls-FilterViewPanel__baseEditor-container_closeButton-${
            props.closeButtonViewMode || 'external'
        }`;
        return (
                ...
                >
                    {expanded ? (
                        <props.editorTemplate
                            ...
                            attrs={editorTemplateAttrs}
                            data-qa="FilterViewPanel__baseEditor"
                            dataQa="FilterViewPanel__baseEditor"
                        />
                    ) : null}
                    {props.closeButtonViewMode !== 'toolButton' ? CloseButton(props) : null}
                </div>
            </div>
        );
    } else {
