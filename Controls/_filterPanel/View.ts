...
export default class View extends Control<IViewPanelOptions> {
    ...
    protected _beforeUpdate(options: IViewPanelOptions): void {
        this._loadEditors(options.source, () => {
            if (
                options.itemsContainerPadding &&
    ...
    private _loadEditors(source: IFilterItem[], readyCallback: () => void): void {
        const promises = [];

        source.forEach(({ editorTemplateName }) => {
            if (editorTemplateName && !isLoaded(editorTemplateName)) {
                promises.push(loadAsync(editorTemplateName));
            }
        });
            ...
            source.forEach((sourceItem) => {
                if (isEqualItems(targetItem, sourceItem)) {
                    ...
                    if (
                        ...
                        targetItem.viewMode = sourceItem.viewMode;
                    }
