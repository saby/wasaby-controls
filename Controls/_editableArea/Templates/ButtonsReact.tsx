...
import { TApplyButtonStyle } from '../interface/IView';
...
export default function Buttons(props: IButtonsProps): JSX.Element {
    const {applyButtonStyle = 'accent'} = props;
    ...
    return (
        ...
        >
            <div
                ...
                className={`controls-EditableArea__applyButton controls-EditableArea__applyButton-${applyButtonStyle}`}
                title={rk('Подтвердить')}
                onClick={props.onApplyClick}
            >
                <div
                    className={
                        `controls-EditableArea__applyButton-icon controls-icon_style-${
                            applyButtonStyle === 'accent' ? 'contrast' : 'success'
                        } icon-Yes`
                    }
                />
            </div>
