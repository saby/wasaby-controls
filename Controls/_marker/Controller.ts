...
export class Controller {
    ...
    constructor(options: IOptions) {
        const markerStrategy = options.markerStrategy || SingleColumnMarkerStrategy;
     ...
     * @param {boolean} force Принудительно обновить ключ маркированной записи в коллекции
     * @param {boolean} silent Устанавливать ключи, не вызывая событий обновления
     * @void
     */
    setMarkedKey(key: CrudEntityKey, force?: boolean, silent?: boolean): void {
        this._model.setMarkedKey(key, force, silent);
        this._markedKey = key;
    }
    ...
    onCollectionAdd(items: CollectionItem<Model>[]): void {
        ...
        // ----
        // force нужен для случая с DnD. При добавлении и затем удалении стратегии DnD
        // Последовательно вызываются startUpdateSession, rebuild, finishUpdateSession,
        // Это приводит к последовательному вызову onCollectionChange(rm), onCollectionChange(a)
        // И, соответственно, onCollectionRemove, onCollectionAdd.
        // В свою очередь, onCollectionRemove бежит по удаляемым записям и ставит им setMarked(false, true), а
        // коллекцию не обновляет. В итоге не проходит проверка в collection.setMarkedKey
        // Этот случай тестируется автотестами по эталонам.
        if (this._containsMarkedItem(items)) {
            this.setMarkedKey(this._markedKey, true, true);
        }
    }
