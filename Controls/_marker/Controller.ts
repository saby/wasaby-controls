...
export class Controller {
    ...
    getNextMarkedKey(): CrudEntityKey | void {
        const index = this._model.getIndex(
    ...
    getMarkedKeyByDirection(direction: TDirection): CrudEntityKey | void {
        ...
        const nextMarkedKey = this._markerStrategy.getMarkedKeyByDirection(
            index,
            direction
        );
        return nextMarkedKey === null ? this._markedKey : nextMarkedKey;
    ...
    onCollectionRemove(
        ...
        const removeMarkedItem = !!removedItems.find((it) => {
            return it.Markable && it.getContents().getKey() === this._markedKey;
        });
        if (!removeMarkedItem) {
            return this._markedKey;
        }

        let markedKeyAfterRemove =
            this._getMarkedKeyAfterRemove(removedItemsIndex);
        ...
        if (
            ...
            if (parent && parent !== this._model.getRoot() && parent.Markable) {
                ...
                if (parentItem) {
                    ...
                    markedKeyAfterRemove = parent.isRoot()
                        ? parentItem
                        : parentItem.getKey();
                }
            }
