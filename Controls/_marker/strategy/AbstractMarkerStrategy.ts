    ...
    implements IMarkerStrategy<TCollection>
{
    ...
    getMarkedKeyByDirection(
        ...
        direction: TDirection | 'backward' | 'forward'
    ): CrudEntityKey | null {
        const correctDirection: TDirection =
            direction === 'backward' ? 'Backward' : direction === 'forward' ? 'Forward' : direction;
        return this._wrapInValidReturn(state, collection, (index) =>
            this._getMarkedKeyByDirection(collection, index, correctDirection)
        );
    }
    ...
    abstract oldGetMarkedKeyByDirection(
        collection: TCollection,
        index: number,
        direction: TDirection
    ): CrudEntityKey | void;

    abstract oldGetNextMarkedKey(collection: TCollection, index: number): CrudEntityKey | null;

    abstract oldGetPrevMarkedKey(collection: TCollection, index: number): CrudEntityKey | null;
}
