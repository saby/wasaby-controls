...
class Search extends Base<ISearchInputOptions> {
    ...
    constructor(props: ISearchInputOptions) {
        ...
        const configForSearchStates = {
            ...props,
            inlineHeight: props.designContextValue?.searchInlineHeight || props.inlineHeight,
        };
        generateStates(this, configForSearchStates);
    }
    ...
    shouldComponentUpdate(nextProps: ISearchInputOptions, nextState): boolean {
        // Лечим приколы васаби. Проблема в том, что при 1 построении, при вводе подгружаются данные,
        // из-за чего перерисовка самого поля ввода происходит раньше, чем придет новый props.
        // Поэтому смотрим на разницу между props и state, и если были изменения, то перерисовываем компонент
        // https://online.sbis.ru/opendoc.html?guid=fe9c0859-f0cf-44ac-a960-21080b9601d3&client=3
        if (this.props !== nextProps || JSON.stringify(this.state) !== JSON.stringify(nextState)) {
            ...
            if (
                ...
                (nextProps.value !== currentDisplayValue ||
                    (!nextProps.value && !currentDisplayValue))
            ) {
                this._updateSearchData(
                    nextProps.value,
                    nextProps.minSearchLength,
                    this.props.value
                );
            }
            return res;
    ...
    componentDidUpdate(prevProps: ISearchInputOptions): void {
        const currentDisplayValue = this._viewModel.displayValue;
        if (
            prevProps.value !== this.props.value &&
            (this.props.value !== currentDisplayValue ||
                (!this.props.value && !currentDisplayValue))
        ) {
            this._updateSearchData(this.props.value, this.props.minSearchLength, prevProps.value);
    ...
    protected _updateViewModelValue(props: IBaseInputOptions = this.props): void {
        if (this.props.value !== props.value) {
            super._updateViewModelValue(props);
        }
    }
    ...
    protected _renderStyle(): string {
        ...
        if (this.props.contrastBackground || this.props.designContextValue?.contrastBackground) {
            style = this.props.isAdaptive ? 'searchContrastAdaptive' : 'searchContrast';
        } else {
            style = this.props.isAdaptive ? 'searchAdaptive' : 'search';
        }
        return style;
    }
    ...
    protected _initProperties(): void {
        ...
        this._rightFieldWrapper.scope.isVisibleReset =
            _private.isVisibleBaseFieldContent.bind(this);
        this._rightFieldWrapper.scope.isVisibleSearchButton =
            _private.isVisibleBaseFieldContent.bind(this);
        this._rightFieldWrapper.scope.readOnly = this._getReadOnly();
    ...
    protected _notifySearchClick(event): void {
        const searchClickResult = this.props.onSearchClick?.(event, event.nativeEvent);

        if (this._viewModel.displayValue && searchClickResult !== false) {
    ...
    protected _keyDownHandler(event: SyntheticEvent<KeyboardEvent>): void {
        if (event.nativeEvent.which === constants.key.enter && !event.ctrlKey) {
            if (!this._getReadOnly()) {
...
/**
 * @event Controls/_search/Input/Search#searchClick Происходит при клике на кнопку поиска или при нажатии на клавишу Enter.
