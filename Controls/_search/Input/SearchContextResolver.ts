...
export default class SearchContextResolver extends Control<ISearchContextResolverOptions> {
    ...
    private _validateSlices(slices: TSlices): void {
        ...
        Object.entries(slices).forEach(([key, slice]) => {
            ...
            } else if (slice['[IListSlice]'] && !slice.searchParam) {
    ...
    protected _beforeMount(options?: ISearchContextResolverOptions): Promise<void> | void {
        if (options.storeId) {
            const slices = this._getSlices(options);
            this._validateSlices(slices);
            this._searchValue = this._getInitialSearchValue(slices);
            this._inputSearchValue = this._getSearchInputValueFromSlice(slices);
            this._minSearchLength = this._getSliceProperty(slices, 'minSearchLength');
    ...
    protected _beforeUpdate(newOptions: ISearchContextResolverOptions): void {
        if (newOptions.storeId) {
            ...
            const newSearchInputValue = this._getSearchInputValueFromSlice(slices);
    ...
    protected _inputSearchValueChanged(e: SyntheticEvent<Event>, value: string): void {
        e.stopPropagation();
