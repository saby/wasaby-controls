...
export default class ControllerClass {
    ...
    search(value: string): Promise<RecordSet | Error | null> {
        ...
        if (this._viewMode !== 'search') {
            ...
            this._hasHiearchyFilterBeforeSearch = FilterResolver.hasHierarchyFilter(filter);
    ...
    private _dataLoadCallback(event: unknown, items: RecordSet): void {
        ...
        if (
            ...
            if (
                ...
                const newRoot = FilterResolver.getRootForSearch(
                    this._path,
                    this._root,
                    this._options.parentProperty,
        ...
        if (this._searchValue) {
            this._misspellValue = FilterResolver.getSwitcherStrFromData(items);
        }
    }
    ...
    private _getRoot(): TKey {
        return FilterResolver.getRootForSearch(
            this._path,
            this._root,
            this._options.parentProperty,
            this._options.startingWith
    ...
    private _getFilterWithSearchValue(
        ...
        return FilterResolver.getFilterForSearch(
            {
                parentProperty,
                searchParam,
    ...
    private _getFilterWithoutSearchValue(
        ...
        return FilterResolver.getResetSearchFilter(
            filter,
