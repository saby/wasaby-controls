...
export default class ControllerClass {
    ...
    private _adaptiveSearchMode: boolean;
    ...
    constructor(options: ISearchControllerOptions) {
        ...
        if (options.sourceController) {
            ...
            this._path = ControllerClass._getPath(
        ...
        this._adaptiveSearchMode = !!options.adaptiveSearchMode;
    }
    ...
    reset(
        dontLoad?: boolean
    ): Promise<RecordSet | Error> | QueryWhereExpression<unknown> {
    ...
    search(value: string): Promise<RecordSet | Error | null> {
        ...
        if (this._viewMode !== 'search') {
            ...
            this._hasHiearchyFilterBeforeSearch =
                ControllerClass._hasHierarchyFilter(filter);
            this._hasRootInFilterBeforeSearch = filter.hasOwnProperty(
        ...
        if (
            ...
            (!this._searchPromise && newSearchValue)
        ) {
            ...
            searchResult = this._updateFilterAndLoad(
                this._getFilter(),
                this._getRoot()
            );
    ...
    private _dataLoadCallback(event: unknown, items: RecordSet): void {
        ...
        if (this.isSearchInProcess() && this._searchValue) {
            ...
            if (
                ...
                const newRoot = ControllerClass._getRoot(
                    this._path,
                    this._root,
                    this._options.parentProperty
                );
            ...
            if (this._root !== undefined) {
                ...
                if (root !== undefined) {
                    if (this._options.deepReload) {
                        filter =
                            NewSourceController.prepareFilterWithExpandedItems(
                                filter,
    ...
    private _getFilterWithoutSearchValue(
        ...
        if (this._options.parentProperty) {
            ...
            if (!this._hasHiearchyFilterBeforeSearch) {
            ...
            if (!this._hasRootInFilterBeforeSearch) {
    ...
    private _updateFilterAndLoad(
        ...
        if (
            this._options.parentProperty &&
            !isLoaded('Controls/searchBreadcrumbsGrid')
        ) {
    ...
    private _preFilterItemsBySearchValue(
        items: RecordSet,
        searchValue: string
    ): Model[] {
        return chainFactory(items)
            .filter((item) => {
                return !!this._options.filterOnSearchCallback(
                    searchValue,
                    item
                );
            })
