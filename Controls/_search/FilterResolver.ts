            ...
            if (root !== undefined) {
                if (listState.deepReload) {
                    searchFilter = prepareFilterWithExpandedItems(
                        searchFilter,
                        listState.sourceController.getExpandedItems(),
                        parentProperty,
                        root
                    );
                } else {
...
function prepareFilterWithExpandedItems(
    ...
    if (expandedItems?.length && expandedItems?.[0] !== null) {
        ...
        if ((resultFilter[parentProperty] as CrudEntityKey[]).indexOf(root) === -1) {
            (resultFilter[parentProperty] as CrudEntityKey[]).push(root);
        }
        // Добавляет отсутствующие expandedItems в фильтр expanded узлов
        resultFilter[parentProperty] = (resultFilter[parentProperty] as CrudEntityKey[]).concat(
            expandedItems.filter((key: CrudEntityKey) => {
                return (resultFilter[parentProperty] as CrudEntityKey[]).indexOf(key) === -1;
            })
        );
