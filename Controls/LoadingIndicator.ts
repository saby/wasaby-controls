 ...
 */
/**
 * Библиотека контролов, которые служат для отображения индикатора загрузки.
 * @library
 * @public
 */
 ...
 *
 * Полезные ссылки:
 * * {@link https://git.sbis.ru/saby/wasaby-controls/-/blob/rc-24.6100/Controls-default-theme/variables/_loadingIndicator.less переменные тем оформления}
 *
 * @example
 * <pre class="brush: js">
 * <!-- JS -->
 ...
 * }
 * </pre>
 * @class Controls/LoadingIndicator:default
...
class LoadingIndicator extends Control<ILoadingIndicatorOptions> implements ILoadingIndicator {
    ...
    _updateProperties(cfg: ILoadingIndicatorOptions): void {
        ...
        if (cfg.overlay !== undefined) {
            this.overlay = this._getOverlayValue(cfg.overlay);
        }
    ...
    _updateZIndex(config: ILoadingIndicatorOptions): void {
        const popupItem =
            ManagerController && ManagerController.getController()?.find((config || {}).popupId);
    ...
    protected _getOverlayValue(value: ILoadingIndicatorOptions['overlay']): string {
    ...
    protected _getFixedZIndex(config: ILoadingIndicatorOptions): number {
        ...
        const popupList = ManagerController && ManagerController.getController()?.getPopupItems();
        let resultZIndex = POPUP_BASE_ZINDEX - 1;
        if (popupList && popupList.getCount()) {
    ...
    _hideHandler(event: SyntheticEvent<Event>, id: string): void {
        event.stopPropagation();
     ...
     * @property {Boolean} isGlobal Определяет, показать индикатор над всей страницей или только над собственным контентом.
     * @property {String} message Текст сообщения индикатора.
     ...
     * @property {Number} delay Задержка перед началом показа индикатора(Задается в миллисекундах).
     * @property {String|Number} id Уникальный ключ открываемого индикатора.
     */
    /**
    ...
    private _toggleIndicator(
        ...
        if (visible) {
            ...
            if (force) {
                ...
                if (!isGlobal) {
                    // TODO: Поправить после перевода на react.
                    this.setState({});
                }
            } else {
...
export { default as ContextProvider, Context } from 'Controls/_LoadingIndicator/ContextProvider';
export { default as ContextConsumer } from 'Controls/_LoadingIndicator/ContextConsumer';
