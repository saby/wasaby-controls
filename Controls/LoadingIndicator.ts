...
class LoadingIndicator
    extends Control<ILoadingIndicatorOptions>
    implements ILoadingIndicator
{
    ...
    _updateZIndex(config: ILoadingIndicatorOptions): void {
        const popupItem =
            ManagerController && ManagerController.find((config || {}).popupId);
    ...
    protected _getFixedZIndex(config: ILoadingIndicatorOptions): number {
        ...
        const popupList =
            ManagerController && ManagerController.getManager()._popupItems;
    ...
    private _toggleIndicator(
        ...
        const isGlobal = config.hasOwnProperty('isGlobal')
            ? config.isGlobal
            : this.isGlobal;
        ...
        if (visible) {
            ...
            const needForceUpdate: boolean =
                this._isOverlayVisible || this._isMessageVisible;
    ...
    private _toggleIndicatorVisible(
        toggle: boolean,
        config?: ILoadingIndicatorOptions
    ): void {
        if (toggle) {
    ...
    private _redrawOverlay(): void {
        ...
        if (nextMessageVisibility) {
            const newMessageClassName = this._getThemedClassName(
    ...
    _recalcMessagePosition(): void {
        ...
        if (this.isGlobal && isMessageDivVisible && ManagerController) {
            ...
            const rootHeight =
                contentData?.height ||
                rootContainerData?.height ||
                document.body.clientHeight;
            if (rootWidth && rootHeight) {
                const offsetLeft = this._getOffset(
                    contentData?.left,
                    rootContainerData?.left
                );
                const offsetTop = this._getOffset(
                    contentData?.top,
                    rootContainerData?.top
                );
                const left =
                    (rootWidth - messageDiv.clientWidth) / 2 + offsetLeft;
                const top =
                    (rootHeight - messageDiv.clientHeight) / 2 + offsetTop;
    ...
    private _calculateOverlayClassName(): string {
        ...
        classList.push(
            this.isGlobal
                ? 'controls-loading-indicator_global'
                : 'controls-loading-indicator_local'
        );
        ...
        if (this.scroll) {
            ...
            classList.push(
                'controls-loading-indicator_sided controls-loading-indicator_sided-' +
                    this.scroll
            );
        }
        if (this.small) {
        ...
        if (this.overlay) {
            const overlayClassName =
                'controls-loading-indicator_overlay-' +
                this._getOverlay(this.overlay);
