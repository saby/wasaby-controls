/**
 * @kaizen_zone 96ed3176-68c2-43f7-9958-0ac21bc22bed
 */
import * as React from 'react';
import type { TInternalProps } from 'UICore/Executor';
import type { TItemEventHandler } from 'Controls/baseList';
import type { TOverflow } from 'Controls/interface';
import { controller as localeController } from 'I18n/i18n';

interface ICompatibleProps {
    // Должна ли кнопка редактирования перекрывать тенью часть многоточия у записи при textOverflow.
    // Если указано, то на блок непосредственно перед стрелкой надо вешать
    // класс controls-Grid__editArrow-overflow-ellipsis.
    // Используется в старом гриде.
    textOverflow?: TOverflow;
}

interface IEditArrowComponentInnerProps extends ICompatibleProps {
    onClick?: TItemEventHandler;
}

/**
 * Свойства компонента "Кнопка редактирования".
 * @public
 */
export interface IProps extends TInternalProps, IEditArrowComponentInnerProps {
    /**
     * Цвет фона подложки стрелки редактирования.
     * @default default фон соответствут фону подсветки ячейки по умолчанию.
     * @see Controls/list/View/Property/hoverBackgroundStyle
     */
    backgroundStyle?: string;
    /**
     * Пользовательский CSS класс
     */
    className?: string;
}

export const EDIT_ARROW_SELECTOR = 'js-controls-Grid__editArrow';

/**
 * Компонент "Кнопка редактирования".
 * @param props
 * @param ref
 */
function EditArrowComponentRef(
    props: IProps,
    ref: React.ForwardedRef<HTMLDivElement>
): React.ReactElement {
    let rootClasses =
        'controls-Grid__editArrow-wrapper js-controls-ListView__visible-on-hoverFreeze';
    rootClasses +=
        ' tw-relative tw-shrink tw-min-w-0 tw-flex tw-justify-end tw-items-baseline tw-invisible tw-pointer-events-none';
    rootClasses += ` ${EDIT_ARROW_SELECTOR}`;
    const externalClassName = props.attrs?.className || props.className;
    if (externalClassName) {
        rootClasses += ` ${externalClassName}`;
    }

    const iconClasses = 'controls-Grid__editArrow-icon';
    const iconTransform =
        localeController.currentLocaleConfig.directionality === 'rtl' ? 'rotate(180, 8, 10)' : '';
    const backgroundStyle = props.backgroundStyle ?? 'default';
    const editArrowTailwindClasses =
        'tw-relative tw-inline-flex tw-items-baseline tw-cursor-pointer tw-pointer-events-auto';
    return (
        <div ref={ref} className={rootClasses}>
            {props.textOverflow === 'ellipsis' && (
                <div className={'controls-Grid__editArrow-withTextOverflow tw-shrink tw-min-w-0'} />
            )}
            <div
                className={`controls-Grid__editArrow-blur tw-h-full tw-inline-block tw-shrink-0 tw-z-0 controls-Grid__editArrow-blur_${backgroundStyle}`}
            />
            <div
                className={`controls-Grid__editArrow ${editArrowTailwindClasses} controls-Grid__editArrow_background_${backgroundStyle}`}
                data-qa={'edit-arrow'}
            >
                <svg
                    className={iconClasses}
                    viewBox={'0 0 16 16'}
                    xmlns={'http://www.w3.org/2000/svg'}
                >
                    <path
                        transform={iconTransform}
                        d={'M9,9.94l-6,6v-2l4-4-4-4v-2Zm5,0-6,6v-2l4-4-4-4v-2Z'}
                    />
                </svg>
            </div>
        </div>
    );
}

const EditArrowComponent = React.forwardRef(EditArrowComponentRef);
export default EditArrowComponent;
