...
export const SELECTORS = {
    ...
    ITEM_ACTIONS: '.js-controls-ItemActions__ItemAction',
    ITEM_DRAGGING: '.controls-ListView__item_dragging',
};
...
export function isGroupItem(event: React.MouseEvent, item: TItem): boolean {
    return item instanceof Array || item instanceof Model
        ...
        : !!item['[Controls/_display/GroupItem]' as keyof typeof item];
}
...
function isItemActions(event: React.MouseEvent): boolean {
    return !!closest(event, SELECTORS.ITEM_ACTIONS);
}

function isDragging(event: React.MouseEvent): boolean {
    return !!closest(event, SELECTORS.ITEM_DRAGGING);
}
...
export function parseViewItemClick<T extends TItem = CollectionItem>(
    ...
    if (cleanScheme) {
        ...
        if (isItemActions(event)) {
            handlers.onItemActions?.(event, item);
            return true;
        }
    }
    ...
    if (cleanScheme && isDragging(event)) {
        return false;
    }
