...
export class AsyncListVirtualScrollController extends ListVirtualScrollController {
    private _nextRange: IItemsRange | null;
    private _isReset: boolean;
    private _renderAfterReset: boolean;
    private _startIndexShift: number | null;
    private _shiftDirection: IDirection | null;
    private _prevHasItemsOutsideRange: IHasItemsOutsideOfRange = {
        backward: false,
        forward: false,
    };
    private _changeItemsOutRangeOnItemsRendered: Function | null;
    ...
    protected _isAllRendered(range?: IItemsRange): boolean {
        return super._isAllRendered(range || this._nextRange || undefined);
    }
    ...
    setRenderedItems(renderedItems: TItemKey[]): void {
        ...
        if (this._nextRange && this._isAllRendered()) {
            ...
            this._applyIndexes(
                ...
                // @ts-expect-error вызов метода в коллекции. Внутри Direction может быть не задан
                this._shiftDirection
            );
            // @ts-expect-error здесь происходит сброс индексов. Скорее всего нужен отдельный метод в коллекции
