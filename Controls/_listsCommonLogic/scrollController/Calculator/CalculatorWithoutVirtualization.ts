...
import AbstractCalculator, { IAbstractCalculatorResult } from './AbstractCalculator';
...
export default class CalculatorWithoutVirtualization extends AbstractCalculator {
    addItems(position: number, count: number, _calcMode: ICalcMode): IAbstractCalculatorResult {
        ...
        this.resetItems(this._totalCount + count, {
            startIndex: 0,
            endIndex: this._totalCount + count,
        });
    ...
    resetItems(totalCount: number, initRange: IItemsRangePartial): IAbstractCalculatorResult {
        const oldState = this._getOldState();
