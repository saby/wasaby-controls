...
import { IEdgeItemCalculatingParams } from '../AbstractListVirtualScrollController';
...
function getItemsSizesSum(params: IGetItemsSizesSumParams): number {
    ...
    if (params.calcByOffset && range.endIndex - range.startIndex > 0) {
        ...
        const result = endItemOffset - (startItemOffset ?? 0) - offsetBetweenLastItems / 2;
...
export function getEdgeVisibleItem(params: IGetEdgeVisibleItemParams): IEdgeItem | null {
    const {
        viewportSize,
        scrollPosition,
        direction,
    ...
    let edgeItem: IEdgeItem | null = null;
    let itemKey: string | undefined;
    let itemOffset: number | undefined;
    let itemBorderBottom: number | undefined;
    ...
    let keepByElementKey: string | undefined;

    if (typeof params.itemKey !== 'undefined') {
        const upperBorder = offset || 0;
        const bottomBorder = upperBorder + viewportSize;
        const middleBorder = bottomBorder / 2;
        for (
            ...
            const itemOffset = item.offset - (placeholders?.backward ?? 0) - scrollPosition;

            // Если средняя линия расположена на элементе, сохраняем его. Это центральный элемент вьюпорта
            if (
                middleBorder >= itemOffset &&
                middleBorder <= itemOffset + item.size &&
                keepByElementKey === undefined
            ) {
                keepByElementKey = item.key;
            }

            // Если элемент расположен во вьюпорте, сохраняем его
            if (itemOffset + item.size >= upperBorder && itemOffset <= bottomBorder) {
                if (item.key === params.itemKey) {
                    keepByElementKey = params.itemKey;
                    break;
                }
            }
        }
    ...
    const fillItemData = (item: IItemSize) => {
        itemKey = item.key;
        itemOffset = item.offset - (placeholders?.backward ?? 0) - scrollPosition;
        itemBorderBottom = Math.round(itemOffset) + Math.round(item.size);
        isValidItem =
            !validateItem ||
            (typeof itemKey !== 'undefined' && validateItem(itemKey)) ||
            (!!keepByElementKey && itemKey === keepByElementKey);
    };
    ...
    for (
        ...
        if (
            typeof itemBorderBottom !== 'undefined' &&
            ((itemBorderBottom > viewportBorderPosition && keepByElementKey === undefined) ||
                isLastItem ||
                isLastRenderedItem ||
                (keepByElementKey && itemKey === keepByElementKey))
        ) {
            ...
            if (
                isValidItem &&
                typeof itemOffset !== 'undefined' &&
                typeof itemKey !== 'undefined'
            ) {
                let borderDistance;
                let border: IDirection;
                if (direction === 'forward') {
