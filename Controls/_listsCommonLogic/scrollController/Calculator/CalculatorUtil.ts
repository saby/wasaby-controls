...
export function getEdgeVisibleItem(params: IGetEdgeVisibleItemParams): IEdgeItem {
    ...
    // invertedRelativeToElement: null инвертирует относительно среднего элемента viewPort
    if (invertedRelativeToElement === null || invertedRelativeToElement) {
        const viewPortItems = [];
        const upperBorder = offset || 0;
        const bottomBorder = upperBorder + viewportSize;
        let isElementInViewPort = false;
        let tempViewportSize = 0;
        for (
            ...
            if (itemOffset > upperBorder && itemOffset < bottomBorder) {
                if (item.key === invertedRelativeToElement) {
                    isElementInViewPort = true;
                    if (direction === 'backward') {
                        tempViewportSize += itemOffset * 2 + item.size;
                    } else {
                        tempViewportSize = -viewportSize + (itemOffset * 2 + item.size);
                    }
                    break;
                }
        ...
        if (!isElementInViewPort) {
            ...
            viewportBorderPosition = tempViewportSize;
        }
    }
