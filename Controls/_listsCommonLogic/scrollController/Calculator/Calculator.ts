...
import type { ITriggersOffsets } from '../ObserverController/AbstractObserversController';
import {
    ICalcMode,
    IDirection,
...
import type { IRenderedOutsideItem } from '../ItemsSizeController/AbstractItemsSizeController';
...
export class Calculator extends AbstractCalculator {
    ...
    private get _pageSize(): number | undefined {
        return this._virtualScrollConfig.pageSize;
    }
    ...
    shiftRangeToIndex(index: number, force?: boolean, position?: string): ICalculatorResult {
        const oldState = this._getOldState();
        let shiftDirection: IDirection | null = null;
        ...
        if (this.shouldShiftRangeBeforeScrollToItem(index, position, force)) {
            ...
            const startIndex =
                position === 'bottom' ? Math.max(index - (this._pageSize ?? 0) + 1, 0) : index;
    ...
    shiftRangeToVirtualScrollPosition(scrollPosition: number): ICalculatorResult {
        ...
        if (this._scrollPosition !== scrollPosition) {
            if (typeof this._virtualScrollConfig.pageSize === 'undefined') {
                throw Error('page size is not defined');
            }
    ...
    addItems(position: number, count: number, calcMode: ICalcMode): ICalculatorResult {
        ...
        const startPosition =
            ...
            (typeof firstItem?.key !== 'undefined' && this._validateItemCallback(firstItem?.key)
                ? 0
                : 1);
     ...
     */
    resetItems(totalCount: number, initRange: IItemsRangePartial): ICalculatorResult {
        const oldState = this._getOldState();
    ...
    private _getSegmentSize(): number {
        ...
        if (!segmentSize) {
            segmentSize = this._pageSize
                ? Math.ceil(this._pageSize / RELATION_COEFFICIENT_BETWEEN_PAGE_AND_SEGMENT)
                : 0;
        }
        return segmentSize;
    }
    ...
    private _calcAddDirection(position: number, count: number): IDirection {
        ...
        const key = this._itemsSizes[this._range.startIndex].key;

        const startIndex =
            typeof key !== 'undefined' && this._validateItemCallback(key)
    ...
    private _calcRestoreDirection(action: 'rm' | 'a', position: number): IDirection {
        ...
        // Исключительным образом обрабатываем поведение. Сделано в рамках решения ошибки:
        // https://online.sbis.ru/opendoc.html?guid=88ea980e-27f4-4c3b-8489-cd7b0cc7c3a0
        // В случае последующих обращений - будем рассматривать вариант "как в Windows/Explorer", т.е. при развороте
        // узла - пытаемся показать его содержимое полностью. Если места недостаточно - проскралливаем на высоту
        // содержимого узла. Максимально - сам узел может прижат к верхней границе, но всё ещё виден во ViewPort.
        if (action === 'a') {
            let addPosition: 'before' | 'after' | 'middle';

            if (position <= this._range.startIndex) {
                addPosition = 'before';
            } else if (position >= this._range.endIndex) {
                addPosition = 'after';
            } else {
                addPosition = 'middle';
            }
