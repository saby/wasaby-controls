...
export interface IAbstractCalculatorState {
    range: IItemsRange;
    itemsSizes: IItemsSizes | undefined;
}
...
export default abstract class AbstractCalculator {
    ...
    shouldShiftRangeBeforeScrollToItem(
        _index: number,
        _position?: string,
        _force?: boolean
    ): boolean {
    ...
    getContentSizeBeforeList(): number {
        return this._contentSizeBeforeList;
    }
    ...
    scrollPositionChange(
        ...
        if (updateActiveElement) {
            const activeElementIndex = getActiveElementIndexByScrollPosition({
