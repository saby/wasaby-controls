 ...
 */
import type { Collection as BaseCollection } from 'Controls/display';

import type { IVirtualScrollConfig } from './interface/IVirtualScroll';
import type { IVirtualCollection } from './interface/IVirtualCollection';
import type {
...
import type { IAbstractCalculatorState } from './Calculator/AbstractCalculator';
import type { TVirtualScrollMode } from './interface/IVirtualScroll';
import type {
...
import type { ICalculatorState, TInvertedIndexes } from './Calculator/Calculator';
...
import InertialScrolling from './InertialScrolling';
import { getMaxScrollPosition } from './ScrollUtil';

const ERROR_PATH =
    'Controls/_listsCommonLogic/scrollController/AbstractListVirtualScrollController';
...
export type TScrollToPosition = 'top' | 'bottom';

export interface IScheduledScrollToElementParams {
    key: TItemKey;
    position: TScrollToPosition;
    force: boolean;
    selfScroll?: boolean;
...
export interface IEdgeItemCalculatingParams {
    ...
    // Ключ элемента, который принудительно будет считаться граничным
    itemKey?: string | null;
}

// IScrollParam и TScrollToPosition пересекаются в контексте позиций пролистывания.
export type IScrollParam = number | TScrollToPosition | 'pageUp' | 'pageDown';
...
export abstract class AbstractListVirtualScrollController<
    TOptions extends
        IAbstractListVirtualScrollControllerOptions = IAbstractListVirtualScrollControllerOptions,
> {
    ...
    private _itemSizeProperty: string | undefined;
    private _virtualScrollMode: TVirtualScrollMode | undefined;
    ...
    protected _scrollOnReset: IScrollOnReset | null = null;
    ...
    private _beforeVirtualRangeChangeCallbackPromise?: Promise<void>;

    private _itemsRangeScheduledSizeUpdate: IItemsRange | null;
    protected _scheduledScrollParams: IScheduledScrollParams | null;
    private _scheduledUpdateHasItemsOutRange: IHasItemsOutsideOfRange | null;
    private _scheduledCheckTriggersVisibility: boolean;
    private _scheduledUpdatePlaceholders: IPlaceholders | null;
    private _handleChangedIndexesAfterSynchronizationCallback: Function | null;

    /**
     * TODO: убрать, когда построение будет только синхронным.
     * @protected
     */
    protected _setItemsContainerOnItemsRendered: Function | null = null;

    private _checkTriggersVisibilityTimeout: ReturnType<typeof setTimeout>;
     ...
     * @private
     */
    private _scrollToItemPromise: Promise<void> | undefined;
     ...
     * @private
     */
    private _predicatedRestoreDirection: IDirection | null = null;
     ...
     * @private
     */
    private _scrollCompletedCallback: (() => void) | null = null;
    ...
    protected readonly _task88221033238881: boolean;
    ...
    private _wasAddingBeforeEndIndex: boolean = false;

    private _$logTmp_itemsSizes: IItemSize[] | undefined;
    ...
    constructor(options: TOptions) {
        ...
        // Сброс скролла при resetItems путем подскролла к первой записи
        this._task88221033238881 = options.task88221033238881;
    ...
    isRenderNewIndexes(): boolean {
        return this._renderNewIndexes;
    }
    ...
    virtualScrollPositionChange(position: number, applyScrollPositionCallback: () => number): void {
    ...
    updateMinMaxIndexes(minIndex?: number, maxIndex?: number): void {
        this._scrollController.updateMinMaxIndexes(minIndex, maxIndex);
    }
    ...
    setScrollBehaviourOnReset(scrollBehaviourOnReset: IScrollOnReset | null): void {
    ...
    viewportResized(viewportSizes: IViewportSize): void {
        this._viewportResized(this._extractViewportSize(viewportSizes));
    }

    protected _extractViewportSize(viewportSizes: IViewportSize) {
        return viewportSizes.height;
    }
    ...
    updatePlaceholderAfterContent(): void {
        return this._scrollController.updatePlaceholderAfterContent();
    }

    getEdgeVisibleItem(
        direction: IDirection,
        offset?: number,
        itemKey?: CrudEntityKey | null
    ): IEdgeItem | null {
        return this._scrollController.getEdgeVisibleItem({
            direction,
            offset,
            itemKey:
                itemKey === null || itemKey === undefined
                    ? itemKey
                    : getValidItemKeyAttribute(itemKey),
        });
    }
    ...
    tryToInvert(edgeItem: IEdgeItem): TInvertedIndexes {
        const itemsSizes = this._scrollController.getItemsSizes();
        const viewHeight = this._scrollController.getViewportSize();
        const index = this._getIndexByKey(edgeItem.key);
        const offsetBeforeItem =
            edgeItem.direction === 'forward'
                ? viewHeight - edgeItem.borderDistance - itemsSizes[index].size
                : edgeItem.borderDistance;

        const result = this._scrollController.getInvertedIndexes(index, offsetBeforeItem);
        return result;
    }

    hasEmptyItemsToDirection(direction: IDirection): boolean {
        const itemsSizes = this._scrollController.getItemsSizes();
        const index = direction === 'backward' ? 0 : itemsSizes.length - 1;
        return !itemsSizes[index].size;
    }

    extendIndexesToDirection(direction: IDirection, totalCount: number): IItemsRange {
        const extendSize = this._options.virtualScrollConfig.pageSize || 0;
        const startIndex = this._collection.getStartIndex();
        const endIndex = this._collection.getStopIndex();

        if (direction === 'backward') {
            return {
                startIndex: Math.max(0, startIndex - extendSize),
                endIndex,
            };
        } else {
            return {
                startIndex,
                endIndex: Math.min(totalCount, endIndex + extendSize),
            };
        }
    }

    shiftIndexesToDirectionEdge(direction: IDirection, totalCount: number): IItemsRange {
        const startIndex = this._collection.getStartIndex();
        const endIndex = this._collection.getStopIndex();
    ...
    addItems(position: number, count: number, scrollMode: IScrollMode, calcMode: ICalcMode): void {
        ...
        this._wasAddingBeforeEndIndex = oldRange.endIndex > position;
    ...
    resetItems(): void {
        ...
        if (shouldResetScrollPosition && this._shouldDoScroll()) {
            ...
            if (this._task88221033238881 && totalCount > 0) {
                let index = 0;
                if (this._isEndInitialScrollPosition()) {
                    const pageSize = this._options.virtualScrollConfig.pageSize;
                    if (pageSize && pageSize < totalCount - 1) {
                        index = pageSize - 1;
                    } else {
                        index = totalCount - 1;
                    }
                ...
                this._scheduleScroll({
                    ...
                    params: {
                        key: this._collection.at(index).key,
     ...
     * @param subtractingListOffset - Вычитать при подскролле отступ до списка
    ...
    scrollToItem(
        ...
        this._scrollToItemPromise.finally(() => {
            return (this._scrollToItemPromise = undefined);
        });
        ...
        if (
            ...
            this._scheduleScroll({
                type: 'scrollToElement',
                // @ts-expect-error position может быть undefined
                params: { key, position, force, selfScroll, subtractingListOffset, offset },
            });
        } else {
     ...
     * @private
     */
    scrollToPage(direction: IDirection): Promise<TItemKey | null> {
     ...
     * @private
     */
    scrollToEdge(edge: IDirection): Promise<void | TItemKey | null> {
        ...
        if (item) {
            ...
            let itemPosition: TScrollToPosition;
            if (edge === 'backward') {
                itemPosition = 'top';
            } else {
            ...
            return new Promise((resolve) => {
                ...
                return this.scrollToItem(itemKey, itemPosition, true, false).then(() => {
                    ...
                    // Изменение позиции скролла может отбаботать асинхронно.
                    // В таком случае, крайний элемент синхронно будет посчитан неверно (по старой позиции)
                    // чтобы считать крайний элемент корректно, нужен requestAnimationFrame.
                    window?.requestAnimationFrame(() => {
                        if (
                            itemPosition === 'bottom' &&
                            itemKey === this._collection.getLast().key
                        ) {
                            resolve(itemKey);
                        } else {
                            resolve(this._getFirstVisibleItemKey());
                        }
                    });
                });
            });
    ...
    protected _getFirstVisibleItemKey(): TItemKey | null {
    ...
    private _indexesInitializedCallback(
        params: IIndexesChangedParams<IAbstractCalculatorState>
    ): void {
        this._handleChangedIndexes(params.range, null, () => {
            if (this._scrollOnReset === 'restore') {
                this._scheduleScroll({
                    ...
                    params: {
                        ...
                        placeholders: (params.oldState as ICalculatorState).placeholders,
            ...
            if (this._isReact && this._feature1184208466 && this._virtualScrollMode === 'hide') {
                (this._collection as unknown as BaseCollection).resetRenderedItems?.();
            }
            this.setScrollBehaviourOnReset(null);
        });
    }

    private _indexesChangedCallback(params: IIndexesChangedParams<IAbstractCalculatorState>): void {
    ...
    private _handleChangedIndexes(
        ...
        const applyRangeWithCallback = () => {
            ...
            if (beforeRangeChangeCallbackResult instanceof Promise) {
                if (this._wasAddingBeforeEndIndex) {
                    this._wasAddingBeforeEndIndex = false;
    ...
    callBeforeRangeChangeCallback(range: IItemsRange): Promise<unknown> | void {
        if (this._beforeVirtualRangeChangeCallbackPromise) {
            return this._beforeVirtualRangeChangeCallbackPromise.then(() => {
                this._beforeVirtualRangeChangeCallbackPromise = undefined;
    ...
    protected _isAllRendered(range?: IItemsRange): boolean {
        ...
        if (this._feature1184208466) {
            ...
            for (let i = startIndex; i < endIndex && allRendered; i++) {
                ...
                if (
                    // @ts-expect-error Нужно пересмотреть тип коллекции
                    (this._collection as unknown as BaseCollection).getSourceIndexByItem?.(item) ===
                    -1
                ) {
                    continue;
                }
    ...
    protected _validateEdgeItem(_itemKey: CrudEntityKey): boolean {
        return true;
    }
        ...
        if (this._scheduledScrollParams) {
            switch (this._scheduledScrollParams.type) {
                case 'calculateRestoreScrollParams':
                    ...
                    if (edgeItem) {
                        addToLog('DebugListVirtualScroll', 1, () => {
                            // eslint-disable-next-line
                            // @ts-ignore-next-line
                            const itemsSizes = [...this._scrollController.getItemsSizes()];
                            // eslint-disable-next-line
                            // @ts-ignore-next-line
                            this._$logTmp_itemsSizes = itemsSizes;

                            return {
                                title: 'AbstractListVirtualScrollController',
                                message: '_handleScheduledScroll.calculateRestoreScrollParams',
                                data: [
                                    ['sizes', itemsSizes],
                ...
                case 'restoreScroll':
                    ...
                    addToLog('DebugListVirtualScroll', 1, () => {
                        const itemsSizes = [...this._scrollController.getItemsSizes()];
                        const diffItemsSizes: unknown[] = [];

                        if (this._$logTmp_itemsSizes) {
                            this._$logTmp_itemsSizes.forEach((beforeItem, index) => {
                                const afterItem = itemsSizes[index];
                                const isNotEqualSizes =
                                    beforeItem.size !== 0 && beforeItem.size !== afterItem.size;

                                if (isNotEqualSizes) {
                                    diffItemsSizes.push({
                                        key: beforeItem.key,
                                        index,
                                        sizeBefore: beforeItem.size,
                                        sizeAfter: afterItem.size,
                                        offsetBefore: beforeItem.offset,
                                        offsetAfter: afterItem.offset,
                                    });
                                }
                            });
                            // eslint-disable-next-line
                            // @ts-ignore-next-line
                            this._$logTmp_itemsSizes = undefined;
                        }

                        return {
                            title: 'AbstractListVirtualScrollController',
                            message: '_handleScheduledScroll.restoreScroll',
                            data: [
                                ['sizes', itemsSizes],
                                ['viewPortSize', this._viewportSize],
                                ['params', restoreScrollParams],
                                ['scrollPosition', scrollPosition],
                                ['diffSizes', diffItemsSizes],
                            ],
                        };
                    });
                ...
                case 'scrollToElement':
                    ...
                    this._scrollToElement(
                        ...
                        undefined,
                        scrollToElementParams.subtractingListOffset,
                        scrollToElementParams.offset
                    );
                    this._scheduledScrollParams = null;
                    break;
                case 'doScroll':
    ...
    private _scrollToElement(
        ...
        const scrollToElement = () => {
            ...
            let element: HTMLElement | null = this.getElement(key);
            ...
            if (element) {
                ...
                if (subtractingListOffset) {
                    scrollOffset = -this._scrollController.getListContainerOffset();
                }

                const result = this._scrollToElementUtil(element, position, !!force, scrollOffset);
    ...
    private _getGivenItemsSizes(): IItemsSizes | undefined {
        if (!this._itemSizeProperty) {
            return undefined;
        }
        let offset = 0;
        const itemSizeProperty = this._itemSizeProperty;
        return this._collection.getItems().map((it) => {
            const itemSize = {
                size: it.getGivenItemsSize(itemSizeProperty),
                offset,
                key: String(it.key),
            };
            ...
            if (!itemSize.size) {
                Logger.error(
                    'Controls/listsCommonLogic:ScrollController | Задана опция itemHeightProperty, ' +
