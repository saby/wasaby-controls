...
import { ICalculatorState } from 'Controls/_listsCommonLogic/scrollController/Calculator/Calculator';
...
type TTryToInvert =
    | { inverted: true; invertedRange: IItemsRange }
...
> {
    ...
    tryToInvert(edgeItem: IEdgeItem, totalCount: number, viewHeight: number): TTryToInvert {
        const itemIndex = this._getIndexByKey(edgeItem.key);
        const currentRange = {
            startIndex: this._collection.getStartIndex(),
            endIndex: this._collection.getStopIndex(),
        };
        ...
        if (edgeItem.direction === 'forward') {
            before -= edgeItem.borderDistance;
            after -= viewHeight - edgeItem.borderDistance - itemsSizes[itemIndex].size;
        }

        for (let i = 0; i < itemIndex; i++) {
            before += itemsSizes[i].size;
            if (before > 0) break;
        }
        for (let i = itemIndex + 1; i < itemsSizes.length; i++) {
            after += itemsSizes[i].size;
            if (after > 0) break;
        }
        if (before < 0 || after < 0) {
            return {
                inverted: false,
                extendDirection: before < 0 ? 'backward' : 'forward',
            };
        }

        const invertedRange = {
            startIndex: totalCount - currentRange.endIndex,
            endIndex: totalCount - currentRange.startIndex,
        };
        return {
            inverted: true,
            invertedRange,
