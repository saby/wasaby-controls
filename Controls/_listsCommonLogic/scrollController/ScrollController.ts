...
export interface IItemsRangePartial {
    startIndex: number;
...
export type IIndexesInitializedCallback = (
    range: IIndexesChangedParams<IAbstractCalculatorState>
) => void;
...
export class ScrollController {
    ...
    shouldDoScroll(): boolean {
        ...
        return (
            viewportOverlaySize !== null &&
            this._viewportSize - viewportOverlaySize < this._listContainerSize
        );
    }
    ...
    updatePlaceholderAfterContent(): void {
        if (this._placeholderAfterContent) {
            ...
            if (listContainer) {
                const resizer: HTMLElement | null = listContainer.querySelector(
                    PLACEHOLDER_AFTER_CONTENT_SELECTOR
                );
                const lastItemSize =
                ...
                if (typeof lastItemSize?.key === 'undefined') {
                ...
                if (lastItem) {
                    const viewport =
                        this._viewportSize -
                        (this._itemsSizesController.getViewportOverlaySize() ?? 0);
    ...
    getFirstVisibleItemIndex(): number {
        ...
        return this._calculator.getFirstVisibleItemIndex(stickyContentSize ?? undefined);
    }
    ...
    isItemsContainersAlive() {
        const itemsContainer = this._itemsSizesController.getItemsContainer();
        return itemsContainer && itemsContainer.parentElement;
    }
    ...
    updateContentSizeBeforeList(): boolean {
        const contentSizeBeforeList = this._itemsSizesController.getContentSizeBeforeList() ?? 0;
     ...
     * @param params
     */
    getEdgeVisibleItem(params: IEdgeItemCalculatingParams): IEdgeItem | null {
    ...
    getInvertedIndexes(index: number, offsetBeforeItem: number): TInvertedIndexes {
        return this._calculator.getInvertedIndexes(index, offsetBeforeItem);
    }
    ...
    updateMinMaxIndexes(minIndex?: number, maxIndex?: number): void {
        if (this._calculator instanceof Calculator) {
    ...
    private _observersCallback(direction: IDirection): void {
        ...
        let triggerOffsets: ITriggersOffsets;
    ...
    private _processCalculatorResult(
        result: IAbstractCalculatorResult,
        scrollMode: IScrollMode | null
    ): void {
        if (this._isCalculatorResult(result)) {
    ...
    private _handleInitializingResult(result: IAbstractCalculatorResult): void {
    ...
    private _isCalculatorResult(result: IAbstractCalculatorResult): result is ICalculatorResult {
        return [
