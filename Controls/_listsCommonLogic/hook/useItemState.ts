...
import { CollectionItemContext, ICollectionItemContextValue } from '../CollectionItemContext';

/**
 * Отследиваемое состояние записи
 * @variant marked Запись отмечена маркером
 * @variant selected Запись отмечена при помощи чекбокса
 * @typedef ItemState
 */
export type ItemState = 'marked' | 'selected' | string;

/**
 * Объект текущего состояния записи, возвращаемое из хука useItemState.
 * @typedef ItemStates
 */
export type ItemStates = Record<ItemState, boolean | null>;
...
/*
 * Внутренний платформенный хук, который вызывает перерисовку при изменении отслеживаемых состояний записи коллекции
...
export function useItemState(watchedStates: ItemState[]): ItemStates {
    ...
    return getItemState(item as CollectionItem);
}
