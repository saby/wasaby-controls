...
export function getRenderHandlers<TViewCommandHandlers extends IAbstractViewCommandHandlers>(
    ...
    const abstract: IAbstractViewCommandHandlers = {
        ...
        onViewTriggerVisibilityChanged(position, state) {
            const { isLatestInteractorVersion, loading } = viewModelState.current;
...
function onKeyDownSpace(
    ...
    if (typeof viewModelState.markedKey !== 'undefined' && viewModelState.markedKey !== null) {
        viewModelAPI.select(viewModelState.markedKey, event.shiftKey ? 'backward' : 'forward');
    }
}
...
function onKeyDownBackSpace(
    ...
    if (typeof backButtonItem !== 'undefined') {
