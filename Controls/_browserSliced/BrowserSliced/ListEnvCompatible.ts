...
import { CompatibleListSlice } from 'Controls/dataFactory';
...
interface IListEnvCompatibleProps extends IControlOptions {
    ...
    slices: Record<string, CompatibleListSlice>;
    useStore?: boolean;
    operationsPanelExpanded?: boolean;
}

interface IListConfig extends IListState {
    listStoreId: string;
}

interface IContextState {
    listsConfigs: Record<string, IListConfig>;
    [key: string]: unknown;
}
...
export default class ListEnvCompatible extends Control<IListEnvCompatibleProps> {
    ...
    _contextState: IContextState;

    protected _beforeMount(options: IListEnvCompatibleProps): Promise<void> | void {
        const contextState: IContextState = {
            listsConfigs: {},
        };
        this._eachSlice((slice, sliceName) => {
            Object.assign(contextState, slice.state.sourceController?.getState());
            contextState[sliceName] = slice;
            contextState.listsConfigs[sliceName] = {
                ...slice.state,
                listStoreId: sliceName,
            };
            contextState.filterController = slice.state.filterController;
        }, options);
            ...
            store.onPropertyChanged('filterSource', (data: unknown) => {
                ...
                if (filterSource || !isEqual(filterSource, sliceState.filterDescription)) {
                    ...
                    if (
                        ...
                        this._filterItemsChanged(new Event('custom'), filterSource);
                    }
                }
            }),
            store.onPropertyChanged('itemsOrderChanged', (itemsOrder) => {
                this._eachSlice((slice) => {
                    slice.setState({
    ...
    _eachSlice(
        callback: (slice: CompatibleListSlice, sliceName: string) => void,
        options = this._options
    ): void {
        Object.keys(options.slices).forEach((sliceName) => {
            callback(options.slices[sliceName], sliceName);
        });
    }

    protected _search(_e: unknown, value: string): void {
        this._eachSlice((slice) => {
            if (slice.state.searchParam) {
                slice.search(value);
            }
        });
    }
    ...
    protected _filterItemsChanged(_e: Event, filterDescription: IFilterItem[]): void {
        this._eachSlice((slice) => {
            if (slice.state.filterDescription) {
                slice.applyFilterDescription(filterDescription);
            }
        });
    }
}
