...
export default class View extends ListView {
    ...
    protected _getItemsContainerClasses(): string {
        let classes =
            'controls-ListViewV controls-TileView controls-AdaptiveTileView_itemsContainer';
        if (this._listModel.hasFreeSpace()) {
            classes += ' tw-content-start';
        }
        return classes;
    }

    protected _getTriggerClasses(direction: string): string {
        return (
            'controls-AdaptiveTile__loadingTrigger_horizontal ' +
            `controls-AdaptiveTile__loadingTrigger_horizontal_${direction}`
        );
    }
