    ...
    implements ICrud
{
    ...
    private _updateHistory(data: any, meta: any): any {
        ...
        if (meta.params) {
            ...
            resultPromise = this._callQuery(
                this._getMethodNameByIdType('Add', 'AddInt', id),
                {
    ...
    private _addFromData(data: {
        items: unknown;
        historyParams: unknown;
    }): unknown {
        if (this._$dataLoaded && this._$historyIds) {
            return this._getHistoryDataSource().call(
                'AddFromDataAndSetParams',
                {
                    history_id: this._$historyId,
                    data: data.items,
    ...
    query(query?: Query): Promise<DataSet> {
        ...
        historyIds.forEach((id) => {
            const data = DataStorage.read(id);
            if (data) {
        ...
        if (storageDef) {
            ...
            storageDef.addBoth((dataSet) => {
                 ...
                 */
                resultDef.callback(
                    this.getHistory(historyId) ? getHistoryDataSet() : dataSet
                );
            });
        } else if (
            !storageDef &&
            (!storageData ||
                (historyIds.length &&
                    storageDataParams.length !== historyIds.length))
        ) {
            /*
              В isDesktop нет сервиса истории и его там нельзя вызывать, в таком случае работаем без истории вообще.
              FIXME: https://online.sbis.ru/opendoc.html?guid=f0e4521b-873a-4b1a-97fe-2ecbb12409d1
