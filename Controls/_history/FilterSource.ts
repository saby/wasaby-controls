...
import { adapter, Model, Record, OptionsToPropertyMixin } from 'Types/entity';
import { Serializer } from 'UI/State';
import { factory } from 'Types/chain';
import { isEqual } from 'Types/object';
import * as clone from 'Core/core-clone';
import HistoryService from 'Controls/_history/Service';
...
const _private = {
    ...
    getRawItem(item: Model, format: object) {
        const rawData = {
    ...
    fillItems(history: object, items: object, historyType: string, checkCallback: Function) {
        let item;
    ...
    fillRecent(self: typeof Source, history: object, items: object) {
        ...
        _private.fillItems(history.recent, items, 'recent', (item) => {
            ...
            return (
                !isPinned && currentCount <= maxLength && item.get('ObjectData') !== DEFAULT_FILTER
            );
        });
    },
    ...
    deleteClient(self: typeof Source, item: Model) {
    ...
    addClient(self: typeof Source, item: Model) {
    ...
    updatePinned(self: typeof Source, item: Model, meta: object) {
    ...
    updateDataItem(item: Model, ObjectData: object) {
        item.set('ObjectData', ObjectData);
    },

    deleteHistoryItem(self: typeof Source, history: object, key: string | number) {
    ...
    updateRecent(self: typeof Source, item: Model) {
        const id = item.getId();
    ...
    getRawHistoryItem(self: typeof Source, id: string | number, objectData: object, hId?: string) {
        return new Model({
            rawData: {
    ...
    findItem(self: typeof Source, items: object[], data: object) {
        ...
        items.forEach((element) => {
            ...
            let deserializedData =
                objectData && JSON.parse(objectData, _private.getSerialize().deserialize);
...
const Source = CoreExtend.extend([OptionsToPropertyMixin], {
    _history: null,
    _serialize: false,
    '[Types/_source/ICrud]': true,

    constructor: function Memory(cfg: object) {
        const historySource = cfg.historySource;
        this.originSource = cfg.originSource;
        this.historySource = historySource;

        if (historySource && historySource instanceof HistoryService) {
            const history = historySource.getHistory(historySource.getHistoryId());
    ...
    update(data: Model, meta: object): Promise<unknown> {
        let serData;
        let item;
    ...
    query(query: sourceLib.Query) {
        ...
        if (where && where.$_history === true) {
            const prepareHistory = () => {
                ...
                _private.saveHistoryWithParams(this.historySource, this._history);
