...
const HISTORY_META_FIELDS: string[] = [
    '$_favorite',
    '$_pinned',
    '$_history',
    '$_addFromData',
    ...
    implements ICrud
{
    ...
    private _getSourceByMeta(
        ...
        const hasHistoryFields = Object.keys(meta).some(
    ...
    private _initHistory(
        ...
        if (data instanceof DataSet) {
            ...
            const pinned = this._prepareHistoryItems(
                row.get('pinned'),
                sourceItems
            );
            const recent = this._prepareHistoryItems(
                row.get('recent'),
                sourceItems
            );
            const frequent = this._prepareHistoryItems(
                row.get('frequent'),
                sourceItems
            );
            ...
            if (this._$pinned instanceof Array) {
                this._$pinned.forEach((pinId: string): void => {
                    if (
                        sourceItems.getRecordById(pinId) &&
                        !this._$history.pinned.getRecordById(pinId)
                    ) {
                        this._$history.pinned.add(
                            this._getRawHistoryItem(
                                pinId,
    ...
    private _getFilterHistory(rawHistoryData: any): any {
        ...
        const frequentIds = this._getFrequentIds(
            rawHistoryData.frequent,
            pinnedIds
        );
        const recentIds = this._getRecentIds(
            rawHistoryData.recent,
            pinnedIds,
            frequentIds
        );
    ...
    private _getRecentIds(
        ...
        const maxCountRecent =
            Constants.MAX_HISTORY -
            (filteredPinned.length + filteredFrequent.length);
        let item;
        let id;
    ...
    private _prepareOriginItems(
        ...
        oldItems.forEach((item: Model): void => {
            ...
            if (
                historyItem === -1 ||
                (item.get(this._$parentProperty) &&
                    item.get(this._$parentProperty) !== this._$root)
            ) {
                newItem = new Model({
                ...
                if (
                    filteredHistory.pinned.indexOf(id) !== -1 ||
                    filteredHistory.recent.indexOf(id) !== -1 ||
                    filteredHistory.frequent.indexOf(id) !== -1
                ) {
                    this._setHistoryFields(newItem, COPY_ORIG_ID, id);
                } else {
        ...
        return new Promise((resolve) => {
            setTimeout(() => {
                ...
                if (recentData) {
                    ...
                    if (
                        this._$historyItems &&
                        !this._updateRecentInItems(recentData)
                    ) {
                        this._$historyItems = null;
                    }
            ...
            return pd
                ...
                .addBoth((data) => {
                    ...
                    if (!isCancelled && data[1] && !this._isError(data[1])) {
                        ...
                        const loadedItems = data[1].getAll
                            ? data[1].getAll()
                            : data[1];
                        ...
                        if (data[0] && !this._isError(data[0])) {
                            ...
                            newItems = this._getItemsWithHistory(
                                this._$history,
                                this._$oldItems
                            );
                            this._$historySource.saveHistory(
