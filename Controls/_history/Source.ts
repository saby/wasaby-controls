    ...
    implements ICrud
{
    ...
    private _resetHistoryFields(historyItem: Model, keyProperty: string): Model {
    ...
    private _needDuplicateItem(item: Model): boolean {
        ...
        return (
            ...
            this._isHiddenNode(item)
        );
    }

    private _isHiddenNode(item: Model): boolean {
        return this._$nodeProperty && item.get(this._$nodeProperty) === false;
    }
            ...
            if (oldItem) {
                ...
                if (isNeedDuplicate) {
                    ...
                    if (this._isHiddenNode(item)) {
                        ...
                        item.set(this._$nodeProperty, null);
                    }
                } else {
    ...
    private _getRawHistoryItem(id: string, hId: string): Model {
        const item = new Model({ adapter: this._$history.recent.getAdapter() });
        item.addField({
            name: 'ObjectId',
            type: 'string',
        });
    ...
    _getHistoryIdForStorage(): string {
        return (
            this._$historySource.getHistoryId() ||
