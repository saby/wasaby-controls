...
interface IFormValidate {
    result: boolean;
    control: ValidateContainer;
    areaKey: number | string | null;
}

interface IFormValidateResult {
    [index: number]: IFormValidate | boolean;
}
...
class ControllerClass {
    ...
    submit(config: IControllerConfig = {}): Promise<IValidateResult | Error> {
        ...
        this._validates.forEach((validate: ValidateContainer) => {
            if (!validate._destroyed && !(validate._options && validate._options.readOnly)) {
            ...
            .then((results: IValidateResult) => {
                ...
                for (key in this._validates) {
                    if (
                        ...
                        !this._validates[key]._destroyed &&
    ...
    getFormValidationResult() {
        const results: IFormValidateResult = {};
        let i: number = 0;
        this._validates.forEach((validate: ValidateContainer) => {
            const res = validate.isValid();
            if (res) {
                results[i++] = res;
            } else {
                results[i++] = {
    ...
    protected _activateValidator(control: ValidateContainer): void {
