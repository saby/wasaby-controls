...
import type { TSelectedKeys } from 'Controls/interface';

export interface IValidateSelectionContainerOptions extends IValidateContainerOptions {
    validateOnFocusOut?: boolean;
    selectedKey?: string;
    onSelectedKeysChanged?: (value: string) => void;
}
 ...
 * @public
 */
class Selection extends Container<IValidateSelectionContainerOptions> {
    ...
    _selectedKeysChangedHandler(
        event: Event,
        value: string,
        added: TSelectedKeys,
        removed: TSelectedKeys
    ): void {
        this._notify('selectedKeysChanged', [value, added, removed]);
        this._shouldValidateByFocusOut = true;
        this._cleanValid();
    }
    ...
    _selectedKeyChangedHandler(
        ...
        this._notify('selectedKeyChanged', [value, added, removed]);
        this._shouldValidateByFocusOut = true;
        this._cleanValid();
    }

    _valueChangedHandler(event, value): void {
        this._notify('valueChanged', [value]);
        this._cleanValid();
    }

    _selectorCallback(): void {
        this._cleanValid();
    }
