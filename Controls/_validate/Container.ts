 ...
 * @public
 */
class ValidateContainer<T extends IValidateContainerOptions = IValidateContainerOptions> extends Control<T> {
    ...
    setDisableOwnInfobox(disableOwnInfobox: boolean) {
        this._disableOwnInfobox = disableOwnInfobox;
    }
    ...
    private _openInfoBox(): void {
        if (!this._destroyed && !this._disableOwnInfobox) {
            this._clearCloseId();
            if (
