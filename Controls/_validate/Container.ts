...
import { IValidator } from 'Controls/_validate/interfaces/Validator';
...
/**
 * @event validateFinished Происходит после заверешения валидации контейнера.
...
> extends Control<T> {
    ...
    validate(validateConfig?: IValidateConfig): Promise<string[] | null> {
        return new Promise((resolve) => {
            ...
            if (
                !validateConfig ||
                (validateConfig && validateConfig.hideIndicator !== true)
            ) {
    ...
    private _getGlobalPopup(): unknown {
        ...
        if (requirejs.defined(ManagerWrapperControllerModule)) {
            return requirejs(
                ManagerWrapperControllerModule
            ).default.getGlobalPopup();
        }
    }
    ...
    _isValidResult(): boolean {
        return (
            this._validationResult &&
            !(this._validationResult instanceof Promise)
        );
    }
...
/**
 * @name Controls/_validate/Container#validators
 * @cfg {Array.<Controls/validate:TValidator.typedef>|Controls/validate:TValidator.typedef} Функция (или массив функций) валидации.
 */
