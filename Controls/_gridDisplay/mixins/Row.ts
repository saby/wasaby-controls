...
import { getRowComponentProps } from '../utils/RowComponent';
import { COLUMN_SCROLL_SELECTORS } from 'Controls/_gridDisplay/constants';
...
export interface IInitializeColumnsOptions {
    prepareStickyLadderCellsStrategy?: 'add' | 'colspan' | 'offset' | string;
...
export default abstract class Row<T extends Model = Model> {
    ...
    // Мы сохраняем результат запроса getRowProps на модели.
    // Этот метод позволит получить их в неизменном виде в утилитах грида.
    getRowProps() {
    ...
    // TODO used in Controls/_baseList/Render/MultiSelectTemplate.tsx,
    //  Controls/_baseList/Render/MultiSelectCircleTemplate.tsx
    getMultiSelectClasses(
        ...
        classes += ` js-controls-ListView__checkbox ${COLUMN_SCROLL_SELECTORS.NOT_DRAG_SCROLLABLE}`;
        ...
        if (this.getMultiSelectVisibility() === 'onhover' && this.isSelected() === false) {
            classes += ' controls-GridView__checkbox-onhover';
        }
        ...
        if (!this.shouldWrapInScrollGroup()) {
            classes += ' controls-GridView__checkbox_relative tw-relative tw-float-left';
        }

        if (this.getOwner().getMultiSelectPosition() === 'custom') {
    ...
    // Это отличается от hasMultiSelectColumn тем, что проверяет свои значения, а не owner.
    // Нужно для проверки наличия колонки checkbox до и после setMultiSelectVisibility
    protected _hasCheckboxCell(): boolean {
        return (
    ...
    protected _setMultiSelectVisibility(
        isChangedMultiSelectVisibility: boolean,
        hadCheckBoxCellBeforeChange: boolean
    ): boolean {
        if (isChangedMultiSelectVisibility) {
            this._reinitializeColumns();

            // update editing cell index
            if (this.isEditing() && this.getEditingConfig()?.mode === 'cell') {
                const hasCheckBoxCell = this._hasCheckboxCell();
                if (hadCheckBoxCellBeforeChange !== hasCheckBoxCell) {
    ...
    getStickyLadderCellsCount(): number {
        const stickyProperties = this.getStickyLadderProperties(
            (this._$gridColumnsConfig || this._$columnsConfig)[0]
        );
    ...
    getLadderMode(): 'visibility' | 'display' {
        return this._$ladderMode;
    }
    ...
    getLadderWrapperClasses(ladderProperty: string, stickyPropertyProp: string): string {
        let ladderWrapperClasses = 'controls-Grid__row-cell__ladder-content';
        const ladder = this.getLadder();
        const stickyProperty = this.isFullGridSupport() ? stickyPropertyProp : undefined;
        const stickyLadder = this.isFullGridSupport() ? this.getStickyLadder() : undefined;
        const stickyProperties = this.isFullGridSupport()
            ? this.getStickyLadderProperties(this._$columnsConfig[0])
            : undefined;
        const index = stickyProperties?.indexOf(stickyProperty);
        ...
        if (
            ...
            ladderWrapperClasses += ' tw-hidden';
        }
    ...
    updateLadder(
        ...
        if (this._$ladder !== newLadder) {
            ...
            const needNotify = this._$ladder === undefined || this._$columnItems;
    ...
    private isHiddenOnThinInteractor() {
        return (
            !!this.contents &&
            !!this.contents.get &&
            this.contents.get('CollectionItemKey<module>') === 'HasMoreFooter'
        );
    }
    ...
    protected _prepareColumnItems(
        ...
        shiftFixedColumnIndexWhenMultiselect: boolean = false
    ): Cell[] {
        ...
        for (
            ...
            const isFixed = this.hasColumnScroll()
                ? (skipColumns
                      ? columnIndex - +shiftFixedColumnIndexWhenMultiselect
                      : resultTotalLength - 1) < this.getStickyColumnsCount()
                : false;
            ...
            if (options.shouldAddMultiSelectCell !== false && this.hasMultiSelectColumn()) {
                ...
                if (this._$columnsConfig?.[0]?.getCellProps) {
                    columnConfig.getCellProps = () => {
                        ...
                        contents =
                            contents instanceof Array ? contents[contents.length - 1] : contents;
                        const cellProps = this._$columnsConfig[0].getCellProps(contents);
                        return {
    ...
    protected _getLeftSeparatorSizeForColumn(
        ...
        if (columnIndex > 0) {
            const columns = this._$owner.getGridColumnsConfig();
