...
import { TColspanCallback, TResultsColspanCallback } from '../interface/IGridControl';
...
// Опции, совместимые со старым API грида
export interface IGridCompatibleOptions {
...
export interface IGridColumnScrollOptions {
...
export interface IOptions
    extends IGridProps,
        ICollectionOptions,
        IGridCompatibleOptions,
        IGridColumnScrollOptions,
        ITrackedProperties {
...
> {
    ...
    // TODO: Флаг добавлен чтобы отличать, когда прикладник указал emptyTemplateColumns, а когда emptyView
    // Решает проблему: https://online.sbis.ru/opendoc.html?guid=b9e56a9f-9a8a-4b39-bcee-a1c24064974f&client=3
    protected _$hasEmptyView: Boolean;
    protected _$results: ResultsRow;
    protected _$ladder: ILadderObject;
    protected _resizer: ColumnResizer;
    ...
    protected _$dragScrolling: boolean;
    ...
    static initMixin(instance, options: IOptions) {
        ...
        instance._$subPixelArtifactFix = options.subPixelArtifactFix;
        ...
        if (options.emptyView) {
            ...
            instance._$hasEmptyView = true;
        }

        if (instance._$resizerVisibility) {
            const stickyColumnsCount = instance.getStickyColumnsCount();
            const columnConfig = instance.getGridColumnsConfig()[stickyColumnsCount - 1];

            instance._resizer = new ColumnResizer({
                maxWidth: parseInt(columnConfig.maxWidth, 10),
                minWidth: parseInt(columnConfig.minWidth, 10),
                resizerOffsetCallback: instance._$resizerOffsetCallback,
                width: parseInt(columnConfig.width, 10),
            });
        }
    ...
    setColumnsWidths(columnsWidths: string[]): void {
        ...
        this._resizer?.setWidth(parseInt(columnsWidths[this.getStickyColumnsCount() - 1], 10));
        this._nextVersion();
    }
    ...
    getResultsConfig(): IResultConfig[] {
        return this._$resultsConfig;
    }

    getHasEmptyView(): Boolean {
        return this._$hasEmptyView;
    }
    ...
    getColumnWidths(isOnlyFixedColumns: boolean = false): string[] {
        ...
        // Если не задали колонки - не нужно падать тут, мы отвалимся на валидации с человеко-понятной ошибкой
        // https://online.sbis.ru/opendoc.html?guid=08f777b8-64ff-4eac-842e-cd45dc68f5b9
        if (!gridColumnsConfig) {
            return [];
        }

        const columns = !isOnlyFixedColumns
            ? gridColumnsConfig
            : gridColumnsConfig.filter((_, index) => {
    ...
    getHeader(): Header {
        if (!this._$headerModel && this._headerIsVisible(this._$header, this._$headerVisibility)) {
            this._initializeHeader({
                ...
                subPixelArtifactFix: this._$subPixelArtifactFix,
    ...
    hasResizer(): boolean {
        const hasResizerByOptions = this._$resizerVisibility && this.hasColumnScroll();
    ...
    setResizerVisibility(isVisible: boolean): void {
        if (this._$resizerVisibility !== isVisible) {
            ...
            const stickyColumnsCount = this.getStickyColumnsCount();
            const columnConfig = this.getGridColumnsConfig()[stickyColumnsCount - 1];

            this._resizer = new ColumnResizer({
                maxWidth: parseInt(columnConfig.maxWidth, 10),
                minWidth: parseInt(columnConfig.minWidth, 10),
    ...
    setColumns(newColumns: TColumns, silent: boolean = true): void {
        ...
        this.getHeader()?.setGridColumnsConfig(this._$columns);
        this.getResults()?.setGridColumnsConfig(this._$resultsConfig || this._$columns);
        this.getFooter()?.setGridColumnsConfig(this._$columns);
        this.getEmptyGridRow()?.setGridColumnsConfig(this._$columns);
    ...
    // Общий код setEmptyTemplate в tree | treeGrid
    protected _setEmptyTemplate(setBaseEmptyTemplateResult: boolean): boolean {
        if (setBaseEmptyTemplateResult) {
        ...
        return setBaseEmptyTemplateResult;
    }
    ...
    protected _initializeEmptyRow(): void {
        this._$emptyGridRow = new EmptyRow({
            ...
            rowTemplateOptions: { ...this._$emptyTemplateOptions, _$isEmptyTemplate: true },
    ...
    setEmptyView(emptyView: IGridAbstractColumn[]): boolean {
        if (Array.isArray(emptyView)) {
            this.setEmptyTemplateColumns(emptyView);
            return true;
        }
    ...
    setResultsVisibility(resultsVisibility: TResultsVisibility): void {
        if (this._$resultsVisibility === resultsVisibility || resultsVisibility === undefined) {
            return;
        }
    ...
    protected _initializeHeader(options: IOptions): void {
        const cOptions = {
            ...
            resizerOffsetCallback: this._$resizerOffsetCallback,
            getRowProps: null,
            itemsContainerPadding: this._$itemsContainerPadding,
        };
    ...
    protected _initializeFooter(options: Partial<IOptions>): FooterRow {
        ...
        return new FooterRow({
            ...
            rowTemplateOptions: { _$isFooterTemplate: true },
    ...
    // Общий код setMultiSelectVisibility в tree | treeGrid
    ...
    setColumnScroll(columnScroll: boolean = false): void {
    ...
    setColumnScrollViewMode(newColumnScrollViewMode: TColumnScrollViewMode): void {
        if (this._$columnScrollViewMode !== newColumnScrollViewMode) {
            ...
            if (this.getResults()) {
                this.getResults().setColumnScrollViewMode(newColumnScrollViewMode);
            }
        }
    }
    ...
    setStickyColumnsCount(
        ...
        columnsLength: number = this._$columns.length
    ): void {
        if (
            startStickyColumnsCount >= this._$columns.length ||
            endStickyColumnsCount >= this._$columns.length
        ) {
            // В случае, когда setColumns вызывается после загрузки данных мы попадем в
            // setStickyColumnsCount, а в this._$columns будут старые колонки. В качестве columnsLength передаем
            // число колонок из newOptions. Проверка нужна внутри if так как, нам не нужно применять изменения
            // stickyColumnsCount до изменения this._$columns.
            if (
                columnsLength === this._$columns.length ||
                startStickyColumnsCount >= columnsLength ||
                endStickyColumnsCount >= columnsLength
            ) {
                executeSyncOrAsync(['Controls/listErrors'], (errs) =>
                    errs.InvalidStickyColumnsCountValueError(this)
                );
            }
            return;
        }
    ...
    hasItemActionsSeparatedCell(): boolean {
        return !!this.getGridColumnsConfig() && this.isStickyItemActions();
    }
    ...
    setResultsTemplateOptions(options: { [key: string]: unknown }): void {
        if (!isEqual(this._$resultsTemplateOptions, options)) {
    ...
    _updateInteractorGridStateProps(
        interactorStateProps: Pick<IInteractorStateProps, 'columns' | 'header'>
    ): boolean {
        let shouldIncrementVersion: boolean = true;
        if (
            'columns' in interactorStateProps &&
            this.interactorStateProps.columns !== interactorStateProps.columns
        ) {
            shouldIncrementVersion = true;
            this.interactorStateProps.columns = interactorStateProps.columns;
            this.setColumns(interactorStateProps.columns, true);
        }

        if (
            'header' in interactorStateProps &&
            this.interactorStateProps.header !== interactorStateProps.header
        ) {
            shouldIncrementVersion = true;
            this.interactorStateProps.header = interactorStateProps.header;
            this.setHeader(interactorStateProps.header);
        }

        if (
            'getRowProps' in interactorStateProps &&
            this.interactorStateProps.getRowProps !== interactorStateProps.getRowProps
        ) {
            this.interactorStateProps.getRowProps = interactorStateProps.getRowProps;
            this.setGetRowPropsCallback(interactorStateProps.getRowProps);
        }

        if (
            this.interactorStateProps.isLatestInteractorVersion &&
            'selectionViewMode' in interactorStateProps &&
            this.interactorStateProps.selectionViewMode !== interactorStateProps.selectionViewMode
        ) {
            this.interactorStateProps.selectionViewMode = interactorStateProps.selectionViewMode;
            switch (interactorStateProps.selectionViewMode) {
                case 'selected': {
                    this.setResultsVisibility('hidden');
                    break;
                }
            }
        }
        ...
        if (
            ...
            this.interactorStateProps.emptyView = interactorStateProps.emptyView;
            this.setEmptyView(interactorStateProps.emptyView as IGridAbstractColumn[]);
        }
...
/**
 * @interface Controls/_gridDisplay/mixins/Grid/IEmptyTemplateColumn
 * @public
 * @description
...
/**
 * @name Controls/_gridDisplay/mixins/Grid/IEmptyTemplateColumn#template
 * @cfg {TemplateFunction} Шаблон колонки пустого представления
 */

/**
 * @name Controls/_gridDisplay/mixins/Grid/IEmptyTemplateColumn#startColumn
 * @cfg {Number} Начальный индекс колонки.
 */

/**
 * @name Controls/_gridDisplay/mixins/Grid/IEmptyTemplateColumn#endColumn
