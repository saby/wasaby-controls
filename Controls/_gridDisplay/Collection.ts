...
import type { TItemsOrder } from 'Controls/interface';
...
export function setEmptyTemplate(superResult: boolean, thisArg: Collection): boolean {
    if (superResult) {
        if (thisArg._$emptyTemplate) {
            if (thisArg._$emptyGridRow) {
                thisArg._$emptyGridRow.setRowTemplate(thisArg._$emptyTemplate);
            } else {
                thisArg._initializeEmptyRow();
            }
        } else {
            thisArg._$emptyGridRow = undefined;
        }
    }
    return superResult;
}
...
>(BaseCollection, GridMixin) {
    ...
    setOrder(order: TItemsOrder): void {
        super.setItemsOrder(order);

        // TODO: ### поправить обновление колонок
...
Object.assign(Collection.prototype, {
    ...
    '[Controls/grid:GridCollection]': true,
