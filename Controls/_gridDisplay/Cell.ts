...
import type { IDataTypeRenderProps } from './interface/IDataTypeRenderProps';
import type {
    ICellComponentProps,
    ICellProps,
...
import { StickyHorizontalPosition } from 'Controls/stickyBlock';
import { getCursor } from './utils/Cursor';
import { loadSync } from 'WasabyLoader/ModulesLoader';
import { COLUMN_SCROLL_SELECTORS } from 'Controls/_gridDisplay/constants';
import { isOldBrowser } from 'Controls/_gridDisplay/utils/isOldBrowser';
    ...
    implements IInstantiable, IVersionable
{
    ...
    getCellContentRender(): React.FunctionComponent<IDataTypeRenderProps<unknown>> {
        ...
        switch (this._$column.displayType) {
            case 'money':
                return loadSync<typeof import('Controls/grid:MoneyTypeRender')>(
                    'Controls/grid:MoneyTypeRender'
                );
            case 'number':
                return loadSync<typeof import('Controls/grid:NumberTypeRender')>(
                    'Controls/grid:NumberTypeRender'
                );
            case 'date':
                return loadSync<typeof import('Controls/grid:DateTypeRender')>(
                    'Controls/grid:DateTypeRender'
                );
            default: {
                if (
                    ...
                    return loadSync<typeof import('Controls/grid:HighlightedTypeRender')>(
                        'Controls/grid:HighlightedTypeRender'
                    );
                }

                return loadSync<typeof import('Controls/grid:StringTypeRender')>(
                    'Controls/grid:StringTypeRender'
                );
            }
        }
    }
    ...
    protected _getRoundBorder(roundBorderProps: IRoundBorder): IRoundBorder {
        ...
        const setBorder = (corner: keyof IRoundBorder, propValue: any) => {
            roundBorder[corner] = propValue || this._$roundBorder?.[corner] || null;
        };

        if (this.isFirstColumn()) {
            setBorder('tl', roundBorderProps?.tl);
            setBorder('bl', roundBorderProps?.bl);
        }

        if (this.isLastColumn()) {
            setBorder('tr', roundBorderProps?.tr);
            setBorder('br', roundBorderProps?.br);
        }
    ...
    protected _getWrapperBaseClasses(templateHighlightOnHover: boolean): string {
        ...
        classes += ' tw-h-full tw-flex tw-box-border';
    ...
    protected _getWrapperSeparatorClasses(): string {
        ...
        if (rowSeparatorSize) {
            if (this._$isTopSeparatorEnabled) {
                classes += ` controls-GridReact-cell_topSeparatorSize-${rowSeparatorSize}`;
        ...
        const columnSeparatorSize = this.getColumnSeparatorSize();
        if (columnSeparatorSize) {
            classes += ` controls-Grid__columnSeparator_size-${columnSeparatorSize}`;
        }
        return classes;
    }

    getColumnSeparatorSize(): string | undefined {
        return this.getColumnIndex() > (this._$owner.hasMultiSelectColumn() ? 1 : 0) &&
    ...
    getCellComponentProps(rowProps: IRowComponentProps): ICellComponentProps {
        ...
        const isColumnScrollFixedCell =
            this.getOwner().hasColumnScroll() && (this._$isFixed || this._$isFixedToEnd);
        ...
        if (!backgroundStyle && this.getHiddenForLadder()) {
        ...
        if (isOldBrowser && !columnConfig.width && !this.isMultiSelectColumn()) {
            //В IE, если не задана ширина у колонки и эта колонка не чекбокс, нужно растянуть ячейку
            className += ' controls-GridReact-IE-cell-without-width ';
        }
        ...
        const roundBorder = this._getRoundBorder(rowProps.roundBorder);

        const isScrollable = this.getOwner().hasColumnScroll() && !this._$isFixed;
        ...
        return {
            ...
            isStickyLadderCell: Boolean(this.$GSLC),
            ...
            tagClassName: cellProps?.tagClassName,
