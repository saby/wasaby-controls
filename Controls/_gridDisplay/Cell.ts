    ...
    implements IInstantiable, IVersionable
{
        ...
        switch (this._$column.displayType) {
            ...
            default: {
                if (
                    ...
                    return loadSync<typeof import('Controls/grid:StringSearchTypeRender')>(
                        'Controls/grid:StringSearchTypeRender'
                    );
                }
    ...
    // TODO Used in EmptyCell & LadderCell
    getWrapperClasses(
        ...
        if (!hasColumnScroll) {
            wrapperClasses += ' tw-min-w-0 tw-max-w-full';
        }
    ...
    getContentClasses(
        ...
        contentClasses +=
            ' tw-w-full tw-inline-flex tw-items-baseline tw-min-w-0 tw-shrink tw-box-border tw-h-full';
        if (this.getOwner().isDragged()) {
            contentClasses += ' controls-ListView__itemContent';
        } else {
            if (cursor === 'default') {
                contentClasses += ' tw-cursor-default';
            }
            if (cursor === 'text') {
                contentClasses += ' tw-cursor-text';
            }
            if (cursor === 'pointer') {
                contentClasses += ' tw-cursor-pointer';
            }
        }
        ...
        if (this._$owner.isEditing()) {
            ...
            if (this._$owner.hasColumnScroll()) {
                contentClasses += ` ${COLUMN_SCROLL_SELECTORS.NOT_DRAG_SCROLLABLE}`;
            }
        }
    ...
    protected _getHorizontalPaddingClasses(cellPadding: ICellPadding): string {
        ...
        if (!this._$owner.hasMultiSelectColumn() && this.isFirstColumn()) {
            classes +=
                leftPadding === 'null'
                    ? ' tw-pl-0'
                    : ` controls-Grid__cell_spacingFirstCol_${leftPadding}`;
        ...
        if (!this.isLastColumn()) {
            ...
            classes +=
                rightPadding === 'null'
                    ? ' tw-pr-0'
                    : ` controls-Grid__cell_spacingLastCol_${rightPadding}`;
        }
    ...
    protected _getContentAlignClasses(): string {
        let classes = '';
        const align = this._$column?.align;
        const valign = this._$column?.valign;
        ...
        if (valign) {
            if (valign === 'top') {
                classes += ' tw-items-start tw-h-full';
            }
            if (valign === 'bottom') {
                classes += ' tw-items-end tw-h-full';
            }
            if (valign === 'center') {
                //Участвует в каскаде Controls/_baseTree/styles/_BaseTree.less:209
                classes += ' controls-Grid__cell_valign_center tw-items-center tw-h-full';
            }
        }
        return classes;
    }
    ...
    getCellComponentProps(rowProps: IRowComponentProps): ICellComponentProps {
        ...
        //TODO: Нужно будет вынести в одно место (Controls/_grid/gridReact/view/View.tsx:41)
        const isOldBrowser =
            detection.isIE || (detection.isNotFullGridSupport && !detection.safari);
