...
import { ColumnTemplate, IColumn, TColumns } from 'Controls/grid';
import { IExplorerOptions } from 'Controls/explorer';
import { Direction, TKeysSelection } from 'Controls/interface';
 ...
 * @interface Controls/_moverDialog/Template/IMoverDialogTemplateOptions
 * @public
 */
export interface IMoverDialogTemplateOptions extends IControlOptions, Partial<IExplorerOptions> {
    /**
     * @name Controls/_moverDialog/Template/IMoverDialogTemplateOptions#searchParam
    ...
    showRoot?: boolean;
    /**
     * @name Controls/_moverDialog/Template/IMoverDialogTemplateOptions#rootVisible
    ...
    rootVisible?: boolean;
    /**
     * @name Controls/_moverDialog/Template/IMoverDialogTemplateOptions#movedItems
     * @cfg {Controls/interface:TKeysSelection} Перемещаемые записи
     */
    movedItems: TKeysSelection;
    /**
     * @name Controls/_moverDialog/Template/IMoverDialogTemplateOptions#excludedItems
     * @cfg {Controls/interface:TKeysSelection} Исключённые записи
     */
    excludedItems?: TKeysSelection;
    /**
     * @name Controls/_moverDialog/Template/IMoverDialogTemplateOptions#headingCaption
     * @cfg {String} Заголовок окна перемещения.
     * @default 'Выбор раздела'
     */
    headingCaption?: string;
    /**
     * @name Controls/_moverDialog/Template/IMoverDialogTemplateOptions#rootTitle
     * @cfg {String} Заголовок корневой записи.
     */
    rootTitle: string;
    /**
     * @name Controls/_moverDialog/Template/IMoverDialogTemplateOptions#rootLabelVisible
...
/**
 * @name Controls/_moverDialog/Template/IMoverDialogTemplateOptions#displayProperty
...
/**
 * @name Controls/_moverDialog/Template/IMoverDialogTemplateOptions#root
 ...
 */
const ROOT_KEY: CrudEntityKey = 3141592653589793;
 ...
 * * {@link https://git.sbis.ru/saby/wasaby-controls/-/blob/rc-24.6100/Controls-default-theme/variables/_moveDialog.less переменные тем оформления}
 *
 ...
 * @implements Controls/_moverDialog/Template/IMoverDialogTemplateOptions
 *
 * @public
 */
...
export default class MoverDialogTemplate extends Control<IMoverDialogTemplateOptions> {
    ...
    protected _onItemClick(event: SyntheticEvent<MouseEvent>, item: Model): void {
        const isRoot = item.getKey() === ROOT_KEY || item.getKey() === `${ROOT_KEY}`;
        this._applyMove(isRoot ? this._options.root : item);
    }
    ...
    protected _setMinWidth(): void {
        const width = DimensionsMeasurer.getBoundingClientRect(this._container).width;
    ...
    protected _dataLoadCallback(items: RecordSet, direction?: Direction): void {
        if (!direction) {
            this._addRootRecordToRecordSet(items);
        }
    ...
    private _getRootRecord(items: RecordSet): Model {
        ...
        record.set({
            ...
            [keyProperty]: this._getRootKey(items),
            [this._columns[0].displayProperty]: this._options.rootTitle || rk('В корень'),
        });
        return record;
    }

    private _getRootKey(items: RecordSet): CrudEntityKey {
        const format = items.getFormat();
        if (format.getCount() === 0) {
            return ROOT_KEY;
        }
        const keyProperty = this._options.keyProperty || items.getKeyProperty();
        const keyPropIndex = format.getFieldIndex(keyProperty);
        const keyPropType = format.at(keyPropIndex).getType();
        return keyPropType === 'string' ? `${ROOT_KEY}` : ROOT_KEY;
    }
    ...
    private _search() {
        ...
        // Сбрасываем ширину контейнера, чтобы он расширился при поиске
        // На основе обращения https://online.saby.ru/opendoc.html?guid=0e2db9a1-d1d2-49d9-a821-0898cc4b1948&client=3
        this._resetWidth();
    }
...
/**
 * Происходит при выборе раздела для перемещения записей.
 * @event Controls/_moverDialog/Template#sendResult
...
/**
 * Происходит при закрытии диалога перемещения записей.
 * @event Controls/_moverDialog/Template#close
 * @param {UI/Events:SyntheticEvent} eventObject Дескриптор события.
 * @remark
 * Событие всплываемое (см. <a href="/doc/platform/developmentapl/interface-development/ui-library/events/">Работа с событиями</a>).
