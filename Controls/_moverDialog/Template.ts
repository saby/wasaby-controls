...
export interface IMoverDialogTemplateOptions
...
export default class MoverDialogTemplate extends Control<IMoverDialogTemplateOptions> {
    ...
    protected _afterMount(
        options?: IMoverDialogTemplateOptions,
        context?: object
    ): void {
        this._setMinWidth();
    }
    ...
    protected _afterUpdate(
        oldOptions?: IMoverDialogTemplateOptions,
        oldContext?: object
    ): void {
        this._setMinWidth();
    }
    ...
    protected _itemsFilterMethod(items: Model | Model[]): boolean {
        ...
        if (item.get) {
            result =
                this._options.movedItems.indexOf(
    ...
    private _getRootRecord(items: RecordSet): Model {
        ...
        record.set({
            ...
            [this._options.keyProperty]: ROOT_KEY,
            [this._columns[0].displayProperty]:
                this._options.rootTitle || rk('В корень'),
        });
        return record;
    }
    ...
    private _initColumns(options: IMoverDialogTemplateOptions): IColumn[] {
        ...
        if (options.rootVisible || options.showRoot) {
            ...
            const templateOptions: IMoverColumnTemplateOptions = {
                ...
                rootLabelVisible:
                    options.rootLabelVisible !== false && !!options.rootTitle,
                rootKey: ROOT_KEY,
            };

            if (!templateOptions.defaultColumnTemplate) {
                templateOptions.defaultColumnTemplate =
