...
export interface IImageDisplayContainerOptions extends IComponentProps {
...
export default class ImageDisplayContainer extends React.Component<IImageDisplayContainerOptions> {
    ...
    protected _updateHasImage(
        ...
        const isReset = this._resetHasItemWithImage();
        let isUpdate;
        if (items) {
            isUpdate = this._updateDisplayImage(
                this._items,
                imageProperty,
                imageCheckMode,
                nodeProperty,
        ...
        if (isReset || isUpdate) {
            this._updateHasItemWithImageState();
        }
    }
    ...
    private _resetHasItemWithImage(): boolean {
        const oldHasItemWithImage = this._hasItemWithImage;
    ...
    /*
     * Метод setState является асинхронным, его нельзя вызывать отдельно для reset и отдкльно для update,
     * т.к. в этом случае он вызывает в этом компоненте две крайне дорогие перерисовки.
     */
    private _updateHasItemWithImageState() {
    ...
    private _onCollectionItemChange(
        ...
        if (
            ...
            isReset = this._resetHasItemWithImage();
            isUpdate = this._updateDisplayImage(
                this._items,
                this._imageProperty,
    ...
    private _onCollectionChange(_event: EventObject, action: string): void {
        let isReset;
        let isUpdate;
        switch (action) {
