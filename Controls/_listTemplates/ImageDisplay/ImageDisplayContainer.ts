...
import { TMarkerSize, TreeItem } from 'Controls/display';
...
export default class ImageDisplayContainer extends Control<IImageDisplayContainerOptions> {
    ...
    /**
     * @name Controls/listTemplates:ImageDisplayContainer#itemTemplate
    ...
    /**
     * @name Controls/listTemplates:ImageDisplayContainer#imageViewMode
     * @cfg {String} Режим отображения изображения.
     */
    protected _imageViewMode: string;

    /**
     * @name Controls/listTemplates:ImageDisplayContainer#nodeImageViewMode
    ...
    protected _beforeMount(
        ...
        if (!options.sourceController) {
            Logger.error(
                'ImageDisplayContainer should be child of Browser or DataContainer',
                this
            );
        }
    ...
    private _updateListenOnCollectionChange(
        curItems: RecordSet,
        newItems: RecordSet
    ): boolean {
    ...
    private _subscribeToCollectionChange(
        items,
    ...
    private _unsubscribeToCollectionChange(
        items,
        onCollectionItemChange,
        onCollectionChange
    ) {
     ...
     * @private
     */
    protected _getImageViewMode(
        item: TreeItem,
        imageCheckMode: TImageCheckMode
    ): string {
        const imageViewMode =
            item.isNode && item.isNode()
                ? this._nodeImageViewMode
                : this._imageViewMode;
        // return (item.isNode && item.isNode()) || this._hasItemWithImage ? imageViewMode : 'none';
        return (imageCheckMode === 'leaf' && item.isNode && item.isNode()) ||
            this._hasItemWithImage
            ? imageViewMode
            : 'none';
    }
    ...
    protected _getMarkerSize(
        ...
        return isLeaf && hasImage && imageViewMode !== 'none'
            ? 'image-l'
            : undefined;
    }
    ...
    private _getPatchedColumns(): TColumns {
        ...
        if (!this._patchedColumns) {
            ...
            this._patchedColumns.forEach((column) => {
                const templateOptions: { imageViewMode?: string } =
