...
import {
    IArrowState,
    TArrowStateVisibility,
} from 'Controls/_paging/interface/IArrowState';
...
class Paging extends Control<IPagingOptions> {
    ...
    private _initArrowDefaultStates(config: IPagingOptions): void {
        if (config.arrowState) {
            if (config.pagingMode === 'numbers') {
                ...
                this._stateTop = this._getState(
                    config.arrowState.begin || 'readonly'
                );
                this._stateBackward = this._getState(
                    config.arrowState.prev || 'readonly'
                );
                this._stateForward = this._getState(
                    config.arrowState.next || 'readonly'
                );
                this._stateBottom = this._getState(
                    config.arrowState.end || 'readonly'
                );
                this._stateReset =
                    config.arrowState.reset === 'day' ||
                    config.arrowState.reset === 'home'
                        ? 'normal'
                        : 'disabled';
            }
        }
    ...
    private _isDigit(): boolean {
        return (
            this._options.showDigits || this._options.pagingMode === 'numbers'
        );
    }
