...
import { TDigitRenderCallback } from 'Controls/interface';
...
class DigitButtons extends Control<IDigitButtonsOptions> {
    ...
    protected _getDigitRender(index: number) {
        if (this._options.digitRenderCallback) {
            return this._options.digitRenderCallback(index) || index;
        }
        return index;
    }
