...
import { EditorDecorator } from 'Controls/_listVisualAspects/EditingComponent/EditorDecorator';
...
function ViewWrapper(
    ...
    if (props.ignoreWrappersForViewRender) {
        return props.viewRender;
    }

    const render = <div className={'controls-EditingTemplateText__inner'}>{props.viewRender}</div>;

    return props.highlightEditor !== 'none' ? (
        <EditorDecorator {...props} title={getTitle(props)}>
            {render}
        </EditorDecorator>
    ) : (
        render
        ...
        >
            {props.editing ? (
                ...
                <ViewWrapper
                    {...props}
                    className={''}
                    ignoreWrappersForViewRender={ignoreWrappersForViewRender}
                />
            )}
        </div>
    );
