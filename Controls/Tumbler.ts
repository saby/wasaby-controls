...
class Tumbler extends Control<ITumblerOptions> {
    ...
    constructor(args: ITumblerOptions) {
        super(args);
    ...
    private _getNextActiveItem(): Model {
        ...
        const thisActiveItem = this._options.items.getRecordById(
            this._selectedKey
        );
    ...
    protected _getButtonBackgroundStyle(): string {
        let style = `width:${
            this._backgroundPosition[this._selectedKey]?.width
        }px;`;
        if (this._options.direction !== 'vertical') {
            style += ` left:${
                this._backgroundPosition[this._selectedKey]?.left
            }px;`;
        } else {
            style += ` height:${
                this._backgroundPosition[this._selectedKey]?.height
            }px;`;
            style += ` top:${
                this._backgroundPosition[this._selectedKey]?.top
            }px;`;
        }
        return style;
    }
