...
function InnerItemTemplate(outerProps, props): JSX.Element {
    const innerClassName = !props.readOnly && !props.multiSelect ?
        ('controls-Selectorbutton_selectedItem_single_fontColorStyle-' + props.fontColorStyle) : '';
    const fontColorStyle = outerProps.fontColorStyle ||
                  !props.readOnly && !props.multiSelect ||
                  props.fontColorStyle !== 'link' ? props.fontColorStyle : 'default';
    const resultProps = {
...
function InnerContent(props: ISelectorButtonOptions, ref): JSX.Element  {
    const onItemClick = useCallback((item, event) => props.itemClickHandler(event, item), [props.itemClickHandler]);
    const onCrossClick = useCallback((item, event) => props.removeItemHandler(event, item), [props.removeItemHandler]);
    if (props.items.getCount()) {
        return (
            <>
                <SelectedCollection
                    ...
                    itemTemplate={(outerProps) => InnerItemTemplate(outerProps, props)}
                    className={`${!props.readOnly && props.multiSelect ? 'controls-Selectorbutton_selectedCollection_multiSelect': ''}`}
                    onItemClick={onItemClick}
                    onCrossClick={onCrossClick}
                    onOpenInfoBox={props.openInfoBoxHandler}
                    onCloseInfobox={props.closeInfoboxHandler}
                    customEvents={['onItemClick', 'onCrossClick', 'onOpenInfoBox', 'onCloseInfobox']}>
                </SelectedCollection>
                {props.multiSelect && !props.readOnly && props.showSelectorCaption ?
                    <span className={`controls-Selectorbutton_button-more 
                ...
                {(props.itemsCount > 1 || props.clearButtonTemplate) && !props.readOnly && props.showClearButton ?
        ...
        return (
            props.readOnly
                ? <span className={`${props.fontColorStyle === 'link' ?
                    'controls-Selectorbutton_button-select_readOnly' : 'controls-text-' + props.fontColorStyle}
                                   controls-fontsize-${props.fontSize} ws-ellipsis`}>
                    {props.caption}
                </span>
                : <Button className="controls-Selectorbutton_link"
                          viewMode="link"
...
function SelectorButtonRender(props: ISelectorButtonOptions, ref: ForwardedRef<HTMLDivElement>): JSX.Element {
    return (
        <div ref={ref}
             onClick={props.onClick}
             data-qa={props.attrs?.['data-qa']}
             className={`controls_lookup_theme-${props.theme} ${props.className}
                        controls-Selectorbutton ${props.items.getCount() === 0 ? ' controls-Selectorbutton_empty': ''}
                        ${props.validationStatus !== 'valid' ? ' controls-invalid-container' : ''}`}>
            <InnerContent { ...props } />
            {props.validationStatus !== 'valid'
                ? <div
                    className={`controls-invalid-border controls-${props.validationStatus}-border`}></div>
                : ''
            }
        </div>
    );
