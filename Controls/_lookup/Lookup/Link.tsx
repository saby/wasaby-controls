...
export interface ILinkOptions extends TInternalProps {
    ...
    underline?: string;
    caption: string;
    onLinkclick?: (e: React.KeyboardEvent<HTMLSpanElement>|React.MouseEvent<HTMLSpanElement, MouseEvent>) => void;
}

function Link(props: ILinkOptions): JSX.Element {
    const linkRef = React.useRef(null);

    const onClick = React.useCallback((e: React.MouseEvent<HTMLSpanElement, MouseEvent>) => {
        if (props.readOnly) {
            e.stopPropagation();
    ...
    const onKeyPress = React.useCallback((e: React.KeyboardEvent<HTMLSpanElement>) => {
    ...
    return (
        <span className={`controls_lookup_theme-${ props.theme } controls-Lookup__link
                         controls-Lookup__link-inlineheight-${ props.inlineHeight ? props.inlineHeight : 'default' }
            ...
            tabIndex={ -1 }
            data-qa="Lookup__link"
            ref={ linkRef }
            onClick={ onClick }
            onMouseDown={ onMouseDown }
            onKeyPress={ onKeyPress }>
            <span className={`controls-Lookup__link__text
