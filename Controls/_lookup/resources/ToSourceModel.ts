...
function getModel(sourceModel: string | typeof Model | Model, config: unknown): Model {
    ...
    if (items) {
        if (dataSource && instanceOfMixin(dataSource, 'Types/_source/ICrud')) {
            ...
            if (items instanceof RecordSet) {
                const itemsModel = getModel(items.getModel() as unknown as Model, {});

                if (itemsModel._moduleName !== dataSourceModelInstance._moduleName) {
