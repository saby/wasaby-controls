...
import { TKey, ISelectorDialogOptions } from 'Controls/interface';
...
export interface ILookupOptions
    ...
    items?: RecordSet;
    onItemsChanged?: (items: RecordSet) => void;
    onTextValueChanged?: (textValue: string) => void;
    onValueChanged?: (value: string) => void;
    onSelectedKeysChanged?: (newSelectedKeys: TKey[], added: TKey[], removed: TKey[]) => void;
    onShowSelector?: (event: SyntheticEvent, options: object) => void;
...
> {
    ...
    protected _selectorOpened: boolean = false;
    ...
    protected _beforeMount(options: T): Promise<LookupReceivedState | Error> | void {
    ...
    protected _beforeUpdate(newOptions: T): Promise<SelectedItems> | void | boolean {
        ...
        if (
            this._selectorOpened &&
            this._isSelectorTemplateOptionsChanged(newOptions) &&
            newOptions.selectorTemplate
        ) {
            this.showSelector(
                newOptions.selectorTemplate.popupOptions,
                newOptions.selectorTemplate.templateOptions
            );
        }
    ...
    protected _showSelector(
        ...
        return this._loadUnloadedItems().then(() => {
            const options = popupOptions || this._options.selectorTemplate?.popupOptions || {};
            if (this._notify('showSelector', [event, options]) !== false) {
                this._selectorOpened = true;
                return this.showSelector(options);
            }
            return false;
        });
    }
    ...
    protected _selectCallback(
        ...
        const selectCallback = (selectRes: SelectedItems) => {
            this._lookupController.setItemsAndSaveToHistory(selectRes);
            this._afterItemsChanged();
        };
        ...
        if (selectResult instanceof Promise) {
            ...
            selectCallback(selectResult as SelectedItems);
        }
    }
    ...
    private _setItems(items: SelectedItems): TKey[] | void {
        this._items = items;
        return this._lookupController.setItems(items);
    }

    _getSelectedItemsCount({ lazyItemsLoading, selectedKeys }: ILookupOptions): number {
        return lazyItemsLoading ? selectedKeys?.length || 0 : this._getLoadedItemsCount();
    }
    ...
    protected _isSelectorTemplateOptionsChanged(newOptions: T): boolean {
        return (
            !isEqual(
                newOptions.selectorTemplate?.popupOptions,
    ...
    abstract showSelector(popupOptions?: IStackPopupOptions, templateOptions?: object): void;
