...
import { Model, FormattableMixin } from 'Types/entity';
...
export interface ILookupBaseControllerOptions
    ...
    itemTemplate?: TemplateFunction | ComponentType;
    items?: RecordSet;
    maxVisibleItems?: number;
    lazyItemsLoading?: boolean;
    beforeSelectionChangedCallback?: Function;
    historySaveCallback?: Function | string;
}

const clone = object.clone;

function isFormattable(items: any): items is FormattableMixin {
    return !!items['[Types/_entity/FormattableMixin]'];
}
...
export default class LookupBaseControllerClass {
    ...
    update(newOptions: ILookupBaseControllerOptions): Promise<RecordSet> | boolean {
        const { selectedKeys, items } = newOptions;
        const hasSelectedKeysInOptions = selectedKeys !== undefined;
        const itemsChanged = this._options.items !== items && this._items !== items;
        let keysChanged;

        if (hasSelectedKeysInOptions) {
            keysChanged =
                !isEqual(selectedKeys, this._options.selectedKeys) ||
                !isEqual(selectedKeys, this.getSelectedKeys());
        }
        ...
        if (sourceController) {
            if (sourceIsChanged) {
                ...
                this._sourceController = undefined;
            } else if (keysChanged) {
        ...
        if (keysChanged || (sourceIsChanged && hasSelectedKeysInOptions)) {
            this._setSelectedKeys(selectedKeys.slice());
    ...
    setItems(items: SelectedItems): Key[] | void {
        const selectedKeys: Key[] = [];
        const selectedKeysMap: Record<string, boolean> = {};
        let isItemsOK = true;
    ...
    setItemsAndSaveToHistory(items: SelectedItems): void | Promise<unknown> {
        const needToSetItems = this._beforeSelectionChanged(
            this._getSelectedKeysByItems(items),
            [],
            []
        );
        const { historyId, historySaveCallback } = this._options;
        if (needToSetItems !== false) {
            this.setItemsAndSelectedKeys(this._prepareItems(items));
            if (items && items.getCount() && historyId) {
                return import('Controls/HistoryStore').then(({ Store }) => {
                    let historySavePromise: Promise<Function>;
                    if (typeof historySaveCallback === 'string') {
                        historySavePromise = loadAsync(historySaveCallback);
                    } else if (typeof historySaveCallback === 'function') {
                        historySavePromise = Promise.resolve(historySaveCallback);
                    }
                    return Store.push(historyId, this._selectedKeys).then(() => {
                        historySavePromise?.then((historySaveCallback) => historySaveCallback?.());
                    });
                });
            }
        }
    ...
    addItem(item: Model): boolean {
        ...
        if (!this.getSelectedKeys().includes(key)) {
            ...
            if (
                isFormattable(items) &&
                Object.getPrototypeOf(items.getAdapter()) !==
                    Object.getPrototypeOf(item.getAdapter())
            ) {
                Logger.error(
                    `Controls/lookup ошибка при выборе записи из автодополнения.
                          Отличается адаптер данных у записей в поле выбора (${
                              items.getAdapter()._moduleName
                          } 
                          и в автодополнеии (${item.getAdapter()._moduleName}.
                          Невозможно выбирать записи с разными адаптерами, т.к. они имеют разный формат данных.
                          Обеспечьте, чтобы адаптер у записей был одинаковым`,
                    this
                );
                return false;
            }
            ...
            const needToAdd = this._beforeSelectionChanged(selectedKeys, [key], []);
            if (needToAdd !== false) {
    ...
    removeItem(item: Model): boolean {
        ...
        const keyProperty = this._getKeyProperty(opts) || (item.getKeyProperty() as string);
        // FIXME удалить item.getKey() после
        // https://online.sbis.ru/opendoc.html?guid=376cc90d-8bb3-41cc-829b-57bfed67c90d
        ...
        if (selectedKeys.includes(key)) {
            ...
            const itemIndex = selectedItems.getIndexByValue(keyProperty, key);

            isChanged = true;
            selectedKeys = selectedKeys.slice();
            selectedKeys.splice(selectedKeys.indexOf(key), 1);

            if (itemIndex !== -1 || !opts.lazyItemsLoading) {
                selectedItems.removeAt(selectedItems.getIndexByValue(keyProperty, key));
            }

            const needToRemove = this._beforeSelectionChanged(selectedKeys, [], [key]);
    ...
    private _dataLoadErrback(error: Error): void {
        if (error) {
            ...
            Logger.error(
                `Controls/lookup ошибка загрузки.
                          Значение опции selectedKeys: ${JSON.stringify(this.getSelectedKeys())}`,
                this,
                error
            );
        }
    ...
    private _beforeSelectionChanged(
        selectedKeys: Key[],
        addedKeys: Key[],
        removedKeys: Key[]
    ): boolean | undefined {
        if (this._options.beforeSelectionChangedCallback) {
            return this._options.beforeSelectionChangedCallback(
                selectedKeys,
                addedKeys,
                removedKeys
            );
    ...
    private _getSelectedKeysByItems(items?: SelectedItems): TKey[] {
        const selectedKeys: TKey[] = [];
        const lookupItems = items || this._getItems();
        lookupItems.each((item) => {
    ...
    private _getSourceController(options: ILookupBaseControllerOptions): SourceController {
        this._sourceController = new SourceController({
            ...
            dataLoadErrback: this._dataLoadErrback.bind(this),
        });
        return this._sourceController;
    }

    private _needLoadItems(): boolean {
        const itemsKeys: TKey[] = [];
    ...
    private _getKeyProperty(options: ILookupBaseControllerOptions): string | void {
        return this._getSourceController(options).getKeyProperty();
    }
