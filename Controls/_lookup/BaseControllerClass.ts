...
export default class LookupBaseControllerClass {
    ...
    constructor(options: ILookupBaseControllerOptions) {
        ...
        this._setSelectedKeys(
            options.selectedKeys ? options.selectedKeys.slice() : []
        );
    }

    update(
        newOptions: ILookupBaseControllerOptions
    ): Promise<RecordSet> | boolean {
    ...
    loadItems(
        keysForLoad: Key[] = this._selectedKeys
    ): Promise<SelectedItems | Error> {
    ...
    removeItem(item: Model): boolean {
        ...
        if (selectedKeys.includes(key)) {
            ...
            selectedItems.removeAt(
                selectedItems.getIndexByValue(keyProperty, key)
            );
    ...
    getKeysForLoad(): Key[] {
        if (this._options.lazyItemsLoading) {
            return this.getSelectedKeys().slice(
                0,
    ...
    private _getHistoryService(): Promise<unknown> {
        if (!this._historyServiceLoad) {
            this._historyServiceLoad = import('Controls/suggestPopup').then(
                ({ LoadService }) => {
    ...
    private _isEqualFormat(oldList: RecordSet, item: Model): boolean {
        const oldListIsRecordSet =
            oldList && oldList['[Types/_collection/RecordSet]'];
        if (oldListIsRecordSet) {
            const oldFormat = oldListIsRecordSet && oldList.getFormat(true);
            const newFormat =
                item &&
                item['[Types/_entity/FormattableMixin]'] &&
                item.getFormat(true);
