            ...
            if (items && items.getCount() && this._options.historyId) {
                return import('Controls/HistoryStore').then(({ Store }) => {
                    ...
                    if (typeof this._options.historySaveCallback === 'string') {
                        historySavePromise = loadAsync(this._options.historySaveCallback);
                    } else if (typeof this._options.historySaveCallback === 'function') {
                        historySavePromise = Promise.resolve(this._options.historySaveCallback);
                    }
                    return Store.push(this._options.historyId, this._selectedKeys).then(() => {
