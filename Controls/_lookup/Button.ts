...
import { IFontColorStyleOptions, IFontSizeOptions, ISelectorTemplate } from 'Controls/interface';
...
export default class Button extends BaseLookup<ISelectorButtonOptions> {
    ...
    protected _opener: StackOpener;

    constructor(options: ISelectorButtonOptions) {
        super(options);
    ...
    protected _itemClickHandler(
        ...
        item: Model
    ): ReturnType<BaseLookup<ISelectorButtonOptions>['_showSelector']> | void {
    ...
    protected _removeItemHandler(_event: SyntheticEvent, item: Model): void {
    ...
    protected _joinItems(currentItems: SelectedItems, newItems: SelectedItems): void {
        currentItems.prepend(newItems);
    }

    protected _inheritorBeforeMount(_options: ILookupOptions): void {
        return undefined;
    }

    protected _inheritorBeforeUpdate(_options: ILookupOptions): void {
        return undefined;
    }
...
/**
 * @name Controls/_lookup/Button#underlineVisible
 * @cfg {Boolean} Определяет наличие подчеркивания у кнопки, открывающей окно выбора записей из справочника, и у выбранного значения.
 * @default false
 * @example
 * <pre class="brush: html">
 * <Controls.lookup:Selector
