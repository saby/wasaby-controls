...
class SelectedCollection extends Control<ISelectedCollectionOptions, number> {
    ...
    protected _notifyHandler: (
        event: SyntheticEvent,
        eventName: string
    ) => void = EventUtils.tmplNotify;
    protected _getItemMaxWidth: Function =
        selectedCollectionUtils.getItemMaxWidth;
    protected _getItemOrder: Function = selectedCollectionUtils.getItemOrder;
    protected _getItemGridStyles: Function =
        selectedCollectionUtils.getItemGridStyles;
    ...
    protected _beforeMount(options: ISelectedCollectionOptions): void {
        ...
        const itemsCount: number =
            options.itemsCount || options.items.getCount();
    ...
    protected _beforeUpdate(newOptions: ISelectedCollectionOptions): void {
        ...
        this._visibleItems =
            selectedCollectionUtils.getVisibleItems(newOptions);
    ...
    protected _afterMount(): void {
        const itemsCount: number =
            this._options.itemsCount || this._options.items.getCount();
    ...
    protected _openInfoBox(infoBox: HTMLElement): void {
        const result =
            (this._notify(
                'openInfoBox',
                []
            ) as Promise<void | IOpenInfoBoxResult>) || Promise.resolve();
        result.then((popupOptions: IOpenInfoBoxResult) => {
            const config = {
                ...
                direction: {
                    vertical: 'bottom',
                    horizontal:
                        this._options.counterAlignment === 'right'
                            ? 'left'
                            : 'right',
                },
                targetPoint: {
                ...
                templateOptions: {
                    items:
                        popupOptions?.templateOptions?.items ||
                        this._options.items,
