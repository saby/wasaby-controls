...
import {
    default as BaseLookup,
    ILookupOptions,
} from 'Controls/_lookup/BaseLookup';
...
export default abstract class BaseLookupInput extends BaseLookup<ILookupInputOptions> {
    ...
    protected _getFieldWrapper(): HTMLElement {
        if (!this._fieldWrapper) {
            ...
            this._fieldWrapper =
                inputRenderContainer[0] || inputRenderContainer;
        }
    ...
    protected _suggestStateChanged(
    ...
    private _resize(): void {
        if (this._isNeedCalculatingSizes(this._options)) {
            ...
            if (
                ...
                newFieldWrapperWidth !== oldFieldWrapperWidth
            ) {
                this._calculateSizes(this._options);
            }
        }
    ...
    private _keyDown(event: SyntheticEvent): void {
        ...
        if (keyCodeEvent === KEY_CODE_F2) {
            ...
            keyCodeEvent === constants.key.backspace &&
            !hasValueInInput &&
            !this._isEmpty()
        ) {
            if (detection.isIE) {
    ...
    protected _suggestDirectionChanged(
        event: SyntheticEvent,
    ...
    protected _isShowCollection(): boolean {
        return (
            !this._isEmpty() &&
            !!(this._maxVisibleItems || this._options.readOnly)
        );
    }

    private _subscribeOnResizeEvent(options: ILookupInputOptions): void {
        if (
            !this._subscribedOnResizeEvent &&
            this._isNeedCalculatingSizes(options)
        ) {
