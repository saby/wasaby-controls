...
export default abstract class BaseLookupInput extends BaseLookup<ILookupInputOptions> {
    ...
    private _fieldWrapperWidth: number | null = null;
    ...
    private _showInputBorderBySuggestPopupWidth: boolean = false;
    ...
    protected _selectCallback(
        ...args: Parameters<BaseLookup<ILookupInputOptions>['_selectCallback']>
    ): void {
        if (this._options.value && this._options.commentVisibility !== 'always') {
            this._notify('valueChanged', ['']);
        }
        super._selectCallback(...args);
    }
    ...
    protected _joinItems(currentItems: SelectedItems, newItems: SelectedItems): void {
        currentItems[this._options.multiLine ? 'prepend' : 'append'](newItems);
    }
    ...
    protected _getInputValue(options: ILookupInputOptions): string | null {
    ...
    protected _suggestStateChanged(_event: SyntheticEvent, state: boolean): void {
        if (
            (this._infoboxOpened ||
        ...
        if (state) {
            this._showInputBorderBySuggestPopupWidth = this._isInputWiderThanSuggest();
        }
    ...
    private _isInputWiderThanSuggest(): boolean {
        if (this._options.suggestPopupOptions?.width) {
            const suggestWidth =
                typeof this._options.suggestPopupOptions.width === 'string'
                    ? Number(this._options.suggestPopupOptions.width.replace(/\D/g, ''))
                    : this._options.suggestPopupOptions.width;
            return this._container.offsetWidth > suggestWidth;
        }
        return false;
    }
    ...
    protected _onClickClearRecords(): void {
    ...
    protected _itemClick(_event: SyntheticEvent, item: Model, nativeEvent: Event): void {
        this.closeSuggest();
    ...
    protected _changeValueHandler(_event: SyntheticEvent, value: string): void {
        this._setInputValue(this._options, value);
        this._notifyValueChanged(value);
    }

    protected _choose(_event: SyntheticEvent, item: Model, tabsSelectedKey?: string): void {
    ...
    protected _crossClick(_event: SyntheticEvent, item: Model, resetInputValue = true): void {
    ...
    protected _getPlaceholder(options: ILookupInputOptions): string | TemplateFunction {
        let placeholder;
    ...
    protected _suggestDirectionChanged(_event: SyntheticEvent, direction: TSuggestDirection): void {
    ...
    showSelector(popupOptions?: IStackPopupOptions, templateOptions?: object): boolean {
        this.closeSuggest();
    ...
    protected _isNeedCalculatingSizes(_options: ILookupInputOptions): boolean {
        return false;
    }

    protected _calculateSizes(_options: ILookupInputOptions): void {
        /* For override */
    }
