...
const getItemGridRowStyles = (index: number, itemsLayout: string): string => {
    if (itemsLayout === 'twoColumns') {
        const elemIndex = index + 1;
        const rowIndex = elemIndex % 2 ? (elemIndex + 1) / 2 : elemIndex / 2;
        return `grid-row: ${rowIndex}; -ms-grid-row: ${rowIndex};`;
    }

    return '';
};

const getItemGridColumnStyles = (
    index: number,
    itemsLayout: string
): string => {
    if (itemsLayout === 'twoColumns') {
        ...
        return `grid-column: ${columnIndex}; -ms-grid-column: ${columnIndex};`;
    }
...
export = {
    ...
    getItemGridStyles(index: number, itemsLayout: string): string {
