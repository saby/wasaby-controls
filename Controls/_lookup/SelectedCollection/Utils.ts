...
export = {
    ...
    getItemMaxWidth(
        ...
        if (indexItem === 0 && itemsLength > maxVisibleItems && itemsLayout === 'default') {
    ...
    getVisibleItems({
        ...
        let ignoreMaxVisibleItems: boolean;
        if (counterPosition === 'static') {
            startIndex = 0;
            ignoreMaxVisibleItems = itemsLayout === 'twoColumns' || maxVisibleItems === undefined;
        } else {
            startIndex = Math.max(
                maxVisibleItems && multiLine ? items.getCount() - maxVisibleItems : 0,
                0
            );
            ignoreMaxVisibleItems = multiLine || itemsLayout === 'twoColumns' || maxVisibleItems === undefined;
        }
