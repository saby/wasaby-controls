...
export function CrossTemplate(props: IItemTemplateOptions): JSX.Element {
    ...
    if (props.crossTemplate) {
        ...
        return (
            <div className={`js-controls-SelectedCollection__item__cross 
...
function ItemTemplate(props: IItemTemplateOptions, ref: React.ForwardedRef<HTMLDivElement>): JSX.Element {
    ...
    if (!fontWeight && typeof props.style === 'string' && props.style === 'bold') {
        ...
        Logger.error('Насыщенность шрифта для \'Controls/lookup:ItemTemplate\' необходимо ' +
    ...
    if (!fontColorStyle && typeof props.style === 'string' && COLORS.includes(props.style)) {
        ...
        Logger.error('Стиль цвета текста для \'Controls/lookup:ItemTemplate\' необходимо ' +
    ...
    const contentProps: Partial<IContentTemplateOptions> = {
        ...
        fontColorStyle: props.itemsLayout === 'twoColumns' || !fontColorStyle ? 'default' : fontColorStyle,
    ...
    return (
        <div className={`controls-SelectedCollection__item ${ props.attrs?.className || props.className }
                        controls-SelectedCollection__item-${ props.isSingleItem ? 'single' : 'multi' }
                        ${ props.readOnly ? 'controls-SelectedCollection__item-' + (props.isSingleItem ? 'single' : 'multi') + '-readOnly' : '' }
            ...
            <span className={`controls-SelectedCollection__item__caption-wrapper
                             ${ props.inlineHeight ?
                             'controls-SelectedCollection__item__caption-wrapper_lineheight-' + props.inlineHeight
                             : props.itemsLayout === 'default' ? 'controls-SelectedCollection__item__caption-wrapper_lineheight-default' : '' }`}>
                <props.contentTemplate { ...contentProps } />
            </span>
            <CrossTemplate { ...props } />
            { props.readOnly && !props.isLastItem && props.itemsLayout !== 'twoColumns' ?
                <span className={`controls-text-${ (props.itemsLayout === 'twoColumns' || !fontColorStyle) ? 'default' : fontColorStyle }`}>,</span>
                : ''
            }
        </div>
    );
