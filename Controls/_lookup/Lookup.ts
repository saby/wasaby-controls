 ...
 */
import {
    default as BaseLookupInput,
    ILookupInputOptions,
} from 'Controls/_lookup/BaseLookupInput';
...
export default class Lookup extends BaseLookupInput {
    ...
    protected _beforeMount(
        ...
        const beforeMountResult = super._beforeMount(
            options,
            context,
            receivedState
        );
    ...
    _calculateSizes(options: ILookupOptions): void {
        ...
        const isShowCounter = this._isShowCounter(
            multiLineState,
            itemsCount,
            maxVisibleItems
        );
        ...
        if (this._isNeedCalculatingSizes(options)) {
            ...
            lastSelectedItems = this._getLastSelectedItems(
                this._items,
                MAX_VISIBLE_ITEMS
            );
            itemsSizesLastRow = this._getItemsSizesLastRow(
                fieldWrapperWidth,
                lastSelectedItems,
                options,
                counterWidth
            ...
            if (options.multiLine) {
                ...
                lastRowCollectionWidth =
                    this._getCollectionWidth(itemsSizesLastRow);
                if (options.needCalculateMultiLine) {
    ...
    private _getInputWidth(
        ...
        if (
            lastRowCollectionWidth <= availableWidth &&
            needCalculateMultiLine
        ) {
            const rowWidth =
    ...
    private _isShowCounter(
        ...
        return (
            (multiLine && itemsCount > maxVisibleItems) ||
            (!multiLine && itemsCount > 1)
        );
    }
    ...
    private _getLastSelectedItems(
        ...
        const startIndex =
            count - maxVisibleItems < 0 ? 0 : count - maxVisibleItems;
    ...
    private _getItemsSizesLastRow(
        ...
        const maxVisibleItems = newOptions.multiLine
        ...
        let itemsTemplateResult = itemsTemplate({
            _options: {
                ...
                ...this._getCollectionOptions(
                    newOptions,
                    maxVisibleItems,
                    counterWidth
                ),
            },
            _visibleItems: this._getLastSelectedItems(
                this._items,
                maxVisibleItems
            ),
            _getItemMaxWidth: selectedCollectionUtils.getItemMaxWidth,
            _getItemOrder: selectedCollectionUtils.getItemOrder,
            _getItemGridStyles: selectedCollectionUtils.getItemGridStyles,
        ...
        if (newOptions.multiLine) {
            measurer.style.width =
                fieldWrapperWidth - SHOW_SELECTOR_WIDTH + 'px';
        }
        ...
        for (
            ...
            index >= 0 &&
            collectionItems[index].offsetTop ===
                collectionItems[itemsCount - 1].offsetTop;
            index--
        ) {
            itemsSizes.unshift(
                Math.ceil(collectionItems[index].getBoundingClientRect().width)
            );
        }
    ...
    private _getInputMinWidth(
        ...
        const minWidthFieldWrapper =
            (fieldWrapperWidth - rightFieldWrapperWidth) / 3;
    ...
    private _getFieldWrapperMinHeight(): number {
        if (this._fieldWrapperMinHeight === null) {
            const fieldWrapperStyles = getComputedStyle(
                this._getFieldWrapper()
            );
    ...
    private _initializeConstants(): void {
        if (!SHOW_SELECTOR_WIDTH) {
            ...
            SHOW_SELECTOR_WIDTH = getWidth(
                this._showSelectorTemplate(
                    templateOptions,
                    undefined,
                    undefined,
                    true
            ...
            CLEAR_RECORDS_WIDTH = getWidth(
                this._clearRecordsTemplate(
                    templateOptions,
                    undefined,
                    undefined,
                    true
    ...
    _isInputVisible(options: ILookupOptions): boolean {
        ...
        return (
            ((!readOnly ||
                (this._getInputValue(options) && !options.multiSelect)) &&
 ...
 * @event Происходит при выборе элемента из автодополнения.
 * @param {UI/Events:SyntheticEvent} eventObject Дескриптор события.
