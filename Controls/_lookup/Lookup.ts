 ...
 * @cfg {Function} Функция, которая вызывается каждый раз  для контрола автодополнения непосредственно после загрузки данных из источника.
 * @remark
 * Первым аргументом в функцию приходит **items** с типом данных {@link Types/collection:RecordSet}, где содержатся загруженные данные. Вторым аргументом - направление подгрузки (up/down, если подгрузка выполнялась вверх/вниз; null, если выполнялась первая загрузка или перезагрузка)
 */
 ...
 *
 * @demo Controls-demo/LookupNew/ReadOnly/MultiLine/Index
 */
/*
 * @name Controls/_lookup/Lookup#multiLine
