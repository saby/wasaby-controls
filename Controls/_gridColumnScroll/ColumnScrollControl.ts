...
import type { IItemsSizes, TLoadingTriggerSelector } from 'Controls/baseList';
import type {
    GridControl,
    GridCollection,
    IGridControlOptions,
...
import type { TColumnKey, TColumnWidth } from 'Controls/grid';
...
const ColumnScrollClassMap = new Map();

const getClass = <TOptions extends IOptions = IOptions>(BaseClass: TBaseControl<TOptions>) => {
    if (!ColumnScrollClassMap.has(BaseClass)) {
        ColumnScrollClassMap.set(
            BaseClass,
            class GridColumnScrollControl<
            ...
            > extends BaseClass<TControlOptions> {
                ...
                scrollToColumn(columnIndex: number): void {
                    const cell: HTMLElement = this._container?.querySelectorAll(
                        `.${this._options.columnScrollSelectors.AUTOSCROLL_TARGET}`
                    )[columnIndex - this.props.stickyColumnsCount];
                    if (cell) {
                        this._options.columnScrollScrollIntoView(cell);
                    }
                }

                protected _getClearModelOptions(options: TControlOptions): IGridCollectionOptions {
                    let stickyColumnsCount = Math.min(
                        typeof options.stickyColumnsCount === 'number'
                            ? options.stickyColumnsCount
                            : 1,
                        options.columns.length - 1
                    );

                    let endStickyColumnsCount = Math.min(
                        typeof options.endStickyColumnsCount === 'number'
                            ? options.endStickyColumnsCount
                            : 0,
                        options.columns.length - 1
                    );

                    if (options.columns.length === stickyColumnsCount) {
                    ...
                    return {
                        ...
                        columnScroll: options.columnScroll,
                ...
                protected _getModelOptions(options: TControlOptions): IGridCollectionOptions {
                    return {
                        ...
                        ...this._getClearModelOptions(options),
                    };
                }
                ...
                protected _beforeUpdate(newOptions: TControlOptions, ...args: unknown[]): void {
                    ...
                    let updateStickyColumnsCount = false;
                    ...
                    if (
                        ...
                        updateStickyColumnsCount = true;
                    }

                    if (this._options.columnScrollViewMode !== newOptions.columnScrollViewMode) {
                        this._listViewModel.setColumnScrollViewMode(
                    ...
                    if (this._options.columns !== newOptions.columns && this._storedColumnsWidths) {
                        const baseListLib =
                            loadSync<typeof import('Controls/baseList')>('Controls/baseList');
                        if (baseListLib) {
                            const newWidths = baseListLib.extractWidthsForColumns(
                            ...
                            if (
                                !isEqual(this._listViewModel.getColumnsWidths(), newWidths) &&
                                newWidths.length
                            ) {
                    ...
                    if (updateStickyColumnsCount) {
                        const { stickyColumnsCount, endStickyColumnsCount } =
                            this._getClearModelOptions(newOptions);

                        this._listViewModel.setStickyColumnsCount(
                            stickyColumnsCount,
                            endStickyColumnsCount,
                            newOptions.columns.length
                        );
                    }
                }
                ...
                protected _keyDownLeft(
                    ...
                    const isSmooth = !(
                        this._options.smoothHorizontalScroll === 'keyboard' ||
                        this._options.smoothHorizontalScroll === 'all'
                    );
                    const columnScrollMoved = event.nativeEvent.shiftKey
                        ? this._options.setColumnScrollPositionToPrevPage?.(isSmooth)
                        : false;
                    super._keyDownLeft(event, canMoveMarker && !columnScrollMoved);
                }
                ...
                protected _keyDownRight(
                    ...
                    const columnScrollMoved = event.nativeEvent.shiftKey
                        ? this._options.setColumnScrollPositionToNextPage?.(isSmooth)
                        : false;
                    super._keyDownRight(event, canMoveMarker && !columnScrollMoved);
                }
    ...
    return ColumnScrollClassMap.get(BaseClass);
};
