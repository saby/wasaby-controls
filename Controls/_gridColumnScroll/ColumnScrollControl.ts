...
const LOADING_TRIGGER_SELECTOR: `.${typeof SCROLLABLE_VIEW_WRAPPER_CLASS_NAME} ${TLoadingTriggerSelector}` =
    `.${SCROLLABLE_VIEW_WRAPPER_CLASS_NAME} .controls-BaseControl__loadingTrigger`;
...
const getClass = <TOptions extends IOptions = IOptions>(BaseClass: TBaseControl<TOptions>) => {
    ...
    > extends BaseClass<TControlOptions> {
        ...
        private _getElementHiddenOffset(target: HTMLElement): number {
            const viewRect = target.closest('.controls-GridReact__view_columnScroll').getBoundingClientRect();
            const targetRect = target.getBoundingClientRect();

            const leftElementOffset = Math.floor(
                targetRect.left - viewRect.left - this._options.getColumnScrollWidths().fixedWidth
            );
            ...
            const rightElementOffset = Math.floor(
                viewRect.right - targetRect.right
            );
