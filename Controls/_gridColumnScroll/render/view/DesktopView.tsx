...
import { getBaseView } from 'Controls/_gridColumnScroll/common/grid';
...
const DesktopViewColumnScroll = React.memo(
    React.forwardRef(function DesktopViewColumnScroll(
        ...
        const ResolvedViewComponent = props.ViewComponent ?? getBaseView();
        return (
            <div
                ref={ref}
...
export default React.memo(
    React.forwardRef(function DesktopViewColumnScrollWrapper(
        ...
        const onWheel = React.useCallback((e: React.WheelEvent) => {
            let shouldUpdateColumnScrollPosition = e.shiftKey || e.deltaX;
            if (detection.isMac) {
                // В MacOs, если скролить тачпадом по диагонали, будут приходить оба значения отступа,
                // по X и по Y (меньшее не обнуляется). Поэтому проверяем, какой отступ меньше самостоятельно.
                // Иначе произойдет скролл в вертикальном и горизонтальном направлениях одновременно
                shouldUpdateColumnScrollPosition =
                    e.shiftKey || (e.deltaX && Math.abs(e.deltaX) > Math.abs(e.deltaY) + 1);
            }
            if (shouldUpdateColumnScrollPosition) {
