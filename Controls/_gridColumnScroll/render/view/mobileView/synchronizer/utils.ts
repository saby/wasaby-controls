...
export function getSizesStyle(
    ...
    const getTopStyle = (size: ISize) => {
        if (typeof beforeContentSize === 'undefined' || beforeContentSize === null) {
            return '';
        }
...
function updateGridColumnEnd(cells: NodeListOf<Element> | null, stickyColumnsCount: number) {
    if (cells) {
        for (const el of cells) {
            if (el.style.gridColumnEnd > stickyColumnsCount) {
...
export function fakeGridColspanSynchronizer(stickyColumnsCount: number, container: HTMLDivElement) {
    const fakeGridStart = container.querySelector(`.${FIXED_START_VIEW_WRAPPER_CLASS_NAME}`);

    const footerCells = fakeGridStart?.querySelectorAll(`.${FOOTER_CELL}`) ?? null;
    const headerCells = fakeGridStart?.querySelectorAll(`.${HEADER_CELL}`) ?? null;
    const resultsCells = fakeGridStart?.querySelectorAll(`.${RESULTS_CELL}`) ?? null;

    updateGridColumnEnd(footerCells, stickyColumnsCount);

    updateGridColumnEnd(headerCells, stickyColumnsCount);

    updateGridColumnEnd(resultsCells, stickyColumnsCount);
}
