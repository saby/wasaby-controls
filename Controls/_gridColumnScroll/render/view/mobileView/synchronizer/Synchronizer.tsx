...
function SynchronizerComponent(
    ...
    const [itemsCellsWidth, setItemsCellsWidth] = React.useState<number>();

    const preparedItemSizes = React.useMemo(() => {
        if (itemsCellsWidth) {
            return props?.itemsSizes?.map((item) => {
                item.width = itemsCellsWidth;
                return item;
            });
        }
        return props?.itemsSizes;
    }, [props.itemsSizes, itemsCellsWidth]);
        ...
        (container: HTMLDivElement) => {
            if (container) {
                unstable_batchedUpdates(() => {
                    ...
                    // Ширина каждой ячейки одной колонки одинаковая. Достаточно получить ширину первой ячейки
                    // и использовать её для каждой ячейки.
                    const itemsWidth = getSizesBySelector(
                        container,
                        `.${SCROLLABLE_VIEW_WRAPPER_CLASS_NAME} .${CELL}.${props.fixedStartElementSelector}`
                    ).sizes?.[0]?.width;

                    setItemsCellsWidth(itemsWidth);
