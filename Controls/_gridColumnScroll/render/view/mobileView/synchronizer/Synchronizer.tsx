...
const getNavigationSelectorMask = (
    fixedStartElementSelector: string,
    scrollableElementSelector: string
): TSelectorMask => {
    return (
        `.${SCROLLABLE_VIEW_WRAPPER_CLASS_NAME} .${NAVIGATION_CELL}.${fixedStartElementSelector}:nth-child($key$), ` +
        `.${SCROLLABLE_VIEW_WRAPPER_CLASS_NAME} .${NAVIGATION_CELL}.${scrollableElementSelector}:nth-child($key$)`
    );
};

const getFakeNavigationSelectorMask = (
    fixedStartElementSelector: string,
    scrollableElementSelector: string
): TSelectorMask => {
    return (
        `.${FIXED_START_VIEW_WRAPPER_CLASS_NAME} .${NAVIGATION_CELL}.${fixedStartElementSelector}:nth-child($key$), ` +
        `.${FIXED_START_VIEW_WRAPPER_CLASS_NAME} .${NAVIGATION_CELL}.${scrollableElementSelector}:nth-child($key$), ` +
        `.${FIXED_END_VIEW_WRAPPER_CLASS_NAME} .${NAVIGATION_CELL}.${fixedStartElementSelector}:nth-child($key$), ` +
        `.${FIXED_END_VIEW_WRAPPER_CLASS_NAME} .${NAVIGATION_CELL}.${scrollableElementSelector}:nth-child($key$)`
    );
};
...
export type TSynchronizerComponentProps = {
    ...
    fixBeforeNavigationStickyContentSize?: boolean;
};
        ...
        (container: HTMLDivElement) => {
            if (container) {
                unstable_batchedUpdates(() => {
                    ...
                    // TODO: тут возможно ошибка и надо установить иногда undefined.
                    let beforeRealNavigationSize =
                        (props.hasStickyTopResults ? results.fullSize : 0) +
                        (props.hasStickyHeader
                            ? (newBeforeContainerContentSize || 0) + header.fullSize
                            : 0);

                    // TODO: Убрать fixBeforeNavigationStickyContentSize по
                    //  Controls/_gridColumnScroll/render/view/mobileView/synchronizer/utils.ts:20
                    if (props.fixBeforeNavigationStickyContentSize) {
                        beforeRealNavigationSize = null;
                    }

                    setBeforeRealNavigationCellsSize(beforeRealNavigationSize);
                    setBeforeFakeNavigationCellsSize(beforeRealNavigationSize);

                    //  Необходимо также синхронизировать стиль grid-column, так как
                    //  в фэйковом ренедре колонок меньше, чем в оригинальном гриде. Если
                    //  мы отрендерим ячейки footer/header/result с grid-column-end как для
                    //  оригинального грида, может возникнуть ситуация когда значение в
                    //  grid-column-end больше, чем всего ячеек в фейковом гриде и тогда
                    //  фейковый грид распердолит
                    fakeGridColspanSynchronizer(fixedStartItemCellsSizes.length, container);
                });
            }
        },
        [
            ...
            {!!realNavigationCellsSize?.length && (
                <SizesSynchronizerComponent
                    ...
                    keyMask={getNavigationSelectorMask(
                        props.fixedStartElementSelector,
                        props.scrollableElementSelector
                    )}
