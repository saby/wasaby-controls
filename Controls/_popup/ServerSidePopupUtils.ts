 ...
 * @property { String } id Иднификатор окна, благодаря которому, можно будет найти окно на клиенте и управлять им.
 */
...
const getTop = (serverSidePopupConfig: IServerSidePopupOptions): string | number => {
    ...
    return `calc(50% - ${verticalOffset}px)`;
};
...
const getLeft = (serverSidePopupConfig: IServerSidePopupOptions): string | number => {
    ...
    return `calc(${windowCenter} - ${horizontalOffset}px)`;
};
...
export const registerServerSidePopup = (serverSidePopupConfig: IServerSidePopupOptions): void => {
    const config = {
        ...
        id: serverSidePopupConfig.id || randomId('popup-'),
