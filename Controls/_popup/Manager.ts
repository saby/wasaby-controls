...
class Manager {
    ...
    show(options: IPopupOptions, controller: IPopupController): string {
        ...
        /*
        todo пока отключаем логирование. Вернем тут https://online.sbis.ru/opendoc.html?guid=8f58e424-723e-416e-93f1-72abd34274b8&client=3
        if (options._type === 'stack') {
            this._logTemplateName(options.template);
        }
        */
    ...
    private _logTemplateName(template: string): void {
        ...
        const cloudStatisticsService = new SbisService({
            endpoint: 'CloudStatistics'
        });

        cloudStatisticsService.call('Send', {
            data: {
                functional: template,
    ...
    private _registerPopupLink(popupConfig: IPopupItem): void {
        if (popupConfig.popupOptions.opener) {
            ...
            const parentPage = this._findParent(popupConfig.popupOptions.opener, pageModuleName);
            if (parentPage) {
    ...
    private _findParent(control: Control | HTMLElement, parentName: string): Popup | boolean {
        const controlContainer = control instanceof HTMLElement ? control : control._container;
        const parentControls: Control[] = goUpByControlTree(controlContainer);
    ...
    private _preparePopupList(): List<IPopupItemInfo> {
        ...
        this._popupItems.each((item: IPopupItem) => {
            ...
            if (item.parentId) {
                const index =
                    this._popupItems && this._popupItems.getIndexByValue('id', item.parentId);
                if (index > -1) {
