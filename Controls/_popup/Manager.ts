...
class Manager {
    ...
    constructor(options: IManagerOptions = {}) {
        ...
        this._popupResizeOuter = debounce(
            this._popupResizeOuterBase,
            RESIZE_DELAY
        );
    }
    ...
    init(): void {
        ...
        EventBus.channel('navigation').subscribe(
            'onBeforeNavigate',
    ...
    updatePosition(id: string): void {
        ...
        if (item) {
            needUpdate = item.controller.updatePosition(
                item,
    ...
    remove(id: string): Promise<void> {
        ...
        if (item) {
            ...
            const needClose = item.controller.beforeElementDestroyed(
                item,
                itemContainer
            );
                ...
                return new Promise((resolve) => {
                    ...
                    if (item.childs.length) {
                        this._finishPendings(id, null, null, () => {
                            this._closeChilds(item).then(removeElement);
                        });
                    } else {
                        this._closeChilds(item).then(() => {
                            this._finishPendings(id, null, null, removeElement);
                        });
                    }
                });
            }
    ...
    isDestroying(id: string): boolean {
        ...
        if (!item) {
            ...
            const removedItems =
                ManagerController.getContainer().getRemovingItems();
    ...
    private _subscribeToPageDragNDrop(): void {
        ...
        EventBus.channel('dragnDrop').subscribe(
            'documentDragStart',
            (event, eventObject) => {
    ...
    private _registerPopupLink(popupConfig: IPopupItem): void {
        if (popupConfig.popupOptions.opener) {
            const popupModuleName = 'Controls/_popup/Manager/Popup';
            const parent: Popup = this._findParent(
            ...
            const pageModuleName = 'Controls/popupTemplate:StackPageWrapper';
            const parentPage = this._findParent(
                popupConfig.popupOptions.opener,
                pageModuleName
            );
            if (parentPage) {
    ...
    private _findParent(control: Control, parentName: string): Popup | boolean {
        const parentControls: Control[] = goUpByControlTree(control._container);
        for (let i = 0; i < parentControls.length; i++) {
            if (parentControls[i]._moduleName === parentName) {
    ...
    private _removeElement(
        ...
        const removeDeferred = item.controller.elementDestroyedWrapper(
            item,
            container
        );
        this._redrawItems();

        Manager._notifyEvent('managerPopupBeforeDestroyed', [
            item,
            this._popupItems,
            container,
        ]);
        return removeDeferred.addCallback(() => {
            ...
            Manager._notifyEvent('managerPopupDestroyed', [
                item,
                this._popupItems,
            ]);
        });
    ...
    protected _popupBeforePaintOnMount(id: string): void {
        const item = this.find(id);
        if (item) {
            item.controller.elementMountedWrapper(
                item,
                this._getItemContainer(id)
            );
            if (!item.popupOptions.isCompoundTemplate) {
                Manager._notifyEvent('managerPopupCreated', [
                    item,
                    this._popupItems,
                ]);
            }
    ...
    protected _popupCreated(id: string): boolean {
        ...
        if (item) {
            ...
            const res = item.controller.elementCreatedWrapper(
                item,
                container
            );
            if (item.popupOptions.trackTarget && item.popupOptions.target &&
                item.popupOptions.actionOnScroll === 'track' && !item.popupOptions.closeOnOutsideClick) {
                item.targetPosition = (item.popupOptions.target as HTMLElement).getBoundingClientRect();
                item.trackIntervalId = setInterval(() => {
                    const targetPosition = (item.popupOptions.target as HTMLElement).getBoundingClientRect();
                    if (item.targetPosition.top !== targetPosition.top ||
    ...
    protected _popupMovingSize(
        ...
        if (element) {
            ...
            Manager._notifyEvent('managerPopupUpdated', [
                element,
                this._popupItems,
            ]);
            return true;
    ...
    protected _popupMaximized(id: string, state: boolean): boolean {
        const element = this.find(id);
        if (element) {
            element.controller.elementMaximized(
                element,
                this._getItemContainer(id),
                state
            );
            Manager._notifyEvent('managerPopupMaximized', [
                element,
                this._popupItems,
            ]);
            return true;
    ...
    protected _orientationChanged(id: string): boolean {
        const element = this.find(id);
        if (element) {
            return element.controller.orientationChanged(
                element,
                this._getItemContainer(id)
            );
        }
    ...
    private _handlePopupWithOverlay(): void {
        ...
        const popupItem =
            overlayIndex !== -1 && this._popupItems.at(overlayIndex);
        if (
            popupItem &&
            popupItem.popupState !==
            popupItem.controller.POPUP_STATE_INITIALIZING
        ) {
    ...
    private _outsideClickHandler(event?: Event): void {
        ...
        const isClickToOverlay = target
            ? this._elementIsPopupOverlay(target)
            : false;
        const isClickToIgnoredArea = target
            ? this._isIgnoreActivationArea(target)
            : false;
        if (isClickToOverlay) {
            this._handlePopupWithOverlay();
            ...
            this._popupItems.each((item) => {
                if (item) {
                    ...
                    const parentControls = target
                        ? goUpByControlTree(target)
                        : [];
                    const popupInstance =
                        ManagerController.getContainer().getPopupById(item.id);

                    // Check the link between target and popup
                    if (
                        this._needClosePopupByOutsideClick(item) &&
                        parentControls.indexOf(popupInstance) === -1
                    ) {
    ...
    protected _popupDragStart(
        ...
        const element = this.find(id);
        if (element) {
            element.controller.popupDragStart(
                element,
                this._getItemContainer(id),
                offset,
                sizes
    ...
    protected _popupMouseEnter(id: string, event: Event): boolean {
        const item = this.find(id);
        if (item) {
            item.controller.popupMouseEnter(
                item,
                this._getItemContainer(id),
                event
            );
    ...
    protected _popupMouseLeave(id: string, event: Event): boolean {
        const item = this.find(id);
        if (item) {
            item.controller.popupMouseLeave(
                item,
                this._getItemContainer(id),
                event
            );
    ...
    protected _popupResizeInner(id: string): boolean {
        ...
        if (item) {
            ...
            if (!parentItem || parentItem.position.hidden !== true) {
                return item.controller.resizeInner(
                    item,
                    this._getItemContainer(id)
                );
            }
    ...
    protected _popupAnimated(id: string): boolean {
        const item = this._findItemById(id);
        if (item) {
            return item.controller.elementAnimated(
                item,
                this._getItemContainer(id)
            );
        }
    ...
    private _callEvents(
        ...
        if (
            ...
            typeof options.eventHandlers[event] === 'function'
        ) {
    ...
    private _updateZIndex(): void {
        ...
        this._popupItems.each((item: IPopupItem, index: number) => {
            ...
            const step = item.popupOptions.topPopup
                ? TOP_POPUP_ZINDEX_STEP
                : POPUP_ZINDEX_STEP;
            ...
            if (
                currentItem.parentZIndex &&
                customZIndex < currentItem.parentZIndex
            ) {
    ...
    private _preparePopupList(): List<IPopupItemInfo> {
        ...
        this._popupItems.each((item: IPopupItem) => {
            ...
            if (item.parentId) {
                const index =
                    this._popupItems &&
                    this._popupItems.getIndexByValue('id', item.parentId);
                if (index > -1) {
        ...
        setTimeout(() => {
            this._popupItems.each((item) => {
                if (item.controller.needRecalcOnKeyboardShow()) {
                    item.controller.elementUpdatedWrapper(
                        item,
                        this._getItemContainer(item.id)
                    );
                }
    ...
    private _finishPendings(
        ...
        if (item && registrator) {
            ...
            if (registrator) {
                const hasRegisterPendings =
                    registrator.hasRegisteredPendings(popupId);
    ...
    private _isIgnoreActivationArea(focusedContainer: HTMLElement): boolean {
        ...
        while (focusedContainer && focusedContainer.classList) {
            ...
            focusedContainer =
                focusedContainer.parentElement || focusedContainer.parentNode;
        }
        return false;
    }
    ...
    private _findItemById(id: string): IPopupItem {
        const index =
            this._popupItems && this._popupItems.getIndexByValue('id', id);
        if (index > -1) {
    ...
    protected _pageDragnDropHandler(
        type: string,
        dragEvent: object = {}
    ): boolean {
        ...
        this._dragTimer = setTimeout(() => {
            ...
            this._popupItems.each((item) => {
                ...
                if (
                    item.controller.dragNDropOnPage(
                        item,
                        popupContainer,
                        isInsideDrag,
                        type
