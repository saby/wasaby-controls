...
class Stack extends BaseOpener<IStackOpenerOptions> implements IStackOpener {
    ...
    open(popupOptions: IStackPopupOptions): Promise<string | undefined> {
        ...
        if (unsafe_getRootAdaptiveMode().device.isPhone() && config.allowAdaptive) {
            return import('Controls/popup').then((popup) => {
                this._prepareNotifyConfig(config);
                return Stack._openSliding(config, this._getSlidingPanelOpener(popup)).then((popupId) => {
    ...
    static _openSliding(popupOptions: IStackPopupOptions, opener): Promise<string | undefined> {
    ...
    static _openPopup(config: IStackPopupOptions): CancelablePromise<string> {
        ...
        if (unsafe_getRootAdaptiveMode().device.isPhone() && newCfg.allowAdaptive) {
            return import('Controls/popup').then((popup) => {
