...
import {
    ...
    IBaseOpener,
    IBasePopupOptions,
} from 'Controls/_popup/interface/IBaseOpener';
import BaseOpenerUtil from 'Controls/_popup/Opener/BaseOpenerUtil';
import {
    loadModule,
    getModuleByName,
} from 'Controls/_popup/utils/moduleHelper';
    ...
    implements IOpener, IBaseOpener
{
    ...
    open(
        popupOptions: TBaseOpenerOptions,
        controller: string
    ...
    private _openPopup(
        ...
        return new Promise((resolve) => {
            const openPopup = (cfg) => {
                const syncResult: ILoadDependencies = this._getModulesSync(
                    cfg,
                    controller
                );
    ...
    private _requireModules(
        ...
        if (!this._loadModulesPromise) {
            this._loadModulesPromise = BaseOpener.requireModules(
                cfg,
                controller
            )
                .then((results: ILoadDependencies) => {
                    ...
                    if (
                        !this._openerUnmounted ||
                        this._options.closePopupBeforeUnmount === false
                    ) {
                        return results;
                    }
                    Logger.warn(
    ...
    protected _getConfig(
        popupOptions: IBaseOpenerOptions = {}
    ): TBaseOpenerOptions {
        const baseConfig = BaseOpenerUtil.getConfig(
            this._options,
            popupOptions
        );
    ...
    static showDialog(
        ...
        if (cfg?.templateOptions?.opener) {
            ...
            Logger.error(
                'Controls/popup: Опция opener не должна задаваться на templateOptions'
            );
        }
        if (
            cfg.opener &&
            !BaseOpenerUtil.isControl(cfg.opener) &&
            !cfg.opener instanceof React.Component
        ) {
            delete cfg.opener;
            Logger.error(
                'Controls/popup: Задано некорректное значение опции opener. Опция в качестве значения принимает инстанс контрола.'
            );
        }
            ...
            BaseOpenerUtil.getManagerWithCallback(() => {
                ...
                if (!cfg.zIndex) {
                    if (!cfg.topPopup) {
                        ...
                        const oldWindowManager =
                            requirejs('Core/WindowManager');
                        ...
                        if (item) {
                            ...
                            const oldMaxZWindow =
                                oldWindowManager.getMaxZWindow((control) => {
                                    return (
                                        control._options
                                            .isCompoundNotification !== true
                                    );
                                });
                            const oldMaxZIndex =
                                oldMaxZWindow?.getZIndex() || baseOldZIndex;
                            const maxZIndex = Math.max(
                                oldMaxZIndex,
                                managerWrapperMaxZIndex
                            );
                ...
                if (!isVDOMTemplate(rootTpl) && !rootTpl.isReact) {
                    requirejs(
                        ...
                        (compatiblePopup) => {
                            compatiblePopup.BaseOpener._prepareConfigForOldTemplate(
                                cfg,
                                rootTpl
                            );
    ...
    static requireModules(
        ...
        return new Promise<ILoadDependencies | Error>((resolve, reject) => {
            ...
            if (ManagerController.getRightPanelBottomTemplate()) {
                modules.push(
                    BaseOpener.requireModule(
                        ManagerController.getRightPanelBottomTemplate()
                    )
                );
            }
            Promise.all(modules)
