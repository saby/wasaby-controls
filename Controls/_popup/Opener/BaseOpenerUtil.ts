...
export default {
    loadCompatibleLayer(callback: Function, errorFn?: Function): void {
        const layerCompatibleModuleName: string =
    ...
    getIndicatorConfig(
        ...
        const findParentPopupId = () => {
            if (!cfg.opener?._destroyed) {
                const parentControls: Control[] = goUpByControlTree(
                    cfg.opener?._container
                );
                for (let i = 0; i < parentControls.length; i++) {
                    if (
                        parentControls[i]._moduleName ===
    ...
    getManager(): Promise<void> {
        if (!ManagerWrapperCreatingPromise) {
            if (!isNewEnvironment()) {
                ...
                ManagerWrapperCreatingPromise = new Promise(
                    (resolve, reject) => {
                        ...
                        Promise.all(compatibleDeps)
                            .then(
                                ([
                                    ...
                                    const managerCfg = {};
                                    const theme =
                                        compatibleController.default.getTheme();
                                    if (theme) {
                                    ...
                                    requirejs(
                                        ...
                                        (PageBaseLib) => {
                                            if (PageBaseLib) {
                                                managerCfg.dataLoaderModule =
                                                    'Page/base:DataLoader';
                                            }
                                            base.AsyncCreator(
