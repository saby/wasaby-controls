...
interface INotificationOpenerOptions
    extends INotificationPopupOptions,
        IBaseOpenerOptions {}
...
const compatibleOpen = (
    popupOptions: INotificationPopupOptions
): Promise<string> => {
    const config: INotificationPopupOptions = BaseOpenerUtil.getConfig(
        {},
        popupOptions
    );
    delete config.id;
    return new Promise((resolve) => {
        Promise.all([
            ...
            const compatibleConfig = getCompatibleConfig(
                BaseOpenerCompat,
                config
            );
...
const getCompatibleConfig = (
    BaseOpenerCompat: any,
    config: INotificationPopupOptions
) => {
 ...
 */
class Notification
    extends BaseOpener<INotificationOpenerOptions>
    implements INotificationOpener
{
    ...
    // for tests
    protected _getCompatibleConfig(
        BaseOpenerCompat: any,
        config: INotificationPopupOptions
    ) {
    ...
    private static _privateOpenPopup(
        config: INotificationPopupOptions
    ): Promise<string> {
        return new Promise((resolve, reject) => {
            if (!config.hasOwnProperty('isHelper')) {
                Logger.warn(
                    'Controls/popup:Dialog: Для открытия нотификационных окон из ' +
            ...
            if (isNewEnvironment()) {
                ...
                BaseOpener.requireModules(config, POPUP_CONTROLLER)
