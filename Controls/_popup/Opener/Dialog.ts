...
import {
    IDialogOpener,
    IDialogPopupOptions,
} from 'Controls/_popup/interface/IDialog';
...
const getDialogConfig = (
    config: IDialogOpenerOptions
): IDialogOpenerOptions => {
    config = config || {};
 ...
 */
class Dialog
    extends BaseOpener<IDialogOpenerOptions>
    implements IDialogOpener, IPropStorage
{
    ...
    open(popupOptions: IDialogOpenerOptions): Promise<string | undefined> {
        ...
        if (getConfig('isAdaptive') && config.allowAdaptive !== false) {
            return import('Controls/popup').then((popup) => {
                return Dialog._openSliding(
                    config,
        ...
        return super.open(
            this._getDialogConfig(popupOptions),
            POPUP_CONTROLLER
        );
    }

    private _getDialogConfig(
        popupOptions: IDialogOpenerOptions
    ): IDialogOpenerOptions {
    ...
    static _openPopup(config: IDialogOpenerOptions): CancelablePromise<string> {
        ...
        if (getConfig('isAdaptive') && newCfg.allowAdaptive !== false) {
            return import('Controls/popup').then((popup) => {
