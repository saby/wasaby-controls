 ...
 * @class Controls/_popup/SlidingPanelOpener
...
export default class SlidingPanelOpener extends Base<ISlidingPanelOptions> {
    ...
    open(popupOptions): Promise<string | void | Error> {
        const adaptivePopupOptions = this._getPopupOptionsWithSizes({
            ...popupOptions,
            ...this._options,
        });
    ...
    private _isMobileMode(): boolean {
        return this._options?.isAdaptive === false || getAdaptiveModeForLoaders().device.isPhone();
    }
    ...
    private _getPopupOptionsWithSizes(
        ...
        /*
            Если открываемся на desktop, то открываемся другим опенером и в контроллер SlidingPanel не попадаем,
