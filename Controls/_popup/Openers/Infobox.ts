...
import {
    ...
    getTargetForSubscribe,
} from 'Controls/_popup/utils/SubscribeToScroll';
...
export default class InfoboxOpener extends Base<IInfoBoxPopupOptions> {
    ...
    open(popupOptions: IInfoBoxPopupOptions): Promise<string | void | Error> {
        ...
        const openCallback = (config) => {
            return super.open(config).then((id) => {
                // На странице может быть только один инфобокс, записываем его
                InfoBoxId = id;
    ...
    private _getInfoBoxConfig(config: IInfoBoxPopupOptions): IInfoBoxPopupOptions {
        ...
        const onOpenHandler = () => {
            ...
            toggleActionOnScroll(newCfg.target, true, this._popupId, () => {
                this.close(0);
            });
        };
        const onCloseHandler = () => {
            ...
            const items = GlobalController.getController().getPopupItems();
            let infoBoxItem;
            items.each((item: IPopupItemInfo) => {
                if (item.controller?.TYPE === 'InfoBox') {
                    infoBoxItem = item;
                }
            });
            // Закрытие одного инфобокса может произойти уже после открытия другого. В таком случае, если они находятся
            // в одном скролл-контейнере, закрывающийся попап сотрет подписку на скролл у открывшегося.
            if (
                getTargetForSubscribe(infoBoxItem?.popupOptions.target) !==
                getTargetForSubscribe(newCfg.target)
            ) {
                toggleActionOnScroll(newCfg.target, false, infoBoxItem?.id);
            }
        };
        ...
        return {
            ...
            loadConfig: newCfg.loadConfig,
            loadConfigGetter: newCfg.loadConfigGetter,
    ...
    private static _preparePosition(targetSide: string, alignment: string = 'center'): string {
