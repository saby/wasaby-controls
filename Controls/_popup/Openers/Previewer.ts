...
/**
 * Отчищает таймаут закрытия дочерних окон
...
const close = (callback: Function, config: IPreviewerPopupOptions, type?: string): void => {
    ...
    if (type === 'hover' || (!type && config.type === 'hover')) {
 ...
 * @remark
 * Используется для открытия окон в универсальном опенере Controls/popup:Opener
 * @private
 */
export default class Previewer extends Base<IPreviewerPopupOptions> {
    ...
    open(popupOptions: IPreviewerPopupOptions, type?: string): Promise<string | void | Error> {
        return new Promise((resolve) => {
            ...
            const baseOnOpen = config.eventHandlers?.onOpen;
            const baseOnClose = config.eventHandlers?.onClose;

            config.eventHandlers?.onOpen = (...args) => {
                if (config.actionOnScroll !== 'none') {
                    toggleActionOnScroll(config.target, true, config.id, () => {
                        super.close(this._popupId);
                    });
                }
                if (baseOnOpen) {
                    baseOnOpen(...args);
            ...
            config.eventHandlers?.onClose = (...args) => {
                if (config.actionOnScroll !== 'none') {
                    toggleActionOnScroll(config.target, false, config.id);
                }
                if (baseOnClose) {
                    baseOnClose(...args);
                }
            ...
            config.id = config.id || this._popupId || randomId('popup-');
            ...
            // Для работы через типажи сводим API опенеров
