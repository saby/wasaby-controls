...
/**
 * Приватный опенер нотификационных окно
 * @implements INotificationPopupOptions
 * @remark
...
export default class Notification extends Base<INotificationPopupOptions> {
    ...
    static zIndexCallback(item: IPopupItemInfo, popupItems: List<IPopupItemInfo>): number {
        ...
        for (let i = 0; i < count; i++) {
            if (
                popupItems.at(i).type === 'Notification' &&
                popupItems.at(i).popupOptions.topPopup
            ) {
                return (popupItems.getIndexByValue('id', item.id) + 1) * TOP_POPUP_ZINDEX_STEP;
            }
