...
export default class Base {
            ...
            if (config.pageId) {
                ...
                if (!isNewEnvironment()) {
                    PopupPageController.setPageTemplate(
                        'SabyPage/popup:Template'
                    );
                    PageController.setPageConfigLoaderModule(
                        'SabyPage/base:ConfigLoader'
                    );
                    PageController.setDataLoaderModule(
                        'SabyPage/base:PrefetchLoader'
                    );
                }
            }
    ...
    private _getOpener(): IOpenerStaticMethods {
        const message =
            'Controls/popup: Попытка открыть окно с помощью задестроенного опенера';
        if (!this._opener) {
    ...
    private _showIndicator(config: IBasePopupOptions): void {
        ...
        if (popupItem) {
            ...
            const indicatorConfig = BaseOpenerUtil.getIndicatorConfig(
                null,
                config
            );
