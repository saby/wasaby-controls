...
class SlidingPanelOpener extends BaseOpener {
    static _openPopup(
        config: ISlidingPanelPopupOptions,
        popupController: string
    ): Promise<string> {
        let controller = popupController;
        if (!controller) {
            controller =
                config.desktopMode === 'stack'
                    ? SLIDING_STACK_CONTROLLER
                    : SLIDING_CONTROLLER;
        }
...
export default class SlidingPanel extends Base {
    ...
    open(popupOptions: ISlidingPanelPopupOptions): unknown {
        const adaptivePopupOptions =
            this._getPopupOptionsWithSizes(popupOptions);
        ...
        if (this._isMobileMode()) {
            ...
            return this._getDesktopOpener(desktopMode).open(
                adaptivePopupOptions
            );
        }
    }
    ...
    private _getPopupOptionsWithSizes(
        ...
        const options = isMobileMode
            ? slidingPanelOptions
            : popupOptions.dialogOptions;
        ...
        resultPopupOptions.templateOptions.adaptiveOptions =
    ...
    private _validateOptions({
        slidingPanelOptions,
    }: ISlidingPanelPopupOptions): void {
        const heightList =
            slidingPanelOptions?.heightList ||
    ...
    protected _validateHeightList(
        heightList: ISlidingPanelOptions['heightList']
    ): void {
