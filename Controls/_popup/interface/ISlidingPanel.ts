 ...
 * @cfg {boolean} Определяет, должна ли шторка адаптировать способ открытия в зависимости от того вызывается открытие на мобильном устройстве или нет.
 ...
 * @property {Number} minHeight Минимально допустимая высота окна, px. С такой высотой оно открывается. Данная опция работает только с desktopMode=dialog.
 * @property {Number} maxHeight Максимально допустимая высота окна, px. Данная опция работает только с desktopMode=dialog.
 ...
 * @property {String} restrictiveContainer Определяет селектор контейнера в рамках которого будет строиться шторка.
 ...
 * @property {Boolean} shouldSwipeOnContent Опредяляет можно ли закроыть шторку свайпом сверху вниз посодержимому
 * @property {Boolean} isMobileMode Определяет нужно ли отображать окно в виде шторки на desktop
 * @property {'dialog' | 'stack' | 'sticky'} desktopMode Определяет режим отображения окна на desktop
 * @property {Number} height Определяет высоту шторки
 */

/**
 * @typedef {Object} Controls/_popup/interface/ISlidingPanel/PopupOptions
 * @description Конфигурация окна.
 * @property {String} className Имена классов, которые будут применены к корневому узлу всплывающего окна.
