...
export interface IStackPopupOptions extends IBasePopupOptions, IAdaptivePopupOptions {
    ...
    minWidth?: number | 'a' | 'b' | 'c' | 'd' | 'e' | 'f' | 'g';
    maxWidth?: number | 'a' | 'b' | 'c' | 'd' | 'e' | 'f' | 'g';
    sidebarCanHide?: boolean;
...
/**
 * Интерфейс для опций {@link https://n.sbis.ru/article/558ad4e6-866f-43a6-8093-af46125111ca стековых окон}.
 * @public
...
/**
 * Метод открытия {@link https://n.sbis.ru/article/558ad4e6-866f-43a6-8093-af46125111ca стекового окна}.
 ...
 * @property {Number|Controls/_popup/interface/IStackOpener/BaseSizes.typedef} minWidth Минимально допустимая ширина стекового окна. Значение указывается в px. Также поддерживается стандартная линейка размеров.
 * @property {Number|Controls/_popup/interface/IStackOpener/BaseSizes.typedef} maxWidth Максимально допустимая ширина стекового окна. Значение указывается в px. Также поддерживается стандартная линейка размеров.
 * @property {Number|Controls/_popup/interface/IStackOpener/BaseSizes.typedef} width Текущая ширина стекового окна. Значение указывается в px. Также поддерживается стандартная линейка размеров.
 ...
 * Значение указывается в px. Также поддерживается стандартная линейка размеров.
 * @remark
 ...
 * указывается в px. Также поддерживается стандартная линейка размеров.
 * @remark
 * Значение может быть задано как на опциях Controls/popup:Stack, так и на дефолтных опциях шаблона {@link template}.
 * Приоритетнее то, которое задано на Controls/popup:Stack.
 * @demo Controls-demo/Popup/Stack/BaseWidthSizes/Index
...
/**
 * @name Controls/_popup/interface/IStackOpener#isWorkspacePopup
 * @cfg {Boolean} Определяет, будет ли окно открыто по рабочей области.
 * @remark
 * Опция позиционирует и растягивает окно по ширине рабочей области. Окно при этом будет
 * выглядеть как часть контента страницы. Опцию стоит использовать в местах, где перерисовка
 * контента занимает много времени.
 * Под рабочей областью подразумевается центральный контент страницы без аккордеона и ЦУ с классом
 * '.sabyPage-MainLayout__content'.
 * ВАЖНО!
 * Опция должна долететь до шаблона, либо проставлена самостоятельно, иначе останется нежелательная тень.
 */
