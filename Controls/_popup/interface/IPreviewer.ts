...
import { ILoadConfigOptions } from 'Controls/popup';

type SyntheticMouseEvent = SyntheticEvent<MouseEvent>;

export interface IPreviewerOptions<T = any> extends IControlOptions, ILoadConfigOptions {
    opener?: Control;
    content?: Function;
    trigger: string;
    ...
    onOpen?: () => void;
    onClose?: () => void;
    onResult?: (event: unknown, ...args: unknown[]) => void;
    onStartClosing?: () => void;
    onStartOpening?: () => IPreviewerOptions;
}
 ...
 * @cfg {number} Задержка перед появлением всплывающего окна при заданной опции trigger, принимающей значение hover или hoverAndClick. Задаётся в мс.
 */

/**
 * @name Controls/_popup/interface/IPreviewer#shouldWaitCursorToStop
 * @cfg {boolean} Определяет необходимость открытия окна сразу при наведении на контрол. При значении true окно открывается только после остановки курсора на контроле.
 * @default true
 */
