...
export type TActionOnScroll = 'close' | 'track' | 'hide' | 'none';
...
export interface IStickyPopupOptions
    ...
    direction?: IStickyPosition;
    offset?: IStickyPositionOffset;
    fittingMode?: TFittingMode | IFittingMode;
...
type TFittingMode = 'fixed' | 'adaptive' | 'overflow' | 'adaptiveOverflow' | 'initialAdaptive';

export interface IFittingMode {
    vertical?: TFittingMode;
    horizontal?: TFittingMode;
}
...
/**
 * Интерфейс для методов {@link /doc/platform/developmentapl/interface-development/controls/openers/sticky/ прилипающих окон}.
 * @public
 ...
 * @variant close Прилипающее окно закрывается.
 * @variant track  Прилипающее окно движется вместе со своей точкой позиционирования.
 * @variant hide Прилипающее окно пропадает, пока скролл не остановится, а затем появляется у таргета.
 * @variant none Прилипающее окно остается на месте расположения, вне зависимости от движения точки позиционирования.
 */

/**
 * @typedef {Object} Controls/_popup/interface/IStickyOpener/PopupOptions
 * @description Конфигурация прилипающего окна.
 * @property {Boolean} autofocus Установится ли фокус на шаблон попапа после его открытия.
 * @property {Controls/_popup/interface/IStickyOpener/TActionOnScroll.typedef} actionOnScroll Реакция прилипающего окна на скролл родительской области.
 * @property {Boolean} modal Будет ли открываемое прилипающее окно блокировать работу пользователя с родительским приложением.
 * @property {String} className Имена классов, которые будут применены к корневой ноде прилипающего окна.
 * @property {Boolean} closeOnOutsideClick Возможность закрытия прилипающего окна по клику вне.
 * @property {function|String} template Шаблон прилипающего окна.
 * @property {function|String} templateOptions Опции для контрола, переданного в {@link template}.
 * @property {Controls/popup:IStickyPosition} targetPoint Точка позиционирования прилипающего окна относительно вызывающего элемента.
 * @property {Controls/_popup/interface/IStickyOpener/Direction.typedef} direction Выравнивание прилипающего окна относительно точки позиционирования.
 * @property {Controls/_popup/interface/IStickyOpener/Offset.typedef} offset Отступы от точки позиционирования до прилипающего окна.
 * @property {Number} minWidth Минимальная ширина прилипающего окна.
 * @property {Number} maxWidth Максимальная ширина прилипающего окна.
 * @property {Number} minHeight Минимальная высота прилипающего окна.
 * @property {Number} maxHeight Максимальная высота прилипающего окна.
 * @property {Number} height Текущая высота прилипающего окна.
 * * @property {Controls/_popup/interface/IPopupWidth/TPopupWidth.typedef} width Текущая ширина прилипающего окна.
 * @property {Node|Control} target Элемент (DOM-элемент или контрол), относительно которого позиционируется прилипающее окно.
 * @property {Node} opener Логический инициатор открытия прилипающего окна (см. {@link /doc/platform/developmentapl/interface-development/ui-library/focus/activate-control/#control-opener Определение понятия "опенер контрола"}).
 * @property {Controls/_popup/interface/IStickyOpener/FittingMode.typedef} fittingMode Поведение окна, в случае, если он не помещается на экране с заданным позиционированием.
 * @property {Controls/_popup/interface/IBasePopupOptions/EventHandlers.typedef} eventHandlers Функции обратного вызова на события прилипающего окна.
 ...
 * @cfg {Number} Минимальная ширина прилипающего окна.
 */

/**
 * @name Controls/_popup/interface/IStickyOpener#maxWidth
 * @cfg {Number} Максимальная ширина прилипающего окна.
 */

/**
 * @name Controls/_popup/interface/IStickyOpener#minHeight
 * @cfg {Number} Минимальная высота прилипающего окна.
 */

/**
 * @name Controls/_popup/interface/IStickyOpener#maxHeight
 * @cfg {Number} Максимальная высота прилипающего окна.
 */
/**
 * @name Controls/_popup/interface/IStickyOpener#height
 * @cfg {Number} Текущая высота прилипающего окна.
 */

/**
 * @name Controls/_popup/interface/IStickyOpener#target
 * @cfg {Node|Control} Элемент, относительно которого позиционируется прилипающее окно.
 * @variant DOM-элемент
 * @variant Контрол
 ...
 * @cfg {Controls/_popup/interface/IStickyOpener/TActionOnScroll.typedef} Определяет реакцию прилипающего окна на скролл родительской области.
 * @default none
 ...
 * @cfg {Controls/_popup/interface/IStickyOpener/Direction.typedef} Точка позиционирования прилипающего окна относительно вызывающего элемента.
 */
 ...
 * @cfg {Controls/_popup/interface/IStickyOpener/Direction.typedef} Выравнивание прилипающего окна относительно точки позиционирования.
 */
 ...
 * @cfg {Controls/_popup/interface/IStickyOpener/Offset.typedef} Конфигурация отступов от точки позиционирования до прилипающего окна.
 */
 ...
 * @cfg {String} Стиль фона прилипающего окна.
 * @demo Controls-demo/dropdown_new/Button/MenuPopupBackground/Index
 */

/**
 * @name Controls/_popup/interface/IStickyOpener#fittingMode
 * @cfg {Controls/_popup/interface/IStickyOpener/FittingMode.typedef} Поведение окна, в случае, если он не помещается на экране с заданным позиционированием.
 */
 ...
 * @variant adaptiveOverflow Координаты точки позиционирования (см. {@link Controls/_popup/interface/IStickyOpener#targetPoint targetPoint}) и выравнивание (см. {@link Controls/_popup/interface/IStickyOpener#direction direction}) меняются на противоположные. Если и в этом случае окно не помещается на экран, выбирается тот способ позиционирования (изначальный или инвертируемый), при котором на экране помещается наибольшая часть контента. Например, если поле ввода с автодополнением находится внизу экрана, то список автодополнения раскроется вверх от поля. Ширина и высота при этом не уменьшаются.
 * @variant initialAdaptive При первом построении ведет себя так же, как значение adaptive, затем фиксирует свою позицию и ведет себя значение fixed.
 *
 */
...
/**
 * Метод открытия прилипающего окна.
 * @function Controls/_popup/interface/IStickyOpener#open
 * @param {Controls/_popup/interface/IStickyOpener/PopupOptions.typedef} popupOptions Конфигурация прилипающего окна.
 * @return Promise<void>
 * @remark
