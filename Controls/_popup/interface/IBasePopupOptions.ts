...
import { ILoadConfigOptions } from 'Controls/_popup/interface/ILoadConfig';
...
export interface IBasePopupOptions<T = object> extends IEventHandlersOptions, ILoadConfigOptions {
     ...
     * @remark 'display: none' навешивается для оптимизации работы с несколькими попапами.
     */
    ...
    asyncShow?: boolean;
    /**
     * @cfg {Boolean} Определяет, открывать ли окно в адаптиве с анимацией.
     * @default true
     * @demo Controls-demo/Popup/SlidingPanel/Animation/Index
     */
    animation?: boolean;
    id?: string;
     ...
     * @see template
     * @remark Также есть возможность описания опций шаблона с помощью дженерика IBasePopupOptions<T>
     */
    templateOptions?: T;
    /**
    ...
    modal?: boolean;
    /**
     * @cfg Определяет стиль оверлея, когда включена модальность окна.
     * @default default
     */
    overlayBackgroundStyle?: 'default' | 'none';
    /**
     ...
     * @deprecated Опция устарела. Воспользуйтесь {@link /docs/js/SabyPage/pageOpener/Opener опенером страницы в окне}.
     ...
     * @example
     * В этом примере открывается окно с подсказкой. Для этого окна z-index выставляется на один больше, чем у родителя,
    ...
    /**
     * @cfg Значение активной темы, которую нужно применить на окно.
     */
    activeTheme?: IActiveTheme;

    /**
     * @cfg {Boolean} Определяет, нужно ли наследовать активную тему опенера, полученную из контекста.
     * @default true
     */
    shouldInheritOpenerTheme?: boolean;

    /**
     * @cfg {string} Имя атрибута data-qa, которое навесится на корневой контейнер окна. Используется для автотестирования.
     */
    dataQa?: string;
}
 ...
 * @public
 */
export interface IAdaptiveOptions extends IBasePopupOptions {
    /**
