...
import {
    IPreviewer,
    IPreviewerOptions,
} from 'Controls/_popup/interface/IPreviewer';
...
class PreviewerTarget extends Control<IPreviewerOptions> implements IPreviewer {
    ...
    open(type: string): Promise<void> {
        if (!this._isPopupOpened()) {
            ...
            this._isOpened = true;
            return PreviewerOpener.openPopup(newConfig, type).then(
                (id: IPreviewerPopupOptions) => {
    ...
    private _getConfig(): IPreviewerPopupOptions {
        const opener =
            typeof this._options.opener === 'undefined'
                ? this
                : this._options.opener;
    ...
    protected _contentMouseenterHandler(
        event: SyntheticEvent<MouseEvent>
    ): void {
        this._wasMouseEnter = true;
        if (
            this._options.trigger === 'hover' ||
            this._options.trigger === 'hoverAndClick'
        ) {
    ...
    protected _contentMouseleaveHandler(
        event: SyntheticEvent<MouseEvent>
    ): void {
        if (
    ...
    protected _contentMousemoveHandler(
        event: SyntheticEvent<MouseEvent>
    ): void {
