 ...
 * @see Controls/_popup/Popup/Controllerr#removeOpenPopupCallback
 */

/**
 * Метод, который удаляет переданную фунцию из стека коллбек функций.
 * @function Controls/_popup/Popup/Controller#removeOpenPopupCallback
 * @param {Function} openPopupCallback Коллбек функция, которую нужно удалить.
 * @see Controls/_popup/Popup/Controller#setOpenPopupCallback
 */

/**
 * Метод, который определяет положение окон на сайте
 * @function Controls/_popup/Popup/Controller#setStackPosition
 * @param {String} position Принимает значения 'left' и 'right'
 */

/**
 * Метод, который вставляет контент внизу правой панели стекового окна
 * @function Controls/_popup/Popup/Controller#setRightPanelBottomTemplate
 * @param {String} template Путь до шаблона контента
 * @param {Boolean} isVisible Определяет видимость контента. Треуется в редких случаях, когда нужно чтобы контент отрисовался, но был невидим для пользователя.
 * @remark
 * Метод достаточно вызывать один раз, поэтому он должен быть объявлен в корне страницы.
 *
 */
...
class GlobalController {
    ...
    private _bottomTemplateVisible: boolean = true;
    ...
    private _accordionTemplate: string;

    private _containerPromise: Promise<Container>;
    private _containerPromiseResolver: Function;
    ...
    // TODO: Убрать после того, как переведем прикладинков на новое API https://online.sbis.ru/opendoc.html?guid=1bd40f3c-89e4-419e-9b21-97aadf293fab&client=3
    find(id: string) {
    ...
    setContainer(container: Container): void {
        ...
        if (this._containerPromiseResolver) {
            this._containerPromiseResolver();
    ...
    getContainerPromise(): Promise<Container> {
        if (!this._containerPromise) {
            this._containerPromise = new Promise((resolve) => {
                this._containerPromiseResolver = resolve;
            });
        }
    ...
    setRightPanelBottomTemplateVisible(visible: boolean): void {
        this._bottomTemplateVisible = visible;
    }

    getRightPanelBottomTemplateVisible(): boolean {
        return this._bottomTemplateVisible;
    }
