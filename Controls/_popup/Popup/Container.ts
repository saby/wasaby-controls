...
class Container extends Control<IControlOptions> {
    ...
    protected _overlayBackgroundStyle: string;
    ...
    protected _afterMount(): void {
        ...
        if (serverSidePopup) {
            const serverSidePopupConfig = {
                ...
                id: serverSidePopup.id,
                isServerSidePopup: true,
            };
    ...
    protected _beforeUnmount(): void {
        GlobalController.setContainer(null);
    }
    ...
    private _calcOverlayIndex(
        ...
        if (typeof maxModalPopupIndex !== 'undefined') {
            this._overlayBackgroundStyle =
                popupItems.at(maxModalPopupIndex)?.popupOptions?.overlayBackgroundStyle ||
                'default';
        }
    }
    ...
    protected _overlayClickHandler(event: Event, item: IPopupItem): void {
        if (
            (!item.popupOptions.closeOnOverlayClick && item.controller.TYPE !== 'SlidingPanel') ||
            (item.controller.TYPE === 'SlidingPanel' &&
                item.popupOptions.closeOnOverlayClick === false)
        ) {
            event.stopPropagation();
        }
        event.preventDefault();
