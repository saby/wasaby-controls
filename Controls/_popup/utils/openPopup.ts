...
import { Loader as LoaderInterface } from 'Controls-DataEnv/dataLoader';
...
/**
 * Запускает лоадеры, если они присутствуют в конфиге,
 * после их вызова вызывает коллбек, в котором должно быть открытие попапа
 * @param config
 * @param openCallback
 */
...
const _getLoader = (): Promise<LoaderInterface> => {
    return import('Controls-DataEnv/dataLoader').then(({ Loader }) => {
        return Loader;
    });
};
...
const showOnOldEnvironment = (
    ...
    return BaseOpenerUtil.getManagerWithCallback(() => {
        // при открытии через стат. метод опенера в верстке нет, нужно взять то что передали в опции
...
const _openPopup = async (
    ...
    if (!item || item.popupState !== 'startDestroying') {
        if (!popupIsCreating) {
            // Только для тестов, ждем инициализации контейнера окон
            const container = GlobalController.getContainer();
            if (!container) {
                await GlobalController.getContainerPromise();
            }
