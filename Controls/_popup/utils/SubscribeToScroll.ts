...
export const toggleActionOnScroll = (
    ...
    // Подписка на событие скрола нужна только если мы с работаем на мобильном IOS с телефона
    if (window?.addEventListener && detection.isMobileIOS && !detection.isMac) {
        if (toggle) {
