...
class InfoboxTarget extends Control<IInfoBoxOptions> implements IInfoBox {
    ...
    protected _beforeUpdate(options: IInfoBoxOptions): void {
        ...
        // дошли до шаблона
        if (this._options.templateOptions !== options.templateOptions && this._opened) {
            this.open(options);
            this._updating = true;
        }
    }
    ...
    private _getConfig(options?: IInfoBoxOptions): IInfoBoxPopupOptions {
        const closeOnOutsideClick =
            ...
            ((this._options.trigger === 'hover|touch' || this._options.trigger === 'hover') && detection.isMobilePlatform);
        ...
        return {
            ...
            eventHandlers,
            templateOptions: options?.templateOptions || this._options.templateOptions,
            indicatorConfig: this._options.indicatorConfig
        };
    }
    ...
    protected _contentTouchStartHandler(event: Event): void {
        if (this._options.trigger === 'hover|touch' || this._options.trigger === 'hover') {
            this._startOpeningPopup();
            const { target } = event;
    ...
    static getOptionTypes(): Record<string, Function> {
        return {
            trigger: descriptor(String).oneOf(['hover', 'click', 'hover|touch', 'demand']),
        };
    }
