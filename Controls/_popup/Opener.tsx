...
import { Component, RefObject, createRef, memo, ReactElement, ContextType } from 'react';
...
import { ThemeConsumer, IActiveTheme } from 'UI/Theme';
...
const EVENT_LIST = ['onClick', 'onMouseDown', 'onTouchStart', 'onContextMenu'];
...
const Portal = memo(function ({
    popupItem,
    className,
    activeTheme,
    getContentProps,
}: IPortal): ReactElement | null {
    if (!popupItem) {
        return null;
    }
    return createPortal(
        <Async
            ...
            templateOptions={{
                ...
                id: popupItem.id,
                item: popupItem,
                position: popupItem.position,
                activeTheme,
                templateOptions: getContentProps(),
                template: popupItem.popupOptions.template,
                name: popupItem.id,
            }}
...
export default class Opener extends Component<IPortalOpener> {
    ...
    context: ContextType<typeof FreezeContext> | null;
    ...
    constructor(props: IPortalOpener) {
        super(props);
        this._getContentProps = this._getContentProps.bind(this);
    ...
    open(props: IPortalOpener = {}): Promise<string> {
        ...
        return this._opener
            .open({
                ...
                allowAdaptive:
                    props.allowAdaptive ?? !!this._getPopupType(props.popupType).adaptiveViewMode,
    ...
    render() {
        return (
            <span ref={this._targetRef} className={`tw-block tw-hidden ${this.props.className}`}>
                <ThemeConsumer>
                    <Portal
                        popupItem={this.state?.popupItem}
                        className={this.state?.className}
                        getContentProps={this._getContentProps}
                    />
                </ThemeConsumer>
            </span>
        );
