...
class Popup extends Control<IPopupControlOptions> {
    ...
    protected _fullscreen(event: SyntheticEvent<Event>, state: boolean): void {
        ManagerController.notifyToManager('popupFullscreen', [this._options.id, state]);
    }
    ...
    protected _popupMovingSize(
        ...
        ManagerController.notifyToManager('popupMovingSize', [this._options.id, offset, position]);
    }
    ...
    private _resizeObserverCallback(entries: object[]): void {
        ...
        const heightChanged = typeof currentHeight === 'number' && currentHeight !== entries[0].contentRect.height;
        const widthChanged = typeof currentWidth === 'number' && currentWidth !== entries[0].contentRect.width;
