...
class Container extends Control<IControlOptions> {
    ...
    protected _getHiddenClass(item: IPopupItem): string {
        ...
        if (item.popupOptions.slidingPanelOptions && unsafe_getRootAdaptiveMode().device.isPhone()) {
            return;
        }
    ...
    protected _getPositionStyle(item: IPopupItem): string {
        if (this._options.adaptiveMode.device.isPhone() && item.controller.TYPE === 'Stack') {
            return 'fixed';
        }
