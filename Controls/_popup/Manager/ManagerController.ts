...
export default {
    ...
    loadData(dataLoaders: unknown[]): Promise<unknown> {
        const loaderModule = this._callManager(
            'getDataLoaderModule',
            arguments
        );
        if (!loaderModule) {
    ...
    isPopupCreating(id: string): boolean {
        ...
        if (item) {
            ...
            return (
                (item.popupState === 'initializing' ||
                    item.popupState === 'creating') &&
                !popupInstance
            );
        }
        return false;
    ...
    hasRightPanel(): boolean {
        ...
        const compatible =
            !isNewEnvironment() && this._theme !== 'default__dark';
        return (
            !!this._rightBottomTemplate || !!this._rightTemplate || compatible
        );
    },
