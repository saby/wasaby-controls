...
export interface IAdaptiveChipsProps extends IChipsOptions {
    multiSelect?: boolean;
}

/**
 * Контрол "Адаптивные чипсы"
 * @class Controls/AdaptiveChips
 * @implements Controls/Chips:IChipsOptions
 * @ignoreOptions multiline
 * @demo Controls-demo/AdaptiveChips/Index
 * @public
 */
export default forwardRef(function AdaptiveChips(
    props: IAdaptiveChipsProps,
    ref: LegacyRef<HTMLDivElement>
) {
    const { className, multiSelect = true, ...chipsProps } = props;
    ...
    const scrollStateChangedHandler = useCallback((state: IScrollState) => {
    ...
    const checkSelectedKeyOnError = (): void => {
        ...
        if (props.items && unexistingKeys?.length) {
            Logger.error(
                'Controls/AdaptiveChips: В RecordSet нет элемент(а/ов) с идентификатор(ом/ами),' +
                    ` переданным в опцию selectedKeys='${unexistingKeys}'`
            );
        }
        if (!multiSelect && props.selectedKeys?.length > 1) {
            Logger.error(
                'Controls/AdaptiveChips: При отключённом режиме множественного выбора, количество выбранных элементов больше 1.'
            );
        }
    };

    const menuSelectedItemKeysChanged = useCallback((keys: unknown[] | unknown) => {
        if (chipsProps.onSelectedKeysChanged) {
            chipsProps.onSelectedKeysChanged(null, Array.isArray(keys) ? keys : [keys]);
        }
    }, []);

    const chipsSelectedKeyChangeHandler = useCallback((_: Event, key: unknown) => {
        if (chipsProps.onSelectedKeysChanged) {
            chipsProps.onSelectedKeysChanged(null, [key]);
        }
    }, []);
        ...
        if (activeContainer) {
            ...
            if (
                ...
                scrollRef.current?.horizontalScrollTo?.(
                    scrollLeft.current > newScrollPosition
                        ? newScrollPosition - SHADOW_OFFSET
        ...
        >
            ...
            >
                <div className="tw-w-full tw-flex">
                    {!multiSelect ? (
                        <SelectedKey
                            selectedKey={chipsProps.selectedKeys[0]}
                            onSelectedKeyChanged={chipsSelectedKeyChangeHandler}
                        >
                            <Control
                                {...chipsProps}
                                className="controls-AdaptiveChips__buttonContainer"
                                multiline={false}
                                items={chipsProps.items}
                            />
                        </SelectedKey>
                    ) : (
                        <Control
            ...
            <Selector
                ...
                displayProperty={chipsProps.displayProperty || 'caption'}
...
/**
 * @name Controls/AdaptiveChips#multiSelect
 * @cfg {Boolean} Определяет наличие множественного выбора.
 * @default true
 * @demo Controls-demo/AdaptiveChips/MultiSelect/Index
 */
