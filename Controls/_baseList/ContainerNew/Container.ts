...
function validateOptionsOnContainer(options: IListContainerOptions): void {
    ...
    VIEW_OPTIONS.forEach((optionName) => {
        if (
            ...
            !excludedOptions.includes(optionName)
        ) {
...
export default class ListContainer extends Control<IListContainerOptions> {
    ...
    constructor(options: IListContainerOptions) {
        ...
        this._selectedKeysCountChanged =
            this._selectedKeysCountChanged.bind(this);
    ...
    protected _registerHandler(
        ...
        this._getSelectionTypeRegister().register(
            event,
            registerType,
            component,
            callback,
    ...
    protected _selectedKeysCountChanged(
        ...
        this._listSlice.setSelectionCount(
            count,
            isAllSelected,
    ...
    protected _rootChanged(root: TKey): void {
        if (this._rootLoadPromise[root]) {
            this._rootLoadPromise[root].then((items) => {
                ...
                this._listSlice.setItems(items);
    ...
    private _isViewOptionsChanged(listSlice: ListSlice): boolean {
        return !!VIEW_OPTIONS.find((optionName) => {
            return !isEqual(
                this._viewOptions[optionName],
                listSlice[optionName]
            );
        });
    }
    ...
    private _showError(): void {
        if (!this._errorConfig) {
            this._getErrorController().then((errorController) => {
                const sliceErrConfig = this._listSlice
                    .errorConfig as IListSliceErrorConfig;
    ...
    private _getErrorController(): Promise<ErrorController> {
        if (this._errorController) {
            ...
            return loadAsync<typeof import('Controls/error')>(
                'Controls/error'
    ...
    private static getSlice({
        _dataOptionsValue,
        storeId,
    }: IListContainerOptions): ListSlice {
        const slice = _dataOptionsValue.getStoreData(storeId) as ListSlice;
        ListContainer.validateSlice(slice, storeId);
        return slice;
    }
