import { TResultsPosition } from 'Controls/_baseGrid/display/interface/IGridControl';
...
export function getBottomPaddingClass(params: IGetBottomPaddingClassParams): TBottomPaddingClass {
    ...
    if (
        ...
        (bottomPaddingMode !== 'none' || hasPaging)
    ) {
        if (itemActionsPosition === 'outside' && (!isTouch || isEditing === true)) {
            return 'controls-itemActionsV_outside-spacing';
        } else {
            if (bottomPaddingMode === 'separate' && !hasPaging) {
