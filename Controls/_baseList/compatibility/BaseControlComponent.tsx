...
import { SingleColumnMarkerStrategy, getMarkerStrategy } from 'Controls/marker';
...
export const NEW_BASE_CONTROL_DEFAULT_OPTIONS = {
    ...
    markerStrategy: SingleColumnMarkerStrategy,
};
...
> extends React.Component<TProps, any> {
    ...
    private _setCollection(collection: Collection | null, isOnInit?: boolean): boolean {
        ...
        const setCollectionInSlice = (doOnFirstSetCallback: () => void) => {
            const isEmptyCollectionBeforeSet = this.props?.slice?.collection === undefined;
        ...
        const doOnFirstSet = () => {
            ...
            if (collection?.setExpansionModel) {
                collection.setExpansionModel(this.props.slice.state.expansionModel);
                collection.setExpandedItems(this.props.slice.state.expandedItems);
                collection.setCollapsedItems(this.props.slice.state.collapsedItems);
            }
...
function moveMarkerToDirection(
    ...
    if (props.markerVisibility !== 'hidden' && !!collection?.getCount()) {
        ...
        const strategy = getMarkerStrategy(control.getViewModel());
        const collection = control.getViewModel();
        let newMarkedKey;
