...
export class AsyncListVirtualScrollController extends ListVirtualScrollController {
    ...
    protected _applyIndexes(
        ...
        if (this._isReset || this._isAllRendered({ startIndex, endIndex })) {
            ...
            if (this._prevHasItemsOutsideRange) {
                this._hasItemsOutRangeChangedHandler(
                    this._prevHasItemsOutsideRange
                );
            }
        } else {
            ...
            if (typeof this._startIndexShift === 'number') {
                super._applyIndexes(
                    startIndex + this._startIndexShift,
                    endIndex,
                    shiftDirection
                );
                this._startIndexShift = null;
            }
            this._collection.setNextIndexes(
                startIndex,
                endIndex,
                shiftDirection
            );
