...
import { IEdgeItemCalculatingParams } from 'Controls/_baseList/Controllers/ScrollController/AbstractListVirtualScrollController';
...
export interface IShiftRangeBySegmentParams
    extends IGetSegmentSizeToHideParams {
    calcMode: ICalcMode;
    pageSize: number;
    totalCount: number;
    segmentSize: number;
...
export interface IGetItemsSizesSumParams {
    ...
    itemsSizes: IItemsSizes;
    contentSizeBeforeList?: number;
    itemsRenderedOutsideRange?: string[];
 ...
 */
export function shiftRangeBySegment(
    params: IShiftRangeBySegmentParams
): IItemsRange {
    const {
        direction,
        segmentSize,
    ...
    const countItemsToFillPageSize = Math.max(
    ...
    const countItemsToEdge =
        direction === 'backward'
            ? countItemsToBackwardEdge
            : countItemsToForwardEdge;
    ...
    if (direction === 'backward') {
        ...
        if (calcMode === 'shift' || startIndex === endIndex) {
            startIndex = Math.max(
                Math.min(
                    startIndex + segmentSizeToHide,
                    Math.max(endIndex - pageSize, 0)
                ),
                0
            );
        }
    ...
    startIndex = correctStartIndexForMultiColumns(
        startIndex,
        params.itemsSizes
    );
 ...
 */
function correctStartIndexForMultiColumns(
    startIndex: number,
    itemsSizes: IItemsSizes
): number {
...
function getSegmentSizeToHideForward(
    params: IGetSegmentSizeToHideParams
): number {
    let segmentSize = 0;
...
function getSegmentSizeToHideBackward(
    params: IGetSegmentSizeToHideParams
): number {
    let segmentSize = 0;
 ...
 */
function contentBeforeItemMoreThanViewport(
    params: IHasEnoughContentToScroll
): boolean {
 ...
 * @param {IHasEnoughContentToScroll} params
 */
function contentAfterItemMoreThanViewport(
    params: IHasEnoughContentToScroll
): boolean {
    const itemsSizesSum = getItemsSizesSum({
        range: {
 ...
 * @param {IHasEnoughContentToScroll} params
 */
export function hasEnoughContentToScroll(
    params: IHasEnoughContentToScroll
): boolean {
    ...
    const hasContentBeforeItem =
        (params.itemIndex === params.range.startIndex &&
            params.itemPosition === 'top') ||
        contentBeforeItemMoreThanViewport(params);
    return hasContentBeforeItem && contentAfterItemMoreThanViewport(params);
}
...
export function getRangeByIndex(params: IGetRangeByIndexParams): IItemsRange {
    const { start, end, pageSize, totalCount } = params;
 ...
 */
export function getRangeByItemsSizes(
    params: IGetRangeByItemsSizesParams
): IItemsRange {
    const itemsSizes = params.itemsSizes;
    let sumHeight = 0;
 ...
 */
export function getRangeByScrollPosition(
    params: IGetByPositionParams
): IItemsRange {
    const { pageSize, totalCount, itemsSizes, triggerOffset, scrollPosition } =
        params;
    ...
    while (
        ...
        itemsSizes[start].offset < scrollPosition - triggerOffset
    ) {
        start++;
    }
...
export function getActiveElementIndexByScrollPosition(
    ...
    if (scrollPosition < 0) {
        ...
        !feature1183225611 &&
        viewportSize + scrollPosition > contentSize
    ) {
    ...
    if (feature1183225611) {
        ...
        for (let i = currentRange.startIndex; i < currentRange.endIndex; i++) {
            ...
            if (
                Math.floor(
                    params.itemsSizes[i].offset - placeholders.backward
                ) <=
    ...
    if (!totalCount) {
        ...
        isRangeOnEdge('backward', currentRange, totalCount) &&
        fixedScrollPosition === 0
    ) {
        ...
        const scrollPositionWithPlaceholder =
            fixedScrollPosition + placeholders.backward;
        const knownContentHeight =
            contentSize + placeholders.forward + placeholders.backward;
        const viewportSizeWithoutOverlay = viewportSize - viewportOverlaySize;
        const indexLineRatio =
            scrollPositionWithPlaceholder /
            (knownContentHeight - viewportSizeWithoutOverlay);
        ...
        for (let i = currentRange.startIndex; i < currentRange.endIndex; i++) {
            ...
            if (
                itemOffset <
                fixedScrollPosition + viewportSizeWithoutOverlay * indexLine
            ) {
                activeElementIndex = i;
            } else {
                break;
 ...
 */
export function getPlaceholdersByRange(
    params: IGetSizesByRangeParams
): IPlaceholders {
...
function getItemsSizesSum(params: IGetItemsSizesSumParams): number {
    ...
    if (params.calcByOffset && range.endIndex - range.startIndex > 0) {
        ...
        const endItemSize = inEnd
            ? itemsSizes[range.endIndex - 1]
            : itemsSizes[range.endIndex];
        const startItemOffset = inStart
            ? params.contentSizeBeforeList
            : startItemSize.offset;
        const endItemOffset = inEnd
            ? endItemSize.size + endItemSize.offset
            : endItemSize.offset;
        ...
        const result =
            endItemOffset - startItemOffset - offsetBetweenLastItems / 2;
    ...
    for (let index = range.startIndex; index < range.endIndex; index++) {
        ...
        if (
            ...
            itemsRenderedOutsideRange.includes(itemSize?.key)
        ) {
            continue;
        }
 ...
 * @param params
 */
export function getFirstVisibleItemIndex(
    params: IGetFirstVisibleItemIndexParams
): number {
    const itemsSizes = params.itemsSizes;
    ...
    const stickyContentSize =
        params.scrollPosition === 0 ? 0 : params.stickyContentSize;
    ...
    while (
        itemIndex < maxIndex &&
        itemsSizes[itemIndex].offset - backwardPlaceholder <
            offsetForVisibleItem
    ) {
        itemIndex++;
    }
...
export function getEdgeVisibleItem(
    params: IGetEdgeVisibleItemParams
): IEdgeItem {
    const {
        viewportSize,
        scrollPosition,
    ...
    for (
        ...
        let itemOffset = item.offset - placeholders.backward - scrollPosition;
        let itemBorderBottom = Math.round(itemOffset) + Math.round(item.size);
        ...
        const isLastItem =
            index === range.endIndex - 1 || index === itemsSizes.length - 1;
        const hasNextRenderedItem = itemsSizes
            .slice(index + 1, range.endIndex)
            .some((it) => {
                return it.size;
            });
        const isLastRenderedItem =
            (!nextItem || !nextItem.size) && !hasNextRenderedItem;
        if (
            itemBorderBottom > viewportBorderPosition ||
            isLastItem ||
            isLastRenderedItem
        ) {
            ...
            let isValidItem = !validateItem || validateItem(itemKey);
            if (!isValidItem && isLastItem && index !== 0) {
                const prevItem = itemsSizes[index - 1];
                itemKey = prevItem.key;
                isValidItem = !validateItem || validateItem(prevItem.key);
                itemOffset =
                    prevItem.offset - placeholders.backward - scrollPosition;
                itemBorderBottom =
                    Math.round(itemOffset) + Math.round(prevItem.size);
            }
...
export function getScrollPositionToEdgeItem(
    params: IGetScrollPositionToEdgeItem
): number {
    const { itemsSizes, edgeItem, placeholders, scrollPosition, viewportSize } =
        params;
    ...
    if (edgeItem.direction === 'backward') {
        if (edgeItem.border === 'forward') {
            scrollPositionOffset =
                itemOffset + (item.size - edgeItem.borderDistance);
        } else {
            scrollPositionOffset = itemOffset + edgeItem.borderDistance;
        }
    } else {
        scrollPositionOffset =
            itemOffset - viewportSize + edgeItem.borderDistance;
    }
