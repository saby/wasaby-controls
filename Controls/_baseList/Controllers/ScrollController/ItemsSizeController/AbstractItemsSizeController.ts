...
export abstract class AbstractItemsSizesController {
    ...
    getContentSizeBeforeItems(): number | null {
        ...
        let scrollContent = this._itemsContainer.closest('.controls-Scroll-ContainerBase__content');
        // делаем 2 проверки чтобы не вызывать getBoundingClientRect лишний раз
        if (getStore('AdaptiveInitializer').get('isScrollOnBody')) {
            if (scrollContent.getBoundingClientRect().height > window.screen.height) {
    ...
    getContentSizeBeforeList(): number {
        ...
        let scrollContent = this._listContainer.closest('.controls-Scroll-ContainerBase__content');
        // делаем 2 проверки чтобы не вызывать getBoundingClientRect лишний раз
        if (getStore('AdaptiveInitializer').get('isScrollOnBody')) {
            if (scrollContent.getBoundingClientRect().height > window.screen.height) {
                scrollContent = document.scrollingElement;
