...
export abstract class AbstractObserversController {
    ...
    setAdditionalTriggersOffsets(
        ...
        if (
            !isEqual(this._additionalTriggersOffsets, additionalTriggersOffsets)
        ) {
    ...
    private _setTriggerVisible(direction: IDirection, visible: boolean): void {
        const trigger =
            direction === 'forward' ? this._triggers[1] : this._triggers[0];
    ...
    resetItems(): ITriggersOffsets {
        ...
        if (
            this._triggersOffsetCoefficients.backward === DEFAULT_TRIGGER_OFFSET
        ) {
            this.setBackwardTriggerPosition('null');
        }
        if (
            this._triggersOffsetCoefficients.forward === DEFAULT_TRIGGER_OFFSET
        ) {
        ...
        const shouldHandleTrigger =
            ...
            (direction === 'backward' &&
                (!this._hasItemsOutRange.forward ||
                    !this._isTriggerVisible(
                        'forward',
                        this._contentSizeBeforeList
                    )));
    ...
    protected _updateTriggers(): void {
        ...
        this._triggers[0].style.display = this._triggersVisibility.backward
            ? ''
            : 'none';
        this._triggers[1].style.display = this._triggersVisibility.forward
            ? ''
            : 'none';
        this._applyTriggerOffset(
            this._triggers[0],
    ...
    protected _getTriggers(): HTMLElement[] {
        ...
        if (
            triggersOfThisList.length !== 0 &&
            triggersOfThisList.length !== COUNT_TRIGGERS
        ) {
            Logger.error(
