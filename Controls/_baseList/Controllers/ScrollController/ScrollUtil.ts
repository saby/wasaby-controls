...
export function getCalcMode(params: IGetModeParams): ICalcMode {
    ...
    const virtualPageIsFilled =
        virtualPageSize &&
        virtualPageSize <= params.range.endIndex - params.range.startIndex;
    ...
    if (params.portionedLoading) {
        ...
        const scrolledToEdge =
            params.scrolledToBackwardEdge || params.scrolledToForwardEdge;
...
export function getScrollMode(params: IGetModeParams): IScrollMode {
    ...
    const isFirstChangesInSession =
        !params.changedItems || params.changedItems.length === 1;
    ...
    if (!isFirstChangesInSession) {
        ...
        scrollMode = params.portionedLoading ? 'fixed' : 'unfixed';
    } else {
        // список проскроллен не в начало и не в конец
        scrollMode = 'fixed';
    }
