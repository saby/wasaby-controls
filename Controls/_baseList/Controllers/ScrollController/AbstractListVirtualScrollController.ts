...
const ERROR_PATH =
    'Controls/_baseList/Controllers/AbstractListVirtualScrollController';
...
type IUpdateVirtualNavigationUtil = (hasItems: IHasItemsOutsideOfRange) => void;
type IHasItemsOutRangeChangedCallback = (
    hasItems: IHasItemsOutsideOfRange
) => void;
type IBeforeVirtualRangeChangeCallback = (
    range: IItemsRange
) => Promise<void> | void;
...
> {
    ...
    constructor(options: TOptions) {
        ...
        this._itemsContainerUniqueSelector =
            options.itemsContainerUniqueSelector;
        ...
        this._beforeVirtualRangeChangeCallback =
            options.beforeVirtualRangeChangeCallback;
    ...
    setItemsContainer(itemsContainer: HTMLElement): void {
        ...
        this._scrollController.setItemsContainer(
            itemsContainer,
    ...
    getElement(key: CrudEntityKey): HTMLElement {
        return this._scrollController.getElement(key);
    }
    ...
    destroy(): void {
        ...
        if (this._scheduledScrollParams) {
            const shouldResetScroll =
                ...
                ((this._scheduledScrollParams.params as IDoScrollParams)
                    .scrollParam === 'top' ||
                    (this._scheduledScrollParams.params as IDoScrollParams)
                        .scrollParam === 'bottom');
    ...
    beforeRenderListControl(): void {
        // TODO: убрать, когда построение будет только синхронным.
        const isAllRendered =
            !(this._isReact && this._feature1184208466) ||
            this._isAllRendered();
        ...
        if (
            this._scheduledScrollParams &&
            this._scheduledScrollParams.type ===
                'calculateRestoreScrollParams' &&
            isAllRendered
        ) {
            this._handleScheduledScroll();
    ...
    restoreScrollPosition(): void {
        if (
            !this._scheduledScrollParams ||
            this._scheduledScrollParams.type !== 'restoreScroll'
        ) {
            return;
        }
        const contentBeforeListChanged =
            this._scrollController.updateContentSizeBeforeList();
    ...
    virtualScrollPositionChange(
        position: number,
        applyScrollPositionCallback: () => void
    ): void {
    ...
    scrollPositionChange(position: number): void {
        ...
        this._scrollController.scrollPositionChange(
            position,
            !(willScroll || this._selfScroll || this._renderNewIndexes) &&
                isAllRendered
        );
    ...
    scrollResized(size: number): void {
        ...
        this._scrollController.isListContainerHidden();
    ...
    protected _contentResized(contentSize: number): boolean {
        const changed =
            this._scrollController.listContainerResized(contentSize);
    ...
    setActiveElementKey(activeElementKey: TItemKey): void {
        if (this._activeElementKey !== activeElementKey) {
            ...
            if (this._scrollController && this._collection) {
                this._scrollController.setActiveElementIndex(
                    this._getIndexByKey(activeElementKey)
                );
            }
        }
    }
    ...
    addItems(
        ...
        if (
            (calcMode === 'nothing' && scrollMode !== 'fixed') ||
            scrollMode === 'unfixed'
        ) {
    ...
    removeItems(
        position: number,
        removedItems: CollectionItem[] | number,
        scrollMode: IScrollMode
    ): void {
        this._scrollController.removeItems(
            position,
            typeof removedItems === 'number'
                ? removedItems
                : removedItems.length,
            scrollMode
        );
    ...
    resetItems(): void {
        ...
        if (shouldResetScrollPosition && this._shouldDoScroll()) {
            const scrollParam = this._isEndInitialScrollPosition()
                ? 'bottom'
                : 'top';
            this._scheduleScroll({
                type: 'doScroll',
        ...
        this._scrollController.updateGivenItemsSizes(
            this._getGivenItemsSizes()
        );
    ...
    scrollToItem(
        ...
        const rangeChanged = this._scrollController.scrollToItem(
            itemIndex,
            force,
            position
        );
        ...
        if (
            ...
            this._scrollToElement(key, position, force, selfScroll);
        }
    ...
    scrollToPage(direction: IDirection): Promise<TItemKey> {
        ...
        if (
            edgeItem &&
            this._scrollController.getScrollToPageMode(edgeItem.key) ===
                'edgeItem'
        ) {
    ...
    scrollToEdge(edge: IDirection): Promise<void | TItemKey> {
        const itemIndex =
            edge === 'backward' ? 0 : this._getCollectionItemsCount() - 1;
    ...
    hasEnoughContentToScrollToItem(
        ...
        return this._scrollController.hasEnoughContentToScrollToItem(
            index,
            position,
            force
        );
    ...
    protected _getFirstVisibleItemKey(): TItemKey {
        ...
        const firstVisibleItemIndex =
            this._scrollController.getFirstVisibleItemIndex();
    ...
    private _createScrollController(options: TOptions): void {
        const scrollControllerOptions =
            this._getScrollControllerOptions(options);
    ...
    protected _getScrollControllerOptions(
        ...
        return {
            ...
            itemsQuerySelector,
            itemsSizeControllerConstructor:
                this._getItemsSizeControllerConstructor(options),
            observerControllerConstructor:
                this._getObserversControllerConstructor(),
            triggersQuerySelector: this._getTriggersSelector(
                options.triggersQuerySelector
            ),
            ...
            viewportSize:
                options.virtualScrollConfig.viewportHeight ||
            ...
            indexesInitializedCallback:
                this._indexesInitializedCallback.bind(this),
            ...
            hasItemsOutRangeChangedCallback:
                this._hasItemsOutRangeChangedHandler.bind(this),
    ...
    private _indexesChangedCallback(
        params: IIndexesChangedParams<ICalculatorState>
    ): void {
        this._handleChangedIndexes(params.range, params.shiftDirection, () => {
            ...
            if (this._isReact && this._feature1184208466) {
                this._scrollController.setTriggersAllowed(
                    this._isAllRendered()
                );
            }
        });
    ...
    private _handleChangedIndexes(
        ...
        const applyRangeWithCallback = () => {
            const beforeRangeChangeCallbackResult =
                this._beforeVirtualRangeChangeCallback?.(range);
            if (beforeRangeChangeCallbackResult instanceof Promise) {
                this._beforeVirtualRangeChangeCallbackPromise =
                    beforeRangeChangeCallbackResult;
    ...
    private _scheduleUpdateHasItemsOutRange(
        hasItemsOutsideOfRange: IHasItemsOutsideOfRange
    ): void {
        ...
        if (this._scheduledScrollParams) {
            switch (this._scheduledScrollParams.type) {
                case 'calculateRestoreScrollParams':
                    const params = this._scheduledScrollParams
                        .params as IEdgeItemCalculatingParams;
                    const edgeItem =
                        this._scrollController.getEdgeVisibleItem(params);
                    ...
                    break;
                case 'restoreScroll':
                    const restoreScrollParams = this._scheduledScrollParams
                        .params as IEdgeItem;
                    const scrollPosition =
                        this._scrollController.getScrollPositionToEdgeItem(
                            restoreScrollParams
                        );
                ...
                case 'scrollToElement':
                    ...
                    break;
                case 'doScroll':
                    const doScrollParams = this._scheduledScrollParams
                        .params as IDoScrollParams;
    ...
    private _scrollToElement(
        ...
        this._inertialScrolling.callAfterScrollStopped(() => {
            let element = this._scrollController.getElement(key);
            ...
            if (element) {
                ...
                const result = this._scrollToElementUtil(
                    element,
                    position,
                    force
                );
    ...
    private _updateItemsRenderedOutsideRange(): void {
        const itemsRenderedOutsideRange = this._collection
            .getItems()
            .map((it, collectionIndex) => {
                return {
                    key: String(it.key),
                    renderedOutsideRange: it.isRenderedOutsideRange(),
                    collectionIndex,
                };
            })
            .filter((it) => {
                return it.renderedOutsideRange;
            })
            .map((it) => {
                return {
                    key: it.key,
            ...
            ;
        if (
            !isEqual(this._itemsRenderedOutsideRange, itemsRenderedOutsideRange)
        ) {
            ...
            this._scrollController.setItemsRenderedOutsideRange(
        ...
        switch (this._virtualScrollMode) {
            ...
            default:
                VirtualScrollController.setup(
                    this
                        ._collection as unknown as VirtualScrollController.IVirtualScrollCollection
                );
                break;
        }
    }
    ...
    protected abstract _getIndexByKey<T extends TItemKey = TItemKey>(
        key: T
    ): number;
