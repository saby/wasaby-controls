...
export default class ScrollPagingController {
    ...
    protected initializePagingData(cfg: IScrollPagingOptions): void {
        const averageElementHeight =
            ...
            this._options.loadedElementsCount;
        const totalHeight =
            averageElementHeight * this._options.totalElementsCount;
        const pagesCount = Math.round(
            totalHeight / this._options.scrollParams.clientHeight
        );
        this._pagingData = {
            totalHeight,
            pagesCount,
    ...
    viewportResize(clientHeight: number): void {
        const pagesCount = Math.round(
            this._pagingData.totalHeight / clientHeight
        );
    ...
    protected updateStateByScrollParams(
        ...
        if (canScrollForward && canScrollBackward) {
            ...
            canScrollForward &&
            !canScrollBackward &&
            this.isHasMoreData(hasMoreData.up)
        ) {
            this.handleScrollTop(hasMoreData);
        } else if (
            !canScrollForward &&
            canScrollBackward &&
            this.isHasMoreData(hasMoreData.down)
        ) {
        ...
        if (this._pagingData.averageElementHeight) {
            return Math.ceil(
                this._options.scrollParams.clientHeight /
                    this._pagingData.averageElementHeight
            );
        }
    }
    ...
    protected getNeededItemsCountForPage(page: number): number {
        if (this._options.pagingMode === 'numbers') {
            ...
            if (this._numbersState === 'up') {
                ...
                neededItems =
                    (this._pagingData.pagesCount - page + 1) * itemsOnPage;
            }
            return Math.min(neededItems, this._options.totalElementsCount);
        }
    }

    protected getPagingCfg(
        arrowState: IArrowState,
        hasMoreData: IHasMoreData
    ): IPagingCfg {
        ...
        switch (this._options.pagingMode) {
            ...
            case 'numbers':
                ...
                if (this._numbersState === 'up') {
                    ...
                    if (
                        this._options.scrollParams.scrollTop === 0 &&
                        !hasMoreData.up
                    ) {
                        pagingCfg.selectedPage = 1;
    ...
    getScrollTopByPage(page: number, scrollParams: IScrollParams): number {
        if (this._options.pagingMode === 'numbers') {
            ...
            if (this._numbersState === 'up') {
                ...
                const scrollBottom =
                    (this._pagingData.pagesCount - page) *
                ...
                this.getPagingCfg(
                    {
                        ...
                        prev: 'visible',
                        next: 'readonly',
                        end: this._options.showEndButton
                            ? 'readonly'
                            : 'hidden',
                        reset:
                            (!this._options.scrollParams.initial &&
