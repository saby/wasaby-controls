...
export default class HoverFreeze {
    ...
    startUnfreezeHoverTimeout(event: SyntheticEvent): void {
        ...
        if (this._isCursorInsideOfMouseMoveArea(x, y)) {
            this._itemUnfreezeHoverTimeout = setTimeout(() => {
                this.unfreezeHover();
                if (
                    event.target === this._delayedItemData?.nativeEvent?.target
                ) {
                    const timeout =
                        HOVER_FREEZE_TIMEOUT - HOVER_UNFREEZE_TIMEOUT;
                    this._startFreezeHoverTimeout(
    ...
    private _calculateHTMLNodeIndex(
        ...
        if (nativeEvent && nativeEvent.target) {
            const itemContainer = nativeEvent.target.closest(
                '.controls-ListView__itemV'
            );
            const listContainer = itemContainer?.parentNode;
            return listContainer
                ? [].slice.call(listContainer.children).indexOf(itemContainer) +
                      1
                : null;
        }
    ...
    private _isCursorInsideOfMouseMoveArea(x: number, y: number): boolean {
        ...
        const hasNegatives =
            firstSegment < 0 || secondSegment < 0 || thirdSegment < 0;
        const hasPositives =
            firstSegment > 0 || secondSegment > 0 || thirdSegment > 0;
     ...
     * @private
     */
    private _freezeHover(
        itemKey: CrudEntityKey,
        itemIndex: number,
        htmlNodeIndex: number
    ): void {
        this._clearFreezeHoverTimeout();
        ...
        if (hoveredContainers.length) {
            ...
            const lastContainer =
                hoveredContainers[hoveredContainers.length - 1];
    ...
    protected _getTextEditorBackgroundColor(
        containers: NodeListOf<HTMLElement>
    ): string {
        let result = '#fff';
        for (let i = 0; i < containers.length; i += 1) {
            const textEditor = containers[i].querySelector(
                TEXT_EDITOR_SELECTOR.join(', ')
            );
            if (textEditor) {
     ...
     * @private
     */
    private _calculateItemAreaRect(
        hoveredContainers: NodeListOf<HTMLElement>
    ): IItemAreaRect {
        const resultRect = {
            bottom: null,
            left: null,
    ...
    static getHoveredItemContainers(
        ...
        const hoveredContainerSelector =
            HoverFreeze.getItemHoveredContainerSelector(
                uniqueClass,
                index,
     ...
     * @private
     */
    private static _calculateVectorProduct(
        a: IPoint,
        b: IPoint,
        c: IPoint
    ): number {
