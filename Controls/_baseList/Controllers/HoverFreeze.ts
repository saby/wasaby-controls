...
const EDITOR_DECORATOR_HAS_BACKGROUND_SELECTOR =
...
export default class HoverFreeze {
    ...
    static getItemHoverFreezeStyles(
        ...
        const rowSelector = `.${uniqueClass} .controls-GridReact__row`;
        const gridNthSelector = `${rowSelector}:nth-child(${index})`;
        ...
        return (
            ...
            TEXT_EDITOR_SELECTOR.map((selector) => {
                return `${listNthSelector} ${selector}:not(${EDITOR_DECORATOR_HAS_BACKGROUND_SELECTOR})`;
            }).join(', ') +
            `
              {
                  background-color: ${textEditorBackgroundStyle};
              }
              ${rowSelector}:hover .js-controls-ListView__visible-on-hoverFreeze {
                 display: none;
              } 
              ${listNthSelector} .js-controls-ListView__visible-on-hoverFreeze,
              ${listNthSelector}:hover .js-controls-ListView__visible-on-hoverFreeze {
                 display: block;
                 opacity: 1;
                 visibility: visible;
              }
