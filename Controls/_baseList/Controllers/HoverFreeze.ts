...
export default class HoverFreeze {
    ...
    updateOptions(options: IHoverFreezeOptions): void {
        ...
        this._isReactView = options.isReactView;
    }
    ...
    private static _getListNthSelector(uniqueClass: string, index: number): string {
    ...
    static getItemHoveredContainerSelector(
        ...
        const listNthSelector = this._getListNthSelector(uniqueClass, index);
        return ` ${listNthSelector} > .${measurableContainerSelector}, ` + ` ${listNthSelector}`;
    }
    ...
    static getItemHoverFreezeStyles(
        ...
        const rowSelector = '.controls-GridReact__row';
        const gridNthSelector = `.${uniqueClass} ${rowSelector}:nth-child(${index})`;
        const cellSelector = '.js-controls-GridReact__cell';

        return (
            `
              ${listNthSelector},
              ${gridNthSelector} ${cellSelector},
              ${gridNthSelector} .controls-Grid__row-cell {
                background-color: ${hoverBackgroundColor};
              }` +
            TEXT_EDITOR_SELECTOR.map((selector) => {
                return `${listNthSelector} ${selector}`;
            }).join(', ') +
            `
              {
                  background-color: ${textEditorBackgroundStyle};
              }
              ${listNthSelector} .js-controls-ListView__visible-on-hoverFreeze {
                 opacity: 1;
                 visibility: visible;
              }
              ${gridNthSelector} ${cellSelector} .controls-background-default,
              ${gridNthSelector} ${cellSelector} .controls-background-infoBox,
              ${gridNthSelector} ${cellSelector} .controls-background-stack {
                 background-color: ${hoverBackgroundColor};
              }
              ${gridNthSelector} ${cellSelector} .controls-BaseCut__shadow_background-default {
                  background: linear-gradient(180deg, rgba(255, 255, 255, 0) 0, ${hoverBackgroundColor} 100%)
              }
              ${listNthSelector}.controls-ListView__item_showActions_outside > ${cellSelector} {
                 border-bottom-right-radius: 0;
              }
              ${gridNthSelector} .controls-Grid__row-cell__last-outsideActions-master,
              ${gridNthSelector} .controls-Grid__row-cell__last-outsideActions-master ${cellSelector} {
                 border-bottom-right-radius: 0;
              }
              `
        );
