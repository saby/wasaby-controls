...
import { IDirection } from 'Controls/_baseList/interface/IVirtualScroll';
...
export default class IndicatorsController {
    ...
    constructor(options: IIndicatorsControllerOptions) {
        ...
        if (this._options.isIterativeLoading) {
            const hasMoreData =
                ...
                this._options.hasMoreDataToTop;
            if (hasMoreData) {
                const direction = this._options.hasMoreDataToBottom
                    ? 'bottom'
                    : 'top';
            ...
            this._countLoadedItemsByIteration = this._model
                .getSourceCollection()
                .getCount();
        }
    }
     ...
     */
    updateOptions(
        options: IIndicatorsControllerOptions,
        isLoading: boolean
    ): void {
        ...
        const shouldRecountAllIndicators =
            options.model && this._model !== options.model;
        const shouldRecountBottomIndicator =
            !shouldRecountAllIndicators &&
            (this._options.hasMoreDataToBottom !==
                options.hasMoreDataToBottom ||
                navigationChanged);
     ...
     * @param direction
     */
    onDataLoadStarted(direction?: IDirection): void {
        if (direction) {
            if (
                !this._loadingToDirectionInProgress &&
                !this._options.isIterativeLoading
            ) {
     ...
     */
    onDataLoad(loadedItems: RecordSet, direction?: IDirection): void {
        ...
        if (isReload) {
            ...
            const isIterativeLoading =
                this._options.isIterativeLoading ||
                !!this.getPortionedSearchDirection();
    ...
    shouldDisplayTopIndicator(): boolean {
        ...
        const allowByViewport =
            this._viewportFilled || !this._options.hasMoreDataToBottom;
        ...
        const allowByAsync =
            ...
            this._options.hasHiddenItemsByVirtualScroll('up');

        return (
            allowByViewport &&
            allowByAttachOption &&
            (this._options.hasMoreDataToTop && allowByVirtualScroll || allowByAsync) &&
    ...
    displayTopIndicator(
        ...
        const wasDisplayedIndicator = this._model
            .getTopIndicator()
            .isDisplayed();
    ...
    afterRenderCallback(): void {
        ...
        if (this._wasReload) {
            ...
            if (
                ...
                this.shouldDisplayTopIndicator()
            ) {
                this.displayTopIndicator(true);
            }
        }
    ...
    hideIndicator(direction: TIndicatorPosition): void {
        const topIndicatorIsDisplayed = this._model
            .getTopIndicator()
            .isDisplayed();
     ...
     * @void
     */
    displayDrawingIndicator(
        indicatorElement: HTMLElement,
        position: 'top' | 'bottom'
    ): void {
        if (
            !this._shouldHandleDrawingIndicator(position) ||
            !indicatorElement
        ) {
            return;
        }
    ...
    recountIndicators(
        ...
        scrollToFirstItem: boolean = false
    ): void {
    ...
    private _recountTopIndicator(scrollToFirstItem: boolean = false): void {
        ...
        const isTopIndicatorDisplayed = this._model
            .getTopIndicator()
            .isDisplayed();
        if (this.shouldDisplayTopIndicator()) {
            ...
            if (this._options.isIterativeLoading) {
                ...
                this.displayTopIndicator(
                    scrollToFirstItem,
                    true,
                    isTopIndicatorDisplayed
                );
            }
        } else {
            this.hideIndicator('top');
            ...
            (this._options.isIterativeLoading &&
                (this._options.isInfinityNavigation ||
                    (this._options.isMaxCountNavigation &&
                        this._countLoadedItemsByIteration <
                            this._options.iterativeLoadPageSize)));
    ...
    private _shouldDisplayIndicatorByVirtualScroll(direction: 'up' | 'down') {
        ...
        return this._options.feature1184208466 ||
    ...
    private _getLoadingIndicatorState(
        direction: TIndicatorPosition
    ): TIndicatorState {
        let state =
    ...
    startDisplayPortionedSearch(direction: TPortionedSearchDirection): void {
        ...
        if (
            ...
            this._startDisplayIndicatorTimer(() => {
                const allowDisplay =
                    !this._options.hasHiddenItemsByVirtualScroll(
    ...
    stopDisplayPortionedSearch(): void {
        ...
        if (this._model && !this._model.destroyed) {
            const displayTopIndicator =
                this._portionedSearchDirection === 'top' &&
                this.shouldDisplayTopIndicator();
            const displayBottomIndicator =
                this._portionedSearchDirection === 'bottom' &&
                this.shouldDisplayBottomIndicator();
            if (displayTopIndicator || displayBottomIndicator) {
                const wasDisplayedIndicator = this._model
                    .getTopIndicator()
                    .isDisplayed();
                const isPortionedSearchIndicator =
                    this._model.getTopIndicator().getState() ===
                    'portioned-search';
    ...
    getPortionedSearchDirection(): 'up' | 'down' {
        ...
        return DIRECTION_COMPATIBILITY[this._portionedSearchDirection] as
            | 'up'
            | 'down';
    }
    ...
    private _startDisplayPortionedSearchTimer(): void {
        const duration =
            (this._options.iterativeLoadTimeout ||
                DEFAULT_ITERATIVE_LOADING_TIMEOUT) * S_TO_MS_FACTOR;
        this._clearPortionedSearchTimer();
