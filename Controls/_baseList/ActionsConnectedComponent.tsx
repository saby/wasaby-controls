...
export default function ActionsConnectedComponent(props: IProps): React.ReactElement {
    const { actionHandlers } = useListContext();
    ...
    const itemActions = hooks.useItemActionsDeferredInitialization({
        item,
        itemActionsTemplateMountedCallback: actionHandlers?.itemActionsTemplateMountedCallback,
        itemActionsTemplateUnmountedCallback: actionHandlers?.itemActionsTemplateUnmountedCallback,
    });
 ...
 * Взаимодействие с записью происходит через контекст строки, распространяемый контролом списка.
 * @demo Controls-demo/ItemActionsPosition/Custom/Index
