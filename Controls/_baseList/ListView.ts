...
export default class ListView extends Control<IListViewOptions> {
    ...
    protected _itemsContainerClass: string;
    ...
    protected _beforeMount(newOptions: IListViewOptions): void {
        ...
        this._itemsContainerClass = this._getItemsContainerClassName(newOptions);

        this._itemTemplate = this._resolveItemTemplate(newOptions);
    }

    protected _getItemsContainerClassName(props: IListViewOptions): string {
        const showActionsClass = `controls-BaseControl_showActions_${
            props.itemActionsVisibility || 'onhover'
        }`;
        let result = 'controls-ListViewV__itemsContainer';
        ...
        if (!props.itemsContainerClass) {
            result += ` ${showActionsClass}`;
        } else {
            result += ` ${props.itemsContainerClass}`;
        }
        return result;
    }
    ...
    protected _beforeUpdate(newOptions: IListViewOptions): void {
        ...
        const itemsContainerClass = this._getItemsContainerClassName(newOptions);
        if (this._itemsContainerClass !== itemsContainerClass) {
            this._itemsContainerClass = itemsContainerClass;
        }
    ...
    protected _onItemClick(event: SyntheticEvent<MouseEvent>, item: CollectionItem): void {
        if (helpers.isWithoutBaseControl(this._options)) {
            helpers.events.parseListViewItemClick(
                ...
                {
                    onCheckbox: () => {
                        this._options.onCheckboxClickNew(event, item.contents);
                    },
                    onItem: () => {
                        this._options.onItemClickNew(event, item.contents);
                    },
                }
            );
        } else {
            helpers.events.parseListViewItemClick(
                { event, item, cleanScheme: this._options.isLatestInteractorVersion },
                {
                    onCheckbox: () => {
    ...
    protected _onItemMouseMove(event: SyntheticEvent, item: CollectionItem): void {
        // Для работы с коллбеками, поставляемыми из списка без BaseControl
        if (this._options.itemHandlers?.onMouseMoveCallback) {
            this._options.itemHandlers?.onMouseMoveCallback(event, item.contents);
            return;
        }
    ...
    protected _onItemContextMenu(event: SyntheticEvent, item: CollectionItem): void {
        if (this._options.contextMenuVisibility !== false && !this._listModel.isEditing()) {
            // Для работы с коллбеками, поставляемыми без BaseControl
            if (this._options.itemHandlers?.onContextMenu) {
                this._options.itemHandlers?.onContextMenu(event, item.contents);
            } else {
    ...
    protected _onItemLongTap(event: SyntheticEvent, item: CollectionItem): void {
        // Не обрабатываем longTap, если событие прилетело из внутреннего подсписка
        const closestListContainer = event.target.closest('.controls-ListViewV__itemsContainer');
        if (
            this._options.uniqueId &&
            !closestListContainer?.className.includes(this._options.uniqueId)
        ) {
            return;
        }
        if (this._options.contextMenuVisibility !== false && !this._listModel.isEditing()) {
            if (this._options.itemHandlers?.onLongTapCallback) {
                this._options.itemHandlers?.onLongTapCallback(event, item.contents);
            } else {
    ...
    protected _onItemSwipe(
        ...
        // Не обрабатываем swipe, если событие прилетело из внутреннего подсписка
        const closestListContainer = event.target.closest('.controls-ListViewV__itemsContainer');
        if (
            this._options.uniqueId &&
            !closestListContainer?.className.includes(this._options.uniqueId)
        ...
        // Для работы с коллбеками, поставляемыми из списка без BaseControl
        if (this._options.itemHandlers?.onSwipeCallback) {
            this._options.itemHandlers?.onSwipeCallback(event, item.contents);
            return;
        }
