...
export default class ListView extends Control<IListViewOptions> {
    ...
    protected _onItemClick(event: SyntheticEvent<MouseEvent>, item: CollectionItem): void {
        ...
        const targetItemElement = event.target.closest('.controls-ListView__itemV');
        const clickOnEditingItem =
            targetItemElement && targetItemElement.matches('.js-controls-ListView__item_editing');
        if (
        ...
        if (event.target.closest('.controls-TileView__previewTemplate_content')) {
            event.stopPropagation();
    ...
    protected _setHoveredItem(item: CollectionItem, event: SyntheticEvent<MouseEvent>): void {
        ...
        if (item !== this._hoveredItem) {
            ...
            const container =
                event && item !== null ? event.target.closest('.controls-ListView__itemV') : null;
