...
import { ANIMATION_DURATION, IAnimationContainerContextValue } from 'Controls/animation';
...
const _private = {
    ...
    getAllDataCount(self: BaseControl, showTotalCounter?: boolean): number | undefined {
        if (showTotalCounter) {
    ...
    prepareFooter(self: BaseControl, options: IBaseControlOptions): void {
        ...
        if (shouldDrawCutNavigationButton && buttonView === 'link') {
            if (typeof loadedDataCount === 'number' && typeof allDataCount === 'number') {
                const hasMoreData = Boolean(allDataCount - loadedDataCount);
                if (!hasMoreData) {
                    self._shouldDrawNavigationButton = false;
                } else {
    ...
    implements IMovableList, IEditableList
{
    ...
    protected _afterMount(): void {
        ...
        } else if (this._indicatorsController.getPortionedSearchDirection()) {
    ...
    protected _handleDelegatedCheckboxClick(
        event: SyntheticEvent,
        contents: Model,
        originalEvent: SyntheticEvent<MouseEvent>
    ) {
