    ...
    implements IMovableList, IEditableList
{
        ...
        if (eventOptions.isShiftKey) {
            ...
            while (isEditableCell !== true) {
                ...
                if (next && next.contents) {
                    ...
                    isEditableCell = columns[columnIndex]?.cellProps?.editable ?? true;
                } else {
                    isEditableCell = true;
                }
            }
    ...
    protected _itemMouseDown(
        ...
        if (
            !hasDragScrolling &&
            !event.nativeEvent.target.closest('.js-controls-ListView__checkbox')
        ) {
    ...
    private _calculateMouseOffsetInItem(event: MouseEvent): {
        ...
        if (targetElement) {
            // Считаем через утилиту для учёта zoom
            // https://online.sbis.ru/opendoc.html?guid=1c82823c-78bf-4577-9ca6-8ccc2c4d5b19
            const dragTargetRect = DimensionsMeasurer.getBoundingClientRect(targetElement);
