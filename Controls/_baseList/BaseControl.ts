...
const _private = {
            ...
            if (self._hasSelectionController() || handleSelection) {
                ...
                switch (action) {
                    case IObservable.ACTION_ADD:
                        ...
                        checkWasabyEvent(self._options.onListSelectedKeysCountChanged)?.(
    ...
    needBottomPadding(self: BaseControl, options: IBaseControlOptions): boolean {
        ...
        // Продолжекние костыля, добавленного по ошибке
        // https://online.sbis.ru/doc/fdc75f6c-c21f-4e25-98c1-72771b840051?client=3.
        // Прикладник отдаёт _needBottomPadding=false, чтобы принудительно убрать отступ снизу.
        if (!listViewModel || options._needBottomPadding === false) {
            return false;
        }
    ...
    implements IMovableList, IEditableList
{
    ...
    protected _afterMount(): void {
        ...
        checkWasabyEvent(this._options.onRegister)?.(
    ...
    protected _beforeUpdate(
        ...
        if (this._items) {
            ...
            if (
                ...
                checkWasabyEvent(this._options.onListSelectedKeysCountChanged)?.(
    ...
    protected _unmount(): void {
        ...
        if (this._hasSelectionController()) {
            checkWasabyEvent(this._options.onListSelectedKeysCountChanged)?.(0, false);
    ...
    protected _changeMarkedKey(
        ...
        const handleResult = (key) => {
            ...
            // если список не замаунчен, то не нужно кидать события, события кинем после маунта
            if (this._isMounted) {
                checkWasabyEvent(this._options.onMarkedKeyChanged)?.(key);
    ...
    protected _onItemClick(
        ...
        this._setMarkerOnItemClick(item, originalEvent);
    ...
    protected _setMarkerOnItemClick(item: CollectionItem, originalEvent: MouseEvent): void {
        ...
        if (this._options._needSetMarkerCallback) {
            canBeMarked =
                canBeMarked &&
                this._options._needSetMarkerCallback(item.getContents(), originalEvent);
        }
        if (canBeMarked) {
            ...
            if (this._editInPlaceController?.isEditing()) {
                ...
                this._markedKeyAfterEditing = item.key;
            } else {
    ...
    private _notifySelection(selection: ISelectionObject): void {
        ...
        if (selectedDiff.added.length || selectedDiff.removed.length) {
            checkWasabyEvent(this._options.onSelectedKeysChanged)?.(
                selectedDiff.keys,
                selectedDiff.added,
        ...
        if (excludedDiff.added.length || excludedDiff.removed.length) {
            checkWasabyEvent(this._options.onExcludedKeysChanged)?.(
                excludedDiff.keys,
                excludedDiff.added,
