...
import { CrudEntityKey, DataSet, LOCAL_MOVE_POSITION, SbisService } from 'Types/source';
...
// Utils imports
import { checkWasabyEvent, EventUtils, SyntheticEvent } from 'UI/Events';
...
import { dndCore } from 'Controls/listDragNDrop';

import { IList, IReloadItemOptions } from './interface/IList';
import type { AnimationContext } from 'Controls/animation';
...
import { IListActionAdditionalConfig, IMovableList } from './interface/IMovableList';
...
import {
    callWithCollectionItem,
    getKey,
    getPlainItemContents,
} from 'Controls/_baseList/resources/utils/helpers';
import {
...
import { IItemEventHandlers } from './ItemComponent';
import type { IItemActionsHandler } from 'Controls/_baseList/interface/IItemActionsHandler';
import type { INavigationButtonProps } from 'Controls/_baseList/interface/INavigationButton';
...
import { ScrollControllerLib, helpers } from 'Controls/listsCommonLogic';
...
// Селектор элемента, который не может быть отредактирован.
export const NOT_EDITABLE_JS_SELECTOR: TNotEditableJsSelector = 'js-controls-ListView__notEditable';
export const EDITABLE_JS_SELECTOR = 'js-controls-ListView__editingTarget';

export type TExtLogInfo = Record<string, string>;

const ERROR_MSG = {
    CANT_USE_IN_READ_ONLY: (methodName: string, extLogInfo?: TExtLogInfo): string => {
        return `List is in readOnly mode. Cant use ${methodName}() in readOnly! Extended info: ${JSON.stringify(
            extLogInfo
        )}`;
    },
};
...
// Немного дольше, чем длительность анимации в css, чтобы анимация точно успела завершиться и не обрывалась.
...
const HOT_KEYS_UP = {
    keyUpDown: constants.key.down,
    keyUpUp: constants.key.up,
    spaceUpHandler: constants.key.space,
};
...
const OLD_LIST_UPDATE_TIME = 150;
/**
...
const CONTEXTMENU_ENABLED_CLASS = 'controls-BaseControl_contextMenu_enabled';

const MENU_TOP_SELECTOR = '.controls-Menu__popup';
const MENU_ROW_SELECTOR = '.controls-Menu__row';
...
const _private = {
    ...
    doAfterComponentDidUpdate(self: BaseControl, callback: Function): void {
        if (self.callbackAfterComponentDidUpdate) {
            self.callbackAfterComponentDidUpdate.push(callback);
        } else {
            self.callbackAfterComponentDidUpdate = [callback];
        }
    },
    ...
    initializeModel(self: BaseControl, options: IBaseControlOptions, data: RecordSet): void {
        ...
        if (!self._destroyed) {
            ...
            if (!options.useCollection) {
                ...
                // SearchGrid сейчас не поддержан в слайсе, поэтому при переходе c viewMode search на другой
                // коллекция в слайс будет ставиться как в первый раз, игнорируя актуальное состояние и устанавливая начальное.
                const searchViewModeChanged =
                    self._options.viewMode === 'search' &&
                    options.viewMode !== self._options.viewMode;
                options.setCollection(self._listViewModel, !searchViewModeChanged);
            }
    ...
    getAllDataCount(self: BaseControl, showTotalCounter?: boolean): number | boolean | undefined {
        if (showTotalCounter) {
            return self._listViewModel?.getMetaData()?.itemsTotalCount;
        }
    ...
    keyDownDown: (self: BaseControl, event: React.KeyboardEvent) => {
        const options = self._options as IBaseControlOptions;
        if (!options.onViewKeyDownArrowDownNew) {
            return;
        }
        if (!self._thottledKeyDown) {
            self._thottledKeyDown = helpers.throttleWrapper(
                options.onViewKeyDownArrowDownNew,
                OLD_LIST_UPDATE_TIME
            );
        }
        if (options.markerVisibility !== 'hidden') {
            event.preventDefault();
            event.stopPropagation();
            self._thottledKeyDown.call(event, self, options);
        }
    },
    keyUpDown: (self: BaseControl) => {
        if (self._thottledKeyDown) {
            self._thottledKeyDown.clearThrottle();
        }
    },

    keyDownUp: (self: BaseControl, event: React.KeyboardEvent) => {
        const options = self._options as IBaseControlOptions;
        if (!options.onViewKeyDownArrowUpNew) {
            return;
        }
        if (!self._thottledKeyUp) {
            self._thottledKeyUp = helpers.throttleWrapper(
                options.onViewKeyDownArrowUpNew,
                OLD_LIST_UPDATE_TIME
            );
        }
        if (options.markerVisibility !== 'hidden') {
            event.preventDefault();
            event.stopPropagation();
            self._thottledKeyUp.call(event, self, options);
        }
    },

    keyUpUp: (self: BaseControl) => {
        if (self._thottledKeyUp) {
            self._thottledKeyUp.clearThrottle();
        }
    },

    // throttle используется, чтобы корректно отрабатывал зажатый пробел.
    // В BaseControl никак не отследить, что состояние по пробелу установилось:
    // selectedKeys и markedKey могут не поменяться, а на рендер не завязать, т.к.
    // при работе со слайсом произодет минимум 2 перерисовки.
    // OLD_LIST_UPDATE_TIME это время выполнения примерно одно обновление слайса в старых списках
    spaceHandler: (self: BaseControl, event: React.KeyboardEvent) => {
        const options = self._options as IBaseControlOptions;
        if (!self._thottledSpace) {
            self._thottledSpace = helpers.throttleWrapper(
                (self: BaseControl, event: React.KeyboardEvent) => {
                ...
                OLD_LIST_UPDATE_TIME * 2
            );
        }
        if (options.markerVisibility !== 'hidden') {
            event.preventDefault();
            event.stopPropagation();
            self._thottledSpace.call(self, event);
        }
    },
    spaceUpHandler: (self: BaseControl) => {
        if (self._thottledSpace) {
            self._thottledSpace.clearThrottle();
        }
    },
    ...
    prepareFooter(self: BaseControl, options: IBaseControlOptions): void {
        const isCustomPositionNavigationButton =
            options.navigation?.viewConfig?.buttonConfig?.buttonPosition === 'custom';

        const shouldDrawDemandNavigationButton =
            _private.isDemandNavigation(options.navigation) && self._hasMoreData('down');

        const shouldDrawCutNavigationButton = _private.shouldDrawCut(
            options.navigation,
            self._items,
        ...
        self._shouldDrawNavigationButton =
            !isCustomPositionNavigationButton &&
            (shouldDrawDemandNavigationButton || shouldDrawCutNavigationButton);
        ...
        const buttonView = options.navigation?.viewConfig?.buttonView;

        if (shouldDrawDemandNavigationButton) {
            if (typeof loadedDataCount === 'number' && typeof allDataCount === 'number') {
                self._linkLabel = allDataCount - loadedDataCount;
                if (self._linkLabel === 0) {
        ...
        if (shouldDrawCutNavigationButton && buttonView === 'link') {
            const hasMoreData =
                (typeof loadedDataCount === 'number' &&
                    typeof allDataCount === 'number' &&
                    Boolean(allDataCount - loadedDataCount)) ||
                (typeof allDataCount === 'object' &&
                    (allDataCount.forward ||
                        allDataCount.backword ||
                        allDataCount.after ||
                        allDataCount.before)) ||
                allDataCount === true;

            if (hasMoreData) {
                self._loadMoreCaption = rk('Показать все');
            } else {
                self._shouldDrawNavigationButton = false;
            }
        }
        ...
        if (self._sourceController) {
            ...
            if (_private.isPortionedLoad(self)) {
                const portionedSearchDirection =
                    self._indicatorsController.getPortionedSearchDirection() ?? direction;
            ...
            self._loadToDirectionPromise = self
                ...
                .addCallback((addedItems: RecordSet) => {
                    ...
                    self._loadToDirectionPromise = undefined;
            ...
            return self._loadToDirectionPromise;
        }
    ...
    reloadToEdge(self: BaseControl, direction: ScrollControllerLib.IDirection): Promise<void> {
        if (self._hasMoreData(direction)) {
            ...
            // при обращении к сурс контроллеру необходимо передавать направление с учетом инвертирования
            // направление, которое приходит в функцию - это вьюшное ожидание данных в той стороне
            let navigationQueryConfig = self._sourceController.shiftToEdge(
                self._getDataLoadDirection(direction),
                self._options.root,
                pagingMode
            );
            ...
            if (
                ...
                const pageConfig =
                    self._getDataLoadDirection(direction) === 'down'
    ...
    prepareScrollPagingControllerOptions(self: BaseControl, options: IBaseControlOptions): object {
        ...
        let showTotalCounter: boolean | undefined;
        if (self._sourceController) {
            showTotalCounter =
                typeof self._listViewModel?.getMetaData()?.itemsTotalCount === 'number';
            elementsCount = _private.getAllDataCount(self, showTotalCounter);
            ...
            if (self._viewportHeight) {
                ...
                const callback = () => {
                    if (self._isMounted) {
                        ...
                        // Вызываем pagingCfgTrigger, так как после изменения _pagingVisible == true
                        // нужно обновить _pagingCfg на закэшированное значение
                        if (self._pagingCfgCache) {
                            self?._scrollPagingCtr?._options?.pagingCfgTrigger?.(
                                self._pagingCfgCache
                            );
                        }
    ...
    handleListScrollSync(self: BaseControl, scrollTop: number): void {
        ...
        if (
            scrollTop === 0 ||
            (self._options.trackedProperties?.length && isRealScroll && !self._scrolledToEdge)
        ) {
            ...
            if (self._hasSelectionController() || handleSelection) {
                ...
                switch (action) {
                    ...
                    case IObservable.ACTION_REPLACE:
                        selectionController.onCollectionReplace();
                        break;
    ...
    handleItemActionClick(
        ...
        let result: boolean | void | Promise<unknown> | unknown;
        const originalAction =
            self._itemActionsController?.findOriginalAction(item, action) || action;
        const eventArgs = [originalAction, contents, itemContainer, clickEvent.nativeEvent];
        if (self._options.onActionClickNew && !!originalAction.execute) {
            result = self._options.onActionClickNew(...eventArgs);
        } else {
            result = self._options.notifyCallback('actionClick', eventArgs);
            if (action.handler) {
                // В случаях, когда прикладник делает всю настройку через конфиг, и даже не вставляет
                // наш контрол (например, в фильтрах), его единственная точка для обработки IA, это handler().
                const handlerResult = action.handler(
                    contents,
                    itemContainer,
    ...
    resolveItemActionsOptions(self: BaseControl, options: IList): IItemActionsControllerOptions {
        ...
        return {
            ...
            itemActionsVisibility: this.getItemActionsVisibility(options),
    ...
    updateItemActions(
        ...
        itemActionsController.updateActions(itemToUpdateActions);
    },
    ...
    addShowActionsClass(self: BaseControl, options: IList): void {
        ...
        if (
            ...
            const classes = [
                SHOW_ACTIONS_CLASS,
                `${SHOW_ACTIONS_CLASS}_${_private.getItemActionsVisibility(options)}`,
            ];
    ...
    removeShowActionsClass(self: BaseControl): void {
        ...
        if (
            ...
            const classes = [
                SHOW_ACTIONS_CLASS,
                `${SHOW_ACTIONS_CLASS}_${_private.getItemActionsVisibility(self._options)}`,
            ];
     ...
     * @param options
     */
    getItemActionsVisibility(options: IBaseControlOptions): string {
    ...
    isAllowedHoverFreeze(self: BaseControl): boolean {
        return (
            !detection.isIE &&
            !self._dndState.controller?.isDragging() &&
            (!self._editInPlaceController || !self._editInPlaceController.isEditing()) &&
    ...
    checkCellIsEditableByColumnIndex(
        self: BaseControl,
        editingItem: IEditableCollectionItem,
        columnIndex: number,
        cellWithCellsContainer?: HTMLDivElement,
        nestedCellEmbeddingPosition?: number
    ): boolean {
        if (editingItem && editingItem.contents) {
            // Получаем все колонки, кроме чекбокса и лесенки
            const columns = editingItem
                .getColumns()
                .filter((el) => !el.isLadderCell() && !el.CheckBoxCell);

            // Если существует ячейка с подячейками, то они должны тоже учитываться.
            // Получаем массив подячеек, встраиваем их в массив колонок с нужной позиции,
            // Узнаем состояние редактируемая/нередактируемая
            if (cellWithCellsContainer) {
                const preparedColumns = [...columns];

                preparedColumns.splice(
                    nestedCellEmbeddingPosition,
                    0,
                    ...cellWithCellsContainer.querySelectorAll('.js-controls-GridReact__cell')
                );

                return (
                    preparedColumns[columnIndex]?.isEditable?.() ??
                    preparedColumns[columnIndex].classList.contains(
                        self._options._$editableCellSelector ?? EDITABLE_JS_SELECTOR
                    ) ??
                    false
                );
            } else {
                return columns[columnIndex]?.isEditable() ?? false;
            }
        } else {
            return true;
        }
...
export interface IListHandlers {
    ...
    onItemMouseDown?: (item: CollectionItem, event: SyntheticEvent<MouseEvent>) => void;
    onItemMouseMove?: (item: CollectionItem, event: SyntheticEvent<MouseEvent>) => void;
}
...
export interface IBaseControlOptions
    extends IControlOptions,
        ...
        IEditableListOptions,
        IStoreIdOptions,
        Partial<TOldBaseControlCompatibility>,
        IListHandlers {
    keyProperty?: string;
    ...
    implements IMovableList, IEditableList
{
    ...
    _restoreScrollDirection: boolean = false;
    ...
    _linkLabel: string = null;
    _loadMoreCaption: string = null;
    _shouldDrawNavigationButton: boolean = false;
    _pagingCfg: IPagingCfg = null;
    _pagingCfgCache: IPagingCfg = null; // Переменная для кэширования _pagingCfg, необходима для оптимизации перерисовок списка
    ...
    // TODO: Костыль для https://online.sbis.ru/opendoc.html?guid=2683cdf7-804e-403c-b8c1-46b95e612eea&client=3
    //  Убрать, когда переведем Controls/columns:View на React
    private _isCustomMouseLeaveFlag: boolean | null = null;
    ...
    private _dndState: dndCore.TState = { insideDragging: false, isDocumentDragging: false };
    private _dndApi: dndCore.IDragNDropAPI | null = null;
    ...
    // Флаг, устанавливающий, что надо обновить ItemActions при update
    private _shouldUpdateItemActionsOnDidUpdate: boolean;
    ...
    private _animationContext: AnimationContext;
    ...
    protected _navigationButtonProps: INavigationButtonProps;

    private _stickyOpener: StickyOpener;

    private _scheduledRestoreScrollAfterInvert;
    private _itemsOrder: TItemsOrder = 'default';
    private _pendingItemsOrder?: TItemsOrder;
    ...
    constructor(options: IBaseControlOptions, context: object) {
        ...
        this._itemHandlers = {
            onMouseOverCallback: (event, item) => {
                return callWithCollectionItem(item, this._listViewModel, (collectionItem) => {
                    return this._itemMouseOver(event, collectionItem, event);
                });
            },
            onMouseEnterCallback: (event, item) => {
                return callWithCollectionItem(item, this._listViewModel, (collectionItem) => {
                    return this._itemMouseEnter(event, collectionItem, event);
                });
            },
            onMouseLeaveCallback: (event, item) => {
                return callWithCollectionItem(item, this._listViewModel, (collectionItem) => {
                    return this._itemMouseLeave(event, collectionItem, event);
                });
            },
            onMouseMoveCallback: (event, item) => {
                return callWithCollectionItem(item, this._listViewModel, (collectionItem) => {
                    return this._itemMouseMove(event, collectionItem, event);
                });
            },
            onMouseDownCallback: (event, item) => {
                return callWithCollectionItem(item, this._listViewModel, (collectionItem) => {
                    return this._itemMouseDown(event, collectionItem, event);
                });
            },
            onMouseUpCallback: (event, item) => {
                return callWithCollectionItem(item, this._listViewModel, (collectionItem) => {
                    return this._itemMouseUp(event, collectionItem, event);
                });
            },
            ...
            onSwipeCallback: (event, item) => {
                return callWithCollectionItem(item, this._listViewModel, (collectionItem) => {
                    return this._onItemSwipe(event, collectionItem, event);
                });
            },
            onLongTapCallback: (event, item) => {
                return callWithCollectionItem(item, this._listViewModel, (collectionItem) => {
                    return this._onItemLongTap(event, collectionItem, event);
                });
            },
            onDeactivatedCallback: (item, params) => {
                return callWithCollectionItem(item, this._listViewModel, (collectionItem) => {
                    return this._onItemDeactivated(collectionItem, params);
                });
            },
        };

        if (options.contextMenuVisibility !== false) {
            this._itemHandlers.onContextMenuCallback = (event, item) => {
                return callWithCollectionItem(item, this._listViewModel, (collectionItem) => {
                    return this._onItemContextMenu(event, collectionItem, event);
                });
            };
        }
        ...
        if (options.navigation?.viewConfig?.buttonConfig?.buttonPosition === 'custom')
            this._navigationButtonProps = {
                buttonConfig: options.navigation?.viewConfig?.buttonConfig,
                buttonView: this._resolveNavigationButtonView(options),
                linkFontColorStyle: options.moreFontColorStyle,
                linkClass: 'controls-BaseControl__loadMore',
                linkLabel: this._linkLabel,
                loadMoreCaption: this._loadMoreCaption,
                onClick: this._onNavigationButtonClick,
                className: this._getNavigationButtonClasses(
                    options,
                    options?.navigation?.viewConfig?.buttonConfig
                ),
            };
    }
    ...
    protected _beforeMount(
        ...
        if (newOptions.itemsOrder !== undefined) {
            this._itemsOrder = newOptions.itemsOrder;
        }
    ...
    protected _getBottomPaddingClass(
        ...
        if (options.placeholderAfterContent) {
            ...
            if (options.style === 'master' && !options.isAdaptive) {
    ...
    private _getAnimationContextValue(context: AnimationContext): void {
    ...
    // region LoadData
    private _getDataLoadDirection(direction: Direction): Direction {
        // опция isInverted должна инвертировать только загрузку, для вьюхи же все должно остаться как прежде
        // Поскольку событием dataLoad стреляет сам sourceController, а в нем направление инвертировано
        // при обработке в dataLoadCallbac необходимо инвертировать обратно
        const isInverted = this._itemsOrder && this._itemsOrder !== 'default';
        return isInverted ? (direction === 'up' ? 'down' : 'up') : direction;
    }
    ...
    private _dataLoadStartedCallback(event: SyntheticEvent, direction: Direction): void {
        ...
        if (isReload) {
            ...
            if (this.isEditing()) {
                const isAdding = this._editInPlaceController.getEditableItem()?.isAdd;
                if (!isAdding && !this._options.keepAddingOnReload) {
    ...
    private _dataLoadCallback(
        ...
        if (items.getCount()) {
            ...
            this._loadedItemsDirection = direction;
        }

        // Если был пустым массив item-ов и послет того, как sourceController делает запрос и возвращается тоже пустой массив item-ов,
        // то RecordSet не вызывает событие обновления коллекции и backwardTrigger остается видимым. Из-за этого
        // не вызываются backward-запросы, хоть и hasMoreData.up === true
        if (!this.getItems().getCount() && !items.getCount()) {
            ...
            this._listVirtualScrollController.setBackwardTriggerVisibility(backwardTriggerVisible);
        }
    ...
    private _itemsChangedOnDataLoad(
        ...
        if (direction) {
            // В функцию приходит направление, в которое добавлены элементы.
            // При инвертировании оно не совпадает с отображением на экране
            // индикаторы и загрузку необходимо обновлять, учитывая инвертирование
            direction = this._getDataLoadDirection(direction);
        }
    ...
    private _preloadDataToBackward(): void {
        ...
        if (
            this._sourceController?.hasMoreData(this._getDataLoadDirection('down')) &&
            !viewportFilled
        ) {
        ...
        // установка начального itemsOrder происходит в beforeMount, предзагрузка срабатывает на afterMount
        // необходимо компенсировать начальное инвертирование для корректной подгрузки данных наверх
        this._preloadedItemsToBackward = _private.loadToDirectionIfNeed(
            this,
            this._getDataLoadDirection('up'),
            renderPreloadedDataRightAway,
            true
        );
        ...
        if (this._preloadedItemsToBackward) {
            ...
            if (!renderPreloadedDataRightAway) {
                this._preloadedItemsToBackward.then(async (result) => {
                    const invertedBackwardDirection = this._getDataLoadDirection('up');
                    if (!result.getCount()) {
                        await this.props._setPreloadedItems(
                            result,
                            invertedBackwardDirection,
                            this
                        );
                        this._listVirtualScrollController._updateShadowsUtil({
                            backward: result.getMetaData().more,
                        });
                    }
                    // onDataLoad вызывается в itemsChangedHandler,
                    // если мы не будем мерджить записи, то обработчик не сработает
                    this._indicatorsController.onDataLoad(result, invertedBackwardDirection);
                    this._indicatorsController.setUnconditionallyDisplayTopIndicator(false);
    ...
    private _setItemsOrder(pendingItemsOrder: TItemsOrder): void {
        this._pendingItemsOrder = pendingItemsOrder;
        const markedKey = this._options.getMarkedKeyCompatible(this, this._options);
        const edgeItem = this._listVirtualScrollController.getEdgeVisibleItem(
            'forward',
            undefined,
            markedKey === undefined ? null : markedKey
        );
        const invert = async () => {
            const totalCount = this._listViewModel.getSourceCollectionCount();
            const {
                inverted,
                invertedIndexes: invertedRange,
                extendDirection,
            } = this._listVirtualScrollController.tryToInvert(edgeItem);
            if (inverted) {
                const shiftDirection =
                    invertedRange.startIndex < this._listViewModel.getStartIndex() ? 'up' : 'down';
                this._scheduledRestoreScrollAfterInvert = {
                    edgeItem,
                    invertedRange,
                ...
                const shouldLoad =
                    !this._listVirtualScrollController.hasEmptyItemsToDirection(extendDirection);
                let loadPromise;

                if (shouldLoad) {
                    // возвращает промис, если загрузка пошла
                    // возвращает void, если данных в сторону нет
                    loadPromise = this._itemsEndedCallback(extendDirection);
                    if (!(loadPromise instanceof Promise)) {
                        const shiftDirection =
                            extendDirection === 'backward' ? 'forward' : 'backward';
                        const edgeRange =
                            this._listVirtualScrollController.shiftIndexesToDirectionEdge(
                                shiftDirection
                            );
                        this._scheduledRestoreScrollAfterInvert = {
                            invertedRange: edgeRange,
                            direction: shiftDirection,
                        };
                        return Promise.resolve();
                    }
                    ...
                    const extendedRange =
                        this._listVirtualScrollController.extendIndexesToDirection(
                            extendDirection,
                            totalCount
                        );
                    const shiftDirection =
                        this._listViewModel.getStartIndex() < extendedRange.startIndex
                            ? 'up'
                            : 'down';
                    this._listVirtualScrollController.saveScrollPosition();
                    this._listVirtualScrollController.setScrollBehaviourOnReset('keep');
                    this._listViewModel.setIndexes(
                        extendedRange.startIndex,
                        extendedRange.endIndex,
                        shiftDirection
                    );
                ...
                // после загрузки необходимо подождать рендер и попробовать подгрузить снова
                return new Promise((resolve) => {
                    loadPromise.then(() => {
                        _private.doAfterComponentDidUpdate(this, resolve);
                    });
                }).then(invert);
            }
        };
        ...
        invert().then(() => {
            this._listViewModel.setItemsOrder(pendingItemsOrder);
        });
    }

    protected _onSetItemsOrderFinished(_event: SyntheticEvent, itemsOrder: TItemsOrder) {
        if (this._scheduledRestoreScrollAfterInvert) {
            const {
                edgeItem,
                invertedRange: { startIndex, endIndex },
                direction,
            } = this._scheduledRestoreScrollAfterInvert;
            this._scheduledRestoreScrollAfterInvert = undefined;
            this._listViewModel.setIndexes(startIndex, endIndex, direction);
            this._listVirtualScrollController.setScrollBehaviourOnReset('keep');
            this._listVirtualScrollController.resetItems();

            if (edgeItem) {
                this._listVirtualScrollController._scheduleScroll({
                    type: 'restoreScroll',
                    params: edgeItem,
                });
            } else {
                this._listVirtualScrollController.scrollToEdge(direction);
            }
            this._itemsOrder = itemsOrder;
            this._pendingItemsOrder = undefined;
            _private.doAfterRender(this, () => {
    ...
    protected _onViewPortResized(
        ...
        if (
            oldSizes.scrollTop !== newSizes.scrollTop ||
            oldSizes.height !== newSizes.height ||
            oldSizes.width !== newSizes.width
        ) {
            this._listVirtualScrollController.viewportResized(newSizes);
    ...
    _getScrollParams(
        ...
        const scrollParams = {
            ...
            scrollHeight:
                this._contentHeight +
                this._listVirtualScrollController.getContentSizeBeforeList() +
                pagingPadding -
                stickyElementsHeight,
    ...
    protected _getListVirtualScrollControllerOptions(
        ...
        const hasMoreDataToBackward =
            ...
            this._sourceController.hasMoreData(this._getDataLoadDirection('up'));
        ...
        return {
            ...
            task88221033238881: options.task88221033238881,
            ...
            scrollToElementUtil: (
                ...
                return this._scrollToElement(container, position, force, scrollOffset);
            },
    ...
    private _itemsEndedCallback(direction): Promise<void> | void {
        const compatibleDirection: Direction = direction === 'forward' ? 'down' : 'up';
        const directionTrigger =
            this._itemsOrder && this._itemsOrder !== 'default' ? 'forward' : 'backward';
        if (this._shouldLoadOnScroll()) {
            if (
                this._preloadedItemsToBackward &&
                // Условие для размещения предзагруженных данных в правильное место.
                // Если инвертированность при загрузке совпадает с текущей, то direction остается
                // иначе реагировать на противоположный
                direction === directionTrigger
            ) {
                return this._preloadedItemsToBackward
                    .then((items) => {
                        this._preloadedItemsToBackward = null;
                        ...
                        const preloadedItemsAreAlreadyMerged =
                            items &&
                            items.getCount() > 0 &&
                            this._items.getRecordById(items.at(0).getKey());
                        if (preloadedItemsAreAlreadyMerged) {
                            return;
                        }
                        return this.props._setPreloadedItems(
                            items,
                            this._getDataLoadDirection(compatibleDirection),
                            this
                        );
                    })
                    .then(() => {
                ...
                if (!this._loadToDirectionPromise) {
                    return this._loadMore(compatibleDirection);
                }
            }
        }
    }
    ...
    _initListViewModelHandler(model: Collection<Model>): void {
        if (model) {
            ...
            model.subscribe('onSetItemsOrderFinished', this._onSetItemsOrderFinished.bind(this));
        }
    }
    ...
    _deleteListViewModelHandler(model: Collection<Model>): void {
        if (model) {
            ...
            model.unsubscribe('onSetItemsOrderFinished', this._onSetItemsOrderFinished.bind(this));
        }
    }
    ...
    protected _onCollectionChangedScroll(
        ...
        switch (action) {
            case IObservable.ACTION_RESET:
                ...
                const backwardTriggerVisible =
                    !this._hasMoreData('up') ||
                    // https://online.sbis.ru/opendoc.html?guid=127beff6-73a0-4d48-9684-2706a2bc1072
                    !this._hasMoreData('down') ||
                    (!this._listViewModel.getCount() && !_private.isPortionedLoad(this)) ||
                    !this._options.attachLoadTopTriggerToNull;
            ...
            case IObservable.ACTION_REMOVE:
                ...
                // После удаления записи, индексы ВС могли не поменяться, но в диапазон могли попасть новые записи.
                // Поэтому, нужно сообщить об изменении сейчас.
                if (
                    this._isMounted &&
                    this._options.virtualRangeChangedCallback &&
                    prevRange.startIndex === this._listViewModel.getStartIndex() &&
                    prevRange.endIndex === this._listViewModel.getStopIndex()
                ) {
                    this._listVirtualScrollController.callBeforeRangeChangeCallback({
                        startIndex: this._listViewModel.getStartIndex(),
                        endIndex: this._listViewModel.getStopIndex(),
    ...
    protected _afterMount(): void {
        ...
        if (this._options.itemsDragNDrop && this._container) {
            ...
            this._options.notifyCallback(
                'registerDndDetail',
                [
                    'dndCustomMouseLeaveDetail',
                    this,
                    this.dndCustomMouseLeaveDetailHandler,
                    { listenAll: true },
                ],
                {
                    bubbling: true,
            ...
            this._options.notifyCallback(
                'registerDndMaster',
                [
                    'dndCustomMouseEnterMaster',
                    this,
                    this.dndCustomMouseEnterMasterHandler,
                    { listenAll: true },
                ],
                {
                    bubbling: true,
        ...
        if (dragObject && dragObject.entity && dragObject.entity instanceof ItemsEntity) {
            this._dndState.isDocumentDragging = true;
            this._dndState.dragEntity = dragObject.entity;
        }
        ...
        // UPD: запрашиваем вторую пачку на маунте только, если не заполнили экран сразу.
        // Иначе, визуально ничего не меняется, а произоводительность падает
        if (!this._indicatorsController.getViewportFilled()) {
            this._tryLoadToDirectionAgain(this._items, this._options.root);
        } else if (
            this._indicatorsController.getPortionedSearchDirection() &&
            this._indicatorsController.shouldStopDisplayPortionedSearch()
        ) {
    ...
    // TODO: Методы dndCustomMouseLeaveDetailHandler и dndCustomMouseEnterMasterHandler костыли
    //  для https://online.sbis.ru/opendoc.html?guid=2683cdf7-804e-403c-b8c1-46b95e612eea&client=3
    //  Убрать, когда переведем Controls/columns:View на React
    protected dndCustomMouseLeaveDetailHandler() {
        if (this._isCustomMouseLeaveFlag !== true && this._listViewModel) {
            this._mouseLeave();
            this._notify('dndCustomMouseEnterMaster', [], {
                bubbling: true,
            });
        }
    }

    protected dndCustomMouseEnterMasterHandler() {
        this._mouseEnter();
    }
    ...
    protected _updateBaseControlModel(newOptions: IBaseControlOptions): void {
        ...
        if (newOptions.loadingIndicatorTemplate !== this._options.loadingIndicatorTemplate) {
            this._listViewModel.setLoadingIndicatorTemplate(
                newOptions.loadingIndicatorTemplate,
                this._indicatorsController.shouldDisplayTopIndicator(),
                this._indicatorsController.shouldDisplayBottomIndicator()
            );
        }

        if (newOptions.portionedSearchTemplate !== this._options.portionedSearchTemplate) {
            this._listViewModel.setPortionedSearchTemplate(
                newOptions.portionedSearchTemplate,
                this._indicatorsController.shouldDisplayTopIndicator(),
                this._indicatorsController.shouldDisplayBottomIndicator()
            );
        }

        if (newOptions.continueSearchTemplate !== this._options.continueSearchTemplate) {
            this._listViewModel.setContinueSearchTemplate(
                newOptions.continueSearchTemplate,
                this._indicatorsController.shouldDisplayTopIndicator(),
                this._indicatorsController.shouldDisplayBottomIndicator()
            );
        }
    ...
    protected _getTrackedValueOffset() {
        // была ошибка, что не должно быть двух одинаковых значений, т.е. значение в trackedProperties должно меняться
        // именно в момент пересечения границы, но не раньше:
        // https://online.sbis.ru/opendoc.html?guid=e8735c82-e745-4765-956a-ea34310fd7ca&client=3
        return getStickyHeadersHeight(this._container, 'top', 'allFixed');
    }

    protected _updateTrackedValues(): void {
        if (!this?._listVirtualScrollController?.isRenderNewIndexes?.()) {
            const offset = this._getTrackedValueOffset();
            const contentBeforeList =
                this._container.getBoundingClientRect().top -
                this._container
                    .closest('.controls-Scroll-ContainerBase__content')
                    .getBoundingClientRect().top;
            const trackedValuesVisible =
                this._scrollTop + this._placeholders?.backward >
                    Math.max(contentBeforeList - offset, 0) ||
    ...
    _beforeUpdateReactView(newOptions: TOptions): void {
        if (newOptions.sorting !== this._options.sorting) {
            this._listViewModel?.setSorting?.(newOptions.sorting);
        }
        if (
            !isEqual(this._options.itemEditorTemplateOptions, newOptions.itemEditorTemplateOptions)
        ) {
            this._listViewModel.setItemEditorTemplateOptions(newOptions.itemEditorTemplateOptions);
        }
    }

    private _keepNavigationSliceReloadId?: number;
    ...
    protected _beforeUpdate(
        ...
        if (
            newOptions.itemsOrder &&
            newOptions.itemsOrder !== this._itemsOrder &&
            this._pendingItemsOrder === undefined
        ) {
            this._setItemsOrder(newOptions.itemsOrder);
        }

        if (
            newOptions?.slice?.state?.keepNavigationSliceReloadId !==
            this._keepNavigationSliceReloadId
        ) {
        ...
        if (this.props._isReactView) {
            this._beforeUpdateReactView(newOptions);
        }
        ...
        const loadedBySourceController =
            this._wasReload &&
            // Если есть слайс, то о загрузке сурса надо судить по флагу loading
            (!this._options.slice || this._options.loading !== newOptions.loading);
        ...
        if (!this._listViewModel.getCount() && _private.isEditing(this)) {
        ...
        if (
            ...
            (newOptions.sourceController &&
                this._keyProperty !== newOptions.sourceController.getKeyProperty())
        ) {
            this._initKeyProperty(newOptions);
            _private.checkRequiredOptions(this, newOptions);
        }
        ...
        if (newOptions.sourceController && sourceControllerChanged) {
            ...
            if (itemsChanged && !isSourceControllerLoadingNow && !this._options.fix1193364926) {
        ...
        // Данный код нужно выполнять после обновления sourceController'a
        // https://online.sbis.ru/opendoc.html?guid=ee38f4d4-304c-4ba5-948c-7effe95b5ba3
        if (filterChanged || rootChanged) {
            if (this._hasSelectionController() && loadedBySourceController) {
                this._getSelectionController().setMassSelect(true);
            }
            this._listViewModel.setResultsVisibility?.(newOptions.resultsVisibility);
            // Обновлять опции трекера при пересоздании коллекции не имеет смысла
            // Кроме того, может привести к проблемам при включении поиска.
            if (!shouldReInitCollection && newOptions.trackedProperties) {
                this._updateTrackedValues();
            }
        }
        ...
        if (newOptions.sourceController || newOptions.items) {
            if (newOptions.storeId ? newOptions.loading : isSourceControllerLoadingNow) {
        ...
        if (shouldReInitCollection && this._listViewModel) {
            ...
            const newItems =
                loadedBySourceController || sourceControllerChanged
        ...
        // У нас не сработает dataLoadCallback, т.к. мы не успели подписаться.
        // Поэтому сами вызываем колбэк в этом кейсе.
        // Флаг fix1193364926 необходим, чтобы загрузка данных в колбэке itemsChangedOnDataLoad
        // произошла после установки новых данных в коллекцию.
        if (
            this._options.fix1193364926 &&
            newOptions.sourceController &&
            ...
            this._dataLoadCallback(new EventObject('dataLoad', this), items, null);
            this._itemsChangedOnDataLoad(new EventObject('itemsChanged', this), items, null, null);
        }
        ...
        if (this._items) {
            ...
            if (
                ...
                const newSelection =
                    newOptions.selectedKeys === undefined || newOptions.selectedKeys === null
        ...
        if (newOptions.hasItemWithImage !== this._options.hasItemWithImage) {
            this._restoreScrollDirection = this._loadedItemsDirection || 'up';
            this._shouldHandleResize = true;
        }
        ...
        if (
            ...
            // Во время рендера обновлять ItemActions нельзя, т.к. будет попытка установить их стейт и их сразу же отрисовать,
            // Что приведёт к ошибкам реакта https://github.com/facebook/react/issues/18178.
            // На будущее надо или убирать из itemActionsController вызов setState при любом обновлении ItemActions,
            // Либо выполнять обновление в хуке после рендера (e.g. useEffect)
            this._shouldUpdateItemActionsOnDidUpdate = true;
        ...
        if (!newOptions.useCollection) {
            newOptions.setCollection(this._listViewModel, false);
        }
    ...
    protected _reinitializeModel(options: TOptions, newItems: RecordSet): void {
        ...
        options.slicelessUpdateControllers?.(this, options);
    ...
    scrollToItem(
        ...
        return _private
            .scrollToItem(this, key, position, force, allowLoad)
            .then(() => this._options.trackedProperties && this._updateTrackedValues());
    }
    ...
    protected _unmount(): void {
        ...
        if (this._listViewModel) {
            ...
            if (!this._options.collection) {
                if (this._options.fix1193694464) {
                    this._options.setCollection(null, false);
                }
    ...
    protected _afterRender(oldOptions: IBaseControlOptions): void {
        ...
        this._dndState.controller?.afterRenderListControl();
        this._updateInProgress = false;
        this._notifyOnDrawItems();
        if (
            this._wasReload &&
            (!this._options.slice || this._options.loading !== oldOptions.loading)
        ) {
            this._preloadDataToBackward();
    ...
    protected _$react_componentDidUpdate(): void {
        ...
        if (this._shouldUpdateItemActionsOnDidUpdate) {
            let itemToUpdateActions: CollectionItem;
            if (this._editInPlaceController?.isEditing()) {
                itemToUpdateActions =
                    this._editInPlaceController.getEditableItem() as unknown as CollectionItem;
            }
            _private.updateInitializedItemActions(this, this._options, itemToUpdateActions);
            this._shouldUpdateItemActionsOnDidUpdate = false;
        }
        ...
        if (this._loadedItems) {
            this._loadedItems = null;
            this._loadedItemsDirection = null;
        }
        ...
        if (this.callbackAfterComponentDidUpdate) {
            this.callbackAfterComponentDidUpdate.forEach((callback) => {
                callback();
            });
            this.callbackAfterComponentDidUpdate = null;
        }
    }
    ...
    private _notifyOnDrawItems(): void {
        if (this._shouldNotifyOnDrawItems) {
            this._options.notifyCallback('drawItems', [], { bubbling: true });
            this._options.drawItemsCallback?.();
        ...
        switch (arrow) {
            ...
            case 'Begin':
                ...
                if (resultEventBegin !== false) {
                    if (resultEventBegin instanceof Promise) {
                        resultEventBegin.then((result) => {
                            if (result !== false) {
            ...
            case 'End':
                ...
                if (resultEventEnd !== false) {
                    if (resultEventEnd instanceof Promise) {
                        resultEventEnd.then((result) => {
                            if (result !== false) {
    ...
    protected _scrollToInitialPosition(): Promise<unknown> {
        ...
        // Если при прямом порядке элементов первый элемент должен быть прижат к верху,
        // то в инвертированном он прижат к низу
        const initialPosition =
            this._itemsOrder && this._itemsOrder !== 'default' ? 'bottom' : 'top';
        ...
        if (canScrollToItem) {
            ...
            return this.scrollToItem(this._initialPosition, initialPosition, true).then(() => {
                this._updateTrackedValues();
            });
        } else {
    ...
    __needShowEmptyTemplate(newOptions?: IBaseControlOptions): boolean {
        ...
        const emptyRender = options.emptyRender;
        ...
        return (
            (emptyTemplate || emptyTemplateColumns || emptyRender) &&
            noEdit &&
            notHasMore &&
    ...
    reload(
        ...
        if (keepNavigation) {
            ...
            if (sourceConfigOption) {
                ...
                if (
                    ...
                    this._options.navigation.sourceConfig.position !== sourceConfigOption.position
                ) {
        ...
        // При reload позиция горизонтального скролла не должна изменяться.
        //  https://online.sbis.ru/opendoc.html?guid=afa5ee95-8af5-4eec-9245-73ded55deab7&client=3
    ...
    private _isSourceControllerLoadingNow(options: IList): boolean {
        return options.isSourceControllerLoadingNow(this, options);
    }
    ...
    // Для них https://git.sbis.ru/contragents/engine/-/blob/rc-24.3200/client/Addressee/Selector/_popup/SelectionResolver.ts?ref_type=heads#L892
    unsafe_getMarkedKey(self: BaseControl): CrudEntityKey | null | undefined {
        return self._options.getMarkedKeyCompatible(self, self._options);
    }

    protected _hasMoreData(direction: Direction): boolean {
        return !!(
            this._sourceController &&
            this._sourceController.hasMoreData(this._getDataLoadDirection(direction))
        );
    }
    ...
    protected _loadItemsToDirection(
        ...
        return this.props._loadItemsToDirection(direction, addItemsAfterLoad, useServicePool, this);
    }

    protected _loadItemsToDirectionInner(
        direction: ScrollControllerLib.IDirection,
        addItemsAfterLoad?: boolean,
        useServicePool?: boolean
    ): Promise<RecordSet | Error> {
        return this._sourceController.load(
            this._getDataLoadDirection(direction),
            undefined,
            undefined,
            addItemsAfterLoad,
            undefined,
    ...
    protected _shouldLoadItemsToDirection(direction: ScrollControllerLib.IDirection): boolean {
        ...
        const allowLoadByDrag = !(
            this._dndState.controller?.isDragging() && this._selectionController?.isAllSelected()
        );
    ...
    protected _handleCheckboxClick(
        ...
        const collectionItem = this._listViewModel.getItemBySourceKey(itemKey);
        if (clickOnCheckbox) {
            this._onCheckBoxClick(event, collectionItem, originalEvent);
            event.stopPropagation();
        }
    ...
    protected _onItemClick(
        ...
        if (this._handleCheckboxClick(event, contents, originalEvent)) {
            return;
        }
        ...
        const editingTarget = originalEvent.target.closest(`.${EDITABLE_JS_SELECTOR}`);
    ...
    private _getItemActionsController(
        options: IList = this._options,
        skipEIPToolbarVisibilityCheck: boolean = false
        ...
        const isEditingButtonsVisible = editingConfig?.toolbarVisibility;
        ...
        if (
            ...
            (skipEIPToolbarVisibilityCheck || !isEditingButtonsVisible) &&
    ...
    protected _onItemActionMouseDown(
        ...
        if (
            action.id !== EDITING_APPLY_BUTTON_KEY &&
            action.id !== EDITING_CLOSE_BUTTON_KEY &&
    ...
    beginEdit(userOptions?: IItemEditOptions): TAsyncOperationResult {
        if (this._options.readOnly) {
            return BaseControl._rejectEditInPlacePromise('beginEdit', this._prepareExtLogInfo());
        }
    ...
    beginAdd(userOptions?: IItemAddOptions): TAsyncOperationResult {
        if (this._options.readOnly) {
            return BaseControl._rejectEditInPlacePromise('beginAdd', this._prepareExtLogInfo());
        }
    ...
    cancelEdit(): TAsyncOperationResult {
        if (this._options.readOnly) {
            return BaseControl._rejectEditInPlacePromise('cancelEdit', this._prepareExtLogInfo());
        }
    ...
    commitEdit(): TAsyncOperationResult {
        if (this._options.readOnly) {
            return BaseControl._rejectEditInPlacePromise('commitEdit', this._prepareExtLogInfo());
        }
        return this._commitEdit();
    }
    ...
    private _onEditingRowKeyDown(event: SyntheticEvent<KeyboardEvent>): TAsyncOperationResult {
        ...
        const editNext = (
            ...
            if (
                this._listViewModel['[Controls/_display/grid/mixins/Grid]'] &&
                !this._options._$rowSelector
            ) {
    ...
    private _onEditingCellTabHandler(
        ...
        let shouldAdd;
        const checkboxOffset = +this._hasMultiSelect();
        let columnsLength = this._options.columns.length;
        const cellWithCells = eventOptions.keyPressedData.target.closest(
            this._options._$rowSelector
        );

        if (cellWithCells) {
            columnsLength += cellWithCells.childNodes.length;
        }

        if (eventOptions.isShiftKey) {
            this._continuationEditingDirection = 'PrevColumn';
            let isEditableCell;
            columnIndex = editingItem._$editingColumnIndex - checkboxOffset;

            while (isEditableCell !== true) {
                columnIndex -= 1;

                const shouldGoToPrevRow = columnIndex < 0;

                if (shouldGoToPrevRow) {
                    next = this._editInPlaceController.getPrevEditableItem(editingItem);
                    columnIndex = columnsLength - 1 + checkboxOffset;
                }
                ...
                isEditableCell = _private.checkCellIsEditableByColumnIndex(
                    this,
                    next,
                    columnIndex,
                    cellWithCells,
            ...
            while (isEditableCell !== true) {
                ...
                const shouldGoToNextRow = columnIndex > columnsLength - 1 - checkboxOffset;
                ...
                shouldAdd =
                    editingConfig.autoAdd && !next && editingConfig.addPosition === 'bottom';
        ...
        return this._tryContinueEditing(
            !!next,
            shouldAdd,
            next && next.contents,
            columnIndex + checkboxOffset
    ...
    _onItemContextMenu(
        ...
        if (!this._options.contextMenuVisibility || this._listViewModel.isEditing()) {
            return;
        }
    ...
    protected _itemMouseDown(
        ...
        if (!!domEvent.target.closest(ITEM_ACTION_SELECTOR)) {
            ...
            // В чистом react рендере вьюшки _itemMouseDown вызывается как коллбек, event === domEvent,
            // и в event оказывается не itemMouseDown, а нативное событие mouseDown.
            // Если стопнуть его, то Application на своём уровне не узнает, что был произведён mouseDown,
            // и не обработает хандлеры, которые к нему привязаны.
            // Например, не будет закрыто открытое меню по клику за его пределами.
            if (event !== domEvent) {
                event.stopPropagation();
            }
            return;
        ...
        // Клик на чекбокс не должен приводить к dnd
        // https://online.sbis.ru/opendoc.html?guid=caa78440-e243-4a86-a644-da6b0bfcdfea&client=3
        if (!event.nativeEvent.target.closest('.js-controls-ListView__checkbox')) {
            const dndProps = this._getDndProps(hasDragScrolling);
            const dndApi = this._getDndAPI();

            dndCore.tryStart(
                this._dndState,
                dndProps,
                (domEvent as SyntheticEvent).nativeEvent ?? domEvent,
                itemData.key,
                dndApi
            );
        } else {
    ...
    protected _startDragNDropCallback(): void {
        const dndProps = this._getDndProps(hasDragScrolling);
        const dndApi = this._getDndAPI();
        dndCore.startDragNDrop(
            this._dndState,
            dndProps,
            (this._savedItemMouseDownEventArgs.domEvent as SyntheticEvent).nativeEvent ??
    ...
    protected _resolveNavigationButtonView(options): TNavigationButtonView {
        const navigation = options ? options.navigation : this._options.navigation;
    ...
    moveItemsWithDialog(
        ...
        return this._callAction<DataSet>(
            ...
            viewCommandName === undefined ? VIEW_COMMAND.PARTIAL_RELOAD : viewCommandName,
            {
                ...
                useDefaultMoveMethod: !!config?.useDefaultMoveMethod,
                popupOptions,
                selection,
                filter: this._options.filter,
            },
    ...
    removeItemsWithConfirmation(
        ...
        resetSelectionOnSuccess?: boolean
    ): Promise<string | void> {
        const viewCommandName = typeof config === 'string' ? config : config?.viewCommandName;
        this._addCoordinatesToActionMenuEvent(config?.target);
        return this._callAction<string | void>(
    ...
    private _addCoordinatesToActionMenuEvent(target: EventTarget | null): void {
        ...
        const popupCoordinatesSource = target.closest(MENU_TOP_SELECTOR);
        const rowCoordinatesSource = target.closest(MENU_ROW_SELECTOR);
        ...
        if (popupCoordinatesSource) {
            const rect = popupCoordinatesSource.getBoundingClientRect();
            target.dataset.popupRect = JSON.stringify(rect);
        }
        if (rowCoordinatesSource) {
            const rect = rowCoordinatesSource.getBoundingClientRect();
            target.dataset.rowRect = JSON.stringify(rect);
        }
    }
    ...
    protected _onViewKeyDown(event: SyntheticEvent<KeyboardEvent>): void {
        ...
        if (allowByLoading) {
            ...
            EventUtils.keysHandler(event, HOT_KEYS_DOWN, _private, this, dontStop);
        } else {
    ...
    protected _onViewKeyUp(event: SyntheticEvent<KeyboardEvent>): void {
        EventUtils.keysHandler(event, HOT_KEYS_UP, _private, this, true);
    }
    ...
    protected _getItemsContainerClasses(): string {
        ...
        if (this._addShowActionsClass) {
            ...
            classes.push(
                `${SHOW_ACTIONS_CLASS}_${_private.getItemActionsVisibility(this._options)}`
            );
        }
        ...
        if (
            ...
            classes.push(CONTEXTMENU_ENABLED_CLASS);
        }
    ...
    protected _itemMouseMove(
        ...
        if (
            ...
            (!this._dndState.controller || !this._dndState.controller.isDragging()) &&
        ...
        if (this._dndState.controller && this._dndState.controller.isDragging()) {
    ...
    protected _draggingItemMouseMove(targetItem: CollectionItem, event: MouseEvent): boolean {
        return dndCore.moveOnItem(this._dndState, this._getDndProps(), event, targetItem.key);
    }
    ...
    protected _mouseEnter(): void {
        this._isCustomMouseLeaveFlag = false;
        if (this._listViewModel) {
            dndCore.dragEnter(this._dndState, this._getDndProps(), {
                entity: this._dndState.dragEntity,
            });
        }
    }
    ...
    protected _mouseLeave(): void {
        this._isCustomMouseLeaveFlag = true;
        if (this._listViewModel) {
            dndCore.dragLeave(this._dndState, this._getDndProps());
        }
    }
    ...
    protected _onTagHoverHandler(
        ...
        this._options.notifyCallback('tagHover', [item?.getContents(), columnIndex, event]);
    }

    _applyPagingNavigationState(params): void {
        const newNavigation = cClone(this._options.navigation);
        const sourceConfig = newNavigation.sourceConfig;
        if (params.pageSize) {
            sourceConfig.pageSize = params.pageSize;
            sourceConfig.page = this._currentPage - 1;
        }
        if (params.page) {
            sourceConfig.page = params.page - 1;
            sourceConfig.pageSize = this._currentPageSize;
        }
    ...
    _onItemSwipe(e: SyntheticEvent<Event>, item: CollectionItem<Model>, swipeEvent): void {
        ...
        if (direction === 'left' && this._options.itemActionsVisibility !== 'visible') {
            this._options.mark(key, this, this._options);
            _private.updateItemActionsOnce(this, this._options);
            itemActionsController = this._getItemActionsController(this._options, true);
            if (itemActionsController) {
    ...
    protected _createNewModel(
        ...
        const modelComposedConfig = {
            ...
            columnsWidths,
            // Провоцирует утечки.
            // В рамках упрощения коллекций необходимо придерживаться подхода, что спред опций это плохо.
            // Он допустим только между приватными модулями, публичные опции надо отрезать.
            // https://online.sbis.ru/opendoc.html?guid=319555ee-ddd5-4caa-b3a4-a1bf6c0f43a3&client=3
            slice: undefined,
        };
     ...
     * нужно скрывать MultiSelect, но оставлять под них место.
    ...
    private _resolveMultiSelectVisibility(multiSelectVisibility: string): string {
        ...
        if (hasSelected && detection.isMobilePlatform) {
            result = 'visible';
        }
        return result;
    }
    ...
    private _scrollToFirstItemAfterDisplayTopIndicator(onDrawItems: boolean = false): void {
        const scrollAndShowTrigger = () => {
            if (this._scrollToItemAfterReloadParams) {
                const { key, position, force, resolveScroll, scrollOffset } =
                    this._scrollToItemAfterReloadParams;
                this._scrollToItemAfterReloadParams = null;
                this._listVirtualScrollController
                    .scrollToItem(key, position, force, undefined, undefined, scrollOffset)
                    .then(() => {
                        this._listVirtualScrollController.setBackwardTriggerVisibility(true);
                        resolveScroll();
                    });
    ...
    protected _getDndListController(): DndController {
        return this._dndState.controller;
    }

    private _documentDragStart(dragObject: IDragObject): void {
        dndCore.onDocumentDragStart(this._dndState, this._getDndProps(), dragObject);

        if (
            typeof this._dndState.draggedKey !== 'undefined' &&
            this._dndState.draggedKey !== null &&
            _private.hasHoverFreezeController(this)
        ) {
            this._hoverFreezeController.unfreezeHover();
        }
    ...
    private _documentDragEnd(dragObject: IDragObject): void {
        dndCore.onDocumentDragEnd(
            this._dndState,
            this._getDndProps(),
            dragObject,
            (endDrag) => {
                // Это функция срабатывает при перетаскивании скролла, поэтому проверяем _dndState.controller
                ...
                if (this._dndState.controller) {
                    if (this._dndState.controller.getDragEndPromise() instanceof Promise) {
                        this._displayGlobalIndicator();
                        this._dndState.controller.getDragEndPromise().finally(() => {
                            endDrag();
    ...
    protected _getDndProps(hasDragScrolling?: boolean): dndCore.IDragNDropProps {
        return {
            ...this._options,
            viewModelState: {
                ...this._options,
            ...
            viewModelAPI: {
                mark: (draggedKey) => this._options.mark(draggedKey, this, this._options),
                resetSelection: () => this._changeSelection({ selected: [], excluded: [] }),
                getSelection: () => {
                    return Promise.resolve({
                        marked: this._options.selectedKeys || [],
                        excluded: this._options.excludedKeys || [],
                    });
                },
                collapse: this._beforeStartDrag.bind(this),
            },
            hasDragScrolling: hasDragScrolling ?? false,
            canStartDragNDrop: this._options.canStartDragNDrop,
            onViewDragStartCompatible: (draggedKey) =>
                this._options.onViewDragStartCompatible?.(draggedKey, this, this._options),
            onDragStart: (items, draggableKey) =>
                this._options.notifyCallback?.('customdragStart', [items, draggableKey]),
            onDragMove: (dragObject) => this._options.notifyCallback?.('dragMove', [dragObject]),
            onDragEnd: (dragEntity, item, targetPosition) =>
                this._options.notifyCallback('customdragEnd', [
                    dragEntity,
                    item,
                    targetPosition,
                ]) as Promise<void> | void,
            onDocumentDragStart: (dragObject) => {
                this._options.notifyCallback?.('_documentDragStart', [dragObject], {
                    bubbling: true,
                });
            },
            onDocumentDragEnd: (dragObject) => {
                this._options.notifyCallback?.('_documentDragEnd', [dragObject], {
                    bubbling: true,
                });
            },
            onChangeDragTarget: (dragEntity, item, targetPosition) =>
                this._options.notifyCallback('changeDragTarget', [
                    dragEntity,
                    item,
                    targetPosition,
                ]),
            onDragEnter: (dragEntity) =>
                this._options.notifyCallback('customdragEnter', [dragEntity]),
            updateDraggingRenderCallback: (dragObject, draggingRender) =>
                this._options.notifyCallback?.(
                    '_updateDraggingTemplate',
                    [dragObject, draggingRender],
                    { bubbling: true }
                ),
            onDraggingRenderRemove: () =>
                this._options.notifyCallback('_removeDraggingTemplate', [], {
                    bubbling: true,
                }),
            draggingRender: this._options.draggingTemplate,
            listContainerRef: {
                current: this._container[0] || this._container,
            },
            register: (eventName, handler) =>
                this._options.notifyCallback?.('register', [eventName, this, handler], {
                    bubbling: true,
                }),
            unregister: (eventName) =>
                this._options.notifyCallback('unregister', [eventName, this], {
                    bubbling: true,
                }),
        };
    }

    private _getDndAPI(): dndCore.IDragNDropAPI {
        if (this._dndApi) {
            return this._dndApi;
        }
        ...
        const dndApi = {
            move: (e) =>
                dndCore.move(
                    this._dndState,
                    this._getDndProps(),
                    (e as SyntheticEvent).nativeEvent ?? e
                ),
            end: (e) =>
                dndCore.end(
                    this._dndState,
    ...
    protected _getSelectionControllerOptions(
        ...
        return {
            ...
            rootKey: null,
            hasMoreUtil: this._hasMoreUtil.bind(this),
            isLoadedUtil: this._isLoadedUtil.bind(this),
    ...
    protected _hasMoreUtil(key: TKey) {
        return (
            this._sourceController &&
            (this._sourceController.hasMoreData(this._getDataLoadDirection('up'), key) ||
                this._sourceController.hasMoreData(this._getDataLoadDirection('down'), key))
        );
    }

    protected _isLoadedUtil(key: CrudEntityKey) {
    ...
    private _onSelectedTypeChanged(typeName: string, limit: number | undefined): void {
        ...
        switch (typeName) {
            case 'selectAll':
                selectionController.resetLimit();
                ...
                loadAsync<typeof import('Controls/listCommands')>('Controls/listCommands').then(
                    ({ ShowOnlySelected }) => {
                        ...
                        const command = new ShowOnlySelected({
                            ...
                            selectDescendants: this._options.selectDescendants,
    ...
    private _notifySelection(selection: ISelectionObject): void {
        ...
        const _notifyDiff = (diff: IKeysDifference, eventName: string) => {
            if (diff.added.length || diff.removed.length) {
                this._notify(eventName, [diff.keys, diff.added, diff.removed]);
            }
        };

        _notifyDiff(selectionDifference.selectedKeysDifference, 'selectedKeysChanged');
        _notifyDiff(selectionDifference.excludedKeysDifference, 'excludedKeysChanged');

        // После вызова _notifyDiff на контроллере может быть вызван destroy
        if (!this._hasSelectionController()) {
            return;
        }
    ...
    _getNavigationButtonClasses(
        ...
        const isSeparatorButton = this._resolveNavigationButtonView(options) === 'separator';
        let classes = '';
        let buttonPosition = buttonConfig?.buttonPosition;
        if (!buttonPosition) {
            buttonPosition = isSeparatorButton ? 'center' : 'start';
        }
        if (buttonPosition === 'start') {
    ...
    setIsColumnScrollVisible(isColumnScroll: boolean) {
        if (this._isColumnScrollVisible !== isColumnScroll && isColumnScroll !== undefined) {
            this._isColumnScrollVisible = isColumnScroll;
        }
    }
    ...
    protected _prepareExtLogInfo(): TExtLogInfo {
        const extLogInfo: TExtLogInfo = {
            keyProperty: this._keyProperty,
        };

        if (this._sourceController) {
            const source = this._sourceController.getSource();
            if (source instanceof SbisService) {
                extLogInfo.endpoint = source.getEndpoint().contract as string;
            } else {
                extLogInfo.endpoint = 'MemorySource';
            }
        }
    ...
    private static _rejectEditInPlacePromise(
        fromWhatMethod: string,
        extLogInfo?: TExtLogInfo
    ): Promise<void> {
        const msg = ERROR_MSG.CANT_USE_IN_READ_ONLY(fromWhatMethod, extLogInfo);
        Logger.warn(msg);
