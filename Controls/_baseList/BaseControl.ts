...
const SPACE_THROTTLE_DELAY = 450;
/**
...
const _private = {
    ...
    // SPACE_THROTTLE_DELAY это время выполнения примерно 3х обновлений слайса в старых списках
    spaceHandler: throttle((self, event) => {
    ...
    implements IMovableList, IEditableList
{
    ...
    private _changeSelection(
        ...
        const handleResult = (selection) => {
            ...
            // Уведомление о новых selectedKeys располагаются ниже уведомления о счетчике для корректной работы
            // выделения при зажатом пробеле в слайсе.
            // Выделение по пробелу изменяет выделение относительно маркируемой записи, поэтому
            // маркер и выделение должны меняться одновременно в слайсе. Пока нет батчинга setState текущий порядок нотификаци
            // позволяет запустить сначала обновление счетчика, а маркер и selection сгруппировать на следующее обновление.
