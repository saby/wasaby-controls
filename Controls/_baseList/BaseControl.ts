...
import { getKey, getPlainItemContents } from 'Controls/_baseList/resources/utils/helpers';
import {
    ...
    implements IMovableList, IEditableList
{
    ...
    constructor(options: IBaseControlOptions, context: object) {
        ...
        const handleWithCollectionItem = (
            item: Model,
            handler: (collectionItem: CollectionItem) => unknown
        ) => {
            const key = getKey(item);
            const collectionItem = this._listViewModel.getItemBySourceKey(key, false);
            if (collectionItem) {
                return handler(collectionItem);
            }
        };

        this._itemHandlers = {
            onMouseOverCallback: (event, item) => {
                return handleWithCollectionItem(item, (collectionItem) => {
