            ...
            if (self._viewportHeight) {
                ...
                const callback = () => {
                    if (self._isMounted) {
                        ...
                        self?._scrollPagingCtr?._options?.pagingCfgTrigger?.(self._pagingCfgCache);
    ...
    implements IMovableList, IEditableList
{
    ...
    private _setItemsOrder(pendingItemsOrder: TItemsOrder): void {
        ...
        const invert = async () => {
            ...
            const { inverted, invertedRange, extendDirection } =
                this._listVirtualScrollController.tryToInvert(
                    edgeItem,
                    totalCount,
    ...
    private _isSourceControllerLoadingNow(options: IList): boolean {
        return typeof options.loading === 'undefined'
            ? options.sourceController && options.sourceController.isLoading()
            : options.loading;
    }
