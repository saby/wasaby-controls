...
const _private = {
    ...
    loadToDirection(
        ...
        if (self._sourceController) {
            ...
            return self
                ._loadItemsToDirection(direction, addItemsAfterLoad)
    ...
    loadToDirectionIfNeed(
        ...
        if (self._shouldLoadItemsToDirection(direction)) {
            ...
            return _private.loadToDirection(self, direction, addItemsAfterLoad);
        }
    },
    ...
    handleListScrollSync(self: BaseControl, scrollTop: number): void {
        ...
        if (self._options.trackedProperties && isRealScroll) {
            const trackedValuesVisible = self._scrollTop > 0 || self._hasMoreData('up');
    ...
    implements IMovableList, IEditableList
{
    ...
    protected _beforeMount(
        ...
        if (newOptions.sourceController) {
            ...
            this._sourceController.subscribe('itemsChanged', this._itemsChangedOnDataLoad);
            this._sourceController.subscribe('dataLoadStarted', this._dataLoadStartedCallback);
            this._sourceController.subscribe('dataLoadCancel', this._dataLoadCancelCallback);
    ...
    private _dataLoadCallback(
        ...
        if (isReload) {
            const shouldResetScroll = this._isMounted && !this._options.keepScrollAfterReload;
    ...
    protected _tryLoadToDirectionAgain(loadedItems: RecordSet, root?: TKey): void {
        if (this._destroyed) {
            return;
        }
        ...
        if (needLoad) {
            ...
            if (
                ...
                direction = this._indicatorsController.getPortionedSearchDirection();
    ...
    private _preloadDataToBackward(): void {
        ...
        if (this._preloadedItemsToBackward && !renderPreloadedDataRightAway) {
        ...
        return {
            ...
            triggersPositions: {
                backward:
                    ...
                    options.topTriggerOffsetCoefficient === DEFAULT_TRIGGER_OFFSET
                        ? 'null'
                        : 'offset',
                forward:
                    hasMoreDataToForward &&
                    options.bottomTriggerOffsetCoefficient === DEFAULT_TRIGGER_OFFSET
                        ? 'null'
                        : 'offset',
            },
    ...
    private _createListVirtualScrollController(
        ...
        const VirtualScrollController =
            options.listVirtualScrollControllerConstructor || ListVirtualScrollController;
    ...
    private _updateVirtualNavigation(hasItemsOutsideOfRange: IHasItemsOutsideOfRange): void {
        ...
        const bottomEnabled = hasItemsOutsideOfRange?.forward || this._hasMoreData('down');
    ...
    protected _beforeUpdate(
        ...
        if (newOptions.multiSelectVisibility === 'hidden' && this._hasSelectionController()) {
    ...
    protected _unmount(): void {
        ...
        if (this._sourceController) {
            ...
            this._sourceController.unsubscribe('itemsChanged', this._itemsChangedOnDataLoad);
            this._sourceController.unsubscribe('dataLoadStarted', this._dataLoadStartedCallback);
            this._sourceController.unsubscribe('dataLoadCancel', this._dataLoadCancelCallback);
    ...
    protected _changeMarkedKey(
        ...
        const eventResult: Promise<CrudEntityKey> | CrudEntityKey = this._isMounted
            ? this._options.notifyCallback('beforeMarkedKeyChanged', [newMarkedKey])
            : undefined;
        ...
        } else if (eventResult !== undefined && (this._environment || this.UNSAFE_isReact)) {
    ...
    protected _loadItemsToDirection(
        ...
        return this._sourceController.load(direction, undefined, undefined, addItemsAfterLoad);
    }
    ...
    protected _shouldLoadItemsToDirection(direction: IDirection): boolean {
        ...
        return allowLoadBySource && allowLoadByLoadedItems && allowLoadBySearch && allowLoadByDrag;
    }
    ...
    _createEditInPlaceControllerAsync(options: IBaseControlOptions = this._options): Promise<void> {
        return Promise.resolve()
            ...
            .then((lib) => {
                this._createEditInPlaceController(options, lib.Controller, lib.InputHelper);
            });
    }
    ...
    onactivated(
        ...
        if (eventOptions?.isTabPressed && this.__needShowEmptyTemplate(this._options)) {
            ...
            const pressedKey = eventOptions?.keyPressedData?.key;
            if ((pressedKey === 'Tab' || pressedKey === 'Enter') && this._canMarkItemOnActivate()) {
    ...
    private _canMarkItemOnActivate(): boolean {
        ...
        return (
            ...
            model.getIndexByKey(_private.getMarkerController(this).getMarkedKey()) === -1
        );
    }
    ...
    protected _itemMouseUp(e: SyntheticEvent, itemData, domEvent: MouseEvent): void {
        ...
        if (this._options._needSetMarkerCallback) {
            canBeMarked =
                canBeMarked && this._options._needSetMarkerCallback(itemData.item, domEvent);
        }
    ...
    protected _getItemContainerByKey(itemKey: CrudEntityKey): HTMLElement {
        const itemsSelector = this._options.itemsSelector || '.controls-ListView__itemV';
        const uniqueItemSelector =
            `.${this._getItemsContainerUniqueClass()} > ` +
            `${itemsSelector}[item-key='${itemKey}']`;
        return this._getItemsContainer().querySelector(uniqueItemSelector) as HTMLElement;
    }
    ...
    protected _getModelOptions(baseControlOptions: TOptions): Partial<ICollectionOptions> {
        return {};
    }
