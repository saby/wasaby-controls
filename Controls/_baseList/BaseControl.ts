    ...
    implements IMovableList, IEditableList
{
    ...
    private _isPreloadedItemsToBackwardInverted?: boolean;
    ...
    private _getDataLoadDirection(direction: Direction): Direction {
        ...
        const order = this._options?.slice?.state?.order;
        const isInverted = order && order !== 'default';
    ...
    private _dataLoadCallback(
        ...
        this._beforeDataLoadCallback(items, this._getDataLoadDirection(direction));
        ...
        if (!this.getItems().getCount() && !items.getCount()) {
            const backwardTriggerVisible =
                !this._hasMoreData(this._getDataLoadDirection('up')) ||
    ...
    private _itemsChangedOnDataLoad(
        ...
        this._indicatorsController.onDataLoad(items, this._getDataLoadDirection(direction));
    ...
    private _preloadDataToBackward(): void {
        ...
        this._isPreloadedItemsToBackwardInverted = this._options?.slice?.state?.order;
    ...
    private savedEdgeElement;
    protected _onInvertSortStarted(_event: SyntheticEvent) {
        const invertedRelativeToElement =
            this._options?.slice?.state?.markedKey ||
            this._markerController?.getMarkedKey() ||
            null;
        let edgeItem = this._listVirtualScrollController.getEdgeVisibleItem(
            'backward',
            undefined,
            invertedRelativeToElement
        );

        // TODO: ### вынужденная мера, чтобы определить размер элемента
        const elementHeight = this._listVirtualScrollController._scrollController
            .getItemsSizes()
            .find((item) => {
                return item.key === edgeItem.key;
            }).size;
        // если до границы больше размера самого элемента
        // значит он сильно далеко за пределами текущего viewPort и восстанавливаться к нему нельзя,
        // поэтому надо запросить другую границу
        if (Math.abs(edgeItem.borderDistance) > elementHeight) {
            edgeItem = this._listVirtualScrollController.getEdgeVisibleItem(
                'forward',
                undefined,
                invertedRelativeToElement
            );
        }
        if (invertedRelativeToElement === null) {
            edgeItem.borderDistance -= elementHeight / 2;
        }
        this.savedEdgeElement = edgeItem;
        this.restoreAfterInvert = true;
    }
        ...
        return {
            ...
            itemsEndedCallback: (direction): void => {
                ...
                const directionTrigger =
                    this._options?.slice?.state?.order &&
                    this._options?.slice?.state?.order !== 'default'
                        ? 'forward'
                        : 'backward';
                    ...
                    if (
                        ...
                        this._preloadedItemsToBackward
                            .then((items) => {
                                ...
                                return this.props._setPreloadedItems(items, compatibleDirection);
                            })
                            .then(() => {
                                if (!this._hasMoreData(this._getDataLoadDirection('up'))) {
    ...
    _initListViewModelHandler(model: Collection<Model>): void {
        if (model) {
            ...
            model.subscribe('onInvertSortStarted', this._onInvertSortStarted.bind(this));
        }
    }
    ...
    _deleteListViewModelHandler(model: Collection<Model>): void {
        if (model) {
            ...
            model.unsubscribe('onInvertSortStarted', this._onInvertSortStarted.bind(this));
        }
    }
    ...
    protected _beforeUpdate(
        ...
        this._options.slicelessUpdateModel?.(this, this._options, newOptions);
    ...
    protected _$react_componentDidUpdate(): void {
        ...
        if (this.restoreAfterInvert) {
            this.restoreAfterInvert = false;
            const compatibleDirection = {
                backward: 'up',
                forward: 'bottom',
            };
            if (this.savedEdgeElement.key) {
                const { key, direction } = this.savedEdgeElement;
                let { borderDistance: offset } = this.savedEdgeElement;
                this.savedEdgeElement = undefined;
                if (direction === 'forward') {
                    offset = offset * -1;
                }
                this._listVirtualScrollController.scrollToItem(
                    key,
