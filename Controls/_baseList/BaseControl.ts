...
import { AnimationContext, ANIMATION_DURATION } from 'Controls/animation';
...
const _private = {
    ...
    callDataLoadCallbackCompatibility(
        ...
        if (self._sourceController && options.dataLoadCallback) {
            ...
            if (sourceControllerDataLoadCallback !== options.dataLoadCallback) {
                options.dataLoadCallback(items, direction, undefined, navigationSourceConfig);
            }
        }
    },
    ...
    keyDownDel(self: BaseControl, event: SyntheticEvent<MouseEvent>): void {
        ...
        if (itemActions) {
            const deleteAction = itemActions.all.find((itemAction: IItemAction) => {
    ...
    loadToDirection(self: BaseControl, direction: IDirection): Promise<RecordSet | void> | void {
    ...
    handleListScrollSync(self: BaseControl, scrollTop: number): void {
        ...
        if (self._options.trackedProperties && !self._scrollControllerInitializeChangeScroll) {
    ...
    onCollectionChanged(
        ...
        if (self._animatedAction === action) {
            if (newItems.length > 1 || removedItems.length > 1) {
                Logger.error('Анимация поддерживается для операций с одной записью.');
            } else {
                if (action === IObservable.ACTION_ADD) {
                    self._listViewModel.setItemAnimation(newItemsIndex, action);
                }
                if (action === IObservable.ACTION_REMOVE) {
                    self._listViewModel.setItemAnimation(removedItemsIndex, action);
                }
                self._animatedAction = null;
            }
        }
    ...
    closePopup(self: BaseControl, itemActionsMenuId?: string): void {
        ...
        if (
            !itemActionsMenuId ||
            (self._itemActionsMenuId && self._itemActionsMenuId === itemActionsMenuId)
        ) {
    ...
    needBottomPadding(self: BaseControl, options: IItemActionsOptions): boolean {
        ...
        const results =
            typeof listViewModel.getResults === 'function' ? listViewModel.getResults() : false;
    ...
    getMarkerController(self: BaseControl, options: IList = self._options): MarkerController {
        if (!_private.hasMarkerController(self)) {
        ...
        if (
            ...
            if (newMarkedKey === controller.getMarkedKey()) {
                ...
                const shouldScrollToEdge =
                    (self._hasMoreData(loadDirection) || self._hasItemsOutRange[edgeDirection]) &&
                    _private.isInfinityNavigation(self._options.navigation);
                if (shouldScrollToEdge) {
                    self._listVirtualScrollController.scrollToEdge(edgeDirection);
    ...
    addShowActionsClass(self: BaseControl, options: IList): void {
        ...
        if (
            ...
            (!TouchDetect.getInstance().isTouch() || options.itemActionsVisibility === 'visible')
        ) {
    ...
    implements IMovableList, IEditableList
{
    ...
    protected _beforeMount(
        ...
        this._processAnimationContext(newOptions);
    ...
    protected _processAnimationContext(options: TOptions): void {
        if (options.animationId) {
            const animationContext = this.context;
            this._getLocation = (position: number): ClientRect => {
                const container = (this._getItemsContainer().children[
                    position
    ...
    private _getListScrollContextValue(
        ...
        if (context?.setArrowButtonClickCallback) {
            context.setArrowButtonClickCallback(
                this._arrowButtonClickCallback
            );
        }
    }
    ...
    private _dataLoadStartedCallback(
        ...
        if (isReload || !this._indicatorsController.hasDisplayedIndicator() && !this._wasPreloadedDataToBackward) {
    ...
    private _preloadDataToBackward(): void {
        ...
        const loadPromise = _private.loadToDirectionIfNeed(this, 'up');
        if (loadPromise) {
            loadPromise.finally(() => this._wasPreloadedDataToBackward = false);
        }
    }
    ...
    protected _onCollectionChangedScroll(
        ...
        const params = {
            ...
            scrolledToForwardEdge:
                this._viewportHeight + this._scrollTop >= this._contentHeight,
            newItemsIndex,
        ...
        switch (action) {
            ...
            case IObservable.ACTION_ADD:
                const calcMode = getCalcMode({
                    ...
                    renderPreloadedItems: this.props._renderPreloadedItemsRightAway,
                });
    ...
    protected _getTrackedValueOffset() {
        const offset = 15 + getStickyHeadersHeight(this._container, 'top', 'allFixed');
        return offset;
    }
    ...
    __onPagingArrowClick(
        ...
        switch (arrow) {
            ...
            case 'Begin':
                ...
                break;
            case 'End':
                const resultEventEnd = this._options.notifyCallback(
                    'pagingArrowClick',
                    ['End'],
                    { bubbling: true }
                );
    ...
    _canScroll(scrollTop: number, direction: string): boolean {
        ...
        return !(
            (direction === 'down' &&
                scrollTop - placeholder + this._viewportHeight >
                    this._contentHeight) ||
    ...
    protected _onItemClick(
        ...
        if (
            !this._items ||
            !this._listViewModel.getSourceItemByKey(getKey(contents))
        ) {
            event.stopPropagation();
        ...
        const stickyHeadersSize =
            this._container && this._scrollTop !== 0
                ? getStickyHeadersHeight(this._container, 'top', 'allFixed') ||
                  0
                : 0;
        const viewportSize = !!this._viewportHeight
    ...
    private _hasHiddenItemsByVirtualScroll(direction: 'up' | 'down'): boolean {
        ...
        // Если предзагрузили данные вверх и мы их не должны сразу же отрисовать,
        // то говорим что нет записей скрытых виртуальным скроллом,
        // чтобы индикатор вверх не скрылся.
        if (
            compatibleDirection === 'backward' &&
            !this.props._renderPreloadedItemsRightAway &&
    ...
    static contextType: typeof AnimationContext = AnimationContext;
