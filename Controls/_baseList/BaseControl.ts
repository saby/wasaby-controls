...
const _private = {
    ...
    handleListScrollSync(self: BaseControl, scrollTop: number): void {
        ...
        if (self._options.trackedProperties && isRealScroll && !self._scrolledToEdge) {
        ...
        if (
            ...
            if (newMarkedKey === controller.getMarkedKey()) {
                ...
                const scrollToItem = (key) => {
                    const index = self._listViewModel.getIndexByKey(key);
                    // У первой записи может быть марджин, поэтому нужно скроллить к началу списка, чтобы тень скрылась.
                    if (index === 0 && !self._hasMoreData('up')) {
                        return self._scrollToElement(self._container, 'top', false);
                    }

                    const position = isMovingForward ? 'bottom' : 'top';
                    return _private.scrollToItem(self, key, position, false);
                };
    ...
    implements IMovableList, IEditableList
{
    ...
    private _selectedItemsShown: boolean = false;
    ...
    private _dataLoadCallback(
        ...
        if (isReload) {
            ...
            if (this._options.trackedProperties && shouldResetScroll) {
    ...
    private _callVirtualRangeChangedCallback(
        ...
        if (startIndex !== undefined) {
            ...
            virtualRangeChangedCallback(
                this._getItemsFromRange(
                    startIndex,
                    Math.min(endIndex, this._listViewModel.getCount())
                )
            );
        }
    }
    ...
    protected _updateTrackedValues(): void {
        const trackedValuesVisible = this._scrollTop > 0 || this._hasMoreData('up');
    ...
    _observeScrollHandler(
        ...
        switch (eventName) {
            case 'scrollMove':
                ...
                if (this._options.trackedProperties && this._scrollTop !== params.scrollTop) {
    ...
    _getNavigationButtonClasses(
        ...
        if (options.itemActionsPosition === 'outside') {
            classes += ' controls-itemActionsV_outside-spacing';
        }
        return classes;
    }
