...
import ScrollPagingController, {
    IPagingCfg,
} from 'Controls/_baseList/Controllers/ScrollPaging';
...
import { IDirection, IVirtualScrollConfig } from './interface/IVirtualScroll';
import {
...
import { getColumnsWidths } from 'Controls/_baseList/resources/utils/resizer';
import {
...
import type { IAbstractListVirtualScrollControllerOptions } from './Controllers/ScrollController/AbstractListVirtualScrollController';
import {
    default as WorkByKeyboardContext,
    IWorkByKeyboardOptions,
} from 'Controls/WorkByKeyboard/Context';
import { activate, IFocusChangedConfig } from 'UICore/Focus';
import * as React from 'react';
import TrackedPropertiesTemplate from 'Controls/_baseList/TrackedPropertiesTemplate';
...
const NOT_EDITABLE_JS_SELECTOR: TNotEditableJsSelector =
...
const LIST_MEASURABLE_CONTAINER_SELECTOR =
...
const _private = {
    ...
    isNewModelItemsChange: (action, newItems) => {
        return (
            action && (action !== 'ch' || (newItems && !newItems.properties))
        );
    },
    ...
    assignItemsToModel(
        ...
        self._initialPosition = self._listViewModel
            ?.getSourceItemBySourceIndex(0)
            ?.getKey();
    },
    ...
    callDataLoadCallbackCompatibility(
        ...
        if (self._sourceController && options.dataLoadCallback) {
            const sourceControllerDataLoadCallback =
                self._sourceController.getState().dataLoadCallback;
    ...
    validateSourceControllerOptions(
        self: BaseControl,
        options: IBaseControlOptions
    ): void {
        const sourceControllerState = self._sourceController.getState();
        const validateIfOptionsIsSetOnBothControls = (
            optionName: string
        ): void => {
            if (
                ...
                Logger.warn(
                    `BaseControl: для корректной работы опцию ${optionName} необходимо задавать на Layout/browser:Browser (Controls/list:DataContainer)`,
                    self
                );
            }
        };
        ...
        const optionsToValidateOnBoth = [
            'source',
            'navigation',
            'sorting',
            'root',
        ];
        const optionsToValidateOnlyOnList = [
            'source',
            'navigation',
            'sorting',
            'dataLoadCallback',
    ...
    scrollToItem(
        ...
        if (allowLoad) {
            const hasItemInRecordset =
                self._listViewModel.getSourceItemByKey(key);
            ...
            return new Promise((resolveScroll) => {
                const currentSourceConfig =
                    self._sourceController.getNavigation()
    ...
    enterHandler(
        ...
        if (_private.hasMarkerController(self)) {
            ...
            if (markedKey !== null && markedKey !== undefined) {
                const markedItem =
                    self._listViewModel.getItemBySourceKey(markedKey);
                const contents = markedItem.getContents();
                const markedItemIndex =
                    self._listViewModel.getIndex(markedItem);
    ...
    keyDownDown(self: BaseControl, event: SyntheticEvent): void {
        _private.moveMarkerToDirection(self, event, 'Down');
    },

    keyDownUp(self: BaseControl, event: SyntheticEvent): void {
    ...
    keyDownDel(self: BaseControl, event: SyntheticEvent<MouseEvent>): void {
        ...
        const toggledItem: CollectionItem<Model> =
            model.getItemBySourceKey(toggledItemId);
        if (!toggledItem) {
            return;
        }
        ...
        if (itemActions) {
            const deleteAction = itemActions.all.find(
                (itemAction: IItemAction) => {
                    return itemAction.id === DELETE_ACTION_KEY;
                }
            );
            if (deleteAction) {
                _private.handleItemActionClick(
                    self,
                    deleteAction,
                    event,
                    toggledItem,
    ...
    shouldDrawCut(
        ...
        return (
            ...
            (hasMoreData ||
                (items && items.getCount() > navigation.sourceConfig.pageSize))
        );
    },
    ...
    prepareFooter(
        ...
        if (
            _private.isDemandNavigation(options.navigation) &&
    ...
    loadToDirection(
        ...
        if (self._sourceController) {
                ...
                if (!self._indicatorsController.isDisplayedPortionedSearch()) {
                    if (direction === 'up') {
                        ...
                        if (portionedSearchDirection === 'down') {
                            ...
                            self._indicatorsController.continueDisplayPortionedSearch(
                                'top'
                            );
                        } else {
            ...
            return self
                ._loadItemsToDirection(direction)
                .addCallback((addedItems: RecordSet) => {
                    if (self._destroyed) {
                        return;
                    }

                    _private.prepareFooter(
                        self,
                        self._options,
                    ...
                    if (
                        _private.hasMarkerController(self) &&
                        _private.isPortionedLoad(self)
                    ) {
                        const newMarkedKey = _private
                            .getMarkerController(self)
                            .onCollectionReset();
        ...
        Logger.error(
            "BaseControl: Source option is undefined. Can't load data",
            self
        );
    },
    ...
    getUpdatedMetaData(
        ...
        if (
            ...
            navigation.sourceConfig.direction !== 'both'
        ) {
    ...
    isMaxCountNavigationConfiguredCorrect(
        ...
        return (
            ...
            typeof navigation.viewConfig.maxCountValue === 'number'
        );
    },

    isItemsCountLessThenMaxCount(
        itemsCount: number,
        navigationMaxCount: number
    ): boolean {
    ...
    loadToDirectionIfNeed(
        ...
        if (self._shouldLoadItemsToDirection(direction)) {
            ...
            return _private.loadToDirection(self, direction);
        }
        return Promise.resolve();
    },

    scrollToEdgeOnAdd(self: BaseControl, direction: IDirection): Promise<void> {
        ...
        const scrollToItem = () => {
            if (!self._listViewModel.getCount()) {
                ...
                const lastItem = self._listViewModel.at(
                    self._listViewModel.getCount() - 1
                ).key;
    ...
    reloadToEdge(self: BaseControl, direction: IDirection): Promise<void> {
        if (self._hasMoreData(direction)) {
            ...
            if (
                ...
                const page =
                    direction === 'up' ? 0 : Math.ceil(metaMore / pageSize) - 1;
                ...
                if (
                    page - neededPagesCount <= neededPagesCount &&
                    direction === 'down'
                ) {
                    neededPage = 0;
            ...
            return self
                ._reload(self._options, navigationQueryConfig)
                .catch((error) => {
                    return error;
                });
        } else {
    ...
    scrollToEdge(self: BaseControl, direction: IDirection): Promise<void> {
        let scrollToEdgePromiseResolver;
        ...
        if (
            ...
            _private
                ...
                .then(() => {
                    self._shouldNotResetPagingCache = false;
                    self._listVirtualScrollController.setScrollBehaviourOnReset(
                        'keep'
                    );
                    ...
                    if (!self._sourceController?.getLoadError()) {
                        if (direction === 'up') {
                            ...
                            self._scrollPagingCtr.shiftToEdge(
                                direction,
                                hasMoreData
                            );
                        } else {
    ...
    scrollPage(self: BaseControl, direction: IDirection): void {
        const directionCompatibility =
            direction === 'Up' ? 'backward' : 'forward';
        self._listVirtualScrollController
            .scrollToPage(directionCompatibility)
            .then((key) => {
                self._setMarkedKeyAfterPaging(key);
            });
    },
    ...
    calcViewSize(
        ...
        pagingPadding: number
    ): number {
        return viewSize - (pagingVisible ? pagingPadding : 0);
    },
    needShowPagingByScrollSize(
        self: BaseControl,
        viewSize: number,
        viewportSize: number
    ): boolean {
        ...
        if (
            ...
            result &&
            _private.needScrollPaging(
                self._options.navigation,
                self._options.isAdaptive
            )
        ) {
    ...
    onScrollShow(self: BaseControl): void {
        _private.doAfterUpdate(self, () => {
            ...
            if (
                ...
                _private.getScrollPagingControllerWithCallback(
                    self,
                    (scrollPagingCtr) => {
    ...
    getScrollPagingControllerWithCallback(
        self: BaseControl,
        callback: Function
    ): void {
        if (self._scrollPagingCtr) {
    ...
    createScrollPagingController(
        ...
        const scrollPagingConfig = {
            ...
            resetButtonMode:
                self._options.navigation.viewConfig.resetButtonMode,
            totalElementsCount: elementsCount,
            loadedElementsCount:
                self._listViewModel.getStopIndex() -
                self._listViewModel.getStartIndex(),
            pagingCfgTrigger: (cfg) => {
                if (
                    cfg?.selectedPage !== self._currentPage &&
                    !self._selectedPageHasChanged
                ) {
    ...
    handleListScroll(self: BaseControl): void {
        if (
            _private.hasHoverFreezeController(self) &&
    ...
    initPaging(self: BaseControl): void {
        if (
            !(
                self._editInPlaceController &&
                self._editInPlaceController.isEditing()
            ) &&
            _private.needScrollPaging(
                self._options.navigation,
                self._options.isAdaptive
            ...
            if (self._viewportSize) {
                ...
                const placeholdersSize =
                    self._placeholders?.backward +
                        self._placeholders?.forward || 0;
                ...
                const callback = () => {
                    self._pagingVisible = isPagingVisible;
    ...
    handleListScrollSync(self: BaseControl, scrollTop: number): void {
        ...
        if (
            !self._scrollControllerInitializeChangeScroll ||
            self._scrolledToEdge
        ) {
            self._scrolled = true;
        }
        ...
        if (self._options.trackedProperties) {
            self._listViewModel.setTrackedValuesVisible(
                self._scrollTop > 0 || self._hasMoreData('up')
            );
        }
    ...
    disablePagingNextButtons(self: BaseControl): void {
        if (self._pagingVisible) {
            self._pagingCfg = { ...self._pagingCfg };
            self._pagingCfg.arrowState.next = self._pagingCfg.arrowState.end =
                'readonly';
        }
    },
    ...
    allowLoadMoreByPortionedSearch(
        ...
        const portionedSearchDirection =
            self._indicatorsController.getPortionedSearchDirection();
        return (
            (!portionedSearchDirection ||
                portionedSearchDirection !== direction) &&
            self._indicatorsController.shouldContinueDisplayPortionedSearch()
        );
    },
    ...
    updateShadowMode(
        self: BaseControl,
        shadowVisibility: { up: boolean; down: boolean }
    ): void {
        const itemsCount =
            self._listViewModel && self._listViewModel.getCount();
        const hasMoreData = (direction: IDirection) => {
        ...
        const showShadowUpByPortionedSearch =
            _private.allowLoadMoreByPortionedSearch(self, 'up');
        const showShadowDownByPortionedSearch =
            _private.allowLoadMoreByPortionedSearch(self, 'down');
     ...
     */
    closeItemActionsMenuForActiveItem(
        self: BaseControl,
        items: CollectionItem<Model>[]
    ): void {
    ...
    onCollectionChanged(
        ...
        const newModelChanged = _private.isNewModelItemsChange(
            action,
            newItems
        );
        ...
        if (changesType === 'collectionChanged' || newModelChanged) {
            ...
            if (
                ...
                if (self._itemActionsMenuId) {
                    _private.closeItemActionsMenuForActiveItem(
                        self,
                        removedItems
                    );
                }
            ...
            if (self._hasSelectionController() || handleSelection) {
                ...
                switch (action) {
                    case IObservable.ACTION_ADD:
                        newSelection = selectionController.onCollectionAdd(
                            newItems,
                            newItemsIndex
                        );
                        self._options.onListSelectedKeysCountChanged?.(
                    ...
                    case IObservable.ACTION_RESET:
                        const entryPath =
                            self._listViewModel.getMetaData().ENTRY_PATH;
            ...
            if (_private.hasMarkerController(self) || handleMarker) {
                ...
                switch (action) {
                    case IObservable.ACTION_REMOVE:
                        newMarkedKey = markerController.onCollectionRemove(
                            removedItemsIndex,
                            removedItems
                        );
                        break;
                    case IObservable.ACTION_RESET:
                        ...
                        if (
                            !self._isSourceControllerLoadingNow(options) &&
                            options.markedKey !== undefined &&
                            options.markedKey !==
                    ...
                    case IObservable.ACTION_CHANGE:
                        ...
                        const isEndEditProcessing =
                            self._editInPlaceController &&
                            self._editInPlaceController.isEndEditProcessing();
        ...
        if (
            action === IObservable.ACTION_RESET &&
            self._itemActionsController?.isSwiped()
        ) {
    ...
    resolveItemContainer(
        ...
        if (item && isMenuClick) {
            return self._getItemContainerByKey(item.key);
        }
    ...
    handleItemActionClick(
        ...
        const itemContainer = _private.resolveItemContainer(
            self,
            item,
            clickEvent,
            isMenuClick
    ...
    openItemActionsMenu(
        ...
        return Sticky.openPopup(menuConfig).then((popupId) => {
            ...
            RegisterUtil(self, 'customscroll', self._scrollHandler.bind(self));
        });
    },
     ...
     * @private
     */
    closeActionsMenu(self: BaseControl, currentPopup?: unknown): void {
        if (self._itemActionsMenuId) {
            const itemActionsMenuId = self._itemActionsMenuId;
            _private.closePopup(
                self,
                currentPopup ? currentPopup.id : itemActionsMenuId
            );
    ...
    openContextMenu(
        ...
        if (!!self._itemActionsController) {
            const item =
                self._listViewModel.getItemBySourceKey(key) || itemData;
            const menuConfig = _private.getItemActionsMenuConfig(
                self,
                item,
                event,
                null,
     ...
     */
    closePopup(self: BaseControl, itemActionsMenuId?: string): void {
        ...
        if (
            ...
            (self._itemActionsMenuId &&
                self._itemActionsMenuId === itemActionsMenuId)
        ) {
            UnregisterUtil(self, 'customscroll');
    ...
    groupsExpandChangeHandler(
        ...
        self._options.notifyCallback(
            changes.changeType === 'expand'
                ? 'groupExpanded'
                : 'groupCollapsed',
            [changes.group],
            { bubbling: true }
        );
        self._options.notifyCallback('collapsedGroupsChanged', [
            changes.collapsedGroups,
        ]);
        _private.prepareFooter(self, self._options, self._sourceController);
        if (
            self._options.historyIdCollapsedGroups ||
            self._options.groupHistoryId
        ) {
    ...
    getSortingOnChange(currentSorting, propName: string) {
    ...
    needBottomPadding(
        ...
        const results =
            typeof listViewModel.getResults === 'function'
        ...
        return (
            ...
            ((!hasHiddenItemsDown && !hasMoreDown) ||
                !_private.isInfinityNavigation(options.navigation))
        );
    },
    ...
    isPagingNavigationVisible(
        ...
        if (
            ...
            return (
                hasMoreData > PAGING_MIN_ELEMENTS_COUNT || hasMoreData === true
            );
        }
    ...
    updatePagingData(self: BaseControl, hasMoreData: boolean, options): void {
        self._pagingCfg = {
            arrowState: {
                ...
                prev: 'visible',
                next: 'visible',
                end: options.navigation?.viewConfig?.showEndButton
                    ? 'visible'
                    : 'hidden',
            },
        };
        self._knownPagesCount = _private.calcPaging(
            self,
            hasMoreData,
    ...
    resetPagingNavigation(
        ...
        self._currentPageSize =
            (navigation &&
                ...
                navigation.sourceConfig.pageSize) ||
            1;
    ...
    closeEditingIfPageChanged(
        ...
        const oldSourceCfg =
            oldNavigation && oldNavigation.sourceConfig
                ? oldNavigation.sourceConfig
                : {};
        const newSourceCfg =
            newNavigation && newNavigation.sourceConfig
                ? newNavigation.sourceConfig
                : {};
    ...
    jumpToEnd(self: BaseControl): Promise<void> {
        ...
        return self._listVirtualScrollController
            .scrollToEdge('forward')
            .then((key) => {
                self._scrolled = true;
                _private.updateScrollPagingButtons(self, {
    ...
    moveMarkerToDirection(
        ...
        if (
            ...
            if (newMarkedKey === controller.getMarkedKey()) {
                ...
                const edgeDirection =
                    loadDirection === 'down' ? 'forward' : 'backward';
                const shouldScrollToEdge =
                    (self._hasMoreData(loadDirection) ||
                        self._hasItemsOutRange[edgeDirection]) &&
                    _private.isInfinityNavigation(self._options.navigation);
                if (shouldScrollToEdge) {
                    self._listVirtualScrollController.scrollToEdge(
                        edgeDirection
                    );
                    self._doAfterDrawItems = () => {
                        _private.moveMarkerToDirection(self, event, direction);
                    };
                }
            } else {
                ...
                if (result instanceof Promise) {
                     ...
                     */
                    result.then((key) => {
                        return _private.scrollToItem(self, key);
                    });
    ...
    resolveItemActionsOptions(
        ...
        return {
            ...
            itemActionsVisibility:
                this.getItemActionsVisibilityByStyle(options),
    ...
    updateItemActions(
        ...
        if (self._itemActionsController) {
            ...
            itemActionsController.updateOptions(
                _private.resolveItemActionsOptions(self, options)
            );
        } else {
    ...
    isValidDndItemsEntity(
        dragStartResult: ItemsEntity,
        dragItemKey: CrudEntityKey
    ): boolean {
        let isValid = true;
        if (
            ...
            Logger.error(
                'ItemsEntity в поле items должен содержать только ключи записей.',
                this
            );
            isValid = false;
        }
    ...
    startDragNDrop(
        ...
        if (
            ...
            self._dndListController = _private.createDndListController(
                self._listViewModel,
                draggableItem,
                self._options
            );
            const options =
                self._getSourceControllerOptionsForGetDraggedItems(selection);
            return self._dndListController
                .getDraggableKeys(selection, options)
                .then((items) => {
                    let dragStartResult = self._options.notifyCallback(
                        'customdragStart',
                        [items, draggableKey]
                    );
                    ...
                    if (
                        ...
                        !_private.isValidDndItemsEntity(
                            dragStartResult,
                            draggableKey
                        )
                    ) {
                    ...
                    if (dragStartResult) {
                        if (self._options.dragControlId) {
                            dragStartResult.dragControlId =
                                self._options.dragControlId;
                        }
                        ...
                        if (self._startEvent && self._startEvent.target) {
                            self._startEvent.target.classList.add(
    ...
    isDragStarted(
        ...
        return (
            Math.abs(offset.x) > DRAG_SHIFT_LIMIT ||
            Math.abs(offset.y) > DRAG_SHIFT_LIMIT
        );
    },
    ...
    onMove(self: BaseControl, nativeEvent: Event): void {
        if (self._startEvent) {
            const dragObject = self._getDragObject(
                nativeEvent,
                self._startEvent
            );
            if (
                (!self._dndListController ||
                    !self._dndListController.isDragging()) &&
            ...
            if (
                ...
                const moveOutsideList = !(
                    self._container[0] || self._container
                ).contains(nativeEvent.target);
                if (
                    moveOutsideList !== self._listViewModel.isDragOutsideList()
                ) {
    ...
    activateEditingRow(
        self: BaseControl,
        enableScrollToElementParam: boolean | 'vertical' = self._options
            .task1184306971
            ? 'vertical'
            : true
    ): void {
        ...
        const hasColumnScroll =
            self._isColumnScrollVisible || self._options.isColumnScrollVisible;
        ...
        const activator = () => {
            ...
            const editingItemContainer =
                self._listVirtualScrollController.getElement(editingItem.key);
    ...
    addShowActionsClass(self: BaseControl, options: IList): void {
        ...
        if (
            ...
            (!TouchDetect.getInstance().isTouch() ||
                options.itemActionsVisibility === 'visible')
        ) {
            self._addShowActionsClass = true;
            const classes = [
                SHOW_ACTIONS_CLASS,
                `${SHOW_ACTIONS_CLASS}_${_private.getItemActionsVisibility(
                    self,
                    options
                )}`,
            ];
    ...
    removeHoverEnabledClass(self: BaseControl): void {
        if (!self._destroyed) {
            ...
            _private.changeItemsContainerClasses(self, null, [
                HOVER_ENABLED_CLASS,
            ]);
        }
    },
    ...
    needHoverFreezeController(self: BaseControl): boolean {
        return (
            ...
            ((self._options.itemActions &&
                self._options.itemActions.length > 0) ||
    ...
    freezeHoveredItem(
        ...
        const itemSelector = `.${self._getViewUniqueClass()} .controls-ListView__itemV[item-key='${itemKey}']`;
        ...
        const htmlNodeIndex =
            [].indexOf.call(listContainer.children, itemNode) + 1;
        const hoveredContainers = HoverFreeze.getHoveredItemContainers(
            self._container,
            htmlNodeIndex,
            self._getViewUniqueClass(),
        ...
        const hoverBackgroundColor =
            getComputedStyle(lastContainer).backgroundColor;

        self._children.itemActionsOutsideStyle.innerHTML =
            HoverFreeze.getItemHoverFreezeStyles(
                self._getViewUniqueClass(),
                htmlNodeIndex,
    ...
    initHoverFreezeController(self: BaseControl): void {
        self._hoverFreezeController = new HoverFreeze({
            uniqueClass: self._getViewUniqueClass(),
            stylesContainer: self._children
                .itemActionsOutsideStyle as HTMLElement,
    ...
    updateWorkByKeyboardStatus(
        ...
        if (self._workByKeyboardContext) {
            self._workByKeyboard = self._workByKeyboardContext.status;
        }
    },
};
...
export default class BaseControl<
        TOptions extends IBaseControlOptions = IBaseControlOptions
    >
    ...
    implements IMovableList, IEditableList
{
    ...
    _needBottomPadding: boolean = false;
    _noDataBeforeReload: boolean = false;

    _bottomVisible: boolean = true;

    private _modelRecreated: boolean = false;
    ...
    _itemActionsMenuId: number = null;

    protected _markerController: MarkerController = null;
    protected _selectionController: SelectionController = null;

    protected _dndListController: DndController<IDragPosition<CollectionItem>> =
        null;
    ...
    protected _editInPlaceInputHelperPreloadCallParams: [
        keyof EditInPlaceInputHelper,
        unknown[]
    ];
    ...
    private _workByKeyboardContext: IWorkByKeyboardOptions;
    protected _getListScrollContextValueCallback: Function;
    protected _setWorkByKeyboardCallback: Function;

    private _savedItemClickArgs: [
        SyntheticEvent,
        Model,
        SyntheticEvent,
        number
    ...
    constructor(options: IBaseControlOptions, context: object) {
        ...
        this._dataLoadStartedCallback =
            this._dataLoadStartedCallback.bind(this);
        ...
        this._itemsContainerReadyHandler =
            this._itemsContainerReadyHandler.bind(this);
        ...
        this._hasHiddenItemsByVirtualScroll =
            this._hasHiddenItemsByVirtualScroll.bind(this);
        this._onCollectionChanged = this._onCollectionChanged.bind(this);
        this._onAfterCollectionChanged =
            this._onAfterCollectionChanged.bind(this);
        this._onIndexesChanged = this._onIndexesChanged.bind(this);
        this._onNextIndexesChanged = this._onNextIndexesChanged.bind(this);
        this._setMarkedKeyAfterPaging =
            this._setMarkedKeyAfterPaging.bind(this);
        this._onItemDeactivated = this._onItemDeactivated.bind(this);
        this._onItemActionsMenuClose = this._onItemActionsMenuClose.bind(this);
        this._onItemActionsMenuResult =
            this._onItemActionsMenuResult.bind(this);

        this._onItemActionsMouseEnter =
            this._onItemActionsMouseEnter.bind(this);
        ...
        this._onActionsSwipeAnimationEnd =
            this._onActionsSwipeAnimationEnd.bind(this);
        this._onNavigationButtonClick =
            this._onNavigationButtonClick.bind(this);
        ...
        const getCollectionItem = (item: Model) => {
            return this._listViewModel.getItemBySourceKey(item.getKey(), false);
        };
        this._itemHandlers = {
            onMouseOverCallback: (event, item) => {
                return this._itemMouseOver(
                    event,
            ...
            onContextMenuCallback: (event, item) => {
                return this._onItemContextMenu(
                    event,
                    getCollectionItem(item),
                    event
                );
    ...
    protected _beforeMount(
        ...
        this._getListScrollContextValueCallback =
            this._getListScrollContextValue.bind(this);
        this._setWorkByKeyboardCallback = this._setWorkByKeyboard.bind(this);
        ...
        if (newOptions.setStartDragNDropCallback) {
            newOptions.setStartDragNDropCallback(this._startDragNDropCallback);
        }

        if (newOptions.setCanStartDragNDropCallback) {
            newOptions.setCanStartDragNDropCallback((target: HTMLElement) => {
                return (
                    !target.closest('.controls-DragNDrop__notDraggable') &&
                    !!target.closest('.controls-GridReact__row')
                );
            });
        }
    ...
    // временный метод для установки значения для контекста
    // удалить после перевода на реакт
    protected _setWorkByKeyboard(workByKeyboard: IWorkByKeyboardOptions): void {
        this._workByKeyboardContext = workByKeyboard;
        if (workByKeyboard && this._workByKeyboard !== workByKeyboard?.status) {
            _private.updateWorkByKeyboardStatus(this);
        }
    }

    private _getListScrollContextValue(
        context: IListScrollContextOptions
    ): void {
        if (context?.setArrowButtonClickCallback) {
            context.setArrowButtonClickCallback(
                this._arrowButtonClickCallback.bind(this)
            );
        }
    }

    private _dataLoadStartedCallback(
        event: SyntheticEvent,
        direction: Direction
    ): void {
        ...
        if (isReload || !this._indicatorsController.hasDisplayedIndicator()) {
    ...
    private _dataLoadCallback(
        ...
        this._indicatorsController.setHasMoreData(
            hasMoreData.up,
            hasMoreData.down
        );
        ...
        if (isReload) {
            const shouldResetScroll =
                this._isMounted && !this._options.keepScrollAfterReload;
    ...
    private _itemsChangedOnDataLoad(
        ...
        // Перезагрузку списков инджикаторами нужно обрабатывать именно тут,
        ...
        if (
            loadedInRoot &&
            (!isIterativeLoading || isIterativeLoadingInProgress)
        ) {
            // Пытаемся грузить тут, т.к. isLoading=true в dataLoadCallback
            this._tryLoadToDirectionAgain(items, rootKey);
        }
    }
            ...
            .addOperation(() => {
                ...
                if (needInitModelState) {
                    if (
                        ...
                        const controller = _private.getMarkerController(
                            this,
                            newOptions
                        );
                        const markedKey =
                            controller.calculateMarkedKeyForVisible();
                        controller.setMarkedKey(markedKey);
                    }
                    if (newOptions.fadedKeys) {
                        const fadeController = _private.getFadeController(
                            this,
                            newOptions
                        );
    ...
    private _onAfterCollectionChanged(): void {
        if (
            ...
            this._removedItemsByOneSession.length
        ) {
            const removedItems = [].concat.apply(
                [],
                this._removedItemsByOneSession
            );
            const newSelection =
                this._getSelectionController().onCollectionRemove(removedItems);
    ...
    private _callVirtualRangeChangedCallback(
        ...
        if (startIndex !== undefined) {
            virtualRangeChangedCallback(
                this._getItemsFromRange(startIndex, endIndex)
            );
        }
    }
    ...
    private _onNextIndexesChanged(
        ...
        if (this._isMounted && shiftDirection) {
            ...
            this._drawingIndicatorDirection =
                shiftDirection === 'forward' ? 'bottom' : 'top';
    ...
    protected _prepareItemsOnMount(
        ...
        if (items) {
            ...
            self._needBottomPadding = _private.needBottomPadding(
                self,
                newOptions
            );
            if (self._pagingNavigation) {
    ...
    _initKeyProperty(options: TOptions): void {
        this._keyProperty =
            ...
            (this._sourceController &&
                this._sourceController.getKeyProperty()) ||
    ...
    freezeHoveredItem(item: Model): void {
        ...
        if (!collectionItem) {
            Logger.error(
                'freezeHoveredItem(). Указанный item отсутствует в коллекции.',
                this
            );
        } else {
    ...
    _updateShadowModeHandler(shadowVisibility: {
        down: boolean;
        up: boolean;
    }): void {
    ...
    protected _viewResize(): void {
        const container =
            (this._children.hasOwnProperty('viewContainer') &&
                this._children.viewContainer) ||
        ...
        if (
            this._contentHeight !== container.clientHeight ||
            this._contentWidth !== container.scrollWidth
        ) {
            this._onContentResized(
    ...
    protected _onContentResized(width: number, height: number): void {
        ...
        if (this._contentHeight !== height) {
            ...
            if (!this._options.task1187242805) {
                this._listVirtualScrollController.contentResized(
                    this._contentHeight
                );
            }
        }
        if (this._options.task1187242805) {
            this._listVirtualScrollController.contentResized(width);
        }
    }
    ...
    protected _viewportResizeHandler(
        ...
        if (this._viewportSize >= this._contentHeight) {
    ...
    _getScrollParams(clear: boolean = false): IScrollParams {
        ...
        if (detection.isBrowserEnv) {
            ...
            stickyElementsHeight +=
                getStickyHeadersHeight(this._container, 'bottom', 'allFixed') ||
                0;
        }
        ...
        const scrollParams = {
            scrollTop: this._scrollTop,
            scrollHeight:
                this._contentHeight + pagingPadding - stickyElementsHeight,
    ...
    protected _getListVirtualScrollControllerOptions(
        ...
        return {
            ...
            viewportSize: this._viewportSize || 0,
            ...
            beforeVirtualRangeChangeCallback: (range) => {
                if (this._options.beforeVirtualRangeChangeCallback) {
                    ...
                    return this._options.beforeVirtualRangeChangeCallback(
                        itemsFromRange
                    );
                }
                return;
            ...
            updateShadowsUtil: (hasItemsOutsideOfRange) => {
                ...
                const shadowVisibleByNavigation =
                    _private.needShowShadowByNavigation(navigation, itemsCount);
                const topShadowVisibleByPortionedSearch =
                    _private.allowLoadMoreByPortionedSearch(this, 'up');
            ...
            updateVirtualNavigationUtil: (hasItemsOutsideOfRange) => {
                this._updateVirtualNavigation(hasItemsOutsideOfRange);
            },
            ...
            activeElementChangedCallback: (activeElementIndex) => {
                ...
                if (activeItem && activeItem.ActivatableItem) {
                    this._options.notifyCallback('activeElementChanged', [
                        activeItem.key,
                    ]);
                }
            },
    ...
    private _createListVirtualScrollController(
        ...
        const controllerOptions =
            this._getListVirtualScrollControllerOptions(options);
        ...
        controllerOptions.collection = shouldInitItemsContainer
            ...
            : null;
        this._listVirtualScrollController = new VirtualScrollController(
            controllerOptions
        );
    }
    ...
    private _setMarkedKeyAfterPaging(key: CrudEntityKey): void {
        ...
        if (markerController.shouldMoveMarkerOnScrollPaging()) {
            const record = this._listViewModel.getSourceItemByKey(key);
    ...
    protected _onCollectionChangedScroll(
        ...
        const params = {
            ...
            scrolledToForwardEdge:
                this._viewportSize + this._scrollTop >= this._contentHeight,
            newItemsIndex,
        ...
        switch (action) {
            case IObservable.ACTION_RESET:
                ...
                this._listVirtualScrollController.setForwardTriggerVisibility(
                    true
                );
    ...
    protected _afterMount(): void {
        ...
        if (
            dragObject &&
            dragObject.entity &&
            dragObject.entity instanceof ItemsEntity
        ) {
        ...
        if (this._contentHeight <= this._viewportSize) {
    ...
    protected _updateBaseControlModel(newOptions: IBaseControlOptions): void {
        ...
        const emptyTemplateChanged =
            this._options.emptyTemplate !== newOptions.emptyTemplate;
        ...
        this._listViewModel.setRowSeparatorVisibility(
            newOptions.rowSeparatorVisibility
        );
        ...
        if (newOptions.collapsedGroups !== this._options.collapsedGroups) {
            GroupingController.setCollapsedGroups(
                this._listViewModel,
        ...
        if (newOptions.editingConfig !== this._options.editingConfig) {
            this._listViewModel.setEditingConfig(
                this._getEditingConfig(newOptions)
            );
        }
        ...
        this._listViewModel.setMultiSelectPosition(
            newOptions.multiSelectPosition
        );
        this._listViewModel.setMultiSelectAccessibilityProperty(
            newOptions.multiSelectAccessibilityProperty
        );
        this._listViewModel.setItemActionsPosition(
            newOptions.itemActionsPosition
        );
        ...
        if (
            !isEqual(
                this._options.trackedProperties,
                newOptions.trackedProperties
            )
        ) {
            this._listViewModel.setTrackedProperties(
                newOptions.trackedProperties
            );
        ...
        this._listViewModel.setHoverBackgroundStyle(
            newOptions.hoverBackgroundStyle
        );
    ...
    protected _updateTrackedValues(): void {
        const offset = 15;
        const edgeItemKey =
            this._listVirtualScrollController.getEdgeVisibleItem(
                'backward',
                offset
            )?.key;
    ...
    protected _keyDownLeft(
        ...
        if (canMoveMarker && !this._options.disableHorizontalMarkerDirection) {
    ...
    protected _beforeUpdate(
        ...
        const navigationChanged = !isEqual(
        ...
        if (!isEqual(newOptions._renderedItems, this._options._renderedItems)) {
            ...
            this._listVirtualScrollController.setRenderedItems(
                newOptions._renderedItems
            );
        }
        const isSourceControllerLoadingNow =
            this._isSourceControllerLoadingNow(newOptions);

        const sourceControllerChanged =
            this._options.sourceController !== newOptions.sourceController;
        const items =
            newOptions.sourceController?.getItems() || newOptions.items;
        ...
        if (
            ...
            this._initialPosition = this._listViewModel
                .getSourceItemBySourceIndex(0)
                ?.getKey();
        }

        if (this._options.activeElement !== newOptions.activeElement) {
            this._listVirtualScrollController.setActiveElementKey(
                newOptions.activeElement
            );
        }
        ...
        const shouldReInitCollection =
            ...
            (newOptions.viewModelConstructor &&
                newOptions.viewModelConstructor !==
                    this._options.viewModelConstructor) ||
        ...
        if (this._editInPlaceController) {
            const isEditingModeChanged =
                ...
                (this._options.editingConfig !== newOptions.editingConfig &&
                    this._getEditingConfig().mode !==
                        this._getEditingConfig(newOptions).mode);
        ...
        if (
            this._options.disableVirtualScroll !==
                newOptions.disableVirtualScroll ||
        ...
        if (shouldReInitCollection && this._listViewModel) {
            ...
            this._updateIndicatorsController(
                newOptions,
        ...
        if (newOptions.sourceController || newOptions.items) {
            ...
            if (
                ...
                if (
                    newOptions.items &&
                    newOptions.items !== this._items &&
                    !newOptions.source
                ) {
                    const scrollBehaviourOnReset =
                        newOptions.keepScrollAfterReload ? 'keep' : 'reset';
                    this._listVirtualScrollController.setScrollBehaviourOnReset(
            ...
            if (newOptions.sourceController) {
                if (sourceControllerChanged) {
                    _private.executeAfterReloadCallbacks(
                        this,
                        this._items,
                        newOptions
                    );
                ...
                if (
                    loadedBySourceController &&
                    !this._sourceController.getLoadError()
                ) {
                    if (this._listViewModel) {
        ...
        if (
            shouldProcessMarker &&
            (!isSourceControllerLoadingNow || _private.isPortionedLoad(this)) &&
            this._listViewModel &&
            this._listViewModel.getCount()
        ) {
            let needCalculateMarkedKey = false;
            if (
                !_private.hasMarkerController(this) &&
                newOptions.markerVisibility === 'visible'
            ) {
            ...
            const markerController = _private.getMarkerController(
                this,
                newOptions
            );
            ...
            if (
                ...
                (this._options.markerVisibility !==
                    newOptions.markerVisibility &&
                    newOptions.markerVisibility === 'visible') ||
                this._modelRecreated ||
                needCalculateMarkedKey
            ) {
                ...
                const newMarkedKey =
                    markerController.calculateMarkedKeyForVisible();
                this._changeMarkedKey(newMarkedKey);
            }
        } else if (
            _private.hasMarkerController(this) &&
            newOptions.markerVisibility === 'hidden'
        ) {
        ...
        if (this._items) {
            ...
            if (
                ...
                this._options.onListSelectedKeysCountChanged?.(
        ...
        if (!isSourceControllerLoadingNow) {
            ...
            if (
                !isEqual(
                    newOptions.groupHistoryId,
                    this._options.groupHistoryId
                )
            ) {
                if (this._listViewModel) {
        ...
        if (!isEqual(newOptions.itemActions, this._options.itemActions)) {
        ...
        if (
            newOptions.itemActionsPosition !==
                this._options.itemActionsPosition &&
        ...
        if (this._options.itemsSelector !== newOptions.itemsSelector) {
            this._listVirtualScrollController.setItemsQuerySelector(
                newOptions.itemsSelector
            );
        }
        ...
        if (
            ...
            if (this._editInPlaceController?.isEditing()) {
                itemToUpdateActions =
                    this._editInPlaceController._getEditingItem();
            }
            _private.updateInitializedItemActions(
                this,
                newOptions,
                itemToUpdateActions
            );
        ...
        // Нужен TileControl
        if (
            this._options.beforeItemsTemplate !== newOptions.beforeItemsTemplate
        ) {
            this._listViewModel.setBeforeItemsTemplate?.(
                newOptions.beforeItemsTemplate
            );
        }
        if (
            this._options.afterItemsTemplate !== newOptions.afterItemsTemplate
        ) {
            this._listViewModel.setAfterItemsTemplate?.(
                newOptions.afterItemsTemplate
            );
        }
    ...
    protected _afterReloadCallback(
        options: IBaseControlOptions,
        loadedList: RecordSet
    ): void {
        /* FIXME: empty */
    ...
    private _unmount(): void {
        ...
        if (this._hasSelectionController()) {
            this._options.onListSelectedKeysCountChanged?.(0, false);
            this._options.notifyCallback(
                'listSelectedKeysCountChanged',
                [0, false],
                { bubbling: true }
    ...
    protected _beforeRender(): void {
        ...
        // При наличии неотрисованной ромашки проверяем что список проскроллен.
    ...
    protected _afterRender(oldOptions: IBaseControlOptions): void {
        ...
        this._notifyOnDrawItems();
        if (this._wasReload) {
            if (this._options.trackedProperties) {
                this._updateTrackedValues();
            }
    ...
    protected _$react_componentDidUpdate(): void {
        ...
        if (shouldFixScrollByIndicatorHeight) {
            this._options.notifyCallback(
                'doScroll',
                [this._scrollTop - INDICATOR_HEIGHT],
                { bubbling: true }
            );
        }
    ...
    protected _afterUpdate(oldOptions: TOptions): void {
        ...
        if (
            this._itemActionsController &&
            this._itemActionsController.isSwiped()
        ) {
    ...
    __onPagingArrowClick(
        e: Event,
        arrow: 'Next' | 'Prev' | 'Begin' | 'End' | 'Reset'
    ): void {
        ...
        switch (arrow) {
            ...
            case 'Prev':
                ...
                break;
            case 'Begin':
                const resultEvent = this._options.notifyCallback(
                    'pagingArrowClick',
                    ['Begin'],
                    { bubbling: true }
                );
    ...
    protected _scrollToInitialPosition(): Promise<unknown> {
        const hasInitialItem = this._listViewModel.getSourceItemByKey(
            this._initialPosition
        );
        const canScrollToItem =
            hasInitialItem &&
            this._listVirtualScrollController.hasEnoughContentToScrollToItem(
        ...
        if (canScrollToItem) {
            ...
            return this.scrollToItem(this._initialPosition, 'top', true);
        } else {
    ...
    _canScroll(scrollTop: number, direction: string): boolean {
        ...
        return !(
            (direction === 'down' &&
                scrollTop - placeholder + this._viewportSize >
                    this._contentHeight) ||
    ...
    _hasEnoughData(page: number): boolean {
        const neededItemsCount =
            this._scrollPagingCtr.getNeededItemsCountForPage(page);
    ...
    __selectedPageChanged(e: Event, pageOption: number): void {
        let page = pageOption;
        let scrollTop = this._scrollPagingCtr.getScrollTopByPage(
            page,
            this._getScrollParams()
        );
        ...
        this._applySelectedPage = () => {
            ...
            scrollTop = this._scrollPagingCtr.getScrollTopByPage(
                page,
                this._getScrollParams()
            );
    ...
    __needShowEmptyTemplate(newOptions?: IBaseControlOptions): boolean {
        ...
        const isLoading =
            this._sourceController && this._sourceController.isLoading();
    ...
    _onCheckBoxClick(
        ...
        if (!readOnly) {
            ...
            if (
                ...
                originalEvent.nativeEvent.shiftKey
            ) {
            ...
            this._options.notifyCallback('checkboxClick', [
                key,
                item.isSelected(),
            ]);
            this._options.notifyCallback('selectedLimitChanged', [
                selectionController.getLimit(),
            ]);
            this._changeSelection(newSelection);
        }
    ...
    reload(
        ...
        if (keepNavigation) {
            ...
            if (!sourceConfig) {
                ...
                if (
                    !isMultiNavigation &&
                    this._options.navigation?.source === 'page'
                ) {
                    const navPageSize =
                        this._options.navigation.sourceConfig.pageSize;
                ...
                if (
                    ...
                    this._options.navigation.sourceConfig.position !==
                        sourceConfig.position
                ) {
                    this._listVirtualScrollController.setScrollBehaviourOnReset(
                        'restore'
                    );
    ...
    protected _changeMarkedKey(
        ...
        if (
            (newMarkedKey === undefined ||
                newMarkedKey === markerController.getMarkedKey()) &&
            !shouldFireEvent
        ) {
            return newMarkedKey;
        }
        ...
        const eventResult: Promise<CrudEntityKey> | CrudEntityKey = this
            ._isMounted
            ? this._options.notifyCallback('beforeMarkedKeyChanged', [
                  newMarkedKey,
              ])
            : undefined;
        ...
        const handleResult = (key) => {
            ...
            // если список не замаунчен, то не нужно кидать события, события кинем после маунта
            if (this._isMounted) {
                this._options.onMarkedKeyChanged?.(key);
                this._options.notifyCallback('markedKeyChanged', [key]);
            }
        };
        ...
        if (eventResult instanceof Promise) {
            ...
            (this._environment || this.UNSAFE_isReact)
        ) {
    ...
    protected _shouldLoadItemsToDirection(direction: IDirection): boolean {
        ...
        const allowLoadBySource =
            sourceController && hasMoreData && !sourceController.isLoading();
        const allowLoadBySearch =
            !_private.isPortionedLoad(this) ||
            this._indicatorsController.shouldContinueDisplayPortionedSearch(
                direction
            );
        ...
        const allowLoadByDrag = !(
            ...
            this._selectionController?.isAllSelected()
        );
    ...
    private _shouldLoadByLoadedItems(items?: RecordSet): boolean {
        ...
        return (
            !items || items.getCount() === 0 || this._loadedItemsIsHidden(items)
        );
    }
    ...
    private _commitEditInGroupBeforeCollapse(groupItem): TAsyncOperationResult {
        ...
        const groupId = this.getViewModel().getGroup()(
            editingItem.getContents()
        );
    ...
    protected _onItemClick(
        ...
        if (
            !this._items ||
            !this._listViewModel.getSourceItemByKey(contents.getKey())
        ) {
            event.stopPropagation();
        ...
        const canEditByClick =
            !this._options.readOnly &&
            this._getEditingConfig().editOnClick &&
            !originalEvent.target.closest(`.${NOT_EDITABLE_JS_SELECTOR}`);
        ...
        const editingTarget = originalEvent.target.closest(
            '.js-controls-ListView__editingTarget'
        );

        if (canEditByClick && editingTarget) {
            event.stopPropagation();
            this._savedItemClickArgs = [
                event,
                contents,
                originalEvent,
                columnIndex,
            ...
            this._beginEdit(
                { item: contents },
                { columnIndex: columnIndex + checkboxOffset }
            ).then((result) => {
                if (!(result && result.canceled)) {
            ...
            if (
                this.isEditing() &&
                !this._editInPlaceController.isTargetEditing(
                    contents,
                    columnIndex
                )
            ) {
            ...
            const eventResult = this._notifyItemClick(
                event,
                contents,
                originalEvent,
                columnIndex
    ...
    protected _notifyItemClick(
        ...
        return this._options.onItemClick
            ? this._options.onItemClick(contents, originalEvent, columnIndex)
            : this._options.notifyCallback(
                  'itemClick',
                  [contents, originalEvent, columnIndex],
                  { bubbling: true }
              );
    }
     ...
     * @private
     */
    protected _onItemActionsMenuClose(currentPopup): void {
    ...
    _createEditInPlaceControllerAsync(
        options: IBaseControlOptions = this._options
    ): Promise<void> {
        return Promise.resolve()
            .then(() => {
                if (this._editInPlaceLib) {
                    return this._editInPlaceLib;
                } else {
                    return (
                        import(
                            'Controls/editInPlace'
                        ) as unknown as Promise<IEditInPlaceLibPublicExports>
                    ).then((lib) => {
                        this._editInPlaceLib = lib;
                        return this._editInPlaceLib;
                    });
                }
            })
    ...
    _createEditInPlaceController(
        options: IBaseControlOptions = this._options,
        ControllerConstructor: typeof EditInPlaceController,
        HelperConstructor: typeof EditInPlaceInputHelper
    ): void {
        if (!this._editInPlaceInputHelper) {
            this._editInPlaceInputHelper = new HelperConstructor();

            if (this._editInPlaceInputHelperPreloadCallParams) {
                const methodName =
                    this._editInPlaceInputHelperPreloadCallParams[0];
                const methodArgs =
                    this._editInPlaceInputHelperPreloadCallParams[1];
        ...
        if (!this._editInPlaceController) {
            this._editInPlaceController = new ControllerConstructor({
        ...
        return new Promise((resolve) => {
            ...
            .then((result) => {
                if (result === LIST_EDITING_CONSTANTS.CANCEL) {
                    if (this._continuationEditingDirection) {
                        ...
                        if (this._savedItemClickArgs && this._isMounted) {
                            ...
                            // элемента.
                            this._notifyItemActivate.apply(
                                this,
                                this._savedItemClickArgs
                            );
                        }
                ...
                if (sourceController && !(addedItem instanceof Model)) {
                    const listMeta = !this._isMounted
    ...
    _afterBeginEditCallback(
        ...
        return new Promise((resolve) => {
            ...
            .then(() => {
                // Редактирование может запуститься при построении.
                if (this._isMounted) {
                    this._options.notifyCallback('afterBeginEdit', [
                        item.contents,
                        isAdd,
                    ]);
                ...
                if (this._isMounted) {
                    ...
                    if (forwardEdgeItem) {
                        ...
                        const forwardEdgeItemIndex =
                            this._listViewModel.getIndexByKey(
                                forwardEdgeItemKey
                            );
                        const itemIndex = this._listViewModel.getIndexByKey(
                            item.key
                        );
                        if (
                            itemIndex >= forwardEdgeItemIndex &&
                            forwardEdgeItemIndex !== -1
                        ) {
                            position = 'center';
                        }
                    }
    ...
    _beforeEndEditCallback(
        params: IBeforeEndEditCallbackParams
    ): Promise<void> {
        if (params.force) {
    ...
    private _validateEditing(): Promise<
        typeof LIST_EDITING_CONSTANTS.CANCEL | void
    > {
            ...
            : this._initValidationContrillerAsync().then(() => {
                  ...
                  return submitPromise.then((validationResult) => {
                      for (const key in validationResult) {
                          if (
                              validationResult.hasOwnProperty(key) &&
    ...
    beginAdd(userOptions?: IItemAddOptions): TAsyncOperationResult {
        ...
        return this._beginAdd(userOptions, {
            addPosition:
                userOptions?.addPosition ||
                this._getEditingConfig().addPosition,
    ...
    _tryStartInitialEditing(
        options: IBaseControlOptions
    ): TAsyncOperationResult {
        const editingConfig: Required<IEditingConfig> =
            this._getEditingConfig(options);
        const hasItems = !!(
        ...
        if (
            ...
            return getCreateControllerPromise().then(() => {
                if (
                    this._listViewModel &&
                    this._listViewModel.getSourceItemByKey(
                        editingConfig.item.getKey()
                    )
                ) {
    ...
    private _onEditingRowKeyDown(
        ...
        const editNext = (
            ...
            return this._beginEdit(
                { item },
                { shouldActivateInput, columnIndex }
            ).then((result) => {
    ...
    private _editingRowEnterHandler(): Promise<void | { canceled: true }> {
        const editingConfig = this._getEditingConfig();
        const columnIndex =
            this._editInPlaceController._getEditingItem()._$editingColumnIndex;
        const next = this._editInPlaceController.getNextEditableItem();
        const shouldEdit =
            editingConfig.sequentialEditingMode !== 'none' && !!next;
        const shouldAdd =
            !next &&
            !shouldEdit &&
    ...
    private _onEditingCellTabHandler(
        ...
        if (eventOptions.isShiftKey) {
            ...
            if (
                columnIndex >
                this._options.columns.length - 1 + checkboxOffset
            ) {
    ...
    _getEditingConfig(
        options: IBaseControlOptions = this._options
    ): Required<IEditingConfig> {
        const editingConfig = options.editingConfig || {};
        const addPosition =
            editingConfig.addPosition === 'top' ? 'top' : 'bottom';
        ...
        const getSequentialEditingMode = () => {
            ...
            if (typeof editingConfig.sequentialEditingMode === 'string') {
                ...
                return editingConfig.sequentialEditing !== false
                    ? 'row'
                    : 'none';
            }
        };
        ...
        return {
            ...
            autoAddByApplyButton:
                editingConfig.autoAddByApplyButton === false
                    ? false
                    : !!(
                          editingConfig.autoAddByApplyButton ||
                          editingConfig.autoAdd
                      ),
    ...
    onactivated(
        ...
        if (
            ...
            if (
                ...
                this._canMarkItemOnActivate()
            ) {
                const item = eventOptions.isShiftKey
                    ? this.getViewModel().getLast()
                    : this.getViewModel().getFirst();
     ...
     * @private
     */
    private _canMarkItemOnActivate(): boolean {
        const model = this.getViewModel();
        return (
            this._workByKeyboard &&
            this._options.markerVisibility === 'onactivated' &&
            !!model?.getCount() &&
            !_private.isEditing(this) &&
            model.getIndexByKey(
                _private.getMarkerController(this).getMarkedKey()
            ) === -1
        );
    }
    ...
    protected _onViewMouseDown(e: SyntheticEvent): void {
        /* Not implemented for plain list */
    }

    protected _onViewMouseMove(e: SyntheticEvent): void {
        /* Not implemented for plain list */
    }

    protected _onViewMouseUp(e: SyntheticEvent): void {
        /* Not implemented for plain list */
    }
    ...
    protected _hasHorizontalScroll(options: IBaseControlOptions): boolean {
        return false;
    }
    ...
    protected _itemMouseDown(
        ...
        if (this._options.onItemMouseDown) {
            this._options.onItemMouseDown(itemData.item, domEvent.nativeEvent);
        } else {
            this._options.notifyCallback('itemMouseDown', [
                itemData.item,
    ...
    protected _itemMouseUp(
        ...
        if (this._options._needSetMarkerCallback) {
            canBeMarked =
                canBeMarked &&
                this._options._needSetMarkerCallback(itemData.item, domEvent);
        }
        ...
        if (this._mouseDownItemKey === key) {
            if (
                ...
                const url = itemData.item.get(this._options.urlProperty);
                const isLinkClick = domEvent.target
                    .closest('a')
                    ?.getAttribute('href');
                if (url && !isLinkClick) {
                    window.open(url);
                    this._onLastMouseUpWasOpenUrl =
                        domEvent.nativeEvent.button === 0;
                }
            }
        ...
        this._onLastMouseUpWasDrag = !!domEvent.target.closest(
            '.controls-ListView__item_dragging'
        );
        this._options.notifyCallback('itemMouseUp', [
            itemData.item,
            domEvent.nativeEvent,
        ]);
    ...
    private _toggleCutClick(): void {
        const result = this._options.notifyCallback(
            'cutClick',
            [this._cutExpanded],
            { bubbling: true }
        );
    ...
    moveItems(
        ...
        const target = this._listViewModel
            .getSourceCollection()
            .getRecordById(targetKey);
    ...
    moveItemUp(
        ...
        return this._moveItemToDirection(
            selectedKey,
    ...
    moveItemsWithDialog(
        selection: ISelectionObject,
        viewCommandName: VIEW_COMMAND = VIEW_COMMAND.PARTIAL_RELOAD
    ): Promise<DataSet> {
        const moveDialogOptions = this._options.moveDialogTemplate;
        if (moveDialogOptions && !moveDialogOptions.templateName) {
            Logger.error(
                'BaseControl: Wrong type of moveDialogTemplate option',
                this
            );
            return;
        }
        ...
        const popupOptions: Partial<IMoveDialogOptions> = {
            ...
            templateOptions: {
                ...
                keyProperty:
                    moveDialogOptions.templateOptions.keyProperty ??
    ...
    private _moveItemToDirection(
        ...
        const target =
            position === LOCAL_MOVE_POSITION.Before
                ? this._getSiblingsStrategy().getPrevByKey(selectedKey)
                : this._getSiblingsStrategy().getNextByKey(selectedKey);
        return this._callAction<void>(
            ...
            viewCommandName,
            {
                ...
                siblingsStrategy: this._getSiblingsStrategy(),
            },
            {
                selection,
                target,
    ...
    protected _onViewKeyDown(event: SyntheticEvent<KeyboardEvent>): void {
        ...
        if (itemContainer) {
            const editingItem =
                this._editInPlaceController &&
                this._editInPlaceController.getEditableItem();
            if (editingItem) {
    ...
    protected _getItemContainerByKey(itemKey: CrudEntityKey): HTMLElement {
        const itemsSelector =
            this._options.itemsSelector || '.controls-ListView__itemV';
        const uniqueItemSelector =
            `.${this._getItemsContainerUniqueClass()} > ` +
            `${itemsSelector}[item-key='${itemKey}']`;
        return this._getItemsContainer().querySelector(
            uniqueItemSelector
        ) as HTMLElement;
    }
    ...
    protected _itemMouseEnter(
        ...
        if (itemData.SupportItemActions) {
            ...
            const itemIndex = this._listViewModel.getIndex(
                itemData.dispItem || itemData
            );
        ...
        this._options.notifyCallback('itemMouseEnter', [
            itemData.item,
            nativeEvent,
        ]);
    }
    ...
    protected _itemMouseMove(
        ...
        if (
            ...
            (!this._editInPlaceController ||
                !this._editInPlaceController.isEditing()) &&
            !this._itemActionsMenuId &&
            (!hoverFreezeController ||
                hoverFreezeController.getCurrentItemKey() === null)
        ) {
    ...
    protected _draggingItemMouseMove(
        targetItem: CollectionItem,
        event: MouseEvent
    ): boolean {
        const targetIsDraggableItem =
            this._dndListController.getDraggableItem()?.getContents() ===
            targetItem.getContents();
        ...
        if (
            dragPosition &&
            !isEqual(this._dndListController.getDragPosition(), dragPosition)
        ) {
            const changeDragTarget = this._options.notifyCallback(
                'changeDragTarget',
                [
    ...
    protected _itemMouseLeave(
        ...
        this._options.notifyCallback('itemMouseLeave', [
            item.item,
            nativeEvent,
        ]);
        if (
    ...
    _onSortingChanged(sortingButton: HTMLElement): void {
        ...
        this._options.onSortingChanged?.(newSorting);
    ...
    _updatePagingPadding(): void {
        ...
        if (!this._pagingPadding && this._children.pagingPaddingContainer) {
            this._pagingPadding =
                this._children.pagingPaddingContainer.offsetHeight;
        }
    }
    ...
    protected _mouseOver(event): void {
        ...
        this._indicatorsController?.onMouseEnter();
    }
    ...
    _updateItemActionsOnItem(
        ...
        if (
            this._itemActionsController?.isActionsAssigned() &&
            !this._itemsChanged
        ) {
    ...
    _onItemSwipeAnimationEnd(e: SyntheticEvent<IAnimationEvent>): void {
        if (
            this._hasSelectionController() &&
            e.nativeEvent.animationName === 'rightSwipe'
        ) {
    ...
    protected _createNewModel(
        ...
        return diCreate(modelName, {
            ...
            emptyTemplateOptions: {
                ...modelConfig.emptyTemplateOptions,
                items,
            ...
            // знать не должна
            moreButtonVisibility: _private.isInfinityNavigation(
                modelConfig.navigation
            )
                ? MoreButtonVisibility.exceptLastNode
                : MoreButtonVisibility.visible,
            portionedSearchTemplate: modelConfig.iterativeLoadingTemplate,
            columnScroll: !!(modelConfig.isFullGridSupport
                ? !modelConfig.newColumnScroll && modelConfig.columnScroll
                : modelConfig.columnScroll || modelConfig.newColumnScroll),
            newColumnScroll: !!(
                modelConfig.newColumnScroll && modelConfig.isFullGridSupport
            ),
            resizerOffsetCallback: this._onResizerOffsetChanged.bind(this),
            multiSelectVisibility: this._resolveMultiSelectVisibility(
                modelConfig.multiSelectVisibility
            ),
            ...this._getModelOptions(modelConfig as unknown as TOptions),
        });
    }

    protected _getModelOptions(
        baseControlOptions: TOptions
    ): Partial<ICollectionOptions> {
        return {};
    }
     ...
     * @private
     */
    private _resolveMultiSelectVisibility(
        multiSelectVisibility: string
    ): string {
        let result = multiSelectVisibility;
        const hasSelected =
            this._selectionController?.getSelection()?.selected?.length;
        if (
            !hasSelected &&
            detection.isMobilePlatform &&
    ...
    protected _itemsContainerReadyHandler(
        ...
        this._listVirtualScrollController.setItemsContainer(
            this._getItemsContainer()
        );
    }
        ...
        if (
            ...
            switch (eventName) {
                case 'virtualScrollMove':
                    ...
                    if (this._viewportSize < this._contentHeight) {
                        _private.throttledVirtualScrollPositionChanged(
                            this,
                            params
                        );
                    }
    ...
    private _getIndicatorsControllerOptions(
        ...
        if (
            ...
            (options.navigation.source === 'position' ||
                options.navigation.source === 'page')
        ) {
            pageSize =
                options.navigation.source === 'position'
        ...
        return {
            model: this._listViewModel,
            items: this._items,
            isInfinityNavigation: _private.isInfinityNavigation(
                options.navigation
            ),
            isMaxCountNavigation: _private.isMaxCountNavigation(
                options.navigation
            ),
    ...
    private _updateViewportFilledInIndicatorsController(): void {
        ...
        if (!this._viewportSize || !this._contentHeight) {
            ...
            viewportFilled =
                this._contentHeight - portionedSearchIndicatorsHeight >
                this._viewportSize;
        }
    ...
    private _countGlobalIndicatorPosition(): number {
        const stickyHeadersSize =
            this._container && this._scrollTop !== 0
                ? getStickyHeadersHeight(this._container, 'top', 'allFixed') ||
                  0
                : 0;
        const viewportSize = !!this._viewportSize
            ? this._viewportSize - stickyHeadersSize
            : this._contentHeight;
        return Math.max(
            this._scrollTop + viewportSize / 2 - INDICATOR_HEIGHT / 2,
            0
        );
    }
    ...
    protected _displayGlobalIndicator(): void {
        if (this._indicatorsController.shouldDisplayGlobalIndicator()) {
            this._indicatorsController.displayGlobalIndicator(
                this._countGlobalIndicatorPosition()
            );
        }
    }
    ...
    private _hasHiddenItemsByVirtualScroll(direction: 'up' | 'down'): boolean {
        ...
        return (
            this._hasItemsOutRange &&
            this._hasItemsOutRange[compatibleDirection]
        );
    }

    private _scrollToFirstItemAfterDisplayTopIndicator(
        onDrawItems: boolean = false
    ): void {
        const scrollAndShowTrigger = () => {
            if (this._scrollToItemAfterReloadParams) {
                const { key, position, force, resolveScroll } =
                    this._scrollToItemAfterReloadParams;
                this._scrollToItemAfterReloadParams = null;
                this._listVirtualScrollController
                    .scrollToItem(key, position, force)
                    .then(() => {
                        this._listVirtualScrollController.setBackwardTriggerVisibility(
                            true
                        );
    ...
    private _scrollToFirstItem(): Promise<void> {
        ...
        const firstItemKey =
            firstItem && firstItem.key !== undefined ? firstItem.key : null;
        if (
            firstItemKey !== null &&
    ...
    _doAfterPagingModuleLoaded(cb: () => void, options): void {
        ...
        if (
            options.navigation?.viewConfig?.totalInfo &&
            !isLoaded('Controls/dropdown')
        ) {
            pagingDeps.push(import('Controls/dropdown'));
        }

        if (pagingDeps.length) {
    ...
    private _dragLeave(): void {
        ...
        if (
            ...
            if (this._dndListController.isDragStartedFromAnotherList()) {
                ...
                const newPosition =
                    this._dndListController.calculateDragPosition({
                        targetItem: null,
        ...
        const hasSorting =
            this._options.sorting && this._options.sorting.length;
        if (!hasSorting) {
    ...
    private _dragEnter(dragObject: IDragObject): void {
        ...
        if (!hasSorting) {
            if (
                this._documentDragging &&
                !this._shouldDisplayDraggingTemplate()
            ) {
                this._options.notifyCallback('_removeDraggingTemplate', [], {
                    bubbling: true,
                });
        ...
        if (this._documentDragging) {
            if (
                ...
                const dragEnterResult = this._options.notifyCallback(
                    'customdragEnter',
                    [dragObject.entity]
                );
    ...
    private _documentDragEnd(dragObject: IDragObject): void {
        ...
        if (this._insideDragging && this._dndListController) {
            ...
            const changeDragTarget =
                this._dndListController.getChangeDragTargetResult();
            if (
                targetPosition &&
        ...
        if (this._dndListController) {
            if (
                this._dndListController.getDragEndPromise() instanceof Promise
            ) {
                this._displayGlobalIndicator();
    ...
    private _dragNDropEnded(event: SyntheticEvent): void {
        ...
        if (this._dndListController && this._dndListController.isDragging()) {
            const dragObject = this._getDragObject(
                event.nativeEvent,
                this._startEvent
            );
    ...
    private _calculateMouseOffsetInItem(event: MouseEvent): {
        ...
        if (targetElement) {
            ...
            const mouseCoords = DimensionsMeasurer.getMouseCoordsByMouseEvent(
                event.nativeEvent
            );
            ...
            if (this._listViewModel['[Controls/_tile/Tile]']) {
                result.top =
                    (mouseCoords.x - dragTargetRect.left) /
                    dragTargetRect.width;
                result.bottom =
                    (dragTargetRect.right - mouseCoords.x) /
                    dragTargetRect.width;
            } else {
                result.top =
                    (mouseCoords.y - dragTargetRect.top) /
    ...
    private _getDndTargetRow(event: MouseEvent): Element {
        ...
        const condition = () => {
            ...
            if (this._listViewModel['[Controls/_tile/Tile]']) {
                ...
                return !target.parentNode.classList.contains(
                    'controls-ListView__itemV'
                );
            }
        };
    ...
    private _registerMouseMove(): void {
        this._options.notifyCallback(
            'register',
            ['mousemove', this, this._onMouseMove],
            { bubbling: true }
        );
        this._options.notifyCallback(
            'register',
            ['touchmove', this, this._onTouchMove],
            { bubbling: true }
        );
    }
    ...
    private _registerMouseUp(): void {
        this._options.notifyCallback(
            'register',
            ['mouseup', this, this._onMouseUp],
            { bubbling: true }
        );
        this._options.notifyCallback(
            'register',
            ['touchend', this, this._onMouseUp],
            { bubbling: true }
        );
    }
    ...
    protected _createSelectionStrategy(
        options: IBaseControlOptions
    ): ISelectionStrategy {
    ...
    protected _getSelectionControllerOptions(
        ...
        return {
            ...
            isLoadedUtil: (key) => {
                return (
                    !this._sourceController ||
                    this._sourceController.hasLoaded(key)
                );
            },
            ...
            feature1188519278: options.feature1188519278,
        };
    }

    private _onSelectedTypeChanged(
        typeName: string,
    ...
    private _notifySelection(selection: ISelectionObject): void {
        ...
        if (excludedDiff.added.length || excludedDiff.removed.length) {
            this._options.onExcludedKeysChanged?.(
                excludedDiff.keys,
                excludedDiff.added,
        ...
        this._options.notifyCallback(
            'listSelectionTypeForAllSelectedChanged',
            [selectionType],
            { bubbling: true }
        );
    }
    ...
    private _changeSelection(
        ...
        const controller = this._getSelectionController();
        const selectionDifference =
            controller.getSelectionDifference(newSelection);
        let result = this._options.notifyCallback('beforeSelectionChanged', [
            selectionDifference,
    ...
    _getFooterSpacingClasses(options: IBaseControlOptions): string {
        ...
        if (shouldAddMultiSelectSpace) {
            ...
            paddingClassName +=
                options.itemPadding?.left?.toLowerCase() || 'default';
        }
    ...
    _getNavigationButtonClasses(
        ...
        const isSeparatorButton =
            this._resolveNavigationButtonView() === 'separator';
    ...
    setStoredColumnsWidths(columnsWidths: string[]): void {
        if (!isEqual(columnsWidths, this._storedColumnsWidths)) {
            ...
            this._listViewModel.setColumnsWidths(columnsWidths);
    ...
    _getBaseControlClasses(): string {
        const theme = this._options.theme;
        let classes = `controls-BaseControl controls_list_theme-${theme} controls_toggle_theme-${theme}`;

        if (
            this.__needShowEmptyTemplate() &&
            !this._options.emptyTemplateColumns
        ) {
            classes += ' controls-BaseButton__emptyTemplate__wrapper';
        }

        if (this._options.isFullGridSupport && this._options.newColumnScroll) {
            classes += ' controls-GridControl_withHorizontalScroll';
        }
    ...
    static getDefaultOptions(): Partial<IBaseControlOptions> {
        return {
            ...
            loadingIndicatorTemplate:
                'Controls/baseList:LoadingIndicatorTemplate',
            iterativeLoadingTemplate:
                'Controls/baseList:IterativeLoadingTemplate',
            ...
            urlProperty: 'url',
            itemsSpacing: null,
            trackedPropertiesTemplate: TrackedPropertiesTemplate,
