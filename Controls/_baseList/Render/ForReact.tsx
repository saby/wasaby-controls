...
import { getItemActionsTemplate } from 'Controls/listVisualAspects';
...
function ItemTemplateWrapper(props: IItemTemplateWrapperProps): React.ReactElement {
    ...
    const collectionItemContextValue: ICollectionItemContextValue = React.useMemo(
        () => ({
            item: itemProps.item,
            itemContents: Array.isArray(itemProps.item?.contents)
                ? itemProps.item.contents[itemProps.item.contents.length - 1]
                : itemProps.item?.contents,
        }),
        [itemProps.item, itemProps.item?.contents]
    );

    return (
        <CollectionItemContext.Provider value={collectionItemContextValue}>
            {React.isValidElement(props.itemTemplate)
                ? cloneElement(props.itemTemplate, itemProps, attrs)
                : createElement(props.itemTemplate, itemProps, attrs)}
