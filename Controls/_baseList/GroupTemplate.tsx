...
function ContentComponent(
    props: IGroupContentProps & IStickyBlockContentProps
) {
    // Модель группы и модель записи могут быть разными моделями
    const group = props.group;
    const item = props.item;
    const displayText = props.textVisible !== false;
    const hasRightTemplate =
        !!props.rightTemplate && props.rightTemplateCondition !== false;
    ...
    if (hasRightTemplate) {
        ...
        if (
            ...
            rightTemplateClasses +=
                ' tw-flex-grow controls-ListView__groupContent_right';
        }
...
export default React.memo((props: IGroupTemplateProps) => {
    ...
    const className =
        ...
        (clearProps.className || userAttrs.className || '');
    const attrs = {
