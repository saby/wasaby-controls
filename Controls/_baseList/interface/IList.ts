 ...
 * @variant placeholder Рассчитываемое значение, добавляющее отступ, чтобы прижать последнюю запись списка к его верху
 * @variant basic Отступ для индикатора и пэйджинга (при их наличии), отступ для опций записи "на ухе" отключен
 * @variant actions Отступ, добавляемый в зависимости от наличия пэйджинга, индикатора и опций записи "на ухе"
 * @variant additional Отступ в 40px под реестром, выводится в случае если указан данный режим и опции записи находятся не на ухе
 */
...
/**
 * Опция, которая решает проблему возникновения разрыва над прилипающем заголовком на масштабах и safary. См {@link /doc/platform/developmentapl/interface-development/debug/scroll-container/#1px Отладка ошибок в скролл-контейнере и фиксированных заголовках}
...
/**
 * Опция, выводящая под реестром отступ в 40px, если в списке не выведена панель пейджинга, опции записи выводятся не на ухе и отсутствует футер/строка результатов внизу
 * @name  Controls/_list/interface/IList#bottomPaddingMode
 * @cfg {Controls/_list/interface/IList/TBottomPaddingMode.typedef}
 * @default actions
 * @demo Controls-demo/list_new/ShowAfterItemsOffset/WI/Index
 */
