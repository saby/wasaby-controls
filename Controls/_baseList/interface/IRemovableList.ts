...
export interface IRemovableList {
     ...
     * @param {Boolean} resetSelectionOnSuccess Флаг, отражающий необходимость сброса множественного выбора. По умолчанию true
     ...
     * @param {Controls/interface:ISelectionObject} selection Объект, содержащий список ключей записей для удаления и список ключей записей, которые необходимо исключить при удалении.
     * @param {Controls/list:IListActionAdditionalConfig} config Конфигурация, необходимая для открытия диалогового окна.  В качестве команды для выполнения действия с RecordSet по умолчанию используется Controls/viewCommands:AtomicRemove
     * @remark Если удаление состоялось, возвращаемый Promise содержит строку 'fullReload'
     * @returns {Promise<String | void>}
     */
