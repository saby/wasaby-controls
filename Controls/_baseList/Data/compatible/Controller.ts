...
function isOneListInside(config: TFactoryConfigs): boolean {
...
> {
    ...
    protected _isOneListInside: boolean;
    protected _children: {
    ...
    protected _beforeMount(options: IDataControllerOptions): Promise<TConfigLoadResult> | void {
        ...
        if (needLoad(options, this) || changedOptions.length) {
            return Loader.load(configs).then((results) => {
                ...
                this._isOneListInside = isOneListInside(configs);
            });
        } else {
            this._configs = configs;
    ...
    protected _beforeUnmount(): void {
        ...
        if (needLoad(this._options, this) && this._loadResults) {
            Object.values(this._loadResults).forEach((loadResult) => {
                loadResult.sourceController?.destroy();
            });
        }
