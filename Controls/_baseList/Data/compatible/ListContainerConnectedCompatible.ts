...
import type { IBrowserOptions } from 'Controls/browser';
...
interface IConnectedCompatibleContainerOptions extends IControlOptions, IBrowserOptions {
    ...
    isBrowser?: boolean;
    listOldScheme?: boolean;
    isInsideDataContainer?: boolean;
    insideList?: boolean;
}

interface IContextState extends IListState {
    listStoreId?: string;
    isInsideDataContainer?: boolean;
}
...
const STATES_FOR_CONTENT = [
    'filter',
    // Навигация участвует в формировании запроса, так же как и фильтр.
    // Прикладник может поменять фильтр и навигацию одновременно в ходе исполнения bas, поэтому за ее изменениями тоже надо следить.
    'navigation',
    'searchValue',
    'searchInputValue',
    'searchMisspellValue',
...
const ORIGINAL_ON_START_UPDATE_METHOD_NAME = Symbol('AbstractListSlice:_startUpdate');
...
// При выключении видимости маркера, маркер должен слетать в null.
function validateMarker(prevState: IListState, nextState: IListState): IListState {
    if (
        prevState.markedKey !== nextState.markedKey &&
        nextState.markedKey !== null &&
        nextState.markerVisibility === 'hidden'
    ) {
        nextState.markedKey = prevState.markedKey;
    }
    return nextState;
}
    ...
    implements INewListSchemeHandlers
{
    ...
    protected _contextState: IContextState;
    ...
    protected _beforeMount(options: IConnectedCompatibleContainerOptions): void {
        ...
        this._bindHandlersForListNewScheme(options);

        if (options.searchValueForHighlight && options.storeId === DATA_SYNTHETIC_STORE_ID) {
            this._searchValue = options.searchValueForHighlight;
        }

        const invalidOptions = options.sourceController
            ? checkOptionsOnMount(options, this._slice.state)
            : [];
        ...
        Как поправить ошибку: `;
            ...
            // а иначе это Browser
            if (options.storeId === DATA_SYNTHETIC_STORE_ID) {
    ...
    private _getNotifyArgs<T extends keyof IListState>(propName: T, newValue: IListState[T]) {
        if (propName === 'selectedKeys' || propName === 'excludedKeys') {
            const diff = ArraySimpleValuesUtil.getArrayDifference(
                this['_' + propName] || [],
                newValue || []
            );
            return [newValue, diff.added, diff.removed];
        } else {
            return [newValue];
        }
    }
        ...
        STATES_FOR_NOTIFY.forEach((stateName) => {
            ...
            if (
                ...
                !(
                    ...
                    options.storeId === DATA_SYNTHETIC_STORE_ID
                )
            ) {
                this._notify(stateName + 'Changed', this._getNotifyArgs(stateName, sliceValue));
                needRerender = true;
            }
        });
        return needRerender;
        ...
        return STATES_FOR_CONTENT.reduce(
            (state, propName) => {
                if (
                    ...
                    // searchValue: null сбрасывается в том случае, если опцией должен управлять только слайс
                    // (прикладник не контролирует), поэтому проставлять его в слайс не надо
                    !(propName === 'searchValue' && options[propName] === null)
                ) {
    ...
    protected _initSlice(options: IConnectedCompatibleContainerOptions): void {
        ...
        this._contextState = {
            ...this._slice.state,
            isInsideDataContainer: options.isInsideDataContainer,
            listStoreId: options.listOldScheme ? undefined : options.storeId,
        };
        ...
        if (!localSlice[LCCC_INITED_FLAG]) {
            ...
            localSlice[ORIGINAL_ON_START_UPDATE_METHOD_NAME] =
                this._slice._startUpdate.bind(localSlice);
        ...
        this._slice._startUpdate = async (initialState) => {
            if (localSlice.isDestroyed()) {
                // при разрушенном слайсе ничего делать не надо
                return;
            }
            const stateAfterActionToDispatch =
                await localSlice[ORIGINAL_ON_START_UPDATE_METHOD_NAME](initialState);
            if (this._unmounted) {
                // если LCCC не отображается на странице, тогда необходимо выполнять чистую логику слайса
                return stateAfterActionToDispatch;
            }
            ...
            const needRerender = this._notifyAboutNewSliceState(
                stateAfterActionToDispatch,
                this._options
            );
            const promise =
            ...
            return promise.then(() => {
                ...
                let stateFromOptions = this._getNewStateByOptions(
                    stateAfterActionToDispatch,
                    this._options
                );
        ...
        this._slice._setState = (newState) => {
            // валидация опций
            newState = validateMarker(this._slice.state, newState);

            if (
                this._slice.state.expandedItems !== newState.expandedItems &&
                newState.expandedItems
            ) {
                newState.expandedItems = [...newState.expandedItems];
            }
            ...
            this._contextState = {
                ...newState,
                listStoreId: this._options.listOldScheme ? undefined : this._options.storeId,
            };
        ...
        if (sliceState.items) {
            if (options.sourceController && !options.sourceController.getDataLoadCallback()) {
                options.dataLoadCallback?.(sliceState.items);
            }
            options.itemsReadyCallback?.(sliceState.items);
        }
    ...
    protected _setOptionToSliceAndNotify(
        ...
        if (this._options.isInsideDataContainer) {
            return;
        }
        if (!isEqual(this._slice.state[optionName], args[0])) {
    ...
    protected _bindHandlersForListNewScheme(options: IConnectedCompatibleContainerOptions) {
        const h = getHandlersNew({
            storeId: options.storeId,
            slice: this._slice,
            changeRootByItemClick: options.changeRootByItemClick,
            context: options._dataOptionsValue,
        });
    ...
    static defaultProps: Partial<IConnectedCompatibleContainerOptions> = {
