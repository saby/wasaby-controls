...
function isEqualWithoutOrder(a1?: unknown[], a2?: unknown[]): boolean {
    ...
    implements INewListSchemeHandlers
{
    ...
    protected _beforeUpdate(options: IConnectedCompatibleContainerOptions): void {
        ...
        if (!sliceChanged) {
            ...
            // Для случаев, когда люди НЕ биндят опцию, а задают ее жестко.
            // Они расчитывают, что список ее никогда не сможет изменить.
            // https://online.sbis.ru/opendoc.html?guid=d2f5b2bc-0739-49cb-b049-1f8109bda1d5&client=3
            if (options.fix1191459314) {
                if (
                    typeof options.expandedItems !== 'undefined' &&
                    isEqualWithoutOrder(this._options.expandedItems, options.expandedItems) &&
                    !isEqualWithoutOrder(slice.state.expandedItems, this._options.expandedItems)
                ) {
                    slice.setState({
            ...
            if (getNamesOfChangedOptions(this._options, options).length && !slice.state.loading) {
