...
const ORIGINAL_START_BAS_METHOD_NAME = Symbol('_$startBeforeApplyState');
...
function validateMarker(prevState: IListState, nextState: IListState): IListState {
    if (prevState.markedKey !== nextState.markedKey && nextState.markerVisibility === 'hidden') {
    ...
    implements INewListSchemeHandlers
{
    ...
    protected _initSlice(options: IConnectedCompatibleContainerOptions): void {
        ...
        if (!localSlice[LCCC_INITED_FLAG]) {
            ...
            localSlice[ORIGINAL_START_BAS_METHOD_NAME] =
                this._slice._startBeforeApplyState.bind(localSlice);
        ...
        this._slice._startBeforeApplyState = async (initialState) => {
            if (localSlice.isDestroyed()) {
                // при разрушенном слайсе ничего делать не надо
                return;
            }
            const stateAfterActionToDispatch =
                await localSlice[ORIGINAL_START_BAS_METHOD_NAME](initialState);
            if (this._unmounted) {
