...
const onMouseEnterHandler = () => {
    if (!isLoaded('Controls/listWebReducers')) {
        loadAsync(LibPaths.ListWebReducers);
    }
};
...
const Content = React.forwardRef<unknown>(function (
    ...
    return React.cloneElement(props.innerContent, {
        ...
        groupProperty: listState.groupProperty || innerProps.groupProperty,
        selectAncestors: listState.selectAncestors || innerProps.selectAncestors,
        selectDescendants: listState.selectDescendants || innerProps.selectDescendants,
        imageProperty: listState.imageProperty || innerProps.imageProperty,
...
const ListContainerConnected = React.forwardRef(function (
    ...
    React.useEffect(() => {
        if (!(slice instanceof ListSlice)) {
            getError('WRONG_SLICE', LibPaths.CurrentList);
        }
        slice.connect();
        return () => {
            slice.disconnect();
