...
import { type ScrollControllerLib, hooks, UILogic } from 'Controls/listsCommonLogic';
...
export function getHandlersNew(props: IGetHandlersProps): INewListSchemeHandlers {
    ...
    return {
        ...
        onViewTriggerVisibilityChanged: renderHandlers.onViewTriggerVisibilityChanged,

        onExpanderClick(itemKey: TKey, params): void {
            renderHandlers.onExpanderClick(
                {},
                slice.collection.getItemBySourceKey(itemKey, false),
                params
            );
        },
        toggleExpansion(itemKey: TKey): void {
            renderHandlers.onExpanderClick({}, slice.collection.getItemBySourceKey(itemKey, false));
        },
        onItemKeyDownEnterNew(e): void {
            renderHandlers.onItemKeyDownEnter(
                e,
                slice.collection.getItemBySourceKey(slice.state.markedKey)
            );
        },
        ...
        isExpanded(itemKey: TKey): boolean {
            return UILogic.Hierarchy.isExpanded(slice.state, itemKey);
        },
        isExpandAll(): boolean {
            return UILogic.Hierarchy.isExpandAll(slice.state);
        },
