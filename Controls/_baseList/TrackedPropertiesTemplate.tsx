...
import { StickyContext, getStickyHeadersHeight } from 'Controls/stickyBlock';

import {
    ITrackedPropertiesTemplateProps,
    useWatchTrackedValues,
    InnerTrackedTmpl,
    ITrackedPropertiesContext,
    TrackedPropertiesContext,
} from 'Controls/listVisualAspects';

export { ITrackedPropertiesContext, TrackedPropertiesContext };

interface ITrackedValuesProps {
    trackedValues: Record<string, unknown>;
}

export { ITrackedPropertiesTemplateProps };
...
const Content = React.memo(function TrackedPropertiesContentComponent({
    ...
    return (
        <div className={classes} style={{ marginTop: props.marginTop + 'px' }}>
            {backgroundStyle === 'contrast' && <div className={trapezeClass}></div>}
...
export default function TrackedPropertiesComponentWrapper(
    ...
    return (
        <InnerTrackedTmpl
            ...
            stickyBlockContextClasses={`controls-ListView__item-leftPadding_${
...
export const TrackedPropertiesComponent = React.memo(function TrackedPropertiesComponent({
    ...
    const afterContentTemplateContextClasses =
        React.useContext(TrackedPropertiesContext).afterContentTemplateContextClasses;

    // Завязываемся на обновление стики контекста, чтобы актуализировать отступ.
    React.useContext(StickyContext);
    ...
    return (
        <>
            ...
            {props.position === 'right' ? (
                ...
                <div className={`${afterContentTemplateContextClasses}`}></div>
            )}
        </>
    );
});
 ...
 * @public
 * @implements Controls/_list/TrackedPropertiesTemplate/ITrackedPropertiesTemplateProps
