...
> extends FlatSelectionStrategy<TItem> {
    ...
    select(selection: ISelection, key: CrudEntityKey): ISelection {
        ...
        if (item && this._isNode(item)) {
            ...
            this._selectLeaf(
                cloneSelection,
                key,
                this._getKey(item?.getParent() as TItem)
            );
        }
    ...
    unselect(
        ...
        if (
            ...
            this._unselectParentNodes(
                cloneSelection,
                item.getParent() as TItem
            );
        }
        if (
            searchMode &&
    ...
    selectAll(selection: ISelection, limit?: number): ISelection {
        ...
        if (!newSelection.excluded.includes(this._rootKey)) {
            newSelection.excluded = ArraySimpleValuesUtil.addSubArray(
                newSelection.excluded,
                [this._rootKey]
            );
        }
    ...
    toggleAll(selection: ISelection, hasMoreData: boolean): ISelection {
        ...
        if (this._rootChanged) {
            this._removeIdsNotFromCurrentRoot(
                cloneSelection,
                childrenIdsInRoot
            );
        }
        ...
        if (this._isAllSelected(selection, this._rootKey)) {
            ...
            if (hasMoreData) {
                oldSelectedKeys.forEach((key) => {
                    return (cloneSelection = this.unselect(
                        cloneSelection,
                        key
                    ));
                });
        ...
        ArraySimpleValuesUtil.addSubArray(
            cloneSelection.excluded,
            ArraySimpleValuesUtil.getIntersection(
                childrenIdsInRoot,
                oldSelectedKeys
            )
        );
    ...
    selectRange(items: TItem[]): ISelection {
        ...
        items.forEach((elem) => {
            if (
                elem.SelectableItem &&
                (!elem.isNode() || (elem.isNode() && !elem.isExpanded()))
            ) {
    ...
    getSelectionForModel(
        ...
        if (searchMode) {
            ...
            doNotSelectNodes =
                this._isAllSelected(selectionWithEntryPath, this._rootKey) &&
                !isOnlyNodesInItems;
        }
    ...
    private _getItemSelected(
        ...
        if (item['[Controls/_baseTree/BreadcrumbsItem]']) {
            ...
            const parentIsSelected = this._getBreadcrumbsSelected(
                parent,
                selection
            );
            ...
            if (!this._selectAncestors && !this._selectDescendants) {
                ...
                isSelected =
                    ...
                    (inSelected ||
                        (hasSelectedParent &&
                            (this._selectDescendants || isAllSelected)));

                if ((this._selectAncestors || searchMode) && isNode) {
                    isSelected = this._getStateNode(
                        item,
                        isSelected,
                        selection,
                        searchMode
    ...
    private _getBreadcrumbsSelected(
        ...
        if (this._isAllSelectedInRoot(selection)) {
            ...
            isSelected =
                hasSelected &&
                (!hasExcluded || selectedKeyIndex < excludedKeyIndex);
    ...
    isAllSelected(
        ...
        if (limit) {
            ...
            const selectionForModel = this.getSelectionForModel(
                selection,
                limit
            );
            const selectedCount =
                selectionForModel.get(true).length +
                selectionForModel.get(null).length;
    ...
    private _unselectParentNodes(selection: ISelection, item: TItem): void {
        ...
        while (
            ...
            allChildrenExcluded = this._isAllChildrenExcluded(
                selection,
                currentParent
            );
        }
    ...
    private _unselectAllInRoot(selection: ISelection): ISelection {
        ...
        if (rootInExcluded) {
            resSelection.excluded = ArraySimpleValuesUtil.removeSubArray(
                resSelection.excluded,
                [this._rootKey]
            );
        }
    ...
    private _selectNode(selection: ISelection, node: TItem): void {
        const nodeKey = this._getKey(node);
        this._selectLeaf(
            selection,
            nodeKey,
            this._getKey(node.getParent() as TItem)
        );

        if (
            selection.selected.includes(nodeKey) &&
            this._isNewNodeSelectionBehavior()
        ) {
    ...
    private _unselectNode(selection: ISelection, node: TItem): void {
    ...
    private _unselectLeaf(selection: ISelection, item: TItem): void {
        ...
        if (
            (!itemInSelected && isSelectedParent) ||
        ...
        if (
            ...
            ArraySimpleValuesUtil.removeSubArray(selection.selected, [
                parentId,
            ]);
        }

        if (item['[Controls/_baseTree/BreadcrumbsItem]']) {
            this._unselectBreadcrumb(
                selection,
    ...
    private _unselectBreadcrumb(
        ...
        if (selection.selected.includes(breadcrumbKey)) {
            ...
            ArraySimpleValuesUtil.addSubArray(selection.excluded, [
                breadcrumbKey,
            ]);
        }
    }
    ...
    private _mergeEntryPath(selectedKeys: TKeys): TKeys {
        ...
        if (this._entryPath) {
            ...
            this._entryPath.forEach((it) => {
                ...
                const parentIsSelected = this._parentFromEntryPathIsSelected(
                    it.id,
                    selectedKeys
                );
                if (parentIsSelected) {
    ...
    private _parentFromEntryPathIsSelected(
        ...
        if (entryPath) {
            ...
            if (selectedKeys.includes(parentKey)) {
                ...
                return this._parentFromEntryPathIsSelected(
                    parentKey,
                    selectedKeys
                );
            }
    ...
    private _getStateNode(
        ...
        for (let index = 0; index < children.getCount(); index++) {
            ...
            if (
                childInList ||
                (childIsNode && childIsSelected !== initialState)
            ) {
                countChildrenInList++;
            }

            isAllChildIsSelectedByOne =
                isAllChildIsSelectedByOne && childIsSelected;
            isAllChildIsExcludedByOne =
                isAllChildIsExcludedByOne && childIsExcluded;
        }
    ...
    private _correctNodeState(
        ...
        if (countChildrenInList > 0) {
            const hasMore =
                ...
                (node.hasMoreStorage('forward') ||
                    node.hasMoreStorage('backward'));
        ...
        const nodeIsPartialSelected =
            selection.selected.includes(nodeKey) &&
            selection.excluded.includes(nodeKey);
        if (
            (hasEntryPath &&
                this._childFromEntryPathIsSelected(
                    nodeKey,
                    selection.selected
                )) ||
            nodeIsPartialSelected ||
            (searchMode && initialState && this._isNewNodeSelectionBehavior())
        ) {
            return null;
        }
    ...
    private _childFromEntryPathIsSelected(
        ...
        if (entryPath) {
            ...
            if (selectedKeys.includes(childKey)) {
                ...
                return this._childFromEntryPathIsSelected(
                    childKey,
                    selectedKeys
                );
            }
     ...
     * @private
     */
    private _isAllChildrenExcluded(
        selection: ISelection,
        node: TItem
    ): boolean {
        if (!node) {
        ...
        if (childes.getCount() && !hasMore) {
            for (let i = 0; i < childes.getCount(); i++) {
                ...
                if (this._isNode(child)) {
                    result =
                        result && this._isAllChildrenExcluded(selection, child);
                }
    ...
    private _getRecursiveChildesInEntryPath(parentId: CrudEntityKey): TKeys {
        ...
        childesFromEntryPath.forEach((childKey) => {
            childrenIds = childrenIds.concat(
                this._getRecursiveChildesInEntryPath(childKey)
            );
        });
    ...
    private _getAllChildrenIds(node: TItem): TKeys {
        ...
        if (this._entryPath) {
            ...
            childrenIds = childrenIds.concat(
                this._getRecursiveChildesInEntryPath(nodeId)
            );
        }
     ...
     */
    private _getKey(item: TItem): CrudEntityKey {
        if (!item) {
            return undefined;
        }
    ...
    private _getRoot(): TItem {
        ...
        return (this._model.getItemBySourceKey(this._rootKey) ||
            this._model.getRoot()) as TItem;
    }
    ...
    private _canBeSelected(
        ...
        const sourceItem = this._model
            .getSourceDataStrategy()
            .getSourceItemByKey(itemKey);
        return (
            Utils.canBeSelected(
                {
                    item: sourceItem,
    ...
    private _validateOptions(options: ITreeSelectionStrategyOptions): void {
        if (options.selectionCountMode === 'leaf') {
            if (
                ...
                options.recursiveSelection === false
            ) {
                throw Error(
        ...
        if (
            options.selectionCountMode === 'node' &&
    ...
    private _isNewNodeSelectionBehavior(): boolean {
        return (
            this._selectionType === 'leaf' &&
            this._recursiveSelection &&
            this._feature1188089336
        );
    }
}
