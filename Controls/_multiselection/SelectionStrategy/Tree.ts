...
import { HierarchySelectionStrategy } from './hierarchy/HierarchySelectionStrategy';
import { IHierarchySelectionState } from './hierarchy/IHierarchySelectionState';
...
> extends FlatSelectionStrategy<TItem> {
    ...
    constructor(options: ITreeSelectionStrategyOptions) {
        super(options);
        this.update(options);
        this._newStrategy = new HierarchySelectionStrategy({
            isNewSearchSelectionBehavior: !!this._feature1188089336,
            hasMoreUtil: options.hasMoreUtil,
            isLoadedUtil: options.isLoadedUtil,
        });
    }
