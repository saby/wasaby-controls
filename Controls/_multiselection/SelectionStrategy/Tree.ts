...
> extends FlatSelectionStrategy<TItem> {
    ...
    private _getItemSelected(
        ...
        const parentKey = contents instanceof Model ? contents.get(parentProperty) : contents[parentProperty];
        let parent = this._model.getItemBySourceKey(parentKey) as unknown as TItem;
        ...
        } else if (parent['[Controls/_baseTree/BreadcrumbsItem]'] && !inSelected) {
     ...
     * @private
     */
    private _isAllChildrenExcluded(selection: ISelection, node: TItem): boolean {
        if (!node) {
            return false;
        }
