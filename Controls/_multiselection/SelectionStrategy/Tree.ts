...
> extends FlatSelectionStrategy<TItem> {
    ...
    unselect(selection: ISelection, key: CrudEntityKey, searchMode?: boolean): ISelection {
        ...
        if (key !== this._rootKey && item && this._selectAncestors && !item[BREADCRUMBS]) {
    ...
    selectAll(selection: ISelection, limit?: number): ISelection {
        ...
        if (!newSelection.excluded.includes(this._rootKey) && this._isMassSelectMode) {
    ...
    toggleAll(selection: ISelection, hasMoreData: boolean): ISelection {
        ...
        if (this._isAllSelected(selection, this._rootKey)) {
            ...
            if (hasMoreData || !this._isMassSelectMode) {
    ...
    getSelectionForModel(
        ...
        const selectionWithEntryPath = {
            selected: this._mergeEntryPath(selection.selected, selection.excluded),
    ...
    private _getItemSelected(
        ...
        if (item[BREADCRUMBS]) {
            ...
            if (!this._selectAncestors && !this._selectDescendants) {
                ...
                const hasSelectedParent = this._isSelectedByParent(key, selection);
    ...
    getSelectedItems(selection: ISelection, selectedItems?: TItem[]): TItem[] {
        ...
        const filterNotSelectedNodes = (item) => {
            ...
            return (
                ...
                (!this._isNode(item) ||
                    this._isSelectedByParent(key, selection) ||
        ...
        if (selectedItems?.length) {
            ...
            const unselectedItemsKeys = unselectedItems.map((item: TItem) => this._getKey(item));
            const filteredUnselectedItems = selectedItems.filter((selectedItem) => {
                return !unselectedItemsKeys.includes(this._getKey(selectedItem));
            });
            const newSelectedItemsKeys = newSelectedItems.map((item: TItem) => this._getKey(item));
            const filteredUnselectedChildren = filteredUnselectedItems.filter((selectedItem) => {
                return !newSelectedItemsKeys.includes(this._getKey(selectedItem));
            });
    ...
    private _unselectLeaf(selection: ISelection, item: TItem): void {
        ...
        const isSelectedParent = this._isSelectedByParent(itemId, selection);
    ...
    private _isSelectedByParent(itemKey: CrudEntityKey, selection: ISelection): boolean {
        return Utils.isSelectedByParent({
            selection,
            itemKey,
            collection: this._model.getSourceDataStrategy(),
            rootKey: this._rootKey,
    ...
    private _mergeEntryPath(selectedKeys: TKeys, excludedKeys: TKeys): TKeys {
        ...
        if (this._entryPath) {
            ...
            this._entryPath.forEach((it) => {
                ...
                if (parentIsSelected && !selectedKeysWithEntryPath.includes(it.id)) {
                    ...
                    if (this._selectAncestors || !excludedKeys.includes(it.id)) {
    ...
    private _getAllChildrenIds(node: TItem): TKeys {
        ...
        if (this._entryPath && this._isMassSelectMode) {
    ...
    protected _getKey(item: TItem): CrudEntityKey {
        ...
        // @ts-ignore
        if (item[BREADCRUMBS] || item.breadCrumbs) {
