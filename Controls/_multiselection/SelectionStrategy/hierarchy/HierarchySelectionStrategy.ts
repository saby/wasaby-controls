...
import { TSelectionModel, TSelectionModelStatus } from '../abstract/IAbstractSelectionState';
import ArraySimpleValuesUtil = require('Controls/Utils/ArraySimpleValuesUtil');
import type { IListState } from 'Controls-DataEnv/list';
import { EntityKey } from 'Types/_source/ICrud';
import { IIsLoadedUtil } from './Utils';
import { IHasMoreUtil } from '../flat/Utils';
...
export interface HierarchySelectionStrategyConstructorArgs {
    hasMoreUtil?: IHasMoreUtil;
    isLoadedUtil?: IIsLoadedUtil;
    isNewSearchSelectionBehavior?: boolean;
}

export interface CorrectNodeStateArgs {
...
export interface IsEmptyNodeArgs {
    node: TreeItem;
    hasMoreUtil: IHasMoreUtil;
    isLoadedUtil: IIsLoadedUtil;
}
...
function copyHierarchySelectionState(
    ...
    Pick<
        ...
        | 'keyProperty'
    > {
    const { keyProperty, items, nodeProperty, parentProperty, declaredChildrenProperty, root } =
        state;
    return {
        keyProperty,
        items,
...
export class HierarchySelectionStrategy
    implements ISelectionStrategy<IHierarchySelectionState, TreeItem>
{
    private readonly _hasMoreUtil: IHasMoreUtil;
    private readonly _isLoadedUtil: IIsLoadedUtil;
    private readonly _isNewSearchSelectionBehavior: boolean;

    constructor({
        isNewSearchSelectionBehavior,
        hasMoreUtil,
        isLoadedUtil,
    }: HierarchySelectionStrategyConstructorArgs) {
        // Дефолтные значения для обратной совместимости. В этом случае селект будет игнорировать то, что узел загружен
        this._hasMoreUtil = hasMoreUtil ?? (() => true);
        this._isLoadedUtil = isLoadedUtil ?? (() => false);
        ...
        items.forEach((item) => {
            ...
            if (
                ...
                (isBreadcrumb ||
                    ...
                    isEmptyNode({
                        node: item,
                        hasMoreUtil: this._hasMoreUtil,
                        isLoadedUtil: this._isLoadedUtil,
                    }))
            ) {
    ...
    isAllSelected(
        ...
        if (limit) {
            ...
            const selectionForModel = this.getSelectionModel(state, undefined, limit);
            let selectedCount = 0;
    ...
    convertCollectionToSelectionSideData(
        collection: IHierarchySelectionState['collection']
    ): THierarchySelectionSideData {
        const resultMap = new Map<CrudEntityKey, TreeItem>();
        let isOnlyNodesInItems = true;

        this._eachItem(collection, (item) => {
    ...
    getSelectionModel(
        state: IHierarchySelectionState,
        selectionData?: THierarchySelectionSideData,
        limit?: number,
        searchMode?: boolean
    ): TSelectionModel {
        return this._getSelectionModel(state, selectionData, limit, searchMode);
    }

    isKeySelectable(state: IHierarchySelectionState, key: EntityKey): boolean {
        const item = this._getItem(state, key);
        return this._canBeSelected(state, item);
    }
    ...
    private _getSelectionModel(
        ...
        const stateWithSelectedEntryPath = copyHierarchySelectionState({
            ...state,
            selectedKeys: getSelectedKeysEntryPath(
                getEntryPath(state),
        ...
        const selectedKeysSet = new Set(stateWithSelectedEntryPath.selectedKeys);
        ...
        data.forEach((item, key) => {
            ...
            if (isSelected !== false && limit && !selectedKeysSet.has(key)) {
            ...
            selectionModel.set(key, isSelected);
        });
    ...
    private _select(
        ...
        if (updateMap) {
            nextState.selectionModel = this._getSelectionModel(
                nextState,
                undefined,
                undefined,
                searchMode
    ...
    private _getAllChildrenIds(state: IHierarchySelectionState, node: TreeItem): CrudEntityKey[] {
        ...
        const _getRecursiveChildesInEntryPath = (parentId: CrudEntityKey): CrudEntityKey[] => {
            ...
            const childesFromEntryPath = getEntryPath(state)
                ?.filter((item) => {
        ...
        if (getEntryPath(state) && state.isMassSelectMode) {
    ...
    private _getItemSelectionModelStatus(
        state: IHierarchySelectionState,
        item: TreeItem,
        collectionMap: Map<CrudEntityKey, TreeItem>,
        ...
        const parent =
            parentKey === state.root ? state.collection.getRoot() : collectionMap.get(parentKey);
    ...
    private _getStateNode(
        ...
        return correctNodeState({
            ...
            entryPath: getEntryPath(state),
            searchMode,
            isNewSearchSelectionBehavior: this._isNewSearchSelectionBehavior,
            hasMoreUtil: this._hasMoreUtil,
            isLoadedUtil: this._isLoadedUtil,
        });
    }
    ...
    private _getBreadcrumbsSelectionModelStatus(
        ...
        if (this._isAllSelectedInRoot(state)) {
            ...
            if (isSelected && (!isSelectedLastCrumb || isAllChildrenExcluded)) {
                // При selectDescendants=false искомая запись в хлебной крошке не выбрана.
                // см. https://online.sbis.ru/opendoc.html?guid=7cc8aeb8-53df-443f-817f-57b76140eb63&client=3
                isSelected = state.selectDescendants !== false ? null : false;
            }
    ...
    private _unselectNodeOnState(
        ...
        const nodeKey = this._getKey(node);
        const entryPathHasNode = getEntryPath(state)?.find((it) => {
...
function correctNodeState({
    ...
    isLoadedUtil,
    hasMoreUtil,
}: CorrectNodeStateArgs): TSelectionModelStatus {
    if (countChildrenInList > 0) {
        const hasMore =
            node[TREE_ITEM as keyof TreeItem] &&
            (node.hasMoreStorage('forward') || node.hasMoreStorage('backward'));
        return (
            (!hasMore && isAllChildIsSelectedByOne) || (isAllChildIsExcludedByOne ? false : null)
        );
    }

    const nodeKey = BaseUtils.getKey(node);
    const isNodeKeyValid = typeof nodeKey !== 'undefined';
    const hasEntryPath = entryPath && entryPath.length;
    const isFullyLoaded = isNodeKeyValid && isLoadedUtil(nodeKey) && !hasMoreUtil(nodeKey);
    const nodeIsPartialSelected =
        isNodeKeyValid &&
        state.selectedKeys.includes(nodeKey) &&
        state.excludedKeys.includes(nodeKey);

    // В ENTRY_PATH должны быть перечислены именно выбранные записи, а не все дети родителя.
    if (
        (!isFullyLoaded &&
            hasEntryPath &&
            isNodeKeyValid &&
            isChildFromEntryPathIsSelected(entryPath, nodeKey, state.selectedKeys)) ||
        nodeIsPartialSelected ||
        (searchMode && initialState && isNewSearchSelectionBehavior)
    ) {
        return null;
    }
...
function getSelectedKeysEntryPath(
    entryPath: IHierarchySelectionState['entryPath'],
    ...
    const selectedKeysWithEntryPath: IHierarchySelectionState['selectedKeys'] = [...selectedKeys];
    ...
    if (entryPath) {
        ...
        entryPath.forEach((it) => {
            ...
            const parentIsSelected = isParentFromEntryPathIsSelected(
                entryPath,
                it.id,
                selectedKeys
            );
...
function isParentFromEntryPathIsSelected(
    ...
    if (entryPathItem) {
        ...
        if (selectedKeys.includes(parentKey)) {
            ...
            return isParentFromEntryPathIsSelected(entryPath, parentKey, selectedKeys);
        }
    }
...
function isChildFromEntryPathIsSelected(
    entryPath: Required<IHierarchySelectionState>['entryPath'],
    parentKey: CrudEntityKey,
    ...
    if (entryPathItem) {
        ...
        if (selectedKeys.includes(childKey)) {
            ...
            return isChildFromEntryPathIsSelected(entryPath, childKey, selectedKeys);
        }
    }
...
/**
 * Функция для получения ENTRY_PATH. BaseControl устанавливает entryPath напрямую.
 * При использовании в middleware entryPath не передается, поэтому берется напрямую из коллекции
 * */
function getEntryPath(state: IHierarchySelectionState): IEntryPathItem[] | undefined {
    return state.entryPath ?? state.collection?.getMetaData()?.ENTRY_PATH;
}

function isEmptyNode({ node, hasMoreUtil, isLoadedUtil }: IsEmptyNodeArgs): boolean {
    const key = BaseUtils.getKey(node);

    if (typeof key === 'undefined') {
        return false;
    }

    return node.getChildren().getCount() === 0 && !hasMoreUtil(key) && isLoadedUtil(key);
}
