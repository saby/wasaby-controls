...
import { ISourceDataStrategy, MultiSelectAccessibility, Collection } from 'Controls/display';
import * as FlatUtils from '../flat/Utils';
import { TSelectionType } from './IHierarchySelectionState';
...
export interface ITreeGetCountParams extends FlatUtils.IFlatGetCountParams, ICollectionParams {
...
export interface ICanBeSelectedParams extends FlatUtils.ICanBeSelectedParams, IIsNodeParams {
    rootKey?: TKeySelection;
    selectionType: TSelectionType;
    recursiveSelection: boolean;
}
...
export function getCount(params: ITreeGetCountParams): number | null {
    ...
    if (limit) {
        ...
        // Если limit превышает количество загруженных объектов, необходимо возвращать null,
        // чтобы была возможность подгрузить корректное значение, либо не устанавливать вовсе
        // Если вернуть значение, оно и будет установлено
        return limit > countItems ? (hasMoreData ? null : countItems) : limit;
    }

    let countItemsSelected: number | null = 0;
    const selectedNodes: TKeysSelection = [];

    const selection = object.clone(params.selection);
    if (!params.isSearchViewMode && selectDescendants && selectAncestors) {
        // Если выбраны все дети какого либо узла, то заменяем ключи детей на ключ узла.
        ...
        let index = 0;
        while (selection.selected.length > index) {
            const selectedItemKey = selection.selected[index];
            ...
            if (
                ...
                ArraySimpleValuesUtil.removeSubArray(selection.selected, childrenKeys);
                ArraySimpleValuesUtil.addSubArray(selection.selected, [parentKey]);
                index = 0;
            } else {
                index++;
            }
    ...
    if (!isAllSelected({ ...params, selection }) || (!hasMoreData && !filter)) {
        for (let index = 0; index < selection.selected.length; index++) {
            ...
            if ((selectDescendants || isExcluded) && isNodeValue) {
        ...
        for (let index = 0; index < selectedNodes.length; index++) {
            ...
            if (
                ...
                (hasMoreUtil(nodeKey) || !isLoadedUtil(nodeKey))
            ) {
...
export function isAllSelected(params: IIsAllSelectedParams): boolean {
    const { selectDescendants, selection, rootKey, byEveryItem, collection } = params;
...
export function getSelectedParent(params: IHasSelectedParentParams): CrudEntityKey | undefined {
...
export function getParentIncludedInArray(
    ...
    const findSelectedParent = (items: ISourceDataStrategy | RecordSet) => {
        ...
        do {
            ...
            if (!item) {
                // Проверяем что это точно корень, а не просто кривая иерархия и проверяем, что корень выбран
                if (
                    params.rootKey !== currentParentKey ||
                    !params.selection.selected.includes(params.rootKey)
                ) {
                    break;
                }
...
function checkSelectionType(selectionType: string, params: ICanBeSelectedParams): boolean {
    ...
    switch (selectionType) {
        ...
        case 'allBySelectAction':
            return true;
        case 'leaf':
            return !isNodeValue || (params.recursiveSelection && isNodeValue) || itemIsRoot;
        case 'node':
            return isNodeValue;
        case 'hiddenNode':
            return isHiddenNode(params);
    }
    return true;
}
...
export function canBeSelected(
    ...
    let multiSelectAccessibility = params.item.get(params.multiSelectAccessibilityProperty);
    ...
    if (Array.isArray(params.selectionType)) {
        canBeSelectedBySelectionType = params.selectionType.some((type) =>
            checkSelectionType(type, params)
        );
    } else {
        canBeSelectedBySelectionType = checkSelectionType(params.selectionType, params);
    }
...
export interface IGetSelectedChildrenCountParams extends ICanBeCountedParams, ICollectionParams {
    itemKey: CrudEntityKey;
    selection: ISelection;
...
function getSelectedChildrenCount(params: IGetSelectedChildrenCountParams): number | null {
    const itemIsRoot = params.itemKey === params.rootKey;
    ...
    const children = getChildren({ ...params, nodeKey: params.itemKey });
    const isNotAllChildrenLoaded = children.length > 0 && params.hasMoreUtil(params.itemKey);

    // В узле могут быть загруженные дети и кнопка еще, в таком случае
    // список не может сам посчитать счетчик выделения.
    if (isNotAllChildrenLoaded) {
        return null;
    }
    ...
    if (children.length) {
        children.forEach((childItem) => {
            if (selectedChildrenCount !== null) {
                ...
                const childInSelected = params.selection.selected.includes(childKey);
                const childInExcluded = params.selection.excluded.includes(childKey);
                ...
                const allowCountBySelected =
                    !childIsNode || isSelectedNode({ ...params, itemKey: childKey });
                // не считаем записи в selected, т.к. они будут посчитаны в вызывающем методе
                if (!childInSelected && canBeCountedValue && allowCountBySelected) {
                    selectedChildrenCount++;
                }

                if (childIsNode && (!params.selectDescendants || !childInExcluded)) {
...
function isSelectedNode(params: IIsSelectedNodeParams): boolean | null {
    ...
    const nodeKeyInSelected = params.selection.selected.includes(params.itemKey);
    const nodeKeyInExcluded = params.selection.excluded.includes(params.itemKey);

    const isAllSelectedValue = isAllSelected(params);
    const isSelectedByParentValue =
        isAllSelectedValue || (isSelectedByParent(params) && params.selectDescendants);

    const hasExcludedChildValue = hasExcludedChild(params);
    const isSelectedByChild = !params.selectAncestors || !hasExcludedChildValue;

    const isSelected =
        (nodeKeyInSelected || isSelectedByParentValue) && !nodeKeyInExcluded && isSelectedByChild;

    if (!params.isSearchViewMode || !isAllSelectedValue) {
        return isSelected;
    }

    const collectionItem = params.collection.getItemBySourceKey(params.itemKey);

    // Если элемент есть в рекордсете, но отсутствует в самой коллекции, то его не нужно учитывать при подсчете
    if (!collectionItem) {
        return null;
    }

    if (collectionItem[BREADCRUMBS]) {
...
interface IIsHiddenNodeParams extends IIsNodeParams {}

function isHiddenNode(params: IIsHiddenNodeParams): boolean {
    const nodeValue = params.item && params.item.get(params.nodeProperty);
    return nodeValue === HIDDEN_NODE;
}
...
function hasChildren(params: IHasChildrenParams): boolean {
    ...
    if (hasChildrenByProperty !== undefined) {
        ...
        hasChildrenValue = params.isLoadedUtil(params.itemKey) ? hasChildrenByRecordset : true;
    }
