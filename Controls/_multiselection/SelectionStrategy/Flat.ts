...
import { IFlatSelectionState } from './flat/IFlatSelectionState';
import { FlatSelectionStrategy as NewFlatSelectionStrategy } from './flat/FlatSelectionStrategy';
    ...
    implements ISelectionStrategy<TItem>
{
    ...
    getSelectionForModel(
        ...
        const selectionData = this._newStrategy.convertCollectionToSelectionSideData(this._model);
    ...
    protected _convertSelectionModelToMap(
        selectionModel: IListState['selectionModel'],
        collectionMap: Map<CrudEntityKey, CollectionItem>
        ...
        selectionModel.forEach((value, key) => {
            const item = collectionMap.get(key);
            if (item) {
                selectedItems.get(value).push(item);
            }
        });
