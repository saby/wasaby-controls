 ...
 * @class Controls/_multiselection/CounterController
 * @public
 */
export default class CounterController {
        ...
        if (this._shouldCountByChildrenCountProperty()) {
            if (selection.selected.length) {
                ...
                excludedKeysDiff.added.forEach((it) => {
                    return (
                        it !== this._options.rootKey &&
                        this._updateCount(it, false)
                    );
                });
            } else {
                this.reset();
    ...
    getCount(limit?: number, selection?: ISelection, filter?: object): number {
        const isTree =
            this._options.parentProperty && this._options.nodeProperty;
        if (isTree) {
            if (this._shouldCountByChildrenCountProperty()) {
    ...
    private _updateCount(itemKey: CrudEntityKey, newState: boolean): void {
        ...
        const isNode = item
            ? item.get(this._options.nodeProperty) !== null
            : false;
        const offsetBySelectionCountMode =
            this._options.selectionCountMode !== 'leaf' ? 1 : 0;
        const count =
            isNode && this._options.childrenCountProperty
                ? item.get(this._options.childrenCountProperty) +
                  offsetBySelectionCountMode
                : 1;
        ...
        if (newState) {
            ...
            if (itemIndex !== -1) {
                ...
                let selectedParentKey = Utils.getSelectedParent({
                    ...this._options,
                ...
                if (selectedParentKey === this._options.rootKey) {
                    ...
                    const parentData = Utils.getParentIncludedInArray({
                        ...this._options,
                        selection: this._selection,
                        itemKey,
                        array: dataKeys,
                ...
                if (parentIndex !== -1) {
                    ...
                    if (selectedParentKey !== this._options.rootKey) {
                        Logger.error(
                            'Не реализован правильный пересчет кол-ва'
                        );
                    }
                }
            }
    ...
    private _getRootItemKeys(rootKey: CrudEntityKey): CrudEntityKey[] {
        const rootItems = Utils.getChildren({
