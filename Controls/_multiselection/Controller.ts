...
export class Controller {
    ...
    private _feature1188519278: boolean = false;
    ...
    constructor(options: ISelectionControllerOptions) {
        ...
        this._selectedKeys = options.selectedKeys
            ? options.selectedKeys.slice()
            : [];
        this._excludedKeys = options.excludedKeys
            ? options.excludedKeys.slice()
            : [];
        ...
        this._feature1188519278 = options.feature1188519278;
    ...
    updateOptions(
        ...
        if (!isEqual(this._sorting, options.sorting)) {
            this._sorting = options.sorting;
     ...
     */
    getSelectedItems(): CollectionItem<Model>[] {
    ...
    getCountOfSelected(selection?: ISelection): number | null {
        return this._counterController.getCount(
            this._limit,
            selection,
            this._filterChanged && this._feature1188519278 ? this._filter : null
        );
    }
    ...
    toggleItem(key: CrudEntityKey): ISelection {
        ...
        if (status === true || status === null) {
            ...
            newSelection = this._strategy.unselect(
                this._selection,
                key,
                this._searchMode
            );
    ...
    selectAll(packSize?: number): ISelection {
        ...
        const newSelection = this._strategy.selectAll(
            initSelection,
            this._limit
        );
        ...
        this._startFilteredItemsCount = this._model
            .getSourceCollection()
            .getCount();
    ...
    toggleAll(): ISelection {
        return this._strategy.toggleAll(
            this._selection,
            this._model.hasMoreData()
        );
    }
    ...
    selectRange(key: CrudEntityKey): ISelection {
        ...
        if (!this._lastCheckedKey) {
            ...
            const sliceStart =
                secondIndex > firstIndex ? firstIndex : secondIndex;
            const sliceEnd =
                sliceStart === secondIndex ? firstIndex + 1 : secondIndex + 1;
            const items = [];
        ...
        if (this._model.getSourceCollection().getCount()) {
            ...
            keys = keys.filter((key) => {
                return !this._model
                    ...
                    .getSourceItemByKey(key);
            });

            const selected = ArraySimpleValuesUtil.removeSubArray(
                this._selectedKeys.slice(),
                keys
            );
            const excluded = ArraySimpleValuesUtil.removeSubArray(
                this._excludedKeys.slice(),
                keys
            );
        ...
        if (this._limit) {
            ...
            for (let i = 0; i < addedItems.length; i++) {
                ...
                if (!isSelectedByLimit) {
                    ...
                    ArraySimpleValuesUtil.addSubArray(newSelection.excluded, [
                        itemKey,
                    ]);
                }
            }
    ...
    getAnimatedItem(): ISelectionItem {
        return this._model.find((item) => {
            return (
                !!item.isAnimatedForSelection && item.isAnimatedForSelection()
            );
        });
    }
    ...
    private _fillSelectionByLimit(
        ...
        if (this._limit) {
            ...
            if (firstSelectPack) {
                ...
                for (
                    let i = 0;
                    i < this._limit - packSize && i < items.length;
                    i++
                ) {
                    const item = items[i];
                    const itemKey = this._getKey(item);
                ...
                if (!currentPackIsFilled) {
                    ...
                    for (
                        let i = this._limit - packSize;
                        i < this._limit && i < items.length;
                        i++
                    ) {
                        const item = items[i];
                        const itemKey = this._getKey(item);
                        ...
                        if (isSelectedByLimit) {
                            if (
                                this._separatedSelectedItems.includes(itemKey)
                            ) {
    ...
    private _shouldResetSelectionOnReload(isAllSelected: boolean): boolean {
        ...
        const loadedItemsMoreStartLoadedItems =
            this._startFilteredItemsCount <
            this._model.getSourceCollection().getCount();
        const resetByFilter =
            this._filterChanged &&
            (!this._feature1188519278 ||
                (!returnedStartedFilter &&
                    (this._model.hasMoreData() ||
                        loadedItemsMoreStartLoadedItems)));
        const resetBySorting = this._sortingChanged && this._limit;
        return (resetByFilter || resetBySorting) && isAllSelected;
    }
