...
export class Controller {
    ...
    updateOptions(options: Partial<ISelectionControllerOptions>): ISelection | void {
        ...
        this._searchMode = options.searchMode;
        const previousRootKey = this._rootKey;
        this._rootKey = options.rootKey;
        ...
        if (itemsChanged && modelChanged) {
            // isAllSelected важно рассчитывать на старом корне, т.к. выделение происходило на старом корне, а при сбросе
            // поиска он меняется на тот, что был до ввода поискового запроса, и к моменту обновления опций в контроллере
            // он уже будет проставлен в коллекцию. По умолчанию isAllSelected отрабатывает на корне, установленном в коллекции.
            const isAllSelected = this.isAllSelected(false, undefined, previousRootKey);
        ...
        if (status === true || status === null) {
            if (this._limit) {
                ...
                if (!this._separatedSelectedItems.includes(key)) {
                    ...
                    this._separatedSelectedItems = this._separatedSelectedItems.filter(
                        (separatedKey) => separatedKey !== key
                    );
                }
            }
    ...
    onCollectionRemove(removedItems: ISelectionItem[]): ISelection {
        if (this._model.getSourceCollection().getCount()) {
            ...
            // При удалении необходимо полностью пересчитать _selectedItems, чтобы не хранить лишние ключи
            this._selectedItems = this._strategy.getSelectedItems(this._selection);
     ...
     * @void
     */
    onCollectionReplace(): void {
        this._updateModel(this._selection, false);
    }
            ...
            if (firstSelectPack) {
                ...
                if (!currentPackIsFilled) {
                    ...
                    if (!currentPackIsFilled) {
                        // Проходим по новой пачке
                        for (
                            let i = Math.min(items.length, this._limit - packSize + countSelected);
                            countSelected !== packSize && i !== items.length;
                            i++
                        ) {
                            const item = items[i];
                            const itemKey = this._getKey(item);
                            if (!item.isSelected()) {
                                countSelected++;
                                ArraySimpleValuesUtil.removeSubArray(newSelection.excluded, [
                                    itemKey,
                                ]);
                            } else {
                                this._limit++;
                                this._separatedSelectedItems = this._separatedSelectedItems.filter(
                                    (separatedKey) => separatedKey !== itemKey
                                );
                            }
                        }
                    }
    ...
    private _getKey(item: ISelectionItem): CrudEntityKey {
        ...
        const contents = this._getContents(item);
        return contents instanceof Object ? contents.getKey() : contents;
    }

    private _getContents(item: ISelectionItem): Model {
