...
export default class DragScroll extends DestroyableMixin {
    ...
    constructor(params: IDragScrollParams) {
        ...
        if (!params.canStartDragScrollCallback) {
            ...
            this._canStartDragScrollCallback =
                params.canStartDragScrollCallback;
        }
    ...
    setStartDragNDropCallback(
        callback: IDragScrollParams['startDragNDropCallback']
    ): void {
        this._startDragNDropCallback = callback;
    }

    setCanStartDragNDropCallback(
        callback: IDragScrollParams['canStartDragScrollCallback']
    ): void {
     ...
     * @private
     */
    private _manageDragScrollStart(
    ...
    private _manageDragScrollMove(newMousePosition: IPoint): number | null {
        ...
        this._maxMouseMoveDistance = {
            x: Math.max(
                Math.abs(this._maxMouseMoveDistance.x),
                Math.abs(mouseMoveDistance.x)
            ),
            y: Math.max(
                Math.abs(this._maxMouseMoveDistance.y),
                Math.abs(mouseMoveDistance.y)
            ),
        };
        ...
        const newScrollPosition = Math.min(
            Math.max(
                this._startScrollPosition -
                    Math.floor(SCROLL_SPEED_BY_DRAG * mouseMoveDistance.x),
                0
            ),
            this._scrollLength
        );
