...
export class ItemsStateManager extends AbstractAspectStateManager<IItemsState> {
    ...
    resolveChanges(prevState: IItemsState, nextState: IItemsState): IListChange[] {
        ...
        if (nextState.keyProperty !== prevState.keyProperty) {
            changes.push({
                name: ListChangeNameEnum.CHANGE_KEY_PROPERTY,
                args: {
    ...
    getNextState(state: IItemsState, changes: IListChange[]): IItemsState {
        ...
        for (const change of changes) {
            switch (change.name) {
                ...
                case ListChangeNameEnum.CHANGE_KEY_PROPERTY: {
                    nextState.keyProperty = change.args.keyProperty;
                    break;
                }
            }
        }
    ...
    applyChangesToCollection<TCollection extends ICollection>(
        ...
        for (const change of changes) {
            switch (change.name) {
                ...
                case ListChangeNameEnum.CHANGE_KEY_PROPERTY: {
                    collection.setKeyProperty(change.args.keyProperty);
                    break;
                }
            }
        }
