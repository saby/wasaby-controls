...
import { ISourceDataStrategy, MultiSelectAccessibility } from 'Controls/display';
import * as FlatUtils from '../_flatSelectionAspect/Utils';
...
export function getCount(params: ITreeGetCountParams): number | null {
    ...
    let selectedNodes: TKeysSelection = [];
        ...
        if (selectDescendants) {
            for (let index = 0; index < selection.selected.length; index++) {
                ...
                if (
                    canBeCountedValue &&
                    !isExcluded &&
                    (!isNodeValue || !hasExcludedChild({ ...params, selection, itemKey }))
                ) {
                    countItemsSelected++;
                }
            }
