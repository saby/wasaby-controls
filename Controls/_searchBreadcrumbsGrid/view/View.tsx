...
import { getKey, TItemEventHandler } from 'Controls/baseList';
...
import { isBreadcrumbRow, isSeparatorRow } from 'Controls/_searchBreadcrumbsGrid/utils/Type';
...
function getRowComponent(
    ...
    if (isBreadcrumbRow(item) || isSeparatorRow(item)) {
        row = (
            <DefaultSBRC
                {...rowProps}
    ...
    const collectionItemContextValue: ICollectionItemContextValue = {
        item,
        itemContents: Array.isArray(item?.contents)
            ? item.contents[item.contents.length - 1]
            : item?.contents,
    };

    return row ? (
        <CollectionItemContext.Provider value={collectionItemContextValue} key={item.key}>
            {row}
...
function isEventStoppedOnReadOnly(
    ...
    if (isBreadcrumbRow(item) && searchNavigationMode === 'readonly') {
        event.stopPropagation();
        return true;
    }
    return false;
...
function isClickedOnCheckbox(
    ...
    if (event.target.closest('.js-controls-ListView__checkbox')) {
        const record = isBreadcrumbRow(item)
            ? item.contents[item.contents.length - 1]
            : item.contents;
        onCheckboxClickCallback(event, record);
        return true;
    }
    return false;
}
...
function isClickedOnBreadCrumb(
    ...
    if (isBreadcrumbRow(item)) {
        const record = item.contents[item.contents.length - 1];
        if (record && onClickCallback) {
            // Хлебная крошка всегда занимает одну ячейку
            onClickCallback(event, record);
        }
        event.stopPropagation();
        return true;
    }
...
function patchItemEventHandlers({
    ...
    onBreadCrumbsItemClick: (event: Event, item: Model) => void;
} {
    ...
    return {
        ...
        onBreadCrumbsItemClick(event: Event, item: Model): void {
            if (searchNavigationMode === 'readonly') {
                event.stopPropagation();
                return;
            }
...
function getColspanBreadcrumbs({
    ...
    columnScrollViewMode: ISearchBreadcrumbsTreeGridViewProps['columnScrollViewMode'];
}) {
    ...
    if (colspan && columnScroll && isColumnScrollVisible && columnScrollViewMode !== 'unaccented') {
        colspan = false;
    }
    return colspan;
}
