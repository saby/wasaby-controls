...
import { IItemActionsHandler, NOT_EDITABLE_JS_SELECTOR } from 'Controls/baseList';
...
function getMaxWidthClasses(colspan?: boolean): string {
    if (colspan) {
        return ' controls-Grid__row-cell_breadCrumbsSearch_max_width';
    } else {
        return '';
    }
}
...
function getPaddingClasses(padding: ISearchBreadcrumbsCellComponentProps['padding']): string {
    return (
        OffsetClassUtils.getHorizontalPaddingsClasses(padding.left, padding.right) +
...
function SearchBreadcrumbsCellComponent(
    ...
    const colspan =
        props.startColumn !== undefined &&
        props.endColumn !== undefined &&
        props.endColumn - props.startColumn > 1;
    ...
    const wrapperRenderClassName =
        ...
        CellPositionClassUtils.getCellPositionClasses(props) +
        getMinHeightClasses() +
        getMaxWidthClasses(colspan) +
