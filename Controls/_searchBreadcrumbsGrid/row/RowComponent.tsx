...
import { isBreadcrumbCell, isSeparatorCell } from 'Controls/_searchBreadcrumbsGrid/utils/Type';

/*
 * Возвращает прикладной шаблон хлебных крошек
 * В новом "чистом" рендере не поддерживаем опцию настройки searchBreadCrumbsItemTemplate,
 * т.к. он не вяжется со стандартом. Но прикладники её используют в своих компонентах,
 * поэтому добавили совместимость как в чистом так и в совместимом рендере.
 */
function getCompatibleSearchBreadcrumbsCell(
    cell: GridCell,
    ...
    return templateLoader(
        rowProps.searchBreadCrumbsItemTemplate,
        templateProps
...
export function getCompatibleCellComponent(
    ...
    if (isBreadcrumbCell(cell)) {
        ...
        if (CellPropsUtils.isFirstDataCell(breadcrumbsCell)) {
            ...
            if (rowProps?.searchBreadCrumbsItemTemplate) {
                getCompatibleSearchBreadcrumbsCell(cell, cellProps, rowProps);
            }
    ...
    if (isSeparatorCell(cell)) {
        if (!CellPropsUtils.isFirstDataCell(cell)) {
            const FunctionalCellComponent = rowProps._$FCC || BaseCellComponent;
            return (
...
export function getDirtyCellComponentContentRender(
    ...
    return getDirtyGridCellComponentContentRender(cell, cellProps, multiSelectTemplate);
}
...
export function getCleanCellComponent(
    ...
    if (isBreadcrumbCell(cell) && CellPropsUtils.isFirstDataCell(cell)) {
        ...
        // Рендерим прикладной шаблон хлебных крошек
        if (rowProps?.searchBreadCrumbsItemTemplate) {
            return getCompatibleSearchBreadcrumbsCell(cell, searchBreadcrumbsProps, rowProps);
        }

        return <SearchBreadcrumbsCellComponent {...searchBreadcrumbsProps} key={cell.key} />;
    }

    if (isSeparatorCell(cell) && CellPropsUtils.isFirstDataCell(cell)) {
