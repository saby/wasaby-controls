...
export default class SearchView extends TreeGridView {
    ...
    _beforeMount(options: IOptions): Promise<void> {
        ...
        if (
            options.breadCrumbsMode === 'cell' ||
            (options.newColumnScroll
                ? options.canHorizontalScroll
                : options._initBreadCrumbsMode === 'cell')
        ) {
    ...
    _beforeUpdate(newOptions: IOptions): void {
        ...
        if (
            ...
            ((newOptions.columnScroll && this.isColumnScrollVisible()) ||
                (newOptions.newColumnScroll && newOptions.canHorizontalScroll))
        ) {
            colspan = false;
        }
