...
export default class BreadcrumbsItemRow<T extends Model = Model> extends GridDataRow<T> {
    ...
    getContents(): T[] {
        ...
        // Проксируем в contents некоторые методы последнего
        // элемента массива для удобства использования.
        if (contents.length) {
            const lastItem = contents[contents.length - 1];
            injectSource({
                instance: contents,
                bindInstance: lastItem,
                source: lastItem,
                include: ['getKey', 'get', 'set', 'getRawData', 'getOwner'],
            });
        }
