...
export default class BreadcrumbsItemRow<T extends Model = Model> extends GridDataRow<T> {
    ...
    protected get _first(): SearchGridDataRow<T> {
        ...
        while (current) {
            ...
            if (!parent || parent === root || parent.$TGGR) {
                break;
            }
            current = parent;
        }
    ...
    getContents(): T[] {
        ...
        while (current) {
            ...
            if (!current || current === root || current.$TGGR) {
                break;
            }
        }
    ...
    // По s.tensor.ru все прикладники ожидают в _$stickyCallback Model, а не Model[]
    isSticked(): boolean {
        const contents = this.getContents();
        const lastItem = contents[contents.length - 1];
        return this._$stickyCallback && lastItem instanceof Model
            ? !!this._$stickyCallback(lastItem)
            : super.isSticked();
    }
