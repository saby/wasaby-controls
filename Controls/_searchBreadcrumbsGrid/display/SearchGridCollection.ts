...
import type { IGridOptions as ITreeGridOptions } from 'Controls/baseGrid';
import type { TreeGridDataRow } from 'Controls/baseTreeGrid';
...
> extends TreeGridCollection<S, T> {
    ...
    setOnBreadcrumbItemClickCallback(callback: (event: Event, item: Model) => void): void {
        if (this._$onBreadcrumbItemClick !== callback) {
            this._$onBreadcrumbItemClick = callback;
            this._updateItemsProperty(
                'setOnBreadcrumbItemClickCallback',
                this._$onBreadcrumbItemClick,
                '[Controls/_baseTree/BreadcrumbsItem]'
            );
            this._nextVersion();
        }
