...
import type { IGridOptions as ITreeGridOptions } from 'Controls/gridDisplay';
...
function itemIsVisible<T extends Model>(item: TreeItem<T>): boolean {
    if (item.$TGGR || item['[Controls/_display/GroupItem]']) {
        return true;
    }
    ...
    if (!parent || parent.isRoot()) {
        return true;
    } else if (parent.$TGGR && !parent.isExpanded()) {
        return false;
    }
...
> extends TreeGridCollection<S, T> {
    ...
    private _$task88221034408419?: boolean;
    ...
    createBreadcrumbsItem(options: object): BreadcrumbsItemRow {
        ...
        const item = this.createItem({
            ...
            fix88221033223603: this._$fix88221033223603,
        });
        return item;
    }
    ...
    createRootSeparator(options: object): BreadcrumbsItemRow {
        ...
        const item = this.createItem({
            ...options,
            owner: this,
            task88221034408419: this._$task88221034408419,
        });
        return item;
    }
