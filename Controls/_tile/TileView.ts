...
const AVAILABLE_CONTAINER_HORIZONTAL_PADDINGS = [
    'null',
    'default',
    'xs',
    's',
...
export default class TileView extends ListView {
    ...
    protected _beforeUpdate(newOptions: ITileOptions): void {
        ...
        if (
            this._options.itemsContainerPadding !==
            newOptions.itemsContainerPadding
        ) {
            this._listModel.setItemsContainerPadding(
                newOptions.itemsContainerPadding
            );
        }
        ...
        if (
            this._options.imageHeightProperty !== newOptions.imageHeightProperty
        ) {
            this._listModel.setImageHeightProperty(
                newOptions.imageHeightProperty
            );
        }
        if (
            this._options.imageWidthProperty !== newOptions.imageWidthProperty
        ) {
            this._listModel.setImageWidthProperty(
                newOptions.imageWidthProperty
            );
        }
    ...
    protected _afterUpdate(): void {
        ...
        const hoveredItem =
            this._listModel.getHoveredItem() as TileCollectionItem;
        if (
            hoveredItem &&
            ...
            if (
                ...
                hoveredItem.setFixedPositionStyle(
                    this._animatedItemTargetPosition
                );
            }
        }
    }
    ...
    protected _beforeUnmount(): void {
        ...
        this._notify(
            'unregister',
            ['customscroll', this, { listenAll: true }],
            { bubbling: true }
        );
        this._notify(
            'unregister',
            ['viewportResize', this, { listenAll: true }],
            { bubbling: true }
        );
    }
    ...
    getImageSizeForMenu(itemContainer: HTMLElement): {
        ...
        const viewContainerRect =
            DimensionsMeasurer.getBoundingClientRect(viewContainer);
    ...
    getActionsMenuConfig(
        ...
        if (this._shouldOpenExtendedMenu(isActionMenu) && menuConfig) {
            ...
            const itemContainer = clickEvent.target.closest(
                '.controls-TileView__item'
            );
            const imageWrapper = itemContainer.querySelector(
            ...
            menuOptions.imageProportion =
                menuOptions.previewWidth / menuOptions.previewHeight;
    ...
    private _setHoveredItemPosition(
        ...
        if (
            tileScalingMode === 'none' ||
            target.closest('.js-controls-TileView__withoutZoom')
        ) {
            if (
                item &&
                item['[Controls/_tile/mixins/TileItem]'] &&
        ...
        const targetItemPositionInDocument =
            this._listModel.getItemContainerPositionInDocument(
                targetItemPosition,
                viewContainerRect,
                documentRect
            );
        ...
        if (targetItemPositionInDocument) {
            ...
            if (tileScalingMode !== 'overlap') {
                const startItemPositionInDocument =
                    this._listModel.getItemContainerStartPosition(
                        itemContainerRect,
                        documentRect
                    );
    ...
    protected _setHoveredItem(
        ...
        const hasItem =
            item === null ||
            !!this._listModel.getItemBySourceItem(item.getContents());
        if (!hasItem) {
            return;
        }
     ...
     * @private
     */
    protected _notifyUpdateActions(
        item: TileCollectionItem,
        event: SyntheticEvent
    ): void {
        if (item && this._options.actionMode === 'adaptive' && !!event) {
            const itemWidth = this._calcTileItemWidth(
                event.target as HTMLElement
            );
            this._notify(
                'updateItemActionsOnItem',
                [item.getContents().getKey(), itemWidth],
                { bubbling: true }
            );
        }
    }
    ...
    protected _calcTileItemWidth(element: HTMLElement): number {
        let itemWidth;
        const itemContainer: HTMLElement = element.closest(
            '.controls-TileView__item'
        );
