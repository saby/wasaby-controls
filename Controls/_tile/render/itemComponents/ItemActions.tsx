...
import {
    ...
    ToolbarChooser as ItemActionsComponent,
    ItemActionsContext,
...
import { extractSwipeActionProps } from 'Controls/listVisualAspects';
...
export function ItemActionsResolverComponent(
    ...
    // В схеме без BaseControl itemActions заранее инициализированы на коллекции.
    if (itemActionsContext?.actionsInitialized && item.getActions()?.all) {
        return (
            <ItemActionsComponent
                actions={item.getActions()?.all}
...
export const ItemActionsControl = React.memo(function ItemActionsControl(
    ...
    if (!shouldDisplay || !ItemActionsControlComponent || !ControlContent) {
        return null;
    }
