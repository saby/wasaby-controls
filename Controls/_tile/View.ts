...
import { resolveViewControls } from './utils/ViewControlsResolver';

/**
 * Контрол "Плитка".
 * Позволяет отображать данные из различных источников в виде элементов плитки в двух режимах в зависимости от опции {@link Controls/_tile/interface/ITile#orientation orientation}:
 *
 * 1. Плитка
 * По умолчанию или при значении опции {@link Controls/_tile/interface/ITile#orientation orientation}="vertical"
 * * Элементы плитки отображаются в несколько строк с вертикальной прокруткой.
 * * Контейнер фиксированной ширины заполняется элементами плитки по ширине в одну строку, остальные элементы переносятся на следующую строку и т.д.
 * * Для вертикальной прокрутки необходимо обернуть контрол в {@link Controls/scroll:Container Контейнер скролла}.
 *
 * 2. Горизонтальная плитка
 * При значении опции {@link Controls/_tile/interface/ITile#orientation orientation}="horizontal"
 * * Элементы плитки отображаются в одну строку с горизонтальной прокруткой, перенос на другие строки не происходит.
 * * Для горизонтальной прокрутки необходимо обернуть контрол в {@link Controls/scroll/Container/Property/scrollOrientation Контейнер горизонтального скролла}.
 *
 * Если необходимо отобразить данные в несколько строк с горизонтальной прокруткой, используйте контрол {@link Controls/adaptiveTile:View "Адаптивная плитка"}.
 *
 * @remark
 * Полезные ссылки:
 ...
 * * {@link https://git.sbis.ru/saby/wasaby-controls/-/blob/rc-24.6100/Controls-default-theme/variables/_tile.less переменные тем оформления tile}
 ...
 * @implements Controls/tile:ITileContextMenuOptions
 ...
 *
 * @public
 * @see Controls/adaptiveTile:View "Адаптивная плитка"
 */
...
export default class View extends List {
    ...
    protected _beforeMount(
        ...
        resolveViewControls(this, options, this._viewName);
 ...
 * @name Controls/_tile/View#contextMenuConfig
 * @cfg {Controls/_tile/interface/IContextTileMenuConfig}
 * @remark
