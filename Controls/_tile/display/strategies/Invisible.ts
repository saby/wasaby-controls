...
> implements IItemsStrategy<S, T>
{
    ...
    protected _removeInvisibleItems(removedItems: T[]): void {
        removedItems.forEach((item) => {
            const index = this._invisibleItems.findIndex((invisibleItem: InvisibleTileItem) => {
    ...
    protected _getItems(): T[] {
        return (this._invisibleItems as undefined as T[]).concat(this.source.items);
    }
    ...
    >(item: T, invisibleItems): boolean {
        const index = invisibleItems.findIndex((invisibleItem: InvisibleTileItem) => {
            return invisibleItem.contents === item.contents;
        });
        return index === -1;
    }
