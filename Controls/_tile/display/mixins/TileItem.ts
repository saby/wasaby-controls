...
export interface IOptions<S extends Model = Model>
    extends ICollectionItemOptions<S> {
    tileSize: TTileSize;
    tileMode: TTileMode;
...
export default abstract class TileItem<T extends Model = Model> {
    ...
    getTileWidth(
        ...
        const itemWidth =
            widthTpl ||
            object.getPropertyValue<number>(
                this.getContents(),
        ...
        if (this.getTileMode() === 'dynamic') {
            if (imageHeight && imageWidth) {
                ...
                return (
                    this.getTileHeight() *
                    (tileFit || DEFAULT_COMPRESSION_COEFF)
                );
            }
        } else {
            return itemWidth;
    ...
    getTileSizes(
        ...
        const tileSizes: ITileSize =
            sizeParams[isVertical ? 'vertical' : 'horizontal'];
        if (isVertical) {
    ...
    shouldDisplayAutoResizer({
        ...
        if (itemType === 'rich') {
            const isVertical =
                imagePosition === 'top' || imagePosition === 'bottom';
            return (
                isVertical && imageViewMode === 'rectangle' && !!imageProportion
            );
        } else {
    ...
    getAutoResizerStyles({
        ...
        if (itemType === 'rich') {
            ...
            const tileProportion =
                this.getTileHeight() /
                this.getTileWidth(width, imagePosition, imageViewMode);
    ...
    getAutoResizerClasses({
        ...
        return this.getTileMode() !== 'dynamic' &&
            !staticHeight &&
            hasTitle &&
            position === 'image'
    ...
    isUnscaleable(): boolean {
        return (
            !this.getTileScalingMode() || this.getTileScalingMode() === 'none'
        );
    }
    ...
    getTileMarkerClasses(
        ...
        const size =
            markerSize || (itemType === 'small' ? 'content-xs' : 'content-xl');
    ...
    getItemActionsClasses(
        ...
        switch (itemType) {
            ...
            case 'small':
                ...
                classes +=
                    templateItemActionsClass ||
                    ' controls-TreeTileView__itemActions_center';
                break;
            case 'medium':
    ...
    getDefaultImageFit(itemType: TTileItem): TImageFit {
        return itemType === 'rich'
            ? DEFAULT_RICH_ITEM_IMAGE_FIT
            : DEFAULT_ITEM_IMAGE_FIT;
    }
    ...
    getImageHeight(): number {
        return object.getPropertyValue<number>(
            this.getContents(),
            this.getImageHeightProperty()
        );
    }
    ...
    getImageClasses(
        ...
        switch (itemType) {
            ...
            case 'rich':
                ...
                classes += getImageClasses(
                    this.getImageFit(imageFit) ||
                        this.getDefaultImageFit(itemType)
                );
    ...
    getImageWrapperStyles({
        ...
        if (itemType === 'rich') {
            ...
            if (
                ...
                styles.backgroundColor = `${rgbaToString(
                    toRgb(gradientColor)
                )}`;
            }
        }
    ...
    getImagePreserveAspectRatio(
        ...
        switch (itemType) {
            ...
            case 'rich':
                return `xMidYMid ${
                    this.getImageFit(imageFit) === 'cover' ? 'slice' : 'meet'
                }`;
        }
    }
        ...
        switch (itemType) {
            ...
            case 'preview':
                ...
                if (
                    ...
                    (this.isUnscaleable() || this.canShowActions())
                ) {
                    classes +=
                        ' controls-TileView__previewTemplate_actions_showed';
                }
                if (hasTitle === false) {
                    classes +=
                        ' controls-TileView__previewTemplate_withoutTitle';
                }
                break;
            case 'small':
                ...
                if (highlightOnHover) {
                    classes +=
                        ' controls-TileView__smallTemplate_item_highlightOnHover';
                }
                classes +=
    ...
    getItemStyles({
        ...
        if (typeof width === 'string') {
            ...
            widthCalculated = this.getTileWidth(
                width,
                imagePosition,
                imageViewMode
            );
            if (
                Number.parseFloat(widthCalculated).toString() ===
                widthCalculated.toString()
            ) {
                flexBasis =
                    widthCalculated * this.getCompressionCoefficient() + 'px';
                widthCalculated += 'px';
            } else {
        ...
        return {
            ...
            height:
                staticHeight && itemType !== 'invisible'
                    ? `${this.getTileHeight()}px`
                    : undefined,
            ...styleProp,
        };
    }
    ...
    getWrapperClasses({
        ...
        classes +=
            ' controls-TileView__itemContent js-controls-ListView__measurableContainer';
        classes += this.getFadedClass();
    ...
    shouldDisplayTitle(
        ...
        switch (itemType) {
            case 'default':
                return (
                    !!this.getDisplayValue() || this.shouldDisplayItemActions()
                );
            case 'small':
            case 'medium':
            case 'rich':
                return (
                    titlePosition === 'underImage' || imageViewMode === 'none'
                );
            case 'preview':
        ...
        switch (itemType) {
            ...
            case 'rich':
                ...
                if (contentPosition !== 'underImage') {
                    ...
                    classes += this._getContentSpacingClasses(
                        imagePosition,
                        contentPadding
                    );
                    break;
    ...
    private _getContentSpacingClasses(
        ...
        const contentPaddingTop =
            imagePosition !== 'top' ? contentPadding : 'default';
        const contentPaddingRight =
            imagePosition !== 'right' ? contentPadding : 'default';
        const contentPaddingBottom =
            imagePosition !== 'bottom' ? contentPadding : 'default';
        const contentPaddingLeft =
            imagePosition !== 'left' ? contentPadding : 'default';
    ...
    getTitleClasses({
        ...
        switch (itemType) {
            ...
            case 'medium':
                classes +=
                    ' controls-TileView__mediumTemplate_title controls-fontweight-bold';
                classes += ' controls-fontsize-l controls-text-secondary';
                classes += ' controls-TileView__mediumTemplate_title';
                break;
            case 'rich':
                classes +=
                    ' controls-TileView__richTemplate_title controls-fontweight-bold';
                ...
                if (
                    ...
                    if (rgbColor) {
                        styles.background = `linear-gradient(to right, ${rgbaToString(
                            rgbToRgba(rgbColor, 0)
                        )} 0%, ${rgbaToString(rgbColor)} 12px, ${rgbaToString(
                            rgbColor
                        )} 100%)`;
                    } else {
    ...
    getDescriptionStyles({
        ...
        switch (itemType) {
            ...
            case 'rich':
                ...
                styles.color =
                    contentPosition !== 'underImage' ? '#fff' : textColor;
                break;
        }
    ...
    abstract getMarkerClasses(
        markerSize: TMarkerSize,
        addVerticalPaddings?: boolean
    ): string;
