...
> {
                ...
                if (this._$orientation === 'horizontal') {
                    this.appendStrategy(AdditionalTileStrategy, {
                        ...
                        usingCustomItemTemplates:
                            this._$usingCustomItemTemplates,
    ...
    getZoomCoefficient(): number {
        if (
            this._$tileScalingMode !== 'none' &&
            this._$tileScalingMode !== 'overlap'
        ) {
    ...
    getItemsContainerTopPadding(): string {
        if (
            !AVAILABLE_CONTAINER_VERTICAL_PADDINGS.includes(
                this._$itemsContainerPadding?.top
            )
        ) {
            return 'default';
        }
    ...
    getItemsContainerBottomPadding(): string {
        if (
            !AVAILABLE_CONTAINER_VERTICAL_PADDINGS.includes(
                this._$itemsContainerPadding?.bottom
            )
        ) {
            return 'default';
        }
    ...
    getItemsContainerLeftPadding(): string {
        if (
            !AVAILABLE_CONTAINER_HORIZONTAL_PADDINGS.includes(
                this._$itemsContainerPadding?.left
            )
        ) {
            return 'default';
        }
    ...
    getItemsContainerRightPadding(): string {
        if (
            !AVAILABLE_CONTAINER_HORIZONTAL_PADDINGS.includes(
                this._$itemsContainerPadding?.right
            )
        ) {
            return 'default';
        }
    ...
    getItemContainerPosition(
        ...
        const additionalHeight =
            (targetItemSize.height - itemRect.height - additionalHeightBottom) /
            2;

        const leftOffset =
            itemRect.left - viewContainerRect.left - additionalWidth;
        const topOffset =
            itemRect.top - viewContainerRect.top - additionalHeight;
        const rightOffset =
            viewContainerRect.right - itemRect.right - additionalWidth;
        const bottomOffset =
     ...
     * @protected
     */
    protected _getItemsFactoryParams(
        params: ITileItemOptions<S>
    ): ITileItemOptions<S> {
        params.tileMode = this.getTileMode();
        params.tileScalingMode = this.getTileScalingMode();
        params.canShowActions =
            this.getTileScalingMode() === 'none' &&
            !this.getActionsDisplayDelay();
    ...
    protected _updateRenderedItemsOutsideVirtualRange(
        ...
        this.each((item, index) => {
            ...
            if (
                insideRange ||
                !item['[Controls/_tile/display/mixins/InvisibleItem]']
            ) {
    ...
    abstract removeStrategy(
        strategy: new () => IItemsStrategy<S, T>,
        withSession?: boolean
    ): void;
    abstract reCreateStrategy(
        strategy: new () => IItemsStrategy<S, T>,
        options?: object
    ): void;
    ...
    protected abstract _finishUpdateSession(
        session: ISession,
        analyze?: boolean
    ): void;
