...
export type { TImageFit, TImageUrlResolver, TTileMode, TTileSize, TTileScalingMode };
...
export default abstract class Tile<S extends Model = Model, T extends TileItem = TileItem> {
    ...
    protected _$emptyRender: React.ReactElement; // Перевод плитки на React

    protected _$footerRender: React.ReactElement; // Перевод плитки на React

    protected _$emptyRenderProps: object; // Перевод плитки на React

    protected _$footerRenderProps: object; // Перевод плитки на React
    ...
    /**
     * Возвращает шаблон пустого представления, заданный через опцию emptyRender
     * @return {React.ReactElement}
     */
    getEmptyRender(): React.ReactElement | null {
        return this._$emptyRender;
    }

    /**
     * Возвращает шаблон футера, заданный через опцию footerRender
     * @return {React.ReactElement}
     */
    getFooterRender(): React.ReactElement | null {
        return this._$footerRender;
    }

    /**
     * Возвращает опции шаблона, для emptyRender
     * @return {React.ReactElement}
     */
    getEmptyRenderProps(): object | null {
        return this._$emptyRenderProps ?? null;
    }

    /**
     * Возвращает опции шаблона, для footerRender
     * @return {React.ReactElement}
     */
    getFooterRenderProps(): object | null {
        return this._$footerRenderProps ?? null;
    }
    ...
    getItemContainerPositionInDocument(
        ...
        const left = targetItemPosition.left + viewContainerRect.left - containerRect.left;
        const top = targetItemPosition.top + viewContainerRect.top - containerRect.top;
        const right =
            targetItemPosition.right +
            (documentRect.width - viewContainerRect.right) -
            (documentRect.width - containerRect.right);
        const bottom =
            targetItemPosition.bottom +
            (documentRect.height - viewContainerRect.bottom) -
            (documentRect.height - containerRect.bottom);
...
Object.assign(Tile.prototype, {
    ...
    _$imageFit: null,
    _$footerRender: null,
    _$footerRenderProps: null,
    _$emptyRender: null,
    _$emptyRenderProps: null,
