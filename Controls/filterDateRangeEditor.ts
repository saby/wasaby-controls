...
import { IFilterDescriptionItem, IPeriodsConfig, IUserPeriod } from 'Controls/filter';
...
export interface IDateRangeEditorOptions
    extends IControlOptions,
        Omit<IFilterDescriptionItem<Date | [Date | null, Date | null]>, 'name'>,
        ICaptionOptions,
        IFontColorStyleOptions,
        IFontWeightOptions,
        IResetValuesOptions {
    ...
    validateValueBeforeChange?: boolean;
...
class DateRangeEditor extends Control<IDateRangeEditorOptions> {
    ...
    private getCaption(
        ...
        if (options.captionFormatter) {
            return options.captionFormatter(startValue, endValue, emptyCaption);
        } else {
            ...
            return dateRangeFormatter(startValue, endValue, {
                hiddenYearIfCurrentYear: !options.showCurrentYear,
            });
        }
    }
    ...
    private _notifyChanges(startValue: Date, endValue?: Date): void {
        ...
        const textValue = this.getCaption(startValue, isDateRange ? endValue : startValue);

        this._notify('textValueChanged', [textValue]);

        const valueIsReset = isEqual(
            isDateRange ? [startValue, endValue] : startValue,
            this._options.resetValue
        );

        let value;
        if ((!startValue && !endValue && this._options.resetValue) || valueIsReset) {
            value = isDateRange ? this._options.resetValue : [this._options.resetValue];
        } else {
...
/**
 * @name Controls/filterDateRangeEditor#showCurrentYear
 * @cfg {boolean} Показывать год в строке выбранных фильтров, если выбран текущий календарный год.
 * @example
 * Пример: в календаре выбрали Июль 2024
 * Если передать showCurrentYear: true, в строке выбранных фильтров отобразиться Июль'24
 * Если передать false, то просто Июль
 * @default false
 * @demo Controls-ListEnv-demo/Filter/NotConnectedView/Editors/DateRange/Index
 * @remark По умолчанию текущий календарный год отображаться не должен, но в редких случаях, когда этого
 * требует бизнес-процесс (например в отчётах), год можно показать.
 */

/**
 * @event Controls/filterDateRangeEditor#textValueChanged Происходит при изменении выбранного значения.
