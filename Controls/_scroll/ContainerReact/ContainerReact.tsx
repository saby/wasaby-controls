...
function Shadows(props): React.ReactElement {
    return (<>
        {props.shadows.top?.isEnabled &&
            !props.isOptimizeShadowEnabled && (
                <div
                    className={`controls-Scroll__shadow ${props.getShadowClasses(
                        'horizontal',
                        'top'
                    )} ${
                        !props.shadows.top.isVisible
                            ? 'ws-invisible'
                            : ''
                    }`}
        ...
        {props.shadows.bottom?.isEnabled &&
            !props.isOptimizeShadowEnabled && (
                <div
                    className={`controls-Scroll__shadow ${props.getShadowClasses(
                        'horizontal',
                        ...
                        !props.shadows.bottom.isVisible
                            ? 'ws-invisible'
                            : ''
                    }`}
        ...
        {props.shadows.left?.isEnabled &&
            props.shadowMode !== 'blur' && (
                <div
                    className={`controls-Scroll__shadow ${props.getShadowClasses(
                        'vertical',
                        'left'
                    )} ${
                        !props.shadows.left.isVisible
                            ? 'ws-invisible'
                            : ''
                    }`}
    ...
    return (<>
        {props.arrowButtons && props.arrowButtons.isTopVisible && (
            <ArrowButton
                ...
                direction={'up'}
                className={props.buttonsMode === 'hover' ? 'controls-scroll_scrollButton__hovered' : ''}
                onClick={props.handleArrowButton}
            />
        )}
        {props.arrowButtons &&
            props.arrowButtons.isBottomVisible && (
                <ArrowButton
                    orientation="vertical"
                    direction={'down'}
                    className={props.buttonsMode === 'hover' ? 'controls-scroll_scrollButton__hovered' : ''}
        ...
        {props.arrowButtons &&
            props.arrowButtons.isRightVisible && (
                <ArrowButton
                    orientation="horizontal"
                    direction={'next'}
                    className={props.buttonsMode === 'hover' ? 'controls-scroll_scrollButton__hovered' : ''}
...
function Scrollbars(props, style: IScrollbarStyle): React.ReactElement {
    return props.scrollbars?.isVisible && (
            <>
                {props.scrollbars.vertical?.isVisible && (
                    <Async
                        ...
                        templateOptions={{
                            ...
                            position:
                            props.scrollbars.vertical.position,
                            contentSize:
                            props.scrollbars.vertical
                                .contentSize,
                            paddings:
                            props.verticalScrollbarPaddings,
                            onPositionchanged:
                            props.scrollbarPositionChangedHandler,
                            onDraggingchanged:
                            props.scrollbarDraggingChangedHandler,
                        }}
                    />
                )}
                ...
                {props.scrollbars.horizontal?.isVisible && (
                    <Async
                        ...
                        templateOptions={{
                            ...
                            paddings:
                            props.horizontalScrollbarPaddings,
...
function ContainerReact(props, ref): React.ReactElement {
    ...
    const [_, forceUpdate] = React.useReducer((x) => {return x + 1;}, 0);
    ...
    const updateContext = (stickyModels, groupModels, scrollModel, needRerender = true) => {
        ...
        if (JSON.stringify(stickyGroupModelsContext.current.scrollState) === JSON.stringify(newScrollState)) {
