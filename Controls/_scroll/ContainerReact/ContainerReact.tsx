...
import { TArrowButtonInlineHeight, TArrowButtonViewMode } from 'Controls/extButtons';
import { StickyContext, _StickyGroupContext } from 'Controls/stickyBlock';
...
interface AsyncButtonTemplateProps {
    buttonsSize: TArrowButtonInlineHeight;
    buttonsViewMode: TArrowButtonViewMode;
    direction: 'up' | 'down' | 'left' | 'right';
    buttonsMode: 'hover' | 'always';
    handleArrowButton: (type: string, orientation: string) => void;
}

function AsyncButtonTemplate(props: AsyncButtonTemplateProps) {
    ...
    const buttonsSize = props.buttonsSize;
    const buttonsViewMode = props.buttonsViewMode;

    const getArrowClassName = () => {
        return (
            'controls-ArrowButton_stretched_scroll' +
            ` controls-ArrowButton_stretched_scroll-${orientation}` +
            ` controls-ArrowButton_stretched_scroll_direction-${direction}` +
            ` controls-ArrowButton_stretched_scroll_offset-${buttonsSize}` +
    ...
    return (
        <Async
            ...
            templateOptions={{
                direction,
                viewMode: buttonsViewMode,
                translucent: 'dark',
                inlineHeight: buttonsSize,
                className: getArrowClassName(),
                'data-qa': `controls_Scroll__ArrowButton_direction_${
                    orientation === 'horizontal' ? type : direction
                }`,
    ...
    return (
        <>
            {props.arrowButtons && props.arrowButtons.isTopVisible && (
                <AsyncButtonTemplate
                    handleArrowButton={props.handleArrowButton}
                    buttonsMode={props.buttonsMode}
                    buttonsViewMode={props.buttonsViewMode}
...
function ContainerReact(props, ref): React.ReactElement {
    ...
    if (props.scrollbarOffsetTop) {
        verticalScrollbarStyles.top = props.scrollbarOffsetTop;
    }
    return (
        <StickyContext.Provider value={stickyModelsContext.current}>
