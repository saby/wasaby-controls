...
export default class StickyHeaderResizeObserver {
    ...
    register(
        ...
        if (
            intersectionObserverObject.observerName !==
            this._options.observerName
        ) {
            return;
        }
        ...
        const item: IIntersectionObserverObject = {
            ...
            threshold:
                intersectionObserverObject.threshold || this._options.threshold,
            rootMargin:
                intersectionObserverObject.rootMargin ||
                this._options.rootMargin,
        };
    ...
    private _getObserverKey(threshold: number[], rootMargin: string): string {
        return `t${threshold.join('_')}_rm${rootMargin.replace(
            REPLACE_SPACE_IN_KEY_REGEXP,
            '_'
        )}`;
    }
    ...
    private _intersectionObserverHandler(
        ...
        for (const entry of entries) {
            ...
            if (itemId) {
                const eventEntry = new SyntheticEntry(
                    entry,
