...
> extends Control<IContainerBaseOptions> {
    ...
    _afterMount(): void {
        ...
        this._notify(
            'register',
            ['documentDragStart', this, this._onDragStart],
            { bubbling: true }
        );
        this._notify('register', ['documentDragEnd', this, this._onDragEnd], {
            bubbling: true,
        });
        this._hasOuterContainer = !!this._container.closest(
    ...
    private _initialScrollPositionInit(options: IContainerBaseOptions): void {
        if (options.initialScrollPosition?.vertical === SCROLL_POSITION.END) {
            this._initialScrollPositionCssClass =
                SCROLL_POSITION_CSS.verticalEnd;
        } else if (
            options.initialScrollPosition?.horizontal === SCROLL_POSITION.END
        ) {
            this._initialScrollPositionCssClass =
                SCROLL_POSITION_CSS.horizontalEnd;
        } else {
    ...
    private _initialScrollPositionResetAfterInitialization(): void {
        if (
            this._options.initialScrollPosition?.vertical ===
                SCROLL_POSITION.END ||
            this._options.initialScrollPosition?.horizontal ===
                SCROLL_POSITION.END
        ) {
    ...
    _unobserveDeleted(): void {
        const contentElements: HTMLElement[] =
            this._getElementsForHeightCalculation();
    ...
    _registerIt(
        ...
        switch (registerType) {
            ...
            case 'listScroll':
                ...
                this._registrars.listScroll.register(
                    event,
                    registerType,
                    component,
                    callback
            ...
            case 'virtualScrollMove':
                ...
                break;
            case 'customscroll':
                this._registrars.customscroll.register(
                    event,
                    registerType,
                    component,
                    callback,
            ...
            case 'virtualNavigation':
                this._virtualNavigationRegistrar.register(
                    event,
                    registerType,
                    component,
                    callback
                ...
                break;
            case 'viewportResize':
                this._registrars.viewportResize.register(
                    event,
                    registerType,
                    component,
                    callback
    ...
    _unRegisterIt(
        ...
        switch (registerType) {
            case 'scrollStateChanged':
                this._registrars.scrollStateChanged.unregister(
                    event,
                    registerType,
                    component
                );
                break;
            case 'listScroll':
                this._registrars.listScroll.unregister(
                    event,
                    registerType,
                    component
                );
                break;
            case 'virtualScrollMove':
                this._registrars.virtualScrollMove.unregister(
                    event,
                    registerType,
                    component
                );
                break;
            case 'customscroll':
                this._registrars.customscroll.unregister(
                    event,
                    registerType,
                    component,
                    { listenAll: true }
                ...
                break;
            case 'virtualNavigation':
                this._virtualNavigationRegistrar.unregister(
                    event,
                    registerType,
                    component
                );
                break;
            case 'viewportResize':
                this._registrars.viewportResize.unregister(
                    event,
                    registerType,
                    component
                );
    ...
    _onRegisterNewComponent(component: Control): void {
        ...
        this._registrars.scrollStateChanged.startOnceTarget(
            component,
            scrollState,
            oldScrollState
        );
    ...
    protected _updateStateAndGenerateEvents(newState: IScrollState): void {
        ...
        if (isStateUpdated) {
            ...
            // Новое событие
            this._generateEvent('scrollStateChanged', [
                scrollState,
                oldScrollState,
            ]);
            ...
            if (entry.target === this._children.content) {
                ...
                if (entry.borderBoxSize) {
                    ...
                    for (const property of Object.keys(scrollStateProperties)) {
                        const borderBoxSizeProperty =
                            scrollStateProperties[property];
                        newState[property] =
    ...
    _getContentType(): CONTENT_TYPE {
        ...
        const firstContentChild: HTMLElement =
            this._children.userContent.children?.[0];
        if (firstContentChild) {
            const classList = firstContentChild.classList;
            if (
                classList.contains('controls-Scroll-Container__notScrollable')
            ) {
    ...
    _getFullStateFromDOM(): IScrollState {
        ...
        const containerRect: DOMRect =
            this._children.content.getBoundingClientRect();
        ...
        return {
            ...
            verticalPosition:
                this._children.content.scrollTop === 0 ? 'start' : 'middle',
            horizontalPosition:
                this._children.content.scrollLeft === 0 ? 'start' : 'middle',
        };
    }
    ...
    private _getMobileClientHeight(currentHeight: number): number {
        // @ts-ignore
        if (this.props.adaptiveMode?.device.isPhone() && currentHeight > window.screen.height) {
    ...
    private _getContentHeight(): number {
        const contentNodes = Array.from(
            this._children.userContent.querySelectorAll<HTMLElement[]>(
                `.${USER_CONTENT_CLASS}`
            )
    ...
    protected _doHorizontalScrollHandler(
        ...
        if (typeof scrollParam === 'string') {
            ...
            const getInRange = (value: number) => {
                return Math.max(
                    Math.min(value, this._scrollModel.scrollWidth),
                    0
                );
            };
    ...
    protected _doScroll(
        ...
        if (scrollParam === 'top') {
            ...
            const scrollHeight =
                ...
                (this._isVirtualPlaceholderMode()
                    ? this._topPlaceholderSize
                    : 0);
    ...
    _scrollToElementByOrientation(
        ...
        const scrollMethod =
            orientation === SCROLL_DIRECTION.VERTICAL
                ? scrollToElement
                : horizontalScrollToElement;
    ...
    private _onDragStart(dragObject: {
        entity: Entity;
        domEvent: MouseEvent;
    }): void {
    ...
    protected _onMouseMove(event: SyntheticEvent): void {
        ...
        const verticalScrollAvailable =
            ...
            this._options.scrollOrientation !== 'verticalHorizontal';
        this._dnDAutoScroll.onMouseMove(
            event,
    ...
    updatePlaceholdersSize(placeholdersSizes: {
        ...
        this._topPlaceholderSize = getValue(
            placeholdersSizes.top,
            this._topPlaceholderSize
        );
        ...
        this._leftPlaceholderSize = getValue(
            placeholdersSizes.left,
            this._leftPlaceholderSize
        );
        this._rightPlaceholderSize = getValue(
            placeholdersSizes.right,
            this._rightPlaceholderSize
        );
    }
    ...
    private _scrollTo(
        ...
        if (smooth) {
            scrollOrientation =
                direction === SCROLL_DIRECTION.VERTICAL ? 'top' : 'left';
    ...
    protected _setScrollTop(
        ...
        if (this._isVirtualPlaceholderMode() && !withoutPlaceholder) {
            ...
            const newScrollTop = this._scrollTo(
                scrollTop,
                SCROLL_DIRECTION.VERTICAL,
                smooth
            );
    ...
    _updatePlaceholdersSize(
        e: SyntheticEvent<Event>,
        placeholdersSizes: object
    ): void {
    ...
    private _logSizes(state: IScrollState, oldState: IScrollState): void {
        if (ContainerBase._debug) {
            ...
            if (msg) {
                Logger.warn(
                    `Controls/scroll:ContainerBase: изменение размеров. ${msg}`,
                    this
                );
            }
        }
