...
import { RegisterClass, RegisterUtil, UnregisterUtil, IListenerOptions } from 'Controls/event';
...
import { OverscrollBehavior, SCROLL_MODE } from './Container/Type';
import { EventUtils } from 'UI/Events';
import {
    getStickyHeadersHeight as getHeadersHeight,
    getStickyHeadersWidth as getHeadersWidth,
    isStickyHidden as isHidden,
    StickyPosition,
    TypeFixedBlocks,
} from 'Controls/stickyBlock';
import { getStore, location } from 'Application/Env';
...
import { IListScrollContextState } from './Contexts/ListScrollContextProvider';
 ...
 *
 */
export default class ContainerBase<T extends IContainerBaseOptions> extends Control<T> {
    ...
    protected _isClipScrollContainer: boolean = false;
    ...
    // Отправка события списку через контекст. Так как на чистом react нет возможности легально делать подписки через register
    protected _contextEvent: IListScrollContextState['contextEvent'] | null = null;
    ...
    _beforeMount(options: T): void {
        this._virtualNavigationRegistrar = new RegisterClass({
            register: 'virtualNavigation',
        });
        if (
            !this._isHorizontalScroll(options.scrollOrientation) ||
            options.forceInitResizeObserver
        ) {
    ...
    protected _$react_componentDidMount(): void {
        ...
        // делать это надо в отдельной таске, т.к.есть нюанс с порядком вызова _notify в слоях react/wasaby
    ...
    protected _setIsClipScrollContainer(): void {
        if (this._container) {
            this._isClipScrollContainer =
                detection.isPhone &&
                this.context.isScrollOnBody &&
                !this._container.closest('.controls-Popup') &&
                this._container.offsetHeight > window.innerHeight;
        }
    }
    ...
    _afterMount(): void {
        ...
        this._setIsClipScrollContainer();
    }

    _beforeUpdate(options: T): void {
    ...
    protected _afterUpdate(oldOptions?: T): void {
        this._updateContentType();
        if (this._contentType !== CONTENT_TYPE.regular) {
            this._observeContentSize();
    ...
    private _initialScrollPositionInit(options: T): void {
    ...
    private _canScrollByWheel(direction: SCROLL_DIRECTION, options: T = this._options): boolean {
    ...
    registerEvent(...args: [SyntheticEvent, string, Control, Function, IListenerOptions]): void {
        this._registerIt(...args);
    }

    unregisterEvent(...args: [SyntheticEvent, string, Control, IListenerOptions]): void {
        this._unRegisterIt(...args);
    }
    ...
    scrollTo(
        ...
        if (detection.isMobileIOS) {
            ...
            // Принудительно вызываем force reflow, чтобы убедиться, что ничего не помешает восстановлению скрола
    ...
    scrollToLeft(smooth: boolean = false) {
        this.scrollTo(0, SCROLL_DIRECTION.HORIZONTAL, smooth);
    }
    ...
    _onResizeContainer(newState: IScrollState): void {
        if (this._resizeObserverSupported) {
            ...
            if (newState.clientHeight || newState.clientWidth) {
                this._updateStateAndGenerateEvents(this._getFullStateFromDOM());
            }
        }
    }
    ...
    protected _updateStateAndGenerateEvents(newState: IScrollState): void {
        ...
        if (isStateUpdated) {
            ...
            // Новое событие
            scrollState.isCustomScroll = this._isCustomScroll;
            ...
            if (
                oldScrollState.scrollTop !== scrollState.scrollTop ||
    ...
    _sendViaContext(event: { type: string; params: object }): void {
    ...
    _getFullStateFromDOM(): IScrollState {
        ...
        return {
            ...
            horizontalPosition: SCROLL_POSITION.START,
        };
    }
    ...
    private _doScrollBaseHandler(
        ...
        // Если вызвать scrollTo с флагом smooth после другого подскролла
        // то плавный скролл отработает от предыдущей позиции и недоскроллит до нужного места
        // Поэтому перед восстановлением выключаем overflow scrolling.
        if (detection.isMobileIOS) {
            this._setOverflowScrolling('hidden');
            // Принудительно вызываем force reflow, чтобы убедиться, что ничего не помешает восстановлению скрола
            this._container.getBoundingClientRect();
    ...
    protected _getScrollContainerCssClass(options: T): string {
    ...
    protected _onTouchMove(sEvent: SyntheticEvent): void {
        let isOverElement = false;
        const event = sEvent.nativeEvent;
        // Нет стандартного события touchleave, поэтому самостоятельно смотрим, где палец пользователя
        // и определяем вышел он за пределы элемента или нет
        for (let i = 0; i < event.touches.length; i++) {
            const touch = event.touches[i];
            const target = document.elementFromPoint(touch.clientX, touch.clientY);
            if (target === this._container || this._container.contains(target)) {
                isOverElement = true;
                break;
            }
        }
    ...
    protected _setScrollTop(
        ...
        if (this._isVirtualPlaceholderMode() && !withoutPlaceholder) {
            ...
            if (realScrollTop >= 0 && !scrollTopOverflow) {
                ...
                this._contextEvent = {
                    type: 'virtualScrollMove',
                    params: {
                        scrollTop,
                        scrollState: scrollStateClone,
                        callback: applyScrollTop,
                    },
                };
    ...
    private _setOverflowScrolling(value: string): void {
        if (this._children.content) {
            ...
            this._children.content.classList[value === 'hidden' ? 'add' : 'remove'](
