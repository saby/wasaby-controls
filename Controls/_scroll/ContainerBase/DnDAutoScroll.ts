...
export default class DnDAutoScroll {
    ...
    onDragStart(dragObject: { entity: Entity; domEvent: MouseEvent }): void {
        ...
        // Если устройство мобильное, то нужно зарегистировать подписку на touchMove, так как он работает не так как mousemove
        // Отличиет заключается в том, что touchmove не будет стрелять если перемещать палец между элементами с подпиской,
        // а будет стрелять только на элементе, где начался touchmove
        if (this._requiredAutoScroll && detection.isMobileIOS) {
