...
export function getScrollPositionType(
    ...
    if (scrollPosition <= 0) {
        ...
        scrollPosition + viewportSize + SCALE_ROUNDING_ERROR_FIX >=
        contentSize
    ) {
...
export function isCursorAtScrollPoint(
    ...
    const inContainer =
        mainAxis.cursor > mainAxis.start && mainAxis.cursor < mainAxis.end;

    const inScrollLine =
        additionalAxis.cursor < additionalAxis.end &&
        additionalAxis.cursor > additionalAxis.start;
    ...
    const nearStart =
        ...
        mainAxis.cursor < mainAxis.start + edge;
    // Определяем находится ли курсор у конечной границы элемента
    const nearEnd =
        mainAxis.cursor < mainAxis.end && mainAxis.cursor > mainAxis.end - edge;

    // Расстояние до начальной границы контейнера (курсор внутри контейнера)
    const coordsToStart =
        mainAxis.cursor - mainAxis.start < edge &&
        mainAxis.cursor - mainAxis.start > 0
            ? mainAxis.cursor - mainAxis.start
            : undefined;
    // Расстояние до конечной границы контейнера (курсор внутри контейнера)
    const coordsToEnd =
        mainAxis.end - mainAxis.cursor < edge &&
        mainAxis.end - mainAxis.cursor > 0
