...
export function getScrollContainerByElement(scrollableElement: HTMLElement): Container {
        ...
        if (force || parentOffset[props.endPosition] < elemOffset[props.endPosition]) {
            if (position === SCROLL_POSITION.center) {
                ...
                if (
                    ...
                    targetPosition = elemToScroll[props.scroll] + elementPosition;
                    ...
                    if (
                        ...
                        const stickyHeight = Math.round(
                            scrollContainer.getBlocksHeightByScrollTop(
                                elementPosition > 0 ? elementPosition : targetPosition
                            )
                        );
...
/**
 * Позволяет определить целевое положение элемента, к которому произойдет скроллирование.
 * @param {HTMLElement} element DOM-элемент, к которому произойдет скроллирование.
 * @param {SCROLL_DIRECTION} direction Направление, в котором будет осуществляться скролл
 * @param {HTMLElement} scrollElement DOM-элемент, в котором произойдет скроллирование. Если параметр не передан, то расчет будет осуществляться для первого найденного скрола
 */
export function getScrollPosition(
    element: HTMLElement,
    direction: SCROLL_DIRECTION = SCROLL_DIRECTION.VERTICAL,
    scrollElement: HTMLElement
): Exclude<SCROLL_POSITION, SCROLL_POSITION.center> {
    const scrollableParent = scrollElement ? [scrollElement] : getScrollableParents(element, false, direction, true);
    const startPositionName =
        direction === SCROLL_DIRECTION.VERTICAL ? SCROLL_POSITION.top : SCROLL_POSITION.left;
    if (scrollableParent.length) {
        const elementClientRect = getDimensions(element);
        const scrollClientRect = getDimensions(scrollableParent[0]);
        const endPositionName =
            direction === SCROLL_DIRECTION.VERTICAL
                ? SCROLL_POSITION.bottom
                : SCROLL_POSITION.right;

        if (elementClientRect[startPositionName] - scrollClientRect[startPositionName] < 0) {
            return startPositionName;
        } else if (elementClientRect[endPositionName] > scrollClientRect[endPositionName]) {
            return endPositionName;
        } else {
            const sizeName = direction === SCROLL_DIRECTION.VERTICAL ? 'height' : 'width';
            const middle = scrollClientRect[sizeName] / 2;
            return middle >=
                elementClientRect[startPositionName] - scrollClientRect[startPositionName]
                ? startPositionName
                : endPositionName;
        }
    }
