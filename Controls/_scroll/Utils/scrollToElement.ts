...
const SCROLL_CONTAINERS_SELECTOR =
    '.controls-Scroll, .controls-Scroll-Container';
...
type TPropertiesNames<T extends SCROLL_DIRECTION> =
    T extends SCROLL_DIRECTION.VERTICAL
        ? {
...
function getPropertiesNames<T extends SCROLL_DIRECTION>(
    scrollDirection: T
): TPropertiesNames<T> {
    return (
...
function getStickyElementOffset(stickyElement: HTMLElement): IOffset {
    ...
    const windowDimensions =
        DimensionsMeasurer.getWindowDimensions(stickyElement);
...
function getOffset(element: HTMLElement): IOffset {
    if (element === document.body || element === document.documentElement) {
        const elementDimensions =
            DimensionsMeasurer.getElementDimensions(element);
        ...
        const windowDimensions =
            DimensionsMeasurer.getWindowDimensions(element);
...
function getScrollContainerByElement(
    ...
    return controls.find((control: IControl) => {
        return cInstance.instanceOfModule(
            control,
            'Controls/scroll:_ContainerBase'
        );
    }) as Container;
}

type TStickyHeaderDimensions<T extends SCROLL_DIRECTION> =
    T extends SCROLL_DIRECTION.VERTICAL
        ? {
              top: number;
              bottom: number;
...
function getStickyHeaderDimensions<T extends SCROLL_DIRECTION>(
    ...
    if (
        targetElement.classList.contains('controls-StickyHeader__isolatedGroup')
    ) {
        return {
            top: 0,
            bottom: 0,
    ...
    const scrollControlNode: HTMLElement = scrollableElement.closest(
        SCROLL_CONTAINERS_SELECTOR
    );
...
function getCenterOffset(
    ...
    if (scrollDirection === SCROLL_DIRECTION.VERTICAL) {
        return (
            (getDimensions(parentElement).height -
                getDimensions(element).height) /
            2
        );
    } else {
        return (
...
function inOuterStickyElement(element: HTMLElement): boolean {
    ...
    const inMasterHeaders =
        !!element.closest('.headers-Layout') &&
        !!element.closest('.controls-MasterDetail_master');
...
function scrollTo(
        ...
        for (const parent of scrollableParent) {
            ...
            if (scrollContainer) {
                if (scrollContainer.containerLoaded === true) {
                    const headerControllerInited =
                        scrollContainer.initHeaderController();
    ...
    for (const parent of scrollableParent) {
        const elemToScroll =
            parent === document.documentElement ? document.body : parent;
        ...
        const innerStickyHeaderSize =
            stickyElement && stickyElement[props.stickyElementSize];
        ...
        if (
            force ||
            parentOffset[props.endPosition] < elemOffset[props.endPosition]
        ) {
            if (position === SCROLL_POSITION.center) {
                ...
                if (
                    ...
                    elemOffset[props.startPosition] <
                        parentOffset[props.startPosition] +
                            stickyHeaderDimensions[
                                props.stickyElementOffsetToEndEdge
                            ] ||
                    elemOffset[props.startPosition] >
                        parentOffset[props.startPosition] +
                    ...
                    if (
                        ...
                        const stickyHeight = Math.round(
                            scrollContainer.getBlocksHeightByScrollTop(
                                targetPosition
                            )
                        );
...
export function scrollToElement(
    ...
    position?:
        ...
        | Exclude<
              SCROLL_POSITION,
              SCROLL_POSITION.left | SCROLL_POSITION.right
          >,
    force?: Boolean,
    waitInitialization: boolean = false,
    offset: number = 0,
...
export function horizontalScrollToElement(
    ...
    position?: Exclude<
        SCROLL_POSITION,
        SCROLL_POSITION.top | SCROLL_POSITION.bottom
    >,
    force?: Boolean,
    waitInitialization: boolean = false,
    offset: number = 0
