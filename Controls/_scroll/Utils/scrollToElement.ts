        ...
        if (force || parentOffset[props.endPosition] < elemOffset[props.endPosition]) {
            if (position === SCROLL_POSITION.center) {
                ...
                if (
                    ...
                    const elementPosition = Math.floor(elemOffset[props.startPosition] -
                        parentOffset[props.startPosition] -
                        stickyHeaderDimensions[props.startPosition] +
                        offset);
                    ...
                    if (
                        ...
                        const stickyHeight = Math.round(
                            scrollContainer.getBlocksHeightByScrollTop(elementPosition > 0 ?
...
export function getScrollPosition(
    ...
    const scrollableParent = getScrollableParents(element, false, direction, true);
    const startPositionName = direction === SCROLL_DIRECTION.VERTICAL ? SCROLL_POSITION.top : SCROLL_POSITION.left;
    if (scrollableParent.length) {
        ...
        const endPositionName = direction === SCROLL_DIRECTION.VERTICAL
            ? SCROLL_POSITION.bottom : SCROLL_POSITION.right;
        ...
        if ((elementClientRect[startPositionName] - scrollClientRect[startPositionName]) < 0) {
            ...
            return middle >= (elementClientRect[startPositionName] - scrollClientRect[startPositionName])
                ? startPositionName : endPositionName;
        }
    }
    return startPositionName;
}
