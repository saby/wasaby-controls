    ...
    implements IVersionable
{
    ...
    updateScrollState(scrollState: IScrollState, needUpdate: boolean = true): void {
        let isChanged = false;
        for (const shadow of Object.keys(this._models)) {
            const oldVisible = this._models[shadow].isVisible;
            const isStateChanged = this._models[shadow].updateScrollState(scrollState);
            if ((isStateChanged && needUpdate) || (oldVisible !== this._models[shadow].isVisible)) {
                isChanged = true;
            }
        }
        if (isChanged) {
    ...
    updateVisibilityByInnerComponents(
        ...
        for (const shadowPosition of Object.keys(this._models)) {
            ...
            if (shadowVisibility) {
                isChanged =
                    this._models[shadowPosition].updateVisibilityByInnerComponents(
                        shadowVisibility
                    ) || isChanged;
            }
        }
