...
import { SCROLL_DIRECTION, SCROLL_POSITION, getScrollPositionByState } from './Utils/Scroll';
...
export interface IContainerOptions
    ...
    content?: ReactElement | TemplateFunction;
    onScrollStateChanged?: (scrollState: IScrollState, oldScrollState: IScrollState) => void;
}
...
export default class Container extends ContainerBase<IContainerOptions> implements IScrollMode {
    ...
    private _scrollbarOffsetTop: number = 0;
    protected _scrollPosition: string = 'start';

    protected _isScrollOnBody: boolean = false;

    protected _fixIOSShadow: boolean = false;
    ...
    _beforeMount(options: IContainerOptions): void {
        const oldIosVersion: number = 15;
        // на iso ниже 15 версии есть проблема с отображением тени, а именно если есть скругление, то тень выходит за скругление.
        this._fixIOSShadow = detection.isMobileIOS && detection.IOSVersion <= oldIosVersion;
        this._isScrollOnBody = this.context?.isScrollOnBody;
        ...
        this._intersectionObserverRegisterHandler =
            this._intersectionObserverRegisterHandler.bind(this);
        this._intersectionObserverUnregisterHandler =
            this._intersectionObserverUnregisterHandler.bind(this);
        this._updatePlaceholdersSize = this._updatePlaceholdersSize.bind(this);
        this._updateShadowVisibility = this._updateShadowVisibility.bind(this);
        ...
        this._getVerticalScrollPosition = this._getVerticalScrollPosition.bind(this);
        this._getViewportRect = this._getViewportRect.bind(this);
    ...
    private _getShadowsModelOptions(options: IContainerOptions): IShadowsOptions {
        ...
        if (options.disableScrollReaction) {
            shadowsModel.topShadowVisibility = SHADOW_VISIBILITY.HIDDEN;
            shadowsModel.bottomShadowVisibility = SHADOW_VISIBILITY.HIDDEN;
        } else {
    ...
    protected _scrollHandler(e: SyntheticEvent): void {
        ...
        if (this._scrollModel.scrollTop > 0) {
            this._scrollPosition = 'scroll';
        } else {
            this._scrollPosition = 'start';
        }
        this.initHeaderController();
    }
    ...
    _getVerticalScrollPosition() {
        return this._scrollModel ? this._scrollModel.scrollTop : 0;
    }

    _getViewportRect() {
        if (
            this._isScrollOnBody &&
            this._scrollModel?.viewPortRect.height > window?.visualViewport?.height
        ) {
            return window?.visualViewport;
        }
        return this._scrollModel ? this._scrollModel.viewPortRect : {};
    }
    ...
    _updateState(...args) {
        ...
        if (isUpdated) {
            ...
            if (!this._container?.closest?.('.controls-Popup__hidden')) {
                this._stickyController.scrollStateChanged(this._scrollModel);
            }
        }
        return isUpdated;
    ...
    protected _updateShadowVisibility(
        ...
        if (this._gridAutoShadows) {
            ...
            const newOptions = this._getShadowsModelOptions(this._options);
            if (
                newOptions.bottomShadowVisibility !== shadowsVisibility.bottom &&
                shadowsVisibility.bottom === SHADOW_VISIBILITY.VISIBLE
            ) {
                newOptions.bottomShadowVisibility = shadowsVisibility.bottom;
                this._gridAutoShadows = true;
            }
            if (
                newOptions.topShadowVisibility !== shadowsVisibility.top &&
                shadowsVisibility.top === SHADOW_VISIBILITY.VISIBLE
            ) {
                newOptions.topShadowVisibility = shadowsVisibility.top;
                this._gridAutoShadows = true;
            }
            this._shadows.updateOptions(newOptions);
        }
    ...
    private _getInitialScrollPositionStyle(options: IContainerOptions): string {
        ...
        if (typeof options.initialScrollPosition?.horizontal === 'number') {
    ...
    protected _updateStateAndGenerateEvents(newState: IScrollState): void {
        if (
            !this._isCustomScroll &&
            typeof this._scrollModel?.scrollTop !== 'undefined' &&
            typeof newState?.scrollTop !== 'undefined' &&
            this._scrollModel.scrollTop !== newState.scrollTop
        ) {
            this._stickyController.updateDynamicPosition(
                this._scrollModel.scrollTop - newState.scrollTop > 0,
                newState.scrollTop - this._scrollModel.scrollTop
            );
        }
    ...
    _stickyFixedCallback(): void {
        ...
        this._updateShadowVisibilityInStickyController();
        this._offsetTop = this._stickyController.getBlocksHeight(
            StickyPosition.Top,
    ...
    private _updateScrollbar(): void {
        ...
        let offsetTop = 0;
        const blocksSizes = this._stickyController.getBlocksSizes(
            StickyPosition.Top,
            TypeFixedBlocks.InitialFixed,
            true,
            true
        );
        if (blocksSizes.sizes.length) {
            offsetTop = blocksSizes.totalHeight;
            blocksSizes.sizes.forEach((size) => {
                if (size.rect.top > this._container.clientHeight) {
                    offsetTop -= size.rect.height;
                }
            });
        } else {
            offsetTop = blocksSizes.totalHeight;
        ...
        this._scrollbarOffsetTop = offsetTop;
        ...
        this._scrollbars.setOffsets({ top: offsetTop, bottom: scrollbarOffsetBottom }, true);
    }
    ...
    getScrollLeft(): number {
        return this._children.content?.scrollLeft || 0;
    }
    ...
    isHorizontalScroll() {
        return (
            Math.round(this._scrollModel.clientWidth) !== Math.round(this._scrollModel.scrollWidth)
        );
    }
 ...
 * @function Controls/_scroll/Container#scrollToLeft
 * @param {Boolean} smooth - плавная прокрутка, по умолчанию false.
 * @see scrollToTop
 * @see scrollToBottom
 * @see scrollToRight
 ...
 * @function Controls/_scroll/Container#scrollToRight
 * @param {Boolean} smooth - плавная прокрутка, по умолчанию false.
 * @see scrollToTop
 * @see scrollToBottom
 * @see scrollToLeft
 ...
 * @cfg {ReactElement|TemplateFunction} Содержимое контейнера.
 */
...
/**
 * @name Controls/_scroll/Container#buttonsSize
 * @cfg {String} Определяет размер кнопок.
 * @variant m
 * @variant xl
 * @variant 5xl
 * @default m
...
/**
 * @name Controls/_scroll/Container#buttonsViewMode
 * @cfg {String} Определяет тип кнопок.
 * @variant filled
 * @variant ghost
 * @variant stretched
 * @default filled
 * @demo Controls-demo/Scroll/ButtonsViewMode/Index
 */
 ...
 * @variant edge Предназначен для пейджинга с отображением одной команды прокрутки. Отображается кнопка в конец, либо в начало, в зависимости от положения. Предназначен для пейджинга с отображением двух команд прокрутки. Отображается кнопка в конец и в начало.
 ...
 * @cfg {Function} Задает произвольный шаблон внутри пэйджинга.
 * @demo Controls-demo/Scroll/Paging/ContentTemplate/Index
 */

/**
 * @name Controls/_scroll/Container#pagingVisible
 * @cfg {Boolean} Опция управляет отображением произвольного шаблона внутри пэйджинга.
 */
 ...
 * @demo Controls-demo/Scroll/ScrollMode/HorizontalScroll/Index
 * @default vertical
 ...
 *
 * Важно учесть, что верстка для вертикального и горизонтального режима немного отличается. Так, в горизонтальном режиме, если на ребенке навешены
 * отступы, фон и т. д., то возможно отображение будет отличаться от того, что вы ожидаете. Чтобы этого избежать, просто оберните ваше содержимое в div.
 *
 * <pre>
 *      <Container>
 *          <div className='tw-flex'>
 *              <div className='tw-flex controls-background-contrast-info'></div>
 *          </div>
 *      </Container>
 * </pre>
 */
...
/**
 * Принудительно отключает отображение скролбара на всех видах устройств.
 * @name Controls/_scroll/Container#forceScrollbarVisible
 * @cfg {Boolean}
 * @default true
 *
 * @remark В отличии от scrollbarVisible отключает отображение как на touch устройствах, так и на устройствах с мышкой.
 * @see scrollbarVisible
 */
 ...
 * @default always
 * @demo Controls-demo/Scroll/ButtonsMode/Index
 */
 ...
 * @param {Number} scrollPosition - позиция, до которой требуется проскролить.
...
/**
 * @typedef {String | Number} TInitialScrollPositionItem
...
/**
 * @typedef {Object} TInitialScrollPosition
 * @property {TInitialScrollPositionItem} [vertical] Определяет положение скрола в момент инициализации по вертикали.
 * @property {TInitialScrollPositionItem} [horizontal] Определяет положение скрола в момент инициализации по горизонтали.
 */

/**
 * Определяет положение скрола в момент инициализации.
 * @name Controls/_scroll/Container#initialScrollPosition
 * @cfg {TInitialScrollPosition}
