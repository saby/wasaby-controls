...
import { IComponentPropsWithReadonly, IDisplayedRangesOptions } from 'Controls/interface';
...
export default class List extends React.Component<ICompactDatePickerOptions> {
    ...
    private _onMonthMouseEnter(date: Date, calendarRef: React.RefObject<MonthView>): void {
        if (
            this.props.selectionType === 'range' &&
            !this.state.readOnly &&
            calendarRef.current?.getIsAvailable()
        ) {
    ...
    private _onMonthMouseLeave(date: Date, calendarRef: React.RefObject<MonthView>): void {
        if (this.props.selectionType === 'range' && calendarRef.current?.getIsAvailable()) {
            const value = this._getMonthValue(date);
            if (!this.props.selectionProcessing) {
                this.setState({
                    hoveredMonth: null,
    ...
    private _onMonthClick(date: Date, calendarRef: React.RefObject<MonthView>): void {
        if (
    ...
    private _getDefaultMonthTemplateHeader(props: object): React.ReactElement {
        if (!this._isLastMonth(props.date)) {
            return (
                <div
                    onMouseEnter={this._onMonthMouseEnter.bind(this, props.date, props.calendarRef)}
                    onMouseLeave={this._onMonthMouseLeave.bind(this, props.date, props.calendarRef)}
                    onClick={this._onMonthClick.bind(this, props.date, props.calendarRef)}
                    className={
                        (this.props.selectionType === 'range' &&
                        (!this.props.selectionProcessing || this._monthSelectionProcessing) &&
                        !this.props.readOnly
    ...
    private _getCaptionTemplate(
        props: object,
        calendarRef?: React.RefObject<MonthView>
    ): React.ReactElement {
        return (
    ...
    private _getDefaultMonthTemplate(props: object): React.ReactElement {
        const calendarRef = React.createRef<MonthView>();
        return (
            <div className={`ws-flexbox ws-flex-column ${props.className || ''}`}>
                <MonthView
                    ...
                    enableWeekends={this.props.enableWeekends}
                    ...
                    captionTemplate={
                        this.props.captionTemplate &&
                        (() => this._getCaptionTemplate(props, calendarRef))
                    }
                />
                {!this.props.captionTemplate && (
