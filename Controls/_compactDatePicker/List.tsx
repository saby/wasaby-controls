...
interface ICompactDatePickerOptions
    ...
    endValue: Date | null;
    dayClickCallback: (value: Date, extData: object) => boolean;
}
...
export default class List extends React.Component<ICompactDatePickerOptions> {
    ...
    private _onItemClickHandler(...args): void {
        let shouldCallItemClick = true;
        if (this.props.dayClickCallback) {
            shouldCallItemClick = this.props.dayClickCallback(args[1], args[3]);
        }
        if (shouldCallItemClick) {
            this._proxyEvent('onItemClick', ...args);
        }
    }
