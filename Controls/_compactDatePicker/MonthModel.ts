...
let multiHovered = false;

const getBorderClass = (direction: string, borderStyle: string) => {
    return (
        BORDER_CLASSES[direction] + '__style-' + borderStyle + (multiHovered ? '_multiHovered' : '')
    );
};

const getBorderRadiusClass = (direction: string, borderStyle: string) => {
    return (
        BORDER_RADIUS_CLASSES[direction] +
        '__style-' +
        borderStyle +
        (multiHovered ? '_multiHovered' : '')
    );
};
...
export default class CompactMonthModel extends MonthModel {
    ...
    protected _prepareClass(
        ...
        if (scope.hovered && !scope.selectionProcessing && !scope.readOnly) {
            ...
            const multiHoveredClassName = ' controls-CompactDatePicker__multiHovered';
            cssClass += multiHoveredClassName;
