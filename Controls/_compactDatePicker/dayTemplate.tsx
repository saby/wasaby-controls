...
function CompactDatePickerDayContentTemplate(props) {
    ...
    const counterClassName = React.useMemo(() => {
        return isAdaptive
            ? 'controls-CompactDatePicker__month-list__item__counter controls-CompactDatePicker__month-list__item__counter-adaptive'
            : 'controls-CompactDatePicker__month-list__item__counter';
    }, [isAdaptive]);
    ...
    return (
        <>
            ...
            >
                {props.value.day}
                {props.hasMark && <div className="controls-CompactDatePickerItem__mark"></div>}
                {props.hasCounter && (
                    <div onClick={props.onCounterClick} className={counterClassName}>
                        <div>
            ...
            {isSelectedStart() ? (
                <div
                    className={`controls-CompactDatePicker__month-list__item_selected__container controls-CompactDatePicker__month-list__item_selected__container-start controls-CompactDatePicker__month-list__item_selected__container-start__size-${props.size}`}
                >
            ...
            {isSelectedEnd() ? (
                <div
                    className={`controls-CompactDatePicker__month-list__item_selected__container controls-CompactDatePicker__month-list__item_selected__container-end controls-CompactDatePicker__month-list__item_selected__container-end__size-${props.size}`}
                >
...
export default function getDayTemplate(
    ...
    function DayTemplate(props: object): React.ReactElement {
        const counter = React.useMemo(() => {
            ...
            if (counterFormatter) {
                return counterFormatter(props.value.date, props.value.extData, props.value);
            }
            return value;
        }, [props.value.extData, counterFormatter]);

        const shouldShowAddButton = React.useCallback((): boolean => {
        ...
        const onCounterClick = React.useCallback(
        ...
        const itemWrapperClassNames = React.useMemo((): string => {
            let className = '';
            if (!props.value.isCurrentMonth || !counterProperty) {
                return className;
            }
            ...
            if (counter) {
                className += ' controls-CompactDatePicker__month-list__item-has-counter';
            }
            if (
                (!counter &&
                    counterClickCallback &&
            ...
            if (
                ...
                if (counter) {
                    ...
                    className +=
                        ' controls-CompactDatePickerItem_withCounter-addButton_withoutDelay';
                }
            }
            return className;
        }, [props.value.selectionProcessing, props.value.selected, showAddButton, counter]);

        const getBaseDayTemplate = () => {
            return (
                <div className={`${itemWrapperClassNames}${offsetClassName}`}>
                    <BaseDayTemplate
                        {...props}
                        ...
                        contentTemplate={(props) => {
                            const { extData } = props.value;
                            const hasMark =
                                extData &&
                                ((typeof extData.get === 'function' && extData.get('isMarked')) ||
                                    extData?.isMarked);
                            ...
                            return (
                                <CompactDatePickerDayContentTemplate
                                    ...
                                    hasCounter={!!(counter || counterClickCallback) && size === 'l'}
                                    onCounterClick={onCounterClick}
                                    counter={counter}
                                    hasMark={!!hasMark && size === 'm'}
                                    size={size}
                                />
                            );
                        }}
