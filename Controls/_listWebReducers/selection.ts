...
export const selection: TListMiddleware = ({ dispatch, getState, setState, getCollection }) => {
    const getStrategy = () => {
        if (isStateHierarchy(getState(), getCollection())) {
    ...
    const getCounterController = () => {
        const stateWithCollection = getStateWithCollection();

        function getDataLoadDirection(direction: Direction): Direction {
            const isInverted =
                stateWithCollection.itemsOrder && stateWithCollection.itemsOrder !== 'default';
        ...
        return new CounterController({
            ...
            hasMoreUtil: (key) => {
                return (
                    !!stateWithCollection.sourceController &&
                    (stateWithCollection.sourceController.hasMoreData(
                        getDataLoadDirection('up'),
                        key
                    ) ||
                        stateWithCollection.sourceController.hasMoreData(
                            getDataLoadDirection('down'),
                            key
                        ))
                );
            },
            isLoadedUtil: (key) => {
                return (
                    !stateWithCollection.sourceController ||
                    stateWithCollection.sourceController.hasLoaded(key)
                );
            },
        });
    };
    ...
    return (next) => async (action) => {
        switch (action.type) {
            ...
            case 'selectAll': {
                const newSelection = getStrategy().selectAll({
                    ...getStateWithCollection(),
            ...
            case 'updateCounter': {
                ...
                setState({
                    ...
                    isAllSelected: getStrategy().isAllSelected(
                        state,
            ...
            case 'onItemsReset': {
                await dispatch(
                    ListWebActions.selection.setSelectionModel(
                        getStrategy().getSelectionModel(getStateWithCollection())
                    )
                );
                break;
            }
...
const isStateHierarchy = (state: IListState, collection: ICollection): boolean =>
    !!(
        state.nodeProperty &&
        state.parentProperty &&
        collection[TREE_COLLECTION as keyof ICollection]
    );
