 ...
 */
import { ListActionCreators, TListMiddleware, IListState } from 'Controls-DataEnv/list';
import { TKey } from 'Controls-DataEnv/interface';
import { UILogic, helpers } from 'Controls/listsCommonLogic';
    ...
    async (action) => {
        switch (action.type) {
            ...
            case 'expandParent': {
                ...
                if (parentKey && !isExpanded(state, parentKey)) {
            ...
            case 'setExpandCollapsedItems': {
                ...
                //# region Обновление состояния узлов
                await dispatch(
                    ListActionCreators.expandCollapse.setExpandedItems(expandedItems, false)
                );
                await dispatch(
                    ListActionCreators.expandCollapse.setCollapsedItems(collapsedItems, false)
                );
                await dispatch(ListActionCreators.expandCollapse.updateExpansionModel());
                //# endregion
                break;
...
function getExpansionModel(state: IListState): IListState['expansionModel'] {
