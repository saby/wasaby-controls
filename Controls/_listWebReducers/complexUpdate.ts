...
export const complexUpdate: TListMiddlewareCreator =
    ({ dispatch, getState, getCollection, snapshots }) =>
    (next) =>
    async (action) => {
        switch (action.type) {
            case 'beforeApplyState': {
                ...
                await dispatch(
                    ListWebActions.operationsPanel.updateOperationsPanel(
                        prevState,
                ...
                await dispatch(
                    ListWebActions.selection.updateSelection(
                        prevState,
                ...
                await dispatch(
                    ListWebActions.items.complexUpdateItems(prevState, action.payload.nextState)
                );

                await dispatch(
                    ListWebActions.marker.complexUpdateMarker(prevState, action.payload.nextState)
                );

                await dispatch(
                    ListWebActions.search.updateSearch(
                        prevState,
                ...
                await dispatch(
                    ListWebActions.filter.updateFilter(prevState, {
                ...
                await dispatch(
                    ListWebActions.root.complexUpdateRoot(prevState, action.payload.nextState.root)
                );

                await dispatch(
                    ListWebActions.source.complexUpdateSource(prevState, action.payload.nextState)
                );

                await dispatch(
                    ListWebActions.complexUpdate.oldBeforeApplyState(
                        prevState,
                ...
                await dispatch(ListWebActions.source.awaitAllRequests());
            ...
            case 'oldBeforeApplyState': {
                ...
                await dispatch(ListWebActions.state.setState(nextState));
                break;
            }
        }
...
type ISnapshotsStore = Parameters<TListMiddlewareCreator>[0]['snapshots'];
...
async function beforeApplyState_fn(
    ...
    if (expandedItemsChanged && currentState.listConfigStoreId) {
        await dispatch(
            ListWebActions.source.setSavedSourceState(currentState.listConfigStoreId, nextState)
        );
    }
    ...
    if (needReload && (!searchValueChanged || !nextState.searchParam)) {
        ...
        dispatch(ListWebActions.source.load(navigationSourceConfig));
    }
    ...
    if (
        ...
        if (expandedItemsDiff.added.length && nextState.source) {
            ...
            await dispatch(
                ListWebActions.source.loadNodes({
                    currentState,
                    nextState,
