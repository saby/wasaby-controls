    ...
    async (action) => {
        switch (action.type) {
            ...
            case 'startUpdate': {
                ...
                // reduceState без publicSetState не отработает, поэтому состояние собирается и тут.
                //@ts-ignore
                setState(getCompatibleNextState(prevState, nextState, getState()));

                if (needRejectBeforeApply(getState())) {
