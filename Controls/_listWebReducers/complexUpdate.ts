    ...
    async (action) => {
        switch (action.type) {
            ...
            case 'reduceState': {
                ...
                await dispatch(
                    ListActionCreators.selection.complexUpdateSelection(prevState, nextState)
                );

                await dispatch(ListActionCreators.items.complexUpdateItems(prevState, nextState));

                await dispatch(
                    ListActionCreators.expandCollapse.complexUpdateExpandCollapse(
                        prevState,
                        nextState
                    )
                );

                await dispatch(ListActionCreators.marker.complexUpdateMarker(prevState, nextState));

                await dispatch(
                    ListActionCreators.search.updateSearch(prevState, nextState.searchValue)
                );
                ...
                await dispatch(
                    ListActionCreators.filter.updateFilter(prevState, {
                        ...
                        countFilterValue: nextState.countFilterValue,
                        countFilterLinkedNames: nextState.countFilterLinkedNames,
                        countFilterValueConverter: nextState.countFilterValueConverter,
                    })
                );

                await dispatch(ListActionCreators.root.complexUpdateRoot(prevState, nextState));

                await dispatch(ListActionCreators.source.complexUpdateSource(prevState, nextState));
            ...
            case 'startUpdate': {
                ...
                if (
                    ...
                    // @ts-ignore
                    (!getState().fix88221033390078 || actions.length > 1) &&
                    needRejectBeforeApply(getState())
                ) {
                    await dispatch(ListActionCreators.source.rejectLoad());
                }
            ...
            case 'beforeApplyState': {
                ...
                await dispatch(ListActionCreators.complexUpdate.reduceState(prevState, nextState));
                ...
                .then(({ items, root, searchValue }) => {
                    ...
                    return dispatch(
                        ...
                        .then(async (resultState: IListState) => {
                            ...
                            await resultState.sourceController?.setItemsAfterLoad(
                                items as RecordSet
                            );
                ...
                .then((newItems) => {
                    ...
                    return dispatch(
                        ...
                        .then(async (newState: IListState) => {
                            await newState.sourceController?.setItemsAfterLoad(
