        ...
        switch (action.type) {
            ...
            case 'loadOnSourceController': {
                ...
                registerPendingPromise(
                    ...
                    // Во время работы поиска записи могут подгружаться порционно. Если во время подгрузки,
                    // изменится значение в поле, запустится новый промис. В этом случае необходимо отменить предыдущий
                    state.viewMode === 'search' ? 'replace' : 'throwError'
                )
                    .then((result) => {
            ...
            case 'oldSliceLoad': {
                ...
                const dLoad = getSafeDecomposedPromise<TLoadResult>(
                    ...
                    // loadOnSourceController включает стратегию replace при viewMode search,
                    // но так как регистрация dLoadOnSourceController происходит раньше,
                    // onReject не успевает отработать. Поэтому приходится использовать такое же условие здесь
                    getState().viewMode === 'search' ? 'replace' : 'throwError'
                );
