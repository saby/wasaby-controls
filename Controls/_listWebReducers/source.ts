...
import { SnapshotName, IListStateParts } from 'Controls-DataEnv/list';
import type { PromiseCanceledError } from 'Types/entity';
import type { TItemsChange, TMetaDataChange } from 'Controls-DataEnv/abstractList';
import type { TreeGridCollection as ITreeGridCollection } from 'Controls/treeGridDisplay';
...
export const source: TListMiddleware = (ctx) => {
    const {
        getState,
        ...
        switch (action.type) {
            ...
            case 'loadToDirectionNew': {
                ...
                try {
                    ...
                    let hasMoreStorage = originalSliceGetState().hasMoreStorage;
                    ...
                    if (itemsAspectManager) {
                        ...
                        hasMoreStorage = itemsAspectManager.calculateHasMoreStorage(
                            originalSliceGetState().hasMoreStorage,
                            items,
                            direction,
                            originalSliceGetState().root
                        );
