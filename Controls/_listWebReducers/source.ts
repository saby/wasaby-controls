...
import { TDecomposedPromise } from 'Controls-DataEnv/abstractList';
...
export const source: TListMiddleware = (ctx: TListSourceMiddlewareContext) => {
    ...
    const getSafeDecomposedPromise = <T>(key: string): TDecomposedPromise<T> => {
        const dP = getDecomposedPromise<T>();

        const promise = registerPendingPromise(key, dP.promise);
        ...
        switch (action.type) {
            ...
            case 'reloadOnSourceController': {
                ...
                registerPendingPromise(
                    ...
                    .catch((e) => {
                        scheduleDispatch(ListWebActions.error.handleLoadError(e));
                        onReject?.(e);
                    });
            ...
            case 'loadToDirection': {
                ...
                // TODO: Надо ли еще оборачивать? Он же и так отслеживается
                registerPendingPromise('dLoadToDirection', dLoad.promise)
                    .then(onThenCallback)
