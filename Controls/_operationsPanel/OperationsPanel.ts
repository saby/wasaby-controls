...
import * as Utils from 'Controls/_operationsPanel/OperationsPanel/Utils';
...
export default class OperationsPanel extends Control<IOperationsPanelOptions> {
    ...
    protected _toolbarSource: RecordSet = null;
    protected _children: {
    ...
    protected _recalculateToolbarItems(items: RecordSet, toolbarWidth: number): void {
        if (items && toolbarWidth) {
            ...
            this._toolbarSource = Utils.fillItemsType(
    ...
    protected _beforeMount(options: IOperationsPanelOptions): Promise<void> | void {
        ...
        this._callListActionCallback = this._callListActionCallback.bind(this);
    ...
    protected _afterMount(): void {
        ...
        this._notify('operationsPanelOpen', [this._callListActionCallback], { bubbling: true });
    }
    ...
    /*
     * Вызов команды напрмую по её id.
     * Необходимо для обработки события горячих клавиш из списка.
     * Отдаётся в Browser или OperationsPanelConnected и
     * сохраняется там в стейт для обработки события от списка.
     */
    protected _callListActionCallback(actionParams: { id?: string; actionName?: string }): void {
        const action = factory(this._items)
            .toArray()
            .find((item) => {
                return (
                    item.getKey() === actionParams.id ||
                    item.get('actionName') === actionParams.actionName
                );
            });
        if (action) {
            ...
            this._itemClickHandler(null, action, event);
        }
    }

    protected _applyClick(event, ...args): void {
        this._notify('applyClick', args);
    }
    ...
    protected _itemClickHandler(
        ...
        let selectedKeysCount = this._options.operationsController?.getSelectedKeysCount();
        let selection;
        if (
            this._options.listMarkedKey &&
            item.get('allowEmptySelection') &&
            this._options.selectionViewMode !== 'selected'
        ) {
            selection = {
        ...
        } else if (this._options.listCommandsSelection) {
            selection = this._options.listCommandsSelection;

            if (
                this._options.selectionViewMode === 'selected' &&
                this._options.showSelectedCount &&
                !selectedKeysCount
            ) {
                selectedKeysCount = this._options.showSelectedCount;
            }
        } else {
            selection = {
        ...
        if (checkWasabyEvent(this._options.onItemClick)) {
            this._options.onItemClick(item, nativeEvent, selection, selectedKeysCount);
        }
 ...
 * @variant all Кнопка "Отобрать отмеченные"
