...
import {
    IOperationsPanelItem,
    IOperationsPanelOptions,
} from './_interface/IOperationsPanel';
...
export default class OperationsPanel extends Control<IOperationsPanelOptions> {
    ...
    protected _recalculateToolbarItems(
        items: RecordSet,
        toolbarWidth: number
    ): void {
    ...
    protected _onSelectedTypeChanged(selectedType: string): void {
        if (this._options.onSelectedTypeChanged) {
    ...
    protected _itemClickHandler(
        ...
        const selectedKeysCount =
            this._options.operationsController?.getSelectedKeysCount();
        let selection;
        if (this._options.listMarkedKey && item.get('allowEmptySelection')) {
            selection = {
                selected: this._options.listParentProperty
                    ? [this._options.root]
                    : [null],
                excluded: this._options.listParentProperty
                    ? [this._options.root]
                    : [null],
            };
        } else {
