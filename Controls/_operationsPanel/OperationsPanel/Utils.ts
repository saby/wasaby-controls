...
const createActualItems = (itemsPram: RecordSet<Record>): RecordSet<Record> => {
    REQUIRED_FIELDS.forEach((field) => {
        if (!hasField(itemsPram, field.name)) {
            addField(itemsPram, field.name, field.defaultValue);
        }
    });
    itemsPram.each((item) => {
    ...
    fillItemsType(
        ...
        if (visibleItemsKeys.length <= 1) {
            ...
            if (currentWidth > availableWidth) {
                ...
                const allItemsIsVisible = visibleItemsKeys.every((visibleItemKey) => {
                    return items.getRecordById(visibleItemKey).get('showType') !== showType.MENU;
                });
