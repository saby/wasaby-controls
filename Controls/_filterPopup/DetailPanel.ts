 ...
 * @cssModifier controls-FilterPanel__width-xl Очень большая ширина панели.
 */
class FilterPanel extends Control<
    IFilterDetailPanelOptions,
    IFilterItem[] | List<IFilterItem[]>
> {
    ...
    protected _beforeMount(
        ...
        this._historySaveMode =
            ...
            options.historySaveMode === 'favorite'
                ? 'favorite'
                : 'pinned';
    ...
    protected _isHistoryVisible(): boolean {
        return (
            ...
            (this._historyItems.getCount() ||
                this._options.emptyHistoryTemplate)
        );
    }
    ...
    private _loadHistoryItems(
        ...
        if (historyId) {
            const config = {
                historyIds: filterSource
                    ? HistoryUtils.getParamHistoryIds(filterSource)
                    : undefined,
                historyId,
                recent: isFavoriteHistory
                    ? 'MAX_HISTORY_REPORTS'
                    : 'MAX_HISTORY',
    ...
    private _removeDublicateFilters(
        items: RecordSet,
        historyId: string
    ): RecordSet {
    ...
    private _removeOutdatedFilters(
        ...
        const getOriginalItem = (historyItem: IFilterItem): IFilterItem => {
            return find(this._items, (origItem) => {
                return isEqualItems(origItem, historyItem);
            });
        };
        let originalItem;
        let hasResetValue;
        ...
        chain.factory(items).each((item) => {
            if (
                (getPropValue(item, 'resetValue') !== undefined &&
                    ...
                    getPropValue(item, 'visibility') === undefined) ||
                getPropValue(item, 'visibility') && item.viewMode === 'extended' && !item.editorTemplateName
            ) {
                isChanged = true;
            }
        });
    ...
    static getDefaultOptions(): IFilterDetailPanelOptions {
        return {
            ...
            additionalPanelTemplate:
                'Controls/filterPopup:AdditionalPanelTemplate',
