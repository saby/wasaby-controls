...
import { FilterUtils, IFilterItem, isEqualItems, FilterHistory } from 'Controls/filter';
...
class FilterPanel extends Control<IFilterDetailPanelOptions, IFilterItem[] | List<IFilterItem[]>> {
    ...
    protected _beforeMount(
        ...
        this._initialFilter = this._getFilter(options.items);
        this._applyButtonVisible = this._getApplyButtonVisible(this._items, options);
    ...
    protected _itemsChangedHandler(event: Event, items: IFilterItem[]): void {
        ...
        this._applyButtonVisible = this._getApplyButtonVisible(items, this._options);
        this._hasBasicItems = this._hasBasicParams();
    ...
    protected _applyHistoryFilter(
        event: Event,
        history: IFilterItem[],
        params: { isFilterFromHistory: boolean }
    ): void {
        const items = history.items || history;
        this._applyFilter(event, items, history, params?.isFilterFromHistory);
    }
    ...
    protected _applyFilter(
        ...
        if (history) {
            ...
            this.validate().then((result: IValidateResult) => {
    ...
    protected _resetFilter(): void {
        ...
        this._applyButtonVisible = this._getApplyButtonVisible(this._items, this._options);
    ...
    private _getHeadingCaption({
        ...
        if (orientation === 'horizontal' || historySaveMode === 'favorite') {
    ...
    private _getApplyButtonVisible(
        items: IFilterItem[],
        options: IFilterDetailPanelOptions
    ): boolean {
        return (
            options.applyButtonVisible !== false &&
            (options.applyButtonVisible ||
                options.orientation === 'horizontal' ||
                !isEqual(this._initialFilter, this._getFilter(items)))
        );
    }
            ...
            return FilterHistory.loadFilterHistory(config).then(
                (historyData) => {
                    ...
                    if (historySaveMode === 'favorite') {
                        historyItems = FilterHistory.getItemsByHistory(
                            historyId,
