...
import {
    Constants,
    FilterSource as FilterHistorySource,
...
class HistoryList extends Control<IHistoryListOptions> {
    ...
    protected _itemSwipe(
        event: SyntheticEvent<ISwipeEvent>,
        item: Model
    ): void {
    ...
    protected _clickHandler(event: Event, item: Model): void {
        const history = this._getSource(this._options.historyId).getDataObject(
            item
        );
    ...
    private _getText(
        ...
        factory(items).each((item, index: number) => {
            ...
            if (history) {
                text =
                    history.linkText ||
                    this._getStringHistoryFromItems(
    ...
    private _getStringHistoryFromItems(
        items: RecordSet,
        resetValues: object
    ): string {
        const textArr = [];
    ...
    private _onResize(): void {
        if (this._options.orientation === 'vertical') {
            ...
            this._arrowVisible =
                this._options.items.getCount() > MAX_NUMBER_ITEMS;
    ...
    private _getEditDialogOptions(
        ...
        const captionsObject = this._mapByField(
            this._options.filterItems,
            'caption'
        );
        items = factory(items)
            .map((historyItem) => {
                historyItem.caption =
                    captionsObject[this._getItemId(historyItem)];
                return historyItem;
            })
            .value();
    ...
    private _saveFavorite(record: Model): void {
        const editItemData = this._getSource(
            this._options.historyId
        ).getDataObject(this._editItem);
    ...
    private _updateFavorite(
        ...
        const templateOptions = this._getEditDialogOptions(
            item,
            this._options.historyId,
            text
        );
