...
interface IEditDialogResult {
    action: string;
    record: Model;
}

interface IEditDialogOptions extends IControlOptions {
    items: IFilterItem[];
    isFavorite: boolean;
...
class HistoryList extends Control<IHistoryListOptions> {
    ...
    protected _onPinClick(event: Event, item: Model, text?: string): void {
        if (this._options.saveMode === 'favorite') {
            this._onFavoriteClick(event, item, text);
        } else {
    ...
    protected _prefetchHistoryClick(event: Event, item: Model): void {
        const history = FilterHistory.getFilterHistoryItemData(item);
        this._notify('applyHistoryFilter', [history, { isFilterFromHistory: true }]);
    }
    ...
    private _updateFavorite(item: Model, text: string, target: EventTarget): void {
        ...
        const popupOptions = {
            template: 'Controls/_filterPanelPopup/_EditDialog',
            opener: this,
            target,
        ...
        this._stickyOpener = this._getStickyOpener();

        this._stickyOpener.open(popupOptions);
    }

    private _getStickyOpener() {
