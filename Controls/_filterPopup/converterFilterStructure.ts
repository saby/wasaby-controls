    ...
    return chain
        ...
        .map((item) => {
            ...
            for (const i in structureMap) {
                if (
                    Utils.object.getPropertyValue(item, structureMap[i]) !==
                        undefined &&
                    structureMap.hasOwnProperty(i)
                ) {
                    itemStructureItem[i] = Utils.object.getPropertyValue(
                        item,
                        structureMap[i]
                    );
                }
...
function convertToSourceDataArray(
    ...
    chain.factory(filterStructure).each((item) => {
        ...
        for (const i in recordToSructureElemMap) {
            if (
                Utils.object.getPropertyValue(
                    item,
                    recordToSructureElemMap[i]
                ) !== undefined &&
                ...
                rsItem[i] = Utils.object.getPropertyValue(
                    item,
                    recordToSructureElemMap[i]
                );
            }
