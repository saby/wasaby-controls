...
class Panel extends Control<ISimplePanelOptions> {
    ...
    protected _afterMount() {
        const hasSearch = !!this._items?.find(({ searchParam }) => searchParam);
        if (this._options.isAdaptive && hasSearch) {
    ...
    protected _itemClickHandler(
        ...
        if (this._isAnyItemSelected(index)) {
            this._updateSelectedKeys(index, keys);
        } else if (!isEqual(item.initSelectedKeys, keys)) {
            const result = {
                action: 'itemClick',
                event,
                selectedKeys: keys,
    ...
    private _isAnyItemSelected(itemIndex: number): boolean {
        return this._items?.some((item, index) => {
            if (index === itemIndex) {
                return false;
            }
            if (item.initSelectedKeys instanceof Array) {
                return !this._isEqualKeys(item.initSelectedKeys, item.selectedKeys);
            }
            return !Object.keys(item.selectedKeys).every((key) =>
                this._isEqualKeys(item.selectedKeys[key], item.initSelectedKeys[key])
            );
        });
    }
    ...
    protected _updateSelectedKeys(index: number, keys: string[]): void {
