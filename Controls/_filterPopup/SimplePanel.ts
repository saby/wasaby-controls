...
class Panel extends Control<ISimplePanelOptions> {
    ...
    private _getMenuMultipleConfig(initItems: RecordSet): Promise<TMenuConfig> {
        ...
        factory(initItems).each((item, index) => {
            ...
            if (curItem.editorTemplateName === DATE_MENU_TEMPLATE) {
                ...
                const curItems = this._getMenuConfig(curItem, index === initItems.getCount() - 1);
    ...
    private _getMenuConfig(curItem, isLast: boolean): IMenuControlOptions[] {
        let folders;
        ...
        if (curItem.nodeProperty && folders?.length) {
            ...
            this._folders[curItem.name] = folders.map((folder) => folder.name);
            folders.forEach((folder, index) => {
                ...
                result = result.concat(
                    this._prepareItemMenuOptions(folder, !isLast && index === folders.length - 1)
                );
            });
            ...
            return this._prepareItemMenuOptions(curItem, !isLast);
        }
    }
    ...
    private _prepareItemMenuOptions(curItem, showFooterSeparator): IMenuControlOptions[] {
        const item = { ...curItem };
