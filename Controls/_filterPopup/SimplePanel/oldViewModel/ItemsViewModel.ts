...
const ItemsViewModel = BaseViewModel.extend({
    ...
    _prepareDisplay(items, cfg) {
        const filter = this.getDisplayFilter(this.prepareDisplayFilterData(), cfg);
    ...
    reset(): void {
        this._startIndex =
            this._isSupportVirtualScroll() && !!this._startIndex ? this._startIndex : 0;
    ...
    _onCollectionChange(event, action, newItems, newItemsIndex, removedItems, removedItemsIndex) {
        ...
        const collectionChangeResult = this._notify.apply(
            this,
            ['onCollectionChange'].concat(Array.prototype.slice.call(arguments, 1))
        );
    ...
    _updateResults(items: RecordSet): boolean {
        ...
        const shouldUpdate =
            !!metaData && !isEqual(metaData, {}) && typeof metaData.results !== 'undefined';
        if (shouldUpdate) {
    ...
    isCachedItemData(itemKey) {
        return (
            typeof itemKey !== 'undefined' && typeof this._itemDataCache[itemKey] !== 'undefined'
        );
    },
    ...
    _resetCacheOnChange(changesType, changedItems?) {
        if (changesType === 'indexesChanged' || changesType === 'itemActionsUpdated') {
            return;
