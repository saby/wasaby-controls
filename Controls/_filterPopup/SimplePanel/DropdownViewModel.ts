...
const _private = {
    ...
    getSeparatorClass(isLast: boolean, nextItem: object): string {
        return !isLast && !nextItem?.isGroup ? ' controls-Menu__row-separator' : '';
    },

    getClassList(options: object, itemData: object, hasHierarchy: boolean) {
        const item = itemData.item;
        let classes = 'controls-SimplePanel-List__row_state_';
        if (options.readOnly || item.get('readOnly')) {
            classes += 'readOnly controls-Menu__content-readOnly';
        } else {
            classes += 'default controls-Menu__defaultItem controls-Menu__row_state_default';
        }
...
const DropdownViewModel = BaseViewModel.extend({
    ...
    getCurrent() {
        ...
        const isLast = this._itemsModel.isLast();
        const nextItem = !isLast ? this._itemsModel.getNext() : null;
        ...
        itemsModelCurrent.itemClassList += _private.getSeparatorClass(isLast, nextItem);
