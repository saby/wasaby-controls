...
const _private = {
    ...
    filterAdditional(item: Model) {
        if (
            !this._options.additionalProperty ||
            this._expanded === true ||
            !item.get
        ) {
            return true;
    ...
    needHideGroup(self: typeof DropdownViewModel, key: string) {
        ...
        return (
            itemsGroup.length === 0 ||
            itemsGroup.length === numberItemsCurrentRoot
        );
    },
    ...
    getClassList(options: object, itemData: object, hasHierarchy: boolean) {
        ...
        if (options.multiSelect && itemData.emptyText) {
            classes +=
                ' controls-SimplePanel-List__emptyItem-leftPadding_multiSelect';
        } else if (!options.multiSelect) {
            classes +=
                ' controls-SimplePanel-List__item-leftPadding_' +
                (paddings.left || 'default');
        }
        classes +=
...
const DropdownViewModel = BaseViewModel.extend({
    ...
    isGroupNext() {
        return !!this._itemsModel.getItemDataByItem(
            this._itemsModel.getNext().dispItem
        ).isGroup;
    },
    ...
    getCurrent() {
        ...
        itemsModelCurrent.hasChildren = this._hasItemChildren(
        ...
        itemsModelCurrent._isSelected = this._isItemSelected(
            itemsModelCurrent.item
        );
        ...
        itemsModelCurrent.levelPadding = this._getLevelPadding(
            itemsModelCurrent.item
        );
    ...
    _isItemSelected(item: Model) {
        ...
        return (
            keys !== undefined &&
            isEqual(keys, item.get(this._options.keyProperty))
        );
    },
    ...
    hasAdditional() {
        ...
        if (
            this._options.additionalProperty &&
            this._options.rootKey === null
        ) {
            this._options.items.each((item) => {
                if (!hasAdditional) {
                    hasAdditional =
                        item.get(this._options.additionalProperty) &&
                        !_private.isHistoryItem(item);
                }
            });
        }
    ...
    getEmptyItem() {
        if (this._options.emptyText) {
            ...
            itemData[this._options.keyProperty] =
                this._options.emptyKey !== undefined
