...
class PropertyGrid extends Control<IFilterPropertyGridOptions> {
    ...
    protected _beforeUpdate(newOptions: IFilterPropertyGridOptions): void {
        if (!isEqual(newOptions.items, this._options.items)) {
            this._changedIndex = this._getIndexChangedVisibility(
                newOptions.items,
                this._items
            );
        ...
        this._lastVisibleIndex = this._getLastVisibleItemIndex(
            newOptions.items
        );
    }
    ...
    protected _visibilityChangedHandler(
        event: Event,
        index: number,
        visibility: boolean
    ): void {
    ...
    private _getIndexChangedVisibility(
            ...
            if (visibility) {
                factory(oldItems).each((oldItem) => {
                    if (
                        ...
                        visibility !==
                            object.getPropertyValue(oldItem, 'visibility')
                    ) {
                        result = index;
                    }
                });
