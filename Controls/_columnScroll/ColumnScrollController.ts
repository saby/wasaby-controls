...
interface IDrawTransformParams
    extends IGetTransformStylesParams,
        IGetShadowsStylesParams {}
...
export default class ColumnScrollController {
    ...
    constructor(
        ...
        this._options.backgroundStyle =
            this._options.backgroundStyle || 'default';
        this._options.stickyColumnsCount =
            this._options.stickyColumnsCount || 1;
    ...
    setContainers(containers: {
        ...
        this._scrollContainer =
            containers.scrollContainer || this._scrollContainer;
        this._contentContainer =
            containers.contentContainer || this._contentContainer;
        this._stylesContainer =
            containers.stylesContainer || this._stylesContainer;
    ...
    private _updateShadowState(): void {
        ...
        this._shadowState.end =
            this._contentSize - this._containerSize - this._scrollPosition >= 1;
    }
    ...
    getShadowStyles(position: 'start' | 'end'): string {
        ...
        if (this._options.isEmptyTemplateShown) {
            ...
            shadowStyles +=
                'height: ' +
                (emptyTemplate as HTMLDivElement).offsetTop +
                'px;';
        }
        return shadowStyles;
    }
    ...
    private _updateSizes(calculatedSizes: {
        ...
        if (newContainerSize + this._scrollPosition > newContentSize) {
            this._scrollPosition -=
                newContainerSize + this._scrollPosition - newContentSize;
        }
        ...
        if (!hasSizesPreSet) {
            this._toggleStickyElementsForScrollCalculation(
                true,
                originStickyDisplayValue
            );
        }
    ...
    private _setBorderScrollPosition(
    ...
    getTransformStyles(
        ...
        if (
            ...
            newTransformHTML += `@keyframes ${scrollAnimationId} { from { transform: translateX(${getPosition(
                -options.oldPosition
            )}px); } to { transform: translateX(${getPosition(
                -position
            )}px); } }`;
        ...
        if (!isFullGridSupport) {
            const maxVisibleScrollPosition =
                position - (this._contentSize - this._containerSize);
    ...
    scrollByWheel(e: SyntheticEvent<WheelEvent>): number {
        ...
        if (nativeEvent.shiftKey || nativeEvent.deltaX) {
            ...
            return Math.max(
                0,
                Math.min(this._scrollPosition + delta, maxPosition)
            );
        }
        return this._scrollPosition;
    }
    ...
    getSizes(): object {
        return {
            ...
            scrollableColumnsWidth:
                this._containerSize - this._fixedColumnsWidth,
    ...
    private getScrollPositionToColumnRectEdge(columnRect: DOMRect): number {
        ...
        if (columnRect.right > scrollableRect.right) {
            const newScrollPosition =
                this._scrollPosition +
                (Math.round(columnRect.right) -
                    Math.floor(scrollableRect.right));
    ...
    static getPreRenderTransformStyles(
        ...
        style += ColumnScrollController._getColumnScrollStyles(
            uniqSelector,
            true,
            true
        );
    ...
    private static _getColumnScrollStyles(
        ...
        let styles = '';
        styles += ` .${uniqSelector} .controls-Grid__header-cell_withColumnScrollArrows { padding-bottom: var(--offset_xl); }`;
        if (useFakeRender) {
    ...
    static shouldDrawColumnScroll(
        ...
        const fixHeaderBreadcrumbsNegativeOffset = (
            ...
            if (task1184956815) {
                const backButtonSelector =
                    '.js-controls-Explorer__HeadingPathBack';
                const checkBoxCellSelector =
                    '.js-controls-Grid__header-cell-checkbox';
                const backButtonHeader =
                    header.querySelector(backButtonSelector);

                if (backButtonHeader) {
                    const checkBoxCell =
                        header.querySelector(checkBoxCellSelector);
                    if (checkBoxCell) {
                        return (
                            contentContainerSize -
                            checkBoxCell.getBoundingClientRect().width
                        );
                    }
                }
            }
        ...
        const calcResult = (): object => {
            ...
            const header =
                'header' in viewContainers
                    ? viewContainers.header
                    : viewContainers.results;

            if (!header) {
                Logger.error('Header is missing!');
            } else {
                contentContainerSize = fixHeaderBreadcrumbsNegativeOffset(
                    header,
        ...
        const scrollBarContainer =
            viewContainers?.horizontalScrollBar?._container;
