...
import type { GridCell, GridHeaderCell, ColumnResizerCell } from 'Controls/grid';
...
function getCellProps(cell: GridCell, rowProps: IRowComponentProps): ICellComponentProps {
    ...
    if (cell['[Controls/_display/grid/HeaderCell]'] && cellComponentProps.cellUnderline) {
        cellComponentProps.render = (
            <div className={'controls-GridReact__cell-underline'}>{cellComponentProps.render}</div>
        );
    }
...
function RowComponent(props: IRowComponentProps): React.ReactElement {
    ...
    props.cellsIterator((cell, index) => {
        ...
        const tabIndex = props.innerFocusElement && index === 0 ? 0 : undefined;
        const cellRender = (
            <CellComponent
                key={cell.key}
                {...cellProps}
