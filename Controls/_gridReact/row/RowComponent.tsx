...
import CellComponent, { getVerticalPaddingsClassName } from 'Controls/_gridReact/cell/CellComponent';
...
function getClassName({
    ...
    if (borderVisibility !== 'hidden') {
        className += ` controls-GridReact__row-border_${borderVisibility}`;
        className += ` controls-GridReact__row-borderStyle_${borderStyle}`;
    }
    return className;
}
...
function getCellProps(cell: GridCell, rowProps: IRowComponentProps): ICellComponentProps {
    ...
    if (cell['[Controls/_display/grid/HeaderCell]']) {
        ...
        const checkboxClassName = getVerticalPaddingsClassName(rowProps.paddingTop, rowProps.paddingBottom);
        render = (
            <CheckboxComponent
        ...
        render = config?.render !== undefined ? config?.render : (
            <DefaultCellRender
