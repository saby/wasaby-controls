...
export default class MoveViewCommand {
    ...
    constructor(options: IMoveViewCommandOptions) {
        ...
        if (options.target !== undefined && options.targetKey === undefined) {
            options.targetKey = options.target?.getKey
                ? options.target.getKey()
                : options.target;
        }
        ...
        return new Promise((resolve, reject) => {
            try {
                if (
                    ...
                    this._moveToSiblingPosition(
                        this._options.items[0],
                        this._options.targetKey
                    );
                } else {
        ...
        if (targetIndex !== -1) {
            items.forEach((item): void => {
                if (item) {
                    ...
                    if (
                        parentProperty &&
                        target.get(parentProperty) !==
                            item.get(parentProperty) &&
    ...
    private _getTargetItemById(id: TKey): Model | void {
        ...
        if (this._options.parentProperty) {
            ...
            let itemIndex = this._options.collection.getIndexByValue(
    ...
    protected _move(
        targetKey: TKey,
        items: TKey[] = this._options.items
    ): void {
