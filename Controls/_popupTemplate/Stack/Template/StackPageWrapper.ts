...
class StackPageWrapper extends Control<IPageTemplate, IReceivedState> implements IPropStorage {
    ...
    protected _maximizeButtonVisibility: boolean;
    ...
    protected _beforeMount(
        ...
        this._maximizeButtonVisibility = !options.adaptiveMode?.device.isTablet();
    ...
    private _updateOffset(
        ...
        this._maxOffset = Math.max(maxWidth + RIGHT_PANEL_WIDTH - this._workspaceWidth, 0) / 2;
    ...
    private _setWorkSpaceWidth(
        ...
        const isTablet = adaptiveMode?.device.isTablet();
        let trueWidth = isTablet ? adaptiveMode.width.value : width;
        // Ширина складывается из установленной ширины + ширины правой панели
        if (!withRightPanel || isTablet) {
