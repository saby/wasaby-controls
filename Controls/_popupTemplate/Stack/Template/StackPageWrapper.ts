 ...
 */
export default class StackPageWrapper
    extends Control<IPageTemplate, IReceivedState>
    implements IPropStorage {
    ...
    protected _beforeMount(
        ...
        if (options.width) {
            ...
            this._setSavedSizes(
                receivedState?.minSavedWidth,
                receivedState?.maxSavedWidth
            );
        }
        if (!this._sizesClass) {
        ...
        if (!receivedState && options.propStorageId && !options.width) {
            return new Promise((resolve) => {
                getPopupWidth(options.propStorageId).then(
                    (data?: number | IStackSavedConfig) => {
                        ...
                        if (data) {
                            ...
                            if (!this._sizesClass) {
                                if (
                                    !(
                                        typeof resultData.width ===
                                        'undefined' && !this._workspaceWidth
                                    )
                                ) {
                                    this._setWorkSpaceWidth(resultData.width);
                                }
                                this._setSavedSizes(
                                    resultData.minSavedWidth,
                                    resultData.maxSavedWidth
                                );
                                this._updateOffset(
                                    options.minWidth,
                                    options.maxWidth
    ...
    private _resizeObserverCallback(entry): void {
        this._templateWorkSpaceWidth =
            entry[0].contentRect.width - getRightPanelWidth();
    }
    ...
    protected _getContentWidth(): string {
        if (!this._sizesClass) {
            const width = this._workspaceWidth > (this._minWidth + this._rightPanelWidth)
                ? this._workspaceWidth
                : (this._minWidth + this._rightPanelWidth);
            return `width:${detection.isPhone ? width : '100vw'}; `;
        }
        return '';
    }
    ...
    protected _offsetHandler(event: Event, offset: number): void {
        this._getStackController().then((StackController) => {
            ...
            const newWidth =
                Math.min(this._workspaceWidth, document.body.clientWidth) +
                fullOffset;
    ...
    private _generateControllerItem(
        options: IPageTemplate = this._options
    ): Partial<IStackItem> {
        return {
    ...
    private _updateOffset(
        ...
        this._maxOffset =
            Math.max(
                maxWidth + this._rightPanelWidth - this._workspaceWidth,
                0
            ) / 2;
        this._minOffset =
            Math.max(
                this._workspaceWidth - (minWidth + this._rightPanelWidth),
                0
            ) / 2;
    }
