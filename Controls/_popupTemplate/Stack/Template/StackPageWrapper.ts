...
const DEBOUNCE_SAVE_WORKSPACE_WIDTH = 150;

let themeConstants = {};

/**
 * Контрол-обертка для базовой раскладки {@link https://n.sbis.ru/article/558ad4e6-866f-43a6-8093-af46125111ca стекового окна} в отдельной вкладке.
 *
...
class StackPageWrapper extends Control<IPageTemplate, IReceivedState> implements IPropStorage {
    ...
    private _debounceSaveWorkSpaceWidth: Function;
    ...
    protected _beforeMount(
        ...
        this._resizeWindow = this._resizeWindow.bind(this);
        this._debounceSaveWorkSpaceWidth = debounce(
            this._saveWorkSpaceWidth,
            DEBOUNCE_SAVE_WORKSPACE_WIDTH
        );
        ...
        if (!this._sizesClass) {
            this._setWorkSpaceWidth(
                ...
                options.minWidth,
                options.adaptiveMode.window.screenWidth.value
            );
            this._updateOffset(options.minWidth, options.maxWidth);
            // Вычисления minOffset и maxOffset могут отличаться на СП и клиенте за счет участия в расчетах document.
    ...
    private _resizeObserverCallback(entry): void {
        this._templateWorkSpaceWidth = entry[0].contentRect.width;
        if (!this._options.adaptiveMode.device.isPhone()) {
            this._templateWorkSpaceWidth -= RIGHT_PANEL_WIDTH;
        }
    ...
    private _resizeWindow(): void {
        ...
        const oldWorkspaceWidth = this._workspaceWidth;
        this._templateWorkSpaceWidth = rect.width;
        if (!this._options.adaptiveMode.device.isPhone()) {
            this._templateWorkSpaceWidth -= RIGHT_PANEL_WIDTH;
        }
        if (Controller.getAccordionTemplate() || this._leftContentTemplateWidth) {
            this._workspaceWidth = Math.min(
                this._workspaceWidth,
                document.body.clientWidth -
                    this._leftContentTemplateWidth * 2 -
                    (Controller.getAccordionTemplate() ? ACCORDION_WIDTH * 2 : 0)
            );
            if (this._options.minWidth) {
                this._workspaceWidth = Math.max(
                    this._options.minWidth + RIGHT_PANEL_WIDTH,
                    this._workspaceWidth
                );
            }
        }
        if (oldWorkspaceWidth !== this._workspaceWidth) {
            this._debounceSaveWorkSpaceWidth();
        }
        this._updateOffset();
        this._maximized = this._getMaximizeState(this._options);
    }
    ...
    protected _componentDidMount(options: IPageTemplate): void {
        ...
        // Дублируем код из StackController'а, чтобы не грузить целую библиотеку
        if (!options.adaptiveMode.device.isPhone()) {
            this._leftContentTemplateWidth =
                this._container?.querySelector('.controls-StackTemplate__leftArea-wrapper')
                    ?.offsetWidth || 0;
        }
        // Аккордеон вставляется в leftContentTemplate и скрывается. Это временный костыль.
        // Не будем учитывать такой аккордеон при расчете размеров
        if (Controller.getAccordionTemplate() && this._leftContentTemplateWidth) {
            const accordionWidth = 54;
            this._leftContentTemplateWidth -= accordionWidth;
            this._leftContentTemplateWidth = Math.max(this._leftContentTemplateWidth, 0);
        }
        if (this._leftContentTemplateWidth || Controller.getAccordionTemplate()) {
            ...
            if (options.minWidth) {
                maxWidth = Math.max(options.minWidth + RIGHT_PANEL_WIDTH, maxWidth);
            }
            const oldWorkspaceWidth = this._workspaceWidth;
            ...
            if (oldWorkspaceWidth !== this._workspaceWidth) {
                ...
                if (options.propStorageId) {
                    setSettings({ [options.propStorageId]: widthState });
                }
            }
        }
    }
    ...
    protected _getContentStyle(adaptiveMode?: AdaptiveModeType): string {
        ...
        const defaultStyle = `min-width: ${minWidth}px; max-width: ${this._workspaceWidth}px;`;
    ...
    private _updateProperties(
        ...
        if (!adaptiveMode.device.isPhone()) {
            this._minWidth = Math.min(minWidth, this._workspaceWidth - RIGHT_PANEL_WIDTH);
        }
        // Если размер фиксирован
        if (
            (this._workspaceWidth && !minWidth && !maxWidth) ||
            (this._workspaceWidth && minWidth && this._workspaceWidth < minWidth)
        ) {
    ...
    private _updateOffset(
        ...
        maxWidth = Math.min(document?.body.offsetWidth, maxWidth + RIGHT_PANEL_WIDTH);
        // protect against wrong options
        const maxDocumentOffset =
            (document?.body.offsetWidth - this._workspaceWidth) / 2 -
            this._leftContentTemplateWidth -
            (Controller.getAccordionTemplate() ? ACCORDION_WIDTH : 0);
        if (
            maxWidth +
                (this._leftContentTemplateWidth +
                    (Controller.getAccordionTemplate() ? ACCORDION_WIDTH : 0)) *
            ...
            this._maxOffset = maxDocumentOffset;
        } else {
            this._maxOffset = (maxWidth - this._workspaceWidth) / 2;
        }
    ...
    private _setWorkSpaceWidth(
        ...
        adaptiveMode: AdaptiveModeType,
        minWidth: number = this._options.minWidth,
        screenWidth: number = adaptiveMode.width.value
    ): void {
        let trueWidth = this._isTablet
            ? screenWidth - (Controller.getAccordionTemplate() ? ACCORDION_WIDTH * 2 : 0)
            : width;
        ...
        } else if (!adaptiveMode.device.isPhone()) {
            trueWidth = Math.min(
                trueWidth,
                screenWidth -
                    (Controller.getAccordionTemplate() ? ACCORDION_WIDTH * 2 : 0) -
        ...
        if (trueWidth && minWidth) {
            trueWidth = Math.max(trueWidth, minWidth);
        }
        this._workspaceWidth = trueWidth ? trueWidth : undefined;
        if (!adaptiveMode.device.isPhone() && this._workspaceWidth) {
            this._workspaceWidth += RIGHT_PANEL_WIDTH;
        }
        this._templateWorkSpaceWidth = trueWidth;
    }

    private _saveWorkSpaceWidth(): void {
        const widthState = {
            width: this._templateWorkSpaceWidth,
            minSavedWidth: this._minSavedWidth,
            maxSavedWidth: this._maxSavedWidth,
        };
        setSettings({ [this._options.propStorageId]: widthState });
    }
 ...
 * @cfg {Number} Ширина, которую пользователь выставил с помощью движения границ.
 * @remark
 * Значение нужно перезагрузить из хранилища и передать в контрол для избавления от асинхронного _beforeMount
 * @demo Controls-demo/Popup/StackPageWrapper/Index
 ...
 * @cfg {Number} Минимальная ширина, которую пользователь выставил с помощью движения границ.
 * @remark
 ...
 * @cfg {Number} Максимальная ширина, которую пользователь выставил с помощью движения границ.
 * @remark
 * Будет использоваться при нажатии пользователем на кнопку свернуть/развернуть
 * Значение нужно перезагрузить из хранилища и передать в контрол для избавления от асинхронного _beforeMount
 * @demo Controls-demo/Popup/StackPageWrapper/Index
