...
import { unsafe_getRootAdaptiveMode, AdaptiveModeType } from 'UI/Adaptive';
...
export default class StackPageWrapper
                    ...
                    if (data) {
                        ...
                        if (!this._sizesClass) {
                            if (
                                ...
                                this._setWorkSpaceWidth(resultData.width, true, options.adaptiveMode);
                            }
    ...
    protected _componentDidMount(options: IPageTemplate): void {
        if (this._sizesClass) {
            ...
            initializationConstants().then((result) => {
                ...
                this._updateProperties(options.propStorageId, options.adaptiveMode, minWidth, maxWidth, width);
            });
        }
    }
    ...
    protected _beforeUpdate(options: IPageTemplate): void {
        if (
            ...
            this._setWorkSpaceWidth(width, true, options.adaptiveMode);
            this._updateOffset(options.minWidth, options.maxWidth);
            this._updateProperties(options.propStorageId, options.adaptiveMode, options.minWidth, options.maxWidth);
        }
    }

    protected _getContentStyle(adaptiveMode?: AdaptiveModeType): string {
        const defaultStyle = `min-width: ${
            this._minWidth + this._rightPanelWidth
        }px; max-width: ${this._workspaceWidth}px;`;
    ...
    private _setWorkSpaceWidth(width: number, withRightPanel: boolean = true, adaptiveMode: AdaptiveModeType): void {
