    ...
    implements IPropStorage
{
    ...
    protected _beforeUpdate(options: IPageTemplate): void {
        if (
            ...
            this._setWorkSpaceWidth(width);
            this._updateOffset(options.minWidth, options.maxWidth);
            this._updateProperties(options.propStorageId, options.minWidth, options.maxWidth);
        }
    }
    ...
    protected _getContentWidth(): string {
        if (!this._sizesClass) {
            const width =
                this._workspaceWidth > this._minWidth + this._rightPanelWidth
                    ? this._workspaceWidth
                    : this._minWidth + this._rightPanelWidth;
            const isPhone = unsafe_getRootAdaptiveMode().device.isPhone();
            return `width:${isPhone ? width : '100vw'}; `;
        }
        return '';
    }
