...
class StackTemplate extends Control<IStackTemplateOptions> implements IPopupTemplate {
    ...
    protected _accordionTemplate: string;

    protected _isNewStack: boolean;

    protected _beforeMount(options: IStackTemplateOptions): void {
        this._isAdaptive = getAdaptiveModeForLoaders()?.device.isPhone();
        if (!constants.isServerSide) {
            ManagerController.getController()
                ?.find(options.popupId)
                ?.controller.updateElementsVisibility?.();
        }
        try {
            this._isNewStack = Feature?.get?.(['entity-accordion'])[0];
        } catch {
            this._isNewStack = false;
        }
        if (!options.isCompoundTemplate && !options.isPopup) {
            this._accordionTemplate = ManagerController.getAccordionTemplate();
        }
        // В теории, приложение может неправильно определить устройство и выставить isAdaptive=true на десктопе,
        // в таком случае стек будет строить шторку, а шторка стек, из-за чего произойдет бесконечный цикл.
        // На этот случай поставим дополнительную проверку.
        this._allowAdaptive =
            options.isAdaptive && this._isAdaptive && ManagerController.getIsAdaptive();
    ...
    protected _beforeUpdate(options: IStackTemplateOptions): void {
        const item = ManagerController.getController()?.find(options.popupId);
        if (item) {
            const result = item.controller.onTemplateUpdated?.(item, this._container);
            if (result) {
                ManagerController.getController()?.update(item.id, item.popupOptions);
            }
        }
        this._updateMaximizeButton(options);
    }
    ...
    private _updateMaximizeButton(options: IStackTemplateOptions): void {
        if (
            options.maxWidth - options.minWidth < MINIMIZED_STEP_FOR_MAXIMIZED_BUTTON ||
            this._isAdaptive
        ) {
            this._maximizeButtonVisibility = false;
        } else {
            this._maximizeButtonVisibility =
                options.maximizeButtonVisibility && !this._options.isTablet;
        }
    }
    ...
    toggleMaximizeState(maximized?: boolean): void {
         ...
         */
        this._notify('maximized', [maximized]);
    }
