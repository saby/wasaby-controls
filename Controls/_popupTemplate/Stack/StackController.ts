...
import {
    ...
    savePopupWidth,
    IStackSavedConfig,
} from 'Controls/_popupTemplate/Util/PopupWidthSettings';
...
export class StackController extends BaseController {
    ...
    elementMaximized(
        ...
        const state =
            maximized !== null
                ? maximized
                : !this.getMaximizedState(item, isPopup);
        const middleWidth = this._getMiddleWidth(
            item,
            this._getMaxPanelWidth(item, isPopup)
        );
        ...
        let maxWidth;
        if (
            item.maxSavedWidth < item.popupOptions.maxWidth &&
            middleWidth >= item.maxSavedWidth
        ) {
            maxWidth = item.popupOptions.maxWidth;
        } else {
            maxWidth = item.maxSavedWidth || item.popupOptions.maxWidth;
        }
        item.popupOptions.width = item.popupOptions.maximized
            ? maxWidth
            : minWidth;
        this._prepareSizes(item, container);
        this._update();
    ...
    private _updateMaximizedState(item: IStackItem, state?: boolean): void {
        ...
        const maximized =
            state !== undefined
                ? state
                : this._getMaximizedState(item, maxPanelWidth);
    ...
    getMaximizedState(item: IStackItem, isPopup: boolean = true): boolean {
        ...
        return this._getMaximizedState(
            item,
            this._getMaxPanelWidth(item, isPopup)
        );
    }

    protected _getMaxPanelWidth(
        item: IStackItem,
        isPopup: boolean = true
    ): number {
    ...
    private _getMaximizedState(
        ...
        if (
            ...
            const middle = this._getMiddleWidth(item, item.popupOptions?.maxWidth || maxPanelWidth);
            let width = item.popupOptions.width;
            if (maxPanelWidth && width > maxPanelWidth) {
                width = maxPanelWidth;
            }
    ...
    private _prepareSizes(item: IStackItem, container?: HTMLElement): void {
        ...
        if (container) {
            ...
            templateContainer =
                this._getStackContentWrapperContainer(container);
        ...
        for (let i = 0; i < optionsSet.length; i++) {
            if (optionsSet[i][property]) {
                if (typeof optionsSet[i][property] === 'string') {
                    if (
                        Object.keys(themeConstants).includes(
                            optionsSet[i][property]
                        )
                    ) {
    ...
    private _getDefaultConfig(item: IStackItem): void {
        ...
        if (
            StackStrategy.isMaximizedPanel(item) &&
            !item.popupOptions.propStorageId
        ) {
            // set default values
            item.popupOptions.templateOptions.maximizeButtonVisibility =
                undefined; // for vdom dirtyChecking
    ...
    private _getItemPosition(item: IStackItem): IPopupPosition {
        ...
        item.popupOptions.currentWidth = position.width;
        ...
        item.popupOptions.stackMinimizedWidth =
            item.popupOptions.minimizedWidth;
    ...
    private _updatePopupWidth(item: IStackItem, width: number): void {
        if (!width && item.popupState !== this.POPUP_STATE_INITIALIZING) {
            item.containerWidth = this._getContainerWidth(
                this._getPopupContainer(item.id)
            );
        }
    }

    private _getStackContentWrapperContainer(
    ...
    private _hidePopup(item: IStackItem): void {
        if (!item.popupOptions.shouldNotHideOverlayedPopup) {
    ...
    private _prepareMaximizedState(item: IStackItem): void {
        ...
        if (!canMaximized) {
            ...
            item.popupOptions.templateOptions.maximized =
                item.popupOptions.maximized;
        }
    }
    ...
    private _getPopupWidth(item: IStackItem): Promise<number | void> {
        return getPopupWidth(item.popupOptions.propStorageId).then(
            (width?: number) => {
                if (width) {
    ...
    static calcStackParentCoords(item: IStackItem): IPopupPosition {
        ...
        if (!isNewEnvironment()) {
            let stackRoot = document.querySelector(
                '.ws-float-area-stack-root'
            ) as HTMLElement;
            const isNewPageTemplate = document.body.classList.contains(
                'ws-new-page-template'
            );
            ...
            rootCoords = BaseController.getRootContainerCoords(
                item,
                selector,
                rightPanelWidth
            );
        ...
        const leftPageScroll =
            detection.isMobilePlatform || detection.safari
                ? 0
                : rootCoords.leftScroll;
        ...
        const documentDimensions =
            DimensionsMeasurer.getElementDimensions(elDimensions);
        const stackParentCoords = {
            top: Math.max(rootCoords.top, 0),
            height: rootCoords.height,
            right:
                documentDimensions.clientWidth -
                rootCoords.right +
                leftPageScroll,
