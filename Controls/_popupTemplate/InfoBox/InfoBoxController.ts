...
class InfoBoxController extends StickyController {
    ...
    private _prepareInfoboxConfig(
        ...
        const config = {
            direction: {
                horizontal: topOrBottomSide
                    ? INVERTED_SIDES[alignSide]
                    : SIDES[side],
                vertical: topOrBottomSide
                    ? SIDES[side]
                    : INVERTED_SIDES[alignSide],
            },
            targetPoint: {
        ...
        const verticalOffset: number = this._getVerticalOffset(
            target,
            topOrBottomSide,
            alignSide
        );
    ...
    private _removeHiddenElement(item: IStickyItem): boolean {
        ...
        const targetHidden: boolean =
            !!this._getTargetNode(item)?.closest('.ws-hidden');
        if (targetHidden) {
