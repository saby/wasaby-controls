...
/**
 * Базовый шаблон {@link https://n.sbis.ru/article/909d49d9-67d2-44d1-88dc-202b2ab5ab4e всплывающей подсказки}.
 *
 * @remark
 * Полезные ссылки:
 * <ul>
 *     <li>{@link https://n.sbis.ru/article/909d49d9-67d2-44d1-88dc-202b2ab5ab4e руководство разработчика}</li>
...
export default class InfoboxTemplate extends Control<IInfoboxTemplateOptions> {
    ...
    protected _getBackgroundStyle(): string {
        let backgroundStyle = this._options.backgroundStyle;
        if (this._borderStyle === 'invalid' || this._borderStyle === 'invalidAccent') {
            backgroundStyle = 'danger';
        }
        return ` controls-InfoBoxTemplate__backgroundGradientStyle-${backgroundStyle}`;
    }
        ...
        if (options.stickyPosition) {
            this._maxWidthOffset = Math.max(
                (options.stickyPosition.position?.maxWidth || DEFAULT_MIN_OFFSET_WIDTH) -
                    (options.stickyPosition.position?.width ||
            ...
            this._maxHeightOffset = Math.max(
                (options.stickyPosition.position?.maxHeight || DEFAULT_MIN_OFFSET_HEIGHT) -
                    (options.stickyPosition.position?.height ||
 ...
 * @variant success
 * @variant danger
 * @demo Controls-demo/PopupTemplate/Infobox/BorderStyle/Index
 */
/**
...
/**
 * @name Controls/_popupTemplate/InfoBox#arrowVisible
 * @demo Controls-demo/PopupTemplate/Infobox/ArrowVisible/Index
 * @cfg {Boolean} Определяет видимость стрелочки, указывающей на target инфобокса
 * @default true
 */
