...
const themeConstantsGetter = (className, hashMap) => {
    ...
    for (const key in hashMap) {
        if (hashMap.hasOwnProperty(key)) {
            obj[key] = parseInt(computedStyles.getPropertyValue(hashMap[key]), 10);
        }
    }
...
function getConstants(themeName: string, className: string, hashMap: object): object {
    ...
    if (!constantsInit) {
        constantsInit = new Promise<object>((resolve, reject) => {
            ...
            import('Controls/popupTemplate')
                ...
                .then(() => {
                    resolve(getConstants(ManagerController.getTheme(), className, hashMap));
                });
        });
    }
