...
} from 'Controls/_popupTemplate/Sticky/StickyController';
import {
    IPopupSizes,
    IStickyPopupOptions,
    IStickyPosition,
...
import getTargetCoords, {
    ITargetCoords,
} from 'Controls/_popupTemplate/TargetCoords';
    ...
    if (item.popupOptions.restrictiveContainer) {
        ...
        if (
            ...
            restrictiveContainer =
                item.popupOptions.restrictiveContainer._container;
        } else if (
            item.popupOptions.restrictiveContainer instanceof HTMLElement
        ) {
            ...
            if (!restrictiveContainer) {
                restrictiveContainer = document.querySelector(
                    item.popupOptions.restrictiveContainer
                );
            }
        }
...
export const _getWindowWidth = (element?: HTMLElement): number => {
    return (
        ...
        DimensionsMeasurer.getWindowDimensions(element).innerWidth
    );
};
...
export const _getWindowHeight = (element?: HTMLElement): number => {
    return (
        constants.isBrowserPlatform &&
        DimensionsMeasurer.getWindowDimensions(element).innerHeight
    );
};
...
export function getStickyConfig(
    item: IStickyItem,
    sizes: IPopupSizes = {}
): IStickyPositionConfig {
    ...
    return {
        ...
        direction: cMerge(
            cClone(DEFAULT_OPTIONS.direction),
            item.popupOptions.direction || {}
        ),
        offset: cMerge(
            cClone(DEFAULT_OPTIONS.offset),
...
export function getRoundClass({ hasRoundedBorder, options, type }): string {
    if (hasRoundedBorder) {
        if (type === 'header') {
            if (
                !(options.bodyContentTemplate || options.footerContentTemplate)
            ) {
