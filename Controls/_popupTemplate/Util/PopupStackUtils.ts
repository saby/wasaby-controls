...
function getMiddleWidth(item: IStackItem, maxPanelWidth: number): number {
    if (
        ...
        if (item.popupOptions.minWidth > MIN_MAXIMIZED_WIDTH || item.popupOptions.maxWidth < MIN_MAXIMIZED_WIDTH) {
...
function getMaximizedState(item: IStackItem, isPopup: boolean = true): boolean {
    ...
    return _getMaximizedState(
        item,
        _getMaxPanelWidth(item, isPopup)
    );
}
