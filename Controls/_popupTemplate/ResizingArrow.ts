...
import { IResizingArrow, IResizingArrowPosition } from './interface/IResizingArrow';
...
class ResizingArrow extends ResizingBase<IResizingArrow> {
    ...
    protected _getMinMaxOffsetValue(offsetValue: number, minMax: number[]): number {
        if (offsetValue > 0) {
    ...
    drag(dragObjectOffset: IDragObjectOffset): void {
        ...
        if (this._options.position.includes('top')) {
            dragObjectOffset.y = -1 * yValue;
        } else {
            dragObjectOffset.y = yValue;
        }
        if (this._options.position.includes('left')) {
            dragObjectOffset.x = -1 * xValue;
        } else {
            dragObjectOffset.x = xValue;
        }
        ...
        if (xValue > 0) {
            horizontalStyle =
                `width: calc( 100% + ${xValue}px); height: ${Math.abs(yValue)}px;` + offset.y.style;
            if (this._options.position.includes('left')) {
        ...
        if (yValue > 0) {
            ...
            verticalStyle =
                `height: calc( 100% + ${yValue}px); width: ${Math.abs(xValue)}px;` + offset.x.style;
            if (this._options.position.includes('top')) {
    ...
    private _getOffset(offsetX: number, offsetY: number): { xValue: number; yValue: number } {
        if (this._options.position.includes('top')) {
            offsetY *= -1;
        }
        if (this._options.position.includes('left')) {
