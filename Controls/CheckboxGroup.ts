    ...
    implements ISource, IMultiSelectable, IHierarchy, IToggleGroup
{
    ...
    protected _beforeUpdate(newOptions: ICheckboxGroupOptions): void {
        ...
        if (
            ...
            !isEqual(this._selectedKeys, newOptions.selectedKeys)
        ) {
    ...
    private sortGroup(options: ICheckboxGroupOptions, items: RecordSet): void {
        this._groups = {};
        items.each((item) => {
            const parent = options.parentProperty
    ...
    private _isSelected(item: Record): boolean | null {
        if (
            this._selectedKeys.indexOf(this._getItemKey(item, this._options)) >
            -1
        ) {
            return true;
        }
        if (
            this._triStateKeys.indexOf(this._getItemKey(item, this._options)) >
            -1
        ) {
            return null;
        }
    ...
    private _setItemsSelection(
        ...
        if (parentId) {
            ...
            const isParentSelected =
                this._selectedKeys.indexOf(this._getItemKey(parent, options)) >
                -1;
            if (isParentSelected) {
