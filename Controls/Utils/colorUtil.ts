...
import {
    complementaryHueForDarkTheme,
    isDarkTheme,
} from 'Controls/Utils/Helpers/functions';
...
export function toRgb(rawColor: string): IRgba {
    ...
    if (result) {
        ...
        if (result) {
            ...
            Logger.warn(
                `hexToRgb: ${color} не является валидным rgb, rgba или hex-цветом.`
            );
            return {
                r: 255,
                g: 255,
...
export function hslToRgb(
    ...
    const s =
        (saturation > MAX_HSL_VAL ? MAX_HSL_VAL : saturation) / MAX_HSL_VAL;
...
export function getComplementaryColor(dominantColor: IRgb): IRgb {
    ...
    if (isDarkTheme(dominantColor)) {
        ...
        complementaryInHsv = {
            ...
            s:
                dominantColorInHsv.s +
                (dominantColorInHsv.s > 0.2 ? 1 - dominantColorInHsv.s : 0.8),
