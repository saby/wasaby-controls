...
export function toRgb(rawColor: string): IRgba {
    ...
    if (result) {
        ...
        if (result) {
            ...
            const hslParts = /hsla?\(([\d.]+),([\d.]+)%?,([\d.]+)%?,?([\d.]+)?\)?/i.exec(color);
            if (hslParts) {
                const a = hslParts[4] ? parseFloat(hslParts[4]) : 1;
                const rgb = hslToRgb(
                    parseInt(hslParts[1], 10),
                    parseInt(hslParts[2], 10),
                    parseInt(hslParts[3], 10),
                    a
                );
                if (rgb) {
                    return {
...
const RGB_BLACK_COLOR = '0, 0, 0';

export function getRGBAMix(rgb1: number[], rgb2: number[]): IRgb {
    const alphaIndex = 3;

    const r = Math.round(
        rgb1[0] * rgb1[alphaIndex] * (1 - rgb2[alphaIndex]) + rgb2[0] * rgb2[alphaIndex]
    );
    const g = Math.round(
        rgb1[1] * rgb1[alphaIndex] * (1 - rgb2[alphaIndex]) + rgb2[1] * rgb2[alphaIndex]
    );
    const b = Math.round(
        rgb1[2] * rgb1[alphaIndex] * (1 - rgb2[alphaIndex]) + rgb2[2] * rgb2[alphaIndex]
    );
    return {
        r,
        g,
...
function getBrightnessRGBA(brightness: number): number[] {
    const maxOpacity = 0.7;
    const maxBrightness = MAX_BRIGHTNESS;
    const maxColor = 255;
    const normalizeOpacityCoefficient = maxOpacity / maxBrightness;
    const opacity = Math.abs(brightness * normalizeOpacityCoefficient);

    const color = brightness < 0 ? 0 : maxColor;
    return [color, color, color, opacity];
}

/**
 * Вычисляет доминантный цвет, в зависимости от яркости.
 * @param dominantColor доминантный цвет
 * @param brightnessRGB яркость
 */
export function getMixDominantColorAndBrightness(
    dominantColor: string,
    brightnessRGB: number[]
): IRgb {
    const dominantColorInRgb = toRgb(`rgb(${dominantColor})`);
    return getRGBAMix(
        [dominantColorInRgb.r, dominantColorInRgb.g, dominantColorInRgb.b, dominantColorInRgb.a],
        brightnessRGB
    );
}

/**
 * Вычисляет комплиментарный цвет, в зависимости от яркости
 * @param dominantColor доминантный цвет
 * @param brightness яркость
 * @param showGradient наличие градиента
 * @public
 */
export function getComplementaryColorRGB(
    dominantColor: string,
    brightness: number,
    showGradient: boolean
): string {
    const isBlackGradient = showGradient && dominantColor === RGB_BLACK_COLOR;
    if (isBlackGradient) {
        return '';
    }

    const brightnessRGB = getBrightnessRGBA(brightness);
    const mixDominantColor = getMixDominantColorAndBrightness(dominantColor, brightnessRGB);
    const complementaryColor = getComplementaryColor(mixDominantColor);
    return `${complementaryColor.r},${Math.round(complementaryColor.g)},${Math.round(
...
export function isDarkTheme(dominantColor: IRgb): boolean {
    ...
    const { a, ...hsl } = hexToHsla(hex);
    return isDarkThemeHSL(hsl);
}

export function isDarkThemeHSL(dominantColor: IHsl): boolean {
    return dominantColor.l <= 65;
}
