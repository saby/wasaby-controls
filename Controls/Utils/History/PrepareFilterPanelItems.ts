...
export function prepareFilterPanelHistoryItems(
    ...
    items.each((item) => {
        ...
        item.setKeyProperty(COPY_ORIG_ID);
    });
...
export function isHistoryItem(item: Model): boolean {
...
export function setHistoryKeyProperty(items: RecordSet) {
    items.setKeyProperty(COPY_ORIG_ID);
    items.each((el) => {
        el.setKeyProperty(COPY_ORIG_ID);
    });
}

export function hasHistoryElements(items: RecordSet) {
