...
import { THistoryField, THistoryFields } from './interfaces/IHistoryFields';
import { THistoryId } from 'Controls/HistoryStore';

export const COPY_ORIG_ID = 'copyOriginalId';
export const COPY_ORIG_PARENT = 'copyOriginalParent';

export const HISTORY_UPDATE_RECENT_DELAY = 50;
...
function setItemsHistoryProperties(items: RecordSet, historyId: THistoryId): void {
...
function prepareOldItems(oldItems: RecordSet, items: RecordSet) {
...
function prepareOriginItems(
    ...
    const items = new RecordSet({
        ...
        format: oldItems.getFormat(true),
    ...
    prepareOldItems(oldItems, items);

    setItemsHistoryProperties(items, historyId);
    items.assign(
        fillHistoryItems(filteredHistory, oldItems, items, historySourceConfig, historyId)
    );

    const itemsToAdd: Model[] = [];
    oldItems.forEach((item: Model): void => {
        ...
        const id =
            typeof key === 'string'
                ? key
                : String(key !== undefined ? key : item.get(historySourceConfig.keyProperty));
        const isHistoryItem = historyIds.indexOf(id) !== -1;
        if (item.get(COPY_ORIG_ID)?.includes('_history') && !isHistoryItem) {
            // Если запись продублирована для истории, игнорируем ее, она может быть уже не актуальна.
            return;
        }
        let newItem;
        if (!isHistoryItem || needDuplicateItem(item, historySourceConfig)) {
            ...
            newItem.set('pinned', filteredHistory.pinned.indexOf(id) !== -1);
            if (isHistoryItem && !needDuplicateItem(item, historySourceConfig)) {
                setHistoryFields(
                    item,
                    COPY_ORIG_ID,
            ...
            if (oldItems.getKeyProperty() === COPY_ORIG_ID) {
                //не дублируем элементы которые уже есть в коллекции с историей
            ...
            const hItem = items.getRecordById(id);
            if (
                hItem &&
                hItem.has('pinned') &&
                hItem.get('pinned') !== filteredHistory.pinned.includes(id)
            ) {
                try {
                    hItem.set('pinned', filteredHistory.pinned.indexOf(id) !== -1);
                } catch (e) {
                    //
                }
            }
    ...
    items.setMetaData(oldItems.getMetaData(), true);

    return items;
}

function fillHistoryItems(
    filteredHistory: any,
    oldItems: RecordSet,
    items: RecordSet,
    historySourceConfig: IHistorySourceConfig,
...
function prepareHistoryItem(item: Model, historyType: string): void {
    ...
    return item.has('group') && item.set('group', 'CONTROLS_HIDDEN_GROUP');
}
...
function fillItems(
    ...
    let historyItem;
    const { parentProperty, nodeProperty, root = null, keyProperty } = historySourceConfig;
    history[historyType].forEach((id: string) => {
        ...
        const isHistoryOldItems = oldItems.getFormat().getFieldIndex(COPY_ORIG_ID) !== -1;
        if (isHistoryOldItems) {
            const oldItemIndex = oldItems.getIndexByValue(COPY_ORIG_ID, id);
            oldItem = oldItems.at(oldItemIndex) || oldItems.getRecordById(id);
        } else {
            if (oldItems.getKeyProperty() === COPY_ORIG_ID) {
                oldItems.setKeyProperty(keyProperty);
            }
...
function fillFrequentItems(
    ...
    // alphabet sorting
    return resultItems.sort((first, second): number => {
...
export function updatePinned(
    ...
    const { Store } = getHistoryStore();
    const needPin = !!meta.$_pinned;

    const pinned = Store.getLocal(historyId)?.pinned;
    if (needPin && !checkPinnedAmount(pinned)) {
        showNotification();
        return Promise.reject();
    }
    return Store.togglePin(historyId, item.getKey(), needPin).catch((error) => {
        process(error);
    });
}
...
export function getItemsWithHistory(
    ...
    if (config && historyId) {
        ...
        return prepareOriginItems(items, historyId, config);
    } else {
        return items;
    }
}

function getHistoryStore(): any {
    let historyStoreLib = null;
    try {
        historyStoreLib = loadSync('Controls/HistoryStore');
    } catch {
        Logger.error(
            'Controls/_dropdown/PrepareHistory: попытка взаимодействия с незагруженной историей'
        );
    }
    return historyStoreLib;
}
