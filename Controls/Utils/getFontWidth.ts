...
function calcFontWidth(value: string, size: TFontSize, fonts: []): number {
    const currentSize = document?.body.classList.contains('_enable-bigger-fonts-for-phones')
    && unsafe_getRootAdaptiveMode().device.isPhone()
        ? hashMapWithSizesForAdaptive[size] : size;
    let text = value;
    if (!text) {
        text = '';
    }
    ...
    for (let i = 0; i < text.length; i++) {
        ...
        const charWidth = fonts[currentSize] ? fonts[currentSize][text[i]] || fonts[currentSize].default : 0;
        textWidth += charWidth;
    }
