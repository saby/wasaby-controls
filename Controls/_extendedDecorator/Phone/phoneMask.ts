 ...
 */
/* eslint-disable */
import { toString, IFormatMaskChars } from 'Controls/baseDecorator';
...
function detectCityCode(phone: string, regionCode: string): string {
    ...
    for (let lengthCode = 1; lengthCode < 3; lengthCode++) {
        const code = phone.substr(
            LENGTH_AREA_CODE + LENGTH_REGION_CODE,
            lengthCode
        );
...
function mobilePhoneMask(phone: string, self?: object): string {
    const controlChars: RegExpMatchArray = toString(phone, self).match(
        CONTROL_CHARS
    );
    ...
    if (areaCode === '7') {
        const regionCode = numPhone.substr(
            LENGTH_AREA_CODE,
            LENGTH_REGION_CODE
        );
    ...
    if (Boolean(DICTIONARY.foreignCodes[areaCode])) {
        const lengthForeignCode: number = detectLengthForeignCode(
            numPhone,
            areaCode
        );
