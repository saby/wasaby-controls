...
interface IMultilineTextDecoratorOptions extends IComponentProps {
    /**
...
export default React.memo(function MultilineText(
    ...
    const mainClass = 'tw-break-words controls-ExtendedDecorator-MultilineText';
    ...
    textLines.forEach((text, index) => {
        ...
        if (text !== '') {
            if (isUseWrapUrls(text)) {
                // WrapUrl по умолчанию строится с display: block; для сохранения переносов в сафари нам нужны <br>,
                // блочные элементы создают лишние переносы строк
                result.push(
                    <WrapURLs
                        value={text}
                        className="tw-inline"
                ...
                result.push(<span key={'text_' + uniqId}>{text}</span>);
            }
        }
    });
