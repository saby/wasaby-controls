...
export default React.memo(function WrapURLs(props: IWrapURLsOptions): React.ReactElement {
    ...
    return (
        <div ref={props.$wasabyRef} {...getAttrs(props.attrs, props.className, mainClass)} data-qa={props['data-qa']}>
            {parsedText.map((path, index) => {
                if (path.type === 'link') {
                    ...
                    /* eslint-disable react/no-array-index-key */
                    return (
                        <a className="controls-WrapURLs_link" key={index} href={'mailto:' + path.address}>
                            {path.address}
                        </a>
                    );
                    /* eslint-enable react/no-array-index-key */
