...
export interface IWrapURLsOptions extends IComponentProps {
    /**
     * @name Controls/_extendedDecorator/WrapURLs#value
     * @cfg {String} Декорируемый текст.
     * @demo Controls-demo/Decorator/WrapURLs/Index
     */
    value: string;
    /**
     * @name Controls/_extendedDecorator/WrapURLs#newTab
    ...
    newTab?: boolean;
    /**
     * @name Controls/_extendedDecorator/WrapURLs#highlightedValue
...
export default React.memo(function WrapURLs(props: IWrapURLsOptions): React.ReactElement {
    ...
    if (props.value && typeof props.value !== 'string') {
        Logger.error(
            '[Controls.extendedDecorator:WrapUrls] Передано не корректно значение для опции "value". ' +
                `Ожидается что придет строка, но пришло "${typeof props.value}". ` +
                'Откройте React profiler, чтобы понять в каком месте передается не корректное значение и исправьте ошибку.'
        );
        return null;
    }
        ...
        >
            {parsedText.map((path, index) => {
                if (path.type === 'link') {
                    ...
                    return (
                        <a
                            className="controls-WrapURLs_link"
                            key={index}
                            rel="noreferrer"
