...
function getLoadPromisesFromDescription(
    ...
    description.forEach((property: IProperty) => {
        ...
        if (property.editorOptions?.source) {
            let propertyValue = object.getPropertyValue(editingObject, property.name);

            // Для совместимости с фильтром, пока значение хранится в структуре, а не в фильтре
            if (propertyValue === undefined && property.value !== undefined) {
    ...
    promises.push(
        loadAsync('Controls/dataSource').then(() => {
            ...
            if (
                ...
                (property.type === 'lookup' || property.type === 'dropdown')
            ) {
            ...
            return sourceController
                ...
                .then((loadResult) => {
                    ...
                    if (property.type === 'dropdown' && propertyChanged) {
