...
const getFactories: () => Record<TCollectionType, IAspectsFactory> = () => {
    ...
    return {
        ...
        Tile: addStub(addOperationsPanel(addFlatSelection(getDefaultFlatFactory()))),
        AdaptiveTile: addStub(addOperationsPanel(addFlatSelection(getDefaultFlatFactory()))),
        TreeTile: addStub(addOperationsPanel(addHierarchySelection(getDefaultHierarchyFactory()))),
        ExpandedCompositeTree: addStub(
            addOperationsPanel(addHierarchySelection(getDefaultHierarchyFactory()))
        ),
    };
};
...
export function createAspects(
    ...
    removedAspects?.forEach((aspectName) => {
        viewModeAspectsFactories.delete(aspectName);
    });
