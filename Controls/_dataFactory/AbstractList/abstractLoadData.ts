...
import { loadCollectionModule } from './collections/loadCollectionModule';
import { IFilterResult, isNeedPrepareFilter, prepareFilterIfNeed } from './loadData/prepareFilter';
import { abstractLoadData as newAbstractLoadData } from 'Controls-DataEnv/abstractList';
...
export async function abstractLoadData(
    ...
    const [operationsController, filterLoadResult, newLoadDataResult] = await Promise.all([
        getOperationsController(config),
        prepareFilterIfNeed(config),
        newAbstractLoadData({
            ...config,
            isLatestInteractorVersion:
                typeof config.isLatestInteractorVersion === 'boolean'
    ...
    return {
        ...
        isLatestInteractorVersion: newLoadDataResult.isLatestInteractorVersion,
    };
}
