...
const stateOwnActionsLogger = DispatcherLogger.getMiddlewareLogger({
    name: 'state',
...
export const state: (
        ...
        if (!savedState) {
            ...
            Logger.error(
                'Внутренняя ошибка платформы.\n' +
                    'Controls/_dataFactory/ListWebDispatcher/middlewares/state :: ' +
                    'вызван setState во время асинхронной очереди, но стейт перед запуском очереди не был сохранен.\n ' +
                    'Возможна коллизия стейтов.'
            );
        }
        savedState = {
            ...savedState,
    ...
    return [
        (next) => async (action: TListActionsWithSetState) => {
            if (isRejected) {
                DispatcherLogger.getMiddlewareLogger({
                    name: 'state',
                }).info(
                ...
                switch (action.type) {
                    ...
                    default: {
                        ...
                        masterDispatchPromise = new Promise<void>((resolve, reject) => {
                            ...
                            .then(() => {
                                DispatcherLogger.info('Успешное завершение верхнего экшена.');

                                if (!savedCallback) {
                                    DispatcherLogger.info('Состояние не менялось.');
                                    return;
                                }

                                DispatcherLogger.info('Начало применения финального состояния.');

                                Promise.resolve(savedCallback()).then(() => {
                                    DispatcherLogger.info('Конец применения финального состояния.');
                                });
                            })
                            .then(() => {
                                onFinally();
                            ...
                            .catch((e) => {
                                if (e === Rejected) {
                                    ...
                                    DispatcherLogger.info('Установка состояния прервана вручную.');
                                } else {
