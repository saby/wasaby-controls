...
export default class ListSlice<T extends IListState = IListState> extends Slice<T> {
    ...
    private _task1189832611: boolean = false;
    ...
    private _subscribeOnControllersChanges(): void {
        ...
        if (this._filterController && !this._task1189832611) {
    ...
    protected _initState(loadResult: IListLoadResult, initConfig: IListDataFactoryArguments): T {
        ...
        this._task1189832611 = config.task1189832611;
        let listActions;
        ...
        if (config.previousViewMode && config.fallbackImage) {
            ...
            this._previousViewMode = config.viewMode;
        }
        this._subscribeOnControllersChanges();
    ...
    private __beforeApplyStateInternal(nextState: T): T | Promise<T> {
        ...
        if (rootChanged && !needReload && this._newItems) {
            const rootItems = this._newItems;
            this._newItems = null;
            return this.__dataLoadedInner(rootItems, undefined, nextState).then((newState) => {
                this.state.sourceController.setItemsAfterLoad(rootItems);
                return {
                    ...nextState,
                    ...newState,
        ...
        if (searchValueChanged) {
            if (nextState.searchValue) {
                const loadPromise = this._search(nextState.searchValue, nextState)
                    .then(({ items, root }) => {
                        const misspellValue = this._getSearchMisspellValue(items);
                        const stateAfterSearch = {
                            ...nextState,
                            root,
                            searchValue: needChangeSearchValueToSwitchedString(items)
                                ? misspellValue
                                : nextState.searchValue,
                            searchMisspellValue: misspellValue,
                        };
        ...
        if (!loadingPromises.length && nextState.operationsPanelVisible) {
    ...
    reload(sourceConfig?: INavigationSourceConfig): Promise<RecordSet> {
        ...
        return this.state.sourceController.reload(sourceConfig).then((items: RecordSet) => {
            return this.__dataLoadedInner(items, undefined, this.state).then((newState) => {
    ...
    load(
        ...
        return this._load.apply(this, [this.state, ...arguments]).catch((error) => {
    ...
    protected _needRejectBeforeApply(partialState: Partial<T>): boolean {
        const currentOptions = this._getSourceControllerOptions(this.state);
        const newOptions = this._getSourceControllerOptions({ ...this.state, ...partialState });
        return Object.keys(currentOptions).some((key) => {
            return !isEqual(currentOptions[key], newOptions[key]);
        });
    }
    ...
    private _search(value: string, state: IListState = this.state): Promise<unknown> {
        ...
        this._hasHierarchyFilterBeforeSearch = hasHierarchyFilter(state.filter);
        this._hasRootInFilterBeforeSearch = state.filter.hasOwnProperty(this.state.parentProperty);
        const promises = [];
        const { sourceController, viewMode, parentProperty } = state;
        const searchViewMode = resolveSearchViewMode(state.adaptiveSearchMode, viewMode);
        ...
        const filterForSearch = getFilterForSearch(state, searchValue, this._rootBeforeSearch);
        promises.push(this._load(state, undefined, rootForSearch, filterForSearch, false));
    ...
    private _resetSearch(state: T): Promise<TLoadResult> {
        ...
        return state.sourceController
            ...
            .then((result: TLoadResult) => {
                ...
                return this.__dataLoadedInner(result as RecordSet, direction, {
                    ...state,
                    loading: false,
                    root: key !== undefined ? key : state.root,
    ...
    private _getLoadResult(nextState: T): Partial<T> {
        ...
        const breadcrumbsState = this._getStateAfterUpdateItems(nextState);
        if (rootChanged) {
            if (
    ...
    private _getSelectionViewMode(nextState): string {
        ...
        if (needCalcSelectionViewMode && isLoaded('Controls/operations')) {
            const { getSelectionViewMode } = loadSync('Controls/operations');
