...
import type { IItemsState } from '../AbstractList/aspects/Items/IItemsState';
...
import { ISelection, TFilter, TKey, TViewMode } from 'Controls-DataEnv/interface';
import { IErrorConfig, IListState } from './_interface/IListState';
...
import { TListAspects } from '../AbstractList/aspects/factory';
 ...
 * @extends Controls/_dataFactory/AbstractList/AbstractListSlice
...
export default class ListSlice<T extends IListState = IListState> extends AbstractListSlice<T> {
    ...
    protected _aspectStateManagers: TListAspects;
    ...
    protected _onCollectionChange(
        ...
        if (this._aspectStateManagers.Items) {
    ...
    protected _initState(loadResult: IListLoadResult, initConfig: IListDataFactoryArguments): T {
        ...
        let state: T = {
            ...
            markItemByExpanderClick:
                config.markItemByExpanderClick !== undefined
                    ? config.markItemByExpanderClick
                    : true,
        };
    ...
    protected _beforeApplyState(nextState: T): T | Promise<T> {
        ...
        const draftNextState = this._collection
            ? {
                  ...nextState,
                  ...this._getNextState(this.state, this._resolveChanges(this.state, nextState)),
              }
            : nextState;

        // Вторая фаза расчета nextState
        let nextStateResult = this.__beforeApplyStateInternal(draftNextState);
        if (nextStateResult instanceof Promise) {
            // Третья фаза расчета nextState асинхронная. Применение изменений к коллекции
            return nextStateResult.then(this._resolveNextState.bind(this)).finally(() => {
                this.observeChanges();
            });
        } else {
            // Третья фаза расчета nextState синхронная. Применение изменений к коллекции
            nextStateResult = this._resolveNextState(nextStateResult);
            this.observeChanges();
        }
                ...
                expandedItemsDiff.added.forEach((key: CrudEntityKey) => {
                    if (!this.hasLoaded(key)) {
                        loadingPromises.push(
                            this._load(nextState, null, key).then(() => {
                                return nextState;
                            })
                        );
                    }
    ...
    private _resolveNextState(nextState: T): T {
        ...
        return {
            ...
            // Не костыль, специчный код, вызванный мутабельностью RecordSet.
    ...
    private _getLoadResult(nextState: T, items: RecordSet): Partial<T> {
        ...
        const markedKey = this._processMarkedKey(nextState);
        if (
     ...
     */
    private _processMarkedKey(nextState: Partial<T>): TKey {
        const rootChanged = nextState.root !== this.state.root;
        const markedKeyChanged = nextState.markedKey !== this.state.markedKey;
        const breadcrumbs = this.state.breadCrumbsItems || [];
        ...
        if (isGoingToDepth) {
            ...
            markedKey = nextMarkedKey !== undefined ? nextMarkedKey : this.state.root;
        }
        return markedKey;
    }
    ...
    mark(key: TKey, params: unknown = {}): void {
        if (this._aspectStateManagers.Marker) {
            this.setState(
                this._aspectStateManagers.Marker.handleItemClick(this.state, key, params)
            );
        }
    }

    select(key: TKey): void {
        let newState: Partial<IListState>;

        if (this._aspectStateManagers.Selection) {
            newState = this._aspectStateManagers.Selection.toggleItemSelection(this.state, key);
        }
        if (this._aspectStateManagers.Marker) {
            newState = {
                ...newState,
                ...this._aspectStateManagers.Marker.setMarker(this.state, key),
            };
        }
        this.setState(newState);
    }
    ...
    expand(key: CrudEntityKey): void {
        ...
        if (this._aspectStateManagers.ExpandCollapse) {
            newState = this._aspectStateManagers.ExpandCollapse.expand(this.state, key);
        }
        if (this._aspectStateManagers.Marker && this.state.markItemByExpanderClick) {
            newState = {
                ...newState,
                ...this._aspectStateManagers.Marker.setMarker(this.state, key),
            };
    ...
    collapse(key: CrudEntityKey): void {
        ...
        if (this._aspectStateManagers.ExpandCollapse) {
            newState = this._aspectStateManagers.ExpandCollapse.collapse(this.state, key);
        }
        if (this._aspectStateManagers.Marker && this.state.markItemByExpanderClick) {
            newState = {
                ...newState,
    ...
    isExpanded(key: CrudEntityKey): boolean {
        if (this._aspectStateManagers.ExpandCollapse) {
            return this._aspectStateManagers.ExpandCollapse.isExpanded(this.state, key);
        }
    }
