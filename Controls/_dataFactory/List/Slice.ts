...
function getRootForSearch(path: Model[], curRoot: TKey, parentProperty: string): TKey {
    let root;
...
function getFilterForSearch(
    ...
    if (parentProperty) {
        if (state.root !== undefined) {
            root = getRootForSearch(state.breadCrumbsItems, state.root, parentProperty);
...
export default class ListSlice<T extends IListState = IListState> extends Slice<T> {
    ...
    protected _getViewModeDependencies(viewMode: TViewMode): string[] {
        ...
        if (viewTemplate) {
            ...
            if (viewModeTemplate && viewMode === 'tile') {
    ...
    private _getSearchMisspellValue(): string {
        return getSwitcherStrFromData(this._sourceController.getItems());
    }
