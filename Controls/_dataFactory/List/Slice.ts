...
import { dataLoadedInner } from '../ListWebDispatcher/middlewares/source';
...
export default class ListSlice<T extends IListState = IListState> extends AbstractListSlice<T> {
    ...
    protected async __beforeApplyStateInternal(draftStateProp: T): Promise<T> {
        this.executingActionToDispatch = draftStateProp._actionToDispatch;
        ...
        await this._dispatcher.dispatch(
            beforeApplyStateAction(
                draftStateProp,
                this._propsForMigrationToDispatcher,
                this.executingActionToDispatch
            )
        );
    ...
    reload(sourceConfig?: INavigationSourceConfig, keepNavigation?: boolean) {
        ...
        this._addAction(sourceActions.reload(sourceConfig, keepNavigation, onResolve, onReject));
    ...
    _setPreloadedItems(items: RecordSet, direction: ScrollControllerLib.IDirection): Promise<void> {
        this._propsForMigrationToDispatcher.sliceProperties.newItems = items;
        this._propsForMigrationToDispatcher.sliceProperties.newItemsDirection = direction;
        const { promise, resolve } = getDecomposedPromise();
        this.setState({
            _loadItemsToDirectionPromiseResolver: resolve,
        });
        return promise;
    }
    ...
    _loadItemsToDirection(
        ...
        return this.state.sourceController
            .load(direction, undefined, undefined, false, undefined, undefined, useServicePool)
            .then((items) => {
                if (addItemsAfterLoad === false) {
                    return items;
                }
                return this._setPreloadedItems(items, direction).then(() => items);
            })
            .catch(async (error) => {
                if (this._needProcessError(error)) {
                    const action = () => {
        ...
        return loadPromise
            .then((result: TLoadResult) => {
                ...
                return dataLoadedInner({
                    ...
                    // Загрузка уйдет в мидлвару, которая умеет получать снапшоты.
                    // Публичный метод выдавать не имеет смысла.
                    // @ts-ignore
                    snapshots: this._dispatcher._snapshotsStore,
                }).then((newState) => {
                    if (!disableSetState) {
                        if (filter) {
