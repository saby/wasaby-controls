...
import * as sourceActions from '../ListWebDispatcher/actions/source';

import type { TCookieLogKey, TDispatcherLoggerInstance } from 'Controls-DataEnv/listDebug';
...
import { cookie, logger as AppLogger } from 'Application/Env';
...
const COOKIE_LOG_KEY: TCookieLogKey = 'ListInteractorDebug';
...
export default class ListSlice<T extends IListState = IListState> extends AbstractListSlice<T> {
    ...
    private _executingActionToDispatch?: Map<string, TListAction>;
    ...
    protected _initAspects(
        ...
        super._initAspects(collectionType, state, [AspectsNames.Marker]);
    }
    ...
    protected async _beforeApplyState(nextStateProp: T): Promise<T> {
        ...
        const nextStateResult = await this.__beforeApplyStateInternal({
            ...draftNextState,
            items: nextState.items,
        });

        await getUnloadedDeps(nextStateResult, UI_DEPENDENCIES);
    ...
    protected async __beforeApplyStateInternal(draftStateProp: T): Promise<T> {
        this._executingActionToDispatch = draftStateProp._actionToDispatch;

        const logger = await this._resolveLoggerIfNeed();

        if (this.isDestroyed()) {
            return this.state;
        }

        logger?.start(this.state, draftStateProp);
        ...
        logger?.end(this.state, nextState);
     ...
     * @param {Partial<IListState> | ((prevState: IListState) => Partial<IListState>} nextState
     * @private
     * @return void
     */
    private _addAction(
        action: TListAction,
        nextState?: Partial<IListState> | ((prevState: IListState) => Partial<IListState>)
    ): void {
        this.setState((currentState) => {
            const notExecutingActions = new Map<string, TListAction>();
            ...
            if (currentState._actionToDispatch) {
                // Экшены, которые уже установлены на стейт и будут выполняться.
            ...
            // Устанавливаем переданный эшен в очередь.
            notExecutingActions.set(action.type, action);
    ...
    private _addActionAsync<T>(action: TListAction): Promise<T> {
        const { promise, resolve, reject } = getDecomposedPromise<T>();
    ...
    private async _resolveLoggerIfNeed(): Promise<TDispatcherLoggerInstance | void> {
        const isLoggerLoaded = isLoaded(DEBUG_LIB);

        if (isLoggerLoaded || (!isLoggerLoaded && cookie.get(COOKIE_LOG_KEY))) {
            if (!isLoggerLoaded) {
                await loadAsync(DEBUG_LIB);
            }
            return loadSync<typeof import('Controls-DataEnv/listDebug')>(
                DEBUG_LIB
            ).DispatcherLogger.create(
    ...
    reload(sourceConfig?: INavigationSourceConfig, keepNavigation?: boolean) {
        return this._addActionAsync(sourceActions.reload(sourceConfig, keepNavigation));
    }
    ...
    async reloadItem(itemKey: CrudEntityKey, options?: IReloadItemOptions): IReloadItemResult {
        return this._addActionAsync(sourceActions.reloadItem(itemKey, options));
    }
    ...
    reloadItems(keys: TKey[]): IReloadItemsResult {
        return this._addActionAsync(sourceActions.reloadItems(keys));
    }
    ...
    _setPreloadedItems(items: RecordSet, direction: ScrollControllerLib.IDirection): Promise<void> {
        return this._addActionAsync(
            sourceActions.setPreloadedItems({
                direction,
                items,
            })
        );
    ...
    _loadItemsToDirection(
        ...
        return this._addActionAsync(
            sourceActions.loadToDirectionOld({
                retryAction: () => {
    ...
    mark(key: CrudEntityKey | null): void {
        this._addAction(ListActionCreators.marker.setMarkedKey(key));
    }
    ...
    changeRoot(root: CrudEntityKey): void {
        ...
        if (rootAspect && this.state.root !== nextPartialState.root) {
            const newMarkedKey = processMarkedKey(state, {
                ...state,
                ...nextPartialState,
            });
            this._addAction(ListActionCreators.marker.setMarkedKey(newMarkedKey), nextPartialState);
        } else {
    ...
    next(): void {
        this._addAction(
            sourceActions.loadToDirectionNew({
                direction: 'down',
            })
        );
    }
