...
function getFilterResolver(): typeof import('Controls/search').FilterResolver {
...
export default class ListSlice<T extends IListState = IListState> extends Slice<T> {
    ...
    private __beforeApplyStateInternal(nextState: T): T | Promise<T> {
        ...
        if (operationsPanelVisibleChanged) {
            if (nextState.operationsPanelVisible) {
                ...
                nextState.selectionViewMode = 'hidden';
        ...
        if (rootChanged) {
            ...
            if (nextState.searchValue) {
                nextState.filter = getFilterResolver().getResetSearchFilter(
                    nextState.filter,
        ...
        this._changeCursorBeforeLoad(nextState);
        const needReload =
            nextState.sourceController.updateOptions(this._getSourceControllerOptions(nextState)) ||
            needReloadBySelectionViewMode;
        if (rootChanged && !needReload && this._newItems) {
            const rootItems = this._newItems;
            this._newItems = null;
            return this.__dataLoadedInner(rootItems, undefined, nextState, viewModePromise).then(
                (newState) => {
                    nextState.sourceController.setItemsAfterLoad(rootItems);
                    return {
                        ...nextState,
                        ...newState,
        ...
        if (viewModeChanged && !this._isViewModeLoaded(nextState.viewMode)) {
            viewModePromise = this._loadViewMode(nextState.viewMode).then(() => {
                return nextState;
            });
    ...
    private _getSearchMisspellValue(items: RecordSet = this._sourceController.getItems()): string {
        return getFilterResolver().getSwitcherStrFromData(items);
    }
    ...
    private _getLoadResult(nextState: T, items: RecordSet): Partial<T> {
        ...
        const breadcrumbsState = { ...this._getStateAfterUpdateItems(nextState), ...breadCrumbs };
        if (rootChanged) {
            if (
        ...
        if (
            ...
            newRoot = getFilterResolver().getRootForSearch(
        ...
        if (searchParam) {
            ...
            if (searchValue) {
                newFilter = getFilterResolver().getFilterForSearch(
                    nextState,
                    nextState.searchValue,
                    this._rootBeforeSearch
                );
            } else if (this.state.searchParam) {
                newFilter = getFilterResolver().getResetSearchFilter(
                    nextState.filter,
                    nextState.searchParam,
                    nextState.parentProperty,
                    !this._hasHierarchyFilterBeforeSearch,
     ...
     * @private
     */
    private _changeCursorBeforeLoad(nextState: T): void {
        // Делаем смену курсора только если настроена курсорная навигация
        const rootChanged = nextState.root !== this.state.root;
        if (!rootChanged || nextState.navigation?.source !== 'position') {
            return;
        }
