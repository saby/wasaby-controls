...
import { isExpanded, isExpandAll } from '../ListWebInitializer/expandCollapse';
import { initMarker } from '../ListWebInitializer/marker';
import { initItemActions } from '../ListWebInitializer/itemActions';
...
import { calculateFilterByFilterDescription } from '../AbstractList/utils/calculateFilterByFilterDescription';
import { createSourceController } from 'Controls/_dataFactory/ListWebInitializer/source';
import { isStateHierarchy } from 'Controls/_dataFactory/List/utils';
...
> {
    ...
    observeChanges(state: IListState = this.state): void {
        this._subscribe(state);
    }

    unobserveChanges(state: IListState = this.state): void {
    ...
    protected _subscribe(state: T): void {
    ...
    protected _unsubscribe(state: T): void {
    ...
    protected _initState(loadResult: IListLoadResult, initConfig: IListDataFactoryArguments): T {
        ...
        const state: T = {
            ...
            fix88221033390078: initConfig.fix88221033390078,
            activeElement,
            data: items,
        ...
        if (this.collection) {
            ...
            initItemActions(state, this.collection);
        }
    ...
    protected _getMiddlewaresContext(): TListMiddlewareContext {
        return {
            ...
            scheduleDispatch: (action: TListActions.TAnyListAction<T>) => {
            ...
            needRejectBeforeApply: this._needRejectBeforeApply.bind(this),
    ...
    resetExpansion(): void {
        this._addAction(ListActionCreators.expandCollapse.resetExpansion());
    }

    isExpanded(key: CrudEntityKey): boolean {
        Logger.warn('Метод Slice.isExpanded устарел и будет в скором времени удален');
        if (isStateHierarchy(this.state)) {
            return isExpanded(this.state, key);
        }
        return false;
    }

    isExpandAll(): boolean {
        Logger.warn('Метод Slice.isExpandAll устарел и будет в скором времени удален');
        if (isStateHierarchy(this.state)) {
            return isExpandAll(this.state);
        }
        return false;
    }
