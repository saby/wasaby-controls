...
export default class ListSlice<T extends IListState = IListState> extends AbstractListSlice<T> {
    ...
    protected async _beforeApplyState(nextState: T): Promise<T> {
        if (this.isDestroyed()) {
            return this.state;
        }
    ...
    protected async __beforeApplyStateInternal(nextState: T): Promise<T> {
        ...
        if (operationsPanelVisibleChanged) {
            if (nextState.operationsPanelVisible) {
                ...
                if (this._aspectStateManagers.has(AspectsNames.Marker)) {
                    nextState.markedKey = this._aspectStateManagers
                        .get(AspectsNames.Marker)
                        .calculateMarkedKeyForVisible(nextState, this._collection);
                }
            } else {
        ...
        if (needReload && (!searchValueChanged || !nextState.searchParam)) {
            ...
            const loadingPromise = this._reload(nextState)
                ...
                .catch((error) => {
                    return getStateAfterLoadError(this.state, nextState, error, nextState.root);
                });
            loadingPromises.push(loadingPromise);
        }
