...
import { IFilterItem, IUserPeriod, IPrefetchOptions } from 'Controls/filter';
...
// @ts-ignore
import type { Form, TFilter, TKey, TViewMode } from 'Controls-DataEnv/interface';
...
/**
 * @name Controls/_dataFactory/List/_interface/IListDataFactoryArguments#items
 * @cfg {RecordSet} Задаёт набор записей, которые требуется отобразить.
 * @remark Если задать опцию {@link items}, то запроса к источнику в методе loadData списочной фабрики не будет.
 */
...
/**
 * Класс для обработки ошибок загрузки с помощью обработчиков.
 * Обработчики - это функции. Они нужны для непосредственной обработки ошибки.
 * Обработчики получают на вход параметры с ошибкой, и возвращают кофигурацию для отображения.
 * Позволяет реализовать специфичную логику обработки ошибок.
 * Старайтесь избегать специфичной логики. В большинстве случаев можно обойтись стандартными инструментами.
 * Подробнее можно прочитать в {@link /doc/platform/developmentapl/interface-development/application-architecture/error-handling/error-handling-tools/ статье}
 * @name Controls/_dataFactory/List/_interface/IListDataFactoryArguments#errorController
...
/**
 * @name Controls/_dataFactory/List/_interface/IListDataFactoryArguments#countFilterValue
 * @cfg {Array.<Date> | String} Выбранное значение {@link Controls-ListEnv/CountFilter фильтра по счетчикам}.
 * @see countFilterLinkedNames
 * @see countFilterValueConverter
 * @see countFilterUserPeriods
 * @see countFilterPeriodType
 */

/**
 * @name Controls/_dataFactory/List/_interface/IListDataFactoryArguments#countFilterLinkedNames
 * @cfg {Array.<String>} Связные фильтры для {@link Controls-ListEnv/CountFilter фильтра по счетчикам}. При изменении выбранного значения контрола "Фильтр по счетчикам" у этих фильтров будет изменено поле filter.
 * @see countFilterValue
 * @see countFilterValueConverter
 * @see countFilterUserPeriods
 * @see countFilterPeriodType
 */
...
/**
 * @name Controls/_dataFactory/List/_interface/IListDataFactoryArguments#countFilterPeriodType
 * @cfg {'last' | 'current'} Тип выбираемого периода для {@link Controls-ListEnv/CountFilter фильтра по счетчикам}
...
/**
 * @name Controls/_dataFactory/List/_interface/IListDataFactoryArguments#countFilterUserPeriods
 * @cfg {Control/filter:IUserPeriod[]} Пользовательские периоды для {@link Controls-ListEnv/CountFilter фильтра по счетчикам}, будут добавлены как пункты меню.
...
/**
 * @name Controls/_dataFactory/List/_interface/IListDataFactoryArguments#countFilterValueConverter
 * @cfg {String} Функция обратного вызова для получения значения фильтра после выбора из {@link Controls-ListEnv/CountFilter фильтра по счетчикам}.
 ...
 * Аргументы функции:
 *
 * * value ({string | Date[]}) — новое значение счетчика;
 * * filterItem ({@link Controls/filter:IFilterItem}) — связанный фильтр, для которого применяется фильтр;
 * * items (тип {@link Controls/filter:IFilterItem}[]) — структура фильтров.
 *
 * @example
 * Пример функции конвертера.
 * <pre>
 *     function MyCountFilterValueConverter(value: string | Date[], filterItem: IFilterItem, items: IFilterItem[]) => {
 *         return {
 *            Счетчик: value,
 *         };
 *     }
 ...
 * @name Controls/_dataFactory/List/_interface/IListDataFactoryArguments#itemActions
 * @cfg {Array.<Controls/interface:IAction>}
 */

/**
 * @name Controls/_dataFactory/List/_interface/IListDataFactoryArguments#itemsOrder
...
export interface IListDataFactoryArguments
    ...
    // Опция необходима для случая, когда загрузку инициализирует не слайс.
    // добавлена по задаче: https://online.sbis.ru/opendoc.html?guid=116fff17-13df-4466-9188-b6fadeccc75c&client=3
    // Будет удалена в 25.1100
    fix88221033390078?: boolean;
}
