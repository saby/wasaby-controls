...
export default function loadData(
    ...
    const operationsPromise = getOperationsController(config, Router);
...
function getOperationsController(
    ...
    // Костыль, пока не починят работу Feature/local в автотестах
    // https://dev.sbis.ru/doc/190123b2-92f7-4aeb-8cc1-8e315a784f17?client=3
    const hostName = (Router || getRootRouter()).url.location.hostname;
    const isAutoTest =
        cookie.get('autotest') ||
        hostName.includes('test-autotest') ||
        hostName.includes('prognix');
    if (!isAutoTest && !config.sliceOwnedByBrowser) {
        featurePromise = Feature.require(['pmo_show_selected']);
    } else {
        featurePromise = Promise.resolve();
    }

    return Promise.all([controllerPromise, featurePromise]).then(([controller]) => {
        return controller;
    });
}
