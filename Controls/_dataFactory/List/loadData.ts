...
export default function loadData(
    ...
    let newSliceEnvPromise;

    if (config.collectionType) {
        newSliceEnvPromise = Promise.all([
...
function getFilterWithHistory(config: IListDataFactoryArguments): Promise<IFilterResult> {
    ...
    if (config.historyId && !filterFromUrl) {
        ...
        if (historyPromise instanceof Promise) {
            historyPromise = wrapHistoryPromise<IFilterItem[] | undefined>(historyPromise);
        }
    } else {
