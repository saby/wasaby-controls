...
export default function loadData(
    ...
    if (config.listConfigStoreId) {
        Object.assign(config, getListState(config));
    }
    ...
    if (isNeedPrepareFilter(config)) {
        ...
        filterPromise
            ...
            .catch((error) => {
                ...
                Logger.error(
                    'DataLoader: ошибка при подготовке фильтра для запроса',
                    this,
                    error
                );
            });
        filterPromise = wrapTimeout(
            filterPromise,
            QUERY_PARAMS_LOAD_TIMEOUT
        ).catch(() => {
            Logger.info(
    ...
    if (config.propStorageId) {
        paramsPromise = loadSavedConfig(config.propStorageId, [
            'sorting',
            'storedColumnsWidths',
        ]);
        paramsPromise = wrapTimeout(
            paramsPromise,
            QUERY_PARAMS_LOAD_TIMEOUT
        ).catch(() => {
            Logger.info(
                'Controls/dataSource:loadData: Параметры не загрузились за 1 секунду'
            );
        });
    }
    ...
    return Promise.all([filterPromise, paramsPromise, operationsPromise]).then(
        ([, paramsPromiseResult, operationsController]: [
            TFilter,
            ISortingOptions,
            any
        ]) => {
            ...
            const storedColumnsWidths =
                paramsPromiseResult?.storedColumnsWidths ||
                config.storedColumnsWidths;
            ...
            if (filterDescription) {
                if (
                    ...
                    isNeedLoadFilterData(filterDescription)
                ) {
...
function loadFilterData(
    ...
    return loadAsync<typeof import('Controls/dataFactory')>(
...
function isNeedPrepareFilter(
    loadDataConfig: IListDataFactoryArguments
): boolean {
    return !!(
...
function needPrepareFilterItem({
    type,
    editorTemplateName,
    viewMode,
}: IFilterItem): boolean {
...
function prepareFilterSource(
    ...
    const getFilter = (
        ...
        if (propertyChanged && editorsViewMode !== 'popupCloudPanelDefault') {
            resultFilter[editorOptions.keyProperty] = Array.isArray(value)
                ? value
                : [value];
        }
...
function loadFilterCallbacks(
    filterDescription: IFilterItem[]
): Promise<Function[]> {
    return loadSync<typeof import('Controls/filter')>(
        'Controls/filter'
    ).loadCallbacks(filterDescription);
}

function isFilterCallbacksLoaded(filterDescription: IFilterItem[]): boolean {
    return loadSync<typeof import('Controls/filter')>(
        'Controls/filter'
    ).isCallbacksLoaded(filterDescription);
}
...
function getFilterControllerWithFilterHistory(
    ...
    if (isFilterCallbacksLoaded(controller.getFilterButtonItems())) {
        historyPromise =
            config.historyItems || controller.loadFilterItemsFromHistory();
    } else {
        historyPromise = loadFilterCallbacks(
            controller.getFilterButtonItems()
        ).then(() => {
            return (
                config.historyItems || controller.loadFilterItemsFromHistory()
            );
    ...
    if (config.propStorageId) {
        ...
        if (historyPromise instanceof Promise) {
            ...
            configurationPromise = controller.loadFilterConfiguration(
                config.propStorageId
            );
        }
    }
    return Promise.all([historyPromise, configurationPromise]).then(
        ([historyPromiseResult]) => {
            ...
            if (
                typeof config.filterButtonSource !== 'function' &&
                config.task1186685666
            ) {
                controller.update({
                    ...config,
...
function getListState(config: IListDataFactoryArguments) {
    const storage = loadSync<typeof import('Controls/dataSource')>(
        'Controls/dataSource'
    ).getControllerState;
    return storage(config.listConfigStoreId);
}
...
export function getFilterController(
...
export function getSourceController(
    options: IListDataFactoryArguments
): NewSourceController {
    ...
    if (options.sourceController) {
        ...
        const controllerClass = loadSync<typeof import('Controls/dataSource')>(
            'Controls/dataSource'
        ).NewSourceController;
