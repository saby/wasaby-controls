...
import { IBaseSourceConfig, ISortingOptions, TSortingOptionValue } from 'Controls/interface';
...
import {
    ...
    IFilterResult,
    isNeedPrepareFilter,
} from '../AbstractList/abstractLoadData';
...
export default async function loadData(
    ...
    return Promise.all([paramsPromise, rootHistoryPromise]).then(
        ([paramsPromiseResult, rootFromHistory]: [
            IFilterResult,
            ISortingOptions,
            any,
            void,
...
function loadParams(propStorageId): Promise<ILoadParamsResult> {
    ...
    let paramsPromise = Promise.all([
        ...
        USER.load([sortingUserParamId, countFilterUserParamId]),
...
function getLoadResult(
    ...
    if (loadResult.error) {
        return getConfigAfterLoadError(
            loadResult,
            loadResult.error,
            config.root
...
function loadFilterDescriptionDeps(
    ...
    if (config.countFilterValueConverter && typeof config.countFilterValueConverter === 'string') {
        countFilterPromise = loadAsync(config.countFilterValueConverter);
        addPageDeps([config.countFilterValueConverter]);
    }
        ...
        loadFilterDataPromise = Promise.all([loadFilterCallbacksPromise, countFilterPromise]).then(
            () => {
                ...
                const filterDescriptionWithFilterCount = FilterDescription.applyFilterCounter(
                    countFilterValue,
                    filterDescr,
                    config.countFilterLinkedNames,
                    config.countFilterValueConverter
                );
