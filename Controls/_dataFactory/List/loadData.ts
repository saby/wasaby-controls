...
export default function loadData(
    ...
    if (isNeedPrepareFilter(config)) {
        if (config.filterHistoryLoader instanceof Function) {
            filterPromise = getFilterControllerWithHistoryFromLoader(config, _clearResult);
        } else {
            if (isLoaded('Controls/filter')) {
                filterPromise = getFilterControllerWithFilterHistory(config, _clearResult);
            } else {
                filterPromise = loadAsync('Controls/filter').then(() => {
                    return getFilterControllerWithFilterHistory(config, _clearResult);
                });
            }
        }
        ...
        filterPromise
            ...
            .catch((error) => {
                filterController = getFilterController(config as IFilterControllerOptions, _clearResult);
    ...
    if (config.listActions && typeof config.listActions === 'string') {
        listActionsPromise = loadAsync(config.listActions).then((listActions: IActionOptions[]) => {
            addPageDeps([config.listActions]);
            if (listActions) {
                const modules = [];
    ...
    return Promise.all([filterPromise, paramsPromise, operationsPromise, listActionsPromise]).then(
        ([, paramsPromiseResult, operationsController]: [TFilter, ISortingOptions, any, void]) => {
...
function loadFilterData(
    ...
    const filterStructurePromise = prepareFilterSource(filterSource, editorsViewMode);
    const dataFactoryPromise = loadAsync<typeof import('Controls/dataFactory')>('Controls/dataFactory');
    return Promise.all([filterStructurePromise, dataFactoryPromise]).then(([filterStructure, {PropertyGrid}]) => {
...
function needLoadEditorOptions({ type, value, resetValue, editorTemplateName }: IFilterItem): boolean {
...
function loadEditorOptions(item: IFilterItem): Promise<object> {
    ...
    return loadAsync<Function | object>(item.editorOptionsName).then(loadedEditorOptions => {
        if (loadedEditorOptions instanceof Function) {
            resultEditorOptions = loadedEditorOptions(editorOptions).then(loadedOptions => {
                return {
                    ...editorOptions,
                    ...loadedOptions,
                };
...
function prepareFilterSource(description: IFilterItem[], editorsViewMode: string): Promise<IFilterItem[]> {
    const getFilter = (
    ...
    return Promise.all(description.map((item) => {
        return item.editorOptionsName && needLoadEditorOptions(item) ? loadEditorOptions(item) : undefined;
    ...
    return config
        ...
        .then((result: IFilterHistoryLoaderResult) => {
            const controller = getFilterController({
                ...config,
                ...result,
            } as IFilterControllerOptions, _clearResult);
...
function getFilterControllerWithFilterHistory(
    ...
    const controller = getFilterController(config as IFilterControllerOptions, _clearResult);
    let historyPromise;
...
export function getFilterController(options: IFilterControllerOptions, _clearResult?: boolean): FilterController {
    ...
    if (_clearResult) {
        delete clearOptions.staticColumns;
        delete clearOptions.dynamicColumn;
        delete clearOptions.staticHeaders;
        delete clearOptions.dynamicHeader;
    }

    return new controllerClass(clearOptions);
}
