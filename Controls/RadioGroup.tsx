...
export class Control extends React.Component<IRadioGroupProps, IRadioGroupState> {
    ...
    protected _getNextActiveItem(): Model {
        ...
        const thisActiveItem = this.state.items.getRecordById(this.state.selectedKey);
    ...
    protected _isSelected(item: Record): boolean {
        ...
        const parent = this.props.parentProperty
            ? (this.state.items.getRecordById(this.state.selectedKey)?.get?.(this.props.parentProperty) || null)
            : null;
        if (parent) {
    ...
    protected _setWorkByKeyboard(workByKeyboard: IWorkByKeyboardOptions): void {
        this._highlightedOnFocus = workByKeyboard?.status && !this._getReadOnly();
    }
