 ...
 * @public
 * @demo Controls-demo/datePopup/Base/Index
 *
 */
export default class DatePopup extends Control {
    ...
    protected _dateRangeYearSelection: boolean = false;
    protected _monthSelectionProcessing: boolean = false;

    protected _monthsRangeYearSelectionProccesing: boolean = false;
    ...
    protected _endKeyDownHandler(event: Event): void {
        if (event.nativeEvent.keyCode === constants.key.enter) {
            this._applyResult();
        }
    }
    ...
    protected _onMonthsRangeYearSelectionStarted(event: Event): void {
        event.stopPropagation();
        this._monthsRangeYearSelectionProccesing = true;
    }
    ...
    _yearsSelectionStarted(e: SyntheticEvent, start: Date, end: Date): void {
        ...
        this._monthsRangeYearSelectionProccesing = false;
    }
    ...
    protected _monthRangeChanged(event: SyntheticEvent, startValue: Date, endValue: Date): void {
        if (!this._monthSelectionProcessing && !this._dateRangeYearSelection) {
            return;
        }
        if (
            endValue &&
            ...
            if (this._dateRangeYearSelection) {
                this.rangeChanged(
                    dateUtils.getStartOfYear(startValue),
                    dateUtils.getEndOfYear(endValue)
                );
            } else {
    ...
    protected _onMonthRangeYearSelectionEnded(): void {
        if (this._monthRangeSelectionProcessing && !detection.isMobilePlatform) {
            this._rangeModel.startValue = this._memorizedValues.startValue || null;
            this._rangeModel.endValue = this._memorizedValues.endValue || null;
        }
        this._monthRangeSelectionProcessing = false;
    ...
    _dateRangeSelectionEnded(e: SyntheticEvent, start: Date, end: Date): void {
        ...
        if (this._dateRangeYearSelection) {
            const endValue = dateUtils.getEndOfYear(end);
            this.sendResult(start, endValue);
        }
    }
    ...
    toggleState(date?: Date): void {
        ...
        } else if (this._options._displayDate) {
            displayedDate = this._options._displayDate;
        } else {
