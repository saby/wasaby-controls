...
import {
    ...
    TDataConfigs,
    TConfigLoadResult,
} from 'Controls-DataEnv/dataLoader';
...
export default class InputContainer extends Control<IInputControllerOptions> {
    ...
    private _getFirstItemByOrder(
        loadResult: TConfigLoadResult
    ): IListLoadResult {
    ...
    private _getSourceController(id = this._tabsSelectedKey): SourceController {
        if (this._loadResult) {
            return (this._loadResult[id] || this._loadResult[DEFAULT_LOADER_ID])
                ?.sourceController;
        }
    }
    ...
    private _prepareFilter(
        ...
        if (
            inputSearchValue.length < minSearchLength &&
            historyKeys &&
            historyKeys.length
        ) {
    ...
    private _setFilter(
        ...
        const suggestConfig =
            resultId && listsOptions ? listsOptions[resultId] : void 0;
    ...
    _makeLoad(options: IInputControllerOptions): Promise<TConfigLoadResult> {
        ...
        this._suggestLoader = new CancelablePromise<TConfigLoadResult>(
            loadPromise
        );
