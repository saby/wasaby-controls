...
export default class InputContainer extends Control<IInputControllerOptions> {
    ...
    private _loadResult = null;
    private _searchLibraryLoader: CancelablePromise<
        typeof import('Controls/search')
    > = null;
    private _suggestTemplate: ISuggestTemplateProp = null;
    private _suggestLoader: CancelablePromise<IDataConfigs> = null;
    ...
    private _getFirstItemByOrder(array) {
    ...
    private _getSourceController(): SourceController {
        let sourceControllerId;

        if (this._options.suggestListsOptions) {
            if (this._tabsSelectedKey !== null) {
                sourceControllerId = this._tabsSelectedKey;
            } else {
                const listsOptionsValues = Object.values(
                    this._options.suggestListsOptions
                );
                sourceControllerId =
                    this._getFirstItemByOrder(listsOptionsValues).id;
            }
        }
        return this._getDataLoader().getSourceController(
            sourceControllerId as string
        );
    }
    ...
    private _close(options?: IInputControllerOptions): void {
        ...
        if (
            ...
            !this._dependenciesDeferred.isReady()
        ) {
    ...
    private _needLoadHistory(): boolean {
        return (
            ...
            !this._shouldSearch(this._inputSearchValue)
        );
    }

    private _loadHistoryKeys(): Promise<HistoryKeys> {
        return this._getRecentKeys().then((keys: string[]) => {
            const filter: QueryWhereExpression<unknown> = clone(
    ...
    private _suggestDirectionChangedCallback(
        direction: TSuggestDirection
    ): void {
    ...
    private _inputActivated(event?: SyntheticEvent): Promise<void | RecordSet> {
        const target = (event?.nativeEvent?.target ||
            this._getActiveElement()) as HTMLElement;

        if (
            (this._options.autoDropDown || this._options.historyId) &&
            !this._options.readOnly &&
            (detection.isMobileIOS || target.tagName === 'INPUT')
        ) {
            if (
                !this._options.suggestState &&
                (this._options.source || this._options.suggestListsOptions) &&
                !this._isLoading() &&
    ...
    private _prepareFilter(
        ...
        if (
            this._inputSearchValue.length < minSearchLength &&
            this._historyKeys &&
            this._historyKeys.length
        ) {
            preparedFilter[HISTORY_KEYS_FIELD] = this._historyKeys;
        }
    ...
    private _setFilter(
        ...
        const resultId = options.suggestListsOptions ? tabId : void 0;
        const suggestConfig =
            resultId && options.suggestListsOptions
                ? options.suggestListsOptions[resultId]
                : void 0;
        ...
        this._getDataLoader(options)
            .getSourceController(resultId)
            .setFilter(this._filter);
    }

    private _getEmptyTemplate(
        emptyTemplate: IEmptyTemplateProp
    ): IEmptyTemplateProp | string {
    ...
    private _shouldSearch(value: string): boolean {
        return (
            this._inputActive &&
            this._isValueLengthLongerThenMinSearchLength(value, this._options)
        );
    }

    private _updateSuggestState(
        isValueReseted: boolean = false,
        tabId?: string | number
    ): boolean {
        const shouldSearch = this._shouldSearch(this._inputSearchValue);
        const shouldShowSuggest = this._shouldShowSuggest(
            this._getSourceController().getItems()
        );
        let state = false;
        ...
        if (this._needLoadHistory() && !this._options.suggestState) {
            ...
            if (!this._options.suggestListsOptions) {
                ...
                this._setFilter(
                    this._options.suggestListsOptions[tabId].filter,
                    undefined,
                    tabId
                );
            }
            ...
            !this._options.autoDropDown &&
            (!shouldShowSuggest || !this._inputSearchValue)
        ) {
    ...
    private _getTemplatesToLoad(options: IInputControllerOptions): string[] {
        ...
        if (options.suggestListsOptions) {
            ...
            keys.forEach((key) => {
                templateNames.push(
                    options.suggestListsOptions[key].footerTemplate
                        ?.templateName
                );
            });
        }
    ...
    private async _openSelector(templateOptions: object): Promise<unknown> {
        if (
            (await this._notify('showSelector', [
                templateOptions,
            ...
            return import('Controls/suggestPopup').then(() => {
                return StackOpener.openPopup(
                    this._getSelectorOptions(templateOptions)
                );
            });
        }
    }

    private _isInvalidValidationStatus(
        options: IInputControllerOptions
    ): boolean {
        return (
            options.validationStatus === 'invalid' ||
        ...
        return {
            templateOptions: {
                ...
                templateOptions: {
                    ...
                    templateOptions:
                        this._options.suggestTemplate.templateOptions,
                    emptyTemplate: this._emptyTemplate,
                    source:
                        this._options.source ||
                        this._getDataLoader().getState()[this._tabsSelectedKey]
                            .source,
                    minSearchLength: this._options.autoDropDown
                        ? 0
                    ...
                    tabsSelectedKeyChangedCallback:
    ...
    protected _beforeMount(options: IInputControllerOptions): void {
        ...
        this._suggestDirectionChangedCallback =
            this._suggestDirectionChangedCallback.bind(this);
    ...
    protected _beforeUpdate(newOptions: IInputControllerOptions): void {
        ...
        if (
            ...
            (this._showContent || this._isLoading())
        ) {
            this._dataLoader = null;
        ...
        if (emptyTemplateChanged) {
            this._emptyTemplate = this._getEmptyTemplate(
                newOptions.emptyTemplate
            );
        }
    ...
    private _isDefaultFooter(): boolean {
        return (
            this._options.footerTemplate.templateName ===
            'Controls/suggestPopup:FooterTemplate'
        );
    }
    ...
    private _getSuggestTemplate({
        ...
        if (
            suggestTemplate &&
            typeof suggestTemplate.templateName !== 'string'
        ) {
            Logger.error(
    ...
    private _getSearchLibrary(): Promise<typeof import('Controls/search')> {
        if (!this._searchLibraryLoader) {
            this._searchLibraryLoader = new CancelablePromise(
        ...
        if (!this._searchResolverController) {
            ...
            result = this._getSearchLibrary()
                .then((searchLibrary) => {
                    ...
                    this._searchResolverController =
                        new searchLibrary.SearchResolver(
    ...
    private _getDataLoader(
        ...
        if (!this._dataLoader) {
            const sourceConfigs = this._getLoaderConfig(options);
            this._dataLoader = new DataLoader({
                loadDataConfigs: sourceConfigs,
            });
        }
    ...
    private _getLoaderConfig(
        options: IInputControllerOptions = this._options
    ): ILoadDataConfig[] | IInputControllerOptions[] {
        const { suggestListsOptions } = options;
        let configs;

        if (suggestListsOptions) {
            configs = Object.values(suggestListsOptions).map((listOptions) => {
                const clonedListOptions = clone(listOptions);
                clonedListOptions.searchValue = this._inputSearchValue;
                clonedListOptions.searchParam =
                    clonedListOptions.searchParam || options.searchParam;
                clonedListOptions.minSearchLength = options.minSearchLength;
                clonedListOptions.type = 'list';
                clonedListOptions.filter = this._prepareFilter(
                    clonedListOptions.filter,
                    clonedListOptions.searchParam,
                    options.minSearchLength,
                    undefined,
                    options.suggestListsOptions
                );
                clonedListOptions.source = this._getOriginalSource(
                    listOptions.source
                );
                return clonedListOptions;
            });
        } else {
            configs = [
                {
                    ...options,
                    filter: this._filter,
                    source: this._getOriginalSource(options.source),
                    sourceController:
                        this._dataLoader?.getSourceController('suggestLoader'),
    ...
    private _getItemByLoadResult(
        result: TLoadResultMap | unknown[],
        tabId: Key
    ) {
        let resultItem;
        if (tabId) {
            resultItem = result.find(({ id }) => {
                return id === tabId;
            });
        ...
        if (value) {
            ...
            return this._makeLoad(options)
                .then((result) => {
                    ...
                    this._loadResult = this._getDataLoader(options).getState();
        ...
        return this._makeLoad(scopeOptions)
            .then((result) => {
                ...
                if (this._inputActive || this._tabsSelectedKey !== null) {
                    ...
                    if (
                        data instanceof RecordSet &&
                        this._shouldShowSuggest(data)
                    ) {
                        this._setItems(data);
                        this._updateSuggestState(false, tabId);
                        this._loadResult =
                            this._getDataLoader(scopeOptions).getState();
                        return data;
                    } else if (error) {
                        this._searchErrback(error);
                        return error;
    ...
    _makeLoad(options: IInputControllerOptions): Promise<IDataConfigs> {
        if (this._suggestLoader) {
            this._suggestLoader.cancel();
            this._suggestLoader = null;
        }
        this._suggestLoader = new CancelablePromise(
            this._getDataLoader(options).load(this._getLoaderConfig(options))
        );
        return this._suggestLoader.promise;
    }
    ...
    private _getSearchResolverOptions(
        options: IInputControllerOptions
    ...
    private _searchResetCallback(): Promise<void> {
        return this._getSearchController().then((searchController) => {
            if (searchController) {
                if (
                    this._updateSuggestState(true) ||
                    this._options.autoDropDown
                ) {
                    return new Promise((resolve) => {
                        this._setFilter(searchController.reset(true));
                        this._loadSuggestDataWithHistory()
                            .then((recordSet) => {
    ...
    protected _getSearchController(): Promise<SearchController | void> {
        if (!this._searchController) {
            return this._getSearchLibrary()
                .then((result) => {
                    this._searchController = new result.ControllerClass(
                        this._getSearchControllerOptions(this._options)
                    );
                    return this._searchController;
                })
                .catch((error) => {
                    return this._searchErrback(error);
                });
        }
        return Promise.resolve(this._searchController);
    }

    private _getSearchControllerOptions(
        options: IInputControllerOptions
    ...
    protected _inputActivatedHandler(
        event: SyntheticEvent
    ): Promise<void | RecordSet> {
        this._inputActive = true;
        if (
            !this._isInvalidValidationStatus(this._options) &&
            !this._options.openSuggestOnClick
        ) {
    ...
    protected _tabsSelectedKeyChanged(
        tabId: Key
    ): Promise<void | RecordSet> | void {
            ...
            if (this._options.suggestListsOptions) {
                if (!this._isLoading() && !this._isSearchDelayTimerStarted()) {
                    ...
                    this._processResultData(
                        this._loadResult[this._tabsSelectedKey].data
                    );
                }
    ...
    protected _markedKeyChangedHandler(
        event: SyntheticEvent,
        key: string
    ): void {
    ...
    protected _moreClick(): void {
        this._openSelector(
            this._getTemplateOptions(this._filter, this._inputSearchValue)
        );
        this._close();
    }
    ...
    protected _keydown(event: SyntheticEvent<KeyboardEvent>): void {
        ...
        const isInputKey =
            PROCESSED_KEYDOWN_KEYS.INPUT.indexOf(eventKeyCode) !== -1;
        const isListKey =
            eventKeyCode === ENTER_KEY
                ? this._suggestMarkedKey !== null
                : PROCESSED_KEYDOWN_KEYS.SUGGESTIONS_LIST.indexOf(
                      eventKeyCode
                  ) !== -1;
