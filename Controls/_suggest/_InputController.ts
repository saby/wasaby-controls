...
import { IPopupOptions, IStackPopupOptions, Stack as StackOpener } from 'Controls/popup';
...
import { ErrorViewConfig, process } from 'Controls/error';
...
interface IInputControllerOptions
    ...
    onChoose?: (item: Model, tabsSelectedKey: string | number | null) => void;
    onDeactivated?: () => void;
    suggestDirectionChanged?: (_event: React.SyntheticEvent, direction: string) => void;
    onSuggestStateChanged?: (value: boolean) => void;
...
export default class InputContainer extends Control<IInputControllerOptions> {
    ...
    private _historyStoreLoad?: Promise<unknown>;
    ...
    private _searchErrback(
        ...
        if (!error?.canceled && !error?.isCancelled) {
            ...
            if (viewConfig) {
                this._pendingErrorConfig = viewConfig;
                this._open();
            }
        }
    }
    ...
    private _processResultData(data: RecordSet): void {
        ...
        if (data) {
            ...
            this._setMisspellingCaption(loadSync('Controls/search').getSwitcherStrFromData(data));
    ...
    private _prepareFilter(
        ...
        searchParam: string,
        { minSearchLength, ...options }: IInputControllerOptions,
        ...
        preparedFilter[searchParam] = this._prepareSearchValue(
    ...
    private _getTemplatesToLoad(options: IInputControllerOptions): string[] {
        ...
        if (options.suggestListsOptions) {
            Object.values(options.suggestListsOptions).forEach(
                ({ footerTemplate, suggestItemTemplate }) => {
                    if (typeof suggestItemTemplate === 'string') {
                        templateNames.push(suggestItemTemplate);
                    }
                    if (footerTemplate?.templateName) {
                        templateNames.push(footerTemplate?.templateName);
                    }
                }
            );
        }
    ...
    private _loadDependencies(options: IInputControllerOptions = this._options): Promise<unknown> {
        ...
        if (templatesToLoad.length) {
            this._dependenciesDeferred = Promise.all(
    ...
    private _getHistoryStore(): Promise<any> {
        if (!this._historyStoreLoad) {
            this._historyStoreLoad = import('Controls/HistoryStore').then(({ Store }) => {
                return Store;
            });
            return this._historyStoreLoad;
        }
        return this._historyStoreLoad;
    }
    ...
    private _getRecentKeys(): Promise<unknown> {
        ...
        this._getHistoryStore().then((store) => {
            store
                .load(this._options.historyId)
                .then((data) => {
                    const keys = [];
                    data.recent?.each((el) => {
                        keys.push(el.getKey());
    ...
    private _validateOptions({
        ...
        searchParam,
        suggestTemplate,
    }: IInputControllerOptions): void {
        if ((source || suggestListsOptions) && !searchParam) {
            Logger.error('Controls/suggest опция searchParam обязательна для установки.', this);
        }

        if (suggestTemplate && typeof suggestTemplate.templateName !== 'string') {
            Logger.error(
                'Controls/suggest передано некорректное значение опции suggestTemplate. Значение поля templateName должно быть строкой.',
                this
            );
        }
    }
    ...
    private _getTemplateOptions(
        ...
        let source =
            this._loadResult?.[this._tabsSelectedKey]?.source ||
            this._loadResult?.[DEFAULT_LOADER_ID]?.source ||
            this._getSourceController().getSource();
    ...
    protected _beforeUpdate(newOptions: IInputControllerOptions): void {
        ...
        const valueCleared = valueChanged && !newOptions.value;
        ...
        if (!isSuggestOpened && valueCleared) {
            this._searchValue = '';
        }
        ...
        if (
            (this._options.searchDelay !== newOptions.searchDelay ||
                newOptions.minSearchLength !== this._options.minSearchLength) &&
    ...
    private _isDefaultFooter(): boolean {
        return this._footerTemplate.templateName === 'Controls/suggestPopup:FooterTemplate';
    }
    ...
    protected _changeValueHandler(event: SyntheticEvent, value: string): Promise<void> {
        value = value || '';
        const searchValue = this._prepareSearchValue(value);
        this._inputSearchValue = value;
        this._setFilter(this._filter);
        if (this._options.suggestTemplate && searchValue !== this._searchValue) {
    ...
    protected _inputCompletedHandler(
        ...
        this._notify('inputCompleted', [value, newValue, inputs]);
    }
    ...
    protected _getSuggestPopupStyles(suggestWidth: number): string {
        ...
        if (maxWidth) {
            ...
            let width = suggestPopupOptions?.width || suggestWidth;
    ...
    protected _prepareSearchValue(
        inputValue: string,
        options: IInputControllerOptions = this._options
    ): string {
        return options?.searchValueTrim ? inputValue.trim() : inputValue;
    }
    ...
    private _getLoaderConfig(options: IInputControllerOptions = this._options): TDataConfigs {
        ...
        Object.values(suggestOptions).forEach((listOptions) => {
            ...
            const dataFactoryArguments = {
                ...
                historyId: null,
                columns: listOptions.suggestColumns,
                filter,
                searchParam,
        ...
        if (value) {
            ...
            return this._makeLoad(options)
                .then((result) => {
                    ...
                    if (resultItem.error) {
                        this._hideIndicator();
                        this._searchErrback(resultItem.error, resultItem.errorViewConfig);
    ...
    private _performLoad(
        ...
        return this._makeLoad(scopeOptions)
            .then((result) => {
                const { id, data, error, errorViewConfig } = this._getItemByLoadResult(
                    result,
                    tabId || this._tabsSelectedKey
                );
                ...
                if (this._inputActive || this._tabsSelectedKey !== null) {
                    if (data instanceof RecordSet && this._shouldShowSuggest(data)) {
                        ...
                        this._searchErrback(error, errorViewConfig);
                        return error;
                    }
                }
                return data;
    ...
    _makeLoad(options: IInputControllerOptions): Promise<TConfigLoadResult> {
        this._clearCachedData();
        const loadSearchLib = loadAsync('Controls/search');
        ...
        this._suggestLoader = new CancelablePromise<TConfigLoadResult>(
            Promise.all([loadPromise, loadSearchLib])
        );
        return this._suggestLoader.promise.then((result) => {
            this._suggestLoader = null;
            this._configs = this._getLoaderConfig(options);
            return (this._loadResult = result[0]);
        });
    }
    ...
    protected _select(
        ...
        const tabsSelectedKey = selectedKey || this._tabsSelectedKey;
        const historyId =
            this._options.suggestListsOptions?.[tabsSelectedKey ?? '']?.historyId ||
        ...
        if (historyId) {
            return this._getHistoryStore().then((historyStore) => {
                historyStore.push(historyId, newItem.getKey());
                this._historyStoreLoad = undefined;
                return historyStore;
            });
        }
    }
    ...
    protected _loadEnd(result?: RecordSet | Error): void {
        ...
        if (!result?.isCanceled) {
            this._searchValue = this._prepareSearchValue(this._inputSearchValue);
        }
