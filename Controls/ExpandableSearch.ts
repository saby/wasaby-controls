...
export default class ExpandableSearch extends Control<IExpandableSearchOptions> {
    ...
    private _searchInputOffsetRight: number = null;
    private _searchInputOffsetLeft: number = null;
        ...
        if (this._container && this._options.inlineWidth === 'auto') {
            ...
            if (this._options.searchInputDirection === 'left') {
                const searchIconOffset =
                    this?._container?.getBoundingClientRect().right -
                    this._children.searchButton?._container.getBoundingClientRect().right;
                searchInputContainerWidth =
                    this?._container?.parentNode?.getBoundingClientRect().right;
                searchButtonNoExpandedPosition = this?._container?.getBoundingClientRect().right;
                ...
                searchInputContainerWidth =
                    this?._container?.parentNode?.getBoundingClientRect().left;
                searchButtonNoExpandedPosition = this?._container?.getBoundingClientRect().left;
