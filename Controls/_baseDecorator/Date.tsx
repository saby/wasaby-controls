...
import { date, DateFormattingStrategyType, IDateFormatConfig } from 'Types/formatter';
...
export interface IDateOptions
     ...
     * Все доступные маски описаны {@link /docs/js/Types/formatter/Function/date здесь}
     * @demo Controls-demo/Decorator/Date/Index
     */
    format: string;
    /**
...
export default React.memo(function DateDecorator(props: IDateOptions): React.ReactElement {
    const formattedDate: string = formatDate(props);
    const mainClasses = `controls-DecoratorDate controls-fontsize-${props.fontSize}
                         controls-text-${props.readOnly ? 'readonly' : props.fontColorStyle}
    ...
    return (
        ...
        >
            {props.highlightedValue !== null && props.highlightedValue !== undefined ? (
                <Highlight value={formattedDate} highlightedValue={props.highlightedValue} />
            ) : (
                formattedDate
            )}
        </span>
