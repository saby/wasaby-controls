...
export function getMaskParsing(maskKeys: string): RegExp {
    ...
    return new RegExp(
        `${key}(?:${quantifier})?|${pairDelimiter}|${singleDelimiter}`,
        'g'
    );
}
...
export function getReplacingKeyFn(
    ...
    if (replacer) {
        return getReplacingKeyAsValueOrReplacer.bind(
            this,
            formatMaskChars,
            replacer
        );
...
function validatePairDelimiters(
    ...
    switch (charData.subtype) {
        ...
        case 'close':
            const lastDelimiter =
                unclosedDelimiters[unclosedDelimiters.length - 1];
 ...
 */
function processingEmptyReplacer(
    searching: string,
    keyPositions: number[]
): string {
    return keyPositions.reduceRight(
        (acc: string, keyPosition: number): string => {
...
export function getFormat(
    ...
    const replaceKey: TReplaceKey = getReplacingKeyFn(
        formatMaskChars,
        escapeReplacer
    );
    ...
    while (execParseMask) {
        ...
        if (
            charData.type === 'singleDelimiter' ||
            charData.type === 'pairDelimiter'
        ) {
    ...
    if (replacer === '') {
        group.searching = processingEmptyReplacer(
            group.searching,
            keyPositions
        );
    }
