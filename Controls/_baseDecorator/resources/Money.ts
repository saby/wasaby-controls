...
import { toString as numberToString } from 'Controls/Utils/PhoneUtils';
...
type TAbbreviationType = 'short' | 'long' | 'none';
...
export function calculateMainClass(fontColorStyle: string, underline: string): string {
    return removeExtraSpaces(
        'controls-DecoratorMoney' +
            `${underline === 'hovered' ? ' controls-DecoratorMoney__underline' : ''}` +
...
export function calculateCurrencyClass(
    ...
    return removeExtraSpaces(`${currencySize ? 'controls-fontsize-' + currencySize : ''} ${
        fontColorStyle ? ' controls-text-' + fontColorStyle : ''
    }
...
export function calculateIntegerClass(
    ...
    return removeExtraSpaces(
        `${
            fontSize ? 'controls-fontsize-' + fontSize + ' ' + getTextFontSizeClass(fontSize) : ''
        } ${fontColorStyle ? ' controls-text-' + fontColorStyle : ''} ${
            fontWeight ? ' controls-fontweight-' + fontWeight : ''
        } ${currency && currencyPosition === 'left' ? ' controls-margin_left-2xs' : ''} ${
            currency && currencyPosition === 'right' && !isDisplayFractionPathParam
...
export function calculateFractionClass(
    ...
    if (fontColorStyle === 'contrast') {
        ...
        !(fontSize === '5xl' || fontSize === '6xl' || fontSize === '7xl' || fontSize === '8xl') &&
        fontColorStyle === 'default'
    ) {
        postfixStyle = 'contrast-label';
    }

    return removeExtraSpaces(
        'controls-fontweight-default controls-DecoratorMoney__fraction__colorStyle-' +
            postfixStyle +
            ` controls-text-${postfixStyle}` +
            (fractionFontSize
                ? ` controls-fontsize-${fractionFontSize} ${getTextFontSizeClass(fractionFontSize)}`
                : '') +
...
export function calculateTooltip(formattedNumber: IPaths, options: IMoneyOptions): string {
    if (options.hasOwnProperty('tooltip') && options.tooltip !== null) {
...
export default function splitIntoTriads(original: string): string {
    ...
    if (part !== null && part[startChar]) {
         ...
         */
        part[startChar] = Array.prototype.reduceRight.call(part[startChar], reducerRight);
...
export function calculateFormattedNumber(
    ...
    const formattedValue = toFormat(toString(value, precision, false), precision);
    ...
    if (abbreviationType === 'long' || abbreviationType === 'short') {
        const numberValue = value ? (typeof value === 'number' ? value : +value) : 0;
        integer = abbreviateNumber(
            numberValue,
            abbreviationType,
            {
                useGrouping,
...
function toFormat(value: string, precision: number): string {
    ...
    if (value) {
        ...
        if (dotPosition === -1) {
            return value + (precision ? (precision === maxPrecision ? '.0000' : '.00') : '');
        }
...
function toString(value: TValue, precision: number, useGrouping: boolean): string {
    if (value === null) {
        return '';
    }
