...
export function paste(
    original: string,
    value: string,
    position: number = original.length
): string {
    const validPosition = Math.max(0, position);

    return (
        original.slice(0, validPosition) + value + original.slice(validPosition)
    );
}
...
export function replace(
    ...
    return (
        original.slice(0, validPosition) +
        value +
        original.slice(validPosition + value.length)
    );
}
 ...
 */
export function pasteWithRepositioning(
    original: IText,
    value: string,
    position: number
): IText {
 ...
 */
export function replaceWithRepositioning(
    original: IText,
    value: string,
    position: number
): IText {
 ...
 */
export function removeWithRepositioning(
    original: IText,
    start: number,
    end: number
): IText {
    ...
    if (original.carriagePosition < start) {
        ...
        start <= original.carriagePosition &&
        original.carriagePosition <= end
    ) {
