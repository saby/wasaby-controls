    ...
    if (groups) {
        groups.forEach((groupValue: string, groupIndex: number): void => {
            ...
            if (groupIndex in format.delimiterGroups) {
                const delimiterLength: number =
                    format.delimiterGroups[groupIndex].value.length;
...
export function formatData(format: IFormat, cleanText: IText): IText {
    ...
    if (groups) {
        const lastGroupOfKeys: number = indexLastGroupOfKeys(
            groups,
            format.delimiterGroups
        );
        ...
        groups.forEach((groupValue: string, groupIndex: number) => {
            ...
            if (groupIndex in format.delimiterGroups) {
                ...
                const delimiterType: string =
                    format.delimiterGroups[groupIndex].type;
...
export function abbreviateNumber(
    ...
    if (
        ...
        return (
            intlFormat((value as number) / BASE_NUMBERS.thsn, options) +
...
function intlFormat(num: number, options?: INumberFormat): string {
    return numberFormatter(
        Math.round(num * BASE_NUMBERS.ten) / BASE_NUMBERS.ten,
        options
    );
}

const valueWithoutTrailingZerosRegExp: RegExp =
    /-?[0-9 ,]*(([1-9]|([0.])(?!0*$))*)?/;
const valueWithOneTrailingZerosRegExp: RegExp =
    /-?[0-9 ,]*(\.[0-9]([1-9]|0(?!0*$))*)?/;

export function trimTrailingZeros(
    str: string,
    leaveOneZero: boolean = false
): string {
    const regExp = leaveOneZero
        ? valueWithOneTrailingZerosRegExp
        : valueWithoutTrailingZerosRegExp;
...
export function fillAdditionalZeros(str: string, precision: number): string {
    ...
    const additionalZeros = Math.max(
        precision - parsedString.fractional.length,
        0
    );
...
export function correctNumberValue(
    value: string,
