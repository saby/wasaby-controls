...
export function prepareParsedText(data: IHighlightOptions): Element[] {
    ...
    if (highlightedValue.length && highlightedValue.filter(isNotEmpty).length !== 0) {
        return parseText(newValue, highlightedValue, highlightMode || 'substring');
    } else {
        return [
            {
                type: 'plain',
...
function parseText(value: string, highlights: string[], highlightMode: HighlightMode): Element[] {
    /**
    ...
    const searchResultByAnd: Element[] = searchBy(value, escapedHighlights, highlightMode, 'and');
