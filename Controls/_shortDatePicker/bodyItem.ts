...
const BORDER_RADIUS_CLASSES = {
    bottomRight: ' controls-PeriodLiteDialog_item_border-bottom-right-radius',
    topRight: ' controls-PeriodLiteDialog_item_border-top-right-radius',
    bottomLeft: ' controls-PeriodLiteDialog_item_border-bottom-left-radius',
    topLeft: ' controls-PeriodLiteDialog_item_border-top-left-radius',
};

const BORDER_CLASSES = {
    top: ' controls-PeriodLiteDialog_item_border-top',
    bottom: ' controls-PeriodLiteDialog_item_border-bottom',
    left: ' controls-PeriodLiteDialog_item_border-left',
    right: ' controls-PeriodLiteDialog_item_border-right',
};
...
class BodyItem extends Control<IShortDatePickerOptions> implements IDateConstructor {
    ...
    protected _startCornerVisible(value: Date, periodType: string): boolean {
        ...
        if (dateUtils.isMonthsEqual(this._options.startValue, this._options.endValue) &&
    ...
    protected _isSelectedStart(value: Date, periodType: string): boolean {
        ...
        const startValue = new Date(this._options?.startValue?.getFullYear(),
            this._options?.startValue?.getMonth() , this._options?.startValue?.getDate());
        return (
            dateUtils.isDatesEqual(startValue, value) && mainPeriodType === periodType
        );
    }
    ...
    protected _isSelectedEnd(value: Date, periodType: string): boolean {
        ...
        const endValue = new Date(this._options?.endValue?.getFullYear(),
            this._options?.endValue?.getMonth() , this._options?.endValue?.getDate());
        return (
            dateUtils.isDatesEqual(endValue, endOfPeriod) &&
            mainPeriodType === periodType
        );
    }
    ...
    protected _getSelectionClassName(value: Date, periodType: string): string {
        ...
        let className = '';
        const startValue = this._options?.startValue ? new Date(this._options?.startValue?.getFullYear(),
            this._options?.startValue?.getMonth() , this._options?.startValue?.getDate()) : this._options?.startValue;
        const endValue = this._options?.endValue ? new Date(this._options?.endValue?.getFullYear(),
            this._options?.endValue?.getMonth() , this._options?.endValue?.getDate()) : this._options?.endValue;
        const periodStartValue = value;
        const periodEndValue = this._getEndValue(value, periodType);
        const isSelected = () => {
