...
const MAX_VISIBLE_YEARS_ALL_YEARS = 9;
 ...
 * @remark
 * Полезные ссылки:
 * * {@link https://git.sbis.ru/saby/wasaby-controls/-/blob/rc-24.6100/Controls-default-theme/variables/_shortDatePicker.less переменные тем оформления}
 *
...
class View extends Control<IDateLitePopupOptions> {
    ...
    private _popupExpanded: boolean = false;
    ...
    protected _beforeMount(options: IDateLitePopupOptions): void {
        ...
        this._startValue = options.startValue
            ? new options.dateConstructor(options.startValue.getTime())
            : options.startValue;
        this._endValue = options.endValue
            ? new options.dateConstructor(options.endValue.getTime())
            : options.endValue;
        ...
        this._getYearItemCssClasses = this._getYearItemCssClasses.bind(this);
        this._isCurrentYear = this._isCurrentYear.bind(this);
        this._getTabindex = this._getTabindex.bind(this);
        this._getYearListClasses = this._getYearListClasses.bind(this);
    ...
    protected _getYearListClasses(): string {
        ...
        if (this._options.stickyPosition) {
            let yearListClass = 'controls-PeriodLiteDialog__year-list';
            if (this._options.chooseAllYears) {
                yearListClass += '_chooseAllYears';
            }
            classes.push(yearListClass);
        } else if (this._options.isAdaptive && !!this._options.slidingPanelOptions) {
    ...
    protected _hitsDisplayedRange(year: number, index: number = 0): boolean {
        ...
        return (
            dateUtils.normalizeDate(this._displayedRanges[index][0]) <= date &&
            (this._displayedRanges[index][1] === null ||
                dateUtils.normalizeDate(this._displayedRanges[index][1]) >= date)
        );
    }
    ...
    private _getAmountOfFollowingItems(delta: number): number {
        ...
        if (delta === 1) {
            ...
            if (
                ...
                if (this._options.chooseAllYears) {
                    // Помимо текущего года, в режиме 'Только года + Выбрать все года' отображаются еще 9 лет снизу.
                    amountOfFollowingItems = MAX_VISIBLE_YEARS_ALL_YEARS;
                } else {
    ...
    protected _yearsRightPanelClick(): void {
        this._notify('sendResult', [null, null], { bubbling: true });
    }

    protected _yearsRightPanelMouseEnter(): void {
        this._hovered = true;
    }

    protected _yearsRightPanelMouseLeave(): void {
        this._hovered = false;
    }
    ...
    protected _rangeChangedHandler(event: Event, startValue: Date, endValue: Date): void {
        ...
        if (this._hoveredItem === 'year') {
            this._startValue = dateUtils.getStartOfYear(startValue);
            this._endValue = dateUtils.getEndOfYear(endValue);
            return;
        }
        this._endValue = dateUtils.getEndOfMonth(endValue);
    }
    ...
    private _getPosition(options: IDateLitePopupOptions): Date {
        let position = options.year || options.startValue || options.endValue || options._date;
        if (
            options.shouldPositionBelow &&
            position &&
            position.getFullYear() >= new Date().getFullYear()
        ) {
            position = options.endValue || options.startValue;
        }
        if (!dateUtils.isValidDate(position)) {
            position = new options.dateConstructor();
        }
        if (!this._displayedRanges) {
            return position;
        }
    ...
    protected _getPositionCaption(): number {
        if (
            this._options.shouldPositionBelow &&
            this._position.getFullYear() >= new Date().getFullYear() &&
            !this._positionChanged
        ) {
            return new Date(this._position.getFullYear() - 1, 0).getFullYear();
        }
        return this._position.getFullYear();
    }

    private _getYearsListPosition(options: IDateLitePopupOptions): Date {
        const { startValue: start, endValue: end, _date, dateConstructor } = options;
        const currentDate = _date || new dateConstructor();
        const startValueYear = dateUtils.isValidDate(start) ? start.getFullYear() : null;
        const endValueYear = dateUtils.isValidDate(end) ? end.getFullYear() : null;

        if (startValueYear >= currentDate.getFullYear()) {
            return new dateConstructor(startValueYear, 0);
        }

        if (startValueYear && endValueYear) {
            const maxYear = startValueYear > endValueYear ? startValueYear : endValueYear;
            return new dateConstructor(maxYear, 0);
        }
