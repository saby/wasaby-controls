...
export default class YearsList extends Control<IYearItemOptions> {
    ...
    protected _endCornerVisible(value: Date): boolean {
        const isSameYear =
            ...
            this._options.startValue.getFullYear() === this._options.endValue.getFullYear();

        if (
            (this._options.selectionProcessing &&
                (this._options.startValue < this._options.selectionBaseValue || isSameYear)) ||
            !this._isSelectedPeriod(value)
        ) {
            return false;
        }
    ...
    protected _getHeightClassName(date: Date): string {
        let className = 'controls-PeriodLiteDialog-item-yearList_';
        if (
            !this._options.displayedRanges ||
            this._options.displayedRanges[0][0]?.getFullYear() !== date.getFullYear()
        ) {
            if (this._options.isAdaptive && !!this._options.slidingPanelOptions) {
                className += 'adaptiveHeight';
            } else {
                className += 'height';
            }
        } else {
