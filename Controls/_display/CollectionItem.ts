...
} from 'Controls/_display/Collection';
...
export interface ISerializableState<T extends Model = Model>
        ...
        IVirtualCollectionItem
{
    ...
    constructor(options?: IOptions<T>) {
        ...
        this._counters = {};
        const superGetInstanceId = this.getInstanceId.bind(this);

        this.getInstanceId = (): string => {
            const result = superGetInstanceId();
            return result.replace('server-', '').replace('client-', '');
        };
    ...
    getOriginalContents(): T {
        if (this._contentsIndex !== undefined) {
            ...
            if (collection['[Types/_collection/IList]']) {
                this._$contents = (collection as any as IList<T>).at(
                    this._contentsIndex
                );
    ...
    getDisplayValue(displayPropertyTpl?: string): string {
        ...
        return displayProperty && contentsIsRecord
            ? this.getContents().get(displayProperty)
            : null;
    }
    ...
    shouldDisplayMarker(templateMarker: boolean = true): boolean {
        return (
            templateMarker && this.isMarked() && !this.getOwner().isEditing()
        );
    }
    ...
    getMarkerClasses(
        ...
        const hasRoundBorder =
            ...
            this.getTopLeftRoundBorder() !== 'default';
        const hasTopSpacing = this.getTopPadding().toLowerCase() !== 'null';
        if (
            (addVerticalPaddings && hasTopSpacing) ||
            (hasRoundBorder && !hasTopSpacing)
        ) {
        ...
        if (this.getEditingConfig()) {
            classes +=
                ' controls-ListView__itemV_marker-baselineWithEditingTemplate';
        }
    ...
    getMultiSelectPositionClasses(
        ...
        if (position === 'default') {
            ...
            if (baseline === 'none') {
                checkboxMargin =
                    topPadding === 's' || topPadding === 'null' ? 'null' : 's';
            } else {
    ...
    protected _getMultiSelectBaseClasses(): string {
        ...
        classes +=
            'js-controls-ListView__checkbox js-controls-DragScroll__notDraggable ';
        classes +=
            'controls-CheckboxMarker_inList controls-ListView__checkbox ';
        if (
    ...
    setEditing(
         ...
         * */
        if (!editing) {
            this._version += this._getVersionableVersion(
                this._$editingContents
            );
        }
        this._$editing = editing;
        if (
    ...
    acceptChanges(): void {
        ...
        (this._$contents as unknown as Model).merge(
            this._$editingContents as unknown as Model
        );
    ...
    hasVisibleActions(): boolean {
        return (
            ...
            this._$actions.showed.length > 0
        );
    }
    ...
    shouldDisplayItemActions(): boolean {
        ...
        return (
            ...
            (this.isEditing() &&
                (!editingConfig || editingConfig.toolbarVisibility === true))
        );
    }
    ...
    getFadedClass(): string {
        return this.isFaded() || this.isDragged()
    ...
    getStickyHeaderPosition(stickyCallback?: Function): string {
        return this.getVerticalStickyHeaderPosition(
            stickyCallback || this._$stickyCallback
        );
    }

    protected getVerticalStickyHeaderPosition(
        stickyCallback?: Function
    ): string {
        if (stickyCallback) {
    ...
    protected _isSupportSticky(): boolean {
        return (
            this.getOwner().isStickyMarkedItem() !== false &&
    ...
    getWrapperClasses(
        ...
        const hoverBackgroundStyle =
            ...
            this.getStyle();
        const editingBackgroundStyle =
            this.getOwner().getEditingBackgroundStyle();
        ...
        wrapperClasses += `controls-ListView__itemV js-controls-ListView__editingTarget ${this._getCursorClasses(
            cursor
        )}`;
    ...
    getItemActionClasses(itemActionsPosition: string): string {
        ...
        const rowSeparatorSize =
            this.isBottomSeparatorEnabled() && this.getRowSeparatorSize();
    ...
    getBottomRightRoundBorder(): string {
        if (
            this.getItemActionsPosition() === 'outside' &&
            this.shouldDisplayItemActions()
        ) {
            return 'default';
        }
    ...
    getContentClasses(
        ...
        if (this.getStyle() === 'master') {
            contentClasses +=
                ' controls-ListView__baseline controls-ListView__baseline_font-size';
        }
        ...
        contentClasses += getBorderClassName(
            borderVisibility,
            borderStyle,
            true,
            true
    ...
    getItemActionsBackgroundClass(
        ...
        if (this.isEditing()) {
            const editingBackground =
                itemEditBackgroundStyle !== 'default'
                    ? itemEditBackgroundStyle
                    : null;
            return (
        ...
        let classes = '';
        const hoverBackgroundStyle =
            itemHoverBackgroundStyle || this.getHoverBackgroundStyle();
    ...
    getItemActionPositionClasses(
        ...
        const classes =
            itemActionsClass || ITEMACTIONS_POSITION_CLASSES.bottomRight;
        const result: string[] = [];
        const itemPadding = {
        ...
        if (itemPadding.top !== 'null' || itemPadding.bottom !== 'null') {
            const themedPositionClassCompile = (position) => {
                return `controls-itemActionsV_padding-${position}_${
                    itemPadding && itemPadding[position] === 'null'
                        ? 'null'
                        : 'default'
                }`;
            };
            if (classes.indexOf(ITEMACTIONS_POSITION_CLASSES.topRight) !== -1) {
                ...
                classes.indexOf(ITEMACTIONS_POSITION_CLASSES.bottomRight) !== -1
            ) {
    ...
    getMultiSelectTemplate(): TemplateFunction | string | React.Component {
        return (
            this._$owner.getMultiSelectTemplate() ||
            DEFAULT_MULTI_SELECT_TEMPLATE
        );
    }
    ...
    protected _setEditingContents(editingContents: T): void {
        ...
        if (
            this._$editingContents &&
            this._$editingContents['[Types/_entity/ObservableMixin]']
        ) {
        ...
        if (
            editingContents &&
            editingContents['[Types/_entity/ObservableMixin]']
        ) {
    ...
    private _initializeContentsHandlers(): void {
        if (
            this.getContents() &&
            this.getContents()['[Types/_entity/ObservableMixin]']
        ) {
    ...
    _getSerializableState(
        ...
        if (resultState.$options.owner) {
            ...
            const index = collection['[Types/_collection/IList]']
                ? (collection as any as IList<T>).getIndex(
                      resultState.$options.contents
                  )
                : -1;
            if (index > -1) {
