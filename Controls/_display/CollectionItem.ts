...
import type { ICollection, IItemPadding, TAnimationState } from './interface/ICollection';
import {
    ICollectionItem,
    TBorderStyle,
    TBorderVisibility,
...
import type { IEditingConfig, TItemActionsPosition } from './Collection';
...
import type { THorizontalItemPadding, TVerticalItemPadding } from './interface/ICollection';
import InitStateByOptionsMixin from './InitStateByOptionsMixin';
import { getBorderClassName } from './utils/ClassNameUtils';

export interface IOptions<T extends Model = Model> {
    itemModule?: string;
    contents?: T;
    selected?: TSelectionModelStatus;
    marked?: boolean;
    editing?: boolean;
    actions?: any;
    swiped?: boolean;
        ...
        IVirtualCollectionItem
{
     ...
     * Элемент выбран
     */
    protected _$selected: TSelectionModelStatus;

    protected _$highlightedValues: THighlightedValues;
    ...
    protected _$className: string;

    constructor(options?: IOptions<T>) {
        super();
        InitStateByOptionsMixin.initMixin(this, options);
        ObservableMixin.initMixin(this, options);
        if (this.Markable && options?.isMarkedCallback) {
            this._$marked = options.isMarkedCallback(this.key);
        } else {
            this._$marked = false;
        }
        this._counters = {};
     ...
     */
    isSelected(): TSelectionModelStatus {
     ...
     */
    setSelected(selected: TSelectionModelStatus, silent?: boolean): void {
    ...
    setHighlightedValues(values: THighlightedValues): void {
        if (this._$highlightedValues === values) {
            return;
        }
        this._$highlightedValues = values;
        this._nextVersion();
    }

    getHighlightedValues(): THighlightedValues {
        return this._$highlightedValues;
    }
    ...
    getMultiSelectPositionClasses(
        ...
        templatePosition?: 'default' | 'custom'
    ): string {
        const topPadding = (itemPadding.top || this.getTopPadding() || 'l').toLowerCase();
        const position = templatePosition || this.getOwner().getMultiSelectPosition();
    ...
    getMultiSelectClasses(
        ...
        classes += this.getMultiSelectPositionClasses(itemPadding, baseline, position);
        return classes;
    }
    ...
    setEditing(
        ...
        if (typeof columnIndex === 'number' && this._$editingColumnIndex !== columnIndex) {
            ...
            this._notify('editColumnIndexChanged');
        }
    ...
    // Если мы переносим запись при помощи DnD,
    // она не должна быть стики-блоком, т.к. это ломает вёрстку.
    isSticked(): boolean {
        return this._$stickyCallback
            ? !!this._$stickyCallback(this.getContents())
            : !this.isDragged() && this.isMarked() && this._isSupportSticky();
    }
    ...
    getWrapperClasses(
        ...
        let wrapperClasses = this._getBaseWrapperClasses(isAdaptive);
        ...
        wrapperClasses += ` js-controls-ListView__editingTarget ${this.getCursorClasses(cursor)}`;
        ...
        const isActive =
            (this.isSwiped() || templateHighlightOnHover) && this.isActive() && !this.isEditing();

        if (isActive && hoverBackgroundStyle !== 'transparent') {
            wrapperClasses += ` controls-background-${hoverBackgroundStyle}_active controls-ListView__item_active`;
        ...
        if (this._$className) {
            wrapperClasses += ' ' + this._$className;
        }

        return wrapperClasses;
    }

    // Добавляет CSS класс на верхний DIV записи.
    setClassName(className: string) {
        this._$className = className;
        this._nextVersion();
    }

    // Возвращает CSS класс записи, есл он был установлен при помощи стратегии.
    getClassName() {
        return this._$className;
    }

    /**
     * Только базовые классы, необходимые для отображения записи в списке
     * @param isAdaptive
     * @protected
     */
    protected _getBaseWrapperClasses(isAdaptive?: boolean): string {
        let wrapperClasses = '';
        ...
        wrapperClasses += ' controls-ListView__item_contentWrapper';
        if (isAdaptive) {
    ...
    getItemActionPositionClasses(itemActionsPosition?: string, itemActionsClass?: string): string {
        let classes = itemActionsClass || ITEMACTIONS_POSITION_CLASSES.bottomRight;
        const result: string[] = [];
        const itemPadding = {
            top: this.getOwner().getTopPadding().toLowerCase(),
            bottom: this.getOwner().getBottomPadding().toLowerCase(),
        };
        // Убираем классы выравнивания для outside, но оставляем классы для fixed.
        if (
            itemActionsPosition === 'outside' &&
            (classes.indexOf(ITEMACTIONS_POSITION_CLASSES.bottomRight) ||
                classes.indexOf(ITEMACTIONS_POSITION_CLASSES.topRight))
        ) {
            classes = classes
                .replace(ITEMACTIONS_POSITION_CLASSES.bottomRight, '')
                .replace(ITEMACTIONS_POSITION_CLASSES.topRight, '');
        }
        result.push(classes);
        if (itemPadding.top !== 'null' || itemPadding.bottom !== 'null') {
            ...
            if (classes.indexOf(ITEMACTIONS_POSITION_CLASSES.topRight) !== -1) {
                ...
                // У последней записи в спике может быть добавлен разделитель снизу.
                // Его же добавляем к ItemActions, которые прибиты к нижней границе записи.
                if (this.getRowSeparatorSize() && this._$isBottomSeparatorEnabled) {
                    result.push(
                        ` controls-ListView__rowSeparator_bottom_size-${this.getRowSeparatorSize()}`
                    );
                }
            }
        }
    ...
    protected _onPropertyChange(event: SyntheticEvent, properties: Record<string, unknown>): void {
        ...
        this._nextVersion();
        this.getOwner().handlePropertyChange(event, this, properties);
    }
