    ...
    implements ICollection<S, T>, IEnumerable<T>, IList<T>
{
    ...
    private _initializeSpacingStrategy() {
        if (!this._$itemsSpacing && this.getStyle() === 'master') {
            this._$itemsSpacing = '3xs';
        }
        ...
        if (
            ...
            if (this.interactorStateProps.itemActionsMap) {
                this._getItems().forEach((item: CollectionItem<S>) => {
                    const itemKey = item.SupportItemActions && item.contents.getKey();
                    item.setActions(
                        {
                            all: this.interactorStateProps?.itemActionsMap?.get(itemKey) || [],
                            showed: [],
                        },
                        true
                    );
    ...
    setItemsSpacing(itemsSpacing: TOffsetSize): void {
        const newItemsSpacing =
            !itemsSpacing && this.getStyle() === 'master' ? '3xs' : itemsSpacing;
        if (this._$itemsSpacing === newItemsSpacing) {
            return;
        }
        const setSpacing = !this._$itemsSpacing && newItemsSpacing;
        const unsetSpacing = this._$itemsSpacing && !newItemsSpacing;
        const changedSpacing = !setSpacing && !unsetSpacing;

        this._$itemsSpacing = newItemsSpacing;
    ...
    setSelectionModel(selectionModel: TSelectionModel, silent?: boolean): void {
        ...
        updateItemsSelection(new Map([...this._$selectionModel, ...selectionModel]));
