    ...
    implements ICollection<S, T>, IEnumerable<T>, IList<T>
{
    ...
    protected _updateInteractorStateProps(interactorStateProps: IInteractorStateProps): boolean {
        ...
        let shouldIncrementVersion: boolean = !(
            Object.keys(interactorStateProps).length === 0 ||
            (Object.keys(interactorStateProps).length === 1 &&
                'isLatestInteractorVersion' in interactorStateProps)
        );
