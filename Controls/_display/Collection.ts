    ...
    implements ICollection<S, T>, IEnumerable<T>, IList<T>
{
    ...
    setSelectionModel(selectionModel: TSelectionModel, silent?: boolean): void {
        const updateItemsSelection = (_selectionModel: TSelectionModel, reset: boolean = false) => {
            _selectionModel.forEach((selectionStatus, key) => {
                const item = this.getItemBySourceKey(key, false);
                if (item) {
                    item.setSelected(reset ? false : selectionStatus, silent);
                }
            });
        };

        updateItemsSelection(this._$selectionModel, true);
