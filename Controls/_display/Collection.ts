...
import GroupItemsStrategy, {
    IHiddenGroupPosition,
} from './itemsStrategy/Group';
 ...
 * @typedef {String} Controls/_display/Collection/MultiSelectAccessibility
 * @variant enabled Чекбокс виден и с ним можно взаимодействовать.
 * @variant disabled Чекбокс виден, но с ним нельзя взаимодействовать. Режим "только для чтения".
...
function onCollectionItemChange<T extends EntityModel>(
    ...
    if (this._sourceCollectionSynchronized) {
        ...
        this._sourceCollectionDelayedCallbacks =
            this._sourceCollectionDelayedCallbacks || [];
    ...
    implements ICollection<S, T>, IEnumerable<T>, IList<T>
{
    ...
    constructor(options: IOptions<S, T>) {
        ...
        if (options.trackedProperties !== undefined) {
            this._$trackedProperties = this._fixTrackedProperties(
                options.trackedProperties
            );
        }
        ...
        if (options.itemsFilterMethod) {
            this._setItemsFilterMethod(
                this._$filter,
                options.itemsFilterMethod
            );
        }
    ...
    private _initializeFilter(userFilter?: FilterFunction<S, T>[]): void {
    ...
    private _deinitializeCollection(): void {
        if (!(this.getSourceCollection() as DestroyableMixin).destroyed) {
            if (this.getSourceCollection()['[Types/_collection/IObservable]']) {
                ...
                this._actualizeSubscriptionOnMetaResults(
                    this._$metaResults,
                    null
                );
            }
            if (
                this.getSourceCollection()['[Types/_entity/EventRaisingMixin]']
            ) {
                this.getSourceCollection().unsubscribe(
                    'onEventRaisingChange',
                    this._oEventRaisingChange
    ...
    getByInstanceId(instanceId: string): T {
        return this.at(
            this._getUtilityEnumerator().getIndexByValue(
                'instanceId',
                instanceId
            )
        );
    ...
    getStartIndex(): number {
        return VirtualScrollController.getStartIndex(
            this as unknown as IVirtualScrollCollection
        );
    }

    getStopIndex(): number {
        return VirtualScrollController.getStopIndex(
            this as unknown as IVirtualScrollCollection
        );
    }
    ...
    getSourceIndexByIndex(index: number): number {
        let sourceIndex =
            this._getUtilityEnumerator().getSourceByInternal(index);
        sourceIndex =
            sourceIndex === undefined || sourceIndex === null
                ? -1
                : sourceIndex;
    ...
    getIndexBySourceIndex(index: number): number {
        ...
        const itemIndex =
            this._getUtilityEnumerator().getInternalBySource(index);
    ...
    getIndexBySourceItem(item: S): number {
        ...
        if (collection && collection['[Types/_collection/IList]']) {
            sourceIndex =
                this.getSourceDataStrategy().getSourceIndexBySourceItem(item);
            ...
            if (sourceIndex === -1 && this._$isEditing) {
                this.each((el, index: number) => {
                    if (
                        el.isEditing() &&
                        el.isAdd &&
                        el.contents.getKey() === item.getKey()
                    ) {
                        sourceIndex = index;
                    }
                });
        ...
        return sourceIndex === -1
            ? -1
            : this.getIndexBySourceIndex(sourceIndex);
    }
    ...
    getSourceItemByKey(key: string | number): S {
        return this.getSourceDataStrategy().getSourceItemByKey(
            key
        ) as unknown as S;
    }

    getSourceItemBySourceIndex(index: number): S {
        return this.getSourceDataStrategy().getSourceItemBySourceIndex(
            index
        ) as unknown as S;
    }
                ...
                if (group === undefined) {
                    Logger.error(
                        `Значение поля ${this._$groupProperty} не может быть undefined. ` +
                            `Проверьте запись с ключом ${item.get(
                                this.getKeyProperty()
                            )}`,
                        this
                    );
    ...
    setKeyProperty(keyProperty: string): void {
        ...
        if (keyProperty !== this.getKeyProperty()) {
            this._composer.getInstance<DirectItemsStrategy<S, T>>(
                DirectItemsStrategy
    ...
    notifyItemChange(item: T, properties: string): void {
        ...
        const shouldRebuild =
            REBUILD_ITEM_PROPERTIES.indexOf(properties) !== -1;
        ...
        if (
            VERSION_UPDATE_ITEM_PROPERTIES.indexOf(
    ...
    setRowSeparatorSize(rowSeparatorSize: string): void {
        if (
            ...
            this._updateItemsProperty(
                'setRowSeparatorSize',
                this._$rowSeparatorSize
            );
        }
    }

    setRowSeparatorVisibility(
        rowSeparatorVisibility: TRowSeparatorVisibility
    ): void {
        if (
            ...
            this._getItems().forEach((item: CollectionItem<S>) => {
                ...
                if (item === firstItem && item !== lastItem) {
                    ...
                    item.setTopSeparatorEnabledReact?.(
                        this._shouldAddListTopSeparator()
                    );
                    item.setBottomSeparatorEnabledReact?.(
                        this._shouldAddListBottomSeparator()
    ...
    setTrackedProperties(
        ...
        this._$trackedProperties =
            this._fixTrackedProperties(trackedProperties);
        this._nextVersion();
    }
        ...
        if (item && this._$trackedProperties) {
            ...
            if (dataItem) {
                ...
                this._$trackedProperties.forEach((property) => {
                    trackedValues[property.propertyName] = record.get(
    ...
    setTrackedValuesVisible(trackedValuesVisible: boolean): void {
    ...
    setItemsSpacing(itemsSpacing: TOffsetSize): void {
        ...
        if (setSpacing) {
            this.appendStrategy(ItemsSpacingStrategy, {
                spaceItemFactory: this._spaceItemFactory.bind(this),
            });
        }
        if (unsetSpacing) {
            this.removeStrategy(ItemsSpacingStrategy);
        }
        if (changedSpacing) {
            this._updateItemsProperty(
                'setItemsSpacing',
    ...
    setMetaResults(metaResults: EntityModel): void {
        if (
            !isEqual(this._$metaResults, metaResults) ||
    ...
    private _actualizeSubscriptionOnMetaResults(
        thisMetaResults: object,
        newMetaResults: object
    ) {
        if (thisMetaResults !== newMetaResults) {
            if (
                thisMetaResults &&
                thisMetaResults['[Types/_entity/IObservableObject]']
            ) {
                thisMetaResults.unsubscribe(
                    'onPropertyChange',
                    this._onMetaResultsChange
                );
            }
            if (
                newMetaResults &&
                newMetaResults['[Types/_entity/IObservableObject]']
            ) {
                newMetaResults.subscribe(
                    'onPropertyChange',
                    this._onMetaResultsChange
                );
            }
    ...
    private _setItemsFilterMethod(
        filter: FilterFunction<S>[],
        method: FilterFunction<S>
    ): void {
        // FIXME: метод для поддержания совместимости с ModerDialog при внедрении. Должен быть удален.
        //  Убрать по задаче https://online.sbis.ru/opendoc.html?guid=7a607ef8-f2bc-461f-9de8-a97e14af88cb
    ...
    protected _updateRenderedItemsOutsideVirtualRange(
        start: number,
        stop: number
    ): void {
        const hasEditing = !!this.getEditingConfig();
        const hasStickyItem =
            (this.isStickyMarkedItem() !== false &&
    ...
    setItemAnimation(index, animation): void {
        if (animation === 'a') {
    ...
    setAddingItem(
        item: T,
        options: { position: 'top' | 'bottom'; index?: number }
    ): void {
    ...
    getStrategyInstance<F extends IItemsStrategy<S, T>>(
        strategy: StrategyConstructor<F>
    ): F {
    ...
    _setSerializableState(state: ISerializableState<S, T>): Function {
        ...
        return function (): void {
            ...
            if (this._composer) {
                ...
                const userStrategy =
                    this._composer.getInstance(UserItemsStrategy);
                if (userStrategy) {
                ...
                const groupStrategy =
                    this._composer.getInstance(GroupItemsStrategy);
                if (groupStrategy) {
    ...
    protected _extractItemId(item: T): string {
        ...
        if (contents['[Types/_entity/Model]']) {
            ...
            throw new Error(
                'Option "keyProperty" must be defined to extract item unique id.'
            );
        }
    ...
    protected _analizeUpdateSession(
    ...
    protected _bindHandlers(): void {
        ...
        this._onCollectionPropertyChange =
            onCollectionPropertyChange.bind(this);
    ...
    protected _getItemsFactory(): ItemsFactory<T> {
        return function CollectionItemsFactory(
            options?: ICollectionItemOptions<S>
        ): T {
            options.owner = this;
    ...
    protected _getCursorEnumerator(): CollectionEnumerator<T> {
        return (
            this._cursorEnumerator ||
            (this._cursorEnumerator = this._getEnumerator())
        );
    }
    ...
    protected _getUtilityEnumerator(): CollectionEnumerator<T> {
        return (
            this._utilityEnumerator ||
            (this._utilityEnumerator = this._getEnumerator())
        );
    }
    ...
    protected _reFilter(start?: number, count?: number): void {
        ...
        const isMatch = (
            ...
            for (
                let filterIndex = 0;
                filterIndex < filtersLength;
                filterIndex++
            ) {
                filter = filters[filterIndex];
                const isAddingItem =
                    this.getStrategyInstance(AddStrategy) &&
                    this.getStrategyInstance(AddStrategy).getAddingItem() ===
                        itemLocal;
                result =
                    isAddingItem ||
                    filter(
    ...
    protected _getItemsDiff(
        before: ISessionItemState<T>[],
        after: ISessionItemState<T>[]
    ): T[] {
        return after
    ...
    protected _checkItemsDiff(
        ...
        const diff = state
            ? this._getItemsDiff(state, this._getItemsState(items))
            : [];
    ...
    protected _handleAfterCollectionChange(
        ...
        this._updateEdgeItems();
        this.getViewIterator().setIndices(
            this.getStartIndex(),
            this.getStopIndex()
        );
    }
