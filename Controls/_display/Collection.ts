...
import GroupItemsStrategy, { groupConstants, IHiddenGroupPosition } from './itemsStrategy/Group';
...
import type { IEditableListOptions } from 'Controls/baseList';
...
interface IUserStrategy<S, T> {
    ...
    options?: object;
    after?: new (options: unknown) => IItemsStrategy<S, T>;
}
...
function onCollectionChange<T>(
    ...
    switch (action) {
        case IObservable.ACTION_RESET:
            ...
            this._reBuild(true);
            const projectionNewItems = toArray(this) as CollectionItem[];

            // Специальный костыль, обеспечивающий перерисовку после reset для wml-рендера.
            // Если мы отрисовали коллекцию, в которой не было изменений, а потом произошел её reset, то пересозданные
            // элементы имеют ту же версию, что и была до пересоздания.
            // Как следствие, react не произведет перерисовки, т.к. memo-callback не увидит изменений.
            // Суть костыля в том, что после пересоздания дополнительно компенсируем версию элемента коллекции.
            // https://online.sbis.ru/opendoc.html?guid=8ed26c76-18f9-4992-a80d-6a59d325930e
            projectionNewItems.forEach((item: CollectionItem) => {
                item.compensateVersionOnReset();
            });
...
function groupingFilter(
    ...
    return (
        collectionItem['[Controls/_display/GroupItem]'] ||
        (collectionItem['[Controls/_display/SpaceCollectionItem]'] &&
            group?.getGroupViewMode() === 'titledBlocks' &&
            collectionItem.key.indexOf('space-collection-item-bottom') !== -1) ||
        !group ||
        group.isExpanded()
    );
}

export interface IInteractorStateProps {
    keyProperty?: string;
    ...
    implements ICollection<S, T>, IEnumerable<T>, IList<T>
{
    ...
    protected _$itemsOrder?: TItemsOrder;
    ...
    protected _$isThinInteractor: boolean;

    protected _$wasMobileExpanderDisplayed: boolean;

    protected _$markedKey?: CrudEntityKey;

    protected _$selectionModel: TSelectionModel;

    /**
     * Модель подсветки полей.
     * Используется, например, при поиске.
     * @protected
     */
    protected _$highlightedFieldsModel: THighlightedFieldsMap;
     ...
     */
    protected _trackedValuesVisible: boolean = false;
    ...
    protected interactorStateProps: IInteractorStateProps = {};
    ...
    constructor(options: IOptions<S, T>) {
        ...
        if (options.isLatestInteractorVersion !== undefined) {
            this.interactorStateProps.isLatestInteractorVersion = options.isLatestInteractorVersion;
        }
        ...
        if (options.itemsOrder !== undefined) {
            this._$itemsOrder = options.itemsOrder;
        }
        ...
        this._updateActionsByInteractorStateProps(options);
    ...
    protected _getInvertItemsOrderSort(): SortFunction<S, T> {
        return (leftItem, rightItem) => {
            if (this._$itemsOrder && this._$itemsOrder !== 'default') {
                return rightItem.index - leftItem.index;
            }
    ...
    setItemsOrder(itemsOrder?: TItemsOrder): void {
        ...
        this._$itemsOrder = itemsOrder;

        this._reBuild(true);
        this.nextVersion();
        this.setEventRaising(true, true);

        this._notify('onSetItemsOrderFinished', itemsOrder);
    }

    getOrder(): TItemsOrder | undefined {
        return this._$itemsOrder;
    }
    ...
    updateInteractorStateProps(interactorStateProps: IInteractorStateProps): void {
        if (this._updateInteractorStateProps(interactorStateProps)) {
            this._nextVersion();
        }
    }

    // TODO
    //  Пока не могу вызывать весь _updateInteractorStateProps на конструкторе коллекции, т.к.
    //  это приводит к смене версии коллекции в неожиданных местах и вызовам ненужных методов, когда коллекция ещё не готова.
    //  Вследствие этого происходит двойная перерисовка списка при построении, валятся ошибки и падают юнит-тесты.
    private _updateActionsByInteractorStateProps(interactorStateProps: IInteractorStateProps) {
        if (
            'itemActionsMap' in interactorStateProps &&
            this.interactorStateProps.itemActionsMap !== interactorStateProps.itemActionsMap
        ) {
            this.interactorStateProps.itemActionsMap = interactorStateProps.itemActionsMap;
            if (this.getCount()) {
                this._getItems().forEach((item: CollectionItem<S>) => {
                    ...
                    const itemKey = item.contents.getKey();
                    item.setActions(
                        {
                            all: interactorStateProps.itemActionsMap?.get(itemKey) || [],
                            showed: [],
                        },
                        true
                    );
    ...
    protected _updateInteractorStateProps(interactorStateProps: IInteractorStateProps): boolean {
        // Если не изменилось ни одного состояния, не нужно вслепую поднимать версию.
        let shouldIncrementVersion: boolean = Object.keys(interactorStateProps).length !== 0;
        this._updateActionsByInteractorStateProps(interactorStateProps);
        if (
            'keyProperty' in interactorStateProps &&
            this.interactorStateProps.keyProperty !== interactorStateProps.keyProperty
        ) {
            shouldIncrementVersion = true;
            this.interactorStateProps.keyProperty = interactorStateProps.keyProperty;
            this.setKeyProperty(interactorStateProps.keyProperty);
        }
        if (
            'markedKey' in interactorStateProps &&
            this.interactorStateProps.markedKey !== interactorStateProps.markedKey
        ) {
            shouldIncrementVersion = true;
            this.interactorStateProps.markedKey = interactorStateProps.markedKey;
            this.setMarkedKey(interactorStateProps.markedKey as CrudEntityKey);
        }
        if (
            'selectionModel' in interactorStateProps &&
            this.interactorStateProps.selectionModel !== interactorStateProps.selectionModel
        ) {
            this.interactorStateProps.selectionModel = interactorStateProps.selectionModel;
            this.setSelectionModel(interactorStateProps.selectionModel || new Map());
        }
        if (
            'highlightedFieldsMap' in interactorStateProps &&
            this.interactorStateProps.highlightedFieldsMap !==
                interactorStateProps.highlightedFieldsMap
        ) {
            this.interactorStateProps.highlightedFieldsMap =
                interactorStateProps.highlightedFieldsMap;
            this.setHighlightedFieldsModel(interactorStateProps.highlightedFieldsMap || new Map());
        }
        if (
            'searchValue' in interactorStateProps &&
            this.interactorStateProps.searchValue !== interactorStateProps.searchValue
        ) {
            this.interactorStateProps.searchValue = interactorStateProps.searchValue;
            this.setSearchValue(interactorStateProps.searchValue || '');
        }
        if (
            'itemPadding' in interactorStateProps &&
            this.interactorStateProps.itemPadding !== interactorStateProps.itemPadding
        ) {
            this.interactorStateProps.itemPadding = interactorStateProps.itemPadding;
            this.setItemPadding(interactorStateProps.itemPadding);
        }
        if (
            'roundBorder' in interactorStateProps &&
            this.interactorStateProps.roundBorder !== interactorStateProps.roundBorder
        ) {
            this.interactorStateProps.roundBorder = interactorStateProps.roundBorder;
            this.setRoundBorder(interactorStateProps.roundBorder);
        }
        if (
            'groupProperty' in interactorStateProps &&
            this.interactorStateProps.groupProperty !== interactorStateProps.groupProperty
        ) {
            this.interactorStateProps.groupProperty = interactorStateProps.groupProperty;
            this.setGroupProperty(interactorStateProps.groupProperty || null);
        }
        if (
            'emptyView' in interactorStateProps &&
            this.interactorStateProps.emptyView !== interactorStateProps.emptyView
        ) {
            this.interactorStateProps.emptyView = interactorStateProps.emptyView;
            this.setEmptyView(interactorStateProps.emptyView);
        }
    ...
    getFirst(conditionProperty?: string): T {
        ...
        if (conditionProperty && !item[conditionProperty]) {
            return this._getFirstNearbyItem(enumerator, item, true, conditionProperty);
        }
     ...
     */
    getItemBySourceKey(
        key: string | number | CrudEntityKey | null,
    ...
    getIndexByKey(key: string | number | null): number {
    ...
    getWasMobileExpanderDisplayed(): boolean {
        return this._$wasMobileExpanderDisplayed;
    }

    setWasMobileExpanderDisplayed(): void {
        if (this._$isThinInteractor) {
            this._$wasMobileExpanderDisplayed = true;
        }
    }

    isThinInteractor(): boolean {
        return this._$isThinInteractor;
    }
    ...
    resetDraggedItems(): void {
        const strategy = this.getStrategyInstance(this._dragStrategy) as unknown as
            | DragStrategy
            | undefined;

        if (strategy) {
            const draggedKey = strategy.avatarItem?.key;
            strategy.destroy();
            this.removeStrategy(this._dragStrategy);
            this._reIndex();
            this._updateEdgeItems();

            // При старте перемещения оригинальная запись в коллекции скрывается фильтром, на ее месте показывается
            // фейковая. Также, по стандарту, мы ставим маркер на перемещаемую заппись. Т.к. ставим маркер мы после
            // фактического начала перемещения, то маркер устанавливается именно на фейковую запись.
            // После завершения перемещения фейковая запись удаляется вместе с маркером, а оригинальная
            // перемещается на ее место.
            // Проблема именно тут и решаться должна тоже в рамках коллекции, т.к. с точки зрения
            // управляющей логики всё верно: один раз сказали отметить запись, маркер должен быть только на ней.
            if (typeof draggedKey !== 'undefined' && this.getMarkedKey() === draggedKey) {
                this.setMarkedKey(draggedKey, true);
            }
        }
    }
    ...
    getRowSeparatorVisibility(): string {
    ...
    setRowSeparatorVisibility(rowSeparatorVisibility: TRowSeparatorVisibility): void {
        if (
            ...
            // Обновление разделитей всех записей
            // Для wasaby-рендера разделители рисуются сверху. Снизу рисуются только при необходимости.
            // Для реакт-рендера логика обратная, разделители рисуются снизу. Сверху рисуются только при необходимости.
            this._getItems().forEach((item: CollectionItem<S>) => {
                ...
                if (item === firstItem && item !== lastItem) {
                    ...
                    // TODO Логика для React ломает базовую линию. Она не используется
                    ...
                    // TODO Логика для React ломает базовую линию. Она не используется
                    item.setTopSeparatorEnabledReact?.(rowSeparatorVisibility !== 'edges');
    ...
    getTrackedValues(): TTrackedValues {
     ...
     * @param options - объект с доп опциями
     */
    protected _spaceItemFactory(
        item: T,
        options?: {
            key: string;
            className: string;
        ...
        const contents = new Model({
            keyProperty,
            rawData: {
                [keyProperty]: options?.key ?? `space-collection-item-${item.key}`,
            },
        });

        return this.createItem({
            itemModule: this._spaceItemModule,
            itemsSpacing: options?.itemsSpacing || this._$itemsSpacing,
            className: options?.className,
            contents,
        }) as unknown as SpaceCollectionItem;
    }

    setMarkedKey(key: CrudEntityKey | undefined | null, force?: boolean, silent?: boolean): void {
        const set = (itemKey: typeof key, status: boolean): void => {
            if (typeof itemKey !== 'undefined') {
                const item = this.getItemBySourceKey(itemKey, false);
                if (item && item.Markable) {
                    item.setMarked(status, silent);
                }
            }
        };
        if (this._$markedKey !== key || force) {
            if (!force) {
                set(this._$markedKey, false);
            }
            set(key, true);
            this._$markedKey = key;
        }
    }

    getMarkedKey(): CrudEntityKey | undefined {
        return this._$markedKey;
    }

    setSelectionModel(selectionModel: TSelectionModel, silent?: boolean): void {
        const updateItemsSelection = (model: TSelectionModel) => {
            const collectionMap = new Map();

            this.each((item) => {
                if (typeof item.key !== 'undefined') {
                    collectionMap.set(item.key, item);
                }
            });

            model.forEach((status, key) => {
                const item =
                    collectionMap.get(key) ||
                    this.getItemBySourceKey(key, false) ||
                    (this.isThinInteractor() && this.getItems().find((i) => i.key === key));
                if (item) {
                    item.setSelected(status, silent);
                }
            });
        };

        updateItemsSelection(selectionModel);
        this._$selectionModel = selectionModel;
    }

    setHighlightedFieldsModel(model: THighlightedFieldsMap): void {
        const updateItemsHighlightedFields = (
            map: THighlightedFieldsMap,
            reset: boolean = false
        ) => {
            map.forEach((value, key) => {
                const item = this.getItemBySourceKey(key, false);
                if (item) {
                    item.setHighlightedValues(reset ? [] : value);
                }
            });
        };

        updateItemsHighlightedFields(this._$highlightedFieldsModel, true);
        this._$highlightedFieldsModel = model;
        updateItemsHighlightedFields(this._$highlightedFieldsModel);
    }
    ...
    setEmptyView(emptyView: IEmptyViewConfig[] | TemplateFunction): boolean {
        if (typeof emptyView === 'function') {
            this.setEmptyTemplate(emptyView);
            return true;
        }

        if (typeof emptyView === 'object') {
            this.setEmptyTemplateOptions(emptyView);
            return true;
        }
    ...
    setEmptyTemplateOptions(options: object): boolean {
        if (!isEqual(this._$emptyTemplateOptions, options)) {
            ...
            if (this._emptyTemplateItem) {
                this._emptyTemplateItem.setItemTemplateOptions(this._$emptyTemplateOptions);
            }
            this._nextVersion();
            return true;
        }
    ...
    setMetaResults(metaResults: EntityModel): void {
        if (this._$metaResults !== metaResults && !isEqual(this._$metaResults, metaResults)) {
    ...
    getActionsVisibility(): TItemActionsVisibility {
        // visible в приоритете, т.к. иногда хотят показывать visible+custom
        if (this._$itemActionsVisibility === 'visible') {
            return 'visible';
        }
        if (this._$itemActionsPosition === 'custom' || this._$itemActionsVisibility === 'hidden') {
            return 'hidden';
        }
        return 'onhover';
    }
    ...
    setStickyHeader(stickyHeader: boolean | undefined): void {
        if (this._$stickyHeader !== stickyHeader && stickyHeader !== undefined) {
            this._$stickyHeader = Boolean(stickyHeader);
            this._nextVersion();
        }
    }

    setStickyFooter(stickyFooter: boolean | undefined): void {
        if (this._$stickyFooter !== stickyFooter && stickyFooter !== undefined) {
            this._$stickyFooter = Boolean(stickyFooter);
            this._nextVersion();
        }
    }

    setStickyResults(stickyResults: boolean | undefined): void {
        if (this._$stickyResults !== stickyResults && stickyResults !== undefined) {
            this._$stickyResults = Boolean(stickyResults);
            this._nextVersion();
        }
    }
    ...
    appendStrategy(
        ...
        if (this._composer) {
            this._composer.append(strategy, strategyOptions, after);
            this._reBuild();
        }
        this._finishUpdateSession(session);
    ...
    handlePropertyChange(
        ...
        if (!this.interactorStateProps?.isLatestInteractorVersion) {
            return;
        }
        this._onCollectionItemChange(
            event,
            item.contents,
            this.getSourceIndexByItem(item),
            properties
        );
    }
    ...
    protected _getItemsFactory(): ItemsFactory<T> {
        return function CollectionItemsFactory(options?: ICollectionItemOptions<S>): T {
            ...
            options.isMarkedCallback = (itemKey) => {
                // Именно itemKey, т.к. мы не всегда можем получить ключ перед созданием.
                // Например - при построении хебкной крошки. Только она может сказать после построения какой у неё ключ.
                // Не потому что это невозможно, а потому что так написано.
                return this._isItemMarked(itemKey);
            };
            options.selected = this._$selectionModel.has(key)
                ? this._$selectionModel.get(key)
                : false;
            options.highlightedValues = this._$highlightedFieldsModel.has(key)
                ? this._$highlightedFieldsModel.get(key)
                : [];
            options.tagStyleProperty = this._$tagStyleProperty;
            if (this.interactorStateProps?.itemActionsMap) {
                options.actions = {
                    all: this.interactorStateProps.itemActionsMap.get(key) || [],
                    showed: [],
                };
            }
    ...
    // Ключи могут быть не равны в случае индексного типа ключа.
    // Запись с ключом ['ZXC'] будет найдена кодом this._$collection.getRecordById('ZXC'),
    // т.к. RecordSet сам умеет клеить ключи.
    // Списочная же коллекция не умеет этого делать, являясь прослойкой и хранилищем стейта.
    // Для нее ['ZXC'] действительно !== 'ZXC'.
    // Маркер при создании записи не проставляется.
    // FIXME: Этим вопросом нужно заняться при упрощении коллекции.
    private _isItemMarked(key): boolean {
        // Нет ключа
        if (typeof this._$markedKey === 'undefined') {
            return false;
        }

        // Эта запись отмечена.
        if (this._$markedKey === key) {
            return true;
        }

        // Ключи неравны по значению(выше), но равны по типу,
        // значит запись не отмечена.
        if (typeof this._$markedKey === typeof key) {
            return false;
        }

        // Самый энергозатратный кейс, когда записи неравны по значению и типу.
        // Получаем ключи с RecordSet'а.
        const sourceCollection = this.getSourceCollection();
        if (!sourceCollection['[Types/_collection/RecordSet]']) {
            Logger.warn(
                'Тип ключа маркерованной записи отличается от типа ключа записей в исходной коллекции.'
            );
            return false;
        }

        return (
            sourceCollection.getRecordById(this._$markedKey) === sourceCollection.getRecordById(key)
        );
    }
    ...
    protected _createComposer(): ItemsStrategyComposer<S, T> {
        ...
        composer
            ...
            .append(UserItemsStrategy, {
                ...
                invertSort: this._getInvertItemsOrderSort(),
            })
            .append(GroupItemsStrategy, {
                ...
                blocksGapCallback: (
                    ...
                    return position === 'top'
                        ? group_id === groupConstants.hiddenGroup
                            ? 'm'
                            : 'xs'
                        : 's';
                },
        ...
        this._getUserStrategies().forEach((us) => {
            return composer.append(us.strategy, us.options, us.after);
        });
    ...
    /**
     * Возвращает ближайший элемент проекции
     ...
     * @protected
     */
    protected _getFirstNearbyItem(
        enumerator: CollectionEnumerator<T>,
        item: T,
        isNext: boolean,
        conditionProperty?: string
    ): T {
        return this._getNearbyItem(enumerator, item, isNext, conditionProperty);
    }
    ...
    protected _getItemsState(items: T[]): ISessionItemState<T>[] {
        return items.map((item) => this._getItemState(item));
    }
...
Object.assign(Collection.prototype, {
    ...
    '[Controls/display:Collection]': true,
    ...
    _$markedKey: undefined,
    _$selectionModel: new Map(),
    _$highlightedFieldsModel: new Map(),
    ...
    _$roundBorder: undefined,
    _$task1184913014: false,
    _$directionality: null,
    _$adaptiveMode: null,
    _$isThinInteractor: false,
    _$wasMobileExpanderDisplayed: false,
