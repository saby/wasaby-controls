...
export type TIndicatorState = 'portioned-search' | 'continue-search' | 'loading' | 'hidden-loading';
export type TIndicatorSelector = 'js-controls-BaseControl__loadingIndicator';
export const IndicatorSelector: TIndicatorSelector = 'js-controls-BaseControl__loadingIndicator';
...
export default class Indicator extends CollectionItem<null> {
    ...
    updateLoadingIndicator(indicatorTemplate: TemplateFunction | string) {
        if (this._$state === 'hidden-loading' || this._$state === 'loading') {
            this._$loadingIndicatorTemplate = indicatorTemplate;
            this._nextVersion();
        }
    }

    updatePortionedSearch(indicatorTemplate: TemplateFunction | string) {
        if (this._$state === 'portioned-search') {
            this._$portionedSearchTemplate = indicatorTemplate;
            this._nextVersion();
        }
    }

    updateContinueSearch(indicatorTemplate: TemplateFunction | string) {
        if (this._$state === 'continue-search') {
            this._$continueSearchTemplate = indicatorTemplate;
            this._nextVersion();
        }
    }
    ...
    getClasses(): string {
        ...
        switch (this._$state) {
            ...
            case 'loading':
                ...
                break;
            case 'portioned-search':
                classes += this._getLoaingInStartClass();
    ...
    setMetaData(metaData: { results?: Model }): void {
        if (
            this.isDisplayed() &&
            (this._$state === 'portioned-search' || this._$state === 'continue-search')
        ) {
    ...
    protected _getLoaingInStartClass(): string {
        return this.getOwner().hasColumnScroll?.()
            ? ''
            : ' controls-BaseControl__loadingIndicator__start';
    }
}
