    ...
    implements IItemsStrategy<S, T>
{
    ...
    private _getAddingItemIndex(): number {
        if (this._addingItemIndex === undefined) {
            this._addingItemIndex = Add._calculateIndex<S, T>(
                this._options,
                this.source
            );
        }
    ...
    private static _calculateIndex<
        S extends Model,
        T extends CollectionItem<S>
    >(options: IOptions<S, T>, source: IItemsStrategy<S, T>): number {
        ...
        if (options.item['[Controls/_display/TreeItem]']) {
            ...
            return Add._calculateGroupedItemIndex(
                options.item,
                options,
                source
            );
    ...
    private static _calculateGroupedItemIndex<
        S extends Model,
        T extends CollectionItem<S>
    >(item: T, options: IOptions<S, T>, source: IItemsStrategy<S, T>): number {
        ...
        if (options.addPosition === 'top') {
            ...
            source.items.forEach((sourceItem, sourceIndex) => {
                if (
                    index === -1 &&
                    options.groupMethod(sourceItem.contents) === groupId
                ) {
                    index = sourceIndex;
                }
            });
