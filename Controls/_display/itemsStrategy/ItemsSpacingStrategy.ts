...
> extends AbstractStrategy<S, T> {
    ...
    private _createSpaceItems(): T[] {
        ...
        for (let i = startIndex; i < sourceItemsCount; i++) {
            const sourceItemIndex = this._getSourceItemIndexBySpaceItemIndex(i, hiddenGroupIndex);
    ...
    protected _createItemsOrder(): number[] {
        ...
        const newSpaceItemsSet = new Set(newSpaceItems.map((item) => item.key));
        // Удаляем элементы, которых больше не существует
        let i = 0;
        while (i < spaceItems.length) {
            const spaceItem = spaceItems[i];
            if (newSpaceItemsSet.has(spaceItem.key)) {
                i++;
            } else {
                spaceItems.splice(i, 1);
            }
        }
        const spaceItemsSet = new Set(spaceItems.map((item) => item.key));
        // Добавляем новые элементы
        newSpaceItems.forEach((newSpaceItem, index) => {
            if (!spaceItemsSet.has(newSpaceItem.key)) {
    ...
    private _getSourceItemIndexBySpaceItemIndex(
        spaceItemIndex: number,
