 ...
 * @private
 */
export interface IItemsSpacingStrategyOptions<S, T>
    extends AbstractStrategyOptions<S, T> {
    /**
...
> extends AbstractStrategy<S, T> {
    ...
    protected _createItemsOrder(): number[] {
        ...
        for (
            ...
            sourceItemIndex < sourceItemsCount;
            sourceItemIndex++
        ) {
    ...
    private _getHiddenGroupIndex(): number {
        return this.source.items.findIndex((item) => {
            return (
                (item as unknown as GroupItem<unknown>).contents ===
                groupConstants.hiddenGroup
            );
        });
    }
