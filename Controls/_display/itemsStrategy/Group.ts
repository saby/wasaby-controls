...
interface IOptions<S, T extends CollectionItem<S>> {
    ...
    /**
     * Фабрика конструктора SpaceItem
     */
    spaceItemFactory: TSpaceItemFactory<T>;
    /**
     * Расстояние между блоками
     */
    blocksGapCallback: (position: 'top' | 'bottom', group_id: string | number) => string;
}

interface ISpaceItems {
    top?: SpaceCollectionItem;
    bottom?: SpaceCollectionItem;
}
...
interface ISerializableState extends IDefaultSerializableState {
    ...
    _itemsOrder: number[];
    _spaceItems: ISpaceItems[];
    _spaceItemsCount: number;
}
    ...
    implements IItemsStrategy<S, T>
{
     ...
     */
    protected _spaceItems: ISpaceItems[] = [];
    ...
    getDisplayIndex(index: number): number {
        ...
        const overallIndex = sourceIndex + this._groups.length + this._spaceItemsCount;
    ...
    getCollectionIndex(index: number): number {
        ...
        let sourceIndex = overallIndex - this._groups.length - this._spaceItemsCount;
    ...
    _getSerializableState(state: IDefaultSerializableState): ISerializableState {
        ...
        resultState._spaceItemsCount = this._spaceItemsCount;
    ...
    _setSerializableState(state: ISerializableState): Function {
        ...
        return function (): void {
            ...
            this._spaceItemsCount = state._spaceItemsCount;
    ...
    protected _getItems(): T[] {
        const spaceItems = this._spaceItems.reduce((acc, item) => {
            if (item.top) {
                acc.push(item.top);
            }
            if (item.bottom) {
        ...
        return (this._groups as any as T[]).concat(spaceItems as any as T[], this.source.items);
    }
    ...
    protected _createItemsOrder(): number[] {
        return Group.sortItems<S, T>(this.source.items, {
            ...
            spaceItemFactory: this._options.spaceItemFactory,
            blocksGapCallback: this._options.blocksGapCallback,
    ...
    static sortItems<S, T extends CollectionItem<S> = CollectionItem<S>>(
        ...
        const createSpaceItem = (
            ...
            return options.spaceItemFactory(group, {
                key: `space-collection-item-${position}-group-${group_id}`,
                className: `${prefix} ${prefix}_${position} ${prefix}_${groupViewMode}`,
                itemsSpacing: options.blocksGapCallback(position, group_id),
        ...
        for (let position = 0; position < items.length; position++) {
            ...
            if (groupsId.indexOf(groupId) === -1) {
                ...
                if (options.groupViewMode === 'titledBlocks') {
                    // Для titledBlocks после обычной группы создаём одну линию-разделитель,
                    ...
                    spaceItems.push(groupSpaceItems);
                } else if (options.groupViewMode === 'blocks') {
        ...
        // Сетим CSS класс для записей внутри каждой группы.
        // Классами выделяются первая, последняя, средние сгруппиррованные записи.
        for (const groupIndex in groupsItems) {
            if (groupsItems.hasOwnProperty(groupIndex)) {
                let cnt = 0;
                const groupedItemsKeys = groupsItems[groupIndex];
                while (cnt < groupedItemsKeys.length) {
                    const itemIndex = groupedItemsKeys[cnt];
                    const prefix = 'controls-ListView__item_grouped';
                    let className = `${prefix}`;
                    if (cnt !== 0 && cnt !== groupedItemsKeys.length - 1) {
                        className += ` ${prefix}_${options.groupViewMode}_middle`;
                    }
                    if (cnt === 0) {
                        const isHidden =
                            options.hiddenGroupPosition === 'first' &&
                            groupsId[groupIndex] === groupConstants.hiddenGroup;
                        className += ` ${prefix}_${options.groupViewMode}${
                            isHidden ? '_hiddenGroup' : ''
                        }_first`;
                    }
                    if (cnt === groupedItemsKeys.length - 1) {
                        className += ` ${prefix}_${options.groupViewMode}_last`;
                    }
                    items[itemIndex].setClassName(className);
                    cnt++;
                }
            }
        }
        ...
        let spaceItemIndex = groupsCount;
        groupsOrder.forEach((groupIndex) => {
            result.push(groupIndex);
            const spaceItem = spaceItems[groupIndex];
            const hasTopSpaceItem = !!spaceItem?.top;
            if (hasTopSpaceItem) {
                // Добавляем индекс линии-разделителя перед сгруппированными записями.
                result.push(spaceItemIndex);
                spaceItemIndex += 1;
            }
            // Добавляем индексы сгруппированных записмей
            groupsItems[groupIndex].forEach((item) => {
                result.push(item + groupsCount + spaceItemsCount);
            });
            if (spaceItem?.bottom) {
                // Добавляем индекс линии-разделителя после сгруппированных записей.
                result.push(spaceItemIndex);
                spaceItemIndex += 1;
            }
        });
