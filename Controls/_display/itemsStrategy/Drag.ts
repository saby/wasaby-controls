    ...
    implements IItemsStrategy<S, T>
{
    ...
    protected _isDisplayItem(
        ...
        if (!collectionItem.DraggableItem) {
            if (collectionItem['[Controls/_display/GroupItem]']) {
                ...
                const endIndex =
                    nextGroupIndex !== -1 ? nextGroupIndex : items.length;
    ...
    static sortItems<
        S extends Model = Model,
        T extends CollectionItem<S> = CollectionItem<S>
    >(items: T[], options: ISortOptions<T>): number[] {
        ...
        if (options.avatarItem) {
            ...
            const targetIndex =
                options.targetIndex < itemsCount
                    ? this._getIndexGivenFilter(
                          options.targetIndex,
                          options.filterMap
                      )
                    : itemsCount - 1;
            const startIndex =
                options.startIndex < itemsCount
                    ? options.startIndex
                    : itemsCount - 1;
     ...
     * @private
     */
    private static _getIndexGivenFilter(
        sourceIndex: number,
        filterMap: boolean[]
    ): number {
