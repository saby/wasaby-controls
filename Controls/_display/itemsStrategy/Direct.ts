...
export default class Direct<S, T> extends AbstractStrategy<S, T> {
    ...
    at(index: number): T {
        ...
        if (position === undefined) {
            throw new ReferenceError(
                `Display index ${index} is out of bounds.`
            );
        }
    ...
    splice(start: number, deleteCount: number, added: S[] = []): T[] {
        ...
        const result = this._getItems().splice(
            start,
            deleteCount,
            ...reallyAdded
        );
    ...
    _getSerializableState(
        state: IDefaultSerializableState<T>
        ...
        (items as any as CollectionItem<any>[]).forEach((item, index) => {
            ...
            if (processed.has(itemId)) {
                if (options.validateUnique) {
                    Logger.error(
                        `Данные содержат дубли записей с ключом="${itemId}". Не возможна корректная работа списка.`
                    );
                }
                return;
            }
