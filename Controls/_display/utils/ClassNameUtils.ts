...
export function getEditorViewRenderClassName(
    params: IGetEditorViewClassNameParams = {}
): string {
    ...
    if (
        ...
        params.inputBorderVisibility === 'partial'
    ) {
        classes += ' controls-EditingTemplateText_borderBottom';
    }
    if (inputBackgroundVisibility !== 'hidden') {
...
export function getEditorClassName(
    align: string,
    editingMode: 'row' | 'cell'
): string {
    let classes =
        'controls-EditingTemplateText__editorWrapper js-controls-List__editingTemplate';
    const resolvedAlign =
        align || (editingMode === 'cell' ? 'default' : 'left');
