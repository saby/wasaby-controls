...
export function getEditorViewRenderClassName(params: IGetEditorViewClassNameParams = {}): string {
    ...
    if (params.baseline === 'default' || params.baseline === undefined) {
        classes += 'controls-EditingTemplateText';
    }

    classes += ' controls-EditingTemplateText_border-partial';
    // TODO В реакт рендере fontSize должен задаваться через опцию fontSize
    //  controls-EditingTemplateText_size_ просто в обход api перебивает
    //  установлнное значение. А потом для узлов оно перебивается дополнительно каскадом
    //  от .controls-TreeGrid__row-cell__node
    if (params.fontSize) {
        classes += ` controls-EditingTemplateText_size_${params.fontSize}`;
    }

    if (params.textAlign) {
        classes += ` controls-Render_textAlign-${params.textAlign}`;
    }

    if (params.inlineHeight) {
        classes +=
    ...
    if (params.backgroundStyle && params.backgroundStyle !== 'none') {
        classes += ` controls-background-${params.backgroundStyle}`;
    }
