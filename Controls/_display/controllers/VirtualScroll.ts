...
export interface IVirtualScrollCollection extends IBaseCollection<unknown, any> {
...
export function isItemAtIndexHidden(collection: IVirtualScrollCollection, index: number): boolean {
    ...
    return (index < start || index >= stop) && (index >= nextStart || index < nextStop);
}
...
export function each(
    ...
    while (enumerator.moveNext()) {
        ...
        const insideRange = currentIndex >= startIndex && currentIndex < stopIndex;
        const isRenderBeforeRange = currentIndex < startIndex && current?.isRenderedOutsideRange();
        const isRenderAfterRange = currentIndex >= stopIndex && current?.isRenderedOutsideRange();
...
export function getNextStartIndex(collection: IVirtualScrollCollection): number {
