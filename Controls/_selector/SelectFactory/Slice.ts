...
import { TSelectorTabsConfigs } from 'Controls/_selector/interfaces/ISelector';
...
export default class SelectSlice extends Slice<ISelectSliceState> {
    ...
    private _getIsSelectedItemsChanged(
        ...
        return Object.keys(this.state.configs).some((key) => {
            ...
            if (selectedItems[key].getCount() === this._initialSelectedItems[key].getCount()) {
                selectedItems[key].each((item) => {
                    const storeId = this.state.configs[this.state.selectedTabKey].storeId;
                    const { keyProperty } =
                        this.state.listResults[this.state.selectedTabKey][storeId];
                    const removingRecordIndex = this._initialSelectedItems[key].getIndexByValue(
            ...
            return !!(this._initialSelectedItems[key].getCount() || selectedItems[key].getCount());
        });
    }
