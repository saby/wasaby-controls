 ...
 * @variant warning
 * @variant danger
 * @variant none (не подсвечивать в выбранном состоянии)
 * @variant info
 */
...
export default class Fingers extends React.Component<IProps> {
    ...
    private _calcWrapperClassName(props: IFingerProps, isSelected: boolean): string {
        let wrapperClassName = 'Controls-Fingers__item';
        if (isSelected && this.props.selectedStyle !== 'none') {
            wrapperClassName += ` Controls-Fingers__item_marked_${this.props.selectedStyle}`;
        } else {
            wrapperClassName += ' Controls-Fingers__item-shadow Controls-Fingers__item_unmarked';
            wrapperClassName += ` Controls-Fingers__item_hover-${this.props.hoverBackgroundStyle}`;
        }
        wrapperClassName +=
    ...
    private _itemTemplate(
        ...
        // При выравнивании слева текст должен выравниваться по одной линии, остаётся место под картинку
        // При выравнивании справа текст должен заезжать на место картинки и обрезаться только у её края.
        // Поручение в стандарт https://online.sbis.ru/opendoc.html?guid=2f4d4ed6-1bb1-4317-8a41-17f29d542942&client=3
        if (imagePath || (this.state._hasItemImages && align === 'left')) {
            contentClassName += ` Controls-Fingers__item__content__title-align_${align}`;
            const imageStyle = {
                backgroundImage:
                    (imagePath ? `url("${imagePath}"),` : '') +
                    (this.props.fallbackImage ? `url("${this.props.fallbackImage}")` : 'none'),
            };
    ...
    render() {
        ...
        return (
            <div ref={this.props.forwardedRef} className={className} style={style} data-qa={this.props['data-qa']}>
                {items}
            </div>
        );
    }
...
/**
 * Контрол Вертикальной навигации с изображениями.
 * Позволяет отобразить навигацию в виде "пальцев".
 * См. также {@link https://www.figma.com/proto/KQKOZj4hU7pqHJ4jdB0BCP/%E2%9C%94%EF%B8%8F-%D0%A1%D0%BF%D0%B8%D1%81%D0%BA%D0%B8-%D0%B4%D0%BB%D1%8F-%D0%B4%D0%B2%D1%83%D1%85%D0%BA%D0%BE%D0%BB%D0%BE%D0%BD%D0%BE%D1%87%D0%BD%D1%8B%D1%85-%D1%80%D0%B5%D0%B5%D1%81%D1%82%D1%80%D0%BE%D0%B2 Стандарт списков для двухколоночного реестра}
 ...
 * @default default
 * @demo Controls-demo/Fingers/SelectedStyle/Index
 */
