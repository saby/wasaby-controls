...
import { getHeaderFontSizeClass } from 'Controls/Utils/getFontClass';
...
/**
 * Варианты стиля подложки для кнопки "Назад"
 * @typedef Controls/_heading/Back/TBackButtonStyle/TBackButtonStyle
 * @variant pale
 * @variant default
 */
export type TBackButtonStyle = 'pale' | 'default';
...
export interface IBackOptions
    ...
    /**
     * Стиль обводки для иконки при значении "functionalButton" в опции {@link iconViewMode}.
     * Применяется, если необходимо разместить кнопку с обводкой на фоне, отличном от "default"
     * @cfg {Controls/_heading/Back/TBackButtonStyle/TBackButtonStyle.typedef}
     * @default pale
     * @demo Controls-demo/Heading/Back/IconViewMode/Index
     */
    buttonStyle?: TBackButtonStyle | string;
    onClick?: Function;
}
 ...
 * * {@link https://git.sbis.ru/saby/wasaby-controls/-/blob/rc-24.6100/Controls-default-theme/variables/_heading.less переменные тем оформления}
 *
 *
    ...
    implements IFontColorStyle, IFontSize, IIconStyle
{
    ...
    _getHeaderFontSize() {
        return getHeaderFontSizeClass(this._options.fontSize);
    }

    _getMobileClass() {
        if (detection.isPhone) {
            return ' controls-Back_mobile';
        }
        return '';
    }
