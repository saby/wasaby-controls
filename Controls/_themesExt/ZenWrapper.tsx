...
import { isDarkTheme } from 'Controls/Utils/Helpers/functions';
import { Wrapper, IContentOptions } from 'Controls/themes';
import 'css!Controls/themesExt';

type TBrightness = 'dark' | 'light';

export interface IZenWrapperOptions extends IComponentPropsWithReadonly {
...
const getDerivedStateFromProps = (options: IZenWrapperOptions): Record<string, string> => {
    let variables: any = {};
    const complementaryRGB = calculateRGB(options.complementaryColor);
    variables = calculateVariables(
        calculateRGB(options.dominantColor),
        complementaryRGB,
        options.brightness
    );
    if (options.complementaryColor) {
        variables['--primary_color_darkness'] = +isDarkTheme(complementaryRGB);
    }
