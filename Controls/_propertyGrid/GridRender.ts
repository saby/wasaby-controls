...
export default class IPropertyGridRender extends Control<IPropertyGridGridRenderOptions> {
    ...
    protected _getColumnsWidth(
        ...
        const captionColumnWidth = isFullGridSupport()
            ? width.caption
            : width.compatibleCaption;
        const editorColumn = isFullGridSupport()
            ? width.editor
            : width.compatibleEditor;
        const multiSelectColumnsWidth =
            multiSelectVisibility !== 'hidden' ? 'max-content' : '';
    ...
    private _getGridStyles(
        ...
        return (
            ...
            (colspanIndex !== undefined
                ? this._getColspanStyles(colspanIndex)
                : '')
        );
    }
    ...
    private _isMultiSelect(): boolean {
        const multiSelectVisibility =
            this._options.listModel.getMultiSelectVisibility();
