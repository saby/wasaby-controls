...
interface IPropertyGridGridRenderOptions extends IControlOptions {
    ...
    toggleEditorsTemplate?: TemplateFunction;
}
...
export default class IPropertyGridRender extends Control<IPropertyGridGridRenderOptions> {
    ...
    private _getItemIndex(item: TPropertyGridCollectionItem): number {
        return this._options.listModel.getIndex(item);
    }

    private _getPreviousItemIndex(item: TPropertyGridCollectionItem): number {
    ...
    protected _itemClick(e: SyntheticEvent<MouseEvent>, item: TPropertyGridCollectionItem): void {
        if (e.target.closest('.js-controls-ListView__checkbox')) {
            ...
            e?.target?.closest('.controls-PropertyGrid__editor-expander')
        ) {
            this._notify('expanderClick', [item, e]);
