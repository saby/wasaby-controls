...
import {
    Move as MoveViewCommand,
    AtomicRemove as RemoveViewCommand,
} from 'Controls/viewCommands';
...
type TPropertyGridCollection = PropertyGridCollection<
    Model,
 ...
 *
 * @class Controls/_propertyGrid/PropertyGridView
 * @remark
...
export default class PropertyGridView extends Control<IPropertyGridOptions> {
    ...
    private _unprocessedDragEnteredItem: PropertyGridCollectionItem<Model> =
        null;
    ...
    protected _beforeMount(options: IPropertyGridOptions): void {
        ...
        this._collapsedGroups = this._getCollapsedGroups(
            options.collapsedGroups
        );
        this._toggledEditors = this._getToggledEditors(options);

        this._collectionChangedHandler =
            this._collectionChangedHandler.bind(this);
    ...
    protected _beforeUpdate(newOptions: IPropertyGridOptions): void {
        ...
        if (collapsedGroups !== this._options.collapsedGroups) {
            ...
            this._listModel.setCollapsedGroups(
                this._getCollapsedGroupsAsArray()
            );
        }
        ...
        if (
            ...
            this._listModel.setMultiSelectAccessibilityProperty(
                multiSelectAccessibilityProperty
            );
        }
    ...
    protected _afterRender(
        oldOptions?: IPropertyGridOptions,
        oldContext?: unknown
    ): void {
    ...
    private _getToggledEditors({
        ...
        typeDescription.forEach((item) => {
            if (
                object.getPropertyValue(item, PROPERTY_TOGGLE_BUTTON_ICON_FIELD)
            ) {
                key = object.getPropertyValue<string>(item, keyProperty);
                result[key] = toggledEditors
                    ? toggledEditors.includes(key)
                    : false;
            }
        });
        return result;
    ...
    protected _propertyValueChanged(
        ...
        this._editingObject = this._updatePropertyValue(
            this._editingObject,
            name,
            value
        );
    ...
    protected _toggleEditor(
        ...
        const currentEditorName: string = item.get(
            this._listModel.getKeyProperty()
        );
        this._toggledEditors = { ...this._toggledEditors };
        const oldToggledEditors = Object.keys(this._toggledEditors).reduce(
            (acc, key) => {
        ...
        const newToggledEditors = Object.keys(this._toggledEditors).reduce(
            (acc, key) => {
                return !this._toggledEditors[key] ? acc.concat([key]) : acc;
            },
            []
        ...
        this._notify('toggledEditorsChanged', [
            newToggledEditors,
            diff.added,
            diff.removed,
        ]);
    ...
    private _createSelectionStrategy(
        ...
        const strategyOptions = this._getSelectionStrategyOptions(
            options,
            collection
        );
        if (options.parentProperty) {
            return new TreeSelectionStrategy(
                strategyOptions as ITreeSelectionStrategyOptions
            );
        } else {
    ...
    private _changeSelection(selection: ISelectionObject): void {
        ...
        const selectionDifference =
            selectionController.getSelectionDifference(selection);
        ...
        if (handleSelection) {
            ...
            switch (action) {
                case IObservable.ACTION_REMOVE:
                    newSelection = selectionController.onCollectionRemove(
                        ...removedItems
                    );
                    break;
    ...
    _dragStart(dragObject: IDragObject, draggedKey: CrudEntityKey): void {
        ...
        if (
            ...
            this._listModel.isDragOutsideList()
        ) {
            this._notify(
    ...
    _dragLeave(): void {
        ...
        if (this._dndController && this._dndController.isDragging()) {
            ...
            if (
                draggableItem &&
                this._listModel.getItemBySourceKey(
                    draggableItem.getContents().getKey()
                )
            ) {
    ...
    _dragEnter(dragObject: IDragObject): void {
        ...
        if (this._documentDragging) {
            if (
                ...
                const dragEnterResult = this._notify('customdragEnter', [
                    dragObject.entity,
                ]);
    ...
    _documentDragEnd(dragObject: IDragObject): void {
        ...
        if (
            !this._listModel ||
            !this._dndController ||
            !this._dndController.isDragging()
        ) {
            return;
        }
    ...
    private _startDragNDrop(
        ...
        if (
            ...
            let dragStartResult = this._notify('customdragStart', [
                [draggableKey],
                draggableKey,
            ]);
            ...
            if (dragStartResult) {
                ...
                if (this._startEvent && this._startEvent.target) {
                    this._startEvent.target.classList.add(
    ...
    private _onMove(nativeEvent): void {
        if (this._startEvent) {
            ...
            if (this._dndController && this._dndController.isDragging()) {
                const moveOutsideList = !this._container.contains(
                    nativeEvent.target
                );
    ...
    private _processItemMouseEnterWithDragNDrop(
        ...
        if (this._dndController.isDragging()) {
            ...
            const mouseOffsetInTargetItem = this._calculateMouseOffsetInItem(
                event,
                targetElement
            );
    ...
    private _calculateMouseOffsetInItem(
        ...
        if (targetElement) {
            ...
            result.top =
                (mouseCoords.y - dragTargetRect.top) / dragTargetRect.height;
            result.bottom =
    ...
    private _draggingItemMouseMove(
        itemData: TreeItem,
        event: SyntheticEvent<MouseEvent>
    ): void {
        const dispItem = itemData;
        const targetIsNotDraggableItem =
            this._dndController.getDraggableItem()?.getContents() !==
        ...
        if (
            ...
            const dragTargetPosition =
                this._dndController.calculateDragPosition({
                    targetItem: dispItem,
                    mouseOffsetInTargetItem,
                });
    ...
    _executeMoveViewCommand(
        ...
        if (!isLoaded('Controls/viewCommands:Move')) {
            ...
            const moveViewCommandClass = loadSync<MoveViewCommand>(
                'Controls/viewCommands:Move'
            );
            this._getMoveViewCommand(
                moveViewCommandClass,
                keys,
    ...
    _executeRemoveViewCommand(selection: ISelectionObject): Promise<unknown> {
        if (!isLoaded('Controls/viewCommands:AtomicRemove')) {
            return loadAsync<RemoveViewCommand>(
                'Controls/viewCommands:AtomicRemove'
            ).then((removeViewCommandClass) => {
                return this._getRemoveViewCommand(
    ...
    moveWithDialog(
        ...
        return import('Controls/listCommands').then(({ MoveWithDialog }) => {
            ...
            return moveCommand.execute({}).then(() => {
                this._executeMoveViewCommand(
                    movedItems,
                    LOCAL_MOVE_POSITION.On,
                    resultTarget
                );
            });
        });
    ...
    startValidation({
        ...
        validators = [],
        value,
    }: IPropertyGridValidatorArguments):
        ...
        if (
            ...
            (collectionItem && collectionItem.isEditing())
        ) {
            ...
            if (validators.length) {
                validatorResult = PropertyGridView._prepareValidators(
                    validators
                ).then((loadedValidators) => {
                    const errors = [];
                    loadedValidators.forEach((validatorFunc) => {
                        const localValidatorResult =
                            validatorFunc(validatorArgs);
    ...
    private async _validate(): Promise<string[] | null> {
        ...
        const validationResult =
            await this._children.validateController.submit();
        ...
        while (collectionIterator.moveNext()) {
            ...
            if (validators && validators.length) {
                const value = object.getPropertyValue(
                    editingObject,
                    item.get(keyProperty)
                );
    ...
    protected _itemClick(
        ...
        const canEditByClick =
            !this._options.readOnly && collectionItem.isEditable();
        if (canEditByClick) {
            e.stopPropagation();
    ...
    beginAdd(
        userOptions: IEditingUserOptions
    ...
    private _activateEditingRow(): void {
        const activator = () => {
            ...
            const editingItemContainer = this._getElementByKeyFromContainer(
                editingItem.getUid()
            );
            return editingItemContainer ? activate(editingItemContainer) : null;
        };

        this._editInPlaceInputHelper.activateInput(
            activator,
            this._listModel.beforeRowActivated
        );
    }
    ...
    private _getEditInPlaceController(): Promise<EditInPlaceController> {
        if (!this._editInPlaceController) {
            return import('Controls/editInPlace').then(
    ...
    private _beforeBeginEditCallback(
    ...
    private _afterBeginEditCallback(
    ...
    private _beforeEndEditCallback(
        ...
        return this._getValidationController()
            ...
            .then((validationResult) => {
                ...
                if (
                    params.willSave &&
                    (editingValue !== currentValue || oldName !== name)
                ) {
    ...
    private _afterEndEditCallback(
        ...
        this._notify('typeDescriptionChanged', [
