...
export default class PropertyGridView extends PropertyGrid {
    ...
    protected _setExpandedItemsToValidate(validateItemKey: TKey): void {
        ...
        const newExpandedItems = this._getItemsParentsKeys(
            expandedItems,
            this._listModel.getSourceCollection()
        );
        this._listModel.setExpandedItems(newExpandedItems);
    }

    private _getItemsParentsKeys(itemsKeys: TKey[], collection: RecordSet): TKey[] {
        const itemKey = [...itemsKeys].pop() as string | number;
        const parentKey = collection
            .getRecordById(itemKey)
            .get(this._options.parentProperty) as TKey;
        if (parentKey) {
            itemsKeys.push(parentKey);
            return this._getItemsParentsKeys(itemsKeys, collection);
        }
        return itemsKeys;
    }
}
