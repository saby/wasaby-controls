 ...
 * @remark В возвращаемом рекордсете записи должны быть связаны с родителем, для которого происходит запрос. Для этого у записей заполните поле parent
...
/**
 * Метод, вызываемый при изменении значения контекста в экшене
 * Можно использовать для изменения визуального состояния экшена в момент изменения контекста.
 * @function Controls/_actions/interface/IAction#updateContext
 * @param {T} newContext
 * @returns {void}
 * @remark Контекст экшена зависит от места внедрения экшена
 * @example
 * <pre class="brush: js">
 * updateContext(newContext: T): void {
 *    if (newContext.shouldHideAllActions) {
 *       this.visible = false;
 *    }
 * }
 * </pre>
...
export interface IActionExecuteParams extends IActionState {
    ...
    target: HTMLElement;
    event: MouseEvent;
    opener: IBasePopupOptions['opener'];
    router: IRouter;
}
