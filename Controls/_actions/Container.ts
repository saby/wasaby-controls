...
export default class ActionsContainer extends Control<IContainerOptions> {
    ...
    protected _beforeMount(options: IContainerOptions): void {
        ...
        this._toolbarItems = this._getToolbarItems(
            this._actionsCollection.getToolbarItems()
        );
        this._actionsCollection.subscribe(
    ...
    protected _selectionChanged(
        ...
        this._actionsCollection.selectionChange(
    ...
    protected _beforeUpdate(options: IContainerOptions): void {
        ...
        if (options.actions !== this._options.actions) {
            ...
            this._actionsCollection.notifyActionsUpdateContext(
                options.prefetchData
            );
        }
    }
    ...
    protected _toolbarItemClick(
        ...
        return this._executeAction(item, clickEvent, [item.getKey()]);
    }
    ...
    protected _executeAction(
        ...
        const canExecuteAction = action?.canExecute(item);
        action?.execute({
            action,
            toolbarItem: item,
            ...
            selectedKeysCount:
                this._operationsController?.getSelectedKeysCount(),
        });
