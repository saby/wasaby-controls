...
/**
 * Источник для тулбара с экшенами
 * @private
 */
export default class ActionsSource extends Memory {
    protected _collection: ActionsCollection;
    ...
    query(query?: Query): Promise<RecordSet> {
        ...
        const isStrictMenuShowType = where.isStrictMenuShowType;
        if (where.parent) {
            return this._collection.getChildren(where.parent, query).then((result) => {
            ...
            return Promise.resolve(
                new RecordSet({
                    keyProperty: 'id',
                    rawData: this._collection.getMenuItems(isStrictMenuShowType),
                })
            );
        }
    }
