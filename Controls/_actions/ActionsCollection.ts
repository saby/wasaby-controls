...
export default class ActionsCollection extends mixin<ObservableMixin>(ObservableMixin) {
    ...
    getActionByItem(item: Model<IAction>): IBaseAction {
        ...
        if (action) {
            ...
            const parentRS = this._findParentRS(parentKey);
    ...
    private _findParentRS(parentKey: any): any {
        return Object.values(this._childItems).find((childs) => {
    ...
    getToolbarItems(customItems?: IAction[]): IAction[] {
        ...
        if (this._options.isAdaptive && this._options.changeShowTypeInAdaptive) {
            ...
            this._options.changeMenuItemsShowType &&
            (rootItems.length <= MIN_ACTIONS_FOR_MENU || this._isAllActionsFrequent(rootItems))
        ) {
            return visibleItems.map((visibleItem) => {
    ...
    /*
     * Метод для получения действий меню. Используется в MenuSource.query
     * @param isStrictMenuShowType Флаг строгий showType. Убирает игнорирование showType если кнопок меньше,
     * чем MIN_ACTIONS_FOR_MENU. По умолчанию false. В значении true кнопки набираются строго согласно showType
     */
    getMenuItems(isStrictMenuShowType: boolean = false): IAction[] {
        const lastToolbarItemOrder = this._getLastItemOrder(
            this._getToolbarItemsByActions(this._actions)
        );
        const toolbarItems = this.getToolbarItems(
            this._getToolbarItemsByActions(this._actions, 'menu')
        );
        const menuItems =
            !isStrictMenuShowType && toolbarItems.length <= MIN_ACTIONS_FOR_MENU
                ? toolbarItems
                : toolbarItems.filter((item) => item.showType !== showType.TOOLBAR);
        return menuItems
            .map((item) => {
                let newOrder = 0;
                // Элементы тулбара которые отображаются только в меню должны идти после других
                if (this._options.changeMenuItemsShowType && item.showType === showType.MENU) {
                    newOrder = lastToolbarItemOrder + (item.order || 1);
                }
                return {
                    ...item,
    ...
    protected _getLastItemOrder(items: IAction[] = []): number {
        let maxOrder = 1;
        items.forEach((item: IAction) => {
            if (item.showType !== showType.MENU && item.order > maxOrder) {
                maxOrder = item.order;
            }
        });
        return maxOrder;
        ...
        return actions
            .map((action) => {
                try {
                    ...
                    Logger.error(
                        `Во время создания действия ${action.actionName} произошла ошибка: ${error}`
                    );
                    return null;
                }
            })
