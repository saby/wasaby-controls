 ...
 * @property {Boolean} startOnPointerDown Определяет момент начала перемещения.
 * @property {Boolean} immediately Определяет расстояние начала смещения при перемещении курсора.
 * @remark
 * startOnPointerDown:
 * * false - dnd начинается на mouseMove.
 * * true - dnd начинается на mouseDown. На touch-устройствах долгого нажатия.
 * immediately:
 * * false - Перемещение начинается после перемещения мыши на 4px по горизонтали или вертикали.
 * * true - Перемещение начинается сразу после перемещения мыши.
 */
...
class Container extends Control<IContainerOptions> {
    ...
    private _startEvent: MouseEvent | TouchEvent = null;
    private _currentEvent: MouseEvent = null;
    private _startImmediately: boolean = null;
    private _startOnPointerDown: boolean = false;
    ...
    private _longTouchTimer: number | undefined;
    ...
    private _registerTouchMove(): void {
    ...
    private _onMove(nativeEvent: MouseEvent): void {
        if (this._startEvent) {
            const dragObject = this._initialDragStart(nativeEvent, this._startEvent, false);
            if (this._documentDragging) {
                this._currentEvent = nativeEvent;
                this._notify('dragMove', [dragObject]);
                this._showDraggingTemplate(dragObject);
            }
        }
    }

    private _showDraggingTemplate(dragObject: IDragObject): void {
    ...
    private _toggleFixIoSDragBug(state: boolean): void {
        if (detection.isMobileSafari) {
            if (state) {
                this.thisDocumentOverflow = document.documentElement.style.overflow;
            }
            document.documentElement.style.overflow = state ? 'hidden' : (this.thisDocumentOverflow || '');
        }
    }
    ...
    private _onLongTouchMove(event: TouchEvent): void {
        if (Container._getIsShifted(this._startEvent, event)) {
            clearTimeout(this._longTouchTimer);
            this._notify('unregister', ['touchmove', this, this._onLongTouchMove], {
                bubbling: true,
            });
        }
    }
    ...
    private _onLongTouch() {
        this._registerTouchMove();
        this._initialDragStart(this._startEvent, this._startEvent);
    }

    private _initialDragStart(
        event: MouseEvent | TouchEvent,
        startEvent: MouseEvent | TouchEvent,
        showDraggingTemplateOnDragStart: boolean = true
    ) {
        const dragObject: IDragObject = this._getDragObject(event, startEvent);
        const dragStarted: boolean = Container._isDragStarted(
            startEvent,
            event,
            this._startImmediately,
            this._startOnPointerDown
        );
        if (!this._documentDragging && dragStarted) {
            ...
            if (showDraggingTemplateOnDragStart) {
                this._showDraggingTemplate(dragObject);
            }
        }
        return dragObject;
    ...
    protected _onPointerDown(pointerDownEvent: MouseEvent | TouchEvent): void {
        if (pointerDownEvent.type === 'touchstart') {
            this._longTouchTimer = setTimeout(
                this._onLongTouch.bind(this),
                Container.LONG_PRESS_DELAY
            );
            this._notify('register', ['touchmove', this, this._onLongTouchMove], {
                bubbling: true,
            });
            return;
        }

        if (pointerDownEvent.type === 'mousedown') {
            this._registerMouseMove();
            this._initialDragStart(pointerDownEvent, pointerDownEvent);
            return;
        }
    }
    ...
    startDragNDrop(
        ...
        pointerDownEvent:
            ...
            | SyntheticEvent<TouchEvent>
            | MouseEvent
            | TouchEvent,
        options: IStartDragOptions = {immediately: false, startOnPointerDown: false},
        draggedKey?: string
    ): void {
        if ((pointerDownEvent.target as HTMLElement).closest(`.${NOT_DRAGGABLE_JS_SELECTOR}`)) {
            return;
        }
        ...
        this._startEvent = pointerDownEvent.nativeEvent ?? pointerDownEvent;
        this._startImmediately = options.immediately;
        this._startOnPointerDown = options.startOnPointerDown;
        ...
        if (this._startOnPointerDown) {
            this._onPointerDown(this._startEvent);
        } else {
            this._registerMouseMove();
            this._registerTouchMove();
        }
    ...
    private static LONG_PRESS_DELAY: number = 500;
    ...
    private static _isDragStarted(
        ...
        if (immediately || startOnPointerDown) {
            return true;
        }

        return Container._getIsShifted(startEvent, moveEvent);
    }

    private static _getIsShifted(
        moveEvent: MouseEvent | TouchEvent,
        startEvent: MouseEvent | TouchEvent
    ) {
        const offset: ICords = Container._getDragOffset(moveEvent, startEvent);
        return (
            Math.abs(offset.x) > Container.SHIFT_LIMIT || Math.abs(offset.y) > Container.SHIFT_LIMIT
        );
    }
    ...
    private static _clearSelection(event: MouseEvent | TouchEvent): void {
...
/**
 * @event Controls/_dragnDrop/Container#customdragStart Происходит, когда пользователь начинает перемещение объект в текущем контроллере.
 * @param {UI/Events:SyntheticEvent} eventObject Дескриптор события.
 * @param {dragObject} dragObject Объект, в котором содержится информация о текущем состоянии Drag'n'drop.
...
/**
 * @event Controls/_dragnDrop/Container#customdragEnd Происходит после того, как пользователь закончил перемещение объекта в текущем контроллере.
 * @param {UI/Events:SyntheticEvent} eventObject Дескриптор события.
 * @param {dragObject} dragObject Объект, в котором содержится информация о текущем состоянии Drag'n'drop.
...
/**
 * @event Controls/_dragnDrop/Container#customdragEnter Происходит после перемещения объекта внутри контроллера.
 * @param {UI/Events:SyntheticEvent} eventObject Дескриптор события.
 * @param {dragObject} dragObject Объект, в котором содержится информация о текущем состоянии Drag'n'drop.
 * @example
...
/**
 * @event Controls/_dragnDrop/Container#customdragLeave Происходит после перемещения объекта за пределы контроллера.
 * @param {UI/Events:SyntheticEvent} eventObject Дескриптор события.
 * @param {dragObject} dragObject Объект, в котором содержится информация о текущем состоянии Drag'n'drop.
 * @example
 * В следующем примере показано, как изменить визуальное состояние контрола при наведении на него курсора.
...
/**
 * @event Controls/_dragnDrop/Container#customdragMove Происходит при перемещении объекта на странице.
 * @param {UI/Events:SyntheticEvent} eventObject Дескриптор события.
 * @param {dragObject} dragObject Объект, в котором содержится информация о текущем состоянии Drag'n'drop.
 ...
 * <pre>
 *     <Controls.dragnDrop:Container name="dragNDropController" on:customdragMove="_dragMove()">
 *        <ws:content>
 *           <div>
 *              <ws:for data="item in _items">
