...
class Container extends Control<IContainerOptions> {
    ...
    startDragNDrop(
        entity: object,
        mouseDownEvent: SyntheticEvent<MouseEvent> | MouseEvent,
        options: IStartDragOptions = { immediately: false },
        draggedKey?: string
    ): void {
        if ((mouseDownEvent.target as HTMLElement).closest(`.${NOT_DRAGGABLE_JS_SELECTOR}`)) {
            return;
        }
        ...
        this._startEvent = mouseDownEvent.nativeEvent ?? mouseDownEvent;
