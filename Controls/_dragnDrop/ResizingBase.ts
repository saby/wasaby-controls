...
import { Container, IDragObject, IResizingBase, IResizingEntity } from 'Controls/dragnDrop';
...
abstract class ResizingBase<T extends IResizingBase> extends Control<IControlOptions, T> {
    ...
    protected _dragObject: IDragObject<IResizingEntity>;
    protected _isStopedClick: boolean = false;
    ...
    protected _handleClick(e: MouseEvent): void {
        if (this._isStopedClick) {
            e.stopPropagation();
            e.preventDefault();
        }
        this._isStopedClick = false;
    ...
    protected _beginDragHandler(event: SyntheticEvent<MouseEvent>): void {
        ...
        // Останавливаем любое событие клика
        if (constants.isBrowserPlatform && document) {
        ...
        this._children.dragNDrop.startDragNDrop(
            {
                ...
                cursorType: `${this._options.orientation}Resize`,
                offset: 0,
            },
            event,
            {
    ...
    protected _onStartDragHandler(_, dragObject): void {
        this.startDrag();
    ...
    endDrag(offset: object | number): void {
        if (this._dragging) {
            ...
            if (offset) {
                this._isStopedClick = true;
            }
            this._notify('offset', [offset, this._options.position]);
        }
        if (constants.isBrowserPlatform && document) {
            // Событие клика не всегда стреляет при завершении драга.
            // Поэтому самостоятельно удаляем подписку на событие клика.
            setTimeout(() => {
                document.removeEventListener('click', this._handleClick, true);
            }, 0);
        }
