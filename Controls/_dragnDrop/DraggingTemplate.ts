...
export interface IDraggingTemplateOptions extends IControlOptions {
...
export default class DraggingTemplate extends Control<IDraggingTemplateOptions> {
 ...
 * @remark Опция должна содержать шаблон изображения. Если этот параметр указан, то параметр logo и параметр image не применяются.
 * @example
 * В следующем примере показано, как использовать стандартный шаблон перемещения.
 * <pre>
 *    <Controls.list:View source="{{_viewSource}}"
