/**
 * @kaizen_zone 1dd941cc-6648-4b6d-b623-66c4ecc3a321
 */
...
export interface IShowMoreButtonOptions extends TInternalProps, ICheckableOptions, IControlProps {
    /**
...
const getIconClassName = (readOnly: boolean, iconStyle: string, viewMode: string): string => {
    ...
    if (readOnly) {
        ...
        iconClassName += iconStyle
            ...
            : ' controls-BigSeparator-icon_style-' + viewMode;
    }
...
function ArrowIcon(
    ...
    return (
        <svg
            ...
            style={style}
            xmlSpace="preserve"
            className={getIconClassName(readOnly, iconStyle, viewMode)}
        >
            <path d="M27.5 9L19 4.1L10.5 9L10 8.1L19 3L28 8.1L27.5 9Z" className="st0" />
        </svg>
    );
}

function EllipsisIcon(readOnly: boolean, iconStyle: string, viewMode: string): React.ReactElement {
    return (
        <svg
            viewBox="0 0 38 12"
            width="100%"
...
export default React.forwardRef(function ShowMoreButton(
    ...
    const size = detection.isPhone ? 'adaptive' : iconSize;
    ...
    const getTooltip = (): string => {
        return value ? rk('Свернуть') : rk('Ещё');
    };

    const highlightedOnFocus = (): boolean => {
        return !!context?.workByKeyboard && !readOnly;
    };
    ...
    const getBackgroundClass = (): string => {
        ...
        if (viewMode === 'filled') {
            const contrastBackgroundStyle = contrastBackground ? 'contrast' : 'same';
    ...
    const wrapperClass =
        ...
        ` controls-BigSeparator-size-${size}` +
        ` controls-BigSeparator__${value ? 'expanded' : 'collapsed'}` +
        `${highlightedOnFocus() ? ' controls-focused-item' : ''} ${getAttrClass()}`;
    ...
    return (
        ...
        >
            <div
                className={
                    `controls-BigSeparator-${readOnly ? 'readonly' : 'iconContainer'}` +
                    getBackgroundClass() +
                    ` controls-BigSeparator-container-icon controls-BigSeparator-container-icon-size-${size}`
                }
                title={getTooltip()}
            >
                {value
                    ? ArrowIcon(0, readOnly, iconStyle, viewMode)
                    : props.iconMode === 'arrow'
                    ? ArrowIcon(180, readOnly, iconStyle, viewMode)
                    : EllipsisIcon(readOnly, iconStyle, viewMode)}
            </div>
        </div>
    );
});
