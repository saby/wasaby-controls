 ...
 */
import { ICheckableOptions, IControlProps } from 'Controls/interface';
...
const getIconClassName = (): string => {
    return 'controls-BigSeparator-icon';
};

function ArrowIcon(rotate: number): React.ReactElement {
    const style = {
...
export default React.forwardRef(function ShowMoreButton(
    ...
    const getBackgroundClass = (): string => {
        const baseClass = `controls-BigSeparator_${viewMode}`;
        if (readOnly) {
            return `${baseClass} controls-BigSeparator-readonly`;
        }
        if (viewMode === 'filled') {
            const contrastStyle = contrastBackground ? 'contrast' : 'same';
            return `${baseClass}_${contrastStyle}`;
        }
        return baseClass;
    };
    ...
    const getIconStyle = () => {
        if (iconStyle) {
            return `controls-icon_style-${iconStyle}`;
        }

        if (readOnly) {
            return 'controls-BigSeparator-icon-readonly';
        }

        return `controls-BigSeparator-icon_style-${viewMode}`;
    };
    ...
    return (
        ...
        >
            <div
                className={
                    'controls-BigSeparator-container-icon' +
                    (readOnly ? '' : ' controls-BigSeparator-iconContainer')
                }
                title={getTooltip()}
            >
                {value
