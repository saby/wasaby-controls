...
import { TIconSize, TSortingValue } from 'Controls/interface';
...
class Selector extends Control<ISortingSelectorOptions> {
    ...
    protected _buttonIconTemplate: Function;
    protected _buttonIconOptions: object = Record<string, unknown>;
    ...
    protected _beforeMount(options: ISortingSelectorOptions): void {
        ...
        this.updateConfig(options.sortingParams, options.value, options.iconSize);
    }
    ...
    private updateConfig(
        ...
        sortingParams.forEach((item: ISortingParam) => {
            ...
            dataElem.icon = !dataElem.icon?.includes('Controls-icons/sort')
                ? dataElem.icon
                : `FONT:${dataElem.icon}`;
            if (dataElem.paramName === this._currentParamName) {
                ...
                if (dataElem.icon) {
                    ...
                    if (!this._buttonIconTemplate) {
                        this._buttonIconTemplate = IconTemplate;
                    }
                    this._buttonIconOptions = {
                        icon: this._getButtonIcon(),
                        // Размер иконки берём из настроек
                        iconSize: this._getButtonIconSize(iconSize),
    ...
    private _getButtonIcon(): string {
        if (this._currentIcon) {
            ...
            if (this._currentParamName) {
                return 'Controls-icons/sort:icon-Arrow_' + this._orders[this._currentParamName];
            }
        }
    }
    ...
    private _getButtonIconSize(iconSize = this._options.iconSize): string {
        return (
            iconSize ||
