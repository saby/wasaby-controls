...
class Selector extends Control<ISortingSelectorOptions> {
    ...
    protected _beforeMount(options: ISortingSelectorOptions): void {
        ...
        if (options.fontColorStyle) {
            this._arrowIconStyle = Selector._getIconStyleFromTextStyle(
    ...
    protected _resetSelectedArrow(): void {
        if (this._options.value && this._options.value.length) {
            const curArrowValue =
                this._options.value[0][this._currentParamName] || 'ASC';
            this._items
                ?.getRecordById(this._currentParamName)
                ?.set('value', curArrowValue);
    ...
    private updateConfig(
        ...
        if (value && value.length) {
            ...
            this._orders[this._currentParamName] =
                value[0][this._currentParamName];
        } else {
    ...
    private _getIcon(): string {
        if (this._currentIcon) {
            return (
                this._currentIcon +
                (this._currentParamName
                    ? `_${this._orders[this._currentParamName]}`
                    : '')
            );
        } else {
            if (this._currentParamName) {
                return (
                    'Controls/sortIcons:arrow_' +
                    this._orders[this._currentParamName]
                );
            }
        }
    ...
    private _getButtonIconSize(): string {
        return (
            ...
            (this._nocaption
                ? DEFAULT_NOCAPTION_ICONSIZE
                : DEFAULT_WITHCAPTION_ICONSIZE)
        );
    }
    ...
    protected _switchValue(): void {
        const newValue: string =
            this._orders[this._currentParamName] === 'ASC' ? 'DESC' : 'ASC';
