...
export default class MultiSelector extends Control<IMultiSelectorOptions, TCount> {
    ...
    protected _getRenderProps(options: IMultiSelectorOptions): Partial<IMultiSelectorOptions> {
        ...
        return {
            ...
            onSelectedTypeChanged:
                checkWasabyEvent(options.onSelectedTypeChanged) || defaultSelectedTypeHandler,
            showSelectedCount: options.showSelectedCount,
            task1192905045: options.task1192905045,
        };
    }
    ...
    protected _beforeUpdate(newOptions: IMultiSelectorOptions): void | Promise<TCount> {
        ...
        const shouldUpdateCount =
            ...
            loadingChanged;
        const shouldUpdateRenderProps =
            (loadingChanged ||
                selectionIsChanged ||
                isAllSelectedChanged ||
                newOptions.selectionViewMode !== options.selectionViewMode ||
                newOptions.showSelectedCount !== options.showSelectedCount) &&
            !newOptions.countLoading;
        if (loadingChanged) {
            this._loading = newOptions.countLoading;
        }
        if (shouldUpdateCount) {
            calculateCountResult = this._calculateCount(newOptions, selectionCfgChanged);
        }

        if (shouldUpdateRenderProps) {
            if (calculateCountResult instanceof Promise) {
                calculateCountResult.finally(() => {
