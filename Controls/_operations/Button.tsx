...
import { useDependencyLoader } from 'Controls/Hooks/useDependencyLoader';
import { useDeprecatedStore } from 'Controls/Hooks/useDeprecatedStore';
import * as rk from 'i18n!Controls';
import 'css!Controls/operations';

const { useCallback, useMemo, memo, forwardRef } = React;

export interface IOperationsButtonProps
...
function OperationsButtonUseStoreWrapper(
...
const OperationsButtonTemplate = function OperationsButtonTemplate(
    ...
    const className = `controls_operations_theme-${props.theme}
     controls-OperationsButton controls-OperationsButton_button ${props.attrs?.className}`;
    const { onMouseEnter, onMouseLeave } = useDependencyLoader(dependencies);

    const onMouseDown = React.useCallback(
        (event) => {
            const shouldToggleExpanded =
                !props.readOnly && isLeftMouseButton(event);
            const expandedCallback =
                props.onExpandedChanged || props.onExpandedchanged;
            if (shouldToggleExpanded && expandedCallback) {
                expandedCallback(event, !props.expanded);
            }
        },
        [
            props.onExpandedChanged,
            props.onExpandedchanged,
            props.readOnly,
            props.expanded,
        ]
    ...
    return (
        <Button
            ...
            tooltip={
                props.expanded ? rk('Завершить выделение') : rk('Отметить')
            }
            className={className}
