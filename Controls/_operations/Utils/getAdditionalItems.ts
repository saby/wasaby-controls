...
export function getAdditionalItems(
    ...
    const isShowSelectedEnabled = Feature.requireLocal(['pmo_show_selected'])?.[0]?.isOn();

    if (selectionViewMode !== 'selected') {
        items.push({ ...SELECT_ALL_ITEM });
    }
    items.push({ ...UNSELECT_ALL_ITEM });
    if (selectionViewMode === 'selected' && isShowSelectedEnabled) {
        items.push({ ...SHOW_ALL_ITEM });
        // Показываем кнопку если есть выбранные и невыбранные записи
    }
    if (!isAllSelected) {
        if (
            (selectionViewMode === 'all' || selectionViewMode === 'selected') &&
            hasSelected &&
