/**
 * @kaizen_zone ddbc0bdc-0710-4e01-9472-8d1982a63a4e
 */
import { TKey, TSelectionViewMode } from 'Controls/interface';
import * as rk from 'i18n!Controls';
import { showType } from 'Controls/toolbars';

const SELECT_ITEM_WITH_COUNTER = {
    id: 'selectAll',
    command: 'selectAll',
    title: rk('Отметить все'),
};

const SELECT_ITEM_WITHOUT_COUNTER = {
    id: 'selectAll',
    command: 'selectAll',
    title: rk('Все'),
};

const SHOW_SELECTED_ITEM = {
    id: 'showSelected',
    command: 'selected',
    title: rk('Отобрать отмеченные'),
};

const SHOW_ALL_ITEM = {
    id: 'showAll',
    command: 'all',
    title: rk('Показать все'),
};

const SHOW_INVERT_ITEM = {
    id: 'toggleSelection',
    command: 'toggleAll',
    showType: showType.MENU_TOOLBAR,
    title: rk('Инвертировать'),
};

const SHOW_SELECT_COUNT = [
    {
        id: 'count-10',
        command: 'count-10',
        title: '10',
    },
    {
        id: 'count-25',
        command: 'count-25',
        title: '25',
    },
    {
        id: 'count-50',
        command: 'count-50',
        title: '50',
    },
    {
        id: 'count-100',
        command: 'count-100',
        title: '100',
    },
    {
        id: 'count-500',
        command: 'count-500',
        title: '500',
    },
    {
        id: 'count-1000',
        command: 'count-1000',
        title: '1000',
    },
];

const SHOW_SELECT_COUNT_SELECTED_ITEMS = [
    {
        id: 'count-10',
        command: 'count-10',
        title: '+10',
    },
    {
        id: 'count-25',
        command: 'count-25',
        title: '+25',
    },
    {
        id: 'count-50',
        command: 'count-50',
        title: '+50',
    },
    {
        id: 'count-100',
        command: 'count-100',
        title: '+100',
    },
    {
        id: 'count-500',
        command: 'count-500',
        title: '+500',
    },
    {
        id: 'count-1000',
        command: 'count-1000',
        title: '+1000',
    },
];
const UNSELECT_ALL_ITEM = {
    id: 'unselectAll',
    command: 'unselectAll',
    title: rk('Снять'),
};

interface IMultiSelectorMenuItem {
    id: string;
    title: string;
    readOnly?: boolean;
}

export function getAdditionalItems(
    selectionViewMode: TSelectionViewMode,
    isAllSelected: boolean,
    selectedKeys: TKey[] = [],
    selectedKeysCount: number | null,
    supportSelection?: boolean,
    task1192905045?: boolean
): IMultiSelectorMenuItem[] {
    const items = [];
    const hasSelected =
        !!selectedKeys.length && (selectedKeysCount > 0 || selectedKeysCount === null);
    const unselectAll = { ...UNSELECT_ALL_ITEM } as IMultiSelectorMenuItem;
    const invert = { ...SHOW_INVERT_ITEM } as IMultiSelectorMenuItem;
    const isItemsSelected = isAllSelected || Number(selectedKeysCount) !== 0;
    const selectItem = isItemsSelected ? SELECT_ITEM_WITH_COUNTER : SELECT_ITEM_WITHOUT_COUNTER;

    if (!hasSelected && !task1192905045) {
        unselectAll.readOnly = true;
        invert.readOnly = true;
    }

    if (selectionViewMode === 'selected') {
        items.push({ ...SHOW_ALL_ITEM }, SELECT_ITEM_WITH_COUNTER);
    } else {
        items.push(selectItem);
    }

    items.push(unselectAll);

    if (selectionViewMode !== 'partial') {
        items.push(invert);
    }

    if (supportSelection && hasSelected) {
        items.push({ ...SHOW_SELECTED_ITEM });
    } else if (!isAllSelected) {
        if ((selectionViewMode === 'all' || selectionViewMode === 'selected') && hasSelected) {
            items.push({ ...SHOW_SELECTED_ITEM });
        } else if (selectionViewMode === 'partial') {
            if (hasSelected) {
                items.push(...SHOW_SELECT_COUNT_SELECTED_ITEMS);
            } else {
                items.push(...SHOW_SELECT_COUNT);
            }
        }
    }

    return items;
}
