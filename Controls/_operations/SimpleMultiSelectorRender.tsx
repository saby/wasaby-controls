...
function getMenuCaption(
    ...
    const hasSelected = task1192905045
        ? !!showSelectedCount || showSelectedCount === null
        : !!selected.length;
    if (task1192905045) {
...
function SimpleMultiSelectorRender(
    ...
    return (
        <div
            className={`controls_operations_theme-${props.theme} ${
                props.className || ''
            } controls-operationsPanelV__container`}
        >
            <Button
                keyProperty={'id'}
                viewMode={'link'}
                ...
                className={`controls-MultiSelector__button ${props.attrs?.className}${
                    props.fontColorStyle === 'operationsPanel'
                        ? ' controls-operations__selectorButton-style'
                        : ''
                }`}
            />
            <LoadingIndicator
