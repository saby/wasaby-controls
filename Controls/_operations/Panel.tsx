...
import { IComponentProps, TKey } from 'Controls/interface';
 ...
 * В остальных случаях, чтобы связать панель операций со списком, используйте {@link Controls-ListEnv/operationsPanelConnected:View}.
 *
 * @remark
 * Полезные ссылки:
...
export interface IOperationsPanelProps extends IComponentProps, TJsxProps {
    source?: Memory;
    items?: RecordSet;
...
function OperationsPanel(
    ...
    const errorCallback = useCallback((_, error: Error) => {
        setHasPanel(false);
    ...
    return hasPanel ? (
        <Async
            ...
            onOperationsPanelOpen={props.onOperationsPanelOpen}
        />
    ) : null;
}
 ...
 * @cfg {SelectionViewMode} Задает отображение кнопки "Отобрать отмеченные" в меню мультивыбора.
 ...
 * <pre class="brush: js">
 * // JavaScript
 * class MyControl extends Control<IComponentProps> {
...
/**
 * @name Controls/_operations/Panel#closeButtonVisible
 * @cfg {Boolean} Определяет видимость кнопки закрытия.
 * @default true
 */

/**
 * @name Controls/_operations/Panel#items
 * @cfg {RecordSet.<Controls/toolbars:IToolBarItem>} Определяет набор записей по которым строится меню тулбара.
 */
