...
import { TCaptionPosition } from 'Controls/_propertyGridBase/IPropertyGrid';
import { loadSync, isLoaded } from 'WasabyLoader/ModulesLoader';
import type { IEditorProps } from 'Controls/propertyGridEditors';

export function getPropertyGridCollectionItem<T extends Model>(
    CollectionItemClass: typeof CollectionItem,
    libraryName: string
) {
    class PropertyGridCollectionItem extends CollectionItemClass<T> {
        ...
        isNode(): boolean {
            // Используется метод, который есть только в TreeItem.
            return super.isNode?.() || false;
        }
        ...
        getEditorTemplateName(): string {
            ...
            if (getValueType(propertyValue) === 'object') {
            ...
            return DEFAULT_EDITORS[
                valueType === 'object' && propertyValue !== null
        ...
        getItemPaddingClasses(
            ...
            let classes =
                `controls-PropertyGrid__editor_spacingTop_${owner.getTopPadding()}` +
        ...
        isEditable(): boolean {
            return (
                this.contents.get('isEditable') && !!(this.getEditorTemplateName() || this.isNode())
            );
        }
        ...
        getEditorReadOnly(templateReadOnly: boolean): boolean {
            ...
            return editorOptions.hasOwnProperty('readOnly')
        ...
        static getEditorOptions(item: Model): IEditorProps<unknown> {
            ...
            if (editorOptionsName && !item.get('__editorOptionsResolved')) {
    ...
    Object.assign(PropertyGridCollectionItem.prototype, {
        [`[Controls/_${libraryName}/PropertyGridCollectionItem]`]: true,
        _moduleName: `Controls/${libraryName}:PropertyGridCollectionItem`,
    });

    register(`Controls/${libraryName}:PropertyGridCollectionItem`, PropertyGridCollectionItem, {
        instantiate: false,
    });
    return PropertyGridCollectionItem;
}
