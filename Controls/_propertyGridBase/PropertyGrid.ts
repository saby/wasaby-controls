...
import * as template from 'wml!Controls/_propertyGridBase/PropertyGrid';
...
import { format, Model, Record as entityRecord } from 'Types/entity';
import { factory } from 'Types/chain';
import { object } from 'Types/util';
import { default as renderTemplate } from 'Controls/_propertyGridBase/Render';
import { isLoaded, loadAsync, loadSync } from 'WasabyLoader/ModulesLoader';
import {
    IPropertyGridOptions,
    TCollapsedGroupsElement,
    TEditingObject,
} from 'Controls/_propertyGridBase/IPropertyGrid';
import { AtomicRemove as RemoveViewCommand, Move as MoveViewCommand } from 'Controls/viewCommands';
...
import PropertyGridCollection, {
    TPropertyGridCollection,
    TPropertyGridCollectionConstructor,
} from './PropertyGridCollection';
import { TPropertyGridCollectionItem as TCollectionItem } from 'Controls/_propertyGridBase/PropertyGridCollectionItem';
import {
...
import { default as getValidateContainerPG } from './ValidateContainer';
import { scrollToElement, TScrollPosition } from 'Controls/scroll';
import ArraySimpleValuesUtil = require('Controls/Utils/ArraySimpleValuesUtil');
import { nodeHistoryUtil } from 'Controls/dataSource';
...
// type TPropertyGridCollection = Collection<Model, CollectionItem<Model>>;
...
export default class PropertyGridView extends Control<IPropertyGridOptions> {
    ...
    protected _render: TemplateFunction;
    protected _groupTemplate: TemplateFunction;
    protected _toggleEditorsTemplate: TemplateFunction;
    ...
    protected _beforeMount(options: IPropertyGridOptions): void {
        ...
        this._render = this.getRenderTemplate(options);
    ...
    protected _beforeUpdate(newOptions: IPropertyGridOptions): void {
        ...
        if (typeDescription !== this._options.typeDescription) {
            ...
            if (collapsedGroups === this._options.collapsedGroups && !collapsedGroups?.length) {
        ...
        this._limitMode = this._isLimitMode(newOptions);

        if (this._limitMode || this._getHasExpander(newOptions)) {
            this._createValidateContainer();
        }
    ...
    protected _afterMount(): void {
        ...
        if (this._limitMode || this._getHasExpander(this._options)) {
            this._createValidateContainer();
        }
    }

    protected _createValidateContainer(): void {
        if (!this._validateContainer) {
            getValidateContainerPG().then((PGValidateContainer) => {
                // За время работы асинхронного кода (require)
                // propertyGrid мог быть разрушен
                if (!this._destroyed && !this._validateContainer) {
    ...
    protected getRenderTemplate(_options: IPropertyGridOptions): TemplateFunction {
    ...
    private _getCollection(options: IPropertyGridOptions): TPropertyGridCollection {
        ...
        const PropertyGridCollectionConstructor = this.getPropertyGridCollectionConstructor();
        return new PropertyGridCollectionConstructor({
            ...
            filter: this._displayFilter.bind(this, options.expanderVisibility),
            ...
            expandedItems: this._getModelExpandedItems(options),
            collapsedGroups: this._getCollapsedGroupsAsArray(),
            unique: true,
        });
    }

    private _getModelExpandedItems(options: IPropertyGridOptions): TKey[] | undefined {
        if (this._listModel && this._listModel.getExpandedItems) {
            return this._listModel.getExpandedItems();
        }
    ...
    private _itemIsVisible<T extends Model>(item: TreeItem<T>): boolean {
        ...
        return this._itemIsVisible(parent);
    }
    ...
    private _displayFilter(
        ...
        if (expanderVisibility !== 'hidden' && item?.['[Controls/_display/TreeItem]']) {
            return this._itemIsVisible(item);
        }
        ...
        if (editingObject instanceof entityRecord) {
            ...
            if (!resultEditingObject.has(name)) {
                ...
                resultEditingObject.addField({
                    ...
                    defaultValue: value,
                    kind:
                        propertyFormat.getType() === 'array'
                            ? (propertyFormat as format.ArrayField).getKind()
                            : undefined,
                });
            }
            ...
            resultEditingObject = clone(editingObject);
    ...
    protected _expanderClick(
        event: SyntheticEvent<Event>,
        displayItem: TreeItem<Model> | PropertyGridCollectionItem<Model>
    ): void {
        const expandedItems = this._getExpandedItemsAsArray(displayItem);
        this._listModel.setExpandedItems(this._getExpandedItemsAsArray(displayItem));
        if (this._options.propStorageId) {
            nodeHistoryUtil.store(expandedItems, this._options.propStorageId);
        }
    }
    ...
    private _getAllExpandedItems(): TKey[] {
        const currentExpandedItems: TKey[] = [];
        this._listModel.getItems().forEach((listItem) => {
            if (listItem && listItem['[Controls/_display/TreeItem]']) {
                currentExpandedItems.push(listItem.getContents().getKey());
            }
        });
        return currentExpandedItems;
    }

    private _getExpandedItems(): TKey[] {
        let currentExpandedItems = [...this._listModel.getExpandedItems()];
        if (currentExpandedItems[0] === null) {
            currentExpandedItems = this._getAllExpandedItems();
        }
        return currentExpandedItems;
    }

    private _getExpandedItemsAsArray(item: PropertyGridCollectionItem<Model>): TKey[] {
        let currentExpandedItems = this._getExpandedItems();
        const itemKey = item.getContents().getKey();
        if (item.isExpanded()) {
            currentExpandedItems = currentExpandedItems.filter(
                (expandedItem) => expandedItem !== itemKey
            );
        } else {
            currentExpandedItems.push(itemKey);
        }
        return currentExpandedItems;
    }
    ...
    protected _toggleEditor(event: SyntheticEvent, item: Model, value: boolean): void {
        ...
        this._listModel.setFilter(this._displayFilter.bind(this, this._options.expanderVisibility));
    }
    ...
    private _getItemActionsController(): ItemActionsController {
        if (!this._itemActionsController) {
    ...
    private _updateItemActions(
        ...
        itemActionsController.updateActions(editingItem);
    }
    ...
    /**
     * Прокрутка до элемента проперти грида
     * @return {Promise<void>}
     * @public
     */
    async scrollToItem(key: string, position?: TScrollPosition, force?: boolean): Promise<void> {
        const element = this._container.querySelector(
            `[item-key=property-grid-item-${key}]`
        ) as HTMLElement;
        if (element) {
            return scrollToElement(element, position, force);
        }
        Logger.error("На найден элемент с item-key='" + `property-grid-item-${key}'`);
    }
    ...
    setFocusedEditor(editorName: string | null): void {
        this._listModel.setFocusedEditor(editorName);
    }

    getFocusedEditor(): string | null {
        return this._listModel.getFocusedEditor();
    }

    // region limit
    private _isLimitMode({ limit }: IPropertyGridOptions): boolean {
        return !!limit && limit < this._listModel.getCollection().getCount();
    }

    private _getHasExpander({ expanderVisibility }: IPropertyGridOptions): boolean {
        const hasTreeItems = this._listModel
            .getItems()
            .some((item) => item.isNode && item.isNode());
        return expanderVisibility !== 'hidden' && hasTreeItems;
    }
    ...
    private async _validate(): Promise<string[] | null> {
        ...
        if (!this._listModel.hasLimit() && this._options.expanderVisibility === 'hidden') {
            return;
        }
        ...
        while (collectionIterator.moveNext()) {
            ...
            if (validators && validators.length) {
                ...
                if (Array.isArray(errors) && errors.length > 0) {
                    this._expandEditors();
                    if (this._options.expanderVisibility !== 'hidden') {
                        this._setExpandedItemsToValidate(item.get(keyProperty));
                    }
    ...
    protected _setExpandedItemsToValidate(): void {}
