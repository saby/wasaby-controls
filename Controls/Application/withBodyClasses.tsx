...
export default function withBodyClasses<T extends IApplicationProps = IApplicationProps>(
    ...
    return class extends Component<T> {
        ...
        private BodyAPI: PageBody = PageBody.getInstance();

        constructor(props: T, context: { isScrollOnBody: boolean }) {
            super(props);
            this._createDragnDropController();
            this._initIsAdaptiveClass(props, context?.isScrollOnBody);

            this._createTouchDetector();
            this._updateTouchClassesState();
            const [classesToDelete, classesToAdd] = this._getInitialClasses();
            this._updateClasses(classesToDelete, classesToAdd);
        }
        ...
        private _setDragType(
            ...
            } else if (cursorType === 'diagonalUpRightResize') {
                this._bodyClassesState.diagonalUpRightResize = true;
            } else if (cursorType === 'diagonalUpLeftResize') {
                this._bodyClassesState.diagonalUpLeftResize = true;
            } else if (cursorType === 'verticalHorizontalResize') {
                this._bodyClassesState.verticalHorizontalResize = true;
            } else if (cursorType === 'horizontalVerticalResize') {
                this._bodyClassesState.horizontalVerticalResize = true;
            }
        }
        ...
        private _dragEndHandler(
            ...
            this._bodyClassesState.diagonalUpRightResize = false;
            this._bodyClassesState.diagonalUpLeftResize = false;
            this._bodyClassesState.verticalHorizontalResize = false;
            this._bodyClassesState.horizontalVerticalResize = false;
        ...
        private _isHover(): boolean {
            return (
                ...
                !this._bodyClassesState.diagonalUpRightResize &&
                !this._bodyClassesState.diagonalUpLeftResize &&
                !this._bodyClassesState.verticalHorizontalResize &&
                !this._bodyClassesState.horizontalVerticalResize
            );
        }
        ...
        private _updateScrollingClass(): void {
            ...
            const [classesToDelete, classesToAdd] = this._getUpdateClasses({
                scrollingClass,
            });
            this._updateClasses(classesToDelete, classesToAdd);
        }
         ...
         * @private
         */
        private _updateClasses(classesToDelete: string[] = [], classesToAdd: string[] = []): void {
            ...
            if (classesToAdd.length || classesToDelete.length) {
                this.BodyAPI.replaceClasses(classesToDelete, classesToAdd);
            }
        }

        /** Задаем классы для body, которые не будут меняться */
        private _getInitialClasses(): [classesToDelete: string[], classesToAdd: string[]] {
            ...
            // todo не знаю зачем
            if (detection.isPhone && this.props.bodyClass?.includes('ws-is-sabypage')) {
                classesToAdd.push('unaccented-adaptive-background');
            }
            // выставляем отступы для безопасных зон
            // https://css-live.ru/articles/ponimanie-vyuporta-webview-v-ios-11.html
            if (detection.isMobileIOS && this.props.bodyClass?.includes('ws-is-sabypage')) {
                classesToAdd.push('fixIosPaddings');
            }
            // todo чтобы шрифт интер перебить на телефонах, точно не знаю зачем
            if (detection.isPhone) {
                classesToAdd.push('familyFontsForPhones');
            }
        ...
        private _getUpdateClasses(
            toUpdate: Record<string, string> & Partial<IBodyClassesField>
        ): [classesToDelete: string[], classesToAdd: string[]] {
            let classesToDelete: string[] = [];
            let classesToAdd: string[] = [];

            for (const key in toUpdate) {
                if (toUpdate.hasOwnProperty(key)) {
                    if (toUpdate[key] === this._bodyClasses[key]) {
                        continue;
                    }
                    classesToAdd = classesToAdd.concat(toUpdate[key].split(' ').filter(Boolean));
                    classesToDelete = classesToDelete.concat(
                        this._bodyClasses[key].split(' ').filter(Boolean)
                    );
                    this._bodyClasses[key] = toUpdate[key] || '';
                }
            }
            ...
            return [classesToDelete, classesToAdd];
        }

        private _updateTouchClassesState(): void {
