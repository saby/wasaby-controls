...
class ApplicationReact extends Component<IApplicationProps> {
    ...
    render() {
        ...
        return (
            ...
            >
                {/* тут вставляем div, т.к. EventSubscriber не знает что такое FocusRoot,
                    значит будем работать по логике для реакта, в таком случае события как пропсы не передаются,
                    а сохраняются в контексте
                    надо разобраться правильно ли так делать,
                    или EventSubscriber должен про реализацию FocusRoot знать */}
                <div
                    className={detection.isIE ? 'Application-contents-ie-fix' : 'tw-contents'}
                ...
                >
                    {/* TODO: удалить onActivated после исправленяи системы событий */}
                    <FocusRoot
                        as="div"
