export interface IPopupSettingsController {
    getSettings: (ids: string[]) => Record<string, any>;
    getSettingsSync?: (ids: string[]) => Record<string, any>;
    setSettings: (config: Record<string, any>, viewSettings?: any) => void;
}

let settingsController: IPopupSettingsController;
...
export function getSettings(ids: string[]): Promise<void | Record<string, any>> {
...
export function getSettingsSync(ids: string[]): void | Record<string, any> {
    if (settingsController && settingsController.getSettingsSync) {
        return settingsController.getSettingsSync(ids);
    }
}

export function setSettings(config: Record<string, any>, viewSettings?: any): void {
    if (settingsController && settingsController.setSettings) {
        settingsController.setSettings(config, viewSettings);
    }
}
...
export async function loadSavedConfig<K extends string[] = string[]>(
    ...
    return new Promise((resolve) => {
        getSettings([propStorageId]).then((storage) => {
            let loadedCfg: Record<string, any> = {};
