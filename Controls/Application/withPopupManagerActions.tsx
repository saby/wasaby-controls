import { Controller, IPopupItem, InfoBoxContextProvider } from 'Controls/popup';
...
export default function withPopupManagerActions<T extends IApplicationProps = IApplicationProps>(
    ...
    return class extends Component<T> {
        ...
        private onOpenInfoBox: (event: Event, config: unknown, withDelay?: boolean) => void =
            mergeHandlers(this._openInfoBoxHandler.bind(this), this.props.openInfoBox, true);

        private onCloseInfoBox: (event: Event, withDelay?: boolean) => void = mergeHandlers(
            this._closeInfoBoxHandler.bind(this),
            this.props.closeInfoBox,
            true
        );

        private onForceCloseInfoBox: (_e: Event) => void = mergeHandlers(
        ...
        render() {
            return (
                <InfoBoxContextProvider
                    onOpenInfoBox={this.onOpenInfoBox}
                    onCloseInfoBox={this.onCloseInfoBox}
                    onForceCloseInfoBox={this.onForceCloseInfoBox}
                >
                    <WrappedComponent
                        {...this.props}
                        ...
                        onTouchEnd={mergeHandlers(
                            this._touchEndPage.bind(this),
                            this.props.onTouchEnd
                        )}
                        /* кастомные обработчики */
                        openInfoBox={this.onOpenInfoBox}
                        closeInfoBox={this.onCloseInfoBox}
                        forceCloseInfoBox={this.onForceCloseInfoBox}
