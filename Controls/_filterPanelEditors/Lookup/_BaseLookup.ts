...
import { IBaseEditor, IFilterEditorHistoryProps } from 'Controls/filterPanel';
...
import { initItems, getRecordSetOptions } from 'Controls/_filterPanelEditors/Utils/InitItems';
...
class LookupEditor extends Control<ILookupEditorOptions> implements ILookupEditor {
    ...
    private _handleItemsChanged(selectedItems: RecordSet | List<Model>): void {
        ...
        this._notify('itemsChanged', [selectedItems]);
    }
    ...
    protected _selectCallback(event: Event, selectedItems: RecordSet | List<Model>): void {
        const deps: [
            Promise<typeof import('Controls/lookup')>,
            Promise<typeof import('Controls/HistoryStore')>?,
            Promise<Function>?,
        ] = [loadAsync('Controls/lookup')];
        if (this._options.historyId) {
            deps[1] = loadAsync('Controls/HistoryStore');
            if (typeof this._options.historySaveCallback === 'string') {
                deps[2] = loadAsync(this._options.historySaveCallback);
            }
        }
        Promise.all(deps).then(([lookup, historyStore, historySaveCallback]) => {
    ...
    protected _extendedCaptionClickHandler(event: SyntheticEvent): void {
        loadAsync<typeof import('Controls/lookup')>('Controls/lookup')
            .then(({ showSelector }) => {
