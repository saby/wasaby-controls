...
class DateRangeEditor extends Control<IDateRangeOptions> {
    ...
    protected _openDatePopup(): void {
        ...
        loadAsync<typeof import('Controls/date')>('Controls/date')
            .then(({ getDatePopupName, getPopupClassName }) => {
                const className = `controls-FilterViewPanel__dateRangeEditor_popup
                                   controls_filterPanel_theme-${this._options.theme} 
                                   ${getPopupClassName(this._options, this._options.theme)}`;
    ...
    private _formatDateRangeCaption(
        _date: Date,
        showCurrentYear: boolean,
        startValue?: Date,
        endValue?: Date,
        ...
        return getFormattedDateRangeCaption(
            startValue,
            endValue,
            emptyCaption,
            _date,
    ...
    private _getTextValue(startValue: Date, endValue: Date): string {
        const captionFormatter = this._options.captionFormatter || getFormattedDateRangeCaption;
    ...
    protected _beforeMount(options: IDateRangeOptions): void {
        this._captionFormatter = this._formatDateRangeCaption.bind(
            this,
            options._date,
            options.showCurrentYear
        );
        this._fittingMode =
            options.datePopupType === 'shortDatePicker' ? 'adaptiveOverflow' : 'overflow';
    }
