...
import { _Controller, getTextSelector } from 'Controls/dropdown';
...
function prepareResultFromMenu(
    result: Model[],
    { keyProperty, displayProperty, parentProperty, items, root, nodeProperty },
    ...
    const selectedKeys = [];
    const text = getTextSelector.getFullTexts(
        result,
        { items, root, parentProperty, nodeProperty, displayProperty },
        controller
    );
    result.forEach((item) => {
...
function handleMenuItemsActivate(data: Model[], props, controller: _Controller) {
    const result = prepareResultFromMenu(data, props, controller);
...
function getDropdownController(props, targetRef): Promise<_Controller> {
    ...
    return loadAsync('Controls/dropdown').then(({ _Controller }) => {
        const isReseted = isEqual(props.propertyValue, props.resetValue);
        let selectedKeys = [props.propertyValue];
        if (props.propertyValue instanceof Array) {
            selectedKeys = props.propertyValue;
        } else if (isReseted) {
            selectedKeys = props.emptyText ? [props.emptyKey || null] : [];
        }
...
function openMenu(ddlController: _Controller, props, popupOptions?: IStickyPopupOptions): void {
        ...
        onResult: (action, data) => {
            switch (action) {
                case 'itemClick':
                    ...
                    handleMenuItemsActivate([item], props, ddlController);
                    ddlController.updateHistoryAndCloseMenu(item);
                    break;
                case 'applyClick':
                    handleMenuItemsActivate(data.items, props, ddlController);
    ...
    ddlController.openMenu({ ...popupOptions, eventHandlers }).then((items) => {
        if (items) {
            handleMenuItemsActivate(items, props, ddlController);
        }
    });
}
...
export default React.memo(
    React.forwardRef((props, ref) => {
        ...
        }, [props.filter, props.source, props.items]);
