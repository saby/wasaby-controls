...
function Dropdown(props) {
    ...
    if (props.items && props.selectedKeys) {
        const hasAllSelectedItems = hasAllSelectedKeysInItems(
            props.items,
            props.selectedKeys,
            props
        );
    ...
    } else if (props.selectedItems && props.selectedKeys) {
        selectedItems = hasAllSelectedKeysInItems(props.selectedItems, props.selectedKeys, props)
            ? props.selectedItems
            : null;
    }
    ...
    const hasHistory = React.useMemo(() => {
        return props.historyId || (props.source && !!props.source['[Controls/_historyOld/Source]']);
...
function hasAllSelectedKeysInItems(
    items: RecordSet,
    ...
    return selectedKeys.every((key) => {
        return (
            items.getRecordById(key) ||
            (props.emptyText && (key === props.emptyKey || (!key && !props.emptyKey)))
        );
    });
}
