...
const ItemTemplate = React.forwardRef((props: IItemProps, ref): React.ReactElement => {
    ...
    return (
        <>
            ...
            <div
                onClick={(event) => {
                    const text = props.originText || props.text;
                    notifyPropertyValueChanged(event, props.id, text);
                }}
    ...
    (props: IContentTemplate & { frequentItems: IItemProps[] }, ref): React.ReactElement => {
        if (props.frequentItems.length) {
            ...
            props.frequentItems.map(({ title, id }, index) => {
                const originText = props.items?.getRecordById(id)?.get(props.displayProperty);
                templates.push(
                    <ItemTemplate
                        text={title}
