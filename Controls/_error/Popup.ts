...
import { ErrorType, ErrorViewConfig } from 'ErrorHandling/interface';
...
function isSingleWindowError(type?: ErrorType): boolean {
    return [ErrorType.internal, ErrorType.connection, ErrorType.require].includes(type);
}
...
export default class Popup implements IPopupHelper {
    ...
    clearPopupId(fn?: Function, reslult?: unknown) {
        Popup._id = null;
    ...
    openDialog<T extends IViewConfigMessage>(
        ...
        // Если уже открыт диалог с ошибкой типа internal,
        // то новый открывать не надо
        if (Popup._id) {
            return Promise.resolve();
        }

        if (isSingleWindowError(config.type) && !config.allowMultiplyDialog) {
            this.setDialogId('singleErrorDialog');
        }
            ...
            return this.openConfirmation(confirmationOptions)
                .then((result) => {
                    ...
                    if (eventHandlers && typeof eventHandlers.onClose === 'function') {
                        this.clearPopupId(eventHandlers.onClose, result);
                    }
                })
                .catch(() => {
                    this.clearPopupId();
        ...
        return Promise.all([this.preloadPopup(), preloadTemplate])
            .then(([popup, popupTemplate]) => {
                ...
                const { eventHandlers } = dialogOptions;
                if (eventHandlers && typeof eventHandlers.onClose === 'function') {
                    const closeHandler = eventHandlers.onClose;
                    eventHandlers.onClose = this.clearPopupId.bind(this, closeHandler);
                } else if (eventHandlers) {
                    eventHandlers.onClose = this.clearPopupId;
                } else {
                    dialogOptions.eventHandlers = { onClose: this.clearPopupId };
                }
                return popup.Dialog.openPopup({
                    template: popupTemplate,
                    modal: true,
                    // Окно с ошибкой должно центрироваться
