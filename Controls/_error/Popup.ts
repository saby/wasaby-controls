...
export default class Popup implements IPopupHelper {
    ...
    preloadPopup(): Promise<IPopupModule | void> {
        if (!this.preloadPromise) {
            this.preloadPromise = Popup.preload(this.modules, this.themes).then(
                ([popup]) => {
                    return popup;
                }
            );
    ...
    openDialog<T extends IViewConfigMessage>(
        ...
        if (typeof template === 'undefined') {
            ...
            return this.openConfirmation(confirmationOptions).then((result) => {
                if (
                    eventHandlers &&
                    typeof eventHandlers.onResult === 'function'
                ) {
                ...
                if (
                    eventHandlers &&
                    typeof eventHandlers.onClose === 'function'
                ) {
        ...
        const preloadTemplate: Promise<void | TemplateFunction> =
            typeof template === 'string'
                ? (
                      loadAsync(template) as Promise<void | TemplateFunction>
                  ).catch(() => {
                      return undefined;
                  })
