...
type ForbiddenPSError = Error & { isForbiddenRPCJSON: boolean };
...
export default function process(options: IProcessOptions): Promise<PopupId | void> {
    ...
    return controller.process(error).then((viewConfig) => {
        ...
        if (constants.isServerSide) {
            /*
             * Ошибка RPC, которая генерируется в результате вызова БЛ на сервисе представления.
             * Вызывать БЛ на сервисе представления нельзя.
             */
            if ((error as ForbiddenPSError).isForbiddenRPCJSON) {
...
function logServerSideError(
    ...
    const method = (error as ForbiddenPSError).isForbiddenRPCJSON ? 'warn' : 'error';
