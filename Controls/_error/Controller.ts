...
export interface IProcessConfig<TError extends ProcessedError = ProcessedError> {
    ...
    /**
     * Допустимо ли показывать нескоько одинаковых ошибок (для типа intermal)
     */
    allowMultiplyDialog?: boolean;
}
...
export default class ErrorController {
    ...
    private createViewConfig<TError extends ProcessedError>(
        ...
        const viewConfig =
            handlerResult ||
            (error.cause?.needShow ? ErrorController.getDefaultViewConfig(error) : null);
        if (!viewConfig) {
            if (isServerSide && 'isForbiddenRPCJSON' in error && error.isForbiddenRPCJSON) {
        ...
        if (!(config instanceof Error) && config.allowMultiplyDialog) {
            viewConfig.allowMultiplyDialog = config.allowMultiplyDialog;
        }
     ...
     * Чтобы показать ошибку созданную на клиенте, необходимо в конструкторе ошибки указать второй параметр { cause: { needShow: true }
     * * @link {https://online.sbis.ru/news/233d9f7c-18bf-4d44-95df-b589611031cb} подробнее
    ...
    process<TError extends ProcessedError = ProcessedError>(
        ...
        const handlers = [...this.handlers, ...getHandlers(), ...this.postHandlers];

        const handlerResult = this.handlerIterator.getViewConfigAsync(handlers, handlerConfig);
    ...
    processSync<TError extends ProcessedError = ProcessedError>(
        ...
        const handlerResult = this.handlerIterator.getViewConfigSync(handlers, handlerConfig);
