...
export class Container extends React.Component<IAnimationContainerProps, IAnimationContainerState> {
    ...
    startAnimation(params: IStartAnimationParams): Promise<void> {
        if (this.validateAnimationParams(params)) {
            const fromLocation = this._users[params.fromId].getLocation(params.fromPosition);
            this.setState({
                animatedEntity: params.entity,
                fromLocation,
                toLocation: this._users[params.toId].getLocation(params.toPosition),
    ...
    validateAnimationParams(params: IStartAnimationParams): boolean {
        let valid = true;
        if (!params.entity) {
            Logger.error('Controls/animation:Container: Не передана сущность для анимации');
            valid = false;
        }
        if (
