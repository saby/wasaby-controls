...
class Controller extends BaseController {
    ...
    private _onTouchMove = (e) => {
        // Если внутри шторки нет скрола, или есть скролл, но скролить нечего, то останавливаем событие touchMove
        const scrollContainer = e.target.closest('.controls-Scroll-ContainerBase');
        if (
            !scrollContainer ||
            (scrollContainer.scrollHeight === scrollContainer.clientHeight &&
                scrollContainer.scrollWidth === scrollContainer.clientWidth &&
                (e.target as HTMLElement)?.closest?.('.controls-SlidingPanel__popup'))
        ) {
            e.preventDefault();
        }
    };
    ...
    elementCreated(item: ISlidingPanelItem, container: HTMLDivElement): boolean {
        ...
        if (item.popupOptions.animation === false) {
        ...
        if (!this._hasOpenedPopups()) {
            ...
            if (!item.isOverflowHidden) {
                this._toggleCancelBodyDragging(true, item.modal);
            }
        }
    ...
    private _updatePosition(
         ...
         */
        if (
            (resizeType !== ResizeType.orientationChange || !detection.isMobileSafari) &&
    ...
    beforeUpdateOptions(item: ISlidingPanelItem): void {
        // sliding must update on elementAfterUpdatePhase
    }

    afterUpdateOptions(item: ISlidingPanelItem): void {
        // sliding must update on elementAfterUpdatePhase
    }
    /**
    ...
    _fixIosBug(item: ISlidingPanelItem, container: HTMLDivElement): void {
        ...
        const bodyHeight = Math.min(window.innerHeight, document.body.clientHeight);
        ...
        if (toggle) {
            ...
            if (viewportHeight === item.position.maxHeight) {
                ...
                item.position.maxHeight = '100dvh';
            } else {
            ...
            // Подскролл нужен только в том случае, если активный элемент это поле ввода
            // https://online.sbis.ru/opendoc.html?guid=c47fc224-b12e-4a61-bcb1-4970ced0c7b0
            if (document.activeElement && this._isKeyboardVisible() && detection.isMobileIOS) {
                setTimeout(() => {
    ...
    cancelElementDestroyed(item: IPopupItem): void {
    ...
    closePopupByOutsideClick(item: IPopupItem): void {
        if (item.popupOptions?.closeOnOverlayClick !== false && item.animationState !== 'showing') {
    ...
    resizeInner(item: ISlidingPanelItem, container: HTMLDivElement): boolean {
        ...
        if (item.animationState === 'showing') {
            ...
            if (item.position.height !== height) {
                ...
                if (containerStyle.maxHeight === containerStyle.height) {
    ...
    protected _isAnimatedClass(item: ISlidingPanelItem): boolean {
        // отключаем анимацию при открытии если явно отключили анимацию, либо была открыта клавиатура
        return item.popupOptions.animation !== false && !this._isKeyboardVisible();
    }

    elementUpdateOptions(item: ISlidingPanelItem, container: HTMLElement): boolean {
        super.elementUpdateOptions(item, container);
        if (
            this._isAnimatedClass(item) &&
            !item.popupOptions?.className?.includes(ANIMATION_CLASS)
        ) {
    ...
    protected _initItemAnimation(item: ISlidingPanelItem): void {
        item.popupOptions.className = `${
            ...
            ${
                this._isAnimatedClass(item) ? ANIMATION_CLASS : ''
            } controls_popupSliding_theme-${PopupController.getTheme()}`;
        item.animationState = AnimationState.initializing;
        if (item.popupState !== item.controller.POPUP_STATE_INITIALIZING) {
            item.popupOptions.className += ' controls-Popup_shown';
        }
        if (detection.isMobileIOS && detection.IOSVersion <= 15) {
            item.popupOptions.className += ' controls-SlidingPanel__popup_transform';
        }
    }
    ...
    popupDragStart(item: ISlidingPanelItem, container: HTMLDivElement, offset: IDragOffset): void {
        ...
        if (isFirstDrag) {
            item.dragStartHeight = Math.max(this._getHeight(item), container.clientHeight || 0);
        }
    ...
    popupDragEnd(item: ISlidingPanelItem): void {
        if (!this._isKeyboardVisible()) {
            ...
            if (
                ...
                PopupController.getController()?.remove(item.id);
            } else {
    ...
    private _getPopupTemplatePosition(
        ...
        return {
            ...
            isMobileMode: true,
            shouldSwipeOnContent: slidingPanelOptions.shouldSwipeOnContent,
        };
    }
    ...
    protected _getRestrictiveContainerCoords(item: ISlidingPanelItem): IPopupPosition | void {
        ...
        return BaseController.getCoordsByContainer(restrictiveContainer, item);
    }
    ...
    protected _updatePopupSizes(item: ISlidingPanelItem, container: HTMLDivElement): void {
        ...
        item.sizes.width = Math.min(item.sizes.width, SlidingPanelStrategy.getWindowWidth());
        item.sizes.height = Math.min(item.sizes.height, SlidingPanelStrategy.getWindowHeight());
    }
     ...
     * @private
     */
    private _toggleCancelBodyDragging(state: boolean, isModal?: boolean): void {
        if (constants.isBrowserPlatform) {
            if (state) {
                if (isModal) {
                    document.documentElement.addEventListener('touchmove', this._onTouchMove, {
                        passive: false,
                    });
                }
            } else {
                document.documentElement.removeEventListener('touchmove', this._onTouchMove);
            }
