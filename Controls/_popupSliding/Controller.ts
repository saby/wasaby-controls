...
class Controller extends BaseController {
    ...
    private _updatePosition(
        ...
        if (
            resizeType === ResizeType.outer ||
            resizeType === ResizeType.orientationChange
        ) {
        ...
        if (!item.popupOptions.slidingPanelData) {
            item.popupOptions.slidingPanelData =
                this._getPopupTemplatePosition(item);
        }
    ...
    popupDragStart(
        ...
        if (
            item.popupOptions.slidingPanelOptions.userMoveLocked ||
        ...
        const { slidingPanelOptions: { position: positionOption } = {} } =
            item.popupOptions;
    ...
    popupDragEnd(item: ISlidingPanelItem): void {
        if (!this._isKeyboardVisible()) {
            ...
            const finishHeight = this._isVerticalDrag(item.dragOffset, item)
                ...
                : item.dragStartHeight;
            item.position.height = finishHeight;

            if (
                finishHeight <
                SlidingPanelStrategy.getMinHeight(
                    item,
                    restrictiveContainerCoords
                )
            ) {
    ...
    private _getPopupTemplatePosition(
        ...
        return {
            minHeight: SlidingPanelStrategy.getMinHeight(
                item,
                restrictiveContainerCoords
            ),
            maxHeight: SlidingPanelStrategy.getMaxHeight(
                item,
                restrictiveContainerCoords
            ),
    ...
    protected _getRestrictiveContainerCoords(
        item: ISlidingPanelItem
    ): IPopupPosition | void {
        const restrictiveContainer =
            item.popupOptions.slidingPanelOptions.restrictiveContainer ||
            'body';
     ...
     * @private
     */
    private _isVerticalDrag(
        offset: IDragOffset,
        item: ISlidingPanelItem
    ): boolean {
        if (
            item.popupOptions?.slidingPanelOptions?.autoHeight &&
            item.sizes?.width
        ) {
