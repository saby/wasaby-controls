 ...
 * @private
 */
class SlidingPanel extends Control<ISlidingPanelTemplateOptions> {
    ...
    protected _beforeMount(options: ISlidingPanelTemplateOptions): void {
        ...
        this._shadowVisible = this._isShadowVisible(options);
    }
    ...
    protected _afterRender() {
        this._shadowVisible = this._isShadowVisible(this._options);
    }
    ...
    protected _getContentStyle(): string {
        const { maxHeight, height } = this._getSlidingSize(this._options);
        return this._scrollAvailable &&
            !(this._options.footerContentTemplate && !(height > maxHeight || height === maxHeight))
            ? ''
            : 'height: auto';
    }
    ...
    protected _getSlidingSize(options: ISlidingPanelTemplateOptions): {
        maxHeight: number;
        height: number;
    } {
        ...
        if (!maxHeight) {
            maxHeight = window?.outerHeight || document?.body?.clientHeight;
        }
        return {
            maxHeight,
            height,
    ...
    protected _isScrollAvailable(options: ISlidingPanelTemplateOptions): boolean {
        const { maxHeight, height } = this._getSlidingSize(options);
    ...
    protected _isShadowVisible(options: ISlidingPanelTemplateOptions): boolean {
        const contentHeight = this._getHeight(options);
    ...
    protected _touchMoveHandler(event: SyntheticEvent<TouchEvent>): void {
        ...
        if (
            ...
            !(this._options.slidingPanelOptions.shouldSwipeOnContent ?? true)
        ) {
    ...
    private _isSwipeForScroll(event: SyntheticEvent<TouchEvent>): boolean {
        ...
        let alreadyScrolled = this._getScrollTop() !== 0;
        if (scrollContainers.length === 1) {
            alreadyScrolled = scrollContainers[0].children?.[0]?.scrollTop !== 0;
        }
        if (
            ...
            if (!alreadyScrolled) {
                if (this._options?.slidingPanelOptions?.position === 'top') {
                    swipeInsideInnerScrollContainer =
                        this._startTouchPosition.y - event.nativeEvent.changedTouches[0].clientY <
                        0;
                } else {
                    swipeInsideInnerScrollContainer =
                        this._startTouchPosition.y - event.nativeEvent.changedTouches[0].clientY >
                ...
                swipeInsideInnerScrollContainer = alreadyScrolled;
            }
        }
    ...
    static getDefaultOptions(): Partial<ISlidingPanelTemplateOptions> {
        return {
            ...
            usingScrollWrapper: true,
            shadowVisible: true,
        };
    }
}

export default withAdaptiveMode(SlidingPanel);
