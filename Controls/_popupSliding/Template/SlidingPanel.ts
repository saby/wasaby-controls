...
export default class SlidingPanel extends Control<ISlidingPanelTemplateOptions> {
    ...
    protected _beforeMount(options: ISlidingPanelTemplateOptions): void {
        this._isAdaptive = getConfig('isAdaptive');
    ...
    protected _isScrollAvailable(
        options: ISlidingPanelTemplateOptions
    ): boolean {
        const { slidingPanelOptions } = options;
        const contentHeight = this._getHeight(options);

        const height =
            slidingPanelOptions.height || this._container?.clientHeight || 0;
    ...
    private _getHeight(options: ISlidingPanelTemplateOptions): number {
        if (this._isPanelMounted) {
            const sliderHeight = options.controlButtonVisibility
                ...
                : 0;
            const customContentHeight =
                this._children.customContent.clientHeight;
    ...
    private _touchOnController(event: SyntheticEvent<TouchEvent>): boolean {
        ...
        return !!(
            target &&
            target.closest('.controls-SlidingPanel__controller-container')
        );
    }
    ...
    private _isSwipeForScroll(event: SyntheticEvent<TouchEvent>): boolean {
        ...
        if (
            ...
            scrollContainers[0]?.closest(
                '.controls-SlidingPanel__scrollWrapper'
            )
        ) {
        ...
        return (
            (this._scrollAvailable &&
                ...
                (alreadyScrolled ||
                    (swipeToScrollSide && this._hasContentForScroll()))) ||
    ...
    private _horizontalScrollSwipe(event: SyntheticEvent<TouchEvent>): boolean {
        ...
        const verticalOffset = Math.abs(
            this._startTouchPosition.y - touchData.clientY
        );
        const horizontalOffset = Math.abs(
            this._startTouchPosition.x - touchData.clientX
        );
        return (
            swipeInsideInnerScrollContainer && verticalOffset < horizontalOffset
        );
    }

    private _getParentScrollContainerElements(
        element: HTMLElement
    ): HTMLElement[] {
        /**
    ...
    private _notifyCustomDragStart(offset: IDragObject['offset']): void {
        const direction =
            offset.y > 0 ? DRAG_DIRECTION.BOTTOM : DRAG_DIRECTION.TOP;
    ...
    private _getDragOffsetByTouchEvent(
    ...
    private _getDragOffsetWithOverflowChecking(
        ...
        const { scrollHeight: startScrollHeight } =
            this._dragStartHeightDimensions;
    ...
    private _scrollHandler(): void {
        ...
        const isFocusInsideCurrentPanel =
            focusedElement.closest('.controls-SlidingPanel') ===
            this._container;
        if (
