...
class Strategy {
        ...
        if (heightList) {
            ...
            for (let i = 1; i < heightList.length; i++) {
                ...
                if (nextStep === 'auto') {
                    ...
                    position.height =
                        previousStepDifference < nextStepDifference && i !== heightList.length - 1
                            ? previousStep
                            : nextStep;
    ...
    getWindowHeight(): number {
        let height = DimensionsMeasurer.getVisualViewportDimensions().height;
    ...
    private _setInvertedPosition(position: IPopupPosition, property: string, value: number): void {
        // Если клавиатура открыта, то отображаемся сразу, без какой либо анимации
        if (this._isKeyboardVisible()) {
            return;
        }
