...
import SlidingStackStrategy, {
    AnimationState,
    ISlidingStackPanelItem,
} from './SlidingStackStrategy';
import { detection } from 'Env/Env';
import { DimensionsMeasurer } from 'Controls/sizeUtils';

const ANIMATION_CLASS = 'controls-SlidingPanel__animation-stack';
...
class Controller extends BaseController {
    ...
    private _getWorkspaceWidth(item: ISlidingStackPanelItem): number {
        return (
            item.position.width ||
            item.sizes.width ||
            DimensionsMeasurer.getVisualViewportDimensions().width
        );
    }
    ...
    protected _initItemAnimation(item: ISlidingStackPanelItem): void {
        item.popupOptions.className = `${
            ...
            ${
                item.popupOptions.animation !== false ? ANIMATION_CLASS : ''
    ...
    beforeUpdateOptions(item: ISlidingStackPanelItem): void {
        // sliding stack must update on elementAfterUpdatePhase
    }

    afterUpdateOptions(item: ISlidingStackPanelItem): void {
        // sliding stack must update on elementAfterUpdatePhase
    }
