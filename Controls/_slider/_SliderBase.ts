...
> extends Control<TSliderBaseOptions> {
    ...
    protected _setValue(e: SyntheticEvent<Event>, dragObject): void {
        ...
        const target =
            this._options.direction === 'vertical' ? dragObject.position.y : dragObject.position.x;
    ...
    protected _getReversePoint(point: IPointData): IPointData {
        ...
        if (
            previousPoint &&
            previousPoint.position >= this._value &&
            previousPoint.name !== 'tooltip'
        ) {
    ...
    protected _getPointPosition(point: IPointData): string {
        ...
        const percents =
            (maxPercentValue * (point.position - this._options.minValue)) / rangeLength;
        return `${percents}%`;
    }
