...
export default class Navigation extends Control<INavigationControlOptions> {
    ...
    protected _afterUpdate(oldOptions?: INavigationControlOptions, oldContext?: unknown): void {
        if (oldOptions.activeElement !== this._options.activeElement) {
            ...
            if (activeContainer) {
                ...
                const parentContainer = activeContainer.offsetParent as HTMLElement;
                if (
    ...
    protected _scrollStateChangedHandler(event: SyntheticEvent, state: IScrollState): void {
        this._scrollLeft = state.scrollLeft;
    }

    protected _onSelectedKeyChanged(event: SyntheticEvent, key: CrudEntityKey): void {
    ...
    private _resolveRoot(
        ...
        const pathToActiveElement = this._getPathToItem(options.items, activeElement, options.root);
    ...
    private _resolveActiveElement(
        ...
        const path = this._getPathToDepth(options.items, options.root, activeElementKey);
    ...
    private _getPathToItem(
        ...
        if (itemKey !== undefined && itemKey !== null) {
            ...
            do {
                ...
                parent = this._hierarchyRelation.getParent(parent, items) as Model | null;
            } while (parent !== null);
        }
        path.unshift(root);
    ...
    private _getPathToDepth(
        ...
        let child = this._hierarchyRelation.getChildren(lastPathItemKey, items)[0] as Model;
        while (child) {
            path.push(child.getKey());
            child = this._hierarchyRelation.getChildren(child, items)[0] as Model;
        }
        ...
        options.items.forEach((item, index) => {
            if (
                ...
                items.push({
                    ...
                    [options.displayProperty]: item.get(options.displayProperty),
