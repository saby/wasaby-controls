import type { Tree, TreeItem, BaseTreeControl, IBaseTreeControlOptions } from 'Controls/baseTree';
...
function getNotificationCompatibility() {
    return loadSync<typeof import('Controls/baseTree')>('Controls/baseTree')
        .NotificationCompatibility.expandCollapse;
}
...
function slicelessBaseTreeEndBeforeUpdate(
    ...
    if (treeControl._expandController) {
        ...
        if (wasResetExpandedItems) {
            if (isSourceControllerLoading) {
                resetExpandedItems(treeControl);
            } else {
                treeControl._shouldResetExpandedItems = true;
            }
...
function slicelessBaseTreeStartAfterReloadCallback(
    ...
    if (treeControl._shouldResetExpandedItems) {
        resetExpandedItems(treeControl);
        treeControl._shouldResetExpandedItems = false;
    }
...
function doExpand(
    ...
    getNotificationCompatibility()[expanded ? 'itemExpand' : 'itemCollapse'](
        nodeKey,
        treeControl,
        options
    );
