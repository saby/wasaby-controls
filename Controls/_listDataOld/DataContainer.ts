...
import template = require('wml!Controls/_listDataOld/DataContainer');
...
import {
    ...
    IAbstractListStateCalculator,
    THandleItemClickParams,
    getErrorConfig,
    processError,
    TErrorQueryConfig,
...
import { TKey, Direction, INavigationSourceConfig } from 'Controls/interface';
 ...
 *
 * @remark
 * Поле контекста "dataOptions" ожидает Controls/listDataOld:ListContainer, который лежит внутри.
 *
 * Полезные ссылки:
 ...
 * * {@link Controls/listDataOld:ListContainer}
 *
 * @class Controls/_listDataOld/DataContainer
...
export default class Data extends Control<IDataOptions, IReceivedState> {
    ...
    protected _stateCalculator: IAbstractListStateCalculator;
    ...
    _beforeMount(
        ...
        this._onExpandedItemsChanged = this._onExpandedItemsChanged.bind(this);
        ...
        if (options.useCollection) {
            this._stateCalculator = StateCalculatorFactory.create({
                initConfig: options,
                state: options,
            });
        }
        ...
        if (options.sourceController) {
            ...
            if (options.sourceController.getLoadError() && options.insideList) {
            ...
            return sourceController
                ...
                .catch((error) => {
                    if (options.insideList) {
                        return this._processAndShowError({ error }, {}, options.theme).then(
                            (errorConfig) => {
                                return { errorConfig };
                            }
                        );
    ...
    protected _errorDialogClosed(): void {
    ...
    protected _beforeUpdate(newOptions: IDataOptions): void | Promise<RecordSet | Error> {
        ...
        const isSourceControllerChanged = this._options.sourceController !== sourceController;
        let currentSourceController = this._sourceController;

        if (isSourceControllerChanged) {
            const loadError = sourceController?.getLoadError();
            ...
            if (!loadError && this._errorConfig) {
                this._hideError();
            }

            if (loadError) {
                this._processAndShowError({ error: loadError }, {});
            }
        }

        if (newOptions.collectionType !== this._options.collectionType) {
            this._stateCalculator.destroy();
            this._stateCalculator = StateCalculatorFactory.create(newOptions);
        }
        ...
        if (currentSourceController) {
            ...
            if (sourceController && newOptions.isSourceControllerFromContext) {
                ...
                this._updateBySourceControllerState(currentSourceController.getState());
            }
        }
    ...
    private _initSourceController(options: IDataOptions, receivedState?: IReceivedState): void {
        const sourceController =
            options.sourceController || this._getSourceController(options, receivedState);
        this._sourceController = sourceController;
    ...
    _updateWithSourceControllerInOptions(): void {
        if (this._isSourceControllerStateChanged() && !this._sourceController.isLoading()) {
            this._updateBySourceControllerState(this._sourceController.getState());
        }
    }

    _isSourceControllerStateChanged(): boolean {
        return !isEqual(this._sourceController.getState(), this._sourceControllerState);
    }

    _updateBySourceControllerState(sourceControllerState: ISourceControllerState): void {
    ...
    _filterChanged(event: SyntheticEvent, filter: QueryWhereExpression<unknown>): void {
        this._filter = filter;
    }
    ...
    private _onDataLoad(event: SyntheticEvent, result: RecordSet, direction: Direction): void {
        if (this._destroyed) {
            return;
        }
        ...
        if (this._options.sourceController && direction && this._isSourceControllerStateChanged()) {
            this._updateBySourceControllerState(this._sourceController.getState());
        }
    }

    private _getOriginalSource(options: IDataOptions): ICrudPlus | (ICrud & ICrudPlus & IData) {
        let source;
    ...
    private _onDataLoadError(
        ...
        if (this._isMounted) {
            ...
            if (!reason) {
                queryConfig.root = this._sourceController.getRoot();
            }

            this._processAndShowError({ error }, queryConfig);
            this._loading = false;
        }
    }
    ...
    private _processAndShowError(
        ...
        return processError(this._errorController, { ...config, ...errConfig }, theme).then(
            (errorConfig) => {
                if (errorConfig) {
    ...
    private _getErrorConfig(queryConfig: TErrorQueryConfig): Partial<ErrorViewConfig> {
        const errorConfig = getErrorConfig(queryConfig);
    ...
    protected _loadExpandedItem(
        event: SyntheticEvent,
        key: TKey
    ): Promise<RecordSet | Error> | void {
        if (!this._sourceController.hasLoaded(key)) {
            event.stopPropagation();
            return this._sourceController.load(void 0, key);
        }
    }
    ...
    protected _bindHandlersForListNewScheme() {
        this._onCheckboxClickNew = this._onCheckboxClickNew.bind(this);
        this._onItemClickNew = this._onItemClickNew.bind(this);
    }

    protected _onCheckboxClickNew(itemKey: TKey): void {
        // TODO: implement INewListSchemeHandlers
    }

    protected _onItemClickNew(itemKey: TKey): void {
        // TODO: implement INewListSchemeHandlers
    }
...
/**
 * @name Controls/_listDataOld/DataContainer#root
...
/**
 * @event Controls/_listDataOld/DataContainer#rootChanged при изменении корня иерархии.
 * @param {eventObject} event Дескриптор события.
 * @param {String|Number} root Идентификатор корневой записи.
 */

/**
 * @name Controls/_listDataOld/DataContainer#dataLoadCallback
 ...
 *    <Controls.listDataOld:DataContainer dataLoadCallback="{{_myDataLoadCallback}}" />
 * </pre>
 * <pre class="brush:js">
