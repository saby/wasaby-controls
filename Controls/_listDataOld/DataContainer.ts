...
export default class Data extends Control<IDataOptions, IReceivedState> {
    ...
    protected _contextState: Record<string, object>;
        ...
        if (options.sourceController) {
            ...
            if (options.sourceController.getLoadError() && options.insideList) {
                ...
                if (
                    ...
                    (!options.isBrowser || !sourceController.getDataLoadCallback())
                ) {
    ...
    private _initSourceController(options: IDataOptions): void {
    ...
    private _processAndShowError(
        ...
        let errConfig: Partial<ErrorViewConfig> = this._getErrorConfig(queryConfig);
        errConfig = { ...config, ...errConfig };
        // Необходимо, чтобы обойти кейс, когда undefined в errConfig затирает config.error
        errConfig.error = errConfig.error ?? config.error;

        return processError(this._errorController, errConfig, theme).then((errorConfig) => {
            if (errorConfig) {
