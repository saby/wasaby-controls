...
import { DATA_SYNTHETIC_STORE_ID } from 'Controls/baseList';
...
class ListContainer extends Control<IListContainerOptions> {
    ...
    protected _beforeMount(options: IListContainerOptions): void {
        ...
        this._listStoreId = this._getListStoreId(options.id);
    }
    ...
    private _getListStoreId(id?: string): string | undefined {
        const dataOptions = this._dataOptions;
        if (!dataOptions) {
            return;
        }
        return id ? dataOptions.listsConfigs?.[id]?.listStoreId : dataOptions.listStoreId;
    }
