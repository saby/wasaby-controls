...
import EventProxyMixin from './_datePopupNew/Mixin/EventProxy';
import periodDialogUtils from './_datePopupNew/Utils';
import componentTmpl = require('wml!Controls/_datePopupNew/DatePopup');
import headerTmpl = require('wml!Controls/_datePopupNew/header');
import dayTmpl = require('wml!Controls/_datePopupNew/day');
...
export default class DatePopup extends Control implements EventProxyMixin {
    ...
    _beforeMount(options: IControlOptions): void {
        ...
        // https://online.sbis.ru/opendoc.html?guid=e7dc54f8-1b15-4c93-9d38-e6f33022b53d
        this._today = options._date
            ? options._date.getDate()
            : new Date().getDate();
        ...
        this._monthStateEnabled =
            periodDialogUtils.isMonthStateEnabled(options);
        ...
        this._state =
            ...
            this.getViewState(
                options,
                this._monthStateEnabled,
                this._yearStateEnabled
            );
        if (this._state === STATES.year && !options.shouldPositionBelow) {
        ...
        if (options.selectionType === 'quantum') {
            ...
            for (const quantumRange of monthsRangeQuantums) {
                if (quantumRange in options.ranges) {
                    this._monthRangeQuantum[quantumRange] =
                        options.ranges[quantumRange];
                }
            }
            if (!Object.keys(this._monthRangeQuantum).length) {
                this._monthRangeSelectionType =
                    IDateRangeSelectable.SELECTION_TYPES.disable;
            }
        }
    ...
    protected _startValuePickerValueChangedHandler(
        event: Event,
        value: Date
    ): void {
    ...
    protected _startValuePickerInputCompleted(
    ...
    private _startValuePickerChanged(value: Date): void {
        this.rangeChanged(
            ...
            this._options.selectionType ===
                IRangeSelectable.SELECTION_TYPES.single
                ? value
                : this._rangeModel.endValue
        );
    }

    protected _endValuePickerValueChangedHandler(
        event: SyntheticEvent,
        value: Date
    ): void {
    ...
    protected _endValuePickerInputCompleted(
        event: SyntheticEvent,
        value: Date
    ): void {
        // Обновим значение на inputCompleted, т.к. кнопка не стреляет невалидным значением из valueChanged
    ...
    private _endValuePickerChanged(value: Date): void {
        ...
        if (
            ...
            !this.isMaskWithDays(this._mask)
        ) {
    ...
    _yearsSelectionChanged(e: SyntheticEvent, start: Date, end: Date): void {
        ...
        if (!this._memorizedValues.startValue) {
    ...
    _inputFocusOutHandler(event: SyntheticEvent): Promise<boolean> {
        ...
        return new Promise((resolve) => {
            if (
                !this._children.inputs.contains(event.nativeEvent.relatedTarget)
            ) {
                return this.isInputsValid().then((valid: boolean) => {
                    if (valid) {
    ...
    private _resetYearSelection(): void {
        if (this._yearsRangeSelectionProcessing) {
            ...
            if (
                ...
                this._yearRangeModel.startValue =
                    this._memorizedValues.startValue;
    ...
    toggleState(date?: Date): void {
        ...
        this._displayedDate =
            this._state === STATES.year && !this._options.shouldPositionBelow
    ...
    isInputsValid(): Promise<boolean> {
        return this._children.formController
            .submit()
            .then((results: object) => {
    ...
    updateStartValueValidators(validators?: Function[]): void {
        const startValueValidators: Function[] =
            validators || this._options.startValueValidators;
    ...
    updateEndValueValidators(validators?: Function[]): void {
        const endValueValidators: Function[] =
            validators || this._options.endValueValidators;
