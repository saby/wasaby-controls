    ...
    for (const coefName in transformTemplate) {
        if (transformTemplate.hasOwnProperty(coefName)) {
            ...
            if (colors[mainColorName]) {
                ...
                transformTemplate[coefName].forEach(
                    (colorDesc: IColorDescriptor) => {
                        const subColorName = prefix
                            ? `--${prefix}_${colorDesc.name}`
                            : colorDesc.name;
                        ...
                        if (!colors[subColorName]) {
                            ...
                            if (colorDesc.transparent) {
                                result[subColorName] = rgbaToString(
