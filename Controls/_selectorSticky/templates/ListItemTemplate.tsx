...
export default function ListItemTemplate(props): React.ReactElement {
    ...
    const historySeparatorAttrs = React.useMemo(() => {
        return isHistorySeparatorVisible
            ...
            : null;
    }, [isHistorySeparatorVisible, item]);

    const expanderIcon = React.useMemo(() => {
        return itemContents?.get('expanderIcon') === 'hidden'
            ? 'selectorHiddenNode'
    ...
    return (
        <>
            <GridItemTemplate
                ...
                marker={false}
                withoutLevelPadding={itemContents?.get('withoutLevelPadding') ?? false}
                withoutExpanderPadding={
                    itemContents?.get('withoutExpanderPadding') ??
                ...
                className={`${
                    props.className
                } controls-Layout-SelectorPopup_item controls-Menu__item ${
            ...
            {isHistorySeparatorVisible ? (
                <GroupTemplate
                    className="controls-margin_top-xs controls-Menu__group_marginBottom"
