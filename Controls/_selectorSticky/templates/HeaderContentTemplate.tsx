...
function HeaderContentTemplate(props: IHeaderTemplateProps): React.ReactElement {
    ...
    if (
        props.allowAdaptive &&
        (!hasBreadcrumbsInHeader || (hasBreadcrumbsInHeader && props.searchParam && expanded))
    ) {
        paddingLeftClassName += ' controls-Layout-SelectorPopup-Sticky-header-margin_left-adaptive';
    } else if (
        (hasBreadcrumbsInHeader && props.searchParam && expanded) ||
        (props.searchParam && !hasHeading) ||
        !hasBreadcrumbsInHeader
    ) {
        paddingLeftClassName += ' controls-Layout-SelectorPopup-Sticky-header-margin_left-expander';
    } else if (!props.allowAdaptive) {
        paddingLeftClassName += ' controls-padding_left-2xs';
    }

    if (
        (props.allowAdaptive && !hasBreadcrumbsInHeader) ||
        (props.searchParam && !hasHeading) ||
        !hasBreadcrumbsInHeader
    ) {
        paddingLeftClassName +=
            ' controls-Layout-SelectorPopup-Sticky-header-padding_left-expander';
    }

    return (
        <div
            className={`tw-flex ${
                !props.allowAdaptive ? 'controls-margin_left-2xs controls-margin_right-2xs' : ''
            }`}
        >
            <div
                style={props.style}
            ...
            controls-Layout-SelectorPopup-Sticky-header-padding_right-${
                allowAdaptive
                    ? props.multiSelect && props.applyButtonVisible
                        ? 'applyButton-visible-adaptive'
                        : 'adaptive'
                    : props.headerTemplate
                    ? 'withHeader'
                    : props.multiSelect
                    ? `applyButton${props.applyButtonVisible ? '-visible' : ''}`
                    : 'default'
            }
            ...
            >
                ...
                {props.multiSelect && !props.applyButtonVisible ? (
                    <div className="tw-flex tw-min-w-0 controls_Layout-SelectorPopup-Sticky-header-applyButtonSpace_left"></div>
                ) : null}
            </div>
        </div>
    );
...
function SearchFilterTemplate(
    ...
    const filterSearchRef = React.useRef(null);
    React.useEffect(() => {
        if (filterSearchRef.current) {
            activate(filterSearchRef.current);
        }
    }, []);
    ...
    if (props.searchParam && props.filterDescription) {
        ...
        return (
            <FilterSearchView
                ref={filterSearchRef}
                className="tw-w-full controls-Layout-SelectorPopup-header_searchInput"
                storeId={props.storeId}
