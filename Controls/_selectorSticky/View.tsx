...
export default forwardRef(function SelectorStickyView(props: ISelectorViewOptions, _) {
    ...
    const selectionChanged = useRef(
        !!props.selectedKeys?.length && !props.selectedKeys?.includes(props.emptyKey)
    );
    ...
    const onItemClick = useCallback(
        (item, event) => {
            ...
            if (isNode === false) {
                ...
                if (
                    ...
                    selectorContext?.select(props.emptyKey);
                }
        ...
        [sendResult, props.selectedKeys, props.emptyKey, applyButtonVisible]
    );
        ...
        <>
            ...
            <ScrollContainer
                ...
                bottomShadowVisibility={
                    props.stickyFooter && props.footerContentTemplate
...
function getSelectedKeys(keys: TKey[], added: TKey[], emptyKey: TKey): TKey[] {
    const removeEmptyKey = !!added?.length && keys?.includes(emptyKey);
...
function needUpdateEmptyKeySelected(newKey: TKey, selectedKeys?: TKey[], emptyKey?: TKey): boolean {
    return (
