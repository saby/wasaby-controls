...
import type { Source as HistorySource } from 'Controls/historyOld';
...
export default class SelectorSlice extends ListSlice<IListState> {
    ...
    protected _initState(
        ...
        this._items = loadResult.items?.clone();
    ...
    protected async _beforeApplyState(nextStateProp: ISelectorSliceState): Promise<IListState> {
        ...
        if (this.state.searchValue !== nextStateProp.searchValue) {
            this._prepareHistorySourceForSearch(!!nextStateProp.searchValue);
        }
    ...
    protected _nodeDataLoaded(
        ...
        if (nextState.historyId) {
            const newItems = loadSync(
                'Controls/Utils/History/PrepareMenuItems:getItemsWithHistory'
            )?.(
                nextState.items,
                nextState.historyId,
                nextState.source,
    ...
    destroy() {
        this._prepareHistorySourceForSearch(false);
        super.destroy();
    }

    private _prepareHistorySourceForSearch(isSearch: boolean) {
        let source = this.state.source;
        if (source instanceof PrefetchProxy) {
            source = source.getOriginal();
        }
        if (isHistorySource(source)) {
            (source as unknown as HistorySource).addOriginItems(null);
            (source as unknown as HistorySource).setUnpinIfNotExist?.(!isSearch);
        }
    }
...
function isHistorySource(source?: any): boolean {
