...
export default class SelectorSlice extends ListSlice<IListState> {
    private _emptyKey: TKey;
    private _historyRoot: TKey;
    ...
    protected _initState(
        ...
        this._emptyKey = initConfig.emptyKey;
    ...
    protected async _beforeApplyState(nextStateProp: ISelectorSliceState): Promise<IListState> {
        if (nextStateProp.selectedKeys.includes(this._emptyKey)) {
            nextStateProp.markedKey = this._emptyKey;
        } else if (
            this.state.selectedKeys?.includes(this._emptyKey) &&
            !nextStateProp.selectedKeys.includes(this._emptyKey)
        ) {
    ...
    protected _nodeDataLoaded(
        ...
        const newItems = loadSync('Controls/dropdown:getItemsWithHistory')?.(
            nextState.items,
            nextState.historyId,
            nextState.source,
    ...
    protected _dataLoaded(
        ...
        if (nextState.historyId) {
            return Promise.resolve(
                loadAsync('Controls/dropdown').then(({ getItemsWithHistory }) => {
    ...
    private _setBreadcrumbsPathIfNeed(state: ISelectorSliceState, items = state.items) {
        if (
            ...
            items.getIndexByValue(state.parentProperty, state.root) !== -1
        ) {
            if (state.parentProperty) {
                ...
                let backButtonItem;
                if ((!state.root && path?.getCount?.()) || (!hasRootInBreadCrumbs && state.root)) {
                    backButtonItem = items.getRecordById(state.root as string);
                    ...
                    state.breadCrumbsItems = backButtonItem ? [backButtonItem] : null;
                    state.backButtonCaption = backButtonItem
                        ? backButtonItem.get(state.displayProperty || 'title')
                        : '';
                    items.assign(items);
                }
            }
            ...
            .filter((item, index) => {
                return (
                    ...
                    (config.expandedItems &&
                        ...
                        config.expandedItems?.includes(item.get(config.parentProperty)))
                );
            })
