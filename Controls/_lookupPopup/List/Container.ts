...
type SelectionChangedEventResult = [Key[], Key[], Key[], boolean];
...
const _private = {
    ...
    isSelectedAll(self): boolean {
        return _private.getListOptions(self._options).root !== undefined
            ? _private
                  .getListOptions(self._options)
                  .selectedKeys.includes(_private.getListOptions(self._options).root)
            : _private.getListOptions(self._options).selectedKeys.includes(null);
    },

    selectItem(self, item: Record): void {
        const itemKey = item.get(_private.getListOptions(self._options).keyProperty);
        const eventResult = [[itemKey], [itemKey], [], true] as SelectionChangedEventResult;
    ...
    selectionChanged(self, item: Record): void {
        ...
        if (isSelectedAll) {
            ...
            keys = _private.getListOptions(self._options).excludedKeys;
        } else {
            eventName = 'notifySelectedKeysChanged';
            keys = _private.getListOptions(self._options).selectedKeys;
        }

        _private[eventName](
            self,
            _private.getItemActivateResult(itemKey, keys, self._options.multiSelect)
        );
    },
    ...
    getItemActionVisibilityCallback(options) {
        return (action, item, isEditing) => {
            ...
            if (action.id === SELECT_ACTION_ID) {
                showByOptions =
                    !options.multiSelect || !_private.getSelectedKeysFromOptions(options).length;
                showByItemType =
                    options.selectionType === 'node'
                        ? true
                        : item.get(_private.getListOptions(options).nodeProperty);
    ...
    itemActivate(self, item: Record): void {
        ...
        const selectedKeys = _private.getSelectedKeysFromOptions(self._options);
...
class Container extends Control {
    ...
    protected _selectedKeys: TKey[] = null;
    protected _markedKey: TKey = null;
    protected _itemActions = null;
    protected _itemActionVisibilityCallback = null;
    protected _itemActionsClick = null;
    ...
    _beforeUpdate(newOptions) {
        const selectedKeys = _private.getSelectedKeysFromOptions(newOptions);
        const selectionTypeChanged = newOptions.selectionType !== this._options.selectionType;
        const selectedKeysChanged = !isEqual(selectedKeys, this._selectedKeys);
    ...
    _itemActivate(event: SyntheticEvent, item: Record): void {
        const nodeValue = item.get(_private.getListOptions(this._options).nodeProperty);
