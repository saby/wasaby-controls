...
const _private = {
    ...
    getSelectedKeys(options): TKey[] {
        ...
        if (options.selectedKeys) {
            ...
            selectedKeys = _private.getKeysByItems(
                items,
                options._dataOptionsValue.keyProperty
            );
        } else {
            selectedKeys = [];
        }
    ...
    getSourceAdapter(source: IData): adapterLib.IAdapter {
        ...
        if (cInstance.instanceOfMixin(source, 'Types/_source/IDecorator')) {
            adapter = (
                (source as IDecorator).getOriginal() as IData
            ).getAdapter();
        } else {
    ...
    needLoadItemsOnSelectComplete(self): boolean {
        const hasSelectedItems =
            self._selectedKeys.length || self._excludedKeys.length;
        let result;
        ...
        if (_private.needLoadItemsOnSelectComplete(self)) {
            if (!self._options.multiSelect) {
                ...
                loadItemsPromise = _private
                    .getCrudWrapper(source)
                    .query({ filter, sorting, select: self._options.selectFields })
                    .catch((error) => {
                        process({ error });
...
const Container = Control.extend({
    ...
    _beforeMount(options): void {
        ...
        this._initialSelection = _private.getInitialSelectedItems(
            this,
            options
        );
    },
    ...
    _beforeUpdate(newOptions): void {
        ...
        if (!isEqual(this._options.excludedKeys, newOptions.excludedKeys)) {
    ...
    _selectComplete(): void {
        ...
        if (
            !isRecursive &&
            !(
                this._selectedKeys.includes(root) &&
                this._excludedKeys.includes(root)
            )
        ) {
        ...
        if (this._options.selectionLoadMode) {
            loadPromise = _private
                .loadSelectedItems(this, filter)
                .then((loadedItems) => {
