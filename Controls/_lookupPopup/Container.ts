...
const _private = {
    ...
    prepareFilter({
        ...
        let resultFilter = Utils.object.clone(filter);
        const hasSearchParamInFilter = searchParam && resultFilter[searchParam];
        const hasSelectedNodes = _private.hasSelectedNodes(items, selectedKeys, nodeProperty);
    ...
    loadSelectedItems(self: object, filter: QueryWhereExpression<unknown>): Promise<RecordSet> {
        const { items, sorting, source, nodeProperty } = _private.getDataOptions(self._options);
                ...
                loadItemsPromise = _private
                    ...
                    .then((rs) => {
                        const isNodeSelected =
                            self._selectedKeys?.length === 1 &&
                            _private.hasSelectedNodes(items, self._selectedKeys, nodeProperty);
                        if (
                            !rs.getCount() &&
                            items.getCount() &&
...
const Container = Control.extend({
    ...
    _selectedKeysChanged(event, selectedKeys, added, removed, isSelectComplete) {
