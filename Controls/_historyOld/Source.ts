...
export default class HistorySource
    ...
    update(data: any, meta: any): Promise<any> {
        if (meta.hasOwnProperty('$_pinned')) {
            return Promise.resolve(this._getDropdownLibrary().then((dropdownLib) => {
    ...
    query(query: Query): Promise<DataSet> {
        ...
        if (where && (where.$_history === true || where.historyKeys)) {
            ...
            return Promise.allSettled([Store.load(this.getHistoryId()), this._$originSource.query(query), this._getDropdownLibrary()])
                .then((data) => {
                    let result: DataSet;
                    // method returns error
                    if (data[1].status !== 'rejected' && data[1].value) {
                        // PrefetchProxy returns RecordSet
                        const loadedItems = data[1].value.getAll ? data[1].value.getAll() : data[1].value;
