    ...
    implements ICrud
{
    ...
    > {
        return loadAsync<typeof import('Controls/Utils/History/PrepareMenuItems')>(
    ...
    private _getHistorySourceConfig() {
        return {
            ...
            unpinIfNotExist: this._$unpinIfNotExist && this._unpinIfNotExist,
    ...
    private _getHistoryStore(): any {
        let historyStoreLib = null;
        try {
            historyStoreLib = loadSync('Controls/HistoryStore');
        } catch {
            Logger.error(
                'Controls/_historyOld/Source: попытка взаимодействия с незагруженной историей'
            );
        }
    ...
    async query(query: Query): Promise<DataSet> {
        let where = query.getWhere() as Record<string, any>;

        let newItems: RecordSet;

        let originSourceQuery: Promise<object>;
        let historySourceQuery: Promise<DataSet> | object;
    ...
    getItems(withHistory: boolean = true): RecordSet {
        ...
        if (Store?.getLocal(this.getHistoryId()) && withHistory) {
            const { getItemsWithHistory } = loadSync<
                typeof import('Controls/Utils/History/PrepareMenuItems')
            >('Controls/Utils/History/PrepareMenuItems');
    ...
    addOriginItems(items: RecordSet | null): void {
        this._$oldItems = items;
    }

    setUnpinIfNotExist(unpinIfNotExist): void {
        this._unpinIfNotExist = this._$unpinIfNotExist && unpinIfNotExist;
    }
    ...
    getHistoryLoadConfig(): THistoryLoadConfig {
        return this._$historySource.getHistoryLoadConfig();
    }
