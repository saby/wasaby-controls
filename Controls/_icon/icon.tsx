...
import { IIconStyleOptions, TIconSize, IComponentProps } from 'Controls/interface';
...
import {
    default as loadFont,
    isCompatibleMode as isCompatibleFontMode,
} from 'RequireJsLoader/loadFont';
...
export interface IIconOptions extends IIconStyleOptions, IComponentProps {
    /**
     * @cfg {string}
...
/**
 * Функция предзагрузки SVG на странице.
 * Необходима так же как и предзагрузка шрифта.
 */
function preloadSvg(svgHref: string) {
    if (typeof window !== 'undefined') {
        return;
    }

    // @ts-ignore не описан crossorigin в HeadAPI
...
function SVG_ICON_RENDER(props: IInnerIconProps): JSX.Element {
    const iconPath = React.useMemo(() => {
        ...
        if (iconData.svgUrl) {
            preloadSvg(iconData.svgUrl);
            return iconData.svgUrl;
        }
        const iconUrl = `${constants.resourceRoot}${iconData.iconModule}/${iconData.iconPackage}.svg`;
        const fileUrl = getResourceUrl(iconUrl, undefined);
        preloadSvg(fileUrl);
...
// По аналогии с https://online.sbis.ru/opendoc.html?guid=3743bfe1-8099-4d16-b9ea-d367863b9bc1&client=3
...
function FONT_ICON_RENDER(props: IInnerIconProps): JSX.Element {
    ...
    if (loadedFontRef.current !== iconData.loadPath) {
        // loadFont По умолчанию загружает иконки размера s
        // и добавляет в генерированный CSS класс постфикс _s
        loadFont(iconData.loadPath, props.iconSize);
        loadedFontRef.current = iconData.loadPath;
    }

    return (
        // controls-icon_lineHeight-strict нужен, чтобы не вешался костыльный important,
        // который тянет иконку вверх на 1px.
        <span
            tabIndex={props.tabIndex}
            {...props.eventHandlers}
            data-qa={props['data-qa']}
            ...
            className={`${props.className} controls-icon controls-icon_lineHeight-strict ${
                iconData.isCompatibleMode ? iconData.icon : ''
            } ${iconData.iconPackage}_${detectSize(props.iconSize)}`}
        >
            {!iconData.isCompatibleMode ? iconData.icon : ''}
        </span>
    );
...
const forwardedIcon = React.forwardRef(IconTemplate);
forwardedIcon.defaultProps = {
    iconSize: 'm',
};
