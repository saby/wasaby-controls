...
export interface IDataSetProps {
     ...
     * Если запрос к источнику будет выполняться дольше, чем указано в опции,
     * то запрос будет отмёнен, а из метода ({@link load}, {@link prev}, {@link next}) будет возвращена ошибка
     * @remark задаётся в милисекундах
     */
...
export default class DataSet {
    ...
    updateQuery(loadedItems: RecordSet, { root, ...queryParams }: IQueryParams): void {
        if (this._hasPaginationBySource()) {
            const queryRoot = root !== undefined ? root : this._root;
            this.applyPaginationChanges(
                this.calculatePaginationChanges(loadedItems, {
                    root: queryRoot,
                    loadKey: queryParams.loadKey !== undefined ? queryParams.loadKey : queryRoot,
                    ...queryParams,
                })
            );
        }
    ...
    _getNavigationControllerProps(props: IDataSetProps): INavigationControllerOptions {
        const pagination = props.pagination as INavigationOptionValue<INavigationSourceConfig>;

        if (!pagination.source) {
            throw new Error('Необходимо настроить тип навигации');
        }
    ...
    _query(queryParams?: IQueryParams): Promise<RecordSet> {
        return this._getSource(queryParams?.direction).then((source) => {
            const query = this._getQuery(queryParams);
            let queryPromise: Promise<RecordSet>;

            if (this._props.loadDataTimeout) {
                queryPromise = (
                    wrapTimeout(
                        source.query(query),
                        this._props.loadDataTimeout
                    ) as Promise<TypesDataSet>
                )
                    .catch((error) => {
                    ...
                    .then((dataSet) => this._queryResult(dataSet));
            } else {
                queryPromise = source.query(query).then((dataSet) => this._queryResult(dataSet));
            }
    ...
    _queryResult(dataSet: TypesDataSet): RecordSet {
    ...
    _hasPaginationBySource(
        pagination = this._props.pagination
    ): pagination is INavigationOptionValue<INavigationSourceConfig> {
        return Boolean(pagination && pagination.source && pagination.sourceConfig);
    }
}
