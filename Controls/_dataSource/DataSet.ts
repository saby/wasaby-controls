...
export default class DataSet {
    ...
    constructor(props: IDataSetProps) {
        ...
        if (props.items) {
            ...
            this._updateQuery(props.items, { loadKey: this._root });
        }
    }
     ...
     */
    load(
        pagination?: TNavSourceConfig,
        updatePaginationAfterLoad?: boolean
    ): Promise<RecordSet | Error> {
        return this._load({ paginationConfig: pagination, updatePaginationAfterLoad });
    }
    ...
    setItems(items: RecordSet, keepNavigation?: boolean): void {
        if (this._hasPaginationBySource() && items && !keepNavigation) {
            this._destroyNavigationController();
            this._updateQuery(items, { loadKey: this._root, keepNavigation });
        }
        this._items = items;
    }
    ...
    setItemsAfterLoad(
        ...
        this._updateQuery(items, { ...queryParams, keepNavigation });
    }
    ...
    hasLoaded(key: TKey): boolean {
        ...
        if (this._hasPaginationBySource()) {
            ...
            loadedResult = Boolean(
                items &&
                    QueryBuilder.getHierarchyRelation(this._props).getChildren(key, childItems)
                        .length
            );
        }

        if (!loadedResult && hasChildrenProperty) {
            loadedResult = !items.getRecordById(key)?.get(hasChildrenProperty);
        }
    ...
    calculatePaginationChanges(
        ...
        const { direction, keepNavigation, root, loadKey } = queryParams;
        ...
        const resetNavigation = !direction && isRoot && !keepNavigation;
    ...
    _updateQuery(loadedItems: RecordSet, queryParams: IQueryParams): void {
        if (this._hasPaginationBySource()) {
            ...
            .then((items) => {
                if (items instanceof RecordSet) {
                    ...
                    if (queryParams.updatePaginationAfterLoad !== false) {
                        this._updateQuery(items, queryParams);
                    }
                } else {
                    Logger.error(
