...
> {
    ...
    protected _beforeMount(
        ...
        | Promise<IErrorContainerReceivedState>
        | IErrorContainerReceivedState
        | void {
        if (receivedState) {
            ...
            return DataLoader.load(sources, requestTimeout).then(
                ({ sources, errors }) => {
                    const errorsCount = errors.length;

                    if (
                        errorsCount === sources.length ||
                        (errorHandlingEnabled && errorsCount !== 0)
                    ) {
    ...
    static load(
        ...
        const loadPromise = new DataLoaderController().load(
            sources,
            loadDataTimeout
        );
        return loadPromise.then((results) => {
            results.forEach((loadDataResult, key) => {
                ...
                sourcesResult[key] =
                    DataLoader._createSourceConfig(loadDataResult);
            });
            return {
