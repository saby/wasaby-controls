...
import {
    ...
    IControllerState,
} from 'Controls/_dataSource/Controller';
...
export default class DataLoader {
    ...
    constructor(options: IDataLoaderOptions = {}) {
        if (options.loadDataConfigs instanceof Array) {
            ...
            this._loadDataConfigs = this._getConfigsWithKeys(
                this._originLoadDataConfigs
            );
            this._loadDataConfigs.forEach((config, index) => {
                ...
                this._loadedConfigStorage.set(
                    this._originLoadDataConfigs[index],
                    config.id
                );
            });
        } else {
    ...
    load<T extends ILoadDataResult>(
        sourceConfigs?: TLoadConfig[] | TLoadersConfigsMap = this
            ._loadDataConfigs,
        loadTimeout?: number,
        Router?: object
    ): Promise<IDataConfigs> {
        const configs = this._loadersCompatibility(sourceConfigs);
        return Loader.load(configs, loadTimeout, false, Router).then(
            (results) => {
    ...
    private _normalizeConfig(config: TLoadConfig): IDataConfig {
        ...
        return {
            dataFactoryName:
                ...
                (config.type
                    ? COMPATIBLE_MAP_TYPE[config.type]
                    : COMPATIBLE_MAP_TYPE.list),
            dataFactoryArguments: {
                ...factoryArgs,
                loadDataTimeout:
                    factoryArgs.loadDataTimeout || factoryArgs.loadTimeout,
            },
    ...
    setSourceController(
        ...
        return this._loadedConfigStorage.setSourceController(
            id,
            sourceController
        );
    }
    ...
    setFilterController(id: string, filterController: FilterController): void {
        return this._loadedConfigStorage.setFilterController(
            id,
            filterController
        );
    }
