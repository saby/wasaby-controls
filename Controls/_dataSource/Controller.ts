...
export default class Controller extends mixin<ObservableMixin>(ObservableMixin) {
    ...
    setItems(items: RecordSet, keepNavigation?: boolean): RecordSet {
        if (this._hasNavigationBySource() && items && !keepNavigation) {
    ...
    getKeyProperty(): string {
        ...
        if (keyProperty) {
            ...
            result = this._items.getKeyProperty();
        }
    ...
    getNavigationState(key: CrudEntityKey): IBaseSourceConfig {}
    ...
    private _prepareQueryParams(
        ...
        const queryDirection = NAVIGATION_DIRECTION_COMPATIBILITY[direction];
        const isMultiNavigation = this._isMultiNavigation(navigationSourceConfig);
        const expandedItems = this.getExpandedItems();
    ...
    private _callSourceMethod<T>(
        ...args: [methodName: keyof ICrud, key: TKey | undefined, ...callArgs: unknown[]]
    ): Promise<T> {
