...
export default class Controller extends mixin<ObservableMixin>(ObservableMixin) {
    ...
    setLoadError(error: Error, key?: TKey, direction?: Direction): void {
    ...
    private _getDataSetProps(): IDataSetProps {
        return {
            ...
            loadDataTimeout: this._options.loadTimeout,
        };
    }
        ...
        if (this._options.source) {
            ...
            this._loadPromise = new CancelablePromise(
                this._prepareFilterPromise.promise.then((preparedFilter) => {
                    const requestResult = this._query({
                        ...loadConfig,
    ...
    private _load(loadConfig: ILoadConfig): LoadResult {
        ...
        if (addItemsAfterLoad !== false) {
            return loader
                ...
                .catch((error) => {
                    if (error) {
                        this.setLoadError(error, key, direction);
                    }
                    return Promise.reject(error);
                });
        } else {
    ...
    private _query(loadConfig: ILoadConfig): LoadResult {
        const { direction, navigationSourceConfig, addItemsAfterLoad, key, keepNavigation } =
            loadConfig;
        let result;
        const filter = (loadConfig.filter || this.getFilter()) as Record<string, unknown>;
        const dataSetProps = {
            ...this._getDataSetProps(),
            filter,
        };
        const isHierarchyLoad = Array.isArray(filter?.[this.getParentProperty()]);
        ...
        } else if (key !== this._root && key !== undefined && !isHierarchyLoad) {
    ...
    private _processQueryError(
        ...
        // Если был отменён запрос на разворот узла, то навигацию для него надо сбросить,
        // чтобы этот узел не считался загруженным
        if (queryError instanceof PromiseCanceledError && key !== this._root && !direction) {
            this._getDataSet().resetPagination(key);
        }
        return queryError;
    }
