...
export default class Controller extends mixin<ObservableMixin>(ObservableMixin) {
    ...
    setItemsAfterLoad(
        ...
        this._getDataSet().setItemsAfterLoad(items, keepNavigation, {
    ...
    private _query(loadConfig: ILoadConfig): LoadResult {
        const { direction, navigationSourceConfig, filter, addItemsAfterLoad, key } = loadConfig;
        let result;
        const dataSetProps = {
        ...
        if (direction) {
            ...
            result = dataSet.load(navigationSourceConfig, addItemsAfterLoad);
        }
