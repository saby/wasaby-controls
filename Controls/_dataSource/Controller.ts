...
function isEqualFormat(oldList: RecordSet, newList: RecordSet): boolean {
    const oldListFormat =
        oldList &&
        oldList['[Types/_entity/FormattableMixin]'] &&
        oldList.getFormat(true);
    const newListFormat =
        newList &&
        newList['[Types/_entity/FormattableMixin]'] &&
        newList.getFormat(true);
    const isListsEmpty = !newList.getCount() || !oldList.getCount();
    return (
        (oldListFormat &&
            newListFormat &&
            oldListFormat.isEqual(newListFormat)) ||
        isListsEmpty ||
...
export function isEqualItems(oldList: RecordSet, newList: RecordSet): boolean {
    ...
    const items1Model =
        oldList &&
        oldList['[Types/_collection/RecordSet]'] &&
        oldList.getModel();
    const items2Model =
        newList &&
        newList['[Types/_collection/RecordSet]'] &&
        newList.getModel();
    let isModelEqual = items1Model === items2Model;

    if (
        !isModelEqual &&
        getModelModuleName(items1Model) === getModelModuleName(items2Model)
    ) {
        isModelEqual = true;
    }
    return (
...
export default class Controller extends mixin<ObservableMixin>(
    ObservableMixin
) {
    ...
    private _prepareFilterPromise: CancelablePromise<
        QueryWhereExpression<unknown> | Error
    >;
    ...
    constructor(cfg: IControllerOptions) {
        ...
        this._collectionPropertyChange =
            this._collectionPropertyChange.bind(this);
        this._onBreadcrumbsCollectionChanged =
            this._onBreadcrumbsCollectionChanged.bind(this);
    ...
    reloadItem(
        ...
        if (options.method === 'query') {
            ...
            reloadItemPromise = this.executeLoad({ filter }).then(
                (loadedItems) => {
            ...
            reloadItemPromise = this.read(key, options.readMeta).then(
                (item) => {
                    if (item) {
                        loadCallback(item);
                    } else {
    ...
    setItemsAfterLoad(items: RecordSet): RecordSet {
        this._processQueryResult(items, { key: this._root });
        return this._items;
    }
    ...
    updateOptions(newOptions: IControllerOptions): boolean {
        ...
        const isNavigationChanged = !isEqual(
        ...
        const isExpandedItemsChanged = this._isOptionChanged(
            'expandedItems',
            newOptions
        );
        ...
        const resetExpandedItemsOnDeepReload =
            newOptions.deepReload && !rootChanged;
        if (
            isChanged &&
            !(
                isExpandedItemsChanged ||
                resetExpandedItemsOnDeepReload ||
                this.isExpandAll()
            )
        ) {
            this.setExpandedItems([]);
    ...
    resetCollapsedNodes(newExpandedItems: TKey[]): void {
        ...
        const hasOldExpandedItems =
            oldExpandedItems instanceof Array && oldExpandedItems.length;
        const hasNewExpandedItems =
            newExpandedItems instanceof Array && newExpandedItems.length;
    ...
    updateExpandedItemsInUserStorage(): void {
        ...
        if (
            ...
            const lastSavedHistory = nodeHistoryUtil.getCached(
                this._options.nodeHistoryId
            );
            expandedItems = this._expandedItems.filter((key) => {
                ...
                if (!record) {
                    return lastSavedHistory
                        ? lastSavedHistory.indexOf(key) !== -1
                        : false;
                }
                const nodeTypeProperty = record.get(
                    this._options.nodeTypeProperty
                );
    ...
    hasMoreData(direction: Direction, key: TKey = this._root): boolean {
        ...
        if (this._hasNavigationBySource()) {
            hasMoreData = this._getNavigationController(
                this._navigation
    ...
    hasLoaded(key: TKey): boolean {
        ...
        if (this._hasNavigationBySource()) {
            loadedResult = this._getNavigationController(
                this._navigation
            ).hasLoaded(key);
            ...
            loadedResult =
                ...
                (this._items &&
                    this._getHierarchyRelation().getChildren(
                        key,
                        this._getHierarchyStrategy().getChildren(
                            this._items,
                            key
                        )
                    ).length);
        ...
        if (!loadedResult && this._options.hasChildrenProperty) {
            loadedResult = !this._items
                .getRecordById(key)
                ?.get(this._options.hasChildrenProperty);
        }
    ...
    private _getNavigationController(
        ...
        if (!this._navigationController) {
            this._navigationController =
                this._createNavigationController(navigation);
        }
    ...
    private _updateQueryPropertiesByItems(
        ...
        const { key, navigationSourceConfig, direction, keepNavigation } =
            loadConfig;
        let hierarchyRelation;

        if (
            this._hasNavigationByConfig(navigationSourceConfig) &&
            this._hasNavigationBySource()
        ) {
            const isMultiNavigation = this._isMultiNavigation(
                navigationSourceConfig
            );
            const isRoot = this._root === key;
            const resetNavigation =
                this._deepReload || (!direction && isRoot && !keepNavigation);
            const hasExpandedItems = !!this.getExpandedItems()?.length;
            if (
                resetNavigation &&
                (!isMultiNavigation || !hasExpandedItems || this.isExpandAll())
            ) {
                this._destroyNavigationController();
            }
            if (
                this._options.parentProperty &&
                (isMultiNavigation || hasExpandedItems)
            ) {
    ...
    private _prepareQueryParams(
        ...
        const navigationController = addItemsAfterLoad
        ...
        if (
            !isHierarchyQueryParamsNeeded ||
            !resultQueryParams ||
            !resultQueryParams.length
        ) {
            const resetNavigation =
                !isMultiNavigation ||
                key !== this._root ||
                !!direction ||
                ((!!navigationSourceConfig || !isHierarchyQueryParamsNeeded) &&
                    !keepNavigation);
    ...
    private _setItems(items: RecordSet): void {
        ...
        this._breadcrumbsRecordSet =
            this._items instanceof RecordSet
                ? this._items.getMetaData().path
                : null;
    ...
    private _resolveNavigationParamsChangedCallback(
        cfg: IControllerOptions
    ): void {
        if (cfg.navigationParamsChangedCallback) {
            this._navigationParamsChangedCallback =
                cfg.navigationParamsChangedCallback;
        }
    }
    ...
    private _load(loadConfig: ILoadConfig): LoadResult {
        ...
        if (addItemsAfterLoad !== false) {
            return loader
                .then((result: RecordSet) => {
                    if (result instanceof RecordSet) {
                        ...
                        Logger.error(
                            'source/Controller: query returns incorrect result',
                            this
                        );
                        return result;
                    }
            ...
            this._addFinallyToLoadPromise(loader);
            return loader;
        }
    }
    ...
    private _getFilterHierarchy(
        ...
        if (parentProperty) {
            return this._resolveExpandedHierarchyItems(
                ...
                const isDeepReload =
                    (!direction || isLoadToDirectionWithExpandedItems) &&
                    root === this._root;

                // Набираем все раскрытые узлы
                if (
                    expandedItems?.length &&
                    expandedItems?.[0] !== null &&
                    isDeepReload
                ) {
    ...
    private _prepareFilterForQuery(
        ...
        return this._getFilterHierarchy(
            filter,
            this._options,
            key,
            isFirstLoad,
    ...
    private _processQueryResult(
        ...
        if (loadedInCurrentRoot && this._dataLoadCallback) {
            dataLoadCallbackResult = this._dataLoadCallback(
                result,
                direction,
                this._options.id
            );
    ...
    private _updateBreadcrumbsData(): void {
        ...
        this._breadCrumbsItemsWithoutBackButton =
            pathResult.pathWithoutItemForBackButton;
    }
    ...
    private _getItemFromRootByIndex(index: number): Model | void {
        ...
        if (this._options.parentProperty && this._root !== undefined) {
            if (
                this._items.at(index).get(this._options.parentProperty) ===
                this._root
            ) {
    ...
    private _hasNavigationBySource(
    ...
    private _hasNavigationByConfig(
        navigationConfig: IBaseSourceConfig
    ): boolean {
        return (
            !navigationConfig ||
            (navigationConfig as IIgnoreNavigationConfig).ignoreNavigation !==
                true
        );
    }
    ...
    private _restoreCollapsedGroups(
        ...
        groupUtil
            ...
            .then((restoredCollapsedGroups: TArrayGroupId) => {
                this._collapsedGroups =
                    restoredCollapsedGroups || collapsedGroups;
            });
    }

    private _isOptionChanged(
        propertyName: string,
        newOptions: IControllerOptions
    ): boolean {
        return (
    ...
    private _addFinallyToLoadPromise(promise: Promise<unknown>): void {
        promise.finally(() => {
            this._loadPromise = null;
        });
    }
    ...
    static prepareFilterWithExpandedItems(
        ...
        if (expandedItems?.length && expandedItems?.[0] !== null) {
            resultFilter[parentProperty] = Array.isArray(
    ...
    private static _getSource(
        source: ICrud | ICrudPlus | PrefetchProxy
    ): IData & ICrud {
        let resultSource;
    ...
    private static _getOptions<T extends IControllerOptions>(
        options: T
    ): IControllerOptions {
        const opts = {};
