...
class PageParamsCalculator implements IParamsCalculator {
        ...
        switch (direction) {
            ...
            default: {
                ...
                if (config && config.pageSize) {
                    ...
                    const pageSizeRemainder =
                        config.pageSize % storeParams.pageSize;
                    const pageSizeCoef =
                        (config.pageSize - pageSizeRemainder) /
                        storeParams.pageSize;
                    ...
                    if (isMetaNumber) {
                        maxPage = Math.max(
                            1,
                            Math.ceil(
                                (metaMore as number) / storeParams.pageSize
                            )
                        );
    ...
    hasMoreData(
        ...
        if (direction === 'forward') {
            ...
            if (more === undefined) {
                ...
                if (storeParams.hasMore === false) {
                    ...
                    result =
                        typeof more === 'boolean'
                            ? more
                            : storeParams.nextPage * storeParams.pageSize <
                              more;
                } else {
                    result = !!more;
                }
            ...
            throw new Error(
                'Parameter direction is not defined in hasMoreData call'
            );
        }
    ...
    shiftToEdge(
        ...
        if (direction === 'backward') {
            ...
            if (typeof metaMore === 'number') {
                config.page =
                    Math.ceil(metaMore / store.getState().pageSize) - 1;
                ...
                if ((metaMore / store.getState().pageSize) % 1 > 0) {
                    config.pageSize =
                        store.getState().pageSize * (config.page - 1);
                    config.page = 1;
                }
            } else {
                config.page = -1;
    ...
    private static _validateNavigation(
        ...
        if (hasMoreOption === false) {
            ...
            if (hasMoreValue && typeof hasMoreValue !== 'number') {
                throw new Error(
                    '"more" Parameter has incorrect type. Must be numeric'
                );
            }
        } else {
            // meta.more can be undefined is is not error
            if (hasMoreValue && typeof hasMoreValue !== 'boolean') {
                throw new Error(
                    '"more" Parameter has incorrect type. Must be boolean'
                );
            }
        }
    }
