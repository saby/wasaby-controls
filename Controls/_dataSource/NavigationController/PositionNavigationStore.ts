...
class PositionNavigationStore implements INavigationStore {
    ...
    constructor(cfg: IPositionNavigationStoreOptions) {
        ...
        if (cfg.position !== undefined) {
            this._position =
                cfg.position instanceof Array ? cfg.position : [cfg.position];
        } else {
            // Default value of position
            this._position = [null];
        }
        if (cfg.direction !== undefined) {
            this._direction = PositionNavigationStore.convertDirection(
                cfg.direction
            );
        } else {
            throw new Error(
                'Option direction is undefined in PositionNavigation'
            );
        }
