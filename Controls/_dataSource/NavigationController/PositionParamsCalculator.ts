...
class PositionParamsCalculator implements IParamsCalculator {
    ...
    updateQueryProperties(
        ...
        initialNavConfig: IBasePositionSourceConfig = {}
    ): IPositionNavigationState {
        const storeParams = store.getState();
        ...
        if (storeParams.limit) {
            const loadedCount = listForCurrentStore?.length || list.getCount();
            let limit: number;

            if (direction) {
                limit = storeParams.limit + loadedCount;
            } else {
                // Список перезагружали с сохранением навигации
                // В метод ушёл limit равный кол-ву загруженных записей
                // метод вернул меньше записей (они были удалены или другая причина),
                // но мы limit не обновляем, и он остаётся таким же как и был.
                // Если долго работать на странице, limit подобным образом может вырасти
                // что приведёт к аварии или ошибке в логах.
                // Надо limit актуализировать
                limit =
                    storeParams.limit > loadedCount
                        ? Math.max(initialNavConfig.limit || 0, loadedCount)
                        : storeParams.limit;
            }
