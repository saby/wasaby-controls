...
class PositionParamsCalculator implements IParamsCalculator {
    getQueryParams(
        ...
        addParams.limit =
            config.limit !== undefined ? config.limit : storeParams.limit;

        const queryField = PositionParamsCalculator._resolveField(
            storeParams.field
        );
    ...
    updateQueryProperties(
        ...
        if (
            config &&
            config.position &&
            config.position[0] === EDGE_FORWARD_POSITION
        ) {
        ...
        if (typeof metaMore === 'boolean') {
            if (queryDirection !== 'bothways') {
                ...
                Logger.error(
                    'NavigationController: Wrong type of "more" value. Must be Object'
                );
            }
        } else {
        ...
        if (storeParams.limit && direction) {
            store.setLimit(
                storeParams.limit +
                    (listForCurrentStore?.length || list.getCount())
            );
        }
            ...
            if (list.getCount() && !isIterativeChanged) {
                if (queryDirection !== 'forward') {
                    ...
                    store.setBackwardPosition(
                        PositionParamsCalculator._resolvePosition(
                            edgeElem,
                            queryField
                        )
                    );
                ...
                if (queryDirection !== 'backward') {
                    edgeElem =
                        listForCurrentStore.pop() ||
                        list.at(list.getCount() - 1);
    ...
    shiftToEdge(
        ...
        const queryDirection =
            PositionParamsCalculator._resolveDirectionForEdge(
                direction,
                position,
    ...
    private static _resolveDirection(
        ...
        if (loadDirection === 'forward') {
            ...
            navDirection =
                PositionNavigationStore.convertDirection(optDirection);
        }
        return navDirection;
    }
    ...
    private static _resolvePosition(
        item: Record,
        field: TFieldValue
    ): TPositionValue {
    ...
    private static _resolveDirectionForEdge(
        ...
        if (direction === 'forward' && position[0] === EDGE_FORWARD_POSITION) {
            ...
            direction === 'backward' &&
            position[0] === EDGE_BACKWARD_POSITION
        ) {
