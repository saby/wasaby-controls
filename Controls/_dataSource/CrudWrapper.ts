...
export class CrudWrapper {
    ...
    private readonly _boundPromiseCatchCallback: (
        error: Error
    ) => Promise<null>;
    ...
    query(
        ...
        if (Array.isArray(queryParams)) {
            ...
            queryParams.forEach((queriesParamsItem) => {
                queriesArray.push(
                    CrudWrapper._getQueryInstance(queriesParamsItem)
                );
            });
            if (queriesArray.length > 1) {
                query = queriesArray[0].union.apply(
                    queriesArray[0],
                    queriesArray.slice(1)
                );
            } else {
