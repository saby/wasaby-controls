...
export default class NavigationController {
    ...
    private _navigationStores?: List<INavigationStoresListItem>;
    private _paramsCalculator?: IParamsCalculator;
    ...
    constructor(cfg: INavigationControllerOptions) {
        ...
        this.setNavigationParamsChangedCallback(cfg.navigationParamsChangedCallback);
    ...
    updateOptions(newOptions: INavigationControllerOptions): void {
        if (
            ...
            this._paramsCalculator = undefined;
            this._navigationType = newOptions.navigationType;
            this._navigationConfig = newOptions.navigationConfig;
        }

        this.setNavigationParamsChangedCallback(newOptions.navigationParamsChangedCallback);
    }

    setNavigationParamsChangedCallback(navigationParamsChangedCallback?: Function): void {
        this._navigationParamsChangedCallback = navigationParamsChangedCallback;
    }

    private _deleteUnprocessedStores(processedStores: TKey[]): void {
        const storesToDelete: TKey[] = [];
    ...
    private _getStore(id: TKey): INavigationStore {
        const storeIndex = (
            this._navigationStores as List<INavigationStoresListItem>
        ).getIndexByValue('id', id);
        let resStoreItem: INavigationStoresListItem | void = this._navigationStores?.at(storeIndex);
    ...
    destroy(): void {
        ...
        this._navigationStores = undefined;
        this._paramsCalculator?.destroy();
        this._paramsCalculator = null;
        this._navigationType = undefined;
        this._navigationConfig = undefined;
    }
    ...
    private static _mergeParams(main: IQueryParams, additional: IQueryParams): IQueryParams {
        ...
        if (additional.filter) {
            ...
            resultParams.filter = { ...resultParams.filter };
