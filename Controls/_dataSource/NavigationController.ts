...
class NavigationStoreFactory {
    ...
    static resolve(
        ...
        Logger.error(
            'NavigationController: Undefined navigation source type "' +
                type +
                '"'
        );
        return;
...
export default class NavigationController {
    ...
    constructor(cfg: INavigationControllerOptions) {
        ...
        if (this._navigationType) {
            ...
            Logger.error(
                'NavigationController: navigationType option is undefined'
            );
        }
    }
    ...
    getQueryParams(
        ...
        const navigationQueryConfig = this._getNavigationQueryConfig(
            navigationConfig,
            reset
        );
        const mainQueryParams =
            NavigationController._getMainQueryParams(userQueryParams);
        ...
        return NavigationController._mergeParams(
            mainQueryParams,
            addQueryParams
        );
    }
    ...
    getQueryParamsForHierarchy(
        ...
        const processStoreItem = (storeItem: INavigationStoresListItem) => {
            ...
            if (
                !ids ||
                !ids.length ||
                ArrayUtil.invertTypeIndexOf(ids, storeItem.id) !== -1
            ) {
        ...
        return NavigationController._mergeParamsHierarchical(
            mainQueryParams,
            addQueryParamsArray
        );
    }
    ...
    updateQueryProperties(
        ...
        if (metaMore instanceof RecordSet) {
            metaMore.each((nav: NavigationRecord) => {
                ...
                if (
                    storeId !== null &&
                    (list.getRecordById(storeIdInvertType) ||
                        storeIdInvertType === id)
                ) {
    ...
    shiftToEdge(
        ...
        const calculator = this._getCalculator();
        const store = this._getStore(id);
        return calculator.shiftToEdge(
            store,
            direction,
            shiftMode,
            this._navigationConfig
        );
    }
    // Если id не передан то берется стор для корневого раздела, для которого жесткий id = null
    ...
    private _deleteUnprocessedStores(processedStores: TKey[]): void {
        ...
        storesToDelete.forEach((id) => {
            this._navigationStores.removeAt(
                this._navigationStores.getIndexByValue('id', id)
            );
        });
    }
    ...
    private _getStore(id: TKey): INavigationStore {
        ...
        let resStoreItem: INavigationStoresListItem =
            this._navigationStores.at(storeIndex);

        if (!resStoreItem) {
            resStoreItem = {
                id,
                store: NavigationStoreFactory.resolve(
    ...
    private static _mergeParams(
        main: IQueryParams,
        additional: IQueryParams
    ): IQueryParams {
    ...
    private static _mergeParamsHierarchical(
        ...
        additional.forEach((addItem) => {
            const resultParams = NavigationController._mergeParams(
                { ...main },
                addItem.addParams
            );
            ...
            (resultParams.filter as Record<string, unknown>).__root =
                new applied.PrimaryKey(addItem.id);
    ...
    private static _getMainQueryParams(
        userQueryParams: IQueryParams
    ): IQueryParams {
        return {
