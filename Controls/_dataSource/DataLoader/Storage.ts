...
export class Storage {
    ...
    getList(id?: string): ILoadDataResult {
        if (id) {
            ...
            this.each((storageConfig) => {
                if (
                    !config &&
                    (storageConfig?.type === 'list' || storageConfig.source)
                ) {
    ...
    getSourceController(id?: string): NewSourceController {
        ...
        if (!sourceController) {
            sourceController = config.sourceController =
                this._getSourceController(config);

            if (filterButtonSource || fastFilterSource) {
                sourceController.setFilter(
                    this.getFilterController(id).getFilter()
                );
            }
        }
    ...
    getSearchController(id?: string): Promise<SearchController> {
        ...
        if (!config.searchController) {
            if (!config.searchControllerCreatePromise) {
                config.searchControllerCreatePromise = import(
                    'Controls/search'
                ).then((result) => {
    ...
    protected _getSourceController(
        ...
        if (options.sourceController) {
            ...
            const controllerClass = loadSync<
                typeof import('Controls/dataSource')
            >('Controls/dataSource').NewSourceController;
