...
export default class Input extends Control<IInputOptions> {
    ...
    private _getLastLookupWidth(lookupContainer: HTMLElement): number | void {
        ...
        const inputWrapperRightOffset = inputWrapper
            ? getComputedStyle(inputWrapper).borderRightWidth
            : 0;
        ...
        if (placeholder) {
            ...
            return Math.min(
                ...
                placeholder.getBoundingClientRect().width +
                    ...
                    parseInt(inputWrapperRightOffset, 10)
            );
        }
    }
    ...
    private _resizeHandler(): void {
        ...
        if (!this._options.readOnly && width !== this._width) {
            this._width = width;
            this._requestResize();
        }
    }
    ...
    protected _changeValidationStatus(event: SyntheticEvent, status: string) {
        if (status) {
            this._validationStatus = TValidationStatus.Invalid;
        }
    }

    protected _updateValidationStatus(options) {
