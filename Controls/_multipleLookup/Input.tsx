 ...
 *
 * @remark
 * Отличается от {@link Controls/_lookup/Lookup поля связи} тем, что контрол разделён на несколько полей, для каждого поля выводится своё автодополнение и открывается свой справочник.
 * Ширина выбранных значений будет пропорционально распределена по ширине контрола, чтобы все значения поместились.
 * Полезные ссылки:
 * * {@link /doc/platform/developmentapl/interface-development/controls/input-elements/directory/multi/ руководство разработчика}
 *
 *
...
export default class Input extends Control<IInputOptions> {
    ...
    protected _validationStatus: string = TValidationStatus.Valid;
    protected _width: number;

    protected _beforeMount(options: IInputOptions): void {
        this._setStateFromLookupsOptions(options);
        this._updateValidationStatus(options);
    }
    ...
    _beforeUpdate(newOptions: IInputOptions): void {
        ...
        this._updateValidationStatus(newOptions);
    }
    ...
    protected _selectedKeysChanged(
        ...
        this._validationStatus = TValidationStatus.Valid;
        if (added.length) {
            this._calcSizes();
        } else {
            this._lookupSizes = {};
    ...
    protected _valueChanged(_event: SyntheticEvent, lookupName: string, value: string): void {
        this._setStateAndNotifyEventByOptionName(value, 'value', lookupName);
        this._updateFakeField(lookupName, value);
        this._validationStatus = TValidationStatus.Valid;
    }

    protected _itemsChanged(_event: SyntheticEvent, lookupName: string, items: RecordSet): void {
    ...
    protected _cloneObject(obj: Record<string, unknown>): object {
    ...
    private _getLastLookupWidth(lookupContainer: HTMLElement): number | void {
        ...
        const inputWrapper = lookupContainer.querySelector(
            '.controls-Lookup__fieldWrapper_style-multipleInputNew'
        );
        const inputWrapperRightOffset = inputWrapper
            ? parseInt(getComputedStyle(inputWrapper).borderRightWidth, 10)
            : 0;
        ...
        if (placeholder) {
            ...
            return Math.min(
                ...
                placeholder.getBoundingClientRect().width +
                    showSelectorButton.getBoundingClientRect().width +
                    inputWrapperRightOffset
            );
        }
    }
    ...
    private _resizeHandler(): void {
        ...
        // width > 0 проверка, что lookup находится не скрыт во время ресайза
        // TODO При переписывании на React надо перейти на FreezeContext
        if (!this._options.readOnly && width > 0 && width !== this._width) {
            this._width = width;
            this._requestResize();
        }
    }
    ...
    protected _itemClick(_event: SyntheticEvent, lookupName: string, item: Model): void {
        this._notify('itemClick', [item, lookupName]);
    }

    protected _choose(_event: SyntheticEvent, lookupName: string, item: Model): void {
        this._notify('choose', [item, lookupName]);
    }

    protected _proxyEvent(_event: SyntheticEvent, eventName: string, lookupName: string): unknown {
    ...
    protected _getLastLookupMinWidth(): string {
        const toolbarItemsCount =
            this._options.lookupsOptions[
                this._options.lookupsOptions.length - 1
            ].toolbarItems?.getCount();
        if (toolbarItemsCount) {
            return `controls-MultipleInputNew__lookup_toolbarItems-count-${toolbarItemsCount}`;
        } else if (this._options.lookupsOptions.length === 2) {
            return 'controls-MultipleInputNew__lookup_last_minWith';
        }
        return '';
    }

    protected _changeValidationStatus(_event: SyntheticEvent, status: string) {
        if (status) {
            this._validationStatus = TValidationStatus.Invalid;
        }
    }

    protected _updateValidationStatus(options: IInputOptions) {
        if (options.validationStatus) {
            this._validationStatus = options.validationStatus;
        }
    }

    protected _showSelectorButton(index: any, options: any): boolean {
        if (options.isAdaptive) {
            return !!options.lookupsOptions[index].selectorTemplate;
        } else {
            return (
                options.showSelectButton !== false &&
                this._isLastLookup(options.lookupsOptions, index)
            );
        }
    }

    activateLookup(lookupName: string): void {
        this._children[lookupName].activate();
    }
    ...
    suggestStateChanged(_event: SyntheticEvent, state: boolean): void {
...
/**
 * @event Controls/_multipleLookup/Input#selectedKeysChanged Происходит при изменении выбранных элементов.
 * @param {UICommon/Events:SyntheticEvent} eventObject Дескриптор события.
 * @param {Object} selectedKeys Объект с массивами ключей, выбранных элементов из нескольких справочников.
 */

/**
 * @event Controls/_multipleLookup/Input#lookupDeactivated Происходит при потере фокуса с поля ввода с автодополнением.
 * @param {UICommon/Events:SyntheticEvent} eventObject Дескриптор события.
 * @param {string} lookupName Имя поля выбора из справочника, потерявшего фокус.
 */
