...
function nodeToJson(node: Node) {
    ...
    if (node.nodeType === Node.TEXT_NODE) {
        ...
        // При записи html-строки в innerHTML, в текстовых нодах, строка "&para" конвертируется в знак абзаца "¶";
        // Это нежелательное поведение, поэтому обратно конвертируем знак абзаца в строку "&para";
        const resultNodeValue = nodeValue?.replaceAll('¶', '&para');
