 ...
 */
/* eslint-disable */
import { base64 } from 'Browser/Transport';
...
// По стандарту WWW максимальная валидная длина URL = 2048, но, фактически,
// все современные браузеры способны обрабатывать намного более длинные ссылки. Google Chrome, например, - 505070.
// Берем эвристическое значение.
const linkMaxLenght = 8192;
const protocolLinkPrefixPattern = `(?:${protocolNames.join('|')})`.replace(
    /[a-z]/g,
    (m) => `[${m + m.toUpperCase()}]`
);
const simpleLinkPrefixPattern = `([\\w\\-а-яёА-ЯЁ]{1,${linkMaxLenght}}(?:\\.[a-zA-Zа-яёА-ЯЁ]{1,${linkMaxLenght}}){0,${linkMaxLenght}}\\.([a-zA-Zа-яёА-ЯЁ\-]{1,${linkMaxLenght}})(?::[0-9]{1,${linkMaxLenght}})?)`;
...
const linkPattern = `(${linkPrefixPattern}(?:[^${notAllowLinkChars}]{0,${linkMaxLenght}}))`;
const emailPattern =
    "([\\wа-яёА-ЯЁ!#$%&'*+\\-/=?^`{|}~.]{1," +
    linkMaxLenght +
    '}@[^\\s@()\\ud800-\\udfff]{1,' +
...
function getHrefAfterExtraBracket(linkHref: string): string {
    ...
    for (const [charIndex, char] of Array.from(charsOfLink.entries())) {
        if (CLOSE_BRACKETS.includes(char)) {
            ...
            const lastOpenBracket = currentOpenBrackets[currentOpenBrackets.length - 1].bracket;
 ...
 */
export function removeHrefAfterExtraBracket(link: object, isLinkObject: boolean): [object, string] {
    ...
    if (isFirstChild) {
        ...
        for (let i = parentNode.length - 1; i >= firstChildIndex; --i) {
            ...
            if (isTextNode(nodeToCheck)) {
                ...
                canBeDecorated = findSubNode(stringReplacer, canBeDecorated, nodeToCheck);
                continue;
            }
            ...
            if (tagName === 'a') {
                ...
                attrs[fakeNeedDecorateAttribute] = String(canBeDecorated);
            } else {
...
export function getDecoratedLink(jsonNode: IJSONML): any[] | string {
    ...
    newLinkAttributes.class = (
        (newLinkAttributes.class ? newLinkAttributes.class.replace('asLink', '') + ' ' : '') +
    ...
    const image =
        ...
        getService() +
        '?method=LinkDecorator.DecorateSVG&params=' +
        encodeURIComponent(
            base64.encode(
                '{"Link":"' + newLinkAttributes.href.replace(charsToScreenRegExp, '\\$1') + '"}'
            )
        ) +
        '&id=0&srv=1';
...
export function wrapLinksInString(
    ...
    if (getTagName(parentNode) === 'a') {
        ...
        [result, hasAnyLink] = parseLinks(stringNode, true, needParseFolderLinks);
    }
...
export function parseLinks(
    ...
    while (linkParseExec !== null) {
        ...
        let [match, email, emailDomain, link, simpleLinkPrefix, simpleLinkDomain, ending, noLink] =
            linkParseExec;
        ...
        if (match.length >= linkMaxLenght) {
            ...
            let hasAnyLinkAfterNormalize = false;
            [hasAnyLinkAfterNormalize, nodeToPush] = normalizeLink(
                link,
                simpleLinkDomain,
                ending,
                simpleLinkPrefix,
            ...
            if (hasAnyLinkAfterNormalize && !needToCreateLinkNode) {
                nodeToPush = createLinkObject(nodeToPush as string, match, position, false);
            }

            if (hasAnyLinkAfterNormalize) {
                [nodeToPush, hrefAfterExtraBracket] = removeHrefAfterExtraBracket(
                    nodeToPush,
                    !needToCreateLinkNode
                );
            }

            hasAnyLink = hasAnyLink || hasAnyLinkAfterNormalize;
        } else if (email) {
            ...
            if (hasAnyLink && !needToCreateLinkNode) {
                ...
                nodeToPush = wrongDomain ? match : createLinkNode(email, undefined, true);
            }
        } else {
            nodeToPush = noLink;
        }
    ...
    let hasAnyFolderLink: boolean = false;
    if (needParseFolderLinks) {
        [result, hasAnyFolderLink] = parseFolderLinks(result, needToCreateLinkNode, hasAnyLink);
    }

    return [result, hasAnyLink || hasAnyFolderLink];
}
            ...
            if (typeof element === 'string') {
                ...
                if (folderParseExec !== null) {
                    ...
                    while (folderParseExec !== null) {
                        const isLinkInQuotes = folderParseExec[0].charAt(0) === '"';
    ...
    while (linkParseResult !== null) {
        ...
        if (linkToCheck) {
            ...
            if (isCorrectLink && typeof linkToPush === 'string') {
                const hrefAfterExtraBracket = getHrefAfterExtraBracket(linkToPush);
                if (hrefAfterExtraBracket) {
                    linkToPush = linkToPush.slice(0, -hrefAfterExtraBracket.length);
                }
...
export function normalizeLink(
    ...
    if (needToCreateLinkNode) {
        const resultNode = isWrongDomain
            ? match
            : createLinkNode((linkPrefix ? 'http://' : '') + linkToCheck, linkToCheck);
