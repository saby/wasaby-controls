...
const COMPONENT_QUALIFIER = 'component:';
...
function recursiveMarkup(
    ...
    if (valueToBuild && Array.isArray(valueToBuild)) {
        valueToBuild = parseIframe(parseEmojis(valueToBuild));
    ...
    const nodeName = valueToBuild[0];
    if (
        isString(nodeName) &&
        nodeName.includes(COMPONENT_QUALIFIER) &&
        nodeName !== `${COMPONENT_QUALIFIER}Controls/markup:Decorator`
    ) {
