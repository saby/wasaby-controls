...
export interface IOptions extends IControlOptions {
     ...
     * @remark
     * Маркап декоратор умеет отображать html элементы и контролы.
     * Чтобы отобразить контрол, нужно передать json-элемент с именем в формате "component:ИмяКонтрола". Контрол должен быть предзагружен.
     * @example
     * В примере ниже оторазим параграфы с курсивным и полужирным текстом, и платформенный контрол кнопки.
     *
     * <pre>
     * <!-- Control.wml -->
     * <Controls.markup:Decorator value="{{ _json }}" />
     * </pre>
     *
     * <pre>
     *  protected _json: IJSONML;
     *
     *  protected _beforeMount(): void {
     *    this._json = [
     *      ['p', ['i', 'курсивный текст для примера]],
     *      ['p', ['b', 'полужирный текст для примера']],
     *      ['component:Controls/buttons:Button', { caption: 'Кнопка', viewMode: 'outlined' }]
     *    ];
     * }
     * </pre>
     */
    ...
    /**
     * @name Controls/_markup/Decorator#rootTagName
     * @cfg {String} Имя корневого узла в которое обернется содержимое.
     * @example
     * <pre>
     * <!-- Control.wml -->
     * <Controls.markup:Decorator value="{{ _json }}" rootTagName="div" />
     * </pre>
     */
    rootTagName: string;
     ...
     *
     * ---
     * В примере ниже добавим на теги img допольные атрибуты.
     * <pre>
     * <!-- Control.wml -->
     * <Controls.markup:Decorator value="{{ _json }}" />
     * </pre>
     ...
     * protected _beforeMount(): void {
     *    this._json = [
     *      ['img', {src: 'https://someurl/someimage1.png'}],
     *      ['span', 'текст1'],
     *      ['img', {src: 'https://someurl/someimage2.png', 'aspect-ratio': '1 / 3'}]
     *    ];
     * }
     *
     * protected _tagResolver(json: IJSONML, parent: IJSONML): IJSONML {
     *   if (
     *      JSONML.getNodeName(json) === 'img' &&
     *      !JSONML.getAttribute(json, 'aspect-ratio')
     *   ) {
     *      JSONML.setAttribute(json, 'aspect-ratio', '1 / 3');
     *   }
     *   return json;
     * }
     * </pre>
     ...
     * <pre>
     * <!-- Control.wml -->
     * <Controls.markup:Decorator value="{{ _json }}" resolverParams="{{ _resolverParams }}"/>
     * </pre>
     *
     * <pre>
     * import { JSONML, IJSONML } from 'Types/entity';
     *
     * protected _resolverParams = {
     *   searchedTags: ['span']
     * }
     *
     * protected _beforeMount(): void {
     ...
     * }
     *
     * protected _tagResolver(json: IJSONML, parent: IJSONML, params: object): IJSONML {
     *   if (params?.searchedTags.includes(JSONML.getNodeName(json))) {
     *      return ['p', json];
     *   }
     *   return json;
     * }
     ...
     * <pre>
     * <!-- Control.wml -->
     * <Controls.markup:Decorator value="{{ _json }}" validHtml="{{ _validHtml }}" />
     * </pre>
     *
     * <pre>
     * // Control.js
     * ...
     * protected _beforeMount(): void {
     *    this._json = Converter.htmlToJson(`
     *       <div>
...
class Decorator extends Control<IOptions> {
    ...
    _copyHandler(event: SyntheticEvent<ClipboardEvent>): void {
        ...
        const decoratedLinkNodes = currentTarget.getElementsByClassName(
 ...
 * @class Controls/_markup/Decorator
 * @implements Controls/_markup/IOptions
 * @public
 */
