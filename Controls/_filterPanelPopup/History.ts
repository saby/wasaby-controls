...
import {
    IEditDialogOptions,
    IEditDialogResult,
} from 'Controls/_filterPanelPopup/_EditDialog';
 ...
 * @class Controls/_filterPanelPopup/History
 * @public
...
export default class History extends Control<IHistoryOptions> {
    ...
    protected _itemActionVisibilityCallback(
        ...
        if (item.get('pinned') || item.get('client')) {
            return this._isFavoriteSaveMode
                ? action.id === 'Favorite'
                : action.id === 'PinOff';
        }
        return this._isFavoriteSaveMode
            ? action.id === 'Unfavorite'
            : action.id === 'PinNull';
    }
    ...
    protected _handleHistoryItemClick(
        ...
        this._notify('historyItemClick', [
            historyObject.items || historyObject,
        ]);
    }
    ...
    /**
     * Метод получения идентификатора элемента истории.
     * @param item
     */
    private _getItemName(item: Model | IFilterItem): string {
        let name;
        if (item.hasOwnProperty('id')) {
            name = getPropValue(item, 'id');
        } else {
            name = getPropValue(item, 'name');
        }
        return name;
    }
    ...
    private _getItemText(filterItem: IFilterItem): string {
        ...
        const historyItems = (historyObject.items ||
            historyObject) as IFilterItem[];
        const textArr = [];
        if (historyObject.linkText) {
            textArr.push(historyObject.linkText);
        } else {
            chain.factory(historyItems).each((elem) => {
                ...
                if (
                    !isEqual(value, getPropValue(sourceItem, 'resetValue')) &&
    ...
    private _loadHistoryItems(historyId: string, source: IFilterItem[]) {
        if (historyId) {
            const config = {
                ...
                historyId,
                recent: this._isFavoriteSaveMode
                    ? 'MAX_HISTORY_REPORTS'
                    : 'MAX_HISTORY',
    ...
    private _removeOutdatedFilters(
        ...
        const getOriginalItem = (historyItem: IFilterItem): IFilterItem => {
            return find(source, (origItem) => {
                return (
                    this._getItemName(origItem) ===
                    this._getItemName(historyItem)
                );
            });
        };
        let originalItem;
            ...
            .map((item) => {
                ...
                if (objectData) {
                    ...
                    const historyItems = history.filter((hItem) => {
                        ...
                        hasResetValue =
                            originalItem &&
                            originalItem.hasOwnProperty('resetValue');
                        const checkResult =
                            originalItem &&
                            (!hasResetValue ||
                                (hasResetValue &&
                                    !isEqual(value, originalItem.resetValue)));
                    ...
                    if (objectData.items) {
                        objectData.items = historyText.join(', ')
                            ? historyItems
                            : [];
                    } else {
        ...
        return chain
            ...
            .filter((item) => {
                ...
                if (objectData) {
                    ...
                    const isDuplicated = processedItemsArray.some(
                        (processedItems) => {
                            return this._isItemsEqual(
                                processedItems,
                                filterItems
                            );
                        }
    ...
    private _isItemsEqual(
        ...
        return (
            ...
            filterItems.every((item) => {
                const filterName = this._getItemName(item);
                const compareFilterItem = compareFilterItems?.find(
                    (compareItem) => {
                        return this._getItemName(compareItem) === filterName;
                    }
                );
                return (
                    compareFilterItem &&
                    isEqual(compareFilterItem.value, item.value)
                );
            })
        );
    }
    ...
    private _openEditDialog(item: Model, target: EventTarget): void {
        ...
        const editorTarget = target.closest(
            '.controls-FilterViewPanel__history-editor-container'
        );
        const popupOptions = {
    ...
    private _getEditDialogOptions(item: Model): IEditDialogOptions {
        const savedTextValue = this._getItemText(item);
        ...
        return {
            ...
            isClient:
                history.globalParams === undefined
                    ? !!history.isClient
                    : !!history.globalParams,
    ...
    private _saveFavorite(record: Model): void {
        ...
        this._editItem.set(
            'ObjectData',
            JSON.stringify(ObjectData, new Serializer().serialize)
        );
