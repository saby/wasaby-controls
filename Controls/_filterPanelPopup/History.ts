...
export default class History extends Control<IHistoryOptions> {
    ...
    protected _handleHistoryItemClick(event: SyntheticEvent, filter: Model, params?: object): void {
        const historyObject = getFilterHistoryItemData(filter);
        if (params && !params.isFilterFromHistory) {
            delete historyObject.prefetchParams;
        }
    ...
    private _loadHistoryItems(historyId: string, source: IFilterItem[]) {
        ...
        return Promise.resolve((this._loadedItems = new RecordSet({ rawData: [] })));
    }
