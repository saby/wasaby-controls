...
export default class Sticky extends Control<IStickyPopup> {
    ...
    protected _beforeMount(options: IStickyPopup): void {
        this._initDescriptionAndCallCallbacks(this._cloneItems(options.items), (newItems) => {
    ...
    protected _initDescriptionAndCallCallbacks(
        items: IFilterItem[],
        callback: (newItems: IFilterItem[]) => void
    ): void {
        const currentFilter = getFilterByFilterDescription({}, items);
        initFilterDescription(items, currentFilter, callback);
    }
