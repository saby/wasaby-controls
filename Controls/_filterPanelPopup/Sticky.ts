...
export default class Sticky extends Control<IStickyPopupOptions> {
    ...
    private _validateFilterDescription(
        ...
        filterDescription.forEach((filterItem) => {
            ...
            if (
                (!hasResetValue || (hasResetValue && resetValue === undefined)) &&
                orientation === 'vertical'
            ) {
                Logger.error(
                    `Controls/filterPopup:Sticky для фильтра c именем ${name} указан resetValue как undefined либо вообще не объявлен. Укажите отличное от undefined значение.`
                );
            }
