...
import 'Controls/filterDateRangeEditor';
...
const MAX_ITEMS_COUNT_WITHOUT_HISTORY: number = 5;
...
export default class Sticky extends Control<IStickyPopupOptions> {
    ...
    protected _hasExtendedItems: boolean;
    ...
    protected _historyItems: RecordSet | void;
    ...
    protected _beforeMount(options: IStickyPopupInternalOptions): void {
        ...
        initFilterDescription(
            ...
            (filterDescription) => {
                ...
                this._validateFilterDescription(
                    filterDescription,
                    options.editorsViewMode,
                    options.orientation
                );
    ...
    protected _initStickyFilterPanelStates(options: IStickyPopupInternalOptions): void {
        ...
        this._hasExtendedItems = this._panelController.hasExtendedItems();
    ...
    protected _historyItemClick(
        ...
        this._updateDescriptionAndCallCallbacks(historyItems, (items) => {
            this._applyFilter(items, historyItems, params?.isFilterFromHistory);
        });
    }

    protected _applyFilterHandler(): void {
        this.validate().then((result: IValidateResult | Error) => {
    ...
    protected _applyFilter(
        ...
        this._notify('sendResult', [{ items, history, isFilterFromHistory }], {
            bubbling: true,
        });
    ...
    private _validateFilterDescription(
        ...
        filterDescription.forEach((filterItem) => {
            ...
            if (visibility !== undefined && !filterVisibilityCallback) {
                Logger.error(
                    `Controls/filterPopup:Sticky для фильтра c именем ${name} указана опция visibility. Видимость редактора можно менять только с помощью filterVisibilityCallback`
                );
            }
        });
    }
    ...
    private _updateFilterParams = (newItems: IFilterItem[]): void => {
        ...
        this._applyButtonVisible =
            this._options.applyButtonVisible !== false &&
            (this._options.applyButtonVisible ||
    ...
    }: IStickyPopupOptions): RecordSet | void {
        if (historyId) {
            const history =
                Store.getLocal(historyId) &&
            ...
            return filterHistoryItems(fullFilterDescription || items, history, historyId);
        }
    }
