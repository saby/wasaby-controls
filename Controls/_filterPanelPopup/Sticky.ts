...
export default class Sticky extends Control<IStickyPopup> {
    ...
    protected _sourceChangedHandler(
    ...
    protected _collapsedGroupsChanged(
        ...
        this._notify(
            'sendResult',
            [{ action: 'collapsedFiltersChanged', collapsedFilters }],
            { bubbling: true }
        );
    }
    ...
    private _cloneItems(items: IFilterItem[]): IFilterItem[] {
        return items.map((item) => {
            const newItem = object.clonePlain(item);
            ...
            if (sourceController) {
                ...
                newItem.editorOptions.sourceController =
