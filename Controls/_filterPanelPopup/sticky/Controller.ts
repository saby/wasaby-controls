 ...
 */
import { IFilterItem, getFilterByFilterDescription, FilterDescription } from 'Controls/filter';
...
export default class FilterPanelPopupController {
    ...
    hasBasicItemsOnMount(): boolean {
        return this._items.some((item) => {
            const viewMode = FilterDescription.getFilterItemProperty(item, 'viewMode', 'window');
            const extendedCaption = FilterDescription.getExtendedCaption(item, 'window');
            const { value, resetValue, editorOptions } = item;
    ...
    /**
     * Метод, позволяющий определить наличие редакторов фильтра, которые отображаются в области "Можно отобрать".
     */
    ...
    getHeadingCaption(hasBasicItems: boolean): string {
        return hasBasicItems ? rk('Фильтр') : rk('Можно отобрать');
    }
    ...
    isFilterChanged(): boolean {
        return this._items.some((item) => {
            return FilterDescription.isFilterItemChanged(item) && item.hasOwnProperty('resetValue');
        });
    }
    ...
    _isBasicItem(item: IFilterItem): boolean {
        const { value, resetValue, visibility, filterVisibilityCallback } = item;
        const viewMode = FilterDescription.getFilterItemProperty(item, 'viewMode', 'window');
        const extCaption = FilterDescription.getExtendedCaption(item, 'window');
        const isVisible = !(visibility === false && filterVisibilityCallback);
        return (
            isVisible &&
            (viewMode === 'basic' ||
