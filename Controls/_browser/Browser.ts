...
import type { IFilterItem, THistoryData, IFilterHistoryData } from 'Controls/filter';
import {
...
import { IListConfiguration, IBrowserOptions } from './interface/IBrowser';
import { DataConfigResolver, TDataConfigs } from 'Controls-DataEnv/dataFactory';
...
export default class Browser extends Control<IBrowserOptions, TReceivedState> {
    ...
    protected _contextState: object;
    protected _configs: TDataConfigs;
            ...
            return this._dataLoader
                ...
                .then((result) => {
                    ...
                    if (Browser._checkLoadResult(options, result as IReceivedState[])) {
                        ...
                        return result.map(({ data, historyItems, id }) => {
                            return {
                                id,
                                historyItems,
                                data,
    ...
    private _update(
        ...
        if (searchValueOptionsChanged && searchValueChanged) {
            ...
            if (!newOptions.searchValue && (sourceChanged || rootChanged) && searchController) {
                // resetSavedRootOnSearchReset - просто костыль, чтобы не вызывать поток ошибок по Browser'у
                if (rootChanged && newOptions.resetSavedRootOnSearchReset) {
                    this._getSearchControllerSync().resetSavedRootBeforeSearch();
                }
        ...
        if (isChanged && sourceController.getSource()) {
            ...
            methodResult = this._rootLoadPromise[this._root].loadPromise
                ...
                .catch((error) => {
                    if (!error?.isCanceled) {
                        sourceController.setLoadError(error, sourceController.getRoot());
                    }
                    return error;
                });
    ...
    private _updateContext(options: IBrowserOptions = this._options): void {
        ...
        if (options.listsOptions?.length) {
            this._configs = DataConfigResolver.convertLoadResultsToFactory(this._contextState);
        }
    }
    ...
    private _getDataLoaderOptions(
        ...
        const loadDataConfigs = Browser._getListsOptions(options).map((listOptions, index) => {
            const receivedStateData = receivedState?.find(
                ({ id }, dataIndex) =>
                    id === (listOptions.id ?? options.storeId) || dataIndex === index
            );
            const listOptionsForDataLoader = {
                ...
                items: receivedStateData?.data,
                historyItems: receivedStateData?.historyItems || listOptions.historyItems,
    ...
    private static _getSourceControllerForDataLoader(
        { sourceController }: IBrowserOptions,
