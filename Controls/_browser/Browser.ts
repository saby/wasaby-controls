...
import { IPrefetchOptions } from 'Controls/filter';
...
export default class Browser extends Control<IBrowserOptions, TReceivedState> {
    ...
    private _selectionViewModeChanged(event: SyntheticEvent, type: string, id?: string): void {
    ...
    private _updateFilterController(
        ...
        if (filterController?.update(filterControllerOptions) || filterChanged) {
            ...
            if (
                (filterStructureChanged && !isEqual(currentFilter, this._filter)) ||
    ...
    private _setSelectionViewMode(options: IBrowserOptions): string {
        if (this._operationsPanelExpanded || options.selectionViewMode) {
            const { getSelectionViewMode } = loadSync('Controls/operations');
            const sourceController = this._getSourceController();
            this._selectionViewMode = getSelectionViewMode(
                this._selectionViewMode,
                { ...options, isAllSelected: this._isAllSelected },
                sourceController
            );
        }
    }
    ...
    private _updateContext(options: IBrowserOptions = this._options): void {
        ...
        if (options.listsOptions && options.listsOptions.length && this._contextState && false) {
            options.listsOptions.forEach((listProps) => {
                const listId = listProps.id;
                const oldSelectionViewMode = this._contextState[listId].selectionViewMode;
                const newSelectionViewMode = listProps.selectionViewMode;
                const selectionViewModeChanged = oldSelectionViewMode !== newSelectionViewMode;
                if (selectionViewModeChanged) {
                    dataLoaderState[listId].selectionViewMode = newSelectionViewMode;
                    dataLoaderState[listId].viewMode =
                        newSelectionViewMode === 'selected'
                            ...
                            ? this._viewMode
                            : options.viewMode;
                    this._dataLoader.updateState(listId, dataLoaderState[listId]);
                }
            });
        }
    ...
    protected _listMarkedKeyChangedHandler(event: SyntheticEvent<null>, markedKey: TKey): unknown {
        ...
        this._saveState(this._options, markedKey);
    ...
    _saveState(options = this._options, markedKey?: TKey): void {
        if (this._options.listConfigStoreId) {
            const newState = this._getStateForSave(options, markedKey);
            saveControllerState(this._options.listConfigStoreId, newState);
        }
    }

    _getStateForSave(options: IBrowserOptions, markedKey: TKey): IListSavedState {
        ...
        // saveNavigationState - временная опция, пока не смогут строиться от курсора по задаче
        // https://online.sbis.ru/opendoc.html?guid=99a58d42-5449-4abf-be46-a2c1e4841bb4&client=3
        if (
            navigation &&
            ...
            const item = sourceController.getItems().getRecordById(markedKey);

            (field instanceof Array ? field : [field]).forEach((fieldName) => {
                position.push(item.get(fieldName));
            });

            state.markedKey = markedKey;
            state.navigationSourceConfig = {
                position,
            };
        }
