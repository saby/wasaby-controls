...
export default class Browser extends Control<IBrowserOptions, TReceivedState> {
    ...
    private _setMisspell(items: RecordSet, id: string): void {
        ...
        this._returnedOnlyByMisspellValue =
            this._getSearchControllerSync(
                id
            ).needChangeSearchValueToSwitchedString(items) &&
            !!this._misspellValue;
    }
    ...
    private _handleDataLoad(
        ...
        if (searchController) {
            ...
            if (
                ...
                (searchController.isSearchInProcess() ||
                    newSearchValue !== this._searchValue)
            ) {
    ...
    _getStateForSave(
        ...
        if (navigation && navigation.source === 'position' && markedKey && options.saveNavigationState) {
            const position = [];
