...
export default class BrowserWrapper extends Control<IBrowserContextResolverOptions> {
    ...
    protected _findPrefetchResult(options: IBrowserContextResolverOptions): ILoadDataResult {
        const hasKey = options.hasOwnProperty('storeId') && options.storeId !== undefined;
        ...
        if (hasKey) {
            ...
            Object.values(context)
                ...
                .forEach((value: ILoadDataResult) => {
                    if (
                        !result &&
                        BrowserWrapper._isSameOptionsInPrefetchAndControl(value, options)
                    ) {
                        result = value;
                    }
                });
    ...
    private static _isSameOptionsInPrefetchAndControl(
        ...
        const isSameHistoryId = value.filterController?.getHistoryId() === options.historyId;
        const isSameSource =
            value.sourceController?.getSource() === BrowserWrapper._getOriginSource(options.source);
        const isSameSourceController = value.sourceController === options.sourceController;
        return (isSameHistoryId && isSameSource) || isSameSourceController;
    }

    private static _getOriginSource(source: PrefetchProxy | ICrudPlus): ICrudPlus {
