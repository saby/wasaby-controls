...
export interface IFooterOptions extends IComponentPropsWithReadonly, IConfirmationFooterOptions {}
...
function Footer(props: IFooterOptions): React.ReactElement {
    ...
    if (adaptiveMode.device.isPhone()) {
        ...
        if (countButtons > 1) {
            ...
            const countButtonsSymbols = buttons.reduce(
                (prev, curr) => (prev += curr.caption.length),
                0
            );
            if (
                (countButtons === 2 && countButtonsSymbols >= MAX_COUNT_SYMBOLS_FOR_TWO_BUTTONS) ||
                (countButtons === 3 &&
                    countButtonsSymbols >= MAX_COUNT_SYMBOLS_FOR_THREE_BUTTONS) ||
                countButtons > 3
            ) {
                isVertical = true;
            }
    ...
    return (
        <div
            className={`controls-ConfirmationDialog__buttonContainer${
