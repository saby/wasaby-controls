...
export default function Content(props): React.ReactElement {
    const getMessageTemplate = (): React.ReactElement => {
        ...
        if (typeof props.message === 'function' || props.message?.isWasabyTemplate) {
            Logger.error(
                'Опция message имеет не верный тип, используйте опцию messageContentTemplate для отображения шаблона основного текста диалога подтверждения',
                this
            );
        }
    ...
    const getDetailsTemplate = (): React.ReactElement => {
        ...
        if (typeof props.details === 'function' || props.details?.isWasabyTemplate) {
            Logger.error(
                'Опция details имеет не верный тип, используйте опцию detailsContentTemplate для отображения шаблона дополнительного текста диалога подтверждения',
                this
            );
        }
