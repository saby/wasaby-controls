...
function Template(props: IConfirmationTemplate, ref): React.ReactElement {
    ...
    const getBodyContentTemplate = (): React.ReactElement => {
        if (props.bodyContentTemplate) {
            return (
                props.bodyContentTemplate.isWasabyTemplate ? <props.bodyContentTemplate/> : props.bodyContentTemplate
            );
        }
    };
    ...
    const getFooterContentTemplate = (): React.ReactElement => {
        if (props.footerContentTemplate) {
            return (
                props.bodyContentTemplate.isWasabyTemplate ?
                    <props.footerContentTemplate $wasabyRef={props.$wasabyRef}/> :
                    React.cloneElement(props.footerContentTemplate, {$wasabyRef: props.$wasabyRef})
            );
        }
    };
    ...
    const getCloseButton = (): React.ReactElement => {
        if (props.closeButtonVisible) {
            return (
                <CloseButton viewMode={props.closeButtonViewMode || 'linkButton'}
                             attrs={{className: 'controls-DialogTemplate__close_button controls-DialogTemplate__close_button_size-m'}}
                             onClick={close}
                />
            );
        }
    ...
    return (
        ...
        >
            <div className="controls-ConfirmationTemplate__body">
                ...
                <div
                    className={
                        props.footerContentTemplate && 'controls-ConfirmationTemplate__footer'
                    }
