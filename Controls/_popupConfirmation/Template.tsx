...
function Template(props: IConfirmationTemplate, forwardedRef: React.LegacyRef<HTMLDivElement>): React.ReactElement {
    const isDesignFeature = React.useMemo(() => {
        return Store.get('design-experiments')?.confirmationGradient;
    }, []);
    React.useEffect(() => {
    ...
    const getContentTemplate = (ContentTemplate) => {
        if (ContentTemplate) {
            return ContentTemplate.isWasabyTemplate ||
            typeof ContentTemplate === 'function' ? <ContentTemplate/> :
        ...
        if (props.closeButtonVisible) {
            return (
                <CloseButton viewMode={props.closeButtonViewMode || 'linkButton'}
                             className='controls-DialogTemplate__close_button controls-DialogTemplate__close_button_size-m'
                             onClick={close}
                />
            );
        }
