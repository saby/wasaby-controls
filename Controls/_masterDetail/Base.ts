...
import { Register, RegisterUtil, UnregisterUtil } from 'Controls/event';
...
interface IMasterDetail extends IControlOptions, IPropStorageOptions, IMasterWidth {
    ...
    designContextValue?: IContextOptionsValue;
    newDesign?: boolean;
    restricted?: boolean;
    isAdaptive?: boolean;
...
class Base extends Control<IMasterDetail, string> {
    ...
    protected _virtualNavigationEnabled: boolean = false;
    ...
    protected _beforeMount(
        ...
        this._newDesign = options.designContextValue?.newDesign || options.newDesign;
    ...
    private _prepareLimitSizes(options: IMasterDetail): void {
        ...
        if (this._isPercentValue(options.masterMaxWidth)) {
            ...
            this._currentMaxWidth = this._getWidthByType(options.masterMaxWidth);
        }
        ...
        if (this._isPercentValue(options.masterMinWidth)) {
            ...
            this._currentMinWidth = this._getWidthByType(options.masterMinWidth);
        }
    }
    ...
    protected _setCurrentWidthByValue(
        ...
        if (
            ...
            const masterRecalculatedWidth = this._getWidthValueInPercents(initialMasterWidth);
    ...
    protected _afterMount(options: IMasterDetail): void {
        ...
        if (options.propStorageId && options.initialMasterWidth && !options.isAdaptive) {
            this._setCurrentWidthByValue(options.initialMasterWidth, options.masterWidth);
        }
        if (options.task1188709641) {
            this._updateOffset(options);
        }

        if (this._isMasterFixedWithoutScrollTopValue(options)) {
    ...
    protected _getMasterStyle(
        ...
        if (this._container && document) {
            ...
            let height;
            if (this._virtualNavigationEnabled) {
    ...
    protected _enableVirtualNavigationHandler(
        ...
        if (position === 'top') {
            this._virtualNavigationEnabled = true;
        }
    }
    ...
    protected _disableVirtualNavigationHandler(
        ...
        if (position === 'top') {
            this._virtualNavigationEnabled = false;
        }
    }

    private _isMasterFixed({
        scrollTop,
        scrollOffsetTop,
        masterOffsetTop,
    ...
    private _isMasterFixedWithoutScrollTopValue(options: IMasterDetail): boolean {
        return this._isMasterFixed(options) && options.scrollTop === undefined;
    }

    private _scrollHandler(event: SyntheticEvent, scrollTop: number): void {
    ...
    private _updateOffset(options: IMasterDetail): void {
        if (
            ...
            const currentMasterWidth = parseFloat(this._currentWidth)
                ? this._currentWidth
                : options.masterWidth;
            const isMasterWidthInPercents = this._isPercentValue(currentMasterWidth);
            ...
            this._detailWidth = Math.max(
                this._getContainerWidth() ? this._getContainerWidth() - masterWidth : 0,
                0
            );
    ...
    private _changeOffset(offset: number): void {
        const isMasterWidthInPercents = this._isPercentValue(this._currentWidth);
    ...
    private _getWidthValueInPercents(value: string | number): string {
        const MaxPercent: number = 100;
        const intValue: number = parseFloat(String(value));
        const calculatedWidth: number = (intValue / this._getContainerWidth()) * MaxPercent;
    ...
    protected _resizeHandler(): void {
        ...
        if (!this._container.closest('.ws-hidden')) {
            if (this._options.propStorageId && !this._options.isAdaptive) {
            ...
            if (this._isMasterFixedWithoutScrollTopValue(this._options)) {
