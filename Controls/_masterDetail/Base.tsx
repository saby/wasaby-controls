...
interface IMasterDetailState {
    ...
    savedWidth?: number | string; // Защита от множ. вызова БЛ
}
...
class Base extends React.Component<IMasterDetail, IMasterDetailState> {
    ...
    constructor(props = {} as IMasterDetail) {
        ...
        const initMaxSizes = this.getPreparedLimitSizes(props);

        let currentWidth: string;
        if (props.propStorageId) {
            if (props.initialMasterWidth) {
                currentWidth = this.getInitCurrentWidth(props.initialMasterWidth);
            } else {
                this.getSettings(props).then((storage: object) => {
                    this.updateSizesByPropStorageId(storage, props);
                    this.setState(this.getPreparedLimitSizes(props));
                });
            }
        } else {
            currentWidth = this.getInitCurrentWidth(props.masterWidth);
        }

        this.state = {
            adaptiveMasterVisible: !props.isAdaptive,
            canResizing: this.isCanResizing(props),
            virtualNavigationEnabled: false,
            masterFixed: this.isMasterFixed(props),
            scrollTop: 0,
            currentWidth,
            ...initMaxSizes,
        };
    ...
    componentDidMount(): void {
        this.setState({
            prevCurrentWidth: this.state.currentWidth,
        });
        ...
        if (this.props.propStorageId && this.props.initialMasterWidth && !this.props.isAdaptive) {
            this.setCurrentWidthByValue(this.props.initialMasterWidth, this.props.masterWidth);
        }
        if (this.props.task1188709641) {
            this.updateOffset(this.props, this.state);
        }
    ...
    shouldComponentUpdate(options: IMasterDetail): boolean {
        let newState = { ...this.state };

        newState.masterFixed = this.isMasterFixed(options);

        // Если изменилась текущая ширина, то сбросим состояние, иначе работаем с тем, что выставил пользователь
        if (options.masterWidth !== this.props.masterWidth) {
            newState.currentWidth = null;
        }

        if (this.isSizeOptionsChanged(options, this.props)) {
            newState.canResizing = this.isCanResizing(options);
            newState = { ...newState, ...this.getPreparedLimitSizes(options) };
        ...
        if (options.propStorageId !== this.props.propStorageId) {
            this.getSettings(options).then((storage) => {
                this.updateSizesByPropStorageId(storage, options);
            });
        }
    ...
    componentDidUpdate(prevProps?: IMasterDetail): void {
        ...
        if (this.state.prevCurrentWidth !== this.state.currentWidth) {
            this.setState({ prevCurrentWidth: this.state.currentWidth });
            this.startResizeRegister();
        }
    }
    ...
    setSettings = (width: number | string | undefined): void => {
        ...
        if (propStorageId) {
            if (this.state.savedWidth !== width) {
                const newSavedWidth = this.getWidthByType(width);

                this.setState({
                    savedWidth: newSavedWidth,
                });

                setSettings({ [propStorageId]: newSavedWidth });
            }
        }
    };

    getPreparedLimitSizes = (
        options: IMasterDetail
    ): { currentMaxWidth?: string; currentMinWidth?: string } => {
        ...
        if (this.isPercentValue(options.masterMaxWidth)) {
            newMaxSizes.currentMaxWidth = options.masterMaxWidth as string;
        } else if (options.masterMaxWidth !== undefined) {
            newMaxSizes.currentMaxWidth = this.getWidthByType(options.masterMaxWidth);
        }

        if (this.isPercentValue(options.masterMinWidth)) {
            newMaxSizes.currentMinWidth = options.masterMinWidth as string;
        } else if (options.masterMinWidth !== undefined) {
            newMaxSizes.currentMinWidth = this.getWidthByType(options.masterMinWidth);
        }

        return newMaxSizes;
    };

    getInitCurrentWidth = (width: string | number): string => {
        ...
        return this.getWidthByType(width);
    };

    setCurrentWidthByValue = (
        initialMasterWidth: string | number,
        masterWidth: string | number
    ): void => {
        if (
            this.isPercentValue(masterWidth) &&
            !this.isPercentValue(initialMasterWidth) &&
            this.getContainerWidth()
        ) {
            const masterRecalculatedWidth = this.getWidthValueInPercents(initialMasterWidth);
    ...
    getWidthByType = (width: string | number | undefined): string => {
        if (this.isPercentValue(width)) {
            return String(width);
        }
        if (width !== undefined) {
    ...
    isMasterFixed = ({ scrollTop, scrollOffsetTop, masterOffsetTop }: IMasterDetail): boolean => {
        return (
            scrollTop !== undefined ||
    ...
    isMasterFixedWithoutScrollTopValue = (options: IMasterDetail): boolean => {
        return this.isMasterFixed(options) && options.scrollTop === undefined;
    };
    ...
    updateSizesByPropStorageId = (storage: object, options: IMasterDetail): void => {
        ...
        if (width) {
            const newState: Partial<IMasterDetailState> = {
                currentWidth: this.getWidthByType(width),
                savedWidth: width,
            };
            this.updateOffset(options, { ...this.state, ...newState });
        } else {
            this.setState({
                currentWidth: this.getInitCurrentWidth(options.masterWidth),
            });
        }
    };
    ...
    isSizeOptionsChanged = (oldOptions: IMasterDetail, newOptions: IMasterDetail): boolean => {
        return (
    ...
    updateOffset = (options: IMasterDetail, state: IMasterDetailState): void => {
        if (
            ...
            const currentMasterWidth = parseFloat(state.currentWidth)
                ? state.currentWidth
                : options.masterWidth;
            const isMasterWidthInPercents = this.isPercentValue(currentMasterWidth);
            let currentWidth = this.getOffsetValue(currentMasterWidth);
            newState.currentWidth = this.getWidthByType(currentMasterWidth);

            const masterWidth = options.masterVisibility === 'visible' ? currentWidth : 0;
            newState.detailWidth = Math.max(
                this.getContainerWidth() ? this.getContainerWidth() - masterWidth : 0,
                0
            );
            ...
            if (this.getContainerWidth() || !this.isPercentValue(options.masterMaxWidth)) {
                newState.maxOffset = this.getOffsetValue(options.masterMaxWidth) - currentWidth;
            ...
            // Пересчитаем после маунта в px, чтобы работало движение
            if (this.getContainerWidth() || !this.isPercentValue(options.masterMinWidth)) {
                newState.minOffset = currentWidth - this.getOffsetValue(options.masterMinWidth);
                // Protect against window resize
    ...
    isCanResizing = (options: IMasterDetail): boolean => {
        if (
            options.propStorageId &&
            (!options.masterWidth || !options.masterMaxWidth || !options.masterMinWidth)
        ) {
            Logger.warn(
    ...
    changeOffset = (offset: number): void => {
        ...
        const isMasterWidthInPercents = this.isPercentValue(newState.currentWidth);
        ...
        newState.currentWidth = isMasterWidthInPercents
            ...
            : newOffsetValue + 'px';
        this.updateOffset(this.props, newState);
        this.onMasterWidthChanged(newState.currentWidth);
        this.setSettings(newState.currentWidth);
    };

    isPercentValue = (value: string | number | undefined): boolean => {
    ...
    getContainerWidth = (): number => {
        ...
        if (!this.state.containerWidth) {
            containerWidth = this._container ? this._container.getBoundingClientRect().width : 0;
    ...
    adaptiveMasterVisibleChanged = (value: boolean) => {
    ...
    getMasterDetailMasterStyles = () => {
        ...
        if (this.state.masterFixed) {
            ...
            masterDetailMasterStyles.top = this.props.masterOffsetTop + 'px;';
        ...
        if (!this.props.isAdaptive) {
            masterDetailMasterStyles.width = this.state.currentWidth;
