...
import { Listener, Register, RegisterClass } from 'Controls/event';
import { getSettings, setSettings, getSettingsSync } from 'Controls/Application/SettingsController';
import {
    IComponentProps,
    IComponentTheme,
    IPropStorageOptions,
...
import {
    SHADOW_MODE as ScrollContainerShadowMode,
    SHADOW_VISIBILITY,
...
const ToggleButton = lazy(() => importer('Controls/ToggleButton'));
...
export interface IMasterDetail
    ...
    masterScrollBottomShadowVisibility?: SHADOW_VISIBILITY;
    masterScrollShadowMode?: ScrollContainerShadowMode;
    masterScrollBackgroundStyle?: TBackgroundStyle;
...
function getMasterWidthNumberValue(masterWidth: number | string | undefined): number | undefined {
    if (masterWidth && typeof masterWidth === 'string' && !isPercentValue(masterWidth)) {
        return Number(masterWidth.replace(/px$/, ''));
    }
    return masterWidth;
}
...
class Base extends React.Component<IMasterDetail, IMasterDetailState> {
    ...
    /**
     * Функция-обработчик изменения ширины мастера в двухколоночном реестре.
     * @typedef {Function} Controls/_masterDetail/Base/TOnMasterWidthChanged
     * @param {UI/Events:SyntheticEvent} event Событие изменения ширины.
     * @param {String} newWidth Обновленное значение ширины мастера.
     */

    /**
     * @name Controls/_masterDetail/Base#onMasterWidthChanged
     * @cfg {Controls/_masterDetail/Base/TOnMasterWidthChanged} Функция-callback, которая будет вызвана при изменении ширины мастера.
     */

    /**
     * @name Controls/_masterDetail/Base#masterProps
     * @cfg {Object} Опции для настройки контента в мастере.
     */

    /**
     * @name Controls/_masterDetail/Base#detailProps
     * @cfg {Object} Опции для настройки контента в детейле.
     */
    ...
    private _resizeDetailRegister?: RegisterClass;
    private _resizeMasterRegister?: RegisterClass;
    private _instanceId: string = 'inst_' + Guid.create();

    // TODO: dndCustomMouseLeaveDetail и dndCustomMouseEnterMaster костыли
    ...
    private dndCustomMouseLeaveDetail: RegisterClass;
    private dndCustomMouseEnterMaster: RegisterClass;
    ...
    constructor(props = {} as IMasterDetail) {
        ...
        const masterContentMaxHeight = this.getMasterMaxHeight(
        ...
        this.state = {
            ...
            masterMaxHeight: masterContentMaxHeight,
            currentWidth: this.initCurrentWidth(props),
            initialMasterWidth: props.initialMasterWidth,
            masterWidth: props.masterWidth,
            ...initMaxSizes,
        };

        this._registerMasterResizeHandler = this._registerMasterResizeHandler.bind(this);
        this._registerDetailResizeHandler = this._registerDetailResizeHandler.bind(this);
        this._registerDndMasterHandler = this._registerDndMasterHandler.bind(this);
        this._registerDndDetailHandler = this._registerDndDetailHandler.bind(this);
        this._container = null;
    }

    private initCurrentWidth = (props: IMasterDetail) => {
        const propStorageId = props.propStorageId;
        const masterWidth = props.masterWidth;
        const state = this.state;
        if (!propStorageId) {
            return getInitCurrentWidth(masterWidth, state);
        }
        const initialMasterWidth = props.initialMasterWidth;
        if (initialMasterWidth) {
            return getInitCurrentWidth(initialMasterWidth, state);
        }
        const settings = getSettingsSync([propStorageId]);
        if (settings) {
            const settingsWidth = settings[propStorageId] || masterWidth;
            return getInitCurrentWidth(settingsWidth, state);
        }
    ...
    componentWillUnmount(): void {
        ...
        this._resizeMasterRegister?.destroy();
        this._resizeMasterRegister = undefined;
        this._resizeDetailRegister?.destroy();
        this._resizeDetailRegister = undefined;
    }
    ...
    scrollHandler = (event: SyntheticEvent, scrollTop: number): void => {
        if (this.props.masterVisibility !== 'hidden') {
            ...
            if (masterContentMaxHeight !== this.state.masterMaxHeight) {
                this.setState({ masterMaxHeight: masterContentMaxHeight });
            }
        }
    };
    ...
    startResizeRegister = (): void => {
        ...
        if (this.props.masterVisibility !== 'hidden') {
            this._resizeMasterRegister?.start(new SyntheticEvent(null, eventCfg));
        }
        this._resizeDetailRegister?.start(new SyntheticEvent(null, eventCfg));
    };
    ...
    getMasterCurrentWidth(props: IMasterDetail, state: IMasterDetailState): number {
        const width = this.getOffsetValue(
            parseFloat(state.currentWidth) ? state.currentWidth : props.masterWidth
        );

        if (state.currentMaxWidth !== undefined) {
            const masterMaxWidth = this.getOffsetValue(state.currentMaxWidth);

            if (width > masterMaxWidth) {
                return masterMaxWidth;
            }
        }
        return width;
    }
    ...
    getMasterTemplateContent = () => {
        ...
        const className = clsx(
            ...
            this.props.masterPosition === 'right' && 'controls-MasterDetail_outerPadding-reset'
        );
        ...
        const masterTemplateProps: Record<string, any> = {
            ...
            masterWidth: getMasterWidthNumberValue(this.state.currentWidth),
            shadowMode: 'js',
    ...
    getMasterDetailMasterStyles = () => {
        ...
        if (this.state.masterFixed) {
            ...
            masterDetailMasterStyles.maxHeight = this.state.masterMaxHeight;
        }
        ...
        const className = clsx(
            ...
            this.props.contrastBackground && this.props.detailContrastBackground
                ? [
                      ...
                      `controls-MasterDetail_details_container${
                          this.props.isAdaptive ? '_adaptive' : ''
                      }`,
    ...
    private _registerDetailResizeHandler(...args: Parameters<RegisterClass['register']>): void {
        if (!this._resizeDetailRegister) {
            this._resizeDetailRegister = new RegisterClass({ register: 'controlResize' });
        }
        this._resizeDetailRegister.register(...args);
        __notifyFromReact(this._container, 'register', [...args.slice(1)], { bubbling: true });
    }

    private _registerDndMasterHandler(...args: Parameters<RegisterClass['register']>): void {
        if (!this.dndCustomMouseEnterMaster) {
            this.dndCustomMouseEnterMaster = new RegisterClass({
                register: 'dndCustomMouseEnterMaster',
            });
        }
        this.dndCustomMouseEnterMaster.register(...args);
    }

    private _registerDndDetailHandler(...args: Parameters<RegisterClass['register']>): void {
        if (!this.dndCustomMouseLeaveDetail) {
            this.dndCustomMouseLeaveDetail = new RegisterClass({
                register: 'dndCustomMouseLeaveDetail',
            });
        }
        this.dndCustomMouseLeaveDetail.register(...args);
    }

    private _registerMasterResizeHandler(...args: Parameters<RegisterClass['register']>): void {
        if (!this._resizeMasterRegister) {
            this._resizeMasterRegister = new RegisterClass({ register: 'controlResize' });
        }
        this._resizeMasterRegister.register(...args);
        __notifyFromReact(this._container, 'register', [...args.slice(1)], { bubbling: true });
    }

    customMouseMoveMasterHandler = () => {
        this.dndCustomMouseLeaveDetail?.start?.();
    };

    customMouseEnterMasterHandler = () => {
        this.dndCustomMouseEnterMaster?.start?.();
    };

    render() {
        const props = this.props;
        const state = this.state;
        const { isAdaptive, adaptiveContent, masterPosition, detailHeaderTemplate } = props;
        const { masterMaxHeight, adaptiveMasterVisible } = state;
        ...
        return (
            <div
                ...
                className={clsx(
                    ...
                    `controls-MasterDetail_masterPosition-${masterPosition}`,
                    isAdaptive && 'controls-MasterDetail_isAdaptive',
                    'ws-flexbox',
                    `controls_list_theme-${props.theme}`,
                    props.restricted && 'controls-MasterDetail-restricted',
                    props.className
                )}
            >
                {props.masterVisibility !== 'hidden' ? (
                    <EventSubscriber
                        fakeControl={true}
                        onRegisterDndMaster={this._registerDndMasterHandler}
                    >
                        <div
                            className={clsx(
                                ...
                                isAdaptive
                                    ...
                                    : [
                                          `controls-MasterDetail_master-${
                                              props.masterContrastBackground
                                                  ? 'contrast'
                                                  : 'transparent'
                                          }`,
                            ...
                            {state.masterFixed ? (
                                ...
                                >
                                    <div
                                        style={{
                                            display: 'contents',
                                            maxHeight: masterMaxHeight,
                                        }}
                                    >
                                        <ScrollContainer
                                            className="controls-MasterDetail_master-content"
                                            backgroundStyle={
                                                props.masterScrollBackgroundStyle || 'master'
                                            }
                                            scrollbarVisible={props.masterScrollbarVisible}
                                            bottomShadowVisibility={
                                                props.masterScrollBottomShadowVisibility
                                            }
                                            shadowMode={props.masterScrollShadowMode}
                                            attrs={{
                                ...
                                <>
                                    {isAdaptive && !adaptiveContent ? (
                                        ...
                                        >
                                            ...
                                            >
                                                <ToggleButton
                                                    icons={[
                                                        'icon-TFFolder2Opened',
                                                        'icon-TFFolder2',
                                                    ]}
                                                    viewMode="link"
                                                    data-qa="controls-Adaptive__FolderButton"
                                                    value={adaptiveMasterVisible}
                                                    onValueChanged={
                                                        this.adaptiveMasterVisibleChanged
                                                    }
                                                />
                                            </div>
                                        </Register>
                                    ...
                                    {props.master &&
                                    (!isAdaptive || adaptiveContent || adaptiveMasterVisible) ? (
                                        <EventSubscriber
                                            onRegister={this._registerMasterResizeHandler}
                                            onSelectedMasterValueChanged={
                ...
                {state.canResizing && !isMasterWidthEqualAuto(props.masterWidth) ? (
                    <div
                        className={`controls-MasterDetail_resizing-line_wrapper controls-MasterDetail_resizingLine_masterPosition-${masterPosition}`}
                    >
                        <ResizingLine
                            ref={this.registerResizingLine}
                            className={`controls-MasterDetail_resizing-line controls-MasterDetail_resizing-line_position-${masterPosition}`}
                            data-qa="controls-MasterDetail_resizing-line"
                            minOffset={state.minOffset}
                            maxOffset={state.maxOffset}
                            direction={masterPosition === 'left' ? 'direct' : 'reverse'}
                ...
                <EventSubscriber
                    onRegister={this._registerDetailResizeHandler}
                    onRegisterDndDetail={this._registerDndDetailHandler}
                    onDndCustomMouseEnterMaster={this.customMouseEnterMasterHandler}
                >
                    {/* TODO: https://online.sbis.ru/opendoc.html?guid=23071290-e50a-4b7c-a60a-ff951133c7d7&client=3  */}
                    <AdaptiveContainer width={undefined}>
                        <div
                            className={clsx(
                                'controls-MasterDetail_details',
                                `controls-MasterDetail_details-${
                                    props.detailContrastBackground ? 'contrast' : 'transparent'
                                }${isAdaptive ? '-adaptive' : ''}`,
                                'ws-flexbox',
                                'ws-flex-grow-1',
                ...
                <Listener event="controlResize" onControlResize={this.resizeHandler} />
            </div>
        );
    }
...
// Хук генерации propStorageId, если он не передан явно.
function usePropStorageId(
    props: Pick<IMasterDetail, 'propStorageId'>
): IMasterDetail['propStorageId'] {
    const router = useRouter();
    const popupContext = React.useContext(PopupContext);
    const isPopup = popupContext?.isPopup;
    const givenPropStorageId = props.propStorageId;

    const propStorageId = React.useMemo(() => {
        if (givenPropStorageId !== undefined || isPopup || !router) {
            return givenPropStorageId;
        }
        const { pageId, traverse } = router.maskResolver.calculateUrlParams(
            `${PAGE_ID_PREFIX}/:pageId/*traverse`,
            router.url.getStateUrl()
        );
        if (pageId) {
            return `MasDet_${pageId}${traverse ? '/' + traverse : ''}`;
        }
    }, [givenPropStorageId, router, isPopup]);

    return propStorageId;
}
...
export default React.forwardRef((props: Omit<IMasterDetail, 'forwardedRef'>, ref) => {
    ...
    const propStorageId = usePropStorageId(props);
    return (
        <Base
            {...props}
            theme={theme}
            isAdaptive={props.isAdaptive ?? isAdaptive}
