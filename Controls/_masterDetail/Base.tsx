...
function isMasterFixed({ scrollTop, scrollOffsetTop, masterOffsetTop }: IMasterDetail): boolean {
    return (
        scrollTop !== undefined || (scrollOffsetTop !== undefined && masterOffsetTop !== undefined)
    );
}

function isSizeOptionsChanged(oldOptions: IMasterDetail, newOptions: IMasterDetail): boolean {
    return (
...
class Base extends React.Component<IMasterDetail, IMasterDetailState> {
    ...
    shouldComponentUpdate(options: IMasterDetail): boolean {
        ...
        newState.masterFixed = isMasterFixed(options);
    ...
    getPreparedLimitSizes = (
        ...
        if (isPercentValue(options.masterMaxWidth)) {
            newMaxSizes.currentMaxWidth = options.masterMaxWidth as string;
        } else if (options.masterMaxWidth !== undefined) {
            newMaxSizes.currentMaxWidth = getWidthByType(options.masterMaxWidth);
        }

        if (isPercentValue(options.masterMinWidth)) {
            newMaxSizes.currentMinWidth = options.masterMinWidth as string;
        } else if (options.masterMinWidth !== undefined) {
            newMaxSizes.currentMinWidth = getWidthByType(options.masterMinWidth);
        }
    ...
    static getDerivedStateFromProps(
        { initialMasterWidth }: IMasterDetail,
        state: IMasterDetailState
    ): IMasterDetailState | null {
        if (initialMasterWidth) {
            if (initialMasterWidth !== state.initialMasterWidth) {
                return {
                    ...state,
