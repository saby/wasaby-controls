...
class Base extends React.Component<IMasterDetail, IMasterDetailState> {
    ...
    shouldComponentUpdate(options: IMasterDetail): boolean {
        const initialMasterWidth = options.initialMasterWidth;
        ...
        if (initialMasterWidth && initialMasterWidth !== this.props.initialMasterWidth) {
            newState.currentWidth = this.getInitCurrentWidth(initialMasterWidth);
        }
    ...
    setSettings = (width: number | string | undefined): void => {
        ...
        if (propStorageId) {
            if (this._savedWidth !== width) {
                ...
                //@ts-ignore
                setSettings({ [propStorageId]: this._savedWidth }, this.props.ViewSettingsController);
            }
        }
    };
    ...
    private _setPercentWidth = (): void => {
        ...
        if (
            ...
            const masterCalculatedMaxWidth = this.isPercentValue(masterMaxWidth)
                ? masterMaxWidth
    ...
    updateOffset = (options: IMasterDetail, state: IMasterDetailState): IMasterDetailState => {
        ...
        if (
            ...
            const reCalcMaxOffset = containerWidth || !this.isPercentValue(masterMaxWidth);
            if (reCalcMaxOffset) {
            ...
            // Пересчитаем после маунта в px, чтобы работало движение
            if (containerWidth || !this.isPercentValue(masterMinWidth)) {
