...
const CompoundArea = CompoundContainer.extend([InstantiableMixin, LikeWindowMixin], {
    ...
    _close() {
        // В старых окнах прикладники ожидают, что до них долетит _notify('close'),
        // они остановят событие и выполнят свою логику.
        // Если не подписаться на on:close, то шаблон окна начнет работать через контексты.
        // Т.к. доступа к шаблону внутри CompoundArea у прикладников нет, сделаем пустую подписку сами.
    },
    ...
    reload() {
        if (this._popupOptions) {
            ...
            if (popupCfg && this._popupOptions.minWidth && this._popupOptions.maxWidth) {
                ...
                Controller.getController().update(popupCfg.id, popupCfg.popupOptions);
            }
        }
        this._removeCustomHeader();
        this.rebuildChildControl();
    ...
    _toggleVisible(visible) {
        ...
        if (popupConfig) {
            ...
            if (popupConfig.popupOptions.modal) {
                ...
                Controller.getController()?.getPopupItems()._nextVersion();
                Controller.getController()?.update(id, popupConfig.popupOptions);
            }
    ...
    setOffset(newOffset) {
        ...
        if (popupConfig) {
            ...
            Controller.getController().update(this._getPopupId(), popupConfig.popupOptions);
        }
    },
    _getManagerConfig() {
        const id = this._getPopupId();
