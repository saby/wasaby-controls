...
const CompoundArea = CompoundContainer.extend(
    [InstantiableMixin, LikeWindowMixin],
    {
        _template: template,
       ...
       */
        ...
        _beforeUpdate(popupOptions): void {
            ...
            if (popupOptions.canMaximize) {
                const maximized = this.getContainer().hasClass(
                ...
                if (popupOptions.maximizeButtonVisibility) {
                    ...
                    this._className = this._className.replace(
                        maximizedButtonClass,
                        ''
                    );
                }
                this.getContainer().toggleClass(
                ...
                if (templateComponent && maximized !== popupOptions.maximized) {
                    ...
                    templateComponent._notify(
                        'onChangeMaximizeState',
                        popupOptions.maximized
                    );
                    templateComponent._options.isPanelMaximized =
                        popupOptions.maximized;
                    this._notify(
                        'onChangeMaximizeState',
                        popupOptions.maximized
        ...
        _changeMaximizedMode(event) {
            ...
            const state = this.getContainer().hasClass(
                'ws-float-area-maximized-mode'
            );
        ...
        rebuildChildControl() {
            ...
            this.once('onInit', () => {
                ...
                EnvEvent.Bus.globalChannel().notify(
                    'onFloatAreaCreating',
                    this
                );
            ...
            const rebuildDeferred =
                CompoundArea.superclass.rebuildChildControl.apply(
                    this,
                    arguments
                );
        ...
        _notifyManagerPopupCreated(): void {
            ...
            EventBus.channel('popupManager').notify(
                'managerPopupCreated',
                item,
                popupItems
            );
        ...
        _fixIos() {
            ...
            let container =
                this._childControl && this._childControl.getContainer();
        ...
        _getReadyDeferred() {
            if (this._childControl.getReadyDeferred) {
                ...
                if (
                    cInstance.instanceOfModule(def, 'Types/deferred:Deferred') &&
                    !def.isReady()
                ) {
                    def.addCallback(() => {
        ...
        _afterMount(cfg) {
            ...
            this._parent =
                this._options.__parentFromCfg || this._options._logicParent;
            ...
            this.detectNextActiveChildControl =
        ...
        _beforeUnmount() {
            if (
                ...
                !this._beforeClosePendingDeferred.isReady()
            ) {
        ...
        isOpened(): boolean {
            if (!this._options.autoShow) {
                const popupContainer =
                    this._container.closest('.controls-Popup');
                if (popupContainer) {
                    return !(
                        popupContainer.classList.contains(
                            popupHiddenClasses[0]
                        ) ||
        ...
        _isValidTarget(target) {
            ...
            if (!isValid) {
                Logger.error(
                    this._moduleName,
                    'Передано некорректное значение опции target',
                    this
                );
            }
            return isValid;
                ...
                trackElement(target)
                    ...
                    .subscribe('onVisible', (event, visibility) => {
                        if (!this.isDestroyed() && !visibility) {
                            ...
                            const parentVdomPopup = $(
                                this._options.target
                            ).closest('.controls-Popup');
                            const hasClass =
                                parentVdomPopup.hasClass(
                                    popupHiddenClasses[0]
                                ) ||
        ...
        _isIosKeyboardVisible(): boolean {
            const isVisible =
                constants.browser.isMobileIOS && window.scrollY > 0;
            if (isVisible) {
        ...
        _updateCustomToolbarCloseButton(content): void {
            ...
            if (menuButton.length) {
                const isCompound = menuButton.hasClass(
                    'compoundarea-processed'
                ); // Чтобы не подписываться много раз
                if (!isCompound) {
                    ...
                    menuButtonControl.subscribe('onPickerOpen', () => {
                        const pickerContainer =
                            menuButtonControl._picker.getContainer();
                        // Скрыли старую кнопку
                        pickerContainer
                            .find('.controls-PopupMixin__closeButton')
                            .addClass('ws-hidden');
                        const newCloseButton = pickerContainer.find(
                            '.compoundarea-closebutton'
                        );
        ...
        _setCustomToolbar(): void {
            if (
                this._options.isToolbarOnRightPanel &&
                Controller.hasRightPanel()
            ) {
                const toolbarContent = $(
                    '.controls-ToolBar:first',
                    this._childControl.getContainer()
                );
                if (toolbarContent.length) {
                    const toolbarContainer = $(
                        '.controls-CompoundArea_toolbar',
                        this.getContainer()
                    );
                    if (toolbarContainer.length) {
                        ...
                        wsControl.setEnabled = (enabled) => {
                            setEnabled(enabled);
                            this._updateCustomToolbarMenuIcon(
                                toolbarContainer[0]
                            );
                        };
        ...
        _setCustomHeader() {
            ...
            if (
                ...
                if (customHeaderContainer.length) {
                    if (
                        $('.ws-float-area-title', customHeaderContainer)
                            .length === 0
                    ) {
                        customHeaderContainer.prepend(
                            '<div class="ws-float-area-title">' +
                    ...
                    this.getContainer().addClass(
                        'controls-CompoundArea-headerPadding'
                    );
            ...
            if (
                ...
                customHeaderContainer.addClass(
                    'controls-CompoundArea__move-cursor'
                );
                customHeaderContainer.bind(
                    'mousedown',
                    this._headerMouseDown.bind(this)
                );
            }
        },
        setRoundedClassOnDialogTemplate(): void {
            ...
            container.addClass(
                'controls-CompoundArea-borderRadius_customHeader'
            );
        },
        ...
        _getCustomHeaderContainer() {
            ...
            if (customHeader.length) {
                ...
                for (let i = 0; i < customHeader.length; i++) {
                    ...
                    while (
                        parent !== this._childControl.getContainer()[0] &&
                        nesting < 5
                    ) {
        ...
        handleCommand(commandName, args) {
            ...
            if (
                this._options._mode === 'recordFloatArea' &&
                commandName === 'save'
            ) {
            ...
            if (commandName === 'resize' || commandName === 'resizeYourself') {
                ...
                return CompoundArea.superclass.handleCommand.apply(
                    this,
                    arguments
                );
            }
        ...
        _isLinkedPanel(event) {
            ...
            const popupMixin = target.closest(
                '.controls-Menu, .controls-FloatArea'
            );
        ...
        _keyDown(event: SyntheticEvent<KeyboardEvent>) {
            ...
            const closingByKeys =
                ...
                nativeEvent.keyCode === constants.key.esc;
            const targetInEditInPlace = Boolean(
                event.target.closest('.controls-editInPlace')
            );
        ...
        reload() {
            if (this._popupOptions) {
                ...
                if (
                    popupCfg &&
                    this._popupOptions.minWidth &&
                    this._popupOptions.maxWidth
                ) {
                    popupCfg.popupOptions.minWidth =
                        this._popupOptions.minWidth;
                    popupCfg.popupOptions.maxWidth =
                        this._popupOptions.maxWidth;
        ...
        openConfirmDialog(noHide) {
            ...
            if (
                (this.getRecord().isChanged() && !this.isSaved()) ||
                this._recordIsChanged
            ) {
                this._openConfirmDialog(false, true).addCallback((result) => {
                    switch (result) {
                        case 'yesButton': {
                            ...
                            this.updateRecord()
                                .addCallback(() => {
                                    this._confirmDialogToCloseActions(
                                        deferred,
                                        noHide
                                    );
                                })
        ...
        _setEnabledForChildControls() {
            DialogRecord.prototype._setEnabledForChildControls.apply(
                this,
                arguments
            );
        },
        ...
        _printMenuItemsIsChanged() {
            return DialogRecord.prototype._printMenuItemsIsChanged.apply(
                this,
                arguments
            );
        },
        _createPrintMenu() {
            return DialogRecord.prototype._createPrintMenu.apply(
                this,
                arguments
            );
        },
        ...
        _openConfirmDialog() {
            return DialogRecord.prototype._openConfirmDialog.apply(
                this,
                arguments
            );
        },
        ...
        close(arg) {
            if (!this.isDestroyed()) {
                ...
                if (this._options.autoCloseOnHide === false) {
                    ...
                    this._childControl &&
                    !this._childControl.isDestroyed()
                ) {
        ...
        _toggleVisible(visible) {
            ...
            const popupContainer =
                this.getContainer().closest('.controls-Popup')[0];
            ...
            if (popupConfig) {
                ...
                popupContainer.className = this._toggleVisibleClass(
                        ...
                        if (item.isHiddenForRecalc) {
                            ...
                            runDelayed(() => {
                                item.popupOptions.className =
                                    item.popupOptions.className.replace(
                                        invisibleRe,
                                        ''
                                    );
                                container.className =
        ...
        setOffset(newOffset) {
            const popupConfig = this._getManagerConfig();
            if (popupConfig) {
                popupConfig.popupOptions.offset =
                    popupConfig.popupOptions.offset || {};
        ...
        destroy() {
            ...
            if (this._popupController && this._baseAfterUpdate) {
                this._popupController._elementAfterUpdated =
                    this._baseAfterUpdate;
            ...
            if (this._isFinishingChildOperations) {
                ...
                Logger.error(
                    'Lib/Mixins/PendingOperationParentMixin: ' + message,
                    this
                );
            }
        ...
        _registerPendingOperation(operationName, finishFunc, registerTarget) {
            const name = this._moduleName
                ? this._moduleName + '/' + operationName
                : operationName;
            const operation = {
                name,
                finishFunc,
            ...
            operation.cleanup = this._removeOpFromCollections.bind(
                this,
                operation
            );
            if (operation.registerTarget) {
                operation.registerTarget.sendCommand(
        ...
        _registerChildPendingOperation(operation) {
            ...
            if (!this._allChildrenPendingOperation) {
                ...
                this._allChildrenPendingOperation =
                    this._registerPendingOperation(
                        name,
                        finishFunc,
                        this.getParent()
                    );
        ...
        _unregisterChildPendingOperation(operation) {
            ...
            if (childOps.length > 0) {
                ...
                if (childOps.length === 0) {
                    ...
                    this._unregisterPendingOperation(
                        allChildrenPendingOperation
                    );
                }
            }
        ...
        setLinkedView(linkedView) {
            ...
            if (linkedView) {
                multiSelectChanged =
                    this._linkedView.getMultiselect() !== multiselect;
                ...
                const currentSelectedKeys =
                    this._options.currentSelectedKeys || [];
        ...
        _registerLinkedView() {
            ...
            this._changeSelectionHandler =
                this._changeSelectionHandler.bind(this);
        ...
        _callCloseCallback(value) {
            if (
                this._options &&
                typeof this._options.closeCallback === 'function'
            ) {
