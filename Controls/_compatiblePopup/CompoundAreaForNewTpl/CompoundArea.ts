...
const moduleClass = CompoundControl.extend({
    ...
    init(): void {
        ...
        this._panel.subscribe(
            'onAfterClose',
            this._callCloseHandler.bind(this)
        );
        ...
        this._runInBatchUpdate(
            'CompoundArea - init - ' + this._id,
            function () {
                ...
                if (this._options.innerComponentOptions) {
                    if (this._options.innerComponentOptions._template) {
                        this._options.template =
                            this._options.innerComponentOptions._template;
                    }
                    this._saveTemplateOptions(
                        this._options.innerComponentOptions
                    );
                    Logger.error(
                ...
                const isBilling =
                    document.body.classList.contains('billing-page');
                ...
                Promise.all(deps).then((result) => {
                    ...
                    if (
                        ...
                        this._settingsController.getSettings
                    ) {
    ...
    _createTemplate(def): void {
        ...
        if (wrapper.length) {
            ...
            this._vDomTemplate = control.createControl(
                ComponentWrapper,
                wrapperOptions,
                wrapper
            );
    ...
    _getSettingsWidth(): Promise<null | number> {
        return new Promise((resolve) => {
            ...
            if (propStorageId) {
                this._settingsController
                    .getSettings([propStorageId])
                    .then((storage) => {
                        if (storage && storage[propStorageId]) {
                            this._options._popupOptions.width =
    ...
    popupBeforeDestroyed() {
        ...
        if (ManagerWrapper) {
            const container = this._container[0]
                ? this._container[0]
                : this._container;
            ManagerWrapper._beforePopupDestroyedHandler(
                null,
                {},
                [],
                container
    ...
    _onRegisterHandler(event, eventName, emitter, handler) {
        ...
        if (event.type === 'register') {
            ...
            ManagerWrapperController.registerListener(
                event,
                eventName,
                emitter,
                handler
    ...
    _getListenerIndex(emitter) {
        ...
        for (let index = 0; index < length; index++) {
            ...
            if (unregisterEmitter && unregisterEmitter.getInstanceId()) {
                if (
                    unregisterEmitter.getInstanceId() ===
                    emitter.getInstanceId()
                ) {
                    return index;
                }
            }
    ...
    _getFloatAreaStackRootCoords() {
        const stackRootContainer = document.querySelector(
        ...
        if (stackRootContainer) {
            right =
                ...
                stackRootContainer.getBoundingClientRect().right;
        }
        return { top, right };
    },
    ...
    _getStrategyData(offset: number = 0): object {
        ...
        const floatAreaMaxWidth =
            document.body.clientWidth - MINIMAL_PANEL_DISTANCE;
    ...
    destroy() {
        ...
        if (this._vDomTemplate) {
            Synchronizer.unMountControlFromDOM(
                this._vDomTemplate,
                this._vDomTemplate._container
            );
        }
    },
    ...
    _activatedHandler(event, args) {
        if (!this.isActive()) {
            ...
            if (
                curContainer.contains(toContainer) &&
                activationTarget._$to._template
            ) {
                this.setActive(
                    true,
                    activationTarget.isShiftKey,
                    true,
                    activationTarget._$to
                );
            }
        }
    },
