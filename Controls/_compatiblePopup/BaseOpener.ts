...
import library = require('WasabyLoader/Library');
...
const BaseOpener = {
    ...
    _prepareConfigForOldTemplate(cfg, templateClass): void {
        ...
        const availableWithForMaximize =
            contentData?.width + contentData?.left || document?.body.clientWidth;
        if (
            cfg.canMaximize &&
    ...
    prepareNotificationConfig(config) {
        const template =
            typeof config.template === 'string' ? loadTemplate(config.template) : config.template;
        config.opener = null;
        config.isVDOM = true;
    ...
    _prepareConfigFromOldToOldByNewEnvironment(cfg) {
        ...
        const openerContainer =
            cfg.opener && (cfg.opener instanceof HTMLElement ? cfg.opener : cfg.opener._container);
        if (openerContainer) {
            let sbis3FloatArea = openerContainer.closest(
            ...
            if (cfg.direction === 'right' || cfg.direction === 'left') {
                if (typeof cfg.horizontalAlign !== 'object') {
                    newDirection.horizontal =
                        (cfg.direction && cfg.direction.horizontal) || cfg.direction;
                }
    ...
    _setSizes(cfg, templateClass) {
        ...
        if (!cfg.minHeight && dimensions.height) {
            ...
            cfg.minHeight =
                parseInt(dimensions.height, 10) + (cfg.templateOptions.caption ? 40 : 0);
        }
