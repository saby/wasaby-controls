...
export default class ManagerWrapper extends Control<IControlOptions> {
    ...
    protected _afterMount(cfg): void {
        ...
        ControllerPopup.setDataLoaderModule(cfg.dataLoaderModule);
        ControllerPopup.setTheme(cfg.theme);
    }
    ...
    private _workspaceResizePage(event, ...args): void {
        if (ControllerPopup.getController()) {
            ControllerPopup.getController().eventHandler.apply(ControllerPopup.getController(), [
                'workspaceResize',
                args,
            ]);
        }
