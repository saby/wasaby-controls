...
export default class ManagerWrapper extends Control<IControlOptions> {
    ...
    protected _afterMount(cfg): void {
        ...
        this._mousemovePage = this._eventRegistratorHandler.bind(
            this,
            'mousemoveDetect'
        );
        this._touchmovePage = this._eventRegistratorHandler.bind(
            this,
            'touchmoveDetect'
        );
        this._touchendPage = this._eventRegistratorHandler.bind(
            this,
            'touchendDetect'
        );
        ...
        this._mouseupPage = this._eventRegistratorHandler.bind(
            this,
            'mouseupDetect'
        );
                ...
                const popups = this._children.PopupContainer.getChildControls(
                    ...
                    (instance) => {
                        return (
                            ...
                            instance._options.template !==
                                'Controls/popupTemplate:templateInfoBox'
                        );
                    }
                );
    ...
    private _listenersSubscribe(
        ...
        if (!this._destroyed) {
            ...
            for (let i = 0; i < registrators.length; i++) {
                this._children[registrators[i]][method](
                    event,
                    registerType,
                    component,
                    callback
