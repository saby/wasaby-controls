...
import type { IResultConfig } from 'Controls/gridRender';
...
import { TBackButtonStyle } from 'Controls/heading';
...
interface IOptions extends IControlOptions, IExplorerOptions {
    ...
    afterBackButtonTemplate: Function | string;
}
...
export default class PathController extends Control<IOptions> {
    ...
    // Хлебные крошки explorer не должны становиться некликабельными, если список в режиме readonly.
    // Пользователь должен иметь возможность переходить по разделам.
    protected _breadcrumbsReadOnly = false;
...
function patchHeader(breadcrumbsLayout: IBreadcrumbsLayout, options: IOptions): IHeaderCell[] {
    ...
    newHeader[0].isBreadCrumbs =
        ...
        breadcrumbsOptions.withoutBackButton === false ||
        breadcrumbsOptions.withoutBreadcrumbs === false;
...
function patchColumns(breadcrumbsLayout: IBreadcrumbsLayout, options: IOptions): IColumn[] {
    ...
    newColumns[0].resultTemplate = React.createElement(resultBreadcrumbs, {
        breadcrumbsOptions,
    });
    (newColumns[0] as { breadcrumbsOptions: object }).breadcrumbsOptions = breadcrumbsOptions;
...
function getResultsBackButtonOptions(
    ...
    return {
        ...
        afterBackButtonTemplate: options.afterBackButtonTemplate,
    };
}
