...
export default class PathController extends Control<IOptions> {
    ...
    protected _beforeMount(options: IOptions): void {
        ...
        this._needShadow = PathController._isNeedShadow(
            this._header,
            options.viewMode
        );
    }
    ...
    protected _beforeUpdate(newOptions: IOptions): void {
        ...
        if (
            headerChanged ||
            !isItemsEqual(
                this._options.breadCrumbsItems,
                newOptions.breadCrumbsItems
            ) ||
    ...
    goBack(e: Event): void {
        ...
        import('Controls/breadcrumbs').then((breadcrumbs) => {
            breadcrumbs.HeadingPathCommon.onBackButtonClick.call(
                this,
                e,
                'breadCrumbsItems'
            );
    ...
    protected _onHeaderBreadcrumbsClick(
     ...
     */
    private static _isNeedShadow(
        header: IHeaderCell[],
        viewModel: TExplorerViewMode
    ): boolean {
...
function patchHeader(
    ...
    newHeader[0].templateOptions = getResultsBackButtonOptions(
        'header',
        breadcrumbsLayout,
        options
...
function getResultsBackButtonOptions(
    ...
    return {
        ...
        backButtonBeforeCaptionTemplate:
            options.backButtonBeforeCaptionTemplate,
        withoutBackButton:
            breadcrumbsLayout.backButtonPosition !==
            (target === 'header' ? 'header' : 'results'),
        withoutBreadcrumbs:
            breadcrumbsLayout.breadcrumbsPosition !==
            (target === 'header' ? 'header' : 'results'),
    };
}
