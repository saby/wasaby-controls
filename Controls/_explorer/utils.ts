...
import {
    IExplorerOptions,
    TBreadcrumbsVisibility,
    TExplorerViewMode,
} from './interface/IExplorer';
 ...
 */
export function firstHeaderCellIsEmpty(
    header: undefined | IHeaderCell[]
): boolean {
    // title - устаревшее поле колонки
    const firstHeaderCell =
        header?.length && (header[0] as IHeaderCell & { title: string });
    return (
        firstHeaderCell &&
...
export function calculateBreadcrumbsLayout(
    ...
    const canUseHeader =
        ...
        (options.headerVisibility === 'visible' ||
            (options.headerVisibility === 'hasdata' &&
                items &&
                items.getCount()));
    if (canUseHeader) {
    ...
    if (options.resultsVisibility === 'hasdata' && items) {
        const rootItemsIndices = items.getIndicesByValue(
            options.parentProperty,
            options.root
        );

        if (rootItemsIndices.length === 1) {
            const rootItemParent = items
                .at(rootItemsIndices[0])
                .get(options.nodeProperty);
