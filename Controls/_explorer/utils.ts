 ...
 */
export function firstCellIsEmpty(row: undefined | IHeaderCell[]): boolean {
    // title - устаревшее поле колонки
    const firstCell = row?.length && (row[0] as IHeaderCell & { title: string });
    return (
        firstCell &&
        !(firstCell.title || firstCell.caption) &&
        !firstCell[hasReactColumns(row) ? 'render' : 'template'] &&
        !firstCell?.templateOptions?.contentTemplate
    );
}
...
export function needBackButtonInHeader(
    ...
    return breadcrumbsVisibility !== 'hidden' && firstCellIsEmpty(header);
}
...
export function calculateBreadcrumbsLayout(
    ...
    // При этом от расположения крошек зависит и высота строки итогов,
    // которая отображается как в режиме таблицы, так и в режиме поиска.
    if (options.viewMode !== 'table' && options.viewMode !== 'search') {
        return layout;
    }
    ...
    const canUseHeader =
        ...
        firstCellIsEmpty(options.header) &&
        // шапка видна
    ...
    const resultsData = items && items.getMetaData().results;
    const noResultsFirstCol =
        firstCellIsEmpty(options.results) ||
        !(resultsData && resultsData.get(firstCol?.displayProperty));
