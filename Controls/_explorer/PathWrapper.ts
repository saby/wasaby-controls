...
export default class PathWrapper extends Control<IOptions> {
    ...
    protected _beforeUpdate(newOptions: IOptions): void {
        ...
        if (
            ...
            this._options.breadcrumbsVisibility !==
                newOptions.breadcrumbsVisibility
        ) {
    ...
    protected _getBreadCrumbsWrapperStyles(options: IOptions): string {
        ...
        if (
            (options.newColumnScroll && options.isFullGridSupport) ||
            typeof options.breadCrumbsMaxWidth !== undefined
        ) {
            const width = options.breadCrumbsMaxWidth
                ? `${options.breadCrumbsMaxWidth}px`
                : '100%';
    ...
    private static _isNeedCrumbs(options: IOptions): boolean {
        ...
        return (
            (!!items &&
                ((!options.withoutBackButton && items.length > 0) ||
                    items.length > 1)) ||
            !!options.rootVisible
        );
    }
