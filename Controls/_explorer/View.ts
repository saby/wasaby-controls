...
import { IColumn, IHeaderCell, resolveViewControls } from 'Controls/grid';
...
import {
    MultiColumnMarkerStrategy as MultiColumnStrategy,
    SingleColumnMarkerStrategy as SingleColumnStrategy,
} from 'Controls/marker';
import type { TNotEditableJsSelector } from 'Controls/editInPlace';
import type { ReactSearchBreadcrumbsTreeGridView } from 'Controls/searchBreadcrumbsGrid';
import { TreeGridControl, ReactTreeGridView, BaseTreeControlComponent } from 'Controls/treeGrid';
import {
    IItemPadding,
    IReloadItemOptions,
    ListView,
...
interface IViewControllers {
    search: typeof TreeGridControl | null;
...
interface IViewNames {
    search: typeof ReactSearchBreadcrumbsTreeGridView | null;
    tile: TreeTileView | null;
    table: typeof ReactTreeGridView;
    list: ListView;
 ...
 *
 * Полезные ссылки:
 * * {@link Controls/_dataFactory/List/_interface/IListDataFactoryArguments Интерфейс аргументов фабрики списка}
 * * {@link /doc/platform/developmentapl/interface-development/context-data/new-data-store/list-slice/ Слайс для работы со списочными компонентами}
 * * {@link /doc/platform/developmentapl/interface-development/context-data/new-data-store/create-context/ Реализация собственного слайса в контексте данных}
 * * {@link Controls/treeTile:View Плитка с проваливанием}
 * * {@link Controls/treeGrid:View Дерево с колонками}
 * * {@link /doc/platform/developmentapl/interface-development/controls/list/explorer/ руководство разработчика}
 * * {@link https://git.sbis.ru/saby/wasaby-controls/-/blob/rc-24.6100/Controls-default-theme/variables/_explorer.less переменные тем оформления explorer}
 * * {@link https://git.sbis.ru/saby/wasaby-controls/-/blob/rc-24.6100/Controls-default-theme/variables/_list.less переменные тем оформления list}
 *
 * @demo Controls-ListEnv-demo/WI/Explorer/Index
 *
 ...
 *
 * @ignoreOptions filter navigation sorting selectedKeys excludedKeys multiSelectVisibility markerVisibility expandedItems hasChildrenProperty selectionType displayProperty groupHistoryId searchValue propStorageId parentProperty nodeProperty nodeHistoryType nodeHistoryId root nodeTypeProperty selectionCountMode markedKey
 *
 * @public
 */
export default class Explorer extends Control<IExplorerOptions> implements IEditableList {
    ...
    protected _ViewComponent?: React.FunctionComponent<any> | null;
    ...
    /**
     * Текущая применяемая настройка горизонтального скролла.
     */
    protected _columnScroll: boolean;
    /**
     * Новая настройка горизонтального скролла..
     */
    protected _newColumnScroll: boolean;
    ...
    /**
     * При горизонтальном скролле представление оборачивается в скролл-контейнер с возможностью
    ...
    protected _beforeMount(
        cfg: IExplorerOptions,
        context: object,
        receivedState?: { uniqueId: string }
    ): Promise<{ uniqueId: string }> | { uniqueId: string } {
        ...
        const setViewModeResult = this._setViewMode(cfg.viewMode, cfg);
        return setViewModeResult instanceof Promise
            ? setViewModeResult.then(() => {
                  return {
    ...
    protected _beforeUpdate(cfg: IExplorerOptions): void {
        ...
        const isTileOrientationChanged = cfg.orientation !== this._options.orientation;
        if (isTileOrientationChanged && this._viewModelConstructors.tile) {
            this._loadTileViewMode(cfg);
            if (cfg.viewMode === 'tile') {
                this._listVirtualScrollControllerConstructor =
                    this._viewVirtualScrollController.tile;
            }
        }
        ...
        if (
            ...
            !isRootChanged &&
            this._potentialMarkedKey !== cfg.markedKey
        ) {
        ...
        // Если отменили проваливание в узел, то нужно сбросить флаг.
        // При этом нельзя сбрасывать флаг, если рут ещё не успел смениться,
        // и загрузка ещё не закончена.
        if (
            !isSourceControllerLoading &&
            !cfg.loading &&
            cfg.loading !== this._options.loading &&
            !isRootChanged &&
            this._isGoingFront
        ) {
        ...
        if (cfg.columnScroll !== this._options.columnScroll) {
            this._newColumnScroll = cfg.columnScroll;
        }
        ...
        if (cfg.breadCrumbsItems !== this._options.breadCrumbsItems) {
            this._onBreadcrumbsChanged(cfg.breadCrumbsItems, cfg, this._options.root);
        }
        ...
        // загрузки новых данных ничего менять не надо чтобы не было лишних скачков и перерисовок
        if (
            willBeDataRequest ||
            ...
            // мы уже загружаем viewMode и _applyNewVisualOptions будет вызван в коллбэке после его загрузки
            // https://online.sbis.ru/opendoc.html?guid=2785c4f9-f339-4536-b531-b59e0890d894
            this._applyNewVisualOptions(cfg);
        }
    }
    ...
    protected _afterRender(): void {
        if (this._markerForRestoredScroll !== null && this._markerForRestoredScroll !== undefined) {
    ...
    protected _onItemClick(
        ...
        const changeRoot = () => {
            ...
            const nodeInRoot = item.get(this.props.parentProperty) === this._options.root;
            if (currRootInfo && isCursorNavigation(this._navigation) && nodeInRoot) {
    ...
    protected _onBreadcrumbsChanged(
        ...
        const isGoingBack =
            options.root !== oldRoot && detectGoingBackByPath(this._breadcrumbs, breadcrumbs);
    ...
    protected _itemMouseDown(item: Model, nativeEvent: MouseEvent): void {
        if (this._viewMode !== 'search' && !this._options.expandByItemClick) {
            ...
            if (nativeEvent.button === 0 && itemType === ITEM_TYPES.node) {
                checkWasabyEvent(this._options.onBeforeRootChanged)?.(item.getKey());
    ...
    scrollToLeft(smooth?: boolean): void {
        if (this._children.treeControl) {
            this._children.treeControl.scrollToLeft(smooth);
        }
    }

    scrollToRight(smooth?: boolean): void {
        if (this._children.treeControl) {
            this._children.treeControl.scrollToRight(smooth);
        }
    }
    ...
    moveItemsWithDialog(
        ...
        return this._children.treeControl.moveItemsWithDialog(selection, config);
    }
    ...
    removeItemsWithConfirmation(
        ...
        return this._children.treeControl.removeItemsWithConfirmation(selection, config);
    }
    ...
    private _isDividedWithSlice(): boolean {
        return (
            !!this._options.useCollection ||
            (!!this._options.hasSlice && !!this._options.slice?.collection)
        );
    }
    ...
    private _itemsSetCallbackFunc(items: RecordSet, newOptions: IExplorerOptions): void {
        if (this._isGoingBack) {
            if (this._potentialMarkedKey) {
                // После загрузки может не быть записи, для которой восстанавливается маркер.
                // В таком случае сбрасываем хранимый потенциальный маркер.
                // https://online.sbis.ru/opendoc.html?guid=7e9756eb-3325-43e3-b9ab-a5b6f5d1a90e&client=3

                if (!items || !items.getRecordById(this._potentialMarkedKey)) {
                    this._potentialMarkedKey = undefined;
                }

                if (!this._isDividedWithSlice()) {
                    this._children.treeControl.setMarkedKey(this._potentialMarkedKey);
                }
                this._markerForRestoredScroll = this._potentialMarkedKey;

                // Во избежание лишних перерисовок вызываем forceUpdate только если реально изменяли маркер.
                if (this._potentialMarkedKey) {
                    this._potentialMarkedKey = undefined;
    ...
    private _setViewConfig(viewMode: TExplorerViewMode, cfg: IExplorerOptions): void {
        ...
        if (resolvedViewMode === 'table' || resolvedViewMode === 'search') {
            this._ViewComponent =
                resolvedViewMode === 'table' ? ReactTreeGridView : this._viewNames.search;
            resolveViewControls(this, cfg, baseView, this._viewControllers[resolvedViewMode]);
        } else {
    ...
    private _applyNewVisualOptions(cfg): void {
        ...
        if (this._newItemTemplate !== null && this._newItemTemplate !== undefined) {
            this._itemTemplate = this._newItemTemplate;
            this._newItemTemplate = null;
        }
        // Если itemTemplate сбросили, то он будет равен undefined
        if (this._newItemTemplateOptions !== null && this._newItemTemplateOptions !== undefined) {
        ...
        if (this._newColumnScroll !== this._columnScroll) {
            this._columnScroll = this._newColumnScroll;
            this._setViewConfig(this._viewMode, cfg);
        }
    ...
    private _loadColumnsViewMode(cfg: IExplorerOptions): Promise<void> | void {
        return executeSyncOrAsync(['Controls/columns'], (columns) => {
            ...
            this._viewItemsSelector.columns = this._disableVirtualScrollForColumns
                ? columns.ViewTemplate.itemsSelector
    ...
    private _loadSearchGridViewMode(): Promise<void> | void {
        return executeSyncOrAsync(['Controls/searchBreadcrumbsGrid'], (searchGrid) => {
            this._viewNames.search = searchGrid.ReactSearchBreadcrumbsTreeGridView;
            this._viewTableNames.search = searchGrid.ReactSearchBreadcrumbsTreeGridView;
            this._viewControllers.search = TreeGridControl;
    ...
    private _checkedChangeViewMode(viewMode: TExplorerViewMode, cfg: IExplorerOptions): void {
        ...
        Promise.resolve(this._setViewMode(viewMode, cfg)).then(() => {
            ...
            // проставится setViewMode, т.к. он может проставиться асинхронно,
...
/**
 * @event Controls/explorer:View#arrowClick Происходит при клике на кнопку "Просмотр записи".
...
/**
 * @event Controls/explorer:View#rootChanged Происходит при изменении корня иерархии (например, при переходе пользователя по хлебным крошкам).
 * @param {UI/Events:SyntheticEvent} eventObject Дескриптор события.
 * @param {String|Number} root Идентификатор корневой записи.
 */

/**
 * @name Controls/explorer:View#filter
 * @deprecated Следует указать опцию {@link Controls/_interface/IStoreId#storeId storeId}, а также настроить предзагрузку. См {@link /doc/platform/developmentapl/interface-development/context-data/new-data-store/list-slice/ Слайс для работы со списочными компонентами}.
...
/**
 * @name Controls/explorer:View#navigation
 * @deprecated Следует указать опцию {@link Controls/_interface/IStoreId#storeId storeId}, а также настроить предзагрузку. См {@link /doc/platform/developmentapl/interface-development/context-data/new-data-store/list-slice/ Слайс для работы со списочными компонентами}.
...
/**
 * @name Controls/explorer:View#sorting
 * @deprecated Следует указать опцию {@link Controls/_interface/IStoreId#storeId storeId}, а также настроить предзагрузку. См {@link /doc/platform/developmentapl/interface-development/context-data/new-data-store/list-slice/ Слайс для работы со списочными компонентами}.
 ...
 * @remark
 * 1. Допустимы значения направления сортировки ASC/DESC.
 *
 ...
 *
 * 2. Выбранную сортировку можно сохранять. Для этого используют опцию {@link Controls/grid:IPropStorage#propStorageId propStorageId}.
 * 3. Список отображает данные в том порядке, в котором их предоставляет БЛ. Для отображения узлов дерева в начале списка, перед листами необходимо так настроить БЛ, чтобы узлы возвращались первыми независимо от запрошенной сортировки.
 */

/**
 * @name Controls/explorer:View#source
 * @deprecated Следует указать опцию {@link Controls/_interface/IStoreId#storeId storeId}, а также настроить предзагрузку. См {@link /doc/platform/developmentapl/interface-development/context-data/new-data-store/list-slice/ Слайс для работы со списочными компонентами}.
...
/**
 * @name Controls/explorer:View#keyProperty
 * @deprecated Следует указать опцию {@link Controls/_interface/IStoreId#storeId storeId}, а также настроить предзагрузку. См {@link /doc/platform/developmentapl/interface-development/context-data/new-data-store/list-slice/ Слайс для работы со списочными компонентами}.
 ...
 */
/**
 * @name Controls/explorer:View#selectedKeys
 * @deprecated Следует указать опцию {@link Controls/_interface/IStoreId#storeId storeId}, а также настроить предзагрузку. См {@link /doc/platform/developmentapl/interface-development/context-data/new-data-store/list-slice/ Слайс для работы со списочными компонентами}.
...
/**
 * @name Controls/explorer:View#excludedKeys
 * @deprecated Следует указать опцию {@link Controls/_interface/IStoreId#storeId storeId}, а также настроить предзагрузку. См {@link /doc/platform/developmentapl/interface-development/context-data/new-data-store/list-slice/ Слайс для работы со списочными компонентами}.
...
/**
 * @name Controls/explorer:View#multiSelectVisibility
 * @deprecated Следует указать опцию {@link Controls/_interface/IStoreId#storeId storeId}, а также настроить предзагрузку. См {@link /doc/platform/developmentapl/interface-development/context-data/new-data-store/list-slice/ Слайс для работы со списочными компонентами}.
...
/**
 * @name Controls/explorer:View#markerVisibility
 * @deprecated Следует указать опцию {@link Controls/_interface/IStoreId#storeId storeId}, а также настроить предзагрузку. См {@link /doc/platform/developmentapl/interface-development/context-data/new-data-store/list-slice/ Слайс для работы со списочными компонентами}.
 * @cfg {Controls/marker/TVisibility.typedef} Режим отображения {@link /doc/platform/developmentapl/interface-development/controls/list/actions/marker/ маркера}.
...
/**
 * @name Controls/explorer:View#expandedItems
 * @deprecated Следует указать опцию {@link Controls/_interface/IStoreId#storeId storeId}, а также настроить предзагрузку. См {@link /doc/platform/developmentapl/interface-development/context-data/new-data-store/list-slice/ Слайс для работы со списочными компонентами}.
...
/**
 * @name Controls/explorer:View#hasChildrenProperty
 * @deprecated Следует указать опцию {@link Controls/_interface/IStoreId#storeId storeId}, а также настроить предзагрузку. См {@link /doc/platform/developmentapl/interface-development/context-data/new-data-store/list-slice/ Слайс для работы со списочными компонентами}.
...
/**
 * @name Controls/explorer:View#selectionType
 * @deprecated Следует указать опцию {@link Controls/_interface/IStoreId#storeId storeId}, а также настроить предзагрузку. См {@link /doc/platform/developmentapl/interface-development/context-data/new-data-store/list-slice/ Слайс для работы со списочными компонентами}.
...
/**
 * @name Controls/explorer:View#searchValue
 * @deprecated Следует указать опцию {@link Controls/_interface/IStoreId#storeId storeId}, а также настроить предзагрузку. См {@link /doc/platform/developmentapl/interface-development/context-data/new-data-store/list-slice/ Слайс для работы со списочными компонентами}.
...
/**
 * @name Controls/explorer:View#displayProperty
 * @deprecated Следует указать опцию {@link Controls/_interface/IStoreId#storeId storeId}, а также настроить предзагрузку. См {@link /doc/platform/developmentapl/interface-development/context-data/new-data-store/list-slice/ Слайс для работы со списочными компонентами}.
...
/**
 * @name Controls/explorer:View#groupHistoryId
 * @deprecated Следует указать опцию {@link Controls/_interface/IStoreId#storeId storeId}, а также настроить предзагрузку. См {@link /doc/platform/developmentapl/interface-development/context-data/new-data-store/list-slice/ Слайс для работы со списочными компонентами}.
...
/**
 * @name Controls/explorer:View#root
 * @deprecated Следует указать опцию {@link Controls/_interface/IStoreId#storeId storeId}, а также настроить предзагрузку. См {@link /doc/platform/developmentapl/interface-development/context-data/new-data-store/list-slice/ Слайс для работы со списочными компонентами}.
 * @cfg {Number|String} Идентификатор корневого узла.
 * Значение опции root добавляется в фильтре в поле {@link Controls/explorer:View#parentProperty parentProperty}.
 * @example
...
/**
 * @name Controls/explorer:View#nodeHistoryId
 * @deprecated Следует указать опцию {@link Controls/_interface/IStoreId#storeId storeId}, а также настроить предзагрузку. См {@link /doc/platform/developmentapl/interface-development/context-data/new-data-store/list-slice/ Слайс для работы со списочными компонентами}.
...
/**
 * @name Controls/explorer:View#nodeHistoryType
 * @deprecated Следует указать опцию {@link Controls/_interface/IStoreId#storeId storeId}, а также настроить предзагрузку. См {@link /doc/platform/developmentapl/interface-development/context-data/new-data-store/list-slice/ Слайс для работы со списочными компонентами}.
...
/**
 * @name Controls/explorer:View#nodeProperty
 * @deprecated Следует указать опцию {@link Controls/_interface/IStoreId#storeId storeId}, а также настроить предзагрузку. См {@link /doc/platform/developmentapl/interface-development/context-data/new-data-store/list-slice/ Слайс для работы со списочными компонентами}.
...
/**
 * @name Controls/explorer:View#parentProperty
 * @deprecated Следует указать опцию {@link Controls/_interface/IStoreId#storeId storeId}, а также настроить предзагрузку. См {@link /doc/platform/developmentapl/interface-development/context-data/new-data-store/list-slice/ Слайс для работы со списочными компонентами}.
...
/**
 * @name Controls/explorer:View#propStorageId
 * @deprecated Следует указать опцию {@link Controls/_interface/IStoreId#storeId storeId}, а также настроить предзагрузку. См {@link /doc/platform/developmentapl/interface-development/context-data/new-data-store/list-slice/ Слайс для работы со списочными компонентами}.
...
/**
 * @name Controls/explorer:View#nodeTypeProperty
 * @deprecated Следует указать опцию {@link Controls/_interface/IStoreId#storeId storeId}, а также настроить предзагрузку. См {@link /doc/platform/developmentapl/interface-development/context-data/new-data-store/list-slice/ Слайс для работы со списочными компонентами}.
...
/**
 * @name Controls/explorer:View#selectionCountMode
 * @deprecated Следует указать опцию {@link Controls/_interface/IStoreId#storeId storeId}, а также настроить предзагрузку. См {@link /doc/platform/developmentapl/interface-development/context-data/new-data-store/list-slice/ Слайс для работы со списочными компонентами}.
...
/**
 * @name Controls/explorer:View#markedKey
 * @deprecated Следует указать опцию {@link Controls/_interface/IStoreId#storeId storeId}, а также настроить предзагрузку. См {@link /doc/platform/developmentapl/interface-development/context-data/new-data-store/list-slice/ Слайс для работы со списочными компонентами}.
 ...
 * @name Controls/explorer:View#expanderVisibility
...
/**
 * @event Controls/explorer:View#expandedItemsChanged Происходит при изменении набора развернутых узлов.
 * @param {UI/Events:SyntheticEvent} eventObject Дескриптор события.
...
/**
 * @name Controls/explorer:View#footerTemplate
 * @cfg {TemplateFunction|String} Пользовательский шаблон отображения подвала контрола.
 * В опцию необходимо передавать сконфигурированный базовый шаблон {@link Controls/treeGrid:FooterTemplate}.
 *
...
/**
 * @name Controls/explorer:View#searchNavigationMode
 * @deprecated Следует указать опцию {@link Controls/_interface/IStoreId#storeId storeId}, а также настроить предзагрузку. См {@link /doc/platform/developmentapl/interface-development/context-data/new-data-store/list-slice/ Слайс для работы со списочными компонентами}.
