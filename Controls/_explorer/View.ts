...
import {
    IExplorerOptions,
    TExplorerViewMode,
} from 'Controls/_explorer/interface/IExplorer';
...
type TreeFnParams<T extends keyof TreeGridControl> = Parameters<
    TreeGridControl[T]
>;
type TreeFnReturn<T extends keyof TreeGridControl> = ReturnType<
    TreeGridControl[T]
>;
...
export default class Explorer
    extends Control<IExplorerOptions>
    implements IEditableList
{
    ...
    protected _notifyHandler: typeof EventUtils.tmplNotify =
     ...
     * @protected
     */
    protected _useScrollContexts: boolean = false;
    ...
    protected _beforeMount(cfg: IExplorerOptions): Promise<void> | void {
        ...
        this._disableVirtualScrollForColumns =
            cfg.disableVirtualScroll !== undefined
                ? cfg.disableVirtualScroll
                : true;
        ...
        if (cfg.header) {
            ...
            this._newHeader = this._header =
                cfg.viewMode === 'tile' ? undefined : cfg.header;
        }
        if (cfg.columns) {
        ...
        this._breadCrumbsDragHighlighter =
            this._breadCrumbsDragHighlighter.bind(this);
        // Определяет нужно или нет ставить маркер на указанную запись
        this._needSetMarkerCallback = (
            item: Model,
            domEvent: Event
        ): boolean => {
            return (
                item instanceof Array ||
                !!(domEvent.target as HTMLElement).closest(
        ...
        if (cfg.newColumnScroll && !cfg.task1185938417) {
    ...
    protected _beforeUpdate(cfg: IExplorerOptions): void {
        const isViewModeChanged =
            cfg.viewMode !== this._options.viewMode ||
            cfg.useColumns !== this._options.useColumns;
        ...
        this._recreateCollection = needRecreateCollection(
        ...
        const isSourceControllerLoading =
            cfg.sourceController && cfg.sourceController.isLoading();
        this._resetScrollAfterViewModeChange =
            isViewModeChanged && !isRootChanged;
        ...
        if (
            willBeDataRequest ||
            (isViewModeChanged &&
                (willBeDataRequest || isSourceControllerLoading)) ||
    ...
    protected _breadCrumbsDragHighlighter(
        itemKey: TKey,
        hasArrow: boolean
    ): string {
    ...
    protected _onItemClick(
        ...
        const res = this._notify('itemClick', [
            item,
            clickEvent,
            columnIndex,
        ]) as boolean;
        ...
        const changeRoot = () => {
            ...
            if (rootChanged === false) {
                ...
                if (savedPosition) {
                    (
                        this
                            ._navigation as INavigationOptionValue<INavigationPositionSourceConfig>
    ...
    protected _onBreadcrumbsChanged(
        ...
        const isGoingBack = detectGoingBackByPath(
            this._breadcrumbs,
            breadcrumbs
        );
        ...
        if (this._isGoingBack) {
            ...
            if (this._restoredMarkedKeys[curRoot]) {
                this._potentialMarkedKey =
                    this._restoredMarkedKeys[curRoot].markedKey;
            }
        }
    ...
    scrollToItem(
        ...
        if (this._children.treeControl) {
            return this._children.treeControl.scrollToItem(
                key,
                position,
                force,
                allowLoad
    ...
    commitEdit(
        ...args: TreeFnParams<'commitEdit'>
    ): TreeFnReturn<'commitEdit'> {
        return this._children.treeControl.commitEdit(...args);
    }

    cancelEdit(
        ...args: TreeFnParams<'cancelEdit'>
    ): TreeFnReturn<'cancelEdit'> {
    ...
    moveItemUp(
        ...
        return this._children.treeControl.moveItemUp(
            selectedKey,
            viewCommandName
        );
    }
    ...
    moveItemDown(
        ...
        return this._children.treeControl.moveItemDown(
            selectedKey,
            viewCommandName
        );
    }
    ...
    moveItemsWithDialog(
        ...
        return this._children.treeControl.moveItemsWithDialog(
            selection,
            viewCommandName
        );
    }
    ...
    removeItems(
        ...
        return this._children.treeControl.removeItems(
            selection,
            viewCommandName
        );
    }
    ...
    removeItemsWithConfirmation(
        ...
        return this._children.treeControl.removeItemsWithConfirmation(
            selection,
            viewCommandName
        );
    }
    ...
    private _itemsSetCallbackFunc(
        items: RecordSet,
        newOptions: IExplorerOptions
    ): void {
        if (this._isGoingBack) {
            if (this._potentialMarkedKey) {
                this._children.treeControl.setMarkedKey(
                    this._potentialMarkedKey
                );
    ...
    private _setViewConfig(
        ...
        const baseView = (
            isFullGridSupport() ? this._viewNames : this._viewTableNames
        )[resolvedViewMode];
        if (resolvedViewMode === 'table' && isReactView(cfg)) {
            resolveViewControls(this, cfg, baseView, TreeGridControl);
        } else {
        ...
        this._viewModelConstructor =
            this._viewModelConstructors[resolvedViewMode];
    ...
    private _dragItemsFromRoot(dragItems: TKey[]): boolean {
        ...
        for (let i = 0; i < dragItems.length; i++) {
            ...
            if (
                !item ||
                item.get(this._options.parentProperty) !== this._topRoot
            ) {
                itemFromRoot = false;
                break;
            }
    ...
    private _loadColumnsViewMode(cfg: IExplorerOptions): Promise<void> | void {
        return executeSyncOrAsync(['Controls/columns'], (columns) => {
            ...
            this._viewModelConstructors.columns =
                'Controls/columns:ColumnsCollection';
            this._viewItemsSelector.columns =
                columns.ViewTemplate.itemsSelector;
            this._viewVirtualScrollController.columns = this
                ._disableVirtualScrollForColumns
                ? ListVirtualScrollController
    ...
    private _checkedChangeViewMode(
        viewMode: TExplorerViewMode,
        cfg: IExplorerOptions
    ): void {
        ...
        Promise.resolve(this._setViewMode(viewMode, cfg)).then(() => {
            ...
            const isAllExpanded =
                cfg.expandedItems && cfg.expandedItems[0] === null;
            if (
                isViewModeChanged &&
                cfg.searchNavigationMode !== 'expand' &&
                !isAllExpanded
            ) {
    ...
    private _restorePositionNavigation(rootId: TKey): unknown {
        ...
        if (typeof rootInfo?.cursorPosition !== 'undefined') {
            ...
            cursor = (
                this._options._navigation
                    ?.sourceConfig as INavigationPositionSourceConfig
            )?.position;
        }

        (
            this._navigation.sourceConfig as INavigationPositionSourceConfig
        ).position = cursor || null;
        return cursor;
    }

    private _setPendingViewMode(
        viewMode: TExplorerViewMode,
    ...
    private _updateRootOnViewModeChanged(
        viewMode: string,
        options: IExplorerOptions
    ): void {
    ...
    private _setInitBreadCrumbsMode(): void {
        if (
            'treeControl' in this._children &&
            this._children.treeControl.isColumnScrollVisible()
        ) {
    ...
    protected _observeScrollHandler(
        ...
        if (
            !this._options.stickyHeader ||
            this._options.breadcrumbsVisibility === 'hidden'
        ) {
            return;
        }
