...
function getRightPaddingClass(item: Model, options): string {
    ...
    if (hasPinIcon(options, item, options.searchValue)) {
        if (
            options.useListRender &&
            !isAdaptive &&
            (item.get(options.nodeProperty) || item.get(options.nodeProperty) === false)
        ) {
            // У таких пунктов стрелка справа и отступ должен быть больше, чтобы опции записи не перекрывали стрелку
            paddingClass = ' controls-ListView__item-rightPadding_menu_expanderRight-pin';
        } else {
...
function getFontStyleClass(treeItem: TreeItem<Model>, options): string {
    ...
    const isEmpty =
        (options.emptyKey instanceof Array && options.emptyKey.includes(key)) ||
        (options.emptyText && options.emptyKey === key);
    if (isEmpty) {
...
export function getMenuClassList(treeItem: TreeItem<Model>, options): string {
    ...
    if (item && item.get) {
        ...
        if (treeItem.isHovered()) {
            ...
            classes += ` controls-ListView__item_active controls-background-${options.hoverBackgroundStyle}_active`;
        }
        ...
        if (
            ...
            ((!hasParent(item, options, options.historyRoot) &&
                item.get(options.nodeProperty) !== false) ||
                options.searchValue)
        ) {
