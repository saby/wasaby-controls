...
export default function GroupTemplate(props): JSX.Element {
    if (props.isHistoryGroup) {
        return (
            <div
                className={`${props.className} tw-flex tw-items-center tw-w-full controls-inlineheight-s controls-padding_bottom-3xs`}
            >
        ...
        const isGroup = isGroupVisible(props.item, props.emptyItem, props.showText);
        if (isGroup) {
            ...
            if (customTemplate) {
                ...
                delete newOptions.customGroupTemplate;
                return (
                    <CustomGroupTemplate
                        {...newOptions}
            ...
            return (
                <GridGroupTemplate
                    ...
                    paddingBottom="3xs"
                    className={`${
                        props.item.isFirstItem() ? 'controls-margin_top-' + props.itemsSpacing : ''
                    } ${
                        props.multiSelect || props.markerVisibility !== 'hidden'
                            ? 'controls-Menu__group_markerVisibility'
...
export function isGroupVisible(
    ...
    return (
        ...
        itemsGroupCount > 0 &&
        (itemsGroupCount !== collectionCount ||
            (showText === true && !groupItem.isHiddenGroup() && groupItem.getContents()))
    );
}
