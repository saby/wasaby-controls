    ...
    implements IMenuControl
{
    ...
    private _getSourceController(options: IMenuControlOptions): SourceController {
        ...
        if (!this._sourceController) {
            ...
            this._sourceController.setDataLoadCallback((loadedItems, direction) => {
                if (historyId) {
                    return Promise.resolve(this._getDropdownLib().then(({getItemsWithHistory}) => {
                        ...
                        return dataLoadCallback && dataLoadCallback(newItems, direction) || newItems;
                    });
                } else {
                    return dataLoadCallback && dataLoadCallback(loadedItems, direction) || loadedItems;
                }
            });
        }
    ...
    private _itemsChangedCallback(): void {
        if (this._listModel) {
            ...
            this._listModel.setCollection(loadSync('Controls/dropdown').getItemsWithHistory(
                items,
    ...
    private _getHistoryMenuKeyProperty(options):string{
        return options.items?.getKeyProperty()
