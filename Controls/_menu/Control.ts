    ...
    implements IMenuControl
{
            ...
            if (
                ...
                if (
                    ...
                    if (!MenuControl._isItemCurrentRoot(item, this._options)) {
                        ...
                        itemClickResult = !!item.get(this._options.nodeProperty);
                        this._notify('itemClick', [item, sourceEvent]);
    ...
    private _getExpandedItems(items: RecordSet, options: IMenuControlOptions): TKey[] {
        let expandedItems = [];
        if (options.hierarchyViewMode !== 'tree' && this._hasHiddenNode(items, options)) {
            expandedItems = [null];
    ...
    protected _setGroupKey(options, item): void {
        ...
        if (groupKey === undefined || groupKey === null || isHistoryItem) {
            ...
            if (options.parentProperty) {
                ...
                const parentGroup = this._listModel
                    ?.getSourceItemByKey(parent)
                    ?.get(options.groupProperty);
                if (parentGroup) {
