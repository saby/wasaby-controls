...
import HistoryController from 'Controls/_menu/Control/HistoryController';
 ...
 * @remark
 * Полезные ссылки:
 * * {@link /doc/platform/developmentapl/interface-development/controls/input-elements/dropdown-menu/base-menu/ руководство разработчика по использованию контрола меню}
 *
    ...
    implements IMenuControl
{
    ...
    private _subMenuPopupPosition: IStickyPopupOptions;
    ...
    private _dropdownLibLoaded: Promise<typeof import('Controls/dropdown')>;
    private _prepareHistoryUtilsLoaded: Promise<
        typeof import('Controls/Utils/History/PrepareMenuItems')
    >;
    ...
    constructor(options: IMenuControlOptions) {
        ...
        this._menuClosed = this._menuClosed.bind(this);
    }

    protected _beforeMount(options: IMenuControlOptions): Promise<RecordSet> {
        this._validateOptions(options);
        this._viewMode = options.viewMode;
        this._selectedKeys = options.selectedKeys || [];
        this._excludedKeys = options.excludedKeys || [];
        this._emptyKeys =
            options.emptyKey instanceof Array ? options.emptyKey : [options.emptyKey ?? null];
        ...
        return [
            {
                ...
                templateOptions: {
                    ...
                    itemTemplateOptions: options.itemTemplateOptions,
                    cellPadding: null,
                    // TODO: удалить menuMode после замены menu:Control на selectorSticky:Template
                    hierarchyViewMode:
                        options.menuMode === 'selector' ? 'default' : options.hierarchyViewMode,
                    subMenuLevel: options.subMenuLevel,
                    useListRender: options.menuMode === 'selector' ? true : options.useListRender,
                    ...
                    hoverBackgroundStyle: this._getHoverBackgroundStyle(options),
    ...
    protected _afterMount(): void {
        if (this._options.openedSubMenuKey && !this._isTreeControl()) {
    ...
    protected _beforeUpdate(newOptions: IMenuControlOptions): void {
        ...
        if (!isEqual(this._options.itemTemplateOptions, newOptions.itemTemplateOptions)) {
            this._columns = [...this._columns];
            this._columns[0].templateOptions.itemTemplateOptions = newOptions.itemTemplateOptions;
        }
    ...
    protected _afterRender(oldOptions: IMenuControlOptions): void {
        ...
        } else if (this._options.openedSubMenuKey && this._options.menuMode === 'selector') {
            this._children.menuRender.scrollToItem(this._options.openedSubMenuKey, 'top', true);
        }
    }
    ...
    protected _handleKeyDown(event: SyntheticEvent<KeyboardEvent>): void {
        ...
        if (code === constants.key.up) {
            ...
            if (hoveredItem) {
                const collectionItem = hoveredItem.contents;
    ...
    private _updateSelectedItemKeyboard(event: SyntheticEvent, direction: string): void {
        ...
        let currentIndex = this._listModel.getIndex(hoveredItem);
        if (!hoveredItem) {
            newItem = this._emptyItem ? this._emptyItem : this._listModel.getFirst();
        } else {
            do {
                if (direction === 'previous') {
                    const isFirstItem = hoveredItem.isFirstItem();
                    if (
                        (isFirstItem && !this._options.emptyText) ||
                        hoveredItem === this._emptyItem
                    ) {
                        newItem = this._listModel.getLast();
                    } else if (isFirstItem) {
                        newItem = this._emptyItem;
                    } else {
                        newItem = this._findNearbyItem(currentIndex - 1, false);
                    }
                } else if (direction === 'next') {
                    const isLastItem = hoveredItem.isLastItem();
                    if (isLastItem && !this._options.emptyText) {
                ...
                hoveredItem = newItem;
                currentIndex = this._listModel.getIndex(hoveredItem);
    ...
    private _initHistoryControllerIfNeed(options: IMenuControlOptions): void {
        ...
        if (options.historyId && options.hasHistory && isHistorySource(source)) {
    ...
    protected _itemClick(
        ...
        const { multiSelect, nodeProperty } = this._options;
    ...
    private _needTouchOpenMenu(): boolean {
        return this._isAdaptive() || !this._isTreeControl();
    }
    ...
    private _updateMarkedKey(newOptions: IMenuControlOptions): void {
        ...
        let selectedKeys = this._selectedKeys;
        if (
            (!this._selectedKeys || !this._selectedKeys.length) &&
            (newOptions.emptyText || this._listModel.getCollection().getRecordById(null))
        ) {
            selectedKeys = [null];
        }
        this._markedKey = markedKey !== undefined ? markedKey : selectedKeys?.[0];

        if (this._isSingleSelectionKey(markedKey, newOptions)) {
            this._setMarkerEmptyItem(true, this._emptyKeys[0]);
        } else {
            this._setMarkerEmptyItem(false, this._emptyKeys[0]);
        }
    }
    ...
    private _getKeysForSelectionController(options: IMenuControlOptions): TSelectedKeys {
        ...
        this._selectedKeys.forEach((key) => {
            ...
            if (
                !this._emptyKeys.includes(key) &&
                key !== options.selectedAllKey &&
                (!item || !MenuControl._isFixedItem(item, options.source))
            ) {
    ...
    protected _isSingleSelectionKey(
        ...
        const isEmptyItem =
            (options.emptyKey instanceof Array || options.emptyText) &&
            this._emptyKeys.includes(key);
    ...
    protected _openSelectorDialog(): void {
        ...
        this._notify('moreButtonClick', [selectedItems, this._options.selectedKeys]);
    }
        ...
        switch (eventName) {
            ...
            case 'expanderClick':
                this._subMenuPopupPosition = {
                    ...
                    fittingMode: {
                        horizontal: 'fixed',
                        vertical: eventResult ? 'overflow' : 'initialAdaptive',
                    },
                };
            ...
            case 'subMenuClose': // закрытие подменю с клавиатуры
                if (!this._listModel.getHoveredItem() && this._subDropdownItem) {
                    this._updateKeyboardItem(
                        this._listModel.getItemBySourceKey(this._subDropdownItem.getKey())
                    );
                }
                this.closeSubMenu();
                this.activate();
    ...
    private _closeSubMenu(needOpenDropDown: boolean = false, closeFromCode: boolean = false): void {
        if (closeFromCode || !this._preventCloseSubMenu) {
            ...
            if (this._subDropdownItem) {
                this._subMenuSourceCache.delete(this._subDropdownItem.getKey());
                if (!needOpenDropDown) {
                    this._subDropdownItem = null;
                }
            }
    ...
    private _changeSelection(key: string | number | null): void {
        ...
        const newExcludedKeys = [...this._excludedKeys];
        const clickedItem = this._listModel.getItemBySourceKey(key);

        if (clickedItem?.isSelected()) {
            if (newSelectedKeys.includes(key)) {
                newSelectedKeys.splice(newSelectedKeys.indexOf(key), 1);
            } else {
                newExcludedKeys.push(key);
            }
        } else {
            if (newExcludedKeys.includes(key)) {
                newExcludedKeys.splice(newExcludedKeys.indexOf(key), 1);
            } else {
        ...
        this._excludedKeys = beforeSelectionResult?.excluded || newExcludedKeys;
    ...
    private _updateSelectionAndNotify(
        ...
        if (
            !selectedKeys.length &&
            (this._options.emptyKey instanceof Array || this._options.emptyText)
        ) {
            selectedKeys.push(this._emptyKeys[0]);
        }
        const selection = {
    ...
    private _setMarkerEmptyItem(marker: boolean, emptyKey?: TKey): void {
        if (this._emptyItem) {
    ...
    private _updateEmptyItemMarker(selection: ISelectionObject): void {
        ...
        if (isEmptySelected) {
            this._markedKey = this._emptyKeys[0];
    ...
    private _loadItemTemplateProperty(
        ...
        if (items && options.itemTemplateProperty) {
            return this._getDropdownLib().then(({ loadItemsTemplates }) => {
    ...
    protected _setStateByItems(items: RecordSet, options: IMenuControlOptions): void {
        ...
        if (options.focusable && !this._listModel.getHoveredItem()) {
            if (this._children.Sticky?.isOpened() && this._subDropdownItem) {
                this._openSubMenuByKey({ autofocus: true }, this._subDropdownItem.getKey());
            } else {
    ...
    private _needExpandHiddenNode(items: RecordSet, options: IMenuControlOptions): boolean {
        return options.hierarchyViewMode !== 'tree' && this._hasHiddenNode(items, options);
    }
    ...
    private _getMarkedKey(
        ...
        if (multiSelect) {
            const selectedEmptyKey = this._emptyKeys.find((emptyKey) =>
                selectedKeys.includes(emptyKey)
            );
            if ((!selectedKeys.length || selectedEmptyKey !== undefined) && emptyText) {
                menuMarkedKey =
                    selectedEmptyKey !== undefined ? selectedEmptyKey : this._emptyKeys[0];
            } else if (
    ...
    private _getCollection(
        ...
        const collectionConfig: ITreeGridCollectionOptions = {
            ...
            itemActionsBackgroundStyle: this._getHoverBackgroundStyle(options),
            root: options.root,
            unique: true,
            multiSelectAccessibilityProperty: options.multiSelectAccessibilityProperty || '',
            topPadding: 'null',
            bottomPadding: 'null',
            leftPadding: 'menu-null',
            itemsSpacing: null,
        ...
        // Проверка на viewMode добавлена, т.к. есть кейсы, когда пустое значение searchValue, но при это отображение
        // в режиме поиска. Например, кейс из ошибки:
        // https://online.sbis.ru/opendoc.html?guid=d43fc989-ccdc-472e-88c0-fecadd351542
        // p.s. Важно передавать в список коллекцию, соответствующую заданному viewMode, иначе возможны сбои.
        const isSearchModel = !!(
            options.searchParam &&
            (!!options.searchValue || this._viewMode === 'search')
        );
        ...
        if (isSearchModel) {
            ...
            if (this._viewMode === 'table') {
                listModel = new TreeGridCollection(searchConfig);
        ...
        if (
            !options.searchValue &&
            !(options.emptyKey instanceof Array) &&
            !items.getRecordById(options.emptyKey)
        ) {
    ...
    private _getCollectionFilters(options: IMenuControlOptions): Function[] {
        ...
        if (isSearchModel) {
            ...
            filters.push(MenuControl._groupVisibleFilter.bind(this, options));
        }

        if (!isSearchModel) {
            if (options.additionalProperty && !this._expander && this._expandButtonVisible) {
    ...
    private _getHoverBackgroundStyle(options: IMenuControlOptions): string {
        return options.hoverBackgroundStyle !== 'default'
            ? options.hoverBackgroundStyle
            : 'menu-default';
    }
    ...
    private _addSingleSelectionItem(
        ...
        const emptyConfig = {
            contents: getEmptyItemConfig(items, {
                ...options,
                emptyText: itemText,
                emptyKey: key,
                keyProperty: this._getKeyProperty(options, items),
    ...
    private _getSourceController(options: IMenuControlOptions): SourceController {
        ...
        if (!this._sourceController) {
            ...
            this._sourceController.setDataLoadCallback((loadedItems, direction) => {
                if (historyId) {
                    return Promise.resolve(
                        this._getPrepareHistoryUtils().then(({ getItemsWithHistory }) => {
                            return getItemsWithHistory(
                                loadedItems,
                                this._getHistoryId(options, source),
                                source,
                                this._getHistorySourceConfig(options)
                            );
                        })
                    ).then((newItems) => {
                        return (
                            (dataLoadCallback && dataLoadCallback(newItems, direction)) || newItems
                        );
                    });
                } else {
                    return (
                        (dataLoadCallback && dataLoadCallback(loadedItems, direction)) ||
                        loadedItems
                    );
                }
            });
    ...
    private _getPrepareHistoryUtils(): Promise<
        typeof import('Controls/Utils/History/PrepareMenuItems')
    > {
        if (!this._prepareHistoryUtilsLoaded) {
            this._prepareHistoryUtilsLoaded = loadAsync('Controls/Utils/History/PrepareMenuItems');
        }
        return this._prepareHistoryUtilsLoaded;
    }

    private _getDropdownLib(): Promise<typeof import('Controls/dropdown')> {
        if (!this._dropdownLibLoaded) {
            this._dropdownLibLoaded = loadAsync('Controls/dropdown');
        }
        return this._dropdownLibLoaded;
    }

    private async _itemsChangedCallback(): Promise<void> {
        if (this._listModel) {
            const items = this._sourceController.getItems();
            this._setButtonVisibleState(items, this._options);
            if (this._getHistoryId(this._options, this._source)) {
                this._listModel.setCollection(
                    loadSync('Controls/Utils/History/PrepareMenuItems').getItemsWithHistory(
                        items,
                        this._getHistoryId(this._options, this._source),
                        this._options.source,
                        this._getHistorySourceConfig(this._options)
                    )
                );
            } else {
    ...
    private _isExpandButtonVisible(items: RecordSet, options: IMenuControlOptions): boolean {
        ...
        if (options.additionalProperty) {
            items.each((item: Model): void => {
                if (!hasAdditional || !hasRegularItem) {
                    const isAdd = MenuControl._isAddItem(options, item);
                    if (!hasAdditional) {
                        hasAdditional = isAdd;
                    }
                    if (!hasRegularItem) {
                        hasRegularItem = !isAdd && getItemParentKey(options, item) === options.root; // Не доп запись из текущего корня
                    }
                }
            });
            hasAdditional = hasRegularItem && hasAdditional;
    ...
    private _needCheckVisibleItems(options: IMenuControlOptions): boolean {
        return options.allowPin && !options.subMenuLevel;
    }
    ...
    private _getPopupOptions(
        ...
        const getOptions = (templateOptions) => {
            const config = {
                ...
                templateOptions,
                target: target.querySelector('.controls-Menu__row') || target,
        ...
        return this._getTemplateOptions(item).then((templateOptions) => {
            return getOptions(templateOptions);
        });
    }
        ...
        return Promise.all(promises).then(([source]) => {
            const subMenuOptions: object = {
                ...
                subMenuTemplateOptions: {
                    ...
                    ...item.get('menuOptions'),
                    ...this._options.subMenuTemplateOptions,
                },
                ...
                menuClosedCallback: this._menuClosed,
    ...
    protected _prepareGroups(options: IMenuControlOptions, items: RecordSet): void {
        if (options.groupProperty) {
            items.setEventRaising(false, false);
            items.forEach((item) => {
                this._setGroupKey(options, item, items);
            });
            items.setEventRaising(true, true);
        }
    }

    protected _setGroupKey(options: IMenuControlOptions, item: Model, items: RecordSet): void {
        ...
        if (groupKey === undefined || groupKey === null || isHistoryItem) {
            ...
            if (options.parentProperty) {
                ...
                while (!parentGroup && parentKey) {
                    const parentItem = items.getRecordById(parentKey);
                    parentGroup = parentItem?.get(options.groupProperty);
                    parentKey = parentItem?.get(options.parentProperty);
                }
    ...
    private _prepareFormat(items: RecordSet): void {
        ...
        items.getFormat().forEach((field) => {
            ...
            addField(name, collection, collection.getFormat());
        });
    }
    ...
    private _itemActionsVisibility(
        ...
        if (action.id === 'pin' || action.id === 'unpin') {
            const hasPin = hasPinIcon(options, item, options.searchValue);
            if (action.id === 'pin') {
    ...
    private _getHistoryId(options, source): string | void {
        if (source instanceof PrefetchProxy) {
            source = source.getOriginal();
        }

        if (isHistorySource(source)) {
            return source.getHistoryId();
        }
    ...
    private _getHistorySourceConfig(options): object {
        return {
            parentProperty: options.parentProperty,
            nodeProperty: options.nodeProperty,
            keyProperty: this._getHistoryMenuKeyProperty(options),
            root: options.historyRoot,
        };
    }

    private _getHistoryMenuKeyProperty(options): string {
        return (
            options.items?.getKeyProperty() ||
            (options.source?.getOriginal && options.source?.getOriginal()?.getKeyProperty()) ||
            this._sourceController?.getItems()?.getKeyProperty()
        );
    }
    ...
    private static _additionalFilterCheck(
        ...
        if (item?.get) {
            ...
            isVisible = groupItems.some((groupItem) => {
                return !MenuControl._isAddItem(options, groupItem.contents);
            });
        }
        return isVisible;
    }
    ...
    private static _displayFilter(
        ...
        if (item && item.get) {
            ...
            isVisible =
                ...
                MenuControl._groupVisibleFilter(options, item, index, treeItem);
            if (isVisible) {
    ...
    private static _groupVisibleFilter(
        options,
        item: Model,
        index: number,
        treeItem: TreeItem
    ): boolean {
        if (treeItem['[Controls/_display/GroupItem]']) {
            const model = treeItem.getOwner();
            const root = model.getRoot().contents;
            const parentProperty = model.getParentProperty();
            const nodeProperty = model.getNodeProperty();
            ...
            return (
                groupItems?.some((gItem) =>
                    MenuControl._isVisibleItem(gItem.contents, items, {
                        root,
                        parentProperty,
                        nodeProperty,
                        hierarchyViewMode: options.hierarchyViewMode,
    ...
    private static _isVisibleItem(
        ...
        return (
            ...
            ((!!subMenuLevel || menuMode === 'selector') && hierarchyViewMode === 'tree')
        );
    }
 ...
 * @variant table Иерархический список.
 */

/**
 * @name Controls/_menu/Control#markerVisibility
 * @cfg {Controls/marker/TVisibility.typedef} Режим отображения маркера.
 * @default hidden
 ...
 * @cfg {Boolean} Определяет строить ли меню с историей. Для правильной работы опции в качестве {@link source источника} должен быть задан {@link Controls/historyOld:Source}
 ...
 * @cfg {String} Уникальный идентификатор истории. Для правильной работы опции в качестве {@link source источника} должен быть задан {@link Controls/historyOld:Source}
 ...
 * @cfg {boolean} Определяет разрешить ли запинивание записей. Для правильной работы опции в качестве {@link source источника} должен быть задан {@link Controls/historyOld:Source}
