    ...
    implements IMenuControl
{
    ...
    protected _itemClick(
        ...
        if (MenuControl._isPinIcon(sourceEvent?.target)) {
            ...
            if (
                ...
                (this._options.menuMode !== 'selector' || !isNode)
            ) {
    ...
    private _getCollection(
        ...
        if (isSearchModel) {
            ...
            const expanderPosition = this._getExpanderPosition(options);
    ...
    private _getLeftPadding(options: IMenuControlOptions): string {
        ...
        if (this._isAdaptive(options) && options.menuMode !== 'selector') {
    ...
    private _getExpanderPosition(
        { menuMode, expanderPosition }: IMenuControlOptions = this._options
    ): string {
        return menuMode === 'selector' ? expanderPosition : 'custom';
    }
    ...
    private _addSingleSelectionItem(
        ...
        const emptyConfig = {
            ...
            expanderPosition: this._getExpanderPosition(options),
    ...
    private _getSourceSubMenu(
        ...
        const targetIsHistorySource = isHistorySource(this._options.source);
        ...
        if (historyId) {
            if (
                (targetIsHistorySource &&
                    this._getHistoryId(this._options, this._options.source) !== historyId) ||
                !targetIsHistorySource
            ) {
                this._getDropdownLib().then(({ dropdownHistoryUtils }) => {
