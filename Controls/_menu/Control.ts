    ...
    implements IMenuControl
{
    ...
    private _changeSelection(key: string | number | null, isSelected?: boolean): void {
        ...
        let selection;
        const beforeSelectionResult = this._options.beforeSelectionChangedCallback?.(selectionDifference);
    ...
    private _updateSelectionAndNotify(selection: ISelectionObject, key: TKey, isSelected: boolean): void {
    ...
    private _getMarkedKey(
        ...
        if (multiSelect) {
            ...
            if (this._listModel.getHoveredItem()) {
                ...
                markedKey =
                    selectedKey === undefined && emptyKey !== undefined ? emptyKey : selectedKey;
            }
        }
        return markedKey;
    }
                ...
                listModel = new Tree({
                    ...
                    filter:
                        options.parentProperty && options.nodeProperty
                            ? MenuControl._displayFilter.bind(this, options.hierarchyViewMode)
                            : null,
                });
            }
    ...
    private _isExpandButtonVisible(items: RecordSet, options: IMenuControlOptions): boolean {
        ...
        if (options.additionalProperty && options.root === null) {
            items.each((item: Model): void => {
                if (!hasAdditional) {
                    hasAdditional =
                        item.get(options.additionalProperty) && !MenuControl._isHistoryItem(item);
                }
            });
        } else if (
    ...
    private _getPopupOptions(
        ...
        const targetPoint = {
            ...
            horizontal: subMenuDirection === 'bottom' ? 'center' : this._options.itemAlign,
        };
        const className =
            'controls-Menu__subMenu' +
            ` controls_popupTemplate_theme-${this._options.theme}
             ${subMenuDirection !== 'bottom' ? 'controls-Menu__subMenu_margin' : ''}`;
    ...
    private static _additionalFilterCheck(options: IMenuControlOptions, item: Model): boolean {
        return (
            !item.get || !item.get(options.additionalProperty) || MenuControl._isHistoryItem(item)
        );
    }
    ...
    private static _isVisibleItem(
        item: Model,
        items: RecordSet,
        { root, parentProperty, nodeProperty, hierarchyViewMode }: Partial<IMenuControlOptions>
    ): boolean {
