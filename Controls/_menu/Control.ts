    ...
    implements IMenuControl
{
    ...
    protected _beforeUpdate(newOptions: IMenuControlOptions): void {
        ...
        if (
            ...
            if (selectedKeysChanged && this._markerController) {
    ...
    private _updateSelectionController(newOptions: IMenuControlOptions): void {
        ...
        selectionController.setSelection({
            selected: this._getKeysForSelectionController(newOptions),
    ...
    private _updateEmptyItemMarker(selection: ISelectionObject): void {
        ...
        if (selection.selected.length === 1) {
            ...
            if (item) {
                isFixedItemSelected = MenuControl._isFixedItem(item, this._options.source);
            }
        }
    ...
    private _getCollection(
        ...
        if (isSearchModel) {
            ...
            if (MenuControl._isTreeViewRender(options)) {
                ...
                if (options.parentProperty && options.nodeProperty) {
                    filters.push(
                        MenuControl._displayFilter.bind(this, options.historyId, options.source)
                    );
                }
                listModel = new Tree({
                    ...collectionConfig,
    ...
    private _getTemplateOptions(item: CollectionItem<Model>): Promise<IMenuControlOptions> {
        ...
        const headingCaption = allowAdaptive
            ? sourceItem.get(this._options.displayProperty)
            : sourceItem.get(this._options.headingCaptionProperty);
    ...
    private static _isVisibleItem(
        ...
        const isHistoryItem =
            (!!historyId || isHistorySource(source)) && MenuControl._isHistoryItem(item);
