...
import {
    MarkerController,
    Visibility as MarkerVisibility,
...
import { ISiblingStrategy, FlatSiblingStrategy } from 'Controls/baseList';
    ...
    implements IMenuControl
{
    ...
    constructor(options: IMenuControlOptions) {
        ...
        this._additionalFilter = MenuControl._additionalFilterCheck.bind(
            this,
            options
        );
        this._limitHistoryFilter = this._limitHistoryCheck.bind(this);
        this._itemActionsVisibilityCallback = this._itemActionsVisibility.bind(
            this,
            options
        );
    }
    ...
    protected _afterMount(): void {
        if (this._options.openedSubMenuKey) {
    ...
    protected _beforeUpdate(newOptions: IMenuControlOptions): void {
        ...
        if (
            newOptions.isDragging &&
            this._options.isDragging !== newOptions.isDragging
        ) {
            this._closeSubMenu();
        }
        ...
        if (
            ...
            this._openSubMenuByKey(
                newOptions.openedSubMenuOptions,
                newOptions.openedSubMenuKey
            );
        }
        ...
        if (
            ...
            (selectedKeysChanged || newOptions.focusable) &&
            this._markerController
        ) {
    ...
    protected _getMoveAction(): MoveAction {
        const controllerOptions: IMoveActionOptions = {
            ...
            siblingStrategy: this._getSiblingsStrategy(),
        };
    ...
    protected _handleKeyDown(event: SyntheticEvent<KeyboardEvent>): void {
        ...
        if (code === constants.key.up) {
            ...
            code === constants.key.enter &&
            this._listModel.getHoveredItem()
        ) {
            this._notify('itemClick', [
                this._listModel.getHoveredItem().contents,
                event,
            ]);
            ...
            this._openSubMenuByKey(
                { autofocus: true },
                this._listModel.getHoveredItem().key
            );
    ...
    private _updateSelectedItemKeyboard(
        ...
        if (!markedItem) {
            markedItem = this._getMarkedItem(
                this._options.selectedKeys,
                this._options
            );
        }
    ...
    private _scrollToMarkedElement(): void {
        const element = this._container.querySelector(
    ...
    private _initHistoryControllerIfNeed(options: IMenuControlOptions): void {
        ...
        if (
            options.historyId &&
            options.hasHistory &&
            source instanceof HistorySource
        ) {
    ...
    protected _itemSwipe(
        ...
        if (!this._isSingleSelectionKey(itemKey)) {
    ...
    protected _itemClick(
        ...
        const treeItem: CollectionItem<Model> =
            this._listModel.getItemBySourceKey(key);
        ...
        if (MenuControl._isPinIcon(sourceEvent.target)) {
            ...
            if (this._options.multiSelectAccessibilityProperty) {
                const checkBoxState = item.get(
                    this._options.multiSelectAccessibilityProperty
                );
                if (
                        ...
                        if (this._historyController) {
                            this._historyController
                                ...
                                .then((items: RecordSet) => {
                                    if (this._sourceController) {
                                        items.setKeyProperty(
                                            this._listModel.getKeyProperty()
                                        );
    ...
    private _needCreateSelectionController(
        options: IMenuControlOptions
    ): boolean {
        return (
    ...
    private _updateMarkerController(newOptions: IMenuControlOptions): void {
        ...
        const markedKey = this._getMarkedKey(
            this._getSelectedKeys(newOptions),
            newOptions
        );
    ...
    private _getKeysForSelectionController(
        options: IMenuControlOptions
    ): TSelectedKeys {
        const selectedKeys = [];
        ...
        this._selectedKeys.forEach((key) => {
            ...
            if (
                ...
                selectedKeys.push(
                    item && MenuControl._isHistoryItem(item) ? String(key) : key
                );
            }
        });
        return selectedKeys;
    ...
    protected _checkBoxClick(
        ...
        if (item && item['[Controls/_display/BreadcrumbsItem]']) {
            if (this._options.multiSelect) {
    ...
    protected _isSingleSelectionKey(
        ...
        const isAllSelectedItem =
            options.selectedAllText && key === options.selectedAllKey;
    ...
    protected _openSelectorDialog(): void {
        ...
        if (this._options.selectorOpenCallback) {
            ...
            selectedItems = new List<Model>({
                items: this._getSelectedItems().filter(
                    (item: Model): boolean => {
    ...
    protected _subMenuResult(
        ...
        switch (eventName) {
            ...
            case 'pinClick':
                if (this._historyController) {
                    this._updateItems(
                        this._historyController.getItems(),
                        this._options
                    );
                }
            default:
                const notifyResult = this._notify(eventName, [
                    eventResult,
                    nativeEvent,
                ]);
                if (
    ...
    private _checkOpenedMenu(
        nativeEvent: MouseEvent,
        newItem?: CollectionItem<Model>
    ): void {
        ...
        if (
            !this._isNeedKeepMenuOpen(needCloseSubMenu, nativeEvent) &&
            needCloseSubMenu
        ) {
            this._closeSubMenu();
        }
    ...
    private _isNeedKeepMenuOpen(
        ...
        if (needCloseDropDown) {
            ...
            this._isMouseInOpenedItemArea =
                this._isMouseInOpenedItemAreaCheck(nativeEvent);
        } else {
    ...
    private _closeSubMenuByKey(key?: TKey): void {
        if (
            this._subDropdownItem &&
            this._subDropdownItem.getContents().getKey() === key
        ) {
            this.closeSubMenu();
    ...
    private _closeSubMenu(
        needOpenDropDown: boolean = false,
        closeFromCode: boolean = false
    ): void {
    ...
    private _isMouseInOpenedItemAreaCheck(curMouseEvent: MouseEvent): boolean {
        const firstSegment: number =
            MenuControl._calculatePointRelativePosition(
    ...
    private _toggleSelectedItem(selectedItems: Model[], key: TKey): void {
        ...
        if (selectedIndex > -1) {
            ...
            selectedItems.push(
                this._listModel.getItemBySourceKey(key).getContents()
            );
        }
    }
    ...
    private _updateEmptyItemMarker(selection: ISelectionObject): void {
        ...
        const isEmptySelected =
            this._options.emptyText && !selection.selected.length;
        const isAllItemSelected =
            this._options.selectedAllText && !selection.selected.length;
        if (isEmptySelected) {
    ...
    private _createControllers(options: IMenuControlOptions): void {
        if (options.markerVisibility !== MarkerVisibility.Hidden) {
            ...
            const markedKey =
                this._markerController.calculateMarkedKeyForVisible();
            this._markerController.setMarkedKey(markedKey);
        ...
        if (this._needCreateSelectionController(options)) {
            ...
            if (options.multiSelectAccessibilityProperty) {
                this._selectionChanged = this._getSelectedItems().some(
                    (item) => {
                        return (
                            item.get(
    ...
    private _setButtonVisibleState(
        items: RecordSet,
        options: IMenuControlOptions
    ): void {
    ...
    private _getMarkedKey(
        ...
        if (multiSelect) {
            if (
                (!selectedKeys.length || selectedKeys.includes(emptyKey)) &&
                emptyText
            ) {
                markedKey = emptyKey;
            } else if (
                selectedKeys.includes(selectedAllKey) &&
                selectedAllText
            ) {
                markedKey = selectedAllKey;
            } else {
                const item = this._listModel.getItemBySourceKey(
                    selectedKeys[0]
                );
            ...
            if (this._listModel.getHoveredItem()) {
                ...
                markedKey =
                    selectedKey === undefined && emptyKey !== undefined
                        ? emptyKey
                        : selectedKey;
            }
        }
    ...
    private _getSelectedKeys(options: IMenuControlOptions): TSelectedKeys {
        ...
        if (options.multiSelect) {
            selectedKeys =
                this._getSelectionController(options).getSelection().selected;
        } else {
    ...
    private _expandedItemsFilterCheck(
        item: CollectionItem<Model>,
        index: number
    ): boolean {
    ...
    private _isSelectedKeysChanged(
    ...
    private _updateSwipeItem(
        newSwipedItem: CollectionItem<Model>,
        isSwipeLeft: boolean
    ): void {
    ...
    protected _checkStickyHistoryItems(options: IMenuControlOptions): boolean {
        ...
        if (options.allowPin) {
            this._listModel.each((item) => {
                if (
                    item.getContents().get &&
                    item.getContents().get('doNotSaveToHistory')
                ) {
                    countSticky++;
                }
            });
    ...
    private _getCollection(
        ...
        if (isSearchModel) {
            const searchConfig = {
                ...
                filter:
                    options.allowPin && options.parentProperty
                        ? MenuControl._searchHistoryDisplayFilter.bind(
                              this,
                              options,
                              items
                          )
            ...
            if (options.hierarchyViewMode === 'tree' && options.subMenuLevel) {
    ...
    private _addSingleSelectionItem(
        ...
        const emptyItem = this._getSingleSelectionItem(
            itemText,
            key,
            items,
            options
    ...
    private _updateControllers(options: IMenuControlOptions): void {
        if (
            this._selectionController ||
            this._needCreateSelectionController(options)
        ) {
    ...
    private _isExpandButtonVisible(
        items: RecordSet,
        options: IMenuControlOptions
    ): boolean {
        ...
        if (options.additionalProperty && options.root === null) {
            ...
            hasAdditional =
                this._visibleIds.length > options.maxHistoryVisibleItems + 1;
            if (hasAdditional) {
    ...
    private _openSubMenuByKey(
        ...
        if (item && this._canOpenSubMenu(item)) {
            const dataName =
                this._options.dataName + '_item_' + item.getContents().getKey();
            const target = this._container.querySelector(
                `[data-target=${dataName}]`
            ) as HTMLElement;
            this._preventCloseSubMenu = true;
            this._openSubMenuEvent = null;
            this._subDropdownItem = item;
            this._openedTarget = target;
            const subMenuOptions = this._listModel.getItemBySourceKey(key)
                ? popupOptions
                : null;
    ...
    private _openSubMenu(
        ...
        if (item && this._children.Sticky && this._subDropdownItem) {
            this._getPopupOptions(target, item, menuPopupOptions).then(
                (popupOptions) => {
                    this._notify(
                        'beforeSubMenuOpen',
                        [
                            popupOptions,
                            this._options.subMenuDirection,
                            this._options.itemAlign,
                        ],
    ...
    private _getTemplateOptions(
        ...
        const selectedKeys = selection?.selected || this._options.selectedKeys;
        const excludedKeys = selection?.excluded || this._options.excludedKeys;
        const promises = [
            this._getSourceSubMenu(
                isLoadedChildItems,
        ...
        return Promise.all(promises).then(([source]) => {
            ...
            const subMenuOptions: object = {
                ...
                showClose: false,
                showHeader: false,
                headerTemplate: headingCaption
                    ? 'Controls/dropdown:HeaderTemplate'
                    : null,
    ...
    private _getSourceSubMenu(
        ...
        const targetIsHistorySource =
            this._options.source instanceof HistorySource;
        ...
        if (historyId) {
            if (
                (targetIsHistorySource &&
                    this._options.source.getHistoryId() !== historyId) ||
                !targetIsHistorySource
            ) {
                targetSource = dropdownHistoryUtils.getSource(
                    targetSource,
                    this._options
                );
            }
    ...
    private _createMenuSource(
        source: ISourcePropertyConfig | ICrudPlus
    ): Promise<ICrudPlus> {
        if (
    ...
    private _isLoadedChildItems(root: TKey): boolean {
        let isLoaded = false;
        const collection =
            this._listModel.getSourceCollection() as unknown as RecordSet<Model>;

        if (
            collection.getIndexByValue(this._options.parentProperty, root) !==
            -1
        ) {
            isLoaded = true;
        }
    ...
    private _subMenuDataLoadCallback(items: RecordSet): void {
        const origCollectionFormat = this._listModel
            .getSourceCollection()
            .getFormat();
    ...
    private _updateItemActions(
        ...
        this._itemActionsController.update({
            ...
            itemActionsClass:
                options.itemActionsClass ||
                'controls-Menu__itemActions_position_rightBottom',
            iconSize: editingConfig ? 's' : 'm',
        });
    }

    private _getItemActionsWithPin(
    ...
    private _processError(error: Error): Promise<ErrorViewConfig | void> {
        ...
        return this._getErrorController()
            ...
            .then(
                (
                    errorConfig: ErrorViewConfig | void
    ...
    private static _calculatePointRelativePosition(
        ...
        return (
            (firstSegmentPointX - curPointX) *
                (secondSegmentPointY - firstSegmentPointY) -
            (secondSegmentPointX - firstSegmentPointX) *
                (firstSegmentPointY - curPointY)
        );
    }
    ...
    private static _isItemCurrentRoot(
        item: Model,
        options: IMenuControlOptions
    ): boolean {
