    ...
    implements IMenuControl
{
    ...
    private _updateSelectedItemKeyboard(event: SyntheticEvent, direction: string): void {
        ...
        if (!hoveredItem) {
            ...
            if (direction === 'previous') {
                ...
                if ((isFirstItem && !this._options.emptyText) || hoveredItem === this._emptyItem) {
    ...
    private _findNearbyItem(index: number, next?: boolean) {
        const isEnabled = (cItem?: CollectionItem) =>
            cItem && cItem.Markable && !cItem.contents?.get?.('readOnly');
    ...
    protected _itemClick(
        ...
        if (MenuControl._isPinIcon(sourceEvent?.target)) {
            ...
            if (this._options.multiSelectAccessibilityProperty) {
                checkBoxState = item.get(this._options.multiSelectAccessibilityProperty);
                if (checkBoxState === MultiSelectAccessibility.disabled) {
                    return;
                }
            }
            if (
