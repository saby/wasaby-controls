...
import { TSelectedKeys, ISelectionObject } from 'Controls/interface';
...
import { TreeCollection as Tree } from 'Controls/tree';
    ...
    implements IMenuControl
{
    ...
    protected _beforeUpdate(newOptions: IMenuControlOptions): void {
        ...
        if (
            ...
            } else if (rootChanged && !this._expander && !newOptions.searchValue) {
    ...
    protected _afterUpdate(oldOptions: IMenuControlOptions) {
        if (
    ...
    protected _itemClick(
        ...
        const treeItem: CollectionItem<Model> = this._listModel.getItemBySourceKey(key, false);
    ...
    private _needTouchOpenMenu(): boolean {
        return (
            (this._options.isAdaptive && this._options.allowAdaptive !== false) ||
            ((this._options.hierarchyViewMode !== 'tree' || !this._options.subMenuLevel) &&
    ...
    private _getKeysForSelectionController(options: IMenuControlOptions): TSelectedKeys {
        ...
        this._selectedKeys.forEach((key) => {
            ...
            if (
                ...
                const itemKey = item?.get(options.keyProperty);
                selectedKeys.push(
                    item && typeof itemKey === 'string' && MenuControl._isHistoryItem(item)
                        ? String(key)
                        : key
                );
            }
    ...
    private _changeSelection(key: string | number | null, isSelected?: boolean): void {
        ...
        this._updateSelectionAndNotify(this._selectedKeys, this._excludedKeys, key, isSelected);
    }

    protected _markedKeyChanged(event, markedKey): void {
        const isNode = this._listModel.getItemBySourceKey(markedKey)?.isNode();
        if (!isNode) {
            this._markedKey = markedKey;
        }
    }

    protected _selectedKeysChanged(event, newKeys, added) {
        if (!isEqual(this._selectedKeys, newKeys)) {
            this._selectedKeys = newKeys;
            this._updateSelectionAndNotify(newKeys, this._excludedKeys, added?.[0], !added?.length);
        }
    }
    ...
    protected _excludedKeysChanged(event, newKeys, added) {
        this._excludedKeys = newKeys;
        this._updateSelectionAndNotify(this._selectedKeys, newKeys, added?.[0], !!added?.length);
    }
    ...
    protected _setStateByItems(items: RecordSet, options: IMenuControlOptions): void {
        ...
        if (options.menuMode !== 'selector' || !this._listModel) {
    ...
    private _getExpandedItems(items: RecordSet, options: IMenuControlOptions): TKey[] {
        let expandedItems = [];
        if (this._hasHiddenNode(items, options)) {
            expandedItems = [null];
    ...
    private _setMarkerPosition(
        items: RecordSet,
        { markerPosition, root, parentProperty }: IMenuControlOptions
    ): void {
        if (markerPosition) {
        ...
        this._columns[0].templateOptions.markerPosition = withIcons ? 'right' : 'left';
    }
    ...
    private _getCollection(
        ...
        if (isSearchModel) {
            const searchFilters = [MenuControl._searchFilter];

            if (options.allowPin && options.parentProperty) {
                searchFilters.push(MenuControl._searchHistoryDisplayFilter.bind(this, items));
            }
        ...
        if (options.additionalProperty && !this._expander) {
            ...
            !isSearchModel &&
            options.allowPin &&
            (!options.subMenuLevel || (options.menuMode === 'selector' && !options.root)) &&
            !this._expander &&
    ...
    private _getTemplateOptions(item: Model): Promise<IMenuControlOptions> {
        const root: TKey = item.get(this._options.keyProperty);
        const allowAdaptive = this._options.isAdaptive && this._options.allowAdaptive;
    ...
    protected _setGroupKey(options, item): void {
        ...
        if (groupKey === undefined || groupKey === null || isHistoryItem) {
            try {
                item.set(options.groupProperty, constView.hiddenGroup);
            } catch (e) {
                Logger.error(
    ...
    private static _displayFilter(
        ...
        let isVisible: boolean = true;
        if (item && item.get && !MenuControl._isSpacer(treeItem)) {
            isVisible = isVisibleItem(item);
        } else if (treeItem['[Controls/_display/GroupItem]']) {
            isVisible =
                model.getNext(treeItem)?.contents.get(model.getGroupProperty()) === item &&
    ...
    private static _isVisibleItem(
        ...
        return (
            ...
            (root !== null && hierarchyViewMode === 'tree') ||
            menuMode === 'selector'
        );
    }
