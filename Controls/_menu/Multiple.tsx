...
export interface IMultipleMenu {
    /**
     * @name Controls/menu:Multiple#menuConfigs
     * @cfg {Object.<string, Controls/multipleDropdown:IDropdownConfigArgs>} Настройки для каждого меню.
     * @example
     * <pre>
     *     menuConfigs = {
     *         traffic: {
    ...
    /**
     * @name Controls/menu:Multiple#selectedKeys
     * @cfg {Object<string, TKey>} Выбранные ключи.
     * @example
     * <pre>
     *     selectedKeys = {
     *         traffic: ['key1'],
    ...
    /**
     * @name Controls/menu:Multiple#onSelectedKeysChanged
 ...
 * Необходимо использовать, когда нужно отобразить несколько меню в одном окне.
 ...
 *
 * @public
 * @demo Controls-demo/Menu/Multiple/Simple/Index
 */
export default React.forwardRef(function MultipleMenu(props: IMultipleMenu, ref): JSX.Element {
    ...
    const dataLoadCallback = React.useCallback(
        (items, name) => {
            return (props.menuConfigs[name].dataLoadCallback || props.dataLoadCallback)?.(
                items,
                name
            );
        },
    ...
    return (
        <div ref={ref} className={`controls-menu-multiple ${props.className || ''}`}>
            {menuNames.map((name, index) => {
                ...
                return (
                    <div
                        key={`controls-multiple-menu-${name}`}
                        className={index < menuNames.length - 1 ? 'controls-margin_bottom-xs' : ''}
                        item-key={name}
                        data-qa={'Controls_menu_Multiple__menu'}
                    >
                        <BeforeMenuTemplate
...
const BodyContentTemplate = React.forwardRef(function (props, ref): JSX.Element {
    ...
    const dataLoadCallback = React.useCallback(
        (items) => {
            return props.dataLoadCallback(items, props.name);
        },
    ...
    const onMoreButtonClick = React.useCallback(
        (selectedItems, selectedKeys) => {
            props.onMoreButtonClick(selectedItems, selectedKeys, props.name);
        },
