...
import { ICollectionItemContextValue } from 'Controls/listsCommonLogic';
...
class MenuRender extends Control<IMenuRenderOptions> {
    ...
    protected _collectionItemContextValue: ICollectionItemContextValue = {
        item: this._options?.emptyItem,
        itemContents: Array.isArray(this._options?.emptyItem?.contents)
            ? this._options.emptyItem.contents[this._options.emptyItem.contents.length - 1]
            : this._options?.emptyItem?.contents,
    };
    ...
    protected _beforeUpdate(newOptions: IMenuRenderOptions): void {
        if (this._options.expandedItems !== newOptions.expandedItems) {
            this._expandedItems = newOptions.expandedItems;
        }
    }

    scrollToItem(key: TKey, position?: string, force?: boolean): void {
        this._children.list.scrollToItem(key, position, force);
    }
    ...
    protected _emptyItemClick(e: SyntheticEvent<MouseEvent>, item: Model): void {
        return this._notify('itemClick', [item, e]);
    }
    ...
    protected _beforeItemExpand(e: SyntheticEvent<MouseEvent>, item: Model): void {
        this._notify('beforeItemExpand', [item, e]);
    }

    protected _afterItemCollapse(e: SyntheticEvent<MouseEvent>, item: Model): void {
        this._notify('afterItemCollapse', [item, e]);
    }
