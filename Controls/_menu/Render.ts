...
class MenuRender extends Control<IMenuRenderOptions> {
    ...
    private removeEmptyItemFromCollection(): void {
        ...
        const emptyItem =
            options.emptyText && listModel.getItemBySourceKey(options.emptyKey);
        const allSelectedItem =
            options.selectedAllText &&
            listModel.getItemBySourceKey(options.selectedAllKey);
        ...
        if (allSelectedItem) {
            listModel
                .getSourceCollection()
                .remove(allSelectedItem.getContents());
        }
    }
    ...
    protected _isSingleSelectionItem(treeItem: TreeItem<Model>): boolean {
        ...
        if (item instanceof Model) {
            if (
                ...
                item.getKey() === this._options.selectedAllKey
            ) {
                result = true;
    ...
    protected _getItemData(treeItem: TreeItem<Model>): object {
        return {
            ...
            iconSize: treeItem.getContents()
                ? this._getIconSize(treeItem.getContents())
                : null,
    ...
    protected _isGroupVisible(groupItem: GroupItem): boolean {
        ...
        const itemsGroupCount = collection.getGroupItems(
            groupItem.getContents()
        ).length;
        return (
            !groupItem.isHiddenGroup() &&
            itemsGroupCount > 0 &&
            itemsGroupCount !== collection.getCount(true)
        );
    }
    ...
    private _getRightPaddingClass(item: Model): string {
        ...
        if (hasPinIcon(this._options, item)) {
            ...
            paddingClass = ' controls-ListView__item-rightPadding_menu-pin';
        } else if (
            item.get(this._options.nodeProperty) ||
            (this._options.itemAlign === 'left' && item.get('icon'))
        ) {
            paddingClass = ' controls-ListView__item-rightPadding_s';
        } else {
            paddingClass = ` controls-ListView__item-rightPadding_${
                this._options.itemPadding.right || 's'
            }`;
        }
        return paddingClass;
    }
