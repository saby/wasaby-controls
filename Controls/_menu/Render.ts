    ...
    protected _getMenuClassList(treeItem: TreeItem<Model>): string {
        ...
        if (item && item.get) {
            ...
            if (
                ...
                classes += ` controls-margin_bottom-${this._options.itemsSpacing || '3xs'}`;
            }
            ...
            if (!treeItem.isLastItem() && this._options.itemsSpacing) {
        ...
        options.listModel.each((item) => {
            ...
            if (icon) {
                ...
                if (parent === options.root) {
                    iconPadding =
                        itemContents.get('iconSize') || this.getIconSize(icon) || options.iconSize;
                }
            }
        });
        return iconPadding;
