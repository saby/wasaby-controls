...
const BROWSER_DEPS = ['Controls/browser', 'Controls/list'];

const SEARCH_DEPS = ['Controls/search', 'Controls/searchDeprecated'];
...
class Popup extends Control<IMenuPopupOptions> implements IMenuPopup {
    ...
    protected _shadowVisible: boolean = true;
    protected _searchValue: string;
    protected _minWidth: number;
    protected _minHeight: number;
    protected _markerPosition: string;
    protected _expanderPosition: string;
    protected _adaptiveMenuIsSwiped = false;
    protected _footerTemplate: Function;
    protected _footerItemData: IFooterItemData | void;
    private _stickyPositionFixed: boolean = false;
    private _openedSubMenuTarget: EventTarget;

    protected _beforeMount(options: IMenuPopupOptions): Promise<void | Error> | void {
        ...
        if (this._closeButtonVisibility && !this._isAdaptive(options)) {
            if (options.multiSelect || !this._hasHeader || options.closeButtonViewMode) {
        ...
        if (options.items) {
            this._updateStateByItems(options.items, options);
        }
        ...
        if (options.searchParam || options.menuMode === 'selector') {
            let deps = BROWSER_DEPS;
            if (options.searchParam) {
                deps = deps.concat(SEARCH_DEPS);
            }
    ...
    protected _afterMount() {
        const isAdaptive = this._isAdaptive(this._options);
        if (isAdaptive) {
            const maxHeight =
                this._options.slidingPanelOptions?.maxHeight || document?.body?.clientHeight;
            this._adaptiveMenuIsSwiped = this._container.clientHeight === maxHeight;

            if (this._options.allowPin) {
    ...
    protected _beforeUpdate(newOptions: IMenuPopupOptions): void {
        ...
        if (newOptions.stickyPosition && newOptions.stickyPosition.direction) {
            ...
            if (newOptions.subMenuLevel && !this._stickyPositionFixed) {
    ...
    protected _menuResized(): void {
        if (this._options.menuMode === 'selector' && !this._isAdaptive(this._options)) {
            const content = this._children.contentTemplate;
            if (this._minWidth < content?.clientWidth) {
                this._minWidth = content?.clientWidth;
            }
            const containerHeight = this._container?.querySelector('.controls-Menu__popup-template')
                ?.clientHeight;
            if (this._minHeight < containerHeight) {
                const bodyDimensions = DimensionsMeasurer.getElementDimensions(document.body);
                this._minHeight = Math.min(containerHeight, bodyDimensions.clientHeight * 0.6);
            }
        }
    }

    protected _getMinHeight(): string {
        if (this._options.isAdaptive && this._options.allowAdaptive !== false) {
            if (!this._options.isMinHeightSet && !this._options.subMenuLevel) {
                return '25vh';
            }
            if (this._options.slidingPanelOptions.minHeight) {
                return `${this._options.slidingPanelOptions.minHeight}px`;
            }
        }
    ...
    protected _swipeMenu(event, direction): void {
        if (this._isAdaptive(this._options) && this._options.searchParam) {
            this._children.menu.activate();
        }
    ...
    protected _headerBreadCrumbsClick(event, breadcrumbsItem: Model): void {
        event.stopPropagation();
        if (this._items) {
            this._root = breadcrumbsItem.get(this._options.parentProperty);
    ...
    protected _dataLoadCallback(options: IMenuPopupOptions, items: RecordSet): void {
        ...
        this._updateStateByItems(items, options);
        if (options.dataLoadCallback) {
            return options.dataLoadCallback(items);
        }
    }
    ...
    protected _prepareSubMenuConfig(
        ...
        event.stopPropagation();
        if (this._isAdaptive(this._options)) {
    ...
    protected _updateCloseButtonState(
        ...
        if (
            !this._isAdaptive(this._options) &&
            !this._hasHeader &&
    ...
    private _isAdaptive(options: IMenuPopupOptions): boolean {
    ...
    private _updateStateByItems(items: RecordSet, options: IMenuPopupOptions): void {
        this._updateHeadingIcon(items, options);
        this._setMarkerPosition(items, options);
        this._setExpanderPosition(items, options);
    }

    private _setExpanderPosition(items: RecordSet, options: IMenuPopupOptions) {
        this._expanderPosition = options.expanderPosition || 'default';
        if (
            options.menuMode === 'selector' &&
            options.nodeProperty &&
            options.parentProperty &&
            this._withIcons(items, options)
        ) {
            this._expanderPosition = 'custom';
        }
    }

    private _setMarkerPosition(
        items: RecordSet,
        { markerPosition, root, parentProperty }: IMenuPopupOptions
    ): void {
        if (markerPosition) {
            this._markerPosition = markerPosition;
            return this._markerPosition;
        }
        this._markerPosition = this._withIcons(items, { root, parentProperty }) ? 'right' : 'left';
    }

    private _withIcons(items: RecordSet, { root, parentProperty }: IMenuPopupOptions): boolean {
        let withIcons = false;
        items.each((item) => {
            ...
            if (
                !withIcons &&
                item.get('icon') &&
                (itemParent === undefined || itemParent === root)
            ) {
                withIcons = true;
            }
        });
    ...
    private _updateFooterTemplate(options: IMenuPopupOptions): void {
        this._footerItemData = options.footerItemData;
        if (options.menuMode !== 'selector' || !this._root) {
            ...
            if (options.menuMode === 'selector') {
                this._footerItemData = {
                    key: this._root,
                    item: this._items?.getRecordById(this._root),
                };
            }
        }
    }
    ...
    private _setItemPadding(options: IMenuPopupOptions): void {
        ...
        } else if (this._closeButtonVisibility && !this._isAdaptive(options)) {
            this._itemPadding = {
                right: 'menu-close',
            };
        }
    ...
    private _getOriginalSource(): TSourceOption {
        let source: TSourceOption = this._options.source as TSourceOption;
        if (source instanceof PrefetchProxy) {
            source = source.getOriginal<TSourceOption>();
        }
        return source;
    }
    ...
    private _setCloseButtonVisibility(options: IMenuPopupOptions): void {
        this._closeButtonVisibility =
            (!this._isAdaptive(options) || !!options.subMenuLevel) &&
            !!(
    ...
    private _getHeaderTemplate(options: IMenuPopupOptions): TemplateFunction {
        ...
        if (options.headerContentTemplate) {
            ...
            options.searchParam ||
            (options.menuMode === 'selector' &&
                (this._headingIcon || this._headingCaption) &&
    ...
    private _updateHeadingIcon(items: RecordSet, options: IMenuPopupOptions): void {
        let iconSize;
        let headingIconSize;
    ...
    private _calculatePinnedItems(): number {
        ...
        itemsContainer?.childNodes.forEach((item, index) => {
            if (!isStop) {
                const itemNode = item.querySelector(MENU_PINNED_SELECTOR);
                const historyNode = item.querySelector(MENU_HISTORY_GROUP);
                if (itemNode) {
                    if (!index) {
                        // Добавим высоту половины элемента, чтобы было видно, что снизу есть еще записи
                        height += itemNode.clientHeight / 2;
                    }
                    height =
                        height +
                        itemNode.clientHeight +
                        parseInt(getComputedStyle(itemNode).marginBottom, 10);
                } else if (historyNode) {
                    height += historyNode.clientHeight;
                    isStop = true;
                } else {
                    isStop = true;
                }
...
/**
 * @name Controls/_menu/Popup#footerBackgroundStyle
 * @cfg {backgroundStyle} Цвет фона подвала меню.
 * @demo Controls-demo/Menu/Popup/FooterBackgroundStyle/Index
 * @default default
 */
