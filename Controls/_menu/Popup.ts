...
import { getMarkerPosition } from 'Controls/_menu/Utils/getMarkerPosition';
...
class Popup extends Control<IMenuPopupOptions> implements IMenuPopup {
    ...
    protected _headingIcon?: string | null;
    protected _headingIconSize?: string | null;
    ...
    protected _beforeUnmount(): void {
        ...
        this._options.menuClosedCallback?.({
            container: this._container,
    ...
    protected _menuResized(): void {
        if (!this._isAdaptive(this._options)) {
            ...
            // Ширину могут задать в настройках попапа, тогда не надо устанавливать min-width
            const stickyWidth = this._options.stickyPosition?.sizes.width;
            if (this._minWidth < content?.clientWidth && content?.clientWidth <= stickyWidth) {
                this._minWidth = content?.clientWidth;
            } else if (this._minWidth > stickyWidth) {
                this._minWidth = 0;
            }
        }
    }
    ...
    private _updateStateByItems(items: RecordSet, options: IMenuPopupOptions): void {
        if (!this._searchValue) {
            this._markerPosition = getMarkerPosition(items, options);
    ...
    private _getHeaderTemplate(options: IMenuPopupOptions): TemplateFunction {
        ...
        } else if ((this._headingIcon || this._headingCaption) && !options.headerTemplate) {
    ...
    private _updateHeadingIcon(items: RecordSet, options: IMenuPopupOptions): void {
        ...
        if (this._headingIcon && !this._searchValue) {
            ...
            const iconInRoot = getIconInRoot(items, options);
            this._headingIconSize = iconInRoot
                ? iconInRoot.get('iconSize') || options.iconSize || 'm'
                : 'm';
        }
    }
