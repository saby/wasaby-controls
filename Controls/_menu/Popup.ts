...
class Popup extends Control<IMenuPopupOptions> implements IMenuPopup {
    ...
    protected _prepareSubMenuConfig(
        ...
        if (this._options.isAdaptive && this._options.allowAdaptive) {
            popupOptions.minHeight = this._container.clientHeight;
        }
    ...
    private _updateApplyButton(): void {
        if (this._options.applyButtonVisibility !== 'hidden') {
    ...
    private _isKeysChanged(newKeys: TSelectedKeys, oldKeys: TSelectedKeys): boolean {
