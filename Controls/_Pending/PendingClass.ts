...
class PendingClass {
    ...
    registerPending(def: Promise<void>, config: IPendingConfig = {}): void {
        ...
        if (config.showLoadingIndicator && !def.isReady()) {
            ...
            const indicatorConfig = {
                id: pending.loadingIndicatorId,
                fixConsiderPopups: config.fixConsiderPopups,
            };
    ...
    private _validatePromise(promise: Promise): boolean {
        return (
            promise &&
            (promise instanceof Promise || promise._moduleName === 'Types/deferred:Deferred')
        );
    }
        ...
        Object.keys(pendingRoot).forEach((key) => {
            ...
            if (isValid) {
                if (pending.onPendingFail) {
                    pending.onPendingFail(forceFinishValue, pending.def, options);
                }
