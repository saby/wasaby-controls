...
import type { THistoryId, IHistoryItem, THistoryLoadConfig } from 'Controls/HistoryStore';
import { Serializer } from 'UI/State';
import { Logger } from 'UI/Utils';
import type { IPrefetchParams } from './interface/IPrefetch';
...
function getFilterHistoryItemData(historyItem: Model<IHistoryItem>): IFilterHistoryData {
    const historyJSON = historyItem.get('ObjectData');
    try {
        return JSON.parse(historyJSON as string, new Serializer().deserialize);
    } catch (e) {
        Logger.error(
            `Не удалось сериализовать историю фильтров с идентификатором: ${historyItem.get(
                'HistoryId'
            )}. Данные в истории: ${historyJSON}`
        );
        return {
            items: [],
        };
 ...
 * если в истории по переданному historyId сохранён применённый фильтр, в противном случае метод вернёт пустой массив.
...
function getHistoryItems(
    ...
    return loadFilterHistory(historyOptions)
        .then(async () => {
            ...
            const recent = getHistoryStore().getLocal(historyId).recent;
            ...
            if (historyResult.items) {
                ...
                if (updateApplyDate) {
                    await getHistoryStore().delete(historyOptions.historyId, historyResultKey);
                    await update(historyResult, historyOptions.historyId);
                }
            }
...
function mergeHistoryParams(
    ...
    historyIds.forEach((id) => {
        const lastFilter = getHistoryStore().getLocal(id)?.recent?.at(0);
        if (lastFilter) {
...
function getItemsByHistory(historyId: string, maxCount: number): RecordSet<IHistoryItem> {
    const historyData = getHistoryStore().getLocal(historyId);
...
function findItemInHistory(
     ...
     */
    if (!getHistoryStore().getLocal(historyId)) {
        return;
    }
...
function updateFilterHistory(
    ...
    if (currentHistoryItem) {
        return getHistoryStore().update(historyId, currentHistoryItem.item.getKey(), historyData);
    } else {
        return getHistoryStore().push(historyId, historyData);
    }
}
...
function update(
    ...
    if (isLoaded('Controls/HistoryStore') && getHistoryStore().getLocal(historyId)) {
...
function getHistoryStore(): typeof import('Controls/HistoryStore').Store {
