...
function getHistoryItems(
    ...
    return loadFilterHistory(historyOptions)
        .then((res) => {
            ...
            let hasChangedDateFilterItem;
            let hasFilterItemResetValue: boolean = true;
            ...
            if (historyResult.applyDate && historyResult.items) {
                if (Array.isArray(filterDescription)) {
                    historyResult = historyResult.items.map((historyResultItem) => {
                        ...
                        if (filterDescriptionItem && isDateFilterItem(filterDescriptionItem)) {
                            ...
                            const daysAfterAppliedFilter = Math.ceil(
                                (new DateTime() - historyResult.applyDate) / millisecondsInDay
                            );
                            hasFilterItemResetValue =
                                filterDescriptionItem.resetValue !== undefined;
                            if (
                                hasFilterItemResetValue &&
                                !isEqual(
                                    filterDescriptionItem.value,
                                    filterDescriptionItem.resetValue
                                )
                            ) {
                                hasChangedDateFilterItem = true;
                            }
                            if (daysAfterAppliedFilter > 3) {
                                const resetItem =
                ...
                if (hasChangedDateFilterItem) {
                    removeItem(historyOptions.historyId, historyResultKey);
                }

                if (hasFilterItemResetValue) {
