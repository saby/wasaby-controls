...
function getHistoryItems(
    ...
    return loadFilterHistory(historyOptions)
        .then(() => {
            ...
            const recent = getHistoryStore().Store.getLocal(historyId).recent;
            ...
            if (historyResult.items) {
                ...
                if (updateApplyDate) {
                    getHistoryStore().Store.delete(historyOptions.historyId, historyResultKey);
...
function mergeHistoryParams(
    ...
    historyIds.forEach((id) => {
        const lastFilter = getHistoryStore().Store.getLocal(id)?.recent?.at(0);
        if (lastFilter) {
...
function getItemsByHistory(historyId: string, maxCount: number): RecordSet<IHistoryItem> {
    const historyData = getHistoryStore().Store.getLocal(historyId);
...
function findItemInHistory(
     ...
     */
    if (!getHistoryStore().Store.getLocal(historyId)) {
        return;
    }
...
function updateFilterHistory(
    ...
    if (currentHistoryItem) {
        return getHistoryStore().Store.update(
            historyId,
            currentHistoryItem.item.getKey(),
            historyData
        );
    } else {
        return getHistoryStore().Store.push(historyId, historyData);
    }
}
...
function update(
    ...
    if (isLoaded('Controls/HistoryStore') && getHistoryStore().Store.getLocal(historyId)) {
...
function getHistoryStore(): typeof import('Controls/HistoryStore') {
    return loadSync('Controls/HistoryStore');
}
