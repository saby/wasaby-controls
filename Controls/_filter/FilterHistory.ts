...
function getFilterHistorySource(historyOptions: IFilterHistoryOptions): typeof FilterSource {
    if (!hasSourceInStore(historyOptions.historyId)) {
        ...
        const source = new FilterSource({
            ...
            historySource: isRegistered('demoSourceHistory')
                ? resolve('demoSourceHistory', historySourceData)
...
function resolveHistoryDataForSave(
    ...
    if (
        FilterDescription.isFilterDescriptionChanged(
            getFilterDescriptionForHistory(filterDescription)
        ) ||
        getHistoryIdsFromDescription(filterDescription).length
    ) {
        result = prefetchParams
            ? getPrefetch().addPrefetchToHistory(result, prefetchParams)
            : result;
        result.items = minimizeFilterDescription(filterDescription);
    }
    return result;
}
