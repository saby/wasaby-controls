...
/**
 * Получает массив двух дат.
 * @param {Date} today
 */
function getYesterdayPeriod(today: Date): [Date, Date] {
    return [
        new EntityDate(today.getFullYear(), today.getMonth(), today.getDate() - 1),
        new EntityDate(today.getFullYear(), today.getMonth(), today.getDate() - 1),
    ];
}
...
export function getDates(filterItem: IFilterItem): [Date, Date] | null {
    ...
    switch (filterItem.value) {
        ...
        case 'yesterday':
            return getYesterdayPeriod(today);
        case 'week':
            return periodType === 'last'
