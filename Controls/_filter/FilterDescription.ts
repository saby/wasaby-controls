...
import type { IPrefetchParams } from 'Controls/_filter/interface/IPrefetch';
import { TFilter } from 'Controls/_interface/IFilter';
import FilterCalculator from './FilterCalculator';
import { TFilterItemLocalName } from './interface/IFilterItemLocal';
import { IAbstractListStateParts } from 'Controls-DataEnv/abstractList';
...
export function applyFilterUserHistoryToDescription(
    ...
    return itemsClone.map((item) => {
        const filterConfiguration = userFilterConfig.find(({ name }) => name === item.name);
            ...
            Object.keys(filterConfiguration).forEach((key) => {
                if (item.hasOwnProperty(key)) {
                    if (typeof item[key] === 'object') {
                        ...
                        item[key] = filterConfiguration[key];
                    }
                }
            });
        }
...
function loadAndCallCallbacks(
    ...
    const FilterLoader = loadSync<typeof import('Controls/filter').FilterLoader>(
        'Controls/filter:FilterLoader'
    );
...
function getItemByName(filterDescription: IFilterItem[], name: string): IFilterItem | undefined {
...
function applyFilterCounter(
    ...
    if (countValue !== undefined) {
        ...
        countFilterLinkedNames?.forEach((linkedFilterName) => {
            ...
            let countFilter;
            if (countValue === null && !countFilterValueConverter) {
                // Когда выбран другой фильтр по дате, фильтр нужно сбросить
                editorOptions.filter = { ...editorOptions.filter };
                delete editorOptions.filter[FILTER_PARAM];
            } else {
                let valueConverterCallback = countFilterValueConverter;
                if (typeof countFilterValueConverter === 'string') {
                    valueConverterCallback = loadSync(countFilterValueConverter);
                }
                if (valueConverterCallback) {
                    countFilter = valueConverterCallback(countValue, linkedItem, filterDescription);
                } else {
                    const dates = FilterCalculator.getDatesByFilterItem({
                        name: '',
                        value: countValue,
                        editorOptions: {
                            userPeriods: countFilterUserPeriods,
                            periodType: countFilterPeriodType,
                        },
                    });
                    countFilter = {
                        [FILTER_PARAM]: dates,
            ...
            setFilterItemProperty(linkedItem, 'editorOptions', editorOptions, 'panel');
        });
        return filterDescription;
    }
...
function isDateType(type: IFilterItem['type']): boolean {
    return type === 'dateRange' || type === 'date';
}

function isDateRangeFilterChanged(filterDescription: IFilterItem[]) {
    return !!filterDescription?.find(
        ({ value, resetValue, editorTemplateName, type }) =>
            (isDateType(type) || DATE_RANGE_EDITORS.includes(editorTemplateName)) &&
            !isEqual(value, resetValue)
    );
}

export function isFrequentDateRangeItem({
    type,
    editorTemplateName,
    viewMode,
}: IFilterItem): boolean {
    return (
        isDateType(type) ||
        (DATE_RANGE_EDITORS.includes(editorTemplateName) && viewMode === 'frequent')
    );
}

export function getFilterItemProperty<T>(
    item: IFilterItem,
    prop: keyof IFilterItem,
    filterItemLocalProperty?: TFilterItemLocalName
): T {
    if (filterItemLocalProperty) {
        return item[filterItemLocalProperty]?.[prop] ?? item[prop];
    } else {
        return item[prop] as T;
    }
}

function setFilterItemProperty(
    item: IFilterItem,
    prop: keyof IFilterItem,
    value: unknown,
    filterItemLocalProperty?: TFilterItemLocalName
): void {
    if (filterItemLocalProperty && item[filterItemLocalProperty]?.[prop]) {
        item[filterItemLocalProperty][prop] = value;
    } else {
        item[prop] = value;
    }
}

function getExtendedCaption(
    item: IFilterItem,
    filterItemLocalProperty?: TFilterItemLocalName
): string | undefined {
    return (
        getFilterItemProperty(item, 'extendedCaption', filterItemLocalProperty) ||
        (item.editorOptions?.extendedCaption as string | undefined)
    );
}
