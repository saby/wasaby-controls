...
import { default as FilterController, IFilterHistoryData } from 'Controls/_filter/ControllerClass';
...
import { connectToDataContext, IContextValue, IFilterSlice } from 'Controls/context';
...
class Container extends Control<IFilterViewContainerOptions> {
    ...
    protected _getFilterController(
        ...
        return dataOptions.getStoreData<Record<string, IFilterSlice>>(key, 'list')
            ?.filterController;
    }
    ...
    private _getSourceByHistory(source: unknown, historyItems: IFilterItem[]): IFilterItem[] {
        let result;
        if (source) {
