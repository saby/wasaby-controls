...
const DateRangeItem = React.forwardRef((props: IFilterViewProps) => {
    ...
    if (props.dateRangeItem && props.dateRangeItem.viewMode !== 'extended') {
        if (props.dateRangeItem.type === 'dateMenu') {
            const text = props.dateRangeItem.textValue ||
                props.dateRangeItem.editorOptions?.emptyCaption || props.dateRangeItem.emptyText;
            return <props.itemTemplate theme={props.theme}
                                       text={text}
                                       title={text}
                                       onMouseDown={(event) => {
            ...
            {isButtonVisible ? (
                ...
                >
                    {props.isAdaptive && (props.filterText || !props.isFastReseted) ? (
                        <div className={`controls-FilterView-buttonIndicator
                        controls-FilterView-buttonIndicator_${props.showFilterText === false ? 'visible' : 'hidden'}
                        `}></div>
                    ) : null}
                </div>
            ) : null}
