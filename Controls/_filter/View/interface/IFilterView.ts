...
export interface IFilterViewOptions extends IControlOptions {
    source: IFilterSourceItem[] | IFilterDescriptionItem[];
    ...
    onItemsChanged?: (filterDescription: IFilterDescriptionItem[]) => void;
}
 ...
 * @remark
 * В качестве шаблона рекомендуется использовать контрол {@link Controls/filterPanelPopup:Sticky}
 * Подробнее о настройке панели фильтров читайте {@link /doc/platform/developmentapl/interface-development/controls/list/filter-and-search/new-filter/filter-view/ здесь}.
...
/**
 * @event Controls/_filter/View/interface/IFilterView#itemsChanged Происходит при изменении структуры фильтра.
 ...
 * Отличается от {@link itemsChanged} тем, что в событие передаётся объект фильтра, а не структура.
 * @name Controls/_filter/View/interface/IFilterView#filterChanged
 * @param {UI/Events:SyntheticEvent} eventObject Дескриптор события.
 * @param {Object} filter Новый объект фильтра.
 */
