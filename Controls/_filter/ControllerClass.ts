   ...
   можно получить идентификатор закэшированных данных для этого фильтра */
    ...
    update(newOptions: IFilterControllerOptions): void | boolean {
        ...
        if (
            filterButtonChanged &&
            prefetchParams &&
            !isFilterChangedWithPrefetch
        ) {
            this._$filter = this._getPrefetch().clearPrefetchSession(
                this._$filter
            );
        }
    ...
    resetPrefetch(): void {
        ...
        this._setFilterAndNotify(
            loadSync(PREFETCH_MODULE).Prefetch.clearPrefetchSession(filter)
        );
    }
    ...
    handleDataLoad(items: RecordSet): void {
        if (this._options.historyId) {
            ...
            if (this._options.prefetchParams) {
                this._getPrefetch().applyPrefetchFromItems(
                    this._$filter,
                    items
                );
            }
    ...
    private _applyPrefetchFromHistoryIfNeed(
        ...
        if (options.prefetchParams) {
            return this._getPrefetch().applyPrefetchFromHistory(
                filter,
                filterItems
            );
        } else {
    ...
    private _preparePrefetchFilter(
        ...
        if (prefetchParams) {
            const prefetchSession =
                prefetchSessionId ||
                this.getFilter()[this._getPrefetch().PREFETCH_SESSION_FIELD];
            return this._getPrefetch().prepareFilter(
                filter,
                prefetchParams,
                prefetchSession
            );
    ...
    private _getHistoryData(
        ...
        if (
            ...
            result = prefetchParams
                ? this._getPrefetch().addPrefetchToHistory(
                      result,
                      prefetchParams
                  )
                : result;
    ...
    private _processPrefetchOnItemsChanged(
        ...
        if (history) {
            const prefetchParams = this._getPrefetch().getPrefetchFromHistory(
                history.data
            );
            const needInvalidate =
                prefetchParams &&
                this._getPrefetch().needInvalidatePrefetch(history.data);
        ...
        if (needApplyPrefetch) {
            filter = this._applyPrefetchFromHistoryIfNeed(
                this._$filter,
                history.data,
                options
            );
            ...
            filter = this._getPrefetch().clearPrefetchSession(this._$filter);
        }
