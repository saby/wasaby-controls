...
>(ObservableMixin, SerializableMixin, OptionsToPropertyMixin) {
   ...
   можно получить идентификатор закэшированных данных для этого фильтра */
    ...
    updateHistory(history: THistoryData, doNotDeleteCurrentHistory?: boolean): void {
        if (this._options.prefetchParams) {
            this._processHistoryOnItemsChanged(history.items || history, this._options, doNotDeleteCurrentHistory);
        }
    }
