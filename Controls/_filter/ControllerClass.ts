 ...
 */
import { IPrefetchHistoryParams, IPrefetchParams } from 'Controls/filter';
import { IFilterItem } from 'Controls/_filter/View/interface/IFilterItem';

import FilterDescription from 'Controls/_filter/FilterDescription';
import FilterHistory from 'Controls/_filter/FilterHistory';
...
>(ObservableMixin, SerializableMixin, OptionsToPropertyMixin) {
   ...
   можно получить идентификатор закэшированных данных для этого фильтра */
    ...
    private _getHistoryData(
        ...
        if (
            FilterDescription.isFilterDescriptionChanged(
    ...
    private static _minimizeItem(item: IFilterItem): IFilterItem {
        ...
        if (visibility !== false && textValue !== item.resetTextValue) {
            ...
            } else if (item.textValue !== null && item.textValue !== undefined) {
                const historyCaption = item.historyCaption || '';
                minimizedItem.textValue = `${historyCaption}${item.textValue}`;
            } else {
