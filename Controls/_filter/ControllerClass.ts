...
type ISerializableState =
    IDefaultSerializableState<IFilterControllerOptionsSerialized>;
   ...
   можно получить идентификатор закэшированных данных для этого фильтра */
    ...
    constructor(options: Partial<IFilterControllerOptions>) {
        ...
        if (
            ...
            updateFilterDescription(
                ...
                (newItems) => {
                    this._setFilterDescription(newItems);
                }
            );
        } else {
    ...
    applyFilterDescriptionFromHistory(
        ...
        filterItems = FilterControllerClass._prepareFilterDescription(
            this._getItemsWithHistory(filterItems, historyItems, filterFromUrl)
        );

        if (
            this._options.prefetchParams &&
            historyItems instanceof Array &&
            historyItems.length
        ) {
        ...
        const filter = getFilterByFilterDescription(
            Prefetch.applyPrefetchFromHistory(this.getFilter(), historyItems),
            filterItems
        );
    ...
    loadFilterItemsFromHistory(): Promise<THistoryData> {
        ...
        if (this._options.useStore && !this._options.filterButtonSource) {
            const state = getStore().getState();
            const loadedSources =
                state && state.loadedSources && state.loadedSources[0];
            if (loadedSources) {
    ...
    loadFilterConfiguration(propStorageId: string): Promise<void> {
        return loadSavedConfig(propStorageId, ['filterUserConfiguration']).then(
            (configurationPromiseResult) => {
                const userSettings =
                    configurationPromiseResult?.filterUserConfiguration;
                if (Array.isArray(userSettings) && userSettings.length) {
                    const filterButtonItems =
                        this._applyFilterConfigurationToSource(
    ...
    update(newOptions: IFilterControllerOptions): void | boolean {
        ...
        if (filterButtonChanged && prefetchParams && !this._isFilterChanged) {
    ...
    resetPrefetch(): void {
        ...
        this._isFilterChanged = true;
        this._setFilterAndNotify(Prefetch.clearPrefetchSession(filter));
    }
    ...
    setFilter(filter: object): void {
        this._setFilterAndNotify(
            this._preparePrefetchFilter(filter, this._options)
        );
    }
    ...
    handleDataError(): void {
        if (this._options.historyId && this._isFilterChanged) {
            ...
            if (currentAppliedHistoryItems) {
                Object.assign(
                    this._$filter,
                    Prefetch.applyPrefetchFromHistory(
                        this._$filter,
    ...
    reloadFilterItem(filterName: string): void | Promise<RecordSet | Error> {
        ...
        return editorOptions.sourceController.reload().catch((error) => {
            return error;
        });
    }
    ...
    private _preparePrefetchFilter(
        ...
        const prefetchSession =
            prefetchSessionId ||
            this.getFilter()[Prefetch.PREFETCH_SESSION_FIELD];
        return Prefetch.prepareFilter(filter, prefetchParams, prefetchSession);
    }
    ...
    private _updateFilterItems(newItems: IFilterItem[]): void {
        if (this._$filterButtonItems) {
            this._$filterButtonItems = FilterControllerClass._cloneItems(
                this._$filterButtonItems
            );
    ...
    private _getFilterFromUrl(
        filterButtonItems: IFilterItem[]
    ): void | IFilterItem[] {
        const filterItems =
            FilterControllerClass._getFilterButtonItems(filterButtonItems);
    ...
    private _loadHistoryItems(
        ...
        if (!historyId) {
            ...
            const filterButtonItems =
                FilterControllerClass._getFilterButtonItems(filterSource);
        ...
        return new Promise((resolve) => {
            this._crudWrapper
                ...
                .then((res) => {
                    ...
                    if (recent.getCount()) {
                        const lastFilter = recent.at(
                            ACTIVE_HISTORY_FILTER_INDEX
                        );
                        const lastFilterData = source.getDataObject(lastFilter);
                        historyResult = Object.keys(lastFilterData).length
                            ? lastFilterData
                            : [];
                    } else {
                        historyResult = [];
                    }
                    historyResult = this._mergeHistoryParams(
                        historyResult,
                        source
                    );
    ...
    private _mergeHistoryParams(
        ...
        for (const historyId in paramsHistoryIds) {
            if (paramsHistoryIds.hasOwnProperty(historyId)) {
                historyParams.push(
                    source.getDataObject(paramsHistoryIds[historyId])
                );
            }
        }
    ...
    private _deleteCurrentFilterFromHistory(): void {
        ...
        if (history) {
            FilterControllerClass._deleteFromHistory(
                history.item,
                this._options.historyId
            );
        }
    ...
    private _getHistoryByItems(
        ...
        result = this._findItemInHistory(
            historyId,
            items,
            prefetchParams,
            historySaveMode
        ...
        if (historyItems && historyItems.getCount()) {
            historyItems.each((item, index) => {
                if (!result) {
                    ...
                    if (historyData) {
                        const itemsToSave =
                            FilterControllerClass._getFilterDescriptionForHistory(
                                items
                            );
                        minimizedItemFromOption =
                            this._minimizeFilterItems(itemsToSave);
                        minimizedItemFromHistory = this._minimizeFilterItems(
                            historyData.items || historyData
                        );
                        if (
                            minimizedItemFromOption.length &&
                            minimizedItemFromHistory.length
                        ) {
                            const keyFromOption =
                                minimizedItemFromOption[0].hasOwnProperty(
                                    'name'
                                )
                                    ? 'name'
                                    : 'id';
                            const keyFromHistory =
                                minimizedItemFromHistory[0].hasOwnProperty(
                                    'name'
                                )
                                    ? 'name'
                                    : 'id';
                            if (keyFromOption !== keyFromHistory) {
                                minimizedItemFromHistory =
                                    minimizedItemFromHistory.map(
                                        (historyItem) => {
                                            historyItem[keyFromOption] =
                                                historyItem[keyFromHistory];
            ...
            if (this._updateMeta) {
                ...
                if (historyData && prefetchParams) {
                    const historyItems = JSON.parse(
                        historyData.get('ObjectData')
                    );
                    const currentSessionId =
                        historyItems.prefetchParams.PrefetchSessionId;
                ...
                historyData = this._getHistoryData(
                    filterButtonItems,
                    prefetchParams
                );
    ...
    private _getSaveToUrlItems(
        filterButtonItems: IFilterItem[]
    ): IFilterItem[] {
        return filterButtonItems.filter((item) => {
            return (
                item.saveToUrl ||
                (this._options.saveToUrl && !item.hasOwnProperty('saveToUrl'))
            );
        });
    }
    ...
    private _prepareHistoryItems(
        filterButtonItems: IFilterItem[]
    ): IFilterItem[] {
        const historyItems =
            FilterControllerClass._cloneItems(filterButtonItems);
    ...
    private _processPrefetchOnItemsChanged(
        ...
        const historyReady = this._getHistorySource(
            ...
            this._$filterButtonItems,
            !!options.prefetchParams ||
                this._isFavoriteMode(options.historySaveMode)
        ).historyReady();
        ...
        if (history) {
            const prefetchParams = Prefetch.getPrefetchFromHistory(
                history.data
            );
        ...
        if (needApplyPrefetch) {
            filter = Prefetch.applyPrefetchFromHistory(
                this._$filter,
                history.data
            );
    ...
    private _getItemsWithHistory(
        ...
        if (history) {
            historyItems =
                history.items || (Array.isArray(history) ? history : []);
        }
    ...
    private _applyItemsToFilter(
        ...
        this._setFilterAndNotify(
            getFilterByFilterDescription(filter, filterButtonItems)
        );
    }
    ...
    _setSerializableState(state: ISerializableState): Function {
        const filterDescriptions =
            state.$options.filterButtonItems ||
            state.$options.filterButtonSource;
        if (filterDescriptions) {
            state.$options.filterButtonSource =
                state.$options.filterButtonItems =
                    FilterControllerClass._prepareFilterDescription(
                        filterDescriptions
                    );
        }
    ...
    private static _isNeedPrepareFilterDescription(
        filterDescription: IFilterItem[]
    ): boolean {
        return (
            Array.isArray(filterDescription) &&
            filterDescription.some((filterItem) => {
                return FilterControllerClass._isNeedPrepareFilterItem(
                    filterItem
                );
            })
        );
    ...
    private static _prepareFilterDescription(
        filterDescription: IFilterItem[]
    ): IFilterItem[] {
        if (
            FilterControllerClass._isNeedPrepareFilterDescription(
                filterDescription
            )
        ) {
            ...
            filterDescription.forEach((filterItem) => {
                if (
                    FilterControllerClass._isNeedPrepareFilterItem(filterItem)
                ) {
                    const newFilterItem = clone(filterItem);
                    newFilterItem.editorOptions.sourceController =
                        new SourceController({
                            ...filterItem.editorOptions,
                            items: filterItem.editorOptions.items?.clone(true),
                        });
    ...
    private static _isNeedPrepareFilterItem({
        ...
        const loadByType =
            FILTER_EDITORS_WHICH_REQUIRED_DATA_LOAD.includes(
                editorTemplateName
            ) || type === 'list';
        return !!(
            editorOptions &&
    ...
    private static _getFilterButtonItems(
        filterSource: IFilterItem[]
    ): IFilterItem[] {
    ...
    private static _minimizeItem(item: IFilterItem): IFilterItem {
        ...
        const isNeedSaveValue =
            getPropValue(item, 'resetValue') !== undefined
                ? !isEqual(value, getPropValue(item, 'resetValue')) &&
                  isNeedSaveHistory
                : true;

        if (
            visibility !== undefined &&
            !getPropValue(item, 'editorTemplateName')
        ) {
        ...
        if (
            visibility !== false &&
            textValue !== getPropValue(item, 'resetTextValue')
        ) {
        ...
        if (getPropValue(item, 'textValueVisible') !== undefined) {
            minimizedItem.textValueVisible = getPropValue(
                item,
                'textValueVisible'
            );
        }
        ...
        .then((items) => {
            ...
            try {
                ...
                if (config.prefetchParams && config.historyId) {
                    ...
                    if (history) {
                        calculatedFilter = Prefetch.applyPrefetchFromHistory(
                            calculatedFilter,
                            history.data
                        );
                    }
                    calculatedFilter = this._preparePrefetchFilter(
                        calculatedFilter,
                        config
                    );
                }
