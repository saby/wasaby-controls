...
function isLookupType({ type, editorTemplateName }: IFilterItem): boolean {
    return (
        ...
        isDropdownEditor(editorTemplateName)
    );
}

function isDropdownEditor(editorTemplateName: string | void): boolean {
    return editorTemplateName === 'Controls/filterPanel:DropdownEditor';
}

function isListType({ type, editorTemplateName }: IFilterItem): boolean {
    return type === 'list' || editorTemplateName === 'Controls/filterPanel:ListEditor';
}
...
function needPrepareFilterItem(filterItem: IFilterItem): boolean {
    return (
...
function loadFilterDescriptionData(
    ...
    const filterStructurePromise = loadFilterData(filterSource, editorsViewMode, onlyChanged);
...
function loadFilterData(
    ...
    const getFilter = (
        ...
        if (
            propertyChanged &&
            (editorOptions.navigation || isLookupType(filterItem)) &&
        ...
        return description.map((property) => {
            if (needPrepareFilterItem(property)) {
                ...
                const isListTypeFilter =
                    isListType(property) ||
                    (isDropdownEditor(property.editorTemplateName) &&
                        !onlyChanged &&
                        !property.editorOptions?.items);
                let filter;
                ...
                if (propertyChanged || isListTypeFilter) {
                    if (isListTypeFilter) {
                        property.type = 'list';
