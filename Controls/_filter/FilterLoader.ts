...
function isLookupType({ type, editorTemplateName }: IFilterItem): boolean {
    return type === 'lookup' || editorTemplateName === 'Controls/filterPanel:LookupEditor';
}
    ...
    return Promise.all(
        ...
        return description.map((property) => {
            if (needPrepareFilterItem(property)) {
                ...
                const isListTypeFilter = isListType(property);
                let filter;
                ...
                if (propertyChanged || isListTypeFilter) {
                    ...
                    if (isListTypeFilter || isLookupType(property)) {
                        property.type = 'list';
                    }
