...
class FilterView extends Control<IFilterViewOptions, IFilterReceivedState> implements IFilterView {
    ...
    private _getPopupConfig(configs: IFilterItemConfigs, items: IFilterItem[]) {
        const popupItems = [];
        factory(items).each((item) => {
            if (this._isFrequentItem(item) && !item.primary && this._isVisibilityItem(item)) {
    ...
    private _openDetailPanelFromSimple(result: IResultPopup): void {
        this._getFilterPopupOpener().close();
        this._popupSource = this._cloneItems(this._source);

        factory(result.selectedKeys).each((sKey, index: string) => {
            if (sKey) {
                const item = this._getItemByName(this._popupSource, index);
                item.value = this._getValue(sKey, index) ?? item.value;
            }
        });

        const popupOptions = this._getPopupOptionsForDetailPanel();
        popupOptions.templateOptions = {
            ...(popupOptions.templateOptions || {}),
            applyButtonVisible: true,
        };
        this.#openDetailPanel(null, this._popupSource, popupOptions);
    }
