...
import {
    ...
    TKey,
    TNavigation,
} from './View/interface/IFilterItem';
 ...
 *
 * Полезные ссылки:
 * * {@link /doc/platform/developmentapl/interface-development/controls/list/filter-and-search/new-filter/filter-view/ руководство разработчика по работе с контролом}
...
class FilterView
    extends Control<IFilterViewOptions, IFilterReceivedState>
    implements IFilterView
{
    ...
    openDetailPanel(name?: string): void {
        if (this._detailPanelTemplateName) {
            const isNewPanel =
                this._detailPanelTemplateName ===
                'Controls/filterPanelPopup:Sticky';
            const panelItems = converterFilterItems.convertToDetailPanelItems(
                this._source
            );
            ...
            popupOptions = Merge(
                popupOptions,
                this._options.detailPanelPopupOptions || {}
            );
            ...
            popupOptions.templateOptions =
                this._options.detailPanelTemplateOptions || {};
    ...
    protected _beforeMount(
        options: IFilterViewOptions,
        context: object,
        receivedState: IFilterReceivedState
    ): Promise<IFilterReceivedState> {
    ...
    protected _getItemsForReload(
        ...
        const getOptionsChecker = (oldItem, newItem) => {
            return (changed, optName) => {
                return (
                    changed ||
                    !isEqual(
                        oldItem.editorOptions?.[optName],
                        newItem.editorOptions?.[optName]
                    )
                );
            };
        };
        ...
        factory(newItems).each((newItem) => {
            ...
            const needHistoryReload =
                configs &&
                configs[newItem.name] &&
                !configs[newItem.name].sourceController;
            const valueChanged =
                oldItem && !isEqual(newItem.value, oldItem.value);
            if (
                newItemIsFrequent &&
                (!oldItem ||
                    !oldItemIsFrequent ||
                    optionsToCheck.reduce(
                        getOptionsChecker(oldItem, newItem),
                        false
                    ) ||
    ...
    protected _beforeUpdate(newOptions: IFilterViewOptions): void {
        ...
        if (newOptions.source && newOptions.source !== this._options.source) {
            ...
            const updateCallback = () => {
                this._updateText(this._source, this._configs);
                if (
                    this._getFilterPopupOpener().isOpened() &&
            ...
            if (itemsForReload.length) {
                ...
                resultDef = this._reload(
                    null,
                    !!newOptions.panelTemplateName,
                    itemsForReload
                );
            } else if (this._loadPromise) {
                resultDef = this._loadPromise.promise.then(() => {
                    return this._loadSelectedItems(
                        this._source,
                        this._configs
                    ).then(() => {
                        updateCallback();
    ...
    protected _openDetailPanel(
        event?: SyntheticEvent<Event>,
        name?: string
    ): void {
        this.openDetailPanel(name);
    }
    ...
    protected _openPanel(
        ...
        const isLoading =
            this._loadPromise && !this._loadPromise.promise.isReady();
        if (
            ...
            const promises = [];
            promises.push(
                this._loadUnloadedFrequentItems(this._configs, this._source)
            );
            promises.push(this._loadFrequentItemTemplates(this._source));
            return Promise.all(promises).then(
                () => {
                    this._loadItemsTemplates(this._configs, this._source).then(
                        () => {
                            const items = new RecordSet({
                                rawData: this._getPopupConfig(
                                    this._configs,
                                    this._source
                                ),
                            });
                         ...
                         */
                            ...
                            popupOptions.templateOptions =
                                this._options.panelTemplateOptions || {};
    ...
    protected _rangeValueChangedHandler(
        event: Event,
        start: Date,
        end: Date
    ): void {
    ...
    protected _getDateItemFontWeight(): string {
        if (
            this._needShowDetailPanelTarget(this._source) &&
            (!this._options.emptyText || this._options.detailPanelTemplateName)
        ) {
            return 'default';
        }
    }
    ...
    protected _allChangedFilterTextValueInvisible(): boolean {
        return this._source.every((item) => {
            return this._isItemChanged(item)
    ...
    protected _needShowDetailPanelTarget(source: IFilterItem[]): boolean {
        return (
            (!!this._options.detailPanelTemplateName ||
                (this._isFastReseted() && this._needShowFastFilter(source))) &&
    ...
    private _resultHandler(result: IResultPopup): void {
        if (!result.action) {
            const filterSource = converterFilterItems.convertToFilterSource(
                result.items
            );
            ...
            this._resolveItems(mergeSource(currentSource, filterSource));
        ...
        if (
            result.action !== 'moreButtonClick' &&
            result.action !== 'collapsedFiltersChanged'
        ) {
            if (result.history) {
    ...
    private _openSelector(
        ...
        const selectedItems = [];
        const items =
            this._configs[filterItem.name]?.popupItems ||
            this._configs[filterItem.name]?.items;
        ...
        const templateOptions =
            object.clone(selectorTemplate.templateOptions) || {};
        ...
        this._configs[filterItem.name].initSelectorItems =
            templateOptions.selectedItems;
    ...
    private _showSelector(filterName?: string): Promise<void> {
        ...
        if (item) {
            ...
            if (item?.editorOptions.selectorTemplate) {
                const items =
                    this._configs[name]?.popupItems ||
                    this._configs[name]?.items;
    ...
    private _isDateItem(item: IFilterItem): boolean {
        return item.type === 'dateRange' || item.type === 'date';
    }

    private _resolveItems(items: IFilterItem[]): void {
        this._source = object.clonePlain(items);
    ...
    private _sourcesIsLoaded(configs: IFilterItemConfigs): boolean {
        ...
        factory(configs).each((config) => {
            if (
                ...
                config.sourceController.isLoading()
            ) {
                result = false;
            }
        });
    ...
    private _getDateRangeItem(items: IFilterItem[]): IFilterItem {
        ...
        factory(items).each((item) => {
            const hasValue =
                ...
                (item.resetValue instanceof Array
                    ? !!item.resetValue.length
                    : !!item.resetValue);
            if (
                this._isDateItem(item) &&
                (item.editorOptions?.emptyCaption || hasValue)
            ) {
                dateRangeItem = item;
            }
        });
    ...
    private _loadFrequentItemTemplates(items: IFilterItem[]): Promise<unknown> {
        const loadPromises = [];
    ...
    private _loadItemsTemplates(
        ...
        factory(items).each((filterItem): void => {
            const itemTemplateProperty =
                filterItem.editorOptions?.itemTemplateProperty;
    ...
    private _getPopupConfig(configs: IFilterItemConfigs, items: IFilterItem[]) {
        ...
        factory(items).each((item) => {
            if (this._isFrequentItem(item)) {
                const popupItem = object.clonePlain(configs[item.name]);
                Merge(popupItem, item.editorOptions);
                popupItem.id = item.name;
                popupItem.selectedKeys =
                    item.value instanceof Object ? item.value : [item.value];
                popupItem.resetValue =
                    item.resetValue instanceof Object
                        ? item.resetValue
                        : [item.resetValue];
                popupItem.items =
                    configs[item.name].popupItems || popupItem.items;
                ...
                if (item.editorOptions.source) {
                    if (
                        ...
                        (!configs[item.name].loadDeferred ||
                            configs[item.name].loadDeferred.isReady())
                    ) {
                    ...
                    popupItem.hasMoreButton =
                        configs[item.name].sourceController.hasMoreData('down');
                    popupItem.sourceController =
                        configs[item.name].sourceController;
                    popupItem.selectorOpener = this._getStackOpener();
                    popupItem.dialogOpener = this._getDialogOpener();
                    popupItem.selectorDialogResult =
                        this._onSelectorTemplateResult.bind(this);
    ...
    private _getHasMoreText(selection: TKey[]): string {
        return selection.length > 1
            ? ', ' + rk('еще') + ' ' + (selection.length - 1)
            : '';
    }

    private _getParentText(
        selectedItem: Model,
        config: IFilterItemConfig
    ): string {
        let text = '';
    ...
    private _getTextFromItems(
        ...
        factory(selectedKeys).each((key) => {
            ...
            if (selectedItem) {
                const parentText =
                    this._getParentText(selectedItem, config) || '';
                text.push(
                    parentText +
    ...
    private _getFastText(
        ...
        if (selectedKeys[0] === config.emptyKey && config.emptyText) {
            ...
            textArr = this._getTextFromItems(
                config.items,
                selectedKeys,
                config
            );
    ...
    private _checkEditorTextVisible(item: IFilterItem): boolean {
        ...
        const menuEditor = 'Controls/filterPanel:DateMenuEditor';

        const isMenuEditorValueVisible =
            item.editorTemplateName === menuEditor &&
    ...
    private _getFilterButtonText(items: IFilterItem[]): string {
        ...
        factory(items).each((item) => {
            ...
            if (
                ...
                (item.userVisibility === false ||
                    this._checkEditorTextVisible(item))
            ) {
                ...
                if (this._isItemChanged(item)) {
                    if (configs[item.name]) {
                        ...
                        if (!displayText.text && detailPanelHandler) {
                            ...
                            displayText.text =
                                item.textValue && item.textValue.split(', ')[0];
                            displayText.hasMoreText =
                                this._getHasMoreText(flatSelectedKeys);
                        }
                        if (
                        ...
                        */
                        const selectedKeys = item.editorOptions?.multiSelect
                            ? item.value
                            : [item.value];
                        this._displayText[item.name] = this._getFastText(
                            {},
                            selectedKeys,
                            item
                        );
    ...
    private _getKeysUnloadedItems(
        ...
        if (config.items) {
            factory(flattenKeys).each((key) => {
                const index = config.items.getIndexByValue(
                    config.keyProperty,
                    key
                );
                if (
    ...
    private _getPreparedItems(
        ...
        let folderItems: RecordSet = getItemsByParentKey(
            config.popupItems
    ...
    private _setItems(
        ...
        if (config.nodeProperty && folders.length) {
            ...
            factory(folders).each((folderId) => {
                if (!resultItems) {
                    resultItems = this._getPreparedItems(
                        config,
                        item,
                        newItems,
                        folderId
                    ...
                    resultItems.append(
                        this._getPreparedItems(config, item, newItems, folderId)
                    );
                }
            });
            ...
            if (isHistorySource(item.editorOptions.source)) {
                config.popupItems = (
                    item.editorOptions.source as HistorySource
                ).prepareItems(config.popupItems);
            }
        } else {
    ...
    private _loadSelectedItems(
        ...
        return Promise.all(
            this._loadSelectedItemsForFrequentFilters(items, configs)
        );
    }
    ...
    private _loadSelectedItemsForFrequentFilters(
        ...
        factory(items).each((item) => {
            if (this._isFrequentItem(item) && configs[item.name]) {
                ...
                const isDateMenuEditor =
                    item.editorTemplateName ===
                    'Controls/filterPanel:DateMenuEditor';
    ...
    private _loadItemsFromSource(
        ...
        queryFilter = withHistory
            ? historyUtils.getSourceFilter(queryFilter, source)
            : queryFilter;
        const sourceController = this._getSourceController(
            instance,
            source,
    ...
    private _setConfigByItem(item: IFilterItem): void {
        ...
        this._configs[item.name].emptyKey = item.hasOwnProperty('emptyKey')
            ? item.emptyKey
            : null;
            ...
            if (this._isFrequentItem(item)) {
                if (!onlyChangedItems || this._isItemChanged(item)) {
                    if (hasSimplePanel) {
                        ...
                        this._setConfigByItem(item);
                        const frequentFiltersPromises =
                            this._loadSelectedItemsForFrequentFilters(
                                [item],
                                this._configs
                            );
                        if (frequentFiltersPromises.length) {
                            loadPromises.push(
                                Promise.all(frequentFiltersPromises)
                            );
                        }
                    }
                }
            ...
            return this._loadPromise.promise
                .then(() => {
                    return this._loadSelectedItems(
                        ...
                        this._loadPromise = null;
                        return {
                            configs: deleteHistorySourceFromConfig(
                                this._configs,
                                'source'
                            ),
                        };
    ...
    private _isHierarchyFilter(config: IFilterItemConfig): boolean {
        let result = false;
        if (config.nodeProperty) {
            result = !!this._getFolderIds(
                config.popupItems || config.items,
                config
            ).length;
        }
        return result;
    ...
    private _setValue(selectedKeys: TKey | TKey[], name: string): void {
        ...
        if (this._isHierarchyFilter(config)) {
            selectedKeys = this._prepareHierarchySelection(
                selectedKeys,
                config,
                resetValue
            );
        ...
        if (
            ...
            (!selectedKeys.length ||
                ...
                (item.emptyText &&
                    !item.hasOwnProperty('emptyKey') &&
                    selectedKeys.includes(null)))
        ) {
            value = object.getPropertyValue(item, 'resetValue');
        } else if (
            this._configs[name].multiSelect ||
            this._configs[item.name].nodeProperty
        ) {
            value = selectedKeys;
        } else {
    ...
    private _getSelectedKeys(
            ...
            factory(folderIds).each((folderId, index) => {
                ...
                factory(items).each((item) => {
                    if (
                        ...
                        selectedKeys[folderId].push(
                            item.get(config.keyProperty)
                        );
                    }
                });
            });
        ...
        if (config.nodeProperty) {
            ...
            factory(items).each((item) => {
                selectedKeys.push(
                    object.getPropertyValue(item, config.keyProperty)
                );
            });
        }
        return selectedKeys;
    ...
    private _prepareHierarchySelection(
        ...
        folderIds.forEach((parentKey, index) => {
            ...
            if (
                nodeSelectedKeys.length &&
                !nodeSelectedKeys.includes(curConfig.emptyKey)
            ) {
    ...
    private _selectorResult(result: IResultPopup): void {
        ...
        if (
            curItem.editorTemplateName &&
            curItem.editorOptions.sourceController
        ) {
    ...
    private _loadDependencies(): Promise<unknown> {
        try {
            ...
            const detailPanelTopTemplateName =
                this._options.detailPanelTopTemplateName;
            ...
            if (!this._loadOperationsPanelPromise) {
                this._loadOperationsPanelPromise = Promise.all([
                    ...
                    typeof detailPanelTemplateName === 'string'
                        ...
                        : null,
                    detailPanelTopTemplateName &&
                    typeof detailPanelTopTemplateName === 'string'
            ...
            return this._loadOperationsPanelPromise.then(() => {
                this._setEditorOptionsToFrequentItem();
            });
        } catch (e) {
    ...
    private _setEditorOptionsToFrequentItem(): object {
        this._source.forEach((item, index) => {
            if (this._isFrequentItem(item) && item.editorOptionsName) {
                item.editorOptions = this._getEditorOptions(item);
                this._setConfigByItem(item);
            }
        });
    }

    private _getEditorOptions(item: IFilterItem): IEditorOptions {
        const editorOptions = item.editorOptions || {};
        let resultEditorOptions;
        const loadedEditorOptions = loadSync<Function | object>(
            item.editorOptionsName
        );
        ...
        if (loadedEditorOptions instanceof Function) {
            ...
            resultEditorOptions = { ...editorOptions, ...loadedEditorOptions };
        }
