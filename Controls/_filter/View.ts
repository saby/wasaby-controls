...
class FilterView extends Control<IFilterViewOptions, IFilterReceivedState> implements IFilterView {
    ...
    private _getPopupConfig(configs: IFilterItemConfigs, items: IFilterItem[]) {
        const popupItems = [];
        factory(items).each((item) => {
            if (this._isFrequentItem(item) && this._isVisibilityItem(item)) {
    ...
    private _loadDependencies(): Promise<unknown> {
        ...
        try {
            ...
            // Старая панель фильтров всё равно грузит историю при открытии сама.
            // Чтобы не было лишних запросов и коллизий параметров делаю проверку
            const loadHistoryPromise = this._isNewPanel()
                ? loadHistory(
