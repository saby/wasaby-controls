...
import FilterCalculator from 'Controls/_filter/FilterCalculator';
import { MAX_COUNT_FREQUENT_FILTERS } from 'Controls/_filter/constants';
...
import FilterHistory, { THistoryData } from 'Controls/_filter/FilterHistory';
...
const { resetFilterItem, mergeFilterDescriptions, resetFilterDescription } = FilterDescription;
 ...
 * Полезные ссылки:
 * <ul>
 *     <li>{@link /doc/platform/developmentapl/interface-development/controls/list/filter-and-search/filter-and-search/#deprecated-c-controlsbrowserbrowser руководство разработчика по организации поиска и фильтрации в реестре}</li>
 *     <li>{@link https://git.sbis.ru/saby/wasaby-controls/-/blob/rc-24.6100/Controls-default-theme/variables/_filter.less переменные тем оформления filter}</li>
 *     <li>{@link https://git.sbis.ru/saby/wasaby-controls/-/blob/rc-24.6100/Controls-default-theme/variables/_filterPopup.less переменные тем оформления filterPopup}</li>
 * </ul>
 *
 ...
 * @see Controls/filterOld:ViewContainer
 */
...
class FilterView extends Control<IFilterViewOptions, IFilterReceivedState> implements IFilterView {
    ...
    private _loadEditorsDataPromise: Promise<unknown>;
    ...
    _openDetailPanel(name?: string, popupOptions?: IStickyPopupOptions): void {
        if (this._detailPanelTemplateName) {
            this._isDetailPanelOpened = true;
            this._open(
                this._popupSource || this._source,
                popupOptions || this._getPopupOptionsForDetailPanel()
            );
        } else {
    ...
    reset(): void {
        resetFilterDescription(this._source);
    ...
    openFrequentPanel(name?: string, target?: HTMLElement): void {
        this._openPanel(null, name, target);
    }

    private _isNewPanel(): boolean {
        return this._detailPanelTemplateName === 'Controls/filterPanelPopup:Sticky';
    }

    protected _setAlignment(alignment: string): void {
    ...
    protected _getViewProps(options: IFilterViewOptions): IFilterViewProps {
        return {
            ...
            setAlignment: this._setAlignment.bind(this),
            source: this._source,
            configs: this._configs,
            dateRangeItem: this._dateRangeItem,
            filterText: this._filterText,
            displayText: this._displayText,
            needShowResetCross: this._needShowResetCross(this._source),
            ...
            isAllFastReseted: this._isAllFastReseted(),
            needShowFastFilter: this._needShowFastFilter(this._source),
            primaryFilter: this._getPrimaryFilter(this._source),
    ...
    protected _getItemsForReload(
        ...
        const optionsToCheck = ['source', 'filter', 'navigation', 'items'];
    ...
    protected _resetDisplayText(
        ...
        factory(newItems).each((newItem) => {
            const { name, viewMode, type } = newItem;
            const oldItem = this._getItemByName(oldItems, name);
            if (
                (!oldItem ||
                    (this._isFrequentItem(oldItem) && oldItem.viewMode !== viewMode) ||
                    (this._isDateItem(newItem) && oldItem.type !== type)) &&
                displayText[name]
            ) {
                displayText[name] = {};
            }
        });
        const newFilterText = this._getFilterButtonText(newItems);
        if (this._filterText !== newFilterText) {
    ...
    protected _beforeUpdate(newOptions: IFilterViewOptions): void {
        ...
        if (newOptions.source && newOptions.source !== this._options.source) {
            ...
            /* Если идет или будет асинхронная загрузка, то будет висеть старый текст, пока не загрузим items
               Нужно научиться понимать, когда можем item сразу перерисовать и делать это точечно
    ...
    openDetailPanel(name?: string): Promise<unknown> {
        return this.#openDetailPanel(name);
    }

    #openDetailPanel(
        name?: string,
        filterDescription: IFilterItem[] = this._source,
        popupOptions?: IStickyPopupOptions
    ): Promise<unknown> {
        if (this._loadEditorsDataPromise) {
            return;
        }
        ...
        return this._loadDependencies()
            .then(() => {
                ...
                return this._setEditorOptionsPromise.then(() => {
                    this._popupSource = this._cloneItems(filterDescription);
                    return (this._loadEditorsDataPromise = FilterLoader.loadFilterDescriptionData(
                        this._popupSource,
                        'cloud',
                        void 0,
                        true,
                        ...
                        .then((result) => {
                            this._popupSource = result;
                            this._openDetailPanel(name, popupOptions);
                        })
                        .finally(() => {
                            this._loadEditorsDataPromise = null;
                        }));
                });
            })
            .catch((error) => {
    ...
    private _cloneItems(filterSource: IFilterItem[]): IFilterItem[] {
        return filterSource.map((item) => {
            ...
            if (sourceController) {
                ...
                newItem.editorOptions.sourceController = new SourceController({
    ...
    protected _openPanel(
        event?: SyntheticEvent<Event>,
        name?: string,
        currentTarget?: HTMLElement
    ): Promise<void> | void {
        const isLoading = this._loadPromise && !this._loadPromise.promise.isReady();
        const item = name ? this._getItemByName(this._source, name) : null;
        const target = event?.currentTarget || currentTarget;
        if (
            (this._options.panelTemplateName || item?.type === 'dateMenu') &&
            this._sourcesIsLoaded(this._configs) &&
            !isLoading
        ) {
            return this._loadEditorOptionsName(this._source).then(() => {
                const promises = [];
                ...
                return Promise.all(promises).then(
                    () => {
                        this._loadItemsTemplates(this._configs, this._source).then(() => {
                            let popupConfig;
                            let filterName = name;
                            if (!name && this._source.length === 1) {
                                filterName = this._source[0].name;
                            }
                            const filterItem = this._getItemByName(this._source, filterName);
                            if (
                                filterItem &&
                                (filterItem?.type === 'dateMenu' || this._isPrimaryItem(filterItem))
                            ) {
                                popupConfig = [
                                    this._getPopupItemConfig(filterItem, this._configs[filterName]),
                                ];
                            } else {
                            ...
                            this._open(
                                items,
                                this._getSimplePanelPopupOptions(popupConfig, target, name)
                            );
                        });
                    },
                    (error) => {
    ...
    protected _isAllFastReseted(): boolean {
        const primaryFilter = this._getPrimaryFilter(this._source);
        return this._isFastReseted() && (!primaryFilter || !this._isItemChanged(primaryFilter));
    }
    ...
    protected _isFiltersReseted(): boolean {
        ...
        return isReseted || (!this._filterText && this._isAllFastReseted());
    }
    ...
    protected _getPrimaryFilter(source: IFilterItem[]): IFilterItem {
        let primaryFilter;

        factory(source).each((item) => {
            if (!primaryFilter && this._isPrimaryItem(item) && this._isVisibilityItem(item)) {
                primaryFilter = item;
            }
        });
    ...
    private _isVisibilityItem(item: IFilterItem): boolean {
        // Проверка на visibility только для новой структуры фильтров
        // В старой структуре редактор, который отображается в "Можно отобрать", имеет visibility: false
        return item.visibility !== false || !item.editorTemplateName;
    }

    private _needShowResetCross(source: IFilterItem[]): boolean {
        return !!source.some((item) => {
            const textVisible = this._isFilterItemTextVisible(item);
            const isValueChanged = this._isItemChanged(item);
            return textVisible && item.resetValue !== undefined && isValueChanged;
        });
    }
    ...
    protected _needShowDetailPanelTarget(
        ...
        const fastChangedFilterCount = source.filter(
            (item) => this._isFrequentItem(item) && !isEqual(item.value, item.resetValue)
        ).length;
        ...
        return (
            (isAdaptiveVisible ||
                ...
                fastChangedFilterCount > MAX_COUNT_FREQUENT_FILTERS) &&
            !options.readOnly
        );
    }

    protected _resetFrequentItem(frequentItem: IFilterItem): void {
        const opener = this._getFilterPopupOpener();
        if (opener.isOpened()) {
            opener.close();
        }
        this._source = this._source.map((item) =>
            item.name === frequentItem.name ? resetFilterItem({ ...frequentItem }) : item
        );
        this._notifyChanges(this._source);
        this._updateText(this._source, this._configs);
    }
    ...
    protected _resetFilterText(): void {
        ...
        factory(this._source).each((item) => {
            ...
            if (this._isFilterItemTextVisible(item)) {
    ...
    private _isFilterItemTextVisible(item: IFilterItem): boolean {
        return (
            !this._isFrequentItem(item) &&
    ...
    private _getSimplePanelPopupOptions(
        popupConfig: unknown,
        currentTarget?: HTMLElement,
        name?: string
    ): IStickyPopupOptions {
        const clickOnFrequentItem = !!name;
        const useButtonTarget =
            !!this._options.detailPanelTemplateName &&
            (!this._filterText || this._options.alignment === 'left');
        const withHeader = popupConfig.length > 1 || !!this._options.detailPanelTemplateName;
        let popupOptions: IStickyPopupOptions = {
            template: this._options.panelTemplateName || 'Controls/filterPopup:SimplePanel',
            fittingMode: {
                horizontal: 'overflow',
                vertical: 'adaptive',
            },
        ...
        if (useButtonTarget) {
            popupOptions = Merge(this._getPopupAlignmentOptions(), popupOptions);
        }

        if (useButtonTarget) {
            popupOptions.target = this._container;
        } else {
            const target = clickOnFrequentItem && currentTarget;
            /*
        ...
        if (useButtonTarget || (!clickOnFrequentItem && withHeader)) {
            popupOptions.className =
                'controls-FilterView-SimplePanel__buttonTarget-popup ' +
                `controls-FilterView-SimplePanel__buttonTarget-popup-${
                    this._alignment === 'right' ? 'left' : 'right'
                }-withHeader`;
        } else {
            popupOptions.className = 'controls-FilterView-SimplePanel-popup ';
            const isButtonTarget = !clickOnFrequentItem ? '__buttonTarget' : '';
            popupOptions.className += `controls-FilterView-SimplePanel${isButtonTarget}-popup_left controls-FilterView-SimplePanel${isButtonTarget}-popup_top-${
                withHeader ? 'withHeader' : 'withoutHeader'
            }`;
        }
    ...
    private _getPopupAlignmentOptions(): IStickyPopupOptions {
    ...
    private _getPopupOptionsForDetailPanel(): IStickyPopupOptions {
        let popupOptions: IStickyPopupOptions = {
            fittingMode: {
                horizontal: 'overflow',
                vertical: 'overflow',
            ...
            ...this._getPopupAlignmentOptions(),
        };
        ...
        popupOptions.className =
            ...
            (this._alignment === 'right' ? 'left' : 'right') +
            ` controls-FilterPanel_${
                this._options.detailPanelTemplateOptions?.orientation || 'vertical'
            }_offsetTop`;
    ...
    private _open(items: RecordSet, panelPopupOptions: IPopupOptions): void {
        ...
        const popupOptions = {
            ...
            templateOptions: {
                ...
                detailPanelTemplateName: this._detailPanelTemplateName,
            },
    ...
    private _resultHandler(result: IResultPopup): void {
        if (!result.action) {
            ...
            // При применении фильтра должны меняться только value, viewMode, textValue
            // Остальное должно решаться с помощью callback'ов (filterChangedCallback, filterVisibilityCallback)
            // Но для старой панели фильтров надо оставить старую логику, т.к. много прикладных завязок на это
            const mergedItems = this._isNewPanel()
                ? mergeFilterDescriptions(
                      object.clonePlain(currentSource, { processCloneable: false }),
                      result.items,
                      ['value', 'viewMode', 'textValue', 'visibility']
                  )
                : mergeFilterDescriptions(currentSource, result.items);
            const changedNames: string[] = [];
            mergedItems.forEach((item) => {
                const sourceItem = this._getItemByName(this._source, item.name);
                if (
                    !isEqual(item.value, item.resetValue && !isEqual(item.value, sourceItem.value))
                ) {
                    changedNames.push(item.name);
                }
            });
            this._resolveItems(mergedItems);
            this._updateText(this._source, this._configs, true);
            this._isDetailPanelOpened = false;
            changedNames.forEach((name) => {
                const sourceItem = this._getItemByName(this._source, name);
                // Для перезагрузки быстрых фильтров, у которых есть "выбор из справочника"
                if (
                    sourceItem &&
                    this._isFrequentItem(sourceItem) &&
                    sourceItem.editorOptions?.selectorTemplate &&
                    this._configs?.[name]
                ) {
                    this._configs[name].items = null;
                    this._configs[name].sourceController = null;
                }
            });
        } else {
            switch (result.action) {
                ...
                case 'moreButtonClick':
                    ...
                    break;
                case 'menuClosed':
                    this._resetSearch(result);
                    break;
                case 'openDetailPanel':
                    this._openDetailPanelFromSimple(result);
                    return;
            }
        }
        if (result.action !== 'moreButtonClick' && result.action !== 'menuClosed') {
            if (result.history) {
                this._notify('historyApply', [result.history]);
            }
            this._notifyChanges(this._source);
        }
        if (!this._isDetailPanelOpened) {
            this._getFilterPopupOpener().close();
        }
    }

    private _onSelectorTemplateClose(): void {
        this._idOpenSelector = null;
    }
    ...
    private _openSelector(filterItem: IFilterItem, selectedKeys: string[]): Promise<void> {
        ...
        return this._getStackOpener().open({
            ...
            eventHandlers: {
                ...
                onClose: this._onSelectorTemplateClose.bind(this),
            },
        });
    }
    ...
    private _isPrimaryItem(item: IFilterItem): boolean {
        return this._isFrequentItem(item) && item.primary && !this._isDateItem(item);
    }
    ...
    private _getDateRangeItem(items: IFilterItem[]): IFilterItem {
        ...
        if (dateRangeItem) {
            const isDateMenuEditor = dateRangeItem.editorTemplateName === DATE_MENU_EDITOR;
            const hasDateItems =
            ...
            if (
                ...
                const dates = FilterCalculator.getDatesByFilterItem(dateRangeItem);
                if (dates && dates.length && (dates[0] || dates[1])) {
                    dateRangeItem.textValue = (
                        dateRangeItem.editorOptions?.captionFormatter ||
                        this._dateRangeItemFormatter
                    )(dates[0], dates[1], dateRangeItem.editorOptions?._date);
                }
            }

            if (dateRangeItem.viewMode === 'extended') {
                Logger.error(
                    `Controls/filter:View быстрый фильтр по периоду
            с именем ${dateRangeItem.name} и типом ${dateRangeItem.type} не может находиться в области 'Можно отобрать' (viewMode: 'extended').
            Установите viewMode: 'basic' или 'frequent'k`,
                    this
                );
            }
        }
    ...
    private _dateRangeItemFormatter(startValue: Date, endValue: Date, _date?: Date): string {
    ...
    private _loadEditorOptionsName(items: IFilterItem[]): Promise<unknown> {
        const promises: Promise<unknown>[] = [];
        items.forEach((filterItem) => {
            const editorOptionsName = filterItem.editorOptionsName;
            if (editorOptionsName && this._isFrequentItem(filterItem)) {
                promises.push(
                    FilterLoader.loadEditorOptions(filterItem).then((loadedEditorOptions) => {
    ...
    private _getPopupItemConfig(item: IFilterItem, config: IFilterItemConfig) {
        ...
        popupItem.name = item.name;
        popupItem.selectedKeys =
            item.value instanceof Object && !(item.value instanceof Date)
                ? item.value
                : [item.value];
        ...
        popupItem.items = config.popupItems || config.items;
        ...
        if (item.editorOptions.source) {
            ...
            if (!config.sourceController) {
                ...
                const items = config.sourceController.getItems();
                items.assign(popupItem.items);
                config.sourceController.setItems(items);
            }
            ...
            popupItem.selectorDialogClose = this._onSelectorTemplateClose.bind(this);
    ...
    private _getPopupConfig(configs: IFilterItemConfigs, items: IFilterItem[]) {
        ...
        factory(items).each((item) => {
            if (
                (this._isFrequentItem(item) || (item.type === 'dateMenu' && items.length === 1)) &&
                !item.primary &&
                this._isVisibilityItem(item)
            ) {
    ...
    private _updateText(
        ...
        factory(items).each((item: IFilterItem) => {
            if (this._isFrequentItem(item)) {
                ...
                if (this._isItemChanged(item) && item.textValueVisible !== false) {
                    if (configs[item.name]) {
                        const isHierarchyFilter =
                            configs[item.name].items && this._isHierarchyFilter(configs[item.name]);
                        const selectedKeys =
                            configs[item.name].multiSelect || isHierarchyFilter
                                ? item.value
                                : [item.value];

                        // [ [selectedKeysList1], [selectedKeysList2] ] in hierarchy list
                        const flatSelectedKeys = isHierarchyFilter
                ...
                } else if (this._isPrimaryItem(item) && item.emptyText) {
                    this._displayText[item.name] = {
                        text: item.emptyText,
                        title: item.emptyText,
                    };
    ...
    private _getValue(selectedKeys: TKey | TKey[], name: string): unknown {
        ...
        if (
            ...
            this._configs[name].multiSelect ||
            this._isHierarchyFilter(this._configs[item.name])
        ) {
            value = selectedKeys;
        } else {
    ...
    private _setValue(selectedKeys: TKey | TKey[], name: string): void {
        const item = this._getItemByName(this._source, name);
        const value = this._getValue(selectedKeys, name);
    ...
    private _itemClick(result: IResultPopup): void {
        this._setValue(result.selectedKeys, result.name);
        if (result.textValue) {
            const item = this._getItemByName(this._source, result.name);
            item.textValue = result.textValue;
        }
        this._updateText(this._source, this._configs);
        this._updateHistory(result.name, result.data, result.selectedKeys);
    }
    ...
    private _applyClick(result: IResultPopup): void {
        ...
        factory(result.selectedKeys).each((sKey, index: string) => {
            if (sKey) {
                ...
                if (result.searchValue && !selectedItems.length) {
                    resetTextValueItems.push(index);
                }
            }
        });
        this._updateText(this._source, this._configs);
        ...
        if (result.searchValue) {
            this._configs = {};
            resetTextValueItems.forEach((name: string) => {
                const item = this._getItemByName(this._source, name);
                item?.textValue = '';
            });
        }
    ...
    private _openDetailPanelFromSimple(result: IResultPopup): void {
        this._getFilterPopupOpener().close();
        this.openDetailPanel();
    }

    private _selectorResult(result: IResultPopup): void {
        const curConfig = this._configs[result.name];
        const curItem = this._getItemByName(this._source, result.name);
        const newItems = this._getNewItems(result.data, curConfig);

        if (this._options.panelTemplateName) {
            this._updateHistory(result.name, factory(result.data).toArray());
        }
        ...
        this._setValue(selectedKeys, result.name);
        this._updateText(this._source, this._configs);
    }

    private _getRecordSetBySelectorResult(
        items: List<Model> | RecordSet,
        source: ICrudPlus | (ICrud & ICrudPlus)
    ): RecordSet {
    ...
    private _moreButtonClick(result: IResultPopup): void {
        this._idOpenSelector = result.name;
        this._configs[result.name].initSelectorItems = result.selectedItems;
    }

    private _resetSearch({ searchValue }): void {
        if (searchValue && !this._idOpenSelector) {
            this._configs = {};
        }
    }
    ...
    private async _updateHistory(
        ...
        const { source, historyId } = this._getItemByName(this._source, name).editorOptions;

        if (this._isHistorySource(source) || historyId) {
            const currentFilter = this._configs[name];
            let selectedItems = items;
            if (selectedKeys) {
                selectedItems = this._getSelectedItems(currentFilter.items, selectedKeys);
            }
            if (this._isHierarchyFilter(currentFilter)) {
                ...
                if (this._isHistorySource(source)) {
                    ...
                    selectedItems.forEach((el) => {
                        historyKeys.push(el.getKey());
                    });
                    const { Store } = await loadAsync('Controls/HistoryStore');
                    Store.push(historyId || source.getHistoryId(), historyKeys);
                }
            }
            if (currentFilter.sourceController && source.getItems) {
                const { getItemsWithHistory } = await loadAsync(
                    'Controls/Utils/History/PrepareMenuItems'
                );
                currentFilter.items = getItemsWithHistory(
                    currentFilter.items,
                    historyId,
                    this._isHistorySource(source) ? source.getOriginSource() : source,
                    {}
                );
            }
        }
    ...
    private _loadDependencies(): Promise<unknown> {
        function loadTmp(tmplName: string | TemplateFunction): Promise<unknown> | null {
            return tmplName && typeof tmplName === 'string' ? loadAsync(tmplName) : null;
        }
        ...
        function loadHistory(
            ...
            source: IFilterItem[],
            saveMode: string
        ): Promise<THistoryData> | null {
            const hasHistoryItems = source.some((item) => !item.doNotSaveToHistory);
            if (historyId && hasHistoryItems) {
                const historyConfig = FilterHistory.getHistorySourceOptions(
                    historyId,
                    source,
                    saveMode
                );
                return FilterHistory.loadFilterHistory(historyConfig);
            }
        }
        ...
        try {
            ...
            const detailPanelEmptyHistoryTemplate =
                this._options.detailPanelTemplateOptions?.emptyHistoryTemplate;
            const loadHistoryPromise = loadHistory(
                this._options.historyId,
                this._source,
                this._options.detailPanelTemplateOptions?.historySaveMode
            );
            if (!this._loadOperationsPanelPromise) {
                this._loadOperationsPanelPromise = Promise.all([
                    loadAsync(popupLibrary),
                    loadTmp(detailPanelTemplateName),
                    loadTmp(detailPanelTopTemplateName),
                    loadTmp(extendedTemplateName),
                    this._loadEditorOptions(),
                    FilterLoader.loadEditorTemplateName(this._source, 'cloud'),
                    FilterLoader.loadCallbacks(this._source),
                    loadTmp(detailPanelEmptyHistoryTemplate),
                ]);
            }
            return Promise.all([this._loadOperationsPanelPromise, loadHistoryPromise]).then(() => {
    ...
    private _setEditorOptionsForFilterDescription(): Promise<void> | void {
        ...
        this._setEditorOptionsPromise = Promise.all(
            this._source.map((item) => {
                return item.editorOptionsName ? FilterLoader.loadEditorOptions(item) : undefined;
            })
        );
        return this._setEditorOptionsPromise.then((editorOptions) => {
            this._source.forEach((item, index) => {
                if (item.editorOptionsName) {
                    ...
                    item.__editorOptionsResolved = true;
                    if (this._isFrequentItem(item)) {
                        this._setConfigByItem(item);
                    }
                }
