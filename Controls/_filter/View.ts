...
class FilterView extends Control<IFilterViewOptions, IFilterReceivedState> implements IFilterView {
    ...
    protected _openDetailPanel(event?: SyntheticEvent<Event>, name?: string): void {
    ...
    private _resultHandler(result: IResultPopup): void {
        if (!result.action) {
            ...
            this._resolveItems(mergeSource(currentSource, result.items));
    ...
    private _getDateRangeItem(items: IFilterItem[]): IFilterItem {
        ...
        factory(items).each((item) => {
            ...
            if (this._isDateItem(item) && (item.editorOptions?.emptyCaption || hasValue)) {
                dateRangeItem = item;
            }
        });
        ...
        factory(items).each((item) => {
            if (this._isFrequentItem(item) && configs[item.name]) {
                ...
                const isDateMenuEditor =
                    item.editorTemplateName === 'Controls/filterPanel:DateMenuEditor';
    ...
    private _loadDependencies(): Promise<unknown> {
        try {
            ...
            const extendedTemplateName = this._options.detailPanelTemplateOptions?.extendedTemplateName;
