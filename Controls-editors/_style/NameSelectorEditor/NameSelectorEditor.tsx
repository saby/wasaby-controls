...
export function NameSelectorEditor({
    ...
    storeId,
    children,
}: PropsWithChildren<INameSelectorEditorProps>): JSX.Element {
    const slice = useStrictSlice<FormSlice>(storeId);
    const [editing, setEditing] = useState<boolean>(initEditing);
    const [focusIndex, setFocusIndex] = useState<number>(TITLE_INDEX);
...
function ViewMode({
    ...
    selectorValue,
    children,
}: PropsWithChildren<IViewModeProps>): ReactElement {
    ...
    const onSelectorClick = useCallback(() => {
        onClick(SELECTOR_INDEX);
    }, [onClick]);

    return (
        <div
            style={style}
            className={`tw-flex tw-justify-between tw-items-baseline tw-overflow-hidden controls-inlineheight-${inlineHeight} NameSelectorEditor ${className}`}
        >
            <Title
                caption={nameValue}
            ...
            {!hideSelector && (
                <Title
                    ...
                    fontWeight="normal"
                    className="tw-truncate controls-margin_left-m tw-flex-grow"
    ...
    return (
        ...
        >
            ...
            >
                <>
                    <Field
                        autofocus={focusIndex === TITLE_INDEX}
                        bold={true}
                    ...
                    {!hideSelector && (
                        <>
                            ...
                            <Field
                                autofocus={focusIndex === SELECTOR_INDEX}
