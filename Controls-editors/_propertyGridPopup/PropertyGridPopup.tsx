...
import { EditorsProvider, ControlSizeContextProvider } from 'Controls-editors/object-type';
...
import { Context, IPendingContextProvider } from 'Controls/Pending';
import {
    Controller as ValidationController,
    ControllerClass as ValidationControllerClass,
    IFormValidateResult,
    IFormValidate,
} from 'Controls/validate';
import { PropertyGrid, useSystemProperties } from 'Controls-editors/propertyGrid';
import { useRecordValue } from 'Controls-editors/_propertyGrid/useRecordValue';

function resetInvalidValues<T extends object>(
    values: T,
    initValues: T,
    validation: IFormValidateResult
): T {
    const resetValues: Record<string, unknown> = {};

    for (const item of Object.values<IFormValidate>(validation as Record<string, IFormValidate>)) {
        if (item && typeof item === 'object' && item.hasOwnProperty('areaKey')) {
            const { areaKey } = item;
            if (typeof areaKey === 'string' && initValues) {
                resetValues[areaKey] = (initValues as Record<string, unknown>)[areaKey as string];
            }
        }
    }
 ...
 */
const PropertyGridPopup = forwardRef((props: IPropertyGridPopup, ref: ForwardedRef<unknown>) => {
    const {
        metaType,
        sort,
        value,
    ...
    const displayedValue: Record<string, unknown> = useMemo(() => {
        return (
    ...
    const displayedValueRef = useRef<Record<string, unknown>>();

    displayedValueRef.current = displayedValue;

    const rootRef = useRef<DialogControl & { _container: HTMLDivElement }>(null);

    const pendingContext = useContext(Context) as unknown as IPendingContextProvider;

    const validationControllerRef = useRef<ValidationController>(null);

    const validationControllerClass = useMemo(() => new ValidationControllerClass(), []);

    const initialValue = useRef<RuntimeInterface | undefined>(value);
    ...
    useEffect(() => {
        let resolvePending: undefined | ((value?: unknown) => void);
        ...
        pendingContext?.registerPending(
            new Promise((resolve) => {
                resolvePending = resolve;
            }),
            {
                onPendingFail: async (forceFinishValue?: boolean) => {
                    if (forceFinishValue || (!autoSave && !isValueUpdatedRef.current())) {
                        resolvePending?.();
                        return;
                    }
                    ...
                    if (validationControllerRef.current?.submit) {
                        validationResult = await validationControllerRef.current.submit({
                            activateInput: false,
                        });
                    }
                    ...
                    if (
                        ...
                        const resetAnswer = await Confirmation.openPopup({
                            message: rk(
                                'У вас есть несохраненные данные. При закрытии окна они сбросятся. Продолжить?'
                            ),
                            type: 'yesno',
                        });

                        if (resetAnswer) {
                            const formResult = validationControllerClass.getFormValidationResult();

                            const resetValues = resetInvalidValues<RuntimeInterface>(
                                displayedValueRef.current as RuntimeInterface,
                                initialValue.current as RuntimeInterface,
                                formResult
                            );

                            propsOnChange?.(resetValues);
                            resolvePending?.();
                        } else {
                    ...
                    const saveAnswer = await Confirmation.openPopup({
    ...
    const onReset = useCallback(
        (newValue: object) => {
            ...
            for (const propertyName of Object.keys(headerEditorMeta?.getProperties())) {
                delete preparedNewValue[propertyName];
            }
            onChange(preparedNewValue);
        },
        [onChange, headerEditorMeta]
    );
    ...
    const onCloseCallback = useCallback(() => {
        if (!autoSave) {
            pendingContext.finishPendingOperations(true, true);
        }

        onClose?.();
    }, [autoSave, onClose, pendingContext]);
    ...
    const ScrollContentMemoized = useMemo(() => {
        ...
        bodyOnChange,
        isLightColorScheme,
    ]); // Получает данные только при первом рендере (прочти доклет вначале файла)
    ...
    }, [props.onClose, displayedValue, displayedMetaType, title, headerOnChange, headerEditorMeta]);
    ...
    let content = (
        <PropertyGridContextProxy
            ...
            dataContextValues={props.dataContextValues}
    ...
    if (!!controlSizeContext) {
        content = (
            <ControlSizeContextProvider
                getSize={controlSizeContext.getSize}
                getParentSize={controlSizeContext.getParentSize}
            >
                {content}
            </ControlSizeContextProvider>
        );
...
const ObjectEditorPopupMemo = memo(
    forwardRef((props: IPropertyGridPopup, ref: ForwardedRef<unknown>) => {
        const { metaType, onChange, value, ...restProps } = props;
        const [objectValue, getRecord] = useRecordValue(value);
        const [preparedMeta, expandValue, foldValue] = useSystemProperties(metaType);

        const expandedValue = useMemo(() => {
            if (objectValue) {
                return expandValue(objectValue);
            }
        }, [objectValue, expandValue]);

        const onChangeCallback = useCallback(
            (newValue: object) => {
                const prepared = foldValue(newValue);
                const resultValue = getRecord(prepared, preparedMeta);
                onChange(resultValue);
            },
            [foldValue, getRecord, onChange, preparedMeta]
        );
