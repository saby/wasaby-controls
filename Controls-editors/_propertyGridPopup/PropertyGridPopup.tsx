...
const PropertyGridPopup = forwardRef(
    <RuntimeInterface extends object>(props: IPropertyGridPopup<RuntimeInterface>, ref) => {
        ...
        const headerEditorMeta = useMemo(() => {
            const _metaType = path.meta ? path.meta : metaType;
            const atttrs = Object.entries(_metaType.getProperties()).filter(([, meta]) => {
                return meta && meta.getOrder() === HEADER_PLACE_ORDER;
            });
            return atttrs.length
                ? _metaType.properties(
        ...
        const bodyEditorMeta = useMemo(() => {
            const _metaType = path.meta ? path.meta : metaType;
            return _metaType.properties(
                Object.entries(_metaType.getProperties())
                    .filter(([, meta]) => {
                        return meta && meta.getOrder() !== HEADER_PLACE_ORDER;
                    })
        ...
        const displayedValue = useMemo(() => {
            return path.path.reduce((value, propertyName) => {
        ...
        }, [props.onClose, displayedValue, metaType, title, headerOnChange, headerEditorMeta]);
