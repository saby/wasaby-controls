...
import { EditorsProvider, extractAttrsWithoutDefaults } from 'Controls-editors/object-type';
...
const PropertyGridPopup = forwardRef(
    <RuntimeInterface extends object>(props: IPropertyGridPopup<RuntimeInterface>, ref) => {
        ...
        const attrsWithoutDefaults = useMemo(
            () => extractAttrsWithoutDefaults(changedValue, metaType),
            [changedValue, metaType]
        );
        ...
        useEffect(() => {
            const observer = new ResizeObserverUtil(rootRef.current, () => {
                __notifyFromReact(rootRef.current, 'controlResize', [], true);
            });

            if (rootRef.current) {
                observer.observe(rootRef.current._container);
            }
        ...
        const filterHeaderValues = useCallback((newValue) => {
        ...
        const bodyOnChange = useCallback((newValue) => {
            onChange({ ...newValue, ...headerChangedValueRef.current });
        }, [onChange]);

        const headerOnChange = useCallback((newValue) => {
        ...
        }, [resetTrigger, changedValue, bodyEditorMeta, bodyOnChange]); // Получает данные только при первом рендере (прочти доклет вначале файла)
        ...
        }, [resetTrigger, applyHandler, ScrollContentMemoized, autoSave]); // Получает данные только при первом рендере (прочти доклет вначале файла)
        ...
        }, [props.onClose, changedValue, metaType, title, attrsWithoutDefaults, headerOnChange, headerEditorMeta]);
