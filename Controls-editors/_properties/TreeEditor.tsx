...
function EditorTemplate(props): JSX.Element {
    ...
    const selectorTemplate = {
        ...
        templateOptions: {
            ...
            sendResultCallback: (result: IResultSelector) => {
                ...
                item[0].name = result.id;
                props.onChange({items: [...props.currentValue.item], selectedKeys: props.selectedKeys});
            },
        },
        popupOptions: {
            width: 500,
    ...
    const onSelectorCallback = (keys) => {
        if (!keys.length) {
            ...
            delete item[0].name;
            props.onChange({items: [...props.currentValue.items], selectedKeys: props.selectedKeys});
        }
    };
            ...
            <div className="tw-flex tw-items-baseline">
                ...
                <EditingTemplate
                    ...
                    editorTemplate={useContent((contentProps) => {
                        return <EditorTemplate {...contentProps}
                                               dataContext={props.dataContext}
...
export const TreeEditor = memo((props: ITreeEditorProps): JSX.Element => {
        ...
        if (value.items) {
            if (currentItem.current.isDefaultValue ||
                ...
                currentItem.current.config = getConfig(currentItem.current.items,
                    allowHierarchy, expanderVisibility, expandedItems);
            }
        } else {
            ...
            case 'nesting':
                for (let i = 0; i < itemsData.length; i++) {
                    if (itemsData[i].id === item.get('id') && itemsData[i - 1]) {
                        itemsData[i].parent = itemsData[i - 1].id;
                        itemsData[i - 1].node = true;
                    }
                }
                onChange({...value, items: [...itemsData]});
                viewRef.current.reload();
    ...
    const listOptions = {
        ...
        customEvents: ['onBeforeEndEdit', 'onAfterEndEdit', 'onActionClick', 'onCustomdragEnd',
            'onDeactivated', 'onAfterItemExpand', 'onAfterItemCollapse'],
    };
