...
import {
    ColumnTemplate,
    EmptyTemplate as GridEmptyTemplate,
    FooterTemplate as GridFooterTemplate,
        ...
        <div className="ws-flexbox ws-flex-column controls-padding_left-2xs">
            <FocusRoot autofocus={true} ref={props.$wasabyRef} as={'div'}>
                <Input
                    ...
                    className="controls-TreeEditor-Lookup controls-Input_negativeOffset"
...
export const TreeEditor = forwardRef((props: ITreeEditorProps, ref): JSX.Element | null => {
    ...
    const reachItemsLimit = useRef<boolean>(false);
    reachItemsLimit.current = maxItems ? value.items.length >= maxItems : false;
    ...
    const itemActionVisibilityCallback = useCallback((action: IItemAction, item: Model) => {
        ...
        switch (action.id) {
            ...
            case 'noNesting':
                ...
                return false;
            case 'copy':
                if (maxItems && reachItemsLimit.current) {
                    return false;
                }
            default:
                return (
    ...
    const footerTemplate = useContent((props) => {
        return (
            <GridFooterTemplate {...props}>
                <Button
                    icon="icon-Add"
    ...
    const emptyTemplate = useContent((props) => {
        return (
            <GridEmptyTemplate
                {...props}
                align={'start'}
                ...
                contentTemplate={
                    emptyText ? (
                        <div className="controls-TreeEditor__emptyText">{rk(emptyText)}</div>
                    ) : null
                }
            />
        );
    ...
    if (!footerCaption) {
        delete listOptions.footerTemplate;
    }
