...
async function getArgumentsMeta(
    ...
    const OriginalParentType = fieldType?.get('AdditionalInfo')?.OriginalParentType;

    const objectNamePrepared = OriginalParentType ?? objectName;

    const argumentType = await getArgumentsType(objectNamePrepared, methodName, objectName);

    const argumentsMeta = buildArgumentMetaType(
        objectNamePrepared,
        methodName,
        argumentType.arguments
    );

    await loadEditors(argumentsMeta);
...
function buildArgument(arg: IDataObjectArgumentType): Meta<unknown> | undefined {
    ...
    switch (type) {
        ...
        case 'Boolean':
            ...
            break;
        case 'Period':
            meta = StringType.editor('Controls-editors/function:PeriodTypeEditor').defaultValue(
                'byFilter'
            );
            break;
        case 'Money':
...
function unpackArgumentValues(
    ...
    metaType?: IFunctionDescription
): {
    value: Record<string, unknown>;
    defaultValueMerged: boolean;
} {
    ...
    let defaultValueMerged = false;
    if (metaType?.arguments) {
        const argumentMeta = metaType.arguments;
        Object.entries(argumentMeta.getProperties()).forEach(([key, propertyMeta]) => {
            if (!result[key] && propertyMeta.getDefaultValue()) {
                defaultValueMerged = true;
                result[key] = propertyMeta.getDefaultValue();
            }
        });
    }
