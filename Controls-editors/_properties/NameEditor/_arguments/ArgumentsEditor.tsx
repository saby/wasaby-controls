...
function ArgumentsEditor(props: IArgumentsEditorProps) {
    ...
    const argumentValues = useMemo(() => {
        const result = unpackArgumentValues(value?.factory.arguments, metaType);
        if (result.defaultValueMerged) {
            value.factory.arguments = packArgumentValues(objectName, methodName, result.value);
            onChange({ ...value });
        }
        return result.value;
    }, [value, metaType]);
