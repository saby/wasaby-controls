...
import 'css!Controls-editors/_properties/NameEditor/NameEditor';
...
export function DataObjectFieldSelector(props: INameSelectorProps) {
    ...
    const [callableDesc, setCallableDesc] = useState();
    ...
    const selectedKeys = useMemo(() => {
        if (FrameBaseModule.isFunctionBinding(value) || FrameBaseModule.isDataSetBinding(value)) {
            setCallableDesc(value);
        ...
        if (callableDesc) {
            setCallableDesc(undefined);
        }
        return [];
    }, [FrameBaseModule, callableDesc, value]);
    ...
    const placeHolder = useMemo(() => {
        return getPlaceholder(fieldType);
    }, [fieldType]);
...
function getPlaceholder(types: string[] = []): string {
    const placeholderEntries: string[] = [];

    if (types.includes('Function')) {
        placeholderEntries.push('метод');
    }

    if (!types.length || types.length > placeholderEntries.length) {
        placeholderEntries.unshift('поле');
    }

    return `Выберите ${placeholderEntries.join(' или ')}`;
}
