...
import { ObjectMeta, WidgetMeta, TextStyleType } from 'Meta/types';

const SYSTEM_PROPERTIES = ['.style'];
const TextStyleId = TextStyleType.getId();
...
/**
 * Работает с системными свойствами:
 * удаляет текстовые стилевые свойства,
 * а остальные системные свойства выносит на один уровень с обычными
 */
 ...
 * а текстовые стилевые свойства удалены
 */
        ...
        if (systemProperties) {
            ...
            for (const [sysName, sysValue] of Object.entries(systemProps)) {
                if ((sysValue as ObjectMeta).isVisible()) {
                    if ((sysValue as ObjectMeta).getId() !== TextStyleId) {
