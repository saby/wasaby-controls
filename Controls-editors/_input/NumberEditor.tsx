import { Fragment, memo } from 'react';
import { Number as NumberInputControl } from 'Controls/input';
import { IPropertyGridPropertyEditorProps } from 'Controls-editors/propertyGrid';
import * as rk from 'i18n!Controls-editors';
import { useInputEditorValue } from './useInputValue';

/**
 * @public
 */
export interface INumberEditorProps
    extends Omit<IPropertyGridPropertyEditorProps<number | null | undefined>, 'LayoutComponent'> {
    /**
     * Текст подсказки, который отображается в пустом поле ввода
     */
    placeholder?: string;
    /**
     * Максимальная длина целой части
     */
    integersLength?: number;
    /**
     * Количество знаков в дробной части
     */
    precision?: number;
    /**
     * Только положительные числа
     */
    onlyPositive?: boolean;
    /**
     * Показывать ли нули в конце дробной части
     */
    showEmptyDecimals?: boolean;

    LayoutComponent?: IPropertyGridPropertyEditorProps<number>['LayoutComponent'];
    dataQa?: string;
}

/**
 * Реакт компонент, редактор числа
 * @class Controls-editors/_input/NumberEditor
 * @implements Controls-editors/input:INumberEditorProps
 * @public
 */
export const NumberEditor = memo((props: INumberEditorProps) => {
    const {
        type,
        metaType,
        value = null,
        onChange: onChangeOrigin,
        placeholder = rk('Ваш ответ'),
        integersLength,
        LayoutComponent = Fragment,
        precision,
        onlyPositive,
        showEmptyDecimals,
        dataQa,
    } = props;
    const readOnly = type?.isDisabled() || metaType?.isDisabled();

    const { changeHandler, localValue, inputCompleteHandler } = useInputEditorValue({
        value,
        onChange: onChangeOrigin,
    });

    return (
        <LayoutComponent>
            <NumberInputControl
                textAlign={'right'}
                className="controls-Input__width-5ch"
                value={localValue}
                readOnly={readOnly}
                precision={precision}
                integersLength={integersLength}
                valueChangedCallback={changeHandler}
                onInputCompleted={inputCompleteHandler}
                placeholder={placeholder}
                onlyPositive={onlyPositive}
                showEmptyDecimals={showEmptyDecimals}
                data-qa={dataQa || 'controls-PropertyGrid__editor_number'}
            />
        </LayoutComponent>
    );
});
