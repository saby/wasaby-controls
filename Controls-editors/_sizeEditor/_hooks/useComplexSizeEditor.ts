...
import { ISize } from 'Controls-editors/object-type';
...
export const useComplexSizeEditor = (props: TUseComplexSizeEditorProps): IUseComplexSizeEditor => {
    const { value, metaType, absoluteSize, onChange } = props;
    ...
    const activeSizeTypes = useMemo(
        ...
        [availableBaseSizeTypes, limitedSizes]
    );
    ...
    const addSizeHandler = useCallback(
        ...
        [changeLimitedSizes, editorValue, metaProperties]
    );
    ...
    const delSizeHandler = useCallback(
        ...
        [changeLimitedSizes, editorValue, limitedSizes, onChange]
    );
    ...
    const changeSizeHandler = useCallback(
        ...
        [changeLimitedSizes, delSizeHandler, editorValue, onChange]
    );
    ...
    }, [metaProperties?.aspectRatio, editorValue]);
    const toggleAspectRatio = useCallback(() => {
        ...
        const { width = 0, height = 0 } = absoluteSize || {};
        const calculatedRatio = width && height ? width / height : undefined;

        // hack with || 1 - need fix
        const newValue = changeAspectRatio(calculatedRatio || defaultAspectRatio || 1, editorValue);

        onChange?.(newValue);
        changeLimitedSizes(newValue);

        return;
    }, [editorValue, onChange, metaType, absoluteSize, changeLimitedSizes]);
