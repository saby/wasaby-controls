...
const checkIsBaseUnit = (unit: KeyUnits) => BASE_UNITS.includes(unit);
...
export const changeAspectRatio = (aspectRatio: number, value: TValue): TValue => {
    ...
    if (!checkIsBaseUnit(parsedWidth.baseUnits) || !checkIsBaseUnit(parsedHeight.baseUnits))
        return value;
