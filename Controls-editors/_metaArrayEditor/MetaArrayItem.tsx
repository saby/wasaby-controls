...
export default function MetaArrayItem(props: IArrayItemProps): JSX.Element {
    ...
    const isMetaWithAttributes = useMemo(() => {
        const attributes = metaType?.getProperties?.();
        const visibleAttributesNames = Object.keys(attributes || {}).filter((attributeName) => {
            /*TODO: убрать проверку на .style, когда фильтр и настройка поиска в дашборде перестанут быть widgetType
             *  https://online.sbis.ru/opendoc.html?guid=71ce2bca-f584-4dc8-813a-ed003840637c&client=3*/
            return !attributes[attributeName].isHidden() && attributeName !== '.style';
        });
        return visibleAttributesNames?.length;
    }, [metaType]);

    const canShowEditors = useEditorsLoader(metaType);
    ...
    return (
        ...
        >
            ...
            {canShowEditors && isMetaWithAttributes ? (
                <PropertyGrid
                    metaType={metaType}
                    onChange={onChange}
                    value={item.value}
