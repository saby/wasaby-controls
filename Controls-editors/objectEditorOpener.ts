...
import { IEditors, loadEditorData, IControlSizeContext } from 'Controls-editors/object-type';
import { ObjectMeta, loadEditors } from 'Meta/types';
import { useState } from 'react';
import type { IDataContextProxyValues } from 'Controls-DataEnv/context';
 ...
 */
export interface IObjectEditorOpenerProps {
     ...
     */
    controlSizeContext?: IControlSizeContext;
    refresh: boolean;
    autoSave?: boolean;
    opener?: IOpener;
}
...
export default class ObjectEditorOpener {
    ...
    private _openerType: OpenerTypeEnum;
    /**
    ...
    async open({
        ...
        const openerTypeChanged = this._openerType !== openerType;
        const metaWidth = getOpenerWidth(metaType);

        if (!this._opener) {
            this._opener = getOpener(openerType);
        } else if (openerTypeChanged) {
            this._opener.destroy();
            this._opener = getOpener(openerType);
        }

        this._openerType = openerType;
        ...
        const editorData = await loadEditorData(metaType, value);
        ...
        this._opener.open({
            template: await this.getContentTemplate(metaType),
            templateOptions: {
                autoSave,
                metaType,
                value,
            ...
            width:
                openerType === OpenerTypeEnum.Stack && metaWidth ? metaWidth : DEFAULT_POPUP_WIDTH,
        });
    }
    ...
    protected getTemplateProps() {
        return {};
    }

    protected getDefaultTemplate(): Promise<string | any> {
        return Promise.resolve('Controls-editors/propertyGridPopup:PropertyGridPopup');
    }

    private getContentTemplate(metaType: ObjectMeta<object>): Promise<string | any> {
