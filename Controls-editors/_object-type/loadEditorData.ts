...
async function loadEditorData(type: Meta<unknown>, props: object): Promise<unknown> {
    if (!type) {
        return;
    }
    // генерируем конфиг для корневого редактора мета-типа
    const rootConfig = getEditorSourceConfig(type, props);

    let attrsEditorsConfig;
    if (type instanceof ObjectMeta) {
        attrsEditorsConfig = Object.values(type.getProperties()).reduce(
            (acc, attrType) => {
                const config = getEditorSourceConfig(attrType, props);
                if (config && !!attrType) {
...
function getEditorSourceConfig(
    ...
    if (configGetter && typeof configGetter === 'string') {
        return {
            configGetter,
            configGetterArgumentsArray: [props, {}, {}],
        };
    }
}
