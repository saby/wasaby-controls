...
function getExtendedFields<RuntimeInterface extends object>(
    metaType: ObjectMeta<RuntimeInterface>,
    value: RuntimeInterface
): IExtendedFieldOption[] {
    const attrs = metaType?.attributes();
    return Object.keys(attrs)
        .filter((key) => !!attrs[key].getExtended() && !value.hasOwnProperty(key))
        .map((key) => ({ id: key, title: attrs[key].getTitle() }));
}
...
export const ObjectTypeEditor = memo(
    ...
    >(
        ...
        const [extendedFields, setExtendedFields] = useState<IExtendedFieldOption[]>(
            getExtendedFields(metaType, value)
        );

        const handleSelectExtendedField = useCallback((fieldId: string) => {
            setExtendedFields((p) => p.filter((field) => field.id !== fieldId));
        }, []);
                                ...
                                return (
                                    <Fragment key={id}>
                                        ...
                                        {attributes.map((attribute) => {
                                            return !extendedFields.find(
                                                (f) => f.id === attribute.name
                                            ) ? (
                                                <AttributeEditor
                                                    {...attribute}
                                                    key={attribute.name}
