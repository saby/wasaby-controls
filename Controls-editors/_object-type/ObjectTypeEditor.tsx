...
import { GroupRender } from './renders/GroupRender';
...
export const ObjectTypeEditor = memo(
    ...
    >(
        ...
        const preparedEditorLayoutProps = EditorLayoutProps ?? rootContext?.EditorLayoutProps;
        ...
        const rootContextValue = useMemo<IObjectTypeEditorRootContext>(() => {
            return {
                ...
                EditorLayoutProps: preparedEditorLayoutProps,
            };
        }, [
            open,
            hidePropertyHandler,
        ...
        const groups = useMemo(() => {
            ...
            if (!!slice) {
                return getGroupsFromMeta(metaType, slice?.getEditor);
            }
                                    ...
                                    {groups.map((x) => {
                                        return (
                                            <GroupRender
                                                ...
                                                EditorLayoutProps={preparedEditorLayoutProps}
                                                extendedItems={extendedItemsRef.current}
...
function AttributesRender(props: IAttributesRenderProps) {
    const { attributes, onChange, extendedItems, EditorLayoutComponent, EditorLayoutProps, value } =
        props;

    return (
        <>
            {attributes.map((attribute) => {
                return !extendedItems.find((extItem) => extItem.id === attribute.name) ? (
