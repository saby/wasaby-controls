...
export function getGroups(options: IGetGroupsOptions): IGroup[] {
    ...
    function handleMeta(name: string, meta: Meta<any>): void {
        ...
        if (originMeta &&
            originKey &&
            (originMeta.getEditor(editorsMap).component || editorsMap[originMeta.getId()])) {
            return handleCompoundMeta(name, meta);
        }

        const Component: any =
            meta.getEditor(editorsMap).component || editorsMap[meta.getId()] ||
            (meta.is(ObjectType) ? editors[ObjectType.getId()] : null);
        if (!Component) {
            console.error('Отсутствует компонент редактора для метатипа: ', meta);
            return;
        }
