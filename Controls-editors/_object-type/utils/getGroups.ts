...
import { StringEditor, NumberEditor, BooleanEditor } from '../../properties';
...
export function getGroups(options: IGetGroupsOptions): IGroup[] {
    const { metaType, attributes, category, editors, sort, defaultGroupName = '' } = options;
    ...
    function getGroup(meta: Meta<any>): IGroup {
        ...
        const metaGroupId = meta.getGroup()?.uid || defaultGroupName;
        let group = groups.find(({ id }) => {
            return id === metaGroupId;
        });
        if (!group) {
            group = { id: metaGroupId, title: metaGroupName, attributes: [] };
            groups.push(group);
        }
        return group;
    }
    ...
    Object.entries((attributes || {}) as Record<string, ObjectMeta<any>>)
        ...
        .map((entry) => {
            const attachedStyles: { [k: string]: Meta<any> } = metaType.getAttachedStyles?.(entry[0]);
            if (attachedStyles) {
                entry[1] = entry[1].appendStyles?.(attachedStyles);
            }
