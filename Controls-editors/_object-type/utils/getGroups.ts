...
export interface IAttribute<T = any> {
    metaType: Meta<T>;
    name: string;
    attributes?: ObjectMetaProperties<any>;
    Component: FC<
...
export function getGroups(options: IGetGroupsOptions): IGroup[] {
    ...
    function handleCompoundMeta(name: string, meta: Meta<any>): void {
        ...
        const converter: IAttributeConverter = {
            ...
            output(value: any): any {
                ...
                Object.entries(converterMap?.output || {}).forEach(([key, key2]) => {
                    const finalValue = value[key2] ?? (required?.[key] ? value[key2] : undefined);
        ...
        let modifiedOriginMeta = originMeta;
        if (originMeta.properties) {
            const newProperties: Record<string, unknown> = {};
            Object.keys(originMeta.getProperties()).forEach((attributeNameFromOriginProperty) => {
                if (!!metaType.getProperties()[attributeNameFromOriginProperty]) {
                    newProperties[attributeNameFromOriginProperty] =
                        metaType.getProperties()[attributeNameFromOriginProperty];
                }
            });
            modifiedOriginMeta = modifiedOriginMeta.properties(newProperties);
        }
        group.attributes.push({
            metaType: modifiedOriginMeta,
            name,
            converter,
