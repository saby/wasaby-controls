import {
    ...
    PropertyGrid,
    DefaultEditorLayoutHierarchyPadding,
...
function Expander({ expanded, onClick }: IExpanderProps): ReactElement {
    const className = 'controls-PropertyGrid__typeExpander';
    const icon = expanded ? 'icon-MarkExpandBold' : 'icon-MarkRightBold';
    const tooltip = rk(expanded ? 'Свернуть' : 'Развернуть');
    return (
        <Icon
            icon={icon}
            iconStyle={'label'}
