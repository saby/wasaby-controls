...
const ObjectEditorPopup = forwardRef(
    <RuntimeInterface extends object>(props: IObjectEditorPopupProps<RuntimeInterface>, ref) => {
        const { metaType, sort, value, onClose } = props;

        const staticValRef = useRef({});

        useEffect(() => {
            staticValRef.current = props.value
                ? Object.keys(props.value)
                      .filter(
                          (valKey) => !Object.keys(props.metaType.attributes()).includes(valKey)
                      )
                      .reduce((accum, key) => ({ ...accum, [key]: props.value[key] }), {})
                : {};
        }, []);
        ...
        const headerEditorMeta = useMemo(() => {
            const atttrs = Object.entries(metaType.attributes()).filter(([, meta]) => {
        ...
        const bodyEditorMeta = useMemo(
            () =>
                metaType.attributes(
                    Object.entries(metaType.attributes())
                        .filter(([, meta]) => {
                            return meta && meta.getOrder() !== HEADER_PLACE_ORDER;
                        })
        ...
        const onChange = (newValue) => {
            ...
            // TODO: staticValRef -- временное решение. удалить по задаче: https://online.sbis.ru/opendoc.html?guid=62f12fe1-286a-4ed0-a460-1e26e8e0f5cd&client=3
            props.onChange({ ...newValue, ...staticValRef.current });
        };
        ...
        const filterHeaderValues = (newValue) => {
            const headerMetaAttributes = headerEditorMeta?.attributes();
        ...
        const filterBodyValues = (newValue) => {
            const bodyMetaAttributes = bodyEditorMeta.attributes();
        ...
        const reset = () => {
            ...
            // TODO: staticValRef -- временное решение. удалить по задаче:
            // https://online.sbis.ru/opendoc.html?guid=62f12fe1-286a-4ed0-a460-1e26e8e0f5cd&client=3
            const newValue = {
                ...changedValue,
                ...defaultValue,
                ...staticValRef.current,
            };
            setChangedValue(newValue);
