...
const extractStaticAttrs = (val, metaType) => {
    return val
        ? Object.keys(val)
              .filter((valKey) => !Object.keys(metaType.attributes()).includes(valKey))
              .reduce((accum, key) => ({ ...accum, [key]: val[key] }), {})
        : {};
};
...
const ObjectEditorPopup = forwardRef(
    <RuntimeInterface extends object>(props: IObjectEditorPopupProps<RuntimeInterface>, ref) => {
        ...
        const staticVal = useMemo(() => extractStaticAttrs(value, metaType), []);
        ...
        const onChange = (newValue) => {
            ...
            props.onChange({ ...newValue, ...staticVal });
        };
        ...
        const reset = () => {
            // TODO: staticVal -- временное решение. удалить по задаче:
        ...
        }, [props.onClose, changedValue, metaType, title, staticVal]);
