...
import { IEditor, IOpenStackParams } from 'Controls-editors/_recordset/interface';
...
export function Editor<RuntimeInterface>(props: IEditorProps<RuntimeInterface>) {
    ...
    const editingItemIndex = React.useRef<number | null>(null);
    const popupVersion = React.useRef<number | null>(null);
    ...
    React.useEffect(() => {
        if (stack.isOpened() && editingItemIndex.current) {
            const item = items.at(editingItemIndex.current);
            if (!editingItemIndex.current) {
                Logger.error('Не найден редактируемый элемент. Возможно, он был удален');
                stack.close();
            } else {
                popupVersion.current += 1;
                (propsOnItemClick ?? openStack)({
                    ...
                    templateOptions: {
                        ...
                        validation,
                        readOnly,
                        version: popupVersion.current,
                    },
                    onClose: () => {
                        editingItemIndex.current = null;
                        popupVersion.current = null;
                    },
    ...
    const onItemClick = React.useCallback(
        (item: Record) => {
            editingItemIndex.current = items.getIndex(item);
            popupVersion.current = 0;
            (propsOnItemClick ?? openStack)({
                stack,
                templateOptions: {
                    value: item,
    ...
    const onAddItem = React.useCallback(
        (newValue: Record) => {
            ...
            editingItemIndex.current = nextItems.getCount() - 1;
            onChange(nextItems);
        },
