...
export default class Player extends Control<IPlayerOptions> {
    ...
    protected _beforeMount(
        ...
        if (constants.isBrowserPlatform) {
            return import('Controls-Lottie/third-party/lottie').then(
                (lottie) => {
                    this._lottie = lottie;
                }
            );
    ...
    protected _beforeUnmount(): void {
        this._animationInstance.removeEventListener(
            'complete',
        ...
        if (!this._options.loop) {
            this._animationInstance.addEventListener(
                'enterFrame',
