...
function createHistoryItem(
    ...
    objectData: string | CrudEntityKey | null,
...
function validateHistory(
    ...
    if (
        !itemsByHistoryId.recent?.getRecordById(id) &&
        !itemsByHistoryId.pinned?.getRecordById(id)
    ) {
...
function stringify(value: unknown): string {
    return JSON.stringify(value, new Serializer().serialize);
}
...
const incrementCounterByObjectData = (
    ...
    if (dataStorage.recent) {
        const objectData = stringify(value);
...
const pushToStore = (historyId: string, value: THistoryData) => {
    ...
    function isObjectValue(value: THistoryData): value is object[] | object {
        const multipleValues = Array.isArray(value);
        return (multipleValues && isObject(value[0])) || isObject(value);
    }
    ...
    if (isObjectValue(value)) {
        ...
        values.forEach((v: CrudEntityKey) => {
            const val = String(v);
        ...
        if (!isCounterChanged) {
            values.forEach((val: CrudEntityKey) => {
                historyStoreData.recent?.prepend([
                    createHistoryItem(String(val), val, historyId, 1),
                ]);
            });
...
export const Store: IHistoryStore = {
    ...
    push(
        ...
        function isObjectType(
            value: THistoryData | Record<THistoryId, THistoryData>
        ): value is Record<THistoryId, THistoryData> {
            return typeof value === 'object' && value?.hasOwnProperty(historyId);
        }
        if (isObjectType(value)) {
            Object.keys(value).forEach((key) => {
                if (value.hasOwnProperty(key)) {
                    pushToStore(key, value[key]);
                }
            });
        } else {
            pushToStore(historyId, value);
        }
        return Promise.resolve(stringify(value));
    },
    ...
    update(historyId: THistoryId, id: CrudEntityKey, value: THistoryData): Promise<void> {
        ...
        if (typeof value === 'string') {
            ...
            objectDataValue = stringify(value);
        }
        if (itemsByHistoryId.pinned?.getRecordById(id)) {
