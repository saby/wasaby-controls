 ...
 * {@link Controls-Colors/dialogTemplate:ColormarkTemplate раскладки} окна, так же может использоваться внутри прикладной
 * раскладки.
 * Дополнительно смотри: {@link Controls-Colors/dialogOpener:DialogOpener Xелпер} открытия окна.
...
export default memo((props: IMarkSelectorPanelOptions): ReactElement => {
    ...
    const emptyTemplate = useMemo(() => {
        ...
        return (
            ...
            >
                <div className="controls-fontsize-4xl">
                    {
                        rk('Здесь пока нет пометок')
                    }
                </div>
                {
                    adding && (
                        rk('Добавляйте новые по кнопке \"+\"')
                    )
                }
            </div>
        );
