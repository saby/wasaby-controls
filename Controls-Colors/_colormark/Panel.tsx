...
export default memo((props: IMarkSelectorPanelOptions): ReactElement => {
    ...
    const resetButtonClickHandler = useCallback(() => {
        const disabledSelectedKeys = props.selectedKeys.filter((item) =>
            props.items.find((arrItem) => arrItem.id === item && arrItem.disabled)
        );
        const disabledPartialSelectedKeys = props.items
            .filter((arrItem) => arrItem.partialSelected && arrItem.disabled)
            .map((item) => item.id);
        props.onSelectedKeysChanged(disabledSelectedKeys);
        props.onExcludedKeysChanged?.(disabledSelectedKeys);
        props.onPartialSelectedKeysChanged?.(disabledPartialSelectedKeys);
            ...
            >
                <div className="tw-flex tw-items-baseline tw-justify-between">
                    ...
                    {(!!props.selectedKeys.length ||
                        !!props.excludedKeys?.length ||
                        !!props.partialSelectedKeys?.length) &&
                        props.items.some(
                            (item) =>
                                !item.disabled &&
                                (props.selectedKeys.some((key) => item.id === key) ||
                                    props.excludedKeys?.some?.((key) => item?.id === key) ||
                                    props.partialSelectedKeys?.some?.((key) => item?.id === key))
                        ) && (
                            <Button
                                className={
                                    'controls-margin_left-m' +
            ...
            <Container className="tw-h-full" data-qa="Controls-Colors_colormark_Panel__scroll">
                <div className="Colormark__Panel_content">
                    <List
                        ...
                        onPartialSelectedKeysChanged={props.onPartialSelectedKeysChanged}
                        partialSelectedKeys={props.partialSelectedKeys}
