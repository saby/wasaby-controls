/**
 * Файл содержащий список, отображающий массив пометок, а также компоненты необходимые
 * для корректной работы списка пометок.
 */
import {
    MutableRefObject,
    ReactElement,
...
/**
 * Компонент список, отображающий массив пометок.
 * @class Controls-Colors/colormark/List
 * @public
 */
export default memo((props: IListOptions): ReactElement => {
    ...
    const selectedKeysChangedHandler = useCallback(
        (
            ...
            if (addedKeys?.[0]) {
                clearPartialSelectedItems(addedKeys?.[0]);
                const item = itemsView.getRecordById(addedKeys?.[0]);
                const hasPartialSelectedKeys = !!props.partialSelectedKeys?.length;
                const isColoredMark = item.get('type') !== undefined;
                const anotherColoredMark = itemsView
                    .getRawData()
                    .filter(
                        (colorMarkItem) =>
                            colorMarkItem.partialSelected &&
                            colorMarkItem.id !== addedKeys?.[0] &&
                            (!!Object.keys(colorMarkItem.value)?.length ||
                                colorMarkItem.type !== undefined)
                    ); // проверка на наличие других цветных пометок с частичным выбором.
                const hasAnotherColoredMark = !!anotherColoredMark.length;
                if (hasPartialSelectedKeys && isColoredMark && hasAnotherColoredMark) {
                    keys = [
                        ...keys.filter(
                            (oldPartialSelectedKey) =>
                                itemsView.getRecordById(oldPartialSelectedKey).get('type') ===
                                undefined
                        ),
                        addedKeys?.[0],
                    ];
            ...
            props.onSelectedKeysChanged(Array.from(new Set(keys)), isItemClick);
        },
        [
            props.onBeforeSelectionChanged,
            props.onSelectedKeysChanged,
            itemsView,
            props.partialSelectedKeys,
        ]
    );
            ...
            if (isMultiSelectMode) {
                ...
                if (itemSelected) {
                    selectedKeysChangedHandler(
                        [...props.selectedKeys.filter((key) => key !== selectKey)],
                        [],
                        [selectKey],
                        true
                    );
    ...
    const changeItemsView = (
        res: Model<TMarkElement> | string,
        isAdd: boolean,
        commit: boolean
    ): void => {
        if (res && typeof res !== 'string') {
            if (isAdd) {
                ...
                const oldId = res.get('oldId');
                if (oldId) {
                    const oldIndex = itemsView.getIndexByValue('id', oldId);
                    if (oldIndex) {
                        itemsView.replace(res, oldIndex);
                    }
                    if (props.selectedKeys.includes(oldId)) {
                        changeOldIndex(props.selectedKeys, res, props.onSelectedKeysChanged);
                    }
                    if (props.partialSelectedKeys?.includes?.(oldId)) {
                        changeOldIndex(
                            props.partialSelectedKeys,
                            res,
                            props.onPartialSelectedKeysChanged
                        );
                    }
                } else {
                    const replacedIndex =
                        itemsView.getIndexByValue('id', res.get?.('id')) !== -1
    ...
    const changeOldIndex = (
        keys: (string | number)[],
        res: Model<TMarkElement>,
        setter?: Function
    ): void => {
        const newKeys = [...keys];
        const oldKeyIndex = keys.findIndex((key) => key === res.get('oldId'));
        newKeys[oldKeyIndex] = res.get('id');
        if (setter) {
            setter(newKeys);
        }
    };
    ...
    let offsetClassName = ' Colormark__List_container_offset';
    if (props.isAdaptive) {
        offsetClassName += '_adaptive';
    } else if (
        !props.footerTemplate &&
        (itemsView.at(itemsView.getCount() - 1)?.getRawData?.()?.type === 'style' ||
            props.addedItemType === 'style' ||
            !!props.changeTypeButtonVisibility)
    ) {
        offsetClassName += '_extended';
    } else {
        offsetClassName += '_default';
    }

    const clearPartialSelectedItems = (key: unknown) => {
        const item = itemsView.getRecordById(key);
        const hasPartialSelectedKeys = !!props.partialSelectedKeys?.length;
        const isColoredMark = item.get('type') !== undefined;
        const anotherColoredMark = itemsView
            .getRawData()
            .filter(
                (colorMarkItem) =>
                    colorMarkItem.partialSelected &&
                    colorMarkItem.id !== key &&
                    (!!Object.keys(colorMarkItem.value)?.length || colorMarkItem.type === 'color')
            ); // проверка на наличие других цветных пометок с частичным выбором.
        const hasAnotherColoredMark = !!anotherColoredMark.length;
        if (hasPartialSelectedKeys && hasAnotherColoredMark) {
            let newPartialSelectedKeys;
            if (isColoredMark) {
                newPartialSelectedKeys = props.partialSelectedKeys.filter(
                    (oldPartialSelectedKey) =>
                        itemsView.getRecordById(oldPartialSelectedKey).get('type') === undefined
                );
                itemsView.each((item) => {
                    if (
                        item.get('partialSelected') &&
                        newPartialSelectedKeys.includes(item.get('id') && !item.get('type'))
                    ) {
                        item.set('partialSelected', undefined);
                    }
                });
            } else {
                newPartialSelectedKeys = props.partialSelectedKeys.filter(
                    (oldPartialSelectedKey) => oldPartialSelectedKey !== key
                );
            }
            props.onPartialSelectedKeysChanged?.(newPartialSelectedKeys);
        }
    };
    ...
    const checkboxClickHandler = useCallback(
        (key: unknown) => {
            const item = itemsView.getRecordById(key);
            if (item.get('partialSelected')) {
                item.set('partialSelected', undefined);
                props.onPartialSelectedKeysChanged?.([
                    ...props.partialSelectedKeys.filter((item) => item !== key),
                ]);
            }
        },
        [props.partialSelectedKeys, props.onPartialSelectedKeysChanged]
    );

    const selectedKeys = useMemo(() => {
        return Array.from(new Set([...props.selectedKeys, ...(props.partialSelectedKeys || [])]));
    }, [props.selectedKeys, props.partialSelectedKeys]);

    const setCheckboxReadonly = useCallback(
        (key: string, value: boolean) => {
            const item = itemsView.getRecordById(key);
            item.set('сheckboxReadOnly', value);
        },
        [itemsView]
    );
            ...
            >
                <ItemsView
                    ...
                    multiSelectVisibility={
                        isMultiSelectMode
                            ? props.isAdaptive || detection.isMobilePlatform
                                ? 'visible'
                                : 'onhover'
                            : 'hidden'
                    }
                    ...
                    multiSelectTemplate={(props) =>
                        MultiSelectTemplateCallback(setCheckboxReadonly, props)
                    }
                        ...
                        (itemTemplateProps) => {
                            return (
                                <ItemTemplateList
                                    ...
                                    changeTypeButtonVisibility={props.changeTypeButtonVisibility}
                                />
                            );
                        },
                ...
                {props.footerTemplate && (
                    <div className="Colormark__List__footer">
                        <props.footerTemplate {...props.footerTemplateOptions} />
                    </div>
                )}
            </div>
...
/**
 * Компонент панели выбора цвета для пометок.
 * @param props
 * @constructor
 */
...
/**
 * Компонент, отображающий выбранный цвет и открывающий окно выбора цвета.
 * @param props
 * @constructor
 */
function Bubble(props) {
    ...
    const getStyle = useCallback(() => {
        if (props.item.get('type') === 'style') {
            return {
                border: 'calc(var(--border-thickness) * 2) solid ' + getColor(props.item),
            };
        }
        return {
    ...
    return (
        <div
            className={
                ...
                (props.item.get('type') === 'style' ? ' Colormark__List_Bubble__styled' : '') +
...
/**
 * Шаблон редактирования содержимого пометки.
 * @param editingTemplateProps
 * @param itemsPaletteView
 * @param itemActionsTemplate
 * @param multiSelectTemplate
 * @param editableStyle
 * @constructor
 */
function EditorTemplate({
    ...
    multiSelectTemplate,
    editableStyle,
    changeTypeButtonVisibility = false,
}) {
    ...
    const changeStyleMode = useCallback(() => {
        const isStyleColormark = item.get('type') === 'style';
        if (isStyleColormark) {
            item.set('value', {
                color: item.get('value').color,
                style: {
                    b: false,
                    u: false,
                    i: false,
        ...
        item.set('type', isStyleColormark ? 'color' : 'style');
    }, [item]);
    return (
        <EditingTemplate
            {...editingTemplateProps}
            ...
            editorTemplate={useCallback(() => {
                const getInputClassName = () => {
                    let result =
                        item.get('type') === 'color' ? 'Colormark__List_input' : 'tw-flex-grow ';
                    if (!changeTypeButtonVisibility && item.get('type') === 'color') {
                        result += ' Colormark__List_inputOffset';
                    }
                ...
                return (
                    <div
                        className={
                            ...
                            (changeTypeButtonVisibility ? 'style' : itemStyleMode)
                        }
                        data-qa={'Colormark__List_editorTemplate'}
                    >
                        <div
                            className={
                        ...
                        >
                            ...
                            {changeTypeButtonVisibility ? (
                                <Icon
                                    icon="icon-LetterA"
                                    iconSize="m"
                                    iconStyle="label"
                                    className="Colormark__List_editorTemplate__styledModeToggle"
                                    tooltip={rk('Сменить тип пометки')}
                                    onClick={changeStyleMode}
                                />
                            ) : item.get('type') === 'color' && CustomItemActionsTemplate ? (
                                <CustomItemActionsTemplate actionPadding={'colormark'} />
                            ) : null}
                        </div>
                        {((item.get('type') === 'color' && changeTypeButtonVisibility) ||
                            item.get('type') === 'style') && (
                            <div
                                className={
                            ...
                            >
                                {item.get('type') === 'style' ? (
                                    ...
                                    <div className="controls-inlineheight-m"></div>
                                ) : null}
                                {CustomItemActionsTemplate &&
                                (changeTypeButtonVisibility || item.get('type') === 'style') ? (
                                    <CustomItemActionsTemplate actionPadding={'colormark'} />
                                ) : null}
                            </div>
                        )}
...
/**
 * Утилита управляющая оторажением и состоянием чекбокса.
 * @param setCheckBoxReadOnly
 * @param multiSelectProps
 * @constructor
 */
function MultiSelectTemplateCallback(
    setCheckBoxReadOnly: (key: string, value: boolean) => void,
    multiSelectProps: object
) {
    const isEditing = useContext(EditingContext);
    const { item } = multiSelectProps;
    const isSelected = item.isSelected();
    const isEditedItem = item.isEditing();
    const itemId = item.contents.get('id');
    const isReadOnly = item.contents.get('сheckboxReadOnly');
    const isDisabled = item.contents.get('disabled');
    const isPartialSelected = item.contents.get('partialSelected');

    if ((!isSelected && isEditing && !isEditedItem) || isDisabled) {
        return null;
    }

    let checkboxValue: boolean | null = false;
    if (isSelected) {
        checkboxValue = isPartialSelected ? null : true;

        if (isEditing && !isEditedItem) {
            setCheckBoxReadOnly(itemId, false);
        }

        if (!isEditing && !isReadOnly) {
            setCheckBoxReadOnly(itemId, true);
        }
    }

    return <MultiSelectTemplate {...multiSelectProps} value={checkboxValue} />;
}

/**
 * Шаблон элемента списка, отображающий пометку.
 * @param editingTemplateProps
 * @param itemActionsTemplate
 * @param multiSelectTemplate
 * @param excludedKeys
...
/**
 * Шаблон редактируемой пометки.
 * @param contentTemplateProps
 * @param palette
 * @param excludedKeys
 * @param excludable
 * @param editableStyle
                ...
                (editingTemplateProps) => {
                    return (
                        <EditorTemplate
                            ...
                            changeTypeButtonVisibility={changeTypeButtonVisibility}
                        />
                    );
                },
...
/**
 * Шаблон элемента списка, отображающий 1 цветовую пометку с указанием списочных опций.
 * @param itemTemplateProps
 * @param palette
 * @param excludedKeys
 * @param excludable
 * @param editableStyle
...
/**
 * Шаблон компонента, позволяющего стилзовать содержимое пометки(задать жирность, подчеркнутость и т.д)
 * в режиме редактирования.
 * @param settings
 * @param onSettingsChanged
 * @param isAdaptive
 * @constructor
 */
