...
export default memo((props: IListOptions): ReactElement => {
    ...
    const clearPartialSelectedItems = (key: unknown) => {
        ...
        const isColoredMark = item.get('type') === 'color';
        const anotherColoredMark = itemsView
            ...
            .filter(
                (colorMarkItem) =>
                    ...
                    (colorMarkItem.value || colorMarkItem.type === 'color')
        ...
        if (hasPartialSelectedKeys && isColoredMark && hasAnotherColoredMark) {
            props.onPartialSelectedKeysChanged((prevPartialSelectedKeys) => {
                const newPartialSelectedKeys = prevPartialSelectedKeys.filter(
                    (oldPartialSelectedKey) => {
                        const partialSelectedItem = itemsView.getRecordById(oldPartialSelectedKey);
                        return !(
                            partialSelectedItem.get('value') ||
                            partialSelectedItem.get('type') === 'color'
                        );
                    }
                );
                return newPartialSelectedKeys;
            });
        }
    };
        ...
        (key: unknown) => {
            ...
            if (item.get('partialSelected')) {
                ...
                props.onPartialSelectedKeysChanged([
                    ...props.partialSelectedKeys.filter(
                        (partialSelectedKey) => partialSelectedKey !== key
                    ),
                ]);
            }
        },
    ...
    const selectedKeys = useMemo(() => {
        return [...props.selectedKeys, ...(props.partialSelectedKeys || [])];
        ...
        <EditingTemplate
            ...
            editorTemplate={useCallback(() => {
                const getInputClassName = () => {
                    let result =
                        item.get('type') === 'color'
                            ? 'Colormark__List_input Colormark__List_inputOffset'
                            : 'tw-flex-grow ';
