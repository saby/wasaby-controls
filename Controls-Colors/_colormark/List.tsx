        ...
        (editableItem: Model, commit: boolean, isAdd: true) => {
            if (props.onBeforeBeginEdit) {
                ...
                if (result instanceof Promise) {
                    result.then((res) => {
                        if (res && isAdd && typeof res !== 'string') {
                            itemsView.replace(res, itemsView.getCount() - 1);
                        }
                    });
                } else {
                    if (isAdd && typeof result !== 'string') {
                        itemsView.replace(result, itemsView.getCount() - 1);
                    }
                }
                return result;
            }
        ...
        <EditingContext.Provider value={isEditing}>
            <div
                className={
                    ...
                    (props.addedItemType === 'style' && itemsView.getCount() === 0)
