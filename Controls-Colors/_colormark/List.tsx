...
export default memo((props: IListOptions): ReactElement => {
    ...
    const [markedKey, setMarkedKey] = useState<string>(props.selectedKeys[0]);
    ...
    const itemClickHandler = useCallback((item: Model) => {
        ...
        if (isMultiSelectMode) {
            ...
            if (itemSelected) {
                const filteredKeys = props.selectedKeys.filter((key) => key !== selectKey);
                selectedKeysChangedHandler(filteredKeys, []);
            } else {
                selectedKeysChangedHandler([...props.selectedKeys, selectKey], [selectKey]);
            }
        } else {
            let keys = [];
    ...
    const afterEndEditHandler = useCallback((item) => {
        setIsEditing(false);
        if (props.onAfterEndEdit) {
            props.onAfterEndEdit({item});
        }
    }, [props.onAfterEndEdit]);

    const beforeBeginEditHandler = useCallback((item) => {
        setIsEditing(true);
        if (props.onBeforeBeginEdit) {
            props.onBeforeBeginEdit({item});
        }
