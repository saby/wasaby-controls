...
export default memo((props: IListOptions): ReactElement => {
    ...
    }, [props.onBeforeEdit, props.onBeforeDelete, itemsView]);
    ...
    return (
        <EditingContext.Provider value={isEditing}>
            <div className={'Colormark__List_container' + (isEditing ? '_blackout' : '') +
                            ' controls-padding_top-' + (props.isAdaptive ? '3xl' : 's') +
                            ' Colormark__List_container_offset' + (props.isAdaptive ? '_adaptive' : '_default') +
                            (props.className || '')
            }>
                <ItemsView ref={setRef}
                           ...
                           multiSelectVisibility={(isMultiSelectMode ? (props.isAdaptive ? 'visible' : 'onhover') : 'hidden')}
...
function Bubble(props) {
    ...
    return (
        <div className={'controls-margin_left-' + leftOffset +
                        ' controls-margin_right-' + (props.isAdaptive ? 'l' : 'm') +
    ...
    return (
        <EditingTemplate {...editingTemplateProps}
                         ...
                         editorTemplate={useCallback(() => {
                             ...
                             return (
                                 <div className={'Colormark__List_editorTemplate' + (editingTemplateProps.isAdaptive ? '_adaptive' : '') +
                                                 ' Colormark__List_editorTemplate_'  + itemStyleMode}
                                 >
                                     <div className={'Colormark__List_editorTemplate_wrapper tw-flex tw-items-center' +
                                     ...
                                     </div>
                                     {
                                         itemStyleMode === 'style' && (
                                             <div className={'Colormark__List_editorTemplate_styleSettings ' +
                                                 ...
                                                 <StyleSettings isAdaptive={editingTemplateProps.isAdaptive}
...
function ViewTemplate ({ editingTemplateProps, itemActionsTemplate, multiSelectTemplate }) {
    ...
    const getIconClasses = () => {
        return (
            ' controls-icon_size-' + ((isAdaptive ? 'l' : (item.get('iconSize') || 'm'))) +
    ...
    return (
        <div className={'Colormark__List_item tw-flex tw-items-center' +
                        ' controls-inlineheight-' + (isAdaptive ? 'xl' : 'm') +
                        ' controls-padding_top-' + (isAdaptive ? 'm' : '2xs') +
                        ' controls-padding_bottom-' + (isAdaptive ? 'm' : '2xs') +
                        ' controls-padding_left-' + (isAdaptive ? 'l' : 'null')
                       }
        >
            {
                (
                    ...
                    <span className={'Colormark__List_item_customCheckbox' + (isAdaptive ? '_adaptive' : '')}>
                        <MultiSelectTemplate/>
                    </span>
                )
            }
...
function ItemTemplateList({itemTemplateProps, palette}) {
    ...
    return (
        <ItemTemplate {...itemTemplateProps}
                      ...
                      markerClassName={'Colormark__List_markerPosition' + (itemTemplateProps.isAdaptive ? '_adaptive' : '_default')}
    ...
    return (
        <div className="Colormark__List_styleSettings_container">
            <div className="tw-flex">
                <div className={'controls-icon controls-icon_size-' + (isAdaptive ? 'l' : 'default') + ' icon-Bold' +
                    ' controls-margin_right-s controls-icon_style-' + (b ? 'secondary' : 'unaccented')
                }
                     onClick={() => styleButtonClickHandler('b')}
                     title={rk('Жирность')}
                ></div>
                <div className={'controls-icon controls-icon_size-' + (isAdaptive ? 'l' : 'default') + ' icon-Underline' +
                    ' controls-margin_right-s controls-icon_style-' + (u ? 'secondary' : 'unaccented')
                }
                     onClick={() => styleButtonClickHandler('u')}
                     title={rk('Подчеркнутый')}
                ></div>
                <div className={'controls-icon controls-icon_size-' + (isAdaptive ? 'l' : 'default') + ' icon-Italic' +
                    ' controls-margin_right-s controls-icon_style-' + (i ? 'secondary' : 'unaccented')
                }
                     onClick={() => styleButtonClickHandler('i')}
                     title={rk('Курсив')}
                ></div>
                <div className={'controls-icon controls-icon_size-' + (isAdaptive ? 'l' : 'default') + ' icon-Stroked' +
