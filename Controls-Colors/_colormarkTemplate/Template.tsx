...
export interface ITemplateProps extends IDialogTemplateOptions, IMarkSelectorPanelOptions {
    /**
     * Функция обратного вызова, которая будет вызвана при подтверждении выбора пометки.
     */
    onSendResult: (keys: string[], excludedKeys: string[], partialSelectedKeys: string[]) => void;
}

function BodyContentTemplate(props: ITemplateProps) {
    const { changeTypeButtonVisibility = false } = props;
    const [keys, setKeys] = useState<string[]>(props.selectedKeys as string[]);
    const [excludedKeys, setExcludedKeys] = useState<string[]>(props.excludedKeys as string[]);
    const [isKeysChanged, setIsKeysChanged] = useState<boolean>(false);
    const [partialSelectedKeys, setPartialSelectedKeys] = useState(() => {
        return props.items.filter((item) => item.partialSelected).map((item) => item.id);
    });
    ...
    const applyResult = useCallback(
        (newKeys, excludedKeys, partialSelectedKeys) => {
            if (props.onSendResult) {
                props.onSendResult(newKeys, excludedKeys, partialSelectedKeys);
                if (props.onClose) {
                    props.onClose();
                }
            }
        },
        [props.onSendResult, partialSelectedKeys]
    );

    const applyButtonClickHandler = useCallback(() => {
        applyResult(
            keys.filter((item) => !partialSelectedKeys.includes(item)),
            excludedKeys,
            partialSelectedKeys
        );
    }, [keys, excludedKeys, partialSelectedKeys]);
    ...
    const onSelectedKeysChangeHandler = useCallback(
        (newKeys, isItemClick) => {
            ...
            if (!isMultiSelectMode || isItemClick) {
                applyResult(newKeys, excludedKeys, partialSelectedKeys);
            }
        },
        [isMultiSelectMode, excludedKeys, partialSelectedKeys]
    );
