...
function BodyContentTemplate(props: ITemplateProps) {
    ...
    const applyResult = useCallback(
        (newKeys, excludedKeys) => {
            if (props.onSendResult) {
                props.onSendResult(newKeys, excludedKeys);
                if (props.onClose) {
                    props.onClose();
                }
            }
    ...
    const applyButtonClickHandler = useCallback(() => {
        applyResult(
            props.items.some((item) => item.partialSelected)
                ? {
                      keys: keys.filter((item) => !partialSelectedKeys.includes(item)),
                      partialSelectedKeys,
                  }
                : keys,
            excludedKeys
