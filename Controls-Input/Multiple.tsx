...
import { InputContainer, Controller } from 'Controls/validate';
...
export default forwardRef(function Multiple(props: IMultipleProps, ref: LegacyRef<HTMLDivElement>) {
    ...
    // Нужен для того, чтобы узнать текущее состояние валидации у полей.
    // Сейчас проще обернуть содержимое в validate:Controller, и работать с ним через ref,
    // Чем создавать новый validate:Container, который будет регистрировать в себе другие Container и запускать валидацию
    const validateRef = useRef<Controller>();
    const handleChangeValidationStatus = useCallback(() => {
        setValidationStatus(validateRef.current?.isValid() ? 'valid' : 'invalid');
    }, []);
 ...
 * @cfg {String} Определяет стиль уголка
 * @demo Controls-Input-demo/Multiple/TagStyle/Index
 */

/**
 * @name Controls-Input/Multiple#inputSettings
 * @cfg {Controls-Input/IInputSetting[]} Массив с настройкой, по которой будет строиться объединенное поле ввода.
 * @demo Controls-Input-demo/Multiple/ValidationStatus/Index
 */
