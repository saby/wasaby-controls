...
function Button(props: IButtonProps & IControlProps & IButtonStyle & IStyle) {
    ...
    useEffect(() => {
        // в сафари есть проблема с открытием ссылок. Переход по ллылке не будет осуществлен, если что-то подгружается.
    ...
    if (props.action?.actionProps?.hotkey) {
        return (
            <KeyHook defaultActions={defaultActions} context="global">
                <Content />
            </KeyHook>
        );
    }
