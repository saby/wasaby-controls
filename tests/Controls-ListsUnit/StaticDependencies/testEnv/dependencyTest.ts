...
export default function dependencyTest(
    ...
    neededLibs: string[],
    unneededLibs: string[],
    customName: string = typeof target === 'string' ? `${target}.` : ''
): void {
    it('Тест статических зависимостей. ' + customName, async () => {
        const needed = [...neededLibs];
        ...
        try {
            if (typeof target === 'string') {
                await ModulesLoader.loadAsync(target);
            } else {
                await target();
            }
        } catch (error) {
