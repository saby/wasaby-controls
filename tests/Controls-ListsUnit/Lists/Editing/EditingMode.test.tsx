...
describe('Controls-ListsUnit/Lists/Editing/EditingMode', () => {
    ...
    it('by default should edit whole row', async () => {
        ...
        await waitFor(() => {
            ...
            const editingCells = screen.getAllByTestId('editing_component-editor_render');
            expect(editingCells.length).toBe(3);
        });
    });

    it('should edit whole row if mode="row"', async () => {
        const { getFirstItem } = renderGrid(<EditingDemo editingMode={'row'} />, { container });
    ...
    it('should edit one cell if mode="cell"', async () => {
        const { getFirstItem } = renderGrid(<EditingDemo editingMode={'cell'} />, { container });
    ...
    it('should rerender if change editing mode', async () => {
        ...
        await userEvent.selectOptions(screen.getByTestId('mode-selector'), 'row');
        await waitFor(async () => {
            const item = getFirstItem();
            await clickCell(item, 1);
        });
