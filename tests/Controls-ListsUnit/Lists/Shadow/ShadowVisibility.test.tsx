...
describe('Controls-ListsUnit/Lists/Shadow/ShadowVisibility', () => {
    ...
    it('should not add classes if default value', () => {
        ...
        expect(column.matches('.controls-ListView__item_shadow_visible')).toBeFalsy();
        expect(column.matches('.controls-ListView__item_shadow_onhover')).toBeFalsy();
    });

    it('should not add classes if "hidden" value', () => {
        const { getFirstItem } = renderGrid(<ShadowDemo shadowVisibility={'hidden'} />, {
            container,
        });
    ...
    it('should add classes if "onhover" value', () => {
        const { getFirstItem } = renderGrid(<ShadowDemo shadowVisibility={'onhover'} />, {
            container,
        });

        const item = getFirstItem();
        const column = getCell(item, 0);
        expect(column.matches('.controls-ListView__item_shadow_onhover')).toBeTruthy();
    });

    it('should add classes if "visible" value', () => {
        const { getFirstItem } = renderGrid(<ShadowDemo shadowVisibility={'visible'} />, {
            container,
        });

        const item = getFirstItem();
        const column = getCell(item, 0);
        expect(column.matches('.controls-ListView__item_shadow_visible')).toBeTruthy();
    });
