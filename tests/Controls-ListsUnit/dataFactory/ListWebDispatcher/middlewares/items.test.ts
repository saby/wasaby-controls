...
import { ChangeAction } from 'Controls-DataEnv/abstractList';
import { TListAction } from 'Controls/dataFactory';
import { RecordSet } from 'Types/collection';
import { Model } from 'Types/entity';
import { clone } from 'Types/object';
import type { CrudEntityKey } from 'Types/source';

const { items: itemsActions } = ListActionCreators;
...
function getMiddleware(initState: Partial<IListState> = {}): {
    state: {
        current: Partial<IListState>;
    };
    ...
    const middleware = itemsMiddleware({
        dispatch: async (action: TListAction) => {
            dispatchedActions.push(action);
        },
        getState: () => state.current as IListState,
        snapshots: new Map(),
        applyState: (s: Partial<IListState>) => {
            state.current = {
                ...state.current,
                ...s,
            };
        },
        setState: (s: Partial<IListState>) => {
            state.current = {
                ...state.current,
                ...s,
            };
    ...
    } as unknown as TListMiddlewareContext)(next);
...
function getDispatchedActionByType<
    TActions extends TListActions.TAnyListAction[],
