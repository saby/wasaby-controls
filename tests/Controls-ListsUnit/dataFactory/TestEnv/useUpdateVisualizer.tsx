...
export function useUpdateVisualizer<T extends keyof IListState>(props: {
    ...
    assertBeforeBas?: (prevState: T, nextState: T) => void;
    assertAfterBas?: (prevState: T, nextState: T) => void;
}) {
    const { Slice } = props;
    ...
    return {
        Slice: class extends Slice {
            ...
            protected async _beforeApplyState(draftStateProp: T): Promise<T> {
        ...
        setSyncs((prevSyncs) => {
            const resolveChanges = (keys, initValue) =>
                keys.reduce(
                    (acc, propertyName) => {
                        const newValue =
                            props?.replacer?.(propertyName, slice.state) ??
                            slice.state[propertyName];
                        const prevValue =
