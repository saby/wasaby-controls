import { IRange, Quantum, Utils } from 'Controls-Lists/timelineGrid';
...
describe('Controls-ListsUnit/TimelineGrid/utils', () => {
    describe('getQuantum', () => {
        ...
        test('halfHour, quantsReplacementMap', () => {
            const expectedResult = 'halfHour';
            const range: IRange = {
                start: new Date(2023, 5, 15, 0, 0, 0, 0),
                end: new Date(2023, 5, 15, 23, 0, 0, 0),
            };

            const result = Utils.getQuantum(range, {[Quantum.Hour]: Quantum.HalfHour});
            expect(result).toEqual(expectedResult);
        });
