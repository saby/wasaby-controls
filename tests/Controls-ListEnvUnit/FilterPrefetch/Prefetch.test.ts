import {Prefetch, IPrefetchHistoryParams} from 'Controls-ListEnv/filterPrefetch';
...
describe('Controls.filter.Prefetch', () => {
      it('applyPrefetchFromItems', () => {
         let filter = {};
         expect(
             Prefetch.applyPrefetchFromItems(filter, getRecordSetWithPrefetch())
         ).toEqual({ PrefetchSessionId: 'test' });

         filter = {};
         expect(
             Prefetch.applyPrefetchFromItems(filter, getRecordSetWithoutPrefetch())
         ).toEqual({});
      });
      ...
      it('applyPrefetchFromHistory', () => {
         const filter = {};
         expect(
             Prefetch.applyPrefetchFromHistory(filter, getHistoryWithPrefetch())
         ).toEqual({ PrefetchSessionId: 'test' });
      });
      ...
      it('prepareFilter', () => {
         ...
         expect(
             Prefetch.prepareFilter({}, prefetchOptions, 'testPrefetchSessionId')
         ).toEqual({
            PrefetchMethod: 'testMethodName',
...
describe('Controls/filter:Controller prefetch', () => {
      it('loadFilterItemsFromHistory with prefetch', () => {
         ...
         jest
            .spyOn(filterController, '_loadHistoryItems')
            .mockClear()
            .mockImplementation(() => {
               itemsLoaded = true;
         ...
         jest
            .spyOn(filterController, '_findItemInHistory')
            .mockClear()
            .mockImplementation(() => {
               return null;
            });
