...
describe('Controls-ListEnv/_filterPanelConnected:ViewModeController', () => {
    ...
    describe('.updateFilterPanelSource()', () => {
        it('should change view mode to "cloud"', () => {
            const newFilterItems = getExtendedFilterItems();
            controller.updateFilterPanelSource(
                newFilterItems,
                extendedFilterNames,
                null,
                'cloud|default'
        ...
        it('should return items filtered for view mode /"cloud/"', () => {
            const newFilterItems = getExtendedFilterItems();
            controller.updateFilterPanelSource(newFilterItems, ['country'], null, 'cloud|default');
            const innerFilterItems = controller.getFilterPanelSource();
            expect(controller.getViewMode()).toEqual('cloud');
        ...
        it('Применение фильтра из панели фильтров, viewMode не должен меняться на cloud', () => {
            const filterPanelItems = filterItems.map((filterItem) => {
                return {
                    ...filterItem,
                    value: 'test',
            ...
            const changedFilterPanelItems = filterItems.map((filterItem) => {
                return {
                    ...filterItem,
                    value: 'newValue',
                    appliedFrom: 'filterPanel',
            ...
            controller.setFilterPanelSource(filterPanelItems, 'cloud|default', filterNames, '');

            controller.updateFilterPanelSource(
                changedFilterPanelItems,
                filterNames,
                '',
                'cloud|default'
            );
        ...
        it('Применение фильтра из панели фильтров c viewMode: cloud, viewMode не должен меняться на defalut', () => {
            const filterPanelItems = filterItems.map((filterItem) => {
                return {
                    ...filterItem,
                    value: 'test',
                    appliedFrom: 'filterPopup',
                };
            });
