...
import { IHistoryItem, THistoryId } from 'Controls/HistoryStore';

describe('Controls/history:Store', (): void => {
    const localStorage = new LocalStorage();
    const historyId: THistoryId = 'myHistoryTest';
    ...
    const recentItems: RecordSet<IHistoryItem> = new RecordSet({
        keyProperty: 'ObjectId',
        rawData: recentItemsRaw,
    });

    const pinnedItems: RecordSet<IHistoryItem> = new RecordSet({
        keyProperty: 'ObjectId',
        rawData: pinnedItemsRaw,
    });

    const getLocalStorageRecentItems = (): RecordSet<IHistoryItem> | null => {
        return Store.getLocal(historyId).recent;
    };

    const getLocalStoragePinnedItems = (): RecordSet<IHistoryItem> | null => {
    ...
    describe('push', () => {
        it('push in not empty history', async () => {
            ...
            expect(getLocalStorageRecentItems()?.getCount()).toEqual(2);
            expect(getLocalStorageRecentItems()?.at(1).get('ObjectData')).toEqual('pushedValue');
        });
    ...
    describe('update', () => {
        ...
        it('update by not exist historyId', async () => {
            await expect(Store.update('notExistedHistoryId', 1, 'updatedValue1')).rejects.toEqual(
                'Элемента с id: 1 не существует в истории с historyId: notExistedHistoryId'
            );
        });

        it('update by not exist item id', async () => {
            await expect(Store.update(historyId, 999, 'updatedValue1')).rejects.toEqual(
    ...
    describe('delete', (): void => {
        ...
        it('delete by not exist historyId', async () => {
            await expect(Store.delete('notExistedHistoryId', 1)).rejects.toEqual(
                'Элемента с id: 1 не существует в истории с historyId: notExistedHistoryId'
            );
        });

        it('delete by not exist item id', async () => {
            await expect(Store.delete(historyId, 999)).rejects.toEqual(
                'Элемента с id: 999 не существует в истории с historyId: myHistoryTest'
            );
        });
    });
