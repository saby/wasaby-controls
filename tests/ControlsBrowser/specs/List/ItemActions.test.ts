...
describe('Controls/list:View - ItemActions', () => {
    ...
    it('клик по операциям над записью обрабатывается', async () => {
        ...
        await openPage('Controls-demo/list_new/ItemActions/ItemActionClickHandler/Index');
        ...
        await expect(menu.container()).not.toExist();
        await expect(page.result()).toHaveText('Кликнули на операцию "Email" у элемента с id="3".');
