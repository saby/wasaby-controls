...
describe('Controls/list:View', () => {
    ...
    it('маркер не теряет позицию при перезагрузке списка', async () => {
        ...
        await expect(markerOnReloadPage.reloadResultText()).toHaveText('list reloaded');
        await expect(listView.marker(appleGadgetsSelector)).toBeDisplayed();
    });
    ...
    it('записи должны отмечаться с помощью пробела', async () => {
        ...
        await openPage('Controls-demo/list_new/Marker/OnBeforeMarkedKeyChanged/Index');
    ...
    it('возникает ромашка при медленном добавлении', async () => {
        ...
        await expect(listView.editor()).toHaveValueContaining('после задержки в 3 сек');
    ...
    it('записи должны отмечаться с помощью пробела', async () => {
        ...
        await openPage('Controls-demo/list_new/MultiSelect/MultiSelectVisibility/Visible/Index');
    ...
    it('записи должны отмечаться с помощью пробела', async () => {
        ...
        await clickWithModifier(listView.checkbox(androidGadgetsSelector), 'Shift');
    ...
    it('в режиме readonly записи не должны отмечаться', async () => {
        ...
        await openPage('Controls-demo/list_new/ItemTemplate/CheckboxReadOnly/Index');
    ...
    it('Работают события активации записи при клике и переходам горячими клавишами', async () => {
        ...
        eventsLogPanelText = 'Выполнили клик по key: 0; активировали item-key: 0';
        await expect(itemClickBasePage.eventsLogPanel()).toHaveText(eventsLogPanelText);
        ...
        eventsLogPanelText = 'Выполнили клик по key: 1; активировали item-key: 1';
        await expect(itemClickBasePage.eventsLogPanel()).toHaveText(eventsLogPanelText);
        await expect(listView.marker({ index: 2 })).toBeDisplayed();
    });
});
