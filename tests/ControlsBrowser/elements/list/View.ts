...
export default class ListView extends BaseElement {
    ...
    editor(): ReturnType<WebdriverIO.Browser['$']> {
        return this.container().$('.controls-EditingTemplateText__editor input');
    }
    ...
    async selectItemAction(selector: ItemSelector, actionSelector: ActionSelector): Promise<void> {
        const item = await this.item(selector);
        await item.moveTo();
    ...
    async checkCheckboxState(selector: ItemSelector, expectedState: boolean | null): Promise<void> {
        let expectedClass;
        if (expectedState) {
