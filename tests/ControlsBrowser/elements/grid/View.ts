...
export default class GridView extends ListView {
    ...
    cellEditor(
        ...
        return this.cell(rowIndex, cellIndex).$(
    ...
    async editCell(
        ...
        if (clearValue) {
            ...
            await this.cellEditor(rowIndex, cellIndex).setValue([
                ' ',
                'Backspace',
            ]);
        }
        ...
        if (commit) {
            if (applyButton) {
                ...
                await this.cellEditor(rowIndex, cellIndex).addValue(
                    UNICODE_CHARACTERS.Enter
                );
            }
        }
    }
