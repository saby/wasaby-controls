...
describe('Controls-Lists-editors/columnsEditor/resizeHandler', function () {
    ...
    test('increase width by 10000 px, when its represented with px in grid template', function () {
        expect(
            resizeHandler({ columnHTMLWidth: 100, columnTemplateWidth: '100px', offset: 10000 })
        ).toEqual('9999px');
    });
    // % UNITS
    ...
    test('increase width by 10000 px, when its represented with % in grid template', function () {
        expect(
            resizeHandler({ columnHTMLWidth: 100, columnTemplateWidth: '5%', offset: 10000 })
        ).toEqual('500%');
    });
    // AUTO WIDTH TESTS
    // MAX-CONTENT
    test('increase width by 50 px, when its represented with max-content in grid template', function () {
        ...
        ).toEqual('minmax(150px,max-content)');
    });
