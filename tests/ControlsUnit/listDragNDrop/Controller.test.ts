...
describe('Controls/_listDragNDrop/Controller', () => {
    ...
    describe('startDrag', () => {
        it('not pass draggedItem', () => {
            ...
            let controller = new DndController(model, undefined, FlatStrategy);

            expect(() => {
                controller.startDrag(new ItemsEntity({ items: [1] }));
            }).toThrow();

            expect(modelSetDraggedItemsSpy).not.toHaveBeenCalled();
    ...
    it('endDrag', () => {
        ...
        expect(controller.getDragPosition()).toBe(undefined);
        expect(controller.getDragEntity()).toBe(undefined);
    });
