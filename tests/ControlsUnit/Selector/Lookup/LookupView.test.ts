...
describe('Controls/_lookup/Lookup/LookupView', () => {
    ...
    it('getAvailableCollectionWidth', () => {
        ...
        expect(
            lookup._getAvailableCollectionWidth(
                afterFieldWrapperWidth,
                false,
                false
            )
        ).toBe(80);
    ...
    it('getInputWidth', () => {
        const lookup = new Lookup();
        expect(
            lookup._getInputWidth(400, 200, 100, 48, true, 0, true)
        ).toBeUndefined();
        expect(lookup._getInputWidth(400, 200, 300, 48, true, 0, true)).toBe(
            152
        );
    });
    ...
    it('_calculatingSizes', () => {
        ...
        lookup._getItemsSizesLastRow = () => {
            ...
            if (newOptions.multiLine) {
                ...
                numberItems =
                    numberItems % MAX_ITEMS_IN_ONE_ROW || MAX_ITEMS_IN_ONE_ROW;
            }
    ...
    it('getCollectionOptions', () => {
        ...
        expect(
            lookup._getCollectionOptions(controlOptions, 10, '10px')
        ).toEqual(standardOptions);

        delete controlOptions.itemTemplate;
        expect(
            lookup._getCollectionOptions(controlOptions, 10, '10px')
        ).not.toHaveProperty('itemTemplate');
    });
});
