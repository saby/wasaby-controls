define([
   ...
   describe('Controls/_lookupPopup/Container', function () {
      ...
      it('_private::getInitialSelectedItems', () => {
         ...
         expect(
            LookupPopupContainer._private
               .getInitialSelectedItems(self, options)
               .getCount()
         ).toEqual(1);
      });
      ...
      it('getFilteredItems', function () {
         ...
         expect(
            LookupPopupContainer._private.getFilteredItems(items, retTrue)
         ).toEqual(items);
         expect(
            LookupPopupContainer._private.getFilteredItems(items, filterFunc)
         ).toEqual(['toSaveItem', 'toSaveItem']);
      });

      it('getKeysByItems', function () {
         expect(
            LookupPopupContainer._private.getKeysByItems(getItems(), 'id')
         ).toEqual([0, 1, 2, 3, 4]);
      });
      ...
      it('getEmptyItems', function () {
         ...
         expect(
            LookupPopupContainer._private
               .getEmptyItems(listWithItems)
               .getCount()
         ).toEqual(0);
         expect(
            LookupPopupContainer._private.getEmptyItems(listWithItems)
               ._moduleName
         ).toEqual('Types/collection:List');
      });

      it('getValidSelectionType', function () {
         expect(
            LookupPopupContainer._private.getValidSelectionType('all')
         ).toEqual('all');
         expect(
            LookupPopupContainer._private.getValidSelectionType('leaf')
         ).toEqual('leaf');
         expect(
            LookupPopupContainer._private.getValidSelectionType('node')
         ).toEqual('node');
         expect(
            LookupPopupContainer._private.getValidSelectionType('test')
         ).toEqual('all');
      });
      ...
      it('getFilterFunction', function () {
         ...
         expect(
            LookupPopupContainer._private.getFilterFunction(retFalse)()
         ).toBe(false);
      });
      ...
      it('getSelectedKeys', function () {
         ...
         expect(LookupPopupContainer._private.getSelectedKeys(options)).toEqual(
            ['testId', 2, 4]
         );
      ...
      describe('prepareFilter', () => {
         ...
         it('searchParam and parentProperty are deleted from filter on select', () => {
            ...
            expect(preparedFilter.selection.get('excluded')).toEqual([
               '3',
               '4'
            ]);
      ...
      it('getCrudWrapper', function () {
         ...
         expect([
            'Controls/dataSource:CrudWrapper',
            'Controls/_dataSource/CrudWrapper'
         ]).toContain(crudWrapper._moduleName);
      });
      ...
      it('_private::prepareRecursiveSelection', () => {
         ...
         let preparedSelection =
            LookupPopupContainer._private.prepareNotRecursiveSelection(
               selection,
               items,
               'id',
               'parent',
      ...
      describe('_selectComplete', function () {
         ...
         it('selected keys is empty', async function () {
            ...
            expect(result.resultSelection.isEqual(clearRecordSet)).toEqual(
               true
            );
         });
         ...
         it('single select', async function () {
            ...
            expect(result.resultSelection.at(0).getRawData()).toEqual(
               getItems()[1].getRawData()
            );
         ...
         it('query returns error', async function () {
            ...
            container._options.multiSelect = true;
            await expect(container._selectComplete()).rejects.toThrow(
               'testError'
            );
