define([
   ...
   const LookupPopupContainer = LookupContainer.default;
   ...
   describe('Controls/_lookupPopup/Container', function () {
      ...
      it('getKeysByItems', function () {
         expect(LookupContainer.getKeysByItems(getItems(), 'id')).toEqual([0, 1, 2, 3, 4]);
      });

      it('getEmptyItems', function () {
         var listWithItems = new collection.List({ items: getItems() });

         expect(Utils.default.getEmptyItems(listWithItems).getCount()).toEqual(0);
         expect(Utils.default.getEmptyItems(listWithItems)._moduleName).toEqual(
      ...
      describe('prepareFilter', () => {
         ...
         it('searchParam and parentProperty are deleted from filter on select', () => {
            ...
            const preparedFilter = Utils.default.prepareFilter({
               filter,
               selection,
         ...
         it('searchParam not deleted from filter on select all in hierarchy list', () => {
            ...
            let preparedFilter = Utils.default.prepareFilter({
               filter,
               selection,
               searchParam: 'searchParam',
               root: 'testRoot'
            ...
            preparedFilter = Utils.default.prepareFilter({
               filter,
               selection,
               searchParam: 'searchParam',
               root: 1
      ...
      it('_private::prepareRecursiveSelection', () => {
         ...
         let preparedSelection = Utils.default.prepareNotRecursiveSelection(
            selection,
            items,
            'id',
            'parent',
      ...
      it('_private::getSelection', () => {
         ...
         let selectionRecord = Utils.default.getSelection(selection, adapter, selectionType, false);
         ...
         selectionType = 'node';
         selectionRecord = Utils.default.getSelection(selection, adapter, selectionType, true);
         expect(selectionRecord.getRawData()).toEqual({
            marked: ['1', '2', '3'],
            excluded: ['1'],
            type: 'node',
      ...
      describe('_selectComplete', function () {
         ...
         it('query returns error', async function () {
            ...
            container._options.dataContextCompatibleValue = {
               source: source,
