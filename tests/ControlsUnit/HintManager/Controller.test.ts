...
describe('Controls/_hintManager/Controller', () => {
    ...
    describe('updateScheme', () => {
        it('scheme is updated correctly', () => {
            const stepsCount = 2;
            const route = getRoute(stepsCount);
            controllerInstance = new Controller(route);

            const newStepsCount = 3;
            const updatedScheme = getScheme(newStepsCount);
            controllerInstance.updateScheme(updatedScheme);

            expect(route.get('scheme').getCount()).toBe(newStepsCount);
            expect(route.get('scheme').at(newStepsCount - 1).get('message')).toBe(`Шаг ${newStepsCount}`);
        });
    });
});
