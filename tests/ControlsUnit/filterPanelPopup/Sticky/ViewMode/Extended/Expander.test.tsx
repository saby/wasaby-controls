 ...
 */
jest.mock('Controls/_scroll/Container', () => {
    return {
        __esModule: true,
        default(props) {
...
describe('Controls/filterPanelPopup:Sticky отображение кнопки-экпандера в блоке дополнительных фильтров', () => {
    ...
    it(
        ...
        async () => {
            const filterDescription = getExtendedItems(countOfFilterItemsForExpanderVisible);
            render(<Sticky items={filterDescription} historyId={'testHistoryId'} />, { container });
            expect(!!screen.getByTestId('FilterViewPanel-additional__more')).toBeTruthy();
            expect(screen.getAllByTestId('FilterViewPanel__additional-editor').length).toBe(
    ...
    it('Клик по кнопке разворота показывает все редакторы', async () => {
        ...
        await userEvent.click(screen.getByTestId('FilterViewPanel-additional__more'));
    ...
    it('Выбор фильтров из дополнительного блока скрывает кнопку разворота', async () => {
        ...
        await Promise.all([
            userEvent.click(screen.getByText('extendedFilterCaption-11')),
            userEvent.click(screen.getByText('extendedFilterCaption-10')),
            userEvent.click(screen.getByText('extendedFilterCaption-0')),
            userEvent.click(screen.getByText('extendedFilterCaption-1')),
        ]);
