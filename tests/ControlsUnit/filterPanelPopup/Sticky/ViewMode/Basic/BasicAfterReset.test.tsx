...
describe('Controls/filterPanelPopup:Sticky сброс значений для редакторов с viewMode: basic', () => {
    ...
    it('После сброса параметры с viewMode: basic должны остаться в основном блоке фильтров', async () => {
        ...
        const resetButton = await screen.getByTestId(
            'controls-FilterPanelPopup__reset-button'
        );
        ...
        await waitFor(() => {
            expect(
                !!screen.getByTestId('controls-FilterPanelPopup__applyButton')
            ).toBeTruthy();
        });
        ...
        await waitFor(() => {
            expect(
                !!screen.getByTestId('controls-FilterViewPanel__basic-editors')
            ).toBeTruthy();
        });
    });
});
