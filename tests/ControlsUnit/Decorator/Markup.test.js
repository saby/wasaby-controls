...
define([
   ...
   'UI/Base',
   'Core/validHtml',
   'Controls/_markup/Markup/resources/modifyJsonRecursively'
], function (
   decorator,
   template,
   linkDecorateUtils,
                     ...
                     [
                        ...
                        [
                           ...
                           [
                              'img',
                              {
                                 src: 'data:image/png;charset=windows-1251;base64,iVBORw0KGgoAAAANS'
                              }
                           ]
                        ],
   ...
   describe('modifyJsonRecursively', () => {
      it('Массив json элементов', () => {
         const json = [
            ['div', ['p', 'https://ya.ru/']],
            ['p', 'https://ya.ru/'],
            ['p', 'текст']
         ...
         const resultJson = modifyJsonRecursively.default(json, decorator.linkWrapResolver);
         expect(resultJson).toStrictEqual(expectedJson);
      });

      it('Один json элемент, tagResolver не применился', () => {
         const json = ['p', 'текст'];
         const resultJson = modifyJsonRecursively.default(json, decorator.linkWrapResolver);
         expect(resultJson).toStrictEqual(json);
      });

      it('Один json элемент, tagResolver применился', () => {
         const json = ['div', ['p', 'https://ya.ru/']];
         const expectedJson = [
            'div',
            [
               'p',
   ...
   describe('linkWrapResolver', () => {
      it('1) Ссылка и текст с точкой', () => {
         const text = 'https://online.sbis.ru/ Яндекс.еда';
         const parent = ['p', text];
         const resultJson = decorator.linkWrapResolver(text, parent, {
            needParseFolderLinks: true
         });
         ...
         expect(resultJson).toStrictEqual(expectedResult);
      });

      it('2) Ссылка и текст с точкой', () => {
         const text = 'https://online.sbis.ru/ /home/sbis/workspace/EventExtFunc.cpp:1708';
         const parent = ['p', text];
         const resultJson = decorator.linkWrapResolver(text, parent, {
            needParseFolderLinks: true
         });
         ...
         const expectedResult = [
            ...
            ' /home/sbis/workspace/EventExtFunc.cpp:1708'
         ];
