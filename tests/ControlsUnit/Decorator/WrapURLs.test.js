   ...
   describe('Controls.extendedDecorator.WrapURLs', function () {
      describe('parseText', function () {
         ...
         it('Simple URL with params', function () {
            expect(ctrl.parseText('http://regexpal.com/home.php?request=q&theme=2')).toEqual([
               {
                  type: 'link',
         ...
         it("Symbol ' inside link", function () {
            expect(
               ctrl.parseText("https://wiki.postgresql.org/wiki/What's_new_in_PostgreSQL_9.5")
            ).toEqual([
               {
                  type: 'link',
         ...
         it('Link with anchor', function () {
            expect(
               ctrl.parseText('http://axure.tensor.ru/ereport/#p=реестр_по_приложению_№4')
            ).toEqual([
               {
                  type: 'link',
         ...
         it('Space after protocol before www', function () {
            expect(ctrl.parseText('https:// www.youtube.com/watch?v=_avffmEHKf8')).toEqual([
               {
                  type: 'plain',
                  value: 'https:// '
               },
         ...
         it('Url in double delimiters', function () {
            expect(ctrl.parseText('(https://pre-test-online.sbis.ru/auth/?ret=%2F)')).toEqual([
               {
                  type: 'plain',
                  value: '('
               },
         ...
         it('Url in double delimiters in the middle of the text', function () {
            expect(
               ctrl.parseText('test test (https://pre-test-online.sbis.ru/auth/?ret=%2F) test test')
            ).toEqual([
               {
                  type: 'plain',
                  value: 'test test ('
