...
describe('Controls/_search/Input/Container', () => {
    it('_beforeMount', () => {
        ...
        expect(cont._getSearchResolverController().isSearchStarted()).toBeTruthy();
    });
    ...
    describe('_resolve', () => {
        it('search', () => {
            ...
            const stub = jest.spyOn(cont, '_notify').mockClear().mockImplementation();
            const dispatchStub = jest.spyOn(Store, 'dispatch').mockClear().mockImplementation();

            cont._notifySearch('test');
            expect(stub).toHaveBeenCalledWith('search', ['test'], expect.anything());
        ...
        it('searchReset', () => {
            ...
            expect(stub).toHaveBeenCalledWith('searchReset', [''], expect.anything());
            expect(dispatchStub).not.toHaveBeenCalled();
        });
    });
    ...
    describe('_valueChanged', () => {
        ...
        it('_beforeMount with inputSearchValue, then valueChanged', () => {
            ...
            expect(stubNotify).not.toHaveBeenCalledWith('search', ['testValue2'], {
                bubbling: true,
            });
