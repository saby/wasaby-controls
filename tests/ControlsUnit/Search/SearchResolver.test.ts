...
describe('Controls/search:SearchDelay', () => {
    ...
    describe('setSearchStarted', () => {
        it('Вызов метода setSearchStarted сбрасывает задержку поиска', () => {
            const { searchResolver } = initSearchDelay({
                minSearchLength: 3,
                searchDelay: 1000,
            });
            searchResolver.resolve('test');
            expect(searchResolver.isDelayTimerStarted());

            searchResolver.setSearchStarted(true);
            expect(searchResolver.isDelayTimerStarted()).toBeFalsy();
        });
    });
});
