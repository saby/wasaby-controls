import { ControllerClass } from 'Controls/searchDeprecated';
import { NewSourceController, NewSourceController as SourceController } from 'Controls/dataSource';
import { Memory, QueryWhereExpression, DataSet } from 'Types/source';
import { IControllerOptions } from 'Controls/_dataSource/Controller';
import { RecordSet } from 'Types/collection';
import { ISearchControllerOptions } from 'Controls/_searchDeprecated/ControllerClass';
...
describe('Controls/search:ControllerClass', () => {
    ...
    it('searchValue in constructor', () => {
        ...
        expect(searchControllerClass.getSearchValue() === 'testSearchValue').toBeTruthy();
    ...
    it('search method', () => {
        ...
        expect(loadSpy).toHaveBeenCalledWith(undefined, undefined, filter, undefined, undefined);
    });

    it('load canceled while searching', async () => {
        const filter = sourceController.getFilter();
        const prom = expect(controllerClass.search('testValue')).rejects.toThrow('Unknown reason');
    ...
    it('needChangeSearchValueToSwitchedString', () => {
        ...
        expect(controllerClass.needChangeSearchValueToSwitchedString(rs)).toBeTruthy();
    });
    ...
    describe('with hierarchy', () => {
        ...
        describe('startingWith: root', () => {
            ...
            it('root before search should saved after reset search', async () => {
                ...
                expect(sourceController.getFilter().parentProperty === null).toBeTruthy();
    ...
    describe('update', () => {
        it("shouldn't call when searchValue is null", () => {
            const searchStub = jest
                ...
                .mockClear()
                .mockImplementation();
            const resetStub = jest.spyOn(controllerClass, 'reset').mockClear().mockImplementation();
        ...
        it('update with sourceController', async () => {
            ...
            expect(!searchController.update(searchControllerOptions)).toBeTruthy();
    ...
    it('search with filterOnSearchCallback option', async () => {
        ...
        expect(sourceController.getItems().at(0).get('title') === 'test').toBeTruthy();
    ...
    describe('search', () => {
        ...
        it('search with root and startingWith: "root"', async () => {
            ...
            await searchController.search('testSearchValue');
            expect(sourceController.getFilter().searchStartedFromRoot === 'testRoot').toBeTruthy();
        });
        ...
        it('search executed while another search in progress', async () => {
            ...
            source.query = () => {
                ...
                return new Promise((resolve) => {
                    setTimeout(() => resolve(new DataSet()), timeout);
                });
            };
            sourceController = getSourceController({ source });
            const searchController = getSearchController({ sourceController });
