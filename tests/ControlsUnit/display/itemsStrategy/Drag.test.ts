...
import { Collection, Collection as CollectionDisplay, CollectionItem } from 'Controls/display';
...
describe('Controls/_display/itemsStrategy/Drag', () => {
    ...
    describe('.getDisplayIndex', () => {
        ...
        it('drag some items separately', () => {
            display = new Tree({
                collection: new RecordSet({
                    rawData: [{ id: 1 }, { id: 2 }, { id: 3 }, { id: 4 }, { id: 5 }, { id: 6 }],
                    keyProperty: 'id',
                }),
                keyProperty: 'id',
            });
    ...
    it('.at', () => {
        expect(strategy.at(0).getContents()).toEqual(display.getItemBySourceKey(1).getContents());
    });

    describe('items', () => {
        it('default', () => {
            const items = strategy.items;
            expect(items[0].getContents()).toEqual(display.getItemBySourceKey(1).getContents());
            expect(items[1].getContents()).toEqual(display.getItemBySourceKey(2).getContents());
            expect(items[2].getContents()).toEqual(display.getItemBySourceKey(3).getContents());
        });
        ...
        it('collection has filtered items', () => {
            display = new Collection({
                collection: new RecordSet({
                    rawData: [{ id: 0 }, { id: 1 }, { id: 2 }, { id: 3 }, { id: 4 }, { id: 5 }],
                    keyProperty: 'id',
                }),
                keyProperty: 'id',
                filter: (item, index) => {
    ...
    it('setPosition', () => {
        ...
        expect(items[0].getContents()).toEqual(display.getItemBySourceKey(2).getContents());
        expect(items[1].getContents()).toEqual(display.getItemBySourceKey(1).getContents());
        expect(items[2].getContents()).toEqual(display.getItemBySourceKey(3).getContents());
