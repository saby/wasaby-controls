define([
   ...
   describe('Menu:Control', function () {
      ...
      let getMenu = function (config) {
         ...
         menuControl._emptyKeys = config?.emptyKey !== undefined ? [config.emptyKey] : [];
      ...
      describe('loadItems', () => {
         ...
         it('with dataLoadCallback in options', async () => {
            ...
            await menuControl._loadAndSetItems(menuOptions);
            expect(isDataLoadCallbackCalled).toBe(true);
         });
      ...
      describe('_beforeUpdate', () => {
         ...
         it('searchValue is changed', async () => {
            ...
            await menuControl._beforeMount(menuOptions);
            const newMenuOptions = { ...menuOptions, searchParam: 'title' };
      ...
      describe('_itemClick', function () {
         ...
         it('select empty item', function () {
            ...
            expect(selectedKeys[0]).toEqual(1);
            emptyMenuControl._listModel.getItemBySourceKey(1).setSelected(true);

            emptyMenuControl._itemClick('itemClick', item, {});
            expect(selectedKeys[0]).toEqual(null);
            emptyMenuControl._listModel.getItemBySourceKey(1).setSelected(false);

            emptyMenuControl._selectedKeys = [];
            emptyMenuControl._itemClick('itemClick', item, {});
            expect(selectedKeys[0]).toEqual(1);
         });
      ...
      it('getTemplateOptions sourceProperty', async function () {
         ...
         menuControl._getSourceSubMenu = (root, isLoadedChildItems, config) => {
            actualConfig = config;
      ...
      it('_getMarkedKey', function () {
         ...
         // empty item
         menuControl._emptyKeys = ['emptyKey'];
