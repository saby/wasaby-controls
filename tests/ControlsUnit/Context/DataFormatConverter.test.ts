...
describe('Controls/_context/DataFormatConverter', () => {
    ...
    it('Проверяем, что пришел старый формат конфигурации', () => {
        expect(
            DataFormatConverter.isOldDataFormat(oldCustomConfig)
        ).toBeTruthy();
        expect(
            DataFormatConverter.isOldDataFormat(actualListConfig)
        ).toBeFalsy();
    });
    it('Конвертация результатов предзагрузки к новому формату конфигурации', () => {
        ...
        expect(DataFormatConverter.convertLoadResultsToFactory(value)).toEqual(
            expectedConfigs
        );
    });
});
