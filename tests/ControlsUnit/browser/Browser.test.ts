...
import { getControllerState, NewSourceController } from 'Controls/dataSource';
import { ControllerClass as SearchController } from 'Controls/searchDeprecated';
import { ControllerClass as FilterController, IFilterControllerOptions } from 'Controls/filterOld';
...
describe('Controls/browser:Browser', () => {
    describe('_beforeMount', () => {
        describe('init states on beforeMount', () => {
            ...
            it('source returns error', async () => {
                ...
                const browser = getBrowser(options);
                await browser._beforeMount(options);
                expect(browser._contextState.source === options.source).toBeTruthy();
            });
            ...
            describe('sourceController on mount', () => {
                it('sourceController in options', async () => {
                    ...
                    const browser = getBrowser(options);
                    await browser._beforeMount(options);
                    expect(browser._getSourceController() === sourceController).toBeTruthy();
                });
            });
            ...
            describe('init expandedItems', () => {
                it('with receivedState', async () => {
                    ...
                    expect(browser._getSourceController().getExpandedItems()).toEqual([1]);
                });
        ...
        describe('searchController', () => {
            describe('searchValue on _beforeMount', () => {
                ...
                it('filterButtonSource and filter in context without source on _beforeMount', async () => {
                    ...
                    const browser = getBrowser(options);
                    await browser._beforeMount(options, {});
                    expect(browser._contextState.filter).toEqual(expectedFilter);
            ...
            describe('search', () => {
                ...
                it('double search call will create searchController once', async () => {
                    ...
                    const searchControllerCreatedPromise1 = browser._getSearchController();
                    const searchControllerCreatedPromise2 = browser._getSearchController();
                ...
                it('loading state on search', async () => {
                    ...
                    await browser._beforeMount(browserOptions);
                    browser.saveOptions(browserOptions);
                    let searchPromise = browser._search(null, 'test');
                ...
                it('search with root was canceled', async () => {
                    ...
                    await searchPromise;
                    expect(sourceController.getRoot() === currentRoot).toBeTruthy();
                });
        ...
        it('source as prefetchProxy', async () => {
            ...
            expect(browser._getSourceController().getSource() === options.source).toBeTruthy();
            expect(browser._source === options.source).toBeTruthy();
        });
        ...
        it('source as prefetchProxy and with receivedState', async () => {
            ...
            await browser._beforeUpdate(options);
            expect(browser._getSourceController().getSource() === source).toBeTruthy();
            expect(browser._source === source).toBeTruthy();
        });
    ...
    describe('_beforeUpdate', () => {
        describe('searchController', () => {
            ...
            it('searchParam is changed', async () => {
                ...
                await browser._beforeUpdate(options);
                expect(
                    browser._getSearchControllerSync()._options.searchParam === 'newSearchParam'
                ).toBeTruthy();
            });
            ...
            it('update with searchValue', async () => {
                ...
                browser._beforeUpdate(options);
                expect(browser._filter.name).toEqual('test');
                expect(browser._getSearchControllerSync().getRoot() === null).toBeTruthy();
            });
            ...
            it('update source and searchValue should reset inputSearchValue', async () => {
                ...
                browser.saveOptions(options);
                expect(browser._inputSearchValue === 'testSearchValue').toBeTruthy();
            ...
            it('search returns recordSet with another format', async () => {
                ...
                await browser._search(null, 'testSearchValue');
                expect(
                    browser._contextState.items.getFormat().getFieldIndex('testName2') !== -1
                ).toBeTruthy();
                expect(browser._items.getFormat().getFieldIndex('testName2') !== -1).toBeTruthy();
            });
        });
        ...
        describe('operationsController', () => {
            it('listMarkedKey is updated by markedKey in options', async () => {
                ...
                await browser._beforeMount(options);
                browser._beforeUpdate(options);
                expect(browser._operationsController._savedListMarkedKey).toEqual('testMarkedKey');
        ...
        describe('sourceController', () => {
            ...
            it('backButtonCaption is updated after items changed in sourceController', async () => {
                ...
                const sourceController = (options.sourceController = new NewSourceController({
                    ...options,
                }));
                const browser = await getBrowserWithMountCall(options);
                ...
                expect(browser._contextState.backButtonCaption === 'test').toBeTruthy();
            });
            ...
            describe('listsOptions', () => {
                it('prefetchProxy source in listsOptions', async () => {
                    ...
                    await browser._beforeUpdate(options);
                    expect(
                        browser._getSourceController().getState().source === source
                    ).toBeTruthy();
                });
                ...
                it('listsOptions are changed', async () => {
                    ...
                    await browser._beforeUpdate(options);
                    expect(
                        browser._getSourceController().getState().source === newSource
                    ).toBeTruthy();
                });
                ...
                it('sourceController on listsOptions', async () => {
                    ...
                    expect(browser._getSourceController() === sourceController).toBeTruthy();
                    expect(!(browserMountResult instanceof Promise)).toBeTruthy();

                    sourceController.setRoot('testRoot');
                    expect(browser._listsOptions[0].root === 'testRoot').toBeTruthy();
                });
                it('filterButtonSource in listsOptions', async () => {
                    ...
                    browser.saveOptions(options);
                    await browser._beforeUpdate(options);
                    expect(browser._dataLoader.getFilterController('list').getFilter()).toEqual({
                        testField: 'testValue',
                        filterField: '',
                    });
                ...
                it('root changed with sourceController in listsOptions', async () => {
                    ...
                    const notifyStub = jest
                        ...
                        .mockImplementation();
                    sourceController.setRoot('testRoot');
                    expect(notifyStub).toHaveBeenCalledWith('rootChanged', ['testRoot', 'list2']);
                });
        ...
        describe('filterController', () => {
            it('filterButtonSource changed', async () => {
                ...
                const notifyStub = jest.spyOn(browser, '_notify').mockClear().mockImplementation();
                browserOptions = { ...browserOptions };
                browserOptions.filterButtonSource = [
                    {
                        name: 'filterField',
            ...
            it('filterController changed', async () => {
                ...
                expect(!(browser._beforeUpdate(browserOptions) instanceof Promise)).toBeTruthy();
            });
        ...
        it('update source', async () => {
            ...
            await browser._beforeUpdate(options);
            expect(browser._items.at(0).get('title') === 'Интерфейсный фреймворк').toBeTruthy();
        });

        it('update source while loading', async () => {
            let options = getBrowserOptions();
            const browser = getBrowser();
            const errorStub = jest.spyOn(browser, '_onDataError').mockClear().mockImplementation();
    ...
    describe('_update', () => {
        it('update without source in options', async () => {
            ...
            await browser._beforeMount(options);
            browser.saveOptions(options);
            const notifyStub = jest.spyOn(browser, '_reload').mockClear().mockImplementation();
    ...
    describe('_dataLoadCallback', () => {
        ...
        it('search view mode changed on dataLoadCallback', async () => {
            ...
            await browser._beforeUpdate(options);
            expect(browser._searchValue === '').toBeTruthy();
            expect(browser._getSearchControllerSync().getViewMode()).not.toBeDefined();
        ...
        it('path is updated in searchController after load', async () => {
            ...
            expect(browser._getSearchControllerSync()._path[0].getRawData()).toEqual(
                path.at(0).getRawData()
            );
        });
