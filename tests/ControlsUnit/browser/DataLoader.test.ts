...
describe('Controls/dataSource:loadData', () => {
    beforeAll(() => {
        getUnloadedDeps = jest.fn().mockImplementation(() => Promise.resolve());
    });
    afterAll(() => {
    ...
    it('load with default load timeout', async () => {
        ...
        return new Promise((resolve) => {
            loadDataResult.then((loadResult) => {
                expect(loadResult[0].error.status === HTTPStatus.GatewayTimeout).toBeTruthy();
                resolve();
            });
        });
    });
    ...
    describe('dependencies', () => {
        ...
        it('static render dependencies', async () => {
            getUnloadedDeps.mockClear();
            const loadDataConfig = {
                source: getSource(),
            };
            await getDataLoader().load([loadDataConfig]);
            expect(getUnloadedDeps).toBeCalled();
        });
    });
});
