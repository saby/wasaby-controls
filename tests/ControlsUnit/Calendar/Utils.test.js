define([
   ...
   describe('Controls.Calendar.Utils', function () {
      ...
      it('getWeeksArray', function () {
         ...
         expect(
            DateUtil.Base.isDatesEqual(weeks[0][0], new Date(2016, 11, 26))
         ).toBe(true);
         expect(
            cInstance.instanceOfModule(weeks[0][0], 'Types/entity:Date')
         ).toBe(true);
      });
      ...
      it('getWeeksArray dateConstructor', function () {
         ...
         expect(
            cInstance.instanceOfModule(weeks[0][0], 'Types/entity:DateTime')
         ).toBe(true);
      });
      ...
      describe('updateRangeByQuantum', function () {
         ...
         tests.forEach(function (test) {
            it(`updateRangeByQuantum(${test.baseDate}, ${test.date}, ${test.ranges})`, function () {
               expect(
                  dateRange.Utils.updateRangeByQuantum(
                     test.baseDate,
                     test.date,
                     test.ranges
                  )
      ...
      describe('updateRangeByWorkdays', () => {
         [
            ...
            it('should return correct period ' + index, () => {
               const dateRangeResult = dateRange.Utils.updateRangeByWorkdays(
                  test.date
               );
