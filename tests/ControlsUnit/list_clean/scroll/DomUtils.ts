...
export function getListContainer(
    ...
    listContainer.appendChild(
        getItemsContainer(collection, customGetItemElement)
    );
...
export function getScrollContainerWithList(
    ...
    jest.spyOn(itemsContainer, 'getBoundingClientRect')
        .mockClear()
        .mockImplementation(() => {
            const beforeListOffset =
                beforeListContent?.getBoundingClientRect().height || 0;
            const indicatorOffset = collection.getTopIndicator().isDisplayed()
                ? 48
                : 0;
            return {
                width: 300,
...
export function renderCollectionChanges(
