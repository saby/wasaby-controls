            ...
            it(test.title, () => {
                ...
                if ('isEnabled' in test) {
                    for (const position of positions) {
                        expect(component._models[position].isEnabled).toBe(test.isEnabled);
                    }
                }
                if ('isVisible' in test) {
                    for (const position of positions) {
                        expect(component._models[position].isVisible).toBe(test.isVisible);
                    }
                }
                if ('isTopVisible' in test) {
                    expect(component._models.top.isVisible).toBe(test.isTopVisible);
                }
                if ('isBottomVisible' in test) {
                    expect(component._models.bottom.isVisible).toBe(test.isBottomVisible);
                }
            });
        });
    });
    ...
    describe('setStickyFixed', () => {
        [
            ...
            it(`should ${
                ...
                jest.spyOn(component, '_nextVersion').mockClear().mockImplementation();
