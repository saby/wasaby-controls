...
import { IStickyDataContext } from 'Controls/_stickyBlock/types';
...
import { ScrollShadowVisibility } from 'Controls/_stickyBlock/types';
...
describe('Controls/_stickyBlock/Controllers/StickyController', () => {
    ...
    function mocksForRegister(stickyController: StickyController): void {
        jest.spyOn(
            StickyIntersectionController.prototype,
            'observe'
        ).mockImplementation();
        jest.spyOn(
            StickyIntersectionController.prototype,
        ...
        jest.spyOn(
            StickySizeController.prototype,
            'unobserve'
        ).mockImplementation();
    ...
    describe('unregister', () => {
        ...
        it('После разрегистрации одного стики блока контроллер пересчитывает оставшиеся', async () => {
            ...
            mocksForRegister(stickyController);
            jest.spyOn(functions, 'getOffsetByContainer').mockImplementation(
                (element) => {
                    switch (element.id) {
                        case '0':
                            return 0;
            ...
            jest.spyOn(functions, 'getStickyBlockHeight').mockImplementation(
                (element) => {
                    return 10;
                }
            );
            const registerPromise = contextData.registerCallback(
                stickyBlocksData.block0
            );
    ...
    describe('modeChanged', () => {
        ...
        it('Пересчитываются тени и оффсеты, если поменялся mode', async () => {
            ...
            mocksForRegister(stickyController);
            const spyUpdateShadows = jest.spyOn(
                stickyController,
                '_updateShadows'
            );
            const spyUpdateOffsets = jest.spyOn(
                stickyController,
                '_updateOffsets'
            );
            jest.spyOn(functions, 'getStickyBlockHeight').mockImplementation(
