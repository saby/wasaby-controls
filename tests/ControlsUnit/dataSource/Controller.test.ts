...
import { default as groupUtil } from 'Controls/_dataSource/Utils/GroupUtil';
...
describe('Controls/dataSource:SourceController', () => {
    ...
    describe('load', () => {
        it('load with parentProperty', async () => {
            ...
            expect((loadedItems as RecordSet).getCount()).toEqual(2);
        });

        it('Загрузка узла с ключом null (не важно, корень это или нет) должнен возвращать результат для этого узла', async () => {
            const controller = getControllerWithHierarchy({
                root: 0,
                source: new Memory({
                    data: hierarchyItems,
            ...
            const loadedItems = await controller.load(undefined, null, { parent: null }, false);
            expect((loadedItems as RecordSet).getCount()).toEqual(3);
        });
        ...
        it('load call while loading', async () => {
            ...
            const promiseCanceled = controller.load(undefined, 0).catch(() => {
            ...
            expect(controller.hasLoaded(0)).toBeFalsy();
            expect(controller.hasLoaded(3)).toBeTruthy();
        });
        ...
        it('load с множественной навигацией и фильтром в параметрах', async () => {
            const navigation = getPagingNavigation();
            navigation.sourceConfig?.multiNavigation = true;
            const controller = getControllerWithHierarchy({
                navigation,
            ...
            // то мокаю метод
            jest.spyOn(controller.getSource() as TSourceOption, 'query')
                .mockClear()
            ...
            await controller.load(undefined, 0, { parent: [0, 1] }, true, undefined, true);

            expect(controller.getItems().getRecordById(0).get('title')).toBe('WithMultiNavigation');
        });
    });
    ...
    describe('reload', () => {
        it('reload should recreate navigation controller', async () => {
            ...
            expect(controller.hasMoreData('down', 0)).toBeTruthy();

            controller.setRoot(3);
            await controller.reload();
            expect(controller.hasLoaded(0)).toBeFalsy();
            expect(controller.hasMoreData('down', 0)).toBeFalsy();
        });
        ...
        it('Множественная навигация должна отправляться при перезагрузке с развёрнутыми узлами', async () => {
            const navigation = getPagingNavigation();
            navigation.sourceConfig?.multiNavigation = true;
            const controller = getControllerWithHierarchy({
                navigation,
        ...
        it('Перезагрузка с сохранением навигации', async () => {
            const controller = getController({
                navigation: getPagingNavigation(false, 1),
            });
            ...
            await controller.reload(undefined, undefined, undefined, true);
            expect(controller.getItems().getCount()).toBe(2);
        });

        it('Перезагрузка с сохранением навигации внутри узла', async () => {
            const controller = getControllerWithHierarchy({
                navigation: getPagingNavigation(false, 1),
                root: 0,
            });
    ...
    describe('setItems', () => {
        it('navigationController is recreated on setItems', async () => {
            const controller = getController({
                navigation: getPagingNavigation(false),
            });
            await controller.reload();
            ...
            expect(controller.hasMoreData('down')).toBeFalsy();
        });
        ...
        it('navigation is updated before assign items', () => {
            ...
            expect(hasMoreResult).toBeFalsy();
    ...
    describe('setItemsAfterLoad', () => {
        it('Установка записей должна обновлять навигацию для текущего корня', async () => {
            const navigation = getPagingNavigation(false, 1);
            const sourceController = getControllerWithHierarchy({ navigation });

            await sourceController.reload();

            const items = (await sourceController.load(
                undefined,
                0,
                undefined,
                false
            )) as RecordSet;
            expect(sourceController.hasMoreData('down', 0)).toBeFalsy();

            sourceController.setRoot(0);
            sourceController.setItemsAfterLoad(items);
            expect(sourceController.hasMoreData('down')).toBeTruthy();
        });
    });

    describe('appendItems', () => {
        it('Добавление записей с навигацией', async () => {
            const navigation = getPagingNavigation(false, 1);
            const sourceController = getController({ navigation });

            await sourceController.reload();

            let items = await sourceController.load('down', undefined, undefined, false);
            sourceController.appendItems(items);
            expect(sourceController.getItems().getCount()).toBe(2);
            expect(sourceController.hasMoreData('down')).toBeTruthy();

            items = await sourceController.load('down', undefined, undefined, false);
            sourceController.appendItems(items);
            expect(sourceController.getItems().getCount()).toBe(3);
            expect(sourceController.hasMoreData('down')).toBeTruthy();

            items = await sourceController.load('down', undefined, undefined, false);
            sourceController.appendItems(items);
            expect(sourceController.getItems().getCount()).toBe(4);
            expect(sourceController.hasMoreData('down')).toBeFalsy();
        });
    });
    ...
    describe('reloadItem', () => {
        ...
        it('Множественная навигация не должна отправляться в запрос при перезагрузке элемента', async () => {
            let navigation = getPagingNavigation(false, 1);
            const controller = getControllerWithHierarchy({
                navigation,
                expandedItems: [],
                root: null,
            ...
            await controller.reload();
            await controller.load(undefined, 0);
            controller.setExpandedItems([0]);
            controller.getItems().getRecordById(0).set('title', 'WrongName');
            ...
            expect(controller.getItems().getRecordById(0).get('title')).toBe(
                'Интерфейсный фреймворк'
            );
        });
        it('Навигация не должна меняться при перезагрузке элемента', async () => {
            const navigation = getPagingNavigation(false, 1);
            const controller = getController({
                navigation,
            });
