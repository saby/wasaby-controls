...
describe('Controls/filterPanel:ViewModel', () => {
    ...
    describe('Клик по группе', () => {
        it('Редактор должен свернуться', () => {
            const source = getSource();
            source[0].expanderVisible = true;
            source[0].extendedCaption = 'text';
            ...
            expect(viewModel.needCloseItemByCaptionClick(source[0].name)).toBeTruthy();
        });

        it('Редактор не должен свернуться', () => {
            const source = getSource();
            source[0].expanderVisible = true;
            ...
            expect(viewModel.needCloseItemByCaptionClick(source[0].name)).toBeFalsy();
        });
    });
    ...
    describe('needShowSeparator', () => {
        ...
        it('Кнопка "Сбросить" отображается с basic фильтрами', () => {
            const source = [
                {
                    name: 'test',
                    value: 'test',
    ...
    describe('adaptive filter', () => {
        ...
        it('В адаптивном режиме выводим все редакторы, если их явно не скрыли c помощью опции isAdaptive', () => {
            const source = [
                {
                    name: 'test - 1',
                    value: '',
    ...
    describe('filterItemLocalProperty', function () {
        it('viewMode определён в filterItemLocalProperty', () => {
            const source = [
                {
                    name: 'test',
                    value: '',
                    ...
                    panel: {
                        viewMode: 'extended',
                        extendedCaption: 'textExtendedCaption',
                    },
                },
            ];
            ...
            expect(viewModel.getSource()[0].viewMode).toEqual('extended');
            expect(viewModel.getSource()[0].extendedCaption).toEqual('textExtendedCaption');
        });

        it('caption определён в filterItemLocalProperty', () => {
            const source = [
                {
                    name: 'test',
                    value: '',
            ...
            expect(viewModel.getSource()[0].editorCaption).toEqual('panelEditorCaption');
        });
        ...
        it('viewMode определён в filterItemLocalProperty', () => {
            ...
            expect(viewModel.getSource()[0].panel.viewMode).toEqual('extended');

            viewModel.setEditingObject({
                test: {
                    value: 'testFilterValue',
                    textValue: 'testFilterTextValue',
                    viewMode: 'basic',
                },
            });

            expect(viewModel.getSource()[0].viewMode).toEqual('basic');
            expect(viewModel.getSource()[0].value).toEqual('testFilterValue');
            expect(viewModel.getSource()[0].textValue).toEqual('testFilterTextValue');
        });
    });
});
