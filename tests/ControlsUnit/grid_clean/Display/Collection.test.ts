    ...
    describe('Update options', () => {
        describe('multiSelectVisibility', () => {
            ...
            it("change multiSelectVisibility onhover => visible, shouldn't update version", () => {
                ...
                const headerInitialVersion = gridCollection.getHeader().getVersion();
                const resultsInitialVersion = gridCollection.getResults().getVersion();
                const footerInitialVersion = gridCollection.getFooter().getVersion();
                const emptyGridInitialVersion = gridCollection.getEmptyGridRow().getVersion();

                // setMultiSelectVisibility
                gridCollection.setMultiSelectVisibility('visible');

                expect(gridCollection.getHeader().getVersion()).toBe(headerInitialVersion);
                expect(gridCollection.getResults().getVersion()).toBe(resultsInitialVersion);
                expect(gridCollection.getFooter().getVersion()).toBe(footerInitialVersion);
                expect(gridCollection.getEmptyGridRow().getVersion()).toBe(emptyGridInitialVersion);
            });
        });
        ...
        describe('emptyTemplateColumns', () => {
            it('Initialize with emptyTemplateColumns', () => {
                ...
                const gridCollection = new GridCollection({
                    collection: [{ key: 1 }, { key: 2 }, { key: 3 }],
                    keyProperty: 'key',
                    columns: [{ displayProperty: 'id' }, { displayProperty: 'name' }],
                    emptyTemplateColumns: emptyColumnsConfig,
                });

                expect(gridCollection.getEmptyGridRow()).toBeDefined();
                expect(gridCollection.getEmptyGridRow().getColumns()[0].config).toEqual(
                    emptyColumnsConfig[0]
                );
            });
            ...
            it('Initialize with emptyTemplateColumns and change it', () => {
                ...
                expect(oldEmptyTemplateRow).toEqual(gridCollection.getEmptyGridRow());
                expect(gridCollection.getEmptyGridRow().getColumns()[0].config).toEqual(
                    newEmptyColumnsConfig[0]
                );
            });
