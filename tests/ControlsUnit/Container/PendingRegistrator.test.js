define(['Controls/Pending', 'Types/deferred'], (PendingRegistrator, defferedLib) => {
   'use strict';
   describe('Controls/Container/PendingRegistrator', () => {
      it('finishPendingOperations', () => {
         let Registrator = new PendingRegistrator.default();
         let def1 = new defferedLib.Deferred();
         let def2 = new defferedLib.Deferred();
         let def3 = new defferedLib.Deferred();
      ...
      it('register/unregister pending', async () => {
         ...
         Registrator._registerPendingHandler(null, def3, {});
         expect(Object.keys(Registrator._pendingController._pendings).length).toEqual(2);
         expect(Object.keys(Registrator._pendingController._pendings[baseRoot]).length).toEqual(2);
         expect(Object.keys(Registrator._pendingController._pendings[firstRoot]).length).toEqual(1);
         ...
         expect(Object.keys(Registrator._pendingController._pendings)).toHaveLength(0);
      ...
      it('hasRegisteredPendings', () => {
         ...
         Registrator._registerPendingHandler(null, new defferedLib.Deferred(), pendingCfg1);
         expect(Registrator._hasRegisteredPendings()).toEqual(false);

         Registrator._registerPendingHandler(null, new defferedLib.Deferred(), pendingCfg2);
         expect(Registrator._hasRegisteredPendings()).toEqual(true);
         expect(Registrator._hasRegisteredPendings(false)).toEqual(false);

         Registrator._registerPendingHandler(null, new defferedLib.Deferred(), {});
         expect(Registrator._hasRegisteredPendings()).toEqual(true);
