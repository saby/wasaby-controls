...
describe('FormController', () => {
    it('initializingWay', (done) => {
        ...
        const p3 = new Promise((resolve) => {
            ...
            expect(isCreating).toEqual(false);
            expect(
                beforeMountResult instanceof Deferred || beforeMountResult instanceof Promise
            ).toBe(true);
            beforeMountResult
                .then(({ data }) => {
    ...
    it('beforeUpdate', async () => {
        ...
        // @ts-ignore
        FC._finishCreatePromiseInMount = true;
    ...
    it('_notifyHandler', () => {
        ...
        jest.spyOn(component, '_notifyToOpener').mockClear().mockImplementation();
        jest.spyOn(component, '_notify').mockClear().mockImplementation();
        component._notifyHandler(name, args);
        expect(component._notifyToOpener.mock.invocationCallOrder[0]).toBeLessThan(
            component._notify.mock.invocationCallOrder[0]
        );
    });
    ...
    it('requestCustomUpdate isNewRecord', async () => {
        ...
        expect(FC._isNewRecord).toEqual(false);
        expect(FC._notify.mock.calls[0]).toEqual(['requestCustomUpdate', [FC._record, updateCfg]]);
        FC.destroy();
    });
    ...
    it('afterMount. Create before mount', () => {
        ...
        const notifyHandler = jest.spyOn(FC, '_notifyHandler').mockClear().mockImplementation();
        const createHandler = jest.spyOn(FC, '_createHandler').mockClear().mockImplementation();
        FC._afterMount();

        expect(FC._isMount).toBe(true);
        expect(notifyHandler).toHaveBeenCalledWith('createSuccessed', expect.anything());
        ...
        FC._afterMount();
        expect(notifyHandler).toHaveBeenCalledWith('createFailed', expect.anything());
        expect(FC._createdInMounting).toBeNull();
        FC.destroy();
    });
    ...
    it('afterMount. Read before mount', () => {
        ...
        const notifyHandler = jest.spyOn(FC, '_notifyHandler').mockClear().mockImplementation();
        const readHandler = jest.spyOn(FC, '_readHandler').mockClear().mockImplementation();
        FC._afterMount();

        expect(FC._isMount).toBe(true);
        expect(notifyHandler).toHaveBeenCalledWith('readSuccessed', expect.anything());
        ...
        FC._afterMount();
        expect(notifyHandler).toHaveBeenCalledWith('readFailed', expect.anything());
        expect(FC._readInMounting).toBeNull();
        FC.destroy();
    });
    ...
    it('afterUpdate', () => {
        const FC = new Controller({});
        const activate = jest.spyOn(FC, 'activate').mockClear().mockImplementation();
    ...
    it('createRecordBeforeMount reject', () => {
        ...
        const setFunctionToRepeat = jest.spyOn(FC, '_setFunctionToRepeat').mockClear();
    ...
    it('updating failed with validation failed', () => {
        ...
        const notify = jest.spyOn(FC, '_notify').mockClear().mockImplementation();
        return FC._update().catch(() => {
            expect(notify).toHaveBeenCalledWith('validationFailed', expect.anything());
        });
    });
    ...
    describe('_onCloseErrorDialog()', () => {
        ...
        it('without record', () => {
            ...
            expect(fc._notify.mock.calls[0]).toEqual(['close', [], { bubbling: true }]);
        });
