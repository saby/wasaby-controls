...
describe('Controls/_operations/Utils/SelectionViewModeHelper', () => {
    describe('getSelectionViewMode', function () {
        ...
        describe('Иерархический список', function () {
            ...
            it('Записи загружены, есть выбранные значения', function () {
                expect(getSelectionViewMode(selectionViewMode, options)).toBe('all');
            });

            it('Записи загружены, ничего не выбрано, кнопка Отобрать отмеченные не нажата', function () {
                options.selectedKeys = [];
                expect(getSelectionViewMode(selectionViewMode, options)).toBe('hidden');
            });

            it('Записи загружены, ничего не выбрано, нажата кнопка Отобрать отмеченные', function () {
                selectionViewMode = 'selected';
                expect(getSelectionViewMode(selectionViewMode, options)).toBe('selected');
            });
            ...
            describe('Построение списка с уже проставленным selectedKeys', () => {
                it('Отмеченная запись загружена', function () {
                    options.selectedKeys = [1];
                    options.excludedKeys = [];
                    expect(getSelectionViewMode('', options)).toBe('all');
                });
                it('Отмеченная запись не загружена', function () {
                    options.selectedKeys = ['notLoaded'];
                    options.excludedKeys = [];
                    expect(getSelectionViewMode('', options)).toBe('');
                });
                it('Записи при построении контрола не загружены', function () {
                    const newOptions = { ...options };
                    newOptions.selectedKeys = ['notLoaded'];
                    newOptions.excludedKeys = [];
                    newOptions.sourceController = getSourceController();
                    newOptions.sourceController.setItems(null);
                    expect(getSelectionViewMode('', options)).toBe('');
                });
            });
        });
