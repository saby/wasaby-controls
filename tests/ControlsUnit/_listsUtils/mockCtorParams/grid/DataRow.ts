...
function _getCtorParamsMock(
    params: ICtorParamsMock
): Required<ICtorParamsMock> {
    return {
        ...
        columnsConfig: extractParam(
            params,
            'columnsConfig',
            params.gridColumnsConfig
        ),
...
function getCtorParamsMock<T extends GridCollection>(
    ...
    if (withOwner) {
        (ctorParams as Required<ICtorParamsMock & { owner: T }>).owner =
            _getOwnerMock<T>(params);
    }
    return ctorParams;
}
