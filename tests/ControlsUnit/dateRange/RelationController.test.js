define([
   ...
   'Types/entity',
   'Controls/dateUtils'
], function (cClone, dateRange, calendarTestUtils, typesEntity, dateUtils) {
   'use strict';
   ...
   describe('Controls.dateRange:RelationController', function () {
      ...
      [
         {
            props: {
               startValue0: new Date(2024, 2, 3),
               endValue0: new Date(2024, 2, 3),
               startValue1: new Date(2024, 2),
               endValue1: new Date(2024, 3, 0)
            },
            checkResult: (ranges) => {
               return (
                  dateUtils.Base.isDatesEqual(ranges[0][0], new Date(2024, 1)) &&
                  dateUtils.Base.isDatesEqual(ranges[0][1], new Date(2024, 2, 0))
               );
            }
         },
         {
            props: {
               startValue0: new Date(2024, 3),
               endValue0: new Date(2024, 6, 0),
               startValue1: new Date(2024, 4),
               endValue1: new Date(2024, 4, 5)
            },
            checkResult: (ranges) => {
               return (
                  dateUtils.Base.isDatesEqual(ranges[1][0], new Date(2024, 6)) &&
                  dateUtils.Base.isDatesEqual(ranges[1][1], new Date(2024, 9, 0))
               );
            }
         }
      ].forEach((props, index) => {
         it(`should validate periods with different quants ${index}`, function () {
            const component = calendarTestUtils.createComponent(RelationController, props);
            expect(component._model.ranges);
         });
      });
