define([
   ...
   'Types/deferred',
   'Types/entity'
], function (MultilinePath, HeadingPath, Path, BreadCrumbsUtil, FontLoadUtil, defferedLib, entity) {
   describe('Controls.BreadCrumbs.HeadingPath', function () {
      ...
      it('_onBackButtonClick', function () {
         path._notify = function (e, args) {
            if (e === 'itemClick') {
               expect(path._options.items[path._options.items.length - 2].get('parent')).toEqual(
                  args[0].get('parent')
               );
            }
         };
   ...
   describe('Controls.BreadCrumbs.MultilinePath', function () {
      ...
      it('2 crumbs', function () {
         ...
         expect(MultilinePathCrumbs._visibleItemsSecond.length === 0).toBe(true);
      });
      ...
      it('несколько крошек, причем последняя не влезает в первый контейнер с сокращением', function () {
         ...
         expect(MultilinePathCrumbs._visibleItemsSecond.length === 1).toBe(true);
      });
