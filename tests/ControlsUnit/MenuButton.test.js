define([
   ...
   'Types/collection',
   'Controls/popup'
], (dropdown, sourceLib, Clone, history, defferedLib, entity, collection, popup) => {
   describe('MenuButton', () => {
      let items;
      ...
      it('check item click', () => {
         let testEvent;
         menu._notify = (e, d) => {
            expect(e === 'menuItemActivate' || e === 'onMenuItemActivate').toBe(true);
            testEvent = d[1];
      ...
      it('check received state', () => {
         return menu._beforeMount(config, null, { items: itemsRecords.clone() }).then(() => {
            expect(menu._controller._items.getRawData()).toEqual(itemsRecords.getRawData());
         });
      });
      ...
      it('received state, selectedItems = [null]', () => {
         ...
         return menu._beforeMount(cfg, {}, { items: itemsRecords.clone() }).then(() => {
