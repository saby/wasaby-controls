...
describe('Controls/_source/NavigationController', () => {
    describe('Page navigation', () => {
        describe('Without config', () => {
            ...
            it('navigationParamsChangedCallback called with new params', () => {
                ...
                expect(newNavigationParams.page === START_PAGE + 1).toBeTruthy();
                expect(newNavigationParams.pageSize === TEST_PAGE_SIZE).toBeTruthy();
            });
        });
    });
    describe('Position navigation', () => {
        describe('Without config', () => {
            ...
            it('Перезагрузка с сохранением навигации. Если метод вернул меньше записей, сохранённый limit должен сброситься', () => {
                const QUERY_LIMIT = 3;
                const navigController = new NavigationController({
                    navigationType: 'position',
                    navigationConfig: {
                ...
                expect(QUERY_LIMIT + rs.getCount()).toEqual(
                    navigController.updateQueryProperties(
                        rs,
                        null,
                        undefined,
                        'forward',
    ...
    describe('getQueryParams', () => {
        ...
        describe('position', () => {
            it('getQueryParams with config', () => {
                ...
                const params = nc.getQueryParams({ filter: {}, sorting: [] }, null, {
                    direction: 'forward',
                    field: 'id',
                    limit: 15,
                    position: 0,
