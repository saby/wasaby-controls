...
define([
   ...
   recordSet
      ._getMediator()
      .addRelationship(model, recordSet, 'customRelationship');
   ...
   describe('Controls/_lookup/resources/ToSourceModel', function () {
      ...
      describe('to source model', function () {
         it('check model by step', function () {
            ...
            expect(
               ToSourceModel(list.clone(), dataSource, '').at(0)._moduleName
            ).toEqual('customModel');
            expect(model.isChanged()).toEqual(false);
            ...
            expect(
               ToSourceModel(list.clone(), prefetchSource, '').at(0)._moduleName
            ).toEqual('customModel');
         });
