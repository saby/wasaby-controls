...
describe('Controls/suggest', () => {
    describe('Controls.Container.Suggest.Layout', () => {
        ...
        const getComponentObject = (customOptions: object = {}) => {
            ...
            jest.spyOn(controller, '_forceUpdate')
                .mockClear()
                .mockImplementation();
            return controller;
        };
        ...
        const getHistorySource = (component: _InputController) => {
            return component._getHistoryService();
        };
        ...
        it('Suggest::_getHistoryService', (done) => {
            ...
            getHistorySource(controller).addCallback((historyService) => {
                expect(12).toEqual(historyService._$recent);
                expect('TEST_HISTORY_ID_GET_SOURCE').toEqual(
                    historyService._$historyId
                );
                done();
            });
        });
        ...
        it('Suggest::_shouldShowSuggest', () => {
            ...
            expect(!!inputContainer._shouldShowSuggest(emptyResult)).toBe(
                false
            );
        ...
        it('Suggest::searchErrback', () => {
            ...
            // это сделано для того, чтобы ручные вызовы _forceUpdate не заваливали консоль ошибками
            jest.spyOn(suggest, '_forceUpdate')
                .mockClear()
                .mockImplementation();
        ...
        it('Suggest::_moreClick', () => {
            ...
            suggest._notify = (eventName, data) => {
                if (eventName === 'showSelector') {
                    ...
                    expect(data[0].templateOptions.filter).toEqual(
                        suggest._filter
                    );
                }
            };
        ...
        describe('Suggest::_resolveLoad', () => {
            ...
            it('value is cleared', async () => {
                const value = '';
                inputContainer._inputActive = true;
                inputContainer._getSourceController().load = () => {
        ...
        describe('Suggest::_searchResetCallback', () => {
            ...
            it('openWithHistory case :: suggest should stay opened', async () => {
                jest.spyOn(inputContainer, '_openWithHistory')
                    .mockClear()
                    .mockImplementation();
            ...
            it('default case with autoDropDown :: suggest should stay opened', async () => {
                ...
                jest.spyOn(inputContainer, '_open')
                    .mockClear()
                    .mockImplementation();
                jest.spyOn(inputContainer, '_setFilter')
                    .mockClear()
                    .mockImplementation();
        ...
        it('Suggest::_beforeUpdate', async () => {
            ...
            expect(suggestComponent._loading).toEqual(null);
            expect(suggestComponent._dependenciesDeferred).toEqual(
            ...
            const notifySpy = jest
                .spyOn(suggestComponent, '_notify')
                .mockClear();
            ...
            expect(notifySpy).toHaveBeenCalledWith('suggestStateChanged', [
                false,
            ]);
            ...
            expect(suggestComponent._inputSearchValue).toEqual('');
            expect(suggestComponent._notify).toHaveBeenCalledTimes(3);
            ...
            expect(resolveLoadStub).toHaveBeenCalledWith(
                'testValue',
                newOptions
            );
        ...
        describe('_beforeUpdate hook', () => {
            it('source is empty on mount', () => {
                ...
                expect(inputController._dataLoader === null).toBeTruthy();

                options.suggestListsOptions[0].source = new Memory();
                inputController._beforeUpdate(options);

                expect(
                    inputController._getSourceController().getState().source !==
                        null
                ).toBeTruthy();
            });
            ...
            it('value and suggestState are changed in options while loading', async () => {
                ...
                inputController._beforeUpdate(options);
                expect(
                    !inputController._searchResolverController._delayTimer
                ).toBeTruthy();
            });
            ...
            it('navigation is updated', async () => {
                ...
                const searchController =
                    await inputController._getSearchController();
                ...
                expect(items.getCount() === 1).toBeTruthy();
            });
        });
        ...
        it('PrefetchProxy source should became to original source type', async () => {
            ...
            expect(
                inputContainer._getSourceController().getState().source
            ).toBeInstanceOf(Memory);
        });
        ...
        describe('Suggest::_updateSuggestState', () => {
            ...
            it('autoDropDown = false', async () => {
                jest.spyOn(inputContainer, '_getRecentKeys')
                    .mockClear()
        ...
        it('Suggest::_select', () => {
            ...
            jest.spyOn(suggestComponent, '_closePopup')
                .mockClear()
                .mockImplementation();

            suggestComponent._select(null, item);
            expect(item._isUpdateHistory).toBe(false);
            suggestComponent._options.historyId = 'testFieldHistoryId';
            suggestComponent._select(null, item);
            expect(item._isUpdateHistory).toBe(true);
        });
        ...
        it('Suggest:_getRecentKeys', () => {
            ...
            inputContainer._getHistoryService = () => {
                const hService = {
                    query: () => {
                        return new Deferred.fail(new Error('History Service'));
                    },
                };
        ...
        describe('_beforeUnmount', () => {
            it('_beforeUnmount while load dependencies', () => {
                const suggestComponent = getComponentObject(
                    _InputController.getDefaultOptions()
                );
            ...
            it('_beforeUnmount while load searchLib', async () => {
                ...
                const spy = jest
                    .spyOn(suggestComponent._searchLibraryLoader, 'cancel')
                    .mockClear();
