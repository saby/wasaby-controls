define([
   ...
   'Controls/_popup/Popup/GlobalController',
   'Types/deferred',
   'jsdom',
   ...
   let Container;
   beforeEach(() => {
      global.HTMLElement = new jsdomLib.JSDOM('');
      global.Element = new jsdomLib.JSDOM('');
      jest.spyOn(BaseController.prototype, '_checkContainer').mockClear().mockReturnValue(true);

      Manager = new popupTemplateStrategy.Controller();
      ...
      Container = new popupMod.ContainerForTest();
      Manager.init({});
   ...
   describe('Controls/_popup/Manager', function () {
      ...
      it('initialize', function () {
         expect(Manager.getPopupItems().getCount()).toEqual(0);
      });
      ...
      it('append popup', function () {
         ...
         expect(Manager.getPopupItems().getCount()).toEqual(1);
      ...
      //
      //    let hasPending = true;
      //    let pendingDeferred = new defferedLib.Deferred();
      //
      //    let Pending = {
      ...
      //    setTimeout(() => {
      //       expect(Manager.getPopupItems().getCount()).toEqual(3);
      //
      //       Pending = false;
      //       Manager.remove(id2).then(() => {
      //          expect(Manager.getPopupItems().getCount()).toEqual(2);
      //       });
      //    }, 10);
      // });
      ...
      it('add modal popup', async function () {
         ...
         let indices = Manager.getPopupItems().getIndicesByValue('modal', true);
         expect(indices.length).toEqual(1);
         expect(indices[0]).toEqual(1);

         await Manager.remove(id1);

         indices = Manager.getPopupItems().getIndicesByValue('modal', true);
         expect(indices.length).toEqual(1);
      ...
      it('managerPopupMaximized notified', function () {
         ...
         const stub = jest
            .spyOn(popupTemplateStrategy.Controller, '_notifyEvent')
            .mockClear()
      ...
      //
      //    Manager.getPopupItems().at(0).childs = [Manager.getPopupItems().at(1)];
      //    Manager.getPopupItems().at(1).parentId = id1;
      //
      //    let removeDeferred2 = new defferedLib.Deferred();
      ...
      it('removeFromParentConfig', function () {
         ...
         Manager.getPopupItems().at(0).childs = [Manager.getPopupItems().at(1)];
         Manager.getPopupItems().at(1).parentId = id1;
         Manager._removeFromParentConfig(Manager.getPopupItems().at(1));
         expect(Manager.getPopupItems().at(0).childs.length).toEqual(0);
      ...
      it('calculateZIndex', () => {
         ...
         expect(Manager.getPopupItems().at(0).currentZIndex).toEqual(10);
         ...
         expect(Manager.getPopupItems().at(1).currentZIndex).toEqual(4000);

         const item3 = {
            id: 3
         };
         Manager.show(item3, controller);
         Manager.getPopupItems().at(2).parentId = 2;
         Manager._updateZIndex();
         expect(Manager.getPopupItems().at(2).currentZIndex).toEqual(4010);
         ...
         expect(Manager.getPopupItems().at(3).currentZIndex).toEqual(5000);
         Manager.destroy();
      });
      ...
      it('update popup poisition by scroll and resize', () => {
         ...
         Manager.getPopupItems().add(item);
      ...
      it('moveToTop', () => {
         ...
         Manager.show(item2, controller);
         Manager.show(item3, controller);
         Manager.getPopupItems().at(0).popupState = 'created';

         Manager.show(item1, controller);

         // assert.equal(Manager.getPopupItems().at(2).id, item1.id);
         Manager.destroy();
      });
      ...
      it('finishPendings', () => {
         ...
         let item = Manager.getPopupItems().at(0);
