define([
   ...
   describe('Controls/_popup/Opener/Dialog', () => {
      ...
      it('dialog positioning base', () => {
         ...
         expect(position.top).toEqual(470);
         expect(position.left).toEqual(880);
      ...
      it('dialog positioned on prop storage coordinates', () => {
         ...
         directions.forEach((direction) => {
            ...
            DialogController.getDefaultConfig(item);
            expect(
               item.position[direction.vertical === 'bottom' ? 'top' : 'bottom']
            ).toEqual(100);
            expect(
               item.position[
                  direction.horizontal === 'right' ? 'left' : 'right'
               ]
            ).toEqual(100);
         });
      });
      ...
      it('dialog draggable position', () => {
         ...
         expect(position.left).toEqual(610);
         expect(position.top).toEqual(320);
      ...
      it('position setted in popupOptions', () => {
         ...
         let position = DialogStrategy.getPosition(
            windowData,
            dialogSizes,
            item
         );
      ...
      describe('position with resizeDirection', () => {
         ...
         it('dragging', () => {
            ...
            position = DialogStrategy.getPosition(
               windowData,
               dialogSizes,
               item
            );
         ...
         it("inner resize can't set height lower", () => {
            ...
            popupItem.popupOptions.fittingMode = 'overflow';
            ...
            expect(position.minHeight).toEqual(newPopupSizes.height);
            DialogController._getPopupSizes = originGetPopupSizes;
         });
         ...
         it("min height that fixed by inner resize can't be more than window height", () => {
            ...
            expect(position.minHeight).toEqual(window.height);
            DialogController._getPopupSizes = originGetPopupSizes;
            item.popupOptions.height = undefined;
            item.popupOptions.width = undefined;
         });
         ...
         it('inner resize should update position for popup with resizeDirection', () => {
            ...
            jest
               .spyOn(DialogStrategy, 'getPosition')
               .mockClear()
               .mockImplementation();
