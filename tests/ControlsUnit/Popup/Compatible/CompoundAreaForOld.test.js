 ...
 */
define(['Controls/compatiblePopup', 'Types/deferred'], function (compatiblePopup, defferedLib) {
   'use strict';
   ...
   describe('Controls/compatiblePopup:CompoundArea', function () {
      ...
      describe('unregister from parent', function () {
         ...
         it('correctly removes information about itself', () => {
            ...
            expect(parent._childContainers.includes(Area._container)).toBe(false);
            expect((Area._options.name || '') in parent._childsMapName).toBeFalsy();
         });
         ...
         it('does not throw exception if parent does not have required fields', () => {
            ...
            expect(Area._clearInformationOnParentFromCfg.bind(Area)).not.toThrow();
         });
      });
   });
});
