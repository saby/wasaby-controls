...
describe('Controls/filter:FilterCalculator', () => {
    ...
    describe('FilterCalculator.getFilterByFilterDescription', () => {
        ...
        it('Должно удалиться значение из фильтра, если из valueConverter вернуть специальное значение', () => {
            const filterDescription = [
                {
                    name: 'test',
                    value: 'deleteTest',
    ...
    describe('prepareFilter', () => {
        it('prepareFilter с параметрами кэширования должен вернуть filterDescription', async () => {
            const filterDescription = [
                {
                    name: 'test',
                    value: null,
            ...
            await Store.push(historyId, {
                items: historyFilterDescription,
                prefetchParams: {
                    PrefetchSessionId: 'testPrefetchSessionId',
                },
            });

            const prepareFilterResult = await FilterCalculator.prepareFilter({
                filterDescription,
                filter: {},
                historyId,
            });
            expect(prepareFilterResult?.items).toBeUndefined();
            expect(prepareFilterResult?.filter).toStrictEqual({ test: 'testValue' });
            expect(prepareFilterResult?.filterDescription?.[0].value).toStrictEqual('testValue');
        });
    });
});
