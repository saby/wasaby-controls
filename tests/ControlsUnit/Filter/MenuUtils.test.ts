...
describe('Controls/_filter/MenuUtils', () => {
    it('prependNewItems', () => {
        const initItems = [
        ...
        const expectedItems = [{ key: 18, title: '18 record' }].concat(initItems.slice(0, 3));

        let resultItems = getItemsWithHistory(items, newItems, sourceController, source, 'key');
        ...
        resultItems = getItemsWithHistory(items, newItems, sourceController, source, 'key');
        expect(resultItems.getCount()).toEqual(4);
    ...
    it('getUniqItems', () => {
        ...
        const resultItems = getUniqItems(oldItems, newItems, 'key');
