define([
   ...
   describe('Filter:View', function () {
      ...
      it('_beforeMount from receivedState', function () {
         ...
         defaultConfig.source[0].editorOptions.selectorTemplate =
            'New Template';
         view._beforeMount(defaultConfig, {}, receivedState);
      });
      ...
      it('_resetFilterText', function () {
         ...
         expect(closed).toBe(true);
         expect(view._source[2].value).toBe('');
         expect(view._source[5].value).toEqual([
            new Date(2019, 5, 1),
            new Date(2019, 5, 31)
         ]);
      });
      ...
      it('_rangeValueChangedHandler', () => {
         ...
         view._rangeValueChangedHandler(
            'rangeChanged',
            new Date(2019, 6, 1),
            new Date(2019, 6, 31)
         );
         expect(view._getDateRangeItem(view._source).value).toEqual([
            new Date(2019, 6, 1),
            new Date(2019, 6, 31)
         ]);
      ...
      it('_rangeTextChangedHandler', () => {
         ...
         expect(view._getDateRangeItem(view._source).textValue).toEqual(
            'Date textValue'
         );
      });
         ...
         let source = [
            {
               ...
               value: [new Date(2019, 7, 1), new Date(2019, 7, 31)],
               textValue: "July'19",
               resetValue: [new Date(2019, 6, 1), new Date(2019, 6, 31)],
               viewMode: 'basic'
            },
            {
               name: 'author',
            ...
            },
            {
               ...
               value: 'test_extended',
               resetValue: '',
               textValue: 'test_extended',
               viewMode: 'extended',
               visibility: true
            },
            {
               name: 'employee',
               value: 'extendedValueVisible',
               textValue: 'extendedValueVisible',
               viewMode: 'extended'
            },
            {
               name: 'frequent',
               value: 'test_frequent',
               textValue: 'test_frequent',
               resetValue: '',
               viewMode: 'frequent'
            },
            {
               name: 'textValueVisible',
               value: 'test_textValueVisible',
               textValue: 'test_textValueVisible',
               resetValue: '',
               textValueVisible: false
            }
         ];
         let view = getView({ source: Clone(defaultSource) });
         view._displayText = {};
         view._updateText(source, {});
         expect(view._filterText).toBe(
            'Author: Ivanov K.K., test_extended, extendedValueVisible'
         );
      });
      ...
      it('_resolveItems', function () {
         ...
         expect(view._source[0].getSQLSerializationMode()).toBe(
      ...
      it('_loadSelectedItems', async function () {
         ...
         let stub = jest
            .spyOn(historyUtils, 'getSourceFilter')
            .mockImplementation();
      ...
      describe('View hierarchyList', function () {
         ...
         it('_setItems', function () {
            ...
            expect(view._configs.document.popupItems.getRawData()).toEqual(
               expectedResult
            );
         ...
         it('loadItemsFromSource', async () => {
            ...
            await view._loadItemsFromSource(
               view._configs.document,
               view._source
            );
