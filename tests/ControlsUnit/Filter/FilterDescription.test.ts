...
describe('Controls/filter:FilterDescription', () => {
    ...
    describe('applyFilterCounter', () => {
        it('Значение фильтра по счётчику записывается в filter связного элемента структуры фильтра', () => {
            const filterDescription = [
                {
                    name: 'testName',
                    value: null,
            ...
            const resultFilterDescription = FilterDescription.applyFilterCounter(
                'testValue',
                filterDescription,
                { countFilterLinkedNames: ['testName', 'testName2'] }
            );
            expect(resultFilterDescription[0].editorOptions.filter.CalcCountBy).toEqual(
                'testValue'
            );
            expect(resultFilterDescription[1].panel.editorOptions.filter.CalcCountBy).toEqual(
                'testValue'
            );
        });
    });
