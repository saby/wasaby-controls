   ...
   describe('Dropdown/Controller', () => {
      ...
      describe('update', function () {
         ...
         it('new source', async () => {
            ...
            expect(dropdownController._items.getCount()).toEqual(
               updatedItems.length
            );
            expect(
               cInstance.instanceOfModule(
                  dropdownController._source,
                  'Types/source:Base'
               )
            ).toBe(true);
            expect(opened).toBe(false);
         ...
         it('change filter', async () => {
            ...
            expect(selectedItems[0].getRawData()).toEqual(
               itemsRecords.at(1).getRawData()
            );
         });
      ...
      it('getItemByKey', () => {
         ...
         let item = dropdownController._getItemByKey(
            itemsWithoutKeyProperty,
            '1',
            'id'
         );
         expect(item.get('title')).toEqual('Запись 1');

         item = dropdownController._getItemByKey(
            itemsWithoutKeyProperty,
            'anyTestId',
            'id'
         );
      ...
      it('loadDependencies, load rejected', () => {
         ...
         jest
            .spyOn(controller, '_loadMenuTemplates')
            .mockClear()
            .mockImplementation(() => {
      ...
      it('_loadItemsTemplates', async () => {
         ...
         await dropdownController._loadItemsTemplates(
            dropdownController,
            config
         );
      ...
      it('_loadItems', async () => {
         ...
         await dropdownController
            ._loadItems(controllerConfig)
            .then(jest.fn(), (error) => {
      ...
      describe('load items by selectedKeys', () => {
         ...
         describe('loadSelectedItems', () => {
            it('loadSelectedItems selectedKey = 8', async () => {
               ...
               expect(dropdownController._selectedItems.at(0).getKey()).toEqual(
                  '8'
               );
               expect(dropdownController._items).toBeNull();
            ...
            it('loadSelectedItems selectedAllKey', async () => {
               ...
               newCfg.selectedItemsChangedCallback = (
         ...
         it('_resolveLoadedItems', () => {
            ...
            const result = dropdownController._resolveLoadedItems(
               newConfig,
               loadedItems
            );
      ...
      it('_private::getItemsTemplates', () => {
         ...
         expect(dropdownController._getItemsTemplates(config)).toEqual([
            'first',
            'second',
            'five'
         ]);
      ...
      describe('menuPopupOptions', () => {
         ...
         it('templateOptions', () => {
            ...
            expect(
               resultPopupConfig.templateOptions.closeButtonVisibility
            ).toBe(true);
            expect(resultPopupConfig.templateOptions.source).toEqual(
               'testSource'
            );
            expect(
               resultPopupConfig.templateOptions.dataLoadCallback
            ).toEqual(dropdownController._options.dataLoadCallback);;
            expect(resultPopupConfig.opener).toEqual('test');
         });
         ...
         it('check keyProperty option', () => {
            ...
            expect(resultPopupConfig.templateOptions.keyProperty).toEqual(
               'copyOriginalId'
            );
      ...
      it('_private::getNewItems', function () {
         ...
         let result = dropdownController._getNewItems(
            curItems,
            selectedItems,
            'id'
         );
      ...
      it('_private::getSourceController', async function () {
         ...
         const sourceController = await dropdownController._getSourceController(
            historyConfig
         );
         expect(
            cInstance.instanceOfModule(
