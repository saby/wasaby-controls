...
describe('Controls/lookup:Input', () => {
    ...
    describe('_beforeMount', () => {
        ...
        it('with source and items but without selectedKeys option', async () => {
            ...
            expect(lookup._items.getCount()).toEqual(data.length);
            expect(lookup._lookupController.getSelectedKeys()).toHaveLength(data.length);
        });
        ...
        it('with dataLoadCallback', async () => {
            ...
            await getBaseLookup({ dataLoadCallback, items } as unknown as ILookupOptions);
    ...
    describe('_selectCallback', () => {
        it('_selectCallback with items', async () => {
            ...
            jest.spyOn(lookup, '_itemsChanged').mockClear().mockImplementation();
            const items = new RecordSet({
                rawData: getData(),
                keyProperty: 'id',
            });
