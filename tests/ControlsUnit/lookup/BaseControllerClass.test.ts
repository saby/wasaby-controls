...
function getLookupControllerWithSelectedKeys(
...
function getLookupControllerWithoutSelectedKeys(
    additionalConfig?: object
): BaseControllerClass {
    let options = getControllerOptions();
...
describe('Controls/_lookup/BaseControllerClass', () => {
        ...
        it('simple loadItems', () => {
            ...
            return new Promise((resolve) => {
                controller.loadItems().then((items) => {
                    ...
                    expect(controller.getItems().getCount()).toBe(
                        resultItemsCount
                    );
                    resolve();
                });
    ...
    describe('update', () => {
        ...
        it('update selectedKeys', async () => {
            ...
            items = await controller.update(
                newOptions as ILookupBaseControllerOptions
            );
        ...
        it('update source', async () => {
            ...
            expect(
                controller.update(newOptions as ILookupBaseControllerOptions)
    ...
    describe('removeItem', () => {
        it('simple removeItem', () => {
            ...
            expect(
                !isEqual(
                    ...
                    controller.getItems().getRawData()
                )
            ).toBeTruthy();
        });
        ...
        it('update after removeItem', async () => {
            ...
            const controller = new BaseControllerClass(
                options as ILookupBaseControllerOptions
            );
            controller.setItems(await controller.loadItems());
    ...
    it('getTextValue', () => {
        ...
        return new Promise((resolve) => {
            controller.loadItems().then((items) => {
                controller.setItems(items);
                expect(controller.getTextValue()).toBe(
                    'Sasha, Aleksey, Dmitry'
                );
                resolve();
            });
        });
    ...
    it('setItemsAndSaveToHistory', async () => {
        ...
        const historyService = await controller.setItemsAndSaveToHistory(
            getRecordSet()
        );
