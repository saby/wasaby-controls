...
describe('_getMenuActions cases', () => {
    ...
    it('-parentAction, =collect only root children', () => {
        ...
        // @ts-ignore
        const expectedCount = config.templateOptions.source.data.filter(
            (action) => {
    ...
    it('should collect only non-toolbar options when no parentAction passed', () => {
        ...
        expect(config.templateOptions.source).toBeDefined();
        // @ts-ignore
        const expectedActions = config.templateOptions.source.data.filter(
            (action) => {
                return action.showType !== TItemActionShowType.TOOLBAR;
            }
        );
        ...
        // @ts-ignore
        const unexpectedActions = config.templateOptions.source.data.filter(
            (action) => {
                return action.showType === TItemActionShowType.TOOLBAR;
            }
        );
    ...
    it('should collect non-"showed" or non-toolbar item actions when item is swiped', () => {
        ...
        expect(config.templateOptions.source).toBeDefined();
        // @ts-ignore
        expect(config.templateOptions.source.data[0]).toEqual(
            localItemActions[1]
        );
        // @ts-ignore
        expect(config.templateOptions.source.data[1]).toEqual(
            localItemActions[3]
        );
    });
