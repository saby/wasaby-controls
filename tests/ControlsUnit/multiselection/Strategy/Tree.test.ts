...
import TreeGridCollection from 'Controls/_treeGridDisplay/TreeGridCollection';
...
function getTreeStrategy(model: Tree): TreeSelectionStrategy {
    return new TreeSelectionStrategy({
...
describe('Controls/_multiselection/SelectionStrategy/Tree', () => {
    ...
    const strategy = getTreeStrategy(model);
    ...
    describe('unselect', () => {
        ...
        it('search model, select node and unselect only one deep inside breadcrumb child', () => {
          ...
          */
            ...
            const res = strategy.getSelectionForModel(selection, undefined, 'sad');
        ...
        it('search model, select breadcrumb and unselect breadcrumb', () => {
          ...
          */
            ...
            const selection = strategy.unselect({ selected: [211], excluded: [] }, 211, 'aaa');
    ...
    describe('selectAll', () => {
        ...
        describe('Отметка всех записей (selectAll) с отключённой массовой отметкой', () => {
            let strategy;

            beforeAll(() => {
                strategy = getTreeStrategy(getModel());
                strategy.setMassSelect(false);
            });

            afterAll(() => {
                strategy.setMassSelect(true);
            });

            it('Отметка всех записей в корне', () => {
                const selection = { selected: [], excluded: [] };
                const resultSelection = strategy.selectAll(selection);
                expect(resultSelection).toEqual({ selected: [3, 4, 5, 6, 7], excluded: [] });
            });

            it('Отметка всех записей в папке', () => {
                const selection = { selected: [], excluded: [] };
                strategy._rootKey = 2;
                const resultSelection = strategy.selectAll(selection);
                expect(resultSelection).toEqual({ selected: [3, 4], excluded: [] });
            });
        });
    });
    ...
    describe('toggleAll', () => {
        ...
        it('selected some items', () => {
            const selection = strategy.toggleAll({ selected: [1, 5], excluded: [] }, false);
            expect(selection.selected).toEqual([null]);
        ...
        it('selected some items, change root, toggle all twice', () => {
            ...
            selection = strategyWithDescendantsAndAncestors.toggleAll(selection, false);
    ...
    describe('selectRange', () => {
        ...
        it('with breadcrumbs', () => {
            ...
            */
            ...
            const selection = strategy.selectRange(searchModel.getItems());
            expect(selection.selected).toEqual([1, 11, 12, 2, 21, 22]);
            expect(selection.excluded).toEqual([]);
        });
    });
    ...
    describe('getSelectionForModel', () => {
        ...
        it('selectAncestors = false; selectedKeys=[null, 2]; excludedKeys=[null, 1, 6, 7]', () => {
            // В тесте рассматривается кейс по ошибке
            // https://online.sbis.ru/opendoc.html?guid=1b86b74f-0151-4745-8527-716aa7ec63a4&client=3
            // selectAncestors = false; выбрать папку с несколькими записями, снять выбор внутри папки с одной
            // записи (id="2"), инвертировать выбор, снять выбор со всех записей, кроме id="2", сделать перезагрузку.
            const model = new Tree({
                collection: new RecordSet({
                    keyProperty: ListData.KEY_PROPERTY,
                    rawData: ListData.getItems(),
            ...
            const selection = { selected: [null, 2], excluded: [null, 1, 6, 7] };
            const res = strategy.getSelectionForModel(selection);

            expect(toArrayKeys(res.get(true))).toEqual([2, 3, 4]);
            expect(toArrayKeys(res.get(null))).toEqual([]);
            expect(toArrayKeys(res.get(false))).toEqual([1, 5, 6, 7]);
        });
        ...
        it('select all, unselect node and go into this node, check that items inside node is not selected', () => {
            const items = new RecordSet({
                rawData: [
                    { id: 11, parent: 1, node: null },
                    { id: 12, parent: 1, node: null },
            ...
            const result = strategy.getSelectionForModel({
                selected: [null],
                excluded: [null, 1],
            });
            expect(toArrayKeys(result.get(true))).toEqual([]);
            expect(toArrayKeys(result.get(false))).toEqual([11, 12]);
            expect(toArrayKeys(result.get(null))).toEqual([]);
        });
        ...
        it('node has selected children deep in entry_path, entry_path reverted', () => {
            ...
            const strategy = new TreeSelectionStrategy({
                ...
                isLoadedUtil: (key) => {
                    return key !== 1 && key !== 11 && key !== 111 && key !== 1111;
                },
            });
        ...
        it('feature1188089336=true', () => {
            ...
            const selection = strategy.select({ selected: [], excluded: [] }, 1, true);
            expect(selection.selected).toEqual([1]);
            expect(selection.excluded).toEqual([1]);
        ...
        it('search model', () => {
          ...
          */
            ...
            res = strategy.getSelectionForModel({ selected: [2], excluded: [] }, undefined, true);
            ...
            res = strategy.getSelectionForModel({ selected: [11], excluded: [] }, undefined, true);
    ...
    describe('isAllSelected', () => {
        it('all selected', () => {
            const selection = { selected: [null], excluded: [null] };
            expect(strategy.isAllSelected(selection, false, 7, null)).toBe(true);
            expect(strategy.isAllSelected(selection, false, 7, null, false)).toBe(true);
        });

        it('all selected and one excluded', () => {
            const selection = { selected: [null], excluded: [null, 2] };
            expect(strategy.isAllSelected(selection, false, 7, null)).toBe(false);
            expect(strategy.isAllSelected(selection, false, 7, null, false)).toBe(true);
        });
        ...
        it('selected current root', () => {
            ...
            expect(strategy.isAllSelected(selection, false, 7, null)).toBe(true);
            expect(strategy.isAllSelected(selection, true, 7, null, false)).toBe(true);
        });

        it('selected by one all elements', () => {
            const selection = { selected: [1, 2, 3, 4, 5, 6, 7], excluded: [] };
            expect(strategy.isAllSelected(selection, false, 7, null)).toBe(true);
            expect(strategy.isAllSelected(selection, true, 7, null, false)).toBe(false);
        });

        it('selected by one all elements and has more data', () => {
            const selection = { selected: [1, 2, 3, 4, 5, 6, 7], excluded: [] };
            expect(strategy.isAllSelected(selection, true, 7, null)).toBe(false);
            expect(strategy.isAllSelected(selection, true, 7, null, false)).toBe(false);
        });
        ...
        it('pass rootKey', () => {
            ...
            expect(strategy.isAllSelected(selection, false, 7, null)).toBe(true);
            expect(strategy.isAllSelected(selection, true, 7, null, false)).toBe(true);
            expect(strategy.isAllSelected(selection, true, 7, null, false, 4)).toBe(false);
            expect(
                strategy.isAllSelected({ selected: [4], excluded: [4] }, true, 7, null, false, 4)
            ).toBe(true);
        });
        ...
        it('selected all root items by one', () => {
            ...
            expect(strategy.isAllSelected(selection, false, 3, null, true)).toBe(true);
        });
        ...
        it('selected all root items by one, one node is expanded', () => {
            ...
            const selection = { selected: [1, 2, 3], excluded: [] };
            expect(strategy.isAllSelected(selection, false, 4, null, true)).toBe(true);
        });
        ...
        it('selected all and has partial selected node', () => {
            ...
            expect(strategy.isAllSelected(selection, false, 5, null, true)).toBe(false);
        });
    });
    ...
    describe('recursiveSelection', () => {
        describe('leaf', () => {
            ...
            it('getSelectionForModel', () => {
                const selection = { selected: [null], excluded: [null] };
                const res = strategy.getSelectionForModel(selection);
                expect(toArrayKeys(res.get(true))).toEqual([1, 2, 3, 4, 5, 6, 7]);
                expect(toArrayKeys(res.get(null))).toEqual([]);
                expect(toArrayKeys(res.get(false))).toEqual([]);
            });
