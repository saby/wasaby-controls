...
describe('Controls/_multiselection/Controller', () => {
    ...
    describe('updateOptions', () => {
        ...
        it('should reset selection after change items and model', () => {
            controller.setSelection({ selected: [null], excluded: [] });
            ...
            const result = controller.updateOptions({
                ...
                excludedKeys: [],
                searchMode: undefined,
                strategyOptions: { model: newModel },
            });
    ...
    describe('selectRange', () => {
        ...
        it('expand empty node, then select range', () => {
            const tree = new TreeGridCollection({
                collection: new RecordSet({
                    keyProperty: ListData.KEY_PROPERTY,
                    rawData: ListData.getItemsWithEmptyNode(),
                }),
                root: null,
                keyProperty: ListData.KEY_PROPERTY,
                parentProperty: ListData.PARENT_PROPERTY,
            ...
            tree.setExpandedItems([4]);

            controller.toggleItem(1);
            const result = controller.selectRange(6);
            expect(result).toEqual({ selected: [1, 4, 5, 6], excluded: [] });
        });
    });
    ...
    describe('getSelectedItems', () => {
        ...
        it('should return one selected item', () => {
            controller.setSelection({
                selected: [model.at(0).getContents().getKey()],
                excluded: [],
            });
        ...
        it('should return two selected items', () => {
            controller.setSelection({
                selected: [model.at(0).getContents().getKey(), model.at(1).getContents().getKey()],
                excluded: [],
            });
            const items = controller.getSelectedItems();
