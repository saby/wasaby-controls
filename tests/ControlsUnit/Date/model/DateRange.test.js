define(['Core/core-instance', 'Controls/dateRange'], function (
   cInstance,
   dateRange
) {
   'use strict';
   ...
   describe('Controls/dateRange:DateRangeModel', function () {
      ...
      describe('.shiftForward', function () {
         [
            ...
            it('should shift forward with displayedRanges', function () {
               ...
               expect(test.startValue.getTime()).not.toEqual(
                  model.startValue.getTime()
               );
               expect(test.endValue.getTime()).not.toEqual(
                  model.endValue.getTime()
               );
            });
         });
         ...
         [
            ...
            it('should not shift forward with displayedRanges', function () {
               ...
               expect(test.startValue.getTime()).toEqual(
                  model.startValue.getTime()
               );
               expect(test.endValue.getTime()).toEqual(
                  model.endValue.getTime()
               );
            });
         });
         [
            ...
            it('should shift period forward', function () {
               ...
               expect(model.startValue.getTime()).toEqual(
                  test.rStart.getTime()
               );
         ...
         [
            ...
            it('should shift period with quantum forward', function () {
               ...
               model.shiftForward();
               expect(model.startValue.getTime()).toEqual(
                  test.result[0].getTime()
               );
               cInstance.instanceOfModule(
                  model.startValue,
                  'Types/entity:DateTime'
               );
               expect(model.endValue.getTime()).toEqual(
                  test.result[1].getTime()
               );
               cInstance.instanceOfModule(
                  model.endValue,
                  'Types/entity:DateTime'
      ...
      describe('.setRange', function () {
         it('should make notification about changes of startValue and endValue', function (done) {
            ...
            setTimeout(function () {
               expect(model._notify).toHaveBeenCalledWith(
                  'rangeChanged',
                  expect.anything()
               );
               done();
