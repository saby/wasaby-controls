   ...
   describe('Controls/Date/MonthView', function () {
      describe('Initialisation', function () {
         it('should create the correct model for the month when creating', function () {
            ...
            mv = calendarTestUtils.createComponentReact(calendar.MonthView, config);
      ...
      describe('_dayClickHandler', function () {
         it('should generate "itemClick" event', function () {
            let mv = calendarTestUtils.createComponentReact(calendar.MonthView, config),
               item = 'item',
               event = 'event';

            jest.spyOn(mv.props, 'onItemClick').mockClear().mockImplementation();
            [
               {
               ...
               expect(mv.props.onItemClick).toHaveBeenCalledWith(event, item, event, false, true);
            });
         });
      ...
      describe('_mouseEnterHandler', function () {
         [
            ...
            it('should generate "itemMouseEnter" event. ' + test.title, function () {
               let item = 'item',
                  mv = calendarTestUtils.createComponentReact(calendar.MonthView, {
                     ...config,
                     mode: test.mode
                  });

               jest.spyOn(mv.props, 'onItemMouseEnter').mockClear().mockImplementation();
               mv._mouseEnterHandler({}, item, test.isCurrentMonth);
               expect(mv.props.onItemMouseEnter).toHaveBeenCalledWith({}, item);
            });
         });
      ...
      describe('_updateView', function () {
         it('should update month and month caption when month changed', function () {
            const mv = calendarTestUtils.createComponentReact(calendar.MonthView, config),
