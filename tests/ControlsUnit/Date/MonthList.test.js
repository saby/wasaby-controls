   ...
   describe('Controls/_calendar/MonthList', function () {
      describe('constructor', function () {
         it('default options', function () {
            let self = calendarTestUtils.createComponentReact(calendar.MonthList, config);
            ...
            expect(+self._lastNotifiedPositionChangedDate).toStrictEqual(+config.position);
         });
         ...
         it('viewMode = "year", position = 2020.03.01', function () {
            ...
            expect(+self._lastNotifiedPositionChangedDate).toStrictEqual(+new Date(2020, 0, 1));
         });
         ...
         it('should initialize _extDataLastVersion if source option passed', function () {
            ...
            jest.spyOn(control, '_enrichItemsDebounced').mockClear().mockImplementation();
            expect(control._extDataLastVersion).toBe(control._extData.getVersion());
         ...
         it('with getInitialData', function () {
            ...
            let ml = calendarTestUtils.createComponentReact(calendar.MonthList, {
               source: { getInitialData: () => new collection.RecordSet() }
            });
      ...
      describe('componentDidMount', function () {
         ...
         it('should not reset lastPositionFromOptions if positionToScroll is different', function () {
            let control = calendarTestUtils.createComponentReact(calendar.MonthList, {
               position: new Date(2017, 1, 1)
            });

            jest.spyOn(control, '_canScroll').mockClear().mockReturnValue([true]);
            jest.spyOn(control, '_scrollToDate').mockClear().mockReturnValue(true);
            ...
            expect(control._positionToScroll).toEqual(control._positionToScroll);
            jest.restoreAllMocks();
         });
      ...
      describe('shouldComponentUpdate', function () {
         it('should update position fields if position changed', function () {
            ...
            expect(DateUtil.Base.isDatesEqual(self._positionToScroll, position)).toBe(true);
            expect(self._displayedPosition).toStrictEqual(position);
            expect(self._startPositionId).toStrictEqual('2018-01-01');
         ...
         it('should not update the position before the list is updated', function () {
            ...
            jest.spyOn(self, '_isHidden').mockClear().mockReturnValue(false);
            jest.spyOn(self, '_findElementByDate').mockClear().mockReturnValue(null);
            self._listRef = { current: { reload: jest.fn() } };
            self._container = {};
         ...
         it('should update source if changed', function () {
            ...
            const ml = calendarTestUtils.createComponentReact(calendar.MonthList, {});
            ml.shouldComponentUpdate({ source: oldSource });

            const stub = jest.spyOn(ml, '_enrichItems').mockClear().mockImplementation();
      ...
      describe('componentDidUpdate', function () {
         it("should notify 'enrichItems' event if model has been changed", function () {
            const component = calendarTestUtils.createComponentReact(calendar.MonthList, {
               ...config,
               onEnrichItems: jest.fn()
            });

            jest.spyOn(component, '_enrichItemsDebounced').mockClear().mockImplementation();
      ...
      describe('_drawItemsHandler', function () {
         it('should set the position if the shouldComponentUpdate function has been called several times', function () {
            ...
            jest.spyOn(self, '_canScroll').mockClear().mockReturnValue([true]);
            self._container = {};
            jest.spyOn(self, '_scrollToPosition').mockClear().mockImplementation();
            self.shouldComponentUpdate(
               calendarTestUtils.prepareOptions(calendar.MonthList, {
                  position: position1
               })
      ...
      describe('_scrollToPosition', function () {
         it('should update position in the list and reload list if it cannot scroll', function () {
            ...
            expect(Object.keys(self._displayedDates)).toHaveLength(0);
            expect(self._startPositionId).toStrictEqual('2018-01-01');
            expect(+self._lastNotifiedPositionChangedDate).toStrictEqual(+position);
         ...
         it('should scroll to days.', function () {
            ...
            expect(self._startPositionId).toStrictEqual('2018-01-01');
            expect(self._positionToScroll).toStrictEqual(position);
            expect(+self._lastNotifiedPositionChangedDate).toStrictEqual(+new Date(2018, 0, 1));
            expect(self._listRef.current.reload).not.toHaveBeenCalled();
            jest.restoreAllMocks();
         });
      });
      ...
      describe('_intersectHandler', function () {
         [
            ...
            it(test.title, function () {
               const component = calendarTestUtils.createComponentReact(
                  ...
                  coreMerge(
                     test.options,
                     { ...config, onPositionChanged: jest.fn() },
                     { preferSource: true }
                  )
               );
               ...
               expect(component.props.onPositionChanged).toHaveBeenCalledWith(event, test.date);
               jest.restoreAllMocks();
            });
         });

         it("Should't update position and displayed items if component invisible", function () {
            const component = calendarTestUtils.createComponentReact(calendar.MonthList, config);

            component._container = { offsetParent: null };
            jest.spyOn(component, '_updateDisplayedItems').mockClear().mockImplementation();
            jest.spyOn(component, '_updateDisplayedPosition').mockClear().mockImplementation();
         ...
         [
            ...
            it(test.title, function () {
               ...
               jest.spyOn(component, '_updateSource').mockClear().mockImplementation();
               component._extData = {
               ...
               component._intersectHandler(test.entries);
               expect(component._displayedDates).toEqual(test.resultDisplayedDates);
               jest.restoreAllMocks();
            });
         });
      });
      ...
      describe('_canScroll', function () {
         [
            ...
            it(test.title, function () {
               ...
               jest.spyOn(control, '_findElementByDate').mockClear().mockReturnValue({});
      ...
      describe('_calculateInitialShadowVisibility', () => {
         [
            ...
            it(`should return correct shadowVisibility value ${index}`, () => {
               const control = calendarTestUtils.createComponentReact(calendar.MonthList, {});
