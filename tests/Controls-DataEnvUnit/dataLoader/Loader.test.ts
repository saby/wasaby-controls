...
describe('Controls-DataEnv/dataLoader:Loader', () => {
    ...
    describe('loadByConfigs', () => {
        ...
        it('dataContext доставляется до configGetter, зависимых от глобальных объектов в контексте данных', async () => {
            const config = {
                root: {
                    configGetter: 'Controls-DataEnvUnit/dataLoader/configLoaders:rootConfig',
                    configGetterArguments: {},
                },
                depData: {
                    configGetter:
                        'Controls-DataEnvUnit/dataLoader/configLoaders:configWithUsingDataContext',
                    configGetterArguments: {},
                },
            };
            const loadResult = await Loader.loadByConfigs(config);
            expect(loadResult.loadResults.depData.Data).toBe('Вася');
        });
    });
});
