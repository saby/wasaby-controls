...
import * as InitOperationsPanel from 'Controls-DataEnv/newLists/_list/ListWebInitializer/operationsPanel';
...
describe('Controls/ListSliceSnapshots', () => {
    ...
    describe('Тесты поиска', () => {
        ...
        it('Если выделены все записи, то при сбросе поиска выделение сбрасывается', async () => {
            const { Slice, Component } = useUpdateVisualizer({
                deps: ['items', 'selectionModel', 'selectedKeys', 'searchValue', 'loading'],
                replacer: (key, sliceState) => {
                    if (sliceState && key === 'items' && sliceState.items) {
                        return sliceState.items.getCount();
                    }
                    if (sliceState && key === 'selectionModel' && sliceState.selectionModel) {
                        let selectedCount = 0;
                        sliceState.selectionModel.forEach((value) => {
                            if (value) {
                                selectedCount++;
                            }
                        });
                        return `выделено ${selectedCount}`;
                    }
                },
                Slice: ListSlice,
            });
            ...
            source.query = jest.fn().mockImplementation(() => {
                return Promise.resolve(
                    new RecordSet({
                        keyProperty: 'key',
                        rawData: hierarchyItems.slice(0, 6),
                    })
                );
            });
            const slice = new Slice({
        ...
        it('Если выделена конкретная запись, то после сброса поиска выделение сохраняется', async () => {
            const { Slice, Component } = useUpdateVisualizer({
                deps: ['items', 'selectionModel', 'selectedKeys', 'searchValue', 'loading'],
                replacer: (key, sliceState) => {
                    if (sliceState && key === 'items' && sliceState.items) {
    ...
    describe('Панель массовых операций.', () => {
        it('Если при инициализации состояния нельзя посчитать счетчик на открытии пмо, необходимо отобразить последнее сохраненное значение', async () => {
            const mockGetControllerState = jest
                .spyOn(DataSource, 'getControllerState')
                .mockImplementation(() => ({
                    count: 13,
                }));
            const mockGetCount = jest
                .spyOn(InitOperationsPanel, 'getCount')
                .mockImplementation(() => null);

            const { Slice, Component } = useUpdateVisualizer({
                deps: ['operationsPanelVisible', 'listActions', 'selectedKeys', 'count'],
                Slice: ListSlice,
            });
            new Slice({
                config: {
                    ...
                    collectionType: 'Tree',
                    selectedKeys: [0],
                    listActions: [{ id: 'name', actionName: 'Controls/testName' }],
                    listConfigStoreId: 'listConfigStoreId',
                },
                loadResult: {
                    items: new RecordSet({
                        keyProperty: 'key',
            ...
            expect(asFragment()).toMatchSnapshot();
            mockGetControllerState.mockClear();
