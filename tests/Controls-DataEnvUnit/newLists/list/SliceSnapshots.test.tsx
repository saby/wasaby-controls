...
describe('Controls/ListSliceSnapshots', () => {
    ...
    describe('Множественное выделение.', () => {
        it('Можно сбросить все выделение, если инвертировать, когда выделены все элементы', async () => {
            const { Slice, Component } = useUpdateVisualizer({
                deps: ['items', 'selectionModel', 'selectedKeys'],
                replacer: (key, sliceState) => {
                    if (sliceState && key === 'items' && sliceState.items) {
                        return sliceState.items.getCount();
                    }
    ...
    describe('Панель массовых операций.', () => {
        it('Если при инициализации состояния есть список экшенов и выбранные элементы, тогда должна открыться пмо с корректным счетчиком', async () => {
            const { Slice, Component } = useUpdateVisualizer({
