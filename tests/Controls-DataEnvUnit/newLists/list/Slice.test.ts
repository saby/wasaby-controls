...
import { DataSet, HierarchicalMemory, Memory, Query } from 'Types/source';
import 'Controls/search';
import 'Controls/operations';
import 'Controls/list';
import { getControllerState, NewSourceController, saveControllerState } from 'Controls/dataSource';
...
describe('Controls/dataFactory:ListSlice', () => {
    describe('Инициализация', () => {
        ...
        it('Маркер корректно встает на запись с ключом 0, если markerVisibility: visible', () => {
            ...
            expect(slice.state.markedKey).toEqual(0);
        });
        ...
        describe('Инициализация поиска', () => {
            it('Состояние поиска не инициализируется без searchParam', () => {
                ...
                expect(state.searchDelay).toEqual(300);
                expect(state.searchValueTrim).toEqual(true);
                expect(state.searchStartingWith).toEqual('current');
            });
    ...
    describe('Поиск', () => {
        ...
        // TODO: Должно сыпать ошибкой в консоль
        // it('Вызов поиска без searchParam приводит к ошибке в runtime', async () => {
        //     const source = new Memory();
        //     const items = new RecordSet();
        //     const onChange = () => {};
        //     const slice = new ListSlice({
...
describe('Открытие и закрытие ПМО', () => {
    ...
    describe('Обновление listCommandsSelection при открытии ПМО', () => {
        it('Первый элемент должен попадать в выбранные', () => {
            const source = getMemory();
            const items = new RecordSet({
                rawData: flatData,
                keyProperty: 'key',
            ...
            return new Promise((resolve1) => {
                ...
                expect(slice.state.operationsPanelVisible).toBeTruthy();
                expect(slice.state.multiSelectVisibility).toBe('visible');
                expect(slice.state.markedKey).toBe(flatData[0].key);
                expect(slice.state.listCommandsSelection?.selected).toHaveLength(1);
            });
        });

        it('Элемент с checkboxAccessibility false не должен попадать в выбранные', () => {
            const data = [
                {
                    key: 'Line',
                    name: 'Line',
            ...
            return new Promise((resolve1) => {
                ...
                expect(slice.state.operationsPanelVisible).toBeTruthy();
                expect(slice.state.multiSelectVisibility).toBe('visible');
                expect(slice.state.multiSelectAccessibilityProperty).toBe('checkboxAccessibility');
                expect(slice.state.markedKey).toBe('Line');
                expect(slice.state.listCommandsSelection?.selected).toHaveLength(0);
            });
        });
    });
});
...
describe('тесты beforeApplyState', () => {
    it('отмена изменений в прикладном слайсе', () => {
        const source = getMemory();
        const items = new RecordSet({
            rawData: [...hierarchyItems],
            keyProperty: 'key',
        ...
        return new Promise((resolve1) => {
            class CustomSlice extends ListSlice {
                protected async _beforeApplyState(nextStateProp: IListState): Promise<IListState> {
                    if (
                        nextStateProp.expandedItems.length === 0 &&
                        nextStateProp.collapsedItems.length === 0
                    ) {
                        nextStateProp.expandedItems = [0, 1];
                    }
                    return super._beforeApplyState(nextStateProp);
                }
            }

            const slice = new CustomSlice({
                onChange: () => {
                    resolve1(slice);
                },
                config: {
            ...
            expect(slice.state.expandedItems).toStrictEqual([0]);
            expect(slice.state.collapsedItems).toStrictEqual([]);
        });
    });
    it('в прикладной bas попадает рассчитанное по экшенам состояние', () => {
        const source = getMemory();
        const items = new RecordSet({
            rawData: [...hierarchyItems],
            keyProperty: 'key',
        ...
        return new Promise((resolve1) => {
            class CustomSlice extends ListSlice {
                protected async _beforeApplyState(nextStateProp: IListState): Promise<IListState> {
                    if (nextStateProp.markedKey !== undefined) {
                        if (nextStateProp.markedKey !== 1) {
                            nextStateProp.markedKey = 2;
                        }
                        resolve1(nextStateProp);
                    }
                    return super._beforeApplyState(nextStateProp);
                }
            }
            ...
            expect(basState.markedKey).toEqual(1);
        });
    });
});
...
describe('setState', () => {
    it('передан объект', () => {
        ...
        return new Promise((resolve1) => {
            ...
            slice.setState({ markedKey: 1, testValue: 'test' });
        }).then((slice) => {
            expect(slice.state.markedKey).toStrictEqual(1);
            expect(slice.state.testValue).toStrictEqual('test');
        });
    });
    it('передана функция', () => {
        const source = getMemory();
    ...
    describe('тесты очереди', () => {
        const source = getMemory();
        const items = new RecordSet({
            rawData: [...hierarchyItems],
            keyProperty: 'key',
        });
        const getSlice = (onChange) => {
            return new ListSlice({
                onChange,
                config: {
                    source,
        ...
        it('переданные объекты накладываются друг на друга', () => {
            return new Promise((resolve1) => {
                const slice = getSlice(() => {
                    if (slice.state.markedKey === 5) {
                        resolve1(slice);
                    }
                });
                slice.setState({
                ...
                expect(slice.state.markedKey).toStrictEqual(5);
                expect(slice.state.testProperty).toStrictEqual('test');
                expect(slice.state.testProperty2).toStrictEqual('test');
            });
        });
        it('колбэки накладывают частичное состояние поверх друг друга', () => {
            return new Promise((resolve1) => {
                const slice = getSlice(() => {
                    if (slice.state.markedKey === 5) {
                        resolve1(slice);
        ...
        it('колбэк получает актуальное состояние', () => {
            return new Promise((resolve1) => {
                const slice = getSlice(() => {
                    if (slice.state.markedKey === 5) {
                        resolve1(slice);
                ...
                slice.setState((currentState) => ({
                    testProperty2: currentState.testProperty,
                    markedKey: 5,
                }));
            }).then((slice) => {
                expect(slice.state.markedKey).toStrictEqual(5);
