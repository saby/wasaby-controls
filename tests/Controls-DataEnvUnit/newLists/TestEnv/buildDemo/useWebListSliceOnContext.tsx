...
export type THookProps<TSlice extends ListSlice> = {
    Slice?: TSliceCtor<TSlice>;
    dataFactoryArguments: IListDataFactoryArguments;
 ...
 */
function useSlicePrepareConfig<TSlice extends ListSlice>(
    sliceCtor: TSliceCtor<TSlice>,
    dataFactoryArguments: IListDataFactoryArguments
) {
    ...
    useEffect(() => {
        ...
        const origin = factory.slice;
        factory.slice = sliceCtor;

        return () => {
            factory.slice = origin;
        };
    }, []);
...
export function useWebListSliceOnContext<TSlice extends ListSlice>({
    Slice = ListSlice as TSliceCtor<TSlice>,
    dataFactoryArguments,
    Component: ChildComponent,
    actions,
}: THookProps<TSlice>): THookResult<TSlice> {
            ...
            if (!renderExtraAwaiter.current) {
                ...
                throw Error(
                    'Множественное построение!\n' +
                        'Демо пример несколько раз мытается зарегистриовать промис о завершении построения!'
                );
            }
        }, [])
    );
    ...
    const { storeId, configs, loadResults } = useSlicePrepareConfig(Slice, dataFactoryArguments);
