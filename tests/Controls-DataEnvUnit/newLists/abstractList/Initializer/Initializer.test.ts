...
import { IAbstractListDataFactoryArguments } from 'Controls-DataEnv/newLists/_abstractList/interface/factory/IAbstractListDataFactoryArguments';
...
describe('Controls-DataEnv/abstractList:Initializer', () => {
    ...
    describe('hierarchy', () => {
        ...
        it('Модель раскрытых узлов инициализируется со значениями состояния узлов из конфига', () => {
            ...
            const expectedExpansionModel = new Map(
                hierarchyItems.map((item) => [item.key, item.key === 0])
            );
            const initState = Initializer.getState(loadResult, customConfig);

            expect(initState.expansionModel).toEqual(expectedExpansionModel);
        });
        it('Если данных нет, то инициализируется пустая модель', () => {
            const customConfig = {
                ...hierarchyConfig,
                items: undefined,
                expandedItems: [],
            ...
            expect(initState.expansionModel).toEqual(new Map());
        });
        it('Если нет раскрытых узлов, то для каждого элемента в модели устанавливается значение по умолчанию', () => {
            const customConfig = {
                ...hierarchyConfig,
                expandedItems: [],
            };
            const expectedExpansionModel = new Map(hierarchyItems.map((item) => [item.key, false]));
            const initState = Initializer.getState(loadResult, customConfig);

            expect(initState.expansionModel).toEqual(expectedExpansionModel);
        });
        it('Можно раскрыть все узлы, передав null в expandedItems', () => {
            const customConfig = {
                ...hierarchyConfig,
                expandedItems: [null],
            };
            const expectedExpansionModel = new Map(hierarchyItems.map((item) => [item.key, true]));
            const initState = Initializer.getState(loadResult, customConfig);

            expect(initState.expansionModel).toEqual(expectedExpansionModel);
        });
        it('При раскрытии всех элементов можно свернуть конкретный узел, передав его ключ в collapsedItems', () => {
            const customConfig = {
                ...hierarchyConfig,
                expandedItems: [null],
                collapsedItems: [0],
    ...
    describe('Group.', () => {
        it('groupProperty можно установить через конфиг', async () => {
            const customConfig = {
                ...config,
                groupProperty: 'group',
            };
            ...
            expect(initState.groupProperty).toEqual(customConfig.groupProperty);
        });
    });

    describe('ViewMode.', () => {
        it('Если передан непустой searchValue, то по умолчанию устанавливается viewMode: search', async () => {
            const customConfig: IAbstractListDataFactoryArguments = {
                ...config,
            ...
            expect(initState.viewMode).toEqual('search');
        });
        it('По умолчанию viewMode устанавливается в значение table', async () => {
            const customConfig: IAbstractListDataFactoryArguments = {
                ...config,
            };
            const initState = Initializer.getState(loadResult, customConfig);

            expect(initState.viewMode).toEqual('table');
        });
    });

    describe('EmptyView', () => {
        it('Если данных нет, необходимо показать заглушку', async () => {
            const customConfig: IAbstractListDataFactoryArguments = {
                ...config,
            };
            const initState = Initializer.getState(loadResult, customConfig);

            expect(initState.needShowStub).toBeTruthy();
        });
        it('Если данные есть, заглушка не должна показываться', async () => {
            const customConfig: IAbstractListDataFactoryArguments = {
                ...config,
                items: new RecordSet({
                    keyProperty: 'key',
            ...
            expect(initState.needShowStub).toBeFalsy();
        });
    });

    describe('Search', () => {
        it('Состояние поиска не инициализируется без searchParam.', async () => {
            const customConfig: IAbstractListDataFactoryArguments = {
                ...config,
                markedKey: '1',
                searchValue: 'Саша',
            ...
            expect(initState.searchInputValue).toEqual('');
        });

        it('Состояние поиска инициализируется если задан searchParam.', async () => {
            const customConfig: IAbstractListDataFactoryArguments = {
                ...config,
                markedKey: '1',
                searchValue: 'Саша',
            ...
            expect(initState.searchInputValue).toEqual('Саша');
        });
    });
});
