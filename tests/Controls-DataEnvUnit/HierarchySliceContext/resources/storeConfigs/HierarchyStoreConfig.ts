...
import { IDataContextAPI } from 'Controls-DataEnv/dataContext';
...
export const dependencyConfigGetter = {
    getConfig(configArguments, dataContext: IDataContextAPI): TDataConfigs {
        return {
            simpleElement: {
                dataFactoryName:
                    'Controls-DataEnvUnit/HierarchySliceContext/resources/dataFactories/withSliceArgumentsOnState',
                dataFactoryArguments: {
                    userName: dataContext.getValue('firstName', ['User']),
                    user: dataContext.getObject('Employee'),
                    notExistObject: dataContext.getObject('notExitstType'),
                },
            },
        };
    },
