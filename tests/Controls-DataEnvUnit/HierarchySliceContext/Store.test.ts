...
import Store from 'Controls-DataEnv/_context/store/Store';
...
import ContextNode from 'Controls-DataEnv/_context/store/ContextNode';
...
describe('Controls-DataEnv/_context/HierarchySliceContext/Store', () => {
    ...
    describe('Обновление значения', () => {
        ...
        it('updateRoot', () => {
            ...
            store.createRoot(['root', 'lvl2'], 'rootNode', {
                configGetter:
                    'Controls-DataEnvUnit/HierarchySliceContext/resources/storeConfigs/HierarchyStoreConfig:simpleConfigGetter',
                configGetterArguments: {},
                data: {
            ...
            const newRoot = store.getNodeByPath(['root', 'lvl2', 'rootNode']);
            const newRootSlice = newRoot?.getValue()?.simpleElement;

            expect(newRoot).toBeInstanceOf(ContextNode);
            expect(newRootSlice).toBeInstanceOf(Slice);
            const lvl2ElementOld = store.getNodeValueByPath(['root', 'lvl2'])?.simpleElement;
            const lvl3ElementOld = store.getNodeValueByPath(['root', 'lvl2', 'lvl3'])
                ?.simpleElement;
            const lvl4ElementOld = store.getNodeValueByPath(['root', 'lvl2', 'lvl3', 'lvl4'])
                ?.simpleElement;

            store.updateRoot(['root', 'lvl2'], {
                configGetter:
                    'Controls-DataEnvUnit/HierarchySliceContext/resources/storeConfigs/HierarchyStoreConfig:simpleConfigGetter',
                configGetterArguments: {},
                data: {
            ...
            const lvl2Element = store.getNodeValueByPath(['root', 'lvl2'])?.simpleElement;
            const lvl3Element = store.getNodeValueByPath(['root', 'lvl2', 'lvl3'])?.simpleElement;
            const lvl4Element = store.getNodeValueByPath(['root', 'lvl2', 'lvl3', 'lvl4'])
                ?.simpleElement;

            expect(lvl2ElementOld.isDestroyed()).toBeTruthy();
            expect(lvl3ElementOld.isDestroyed()).toBeTruthy();
            expect(lvl4ElementOld.isDestroyed()).toBeTruthy();

            expect(lvl2Element.isDestroyed()).toBeFalsy();

            expect(lvl3Element).toBeUndefined();
            expect(lvl4Element).toBeUndefined();

            const newRootAfterUpdate = store.getNodeByPath(['root', 'lvl2', 'rootNode']);
            const newRootSliceAfterUpdate = newRootAfterUpdate?.getValue()?.simpleElement;

            expect(newRootSlice).toStrictEqual(newRootSliceAfterUpdate);
            expect(newRootSliceAfterUpdate.isDestroyed()).toBeFalsy();
        });
    });

    it('updateRoot с режимом частичного обновления', () => {
        const contextConfigs = {
            configGetter:
                'Controls-DataEnvUnit/HierarchySliceContext/resources/storeConfigs/HierarchyStoreConfig:simpleConfigGetter',
            configGetterArguments: {},
        ...
        const lvl2ElementOld = store.getNodeValueByPath(['root', 'lvl2'])?.simpleElement;
        const lvl3ElementOld = store.getNodeValueByPath(['root', 'lvl2', 'lvl3'])?.simpleElement;
        const lvl4ElementOld = store.getNodeValueByPath(['root', 'lvl2', 'lvl3', 'lvl4'])
            ?.simpleElement;

        const newConfigs = clone(contextConfigs);
        newConfigs.children.lvl2.data.simpleElement = { newData: 2 };
        newConfigs.children.lvl2.children.lvl3.children.lvl4.data.simpleElement = {
            newDataElement4: 3,
        };
        store.updateRoot(['root'], newConfigs, true);
        const lvl2Element = store.getNodeValueByPath(['root', 'lvl2'])?.simpleElement;
        const lvl3Element = store.getNodeValueByPath(['root', 'lvl2', 'lvl3'])?.simpleElement;
        const lvl4Element = store.getNodeValueByPath(['root', 'lvl2', 'lvl3', 'lvl4'])
            ?.simpleElement;
        ...
        expect(lvl2ElementOld.isDestroyed()).toBeTruthy();
        expect(lvl4ElementOld.isDestroyed()).toBeTruthy();
        expect(!!lvl3ElementOld.isDestroyed()).toBeFalsy();
        expect(lvl3Element).toStrictEqual(lvl3ElementOld);
    });
