...
import * as template from 'wml!Controls-Name-demo/Demo/Demo';
...
class Demo extends Control<IControlOptions> {
    ...
    private _searchParam: string = 'value';
    ...
    protected copyItems: object[] = [];
    private _suggestSource: Memory = null;

    _beforeMount(): void {
        this._nameFields = ['lastName', 'firstName', 'middleName'];
        ...
        this._suggestSource = new Memory({
            ...
            filter: (item, queryFilter: Record<string, string | unknown>) => {
                let addToData = true;
                // значение контрола
                const filterValue = queryFilter[this._searchParam];
    ...
    _copyClickHandler(event: SyntheticEvent<Event>, childrenName: string): void {
