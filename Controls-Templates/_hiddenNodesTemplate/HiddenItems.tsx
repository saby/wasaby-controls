...
export function HeaderColumn(props) {
    return <HeaderContent {...props} className={'controls-Grid__cell_spacing_money'} />;
}

/**
 * Интерфейс для опций контрола {@link Controls-Templates/HiddenItems}.
 * @interface Controls-Templates/IHiddenItemsProps
 * @implements UI/Base/IComponentProps
 * @public
 */
export interface IHiddenItemsProps extends IComponentProps {
        ...
        <div className={'tw-flex tw-w-full'}>
            ...
            >
                {hiddenPrices.map((price: number, index) => {
                    ...
                    return (
                        <div
                            className={'Controls-Templates-HiddenItems-CardWrapper tw-w-full'}
                            key={index.toString()}
                        >
                            <ButtonControl
                                readOnly={false}
                                ...
                                {...buttonsOptions}
                            />
                            <MoneyViewer fontSize="xl" fontWeight="bold" value={price} />
                        </div>
                    );
...
function Template(props: IComponentProps) {
