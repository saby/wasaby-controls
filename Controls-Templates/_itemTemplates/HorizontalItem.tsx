...
import {
    EDITOR_SELECTOR_CAPTION,
    EDITOR_SELECTOR_DESCRIPTION,
} from 'Controls-Templates/_itemTemplates/common/constants';
import { useNoZenTheme } from './common/useNoZenTheme';

/**
 * @typedef {String} Controls-Templates/HorizontalItem/IHorizontalItemImageProps/THorizontalImageSize
 * @description Линейка размеров горизонтального изображения.
 * @variant s
 * @variant m
 * @variant l
 * @variant xlt
 ...
 * @author Колесов В.А.
 */
export type THorizontalImageSize = TImageSize | 'half';

/**
 * Варианты значений эффекта изображения плитки
 * @typedef {String} Controls-Templates/HorizontalItem/IHorizontalItemImageProps/THorizontalTileImageEffect
 ...
 * @see gradientColor
 */
export type THorizontalTileImageEffect = 'none' | 'custom';
...
export interface IHorizontalItemImageProps extends IImageProps {
    ...
    /**
     * @cfg {Controls/interface/THorizontalPosition | 'left-fit' | 'right-fit'} Положение изображения
     */
    imagePosition?: THorizontalPosition | 'left-fit' | 'right-fit';
    /**
     * @cfg {Controls-Templates/HorizontalItem/IHorizontalItemImageProps/THorizontalImageSize.typedef} Размер изображения
     */
    imageSize?: THorizontalImageSize;
    /**
     * Эффект на изображении.
     * @cfg {Controls-Templates/HorizontalItem/IHorizontalItemImageProps/THorizontalTileImageEffect}
     */
    imageEffect?: THorizontalTileImageEffect;
    /**
     * @cfg {ReactElement} Шаблон, отображаемый на месте изображения.
     */
...
export interface IHorizontalItemProps
    ...
    /**
     * Находится ли шаблон внутри реакт рендера плитки
     */
    isReactTile?: boolean;
    isEditing?: boolean;
}
...
function getItemClassName(
    ...
    if (dominantColor && (imageEffect === 'custom' || backgroundColorStyle === 'custom')) {
        if (noZen) {
            classes += ` t-${brightness}`;
        } else {
...
function getCaptionWrapperClassName(props: IHorizontalItemProps): string {
    let classes = ' Controls-Templates-HorizontalItem__caption ';
...
function getCaptionClassName(props: IHorizontalItemProps): string {
    let classes = `${EDITOR_SELECTOR_CAPTION}`;
    if (props.captionClass) {
        classes += ` ${props.captionClass}`;
    }
    return classes;
}
...
function getItemStyles(props: IHorizontalItemProps): React.CSSProperties {
    let styles: React.CSSProperties =
        props.styleProp || (!!props.style && typeof props.style === 'object' && props.style) || {};
    if (
        props.dominantColor &&
        (props.imageEffect === 'custom' || props.backgroundColorStyle === 'custom')
    ) {
        styles = {
            ...styles,
            ...calculateVariables(
...
export function HorizontalItem(
    ...
    const noZen = useNoZenTheme();
    return (
        <BaseItem
            className={getItemClassName(props, noZen)}
        ...
        >
            <div className={'Controls-Templates-TileItem__content_as_column'}>
                ...
                <div className={getContentClassName()}>
                    {props.imageViewMode !== 'none' && (props.imageSrc || props.imageRender) ? (
                    ...
                    <div className={getTextContentClassName(props)}>
                        ...
                        <div
                            className={getCaptionWrapperClassName(props)}
                        ...
                        {(props.description || props.footer) && props.descriptionLines !== 0 ? (
                            <div className={getDescriptionClassName(props)}>
                                {props.description ? (
                                    <TextRender
                                        className={EDITOR_SELECTOR_DESCRIPTION}
 ...
 * @public
 * @demo Controls-Templates-demo/HorizontalItem/WI/Index
 */
