...
import { ItemActionsTemplateSelector } from 'Controls/listsCommonLogic';
...
function getContentClasses({ faded = false, contentClassName = '' }): string {
    let classes = '';

    classes += ' Controls-Templates-BaseItem__content';
    classes += faded ? ' controls-ListView__itemContent_faded ' : '';
    classes += contentClassName ? ` ${contentClassName} ` : '';
    return classes;
}
...
export function BaseItem(props: IBaseItemProps): React.ReactElement<IBaseItemProps> {
    ...
    let attrs = {};
    let handlers;
    let isEditing: boolean | undefined = props.isEditing;
    // FIXME Плитка не должна зависить от наших моделей.
    if (props.item && props.item['[Controls/_display/CollectionItem]'] && !props.isReactTile) {
    ...
    return (
        <div
            ...
            {...(!attrs ? props.attrs : {})}
            title={props.tooltip}
            style={typeof props.style === 'object' ? props.style : props.styleProp}
            ref={props.$wasabyRef}
            onMouseEnter={
                handlers?.onMouseEnter || props.onMouseEnterCallback || props.onMouseEnter
            }
            onMouseLeave={
                handlers?.onMouseLeave || props.onMouseLeaveCallback || props.onMouseLeave
            }
            onMouseMove={handlers?.onMouseMove || props.onMouseMoveCallback || props.onMouseMove}
            onMouseDown={handlers?.onMouseDown || props.onMouseDownCallback || props.onMouseDown}
            onMouseUp={handlers?.onMouseUp || props.onMouseUpCallback || props.onMouseUp}
            onClick={handlers?.onClick || props.onClickCallback || props.onClick}
            onContextMenu={
                handlers?.onContextMenu || props.onContextMenuCallback || props.onContextMenu
            }
            onWheel={handlers?.onWheel || props.onWheelCallback || props.onWheel}
            onTouchStart={
                handlers?.onTouchStart || props.onTouchStartCallback || props.onTouchStart
            }
            onTouchMove={handlers?.onTouchMove || props.onTouchMoveCallback || props.onTouchMove}
            onTouchEnd={handlers?.onTouchEnd || props.onTouchEndCallback || props.onTouchEnd}
        >
