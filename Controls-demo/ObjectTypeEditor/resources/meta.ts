...
const TreeEditorExampleType = ObjectType.id('Controls/meta:TreeEditorExampleType').properties({
    // @ts-ignore
...
const BaseEditorsAttrsDefaults = {
    ...
    ...group('Выпадающее меню', {
        enumString: StringType.title(rk('Единичный выбор'))
            .description(rk('Демонстрационный тип для редактора StringEnum'))
            .oneOf(StringEnumEditorOptions)
            .editor<
                ComponentProps<
                    typeof import('Controls-editors/dropdown').EnumStringEditor<
                        typeof BaseEditorsAttrsDefaults.enumString
                    >
                >
...
const BaseEditorsAttrsRequired = Object.keys(BaseEditorsAttrsDefaults).reduce((accum, key) => {
    // @ts-ignore
    let result = BaseEditorsAttrsDefaults[key].defaultValue(null).required();
    if (key === 'variants') {
        // @ts-ignore
        result = BaseEditorsAttrsDefaults[key]
            .defaultValue({
                selectedKeys: [],
                items: [],
            })
