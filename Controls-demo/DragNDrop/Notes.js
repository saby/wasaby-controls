...
], function (Base, cInstance, source, template, EntityTriangle, EntityNote) {
   'use strict';

   var Notes = Base.Control.extend({
      _template: template,
      ...
      _dragEnd: function (event, dragObject) {
         ...
         if (cInstance.instanceOfModule(entity, 'Controls-demo/DragNDrop/Notes/EntityNote')) {
            ...
            cInstance.instanceOfModule(entity, 'Controls-demo/DragNDrop/Notes/EntityTriangle')
         ) {
      ...
      _allowDrop: function (id, position) {
         ...
         this._items.forEach(function (item) {
            if (item.id !== id) {
               badPosition =
                  position.y > item.position.top &&
                  position.y < item.position.top + item.size.height &&
                  position.x > item.position.left &&
                  position.x < item.position.left + item.size.width;
      ...
      _dragMove: function (event, dragObject) {
         ...
         if (cInstance.instanceOfModule(entity, 'Controls-demo/DragNDrop/Notes/EntityTriangle')) {
            ...
            cInstance.instanceOfModule(entity, 'Controls-demo/DragNDrop/Notes/EntityNote')
         ) {
