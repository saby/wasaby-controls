...
export type TGridColumnScrollProps = Pick<
    IGridColumnScrollProps,
    | 'stickyColumnsCount'
    | 'endStickyColumnsCount'
    | 'columnScrollViewMode'
    | 'columnScrollAutoScrollMode'
    | 'dragScrolling'
...
function Editor(props: IEditorProps): React.ReactElement {
    ...
    const onSaveToSession = React.useCallback(() => {
        ...
        cookie.set(SESSION_ID, JSON.stringify(res));
    }, [gridProps]);
    ...
    const ScrollIntoViewTargetModeTumbler = useTumbler('columnScrollAutoScrollMode', [
        undefined,
        'closest',
        'mostVisible',
    ]);
    ...
    return (
        <BaseEditor header={'Grid column scroll editor'} level={2}>
            ...
            {ScrollIntoViewTargetModeTumbler}
...
>(name: TKey, initialProps: TGridColumnScrollProps): TValue {
    const options = JSON.parse(cookie.get(SESSION_ID) || '{}');
    if (options) {
        if (options[name] === 'undefined') {
            return undefined;
        }
        return name in options ? options[name] : initialProps[name];
    }
