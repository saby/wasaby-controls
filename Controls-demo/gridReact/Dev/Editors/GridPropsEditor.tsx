        ...
        <BaseEditor header={'Grid Props Editor'} level={2}>
            ...
            <BaseSelector header={'colspanCallback'}>
                <input
                    ...
                    onChange={(event) => {
                        return updateGridProps({
                            colspanCallback: event.target.checked ? COLSPAN_CALLBACK : null,
                        });
                    }}
                />
            </BaseSelector>
...
export function useGridPropsEditor(initialGridProps: Partial<IGridProps> = {}): IHookResult {
    const [gridProps, setGridProps] = React.useState<Partial<IGridProps>>(initialGridProps);

    const gridPropsEditor = <Editor gridProps={gridProps} onChange={setGridProps} />;
