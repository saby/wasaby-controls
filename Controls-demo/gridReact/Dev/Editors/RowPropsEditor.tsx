    ...
    return (
        <BaseEditor header={'Row Props Editor'} level={2}>
            ...
            <BaseSelector header={'markerVisible'}>
                <input
                    type={'checkbox'}
                    checked={rowProps.markerVisible === undefined ? true : rowProps.markerVisible}
                    onChange={(event) => {
                        return updateRowProps({
...
export function useRowPropsEditor(initialRowProps: Partial<IRowProps> = {}): IHookResult {
    ...
    const rowPropsEditor = <Editor rowProps={rowProps} onChange={setRowProps} />;
