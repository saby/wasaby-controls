...
export default React.forwardRef((props: TInternalProps, ref: React.ForwardedRef<HTMLDivElement>) => {
    ...
    const columns = React.useMemo<IColumnConfig[]>(() => (
        [
            ...
            },
            {
                ...
                width: '150px',
                getCellProps: (_item: Model) => ({ tagStyle: _item.get('tagStyle'), halign: 'end' })
            }
        ]
    ), []);
    ...
    const onTagClick = () => {
        const [_, item, columnIndex, nativeEvent] = getArgs(arguments);
        iBoxOpener.open({
            target: nativeEvent.target,
            message: `You've clicked the ${columnIndex} column of item "${item.get('country')}"`
        });
    ...
    const onTagHover = () => {
        ...
        iBoxOpener.open({
            target: nativeEvent.target,
            message: `You've hovered the ${columnIndex} column of item "${item.get('country')}"`
        });
    ...
    return (
        <div ref={ref}>
            <GridItemsView items={items}
                           columns={columns}
                           onTagClick={onTagClick}
                           onTagHover={onTagHover}
                           customEvents={['onTagClick', 'onTagHover']}/>;
            <div className={'controlsDemo-logger'}>
                <div><b>Событие: </b>:&nbsp;<span data-qa={'logger-event'}>{eventResult.type}</span></div>
                <div><b>Колонка: </b>:&nbsp;<span data-qa={'logger-column'}>{eventResult.column}</span></div>
            </div>
        </div>
    );
});
