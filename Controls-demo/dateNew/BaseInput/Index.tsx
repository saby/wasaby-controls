...
export default forwardRef(function BaseInputDemo(_: unknown, ref: LegacyRef<HTMLDivElement>) {
    ...
    const handleChangeFirstValue = useCallback((date: Date) => {
        setFirstValue(date);
    }, []);

    const [secondValue, setSecondValue] = useState(date);
    const handleChangeSecondValue = useCallback((date: Date) => {
        setSecondValue(date);
    }, []);

    const [thirdValue, setThirdValue] = useState(date);
    const handleChangeThirdValue = useCallback((date: Date) => {
        setThirdValue(date);
    }, []);

    const [fourthValue, setFourthValue] = useState(date);
    const handleChangeFourthValue = useCallback((date: Date) => {
        setFourthValue(date);
    }, []);

    const [fifthValue, setFifthValue] = useState(date);
    const handleChangeFifthValue = useCallback((date: Date) => {
        setFifthValue(date);
    }, []);

    const [sixthValue, setSixthValue] = useState(date);
    const handleChangeSixthValue = useCallback((date: Date) => {
        setSixthValue(date);
    }, []);

    const [seventhValue, setSeventhValue] = useState(date);
    const handleChangeSeventhValue = useCallback((date: Date) => {
