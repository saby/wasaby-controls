...
export default forwardRef(function DateBaseDemo(_, ref) {
    ...
    const handleChangeFirstDate = useCallback((event: Event, date: Date) => {
        setFirstDate(date);
    }, []);

    const [secondDate, setSecondDate] = useState(date);
    const handleChangeSecondDate = useCallback((event: Event, date: Date) => {
        setSecondDate(date);
    }, []);

    const [thirdDate, setThirdDate] = useState(date);
    const handleChangeThirdDate = useCallback((event: Event, date: Date) => {
        setThirdDate(date);
    }, []);
