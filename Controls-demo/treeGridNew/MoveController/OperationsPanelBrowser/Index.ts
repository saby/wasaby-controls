...
import { itemActions, listData, getPanelData } from './Data';
...
export default class ListWithoutMarker extends Control<IControlOptions> {
    ...
    protected _placeLeft: boolean = true;
    ...
    protected _beforeMount(
        ...
        this._panelSource = this.createPanelSource();
    }

    private createPanelSource(): Memory {
        return new Memory({
            keyProperty: 'id',
            data: getPanelData(this._placeLeft),
        });
    }
    ...
    protected _onPanelItemClick(
        ...
        if (itemId === 'move') {
            ...
            this._children.list.removeItemsWithConfirmation(selection, {
                target: nativeEvent.target,
            });
        } else if (itemId === 'replace') {
            this._placeLeft = !this._placeLeft;
            this._panelSource = this.createPanelSource();
        }
    }
}
