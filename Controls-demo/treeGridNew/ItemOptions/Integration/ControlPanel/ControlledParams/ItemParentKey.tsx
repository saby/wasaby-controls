...
export function ItemParentKey(): JSX.Element {
    const [parentNode, setParentNode] = React.useState<string | null>(() => {
        const params = new URLSearchParams(window.location.search);
        return params.get('parentNode') || null;
    });
    ...
    function handleChange(event: React.ChangeEvent<HTMLSelectElement>): void {
        ...
        const params = new URLSearchParams(window.location.search);
        params.set('parentNode', value);
        window.history.pushState({}, '', `${window.location.pathname}?${params.toString()}`);
    }
    React.useEffect(() => record?.set('parent', parentNode), []);

    return (
        <ControlSection
            title="Атрибут item-parent-key"
