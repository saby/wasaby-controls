...
import { CopyPageUrlWithListParams } from 'Controls-ListEnv/actions';
...
export default class WidgetWrapper extends Control<IControlOptions, void> {
    ...
    protected _toolbarItems: RecordSet = new RecordSet({
        rawData: [
            ...
            },
            {
                ...
                icon: 'icon-Link',
                tooltip: 'Скопировать ссылку на страницу',
    ...
    protected _copyPageUrlAction: typeof CopyPageUrlWithListParams;

    protected _beforeMount(options: IOptions): void {
        this._prefetchResult = options._dataOptionsValue.nomenclature;
        this._copyPageUrlAction = new CopyPageUrlWithListParams({
            context: options._dataOptionsValue,
        });
    }

    protected _afterMount(): void {
        this._callbackViewMode = Store.onPropertyChanged('viewMode', (value: string) => {
    ...
    protected _itemClick(event: Event, item: Model): void {
        ...
        if (itemId !== 'viewMode' && itemId !== 'copyListParamsUrl') {
            Store.dispatch('viewMode', itemId);
        }
        if (itemId === 'copyListParamsUrl') {
            this._copyPageUrlAction.execute();
        }
    }
