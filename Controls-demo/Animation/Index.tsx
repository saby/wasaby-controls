...
import { ItemsView as Grid } from 'Controls/grid';
...
export default class Index extends React.Component<{}, IState> {
    ...
    _moveItemDown(): void {
        this._list2ref.current.animateMoving();
        const item = this._items2.at(0);
        this._items2.move(0, this._items2.getCount() - 1);
        this._animationRef.current.startAnimation({
            fromId: 'list2',
            toId: 'list2',
            fromPosition: 0,
            toPosition: this._items2.getCount() - 1,
            entity: item.clone(),
        });
    }
    ...
    _moveItemUp(): void {
        this._list2ref.current.animateMoving();
        const item = this._items2.at(0);
        this._items2.move(this._items2.getCount() - 1, 0);
        this._animationRef.current.startAnimation({
            fromId: 'list2',
            toId: 'list2',
            fromPosition: this._items2.getCount() - 1,
            toPosition: 0,
            entity: item.clone(),
        });
    }
                ...
                <Container ref={this._animationRef} animationTemplate={animationTemplate}>
                    <div className={'controlsDemo__flexRow'}>
                        ...
                        <div className={'controlsDemo__wrapper ws-flex-column ws-flexbox'}>
                            ...
                            <button
                                disabled={this._items2.getCount() < 3}
                                onClick={() => {
                                    this._moveItemDown();
                                }}
                            >
