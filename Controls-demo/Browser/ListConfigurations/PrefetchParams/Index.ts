...
class Demo extends Control<IControlOptions> {
    ...
    protected _firstListSource: PrefetchSource = new PrefetchSource({
        ...
        filter: (item, query) => {
            return (
                !query.title || item.get('title').toLowerCase().includes(query.title.toLowerCase())
            );
        },
    });
    ...
    protected _secondListSource: PrefetchSource = new PrefetchSource({
        ...
        filter: (item, query) => {
            const isSearched =
                !query.title || item.get('title').toLowerCase().includes(query.title.toLowerCase());
            const isCity =
                !query.city || !query.city.length || query.city.includes(item.get('title'));
    ...
    protected _filterChanged(event: SyntheticEvent, filter: object, listId: string): void {
