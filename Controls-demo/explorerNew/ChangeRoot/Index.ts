...
class Demo extends Control<IProps> {
    ...
    protected _beforeUpdate(props: IProps): void {
        const slice = props._dataOptionsValue.ChangeRoot;
        if (slice.root !== this._root) {
            this._root = slice.root;
        }
    }

    protected _onToggleRoot(): void {
        const slice = this._options._dataOptionsValue.ChangeRoot;
        if (this._root === null) {
            this._root = 1;
        } else {
            this._root = null;
        }
        slice.changeRoot(this._root);
    }
