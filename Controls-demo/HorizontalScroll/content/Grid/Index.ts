...
export default class Grid extends Control<IGridOptions> {
    ...
    constructor(options: IControlOptions) {
        super(options);
        this._onHeaderAddButtonClickCallback = this._onHeaderAddButtonClickCallback.bind(this);
    }
    ...
    protected _beforeMount(options: IGridOptions): void {
        ...
        this._header = getHeader(this._columnsCount, this._onHeaderAddButtonClickCallback);
    ...
    protected _beforeUpdate(newOptions: IGridOptions): void {
        ...
        const newColumnsCount = newOptions.columnsCount || DEFAULT_COLUMNS_COUNT;
    ...
    private _onHeaderAddButtonClickCallback(type: TActionClickCallbackType): void {
        switch (type) {
            case 'controls':
