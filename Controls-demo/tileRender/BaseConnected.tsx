...
function FooterRenderComponent() {
    return <div>footer_render</div>;
}
...
function Demo(_: {}, ref: React.ForwardedRef<HTMLDivElement>): React.ReactElement {
    ...
    const [selectedRender, setSelectedRender] = React.useState('vertical');
    ...
    return (
        <div>
            <div style={{ margin: 20 }}>
                <select value={selectedRender} onChange={(e) => setSelectedRender(e.target.value)}>
            ...
            <ScrollContainer style={{ width: '1100px', height: '700px' }}>
                <View
                    ...
                    footerRender={<FooterRenderComponent />}
                    onActionClick={onActionItemClick}
                    itemRender={ITEM_RENDER[selectedRender].Component}
