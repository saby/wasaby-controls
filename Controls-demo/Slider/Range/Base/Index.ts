...
class Base extends Control<IControlOptions> {
    ...
    protected _keyDownHandler(event: SyntheticEvent<KeyboardEvent>, type: TInput): void {
        if (event.nativeEvent.keyCode === 13) {
            this._deactivatedHandler(event, type);
        }
    }

    protected _changeStartSliderHandler(event: SyntheticEvent, value: number): void {
        this._startValue = value;
    }

    protected _changeEndSliderHandler(event: SyntheticEvent, value: number): void {
