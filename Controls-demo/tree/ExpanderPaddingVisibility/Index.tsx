...
const VISIBILITY_VALUES: TExpanderPaddingVisibility[] = ['visible', 'hidden', 'hasExpander'];
let SELECTED_VISIBILITY_INDEX = 0;
export default React.forwardRef((_, ref) => {
    const [expanderPaddingVisibility, setExpanderPaddingVisibility] =
        React.useState<TExpanderPaddingVisibility>(VISIBILITY_VALUES[SELECTED_VISIBILITY_INDEX]);

    const changeExpanderPaddingVisibility = () => {
        SELECTED_VISIBILITY_INDEX++;
        const newMode = VISIBILITY_VALUES[SELECTED_VISIBILITY_INDEX % VISIBILITY_VALUES.length];
