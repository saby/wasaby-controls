...
export default class extends Memory {
    query(query: Query): DataSet {
        ...
        const parent =
            filter.Раздел instanceof Array ? filter.Раздел[0] : filter.Раздел;

        if (selection) {
            const isAllSelected =
                selection.get('marked').includes(null) &&
                selection.get('excluded').includes(null);

            query.where((item) => {
                item = getById(items, item.get('id'));
                if (
                    (isSelected(item, items, selection) &&
                        isChildByNode(item, items, parent)) ||
                    (isAllSelected &&
                        item.Раздел === null &&
