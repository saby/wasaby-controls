define('Controls-demo/Popup/Compatible/TestStack', [
   ...
   var TestStack = CompoundControl.extend({
      ...
      init: function () {
         ...
         this._onAfterVisibilityChangeElement = this.getContainer().find(
            '.onAfterVisibilityChangeTimes'
         );
         this._onControlResizeElement = this.getContainer().find(
            '.onControlResizeTimes'
         );
         this._leftPositionTestElement =
            this.getContainer().find('.leftPositionTest');

         var compoundArea = this.getParent(),
         ...
         compoundArea.subscribe(
            'onAfterVisibilityChange',
            this._onAfterVisibilityChange.bind(this)
         );

         // eventCheck
         this.getChildControlByName('eventCheckShow').subscribe(
            'onActivated',
            function () {
               compoundArea.show();
            }
         );
         this.getChildControlByName('eventCheckHide').subscribe(
            'onActivated',
            function () {
         ...
         // crashTest
         this.getChildControlByName('crashTest').subscribe(
            'onActivated',
            function () {
         ...
         // resizeTest
         this.getChildControlByName('resizeCheckPanel').subscribe(
            'onActivated',
            function () {
               this._notifyOnSizeChanged();
            }.bind(this)
         );
         this.getChildControlByName('resizeCheckButton').subscribe(
            'onActivated',
            function () {
      ...
      _onAfterVisibilityChange: function (ev, visible) {
         ...
         self._onAfterVisibilityChangeElement.text(
            ++self._onAfterVisibilityChangeTimes
         );
         if (visible && !self._checkLeftPositionInterval) {
            ...
            self._checkLeftPositionInterval = setInterval(function () {
               if (
                  !self._popupContainer.hasClass('ws-hidden') &&
                  !self._popupContainer.hasClass('ws-invisible')
               ) {
                  // if popup is visible, check the `left` position and compare it to the previous one
                  var newLeft = self._popupContainer.css('left'),
