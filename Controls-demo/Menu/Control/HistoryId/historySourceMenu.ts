...
class HistorySourceMenu extends Source {
    ...
    private _serviceUpdate(
        ...
        const clickedItemId = item?.ids ? item?.ids[0] : item;
        let historyItem;
        if (meta.$_pinned) {
            historyItem = new Model({
                rawData: {
                    d: [String(clickedItemId), clickedItemId],
                    s: [
                        { n: 'ObjectId', t: 'Строка' },
                        { n: 'HistoryId', t: 'Строка' },
                    ],
            ...
            pinned.remove(pinned.getRecordById(clickedItemId));
        } else if (meta.$_history && clickedItemId && !recent.getRecordById(clickedItemId)) {
            historyItem = new Model({
                rawData: {
                    d: [String(clickedItemId), clickedItemId],
                    s: [
