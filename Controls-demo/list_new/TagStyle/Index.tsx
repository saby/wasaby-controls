...
function Demo(props: TInternalProps, ref: React.ForwardedRef<HTMLDivElement>) {
    ...
    const itemTemplate = React.useMemo(() => {
        return React.forwardRef((itemTemplateProps: { item: CollectionItem }) => {
            const tagStyle = itemTemplateProps.item.contents?.get('tagStyle');
            return <ListItemTemplate {...itemTemplateProps} tagStyle={tagStyle} />;
        });
    }, []);
