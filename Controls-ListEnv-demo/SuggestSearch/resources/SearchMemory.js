define('Controls-ListEnv-demo/SuggestSearch/resources/SearchMemory', [
   'Types/deferred',
   'Controls-ListEnv-demo/SuggestSearch/resources/kbLayoutRevert',
   'Types/source',
   'Types/entity',
   'Core/core-clone'
], function (defferedLib, kbLayoutRevert, source, entity, clone) {
   'use strict';
   ...
   var BrowserMemory = source.Memory.extend({
      ...
      query: function (queryInst) {
         ...
         superQuery.addCallback(function (dataSet) {
            if (queryInst.getWhere()[self.searchParam]) {
               var switchedStr = kbLayoutRevert.process(queryInst.getWhere()[self.searchParam]);
               queryInst.where = clone(queryInst.getWhere());
               queryInst.getWhere()[self.searchParam] = switchedStr;
