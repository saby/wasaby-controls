...
function checkGender(item: adapter.IRecord, queryFilter: IFilter): boolean {
    ...
    const genderDropdown =
        queryFilter.genderDropdown === item.get('gender') ||
        !queryFilter.genderDropdown?.length;
    const radioGender =
        queryFilter.radioGender === item.get('gender') ||
        !queryFilter.radioGender?.length;
...
function checkDate(item: adapter.IRecord, queryFilter: IFilter): boolean {
    ...
    const dateRange =
        (itemDate >= minDate || !minDate) && (itemDate <= maxDate || !maxDate);
...
function filter(item: adapter.IRecord, queryFilter: IFilter): boolean {
    ...
    const cityDropdown =
        queryFilter.cityDropdown?.includes(item.get('city')) ||
        !queryFilter.cityDropdown?.length;
    const owner =
    ...
    const inputDepartment =
        item
            .get('department')
            .toLowerCase()
            .includes(queryFilter.departmentInput.toLowerCase()) ||
