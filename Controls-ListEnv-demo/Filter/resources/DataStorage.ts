import { groupConstants } from 'Controls/list';
import SearchMemory from 'Controls-ListEnv-demo/Filter/resources/SearchMemory';
import { companyFilterData } from 'Controls-ListEnv-demo/Filter/resources/Data';

/* eslint-disable max-len */
export const Images = {
    Novikov:
        'data:image/png;base64,',
    Krainov:
        'data:image/png;base64,',
};

export const departments = [
    {
        id: 1,
        department: 'Разработка',
        title: 'Разработка',
        sourceImage: Images.Novikov,
        isDevelopment: true,
        salary: 100,
        amount: '999+',
    },
    {
        id: 2,
        department: 'Продвижение СБИС',
        title: 'Продвижение СБИС',
        isDevelopment: false,
        sourceImage: Images.Krainov,
        salary: 200,
        amount: 30,
    },
    {
        id: 3,
        department: 'Федеральная клиентская служка',
        title: 'Федеральная клиентская служка',
        isDevelopment: false,
        sourceImage: Images.Novikov,
        salary: 300,
        amount: '999+',
    },
];

export const cities = [
    { id: 'Yaroslavl', title: 'Yaroslavl' },
    { id: 'Moscow', title: 'Moscow' },
    { id: 'Kazan', title: 'Kazan' },
    { id: 'Rostov', title: 'Rostov' },
    { id: 'Saint Petersburg', title: 'Saint Petersburg' },
    { id: 'Anapa', title: 'Anapa' },
    { id: 'Belgorod', title: 'Belgorod' },
    { id: 'Veliky Novgorod', title: 'Veliky Novgorod' },
];

export const owners = [
    { id: 'Новиков Д.В.', title: 'Новиков Д.В.' },
    { id: 'Кошелев А.Е.', title: 'Кошелев А.Е.' },
    { id: 'Субботин А.В.', title: 'Субботин А.В.' },
];

export const bigListOfDepartments = [
    {
        id: 1,
        department: 'Разработка',
        title: 'Разработка',
        sourceImage: Images.Novikov,
        isDevelopment: true,
        salary: 100,
        amount: '999+',
        group: groupConstants.hiddenGroup,
    },
    {
        id: 2,
        department: 'Продвижение СБИС',
        title: 'Продвижение СБИС',
        isDevelopment: false,
        sourceImage: Images.Krainov,
        salary: 200,
        amount: 30,
        group: groupConstants.hiddenGroup,
    },
    {
        id: 3,
        department: 'Федеральная клиентская служка',
        title: 'Федеральная клиентская служка',
        isDevelopment: false,
        sourceImage: Images.Novikov,
        salary: 300,
        amount: '999+',
        group: '1',
    },
    {
        id: 4,
        department: 'Служба эксплуатации',
        owner: 'Субботин А.В.',
        title: 'Служба эксплуатации',
        sourceImage: Images.Novikov,
        amount: 15,
        group: '1',
    },
    {
        id: 5,
        department: 'Технологии и маркетинг',
        owner: 'Чеперегин А.С.',
        title: 'Технологии и маркетинг',
        sourceImage: Images.Novikov,
        amount: 11,
        group: '1',
    },
    {
        id: 6,
        department: 'Федеральный центр продаж. Call-центр Ярославль',
        owner: 'Кошелев А.Е.',
        title: 'Федеральный центр продаж. Call-центр Ярославль',
        sourceImage: Images.Novikov,
        amount: 150,
        group: '2',
    },
    {
        id: 7,
        department: 'Сопровождение информационных систем',
        owner: 'Кошелев А.Е.',
        title: 'Сопровождение информационных систем',
        sourceImage: Images.Novikov,
        amount: 18,
        group: '2',
    },
    {
        id: 8,
        department: 'Платформа',
        title: 'Платформа',
        isDevelopment: false,
        sourceImage: Images.Krainov,
        salary: 200,
        amount: 30,
        group: '2',
    },
];

export const sourceData = [
    {
        id: 1,
        department: 'Разработка',
        title: 'Разработка',
        owner: 'Новиков Д.В.',
        isDevelopment: true,
        salary: 100,
        city: 'Yaroslavl',
        gender: '1',
        date: 2022,
    },
    {
        id: 2,
        department: 'Продвижение СБИС',
        title: 'Продвижение СБИС',
        owner: 'Кошелев А.Е.',
        isDevelopment: false,
        salary: 200,
        city: 'Moscow',
        gender: '1',
        date: 2021,
    },
    {
        id: 3,
        department: 'Федеральная клиентская служка',
        title: 'Федеральная клиентская служка',
        owner: 'Мануйлова Ю.А.',
        isDevelopment: false,
        salary: 300,
        city: 'Kazan',
        gender: '2',
        date: 2020,
    },
];

export const tasks = [
    {
        id: 1,
        document: 'Задачи в разработке',
        department: 'Разработка',
        task: 'Разработать компонент поиска',
        sourceImage: Images.Krainov,
    },
    {
        id: 2,
        document: 'Задачи в разработке',
        department: 'Разработка',
        task: 'Разработать компонент списка',
        sourceImage: Images.Krainov,
    },
    {
        id: 3,
        document: 'Задачи на тестировании',
        department: 'Тестирование',
        task: 'Протестировать окно фильтра',
        sourceImage: Images.Novikov,
    },
    {
        id: 4,
        document: 'Ошибки в разработке',
        department: 'Разработка',
        task: 'Не отображается кнопка',
        sourceImage: Images.Krainov,
    },
    {
        id: 5,
        document: 'Ошибки на тестировании',
        department: 'Тестирование',
        task: 'Проверить воспроизведение ошибки',
        sourceImage: Images.Novikov,
    },
    {
        id: 6,
        document: 'Ошибки на тестировании',
        department: 'Тестирование',
        task: 'Проверить отображение браузера',
        sourceImage: Images.Novikov,
    },
];

export const tasksFolders = [
    { document: 'Задачи', node: true, parent: null },
    { document: 'Просроченные задачи', node: true, parent: 'Задачи' },
    { document: 'Ошибки', node: true, parent: null },
];

export const tasksData = [
    ...tasksFolders,
    ...[
        { document: 'Задачи', node: true, parent: null },
        { document: 'Ошибки', node: true, parent: null },
        { document: 'Ошибки в разработке', node: null, parent: 'Ошибки' },
        { document: 'Ошибки на тестировании', node: null, parent: 'Ошибки' },
        {
            document: 'Задачи в разработке',
            node: null,
            parent: 'Просроченные задачи',
        },
        { document: 'Задачи на тестировании', node: null, parent: 'Задачи' },
        { document: 'Документ в корне', node: null, parent: null },
    ],
];

export const filterItems = [
    {
        id: 1,
        title: 'Новиков Д.В.',
        owner: 'Новиков Д.В.',
        sourceImage: Images.Novikov,
        additionalCounter: 23,
    },
    {
        id: 2,
        title: 'Кошелев А.Е.',
        owner: 'Кошелев А.Е.',
        sourceImage: Images.Krainov,
        additionalCounter: 5,
    },
    {
        id: 3,
        title: 'Субботин А.В.',
        owner: 'Субботин А.В.',
        sourceImage: Images.Novikov,
        additionalCounter: '999+',
    },
    {
        id: 4,
        title: 'Чеперегин А.С.',
        owner: 'Чеперегин А.С.',
        sourceImage: Images.Krainov,
        additionalCounter: '999+',
    },
];

export const departmentsWithAmount = [
    {
        id: 1,
        department: 'Разработка',
        owner: 'Новиков Д.В.',
        title: 'Разработка',
        amount: 1,
    },
    {
        id: 2,
        department: 'Продвижение СБИС',
        owner: 'Кошелев А.Е.',
        title: 'Продвижение СБИС',
        amount: 10,
    },
    {
        id: 3,
        department: 'Федеральная клиентская служка',
        owner: 'Мануйлова Ю.А.',
        title: 'Федеральная клиентская служка',
        amount: 100,
    },
    {
        id: 4,
        department: 'Служба эксплуатации',
        owner: 'Субботин А.В.',
        title: 'Служба эксплуатации',
        amount: 15,
    },
    {
        id: 5,
        department: 'Технологии и маркетинг',
        owner: 'Чеперегин А.С.',
        title: 'Технологии и маркетинг',
        amount: 11,
    },
    {
        id: 6,
        department: 'Федеральный центр продаж. Call-центр Ярославль',
        owner: 'Кошелев А.Е.',
        title: 'Федеральный центр продаж. Call-центр Ярославль',
        amount: 150,
    },
    {
        id: 7,
        department: 'Сопровождение информационных систем',
        owner: 'Кошелев А.Е.',
        title: 'Сопровождение информационных систем',
        amount: 18,
    },
];

export const companyBasicFilter = {
    name: 'company',
    type: 'list',
    value: null,
    resetValue: null,
    caption: 'Компании',
    viewMode: 'basic',
    textValue: '',
    editorTemplateName: 'Controls/filterPanelEditors:Lookup',
    editorOptions: {
        source: new SearchMemory({
            keyProperty: 'id',
            data: companyFilterData,
        }),
        navigation: {
            source: 'page',
            view: 'page',
            sourceConfig: {
                pageSize: 5,
                page: 0,
                hasMore: false,
            },
        },
        selectorTemplate: {
            templateName:
                'Controls-ListEnv-demo/Filter/View/Editors/LookupEditor/resources/StackTemplate',
            templateOptions: {
                items: companyFilterData,
            },
        },
        displayProperty: 'title',
        keyProperty: 'id',
    },
};

export const deletedExtendedFilter = {
    name: 'deleted',
    value: false,
    resetValue: false,
    textValue: '',
    viewMode: 'extended',
    editorTemplateName: 'Controls/filterPanelEditors:Boolean',
    extendedCaption: 'Удаленные',
    editorOptions: {
        filterValue: true,
    },
};
