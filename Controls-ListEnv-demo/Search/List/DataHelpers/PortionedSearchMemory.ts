...
export default class PortionedSearchMemory extends Memory {
    protected _moduleName: string = 'Controls-ListEnv-demo/Search/List/DataHelpers/PortionedSearchMemory';
    ...
    constructor(options: IOptions) {
        ...
        this._responseItemsCount = options.responseItemsCount || ITEMS_COUNT_DEFAULT;
    ...
    query(query?: Query<unknown>): Promise<DataSet> {
        ...
        if (filterFewItems) {
            ...
            return this._getSearchItems(position).then((items) => {
                const nextPosition = (this._direction === 'up' ? -1 : 1) * items.length + position;
                const hasMore =
                    this._direction === 'up'
                ...
                return this._prepareQueryResult(
                    {
                        items,
                        meta: {
                            total: isPosition ? { before: true, after: true } : hasMore,
                            more: hasMore,
    ...
    private _getItems(position: number, limit: number): IItem[] {
        ...
        while ((this._direction === 'up' ? -1 : 1) * position + limit > ITEMS_TOTAL) {
            limit--;
        }
