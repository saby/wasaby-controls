...
import * as MemorySourceFilter from 'Controls-ListEnv-demo/ExtSearch/resources/MemorySourceFilter';
    ...
    query(queryInst: Query): Promise<DataSet> {
        return new Promise((resolve) => {
            ...
            superQuery.then((dataSet) => {
                ...
                const currentTabProperty = 'currentTab';
                originAllMeta.results = new Model({
                    rawData: {
                        tabsSelectedKey: queryInst.getWhere()[currentTabProperty]
                            ? queryInst.getWhere()[currentTabProperty]
                            : 1,
                        tabs: [
                            { id: 1, title: 'Вкладка' },
                            { id: 2, title: 'Вкладка2' },
