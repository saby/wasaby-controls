...
import { useSelector } from 'Controls-DataEnv/context';
...
function useGetDetailPanelVisible(storeId: IBaseFilterViewWidgetProps['storeId']): boolean {
    const detailPanelVisibleSelector = React.useCallback(
...
const FilterConnectedViewContent = React.forwardRef(
    (props: IBaseFilterViewWidgetProps & IUseFilterDescription, ref): JSX.Element => {
        const { filterDescription, storeId, fullFilterDescription } = props;
        const { applyFilterDescription, closeDetailPanel } = useFilterDescription(props);
        ...
        const slices = getSlicesActions(storeId);
        ...
        const detailPanelClose = React.useCallback(() => closeDetailPanel(), [closeDetailPanel]);
