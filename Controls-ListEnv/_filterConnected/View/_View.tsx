...
function validateProps(storeId?: string | string[], useStore?: boolean): void {
    if (useStore) {
        logger.error(
            `Controls-ListEnv/filterConnected:View не поддерживает опцию useStore. Настройте загрузку данных
...
const FilterConnectedViewContent = React.forwardRef(
    (props: IBaseFilterViewWidgetProps & IUseFilterDescription, ref): JSX.Element => {
        ...
        const sliceViewMode = getPropertyFromSlice<string>(props.storeId, 'editorsViewMode');
        ...
        React.useMemo(() => {
            validateProps(props.storeId, props.useStore);
        }, [props.storeId, props.useStore]);
