...
import { forwardRef, ForwardedRef, useRef, useImperativeHandle, useCallback } from 'react';
import { InputSearchContextResolver, ISearchInputOptions } from 'Controls/search';
import { IFieldTemplateOptions } from 'Controls/input';
import { default as ExtSearch, IExtSearchOptions } from 'Controls-ListEnv/ExtSearch';
...
export interface ISearchSuggestWidgetOptions
    extends IContrastBackgroundOptions,
        ...
        Pick<ISuggest, 'suggestWidth' | 'footerTemplate'>,
        Pick<ISearchInputOptions, 'searchButtonVisible'>,
        ITextOptions,
    ...
    onTabsSelectedKeyChanged?: Function;

    __selectedCollectionAlign?: string;
    __suggestSearchButtonVisible?: boolean;
}

const customEvents = ['onFilterDescriptionChanged', 'onTabsSelectedKeyChanged'];

const Content = forwardRef(
    (
        props: IExtSearchOptions & Omit<ISearchSuggestWidgetOptions, 'storeId' | 'filterNames'>,
 ...
 */
function SearchSuggestWidgetWidget(props: ISearchSuggestWidgetOptions, ref: ForwardedRef<unknown>) {
    const { filterDescription, applyFilterDescription } = useFilterDescription(props);
    const inputRef = useRef<HTMLDivElement>(null);
    const onBlurHandler = useSearchConnectedFocus(inputRef, props.storeId);
    ...
    return (
        <InputSearchContextResolver className={props.className} storeId={props.storeId}>
            <Content
                ref={inputRef}
                attrs={props.attrs}
                ...
                onTabsSelectedKeyChanged={props.onTabsSelectedKeyChanged}
                ...
                __selectedCollectionAlign={props.__selectedCollectionAlign}
                __suggestSearchButtonVisible={props.__suggestSearchButtonVisible}
            />
        </InputSearchContextResolver>
    );
}

export default forwardRef(SearchSuggestWidgetWidget);
 ...
 *    <Controls-ListEnv.ExtSearchConnected
 *       suggestWidth="400px"
 *    />
 * </pre>
 */
