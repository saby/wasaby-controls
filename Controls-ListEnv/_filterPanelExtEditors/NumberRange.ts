...
import { INumberInputOptions, getNumberInputWidth } from 'Controls/input';
import 'css!Controls-ListEnv/filterPanelExtEditors';

export interface INumberRangeOptions extends IEditorOptions<number[]>, INumberInputOptions {
...
export interface IAfterEditorTemplateOptions {
 ...
 * @ignoreOptions value changeValueByArrows
 * @public
 */
...
class NumberRangeEditor extends Control<INumberRangeOptions> implements INumberRange {
    ...
    protected _beforeUpdate(options?: INumberRangeOptions): void {
        ...
        if (options.integersLength !== this._options.integersLength) {
            this._inputWidth = getNumberInputWidth(options);
        }
    }
    ...
    protected _handleInputCompleted(event: SyntheticEvent, value: number): void {
        const replaceValue = this._needReplaceMinMaxValues();

        if (replaceValue) {
            this._replaceMinMaxValues(this._minValue, this._maxValue);
        }

        if (!this._isInPopup() || replaceValue) {
            this._processPropertyValueChanged(event, [this._minValue, this._maxValue]);
        }
    }
    ...
    private _isInPopup(options: INumberRangeOptions = this._options): boolean {
        return options.filterViewMode === 'popup';
    }

    private _getTextValue(value: number[]): string | number {
        let resultString = '';
        if (value[0] !== null) {
            resultString += rk('от', 'ЧисловойДиапазон') + ' ' + this._getFormattedNumber(value[0]);
        }
        if (value[1] !== null) {
            resultString +=
                (resultString.length ? ' ' : '') +
                rk('до', 'ЧисловойДиапазон') +
                ' ' +
    ...
    private _getFormattedNumber(number: number): string {
        return NumberFunctions.calculateFormattedNumber(
            ...
            'none',
            this._options.showEmptyDecimals,
            this._options
        );
    }
...
/**
 * @name Controls-ListEnv/_filterPanelExtEditors/NumberRange#showEmptyDecimals
 * @cfg {Boolean} Видимость нулей в конце десятичной части при отображении в быстром фильтре.
 * @default false
 */

/**
 * @name Controls-ListEnv/_filterPanelExtEditors/NumberRange#precision
 * @cfg {Number} Количество знаков после запятой при отображении в быстром фильтре. Диапазон от 0 до 20.
 */
