...
function getTextValue(itemIds: string[] | number[], props: IChipsEditorOptions): string {
    const textArray = [];
    itemIds.forEach((id) => {
...
export default React.forwardRef(function ChipsEditor(
    ...
    const onPropertyValueChange = React.useCallback(
        (event, newValue) => {
            const value = {
                value: newValue,
                textValue: getTextValue(newValue, props),
                viewMode: 'basic',
            };

            props.onPropertyValueChanged?.(event, value);
        },
        [props.propertyValue, props.onPropertyValueChanged, props.displayProperty, props.items]
    );
