...
import TumblerTemplate = require('wml!Controls-ListEnv/_filterPanelExtEditors/Tumbler/Tumbler');
...
interface ITumbler {
    readonly '[Controls-ListEnv/_filterPanelExtEditors/Tumbler]': boolean;
}
...
class Tumbler extends Control<ITumblerOptions> implements ITumbler {
    readonly '[Controls-ListEnv/_filterPanelExtEditors/Tumbler]': boolean = true;
    ...
    private _checkViewMode(viewMode: string): void {
        if (viewMode === 'extended') {
            Logger.error(
                'Controls-ListEnv/filterPanelExtEditors:TumblerEditor: В опцию viewMode было передано значение extended. Данный редактор может отображаться только в основном блоке.',
                this
            );
        }
    }
