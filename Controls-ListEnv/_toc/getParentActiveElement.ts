    ...
    if (startItemKey !== undefined && startItemKey !== null) {
        ...
        if (parent !== undefined) {
            do {
                ...
                parent = hierarchyRelation.getParent(parent, items) as Model | null;
            } while (parent !== null);
        }
    }
    path.unshift(root);
...
function getPathToDepth(
    ...
    let child = hierarchyRelation.getChildren(lastPathItemKey, items)[0] as Model;
    while (child) {
...
export function getParentActiveElement(
    ...
    const path = getPathToDepth(props.items, props.root, props.activeItemKey, hierarchyRelation);
