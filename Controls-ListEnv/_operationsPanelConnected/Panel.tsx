...
import { useCount } from 'Controls-ListEnv/operationsConnected';
...
function getPanelExpanded(
    ...
    return (
        !!operationsController?.getOperationsPanelVisible() ||
        !!slice?.state?.operationsPanelVisible
    );
}
...
function OperationsPanel(props: IOperationsPanelWidgetOptions, ref) {
    ...
    function getActions(actions: IActionOptions[]): IActionOptions[] {
        ...
        if (typeof actions === 'string') {
            resultActions = loadSync(actions);
        }
        resultActions = object.clone(resultActions) || [];
    ...
    const sliceCount = useCount(slice);
    ...
    React.useEffect(() => {
        ...
        function unsubscribeFromOperationsControllerEvents(
    ...
    if (panelExpanded) {
        return (
            <PanelContent
                ref={ref}
                attrs={props.attrs}
                task1192905045={!compatibleMode && slice['[ISliceWithSelection]']}
                ...
                selectedKeysCount={compatibleMode ? counterConfig?.count : sliceCount}
