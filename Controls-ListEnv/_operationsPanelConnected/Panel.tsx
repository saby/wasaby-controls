...
function getPanelExpanded(
    ...
    return operationsController.getOperationsPanelVisible() || slice.operationsPanelVisible;
}
 ...
 */
function OperationsPanelContent(props: IOperationsPanelWidgetOptions, ref) {
    ...
    function getActionsByProps(
        ...
        const actions =
            slice.state.listActions || operationsController.getListActions() || props.actions || [];
        return getActions(actions);
    }
    const dataContext = React.useContext(DataContext);
    const slice = dataContext[props.storeId];
    ...
    const operationsController = React.useRef(slice.operationsController);
    ...
    const { listActionsChanged, counterChanged } = React.useMemo(() => {
        return {
            ...
            counterChanged: (counter: ICounterConfig) => {
    ...
    const { operationsPanelOpened, operationsPanelClose, selectionChanged } = React.useMemo(() => {
        return {
            operationsPanelOpened: () => {
                ...
                if (!slice.operationsPanelVisible) {
    ...
    }, [operationsController.current, slice]);
    ...
    }, [operationsController.current, slice, props.storeId]);
    ...
    React.useEffect(() => {
        ...
        operationsController.current = slice.operationsController;
    ...
    }, [slice.listActions, props.actions]);
    ...
    if (panelExpanded) {
        return (
            <PanelContent
                ...
                isAllSelected={compatibleMode ? counterConfig.count : slice.isAllSelected}
                countLoading={slice.countLoading}
                multiSelectSize={props.multiSelectSize}
                selectionViewMode={slice.selectionViewMode}
...
export default React.forwardRef(OperationsPanelContent);
/**
