...
export default React.forwardRef(function CountFilter(props: IFilterCounter, ref): JSX.Element {
    ...
    if (!isDateFilterSelected) {
        return (
            <div
                ref={ref}
                className="controls-CountFilter controls-margin_left-s tw-flex tw-items-baseline controls-fontweight-normal"
            >
                {props.caption ? (
                    <>
                ...
                <DropdownSelector
                    ...
                    fontColorStyle="link"
                    className={needToTransformText ? 'controls-CountFilter__withCaption' : ''}
                />
            </div>
        );
    } else {
