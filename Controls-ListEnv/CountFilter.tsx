...
export default React.forwardRef(function CountFilter(props: IFilterCounter, ref): JSX.Element {
    ...
    const needToTransformText = React.useMemo(() => {
        return props.caption && !Array.isArray(value.selectedKey);
    }, [props.caption, value.selectedKey]);

    if (!isDateFilterSelected) {
        return (
            <div
                ref={ref}
                className="controls-margin_left-s tw-flex tw-items-baseline controls-fontweight-normal"
            >
                {props.caption ? (
                    <>
