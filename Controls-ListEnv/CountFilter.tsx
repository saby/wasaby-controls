...
import { IDayTemplateOptions, IStoreIdOptions, IComponentProps } from 'Controls/interface';
...
import {
    inputDefaultContentTemplate as InputDefaultContentTemplate,
    Selector as DropdownSelector,
...
interface IFilterCounter extends Required<IStoreIdOptions>, IDateMenuOptions, IComponentProps {
    /**
     ...
     * @cfg {Controls/datePopup} Опция для окна выбора периода.
     */
...
function getTextValue(
    ...
    item,
    selectedValue,
    userPeriods?: IPeriodsConfig['userPeriods']
): string | void {
    let textValue;
    let dates;
 ...
 * @public
 */
export default React.forwardRef(function CountFilter(
    props: IFilterCounter,
    ref: React.ForwardedRef<HTMLDivElement>
): React.ReactElement | null {
    const dropdownRef = React.useRef<DropdownSelector>(null);
    const slice = useStrictSlice<ListSlice>(props.storeId);
    ...
    const textValue = React.useMemo(() => {
        const emptyCaption = props.caption ? 'весь период' : 'Весь период';
        return value?.textValue || rk(emptyCaption);
    }, [value]);

    const onMenuItemClick = React.useCallback(
        (_event, newValue, newTextValue) => {
    ...
    const needToTransformText = React.useMemo(() => {
        const isPeriodSelected = value?.textValue?.search(/[IV]/) !== -1;
        return props.caption && !(Array.isArray(value?.selectedKey) && isPeriodSelected);
    }, [props.caption, value]);

    const captionVisible =
        !(value?.selectedKey instanceof Array) || (value?.selectedKey[0] && value?.selectedKey[1]);
    ...
    if (!isDateFilterSelected) {
        return (
            <div
                ...
                className={`${
                    props.className || ''
                } controls-CountFilter controls-margin_left-s tw-flex tw-items-baseline controls-fontweight-normal tw-truncate`}
            >
                {props.caption && captionVisible ? (
                    <>
                ...
                <DropdownSelector
                    ref={dropdownRef}
                    items={dropdownItems}
                    contentTemplate={(contentProps: Record<string, unknown>) => (
                    ...
                    className={`tw-truncate ${
                        needToTransformText ? 'controls-CountFilter__withCaption' : ''
                    }`}
                />
            </div>
        );
