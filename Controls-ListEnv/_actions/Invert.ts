/**
 * @kaizen_zone c9246e72-a854-4853-b4e3-e62e54369586
 */
import * as rk from 'i18n!Controls';
import { RecordSet } from 'Types/collection';
import { TItemsOrder, TStoreImport } from 'Controls/interface';
import { BaseAction, IActionOptions, IListActionOptions } from 'Controls/actions';
...
/**
 * Интерфейс для опций действия инвертирования
 * @interface Controls-ListEnv/_actions/Invert/IInvertActionOptions
 * @public
 * @extends Controls/actions/IActionOptions
 */
export interface IInvertActionOptions extends IActionOptions {
    /**
     * Уникальный идентификатор, по которому в хранилище данных будет сохранена конфигурация контрола.
     */
    ...
    storeId?: string;
    /**
     * Индентификатор загрузчика в контексте для реестров без слайсов. Используется вместе с функцией предзагрузки getItemsOrder.
     */
    loaderId?: string;
}
 ...
 */
export default class Invert extends BaseAction<IInvertActionOptions> {
    ...
    constructor(options: IInvertActionOptions) {
        ...
        this._items = new RecordSet({
            ...
            rawData: [
                {
                    id: 'default',
                    tooltip: rk(options.tooltip || 'Инвертировать'),
                    icon: 'Controls-icons/sort:icon-Date_ASC',
                },
                {
                    id: 'reverse',
                    tooltip: rk(options.tooltip || 'По умолчанию'),
                    icon: 'Controls-icons/sort:icon-Date_DESC',
                },
            ],
        });
        ...
        if (this._compatibleMode) {
            ...
            initialOrder =
                typeof options.loaderId !== 'undefined'
                    ? (options.context?.[options.loaderId] as TItemsOrder)
                    : compatibleStore.get(STORE_EVENT_NAME);
        } else {
    ...
    execute(): void {
        ...
        const storageId = this._getPropStorageId();
        if (storageId) {
            USER.set(storageId + USER_PARAMS_POSTFIX, newItemsOrder);
        }
    ...
    private _getPropStorageId(): string | undefined {
        return this._compatibleMode
            ? this._options.propStorageId
            : this._getSlice().state.propStorageId;
    }
...
/**
 * Функция для предзагрузки данных кнопки инвертирования из сервиса параметров для реестров без слайса.
 * @function
 * @param {string} propsStorageId Идентификатор, по которому в сервисе параметров хранятся данные.
 * @return Promise<string>
 * @remark
 * Использовать для предзагрузки данных в реестрах без слайсов.
 * Дополнительно необходимо указать loaderId - идентификатор, по которому в контексте можно найти предзагруженное значение.
 */
export async function getItemsOrder(propsStorageId: string): Promise<string> {
    const storageId = propsStorageId + USER_PARAMS_POSTFIX;
    const result = await USER.load([storageId]);
