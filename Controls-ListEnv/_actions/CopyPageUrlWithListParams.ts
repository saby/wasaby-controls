...
export default class CopyPageUrlWithListParams extends BaseAction<ICopyPageUrlWithListParams> {
    ...
    private _clearUrl(): void {
        if (PageUrl.getQueryParam('listParams')) {
            const Router: IRouter = getRootRouter();
            // В старых версия nodejs URL не глобальный объект
            const URLLib = URL ? URL : loadSync('url').URL;
            const url = new URLLib(window.location.href);
    ...
    private _getListParams(
        ...
        idsArray.forEach((id) => {
            listParams[id || COMPATIBLE_STORE_ID] = {
    ...
    async execute(): Promise<void> {
        ...
        const Router: IRouter = getRootRouter();
        const configToSave = {
            listParams: this._getListParams(fabricId, minFilterDescription, expandedItems, root),
        };
