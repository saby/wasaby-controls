...
export default class OperationsWidget extends Control<IOperationsPanelWidgetOptions> {
    ...
    constructor(options: IOperationsPanelWidgetOptions) {
        ...
        this._listCommandCompatibleExecute =
            this._listCommandCompatibleExecute.bind(this);

        if (
            !options.storeId ||
            (options.storeId && options.slice['[ICompatibleSlice]'])
        ) {
    ...
    protected _beforeMount(options?: IOperationsPanelWidgetOptions): void {
        ...
        if (this._operationsController) {
            this._subscribeToOperationsControllerEvents(
                this._operationsController
            );
        }
        const { selected, excluded } =
            options.operationsController.getSelection();
    ...
    protected _beforeUpdate(options: IOperationsPanelWidgetOptions): void {
        ...
        if (options.slice !== this._options.slice) {
            this._unsubscribeFromOperationsControllerEvents(
                this._operationsController
            );
            this._operationsController = options.operationsController;
            this._subscribeToOperationsControllerEvents(
    ...
    protected _selectedTypeChanged(
        e: SyntheticEvent,
        selectedType: string
    ): void {
    ...
    protected _expandedChanged(e: SyntheticEvent, expanded: boolean): void {
        if (
            expanded !== this._operationsController.getOperationsPanelVisible()
        ) {
    ...
    protected _subscribeToOperationsControllerEvents(
        ...
        controller.subscribe(
            ...
            this._operationsPanelOpened
        );
        controller.subscribe(
            'operationsPanelClose',
            this._operationsPanelClose
        );
        controller.subscribe('listActionsChanged', this._listActionsChanged);
    }

    protected _unsubscribeFromOperationsControllerEvents(
        controller: OperationsController
    ): void {
