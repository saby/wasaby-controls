...
export type ViewModeType = 'default' | 'cloud' | 'popupCloudPanelDefault';
...
export default class ViewModeController implements IViewModeController {
    ...
    setFilterPanelSource(
        ...
        const isEditorsViewModeChangeable =
            this._isEditorsViewModeChangeable(viewMode);
        const changedFilterItems = this._getChangedFilterItems(filterItems);
        ...
        if (isAdaptive) {
            filterPanelSource = this._getAdaptiveItems(
                filterItems,
                filterNames
            );
        } else if (
            ...
            filterPanelSource = this._getFilterPanelSourceByNames(
                filterItems,
                filterNames
            );
        } else {
            filterPanelSource = changedFilterItems;
        }
    ...
    updateFilterPanelSource(
        ...
        const isEditorsViewModeChangeable =
            this._isEditorsViewModeChangeable(editorsViewMode);

        const currentFilterPanelSource = this._getFilterPanelSourceByNames(
            this._filterPanelSource,
            filterNames
        );
        const filterPanelSource = this._getFilterPanelSourceByNames(
            filterItems,
            filterNames
        );
        const changedFilterItems = this._getChangedFilterItems(filterItems);
        ...
        const isFilterPanelSourceChanged = !isEqual(
            filterPanelSource,
            currentFilterPanelSource
        );
        ...
        if (
            !hasChangedFilterItems ||
            (!isEditorsViewModeChangeable && isFilterSourceChanged)
        ) {
            newFilterPanelSource = this._getFilterPanelSourceByNames(
                filterItems,
                filterNames
            );
        } else if (
            ...
            newFilterPanelSource = changedFilterItems;
        } else if (!isEditorsViewModeChangeable && isFilterPanelSourceChanged) {
            newFilterPanelSource = filterPanelSource;
        } else {
            newFilterPanelSource = currentFilterPanelSource;
        }
    ...
    protected _getEditorsViewMode(
        ...
        if (!changedFilterItems.length) {
            ...
            this._isFilterSourceChanged(currentSource, changedFilterItems)
        ) {
            return 'cloud';
        }
        return this._viewMode;
    ...
    private _isEditorsViewModeChangeable(
        editorsViewModeOption: string
    ): boolean {
        return editorsViewModeOption === undefined;
    }

    private _getChangedFilterItems(source: IFilterItem[]): IFilterItem[] {
        return this._getPanelPopupSource(source).filter(
            ({ value, resetValue }) => {
    ...
    private _isFilterPopupItemsChanged(
        ...
        const isChangedPopupItem = filterPanelSource.find((item) => {
            if (!filterNames || !filterNames.includes(item.name)) {
                const isPopupItem =
                    item.viewMode === 'basic' || item.viewMode === 'extended';
    ...
    private _isFilterSourceChanged(
        ...
        return newSource.some((item) => {
            ...
            return (
                !isEqual(newSourceItem?.value, item.value) &&
                !isEqual(item.resetValue, item.value)
            );
        });
    }
}
