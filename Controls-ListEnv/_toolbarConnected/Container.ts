...
import * as template from 'wml!Controls-ListEnv/_toolbarConnected/Container';
...
export default class ToolbarWrapper extends Control<IToolbarWrapperOptions> {
    ...
    protected _getActions(actions: string | IActionOptions[]): IActionOptions[] {
    ...
    protected _beforeMount(options?: IToolbarWrapperOptions): void {
        ...
        if (this._permissionsRequired(actions)) {
            actions = this._getActionsByPermissions(actions, options.permissionsResolver);
        }
