...
class Input extends BaseLookupInput {
    ...
    protected _checkRadioGroup(): boolean {
        return this._suggestSettingsOptions.some((item) => {
            return (
                ...
                'Controls-ListEnv/filterPanelExtEditors:RadioGroupEditor'
            );
        });
    }
    ...
    protected _searchButtonClick(event: SyntheticEvent): void {
        this._children.layout.search(event);
        this._closeSuggest();
    }
    ...
    protected _addItem(item: Model, tabsSelectedKey?: Key): void {
        ...
        if (
            ...
            selectedItem.editorOptionsName === 'Controls/filterPanelEditors:Lookup'
        ) {
        ...
        this._notifyFilterSource(filterDescription, { searchValue: '', searchInputValue: '' });
    }
    ...
    protected _crossClick(event: SyntheticEvent, item: Model): void {
        ...
        filterDescription.find((filterItem) => {
            ...
            if ((isMultiSelect && value && value[0] === key) || (!isMultiSelect && value === key)) {
                ...
                if (
                    !editorOptions.multiSelect &&
                    editorOptions?.sourceController?.getItems() &&
                    filterItem.editorOptionsName === 'Controls/filterPanelEditors:Lookup'
                ) {
