...
export default class FilterSlice extends Slice<IFilterState> {
    ...
    applyFilterDescription(
        ...
        _newState?: Partial<IFilterState>,
        appliedFrom?: string
    ): void {
    ...
    override _initState(loadResult: IFilterLoadResult, config: IFilterArguments): IFilterState {
        let filterDescription = loadResult.filterDescription || config.filterDescription;

        if (filterDescription) {
            filterDescription = FilterLoader.initFilterDescriptionFromData(filterDescription);
        }
