...
export interface IExtSearchOptions extends IControlOptions, Pick<IRenderOptions, 'onBlur'> {
 ...
 * @public
 */
export default class InputWrapper extends Control<IExtSearchOptions> {
    ...
    protected _beforeMount({ filterDescription, value }: IExtSearchOptions): void {
        this._setFilterDescription(filterDescription);
        this._setItemsByFilterDescription(filterDescription);
        this._value = value || '';
    }

    protected _beforeUpdate(newOptions: IExtSearchOptions): void {
    ...
    private _getItemFromSourceControllerByValue(filterItem: IFilterItem): void | Model {
        ...
        const editorItems =
            editorOptions?.sourceController?.getItems() || editorOptions?.selectedItems;
    ...
    private _getItemsByTextValue(
        textValue: string,
        value: TKey | TKey[],
        { keyProperty, displayProperty, multiSelect, source, suggestSource }: ILookupEditorOptions
    ): RecordSet {
        const controlSource = source || suggestSource;
        const collectionOptions = {
            model: controlSource.getModel(),
            adapter: controlSource.getAdapter(),
            keyProperty,
        };
...
/**
 * @name Controls-ListEnv/ExtSearch#filterDescription
 * @cfg {Array.<Controls/filter:IFilterItem>} Элементы структуры фильтров.
 * @remark Для настройки отображения редактора, выводящегося при вводе поискового значения в окне автодоплнения, нужно
 * задать поля category и editorTemplateName, при этом редактор заданный в editorTemplateName при изменении значения
 * должен стрелять событием {@link Controls/_propertyGridEditors/IEditor#propertyValueChanged propertyValueChanged}
 */
