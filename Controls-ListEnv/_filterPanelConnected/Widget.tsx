...
import { getPropertyFromSlice, getSlice, useFilterDescription } from 'Controls-ListEnv/filterBase';
...
function getEditorsViewMode(context: unknown, props: IFilterPanelWidgetOptions): TEditorsViewMode {
    return (
        props.editorsViewMode ||
        getPropertyFromSlice<TEditorsViewMode>(context, props.storeId, 'editorsViewMode')
    );
}

function getSearchParam(context: unknown, props: IFilterPanelWidgetOptions): string {
    return getPropertyFromSlice<string>(context, props.storeId, 'searchParam');
}
