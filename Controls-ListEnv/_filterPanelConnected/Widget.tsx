...
import { IFilterItem, FilterDescription, IFilterItemInternal } from 'Controls/filter';
...
interface IFilterPanelWidgetOptions
    extends IContrastBackgroundOptions,
        IBackgroundStyleOptions,
        Required<IStoreIdOptions>,
        Required<Pick<IFilterNamesOptions, 'filterNames'>>,
        TInternalProps,
        IComponentProps {
...
function getEditorsViewMode(
    ...
    sliceViewMode?: TEditorsViewMode
): TEditorsViewMode {
...
function getResetedFilterPanelItems(filterPanelSource: IFilterItem[]): IFilterItem[] {
    const filterPanel = cloneItems(filterPanelSource);
    filterPanel.forEach((item) => {
        item.value = item.editorOptions?.emptyKey || item.resetValue;
    });
...
const findItem = FilterDescription.getItemByName;
...
function updateChangedItems(
    newFilterSource: IFilterItemInternal[],
    oldFilterSource: IFilterItemInternal[]
): IFilterItem[] {
    ...
    filterPanel.forEach((item) => {
        const newItem = findItem(newFilterSource, item.name) as IFilterItemInternal;
        if (
            newItem &&
            !newItem.appliedFrom &&
...
const FilterPanelConnectedWidget = React.forwardRef<View, IFilterPanelWidgetOptions>(
    (props: IFilterPanelWidgetOptions, ref) => {
        ...
        const sliceViewMode = getPropertyFromSlice<TEditorsViewMode>(
            props.storeId,
            'editorsViewMode'
        );
        React.useMemo(() => {
