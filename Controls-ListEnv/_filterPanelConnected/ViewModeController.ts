...
export default class ViewModeController implements IViewModeController {
    ...
    private _isFilterPanelCloudItem(
        { name, value, resetValue, visibility, appliedFrom, type }: IFilterItem,
        searchParam: string
    ): void {
        return (
            value !== undefined &&
    ...
    private _getChangedFilterItems(source: IFilterItem[], searchParam: string): IFilterItem[] {
        return this._getPanelPopupSource(source).filter((item) =>
            this._isFilterPanelCloudItem(item, searchParam)
        );
    }

    private _getFilterPanelCloudItems(source: IFilterItem[], searchParam: string): IFilterItem[] {
        return source.filter(
            (item) => !!item.editorTemplateName && this._isFilterPanelCloudItem(item, searchParam)
        );
    }
