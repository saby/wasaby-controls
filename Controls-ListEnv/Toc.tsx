...
import Fingers, { IProps as IFingersProps, TSelectedStyle } from 'Controls/Fingers';
...
import {
    default as ActiveItemContextProvider,
    ActiveItemContext,
} from './_toc/Context/ActiveItemContext';

export { ActiveItemContextProvider, ActiveItemContext };
export { default as ActiveItemContextConsumer } from './_toc/Context/ActiveItemContextConsumer';
...
function getSelectedKey(
    ...
    if (allowSelection) {
        const configSelectedKey = activeElement;
...
function Toc(props: IProps, ref: React.ForwardedRef<object>): React.ReactElement {
    const activeElementContext = React.useContext(ActiveItemContext);
    ...
    const activeElement = activeElementContext
        ? activeElementContext.activeElement
        : slice?.state.activeElement;
    properties.selectedKey = props.selectedKey || activeElement;
    const hierarchyRelation: relation.Hierarchy = new relation.Hierarchy({
        ...properties,
    });

    const selectedKey = getSelectedKey(properties, slice, activeElement, hierarchyRelation);

    const onSelectedKeyChanged = function (key: CrudEntityKey): void {
        if (activeElementContext) {
            activeElementContext.activeElement = key;
        } else {
    ...
    if (slice.state.viewMode === 'search' && activeElementContext.activeElement) {
        if (activeElementContext) {
            activeElementContext.activeElement = null;
        } else {
