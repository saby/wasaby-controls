...
function getPrefetchFromHistory({
                                    prefetchParams,
                                }): IPrefetchHistoryParams | undefined {
...
function getPrefetchParamsForSave(
    items: RecordSet
): IPrefetchHistoryParams | undefined {
...
function applyPrefetchFromHistory(filter: object, history: object): object {
    const prefetchParams = getPrefetchFromHistory(history);
    let resultFilter;

    if (prefetchParams) {
        resultFilter = { ...filter };
        resultFilter[PREFETCH_SESSION_FIELD] =
            prefetchParams[PREFETCH_SESSION_FIELD];
    } else {
        resultFilter = filter;
    }
...
function prepareFilter(
    ...
    if (prefetchSessionId) {
        resultFilter[PREFETCH_SESSION_FIELD] = prefetchSessionId;
    }
