...
export default class ViewModel {
    ...
    private _handleOperand(operandSymbol: IOperandSymbol): boolean {
        ...
        if (isNewInput) {
            ...
            // Ограничиваем количество символов которые можно ввести.
            if (
    ...
    private _handleOperator(operator: IOperator): boolean {
        ...
        if (!this._containException) {
            ...
            if (
                ViewModel._convertToExponentialIfNeed(this._resultValue, MAX_RESULT_LENGTH) ===
                this._value
            ) {
    ...
    private static _convertToExponentialIfNeed(value: string, maxLength: number): string {
        try {
            ...
            if (value.length > maxLength && value.replace('.', '').length > maxLength) {
                result = true;
            }
            return result ? new Big(value).toExponential(RESULT_DECIMAL_PLACES) : value;
        } catch (e) {
            return value;
        }
    }
