...
export default class View extends Control<ICalculatorOptions> implements ICalculator {
    ...
    protected _beforeMount(options: ICalculatorOptions): void {
        ...
        // На СП не работает SessionStorage (по крайней мере на демках)
        // В реальных кейсах калькулятор на вставляют в страницу, он всегда открыватся в окне
        // Правим ошибки гидратации
        if (!query.get.test) {
            this._initModel();
        }
    ...
    protected _notifyResult(): void {
        this._notify('sendResult', [this._inputValue, this._calculatorModel.resultValue], {
            bubbling: true,
        });
    }
    ...
    protected _prepareValue(value: string): string {
        ...
        if (replacedValue) {
            if (
                replacedValue.length > MAX_INPUT_VALUE_LENGTH &&
                replacedValue.replace('.', '').length > MAX_INPUT_VALUE_LENGTH
            ) {
