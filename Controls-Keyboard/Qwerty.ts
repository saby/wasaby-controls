...
} from 'Controls-Keyboard/_Qwerty/constants';
 ...
 * @interface Controls-Keyboard/Qwerty/IQwertyOptions
...
export interface IQwertyOptions extends IControlOptions {
    ...
    /**
     * @name Controls-Keyboard/Qwerty#availableLanguages
     * @cfg {Langs[]} Массив доступных языков для переключения.
     * @default ['Ru', 'En']
     * @demo Controls-Keyboard-demo/Qwerty/availableLanguages
     */
 ...
 * @class Controls-Keyboard/Qwerty
 * @control
 * @public
...
export default class Qwerty extends Control<IQwertyOptions> {
    ...
    // Typescript не видит getDefaultOptions
    protected _options: Required<Pick<IQwertyOptions, 'availableLanguages' | 'itemPressCallback'>>;
    ...
    private _changeLanguage(): void {
        ...
        this._lang = this._options.availableLanguages[this._getNextLangIndex()];
    }

    private _getNextLangIndex(): number {
        return (
            (this._options.availableLanguages.indexOf(this._lang) + 1) %
            this._options.availableLanguages.length
        );
    }
    ...
    private _chooseCase(): string {
        const langName = Langs[this._lang];
        return this._isShift ? `${langName}Up` : `${langName}Down`;
    }
    ...
    protected _beforeMount(options: Required<IQwertyOptions>): void {
        // Если не задан размер, то установить исходя из размера окна
        this._size = options.size || options.getSizeOfKeyboard?.() || 's';
        this._itemFontSize = this._size === 's' ? 'm' : 'l';

        if (options.itemSpacing === 'default') {
            this._itemSpacing = this._size;
        } else {
            this._itemSpacing = options.itemSpacing || 's';
        }

        this._lang = options.lang ? Langs[options.lang] : options.availableLanguages[0];
    ...
    /**
     * Получить текст для кнопки переключения языка
     */
    protected _getLangButtonCaption(): string {
        const nextLang = this._options.availableLanguages[this._getNextLangIndex()];
