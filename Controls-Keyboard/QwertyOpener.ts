 ...
 * @public
 * @remark
 * Если необходимо, чтобы виртуальная клавиатура не открывалась при фокусе (on:focus)
 * на определённом поле ввода, добавьте этому полю или одному из его родителей класс,
 * определённый в `Controls-Keyboard/QwertyOpener:IGNORE_CLASS_NAME`.
...
export default class QwertyOpener extends Control<IQwertyOpener> {
    ...
    protected _onFocusIn(event: SyntheticEvent): void {
        ...
        inputInfo.input.scrollIntoView({ behavior: 'smooth', block: 'center', inline: 'nearest' });
