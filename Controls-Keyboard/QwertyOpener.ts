...
const UNFOCUS_DELAY = 50;

/**
 * CSS класс для игнорирования фокуса на поле ввода
 * @public
 */
export const IGNORE_CLASS_NAME = 'keyboard-QwertyOpener-ignore';
 ...
 * @extends Controls-Keyboard/Qwerty:IQwertyOptions
 * @public
 */
 ...
 * @public
 * @remark
 * Чтобы отключить автоматическое открытие виртуальной клавиатуры при фокусе на поле ввода, добавьте класс 'keyboard-QwertyOpener-ignore' к этому полю или его родительскому элементу.
 * Этот класс указан вконстанте {@link Controls-Keyboard/QwertyOpener:IGNORE_CLASS_NAME}.
 * @demo Controls-Keyboard-demo/QwertyOpener/QwertyOpener
 * @demo Controls-Keyboard-demo/QwertyOpener/Ignored/Index
 */
export default class QwertyOpener extends Control<IQwertyOpener> {
     ...
     * Активное поле ввода
     */
    protected _focusedInput: HTMLInputElement | null;
    ...
    protected _unfocusTimeoutId: number | null;
    ...
    protected _onFocusIn(event: SyntheticEvent): void {
        ...
        if (this._unfocusTimeoutId) {
            clearTimeout(this._unfocusTimeoutId);
            this._unfocusTimeoutId = null;
        }
        if (
            !inputInfo ||
            (this._focusedInput && !document.body.contains(this._focusedInput)) ||
            inputInfo.input.closest(`.${IGNORE_CLASS_NAME}`)
        ) {
        ...
        inputInfo.input.scrollIntoView({
            behavior: 'smooth',
            block: 'center',
            inline: 'nearest',
        });
    ...
    protected _onFocusOut(): void {
        // Могут возникать ситуации, когда onFocusIn вызывается практически сразу после onFocusOut. Из-за чего клавиатура закрывалась и сразу открывалась, что приводит к морганию. Поэтому закрываем клавиатуру с задержкой
        this._unfocusTimeoutId = setTimeout(() => {
            this.toggleKeyboard(false);
            this._focusedInput = null;
            this._notify('inputDeactivated');
        }, UNFOCUS_DELAY);
