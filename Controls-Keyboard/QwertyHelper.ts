...
const validInputTagNames = ['input', 'textarea'];
...
function focusIn(event: SyntheticEvent): IInputInfo | void {
    ...
    if (
        target &&
        target.tagName &&
        (validInputTagNames.includes(target.tagName.toLowerCase()) || target.isContentEditable)
    ) {
...
function itemPressCallback(options: IOptionsItemPressCallback, button: string): void {
    const { accentButtonPressCallback, focusedInput, elemForChangeFocus } = options;
    switch (button) {
        case 'keyboard':
