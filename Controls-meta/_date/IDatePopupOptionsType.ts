...
const options = ['datePicker', 'compactDatePicker', 'shortDatePicker'] as const;

export const IDatePopupOptionsType = ObjectType.id(
    'Controls/meta:IDatePopupOptionsType'
).attributes<IDatePopupTypeOptions>({
    datePopupType: StringType.title(rk('Тип календаря'))
        .description(rk('Календарь, который откроется при нажатии на вызывающий элемент'))
