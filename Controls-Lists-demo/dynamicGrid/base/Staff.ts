...
// Рабочие события февраля
const FEBRUARY_EVENTS: { [p: number]: IEvent[] } = {
    2: [
        {
            day: 1,
            startHour: 6,
...
// Список смен
const FEBRUARY_SHIFT: { [p: number]: IEvent[] } = {
    2: [
        {
            day: 1,
            startHour: 6,
...
// Получаем события февраля для четырёх сотрудников.
function getFebruaryEvents(adapter: EntityAdapter.IAdapter, rowNumber: number): Record[] {
    if ([2, 3, 4, 6].indexOf(rowNumber) !== -1) {
        return FEBRUARY_EVENTS[rowNumber as 2 | 3 | 4 | 6].map((e, index) => {
            ...
            startDate.setMinutes(e.startMinutes);
            const endDate = new Date(DATE);
            endDate.setMonth(endDate.getMonth() + 1);
            endDate.setDate(e.day);
            endDate.setHours(e.endHour);
            endDate.setMinutes(e.endMinutes || 0);

            const event = new Record({ adapter });

            event.addField(
                new EntityFormat.StringField({ name: 'eventId' }),
                null,
                'work-f-' + index
            );
...
// Генерирует смены для сотрудников в феврале.
// Смена - событие со специальным типом (eventType).
// Смены отображаются только при сменном графике.
export function getFebruaryShifts(adapter: EntityAdapter.IAdapter, rowNumber: number): Record[] {
    if ([2, 3, 4, 6].indexOf(rowNumber) !== -1) {
        return FEBRUARY_SHIFT[rowNumber as 2 | 3 | 4 | 6].map((e, index) => {
            const startDate = new Date(DATE);
            startDate.setMonth(startDate.getMonth() + 1);
            startDate.setDate(e.day);
            startDate.setHours(e.startHour);
            ...
            event.addField(new EntityFormat.StringField({ name: 'interval' }), null, null);
            event.addField(new EntityFormat.StringField({ name: 'eventType' }), null, 'Смена');
            event.addField(new EntityFormat.DateTimeField({ name: 'DTStart' }), null, startDate);
            event.addField(new EntityFormat.DateTimeField({ name: 'DTEnd' }), null, endDate);
...
export function getEventsByNumber(adapter: EntityAdapter.IAdapter, number: number): RecordSet {
    ...
    const shiftEvents = getFebruaryShifts(adapter, number);
