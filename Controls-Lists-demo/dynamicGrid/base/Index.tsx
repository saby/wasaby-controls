    ...
    return {
        ...
        getCellProps: (item: Model, date: Date) => {
            ...
            if (constants.isServerSide) {
                date = correctDateFromClientToServer(date);
            }
            ...
            const outOfWorkPeriodBorderVisibility =
                params.hoverMode === 'cell' ? 'hidden' : 'visible';
...
function getInitialRange(): IRange {
    // Для имитации ЕКБ
    // const start = new Date('2022-12-31T19:00:00.000Z');
    // const end = new Date('2023-01-30T19:00:00.000Z');
    // return {
    //     start,
    //     end,
    // };
...
const Index = React.forwardRef(function Index(props, forwardedRef) {
    ...
    const [viewportInputWidth, setViewportInputWidth] = React.useState<number>(viewportWidth);
    const [hoverMode, setHoverMode] = React.useState<THoverMode>('cross');
    const [hasCustomRanges, setHasCustomRanges] = React.useState<boolean>(false);
    const [cellsMultiSelectVisibility, setCellsMultiSelectVisibility] =
        React.useState<TCellsMultiSelectVisibility>('onhover');

    const adaptiveMode = useAdaptiveMode();
    const adaptiveViewportWidth = adaptiveMode?.width.value;
    const adaptiveViewportHeight = adaptiveMode?.window.viewportHeight.value;
    ...
    const onCellsMultiSelectVisibilityChanged = React.useCallback(
        (value: { [p: string]: TCellsMultiSelectVisibility }) => {
            setCellsMultiSelectVisibility(value.cellsMultiSelectVisibility);
            slice.setState({
                cellsMultiSelectVisibility: value.cellsMultiSelectVisibility,
            });
        },
        [slice]
    );
    ...
    const getEventRenderProps = React.useCallback((event: Model) => {
        if (event.get('eventType') === 'Отпуск') {
            return {
                resizerVisible: true,
            };
        } else return {};
    ...
    const onEventResized = React.useCallback((params: { item: Model; event: Model }) => {
        const { item, event } = params;
        if (event.get('DTEnd').getDate() % 2 !== 0) {
            const promise = new Promise((resolve, reject) => {
                setTimeout(() => {
                    if (event.get('DTEnd').getDate() % 3 === 0) {
                        reject('fail');
                    } else {
                        resolve('success');
                    }
    ...
    return (
        <div
            ...
            ref={forwardedRef}
            style={{
                width: `${props.isAdaptive ? adaptiveViewportWidth : viewportWidth}px`,
            }}
        >
            {!props.isAdaptive ? <div>{'лог:' + clickLog}</div> : null}
            <div ref={actionRef}>
                <div
                    style={{
                        paddingLeft: 15,
                ...
                {!props.isAdaptive ? (
                    ...
                    >
                        ...
                        />
                        <SelectInput
                            name="cellsMultiSelectVisibility"
                            title="Режим выделения ячеек"
                            values={['onhover', 'hidden']}
                            value={cellsMultiSelectVisibility}
                            onChange={onCellsMultiSelectVisibilityChanged}
                        />

                        <Checkbox
                            caption={'Пользовательские заголовки'}
                            value={hasCustomRanges}
                            onValueChanged={(newValue) => {
                                if (newValue) {
                                    slice.setState({ quantums: CUSTOM_QUANTUMS });
                                } else {
                                    slice.setState({ quantums: BASE_QUANTUMS });
                                }
                                setHasCustomRanges(newValue);
                            }}
                        />
                        ...
                        <div>
                            ...
                            <input
                                ...
                                onChange={(e) => {
                                    ...
                                    setViewportInputWidth(Number(e.target.value));
                                }}
                            />
                            <input
                                type={'text'}
                                value={viewportInputWidth}
                                onChange={(e) => setViewportInputWidth(Number(e.target.value))}
                                onBlur={(e) => setViewportWidth(viewportInputWidth)}
                            />
                        </div>
                    </div>
                ) : null}
            ...
            <ScrollContainer
                style={{
                    height: props.isAdaptive
                        ? `${adaptiveViewportHeight - actionRef.current?.offsetHeight}px`
                        : '1000px',
                }}
            ...
            >
                <TimelineGridConnectedComponentMemo
                    ...
                    hoverMode={hoverMode}
                    itemActions={!props.isAdaptive ? ACTIONS : undefined}
                    onEditArrowClick={editArrowClicked}
                    showEditArrow={!props.isAdaptive}
                    eventRender={<DemoEventRenderComponent />}
                    aggregationRender={<AggregationRender />}
                    viewportWidth={props.isAdaptive ? 844 : viewportWidth}
