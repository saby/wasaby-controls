...
function TimelineGridDemo(props: TInternalProps, ref: React.ForwardedRef<HTMLDivElement>) {
    ...
    const [horizontalSeparatorsMode, setHorizontalSeparatorsMode] =
        React.useState<TSeparatorMode>('gap');

    const onHorizontalSeparatorsModeChanges = React.useCallback(
        (value: { [p: string]: TSeparatorMode }) => {
            setHorizontalSeparatorsMode(value.horizontalSeparatorsMode);
        },
        []
    );
    ...
    return (
        <div className="controlsListsDemo__dynamicGridBase" ref={ref}>
            <SelectInput
                name="horizontalSeparatorsMode"
                title="Режим отображения горизонтальных разделителей"
                values={['gap', 'line', 'none']}
                value={horizontalSeparatorsMode}
                onChange={onHorizontalSeparatorsModeChanges}
            />
            <ScrollContainer
            ...
            >
                <TimelineGridConnectedComponent
                    ...
                    horizontalSeparatorsMode={horizontalSeparatorsMode}
