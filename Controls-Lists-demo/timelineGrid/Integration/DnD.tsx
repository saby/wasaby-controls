...
import { LOCAL_MOVE_POSITION } from 'Types/_source/Local';
import { ISelectionObject } from 'Controls/_interface/ISelectionType';
import TreeGrid from 'Controls/_treeGrid/TreeGrid';
...
// Ширина области динамических колонок
const DYNAMIC_VIEWPORT_WIDTH = parseInt(dynamicColumnMinWidths.hour, 10) * HOURS_IN_DAY;
...
function Demo(_props: TInternalProps, ref: React.ForwardedRef<HTMLDivElement>): React.ReactElement {
    ...
    const gridViewRef = React.useRef<TreeGrid>(null);
    ...
    const onDragStart = React.useCallback(
        (movedKeys: number[]) => {
            ...
            return new ItemsEntity({
                items: movedKeys,
                title: firstItem ? `${firstItem.get('name')} (${firstItem.get('position')})` : '',
            });
        },
        [items]
    );
