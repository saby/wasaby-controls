            ...
            <div style={{ height: '50px' }}>
                <label style={{ marginRight: '10px' }}>
                    <input
                        type="radio"
                        name="scrollViewMode"
                        onChange={() => setScrollViewMode('unaccented')}
                        checked={scrollViewMode === 'unaccented'}
                    />
                    Без теней по краям
                </label>
                <label>
                    <input
                        type="radio"
                        name="scrollViewMode"
                        onChange={() => setScrollViewMode('withShadows')}
                        checked={scrollViewMode === 'withShadows'}
                    />
                    С тенями по краям
                </label>
            </div>
                ...
                >
                    ...
                    >
                        <TimelineGridConnectedComponent
                            ...
                            onTagClick={(event, item, date) => {
                                Confirmation.openPopup({
                                    markerStyle: 'success',
                                    message: `Клик по тегу в ячейке ${date.toLocaleDateString(
                                        'ru-RU'
                                    )} в строке человека ${item.get('name')}`,
                                    type: 'ok',
                                });
                            }}
...
export default Object.assign(React.forwardRef(Demo), {
    getLoadConfig(): Record<string, IDataConfig<ITimelineGridDataFactoryArguments>> {
        ...
        // * daysAndMonths Доступны только дни и месяцы
        // * all Все варианты: месяцы(год) -> месяцы(полугодия) -> месяцы(четверти) -> дни(месяц) -> дни(неделя) -> часы -> 30мин -> 15мин.
