...
function Demo(props: TInternalProps, ref: React.ForwardedRef<HTMLDivElement>): React.ReactElement {
    ...
    const onAddEventDependency = (
        fromEvent: string,
        toEvent: string,
        buttonPosition: 'left' | 'right'
    ) => {
        setLayers((prevLayers) => {
            const firstLayerData = prevLayers[0].props.data;

            const newData = {
                from: buttonPosition === 'right' ? fromEvent : toEvent,
                to: buttonPosition === 'left' ? fromEvent : toEvent,
                style: { color: '#0053ff' },
            };

            const updatedData = [...firstLayerData, newData];

            return [<ArrowsLayer data={updatedData} key={'layer-1'} />, ...prevLayers.slice(1)];
        });
    };

    const onDependencyButtonClick = (eventId: string, buttonPosition: 'left' | 'right') => {
        Confirmation.openPopup({
            markerStyle: 'success',
            message: `Кнопка ${eventId} нажата с ${buttonPosition} стороны`,
            type: 'ok',
        });
    };
    ...
    const getEventRenderProps = React.useCallback(
        (event: Model) => {
            if (
                ...
                return {
                    dependencyButtonsConfig: {
                        showButtons: { right: true, left: true },
                        style: {
                            backgroundColor: 'var(--success_background-color)',
                        },
                        iconStyle: 'danger',
                    },
                ...
                >
                    ...
                    >
                        <TimelineGridConnectedComponent
                            ...
                            onAddEventDependency={onAddEventDependency}
...
function getDynamicColumn(params: IGetDynamicColumnParams): IDynamicColumnConfig<Date> {
    return {
        ...
        getCellProps: (item: Model<IStaff>, date: Date) => {
            ...
            const backgroundStyle = 'schedule_timelineDemo_workday';
