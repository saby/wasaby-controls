...
export default class CreateCEDWDoc {
    async execute(config: { regulation: string; docType: string }, opener: Control): void {
        ...
        if (docType === 'BusinessTrip') {
            ...
            source
                ...
                .then(async (result) => {
                    ...
                    const index = regulations.getIndexByValue(
                        docType === 'StaffStatements' ? 'ComplexKey' : 'Uuid',
                        config.regulation
                    );
