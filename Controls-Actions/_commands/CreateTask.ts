        ...
        if (regulations) {
            Promise.all([
                ...
                new SbisService({
                    ...
                    /*
                    Если команду выполняют из панели документа, в параметрах прилетает контекст contextData и надо
                    создавать связанный документ по documentContext.
                    !!!НО!!! Команду могут прикрутить куда угодно, например в ПТИЦУ в аккордеоне и тогда в параметре
                    contextData прилетит undefined и обращение типа contextData.documentContext будет ошибочным и
                    все к чему это придет - ошибка.
                    Добавил корректную обработку параметра contextData. Если оная не прилетает - просто создадим
                    документ и откроем его в новом инстансе EDO3/opener
                    */
                    const documentContext = contextData?.documentContext;
                    if (documentContext) {
                        const record = documentContext.get('record');
                        documentContext.processAction('createLinkedDocument', {
                            rule,
                            record,
                        });
                    } else {
