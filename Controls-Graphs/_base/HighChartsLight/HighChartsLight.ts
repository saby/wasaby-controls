...
import rk = require('i18n!Controls-Graphs');
import { throttle } from 'Types/function';
import 'css!Controls-Graphs/base';
import 'css!Graphs/baseChart';
import 'css!Graphs/HighChartsLight';
...
export const getLocaleStorageParams = (): ILocaleStorageParamsForGraphs => {
    let result;
    try {
        result = JSON.parse(LocalStorageNative.getItem(UNIQ_KEY_FOR_GRAPHS) as string) || {};
    } catch (e) {
        result = {};
    }
    return result;
...
export default class HighChartsLight extends Control<IOptions> {
    ...
    private _loadingPromise: Promise<unknown> | null | undefined;
    ...
    private _initHighCharts(): void {
        //@ts-expect-error import highchart and more lib todo поправить при переходе на tsx
    ...
    callMethodOnChartInstance(
        ...
        return new Promise((resolve, reject) => {
            const callback = () => {
                return resolve(
                    this._chartInstance[methodName as unknown as keyof IChartInstance](
                        // @ts-expect-error args for highcharts methods todo поправить при переходе на tsx
                        ...(args as THighChartsMethodArg[])
                    )
                );
            };
            if (this._chartInstance) {
