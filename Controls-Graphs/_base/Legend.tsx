...
export interface ILegendProps extends Pick<ILegend, 'legendHorizontalAlignment'> {
    ...
    legendItemClickHandler: (caption: ILegendItem['caption'], show: boolean) => void;
    className?: string;
    theme?: string;
}
...
function Legend(props: ILegendProps, ref: LegacyRef<HTMLDivElement>) {
    ...
    const onLegendItemClick = useCallback(
        (
            ...
            colorIndex: ILegendItem['colorIndex'],
            caption: ILegendItem['caption']
        ) => {
            let newVisibleItems;
            ...
            props.legendItemClickHandler(caption, newVisibleItems.includes(colorIndex));
        },
        ...
        >
            {props.items.map((item, idx) => (
                <div
                    ...
                    onClick={(event) => onLegendItemClick(event, item.colorIndex, item.caption)}
